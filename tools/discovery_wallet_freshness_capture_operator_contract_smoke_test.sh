...
assert "15m timer cadence + 60s jitter" in readme, "README must explain why the standalone evidence lookback stays gap-free"
assert "execution.enabled = false" in readme, "README must restate no execution activation"
assert "copybot-discovery-wallet-freshness-capture.timer` is the operational Stage 3" not in readme, "README must not keep the timer as the primary Stage 3 path"
...
assert "copybot-discovery-wallet-freshness-capture.service" in road, "ROAD must keep the standalone capture tool as manual/debug context"
