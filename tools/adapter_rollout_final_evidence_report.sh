...
rollout_reason="rollout helper not executed"
...
if ((${#input_errors[@]} == 0)); then
  ...
  if ! rollout_artifacts_written="$(extract_bool_field_strict "artifacts_written" "$rollout_output")"; then
    input_errors+=("nested adapter rollout artifacts_written must be boolean token, got: ${rollout_artifacts_written_raw:-<empty>}")
    rollout_artifacts_written="unknown"
  elif [[ "$rollout_artifacts_written" != "true" ]]; then
    input_errors+=("nested adapter rollout artifacts_written must be true")
  fi
  ...
  if ! rollout_nested_package_bundle_enabled="$(extract_bool_field_strict "package_bundle_enabled" "$rollout_output")"; then
    input_errors+=("nested adapter rollout package_bundle_enabled must be boolean token, got: ${rollout_nested_package_bundle_enabled_raw:-<empty>}")
    rollout_nested_package_bundle_enabled="unknown"
  ...
  if ! rollout_nested_go_nogo_require_ingestion_grpc="$(extract_bool_field_strict "rehearsal_nested_go_nogo_require_ingestion_grpc" "$rollout_output")"; then
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_require_ingestion_grpc must be boolean token, got: ${rollout_nested_go_nogo_require_ingestion_grpc_raw:-<empty>}")
    ...
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_require_ingestion_grpc mismatch: nested=${rollout_nested_go_nogo_require_ingestion_grpc} expected=${go_nogo_require_ingestion_grpc_norm}")
  fi
  ...
  if ! rollout_nested_go_nogo_require_followlist_activity="$(extract_bool_field_strict "rehearsal_nested_go_nogo_require_followlist_activity" "$rollout_output")"; then
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_require_followlist_activity must be boolean token, got: ${rollout_nested_go_nogo_require_followlist_activity_raw:-<empty>}")
    ...
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_require_followlist_activity mismatch: nested=${rollout_nested_go_nogo_require_followlist_activity} expected=${go_nogo_require_followlist_activity_norm}")
  fi
  ...
  if ! rollout_nested_go_nogo_require_confirmed_execution_sample="$(extract_bool_field_strict "rehearsal_nested_go_nogo_require_confirmed_execution_sample" "$rollout_output")"; then
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_require_confirmed_execution_sample must be boolean token, got: ${rollout_nested_go_nogo_require_confirmed_execution_sample_raw:-<empty>}")
    ...
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_require_confirmed_execution_sample mismatch: nested=${rollout_nested_go_nogo_require_confirmed_execution_sample} expected=${go_nogo_require_confirmed_execution_sample_norm}")
  fi
  ...
  if ! rollout_nested_go_nogo_min_confirmed_orders="$(parse_positive_u64_token_strict "$rollout_nested_go_nogo_min_confirmed_orders_raw")"; then
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_min_confirmed_orders must be an integer >= 1, got: ${rollout_nested_go_nogo_min_confirmed_orders_raw:-<empty>}")
  ...
  if ! rollout_nested_go_nogo_require_jito_rpc_policy="$(extract_bool_field_strict "go_nogo_require_jito_rpc_policy" "$rollout_output")"; then
    input_errors+=("nested adapter rollout go_nogo_require_jito_rpc_policy must be boolean token, got: ${rollout_nested_go_nogo_require_jito_rpc_policy_raw:-<empty>}")
    ...
    input_errors+=("nested adapter rollout go_nogo_require_jito_rpc_policy mismatch: nested=${rollout_nested_go_nogo_require_jito_rpc_policy} expected=${go_nogo_require_jito_rpc_policy_norm}")
  fi
  rollout_nested_jito_rpc_policy_verdict_raw="$(trim_string "$(extract_field "jito_rpc_policy_verdict" "$rollout_output")")"
  ...
  if [[ -z "$rollout_nested_jito_rpc_policy_verdict_raw" ]]; then
    input_errors+=("nested adapter rollout jito_rpc_policy_verdict must be non-empty")
    rollout_nested_jito_rpc_policy_verdict="UNKNOWN"
    ...
    input_errors+=("nested adapter rollout jito_rpc_policy_verdict must be one of PASS,WARN,NO_DATA,SKIP (got: ${rollout_nested_jito_rpc_policy_verdict_raw})")
    rollout_nested_jito_rpc_policy_verdict="UNKNOWN"
  fi
  rollout_nested_jito_rpc_policy_reason_code="$(trim_string "$(extract_field "jito_rpc_policy_reason_code" "$rollout_output")")"
  if [[ -z "$rollout_nested_jito_rpc_policy_reason_code" ]]; then
    input_errors+=("nested adapter rollout jito_rpc_policy_reason_code must be non-empty")
    rollout_nested_jito_rpc_policy_reason_code="n/a"
  fi
  rollout_nested_go_nogo_require_fastlane_disabled_raw="$(trim_string "$(extract_field "go_nogo_require_fastlane_disabled" "$rollout_output")")"
  if ! rollout_nested_go_nogo_require_fastlane_disabled="$(extract_bool_field_strict "go_nogo_require_fastlane_disabled" "$rollout_output")"; then
    input_errors+=("nested adapter rollout go_nogo_require_fastlane_disabled must be boolean token, got: ${rollout_nested_go_nogo_require_fastlane_disabled_raw:-<empty>}")
    ...
    input_errors+=("nested adapter rollout go_nogo_require_fastlane_disabled mismatch: nested=${rollout_nested_go_nogo_require_fastlane_disabled} expected=${go_nogo_require_fastlane_disabled_norm}")
  fi
  rollout_nested_fastlane_feature_flag_verdict_raw="$(trim_string "$(extract_field "fastlane_feature_flag_verdict" "$rollout_output")")"
  ...
  if [[ -z "$rollout_nested_fastlane_feature_flag_verdict_raw" ]]; then
    input_errors+=("nested adapter rollout fastlane_feature_flag_verdict must be non-empty")
    rollout_nested_fastlane_feature_flag_verdict="UNKNOWN"
    ...
    input_errors+=("nested adapter rollout fastlane_feature_flag_verdict must be one of PASS,WARN,NO_DATA,SKIP (got: ${rollout_nested_fastlane_feature_flag_verdict_raw})")
    rollout_nested_fastlane_feature_flag_verdict="UNKNOWN"
  fi
  rollout_nested_fastlane_feature_flag_reason_code="$(trim_string "$(extract_field "fastlane_feature_flag_reason_code" "$rollout_output")")"
  if [[ -z "$rollout_nested_fastlane_feature_flag_reason_code" ]]; then
    input_errors+=("nested adapter rollout fastlane_feature_flag_reason_code must be non-empty")
    rollout_nested_fastlane_feature_flag_reason_code="n/a"
  fi
  ...
  if ! rollout_nested_go_nogo_require_non_bootstrap_signer="$(extract_bool_field_strict "rehearsal_nested_go_nogo_require_non_bootstrap_signer" "$rollout_output")"; then
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_require_non_bootstrap_signer must be boolean token, got: ${rollout_nested_go_nogo_require_non_bootstrap_signer_raw:-<empty>}")
    ...
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_require_non_bootstrap_signer mismatch: nested=${rollout_nested_go_nogo_require_non_bootstrap_signer} expected=${go_nogo_require_non_bootstrap_signer_norm}")
  fi
  rollout_nested_go_nogo_require_submit_verify_strict_raw="$(trim_string "$(extract_field "rehearsal_nested_go_nogo_require_submit_verify_strict" "$rollout_output")")"
  if ! rollout_nested_go_nogo_require_submit_verify_strict="$(extract_bool_field_strict "rehearsal_nested_go_nogo_require_submit_verify_strict" "$rollout_output")"; then
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_require_submit_verify_strict must be boolean token, got: ${rollout_nested_go_nogo_require_submit_verify_strict_raw:-<empty>}")
    ...
    input_errors+=("nested adapter rollout rehearsal_nested_go_nogo_require_submit_verify_strict mismatch: nested=${rollout_nested_go_nogo_require_submit_verify_strict} expected=${go_nogo_require_submit_verify_strict_norm}")
  fi
  ...
  if [[ -z "$rollout_nested_ingestion_grpc_guard_verdict_raw" ]]; then
    input_errors+=("nested adapter rollout rehearsal_nested_ingestion_grpc_guard_verdict must be non-empty")
    rollout_nested_ingestion_grpc_guard_verdict="UNKNOWN"
    ...
    input_errors+=("nested adapter rollout rehearsal_nested_ingestion_grpc_guard_verdict must be one of PASS,WARN,UNKNOWN,SKIP (got: ${rollout_nested_ingestion_grpc_guard_verdict_raw})")
    rollout_nested_ingestion_grpc_guard_verdict="UNKNOWN"
  fi
  ...
  if [[ -z "$rollout_nested_ingestion_grpc_guard_reason_code" ]]; then
    input_errors+=("nested adapter rollout rehearsal_nested_ingestion_grpc_guard_reason_code must be non-empty")
    rollout_nested_ingestion_grpc_guard_reason_code="n/a"
  fi
  ...
  if [[ -z "$rollout_nested_followlist_activity_guard_verdict_raw" ]]; then
    input_errors+=("nested adapter rollout rehearsal_nested_followlist_activity_guard_verdict must be non-empty")
    rollout_nested_followlist_activity_guard_verdict="UNKNOWN"
    ...
    input_errors+=("nested adapter rollout rehearsal_nested_followlist_activity_guard_verdict must be one of PASS,WARN,UNKNOWN,SKIP (got: ${rollout_nested_followlist_activity_guard_verdict_raw})")
    rollout_nested_followlist_activity_guard_verdict="UNKNOWN"
  fi
  ...
  if [[ -z "$rollout_nested_followlist_activity_guard_reason_code" ]]; then
    input_errors+=("nested adapter rollout rehearsal_nested_followlist_activity_guard_reason_code must be non-empty")
    rollout_nested_followlist_activity_guard_reason_code="n/a"
  fi
  ...
  if [[ -z "$rollout_nested_non_bootstrap_signer_guard_verdict_raw" ]]; then
    input_errors+=("nested adapter rollout rehearsal_nested_non_bootstrap_signer_guard_verdict must be non-empty")
    rollout_nested_non_bootstrap_signer_guard_verdict="UNKNOWN"
    ...
    input_errors+=("nested adapter rollout rehearsal_nested_non_bootstrap_signer_guard_verdict must be one of PASS,WARN,UNKNOWN,SKIP (got: ${rollout_nested_non_bootstrap_signer_guard_verdict_raw})")
    rollout_nested_non_bootstrap_signer_guard_verdict="UNKNOWN"
  fi
  ...
  if [[ -z "$rollout_nested_non_bootstrap_signer_guard_reason_code" ]]; then
    input_errors+=("nested adapter rollout rehearsal_nested_non_bootstrap_signer_guard_reason_code must be non-empty")
    rollout_nested_non_bootstrap_signer_guard_reason_code="n/a"
  fi
  rollout_nested_submit_verify_guard_verdict_raw="$(trim_string "$(extract_field "rehearsal_nested_submit_verify_guard_verdict" "$rollout_output")")"
  rollout_nested_submit_verify_guard_verdict_raw_upper="$(printf '%s' "$rollout_nested_submit_verify_guard_verdict_raw" | tr '[:lower:]' '[:upper:]')"
  rollout_nested_submit_verify_guard_verdict="$(normalize_strict_guard_verdict "$rollout_nested_submit_verify_guard_verdict_raw")"
  if [[ -z "$rollout_nested_submit_verify_guard_verdict_raw" ]]; then
    input_errors+=("nested adapter rollout rehearsal_nested_submit_verify_guard_verdict must be non-empty")
    rollout_nested_submit_verify_guard_verdict="UNKNOWN"
  elif [[ "$rollout_nested_submit_verify_guard_verdict_raw_upper" != "PASS" && "$rollout_nested_submit_verify_guard_verdict_raw_upper" != "WARN" && "$rollout_nested_submit_verify_guard_verdict_raw_upper" != "UNKNOWN" && "$rollout_nested_submit_verify_guard_verdict_raw_upper" != "SKIP" ]]; then
    input_errors+=("nested adapter rollout rehearsal_nested_submit_verify_guard_verdict must be one of PASS,WARN,UNKNOWN,SKIP (got: ${rollout_nested_submit_verify_guard_verdict_raw})")
    rollout_nested_submit_verify_guard_verdict="UNKNOWN"
  fi
  rollout_nested_submit_verify_guard_reason_code="$(trim_string "$(extract_field "rehearsal_nested_submit_verify_guard_reason_code" "$rollout_output")")"
  if [[ -z "$rollout_nested_submit_verify_guard_reason_code" ]]; then
    input_errors+=("nested adapter rollout rehearsal_nested_submit_verify_guard_reason_code must be non-empty")
    rollout_nested_submit_verify_guard_reason_code="n/a"
  fi
  ...
  if [[ -z "$rollout_nested_confirmed_execution_sample_guard_verdict_raw" ]]; then
    input_errors+=("nested adapter rollout rehearsal_nested_confirmed_execution_sample_guard_verdict must be non-empty")
    rollout_nested_confirmed_execution_sample_guard_verdict="UNKNOWN"
    ...
    input_errors+=("nested adapter rollout rehearsal_nested_confirmed_execution_sample_guard_verdict must be one of PASS,WARN,UNKNOWN,SKIP (got: ${rollout_nested_confirmed_execution_sample_guard_verdict_raw})")
    rollout_nested_confirmed_execution_sample_guard_verdict="UNKNOWN"
  fi
  ...
  if [[ "$go_nogo_require_ingestion_grpc_norm" == "true" ]]; then
    if [[ "$rollout_nested_ingestion_grpc_guard_verdict" == "SKIP" ]]; then
      input_errors+=("nested adapter rollout rehearsal_nested_ingestion_grpc_guard_verdict cannot be SKIP when GO_NOGO_REQUIRE_INGESTION_GRPC=true")
    fi
  else
    if [[ "$rollout_nested_ingestion_grpc_guard_verdict" != "SKIP" ]]; then
      input_errors+=("nested adapter rollout rehearsal_nested_ingestion_grpc_guard_verdict must be SKIP when GO_NOGO_REQUIRE_INGESTION_GRPC=false (got: ${rollout_nested_ingestion_grpc_guard_verdict})")
    fi
  fi
  if [[ "$go_nogo_require_followlist_activity_norm" == "true" ]]; then
    if [[ "$rollout_nested_followlist_activity_guard_verdict" == "SKIP" ]]; then
      input_errors+=("nested adapter rollout rehearsal_nested_followlist_activity_guard_verdict cannot be SKIP when GO_NOGO_REQUIRE_FOLLOWLIST_ACTIVITY=true")
    fi
  else
    if [[ "$rollout_nested_followlist_activity_guard_verdict" != "SKIP" ]]; then
  ...
  if [[ "$go_nogo_require_confirmed_execution_sample_norm" == "true" ]]; then
    if [[ "$rollout_nested_confirmed_execution_sample_guard_verdict" == "SKIP" ]]; then
      input_errors+=("nested adapter rollout rehearsal_nested_confirmed_execution_sample_guard_verdict cannot be SKIP when GO_NOGO_REQUIRE_CONFIRMED_EXECUTION_SAMPLE=true")
    fi
  else
    if [[ "$rollout_nested_confirmed_execution_sample_guard_verdict" != "SKIP" ]]; then
      input_errors+=("nested adapter rollout rehearsal_nested_confirmed_execution_sample_guard_verdict must be SKIP when GO_NOGO_REQUIRE_CONFIRMED_EXECUTION_SAMPLE=false (got: ${rollout_nested_confirmed_execution_sample_guard_verdict})")
    fi
  fi
  if [[ "$go_nogo_require_non_bootstrap_signer_norm" == "true" ]]; then
    if [[ "$rollout_nested_non_bootstrap_signer_guard_verdict" == "SKIP" ]]; then
      input_errors+=("nested adapter rollout rehearsal_nested_non_bootstrap_signer_guard_verdict cannot be SKIP when GO_NOGO_REQUIRE_NON_BOOTSTRAP_SIGNER=true")
    fi
  else
    if [[ "$rollout_nested_non_bootstrap_signer_guard_verdict" != "SKIP" ]]; then
      input_errors+=("nested adapter rollout rehearsal_nested_non_bootstrap_signer_guard_verdict must be SKIP when GO_NOGO_REQUIRE_NON_BOOTSTRAP_SIGNER=false (got: ${rollout_nested_non_bootstrap_signer_guard_verdict})")
    fi
  fi
  if [[ "$go_nogo_require_submit_verify_strict_norm" == "true" ]]; then
    if [[ "$rollout_nested_submit_verify_guard_verdict" == "SKIP" ]]; then
      input_errors+=("nested adapter rollout rehearsal_nested_submit_verify_guard_verdict cannot be SKIP when GO_NOGO_REQUIRE_SUBMIT_VERIFY_STRICT=true")
    fi
  else
    if [[ "$rollout_nested_submit_verify_guard_verdict" != "SKIP" ]]; then
      input_errors+=("nested adapter rollout rehearsal_nested_submit_verify_guard_verdict must be SKIP when GO_NOGO_REQUIRE_SUBMIT_VERIFY_STRICT=false (got: ${rollout_nested_submit_verify_guard_verdict})")
    fi
  fi
  if [[ "$go_nogo_require_jito_rpc_policy_norm" == "true" ]]; then
    if [[ "$rollout_nested_jito_rpc_policy_verdict" == "SKIP" ]]; then
      input_errors+=("nested adapter rollout jito_rpc_policy_verdict cannot be SKIP when GO_NOGO_REQUIRE_JITO_RPC_POLICY=true")
    fi
  else
    if [[ "$rollout_nested_jito_rpc_policy_verdict" != "SKIP" ]]; then
      input_errors+=("nested adapter rollout jito_rpc_policy_verdict must be SKIP when GO_NOGO_REQUIRE_JITO_RPC_POLICY=false (got: ${rollout_nested_jito_rpc_policy_verdict})")
    fi
  fi
  if [[ "$go_nogo_require_fastlane_disabled_norm" == "true" ]]; then
    if [[ "$rollout_nested_fastlane_feature_flag_verdict" == "SKIP" ]]; then
      input_errors+=("nested adapter rollout fastlane_feature_flag_verdict cannot be SKIP when GO_NOGO_REQUIRE_FASTLANE_DISABLED=true")
    fi
  else
    if [[ "$rollout_nested_fastlane_feature_flag_verdict" != "SKIP" ]]; then
      input_errors+=("nested adapter rollout fastlane_feature_flag_verdict must be SKIP when GO_NOGO_REQUIRE_FASTLANE_DISABLED=false (got: ${rollout_nested_fastlane_feature_flag_verdict})")
    fi
  fi
