...
102. strict policy diagnostics now emit stable reason codes (`jito_rpc_policy_reason_code`, `fastlane_feature_flag_reason_code`, `devnet_rehearsal_reason_code`, `adapter_rollout_reason_code`) across go-no-go/windowed/rehearsal/rollout summaries, and smoke assertions use these codes to reduce false failures from copy-edits in human-readable reason text.
