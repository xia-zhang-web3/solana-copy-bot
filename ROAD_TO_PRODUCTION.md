...
496. targeted smoke case-groups and deterministic expansion added: `tools/ops_scripts_smoke_test.sh` now supports group aliases `common_parsers` and `heavy_runtime_chain`, expands and de-duplicates requested cases before dispatch, and emits `ops smoke targeted expanded cases` in summary for reproducible targeted-run observability; dispatcher smoke now pins group expansion behavior.
...
501. audit-wrapper targeted-fast mode added for ops-smoke loops: `tools/audit_standard.sh` and `tools/audit_full.sh` now accept `AUDIT_OPS_SMOKE_MODE=targeted_fast` as shorthand for targeted mode with default `AUDIT_OPS_SMOKE_PROFILE=fast` (overrideable), and smoke guard coverage pins invalid mode whitelist update plus positive `targeted_fast` execution path.
502. audit-standard targeted ops-smoke no-longer depends on touched-op scope: `tools/audit_standard.sh` now always executes ops smoke when resolved mode is `targeted` (unless `AUDIT_SKIP_OPS_SMOKE=true`), and smoke guard pins were updated to assert explicit `targeted ops-smoke requested -> running` path for standard targeted/full+fast branches without marker-file bootstrap.
503. smart targeted profile auto-resolution added across wrappers/smoke: `tools/ops_scripts_smoke_test.sh` now supports `OPS_SMOKE_PROFILE=auto` (resolves to `fast` when expanded targets include heavy runtime/orchestrator cases, otherwise `full`), while `tools/audit_standard.sh` and `tools/audit_full.sh` now accept `AUDIT_OPS_SMOKE_PROFILE={full|fast|auto}` and propagate `auto` unchanged; smoke guard pins cover invalid profile token whitelist update and positive `AUDIT_OPS_SMOKE_PROFILE=auto` heavy-case path.
504. targeted-fast default-case contract + resolved-case observability for audit wrappers: `tools/audit_standard.sh` and `tools/audit_full.sh` now auto-seed `ops_smoke_target_cases=heavy_runtime_chain` when resolved mode is `targeted` with `targeted_fast` defaults and no explicit target/preset, and wrapper logs (run/skip) now include resolved `cases=...`; smoke guard pins cover both full/standard skip branches for `AUDIT_OPS_SMOKE_MODE=targeted_fast` without explicit targets.
505. targeted-mode wrapper profile default switched to smart auto: `tools/audit_standard.sh` and `tools/audit_full.sh` now default `AUDIT_OPS_SMOKE_PROFILE=auto` for resolved `mode=targeted` (while `targeted_fast` still defaults to `fast`), so non-heavy targeted sets remain `full` and heavy targeted sets auto-resolve to `fast`; guard smoke now pins both non-heavy/full and heavy/fast default paths for full+standard wrappers.
506. audit-profile parity for quick/full/standard wrappers: `tools/audit_standard.sh` and `tools/audit_full.sh` now parse strict `AUDIT_PROFILE={default|ops_fast}` (same as `tools/audit_quick.sh`), propagate profile into nested quick baseline, and apply `ops_fast` defaults for local acceleration (`SKIP_CONTRACT_SMOKE=true`, `SKIP_EXECUTOR_TESTS=true`, package/workspace tests skipped, ops-smoke mode defaults to `targeted_fast` with heavy chain). Smoke coverage now pins invalid `AUDIT_PROFILE` reject paths plus default `ops_fast` skip contracts in both wrappers.
507. strict ingestion gRPC guard added to go/no-go + server-rollout orchestration: `tools/execution_go_nogo_report.sh` now supports `GO_NOGO_REQUIRE_INGESTION_GRPC` (strict bool, fail-closed) and emits `ingestion_grpc_guard_*` verdict/reason fields based on ingestion source + `runtime_snapshot` gRPC counters (`grpc_message_total`), while `tools/execution_server_rollout_report.sh` now defaults this gate to `true`, propagates it to nested go/no-go calls, validates nested parity fields fail-closed, and surfaces guard observability in top-level summary.
...
512. refactor phase-gate baseline strict-ingestion parity harness: `tools/refactor_baseline_prepare.sh` now seeds baseline fixtures with deterministic upstream topology (`COPYBOT_EXECUTOR_ROUTE_ALLOWLIST=paper`, submit/simulate URLs) and explicit `ingestion.source=yellowstone_grpc`; `tools/refactor_phase_gate.sh` now runs go/no-go, devnet rehearsal, and adapter rollout with strict gates enabled (`GO_NOGO_REQUIRE_EXECUTOR_UPSTREAM=true`, `GO_NOGO_REQUIRE_INGESTION_GRPC=true`) and emits strict-gate summary fields, while `tools/ops_scripts_smoke_test.sh` adds targeted case `refactor_phase_gate` that fail-closes on missing manifests/normalized outputs and pins nested ingestion guard PASS fields across normalized go/no-go/rehearsal/rollout outputs.
513. refactor phase-gate strict fail-closed contract hardening: `tools/refactor_phase_gate.sh` now parses strict env controls (`REFACTOR_PHASE_GATE_REQUIRE_EXECUTOR_UPSTREAM`, `REFACTOR_PHASE_GATE_REQUIRE_INGESTION_GRPC`, `REFACTOR_PHASE_GATE_INGESTION_SOURCE`) with fail-closed validation, enforces stage-level strict guard invariants across go-no-go/rehearsal/rollout captures (`required=true => PASS`, `required=false => SKIP`, and `GO` verdict expectations), and emits deterministic stage-failure diagnostics when nested helpers exit non-zero; `tools/ops_scripts_smoke_test.sh` extends `refactor_phase_gate` coverage with negative pins (invalid ingestion source + invalid strict bool token), introduces `refactor_phase_gate_fast`, classifies phase-gate in heavy auto-profile resolution, and switches full-suite invocation to fast profile for shorter default smoke runtime.
...
519. targeted smoke default profile auto-resolution hardening: `tools/ops_scripts_smoke_test.sh` now defaults `OPS_SMOKE_PROFILE` to `auto` (instead of `full`) and resolves heavy targeted cases (`executor_preflight`, signoff/rehearsal/rollout/readiness chains, `refactor_phase_gate`) to `fast` automatically when profile is not explicitly specified; non-heavy targeted cases still resolve to `full`, while explicit `OPS_SMOKE_PROFILE=full|fast` keeps manual override behavior unchanged. Coverage in `run_ops_smoke_targeted_dispatch_case` now pins default heavy-path resolution (`executor_preflight` => `fast`) and keeps non-heavy default behavior (`common_parsers` => `full`).
520. refactor phase-gate strict non-bootstrap signer parity closure (building on smoke profile acceleration in `4cc86fc`): `tools/refactor_phase_gate.sh` now parses fail-closed `REFACTOR_PHASE_GATE_REQUIRE_NON_BOOTSTRAP_SIGNER` and propagates it to nested go-no-go/devnet-rehearsal/rollout helpers, then validates end-to-end signer strictness parity (`go_nogo_require_non_bootstrap_signer`, `non_bootstrap_signer_guard_*`, `go_nogo_non_bootstrap_signer_guard_*`, `rehearsal_nested_non_bootstrap_signer_guard_*`) with the same contract (`required=true => PASS`, `required=false => SKIP`, reason-code `gate_disabled` in relaxed mode) and emits normalized summary flag. `tools/ops_scripts_smoke_test.sh` `refactor_phase_gate` full profile now pins baseline relaxed signer path, strict signer fail-close branch, and invalid-bool token reject for `REFACTOR_PHASE_GATE_REQUIRE_NON_BOOTSTRAP_SIGNER`; fast/auto loops keep accelerated behavior from `4cc86fc`.
521. refactor phase-gate fail-fast stage short-circuit (runtime/smoke cycle reduction on strict-negative paths, aligned with `4cc86fc` acceleration): `tools/refactor_phase_gate.sh` now validates each stage immediately after capture (`go_nogo` -> `rehearsal` -> `rollout`) and exits at the earliest failing stage with captured output + deterministic `phase-gate error: <script> failed for stage=<stage>` marker, instead of always launching downstream stages first; strict negative matrices (`REFACTOR_PHASE_GATE_REQUIRE_JITO_RPC_POLICY=true`, strict ingestion source mismatch, strict non-bootstrap signer) now terminate at `go_nogo` stage. `tools/ops_scripts_smoke_test.sh` `refactor_phase_gate` assertions were updated to pin new fail-fast stage markers and direct go-no-go guard fields, preserving full-profile negative coverage while reducing unnecessary downstream helper runs.
522. refactor phase-gate strict non-bootstrap signer positive-path coverage completion (on top of `4cc86fc` auto-fast loops and slice 521 fail-fast): `tools/refactor_baseline_prepare.sh` now supports optional fixture-level signer injection via `REFACTOR_BASELINE_EXECUTOR_SIGNER_PUBKEY`, enabling deterministic strict-signer PASS path without changing default baseline behavior; `tools/ops_scripts_smoke_test.sh` `run_refactor_phase_gate_case` now adds strict-signer positive matrix (`REFACTOR_PHASE_GATE_REQUIRE_NON_BOOTSTRAP_SIGNER=true` + non-bootstrap signer override) and pins PASS parity across all normalized chain levels (`non_bootstrap_signer_guard_*`, `go_nogo_non_bootstrap_signer_guard_*`, `rehearsal_nested_non_bootstrap_signer_guard_*`), while preserving existing strict fail and invalid-token pins.
523. executor runtime upstream placeholder endpoint fail-close hardening: `crates/executor/src/executor_config_env.rs` now rejects placeholder hosts (`example.com` family, `executor.mock.local` family) for route-level upstream endpoints (`submit/simulate` + fallbacks + send-rpc + send-rpc fallback) and submit-verify RPC endpoints when `COPYBOT_EXECUTOR_BACKEND_MODE=upstream`, preventing accidental startup with staging/mock topology; `backend_mode=mock` remains explicitly allowed for synthetic `executor.mock.local` URLs. `crates/executor/src/http_utils.rs` adds shared `endpoint_placeholder_host` detector with unit coverage, and `executor_config_env` tests now pin upstream reject + mock accept behavior for placeholder route/verify endpoints.
524. submit-verify strict-mode fail-close hardening: `crates/executor/src/submit_verify_config.rs` now rejects `COPYBOT_EXECUTOR_SUBMIT_VERIFY_STRICT=true` when `COPYBOT_EXECUTOR_SUBMIT_VERIFY_RPC_URL` is absent (instead of silently disabling verify config), and preserves strict mode when a primary verify endpoint is present; `crates/executor/src/executor_config_env.rs` integration coverage now pins startup reject for this misconfiguration, preventing strict-verify drift in runtime deployment.
525. executor preflight submit-verify topology parity hardening: `tools/executor_preflight.sh` now validates submit-verify config contracts from `executor.env` fail-closed (`COPYBOT_EXECUTOR_SUBMIT_VERIFY_STRICT` strict-bool parse, strict=>primary URL required, fallback URL requires primary, fallback identity must differ from primary, and placeholder host reject in `backend_mode=upstream` for verify primary/fallback URLs). Preflight summary now emits submit-verify observability fields (`executor_submit_verify_strict`, `executor_submit_verify_configured`, `executor_submit_verify_fallback_configured`), and `tools/ops_scripts_smoke_test.sh` executor-preflight coverage now pins strict-missing-primary, fallback-without-primary, placeholder verify URL, and verify identity-collision negative paths.
526. submit-verify preflight parity propagation through rollout/final/orchestrator chain: `tools/executor_rollout_evidence_report.sh` now fail-closes on malformed preflight submit-verify fields and enforces internal contract (`strict=true => configured=true`, `fallback_configured=true => configured=true`) while exporting them as `preflight_executor_submit_verify_*`; `tools/executor_final_evidence_report.sh` now extracts/validates these nested rollout fields as `rollout_nested_preflight_executor_submit_verify_*`; and `tools/execution_server_rollout_report.sh` now validates/surfaces executor-final nested submit-verify preflight fields (`executor_final_rollout_nested_preflight_executor_submit_verify_*`) fail-closed. `tools/ops_scripts_smoke_test.sh` coverage now pins these fields across executor rollout/final and server-rollout pass/skip/profile/bundle/mock-allowed paths.
527. strict submit-verify guard enabled in go/no-go + parity wiring completion: `tools/execution_go_nogo_report.sh` now parses fail-closed `GO_NOGO_REQUIRE_SUBMIT_VERIFY_STRICT`, evaluates executor submit-verify topology (`COPYBOT_EXECUTOR_SUBMIT_VERIFY_STRICT`, primary/fallback verify URLs, fallback identity, upstream placeholder host) into `submit_verify_guard_*`, and blocks GO on `WARN/UNKNOWN` under strict mode; `tools/execution_server_rollout_report.sh`/`tools/executor_final_evidence_report.sh`/`tools/executor_rollout_evidence_report.sh` now consistently propagate/validate `go_nogo_require_submit_verify_strict` through nested summaries; `tools/ops_scripts_smoke_test.sh` now pins strict submit-verify `PASS/WARN/UNKNOWN` go/no-go paths plus invalid-bool fail-closed paths in go-no-go/rollout/final/server-rollout helpers.
528. submit-verify strict guard propagation into signoff/rehearsal/final route-fee chain: `tools/execution_devnet_rehearsal.sh`, `tools/execution_windowed_signoff_report.sh`, `tools/execution_route_fee_signoff_report.sh`, and `tools/execution_route_fee_final_evidence_report.sh` now parse+propagate+validate `GO_NOGO_REQUIRE_SUBMIT_VERIFY_STRICT` and nested `submit_verify_guard_*` fields with fail-closed parity (`required=true => non-SKIP`, `required=false => SKIP`), including dynamic `WINDOWS_CSV` guard-key extraction in route-fee final evidence; `tools/ops_scripts_smoke_test.sh` coverage now pins default relaxed (`false`) SKIP/gate_disabled submit-verify fields across windowed signoff, route/fee signoff, devnet rehearsal, and route-fee final package paths (24h + 1,6 + bundle).
529. go/no-go strict submit-verify identity-collision fail-open hotfix: `tools/execution_go_nogo_report.sh` now normalizes submit-verify primary/fallback endpoint identity (scheme+host+port+path) before distinctness checks instead of comparing raw URL strings, and fails closed with `submit_verify_primary_invalid` / `submit_verify_fallback_invalid` when strict guard encounters invalid verify URLs; `tools/ops_scripts_smoke_test.sh` now pins strict-mode negative paths for normalized identity collision (`https://verify.integration.test` vs `https://VERIFY.integration.test:443/`) and invalid primary verify URL (`UNKNOWN` guard).
...
534. strict followlist activity parity propagation closure across rollout/readiness/refactor chains (ordering policy checkpoint: `N=0` from `ops/executor_ordering_coverage_policy.md`; slice type = runtime/e2e, non-coverage; continues accelerated targeted loop behavior from `4cc86fc`): `tools/execution_windowed_signoff_report.sh`, `tools/execution_route_fee_signoff_report.sh`, `tools/execution_route_fee_final_evidence_report.sh`, `tools/execution_devnet_rehearsal.sh`, `tools/executor_rollout_evidence_report.sh`, `tools/executor_final_evidence_report.sh`, `tools/adapter_rollout_evidence_report.sh`, `tools/adapter_rollout_final_evidence_report.sh`, `tools/execution_server_rollout_report.sh`, `tools/execution_runtime_readiness_report.sh`, and `tools/refactor_phase_gate.sh` now carry fail-closed `GO_NOGO_REQUIRE_FOLLOWLIST_ACTIVITY` parity (`required=true => non-SKIP`, `required=false => SKIP`) for nested followlist guard fields through server/final/readiness/refactor chains, with top-level observability fields exported at each level. `tools/ops_scripts_smoke_test.sh` now pins these followlist fields across `execution_server_rollout`, `execution_runtime_readiness`, and `refactor_phase_gate` pass/skip/profile/bundle/mock/strict paths, including strict followlist fail-close (`followlist_inactive`) and strict pass (`followlist_active`) matrices plus invalid bool reject pins for `GO_NOGO_REQUIRE_FOLLOWLIST_ACTIVITY` and `REFACTOR_PHASE_GATE_REQUIRE_FOLLOWLIST_ACTIVITY`.
535. targeted smoke runtime protection against accidental heavy `full` profile runs (DX/runtime hardening; directly addresses loop friction after accelerated targeted flow from `4cc86fc`): `tools/ops_scripts_smoke_test.sh` now parses fail-closed `OPS_SMOKE_ALLOW_HEAVY_FULL` (default `false`) and rejects `OPS_SMOKE_PROFILE=full` when targeted case expansion includes heavy runtime chains unless explicit override is set (`OPS_SMOKE_ALLOW_HEAVY_FULL=true`), emitting deterministic diagnostics with case list + migration hint (`fast|auto`). `run_ops_smoke_targeted_dispatch_case` now pins this guard with negative/positive coverage: invalid override token reject, blocked `full+heavy` path, and explicit override allow-path.
536. targeted smoke per-case runtime budget guard (DX/runtime hardening continuation over `4cc86fc` accelerated loops): `tools/ops_scripts_smoke_test.sh` now enforces fail-closed `OPS_SMOKE_CASE_TIMEOUT_SEC` (integer range `1..120`, default `120`) and executes heavy targeted cases through timeout wrapper (`run_target_case_with_timeout`), terminating cases that exceed budget with deterministic `targeted smoke case timed out after <N>s: <case>` marker. `run_ops_smoke_targeted_dispatch_case` now pins invalid timeout token/range rejects and timeout enforcement path (`OPS_SMOKE_CASE_TIMEOUT_SEC=1` on `executor_preflight_fast`), while preserving explicit heavy-full override flow from slice 535.
537. executor runtime internal paper backend for upstream-mode bring-up (runtime/e2e progress aligned with backend plan item and accelerated loop baseline from `4cc86fc`): `crates/executor/src/route_adapters.rs` now short-circuits `paper` route simulate/submit actions to internal contract-compatible success responses (`executor_paper_simulation_ok`, `executor_paper_submit_ok`) with deterministic paper submit signature generation, so paper non-live flow no longer depends on external upstream transport. `crates/executor/src/executor_config_env.rs` now permits `COPYBOT_EXECUTOR_BACKEND_MODE=upstream` with allowlist-only `paper` route even when external upstream submit/simulate URLs are not provided by synthesizing internal paper endpoints (`https://executor.paper.local/<route>/<action>`). `tools/executor_preflight.sh` and `tools/execution_go_nogo_report.sh` now apply the same paper internal endpoint defaults in topology resolution, keeping preflight/go-no-go strict upstream parity with runtime behavior.
538. paper-route submit verify bypass for strict non-live contour: `crates/executor/src/submit_handler.rs` now classifies normalized route kind and skips submit-signature verification network probes for `paper` submits (returns `submit_signature_verify.enabled=false`) while preserving strict verify behavior for non-paper routes; submit success payload now emits explicit transport `executor_paper_internal` for paper upstream-signature artifacts (instead of generic `upstream_signature`) to keep runtime telemetry contract aligned with internal paper execution semantics. `crates/executor/src/main.rs` adds integration coverage `handle_submit_skips_signature_verify_for_internal_paper_route_in_strict_mode`, and strict non-paper path coverage remains pinned by `handle_submit_rejects_after_send_rpc_when_signature_verify_strict_unseen`.
539. mock-backend strict submit-verify bypass parity for non-live executor contour: `crates/executor/src/submit_handler.rs` now skips submit-signature verification probes when `COPYBOT_EXECUTOR_BACKEND_MODE=mock` (same `submit_signature_verify.enabled=false` contract used by internal paper path), preventing strict verify network failures in embedded mock execution while preserving strict verify behavior for real upstream mode; submit success payload now emits explicit mock transport label `executor_mock_internal` for mock upstream-signature artifacts. `crates/executor/src/main.rs` adds integration coverage `handle_submit_skips_signature_verify_for_mock_backend_in_strict_mode` and keeps strict non-mock failure guard pinned by `handle_submit_rejects_after_send_rpc_when_signature_verify_strict_unseen`.
540. mock+paper transport observability priority alignment: `crates/executor/src/submit_handler.rs` now prioritizes route-kind transport labeling over backend-mode labeling for upstream-signature artifacts (`paper` label wins before `mock`), aligning `submit_transport` with actual route-adapter execution order (paper branch is selected before mock branch). `crates/executor/src/main.rs` adds edge-case integration guard `handle_submit_uses_paper_transport_label_when_route_is_paper_in_mock_mode`, pinning `submit_transport=executor_paper_internal` and `submit_signature_verify.enabled=false` for `route=paper` in mock mode.
541. deterministic mock submit signature identity hardening: `crates/executor/src/route_adapters.rs` now generates mock-mode submit `tx_signature` from request identity (`request_id` + `client_order_id`) via deterministic SHA-256 namespace hashing (instead of static constant signature), keeping non-live mock submit traces stable for idempotent repeats while preventing cross-request signature collisions; paper signature generation now reuses the same shared deterministic helper with a separate namespace. Coverage adds `build_mock_submit_backend_response_uses_deterministic_signature_per_identity` to pin same-identity stability and cross-identity divergence.
542. deterministic submit-signature domain split hardening: `crates/executor/src/route_adapters.rs` now separates signature field domains by backend intent: mock signatures include extended request identity (`request_id`, `client_order_id`, `signal_id`, `route_hint`, `side`, `token`) to reduce same-request collisions in non-live tracing, while paper signatures intentionally remain on legacy identity inputs (`request_id`, `client_order_id`) for backward-compatible artifacts. Coverage adds guards for mock extended-identity divergence, paper legacy-input stability, and namespace separation (`mock` vs `paper` signatures must differ for same payload identity).
543. mock signature canonical-identity normalization: `crates/executor/src/route_adapters.rs` now normalizes mock signature identity fields `route_hint`/`side` before hashing (`route_hint` via `normalize_route`, `side` via `trim().to_ascii_lowercase()`), preventing accidental signature drift from case/whitespace-only payload differences while keeping paper legacy signature domain unchanged; coverage adds `build_mock_submit_backend_response_normalizes_route_hint_and_side_for_identity`.
544. internal non-live submit identity echo expansion: `crates/executor/src/route_adapters.rs` now includes optional submit identity echo fields in internal mock/paper submit responses (`signal_id`, canonical `side`, trimmed `token`) via shared helper, improving adapter/executor non-live trace correlation without changing required response-contract fields; coverage extends existing mock/paper submit response tests and adds explicit side/token normalization echo guard for mock submit payloads.
545. internal non-live submit fee-hint parity for mock/paper execution path: internal submit branches in `crates/executor/src/route_adapters.rs` (`paper` and `mock`) now append derived fee hints from validated `instruction_plan` (`base_fee_lamports=5000`, `priority_fee_lamports=cu_limit*cu_price/1_000_000`, `network_fee_lamports=base+priority`) so non-live submit telemetry aligns with downstream fee-decomposition expectations; integration coverage in `crates/executor/src/main.rs` `execute_route_action_uses_embedded_mock_backend_for_submit` and `execute_route_action_uses_internal_paper_backend_for_submit_in_upstream_mode` now pins these fields.
546. mock submit signature delimiter-collision hardening with legacy paper compatibility: `crates/executor/src/route_adapters.rs` now uses length-tagged field hashing for mock deterministic signatures (`hash_tagged_fields` for v1/v2 domains) to avoid ambiguous concatenation collisions when identity fields include delimiters, while paper signature generation remains on legacy colon-joined encoding for backward-compatible non-live artifacts. Coverage adds `mock_submit_signature_avoids_delimiter_collision_with_tagged_fields` and preserves paper legacy stability guards.
547. mock submit signature token canonicalization parity: `crates/executor/src/route_adapters.rs` now normalizes mock signature `token` identity input via `trim()` (alongside existing route/side canonicalization), eliminating signature drift from whitespace-only token formatting while preserving token-value sensitivity. Coverage extends mock identity divergence guard with explicit token-change assertion and updates canonicalization test to pin token-whitespace equivalence.
548. internal fee-hint signed-range hardening for non-live submit path: `crates/executor/src/route_adapters.rs` `append_internal_submit_fee_hint_fields` now skips fee hint emission when derived `priority_fee_lamports`/`network_fee_lamports` exceed signed 64-bit range (`i64::MAX`) to prevent downstream fee-hint parser rejects on extreme compute-budget configurations; coverage adds `append_internal_submit_fee_hint_fields_skips_values_outside_i64_range`.
549. internal non-live simulate identity echo parity: `crates/executor/src/route_adapters.rs` now propagates optional identity echo fields into internal simulate responses for both mock and paper paths (`request_id`, `signal_id`, canonical `side`, trimmed `token`) via shared helper, aligning non-live simulate telemetry/correlation with internal submit response behavior. Coverage updates route-adapter unit tests plus integration checks in `crates/executor/src/main.rs` simulate-path tests for mock/paper internal backends.
550. request-identity canonicalization parity hardening across internal echoes and submit response validation: `crates/executor/src/route_adapters.rs` now trims echoed `request_id`/`signal_id`/`client_order_id` in internal mock/paper response builders (and removes redundant duplicate `request_id` assignment in submit builders), while `crates/executor/src/submit_response.rs` now trims expected request identity tokens before comparison to parsed response identity. This closes whitespace-only mismatch drift between request payload, internal echoes, and submit response validator. Coverage adds route-adapter trim guards (`build_mock_{simulate,submit}_backend_response_trims_*`) and submit-response trim parity tests.
551. submit-response extended identity parity enforcement: `crates/executor/src/submit_response.rs` now validates optional upstream submit identity echoes for `signal_id`, `side`, and `token` when present (in addition to existing `request_id`/`client_order_id`), with canonical side comparison (`case-insensitive`) and token/request identity trim parity. `crates/executor/src/submit_handler.rs` now enforces this validator on live submit path, and `crates/executor/src/reject_mapping.rs` adds explicit terminal mismatch classifications (`submit_adapter_signal_id_mismatch`, `submit_adapter_side_mismatch`, `submit_adapter_token_mismatch`). Coverage adds unit guards in `submit_response.rs` plus integration submit-handler checks in `crates/executor/src/main.rs` for signal mismatch reject and side case-insensitive accept path.
552. simulate-response extended identity parity enforcement: `crates/executor/src/simulate_response.rs` now validates optional upstream simulate identity echoes for `request_id`, `signal_id`, `side`, and `token` when present, including canonical side comparison (`case-insensitive`) and trim parity for request/token identities. `crates/executor/src/simulate_handler.rs` now enforces this validator on runtime simulate path, and `crates/executor/src/reject_mapping.rs` adds explicit terminal mismatch classifications (`simulation_request_id_mismatch`, `simulation_signal_id_mismatch`, `simulation_side_mismatch`, `simulation_token_mismatch`). Coverage adds unit guards in `simulate_response.rs` and integration simulate-handler checks in `crates/executor/src/main.rs` for signal mismatch reject and side case-insensitive accept path.
553. executor success-payload identity echo parity hardening: `crates/executor/src/submit_payload.rs` and `crates/executor/src/simulate_response.rs` now emit canonical identity echoes (`request_id`, `signal_id`, `side`, `token`; plus `client_order_id` for submit) in final executor success payloads, with trim parity and case-normalized `side` to align outbound executor payloads with previously hardened upstream/internal identity validation. `crates/executor/src/submit_handler.rs` and `crates/executor/src/simulate_handler.rs` now wire these fields explicitly into payload builders. Coverage adds payload canonicalization unit tests and validates submit/simulate handler success paths with existing integration tests.
...
1. `DONE 2026-03-14` decouple `ingestion.next_swap()` from synchronous observed-swap ack.
   ...
   3. Follow-up hardening included:
      ...
      6. irrelevant swaps now use `try_enqueue()` with immediate awaited fallback on the same event, so there is no app-local deferred raw backlog or new crash-loss window,
      7. old ack-based `write()` path remains for callers that still need commit outcome.
...
5. `P1` investigate SQLite/WAL reliability as a separate track.
   ...
   23. `DONE 2026-03-14` discovery-cycle universe stop / clear event writes now fail closed on fatal SQLite I/O while busy/locked universe event writes stay warn-only and fatal writes preserve the pre-existing runtime `universe_blocked` state instead of half-applying transitions,
   24. `DONE 2026-03-14` ingestion-snapshot infra stop / clear event writes now fail closed on fatal SQLite I/O while busy/locked infra event writes stay warn-only and fatal writes preserve the pre-existing runtime infra block state instead of half-applying transitions,
   25. `DONE 2026-03-14` `shadow_risk_fail_closed` event writes inside `can_open_buy()` now stop degrading to local warn-only on fatal SQLite I/O while both fail-closed branches remain blocked and restart semantics stay unchanged,
   26. `DONE 2026-03-14` shadow worker task outcomes now fail closed on fatal SQLite I/O while busy/locked worker errors stay warn-only and existing join-failure scheduler-reset behavior remains unchanged,
   27. `DONE 2026-03-14` shadow snapshot task outcomes now fail closed on fatal SQLite I/O while busy/locked snapshot failures stay warn-only and existing join-failure semantics remain unchanged,
   ...
   30. remaining work is still broader than startup / heartbeat / history retention / observed-swap retention / stale-close cleanup / alert delivery / discovery cursor persistence / writer-owned discovery scoring materialization / discovery token-quality cache / discovery activity-day counts / discovery cursor-load restore / discovery warm-load / direct `main.rs` risk-event writes / startup durable pause restore / periodic shadow-risk background refresh / shadow open-lot refresh / `ShadowRiskGuard` state-event writes / drawdown timed-pause activation writes / expired timed-pause clear writes / discovery-cycle universe event writes / ingestion-snapshot infra event writes / `shadow_risk_fail_closed` event writes / shadow worker task outcomes / shadow snapshot task outcomes / execution batch task outcomes / soft-exposure pause clear writes:
   ...
   31. not closed by either `e24eca2` or `8caa9b7`.
