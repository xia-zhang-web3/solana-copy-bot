...
44. execution BUY-risk enforcement extended to live loss controls: runtime now applies `risk.daily_loss_limit_pct` (24h realized PnL guard) and `risk.max_drawdown_pct` (rolling 24h closed-position drawdown guard) as `risk_blocked` BUY gates, backed by storage queries (`live_realized_pnl_since`, `live_max_drawdown_since`) and runtime risk contract validation for both percent fields (`[0, 100]` bounds).
