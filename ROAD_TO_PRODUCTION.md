...
14. ingestion telemetry now tracks parse rejects by reason (in addition to `parse_rejected_total`) and parser fallback reasons (for example `missing_program_ids_fallback`) as separate reason breakdown.
...
26. adapter response contract pinning added: runtime enforces `contract_version` match and optional strict policy-echo checks via `submit_adapter_require_policy_echo` (slippage/tip/CU + fee-breakdown echo `base_fee_lamports`/`priority_fee_lamports`).
...
38. adapter endpoint runtime validation hardened: `execution.submit_adapter_http_url` / `execution.submit_adapter_fallback_http_url` must be explicit `http(s)` URLs (no bare host), and production-like env profiles now fail-closed for non-loopback `http://` endpoints (`https://` required except local loopback).
