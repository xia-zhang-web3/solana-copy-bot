...
1. wire production adapter backend for real signed tx send path (using `adapter_submit_confirm` contract) and complete server-side secret distribution/rotation rollout (systemd mounts + rotation drill + evidence).
