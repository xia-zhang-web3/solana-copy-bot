...
impl DiscoveryService {
    pub(crate) fn resolve_run_cycle_persisted_recompute_in_progress(
        &self,
        store: &SqliteStore,
        now: DateTime<Utc>,
        ...
        let catch_up_requested = should_request_persisted_stream_catch_up(&telemetry);
        let catch_up_pressure_override_requested =
            should_request_persisted_stream_catch_up_pressure_override(&telemetry);
        if let Some(active_wallets) = recent_published_follow_wallets {
            let summary = self
            ...
            self.log_persisted_recompute_in_progress_summary(
                &summary,
                cycle_started,
                metrics_window_start,
                fetch_limit,
        ...
        if let Some(active_wallets) = bootstrap_degraded_follow_wallets {
            let summary = self
        ...
        Ok(RunCyclePreparedResolution::Returned(summary))
    }

    fn log_persisted_recompute_in_progress_summary(
        &self,
