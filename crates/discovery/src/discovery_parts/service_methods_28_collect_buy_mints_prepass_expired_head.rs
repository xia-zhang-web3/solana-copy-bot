...
impl DiscoveryService {
    pub(crate) fn advance_collect_buy_mints_reconcile_expired_head_prepass(
        &self,
        store: &SqliteStore,
        ...
        {
            let candidate_page =
                store.load_observed_buy_mints_in_time_bounds_after_token_with_budget(
                    source_window_start,
                    true,
                    state.window_start,
                    false,
            ...
            if candidate_page.mints.is_empty() {
                ...
                state
                    ...
                    .collect_buy_mints_reconcile_expired_head_cursor_token = None;
                Self::clear_reconcile_expired_head_pending_batch(&mut state.payload);
                Self::log_collect_buy_mints_expired_head_complete(state);
                return Ok(CollectBuyMintsPrepassStep::Continue);
            }
            state
                .payload
    ...
    fn log_collect_buy_mints_expired_head_complete(state: &PersistedStreamRebuildState) {
        info!(
