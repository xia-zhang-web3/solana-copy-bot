...
impl DiscoveryService {
    fn persisted_stream_cycle_telemetry(
        ...
        let phase_cursor = if state.phase == DiscoveryPersistedRebuildPhase::PublishPending {
            None
        } else {
            state.phase_cursor.clone()
        };
        let collect_buy_mints_cursor_token =
            if state.phase == DiscoveryPersistedRebuildPhase::PublishPending {
                None
            } else {
                state.payload.collect_buy_mints_cursor_token.clone()
            };
        let wallets_buffered = if state.phase == DiscoveryPersistedRebuildPhase::PublishPending {
            state.payload.completed_snapshots.len()
        } else {
            state.payload.by_wallet.len()
        };
        let publish_pending_requested_wallet_count =
            if state.phase == DiscoveryPersistedRebuildPhase::PublishPending {
        ...
        PersistedStreamProgressTelemetry {
            ...
            replay_wallet_stats_publishable_horizon_remaining_ms: self
                .replay_wallet_stats_remaining_publishable_horizon_ms(state, input.now),
            ...
            replay_sol_leg_publishable_horizon_remaining_ms: self
                .replay_sol_leg_remaining_publishable_horizon_ms(state, input.now),
            ...
            replay_candidate_activity_backfill_wallet_cursor: state
                ...
                .clone(),
            replay_sol_leg_access_path: input.cycle_replay_sol_leg_access_path,
            ...
            cycle_rows_processed: input.cycle_rows_processed,
            cycle_pages_processed: input.cycle_pages_processed,
            cycle_replay_wallet_stats_day_count_source_progress: input
                .cycle_replay_wallet_stats_day_count_source_progress,
            cycle_replay_wallet_stats_wallet_cursor_before: input
                .cycle_replay_wallet_stats_wallet_cursor_before,
            cycle_replay_wallet_stats_wallet_cursor_after: state
                .payload
                .replay_wallet_stats_wallet_cursor
                .clone(),
            cycle_unique_buy_mints_discovered: input.cycle_unique_buy_mints_discovered,
            ...
            cycle_elapsed_ms: input.cycle_elapsed_ms,
            total_elapsed_ms: (input
                .now
                .signed_duration_since(state.started_at)
                .num_milliseconds()
                .max(0) as u64)
                .saturating_add(input.cycle_elapsed_ms),
            partial: input.partial,
            completed: input.completed,
            budget_exhausted_reason: input.budget_exhausted_reason,
        }
    }
}
