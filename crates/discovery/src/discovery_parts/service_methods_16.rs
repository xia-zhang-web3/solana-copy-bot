use crate::*;

#[path = "service_methods_16_replay_candidate_activity.rs"]
mod service_methods_16_replay_candidate_activity;
#[path = "service_methods_16_buy_mint_counts_and_replay_reset.rs"]
mod service_methods_16_buy_mint_counts_and_replay_reset;
#[path = "service_methods_16_token_quality_transitions.rs"]
mod service_methods_16_token_quality_transitions;
