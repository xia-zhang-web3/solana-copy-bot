...
const USAGE: &str =
    "usage: discovery_replay_sol_leg_progress_audit --db <path> --config <path> [--json]";

const CONCLUSION_CURSOR_LAGS_SOURCE_FRONTIER: &str =
    "replay_sol_leg_incomplete_because_cursor_still_lags_source_frontier";
const CONCLUSION_REPLAY_STATE_LOOKS_STALLED: &str =
    "replay_sol_leg_incomplete_because_replay_state_looks_stalled";
const CONCLUSION_COMPLETION_GATE_NOT_REACHED_AFTER_PARTIAL_PROGRESS: &str =
    "replay_sol_leg_incomplete_because_completion_gate_not_reached_after_partial_progress";
...
const REPLAY_PHASE_TRANSITION_SURFACE: &str =
    "copybot_discovery::DiscoveryService::replay_subphase";
const REPLAY_SOL_LEG_BLOCKER_SURFACE: &str =
    "copybot_discovery::DiscoveryService::explain_replay_sol_leg_incomplete_read_only";
const REPLAY_PROGRESS_STATE_SOURCE_NAME: &str = "discovery_persisted_rebuild_state";
...
struct ReplaySolLegProgressAuditReport {
    ...
    replay_sol_leg_progress_conclusion: String,
}
...
struct ReplayStateFacts {
    ...
    cursor_present: bool,
    progress_state_source: Option<String>,
    publishable: bool,
    completed: bool,
}
...
fn run(config: &Config) -> Result<ReplaySolLegProgressAuditReport> {
    ...
    let comparison = compare_frontiers(&replay_state, &frontier, publication_window_start_utc);
    let code_paths = inspect_code_path_facts();
    let conclusion = choose_conclusion(&replay_state, &comparison, &code_paths);

    Ok(ReplaySolLegProgressAuditReport {
        ...
        replay_cursor_before_publication_window_start: comparison.cursor_before_publication_window_start,
        replay_cursor_before_observed_swaps_frontier: comparison.cursor_before_observed_swaps_frontier,
        replay_cursor_lag_seconds_vs_observed_swaps_frontier: comparison
            .cursor_lag_seconds_vs_observed_swaps_frontier,
        replay_cursor_lag_days_vs_observed_swaps_frontier: comparison
        ...
        replay_progress_state_source: replay_state.progress_state_source,
        ...
        replay_phase_transition_surface: REPLAY_PHASE_TRANSITION_SURFACE.to_string(),
        ...
        replay_sol_leg_progress_conclusion: conclusion.to_string(),
    })
}
...
fn load_replay_state_facts(store: &SqliteStore) -> Result<ReplayStateFacts> {
    ...
    Ok(ReplayStateFacts {
        ...
        cursor_present,
        progress_state_source: Some(REPLAY_PROGRESS_STATE_SOURCE_NAME.to_string()),
        publishable,
        completed,
    })
}
...
fn inspect_code_path_facts() -> CodePathFacts {
    let publishable_depends_on_replay_cursor_reaching_required_frontier = DISCOVERY_SOURCE
        ...
        && DISCOVERY_SOURCE.contains("Some(\"sol_leg\")")
...
fn choose_conclusion(
    replay_state: &ReplayStateFacts,
    comparison: &FrontierComparisonFacts,
    code_paths: &CodePathFacts,
) -> &'static str {
    let degenerate_progress = replay_state.state_present
        && !replay_state.cursor_present
        && !replay_state.has_nonzero_rows_processed
        && !replay_state.has_nonzero_chunks_completed;
    let unchanged_progress = replay_state.state_present
        && !replay_state.updated_after_started
        && !replay_state.has_nonzero_chunks_completed
        && !replay_state.has_nonzero_rows_processed;
    if degenerate_progress || unchanged_progress {
        return CONCLUSION_REPLAY_STATE_LOOKS_STALLED;
    }

    if replay_state.state_present
        && replay_state.cursor_present
        && (replay_state.has_nonzero_rows_processed || replay_state.has_nonzero_chunks_completed)
        && comparison.cursor_before_observed_swaps_frontier == Some(true)
        && comparison
            .cursor_lag_seconds_vs_observed_swaps_frontier
            .is_some_and(|lag| lag > 0)
    {
        return CONCLUSION_CURSOR_LAGS_SOURCE_FRONTIER;
    }
    ...
    {
        return CONCLUSION_COMPLETION_GATE_NOT_REACHED_AFTER_PARTIAL_PROGRESS;
    }

    CONCLUSION_INSUFFICIENT_EVIDENCE
}

fn render_output(report: &ReplaySolLegProgressAuditReport, json: bool) -> Result<String> {
    if json {
        return serde_json::to_string_pretty(report)
            .context("failed to serialize replay sol leg progress audit report");
    }
    ...
    Ok(format!(
        concat!(
            ...
            "replay_progress_state_source={replay_progress_state_source}\n",
            "replay_publishable={replay_publishable}\n",
            "replay_completed={replay_completed}\n",
            "replay_publishable_checkpoint_surface={replay_publishable_checkpoint_surface}\n",
            "replay_phase_transition_surface={replay_phase_transition_surface}\n",
            ...
            "replay_sol_leg_progress_conclusion={replay_sol_leg_progress_conclusion}\n",
        ),
        ...
        replay_progress_state_source =
            format_optional_str(report.replay_progress_state_source.as_deref()),
        ...
        replay_phase_transition_surface = report.replay_phase_transition_surface,
        ...
        replay_sol_leg_progress_conclusion = report.replay_sol_leg_progress_conclusion,
    ))
}
...
mod tests {
    ...
    fn create_fixture() -> Result<(TempDir, PathBuf, SqliteStore)> {
        ...
        let db_path = temp.path().join("replay-sol-leg-progress-audit.db");
    ...
    #[test]
    fn parse_args_reads_required_replay_sol_leg_progress_audit_flags() -> Result<()> {
    ...
    fn json_output_contains_all_required_fields() -> Result<()> {
        ...
        for key in [
            ...
            "replay_phase_transition_surface",
            "replay_sol_leg_blocker_surface",
            "publishable_depends_on_replay_cursor_reaching_required_frontier",
            "fail_closed_persists_while_replay_blocker_present",
            "replay_sol_leg_progress_conclusion",
        ] {
    ...
    #[test]
    fn lagging_cursor_fixture_yields_lag_conclusion() -> Result<()> {
        let (_temp, db_path, store) = create_fixture()?;
        let updated_at = DateTime::parse_from_rfc3339("2026-04-21T16:07:06Z")
            .expect("valid timestamp")
            .with_timezone(&Utc);
        seed_publication_state(&store)?;
        seed_frontier_context(&store)?;
        seed_rebuild_state(
            ...
            ReplaySeed {
                ...
                pages_processed: 5,
                chunks_completed: 2,
                started_at: DateTime::parse_from_rfc3339("2026-04-21T15:50:00Z")
                    .expect("valid timestamp")
                    .with_timezone(&Utc),
                updated_at,
        ...
        let report = run(&fixture_config(&db_path))?;
        assert_eq!(
            report.replay_sol_leg_progress_conclusion,
            CONCLUSION_CURSOR_LAGS_SOURCE_FRONTIER
        );
        assert_eq!(report.replay_cursor_before_observed_swaps_frontier, Some(true));
        assert!(report.replay_cursor_lag_seconds_vs_observed_swaps_frontier.unwrap_or(0) > 0);
        Ok(())
    }

    #[test]
    fn degenerate_replay_progress_fixture_never_overclaims_beyond_stalled_or_insufficient(
    ) -> Result<()> {
        let (_temp, db_path, store) = create_fixture()?;
        seed_publication_state(&store)?;
        seed_frontier_context(&store)?;
        ...
        let report = run(&fixture_config(&db_path))?;
        assert!(
            report.replay_sol_leg_progress_conclusion == CONCLUSION_REPLAY_STATE_LOOKS_STALLED
                || report.replay_sol_leg_progress_conclusion == CONCLUSION_INSUFFICIENT_EVIDENCE
        );
        assert_ne!(
            report.replay_sol_leg_progress_conclusion,
            CONCLUSION_CURSOR_LAGS_SOURCE_FRONTIER
