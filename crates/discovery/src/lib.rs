...
const DISCOVERY_PUBLICATION_TRUTH_REPAIR_DEEP_REPLAY_WALLET_STATS_PAGE_HEADROOM_DENOMINATOR: usize =
    2;
const DISCOVERY_PUBLICATION_TRUTH_REPLAY_WALLET_STATS_FRONTIER_SATURATION_NUMERATOR: usize = 19;
...
#[cfg(test)]
fn arm_test_force_collect_buy_mints_fresh_scan_row_limit(limit: usize) {
    TEST_FORCE_COLLECT_BUY_MINTS_FRESH_SCAN_ROW_LIMIT.with(|value| value.set(limit));
}

#[cfg(test)]
fn take_test_force_collect_buy_mints_fresh_scan_row_limit() -> Option<usize> {
...
impl DiscoveryService {
    fn persisted_stream_publishable_checkpoint_blocker_for_phase(
        ...
        match phase {
            ...
            DiscoveryPersistedRebuildPhase::Replay => {
                if !replay_wallet_stats_complete {
                    "replay_wallet_stats_incomplete"
                } else if phase_cursor_present {
                    "replay_sol_leg_incomplete"
                } else {
                    "replay_post_wallet_stats_handoff_pending"
                }
            }
    ...
    fn persisted_stream_publishable_checkpoint_blocker(
        ...
        Self::persisted_stream_publishable_checkpoint_blocker_for_phase(
            ...
            telemetry.replay_wallet_stats_complete,
            telemetry.phase_cursor.is_some(),
        )
    }
    ...
    fn state_needs_deep_replay_wallet_stats_priority_recovery_contract(
        state: &PersistedStreamRebuildState,
    ) -> bool {
    ...
    fn replay_wallet_stats_total_wallets_processed(state: &PersistedStreamRebuildState) -> usize {
        state
            .payload
            .replay_wallet_stats_day_count_source_progress
    ...
    fn replay_wallet_stats_last_partial_cycle_frontier_saturated(
        fetch_limit: usize,
        state: &PersistedStreamRebuildState,
    ) -> bool {
    ...
    fn replay_wallet_stats_open_frontier_floor_pages(
        fetch_limit: usize,
        state: &PersistedStreamRebuildState,
    ) -> usize {
    ...
    fn replay_wallet_stats_remaining_publishable_horizon_ms(
        &self,
        state: &PersistedStreamRebuildState,
        now: DateTime<Utc>,
    ) -> Option<u64> {
    ...
    fn deep_replay_sol_leg_target_time_budget(
        ...
        baseline_time_budget: StdDuration,
        fetch_page_limit: usize,
        processed_floor_pages: usize,
    ) -> StdDuration {
        let floor_pages_with_headroom = processed_floor_pages
            .saturating_mul(
                DISCOVERY_PUBLICATION_TRUTH_REPAIR_DEEP_REPLAY_WALLET_STATS_PAGE_HEADROOM_NUMERATOR,
            )
            .div_ceil(
        ...
        let base_fetch_budget_ms = self.config.fetch_time_budget_ms.max(1) as u128;
        let floor_budget_ms = (floor_pages_with_headroom as u128)
            .saturating_mul(base_fetch_budget_ms)
            .div_ceil(fetch_page_limit.max(1) as u128);
        let target_budget_ms = baseline_time_budget
            .as_millis()
            .max(floor_budget_ms)
    ...
    fn deepen_persisted_stream_priority_recovery_contract_for_state_at(
        ...
        if Self::state_needs_deep_replay_wallet_stats_priority_recovery_contract(state) {
            ...
            let replay_wallet_stats_has_persistently_open_frontier =
                open_frontier_floor_pages >= replay_wallet_stats_catch_up_page_limit
            ...
            if let Some(remaining_publishable_horizon_ms) = now
                .filter(|_| replay_wallet_stats_has_persistently_open_frontier)
                .and_then(|now| {
                ...
                })
            {
                ...
                if remaining_publishable_horizon_budget > deep_time_budget {
                    ...
                    deep_reason = Some("deep_replay_wallet_stats_publishable_horizon_backlog");
                }
            }
            contract = PersistedStreamPriorityRecoveryContract {
    ...
    fn collect_buy_mints_fresh_scan_batch_size(fetch_limit: usize) -> usize {
        fetch_limit
            .max(1)
    ...
    fn collect_buy_mints_repair_phase_page_limit(
        &self,
        fetch_limit: usize,
        fetch_page_limit: usize,
        repair_time_budget: StdDuration,
                ...
                < inferred_budget_floor_wallets
        {
            ...
            info!(
                ...
                rebuild_replay_subphase =
                    Self::replay_subphase(state.phase, state.payload.replay_wallet_stats_complete),
    ...
    fn advance_persisted_stream_rebuild_with_phase_page_limits(
        ...
        let rebuild_time_budget = adjusted_contract.time_budget;
            ...
            info!(
                ...
                rebuild_replay_wallet_stats_publishable_horizon_budget_floor_applied =
                    adjusted_contract.reason
                        == Some("deep_replay_wallet_stats_publishable_horizon_backlog"),
                rebuild_replay_wallet_stats_target_ms_per_page =
...
mod tests {
    ...
    fn persisted_stream_replay_bucket_roll_without_immediate_catch_up_ages_out_of_publishable_gate_stage1(
        ...
        assert_ne!(
            ...
            DiscoveryPersistedRebuildPhase::Replay,
            "without the prompt catch-up cycle that stale replay requested, the frozen replay target should age out of the publication gate and move onto a current-bucket rebuild path instead of pretending it can still publish on the stale replay target"
        );
        assert_eq!(
            aged_out_state.metrics_window_start,
            metrics_window_start_for_test(&config, aged_out_now),
            "once the stale replay target is no longer publishable under the freshness gate, the rebuild should truthfully resume on the current metrics bucket"
        );
        assert!(
            aged_out_state.phase != DiscoveryPersistedRebuildPhase::CollectBuyMints
                || aged_out_state.payload.collect_buy_mints_mode != CollectBuyMintsMode::FreshScan,
            "aged-out replay checkpoints must carry forward exact buy-mint membership onto target-window reconcile instead of discarding to collect_buy_mints fresh-scan"
        );
        assert!(
            aged_out_state.prepass_rows_processed > 0,
            "aged-out replay checkpoints must preserve or reuse accumulated exact buy-mint progress instead of restarting the target-window rebuild from zero"
        );
        Ok(())
    }
    ...
    fn persisted_stream_replay_bucket_roll_same_bucket_catch_up_advances_replay_toward_publish_stage1(
        ...
        replay_state.phase_cursor = Some(DiscoveryRuntimeCursor {
            ts_utc: window_start + Duration::minutes(10),
            slot: 42,
            signature: "stage1-same-bucket-replay-sol-leg-cursor".to_string(),
        });
        replay_state.replay_rows_processed = 111;
        replay_state.replay_pages_processed = 7;
        replay_state.payload.by_wallet.insert(
            "wallet_partial".to_string(),
            WalletAccumulator {
                trades: 11,
                ..WalletAccumulator::default()
        ...
        assert!(
            matches!(
                first_summary.runtime_mode,
                DiscoveryRuntimeMode::FailClosed | DiscoveryRuntimeMode::Healthy
            ),
            "same-bucket immediate catch-up should either keep advancing the stale replay target or finish the tiny fixture to healthy, but must not regress to an unrelated runtime mode"
        );
        if first_summary.runtime_mode == DiscoveryRuntimeMode::Healthy {
            assert!(
                store.load_discovery_persisted_rebuild_state()?.is_none(),
                "once same-bucket immediate catch-up reaches a publishable checkpoint, the rebuild checkpoint should clear instead of hanging around in a stale partial state"
            );
            return Ok(());
        }
        assert!(!first_summary.published);
    ...
    #[test]
    fn persisted_stream_priority_recovery_contract_scales_to_publishable_horizon_for_persistently_open_live_wallet_stats_frontier_stage1(
    ) {
        let now = DateTime::parse_from_rfc3339("2026-04-03T15:54:52Z")
            .expect("valid timestamp")
            .with_timezone(&Utc);
        let mut config = stage1_runtime_config();
        config.metric_snapshot_interval_seconds = 3_600;
        ...
        state.phase = DiscoveryPersistedRebuildPhase::Replay;
        state.horizon_end = now - Duration::seconds(27);
        state.payload.replay_mode = ReplayMode::WalletStatsThenSolLeg;
        state.payload.unique_buy_mints =
            vec!["TokenReplayDeep11111111111111111111111111".to_string()];
        state.payload.buy_mint_counts = state
            ...
            .collect();
        state.payload.token_quality_progress.next_mint_index = state.payload.unique_buy_mints.len();
        state.payload.replay_wallet_stats_rows_processed = 3_302_227;
        state.payload.replay_wallet_stats_pages_processed = 214;
        state.payload.replay_wallet_stats_wallet_cursor =
            Some("2bvSuYg272Mowh5QqsvGs5drDwgMNetq3X8njmjaidQ8".to_string());
        state
            .payload
            .replay_wallet_stats_day_count_source_progress
            .fast_path_wallets_processed = 189_900;
        state.payload.replay_wallet_stats_budget_floor_wallets = 189_900;
        state
            .payload
            .replay_wallet_stats_last_partial_cycle_pages_processed = 168;
        state
            .payload
            .replay_wallet_stats_last_partial_cycle_wallets_processed = 151_200;
        state
            .payload
            .replay_wallet_stats_last_partial_cycle_elapsed_ms = 297_004;
        ...
        let metrics_window_gate_remaining_ms = (state.metrics_window_start
            ...
            .num_milliseconds()
            .max(0) as u64;
        assert_eq!(horizon_gate_remaining_ms, 3_573_000);
        assert_eq!(metrics_window_gate_remaining_ms, 3_908_000);
        assert!(
            metrics_window_gate_remaining_ms > horizon_gate_remaining_ms,
            "for the exact fresh live restart checkpoint, the stale metrics-window gate still lasts longer than the horizon gate, so the effective publishable lifetime is bounded by horizon_end rather than by the bucket start"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            StdDuration::from_millis(1_289_250),
            "the pure backlog/open-frontier heuristic still reproduces the exact fresh live restart checkpoint budget from the measured logs"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            StdDuration::from_millis(2_700_000),
            "once the same persistently saturated wallet-stats frontier is resumed on a still-publishable target window, the owning run_cycle should be allowed to spend the remaining safe checkpoint lifetime instead of yielding partial again under the smaller backlog-only contract"
        );
        assert_eq!(
        ...
        assert!(
            ...
            "the publishable-horizon floor should materially widen the resumed wallet-stats replay ceiling beyond the smaller open-frontier-only budget"
        );
    }
