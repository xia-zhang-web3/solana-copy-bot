...
fn execution_defaults_are_fail_closed_and_canary_dry_run_only() {
    ...
    assert!(!execution.entry_quote_shadow_diagnostic_enabled);
    assert_eq!(execution.entry_quote_shadow_diagnostic_batch_limit, 5);
    assert_eq!(
        execution.entry_quote_shadow_diagnostic_max_signal_age_seconds,
        300
    );
    assert!(!execution.exit_policy_shadow_quote_enabled);
    assert_eq!(execution.exit_policy_shadow_quote_hold_minutes, 30);
    assert_eq!(execution.exit_policy_shadow_quote_batch_limit, 5);
    assert!(!execution.market_exit_shadow_quote_enabled);
    assert_eq!(execution.market_exit_shadow_quote_batch_limit, 5);
    assert_eq!(execution.market_exit_shadow_quote_max_close_age_seconds, 300);
