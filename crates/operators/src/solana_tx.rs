...
impl SolanaAccountMeta {
    ...
    pub(crate) fn signer(pubkey: PubkeyBytes) -> Self {
        Self {
            pubkey,
            is_signer: true,
            is_writable: false,
...
fn compile_legacy_message(
    ...
    if readonly_signed > u8::MAX as usize || readonly_unsigned > u8::MAX as usize {
        anyhow::bail!("legacy transaction header account count overflow");
    }
    ...
    let mut message = vec![
        ...
        readonly_signed as u8,
        readonly_unsigned as u8,
    ];
