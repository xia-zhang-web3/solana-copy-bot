    ...
    let runtime_handle = thread::spawn(move || -> Result<bool> {
        ...
        runtime.block_on(async move {
            ...
            timeout(Duration::from_millis(50), sleep(Duration::from_millis(10)))
                .await
                .context("current-thread runtime stalled while observed swap writer was blocked")?;
    ...
    let runtime_handle = thread::spawn(move || -> Result<()> {
        ...
        runtime.block_on(async move {
            ...
            timeout(Duration::from_millis(50), sleep(Duration::from_millis(10)))
                .await
                .context(
                    "current-thread runtime stalled while raw writer retried retryable lock",
                )?;
            ...
            .await
            .context(
                "retryable raw lock batch should eventually drain without terminal writer failure",
            )??;
            writer.ensure_running()?;
            writer.shutdown()?;
            Ok::<(), anyhow::Error>(())
