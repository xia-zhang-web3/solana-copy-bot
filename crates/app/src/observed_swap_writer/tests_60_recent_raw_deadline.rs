...
#[test]
fn recent_raw_journal_partial_deadline_exhaustion_retries_unprocessed_suffix_stage1() -> Result<()>
{
...
#[test]
fn recent_raw_journal_hot_writer_prunes_rows_older_than_retention_horizon() -> Result<()> {
    let unique = format!(
    ...
    assert_eq!(
        journal_rows.len(),
        1,
        "hot writer should prune stale recent_raw rows from the live write path"
    );
    assert!(journal_rows
        .iter()
...
fn recent_raw_journal_full_overflow_coalesces_without_blocking_raw_persistence_stage1() -> Result<()>
{
    ...
    let write_result = (1..42usize).try_for_each(|idx| -> Result<()> {
        ...
        match runtime
            .block_on(async { timeout(StdDuration::from_millis(250), writer.write(&swap)).await })
        {
            Ok(Ok(inserted)) => {
