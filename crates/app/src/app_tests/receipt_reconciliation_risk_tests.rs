    ...
    for side in ["buy", "sell"] {
        ...
        *rpc.status.lock().unwrap() = json!({"result":{"value":[null]}});
        ...
        f.store.record_execution_canary_open_position(
            ...
            "OtherMint",
            2.0,
            Some(copybot_core_types::TokenQuantity::new(2000, 3)),
            0.2,
            now,
        )?;
        let other = add_order(&f.store, "other-sell", "sell", "OtherMint", now, false)?;
        rpc.set(json!({"result":"only-other-sell-submit"}));
        assert_eq!(try_submit(&f, &rpc, &other).await?.submitted, 1);
        assert_eq!(
            rpc.calls
                .lock()
                .unwrap()
...
async fn try_submit(
    ...
    let envelope = if blocked {
        ...
        crate::app_tests::priority_fee_fixture::envelope(&f.store, &request, f.now)?
    };
...
impl ExecutionSubmitAdapter for Ready {
    ...
    fn plan_submit(&self, request: &ExecutionSubmitRequest) -> Result<ExecutionSubmitPlan> {
        Ok(ExecutionSubmitPlan::SubmitReady(ExecutionSubmitIntent {
            ...
            signed_transaction_base64: crate::app_tests::priority_fee_fixture::transaction(
                [7; 32], 200_000, 10_000,
            ),
            tx_signature_hint: None,
        }))
