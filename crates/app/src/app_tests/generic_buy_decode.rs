...
pub(super) fn verify(payload: &str, response: &Value, payer: &str, reserve: u64) -> usize {
    ...
    privileges.insert("11111111111111111111111111111111".into(), (false, false));
    for ix in &expected {
        privileges
    ...
    assert_eq!(wire.short(), expected.len() + 1);
    for ix in expected {
    ...
    let program = wire.byte() as usize;
    assert_eq!(keys[program], "11111111111111111111111111111111");
    assert_eq!(wire.short(), 2);
    assert_eq!(wire.take(2), &[0, 0]);
    assert_eq!(wire.short(), 12);
    assert_eq!(wire.take(4), 2u32.to_le_bytes());
    assert_eq!(wire.take(8), reserve.to_le_bytes());
