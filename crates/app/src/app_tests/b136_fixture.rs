...
pub(super) fn inputs() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/app_tests/fixtures/b136")
}
impl Fixture {
    ...
    pub async fn with_protected(protected: bool) -> Result<Self> {
        let root = super::temporary_output_fixture::OutputRoot::new("b136")?;
        ...
        if protected {
            ...
            db.store
                .activate_tiny_experiment("b136", wallet, Utc::now())?;
        }
        ...
        db.sql.execute("INSERT INTO execution_tiny_reservations(order_id,experiment_id,tx_signature,wallet,side,message_sha256,transaction_sha256,buy_lamports,fee_bound,priority_fee,fee_slot,reserved_at) SELECT order_id,'b136',tx_signature,?1,'buy',?2,?3,10000000,100000,10000,'120','2026-09-09T00:00:00Z' FROM orders WHERE order_id=?4",rusqlite::params![wallet,"a".repeat(64),"b".repeat(64),buy])?;
    ...
    pub fn config(&self, url: &str) -> Result<AppConfig> {
        ...
        c.execution = serde_json::from_value(json!({
            ...
            "tiny_experiment":{"id":"b136","activate":false},
            "owned_sell_preparation":{"policy":"rpc_finalized_cross_slot_owned_sell_v1","rpc_url":url,"genesis_hash":"11111111111111111111111111111111","identity":"b136-loopback"}
        }))?;
        copybot_config::validate_association_delivery(&c)?;
        Ok(c)
    }
    pub async fn ingress(&self, c: &AppConfig) -> Result<()> {
        let (mut consumer, tx) = f::start(&self.db, c, "b136-actual").await?;
        ...
        let (mut reopened, tx) = f::start(&self.db, c, "b136-recovery").await?;
