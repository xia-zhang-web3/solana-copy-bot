...
pub fn meta() -> Result<Value> {
    Ok(serde_json::from_slice(&std::fs::read(
        std::path::Path::new(&std::env::var("B90_FIXTURE_DIR")?).join("chain.json"),
    )?)?)
}
pub fn update(name: &str, ns: u64) -> copybot_ingestion::ReplayInput {
    copybot_ingestion::ReplayInput::Update {
        offset_ns: ns,
        payload: std::fs::read(
            std::path::Path::new(&std::env::var("B90_FIXTURE_DIR").unwrap())
                .join(format!("{name}.pb")),
        )
        .unwrap(),
    }
...
pub fn seed(db: &Db, m: &Value) -> Result<()> {
    ...
    let at = "2026-09-09T00:00:00Z".parse()?;
...
pub fn snapshot(db: &Db) -> Result<Vec<String>> {
    let mut result = vec![];
    for table in [
        "copy_signals",
        "orders",
