use super::generic_sell_fixture::INSTRUCTIONS;
...
#[tokio::test]
async fn batch116_malformed_bundle_refusals_are_terminal() -> Result<()> {
    ...
    let mut v = original.clone();
    v["computeBudgetInstructions"]
        .as_array_mut()
        .unwrap()
        .push(original["computeBudgetInstructions"][0].clone());
    cases.push(("duplicate-CU".into(), v));
    for field in ["error", "simulationError"] {
        let mut v = original.clone();
        v[field] = json!("unrelated provider failure");
        cases.push((format!("provider-{field}"), v));
    }
    let mut v = original.clone();
    let data = STANDARD.decode(v["swapInstruction"]["data"].as_str().unwrap())?;
    v["swapInstruction"]["data"] = json!(STANDARD.encode([data, vec![0; 150]].concat()));
    cases.push(("oversized-packet".into(), v));
...
#[tokio::test]
async fn batch116_simulation_refusal_timeout_and_provider_success_are_terminal() -> Result<()> {
    ...
    for (i, replies) in cases.into_iter().enumerate() {
        let r = run(replies, |c| c.quote_canary_timeout_ms = 500, |_| {}).await?;
        ...
        super::generic_sell_decode::verify(
            ...
            super::generic_sell_fixture::PAYER,
        );
    }
    Ok(())
}
