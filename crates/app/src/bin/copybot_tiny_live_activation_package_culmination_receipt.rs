...
use copybot_app::tiny_live_activation::package_completion_certificate_culmination_receipt;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_culmination_receipt --completion-certificate-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-culmination-receipt | --render-live-package-culmination-receipt | --run-live-package-culmination-receipt | --verify-live-package-culmination-receipt)";
const CULMINATION_RECEIPT_SESSION_VERSION: &str = "1";
const CULMINATION_RECEIPT_STATUS_VERSION: &str = "1";
const CULMINATION_RECEIPT_ALGORITHM: &str = "sha256";
const CULMINATION_RECEIPT_SESSION_EXPLICIT_STATEMENT: &str =
...
const CULMINATION_RECEIPT_STATUS_EXPLICIT_STATEMENT: &str =
    "this culmination receipt is archival and read-only; it never executes or enables the frozen controller";
const CULMINATION_RECEIPT_VERIFY_EXPLICIT_STATEMENT: &str =
...
enum Mode {
    PlanLivePackageCulminationReceipt,
    RenderLivePackageCulminationReceipt,
    RunLivePackageCulminationReceipt,
    VerifyLivePackageCulminationReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageCulminationReceiptVerdict {
    TinyLivePackageCulminationReceiptPlanReady,
    TinyLivePackageCulminationReceiptRendered,
    TinyLivePackageCulminationReceiptRefusedNowByStage3,
    TinyLivePackageCulminationReceiptRefusedNowByPreActivationGate,
    TinyLivePackageCulminationReceiptRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageCulminationReceiptReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageCulminationReceiptVerifyOk,
    TinyLivePackageCulminationReceiptVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageCulminationReceiptResult {
    RefusedNowByStage3,
...
struct PackageCulminationReceiptStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageCulminationReceiptSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageCulminationReceiptStatus {
    status_version: String,
    updated_at: DateTime<Utc>,
    session_dir: String,
    completion_certificate_session_dir: String,
    result: LivePackageCulminationReceiptResult,
    reason: String,
    ...
    completion_certificate_step: Option<PackageCulminationReceiptStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageCulminationReceiptPaths {
...
struct PackageCulminationReceiptReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageCulminationReceiptVerdict,
    reason: String,
    completion_certificate_session_dir: String,
...
struct CulminationReceiptPayload<'a> {
    result: &'a str,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut completion_certificate_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--completion-certificate-session-dir" => {
                completion_certificate_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--completion-certificate-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-culmination-receipt" => set_mode(
                &mut mode,
                Mode::PlanLivePackageCulminationReceipt,
                arg.as_str(),
            )?,
            "--render-live-package-culmination-receipt" => set_mode(
                &mut mode,
                Mode::RenderLivePackageCulminationReceipt,
                arg.as_str(),
            )?,
            "--run-live-package-culmination-receipt" => set_mode(
                &mut mode,
                Mode::RunLivePackageCulminationReceipt,
                arg.as_str(),
            )?,
            "--verify-live-package-culmination-receipt" => set_mode(
                &mut mode,
                Mode::VerifyLivePackageCulminationReceipt,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackageCulminationReceipt) && output_path.is_none() {
        bail!("missing --output for --render-live-package-culmination-receipt");
    }
    if matches!(
        mode,
        Mode::RunLivePackageCulminationReceipt | Mode::VerifyLivePackageCulminationReceipt
    ) && session_dir.is_none()
    {
    ...
    let completion_certificate_session_dir = completion_certificate_session_dir
        .ok_or_else(|| anyhow!("missing --completion-certificate-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageCulminationReceipt => {
            plan_live_package_culmination_receipt_report(&config)?
        }
        Mode::RenderLivePackageCulminationReceipt => {
            render_live_package_culmination_receipt_report(&config)?
        }
        Mode::RunLivePackageCulminationReceipt => {
            run_live_package_culmination_receipt_report(&config)?
        }
        Mode::VerifyLivePackageCulminationReceipt => {
            verify_live_package_culmination_receipt_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_culmination_receipt_report(
    config: &Config,
) -> Result<PackageCulminationReceiptReport> {
    let (consummation_record, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_completion_certificate_culmination_receipt::verify_live_package_completion_certificate_for_culmination_receipt(
                    ...
                    Err(_) => {
                        let raw_contract = package_completion_certificate_culmination_receipt::load_live_package_completion_certificate_contract_for_culmination_receipt(
                            &config.completion_certificate_session_dir,
                        )?;
    ...
    let culmination_receipt_sha256 = compute_culmination_receipt_sha256(
        &result,
        ...
        &consummation_record.completion_certificate_summary,
        &consummation_record.completion_certificate_sha256,
        &consummation_record.completion_certificate_algorithm,
    )?;

    Ok(PackageCulminationReceiptReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_culmination_receipt".to_string(),
        verdict:
            TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptPlanReady,
        reason: format!(
            "completion-certificate-native culmination receipt is explicit for verified completion-certificate session {}; this command stays archival and read-only while freezing one final apex identity over the fully completed chain",
            config.completion_certificate_session_dir.display()
        ),
        ...
        culmination_receipt_session_path: None,
        culmination_receipt_status_path: None,
        archived_completion_certificate_report_path: None,
        result: Some(result),
        ...
        completion_certificate_summary: Some(
            consummation_record.completion_certificate_summary.clone(),
        ),
        completion_certificate_sha256: Some(
            consummation_record.completion_certificate_sha256.clone(),
        ),
        completion_certificate_algorithm: Some(
            consummation_record.completion_certificate_algorithm.clone(),
        ),
        culmination_receipt_summary: Some(culmination_receipt_summary(
            classification.0,
            &consummation_record.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        culmination_receipt_sha256: Some(culmination_receipt_sha256),
        culmination_receipt_algorithm: Some(CULMINATION_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: CULMINATION_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_culmination_receipt_report(
    config: &Config,
) -> Result<PackageCulminationReceiptReport> {
    let plan = plan_live_package_culmination_receipt_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_culmination_receipt --completion-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-culmination-receipt --json\ncopybot_tiny_live_activation_package_culmination_receipt --completion-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-culmination-receipt --json\n",
        shell_escape_path(&config.completion_certificate_session_dir),
    ...
    Ok(PackageCulminationReceiptReport {
        generated_at: Utc::now(),
        mode: "render_live_package_culmination_receipt".to_string(),
        verdict:
            TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptRendered,
        reason: format!(
            "rendered culmination-receipt run/verify script to {}; the primary input remains the immutable completion-certificate session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: CULMINATION_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_culmination_receipt_report(
    config: &Config,
) -> Result<PackageCulminationReceiptReport> {
    ...
    let verified_consummation_record = match package_completion_certificate_culmination_receipt::verify_live_package_completion_certificate_for_culmination_receipt(
        &config.completion_certificate_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify completion-certificate session under {}: {error}",
                    config.completion_certificate_session_dir.display()
                ),
            ))
        }
    ...
    .with_context(|| {
        format!(
            "culmination-receipt session dir {} overlaps the managed live target surface derived from verified completion-certificate install_root {}",
            session_dir.display(),
            verified_consummation_record.contract.install_root.display()
        )
    })?;
    ensure_clean_culmination_receipt_session_dir(session_dir)?;
    let paths = culmination_receipt_paths(session_dir);
    ...
    let completion_certificate_step = Some(step_artifact(
        &paths.completion_certificate_report_path,
        "verify_live_package_completion_certificate",
        &verified_consummation_record.verdict,
        &verified_consummation_record.reason,
        verified_consummation_record.generated_at,
    ));
    let status = PackageCulminationReceiptStatus {
        status_version: CULMINATION_RECEIPT_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        current_pre_activation_gate_reason: verified_consummation_record
            ...
            .current_pre_activation_gate_reason
            .clone(),
        completion_certificate_step: completion_certificate_step.clone(),
        ...
        completion_certificate_algorithm: verified_consummation_record
            ...
            .completion_certificate_algorithm
            .clone(),
        culmination_receipt_summary: culmination_receipt_summary(
            result,
            &verified_consummation_record
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        culmination_receipt_sha256: culmination_receipt_sha256.clone(),
        culmination_receipt_algorithm: CULMINATION_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: CULMINATION_RECEIPT_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_culmination_receipt_report(
    config: &Config,
) -> Result<PackageCulminationReceiptReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = culmination_receipt_paths(session_dir);
    let session: PackageCulminationReceiptSession = load_json(&paths.session_path)?;
    let status: PackageCulminationReceiptStatus = load_json(&paths.status_path)?;
    ...
    compare_string(
        ...
        "culmination-receipt session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "culmination-receipt status session_dir",
        &mut mismatches,
    );
    compare_string(
    ...
    let stored_completion_certificate_report: serde_json::Value = load_required_step_json(
    ...
    let verified_completion_certificate_report_view: NestedFinalityReceiptReportView =
        serde_json::from_value(verified_consummation_record.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested completion-certificate report for {}",
                    config.completion_certificate_session_dir.display()
                )
            },
        )?;
    compare_json_ignoring_generated_at(
        &stored_completion_certificate_report,
        &verified_consummation_record.report_json,
        "culmination-receipt archived completion-certificate report json",
        &mut mismatches,
    );
    compare_string(
        &stored_completion_certificate_report_view.mode,
        &verified_completion_certificate_report_view.mode,
        "culmination-receipt archived completion-certificate report mode",
        &mut mismatches,
    );
    compare_string(
        &stored_completion_certificate_report_view.verdict,
        &verified_completion_certificate_report_view.verdict,
        "culmination-receipt archived completion-certificate report verdict",
        &mut mismatches,
    );
    compare_string(
        &stored_completion_certificate_report_view.reason,
        &verified_completion_certificate_report_view.reason,
        "culmination-receipt archived completion-certificate report reason",
        &mut mismatches,
    );
    if stored_completion_certificate_report_view.generated_at
        != verified_completion_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "culmination-receipt archived completion-certificate report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_completion_certificate_report_view.generated_at,
            verified_completion_certificate_report_view.generated_at
        ));
    }
    let expected_completion_certificate_step = step_artifact(
        &paths.completion_certificate_report_path,
        &verified_completion_certificate_report_view.mode,
        &verified_completion_certificate_report_view.verdict,
        &verified_completion_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_completion_certificate_step,
    ...
    let classification = classify_verified_consummation_record(&verified_consummation_record);
    let expected_culmination_receipt_sha256 = compute_culmination_receipt_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_consummation_record.contract,
        classification.0,
        &expected_culmination_receipt_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "completion-certificate session contract does not match freshly verified completion-certificate truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_consummation_record.contract,
        classification,
        expected_completion_certificate_step.clone(),
        &expected_culmination_receipt_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "completion-certificate status contract does not match freshly verified completion-certificate truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_consummation_record.contract,
        TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptVerifyOk,
        &expected_status,
    ))
}
...
fn classify_consummation_contract(
    contract: &package_completion_certificate_culmination_receipt::LivePackageCompletionCertificateContractView,
    assume_verified: bool,
) -> (
    LivePackageCulminationReceiptResult,
    TinyLivePackageCulminationReceiptVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageCulminationReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptRefusedNowByInvalidOrDriftedContract,
            "the completion-certificate chain is not verified, so this culmination receipt remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageCulminationReceiptResult::RefusedNowByStage3,
            TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptRefusedNowByStage3,
            "the immutable completion certificate remains refused now by Stage 3, so this culmination receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageCulminationReceiptResult::RefusedNowByPreActivationGate,
            TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptRefusedNowByPreActivationGate,
            "the immutable completion certificate remains refused now by the pre-activation gate, so this culmination receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageCulminationReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable completion certificate is coherent and this culmination receipt is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageCulminationReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptRefusedNowByInvalidOrDriftedContract,
            "the verified completion-certificate chain is invalid, drifted, or non-runnable, so this culmination receipt remains refused"
                .to_string(),
        ),
    }
}
...
fn classify_verified_consummation_record(
    verified: &package_completion_certificate_culmination_receipt::VerifiedLivePackageCompletionCertificateCulminationReceiptStep,
) -> (
    LivePackageCulminationReceiptResult,
    TinyLivePackageCulminationReceiptVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_completion_certificate_verify_ok" {
        return (
            LivePackageCulminationReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptRefusedNowByInvalidOrDriftedContract,
            "the immutable completion certificate did not verify cleanly, so this culmination receipt remains refused"
                .to_string(),
        );
    }
...
fn culmination_receipt_summary(
    result: LivePackageCulminationReceiptResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageCulminationReceiptResult::RefusedNowByStage3 => {
            "Culmination receipt outcome: the immutable completed chain remains refused now by Stage 3; archive this final apex seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageCulminationReceiptResult::RefusedNowByPreActivationGate => {
            "Culmination receipt outcome: the immutable completed chain remains refused now by the pre-activation gate; archive this final apex seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageCulminationReceiptResult::RefusedNowByInvalidOrDriftedContract => {
            "Culmination receipt outcome: the immutable completed chain is invalid or drifted; archive this final apex seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackageCulminationReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Culmination receipt outcome: this immutable apex seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn culmination_receipt_paths(session_dir: &Path) -> PackageCulminationReceiptPaths {
    PackageCulminationReceiptPaths {
...
fn ensure_clean_culmination_receipt_session_dir(session_dir: &Path) -> Result<()> {
...
) -> PackageCulminationReceiptSession {
    PackageCulminationReceiptSession {
        session_version: CULMINATION_RECEIPT_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        completion_certificate_session_dir: config
            .completion_certificate_session_dir
            .display()
        ...
        culmination_receipt_sha256: culmination_receipt_sha256.to_string(),
        culmination_receipt_algorithm: CULMINATION_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: CULMINATION_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    completion_certificate_step: PackageCulminationReceiptStepArtifact,
    culmination_receipt_sha256: &str,
) -> PackageCulminationReceiptStatus {
    PackageCulminationReceiptStatus {
        status_version: CULMINATION_RECEIPT_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        completion_certificate_step: Some(completion_certificate_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageCulminationReceiptPaths,
    contract: &package_completion_certificate_culmination_receipt::LivePackageCompletionCertificateContractView,
    verdict: TinyLivePackageCulminationReceiptVerdict,
    status: &PackageCulminationReceiptStatus,
) -> PackageCulminationReceiptReport {
    PackageCulminationReceiptReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptVerifyOk
        {
            "verify_live_package_culmination_receipt".to_string()
        } else {
            "run_live_package_culmination_receipt".to_string()
        },
        verdict,
        ...
        explicit_statement: if verdict
            == TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptVerifyOk
        {
            CULMINATION_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageCulminationReceiptReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageCulminationReceipt => {
                "plan_live_package_culmination_receipt".to_string()
            }
            Mode::RenderLivePackageCulminationReceipt => {
                "render_live_package_culmination_receipt".to_string()
            }
            Mode::RunLivePackageCulminationReceipt => {
                "run_live_package_culmination_receipt".to_string()
            }
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable culmination receipt refused before persisting because the completion-certificate artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "completion-certificate-native culmination-receipt verification failed".to_string()
    });
    PackageCulminationReceiptReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_culmination_receipt".to_string(),
        verdict:
        ...
        explicit_statement: CULMINATION_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageCulminationReceiptStepArtifact>,
    expected_path: &Path,
    label: &str,
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageCulminationReceiptStepArtifact {
    PackageCulminationReceiptStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageCulminationReceiptStepArtifact>,
    expected: &PackageCulminationReceiptStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_culmination_receipt_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&CulminationReceiptPayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing culmination-receipt script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackageCulminationReceiptReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "completion_certificate_session_dir={}",
            report.completion_certificate_session_dir
        ),
    ];
    ...
    if let Some(value) = &report.culmination_receipt_sha256 {
        lines.push(format!("culmination_receipt_sha256={value}"));
    }
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_culmination_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageCulminationReceiptVerdict::TinyLivePackageCulminationReceiptReadyForManualExecutionWhenGateTurnsGreen
        );
    ...
    fn drifted_finality_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            ...
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_completion_certificate",
                "verdict": "tiny_live_package_completion_certificate_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        let overlap_session_dir = managed_paths
            .runtime_dir
            .join("culmination-receipt-session");
        ...
        let error = run_live_package_culmination_receipt_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = culmination_receipt_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_completion_certificate_step_path() {
        let fixture = fake_command_fixture(
        ...
        let mut status: PackageCulminationReceiptStatus = load_json(&paths.status_path).unwrap();
        status.completion_certificate_step.as_mut().unwrap().path =
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = culmination_receipt_paths(&fixture.culmination_receipt_session_dir);
        let mut status: PackageCulminationReceiptStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageCulminationReceiptResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    fn verify_rejects_tampered_nested_finality_receipt_report_content() {
        ...
        let mut report: serde_json::Value =
            load_json(&paths.completion_certificate_report_path).unwrap();
    ...
    fn verify_rejects_tampered_chain_ledger_registry_filing_archive_closure_finality_or_consummation_identity_fields(
    ) {
        {
            ...
            run_live_package_culmination_receipt_report(&fixture.run_config()).unwrap();
            let paths = culmination_receipt_paths(&fixture.culmination_receipt_session_dir);
            let mut status: PackageCulminationReceiptStatus =
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageCulminationReceipt,
                completion_certificate_session_dir: self.completion_certificate_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.culmination_receipt_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageCulminationReceipt,
                completion_certificate_session_dir: self.completion_certificate_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        .unwrap();
        persist_json(
            &completion_certificate_session_dir.join(
        ...
        bind_completion_certificate_report_to_fixture(
            &mut verify_report,
            &finality_receipt_session_dir,
            &archive_receipt_session_dir,
            &registry_entry_session_dir,
        ...
        let verify_path = fixture_dir.join("completion-certificate.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_completion_certificate_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_completion_certificate"),
            &verify_path,
            &consummation_record_session_dir,
            &decision_packet_session_dir,
            &completion_certificate_session_dir,
    ...
    fn bind_completion_certificate_report_to_fixture(
        ...
        object.insert(
            "consummation_record_session_dir".to_string(),
            json!(consummation_record_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            ...
            json!("completion-certificate verify statement"),
        );
    }

    fn completion_certificate_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_completion_certificate",
            "verdict": "tiny_live_package_completion_certificate_verify_ok",
            "reason": "completion-certificate chain is coherent",
            "result": result,
    ...
    fn consummation_record_verify_report(
        ...
        completion_certificate_verify_report(
            result,
            handoff_bundle_result,
            decision_packet_result,
            execute_frozen_result,
    ...
    fn write_fake_completion_certificate_verify_command(
        path: &Path,
        verify_path: &Path,
        expected_consummation_record_session_dir: &Path,
        ...
        let script = format!(
            ...
            shell_escape_path(expected_consummation_record_session_dir),
    ...
    fn stored_consummation_record_session_fixture(
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("completion-certificate statement"),
        );
        Value::Object(object)
    }
    ...
    fn stored_consummation_record_status_fixture(verify_report: &Value) -> Value {
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("completion-certificate status statement"),
        );
        Value::Object(object)
    }
}
