...
use copybot_app::tiny_live_activation::package_nave_receipt_transept_certificate;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_transept_certificate --nave-receipt-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-transept-certificate | --render-live-package-transept-certificate | --run-live-package-transept-certificate | --verify-live-package-transept-certificate)";
const TRANSEPT_CERTIFICATE_SESSION_VERSION: &str = "1";
const TRANSEPT_CERTIFICATE_STATUS_VERSION: &str = "1";
const TRANSEPT_CERTIFICATE_ALGORITHM: &str = "sha256";
const TRANSEPT_CERTIFICATE_SESSION_EXPLICIT_STATEMENT: &str =
...
const TRANSEPT_CERTIFICATE_STATUS_EXPLICIT_STATEMENT: &str =
    "this transept certificate is archival and read-only; it never executes or enables the frozen controller";
const TRANSEPT_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    nave_receipt_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackageTranseptCertificate,
    RenderLivePackageTranseptCertificate,
    RunLivePackageTranseptCertificate,
    VerifyLivePackageTranseptCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageTranseptCertificateVerdict {
    TinyLivePackageTranseptCertificatePlanReady,
    TinyLivePackageTranseptCertificateRendered,
    TinyLivePackageTranseptCertificateRefusedNowByStage3,
    TinyLivePackageTranseptCertificateRefusedNowByPreActivationGate,
    TinyLivePackageTranseptCertificateRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageTranseptCertificateReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageTranseptCertificateVerifyOk,
    TinyLivePackageTranseptCertificateVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageTranseptCertificateResult {
    RefusedNowByStage3,
...
struct PackageTranseptCertificateStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageTranseptCertificateSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageTranseptCertificateStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackageTranseptCertificateResult,
    reason: String,
    ...
    nave_receipt_step: Option<PackageTranseptCertificateStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageTranseptCertificatePaths {
...
struct PackageTranseptCertificateReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageTranseptCertificateVerdict,
    reason: String,
    nave_receipt_session_dir: String,
...
struct TranseptCertificatePayload<'a> {
    result: &'a str,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut nave_receipt_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--nave-receipt-session-dir" => {
                nave_receipt_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--nave-receipt-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-transept-certificate" => set_mode(
                &mut mode,
                Mode::PlanLivePackageTranseptCertificate,
                arg.as_str(),
            )?,
            "--render-live-package-transept-certificate" => set_mode(
                &mut mode,
                Mode::RenderLivePackageTranseptCertificate,
                arg.as_str(),
            )?,
            "--run-live-package-transept-certificate" => set_mode(
                &mut mode,
                Mode::RunLivePackageTranseptCertificate,
                arg.as_str(),
            )?,
            "--verify-live-package-transept-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLivePackageTranseptCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackageTranseptCertificate) && output_path.is_none() {
        bail!("missing --output for --render-live-package-transept-certificate");
    }
    if matches!(
        mode,
        Mode::RunLivePackageTranseptCertificate | Mode::VerifyLivePackageTranseptCertificate
    ) && session_dir.is_none()
    {
    ...
    let nave_receipt_session_dir =
        nave_receipt_session_dir.ok_or_else(|| anyhow!("missing --nave-receipt-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageTranseptCertificate => {
            plan_live_package_transept_certificate_report(&config)?
        }
        Mode::RenderLivePackageTranseptCertificate => {
            render_live_package_transept_certificate_report(&config)?
        }
        Mode::RunLivePackageTranseptCertificate => {
            run_live_package_transept_certificate_report(&config)?
        }
        Mode::VerifyLivePackageTranseptCertificate => {
            verify_live_package_transept_certificate_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_transept_certificate_report(
    config: &Config,
) -> Result<PackageTranseptCertificateReport> {
    let (nave_receipt_contract, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_nave_receipt_transept_certificate::verify_live_package_nave_receipt_for_transept_certificate(
                    &config.nave_receipt_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_nave_receipt(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_nave_receipt_transept_certificate::load_live_package_nave_receipt_contract_for_transept_certificate(
                            &config.nave_receipt_session_dir,
                        )?;
                        let classification =
                            classify_nave_receipt_contract(&raw_contract, false);
    ...
    let session_dir = config
        .session_dir
        .clone()
        .unwrap_or_else(|| config.nave_receipt_session_dir.join("transept-certificate"));
    let result = serialize_enum(&classification.0);
    let transept_certificate_sha256 = compute_transept_certificate_sha256(
        &result,
    ...
    Ok(PackageTranseptCertificateReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_transept_certificate".to_string(),
        verdict:
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificatePlanReady,
        reason: format!(
            "nave-receipt-native transept certificate is explicit for verified nave-receipt session {}; this command stays archival and read-only while freezing one final guidon-seal identity over the fully culminated chain",
            config.nave_receipt_session_dir.display()
        ),
        nave_receipt_session_dir: config.nave_receipt_session_dir.display().to_string(),
        ...
        nave_receipt_session_path: None,
        nave_receipt_status_path: None,
        archived_nave_receipt_report_path: None,
        result: Some(result),
        ...
        transept_certificate_summary: Some(transept_certificate_summary(
            classification.0,
            &nave_receipt_contract.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        transept_certificate_sha256: Some(transept_certificate_sha256),
        transept_certificate_algorithm: Some(TRANSEPT_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: TRANSEPT_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_transept_certificate_report(
    config: &Config,
) -> Result<PackageTranseptCertificateReport> {
    let plan = plan_live_package_transept_certificate_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_transept_certificate --nave-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-transept-certificate --json\ncopybot_tiny_live_activation_package_transept_certificate --nave-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-transept-certificate --json\n",
        shell_escape_path(&config.nave_receipt_session_dir),
    ...
    Ok(PackageTranseptCertificateReport {
        generated_at: Utc::now(),
        mode: "render_live_package_transept_certificate".to_string(),
        verdict:
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateRendered,
        reason: format!(
            "rendered transept-certificate run/verify script to {}; the primary input remains the immutable nave-receipt session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: TRANSEPT_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_transept_certificate_report(
    config: &Config,
) -> Result<PackageTranseptCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let verified_lectern_certificate = match package_nave_receipt_transept_certificate::verify_live_package_nave_receipt_for_transept_certificate(
        &config.nave_receipt_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify nave-receipt session under {}: {error}",
                    config.nave_receipt_session_dir.display()
                ),
            ))
        }
    ...
    .with_context(|| {
        format!(
            "transept-certificate session dir {} overlaps the managed live target surface derived from verified nave-receipt install_root {}",
            session_dir.display(),
            verified_lectern_certificate.contract.install_root.display()
        )
    })?;
    ensure_clean_transept_certificate_session_dir(session_dir)?;
    let paths = transept_certificate_paths(session_dir);
    let classification = classify_verified_nave_receipt(&verified_lectern_certificate);
    ...
    let nave_receipt_step = Some(step_artifact(
        &paths.nave_receipt_report_path,
        "verify_live_package_nave_receipt",
        &verified_lectern_certificate.verdict,
        &verified_lectern_certificate.reason,
        verified_lectern_certificate.generated_at,
    ));
    let status = PackageTranseptCertificateStatus {
        status_version: TRANSEPT_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nave_receipt_algorithm: verified_lectern_certificate
            ...
            .clone(),
        transept_certificate_summary: transept_certificate_summary(
            result,
            &verified_lectern_certificate
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        transept_certificate_sha256: transept_certificate_sha256.clone(),
        transept_certificate_algorithm: TRANSEPT_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: TRANSEPT_CERTIFICATE_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_transept_certificate_report(
    config: &Config,
) -> Result<PackageTranseptCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = transept_certificate_paths(session_dir);
    let session: PackageTranseptCertificateSession = load_json(&paths.session_path)?;
    let status: PackageTranseptCertificateStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.nave_receipt_session_dir,
        &config.nave_receipt_session_dir.display().to_string(),
        "transept-certificate nave_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "transept-certificate session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "transept-certificate status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nave_receipt_session_dir,
        &config.nave_receipt_session_dir.display().to_string(),
        "transept-certificate status nave_receipt_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "transept-certificate substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "transept-certificate status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "transept-certificate bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "transept-certificate status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "transept-certificate foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "transept-certificate status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(stored_lectern_certificate_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested nave-receipt report {}",
                paths.nave_receipt_report_path.display()
            )
        })?;
    let verified_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(verified_lectern_certificate.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested nave-receipt report for {}",
                    config.nave_receipt_session_dir.display()
                )
            },
        )?;
        ...
        != verified_lectern_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "transept-certificate archived nave-receipt report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_lectern_certificate_report_view.generated_at,
            verified_lectern_certificate_report_view.generated_at
        ));
    }
    let expected_nave_receipt_step = step_artifact(
        &paths.nave_receipt_report_path,
        &verified_lectern_certificate_report_view.mode,
        &verified_lectern_certificate_report_view.verdict,
        &verified_lectern_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_nave_receipt_step,
        "transept-certificate nave_receipt_step",
        &mut mismatches,
    );
    if verified_lectern_certificate.verdict != "tiny_live_package_nave_receipt_verify_ok" {
        mismatches.push(format!(
            "transept-certificate nested nave-receipt verification is non-green: {}",
            verified_lectern_certificate.reason
        ));
    }

    let classification = classify_verified_nave_receipt(&verified_lectern_certificate);
    let expected_transept_certificate_sha256 = compute_transept_certificate_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_lectern_certificate.contract,
        classification.0,
        &expected_transept_certificate_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "transept-certificate session contract does not match freshly verified nave-receipt truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_lectern_certificate.contract,
        classification,
        expected_nave_receipt_step.clone(),
        &expected_transept_certificate_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "transept-certificate status contract does not match freshly verified nave-receipt truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_lectern_certificate.contract,
        TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateVerifyOk,
        &expected_status,
    ))
}

fn verify_nave_receipt_command_summary(nave_receipt_session_dir: &Path) -> String {
    format!(
...
fn classify_nave_receipt_contract(
    contract: &package_nave_receipt_transept_certificate::LivePackageNaveReceiptContractView,
    assume_verified: bool,
) -> (
    LivePackageTranseptCertificateResult,
    TinyLivePackageTranseptCertificateVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageTranseptCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateRefusedNowByInvalidOrDriftedContract,
            "the nave-receipt chain is not verified, so this transept certificate remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageTranseptCertificateResult::RefusedNowByStage3,
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateRefusedNowByStage3,
            "the immutable nave receipt remains refused now by Stage 3, so this transept certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageTranseptCertificateResult::RefusedNowByPreActivationGate,
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateRefusedNowByPreActivationGate,
            "the immutable nave receipt remains refused now by the pre-activation gate, so this transept certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageTranseptCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable nave receipt is coherent and this transept certificate is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageTranseptCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateRefusedNowByInvalidOrDriftedContract,
            "the verified nave-receipt chain is invalid, drifted, or non-runnable, so this transept certificate remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_nave_receipt(
    verified: &package_nave_receipt_transept_certificate::VerifiedLivePackageNaveReceiptTranseptCertificateStep,
) -> (
    LivePackageTranseptCertificateResult,
    TinyLivePackageTranseptCertificateVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_nave_receipt_verify_ok" {
        return (
            LivePackageTranseptCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateRefusedNowByInvalidOrDriftedContract,
            "the immutable nave receipt did not verify cleanly, so this transept certificate remains refused"
                .to_string(),
        );
    }
    classify_nave_receipt_contract(&verified.contract, true)
}

fn transept_certificate_summary(
    result: LivePackageTranseptCertificateResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageTranseptCertificateResult::RefusedNowByStage3 => {
            "Transept certificate outcome: the immutable culminated chain remains refused now by Stage 3; archive this final guidon seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageTranseptCertificateResult::RefusedNowByPreActivationGate => {
            "Transept certificate outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final guidon seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageTranseptCertificateResult::RefusedNowByInvalidOrDriftedContract => {
            "Transept certificate outcome: the immutable culminated chain is invalid or drifted; archive this final guidon seal for audit only and mint a new coherent receipt before any later execution handoff.".to_string()
        }
        LivePackageTranseptCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Transept certificate outcome: this immutable guidon seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn transept_certificate_paths(session_dir: &Path) -> PackageTranseptCertificatePaths {
    PackageTranseptCertificatePaths {
        ...
        nave_receipt_report_path: session_dir
            .join("tiny_live_activation_package_transept_certificate.nave_receipt.report.json"),
    }
}

fn ensure_clean_transept_certificate_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing transept-certificate session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating transept-certificate session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageTranseptCertificateSession {
    PackageTranseptCertificateSession {
        session_version: TRANSEPT_CERTIFICATE_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        nave_receipt_session_dir: config.nave_receipt_session_dir.display().to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_nave_receipt_command_summary: verify_nave_receipt_command_summary(
            &config.nave_receipt_session_dir,
        ),
        ...
        transept_certificate_sha256: transept_certificate_sha256.to_string(),
        transept_certificate_algorithm: TRANSEPT_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: TRANSEPT_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    nave_receipt_step: PackageTranseptCertificateStepArtifact,
    transept_certificate_sha256: &str,
) -> PackageTranseptCertificateStatus {
    PackageTranseptCertificateStatus {
        status_version: TRANSEPT_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        nave_receipt_step: Some(nave_receipt_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageTranseptCertificatePaths,
    contract: &package_nave_receipt_transept_certificate::LivePackageNaveReceiptContractView,
    verdict: TinyLivePackageTranseptCertificateVerdict,
    status: &PackageTranseptCertificateStatus,
) -> PackageTranseptCertificateReport {
    PackageTranseptCertificateReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateVerifyOk
        {
            "verify_live_package_transept_certificate".to_string()
        } else {
            "run_live_package_transept_certificate".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        nave_receipt_session_path: Some(paths.session_path.display().to_string()),
        nave_receipt_status_path: Some(paths.status_path.display().to_string()),
        archived_nave_receipt_report_path: Some(
            paths.nave_receipt_report_path.display().to_string(),
        ),
        ...
        nave_receipt_step: status.nave_receipt_step.clone(),
        verify_nave_receipt_command_summary: Some(verify_nave_receipt_command_summary(
            &config.nave_receipt_session_dir,
        )),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
        ...
        explicit_statement: if verdict
            == TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateVerifyOk
        {
            TRANSEPT_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageTranseptCertificateReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageTranseptCertificate => {
                "plan_live_package_transept_certificate".to_string()
            }
            Mode::RenderLivePackageTranseptCertificate => {
                "render_live_package_transept_certificate".to_string()
            }
            Mode::RunLivePackageTranseptCertificate => {
                "run_live_package_transept_certificate".to_string()
            }
        ...
        verdict,
        reason,
        nave_receipt_session_dir: config
            .nave_receipt_session_dir
            .display()
            .to_string(),
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable transept certificate refused before persisting because the nave-receipt artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "nave-receipt-native transept-certificate verification failed".to_string()
    });
    PackageTranseptCertificateReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_transept_certificate".to_string(),
        verdict: TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateVerifyInvalid,
        reason,
        nave_receipt_session_dir: config
            .nave_receipt_session_dir
            .display()
        ...
        verify_nave_receipt_command_summary: Some(
            verify_nave_receipt_command_summary(&config.nave_receipt_session_dir),
        ),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
            contract
                .reviewed_frozen_live_cutover_controller_command_summary
        ...
        explicit_statement: TRANSEPT_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageTranseptCertificateStepArtifact>,
    expected_path: &Path,
    label: &str,
    mismatches: &mut Vec<String>,
) -> Result<serde_json::Value> {
    let Some(step) = step else {
        mismatches.push(format!("{label} is missing from nave-receipt status"));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageTranseptCertificateStepArtifact {
    PackageTranseptCertificateStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageTranseptCertificateStepArtifact>,
    expected: &PackageTranseptCertificateStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_transept_certificate_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&TranseptCertificatePayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing transept-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackageTranseptCertificateReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "nave_receipt_session_dir={}",
            report.nave_receipt_session_dir
        ),
    ];
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_transept_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify =
            verify_live_package_transept_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateVerifyOk
        );
    ...
    #[test]
    fn drifted_nave_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            "nave_receipt_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_nave_receipt",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        let fixture = fake_command_fixture(
            "transept_certificate_managed_overlap",
        ...
        let overlap_session_dir = managed_paths
            .runtime_dir
            .join("transept-certificate-session");
        ...
        let error = run_live_package_transept_certificate_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = transept_certificate_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_nave_receipt_step_path() {
        let fixture = fake_command_fixture(
            "transept_certificate_tampered_nave_receipt_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_transept_certificate_report(&fixture.run_config()).unwrap();
        let paths = transept_certificate_paths(&fixture.nave_receipt_session_dir);
        let mut status: PackageTranseptCertificateStatus = load_json(&paths.status_path).unwrap();
        status.nave_receipt_step.as_mut().unwrap().path =
        ...
        assert_eq!(
            verify.verdict,
            TinyLivePackageTranseptCertificateVerdict::TinyLivePackageTranseptCertificateVerifyInvalid
        );
    }
    ...
    fn verify_rejects_tampered_rostrum_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_rostrum_certificate_sha256",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            run_live_package_transept_certificate_report(&fixture.run_config()).unwrap();
            let paths = transept_certificate_paths(&fixture.nave_receipt_session_dir);
            let mut session: PackageTranseptCertificateSession =
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_rostrum_certificate_summary",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_rostrum_certificate_algorithm",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = transept_certificate_paths(&fixture.nave_receipt_session_dir);
        let mut status: PackageTranseptCertificateStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageTranseptCertificateResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_nave_receipt_report_content() {
        let fixture = fake_command_fixture(
            "transept_certificate_tampered_nested_nave_receipt_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        run_live_package_transept_certificate_report(&fixture.run_config()).unwrap();
        let paths = transept_certificate_paths(&fixture.nave_receipt_session_dir);
        let mut report: serde_json::Value = load_json(&paths.nave_receipt_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested nave-receipt reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_nave_receipt_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
        ...
        let mut status: PackageTranseptCertificateStatus = load_json(&paths.status_path).unwrap();
        status.nave_receipt_step.as_mut().unwrap().generated_at = forged_generated_at;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    fn verify_rejects_tampered_chain_ledger_registry_filing_archive_summit_closure_finality_or_consummation_identity_fields(
    ) {
        {
            ...
            run_live_package_transept_certificate_report(&fixture.run_config()).unwrap();
            let paths = transept_certificate_paths(&fixture.nave_receipt_session_dir);
            let mut status: PackageTranseptCertificateStatus =
    ...
    fn verify_rejects_tampered_bedrock_capstone_keystone_cornerstone_or_foundation_identity_fields()
    {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_bedrock_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_capstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_keystone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_cornerstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_foundation_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_basal_receipt_sha256() {
        let fixture = fake_command_fixture(
            "transept_certificate_tampered_basal_receipt_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_sha256() {
        let fixture = fake_command_fixture(
            "transept_certificate_tampered_substructure_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_substructure_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_substructure_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_plinth_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_plinth_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_plinth_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_plinth_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pedestal_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_pedestal_certificate_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_pedestal_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_pedestal_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_podium_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_podium_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_podium_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_podium_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_lectern_certificate_sha256() {
        let fixture = fake_command_fixture(
            "transept_certificate_tampered_lectern_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_lectern_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_lectern_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_lectern_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_bedrock_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_bedrock_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pulpit_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_pulpit_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_pulpit_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_pulpit_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_apse_receipt_sha256() {
        let fixture = fake_command_fixture(
            "transept_certificate_tampered_apse_receipt_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_chancel_certificate_sha256() {
        let fixture = fake_command_fixture(
            "transept_certificate_tampered_chancel_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_chancel_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_chancel_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_chancel_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_apse_receipt_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_apse_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_apse_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_sanctuary_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_sanctuary_certificate_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageTranseptCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.sanctuary_certificate_sha256 =
                "tampered-sanctuary-certificate-sha256".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_transept_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_sanctuary_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageTranseptCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.sanctuary_certificate_summary =
                "tampered sanctuary certificate summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_transept_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_sanctuary_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageTranseptCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.sanctuary_certificate_algorithm =
                "tampered-sanctuary-certificate-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_transept_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
    ...
    fn verify_rejects_tampered_nave_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_nave_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_nave_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_nave_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_transept_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_transept_certificate_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_transept_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "transept_certificate_tampered_transept_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        nave_receipt_session_dir: PathBuf,
        input_nave_receipt_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageTranseptCertificate,
                nave_receipt_session_dir: self.input_nave_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.nave_receipt_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageTranseptCertificate,
                nave_receipt_session_dir: self.input_nave_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let nave_receipt_session_dir = fixture_dir.join("transept-certificate-session");
        let input_nave_receipt_session_dir = fixture_dir.join("nave-receipt-session");
        ...
        for path in [
            &input_nave_receipt_session_dir,
        ...
        persist_json(
            ...
            &stored_nave_receipt_session_fixture(
                &lectern_certificate_session_dir,
                &apse_receipt_session_dir,
                &rostrum_certificate_session_dir,
                &pedestal_certificate_session_dir,
        ...
        persist_json(
            ...
            &stored_nave_receipt_status_fixture(
                &lectern_certificate_session_dir,
                &apse_receipt_session_dir,
                &rostrum_certificate_session_dir,
                &pedestal_certificate_session_dir,
        ...
        .unwrap();
        persist_json(
            &input_nave_receipt_session_dir.join(
                "tiny_live_activation_package_nave_receipt.sanctuary_certificate.report.json",
            ),
            &json!({
        ...
        bind_nave_receipt_report_to_fixture(
            &mut verify_report,
            &lectern_certificate_session_dir,
            &apse_receipt_session_dir,
            &rostrum_certificate_session_dir,
        ...
        let verify_path = fixture_dir.join("nave-receipt.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_nave_receipt_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_nave_receipt"),
            &verify_path,
            &apse_receipt_session_dir,
            &decision_packet_session_dir,
            &input_nave_receipt_session_dir,
    ...
    fn bind_nave_receipt_report_to_fixture(
        ...
        object.insert(
            "verify_sanctuary_certificate_command_summary".to_string(),
            json!(format!(
                "verify immutable nave-receipt session under {} before freezing the final transept certificate",
        ...
        object.insert(
            ...
            json!("nave-receipt verify statement"),
        );
    }

    fn nave_receipt_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_nave_receipt",
            "verdict": "tiny_live_package_nave_receipt_verify_ok",
            "reason": "nave-receipt chain is coherent",
            "result": result,
    ...
    fn stored_nave_receipt_session_fixture(
        lectern_certificate_session_dir: &Path,
        apse_receipt_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("nave-receipt statement"),
        );
        Value::Object(object)
    }

    fn stored_nave_receipt_status_fixture(
        lectern_certificate_session_dir: &Path,
        apse_receipt_session_dir: &Path,
        _rostrum_certificate_session_dir: &Path,
        pedestal_certificate_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("nave-receipt status statement"),
        );
        Value::Object(object)
    }
}
