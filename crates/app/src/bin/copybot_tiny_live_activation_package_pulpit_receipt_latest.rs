...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_pulpit_receipt_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-pulpit-receipt | --render-latest-pulpit-receipt-script | --run-latest-pulpit-receipt | --verify-latest-pulpit-receipt)";
const PULPIT_RECEIPT_LATEST_SESSION_VERSION: &str = "1";
const PULPIT_RECEIPT_LATEST_STATUS_VERSION: &str = "1";
...
const PULPIT_RECEIPT_BIN: &str = "copybot_tiny_live_activation_package_pulpit_receipt";
const LECTERN_CERTIFICATE_LATEST_PLAN_READY: &str =
    "tiny_live_package_lectern_certificate_latest_plan_ready";
const LECTERN_CERTIFICATE_LATEST_VERIFY_OK: &str =
    "tiny_live_package_lectern_certificate_latest_verify_ok";
const PULPIT_RECEIPT_VERIFY_OK: &str = "tiny_live_package_pulpit_receipt_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-pulpit-receipt handoff binds the latest persisted immutable package chain to the accepted pulpit-receipt contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestPulpitReceipt,
    RenderLatestPulpitReceiptScript,
    RunLatestPulpitReceipt,
    VerifyLatestPulpitReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackagePulpitReceiptLatestVerdict {
    TinyLivePackagePulpitReceiptLatestPlanReady,
    TinyLivePackagePulpitReceiptLatestScriptRendered,
    TinyLivePackagePulpitReceiptLatestRefusedByMissingLatestChain,
    TinyLivePackagePulpitReceiptLatestRefusedByInvalidLatestChain,
    TinyLivePackagePulpitReceiptLatestRefusedByDriftedPulpitReceiptContract,
    TinyLivePackagePulpitReceiptLatestRefusedNowByStage3,
    TinyLivePackagePulpitReceiptLatestRefusedNowByPreActivationGate,
    TinyLivePackagePulpitReceiptLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackagePulpitReceiptLatestVerifyOk,
    TinyLivePackagePulpitReceiptLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackagePulpitReceiptLatestResult {
    RefusedByMissingLatestChain,
    RefusedByInvalidLatestChain,
    RefusedByDriftedPulpitReceiptContract,
    RefusedNowByStage3,
...
struct PackagePulpitReceiptLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackagePulpitReceiptLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_lectern_certificate_latest_session_dir: String,
    nested_pulpit_receipt_session_dir: String,
    ...
    resolve_latest_lectern_certificate_command_summary: String,
    run_latest_pulpit_receipt_command_summary: String,
    verify_latest_pulpit_receipt_command_summary: String,
    downstream_pulpit_receipt_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackagePulpitReceiptLatestStatus {
    ...
    nested_pulpit_receipt_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackagePulpitReceiptLatestResult,
    reason: String,
    latest_lectern_certificate_plan_verdict: Option<String>,
    latest_lectern_certificate_verdict: Option<String>,
    pulpit_receipt_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_lectern_certificate_plan_step: Option<PackagePulpitReceiptLatestStepArtifact>,
    latest_lectern_certificate_step: Option<PackagePulpitReceiptLatestStepArtifact>,
    pulpit_receipt_step: Option<PackagePulpitReceiptLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackagePulpitReceiptLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_lectern_certificate_plan_report_path: PathBuf,
    latest_lectern_certificate_report_path: PathBuf,
    pulpit_receipt_report_path: PathBuf,
    nested_lectern_certificate_latest_session_dir: PathBuf,
    nested_pulpit_receipt_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackagePulpitReceiptLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackagePulpitReceiptLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    pulpit_receipt_latest_session_path: Option<String>,
    pulpit_receipt_latest_status_path: Option<String>,
    pulpit_receipt_latest_report_path: Option<String>,
    latest_lectern_certificate_plan_report_path: Option<String>,
    latest_lectern_certificate_report_path: Option<String>,
    pulpit_receipt_report_path: Option<String>,
    nested_lectern_certificate_latest_session_dir: Option<String>,
    nested_pulpit_receipt_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_lectern_certificate_command_summary: Option<String>,
    run_latest_pulpit_receipt_command_summary: Option<String>,
    verify_latest_pulpit_receipt_command_summary: Option<String>,
    downstream_pulpit_receipt_command_summary: Option<String>,
...
#[derive(Debug, Clone)]
struct LatestLecternCertificateSnapshot {
...
#[derive(Debug, Clone)]
struct ResolvedLatestLecternCertificate {
    snapshot: LatestLecternCertificateSnapshot,
    latest_lectern_certificate_plan_raw: Value,
    latest_lectern_certificate_plan: LecternCertificateLatestReportView,
}
...
enum ResolutionFailureKind {
    MissingLatestChain,
    InvalidLatestChain,
    DriftedLecternCertificateContract,
}

#[derive(Debug, Clone)]
struct LatestLecternCertificateResolutionFailure {
    kind: ResolutionFailureKind,
    reason: String,
    snapshot: Option<LatestLecternCertificateSnapshot>,
    latest_lectern_certificate_plan_raw: Option<Value>,
    latest_lectern_certificate_plan_verdict: Option<String>,
    latest_lectern_certificate_plan_reason: Option<String>,
    latest_lectern_certificate_plan_generated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone, Deserialize)]
struct LecternCertificateLatestReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Deserialize)]
struct PulpitReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    ...
    service_status_max_staleness_ms: Option<u64>,
    session_dir: Option<String>,
    pulpit_receipt_session_path: Option<String>,
    pulpit_receipt_status_path: Option<String>,
    archived_lectern_certificate_report_path: Option<String>,
    result: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    lectern_certificate_step: Option<PackagePulpitReceiptLatestStepArtifact>,
...
#[derive(Debug, Clone, Deserialize)]
struct PulpitReceiptSessionView {
    lectern_certificate_session_dir: String,
...
#[derive(Debug, Clone, Deserialize)]
struct PulpitReceiptStatusView {
    session_dir: String,
    lectern_certificate_session_dir: String,
    pedestal_certificate_session_dir: String,
    plinth_receipt_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-pulpit-receipt" => {
                set_mode(&mut mode, Mode::PlanLatestPulpitReceipt, arg.as_str())?
            }
            "--render-latest-pulpit-receipt-script" => set_mode(
                &mut mode,
                Mode::RenderLatestPulpitReceiptScript,
                arg.as_str(),
            )?,
            "--run-latest-pulpit-receipt" => {
                set_mode(&mut mode, Mode::RunLatestPulpitReceipt, arg.as_str())?
            }
            "--verify-latest-pulpit-receipt" => {
                set_mode(&mut mode, Mode::VerifyLatestPulpitReceipt, arg.as_str())?
            }
    ...
    if matches!(
        mode,
        Mode::PlanLatestPulpitReceipt
            | Mode::RenderLatestPulpitReceiptScript
            | Mode::RunLatestPulpitReceipt
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestPulpitReceiptScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestPulpitReceipt | Mode::VerifyLatestPulpitReceipt
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestPulpitReceipt => plan_latest_pulpit_receipt_report(&config)?,
        Mode::RenderLatestPulpitReceiptScript => {
            render_latest_pulpit_receipt_script_report(&config)?
        }
        Mode::RunLatestPulpitReceipt => run_latest_pulpit_receipt_report(&config)?,
        Mode::VerifyLatestPulpitReceipt => verify_latest_pulpit_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_latest_pulpit_receipt_report(config: &Config) -> Result<PackagePulpitReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
    ...
    let suggested_session_dir = config
        .session_dir
        .clone()
        .unwrap_or_else(|| root.join("tiny_live_activation_package_pulpit_receipt_latest.session"));
    let paths = pulpit_receipt_latest_paths(&suggested_session_dir);

    match resolve_latest_lectern_certificate(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            "plan_latest_pulpit_receipt",
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-lectern-certificate lineage via accepted latest-lectern-certificate session {}, exact decision-packet confirmation anchor {}, and the accepted pulpit-receipt contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_lectern_certificate_latest_session_dir.display(),
...
fn render_latest_pulpit_receipt_script_report(
    config: &Config,
) -> Result<PackagePulpitReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    ...
    match resolve_latest_lectern_certificate(root, &paths) {
        Ok(resolved) => {
            write_script(
                output_path,
                &format!(
                    "#!/usr/bin/env bash\nset -euo pipefail\n\n{run_latest}\n{verify_latest}\n{run_pulpit_receipt}\n{verify_pulpit_receipt}\n",
                    run_latest = run_latest_pulpit_receipt_command_summary(
                        root,
                        &suggested_session_dir,
                    ),
                    verify_latest =
                        verify_latest_pulpit_receipt_command_summary(&suggested_session_dir),
                    run_pulpit_receipt = downstream_pulpit_receipt_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_lectern_certificate_session_dir(&paths),
                        &paths.nested_pulpit_receipt_session_dir,
                    ),
                    verify_pulpit_receipt = verify_pulpit_receipt_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_lectern_certificate_session_dir(&paths),
                        &paths.nested_pulpit_receipt_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                "render_latest_pulpit_receipt_script",
                TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestScriptRendered,
                format!(
                    "rendered latest-pulpit-receipt handoff script to {} using the accepted latest-lectern-certificate lineage and the accepted pulpit-receipt contract",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_pulpit_receipt_report(config: &Config) -> Result<PackagePulpitReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-pulpit-receipt session dir {}",
            session_dir.display()
        );
    }
    let paths = pulpit_receipt_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-pulpit-receipt")?;

    let resolution = resolve_latest_lectern_certificate(root, &paths);
    match resolution {
        Ok(resolved) => {
            run_latest_pulpit_receipt_with_resolution(root, session_dir, &paths, &resolved)
        }
        Err(failure) => run_latest_pulpit_receipt_failure(root, session_dir, &paths, &failure),
    }
}

fn verify_latest_pulpit_receipt_report(
    config: &Config,
) -> Result<PackagePulpitReceiptLatestReport> {
    let session_dir = config
        .session_dir
    ...
    let session: PackagePulpitReceiptLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_pulpit_receipt_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-pulpit-receipt session artifact {}: {error}",
                    paths.session_path.display()
                )],
            ))
        }
    };
    let status: PackagePulpitReceiptLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_pulpit_receipt_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-pulpit-receipt status artifact {}: {error}",
                    paths.status_path.display()
                )],
            ))
        }
    };
    let stored_report: PackagePulpitReceiptLatestReport = match load_json(&paths.report_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_pulpit_receipt_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-pulpit-receipt report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-pulpit-receipt root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-pulpit-receipt session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-pulpit-receipt status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-pulpit-receipt report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_lectern_certificate_latest_session_dir,
        &paths
            .nested_lectern_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-pulpit-receipt session nested_lectern_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_lectern_certificate_latest_session_dir,
        &paths
            .nested_lectern_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-pulpit-receipt status nested_lectern_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-pulpit-receipt report nested_lectern_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_pulpit_receipt_session_dir,
        &paths
            .nested_pulpit_receipt_session_dir
            .display()
            .to_string(),
        "latest-pulpit-receipt session nested_pulpit_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_pulpit_receipt_session_dir,
        &paths
            .nested_pulpit_receipt_session_dir
            .display()
            .to_string(),
        "latest-pulpit-receipt status nested_pulpit_receipt_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.nested_pulpit_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_pulpit_receipt_session_dir
                .display()
        ...
        "latest-pulpit-receipt report nested_pulpit_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-pulpit-receipt session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-pulpit-receipt status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-pulpit-receipt report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.pulpit_receipt_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-pulpit-receipt report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.pulpit_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-pulpit-receipt report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.pulpit_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-pulpit-receipt report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-pulpit-receipt session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-pulpit-receipt status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-pulpit-receipt report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_pulpit_receipt",
        "latest-pulpit-receipt stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-pulpit-receipt stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-pulpit-receipt stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-pulpit-receipt stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-pulpit-receipt stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report
            .latest_lectern_certificate_plan_verdict
            .as_deref(),
        status.latest_lectern_certificate_plan_verdict.as_deref(),
        "stored report latest_lectern_certificate_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.latest_lectern_certificate_verdict.as_deref(),
        status.latest_lectern_certificate_verdict.as_deref(),
        "stored report latest_lectern_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.pulpit_receipt_verdict.as_deref(),
        status.pulpit_receipt_verdict.as_deref(),
        "stored report pulpit_receipt_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: LecternCertificateLatestReportView = load_required_step_json(
                &status.latest_lectern_certificate_plan_step,
                &paths.latest_lectern_certificate_plan_report_path,
                "latest_lectern_certificate_plan_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_lectern_certificate_plan_step.as_ref(),
                &paths.latest_lectern_certificate_plan_report_path,
                "plan_latest_lectern_certificate",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_lectern_certificate_plan_step",
                &mut mismatches,
            );
            compare_lectern_certificate_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_lectern_certificate_latest_session_dir,
                &mut mismatches,
            );
            compare_option_string(
                status.latest_lectern_certificate_plan_verdict.as_deref(),
                Some(stored_latest_plan.verdict.as_str()),
                "stored status latest_lectern_certificate_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let stored_latest_run: LecternCertificateLatestReportView = load_required_step_json(
                &status.latest_lectern_certificate_step,
                &paths.latest_lectern_certificate_report_path,
                "latest_lectern_certificate_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_lectern_certificate_step.as_ref(),
                &paths.latest_lectern_certificate_report_path,
                "run_latest_lectern_certificate",
                &stored_latest_run.verdict,
                &stored_latest_run.reason,
                stored_latest_run.generated_at,
                "stored latest_lectern_certificate_step",
                &mut mismatches,
            );
            compare_lectern_certificate_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_lectern_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_lectern_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_lectern_certificate_verdict.as_deref(),
            ...
            let actual_latest_run: LecternCertificateLatestReportView = match load_json(
                &nested_lectern_certificate_latest_report_path(
                    &paths.nested_lectern_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed loading actual nested latest-lectern-certificate report under {}: {error}",
                        paths.nested_lectern_certificate_latest_session_dir.display()
                    ));
                    fallback_lectern_certificate_latest_copy()
                }
            };
            compare_lectern_certificate_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                LECTERN_CERTIFICATE_LATEST_BIN,
                &latest_lectern_certificate_verify_args(
                    &paths.nested_lectern_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-lectern-certificate session under {}: {error}",
                        paths.nested_lectern_certificate_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: LecternCertificateLatestReportView =
                    serde_json::from_value(latest_verify_raw)
                        .context("failed parsing nested latest-lectern-certificate verify json")?;
                if latest_verify.verdict != LECTERN_CERTIFICATE_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-lectern-certificate verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: PulpitReceiptReportView = load_required_step_json(
                &status.pulpit_receipt_step,
                &paths.pulpit_receipt_report_path,
                "pulpit_receipt_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.pulpit_receipt_step.as_ref(),
                &paths.pulpit_receipt_report_path,
                "run_live_package_pulpit_receipt",
                &stored_handoff_bundle.verdict,
                &stored_handoff_bundle.reason,
                stored_handoff_bundle.generated_at,
                "stored pulpit_receipt_step",
                &mut mismatches,
            );
            compare_pulpit_receipt_run_against_snapshot(
            ...
            compare_option_string(
                ...
                "stored status pulpit_receipt_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.pulpit_receipt_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: PulpitReceiptSessionView = match load_json(
                &nested_pulpit_receipt_session_path(&paths.nested_pulpit_receipt_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed reading nested pulpit-receipt session under {}: {error}",
                        paths.nested_pulpit_receipt_session_dir.display()
                    ));
            ...
            let nested_handoff_bundle_status: PulpitReceiptStatusView = match load_json(
                &nested_pulpit_receipt_status_path(&paths.nested_pulpit_receipt_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed reading nested pulpit-receipt status under {}: {error}",
                        paths.nested_pulpit_receipt_session_dir.display()
                    ));
            ...
            compare_pulpit_receipt_copy_against_nested_truth(
                &stored_handoff_bundle,
            ...
            let handoff_bundle_verify_raw = match run_json_command(
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested pulpit-receipt session under {}: {error}",
                        paths.nested_pulpit_receipt_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: PulpitReceiptReportView =
                    serde_json::from_value(handoff_bundle_verify_raw)
                        .context("failed parsing nested pulpit-receipt verify json")?;
                compare_option_string(
                    stored_handoff_bundle.registry_entry_session_dir.as_deref(),
                    handoff_bundle_verify.registry_entry_session_dir.as_deref(),
                    "stored pulpit-receipt copy registry_entry_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored pulpit-receipt copy notarization_receipt_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .consummation_record_algorithm
                        .as_deref(),
                    "stored pulpit-receipt copy consummation_record_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .completion_certificate_summary
                        .as_deref(),
                    "stored pulpit-receipt copy completion_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .completion_certificate_sha256
                        .as_deref(),
                    "stored pulpit-receipt copy completion_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_summary
                        .as_deref(),
                    "stored pulpit-receipt copy cornerstone_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_sha256
                        .as_deref(),
                    "stored pulpit-receipt copy cornerstone_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_algorithm
                        .as_deref(),
                    "stored pulpit-receipt copy cornerstone_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .capstone_certificate_summary
                        .as_deref(),
                    "stored pulpit-receipt copy capstone_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.capstone_certificate_sha256.as_deref(),
                    handoff_bundle_verify.capstone_certificate_sha256.as_deref(),
                    "stored pulpit-receipt copy capstone_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .substructure_certificate_summary
                        .as_deref(),
                    "stored pulpit-receipt copy substructure_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .substructure_certificate_sha256
                        .as_deref(),
                    "stored pulpit-receipt copy substructure_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .pedestal_certificate_summary
                        .as_deref(),
                    "stored pulpit-receipt copy pedestal_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.pedestal_certificate_sha256.as_deref(),
                    handoff_bundle_verify.pedestal_certificate_sha256.as_deref(),
                    "stored pulpit-receipt copy pedestal_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .pedestal_certificate_algorithm
                        .as_deref(),
                    "stored pulpit-receipt copy pedestal_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.lectern_certificate_summary.as_deref(),
                    handoff_bundle_verify.lectern_certificate_summary.as_deref(),
                    "stored pulpit-receipt copy lectern_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.lectern_certificate_sha256.as_deref(),
                    handoff_bundle_verify.lectern_certificate_sha256.as_deref(),
                    "stored pulpit-receipt copy lectern_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .lectern_certificate_algorithm
                        .as_deref(),
                    "stored pulpit-receipt copy lectern_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .reviewed_frozen_live_cutover_controller_command_summary
                        .as_deref(),
                    "stored pulpit-receipt copy reviewed_frozen_live_cutover_controller_command_summary",
                    &mut mismatches,
                );
                if handoff_bundle_verify.verdict != PULPIT_RECEIPT_VERIFY_OK {
                    mismatches.push(format!(
                        "nested pulpit-receipt verification is non-green: {}",
                        handoff_bundle_verify.reason
                    ));
                }
            }

            let expected_result = lectern_certificate_latest_result_from_nested_status(
                nested_handoff_bundle_status.result.as_str(),
            )
            .unwrap_or(LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract);
            if status.result != expected_result {
                mismatches.push(format!(
                    "stored latest-pulpit-receipt result {} does not match nested pulpit-receipt result {}",
        ...
        Err(failure) => {
            ...
            if failure.latest_lectern_certificate_plan_raw.is_some() {
                let stored_latest_plan: LecternCertificateLatestReportView =
                ...
                compare_option_string(
                    status.latest_lectern_certificate_plan_verdict.as_deref(),
                    failure.latest_lectern_certificate_plan_verdict.as_deref(),
                    "stored status latest_lectern_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report
                        ...
                        .as_deref(),
                    failure.latest_lectern_certificate_plan_verdict.as_deref(),
                    "stored report latest_lectern_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
                status.latest_lectern_certificate_verdict.as_deref(),
                None,
                "stored status latest_lectern_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_lectern_certificate_verdict.as_deref(),
                None,
                "stored report latest_lectern_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.pulpit_receipt_verdict.as_deref(),
                None,
                "stored status pulpit_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.pulpit_receipt_verdict.as_deref(),
                None,
                "stored report pulpit_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestVerifyOk
    } else {
        TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-pulpit-receipt session under {} is coherent with the current latest immutable chain and the accepted pulpit-receipt contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-pulpit-receipt verification failed".to_string())
    };

    Ok(PackagePulpitReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_pulpit_receipt".to_string(),
        verdict,
        reason,
        ...
        pulpit_receipt_latest_session_path: Some(paths.session_path.display().to_string()),
        pulpit_receipt_latest_status_path: Some(paths.status_path.display().to_string()),
        pulpit_receipt_latest_report_path: Some(paths.report_path.display().to_string()),
        latest_lectern_certificate_plan_report_path: Some(
            paths
                .latest_lectern_certificate_plan_report_path
                .display()
                .to_string(),
        ),
        latest_lectern_certificate_report_path: Some(
            paths.latest_lectern_certificate_report_path.display().to_string(),
        ),
        pulpit_receipt_report_path: Some(paths.pulpit_receipt_report_path.display().to_string()),
        nested_lectern_certificate_latest_session_dir: Some(
            paths
                .nested_lectern_certificate_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_pulpit_receipt_session_dir: Some(
            paths.nested_pulpit_receipt_session_dir.display().to_string(),
        ),
        ...
        latest_lectern_certificate_plan_verdict: status.latest_lectern_certificate_plan_verdict.clone(),
        latest_lectern_certificate_verdict: status.latest_lectern_certificate_verdict.clone(),
        pulpit_receipt_verdict: status.pulpit_receipt_verdict.clone(),
        ...
        resolve_latest_lectern_certificate_command_summary: Some(
            session.resolve_latest_lectern_certificate_command_summary.clone(),
        ),
        run_latest_pulpit_receipt_command_summary: Some(
            session.run_latest_pulpit_receipt_command_summary.clone(),
        ),
        verify_latest_pulpit_receipt_command_summary: Some(
            session.verify_latest_pulpit_receipt_command_summary.clone(),
        ),
        downstream_pulpit_receipt_command_summary: Some(
            session.downstream_pulpit_receipt_command_summary.clone(),
        ),
        verify_pulpit_receipt_command_summary: Some(
            session.verify_pulpit_receipt_command_summary.clone(),
        ),
        ...
        explicit_statement:
            "this verification proves only whether the latest immutable chain still resolves cleanly into the accepted pulpit-receipt contract; it never runs the frozen controller"
                .to_string(),
    })
}

fn run_latest_pulpit_receipt_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackagePulpitReceiptLatestPaths,
    resolved: &ResolvedLatestLecternCertificate,
) -> Result<PackagePulpitReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, Some(resolved), None);
    persist_json(&paths.session_path, &session)?;
    persist_json(
        &paths.latest_lectern_certificate_plan_report_path,
        &resolved.latest_lectern_certificate_plan_raw,
    )?;

    let latest_lectern_certificate_plan_step = Some(step_artifact(
        &paths.latest_lectern_certificate_plan_report_path,
        "plan_latest_lectern_certificate",
        &resolved.latest_lectern_certificate_plan.verdict,
        &resolved.latest_lectern_certificate_plan.reason,
        resolved.latest_lectern_certificate_plan.generated_at,
    ));

    let latest_lectern_certificate_run_raw = match run_json_command(
        LECTERN_CERTIFICATE_LATEST_BIN,
        &latest_lectern_certificate_run_args(
            root,
            &paths.nested_lectern_certificate_latest_session_dir,
        ),
    ) {
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                paths,
                Some(resolved),
                LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract,
                format!(
                    "failed running nested latest-lectern-certificate handoff under {}: {error}",
                    paths
                        .nested_lectern_certificate_latest_session_dir
                        .display()
                ),
    ...
    persist_json(
        &paths.latest_lectern_certificate_report_path,
        &latest_lectern_certificate_run_raw,
    )?;
    let latest_lectern_certificate_run: LecternCertificateLatestReportView =
        serde_json::from_value(latest_lectern_certificate_run_raw)
            .context("failed parsing nested latest-lectern-certificate run json")?;
    let latest_lectern_certificate_step = Some(step_artifact(
        &paths.latest_lectern_certificate_report_path,
        "run_latest_lectern_certificate",
        &latest_lectern_certificate_run.verdict,
        &latest_lectern_certificate_run.reason,
        latest_lectern_certificate_run.generated_at,
    ));

    if let Some(terminal_failure) =
        latest_lectern_certificate_terminal_failure(&latest_lectern_certificate_run)
    {
        let status = refusal_status(
            root,
            session_dir,
            ...
            Some(resolved),
            terminal_failure,
            latest_lectern_certificate_run.reason.clone(),
            latest_lectern_certificate_plan_step,
            latest_lectern_certificate_step,
            None,
        );
    ...
    compare_lectern_certificate_latest_run_against_snapshot(
        &latest_lectern_certificate_run,
        &resolved.snapshot,
        &paths.nested_lectern_certificate_latest_session_dir,
        &mut run_mismatches,
    );
    ...
    let latest_lectern_certificate_verify: LecternCertificateLatestReportView =
        match run_json_command(
            ...
            Err(error) => {
                let status = refusal_status(
                    ...
                    LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract,
                    format!(
                    "failed verifying nested latest-lectern-certificate handoff under {}: {error}",
                    paths.nested_lectern_certificate_latest_session_dir.display()
                ),
                    latest_lectern_certificate_plan_step,
                    latest_lectern_certificate_step,
    ...
    if latest_lectern_certificate_verify.verdict != LECTERN_CERTIFICATE_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract,
            latest_lectern_certificate_verify.reason,
            latest_lectern_certificate_plan_step,
            latest_lectern_certificate_step,
            None,
        );
    ...
    let release_capsule_run_raw = match run_json_command(
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract,
                format!(
                    "failed running downstream pulpit-receipt contract under {}: {error}",
                    paths.nested_pulpit_receipt_session_dir.display()
                ),
                latest_lectern_certificate_plan_step,
                latest_lectern_certificate_step,
    ...
    persist_json(&paths.pulpit_receipt_report_path, &release_capsule_run_raw)?;
    let provenance_certificate_run: PulpitReceiptReportView =
        serde_json::from_value(release_capsule_run_raw)
            .context("failed parsing nested pulpit-receipt run json")?;
    let pulpit_receipt_step = Some(step_artifact(
        &paths.pulpit_receipt_report_path,
        "run_live_package_pulpit_receipt",
    ...
    let provenance_certificate_verify: PulpitReceiptReportView = match run_json_command(
        PULPIT_RECEIPT_BIN,
        &pulpit_receipt_verify_args(
            &resolved.snapshot,
            &expected_downstream_lectern_certificate_session_dir(paths),
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract,
                format!(
                    "failed verifying downstream pulpit-receipt contract under {}: {error}",
                    paths.nested_pulpit_receipt_session_dir.display()
                ),
                latest_lectern_certificate_plan_step,
                latest_lectern_certificate_step,
    ...
    if provenance_certificate_verify.verdict != PULPIT_RECEIPT_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    let result = lectern_certificate_latest_result_from_nested_status(
        provenance_certificate_run
            .result
            .as_deref()
            .unwrap_or_default(),
    ...
    .unwrap_or(LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract);
    let status = PackagePulpitReceiptLatestStatus {
        status_version: PULPIT_RECEIPT_LATEST_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nested_lectern_certificate_latest_session_dir: paths
            .nested_lectern_certificate_latest_session_dir
            .display()
            .to_string(),
        nested_pulpit_receipt_session_dir: paths
            .nested_pulpit_receipt_session_dir
            .display()
            .to_string(),
        ...
        latest_lectern_certificate_plan_verdict: Some(
            resolved.latest_lectern_certificate_plan.verdict.clone(),
        ),
        latest_lectern_certificate_verdict: Some(latest_lectern_certificate_run.verdict.clone()),
        pulpit_receipt_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_pulpit_receipt_failure(
    root: &Path,
    session_dir: &Path,
    paths: &PackagePulpitReceiptLatestPaths,
    failure: &LatestLecternCertificateResolutionFailure,
) -> Result<PackagePulpitReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, None, Some(failure));
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_lectern_certificate_plan_raw {
        persist_json(&paths.latest_lectern_certificate_plan_report_path, value)?;
    }
...
fn resolve_latest_lectern_certificate(
    root: &Path,
    paths: &PackagePulpitReceiptLatestPaths,
) -> std::result::Result<ResolvedLatestLecternCertificate, LatestLecternCertificateResolutionFailure>
{
    let latest_lectern_certificate_plan_raw = match run_json_command(
        LECTERN_CERTIFICATE_LATEST_BIN,
        &latest_lectern_certificate_plan_args(
            root,
            &paths.nested_lectern_certificate_latest_session_dir,
        ),
    ) {
        Ok(value) => value,
        Err(error) => {
            return Err(LatestLecternCertificateResolutionFailure {
                ...
                snapshot: None,
                latest_lectern_certificate_plan_raw: None,
                latest_lectern_certificate_plan_verdict: None,
                latest_lectern_certificate_plan_reason: None,
                latest_lectern_certificate_plan_generated_at: None,
            });
        }
    };
    let latest_lectern_certificate_plan: LecternCertificateLatestReportView =
        serde_json::from_value(latest_lectern_certificate_plan_raw.clone()).map_err(
            |error| LatestLecternCertificateResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-lectern-certificate upstream latest-lectern-certificate plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_lectern_certificate_plan_raw: Some(
                    latest_lectern_certificate_plan_raw.clone(),
                ),
                latest_lectern_certificate_plan_verdict: None,
                latest_lectern_certificate_plan_reason: None,
                latest_lectern_certificate_plan_generated_at: None,
    ...
    let snapshot =
        parse_snapshot_from_latest_lectern_certificate(&latest_lectern_certificate_plan).ok();
    if latest_lectern_certificate_plan.verdict != LECTERN_CERTIFICATE_LATEST_PLAN_READY {
        return Err(LatestLecternCertificateResolutionFailure {
            kind: map_lectern_certificate_latest_failure_kind(
                &latest_lectern_certificate_plan.verdict,
            ),
            reason: format!(
                "latest-lectern-certificate resolution refused because nested latest-lectern-certificate reported: {}",
                latest_lectern_certificate_plan.reason
            ),
            snapshot,
            latest_lectern_certificate_plan_raw: Some(latest_lectern_certificate_plan_raw),
            latest_lectern_certificate_plan_verdict: Some(
                latest_lectern_certificate_plan.verdict,
            ),
            latest_lectern_certificate_plan_reason: Some(
                latest_lectern_certificate_plan.reason,
            ),
            latest_lectern_certificate_plan_generated_at: Some(
                latest_lectern_certificate_plan.generated_at,
            ),
        });
    }

    let snapshot = parse_snapshot_from_latest_lectern_certificate(&latest_lectern_certificate_plan)
        .map_err(|error| LatestLecternCertificateResolutionFailure {
            kind: ResolutionFailureKind::InvalidLatestChain,
            reason: format!("latest-lectern-certificate resolution failed: {error}"),
            snapshot: None,
            latest_lectern_certificate_plan_raw: Some(latest_lectern_certificate_plan_raw.clone()),
            latest_lectern_certificate_plan_verdict: Some(
                latest_lectern_certificate_plan.verdict.clone(),
            ),
            latest_lectern_certificate_plan_reason: Some(
                latest_lectern_certificate_plan.reason.clone(),
            ),
            latest_lectern_certificate_plan_generated_at: Some(
                latest_lectern_certificate_plan.generated_at,
            ),
    ...
    let mut mismatches = Vec::new();
    compare_lectern_certificate_latest_plan_against_snapshot(
        &latest_lectern_certificate_plan,
        &snapshot,
        &paths.nested_lectern_certificate_latest_session_dir,
        &mut mismatches,
    );
    if !mismatches.is_empty() {
        return Err(LatestLecternCertificateResolutionFailure {
            kind: ResolutionFailureKind::DriftedLecternCertificateContract,
            reason: format!(
                "latest-lectern-certificate drift detected: {}",
                mismatches[0]
            ),
            snapshot: Some(snapshot),
            latest_lectern_certificate_plan_raw: Some(latest_lectern_certificate_plan_raw),
            latest_lectern_certificate_plan_verdict: Some(latest_lectern_certificate_plan.verdict),
            latest_lectern_certificate_plan_reason: Some(latest_lectern_certificate_plan.reason),
            latest_lectern_certificate_plan_generated_at: Some(
                latest_lectern_certificate_plan.generated_at,
            ),
        });
    ...
    Ok(ResolvedLatestLecternCertificate {
        snapshot,
        latest_lectern_certificate_plan_raw,
        latest_lectern_certificate_plan,
    })
}

fn parse_snapshot_from_latest_lectern_certificate(
    view: &LecternCertificateLatestReportView,
) -> Result<LatestLecternCertificateSnapshot> {
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-lectern-certificate nested latest-lectern-certificate plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-lectern-certificate nested latest-lectern-certificate plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestLecternCertificateSnapshot {
        latest_top_step_name,
        ...
        wrapper_timeout_ms: view.wrapper_timeout_ms.ok_or_else(|| {
            anyhow!(
                "latest-lectern-certificate nested latest-lectern-certificate plan is missing wrapper_timeout_ms"
            )
        })?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!(
                "latest-lectern-certificate nested latest-lectern-certificate plan is missing service_status_max_staleness_ms"
            )
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackagePulpitReceiptLatestPaths,
    resolved: &ResolvedLatestLecternCertificate,
) -> PackagePulpitReceiptLatestReport {
    PackagePulpitReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        latest_lectern_certificate_report_path: Some(
            paths
                .latest_lectern_certificate_report_path
                .display()
                .to_string(),
        ),
        pulpit_receipt_report_path: Some(paths.pulpit_receipt_report_path.display().to_string()),
        ...
        latest_lectern_certificate_verdict: None,
        pulpit_receipt_verdict: None,
        result: None,
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_lectern_certificate_command_summary: Some(
            resolve_latest_lectern_certificate_command_summary(
                root,
                &paths.nested_lectern_certificate_latest_session_dir,
            ),
        ),
        run_latest_pulpit_receipt_command_summary: Some(run_latest_pulpit_receipt_command_summary(
            root,
            session_dir,
        )),
        verify_latest_pulpit_receipt_command_summary: Some(
            verify_latest_pulpit_receipt_command_summary(session_dir),
        ),
        downstream_pulpit_receipt_command_summary: Some(downstream_pulpit_receipt_command_summary(
            &resolved.snapshot,
            &expected_downstream_lectern_certificate_session_dir(paths),
            &paths.nested_pulpit_receipt_session_dir,
        )),
        verify_pulpit_receipt_command_summary: Some(verify_pulpit_receipt_command_summary(
            &resolved.snapshot,
            &expected_downstream_lectern_certificate_session_dir(paths),
            &paths.nested_pulpit_receipt_session_dir,
        )),
...
fn report_from_resolution_failure(
    ...
    PackagePulpitReceiptLatestReport {
        ...
        latest_lectern_certificate_plan_verdict: failure
            .latest_lectern_certificate_plan_verdict
            .clone(),
        latest_lectern_certificate_verdict: None,
        pulpit_receipt_verdict: None,
        ...
        downstream_pulpit_receipt_command_summary: snapshot.map(|value| {
            downstream_pulpit_receipt_command_summary(
                value,
                &expected_downstream_lectern_certificate_session_dir(paths),
                &paths.nested_pulpit_receipt_session_dir,
            )
        }),
        verify_pulpit_receipt_command_summary: snapshot.map(|value| {
...
fn planned_session(
    ...
    session_dir: &Path,
    paths: &PackagePulpitReceiptLatestPaths,
    resolved: Option<&ResolvedLatestLecternCertificate>,
    failure: Option<&LatestLecternCertificateResolutionFailure>,
) -> PackagePulpitReceiptLatestSession {
    let snapshot = resolved
        .map(|value| &value.snapshot)
        .or_else(|| failure.and_then(|value| value.snapshot.as_ref()));
    PackagePulpitReceiptLatestSession {
        session_version: PULPIT_RECEIPT_LATEST_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        root: root.display().to_string(),
        session_dir: session_dir.display().to_string(),
        nested_lectern_certificate_latest_session_dir: paths
        ...
        run_latest_pulpit_receipt_command_summary: run_latest_pulpit_receipt_command_summary(
            root,
            session_dir,
        ),
        verify_latest_pulpit_receipt_command_summary: verify_latest_pulpit_receipt_command_summary(
            session_dir,
        ),
        downstream_pulpit_receipt_command_summary: snapshot
            .map(|value| {
                downstream_pulpit_receipt_command_summary(
                    value,
                    &expected_downstream_lectern_certificate_session_dir(paths),
            ...
            .unwrap_or_else(|| "<pulpit-receipt-run-unavailable>".to_string()),
        verify_pulpit_receipt_command_summary: snapshot
            .map(|value| {
                verify_pulpit_receipt_command_summary(
                    value,
                    &expected_downstream_lectern_certificate_session_dir(paths),
            ...
            .unwrap_or_else(|| "<pulpit-receipt-verify-unavailable>".to_string()),
        reviewed_frozen_live_cutover_controller_command_summary: snapshot.map(|value| {
            value
                .reviewed_frozen_live_cutover_controller_command_summary
                .clone()
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackagePulpitReceiptLatestPaths,
    session: &PackagePulpitReceiptLatestSession,
    status: &PackagePulpitReceiptLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackagePulpitReceiptLatestReport {
    PackagePulpitReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        nested_pulpit_receipt_session_dir: Some(session.nested_pulpit_receipt_session_dir.clone()),
        downstream_decision_packet_session_dir: Some(
            session.downstream_decision_packet_session_dir.clone(),
        ),
        latest_lectern_certificate_plan_verdict: status
            .latest_lectern_certificate_plan_verdict
            .clone(),
        latest_lectern_certificate_verdict: status.latest_lectern_certificate_verdict.clone(),
        pulpit_receipt_verdict: status.pulpit_receipt_verdict.clone(),
...
fn pulpit_receipt_latest_paths(session_dir: &Path) -> PackagePulpitReceiptLatestPaths {
    PackagePulpitReceiptLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_pulpit_receipt_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_pulpit_receipt_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_pulpit_receipt_latest.report.json"),
        latest_lectern_certificate_plan_report_path: session_dir.join(
            "tiny_live_activation_package_pulpit_receipt_latest.lectern_certificate_latest_plan.report.json",
        ),
        latest_lectern_certificate_report_path: session_dir.join(
            "tiny_live_activation_package_pulpit_receipt_latest.lectern_certificate_latest.report.json",
        ),
        pulpit_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_pulpit_receipt_latest.pulpit_receipt.report.json",
        ),
        nested_lectern_certificate_latest_session_dir: session_dir.join(
            "tiny_live_activation_package_pulpit_receipt_latest.lectern_certificate_latest_session",
        ),
        nested_pulpit_receipt_session_dir: session_dir.join(
            "tiny_live_activation_package_pulpit_receipt_latest.pulpit_receipt_session",
        ),
    }
}
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_lectern_certificate_latest_session_dir
        .join("tiny_live_activation_package_lectern_certificate_latest.decision_packet_session")
}

fn expected_downstream_lectern_certificate_session_dir(
    paths: &PackagePulpitReceiptLatestPaths,
) -> PathBuf {
    paths
        .nested_lectern_certificate_latest_session_dir
        .join("tiny_live_activation_package_lectern_certificate_latest.lectern_certificate_session")
}
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_lectern_certificate_latest_session_dir
        .join("tiny_live_activation_package_lectern_certificate_latest.review_receipt_session")
}

fn latest_lectern_certificate_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-lectern-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn latest_lectern_certificate_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-lectern-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn latest_lectern_certificate_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-lectern-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn pulpit_receipt_run_args(
    snapshot: &LatestLecternCertificateSnapshot,
    lectern_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        "--lectern-certificate-session-dir".to_string(),
        lectern_certificate_session_dir.display().to_string(),
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-pulpit-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn pulpit_receipt_verify_args(
    snapshot: &LatestLecternCertificateSnapshot,
    lectern_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-live-package-pulpit-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn resolve_latest_lectern_certificate_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{LECTERN_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --plan-latest-lectern-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn run_latest_pulpit_receipt_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{PULPIT_RECEIPT_LATEST_BIN} --root {} --session-dir {} --run-latest-pulpit-receipt --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn verify_latest_pulpit_receipt_command_summary(session_dir: &Path) -> String {
    format!(
        "{PULPIT_RECEIPT_LATEST_BIN} --session-dir {} --verify-latest-pulpit-receipt --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_pulpit_receipt_command_summary(
    snapshot: &LatestLecternCertificateSnapshot,
    lectern_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{PULPIT_RECEIPT_BIN} --lectern-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-pulpit-receipt --json",
        shell_escape_path(lectern_certificate_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
}

fn verify_pulpit_receipt_command_summary(
    snapshot: &LatestLecternCertificateSnapshot,
    lectern_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{PULPIT_RECEIPT_BIN} --lectern-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-pulpit-receipt --json",
        shell_escape_path(lectern_certificate_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
...
fn nested_lectern_certificate_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_lectern_certificate_latest.report.json")
}

fn nested_pulpit_receipt_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_pulpit_receipt.session.json")
}

fn nested_pulpit_receipt_status_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_pulpit_receipt.status.json")
}
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-pulpit-receipt script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackagePulpitReceiptLatestStepArtifact {
    PackagePulpitReceiptLatestStepArtifact {
...
fn canonical_wrapper_mode(mode: &str) -> &str {
    match mode {
        "plan_latest_pulpit_receipt" => "plan_latest_pulpit_receipt",
        "render_latest_pulpit_receipt_script" => "render_latest_pulpit_receipt_script",
        "run_latest_pulpit_receipt" => "run_latest_pulpit_receipt",
        "verify_latest_pulpit_receipt" => "verify_latest_pulpit_receipt",
        other => other,
    }
}

fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackagePulpitReceiptLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-pulpit-receipt status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-pulpit-receipt status"
        ));
        return;
    };
    compare_string(
...
fn compare_lectern_certificate_latest_plan_against_snapshot(
    stored: &LecternCertificateLatestReportView,
    snapshot: &LatestLecternCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        ...
        Some(LECTERN_CERTIFICATE_LATEST_PLAN_READY),
        "stored latest-lectern-certificate plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_lectern_certificate",
        "stored latest-lectern-certificate plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-lectern-certificate plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-lectern-certificate plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-lectern-certificate plan session_dir",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        ...
        "stored latest-lectern-certificate plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-lectern-certificate plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-lectern-certificate plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-lectern-certificate plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_lectern_certificate_latest_run_against_snapshot(
    stored: &LecternCertificateLatestReportView,
    snapshot: &LatestLecternCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_lectern_certificate",
        "stored latest-lectern-certificate run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-lectern-certificate run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-lectern-certificate run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-lectern-certificate run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-lectern-certificate run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-lectern-certificate run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-lectern-certificate run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-lectern-certificate run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-lectern-certificate run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-lectern-certificate run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-lectern-certificate run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-lectern-certificate run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-lectern-certificate run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-lectern-certificate run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-lectern-certificate run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_lectern_certificate_latest_copy_matches_actual(
    stored: &LecternCertificateLatestReportView,
    actual: &LecternCertificateLatestReportView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        Some(stored.verdict.as_str()),
        Some(actual.verdict.as_str()),
        "stored latest-lectern-certificate copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-lectern-certificate copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-lectern-certificate copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-lectern-certificate copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-lectern-certificate copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-lectern-certificate copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-lectern-certificate copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-lectern-certificate copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-lectern-certificate copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-lectern-certificate copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-lectern-certificate copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-lectern-certificate copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_pulpit_receipt_run_against_snapshot(
    stored: &PulpitReceiptReportView,
    snapshot: &LatestLecternCertificateSnapshot,
    lectern_certificate_session_dir: &Path,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_live_package_pulpit_receipt",
        "stored pulpit-receipt run mode",
        mismatches,
    );
    compare_string(
        &stored.lectern_certificate_session_dir,
        &lectern_certificate_session_dir.display().to_string(),
        "stored pulpit-receipt run lectern_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored pulpit-receipt run decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored pulpit-receipt run execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored pulpit-receipt run turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored pulpit-receipt run launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored pulpit-receipt run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored pulpit-receipt run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored pulpit-receipt run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored pulpit-receipt run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored pulpit-receipt run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored pulpit-receipt run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored pulpit-receipt run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.pulpit_receipt_session_path.as_deref(),
        Some(
            nested_pulpit_receipt_session_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored pulpit-receipt run pulpit_receipt_session_path",
        mismatches,
    );
    compare_option_string(
        stored.pulpit_receipt_status_path.as_deref(),
        Some(
            nested_pulpit_receipt_status_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored pulpit-receipt run pulpit_receipt_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_lectern_certificate_report_path.as_deref(),
        Some(
            session_dir
                .join("tiny_live_activation_package_pulpit_receipt.lectern_certificate.report.json")
                .display()
                .to_string()
                .as_str(),
        ),
        "stored pulpit-receipt run archived_lectern_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        stored.verify_lectern_certificate_command_summary.as_deref(),
        Some(verify_pulpit_receipt_input_command_summary(lectern_certificate_session_dir).as_str()),
        "stored pulpit-receipt run verify_lectern_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
...
fn compare_pulpit_receipt_copy_against_nested_truth(
    ...
    compare_option_string(
        ...
        "stored pulpit-receipt copy verify_lectern_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored pulpit-receipt copy release_capsule_digest_manifest_sha256",
        mismatches,
    );
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_lectern_certificate_command_summary,
        &resolve_latest_lectern_certificate_command_summary(
            root,
            &paths.nested_lectern_certificate_latest_session_dir,
        ),
        "stored session resolve_latest_lectern_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.run_latest_pulpit_receipt_command_summary,
        &run_latest_pulpit_receipt_command_summary(root, session_dir),
        "stored session run_latest_pulpit_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_latest_pulpit_receipt_command_summary,
        &verify_latest_pulpit_receipt_command_summary(session_dir),
        "stored session verify_latest_pulpit_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.downstream_pulpit_receipt_command_summary,
        &downstream_pulpit_receipt_command_summary(
            snapshot,
            &expected_downstream_lectern_certificate_session_dir(paths),
            &paths.nested_pulpit_receipt_session_dir,
        ),
        "stored session downstream_pulpit_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_pulpit_receipt_command_summary,
        &verify_pulpit_receipt_command_summary(
            snapshot,
            &expected_downstream_lectern_certificate_session_dir(paths),
            &paths.nested_pulpit_receipt_session_dir,
        ),
        "stored session verify_pulpit_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_lectern_certificate_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_pulpit_receipt_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_pulpit_receipt_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackagePulpitReceiptLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackagePulpitReceiptLatestPaths,
    snapshot: &LatestLecternCertificateSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_lectern_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report.run_latest_pulpit_receipt_command_summary.as_deref(),
        Some(run_latest_pulpit_receipt_command_summary(root, session_dir).as_str()),
        "stored report run_latest_pulpit_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
            .verify_latest_pulpit_receipt_command_summary
            .as_deref(),
        Some(verify_latest_pulpit_receipt_command_summary(session_dir).as_str()),
        "stored report verify_latest_pulpit_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report.downstream_pulpit_receipt_command_summary.as_deref(),
        Some(
            downstream_pulpit_receipt_command_summary(
                snapshot,
                &expected_downstream_lectern_certificate_session_dir(paths),
        ...
        "stored report downstream_pulpit_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report.verify_pulpit_receipt_command_summary.as_deref(),
        Some(
            verify_pulpit_receipt_command_summary(
                snapshot,
                &expected_downstream_lectern_certificate_session_dir(paths),
        ...
        "stored report verify_pulpit_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
    ...
    compare_option_string(
        report.pulpit_receipt_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "stored report pulpit_receipt_latest_session_path",
        mismatches,
    );
    compare_option_string(
        report.pulpit_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "stored report pulpit_receipt_latest_status_path",
        mismatches,
    );
    compare_option_string(
        report.pulpit_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "stored report pulpit_receipt_latest_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report latest_lectern_certificate_plan_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_lectern_certificate_report_path.as_deref(),
        Some(
            paths
                .latest_lectern_certificate_report_path
                .display()
        ...
        "stored report latest_lectern_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        report.pulpit_receipt_report_path.as_deref(),
        Some(
            paths
                .pulpit_receipt_report_path
                .display()
                .to_string()
                .as_str(),
        ),
        "stored report pulpit_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report nested_lectern_certificate_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_pulpit_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_pulpit_receipt_session_dir
                .display()
        ...
        "stored report nested_pulpit_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_lectern_certificate_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_pulpit_receipt_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_pulpit_receipt_session_dir on failure",
        mismatches,
    );
...
fn map_lectern_certificate_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_lectern_certificate_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_lectern_certificate_latest_refused_by_drifted_lectern_certificate_contract" => {
            ResolutionFailureKind::DriftedLecternCertificateContract
        }
        _ => ResolutionFailureKind::InvalidLatestChain,
    }
}

fn latest_lectern_certificate_terminal_failure(
    report: &LecternCertificateLatestReportView,
) -> Option<LivePackagePulpitReceiptLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackagePulpitReceiptLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackagePulpitReceiptLatestResult::RefusedByInvalidLatestChain)
        }
        Some("refused_by_drifted_lectern_certificate_contract") => {
            Some(LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract)
        }
        _ => None,
    }
}

fn lectern_certificate_latest_result_from_nested_status(
    result: &str,
) -> Option<LivePackagePulpitReceiptLatestResult> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => {
            Some(LivePackagePulpitReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen)
        }
        "refused_now_by_stage3" => Some(LivePackagePulpitReceiptLatestResult::RefusedNowByStage3),
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackagePulpitReceiptLatestResult::RefusedNowByPreActivationGate)
        }
...
fn pulpit_receipt_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        ...
        "refused_now_by_stage3" => Some("tiny_live_package_pulpit_receipt_refused_now_by_stage3"),
        "refused_now_by_pre_activation_gate" => {
            Some("tiny_live_package_pulpit_receipt_refused_now_by_pre_activation_gate")
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_pulpit_receipt_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}

fn result_from_failure_kind(kind: ResolutionFailureKind) -> LivePackagePulpitReceiptLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackagePulpitReceiptLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackagePulpitReceiptLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedLecternCertificateContract => {
            LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract
        }
...
fn verdict_for_result(
    result: LivePackagePulpitReceiptLatestResult,
) -> TinyLivePackagePulpitReceiptLatestVerdict {
    match result {
        LivePackagePulpitReceiptLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestRefusedByMissingLatestChain
        }
        LivePackagePulpitReceiptLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestRefusedByInvalidLatestChain
        }
        LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract => {
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestRefusedByDriftedPulpitReceiptContract
        }
        LivePackagePulpitReceiptLatestResult::RefusedNowByStage3 => {
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestRefusedNowByStage3
        }
        LivePackagePulpitReceiptLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestRefusedNowByPreActivationGate
        }
        LivePackagePulpitReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackagePulpitReceiptLatestResult) -> String {
    match result {
        LivePackagePulpitReceiptLatestResult::RefusedByMissingLatestChain => {
            "no immutable clerestory chain is available yet; do not manually hunt latest-lectern-certificate or pulpit-receipt lineage by hand".to_string()
        }
        LivePackagePulpitReceiptLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-lectern-certificate / pulpit-receipt path".to_string()
        }
        LivePackagePulpitReceiptLatestResult::RefusedByDriftedPulpitReceiptContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-lectern-certificate / pulpit-receipt contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackagePulpitReceiptLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-pulpit-receipt refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackagePulpitReceiptLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-pulpit-receipt refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackagePulpitReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            "the latest immutable chain now resolves into the accepted pulpit-receipt contract and one bounded latest-pulpit-receipt handoff is ready for manual execution when gate truth turns green; this wrapper still did not run or authorize the frozen controller".to_string()
        }
    }
}
...
fn verify_pulpit_receipt_input_command_summary(lectern_certificate_session_dir: &Path) -> String {
    format!(
        "verify immutable lectern-certificate session under {} before freezing the final pulpit receipt",
        lectern_certificate_session_dir.display()
    )
}
...
fn refusal_status(
    ...
    latest_lectern_certificate_step: Option<PackagePulpitReceiptLatestStepArtifact>,
    pulpit_receipt_step: Option<PackagePulpitReceiptLatestStepArtifact>,
) -> PackagePulpitReceiptLatestStatus {
    ...
    PackagePulpitReceiptLatestStatus {
        ...
        result,
        reason,
        latest_lectern_certificate_plan_verdict: resolved
            .map(|value| value.latest_lectern_certificate_plan.verdict.clone()),
        latest_lectern_certificate_verdict: latest_lectern_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        pulpit_receipt_verdict: pulpit_receipt_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackagePulpitReceiptLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_lectern_certificate_plan_step: failure
            .latest_lectern_certificate_plan_raw
            .as_ref()
            .map(|_| {
                step_artifact(
                    ...
                    failure
                        ...
                        .unwrap_or("<missing>"),
                    failure
                        .latest_lectern_certificate_plan_reason
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_lectern_certificate_plan_generated_at
                        .unwrap_or_else(Utc::now),
                )
            }),
        latest_lectern_certificate_step: None,
...
fn verify_invalid_pulpit_receipt_latest_report_without_session(
    session_dir: &Path,
    verification_mismatches: Vec<String>,
) -> PackagePulpitReceiptLatestReport {
    ...
    PackagePulpitReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_pulpit_receipt".to_string(),
        verdict: TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestVerifyInvalid,
        reason,
        root: None,
        ...
        session_dir: Some(session_dir.display().to_string()),
        pulpit_receipt_latest_session_path: Some(
            pulpit_receipt_latest_paths(session_dir)
                .session_path
                .display()
                .to_string(),
        ),
        pulpit_receipt_latest_status_path: Some(
            pulpit_receipt_latest_paths(session_dir)
                .status_path
                .display()
                .to_string(),
        ),
        pulpit_receipt_latest_report_path: Some(
            pulpit_receipt_latest_paths(session_dir)
                .report_path
                .display()
                .to_string(),
        ),
        latest_lectern_certificate_plan_report_path: None,
        latest_lectern_certificate_report_path: None,
        pulpit_receipt_report_path: None,
        nested_lectern_certificate_latest_session_dir: None,
        nested_pulpit_receipt_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_lectern_certificate_command_summary: None,
        run_latest_pulpit_receipt_command_summary: None,
        verify_latest_pulpit_receipt_command_summary: None,
        downstream_pulpit_receipt_command_summary: None,
        verify_pulpit_receipt_command_summary: None,
...
fn fallback_lectern_certificate_latest_copy() -> LecternCertificateLatestReportView {
    LecternCertificateLatestReportView {
        generated_at: Utc::now(),
...
fn fallback_notarization_receipt_session(
    snapshot: &LatestLecternCertificateSnapshot,
    session_dir: &Path,
) -> PulpitReceiptSessionView {
    PulpitReceiptSessionView {
        lectern_certificate_session_dir: "<missing>".to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_lectern_certificate_command_summary:
            verify_pulpit_receipt_input_command_summary(Path::new("<missing>")),
        ...
        explicit_statement:
            "the verified lectern-certificate session is the primary input here; this immutable pulpit receipt freezes one final crest identity over the fully culminated chain without enabling production execution"
                .to_string(),
    }
}

fn fallback_notarization_receipt_status(
    _snapshot: &LatestLecternCertificateSnapshot,
    session_dir: &Path,
) -> PulpitReceiptStatusView {
    PulpitReceiptStatusView {
        session_dir: session_dir.display().to_string(),
        lectern_certificate_session_dir: "<missing>".to_string(),
        pedestal_certificate_session_dir: "<missing>".to_string(),
        ...
        pulpit_receipt_algorithm: "sha256".to_string(),
        explicit_statement:
            "this pulpit receipt is archival and read-only; it never executes or enables the frozen controller"
                .to_string(),
    }
}

fn render_report_lines(report: &PackagePulpitReceiptLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_lectern_certificate_latest_session_dir {
        lines.push(format!(
            "nested_lectern_certificate_latest_session_dir={value}"
        ));
    }
    ...
    if let Some(value) = &report.nested_pulpit_receipt_session_dir {
        lines.push(format!("nested_pulpit_receipt_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_pulpit_receipt_command_summary {
        lines.push(format!("downstream_pulpit_receipt_command_summary={value}"));
    }
...
mod tests {
    ...
    struct FakeLecternCertificateLatestFixture {
        temp_dir: PathBuf,
        root: PathBuf,
        ...
        lectern_certificate_latest_plan_json_path: PathBuf,
        lectern_certificate_latest_run_json_path: PathBuf,
        lectern_certificate_latest_verify_json_path: PathBuf,
    ...
    impl FakeLecternCertificateLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestPulpitReceipt,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestPulpitReceiptScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestPulpitReceipt,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestPulpitReceipt,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    #[test]
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_lectern_certificate_lineage(
    ) {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_plan_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = plan_latest_pulpit_receipt_report(&fixture.plan_config()).unwrap();
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert_eq!(
            report.verdict,
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestPlanReady
        );
        assert_eq!(report.mode, "plan_latest_pulpit_receipt");
        assert_eq!(
            report.latest_top_step_name.as_deref(),
        ...
        assert!(report
            ...
            .as_deref()
            .unwrap()
            .contains(PULPIT_RECEIPT_BIN));
    }

    #[test]
    fn plan_mode_without_session_dir_uses_pulpit_latest_default_wrapper_session_dir() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_latest_plan_default_session_dir",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        ...
        let stale_lectern_wrapper_session_dir = fixture
            .root
            .join("tiny_live_activation_package_lectern_certificate_latest.session");
        let default_paths = pulpit_receipt_latest_paths(&default_wrapper_session_dir);
        let mut plan_json = default_lectern_certificate_latest_plan_report(&fixture);
        ...
        persist_json(
            &fixture.lectern_certificate_latest_plan_json_path,
            &plan_json,
        )
        .unwrap();
        write_fake_lectern_certificate_latest_command(
            &fixture.bin_dir.join(LECTERN_CERTIFICATE_LATEST_BIN),
            &fixture.lectern_certificate_latest_plan_json_path,
            &fixture.lectern_certificate_latest_run_json_path,
            &fixture.lectern_certificate_latest_verify_json_path,
            &fixture.root,
            &default_paths.nested_lectern_certificate_latest_session_dir,
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let report = plan_latest_pulpit_receipt_report(&config).unwrap();
        let default_wrapper_session_dir = default_wrapper_session_dir.display().to_string();
        let stale_lectern_wrapper_session_dir =
            stale_lectern_wrapper_session_dir.display().to_string();
        ...
        assert_eq!(
            report
                .pulpit_receipt_latest_session_path
                .as_deref()
                .unwrap(),
        ...
        for summary in [
            report
                .run_latest_pulpit_receipt_command_summary
                .as_deref()
                .expect("missing run_latest_pulpit_receipt_command_summary"),
            report
                .verify_latest_pulpit_receipt_command_summary
                .as_deref()
                .expect("missing verify_latest_pulpit_receipt_command_summary"),
        ] {
            assert!(
                summary.contains(default_wrapper_session_dir.as_str()),
                "self command summary must use pulpit latest default session dir: {summary}"
            );
            assert!(
                !summary.contains(stale_lectern_wrapper_session_dir.as_str()),
                "self command summary must not use lectern latest default wrapper session dir: {summary}"
            );
        }
        assert_eq!(
            report
    ...
    #[test]
    fn render_mode_emits_exact_downstream_pulpit_receipt_contract_not_second_controller_path() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_render_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = render_latest_pulpit_receipt_script_report(&fixture.render_config()).unwrap();
        assert_eq!(
            report.verdict,
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestScriptRendered
        );
        assert_eq!(report.mode, "render_latest_pulpit_receipt_script");
        ...
        assert!(script.contains(PULPIT_RECEIPT_BIN));
        ...
        assert!(script.contains("--lectern-certificate-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-pulpit-receipt"));
        assert!(script.contains("--verify-live-package-pulpit-receipt"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            (
                "lectern_certificate_latest_missing",
                "tiny_live_package_lectern_certificate_latest_refused_by_missing_latest_chain",
                "no persisted immutable chain exists",
                TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestRefusedByMissingLatestChain,
            ),
            (
                "lectern_certificate_latest_invalid",
                "tiny_live_package_lectern_certificate_latest_refused_by_invalid_latest_chain",
                "latest immutable chain is incomplete",
                TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestRefusedByInvalidLatestChain,
            ),
            (
                "lectern_certificate_latest_below",
                "tiny_live_package_lectern_certificate_latest_refused_by_invalid_latest_chain",
            ...
            let mut plan: Value = load_json(&fixture.lectern_certificate_latest_plan_json_path).unwrap();
            plan["verdict"] = json!(verdict);
            plan["reason"] = json!(reason);
            if label.ends_with("below") {
                plan["latest_top_step_name"] = json!("choir_receipt");
            }
            persist_json(&fixture.lectern_certificate_latest_plan_json_path, &plan).unwrap();

            let report = run_latest_pulpit_receipt_report(&fixture.run_config()).unwrap();
            assert_eq!(report.verdict, expected);
        }
    }

    #[test]
    fn plan_mode_maps_real_latest_lectern_certificate_drift_verdict_to_local_drift_refusal() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_plan_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value =
            load_json(&fixture.lectern_certificate_latest_plan_json_path).unwrap();
        plan["verdict"] = json!(
            "tiny_live_package_lectern_certificate_latest_refused_by_drifted_lectern_certificate_contract"
        );
        plan["reason"] = json!("real accepted latest-lectern-certificate drift");
        persist_json(&fixture.lectern_certificate_latest_plan_json_path, &plan).unwrap();
        ...
        assert_eq!(
            report.result.as_deref(),
            Some("refused_by_drifted_pulpit_receipt_contract")
        );
    }

    #[test]
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_lectern_certificate_reports_real_drift(
    ) {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_run_drift",
            "refused_by_drifted_lectern_certificate_contract",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert_eq!(report.pulpit_receipt_verdict.as_deref(), None);
    }

    #[test]
    fn run_mode_refuses_when_nested_latest_lectern_certificate_lineage_drifts_from_latest_immutable_chain(
    ) {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_handoff_bundle_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let mut run: Value = load_json(&fixture.lectern_certificate_latest_run_json_path).unwrap();
        run["package_dir"] = json!("/tampered/package");
        persist_json(&fixture.lectern_certificate_latest_run_json_path, &run).unwrap();
        ...
        assert!(report
            .reason
            .contains("stored latest-lectern-certificate run package_dir"));
    }

    #[test]
    fn valid_nested_latest_lectern_modes_are_accepted_by_latest_pulpit_plan_run_verify() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_latest_accepts_real_nested_lectern_modes",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let plan = plan_latest_pulpit_receipt_report(&fixture.plan_config()).unwrap();
        assert_eq!(
            plan.verdict,
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestPlanReady
        );

        let run = run_latest_pulpit_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestReadyForManualExecutionWhenGateTurnsGreen
        );

        let verify = verify_latest_pulpit_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackagePulpitReceiptLatestVerdict::TinyLivePackagePulpitReceiptLatestVerifyOk,
            "{:#?}",
        ...
        let stored_plan: Value =
            load_json(&paths.latest_lectern_certificate_plan_report_path).unwrap();
        let stored_run: Value = load_json(&paths.latest_lectern_certificate_report_path).unwrap();
        let status: Value = load_json(&paths.status_path).unwrap();

        assert_eq!(
            stored_plan["mode"],
            json!("plan_latest_lectern_certificate")
        );
        assert_eq!(stored_run["mode"], json!("run_latest_lectern_certificate"));
        assert_eq!(
            status["latest_lectern_certificate_plan_step"]["mode"],
            json!("plan_latest_lectern_certificate")
        );
        assert_eq!(
            status["latest_lectern_certificate_step"]["mode"],
            json!("run_latest_lectern_certificate")
        );
    }

    #[test]
    fn plan_rejects_stale_parent_mode_in_nested_latest_lectern_plan_payload() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_latest_rejects_parent_mode_in_nested_lectern_plan",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value =
            load_json(&fixture.lectern_certificate_latest_plan_json_path).unwrap();
        plan["mode"] = json!("plan_latest_pulpit_receipt");
        persist_json(&fixture.lectern_certificate_latest_plan_json_path, &plan).unwrap();
        ...
        assert!(report.reason.contains(
            "stored latest-lectern-certificate plan mode \"plan_latest_pulpit_receipt\" does not match expected \"plan_latest_lectern_certificate\""
        ));
    }

    #[test]
    fn run_rejects_stale_parent_mode_in_nested_latest_lectern_run_payload() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_latest_rejects_parent_mode_in_nested_lectern_run",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut run: Value = load_json(&fixture.lectern_certificate_latest_run_json_path).unwrap();
        run["mode"] = json!("run_latest_pulpit_receipt");
        persist_json(&fixture.lectern_certificate_latest_run_json_path, &run).unwrap();
        ...
        assert!(report.reason.contains(
            "stored latest-lectern-certificate run mode \"run_latest_pulpit_receipt\" does not match expected \"run_latest_lectern_certificate\""
        ));
    }

    #[test]
    fn verify_rejects_stale_parent_modes_in_copied_nested_latest_lectern_reports() {
        {
            let fixture = fake_lectern_certificate_latest_fixture(
                "pulpit_receipt_latest_rejects_parent_mode_in_copied_lectern_plan",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            run_latest_pulpit_receipt_report(&fixture.run_config()).unwrap();

            let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
            let mut plan: Value =
                load_json(&paths.latest_lectern_certificate_plan_report_path).unwrap();
            plan["mode"] = json!("plan_latest_pulpit_receipt");
            persist_json(&paths.latest_lectern_certificate_plan_report_path, &plan).unwrap();
        ...
        {
            let fixture = fake_lectern_certificate_latest_fixture(
                "pulpit_receipt_latest_rejects_parent_mode_in_copied_lectern_run",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
            let mut run: Value = load_json(&paths.latest_lectern_certificate_report_path).unwrap();
            run["mode"] = json!("run_latest_pulpit_receipt");
            persist_json(&paths.latest_lectern_certificate_report_path, &run).unwrap();
    ...
    #[test]
    fn verify_mode_fails_when_persisted_latest_lectern_certificate_artifacts_are_tampered_or_missing_nested_lineage_truth(
    ) {
        {
            let fixture = fake_lectern_certificate_latest_fixture(
                "lectern_certificate_latest_verify_tampered_copy",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.pulpit_receipt_report_path).unwrap();
            report["decision_packet_session_dir"] = json!("/tampered/run-decision-packet-session");
            persist_json(&paths.pulpit_receipt_report_path, &report).unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains("stored pulpit-receipt run decision_packet_session_dir")
            }));
        }

        {
            let fixture = fake_lectern_certificate_latest_fixture(
                "lectern_certificate_latest_verify_missing_nested_truth",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
            fs::remove_file(nested_lectern_certificate_latest_report_path(
                &paths.nested_lectern_certificate_latest_session_dir,
            ))
            .unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry
                    .contains("tiny_live_activation_package_lectern_certificate_latest.report.json")
            }));
        }
    }

    #[test]
    fn verify_accepts_real_shaped_native_lectern_and_pulpit_identity_layers_on_happy_path() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_latest_verify_real_shaped_rostrum_paths_green",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    #[test]
    fn fake_native_pulpit_payload_uses_crest_and_lectern_armorial_input_wording() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_latest_fixture_identity_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        let snapshot = LatestLecternCertificateSnapshot {
        ...
        for (label, payload) in payloads {
            ...
            assert!(
                ...
                "{label} pulpit summary must use accepted crest wording: {pulpit_summary}"
            );
            assert!(
                !pulpit_summary.contains("hallmark"),
                "{label} pulpit summary must not use stale hallmark wording: {pulpit_summary}"
            );
            assert!(
                !pulpit_summary.contains("escutcheon"),
                "{label} pulpit summary must not use upstream rostrum escutcheon wording: {pulpit_summary}"
            );
            assert!(
                !pulpit_summary.contains("blazon"),
                "{label} pulpit summary must not use upstream podium blazon wording: {pulpit_summary}"
            );
            assert!(
                !pulpit_summary.contains("armorial"),
                "{label} pulpit summary must not use upstream lectern armorial wording: {pulpit_summary}"
            );
            ...
            if explicit_statement.contains("freezes one final") {
                assert!(
                    ...
                    "{label} pulpit session/run explicit statement must use crest wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("escutcheon"),
                    "{label} pulpit session/run explicit statement must not use upstream rostrum escutcheon wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("blazon"),
                    "{label} pulpit session/run explicit statement must not use upstream podium blazon wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("armorial"),
                    "{label} pulpit session/run explicit statement must not use upstream lectern armorial wording: {explicit_statement}"
                );
            }
            ...
            assert!(
                ...
                "{label} lectern input summary must use accepted armorial wording: {lectern_summary}"
            );
            assert!(
                lectern_summary.contains("podium-receipt"),
                "{label} lectern input summary must bind to podium-receipt truth: {lectern_summary}"
            );
            assert!(
                !lectern_summary.contains("hallmark"),
                "{label} lectern input summary must not use stale hallmark wording: {lectern_summary}"
            );
            assert!(
                !lectern_summary.contains("escutcheon"),
                "{label} lectern input summary must not use upstream rostrum escutcheon wording: {lectern_summary}"
            );
            assert!(
                !lectern_summary.contains("blazon"),
                "{label} lectern input summary must not use upstream podium blazon wording: {lectern_summary}"
            );
            assert!(
                !lectern_summary.contains("rostrum-certificate"),
                "{label} lectern input summary must not bind directly to rostrum-certificate truth: {lectern_summary}"
            );
            assert!(
                !lectern_summary.contains("dais-receipt"),
                "{label} lectern input summary must not bind directly to dais-receipt truth: {lectern_summary}"
            );
            assert!(
                !lectern_summary.contains("plinth-receipt"),
                "{label} lectern input summary must not bind directly to plinth-receipt truth: {lectern_summary}"
            );
        }
    }

    #[test]
    fn run_and_verify_reject_stale_foreign_wrapper_schema_for_downstream_pulpit_copy() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_latest_verify_rejects_stale_keystone_wrapper_schema",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert!(paths.pulpit_receipt_report_path.ends_with(
            "tiny_live_activation_package_pulpit_receipt_latest.pulpit_receipt.report.json"
        ));
        assert!(paths.pulpit_receipt_report_path.exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_pulpit_receipt_latest.lectern_certificate.report.json"
            )
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_pulpit_receipt_latest.pedestal_certificate.report.json"
            )
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_pulpit_receipt_latest.plinth_receipt.report.json")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_pulpit_receipt_latest.substructure_certificate.report.json"
            )
            .exists());
        assert!(paths.nested_pulpit_receipt_session_dir.ends_with(
            "tiny_live_activation_package_pulpit_receipt_latest.pulpit_receipt_session"
        ));
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_pulpit_receipt_latest.lectern_certificate_session")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_pulpit_receipt_latest.pedestal_certificate_session")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_pulpit_receipt_latest.plinth_receipt_session")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_pulpit_receipt_latest.substructure_certificate_session")
            .exists());

        let mut status: Value = load_json(&paths.status_path).unwrap();
        let stored_step = status
            .get("pulpit_receipt_step")
            .cloned()
            .expect("pulpit_receipt_step should be present");
        assert!(status.get("keystone_receipt_step").is_none());
        status["keystone_receipt_step"] = stored_step;
        if let Some(object) = status.as_object_mut() {
            object.remove("pulpit_receipt_step");
        }
        persist_json(&paths.status_path, &status).unwrap();

        let error = verify_latest_pulpit_receipt_report(&fixture.verify_config()).unwrap_err();
        assert!(error.to_string().contains("pulpit_receipt_step"));
    }

    #[test]
    fn verify_names_pulpit_receipt_when_nested_native_session_load_breaks() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_nested_dais_session_load_break",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        fs::remove_file(nested_pulpit_receipt_session_path(
            &paths.nested_pulpit_receipt_session_dir,
        ))
        .unwrap();
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("failed reading nested pulpit-receipt session under") }));
    }

    #[test]
    fn verify_fails_when_copied_native_pulpit_receipt_session_path_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_dais_session_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.pulpit_receipt_report_path).unwrap();
        report["pulpit_receipt_session_path"] = json!("/tampered/finality-receipt-session.json");
        persist_json(&paths.pulpit_receipt_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt run pulpit_receipt_session_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_pulpit_receipt_status_path_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_dais_status_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.pulpit_receipt_report_path).unwrap();
        report["pulpit_receipt_status_path"] = json!("/tampered/finality-receipt-status.json");
        persist_json(&paths.pulpit_receipt_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt run pulpit_receipt_status_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_pulpit_pedestal_certificate_identity_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_verify_pedestal_identity_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy pedestal_certificate_sha256")
        }));
    }

    #[test]
    fn run_names_latest_lectern_certificate_when_nested_latest_wrapper_run_fails() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_nested_run_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        overwrite_fake_lectern_certificate_latest_command(
            &fixture.bin_dir.join(LECTERN_CERTIFICATE_LATEST_BIN),
            &fixture.lectern_certificate_latest_plan_json_path,
            &fixture.lectern_certificate_latest_run_json_path,
            &fixture.lectern_certificate_latest_verify_json_path,
            None,
            Some("forced nested latest-lectern-certificate run failure"),
            None,
            &fixture.root,
            &paths.nested_lectern_certificate_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed running nested latest-lectern-certificate handoff under"));
    }

    #[test]
    fn run_names_latest_lectern_certificate_when_nested_latest_wrapper_verify_fails() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_nested_verify_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_lectern_certificate_latest_command(
            ...
            None,
            None,
            Some("forced nested latest-lectern-certificate verify failure"),
            &fixture.root,
            &paths.nested_lectern_certificate_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed verifying nested latest-lectern-certificate handoff under"));
    }

    #[test]
    fn run_names_pulpit_receipt_when_downstream_native_run_fails() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_downstream_run_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_release_capsule_command(
            &fixture.bin_dir.join(PULPIT_RECEIPT_BIN),
            ...
            Some("forced downstream native pulpit run failure"),
            None,
            &expected_downstream_lectern_certificate_session_dir(&paths),
            &expected_downstream_decision_packet_session_dir(&paths),
            &paths.nested_pulpit_receipt_session_dir,
    ...
    #[test]
    fn run_names_pulpit_receipt_when_downstream_native_verify_fails() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_downstream_verify_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_release_capsule_command(
            ...
            &fixture.handoff_bundle_status_json_path,
            None,
            Some("forced downstream native pulpit verify failure"),
            &expected_downstream_lectern_certificate_session_dir(&paths),
            &expected_downstream_decision_packet_session_dir(&paths),
            &paths.nested_pulpit_receipt_session_dir,
        );
    ...
    #[test]
    fn run_uses_pulpit_placeholders_when_missing_snapshot_blocks_nested_handoff_summaries() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_latest_missing_snapshot_placeholders",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_lectern_certificate_latest_command(
            ...
            &fixture.lectern_certificate_latest_run_json_path,
            &fixture.lectern_certificate_latest_verify_json_path,
            Some("forced latest-lectern-certificate plan failure without snapshot"),
            None,
            None,
            &fixture.root,
            &paths.nested_lectern_certificate_latest_session_dir,
        ...
        assert_eq!(
            report.downstream_pulpit_receipt_command_summary.as_deref(),
            Some("<pulpit-receipt-run-unavailable>")
        );
        assert_eq!(
            report.verify_pulpit_receipt_command_summary.as_deref(),
            Some("<pulpit-receipt-verify-unavailable>")
        );

        let session: PackagePulpitReceiptLatestSession = load_json(&paths.session_path).unwrap();
        assert_eq!(
            session.downstream_pulpit_receipt_command_summary,
            "<pulpit-receipt-run-unavailable>"
        );
        assert_eq!(
            session.verify_pulpit_receipt_command_summary,
            "<pulpit-receipt-verify-unavailable>"
        );
    }

    #[test]
    fn verify_names_pulpit_receipt_when_nested_native_verify_is_non_green() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_verify_non_green_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        verify_json["verdict"] = json!("tiny_live_package_pulpit_receipt_verify_invalid");
        verify_json["reason"] = json!("forced nested pulpit-receipt verify non-green");
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("nested pulpit-receipt verification is non-green") }));
        assert!(!verify.verification_mismatches.iter().any(|entry| {
    ...
    #[test]
    fn verify_names_latest_pulpit_receipt_when_wrapper_result_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_wrapper_result_mismatch_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    #[test]
    fn verify_names_latest_pulpit_receipt_when_activation_authorized_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_activation_authorized_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn verify_fails_when_copied_native_pinnacle_receipt_sha256_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_pinnacle_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored pulpit-receipt copy pinnacle_receipt_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_latest_lectern_certificate_explicit_statement_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_launch_packet_explicit_statement_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.latest_lectern_certificate_report_path).unwrap();
        report["explicit_statement"] =
            json!("tampered nested latest release capsule explicit statement");
        persist_json(&paths.latest_lectern_certificate_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored latest-lectern-certificate copy explicit_statement")
        }));
    }
    ...
    fn verify_rejects_tampered_stale_substructure_wrapper_mode_string() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_wrapper_mode_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("latest-pulpit-receipt stored report mode") }));
    }

    #[test]
    fn wrapper_self_modes_and_self_command_summaries_use_pulpit_latest_contract() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_latest_self_mode_and_summary_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let plan = plan_latest_pulpit_receipt_report(&fixture.plan_config()).unwrap();
        assert_eq!(plan.mode, "plan_latest_pulpit_receipt");
        assert!(plan
            .run_latest_pulpit_receipt_command_summary
            .as_deref()
            .unwrap()
            .contains(PULPIT_RECEIPT_LATEST_BIN));
        assert!(plan
            .run_latest_pulpit_receipt_command_summary
            .as_deref()
            .unwrap()
        ...
        let run = run_latest_pulpit_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(run.mode, "run_latest_pulpit_receipt");

        let verify = verify_latest_pulpit_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(verify.mode, "verify_latest_pulpit_receipt");

        let session: PackagePulpitReceiptLatestSession =
            load_json(&pulpit_receipt_latest_paths(&fixture.wrapper_session_dir).session_path)
                .unwrap();
        assert!(session
            .run_latest_pulpit_receipt_command_summary
            .contains(PULPIT_RECEIPT_LATEST_BIN));
    ...
    #[test]
    fn native_lectern_input_uses_real_latest_lectern_certificate_session_path() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_native_input_session_dir_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);

        let downstream = expected_downstream_lectern_certificate_session_dir(&paths);
        let expected = paths.nested_lectern_certificate_latest_session_dir.join(
    ...
    #[test]
    fn run_persists_copied_native_pulpit_session_and_status_under_real_pulpit_filenames() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "pulpit_receipt_latest_native_pulpit_filenames",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let stale_lectern_session = paths
            .nested_pulpit_receipt_session_dir
            .join("tiny_live_activation_package_lectern_certificate.session.json");
        let stale_lectern_status = paths
            .nested_pulpit_receipt_session_dir
            .join("tiny_live_activation_package_lectern_certificate.status.json");

        assert!(
            real_session.exists(),
            "missing real native pulpit session path"
        );
        assert!(
            real_status.exists(),
            "missing real native pulpit status path"
        );
        assert!(
        ...
        assert!(
            !stale_lectern_session.exists(),
            "stale native lectern session filename should not be used"
        );
        assert!(
            !stale_lectern_status.exists(),
            "stale native lectern status filename should not be used"
        );
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_decision_packet_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored pulpit-receipt copy decision_packet_result") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_attestation_seal_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy attestation_seal_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_registry_entry_session_dir_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_registry_entry_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy registry_entry_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_notarization_receipt_session_dir_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_notarization_receipt_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy notarization_receipt_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_chain_fingerprint_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy chain_fingerprint_summary")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_chain_fingerprint_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy chain_fingerprint_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_chain_fingerprint_algorithm_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy chain_fingerprint_algorithm")
        }));
    }
    ...
    fn verify_fails_when_copied_native_completion_certificate_closure_certificate_sha256_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_closure_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy closure_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_archive_receipt_sha256_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_archive_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored pulpit-receipt copy archive_receipt_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_native_finality_receipt_finality_receipt_sha256_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_finality_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored pulpit-receipt copy finality_receipt_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_native_consummation_record_sha256_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_consummation_record_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy consummation_record_sha256")
        }));
    }
    ...
    {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_completion_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy completion_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_decision_packet_anchor_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored pulpit-receipt copy decision_packet_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_current_authorization_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored pulpit-receipt copy execute_frozen_result") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_pulpit_receipt_verdict_drifts() {
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_release_capsule_verdict_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| entry.contains("stored pulpit-receipt copy verdict")));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_current_live_cutover_controller_summary_drifts(
        ...
        let fixture = fake_lectern_certificate_latest_fixture(
            "lectern_certificate_latest_verify_current_live_cutover_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn all_modes_preserve_stage3_and_pre_activation_refusal_semantics_and_do_not_imply_activation_authorization_by_themselves(
        ...
        {
            let fixture = fake_lectern_certificate_latest_fixture(
                "lectern_certificate_latest_stage3",
        ...
        {
            let fixture = fake_lectern_certificate_latest_fixture(
                "lectern_certificate_latest_pre_activation",
                "refused_now_by_pre_activation_gate",
                "refused_now_by_pre_activation_gate",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        {
            let fixture = fake_lectern_certificate_latest_fixture(
                "lectern_certificate_latest_green",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn fake_lectern_certificate_latest_fixture(
        label: &str,
        lectern_certificate_latest_run_result: &str,
        handoff_bundle_result: &str,
    ) -> FakeLecternCertificateLatestFixture {
        ...
        let lectern_certificate_latest_plan_json_path =
            temp_dir.join("lectern_certificate_latest.plan.json");
        let lectern_certificate_latest_run_json_path =
            temp_dir.join("lectern_certificate_latest.run.json");
        let lectern_certificate_latest_verify_json_path =
            temp_dir.join("lectern_certificate_latest.verify.json");
        ...
        let fixture = FakeLecternCertificateLatestFixture {
            temp_dir,
            root,
            wrapper_session_dir,
            output_script_path,
            ...
            install_root,
            backend_command,
            lectern_certificate_latest_plan_json_path,
            lectern_certificate_latest_run_json_path,
            lectern_certificate_latest_verify_json_path,
        ...
        persist_json(
            &fixture.lectern_certificate_latest_plan_json_path,
            &default_lectern_certificate_latest_plan_report(&fixture),
        )
        .unwrap();
        persist_json(
            &fixture.lectern_certificate_latest_run_json_path,
            &default_lectern_certificate_latest_run_report(
                &fixture,
                lectern_certificate_latest_run_result,
            ),
        )
        .unwrap();
        persist_json(
            &fixture.lectern_certificate_latest_verify_json_path,
            &default_lectern_certificate_latest_verify_report(
                &fixture,
                lectern_certificate_latest_run_result,
            ),
        )
    ...
    fn default_lectern_certificate_latest_plan_report(
        fixture: &FakeLecternCertificateLatestFixture,
    ) -> Value {
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        json!({
            "generated_at": Utc::now(),
            "mode": "plan_latest_lectern_certificate",
            "verdict": LECTERN_CERTIFICATE_LATEST_PLAN_READY,
            "reason": "latest immutable chain resolves cleanly into accepted latest-lectern-certificate contract",
            ...
            "wrapper_timeout_ms": 1200,
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_lectern_certificate_latest_session_dir.display().to_string(),
            ...
            "activation_authorized": false,
            "explicit_statement": "latest lectern-certificate wrapper plan"
        })
    }

    fn canonical_lectern_certificate_latest_result(result: &str) -> &str {
        match result {
            "runnable_when_gate_truth_turns_green" => {
                "ready_for_manual_execution_when_gate_turns_green"
            }
    ...
    fn default_lectern_certificate_latest_run_report(
        fixture: &FakeLecternCertificateLatestFixture,
        result: &str,
    ) -> Value {
        let result = canonical_lectern_certificate_latest_result(result);
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_lectern_certificate_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_lectern_certificate_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_lectern_certificate_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_latest_lectern_certificate",
            "verdict": verdict,
            "reason": format!("latest-lectern-certificate {result}"),
            "latest_top_step_name": "clerestory_certificate",
            "latest_top_session_dir": fixture.latest_top_session_dir.display().to_string(),
            "historical_execute_frozen_session_dir": fixture.historical_execute_frozen_session_dir.display().to_string(),
            "turn_green_session_dir": fixture.turn_green_session_dir.display().to_string(),
            ...
            "clerestory_certificate_algorithm": "sha256",
            "activation_authorized": false,
            "explicit_statement": "latest lectern-certificate wrapper run"
        })
    }

    fn default_lectern_certificate_latest_verify_report(
        fixture: &FakeLecternCertificateLatestFixture,
        result: &str,
    ) -> Value {
        let mut report = default_lectern_certificate_latest_run_report(fixture, result);
        report["mode"] = json!("verify_latest_lectern_certificate");
        report["verdict"] = json!(LECTERN_CERTIFICATE_LATEST_VERIFY_OK);
    ...
    fn default_provenance_certificate_session(
        ...
        let paths = pulpit_receipt_latest_paths(&fixture.wrapper_session_dir);
        let lectern_certificate_session_dir =
            expected_downstream_lectern_certificate_session_dir(&paths);
        ...
        let pedestal_certificate_session_dir = fixture
            .temp_dir
            .join("downstream-lectern-certificate-pedestal-certificate-session");
        ...
        json!({
            "session_version": "1",
            "planned_at": Utc::now(),
            "lectern_certificate_session_dir": lectern_certificate_session_dir.display().to_string(),
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": pulpit_receipt_latest_paths(&fixture.wrapper_session_dir).nested_pulpit_receipt_session_dir.display().to_string(),
            "verify_lectern_certificate_command_summary": verify_pulpit_receipt_input_command_summary(&lectern_certificate_session_dir),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
            ...
            "explicit_statement": "the verified lectern-certificate session is the primary input here; this immutable pulpit receipt freezes one final crest identity over the fully culminated chain without enabling production execution"
        })
    }
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_pulpit_receipt_session_dir.display().to_string(),
            "lectern_certificate_session_dir": expected_downstream_lectern_certificate_session_dir(&paths).display().to_string(),
            "pedestal_certificate_session_dir": pedestal_certificate_session_dir.display().to_string(),
            "plinth_receipt_session_dir": plinth_receipt_session_dir.display().to_string(),
            "substructure_certificate_session_dir": substructure_certificate_session_dir.display().to_string(),
            "basal_receipt_session_dir": basal_receipt_session_dir.display().to_string(),
            ...
            "current_pre_activation_gate_verdict": gate_verdict,
            "current_pre_activation_gate_reason": gate_reason,
            "lectern_certificate_step": Value::Null,
            ...
            "pulpit_receipt_sha256": "fixture-pulpit-receipt-sha256",
            "pulpit_receipt_algorithm": "sha256",
            "explicit_statement": "this pulpit receipt is archival and read-only; it never executes or enables the frozen controller"
        })
    }
    ...
    fn default_provenance_certificate_run_report(
        ...
        let pulpit_receipt_verdict = match result {
            "ready_for_manual_execution_when_gate_turns_green" => {
                "tiny_live_package_pulpit_receipt_ready_for_manual_execution_when_gate_turns_green"
            }
            "refused_now_by_stage3" => "tiny_live_package_pulpit_receipt_refused_now_by_stage3",
            "refused_now_by_pre_activation_gate" => {
                "tiny_live_package_pulpit_receipt_refused_now_by_pre_activation_gate"
            }
            _ => "tiny_live_package_pulpit_receipt_refused_now_by_invalid_or_drifted_contract",
        };
        let substructure_certificate_session_dir = fixture
            .temp_dir
            .join("downstream-basal-receipt-substructure-certificate-session");
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_pulpit_receipt",
            "verdict": pulpit_receipt_verdict,
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_pulpit_receipt_session_dir.display().to_string(),
            "pulpit_receipt_session_path": nested_pulpit_receipt_session_path(&paths.nested_pulpit_receipt_session_dir).display().to_string(),
            "pulpit_receipt_status_path": nested_pulpit_receipt_status_path(&paths.nested_pulpit_receipt_session_dir).display().to_string(),
            "archived_lectern_certificate_report_path": paths.nested_pulpit_receipt_session_dir.join("tiny_live_activation_package_pulpit_receipt.lectern_certificate.report.json").display().to_string(),
            "result": result,
            ...
            "lectern_certificate_step": status["lectern_certificate_step"],
            "verify_lectern_certificate_command_summary": verify_pulpit_receipt_input_command_summary(&expected_downstream_lectern_certificate_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn write_fake_lectern_certificate_latest_command(
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-lectern-certificate session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-lectern-certificate \"*) mode='plan';;\n  *\" --run-latest-lectern-certificate \"*) mode='run';;\n  *\" --verify-latest-lectern-certificate \"*) mode='verify';;\n  *) echo 'unexpected latest-lectern-certificate mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_lectern_certificate_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
    ...
    fn overwrite_fake_lectern_certificate_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        verify_json_path: &Path,
    ...
    fn write_fake_release_capsule_command(
        ...
        let nested_session_path = nested_pulpit_receipt_session_path(session_dir);
        let nested_status_path = nested_pulpit_receipt_status_path(session_dir);
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --lectern-certificate-session-dir {} \"*) : ;;\n  *) echo 'unexpected lectern-certificate-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected confirm-decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected pulpit-receipt session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --run-live-package-pulpit-receipt \"*) mode='run';;\n  *\" --verify-live-package-pulpit-receipt \"*) mode='verify';;\n  *) echo 'unexpected pulpit-receipt mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  run) mkdir -p '{}'; cp '{}' '{}'; cp '{}' '{}'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
