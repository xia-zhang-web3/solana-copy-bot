...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_manifest [--root <path> | --session-dir <path>] [--json] (--latest-manifest | --session-manifest | --verify-manifest)";
...
const NOT_AUTHORIZED_SUMMARY: &str =
    "This tiny-live package manifest surface only audits persisted package session/status/report artifacts. It stays planning-safe, does not authorize activation, and does not override the Stage 3 production gate.";
...
enum TinyLivePackageManifestVerdict {
    TinyLivePackageManifestAvailable,
    TinyLivePackageManifestEmpty,
    TinyLivePackageManifestInvalid,
    TinyLivePackageManifestVerifyOk,
    TinyLivePackageManifestVerifyInvalid,
}
...
struct ManifestSurface {
    ...
    manifest_summary: Option<String>,
...
#[derive(Debug, Clone, Serialize)]
struct PackageManifestReport {
    mode: String,
    verdict: TinyLivePackageManifestVerdict,
    reason: String,
    root: Option<String>,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--latest-manifest" => set_mode(&mut mode, Mode::LatestManifest, "--latest-manifest")?,
            "--session-manifest" => {
                set_mode(&mut mode, Mode::SessionManifest, "--session-manifest")?
            }
            "--verify-manifest" => set_mode(&mut mode, Mode::VerifyManifest, "--verify-manifest")?,
            "--json" => json = true,
    ...
    let Some(mode) = mode else {
        bail!(
            "exactly one of --latest-manifest, --session-manifest, or --verify-manifest is required"
        );
    };

    match mode {
        Mode::LatestManifest | Mode::VerifyManifest => {
            if root.is_none() {
                bail!("missing required --root");
            }
            if session_dir.is_some() {
                bail!("--session-dir is only valid with --session-manifest");
            }
        }
        Mode::SessionManifest => {
            if session_dir.is_none() {
                bail!("missing required --session-dir");
            }
            if root.is_some() {
                bail!("--root is only valid with --latest-manifest or --verify-manifest");
            }
        }
    }
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::LatestManifest => build_latest_manifest_report(&config)?,
        Mode::SessionManifest => build_session_manifest_report(&config)?,
        Mode::VerifyManifest => build_verify_manifest_report(&config)?,
    };
    if config.json {
        serde_json::to_string_pretty(&report)
            .context("failed serializing tiny-live package manifest report json")
    } else {
        Ok(render_manifest_human(&report))
    }
}

fn build_latest_manifest_report(config: &Config) -> Result<PackageManifestReport> {
    ...
    let Some(candidate) = select_latest_top_chain_candidate(&sessions) else {
        return Ok(empty_manifest_report(
            "latest_manifest",
    ...
    let mut invalid_reasons = manifest.invalid_reasons.clone();
    ...
    let manifest_complete = invalid_reasons.is_empty();
    let (verdict, reason) = if manifest_complete {
        (
            TinyLivePackageManifestVerdict::TinyLivePackageManifestAvailable,
            format!(
                "latest immutable tiny-live package manifest is complete from turn_green through {}",
                resolution.top_step_name
            ),
        )
    } else {
        (
            TinyLivePackageManifestVerdict::TinyLivePackageManifestInvalid,
            invalid_reasons.join("; "),
        )
    };

    Ok(PackageManifestReport {
        mode: "latest_manifest".to_string(),
        verdict,
        reason,
        ...
        lineage_chain_summary: manifest.lineage_chain_summary,
...
fn build_session_manifest_report(config: &Config) -> Result<PackageManifestReport> {
    ...
    let manifest = build_manifest_surface(&resolution);
    let invalid_reasons = manifest.invalid_reasons.clone();
    let manifest_complete = invalid_reasons.is_empty();
    let (verdict, reason) = if manifest_complete {
        (
            TinyLivePackageManifestVerdict::TinyLivePackageManifestAvailable,
            format!(
                "persisted immutable tiny-live package manifest is complete from turn_green through {}",
                resolution.top_step_name
            ),
        )
    } else {
    ...
    Ok(PackageManifestReport {
        mode: "session_manifest".to_string(),
        verdict,
        reason,
        root: None,
...
fn build_verify_manifest_report(config: &Config) -> Result<PackageManifestReport> {
    let root = config.root.as_ref().expect("validated root");
    let sessions = scan_package_sessions(root)?;
    let Some(candidate) = select_latest_top_chain_candidate(&sessions) else {
        return Ok(empty_manifest_report(
    ...
    let (verdict, reason) = if manifest_complete {
        (
            TinyLivePackageManifestVerdict::TinyLivePackageManifestVerifyOk,
            format!(
                "latest immutable tiny-live package manifest verified cleanly through {}",
                resolution.top_step_name
            ),
        )
    } else {
        (
            TinyLivePackageManifestVerdict::TinyLivePackageManifestVerifyInvalid,
            invalid_reasons.join("; "),
        )
    };
...
fn empty_manifest_report(
    ...
    PackageManifestReport {
        mode: mode.to_string(),
        verdict: TinyLivePackageManifestVerdict::TinyLivePackageManifestEmpty,
        reason,
        root,
        session_dir,
        top_accepted_package_step: TOP_ACCEPTED_PACKAGE_STEP.to_string(),
...
fn build_manifest_surface(resolution: &LineageResolution) -> ManifestSurface {
    ...
    for artifact in &artifacts {
        invalid_reasons.extend(artifact.invalid_reasons.iter().cloned());
    }
    ...
    let manifest_summary = Some(format!(
        "{required_artifact_count} required persisted artifacts over {} (present {present_artifact_count} parseable {parseable_artifact_count})",
        lineage_chain_summary
            .as_deref()
            .unwrap_or("<no lineage resolved>")
    ));
    let manifest_digest_sha256 = Some(compute_manifest_digest(&artifacts));
...
fn render_manifest_human(report: &PackageManifestReport) -> String {
    let mut lines = vec![
        "event=copybot_tiny_live_activation_package_manifest".to_string(),
        ...
        format!("manifest_complete={}", report.manifest_complete),
    ...
    if let Some(summary) = &report.manifest_summary {
        lines.push(format!("manifest_summary={summary}"));
    }
    ...
    for (index, artifact) in report.artifacts.iter().enumerate() {
        lines.push(format!(
            "artifact[{index}].step_name={}",
            artifact.step_name
        ));
        lines.push(format!(
            "artifact[{index}].artifact_kind={}",
            artifact.artifact_kind.as_str()
        ));
        lines.push(format!(
            "artifact[{index}].absolute_path={}",
            artifact.absolute_path
        ));
        lines.push(format!("artifact[{index}].exists={}", artifact.exists));
        lines.push(format!(
            "artifact[{index}].parseable={}",
            artifact.parseable
        ));
        if let Some(digest) = &artifact.sha256_digest {
            lines.push(format!("artifact[{index}].sha256_digest={digest}"));
        }
    }
...
mod tests {
    ...
    #[test]
    fn latest_manifest_returns_full_ordered_artifact_set_for_valid_persisted_latest_chain() {
        let fixture = ManifestFixture::new(
            "tiny_live_package_manifest_latest_valid",
            TOP_ACCEPTED_PACKAGE_STEP,
        );

        let report = build_latest_manifest_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::LatestManifest,
        })
        .expect("latest-manifest report");

        assert_eq!(
            report.verdict,
            TinyLivePackageManifestVerdict::TinyLivePackageManifestAvailable
        );
        assert!(report.manifest_complete);
        assert_eq!(
            report.required_artifact_count,
            step_rank(TOP_ACCEPTED_PACKAGE_STEP) * 3 - 1
        );
        assert_eq!(report.artifacts.len(), report.required_artifact_count);
        assert_eq!(report.artifacts[0].step_name, "turn_green");
        assert_eq!(report.artifacts[0].artifact_kind, ArtifactKind::Session);
        assert!(Path::new(&report.artifacts[0].absolute_path).is_absolute());
        assert!(report.artifacts[0].sha256_digest.is_some());
        let last = report.artifacts.last().expect("last artifact");
        assert_eq!(last.step_name, TOP_ACCEPTED_PACKAGE_STEP);
        assert_eq!(last.artifact_kind, ArtifactKind::Report);
        assert_eq!(last.nested_step_name.as_deref(), Some("choir_receipt"));
        assert!(report.manifest_digest_sha256.is_some());
    }

    #[test]
    fn session_manifest_works_for_explicit_session_dir_without_scanning_unrelated_roots() {
        let fixture = ManifestFixture::new(
            "tiny_live_package_manifest_session_explicit",
            "review_receipt",
        );
        let _unrelated = ManifestFixture::new(
            "tiny_live_package_manifest_session_unrelated",
            TOP_ACCEPTED_PACKAGE_STEP,
        );

        let report = build_session_manifest_report(&Config {
            root: None,
            session_dir: Some(fixture.step_dir("review_receipt").to_path_buf()),
            json: true,
            mode: Mode::SessionManifest,
        })
        .expect("session-manifest report");
        ...
        assert_eq!(
            report.required_artifact_count,
            step_rank("review_receipt") * 3 - 1
        );
    }

    #[test]
    fn verify_manifest_fails_when_one_required_nested_report_artifact_is_missing() {
        let fixture = ManifestFixture::new(
            "tiny_live_package_manifest_missing_report",
            TOP_ACCEPTED_PACKAGE_STEP,
        );
        fs::remove_file(
            fixture
        ...
        let report = build_verify_manifest_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::VerifyManifest,
        })
        .expect("verify-manifest report");
    ...
    #[test]
    fn verify_manifest_fails_when_top_level_latest_chain_is_below_clerestory_certificate() {
        let fixture = ManifestFixture::new("tiny_live_package_manifest_below_top", "choir_receipt");
    ...
    #[test]
    fn verify_manifest_fails_when_one_persisted_artifact_is_tampered_and_breaks_continuity() {
        let fixture = ManifestFixture::new(
            "tiny_live_package_manifest_tampered",
            TOP_ACCEPTED_PACKAGE_STEP,
        );
        set_json_string(
            fixture
        ...
        let tampered_status = report
            .artifacts
            .iter()
            .find(|artifact| {
                artifact.step_name == TOP_ACCEPTED_PACKAGE_STEP
                    && artifact.artifact_kind == ArtifactKind::Status
            })
            .expect("tampered status artifact");
        assert!(tampered_status.sha256_digest.is_some());
    }

    #[test]
    fn all_modes_stay_planning_safe_and_do_not_imply_activation_authorization() {
        let fixture = ManifestFixture::new(
            "tiny_live_package_manifest_planning_safe",
            TOP_ACCEPTED_PACKAGE_STEP,
        );

        let latest = build_latest_manifest_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::LatestManifest,
        ...
        .expect("latest-manifest report");
        let verify = build_verify_manifest_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::VerifyManifest,
        ...
        .expect("verify-manifest report");
        let session = build_session_manifest_report(&Config {
            root: None,
        ...
        if let Some(lower) = lower {
            ...
            fs::write(
                ...
                serde_json::to_vec_pretty(&json!({
                    ...
                    "reason": "nested manifest continuity ok"
                }))
                .unwrap(),
            )
            .unwrap();
