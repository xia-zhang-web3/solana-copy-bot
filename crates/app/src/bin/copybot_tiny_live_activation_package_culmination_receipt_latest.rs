...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_culmination_receipt_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-culmination-receipt | --render-latest-culmination-receipt-script | --run-latest-culmination-receipt | --verify-latest-culmination-receipt)";
const CULMINATION_RECEIPT_LATEST_SESSION_VERSION: &str = "1";
const CULMINATION_RECEIPT_LATEST_STATUS_VERSION: &str = "1";
const COMPLETION_CERTIFICATE_LATEST_BIN: &str =
    "copybot_tiny_live_activation_package_completion_certificate_latest";
const CULMINATION_RECEIPT_BIN: &str = "copybot_tiny_live_activation_package_culmination_receipt";
const COMPLETION_CERTIFICATE_LATEST_PLAN_READY: &str =
    "tiny_live_package_completion_certificate_latest_plan_ready";
const COMPLETION_CERTIFICATE_LATEST_VERIFY_OK: &str =
    "tiny_live_package_completion_certificate_latest_verify_ok";
const CULMINATION_RECEIPT_VERIFY_OK: &str = "tiny_live_package_culmination_receipt_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-culmination-receipt handoff binds the latest persisted immutable package chain to the accepted culmination-receipt contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestCulminationReceipt,
    RenderLatestCulminationReceiptScript,
    RunLatestCulminationReceipt,
    VerifyLatestCulminationReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageCulminationReceiptLatestVerdict {
    TinyLivePackageCulminationReceiptLatestPlanReady,
    TinyLivePackageCulminationReceiptLatestScriptRendered,
    TinyLivePackageCulminationReceiptLatestRefusedByMissingLatestChain,
    TinyLivePackageCulminationReceiptLatestRefusedByInvalidLatestChain,
    TinyLivePackageCulminationReceiptLatestRefusedByDriftedCulminationReceiptContract,
    TinyLivePackageCulminationReceiptLatestRefusedNowByStage3,
    TinyLivePackageCulminationReceiptLatestRefusedNowByPreActivationGate,
    TinyLivePackageCulminationReceiptLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageCulminationReceiptLatestVerifyOk,
    TinyLivePackageCulminationReceiptLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageCulminationReceiptLatestResult {
    RefusedByMissingLatestChain,
    RefusedByInvalidLatestChain,
    RefusedByDriftedCulminationReceiptContract,
    RefusedNowByStage3,
...
struct PackageCulminationReceiptLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageCulminationReceiptLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_completion_certificate_latest_session_dir: String,
    nested_culmination_receipt_session_dir: String,
    ...
    resolve_latest_completion_certificate_command_summary: String,
    run_latest_completion_certificate_command_summary: String,
    verify_latest_completion_certificate_command_summary: String,
    downstream_culmination_receipt_command_summary: String,
    verify_completion_certificate_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageCulminationReceiptLatestStatus {
    ...
    nested_culmination_receipt_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackageCulminationReceiptLatestResult,
    reason: String,
    latest_completion_certificate_plan_verdict: Option<String>,
    latest_completion_certificate_verdict: Option<String>,
    completion_certificate_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_completion_certificate_plan_step: Option<PackageCulminationReceiptLatestStepArtifact>,
    latest_completion_certificate_step: Option<PackageCulminationReceiptLatestStepArtifact>,
    completion_certificate_step: Option<PackageCulminationReceiptLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackageCulminationReceiptLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_completion_certificate_plan_report_path: PathBuf,
    latest_completion_certificate_report_path: PathBuf,
    culmination_receipt_report_path: PathBuf,
    nested_completion_certificate_latest_session_dir: PathBuf,
    nested_culmination_receipt_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageCulminationReceiptLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageCulminationReceiptLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    culmination_receipt_latest_session_path: Option<String>,
    culmination_receipt_latest_status_path: Option<String>,
    culmination_receipt_latest_report_path: Option<String>,
    latest_completion_certificate_plan_report_path: Option<String>,
    latest_completion_certificate_report_path: Option<String>,
    culmination_receipt_report_path: Option<String>,
    nested_completion_certificate_latest_session_dir: Option<String>,
    nested_culmination_receipt_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_completion_certificate_command_summary: Option<String>,
    run_latest_completion_certificate_command_summary: Option<String>,
    verify_latest_completion_certificate_command_summary: Option<String>,
    downstream_culmination_receipt_command_summary: Option<String>,
    verify_completion_certificate_command_summary: Option<String>,
    reviewed_frozen_live_cutover_controller_command_summary: Option<String>,
    clerestory_certificate_summary: Option<String>,
    clerestory_certificate_sha256: Option<String>,
    clerestory_certificate_algorithm: Option<String>,
...
#[derive(Debug, Clone)]
struct LatestCompletionCertificateSnapshot {
...
#[derive(Debug, Clone)]
struct ResolvedLatestCompletionCertificate {
    snapshot: LatestCompletionCertificateSnapshot,
    latest_completion_certificate_plan_raw: Value,
    latest_completion_certificate_plan: CompletionCertificateLatestReportView,
}
...
enum ResolutionFailureKind {
    MissingLatestChain,
    InvalidLatestChain,
    DriftedCulminationReceiptContract,
}

#[derive(Debug, Clone)]
struct LatestCompletionCertificateResolutionFailure {
    kind: ResolutionFailureKind,
    reason: String,
    snapshot: Option<LatestCompletionCertificateSnapshot>,
    latest_completion_certificate_plan_raw: Option<Value>,
    latest_completion_certificate_plan_verdict: Option<String>,
    latest_completion_certificate_plan_reason: Option<String>,
    latest_completion_certificate_plan_generated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone, Deserialize)]
struct CompletionCertificateLatestReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Deserialize)]
struct CulminationReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    completion_certificate_session_dir: String,
    ...
    service_status_max_staleness_ms: Option<u64>,
    session_dir: Option<String>,
    culmination_receipt_session_path: Option<String>,
    culmination_receipt_status_path: Option<String>,
    archived_completion_certificate_report_path: Option<String>,
    result: Option<String>,
...
#[derive(Debug, Clone, Deserialize)]
struct CulminationReceiptSessionView {
    completion_certificate_session_dir: String,
...
#[derive(Debug, Clone, Deserialize)]
struct CulminationReceiptStatusView {
    session_dir: String,
    completion_certificate_session_dir: String,
    result: String,
    reason: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-culmination-receipt" => {
                set_mode(&mut mode, Mode::PlanLatestCulminationReceipt, arg.as_str())?
            }
            "--render-latest-culmination-receipt-script" => set_mode(
                &mut mode,
                Mode::RenderLatestCulminationReceiptScript,
                arg.as_str(),
            )?,
            "--run-latest-culmination-receipt" => {
                set_mode(&mut mode, Mode::RunLatestCulminationReceipt, arg.as_str())?
            }
            "--verify-latest-culmination-receipt" => set_mode(
                &mut mode,
                Mode::VerifyLatestCulminationReceipt,
                arg.as_str(),
            )?,
    ...
    if matches!(
        mode,
        Mode::PlanLatestCulminationReceipt
            | Mode::RenderLatestCulminationReceiptScript
            | Mode::RunLatestCulminationReceipt
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestCulminationReceiptScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestCulminationReceipt | Mode::VerifyLatestCulminationReceipt
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestCulminationReceipt => plan_latest_completion_certificate_report(&config)?,
        Mode::RenderLatestCulminationReceiptScript => {
            render_latest_completion_certificate_script_report(&config)?
        }
        Mode::RunLatestCulminationReceipt => run_latest_culmination_receipt_report(&config)?,
        Mode::VerifyLatestCulminationReceipt => verify_latest_culmination_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_latest_completion_certificate_report(
    config: &Config,
) -> Result<PackageCulminationReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    let suggested_session_dir = config.session_dir.clone().unwrap_or_else(|| {
        root.join("tiny_live_activation_package_culmination_receipt_latest.session")
    });
    let paths = culmination_receipt_latest_paths(&suggested_session_dir);

    match resolve_latest_completion_certificate(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            "plan_latest_culmination_receipt",
            TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-culmination-receipt lineage via accepted latest-completion-certificate session {}, exact decision-packet confirmation anchor {}, and the accepted culmination-receipt contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_completion_certificate_latest_session_dir.display(),
...
fn render_latest_completion_certificate_script_report(
    config: &Config,
) -> Result<PackageCulminationReceiptLatestReport> {
    let root = config
        .root
        ...
        Ok(resolved) => {
            write_script(
                ...
                &format!(
                    ...
                    run_latest = run_latest_completion_certificate_command_summary(
                        root,
                        &paths.nested_completion_certificate_latest_session_dir,
                    ),
                    verify_latest = verify_latest_completion_certificate_command_summary(
                        &paths.nested_completion_certificate_latest_session_dir,
                    ),
                    run_handoff_bundle = downstream_culmination_receipt_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_completion_certificate_session_dir(&paths),
                        &paths.nested_culmination_receipt_session_dir,
                    ),
                    verify_handoff_bundle = verify_completion_certificate_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_completion_certificate_session_dir(&paths),
                        &paths.nested_culmination_receipt_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                "render_latest_culmination_receipt_script",
                TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestScriptRendered,
                format!(
                    "rendered latest-culmination-receipt handoff script to {} using the accepted latest-completion-certificate lineage and the accepted culmination-receipt contract",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_culmination_receipt_report(
    config: &Config,
) -> Result<PackageCulminationReceiptLatestReport> {
    let root = config
        .root
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-culmination-receipt session dir {}",
            session_dir.display()
        );
    }
    let paths = culmination_receipt_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-culmination-receipt")?;

    let resolution = resolve_latest_completion_certificate(root, &paths);
    match resolution {
        Ok(resolved) => {
            run_latest_completion_certificate_with_resolution(root, session_dir, &paths, &resolved)
        }
        Err(failure) => {
            run_latest_completion_certificate_failure(root, session_dir, &paths, &failure)
        }
    }
}

fn verify_latest_culmination_receipt_report(
    config: &Config,
) -> Result<PackageCulminationReceiptLatestReport> {
    let session_dir = config
        .session_dir
    ...
    let session: PackageCulminationReceiptLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_culmination_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-culmination-receipt session artifact {}: {error}",
    ...
    let status: PackageCulminationReceiptLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_culmination_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-culmination-receipt status artifact {}: {error}",
    ...
    let stored_report: PackageCulminationReceiptLatestReport = match load_json(&paths.report_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_culmination_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-culmination-receipt report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-culmination-receipt root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-culmination-receipt session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-culmination-receipt status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-culmination-receipt report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_completion_certificate_latest_session_dir,
        &paths
            .nested_completion_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-culmination-receipt session nested_completion_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_completion_certificate_latest_session_dir,
        &paths
            .nested_completion_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-culmination-receipt status nested_completion_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-culmination-receipt report nested_completion_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_culmination_receipt_session_dir,
        &paths
            .nested_culmination_receipt_session_dir
            .display()
            .to_string(),
        "latest-culmination-receipt session nested_culmination_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_culmination_receipt_session_dir,
        &paths
            .nested_culmination_receipt_session_dir
            .display()
            .to_string(),
        "latest-culmination-receipt status nested_culmination_receipt_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-culmination-receipt report nested_culmination_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-culmination-receipt session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-culmination-receipt status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-culmination-receipt report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .culmination_receipt_latest_session_path
            .as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-culmination-receipt report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .culmination_receipt_latest_status_path
            .as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-culmination-receipt report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .culmination_receipt_latest_report_path
            .as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-culmination-receipt report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-culmination-receipt session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-culmination-receipt status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-culmination-receipt report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_culmination_receipt",
        "latest-culmination-receipt stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-culmination-receipt stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-culmination-receipt stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-culmination-receipt stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-culmination-receipt stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report
            .latest_completion_certificate_plan_verdict
            .as_deref(),
        status.latest_completion_certificate_plan_verdict.as_deref(),
        "stored report latest_completion_certificate_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .latest_completion_certificate_verdict
            .as_deref(),
        status.latest_completion_certificate_verdict.as_deref(),
        "stored report latest_completion_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.completion_certificate_verdict.as_deref(),
        status.completion_certificate_verdict.as_deref(),
        "stored report completion_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: CompletionCertificateLatestReportView =
                load_required_step_json(
                    &status.latest_completion_certificate_plan_step,
                    &paths.latest_completion_certificate_plan_report_path,
                    "latest_completion_certificate_plan_step",
                    &mut mismatches,
                )?;
            compare_step_artifact_against_report_metadata(
                status.latest_completion_certificate_plan_step.as_ref(),
                &paths.latest_completion_certificate_plan_report_path,
                "plan_latest_completion_certificate",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_completion_certificate_plan_step",
                &mut mismatches,
            );
            compare_completion_certificate_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_completion_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_completion_certificate_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let stored_latest_run: CompletionCertificateLatestReportView = load_required_step_json(
                &status.latest_completion_certificate_step,
                &paths.latest_completion_certificate_report_path,
                "latest_completion_certificate_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_completion_certificate_step.as_ref(),
                &paths.latest_completion_certificate_report_path,
                "run_latest_completion_certificate",
                &stored_latest_run.verdict,
                &stored_latest_run.reason,
                stored_latest_run.generated_at,
                "stored latest_completion_certificate_step",
                &mut mismatches,
            );
            compare_completion_certificate_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_completion_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_completion_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let actual_latest_run: CompletionCertificateLatestReportView = match load_json(
                &nested_completion_certificate_latest_report_path(
                    &paths.nested_completion_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed loading actual nested latest-completion-certificate report under {}: {error}",
                        paths.nested_completion_certificate_latest_session_dir.display()
                    ));
                    fallback_completion_certificate_latest_copy()
                }
            };
            compare_completion_certificate_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                COMPLETION_CERTIFICATE_LATEST_BIN,
                &latest_completion_certificate_verify_args(
                    &paths.nested_completion_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-completion-certificate session under {}: {error}",
                        paths.nested_completion_certificate_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: CompletionCertificateLatestReportView = serde_json::from_value(
                    latest_verify_raw,
                )
                .context("failed parsing nested latest-completion-certificate verify json")?;
                if latest_verify.verdict != COMPLETION_CERTIFICATE_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-completion-certificate verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: CulminationReceiptReportView = load_required_step_json(
                &status.completion_certificate_step,
                &paths.culmination_receipt_report_path,
                "culmination_receipt_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.completion_certificate_step.as_ref(),
                &paths.culmination_receipt_report_path,
                "run_live_package_culmination_receipt",
                &stored_handoff_bundle.verdict,
                &stored_handoff_bundle.reason,
                stored_handoff_bundle.generated_at,
                "stored culmination_receipt_step",
                &mut mismatches,
            );
            compare_completion_certificate_run_against_snapshot(
            ...
            compare_option_string(
                ...
                "stored status completion_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.completion_certificate_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: CulminationReceiptSessionView =
                match load_json(&nested_culmination_receipt_session_path(
                    &paths.nested_culmination_receipt_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                            "failed reading nested culmination-receipt session under {}: {error}",
                            paths.nested_culmination_receipt_session_dir.display()
                        ));
            ...
            let nested_handoff_bundle_status: CulminationReceiptStatusView =
                match load_json(&nested_culmination_receipt_status_path(
                    &paths.nested_culmination_receipt_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                            "failed reading nested culmination-receipt status under {}: {error}",
                            paths.nested_culmination_receipt_session_dir.display()
                        ));
            ...
            compare_completion_certificate_copy_against_nested_truth(
                &stored_handoff_bundle,
            ...
            let handoff_bundle_verify_raw = match run_json_command(
                CULMINATION_RECEIPT_BIN,
                &culmination_receipt_verify_args(
                    &resolved.snapshot,
                    &expected_downstream_completion_certificate_session_dir(&paths),
                    &paths.nested_culmination_receipt_session_dir,
                ),
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested culmination-receipt session under {}: {error}",
                        paths.nested_culmination_receipt_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: CulminationReceiptReportView =
                    serde_json::from_value(handoff_bundle_verify_raw)
                        .context("failed parsing nested culmination-receipt verify json")?;
                compare_option_string(
                    stored_handoff_bundle.registry_entry_session_dir.as_deref(),
                    handoff_bundle_verify.registry_entry_session_dir.as_deref(),
                    "stored culmination-receipt copy registry_entry_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored culmination-receipt copy notarization_receipt_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .consummation_record_summary
                        .as_deref(),
                    "stored culmination-receipt copy consummation_record_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .consummation_record_sha256
                        .as_deref(),
                    "stored culmination-receipt copy consummation_record_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .consummation_record_algorithm
                        .as_deref(),
                    "stored culmination-receipt copy consummation_record_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored culmination-receipt copy completion_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.completion_certificate_sha256.as_deref(),
                    handoff_bundle_verify.completion_certificate_sha256.as_deref(),
                    "stored culmination-receipt copy completion_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .reviewed_frozen_live_cutover_controller_command_summary
                        .as_deref(),
                    "stored culmination-receipt copy reviewed_frozen_live_cutover_controller_command_summary",
                    &mut mismatches,
                );
                if handoff_bundle_verify.verdict != CULMINATION_RECEIPT_VERIFY_OK {
                    mismatches.push(format!(
                        "nested culmination-receipt verification is non-green: {}",
                        handoff_bundle_verify.reason
                    ));
                }
            }

            let expected_result = completion_certificate_latest_result_from_nested_status(
                nested_handoff_bundle_status.result.as_str(),
            )
            .unwrap_or(
                LivePackageCulminationReceiptLatestResult::RefusedByDriftedCulminationReceiptContract,
            );
            if status.result != expected_result {
                mismatches.push(format!(
                    "stored latest-culmination-receipt result {} does not match nested culmination-receipt result {}",
            ...
            if status.activation_authorized {
                mismatches.push(
                    "stored status activation_authorized must remain false for latest-culmination-receipt"
                        .to_string(),
                );
            }
            compare_option_string(
        ...
        Err(failure) => {
            ...
            if failure.latest_completion_certificate_plan_raw.is_some() {
                let stored_latest_plan: CompletionCertificateLatestReportView =
                    load_required_step_json(
                        &status.latest_completion_certificate_plan_step,
                        &paths.latest_completion_certificate_plan_report_path,
                ...
                compare_option_string(
                    ...
                    failure
                        .latest_completion_certificate_plan_verdict
                        .as_deref(),
                    "stored status latest_completion_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report
                    ...
                    failure
                        .latest_completion_certificate_plan_verdict
                        .as_deref(),
                    "stored report latest_completion_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
                status.latest_completion_certificate_verdict.as_deref(),
                None,
                "stored status latest_completion_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
                    ...
                    .as_deref(),
                None,
                "stored report latest_completion_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.completion_certificate_verdict.as_deref(),
                None,
                "stored status completion_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.completion_certificate_verdict.as_deref(),
                None,
                "stored report completion_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestVerifyOk
    } else {
        TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-culmination-receipt session under {} is coherent with the current latest immutable chain and the accepted culmination-receipt contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-culmination-receipt verification failed".to_string())
    };

    Ok(PackageCulminationReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_culmination_receipt".to_string(),
        verdict,
        reason,
        ...
        culmination_receipt_latest_session_path: Some(paths.session_path.display().to_string()),
        culmination_receipt_latest_status_path: Some(paths.status_path.display().to_string()),
        culmination_receipt_latest_report_path: Some(paths.report_path.display().to_string()),
        latest_completion_certificate_plan_report_path: Some(
            paths
                .latest_completion_certificate_plan_report_path
                .display()
                .to_string(),
        ),
        latest_completion_certificate_report_path: Some(
            paths.latest_completion_certificate_report_path.display().to_string(),
        ),
        culmination_receipt_report_path: Some(paths.culmination_receipt_report_path.display().to_string()),
        nested_completion_certificate_latest_session_dir: Some(
            paths
                .nested_completion_certificate_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_culmination_receipt_session_dir: Some(
            paths.nested_culmination_receipt_session_dir.display().to_string(),
        ),
        ...
        latest_completion_certificate_plan_verdict: status.latest_completion_certificate_plan_verdict.clone(),
        latest_completion_certificate_verdict: status.latest_completion_certificate_verdict.clone(),
        completion_certificate_verdict: status.completion_certificate_verdict.clone(),
        ...
        resolve_latest_completion_certificate_command_summary: Some(
            session.resolve_latest_completion_certificate_command_summary.clone(),
        ),
        run_latest_completion_certificate_command_summary: Some(
            session.run_latest_completion_certificate_command_summary.clone(),
        ),
        verify_latest_completion_certificate_command_summary: Some(
            session.verify_latest_completion_certificate_command_summary.clone(),
        ),
        downstream_culmination_receipt_command_summary: Some(
            session.downstream_culmination_receipt_command_summary.clone(),
        ),
        verify_completion_certificate_command_summary: Some(session.verify_completion_certificate_command_summary.clone()),
        ...
        explicit_statement:
            "this verification proves only whether the latest immutable chain still resolves cleanly into the accepted culmination-receipt contract; it never runs the frozen controller"
                .to_string(),
    })
}

fn run_latest_completion_certificate_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackageCulminationReceiptLatestPaths,
    resolved: &ResolvedLatestCompletionCertificate,
) -> Result<PackageCulminationReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, Some(resolved), None);
    persist_json(&paths.session_path, &session)?;
    persist_json(
        &paths.latest_completion_certificate_plan_report_path,
        &resolved.latest_completion_certificate_plan_raw,
    )?;

    let latest_completion_certificate_plan_step = Some(step_artifact(
        &paths.latest_completion_certificate_plan_report_path,
        "plan_latest_completion_certificate",
        &resolved.latest_completion_certificate_plan.verdict,
        &resolved.latest_completion_certificate_plan.reason,
        resolved.latest_completion_certificate_plan.generated_at,
    ));

    let latest_completion_certificate_run_raw = match run_json_command(
        COMPLETION_CERTIFICATE_LATEST_BIN,
        &latest_completion_certificate_run_args(
            root,
            &paths.nested_completion_certificate_latest_session_dir,
        ),
    ) {
        ...
        Err(error) => {
            let status =
                refusal_status(
                    ...
                    LivePackageCulminationReceiptLatestResult::RefusedByDriftedCulminationReceiptContract,
                    format!(
                    "failed running nested latest-completion-certificate handoff under {}: {error}",
                    paths.nested_completion_certificate_latest_session_dir.display()
                ),
                    latest_completion_certificate_plan_step,
                    None,
    ...
    persist_json(
        &paths.latest_completion_certificate_report_path,
        &latest_completion_certificate_run_raw,
    )?;
    let latest_completion_certificate_run: CompletionCertificateLatestReportView =
        serde_json::from_value(latest_completion_certificate_run_raw)
            .context("failed parsing nested latest-completion-certificate run json")?;
    let latest_completion_certificate_step = Some(step_artifact(
        &paths.latest_completion_certificate_report_path,
        "run_latest_completion_certificate",
        &latest_completion_certificate_run.verdict,
        &latest_completion_certificate_run.reason,
        latest_completion_certificate_run.generated_at,
    ));

    if let Some(terminal_failure) =
        latest_completion_certificate_terminal_failure(&latest_completion_certificate_run)
    {
        let status = refusal_status(
            ...
            Some(resolved),
            terminal_failure,
            latest_completion_certificate_run.reason.clone(),
            latest_completion_certificate_plan_step,
            latest_completion_certificate_step,
            None,
        );
    ...
    compare_completion_certificate_latest_run_against_snapshot(
        &latest_completion_certificate_run,
        &resolved.snapshot,
        &paths.nested_completion_certificate_latest_session_dir,
        &mut run_mismatches,
    );
    ...
    let latest_completion_certificate_verify: CompletionCertificateLatestReportView =
        match run_json_command(
            ...
            Err(error) => {
                ...
                refusal_status(
                    ...
                    LivePackageCulminationReceiptLatestResult::RefusedByDriftedCulminationReceiptContract,
                    format!(
                    "failed verifying nested latest-completion-certificate handoff under {}: {error}",
                    paths.nested_completion_certificate_latest_session_dir.display()
                ),
                    latest_completion_certificate_plan_step,
                    latest_completion_certificate_step,
    ...
    if latest_completion_certificate_verify.verdict != COMPLETION_CERTIFICATE_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            LivePackageCulminationReceiptLatestResult::RefusedByDriftedCulminationReceiptContract,
            latest_completion_certificate_verify.reason,
            latest_completion_certificate_plan_step,
            latest_completion_certificate_step,
            None,
        );
    ...
    let release_capsule_run_raw = match run_json_command(
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageCulminationReceiptLatestResult::RefusedByDriftedCulminationReceiptContract,
                format!(
                    "failed running downstream culmination-receipt contract under {}: {error}",
                    paths.nested_culmination_receipt_session_dir.display()
                ),
                latest_completion_certificate_plan_step,
                latest_completion_certificate_step,
    ...
    let provenance_certificate_run: CulminationReceiptReportView =
        serde_json::from_value(release_capsule_run_raw)
            .context("failed parsing nested culmination-receipt run json")?;
    let completion_certificate_step = Some(step_artifact(
        &paths.culmination_receipt_report_path,
        "run_live_package_culmination_receipt",
    ...
    let provenance_certificate_verify: CulminationReceiptReportView = match run_json_command(
        CULMINATION_RECEIPT_BIN,
        &culmination_receipt_verify_args(
            &resolved.snapshot,
            &expected_downstream_completion_certificate_session_dir(paths),
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageCulminationReceiptLatestResult::RefusedByDriftedCulminationReceiptContract,
                format!(
                    "failed verifying downstream culmination-receipt contract under {}: {error}",
                    paths.nested_culmination_receipt_session_dir.display()
                ),
                latest_completion_certificate_plan_step,
                latest_completion_certificate_step,
    ...
    if provenance_certificate_verify.verdict != CULMINATION_RECEIPT_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    let result = completion_certificate_latest_result_from_nested_status(
        provenance_certificate_run
            .result
            .as_deref()
            .unwrap_or_default(),
    ...
    let status = PackageCulminationReceiptLatestStatus {
        status_version: CULMINATION_RECEIPT_LATEST_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nested_completion_certificate_latest_session_dir: paths
            .nested_completion_certificate_latest_session_dir
            .display()
            .to_string(),
        nested_culmination_receipt_session_dir: paths
            .nested_culmination_receipt_session_dir
            .display()
            .to_string(),
        ...
        latest_completion_certificate_plan_verdict: Some(
            resolved.latest_completion_certificate_plan.verdict.clone(),
        ),
        latest_completion_certificate_verdict: Some(
            latest_completion_certificate_run.verdict.clone(),
        ),
        completion_certificate_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_completion_certificate_failure(
    root: &Path,
    session_dir: &Path,
    paths: &PackageCulminationReceiptLatestPaths,
    failure: &LatestCompletionCertificateResolutionFailure,
) -> Result<PackageCulminationReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, None, Some(failure));
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_completion_certificate_plan_raw {
        persist_json(&paths.latest_completion_certificate_plan_report_path, value)?;
    }
...
fn resolve_latest_completion_certificate(
    root: &Path,
    paths: &PackageCulminationReceiptLatestPaths,
) -> std::result::Result<
    ResolvedLatestCompletionCertificate,
    LatestCompletionCertificateResolutionFailure,
> {
    let latest_completion_certificate_plan_raw = match run_json_command(
        COMPLETION_CERTIFICATE_LATEST_BIN,
        &latest_completion_certificate_plan_args(
            root,
            &paths.nested_completion_certificate_latest_session_dir,
        ),
    ) {
        Ok(value) => value,
        Err(error) => {
            return Err(LatestCompletionCertificateResolutionFailure {
                ...
                snapshot: None,
                latest_completion_certificate_plan_raw: None,
                latest_completion_certificate_plan_verdict: None,
                latest_completion_certificate_plan_reason: None,
                latest_completion_certificate_plan_generated_at: None,
            });
        }
    };
    let latest_completion_certificate_plan: CompletionCertificateLatestReportView =
        serde_json::from_value(latest_completion_certificate_plan_raw.clone()).map_err(
            |error| LatestCompletionCertificateResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-culmination-receipt upstream latest-completion-certificate plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_completion_certificate_plan_raw: Some(
                    latest_completion_certificate_plan_raw.clone(),
                ),
                latest_completion_certificate_plan_verdict: None,
                latest_completion_certificate_plan_reason: None,
                latest_completion_certificate_plan_generated_at: None,
    ...
    let snapshot =
        parse_snapshot_from_latest_completion_certificate(&latest_completion_certificate_plan).ok();
    if latest_completion_certificate_plan.verdict != COMPLETION_CERTIFICATE_LATEST_PLAN_READY {
        return Err(LatestCompletionCertificateResolutionFailure {
            kind: map_completion_certificate_latest_failure_kind(
                &latest_completion_certificate_plan.verdict,
            ),
            reason: format!(
                "latest-culmination-receipt resolution refused because nested latest-completion-certificate reported: {}",
                latest_completion_certificate_plan.reason
            ),
            snapshot,
            latest_completion_certificate_plan_raw: Some(latest_completion_certificate_plan_raw),
            latest_completion_certificate_plan_verdict: Some(
                latest_completion_certificate_plan.verdict,
            ),
            latest_completion_certificate_plan_reason: Some(
                latest_completion_certificate_plan.reason,
            ),
            latest_completion_certificate_plan_generated_at: Some(
                latest_completion_certificate_plan.generated_at,
            ),
        });
    }

    let snapshot =
        parse_snapshot_from_latest_completion_certificate(&latest_completion_certificate_plan)
            .map_err(|error| LatestCompletionCertificateResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!("latest-culmination-receipt resolution failed: {error}"),
                snapshot: None,
                latest_completion_certificate_plan_raw: Some(
                    latest_completion_certificate_plan_raw.clone(),
                ),
                latest_completion_certificate_plan_verdict: Some(
                    latest_completion_certificate_plan.verdict.clone(),
                ),
                latest_completion_certificate_plan_reason: Some(
                    latest_completion_certificate_plan.reason.clone(),
                ),
                latest_completion_certificate_plan_generated_at: Some(
                    latest_completion_certificate_plan.generated_at,
                ),
    ...
    let mut mismatches = Vec::new();
    compare_completion_certificate_latest_plan_against_snapshot(
        &latest_completion_certificate_plan,
        &snapshot,
        &paths.nested_completion_certificate_latest_session_dir,
        &mut mismatches,
    );
    if !mismatches.is_empty() {
        return Err(LatestCompletionCertificateResolutionFailure {
            kind: ResolutionFailureKind::DriftedCulminationReceiptContract,
            reason: format!(
                "latest-culmination-receipt drift detected: {}",
                mismatches[0]
            ),
    ...
    Ok(ResolvedLatestCompletionCertificate {
        snapshot,
        latest_completion_certificate_plan_raw,
        latest_completion_certificate_plan,
    })
}

fn parse_snapshot_from_latest_completion_certificate(
    view: &CompletionCertificateLatestReportView,
) -> Result<LatestCompletionCertificateSnapshot> {
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-culmination-receipt nested latest-completion-certificate plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-culmination-receipt nested latest-completion-certificate plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestCompletionCertificateSnapshot {
        latest_top_step_name,
        ...
        wrapper_timeout_ms: view.wrapper_timeout_ms.ok_or_else(|| {
            anyhow!(
                "latest-culmination-receipt nested latest-completion-certificate plan is missing wrapper_timeout_ms"
            )
        })?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!(
                "latest-culmination-receipt nested latest-completion-certificate plan is missing service_status_max_staleness_ms"
            )
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackageCulminationReceiptLatestPaths,
    resolved: &ResolvedLatestCompletionCertificate,
) -> PackageCulminationReceiptLatestReport {
    PackageCulminationReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        latest_completion_certificate_report_path: Some(
            paths
                .latest_completion_certificate_report_path
                .display()
                .to_string(),
        ),
        culmination_receipt_report_path: Some(
            paths.culmination_receipt_report_path.display().to_string(),
        ),
        nested_completion_certificate_latest_session_dir: Some(
            paths
                .nested_completion_certificate_latest_session_dir
        ...
        latest_completion_certificate_verdict: None,
        completion_certificate_verdict: None,
        result: None,
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_completion_certificate_command_summary: Some(
            resolve_latest_completion_certificate_command_summary(
                root,
                &paths.nested_completion_certificate_latest_session_dir,
            ),
        ),
        run_latest_completion_certificate_command_summary: Some(
            run_latest_completion_certificate_command_summary(
                root,
                &paths.nested_completion_certificate_latest_session_dir,
            ),
        ),
        verify_latest_completion_certificate_command_summary: Some(
            verify_latest_completion_certificate_command_summary(
                &paths.nested_completion_certificate_latest_session_dir,
            ),
        ),
        downstream_culmination_receipt_command_summary: Some(
            downstream_culmination_receipt_command_summary(
                &resolved.snapshot,
                &expected_downstream_completion_certificate_session_dir(paths),
                &paths.nested_culmination_receipt_session_dir,
            ),
...
fn report_from_resolution_failure(
    ...
    PackageCulminationReceiptLatestReport {
        ...
        latest_completion_certificate_plan_verdict: failure
            .latest_completion_certificate_plan_verdict
            .clone(),
        latest_completion_certificate_verdict: None,
        completion_certificate_verdict: None,
        ...
        downstream_culmination_receipt_command_summary: snapshot.map(|value| {
            downstream_culmination_receipt_command_summary(
                value,
                &expected_downstream_completion_certificate_session_dir(paths),
                &paths.nested_culmination_receipt_session_dir,
        ...
        verify_completion_certificate_command_summary: snapshot.map(|value| {
            verify_completion_certificate_command_summary(
                value,
                &expected_downstream_completion_certificate_session_dir(paths),
                &paths.nested_culmination_receipt_session_dir,
...
fn planned_session(
    ...
    session_dir: &Path,
    paths: &PackageCulminationReceiptLatestPaths,
    resolved: Option<&ResolvedLatestCompletionCertificate>,
    failure: Option<&LatestCompletionCertificateResolutionFailure>,
) -> PackageCulminationReceiptLatestSession {
    let snapshot = resolved
        .map(|value| &value.snapshot)
        .or_else(|| failure.and_then(|value| value.snapshot.as_ref()));
    PackageCulminationReceiptLatestSession {
        session_version: CULMINATION_RECEIPT_LATEST_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        root: root.display().to_string(),
        session_dir: session_dir.display().to_string(),
        nested_completion_certificate_latest_session_dir: paths
        ...
        downstream_culmination_receipt_command_summary: snapshot
            .map(|value| {
                downstream_culmination_receipt_command_summary(
                    value,
                    &expected_downstream_completion_certificate_session_dir(paths),
        ...
        verify_completion_certificate_command_summary: snapshot
            .map(|value| {
                verify_completion_certificate_command_summary(
                    value,
                    &expected_downstream_completion_certificate_session_dir(paths),
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackageCulminationReceiptLatestPaths,
    session: &PackageCulminationReceiptLatestSession,
    status: &PackageCulminationReceiptLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackageCulminationReceiptLatestReport {
    PackageCulminationReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        nested_culmination_receipt_session_dir: Some(
            session.nested_culmination_receipt_session_dir.clone(),
        ),
        downstream_decision_packet_session_dir: Some(
            session.downstream_decision_packet_session_dir.clone(),
        ),
        latest_completion_certificate_plan_verdict: status
            .latest_completion_certificate_plan_verdict
            .clone(),
        latest_completion_certificate_verdict: status.latest_completion_certificate_verdict.clone(),
        completion_certificate_verdict: status.completion_certificate_verdict.clone(),
...
fn culmination_receipt_latest_paths(session_dir: &Path) -> PackageCulminationReceiptLatestPaths {
    PackageCulminationReceiptLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_culmination_receipt_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_culmination_receipt_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_culmination_receipt_latest.report.json"),
        latest_completion_certificate_plan_report_path: session_dir.join(
            "tiny_live_activation_package_culmination_receipt_latest.completion_certificate_latest_plan.report.json",
        ),
        latest_completion_certificate_report_path: session_dir.join(
            "tiny_live_activation_package_culmination_receipt_latest.completion_certificate_latest.report.json",
        ),
        culmination_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_culmination_receipt_latest.culmination_receipt.report.json",
        ),
        nested_completion_certificate_latest_session_dir: session_dir
            .join("tiny_live_activation_package_culmination_receipt_latest.completion_certificate_latest_session"),
        nested_culmination_receipt_session_dir: session_dir.join(
            "tiny_live_activation_package_culmination_receipt_latest.culmination_receipt_session",
        ),
    }
}
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_completion_certificate_latest_session_dir
        .join("tiny_live_activation_package_completion_certificate_latest.decision_packet_session")
}

fn expected_downstream_completion_certificate_session_dir(
    paths: &PackageCulminationReceiptLatestPaths,
) -> PathBuf {
    paths.nested_completion_certificate_latest_session_dir.join(
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_completion_certificate_latest_session_dir
        .join("tiny_live_activation_package_completion_certificate_latest.review_receipt_session")
}

fn latest_completion_certificate_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-culmination-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn latest_completion_certificate_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-culmination-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn latest_completion_certificate_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-culmination-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn culmination_receipt_run_args(
    snapshot: &LatestCompletionCertificateSnapshot,
    completion_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        "--completion-certificate-session-dir".to_string(),
        completion_certificate_session_dir.display().to_string(),
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-culmination-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn culmination_receipt_verify_args(
    snapshot: &LatestCompletionCertificateSnapshot,
    completion_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-live-package-culmination-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn resolve_latest_completion_certificate_command_summary(
    root: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{COMPLETION_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --plan-latest-culmination-receipt --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn run_latest_completion_certificate_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{COMPLETION_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --run-latest-culmination-receipt --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn verify_latest_completion_certificate_command_summary(session_dir: &Path) -> String {
    format!(
        "{COMPLETION_CERTIFICATE_LATEST_BIN} --session-dir {} --verify-latest-culmination-receipt --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_culmination_receipt_command_summary(
    snapshot: &LatestCompletionCertificateSnapshot,
    completion_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{CULMINATION_RECEIPT_BIN} --completion-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-culmination-receipt --json",
        shell_escape_path(completion_certificate_session_dir),
...
fn verify_completion_certificate_command_summary(
    ...
    format!(
        "{CULMINATION_RECEIPT_BIN} --completion-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-culmination-receipt --json",
        shell_escape_path(completion_certificate_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
...
fn nested_completion_certificate_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_completion_certificate_latest.report.json")
}

fn nested_culmination_receipt_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_culmination_receipt.session.json")
}

fn nested_culmination_receipt_status_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_culmination_receipt.status.json")
}
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-culmination-receipt script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageCulminationReceiptLatestStepArtifact {
    PackageCulminationReceiptLatestStepArtifact {
...
fn canonical_wrapper_mode(mode: &str) -> &str {
    match mode {
        "plan_latest_culmination_receipt" => "plan_latest_culmination_receipt",
        "render_latest_culmination_receipt_script" => "render_latest_culmination_receipt_script",
        "run_latest_culmination_receipt" => "run_latest_culmination_receipt",
        "verify_latest_culmination_receipt" => "verify_latest_culmination_receipt",
        other => other,
    }
}

fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackageCulminationReceiptLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-culmination-receipt status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-culmination-receipt status"
        ));
        return;
    };
    compare_string(
...
fn compare_completion_certificate_latest_plan_against_snapshot(
    stored: &CompletionCertificateLatestReportView,
    snapshot: &LatestCompletionCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        ...
        Some(COMPLETION_CERTIFICATE_LATEST_PLAN_READY),
        "stored latest-completion-certificate plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_completion_certificate",
        "stored latest-completion-certificate plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-completion-certificate plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-completion-certificate plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-completion-certificate plan session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-completion-certificate plan downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-completion-certificate plan package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-completion-certificate plan install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-completion-certificate plan target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-completion-certificate plan backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-completion-certificate plan wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-completion-certificate plan service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-completion-certificate plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-completion-certificate plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-completion-certificate plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-completion-certificate plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_completion_certificate_latest_run_against_snapshot(
    stored: &CompletionCertificateLatestReportView,
    snapshot: &LatestCompletionCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_completion_certificate",
        "stored latest-completion-certificate run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-completion-certificate run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-completion-certificate run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-completion-certificate run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-completion-certificate run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-completion-certificate run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-completion-certificate run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-completion-certificate run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-completion-certificate run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-completion-certificate run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-completion-certificate run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-completion-certificate run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-completion-certificate run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-completion-certificate run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-completion-certificate run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_completion_certificate_latest_copy_matches_actual(
    stored: &CompletionCertificateLatestReportView,
    actual: &CompletionCertificateLatestReportView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        Some(stored.verdict.as_str()),
        Some(actual.verdict.as_str()),
        "stored latest-completion-certificate copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-completion-certificate copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-completion-certificate copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-completion-certificate copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-completion-certificate copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-completion-certificate copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-completion-certificate copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-completion-certificate copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-completion-certificate copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-completion-certificate copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-completion-certificate copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-completion-certificate copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_completion_certificate_run_against_snapshot(
    stored: &CulminationReceiptReportView,
    snapshot: &LatestCompletionCertificateSnapshot,
    completion_certificate_session_dir: &Path,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_live_package_culmination_receipt",
        "stored culmination-receipt run mode",
        mismatches,
    );
    compare_string(
        &stored.completion_certificate_session_dir,
        &completion_certificate_session_dir.display().to_string(),
        "stored culmination-receipt run completion_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored culmination-receipt run decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored culmination-receipt run execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored culmination-receipt run turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored culmination-receipt run launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored culmination-receipt run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored culmination-receipt run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored culmination-receipt run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored culmination-receipt run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored culmination-receipt run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored culmination-receipt run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored culmination-receipt run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.culmination_receipt_session_path.as_deref(),
        Some(
            nested_culmination_receipt_session_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored culmination-receipt run culmination_receipt_session_path",
        mismatches,
    );
    compare_option_string(
        stored.culmination_receipt_status_path.as_deref(),
        Some(
            nested_culmination_receipt_status_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored culmination-receipt run culmination_receipt_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_completion_certificate_report_path.as_deref(),
        Some(
            session_dir
                .join(
                    "tiny_live_activation_package_culmination_receipt.completion_certificate.report.json",
                )
                .display()
                .to_string()
                .as_str(),
        ),
        "stored culmination-receipt run archived_completion_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        stored
            ...
            .as_deref(),
        Some(
            culmination_receipt_verify_completion_certificate_command_summary(
                completion_certificate_session_dir,
            )
            .as_str(),
        ),
        "stored culmination-receipt run verify_completion_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored culmination-receipt run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_completion_certificate_copy_against_nested_truth(
    stored: &CulminationReceiptReportView,
    nested_session: &CulminationReceiptSessionView,
    nested_status: &CulminationReceiptStatusView,
    mismatches: &mut Vec<String>,
) {
    compare_string(
        &stored.completion_certificate_session_dir,
        &nested_session.completion_certificate_session_dir,
        "stored culmination-receipt copy completion_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.registry_entry_session_dir.as_deref(),
        Some(nested_session.registry_entry_session_dir.as_str()),
        "stored culmination-receipt copy registry_entry_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.notarization_receipt_session_dir.as_deref(),
        Some(nested_session.notarization_receipt_session_dir.as_str()),
        "stored culmination-receipt copy notarization_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.provenance_certificate_session_dir.as_deref(),
        Some(nested_session.provenance_certificate_session_dir.as_str()),
        "stored culmination-receipt copy provenance_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.attestation_seal_session_dir.as_deref(),
        Some(nested_session.attestation_seal_session_dir.as_str()),
        "stored culmination-receipt copy attestation_seal_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.release_capsule_session_dir.as_deref(),
        Some(nested_session.release_capsule_session_dir.as_str()),
        "stored culmination-receipt copy release_capsule_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.activation_ticket_session_dir.as_deref(),
        Some(nested_session.activation_ticket_session_dir.as_str()),
        "stored culmination-receipt copy activation_ticket_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.review_receipt_session_dir.as_deref(),
        Some(nested_session.review_receipt_session_dir.as_str()),
        "stored culmination-receipt copy review_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.handoff_bundle_session_dir.as_deref(),
        Some(nested_session.handoff_bundle_session_dir.as_str()),
        "stored culmination-receipt copy handoff_bundle_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.decision_packet_session_dir.as_deref(),
        Some(nested_session.decision_packet_session_dir.as_str()),
        "stored culmination-receipt copy decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.execute_frozen_session_dir.as_deref(),
        Some(nested_session.execute_frozen_session_dir.as_str()),
        "stored culmination-receipt copy execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.turn_green_session_dir.as_deref(),
        Some(nested_session.turn_green_session_dir.as_str()),
        "stored culmination-receipt copy turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.launch_packet_session_dir.as_deref(),
        Some(nested_session.launch_packet_session_dir.as_str()),
        "stored culmination-receipt copy launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(nested_session.session_dir.as_str()),
        "stored culmination-receipt copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        Some(nested_status.result.as_str()),
        "stored culmination-receipt copy result",
        mismatches,
    );
    compare_option_string(
        Some(stored.verdict.as_str()),
        completion_certificate_run_verdict_for_result(&nested_status.result),
        "stored culmination-receipt copy verdict",
        mismatches,
    );
    compare_option_string(
        stored.handoff_bundle_result.as_deref(),
        nested_status.handoff_bundle_result.as_deref(),
        "stored culmination-receipt copy handoff_bundle_result",
        mismatches,
    );
    compare_option_string(
        stored.decision_packet_result.as_deref(),
        nested_status.decision_packet_result.as_deref(),
        "stored culmination-receipt copy decision_packet_result",
        mismatches,
    );
    compare_option_string(
        stored.execute_frozen_result.as_deref(),
        nested_status.execute_frozen_result.as_deref(),
        "stored culmination-receipt copy execute_frozen_result",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        nested_status.current_pre_activation_gate_verdict.as_deref(),
        "stored culmination-receipt copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        nested_status.current_pre_activation_gate_reason.as_deref(),
        "stored culmination-receipt copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored culmination-receipt copy verify_completion_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored culmination-receipt copy release_capsule_digest_manifest_sha256",
        mismatches,
    );
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_completion_certificate_command_summary,
        &resolve_latest_completion_certificate_command_summary(
            root,
            &paths.nested_completion_certificate_latest_session_dir,
        ),
        "stored session resolve_latest_completion_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.run_latest_completion_certificate_command_summary,
        &run_latest_completion_certificate_command_summary(
            root,
            &paths.nested_completion_certificate_latest_session_dir,
        ),
        "stored session run_latest_completion_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_latest_completion_certificate_command_summary,
        &verify_latest_completion_certificate_command_summary(
            &paths.nested_completion_certificate_latest_session_dir,
        ),
        "stored session verify_latest_completion_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.downstream_culmination_receipt_command_summary,
        &downstream_culmination_receipt_command_summary(
            snapshot,
            &expected_downstream_completion_certificate_session_dir(paths),
            &paths.nested_culmination_receipt_session_dir,
        ),
        "stored session downstream_culmination_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_completion_certificate_command_summary,
        &verify_completion_certificate_command_summary(
            snapshot,
            &expected_downstream_completion_certificate_session_dir(paths),
            &paths.nested_culmination_receipt_session_dir,
        ),
        "stored session verify_completion_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_completion_certificate_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_culmination_receipt_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_culmination_receipt_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackageCulminationReceiptLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackageCulminationReceiptLatestPaths,
    snapshot: &LatestCompletionCertificateSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_completion_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report run_latest_completion_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report verify_latest_completion_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report downstream_culmination_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report verify_completion_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
    ...
    compare_option_string(
        report.culmination_receipt_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "stored report culmination_receipt_latest_session_path",
        mismatches,
    );
    compare_option_string(
        report.culmination_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "stored report culmination_receipt_latest_status_path",
        mismatches,
    );
    compare_option_string(
        report.culmination_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "stored report culmination_receipt_latest_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report latest_completion_certificate_plan_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_completion_certificate_report_path.as_deref(),
        Some(
            paths
                .latest_completion_certificate_report_path
                .display()
        ...
        "stored report latest_completion_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        report.culmination_receipt_report_path.as_deref(),
        Some(
            paths
                .culmination_receipt_report_path
                .display()
                .to_string()
                .as_str(),
        ),
        "stored report culmination_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report nested_completion_certificate_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_culmination_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_culmination_receipt_session_dir
                .display()
        ...
        "stored report nested_culmination_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_completion_certificate_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_culmination_receipt_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_culmination_receipt_session_dir on failure",
        mismatches,
    );
...
fn map_completion_certificate_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_completion_certificate_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_completion_certificate_latest_refused_by_drifted_completion_certificate_contract" => {
            ResolutionFailureKind::DriftedCulminationReceiptContract
        }
        _ => ResolutionFailureKind::InvalidLatestChain,
    }
}

fn latest_completion_certificate_terminal_failure(
    report: &CompletionCertificateLatestReportView,
) -> Option<LivePackageCulminationReceiptLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackageCulminationReceiptLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackageCulminationReceiptLatestResult::RefusedByInvalidLatestChain)
        }
        Some("refused_by_drifted_completion_certificate_contract") => Some(
            LivePackageCulminationReceiptLatestResult::RefusedByDriftedCulminationReceiptContract,
        ),
        _ => None,
    }
...
fn completion_certificate_latest_result_from_nested_status(
    result: &str,
) -> Option<LivePackageCulminationReceiptLatestResult> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => Some(
            LivePackageCulminationReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen,
        ),
        "refused_now_by_stage3" => {
            Some(LivePackageCulminationReceiptLatestResult::RefusedNowByStage3)
        }
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackageCulminationReceiptLatestResult::RefusedNowByPreActivationGate)
        }
...
fn completion_certificate_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => Some(
            "tiny_live_package_culmination_receipt_ready_for_manual_execution_when_gate_turns_green",
        ),
        "refused_now_by_stage3" => Some("tiny_live_package_culmination_receipt_refused_now_by_stage3"),
        "refused_now_by_pre_activation_gate" => {
            Some("tiny_live_package_culmination_receipt_refused_now_by_pre_activation_gate")
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_culmination_receipt_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}
...
fn result_from_failure_kind(
    kind: ResolutionFailureKind,
) -> LivePackageCulminationReceiptLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackageCulminationReceiptLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackageCulminationReceiptLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedCulminationReceiptContract => {
            LivePackageCulminationReceiptLatestResult::RefusedByDriftedCulminationReceiptContract
        }
...
fn verdict_for_result(
    result: LivePackageCulminationReceiptLatestResult,
) -> TinyLivePackageCulminationReceiptLatestVerdict {
    match result {
        LivePackageCulminationReceiptLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestRefusedByMissingLatestChain
        }
        LivePackageCulminationReceiptLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestRefusedByInvalidLatestChain
        }
        LivePackageCulminationReceiptLatestResult::RefusedByDriftedCulminationReceiptContract => {
            TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestRefusedByDriftedCulminationReceiptContract
        }
        LivePackageCulminationReceiptLatestResult::RefusedNowByStage3 => {
            TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestRefusedNowByStage3
        }
        LivePackageCulminationReceiptLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestRefusedNowByPreActivationGate
        }
        LivePackageCulminationReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackageCulminationReceiptLatestResult) -> String {
    match result {
        LivePackageCulminationReceiptLatestResult::RefusedByMissingLatestChain => {
            "no immutable clerestory chain is available yet; do not manually hunt completion-certificate or culmination-receipt lineage by hand".to_string()
        }
        LivePackageCulminationReceiptLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-culmination-receipt / culmination-receipt path".to_string()
        }
        LivePackageCulminationReceiptLatestResult::RefusedByDriftedCulminationReceiptContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-culmination-receipt / culmination-receipt contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackageCulminationReceiptLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-culmination-receipt refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackageCulminationReceiptLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-culmination-receipt refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackageCulminationReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            "the latest immutable chain now resolves into the accepted culmination-receipt contract and one bounded latest culmination-receipt handoff is ready for manual execution when gate truth turns green; this wrapper still did not run or authorize the frozen controller".to_string()
        }
    }
}
...
fn culmination_receipt_verify_completion_certificate_command_summary(
    completion_certificate_session_dir: &Path,
) -> String {
    format!(
        "verify immutable completion-certificate session under {} before freezing the final completion certificate",
...
fn refusal_status(
    ...
    latest_completion_certificate_step: Option<PackageCulminationReceiptLatestStepArtifact>,
    completion_certificate_step: Option<PackageCulminationReceiptLatestStepArtifact>,
) -> PackageCulminationReceiptLatestStatus {
    ...
    PackageCulminationReceiptLatestStatus {
        ...
        result,
        reason,
        latest_completion_certificate_plan_verdict: resolved
            .map(|value| value.latest_completion_certificate_plan.verdict.clone()),
        latest_completion_certificate_verdict: latest_completion_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        completion_certificate_verdict: completion_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackageCulminationReceiptLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_completion_certificate_plan_step: failure
            .latest_completion_certificate_plan_raw
            .as_ref()
            .map(|_| {
                step_artifact(
                    ...
                    failure
                        ...
                        .unwrap_or("<missing>"),
                    failure
                        .latest_completion_certificate_plan_reason
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_completion_certificate_plan_generated_at
                        .unwrap_or_else(Utc::now),
                )
            }),
        latest_completion_certificate_step: None,
...
fn verify_invalid_culmination_receipt_latest_report_without_session(
    session_dir: &Path,
    verification_mismatches: Vec<String>,
) -> PackageCulminationReceiptLatestReport {
    ...
    PackageCulminationReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_culmination_receipt".to_string(),
        verdict: TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestVerifyInvalid,
        reason,
        root: None,
        ...
        session_dir: Some(session_dir.display().to_string()),
        culmination_receipt_latest_session_path: Some(
            culmination_receipt_latest_paths(session_dir)
                .session_path
                .display()
                .to_string(),
        ),
        culmination_receipt_latest_status_path: Some(
            culmination_receipt_latest_paths(session_dir)
                .status_path
                .display()
                .to_string(),
        ),
        culmination_receipt_latest_report_path: Some(
            culmination_receipt_latest_paths(session_dir)
                .report_path
                .display()
                .to_string(),
        ),
        latest_completion_certificate_plan_report_path: None,
        latest_completion_certificate_report_path: None,
        culmination_receipt_report_path: None,
        nested_completion_certificate_latest_session_dir: None,
        nested_culmination_receipt_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_completion_certificate_command_summary: None,
        run_latest_completion_certificate_command_summary: None,
        verify_latest_completion_certificate_command_summary: None,
        downstream_culmination_receipt_command_summary: None,
        verify_completion_certificate_command_summary: None,
...
fn fallback_completion_certificate_latest_copy() -> CompletionCertificateLatestReportView {
    CompletionCertificateLatestReportView {
        generated_at: Utc::now(),
...
fn fallback_notarization_receipt_session(
    snapshot: &LatestCompletionCertificateSnapshot,
    session_dir: &Path,
) -> CulminationReceiptSessionView {
    CulminationReceiptSessionView {
        completion_certificate_session_dir: "<missing>".to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_completion_certificate_command_summary:
            culmination_receipt_verify_completion_certificate_command_summary(Path::new("<missing>")),
        ...
        explicit_statement:
            "the verified completion-certificate session is the primary input here; this immutable culmination receipt freezes one final apex identity over the fully completed chain without enabling production execution"
                .to_string(),
    }
}

fn fallback_notarization_receipt_status(
    _snapshot: &LatestCompletionCertificateSnapshot,
    session_dir: &Path,
) -> CulminationReceiptStatusView {
    CulminationReceiptStatusView {
        session_dir: session_dir.display().to_string(),
        completion_certificate_session_dir: "<missing>".to_string(),
        ...
        culmination_receipt_algorithm: "sha256".to_string(),
        explicit_statement:
            "this culmination receipt is archival and read-only; it never executes or enables the frozen controller"
                .to_string(),
    }
}

fn render_report_lines(report: &PackageCulminationReceiptLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_completion_certificate_latest_session_dir {
        lines.push(format!(
            "nested_completion_certificate_latest_session_dir={value}"
        ));
    }
    ...
    if let Some(value) = &report.nested_culmination_receipt_session_dir {
        lines.push(format!("nested_culmination_receipt_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_culmination_receipt_command_summary {
        lines.push(format!(
            "downstream_culmination_receipt_command_summary={value}"
        ));
    }
...
mod tests {
    ...
    struct FakeNotarizationReceiptLatestFixture {
        ...
        completion_certificate_latest_plan_json_path: PathBuf,
        completion_certificate_latest_run_json_path: PathBuf,
        completion_certificate_latest_verify_json_path: PathBuf,
    ...
    impl FakeNotarizationReceiptLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestCulminationReceipt,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestCulminationReceiptScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestCulminationReceipt,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestCulminationReceipt,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    #[test]
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_completion_certificate_lineage(
    ) {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_plan_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = plan_latest_completion_certificate_report(&fixture.plan_config()).unwrap();
        let paths = culmination_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert_eq!(
            report.verdict,
            TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestPlanReady
        );
        assert_eq!(report.mode, "plan_latest_culmination_receipt");
        assert_eq!(
            report.latest_top_step_name.as_deref(),
        ...
        assert!(report
            ...
            .as_deref()
            .unwrap()
            .contains(CULMINATION_RECEIPT_BIN));
    }
    ...
    fn render_mode_emits_exact_downstream_provenance_certificate_contract_not_second_controller_path(
        ...
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_render_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let report =
            render_latest_completion_certificate_script_report(&fixture.render_config()).unwrap();
        assert_eq!(
            report.verdict,
            TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestScriptRendered
        );
        assert_eq!(report.mode, "render_latest_culmination_receipt_script");
        let script = fs::read_to_string(&fixture.output_script_path).unwrap();
        assert!(script.contains(COMPLETION_CERTIFICATE_LATEST_BIN));
        assert!(script.contains(CULMINATION_RECEIPT_BIN));
        assert!(script.contains("--completion-certificate-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-culmination-receipt"));
        assert!(script.contains("--verify-live-package-culmination-receipt"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            (
                "completion_certificate_latest_missing",
                "tiny_live_package_completion_certificate_latest_refused_by_missing_latest_chain",
                "no persisted immutable chain exists",
                TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestRefusedByMissingLatestChain,
            ),
            (
                "completion_certificate_latest_invalid",
                "tiny_live_package_completion_certificate_latest_refused_by_invalid_latest_chain",
                "latest immutable chain is incomplete",
                TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestRefusedByInvalidLatestChain,
            ),
            (
                "completion_certificate_latest_below",
                "tiny_live_package_completion_certificate_latest_refused_by_invalid_latest_chain",
            ...
            let mut plan: Value = load_json(&fixture.completion_certificate_latest_plan_json_path).unwrap();
            plan["verdict"] = json!(verdict);
            plan["reason"] = json!(reason);
            if label.ends_with("below") {
                plan["latest_top_step_name"] = json!("choir_receipt");
            }
            persist_json(&fixture.completion_certificate_latest_plan_json_path, &plan).unwrap();

            let report = run_latest_culmination_receipt_report(&fixture.run_config()).unwrap();
            assert_eq!(report.verdict, expected);
        }
    }

    #[test]
    fn plan_mode_maps_real_latest_completion_certificate_drift_verdict_to_local_drift_refusal() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_plan_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value =
            load_json(&fixture.completion_certificate_latest_plan_json_path).unwrap();
        plan["verdict"] = json!(
            "tiny_live_package_completion_certificate_latest_refused_by_drifted_completion_certificate_contract"
        );
        plan["reason"] = json!("real accepted latest-completion-certificate drift");
        persist_json(&fixture.completion_certificate_latest_plan_json_path, &plan).unwrap();
        ...
        assert_eq!(
            report.result.as_deref(),
            Some("refused_by_drifted_culmination_receipt_contract")
        );
    }

    #[test]
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_completion_certificate_reports_real_drift(
    ) {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_run_drift",
            "refused_by_drifted_completion_certificate_contract",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert_eq!(report.completion_certificate_verdict.as_deref(), None);
    }

    #[test]
    fn run_mode_refuses_when_nested_release_capsule_lineage_drifts_from_latest_immutable_chain() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_handoff_bundle_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut run: Value =
            load_json(&fixture.completion_certificate_latest_run_json_path).unwrap();
        run["package_dir"] = json!("/tampered/package");
        persist_json(&fixture.completion_certificate_latest_run_json_path, &run).unwrap();
        ...
        assert!(report
            .reason
            .contains("stored latest-completion-certificate run package_dir"));
    }

    #[test]
    fn verify_mode_fails_when_persisted_latest_completion_certificate_artifacts_are_tampered_or_missing_nested_lineage_truth(
    ) {
        {
            let fixture = fake_completion_certificate_latest_fixture(
                "completion_certificate_latest_verify_tampered_copy",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            run_latest_culmination_receipt_report(&fixture.run_config()).unwrap();

            let paths = culmination_receipt_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.culmination_receipt_report_path).unwrap();
            report["reason"] = json!("tampered stored provenance-certificate copy reason");
            persist_json(&paths.culmination_receipt_report_path, &report).unwrap();

            let verify =
                verify_latest_culmination_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestVerifyInvalid
            );
        ...
        {
            let fixture = fake_completion_certificate_latest_fixture(
                "completion_certificate_latest_verify_missing_nested_truth",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = culmination_receipt_latest_paths(&fixture.wrapper_session_dir);
            fs::remove_file(nested_completion_certificate_latest_report_path(
                &paths.nested_completion_certificate_latest_session_dir,
            ))
            .unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains(
                    "tiny_live_activation_package_completion_certificate_latest.report.json",
                )
            }));
        }
    }
    ...
    fn verify_accepts_real_shaped_native_consummation_identity_layers_on_happy_path() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_real_shaped_finality_paths_green",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let verify = verify_latest_culmination_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestVerifyOk,
            "{:#?}",
    ...
    #[test]
    fn verify_fails_when_copied_native_culmination_receipt_session_path_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_finality_session_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = culmination_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.culmination_receipt_report_path).unwrap();
        report["culmination_receipt_session_path"] =
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt run culmination_receipt_session_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_culmination_receipt_status_path_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_finality_status_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = culmination_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.culmination_receipt_report_path).unwrap();
        report["culmination_receipt_status_path"] = json!("/tampered/finality-receipt-status.json");
        persist_json(&paths.culmination_receipt_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt run culmination_receipt_status_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_nested_latest_completion_certificate_explicit_statement_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_launch_packet_explicit_statement_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut report: Value =
            load_json(&paths.latest_completion_certificate_report_path).unwrap();
        report["explicit_statement"] =
            json!("tampered nested latest release capsule explicit statement");
        persist_json(&paths.latest_completion_certificate_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored latest-completion-certificate copy explicit_statement")
        }));
    }
    ...
    fn verify_rejects_tampered_stale_completion_certificate_wrapper_mode_string() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_wrapper_mode_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        report["mode"] = json!("run_latest_completion_certificate");
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("latest-culmination-receipt stored report mode") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_decision_packet_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy decision_packet_result")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_attestation_seal_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy attestation_seal_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_registry_entry_session_dir_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_registry_entry_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy registry_entry_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_notarization_receipt_session_dir_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_notarization_receipt_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy notarization_receipt_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_chain_fingerprint_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy chain_fingerprint_summary")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_chain_fingerprint_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy chain_fingerprint_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_chain_fingerprint_algorithm_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy chain_fingerprint_algorithm")
        }));
    }
    ...
    fn verify_fails_when_copied_native_completion_certificate_closure_certificate_sha256_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_closure_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy closure_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_archive_receipt_sha256_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_archive_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy archive_receipt_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_finality_receipt_sha256_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_finality_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy finality_receipt_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_consummation_record_sha256_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_consummation_record_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy consummation_record_sha256")
        }));
    }
    ...
    {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_completion_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy completion_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_decision_packet_anchor_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy decision_packet_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_current_authorization_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored culmination-receipt copy execute_frozen_result")
        }));
    }

    #[test]
    fn verify_fails_when_copied_nested_completion_certificate_verdict_drifts() {
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_release_capsule_verdict_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| entry.contains("stored culmination-receipt copy verdict")));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_current_live_cutover_controller_summary_drifts(
        ...
        let fixture = fake_completion_certificate_latest_fixture(
            "completion_certificate_latest_verify_current_live_cutover_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn all_modes_preserve_stage3_and_pre_activation_refusal_semantics_and_do_not_imply_activation_authorization_by_themselves(
        ...
        {
            let fixture = fake_completion_certificate_latest_fixture(
                "completion_certificate_latest_stage3",
                "refused_now_by_stage3",
                "refused_now_by_stage3",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);

            let plan = plan_latest_completion_certificate_report(&fixture.plan_config()).unwrap();
            assert_eq!(
                plan.verdict,
                TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestPlanReady
            );
            assert_eq!(plan.mode, "plan_latest_culmination_receipt");
            assert!(!plan.activation_authorized);

            let run = run_latest_culmination_receipt_report(&fixture.run_config()).unwrap();
            assert_eq!(
                run.verdict,
                TinyLivePackageCulminationReceiptLatestVerdict::TinyLivePackageCulminationReceiptLatestRefusedNowByStage3
            );
            assert_eq!(run.mode, "run_latest_culmination_receipt");
            ...
            assert_eq!(verify.mode, "verify_latest_culmination_receipt");
            assert!(!verify.activation_authorized);
        }

        {
            let fixture = fake_completion_certificate_latest_fixture(
                "completion_certificate_latest_pre_activation",
                "refused_now_by_pre_activation_gate",
                "refused_now_by_pre_activation_gate",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        {
            let fixture = fake_completion_certificate_latest_fixture(
                "completion_certificate_latest_green",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn fake_completion_certificate_latest_fixture(
        label: &str,
        completion_certificate_latest_run_result: &str,
        ...
        let completion_certificate_latest_plan_json_path =
            temp_dir.join("completion_certificate_latest.plan.json");
        let completion_certificate_latest_run_json_path =
            temp_dir.join("completion_certificate_latest.run.json");
        let completion_certificate_latest_verify_json_path =
            temp_dir.join("completion_certificate_latest.verify.json");
        ...
        let fixture = FakeNotarizationReceiptLatestFixture {
            ...
            install_root,
            backend_command,
            completion_certificate_latest_plan_json_path,
            completion_certificate_latest_run_json_path,
            completion_certificate_latest_verify_json_path,
        ...
        persist_json(
            &fixture.completion_certificate_latest_plan_json_path,
            &default_completion_certificate_latest_plan_report(&fixture),
        )
        .unwrap();
        persist_json(
            &fixture.completion_certificate_latest_run_json_path,
            &default_completion_certificate_latest_run_report(
                &fixture,
                completion_certificate_latest_run_result,
            ),
        )
        .unwrap();
        persist_json(
            &fixture.completion_certificate_latest_verify_json_path,
            &default_completion_certificate_latest_verify_report(
                &fixture,
                completion_certificate_latest_run_result,
            ),
        )
        ...
        let paths = culmination_receipt_latest_paths(&fixture.wrapper_session_dir);
        write_fake_completion_certificate_latest_command(
            &fixture.bin_dir.join(COMPLETION_CERTIFICATE_LATEST_BIN),
            &fixture.completion_certificate_latest_plan_json_path,
            &fixture.completion_certificate_latest_run_json_path,
            &fixture.completion_certificate_latest_verify_json_path,
            &fixture.root,
        ...
        write_fake_release_capsule_command(
            &fixture.bin_dir.join(CULMINATION_RECEIPT_BIN),
            &fixture.handoff_bundle_run_json_path,
            &fixture.handoff_bundle_verify_json_path,
            &fixture.handoff_bundle_session_json_path,
            &fixture.handoff_bundle_status_json_path,
    ...
    fn default_completion_certificate_latest_plan_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
    ) -> Value {
        let paths = culmination_receipt_latest_paths(&fixture.wrapper_session_dir);
        json!({
            "generated_at": Utc::now(),
            "mode": "plan_latest_completion_certificate",
            "verdict": COMPLETION_CERTIFICATE_LATEST_PLAN_READY,
            "reason": "latest immutable chain resolves cleanly into accepted latest-completion-certificate contract",
            ...
            "session_dir": paths.nested_completion_certificate_latest_session_dir.display().to_string(),
            ...
            "activation_authorized": false,
            "explicit_statement": "latest culmination-receipt wrapper plan"
        })
    }

    fn canonical_completion_certificate_latest_result(result: &str) -> &str {
        match result {
            "runnable_when_gate_truth_turns_green" => {
    ...
    fn default_completion_certificate_latest_run_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let result = canonical_completion_certificate_latest_result(result);
        let paths = culmination_receipt_latest_paths(&fixture.wrapper_session_dir);
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_completion_certificate_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_completion_certificate_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_completion_certificate_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_latest_completion_certificate",
            "verdict": verdict,
            "reason": format!("latest culmination-receipt {result}"),
            "latest_top_step_name": "clerestory_certificate",
            "latest_top_session_dir": fixture.latest_top_session_dir.display().to_string(),
            "historical_execute_frozen_session_dir": fixture.historical_execute_frozen_session_dir.display().to_string(),
            "turn_green_session_dir": fixture.turn_green_session_dir.display().to_string(),
            ...
            "clerestory_certificate_algorithm": "sha256",
            "activation_authorized": false,
            "explicit_statement": "latest culmination-receipt wrapper run"
        })
    }

    fn default_completion_certificate_latest_verify_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let mut report = default_completion_certificate_latest_run_report(fixture, result);
        report["mode"] = json!("verify_latest_completion_certificate");
        report["verdict"] = json!(COMPLETION_CERTIFICATE_LATEST_VERIFY_OK);
    ...
    fn default_provenance_certificate_session(
        ...
        let paths = culmination_receipt_latest_paths(&fixture.wrapper_session_dir);
        let completion_certificate_session_dir =
            expected_downstream_completion_certificate_session_dir(&paths);
        ...
        let snapshot = LatestCompletionCertificateSnapshot {
        ...
        json!({
            "session_version": "1",
            "planned_at": Utc::now(),
            "completion_certificate_session_dir": completion_certificate_session_dir.display().to_string(),
            ...
            "release_capsule_session_dir": completion_certificate_session_dir.display().to_string(),
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": culmination_receipt_latest_paths(&fixture.wrapper_session_dir).nested_culmination_receipt_session_dir.display().to_string(),
            "verify_completion_certificate_command_summary": culmination_receipt_verify_completion_certificate_command_summary(&completion_certificate_session_dir),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
            ...
            "explicit_statement": "the verified completion-certificate session is the primary input here; this immutable culmination receipt freezes one final apex identity over the fully completed chain without enabling production execution"
        })
    }
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_culmination_receipt_session_dir.display().to_string(),
            "completion_certificate_session_dir": expected_downstream_completion_certificate_session_dir(&paths).display().to_string(),
            "result": result,
            "reason": reason,
            ...
            "current_pre_activation_gate_verdict": gate_verdict,
            "current_pre_activation_gate_reason": gate_reason,
            "completion_certificate_step": Value::Null,
            ...
            "culmination_receipt_sha256": "fixture-culmination-receipt-sha256",
            "culmination_receipt_algorithm": "sha256",
            "explicit_statement": "this culmination receipt is archival and read-only; it never executes or enables the frozen controller"
        })
    }
    ...
    fn default_provenance_certificate_run_report(
        ...
        let culmination_receipt_verdict = match result {
            "ready_for_manual_execution_when_gate_turns_green" => {
                "tiny_live_package_culmination_receipt_ready_for_manual_execution_when_gate_turns_green"
            }
            "refused_now_by_stage3" => "tiny_live_package_culmination_receipt_refused_now_by_stage3",
            "refused_now_by_pre_activation_gate" => {
                "tiny_live_package_culmination_receipt_refused_now_by_pre_activation_gate"
            }
            _ => "tiny_live_package_culmination_receipt_refused_now_by_invalid_or_drifted_contract",
        };
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_culmination_receipt",
            "verdict": culmination_receipt_verdict,
            ...
            "release_capsule_session_dir": expected_downstream_completion_certificate_session_dir(&paths).display().to_string(),
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_culmination_receipt_session_dir.display().to_string(),
            "culmination_receipt_session_path": nested_culmination_receipt_session_path(&paths.nested_culmination_receipt_session_dir).display().to_string(),
            "culmination_receipt_status_path": nested_culmination_receipt_status_path(&paths.nested_culmination_receipt_session_dir).display().to_string(),
            "archived_completion_certificate_report_path": paths.nested_culmination_receipt_session_dir.join("tiny_live_activation_package_culmination_receipt.completion_certificate.report.json").display().to_string(),
            "result": result,
            ...
            "completion_certificate_step": Value::Null,
            "verify_completion_certificate_command_summary": culmination_receipt_verify_completion_certificate_command_summary(&expected_downstream_completion_certificate_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn default_provenance_certificate_verify_report(
        ...
        report["mode"] = json!("verify_live_package_culmination_receipt");
        report["verdict"] = json!(CULMINATION_RECEIPT_VERIFY_OK);
        report["reason"] = json!("verify ok");
        report["explicit_statement"] = json!("this verification binds one immutable culmination receipt to verified completion-certificate truth, the exact reviewed frozen controller summary, the canonical chain fingerprint, the ledger seal, the registry entry, the filing-certificate identity, the archive-receipt identity, the closure-certificate identity, the finality-receipt identity, and the consummation-record identity");
        report
    }

    fn write_fake_completion_certificate_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-completion-certificate session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-culmination-receipt \"*) mode='plan';;\n  *\" --run-latest-culmination-receipt \"*) mode='run';;\n  *\" --verify-latest-culmination-receipt \"*) mode='verify';;\n  *) echo 'unexpected latest-completion-certificate mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_completion_certificate_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
    ...
    fn write_fake_release_capsule_command(
        ...
        let nested_session_path = nested_culmination_receipt_session_path(session_dir);
        let nested_status_path = nested_culmination_receipt_status_path(session_dir);
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --completion-certificate-session-dir {} \"*) : ;;\n  *) echo 'unexpected culmination-receipt completion-certificate-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected culmination-receipt confirm-decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected culmination-receipt session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --run-live-package-culmination-receipt \"*) mode='run';;\n  *\" --verify-live-package-culmination-receipt \"*) mode='verify';;\n  *) echo 'unexpected culmination-receipt mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  run) mkdir -p '{}'; cp '{}' '{}'; cp '{}' '{}'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
