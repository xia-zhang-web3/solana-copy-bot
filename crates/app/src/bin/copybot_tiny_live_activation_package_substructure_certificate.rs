...
use copybot_app::tiny_live_activation::package_basal_receipt_substructure_certificate;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_substructure_certificate --basal-receipt-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-substructure-certificate | --render-live-package-substructure-certificate | --run-live-package-substructure-certificate | --verify-live-package-substructure-certificate)";
const SUBSTRUCTURE_CERTIFICATE_SESSION_VERSION: &str = "1";
const SUBSTRUCTURE_CERTIFICATE_STATUS_VERSION: &str = "1";
const SUBSTRUCTURE_CERTIFICATE_ALGORITHM: &str = "sha256";
const SUBSTRUCTURE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT: &str =
...
const SUBSTRUCTURE_CERTIFICATE_STATUS_EXPLICIT_STATEMENT: &str =
    "this substructure certificate is archival and read-only; it never executes or enables the frozen controller";
const SUBSTRUCTURE_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT: &str =
...
enum Mode {
    PlanLivePackageSubstructureCertificate,
    RenderLivePackageSubstructureCertificate,
    RunLivePackageSubstructureCertificate,
    VerifyLivePackageSubstructureCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageSubstructureCertificateVerdict {
    TinyLivePackageSubstructureCertificatePlanReady,
    TinyLivePackageSubstructureCertificateRendered,
    TinyLivePackageSubstructureCertificateRefusedNowByStage3,
    TinyLivePackageSubstructureCertificateRefusedNowByPreActivationGate,
    TinyLivePackageSubstructureCertificateRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageSubstructureCertificateReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageSubstructureCertificateVerifyOk,
    TinyLivePackageSubstructureCertificateVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageSubstructureCertificateResult {
    RefusedNowByStage3,
...
struct PackageSubstructureCertificateStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageSubstructureCertificateSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageSubstructureCertificateStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackageSubstructureCertificateResult,
    reason: String,
    ...
    basal_receipt_step: Option<PackageSubstructureCertificateStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageSubstructureCertificatePaths {
...
struct PackageSubstructureCertificateReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageSubstructureCertificateVerdict,
    reason: String,
    basal_receipt_session_dir: String,
...
struct NestedBasalReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut basal_receipt_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--basal-receipt-session-dir" => {
                basal_receipt_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--basal-receipt-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-substructure-certificate" => set_mode(
                &mut mode,
                Mode::PlanLivePackageSubstructureCertificate,
                arg.as_str(),
            )?,
            "--render-live-package-substructure-certificate" => set_mode(
                &mut mode,
                Mode::RenderLivePackageSubstructureCertificate,
                arg.as_str(),
            )?,
            "--run-live-package-substructure-certificate" => set_mode(
                &mut mode,
                Mode::RunLivePackageSubstructureCertificate,
                arg.as_str(),
            )?,
            "--verify-live-package-substructure-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLivePackageSubstructureCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackageSubstructureCertificate) && output_path.is_none() {
        bail!("missing --output for --render-live-package-substructure-certificate");
    }
    if matches!(
        mode,
        Mode::RunLivePackageSubstructureCertificate
    ...
    let basal_receipt_session_dir =
        basal_receipt_session_dir.ok_or_else(|| anyhow!("missing --basal-receipt-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageSubstructureCertificate => {
            plan_live_package_substructure_certificate_report(&config)?
        }
        Mode::RenderLivePackageSubstructureCertificate => {
            render_live_package_substructure_certificate_report(&config)?
        }
        Mode::RunLivePackageSubstructureCertificate => {
            run_live_package_substructure_certificate_report(&config)?
        }
        Mode::VerifyLivePackageSubstructureCertificate => {
            verify_live_package_substructure_certificate_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_substructure_certificate_report(
    config: &Config,
) -> Result<PackageSubstructureCertificateReport> {
    let (basal_receipt, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_basal_receipt_substructure_certificate::verify_live_package_basal_receipt_for_substructure_certificate(
                    &config.basal_receipt_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_basal_receipt(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_basal_receipt_substructure_certificate::load_live_package_basal_receipt_contract_for_substructure_certificate(
                            &config.basal_receipt_session_dir,
                        )?;
                        let classification = classify_basal_receipt_contract(&raw_contract, false);
    ...
    let session_dir = config.session_dir.clone().unwrap_or_else(|| {
        config
            .basal_receipt_session_dir
            .join("substructure-certificate")
    });
    let result = serialize_enum(&classification.0);
    let substructure_certificate_sha256 = compute_substructure_certificate_sha256(
        &result,
    ...
    Ok(PackageSubstructureCertificateReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_substructure_certificate".to_string(),
        verdict:
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificatePlanReady,
        reason: format!(
            "basal-receipt-native substructure certificate is explicit for verified basal-receipt session {}; this command stays archival and read-only while freezing one final signet identity over the fully culminated chain",
            config.basal_receipt_session_dir.display()
        ),
        basal_receipt_session_dir: config.basal_receipt_session_dir.display().to_string(),
        ...
        substructure_certificate_session_path: None,
        substructure_certificate_status_path: None,
        archived_basal_receipt_report_path: None,
        result: Some(result),
        ...
        verify_basal_receipt_command_summary: Some(verify_basal_receipt_command_summary(
            &config.basal_receipt_session_dir,
        )),
        ...
        substructure_certificate_summary: Some(substructure_certificate_summary(
            classification.0,
            &basal_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        substructure_certificate_sha256: Some(substructure_certificate_sha256),
        substructure_certificate_algorithm: Some(SUBSTRUCTURE_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: SUBSTRUCTURE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_substructure_certificate_report(
    config: &Config,
) -> Result<PackageSubstructureCertificateReport> {
    let plan = plan_live_package_substructure_certificate_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_substructure_certificate --basal-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-substructure-certificate --json\ncopybot_tiny_live_activation_package_substructure_certificate --basal-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-substructure-certificate --json\n",
        shell_escape_path(&config.basal_receipt_session_dir),
    ...
    Ok(PackageSubstructureCertificateReport {
        generated_at: Utc::now(),
        mode: "render_live_package_substructure_certificate".to_string(),
        verdict:
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateRendered,
        reason: format!(
            "rendered substructure-certificate run/verify script to {}; the primary input remains the immutable basal-receipt session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: SUBSTRUCTURE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_substructure_certificate_report(
    config: &Config,
) -> Result<PackageSubstructureCertificateReport> {
    ...
    let verified_basal_receipt = match package_basal_receipt_substructure_certificate::verify_live_package_basal_receipt_for_substructure_certificate(
        &config.basal_receipt_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify basal-receipt session under {}: {error}",
                    config.basal_receipt_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_basal_receipt.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "substructure-certificate session dir {} overlaps the managed live target surface derived from verified basal-receipt install_root {}",
            session_dir.display(),
            verified_basal_receipt.contract.install_root.display()
        )
    })?;
    ensure_clean_substructure_certificate_session_dir(session_dir)?;
    let paths = substructure_certificate_paths(session_dir);
    let classification = classify_verified_basal_receipt(&verified_basal_receipt);
    ...
    let substructure_certificate_sha256 = compute_substructure_certificate_sha256(
        ...
        &verified_basal_receipt
            .contract
            .reviewed_frozen_live_cutover_controller_command_summary,
        &verified_basal_receipt.contract.chain_fingerprint_sha256,
        &verified_basal_receipt.contract.chain_fingerprint_algorithm,
        &verified_basal_receipt
            .contract
        ...
        &verified_basal_receipt.contract.ledger_seal_sha256,
        &verified_basal_receipt.contract.ledger_seal_algorithm,
        &verified_basal_receipt.contract.registry_entry_sha256,
        &verified_basal_receipt.contract.registry_entry_algorithm,
        &verified_basal_receipt.contract.filing_certificate_sha256,
        &verified_basal_receipt.contract.filing_certificate_algorithm,
        &verified_basal_receipt.contract.archive_receipt_sha256,
        &verified_basal_receipt.contract.archive_receipt_algorithm,
        &verified_basal_receipt.contract.closure_certificate_summary,
        &verified_basal_receipt.contract.closure_certificate_sha256,
        &verified_basal_receipt
            .contract
            .closure_certificate_algorithm,
        &verified_basal_receipt.contract.finality_receipt_sha256,
        &verified_basal_receipt.contract.finality_receipt_algorithm,
        &verified_basal_receipt.contract.consummation_record_sha256,
        &verified_basal_receipt
            .contract
        ...
        &verified_basal_receipt.contract.summit_certificate_summary,
        &verified_basal_receipt.contract.summit_certificate_sha256,
        &verified_basal_receipt.contract.summit_certificate_algorithm,
        &verified_basal_receipt.contract.culmination_receipt_summary,
        &verified_basal_receipt.contract.culmination_receipt_sha256,
        &verified_basal_receipt
            .contract
            .culmination_receipt_algorithm,
        &verified_basal_receipt.contract.pinnacle_receipt_summary,
        &verified_basal_receipt.contract.pinnacle_receipt_sha256,
        &verified_basal_receipt.contract.pinnacle_receipt_algorithm,
        &verified_basal_receipt.contract.capstone_certificate_summary,
        &verified_basal_receipt.contract.capstone_certificate_sha256,
        &verified_basal_receipt
            .contract
            .capstone_certificate_algorithm,
        &verified_basal_receipt.contract.keystone_receipt_summary,
        &verified_basal_receipt.contract.keystone_receipt_sha256,
        &verified_basal_receipt.contract.keystone_receipt_algorithm,
        &verified_basal_receipt
            .contract
        ...
        &verified_basal_receipt.contract.foundation_receipt_summary,
        &verified_basal_receipt.contract.foundation_receipt_sha256,
        &verified_basal_receipt.contract.foundation_receipt_algorithm,
        &verified_basal_receipt.contract.bedrock_certificate_summary,
        &verified_basal_receipt.contract.bedrock_certificate_sha256,
        &verified_basal_receipt
            .contract
            .bedrock_certificate_algorithm,
        &verified_basal_receipt.contract.basal_receipt_summary,
        &verified_basal_receipt.contract.basal_receipt_sha256,
        &verified_basal_receipt.contract.basal_receipt_algorithm,
    )?;
    let session = planned_session(
        config,
        session_dir,
        &verified_basal_receipt.contract,
        result,
    ...
    persist_json(
        ...
        &verified_basal_receipt.report_json,
    )?;
    let basal_receipt_step = Some(step_artifact(
        &paths.basal_receipt_report_path,
        "verify_live_package_basal_receipt",
        &verified_basal_receipt.verdict,
        &verified_basal_receipt.reason,
        verified_basal_receipt.generated_at,
    ));
    let status = PackageSubstructureCertificateStatus {
        status_version: SUBSTRUCTURE_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        basal_receipt_session_dir: config.basal_receipt_session_dir.display().to_string(),
        bedrock_certificate_session_dir: verified_basal_receipt
            .contract
            .bedrock_certificate_session_dir
            .display()
            .to_string(),
        foundation_receipt_session_dir: verified_basal_receipt
            .contract
            .foundation_receipt_session_dir
            .display()
            .to_string(),
        ...
        handoff_bundle_result: verified_basal_receipt
            .contract
            .handoff_bundle_result
            .clone(),
        decision_packet_result: verified_basal_receipt
            .contract
            .decision_packet_result
            .clone(),
        execute_frozen_result: verified_basal_receipt
            .contract
            .execute_frozen_result
            .clone(),
        current_pre_activation_gate_verdict: verified_basal_receipt
            .contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_basal_receipt
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        basal_receipt_step,
        provenance_certificate_summary: verified_basal_receipt
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_basal_receipt
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_basal_receipt
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_basal_receipt
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_basal_receipt
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_basal_receipt
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_basal_receipt
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_basal_receipt
            .contract
            .notarization_receipt_summary
            .clone(),
        ledger_seal_summary: verified_basal_receipt.contract.ledger_seal_summary.clone(),
        ledger_seal_sha256: verified_basal_receipt.contract.ledger_seal_sha256.clone(),
        ledger_seal_algorithm: verified_basal_receipt
            .contract
            .ledger_seal_algorithm
            .clone(),
        registry_entry_summary: verified_basal_receipt
            .contract
            .registry_entry_summary
            .clone(),
        registry_entry_sha256: verified_basal_receipt
            .contract
            .registry_entry_sha256
            .clone(),
        registry_entry_algorithm: verified_basal_receipt
            .contract
            .registry_entry_algorithm
            .clone(),
        filing_certificate_summary: verified_basal_receipt
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_basal_receipt
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_basal_receipt
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_basal_receipt
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_basal_receipt
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_basal_receipt
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_basal_receipt
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_basal_receipt
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_basal_receipt
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_basal_receipt
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_basal_receipt
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_basal_receipt
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_basal_receipt
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_basal_receipt
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_basal_receipt
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_basal_receipt
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_basal_receipt
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_basal_receipt
            .contract
            .completion_certificate_algorithm
            .clone(),
        summit_certificate_summary: verified_basal_receipt
            .contract
            .summit_certificate_summary
            .clone(),
        summit_certificate_sha256: verified_basal_receipt
            .contract
            .summit_certificate_sha256
            .clone(),
        summit_certificate_algorithm: verified_basal_receipt
            .contract
            .summit_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_basal_receipt
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_basal_receipt
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_basal_receipt
            .contract
            .culmination_receipt_algorithm
            .clone(),
        pinnacle_receipt_summary: verified_basal_receipt
            .contract
            .pinnacle_receipt_summary
            .clone(),
        pinnacle_receipt_sha256: verified_basal_receipt
            .contract
            .pinnacle_receipt_sha256
            .clone(),
        pinnacle_receipt_algorithm: verified_basal_receipt
            .contract
            .pinnacle_receipt_algorithm
            .clone(),
        capstone_certificate_summary: verified_basal_receipt
            .contract
            .capstone_certificate_summary
            .clone(),
        capstone_certificate_sha256: verified_basal_receipt
            .contract
            .capstone_certificate_sha256
            .clone(),
        capstone_certificate_algorithm: verified_basal_receipt
            .contract
            .capstone_certificate_algorithm
            .clone(),
        keystone_receipt_summary: verified_basal_receipt
            .contract
            .keystone_receipt_summary
            .clone(),
        keystone_receipt_sha256: verified_basal_receipt
            .contract
            .keystone_receipt_sha256
            .clone(),
        keystone_receipt_algorithm: verified_basal_receipt
            .contract
            .keystone_receipt_algorithm
            .clone(),
        cornerstone_certificate_summary: verified_basal_receipt
            .contract
            .cornerstone_certificate_summary
            .clone(),
        cornerstone_certificate_sha256: verified_basal_receipt
            .contract
            .cornerstone_certificate_sha256
            .clone(),
        cornerstone_certificate_algorithm: verified_basal_receipt
            .contract
            .cornerstone_certificate_algorithm
            .clone(),
        foundation_receipt_summary: verified_basal_receipt
            .contract
            .foundation_receipt_summary
            .clone(),
        foundation_receipt_sha256: verified_basal_receipt
            .contract
            .foundation_receipt_sha256
            .clone(),
        foundation_receipt_algorithm: verified_basal_receipt
            .contract
            .foundation_receipt_algorithm
            .clone(),
        bedrock_certificate_summary: verified_basal_receipt
            .contract
            .bedrock_certificate_summary
            .clone(),
        bedrock_certificate_sha256: verified_basal_receipt
            .contract
            .bedrock_certificate_sha256
            .clone(),
        bedrock_certificate_algorithm: verified_basal_receipt
            .contract
            .bedrock_certificate_algorithm
            .clone(),
        basal_receipt_summary: verified_basal_receipt
            .contract
            .basal_receipt_summary
            .clone(),
        basal_receipt_sha256: verified_basal_receipt.contract.basal_receipt_sha256.clone(),
        basal_receipt_algorithm: verified_basal_receipt
            .contract
            .basal_receipt_algorithm
            .clone(),
        substructure_certificate_summary: substructure_certificate_summary(
            result,
            &verified_basal_receipt
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        substructure_certificate_sha256: substructure_certificate_sha256.clone(),
        substructure_certificate_algorithm: SUBSTRUCTURE_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: SUBSTRUCTURE_CERTIFICATE_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_substructure_certificate_report(
    config: &Config,
) -> Result<PackageSubstructureCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = substructure_certificate_paths(session_dir);
    let session: PackageSubstructureCertificateSession = load_json(&paths.session_path)?;
    let status: PackageSubstructureCertificateStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.basal_receipt_session_dir,
        &config.basal_receipt_session_dir.display().to_string(),
        "basal-receipt basal_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
    ...
    compare_string(
        &status.basal_receipt_session_dir,
        &config.basal_receipt_session_dir.display().to_string(),
        "basal-receipt status basal_receipt_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_basal_receipt
            ...
            .display()
            .to_string(),
        "substructure-certificate bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_basal_receipt
            ...
            .display()
            .to_string(),
        "substructure-certificate status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_basal_receipt
            ...
            .display()
            .to_string(),
        "substructure-certificate foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_basal_receipt
            ...
            .display()
            .to_string(),
        "substructure-certificate status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_basal_receipt_report_view: NestedBasalReceiptReportView =
        serde_json::from_value(stored_basal_receipt_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested basal-receipt report {}",
                paths.basal_receipt_report_path.display()
            )
        })?;
    let verified_basal_receipt_report_view: NestedBasalReceiptReportView =
        serde_json::from_value(verified_basal_receipt.report_json.clone()).with_context(|| {
            format!(
                "failed parsing freshly verified nested basal-receipt report for {}",
                config.basal_receipt_session_dir.display()
            )
        })?;
    compare_json_ignoring_generated_at(
        ...
        &verified_basal_receipt.report_json,
        "substructure-certificate archived basal-receipt report json",
        &mut mismatches,
    );
    compare_string(
        &stored_basal_receipt_report_view.mode,
        &verified_basal_receipt_report_view.mode,
        "substructure-certificate archived basal-receipt report mode",
        &mut mismatches,
    );
    compare_string(
        &stored_basal_receipt_report_view.verdict,
        &verified_basal_receipt_report_view.verdict,
        "substructure-certificate archived basal-receipt report verdict",
        &mut mismatches,
    );
    compare_string(
        &stored_basal_receipt_report_view.reason,
        &verified_basal_receipt_report_view.reason,
        "substructure-certificate archived basal-receipt report reason",
        &mut mismatches,
    );
    if stored_basal_receipt_report_view.generated_at
        != verified_basal_receipt_report_view.generated_at
    {
        mismatches.push(format!(
            "substructure-certificate archived basal-receipt report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_basal_receipt_report_view.generated_at,
            verified_basal_receipt_report_view.generated_at
        ));
    }
    let expected_basal_receipt_step = step_artifact(
        &paths.basal_receipt_report_path,
        &verified_basal_receipt_report_view.mode,
        &verified_basal_receipt_report_view.verdict,
        &verified_basal_receipt_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_basal_receipt_step,
        "substructure-certificate basal_receipt_step",
        &mut mismatches,
    );
    if verified_basal_receipt.verdict != "tiny_live_package_basal_receipt_verify_ok" {
        mismatches.push(format!(
            "substructure-certificate nested basal-receipt verification is non-green: {}",
            verified_basal_receipt.reason
        ));
    }

    let classification = classify_verified_basal_receipt(&verified_basal_receipt);
    let expected_substructure_certificate_sha256 = compute_substructure_certificate_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_basal_receipt.contract,
        classification.0,
        &expected_substructure_certificate_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "substructure-certificate session contract does not match freshly verified basal-receipt truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_basal_receipt.contract,
        classification,
        expected_basal_receipt_step.clone(),
        &expected_substructure_certificate_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "substructure-certificate status contract does not match freshly verified basal-receipt truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_basal_receipt.contract,
        TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateVerifyOk,
        &expected_status,
    ))
}
...
fn classify_basal_receipt_contract(
    contract: &package_basal_receipt_substructure_certificate::LivePackageBasalReceiptContractView,
    assume_verified: bool,
) -> (
    LivePackageSubstructureCertificateResult,
    TinyLivePackageSubstructureCertificateVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageSubstructureCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateRefusedNowByInvalidOrDriftedContract,
            "the basal-receipt chain is not verified, so this substructure certificate remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageSubstructureCertificateResult::RefusedNowByStage3,
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateRefusedNowByStage3,
            "the immutable basal receipt remains refused now by Stage 3, so this substructure certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageSubstructureCertificateResult::RefusedNowByPreActivationGate,
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateRefusedNowByPreActivationGate,
            "the immutable basal receipt remains refused now by the pre-activation gate, so this substructure certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageSubstructureCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable basal receipt is coherent and this substructure certificate is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageSubstructureCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateRefusedNowByInvalidOrDriftedContract,
            "the verified basal-receipt chain is invalid, drifted, or non-runnable, so this substructure certificate remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_basal_receipt(
    verified: &package_basal_receipt_substructure_certificate::VerifiedLivePackageBasalReceiptSubstructureCertificateStep,
) -> (
    LivePackageSubstructureCertificateResult,
    TinyLivePackageSubstructureCertificateVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_basal_receipt_verify_ok" {
        return (
            LivePackageSubstructureCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateRefusedNowByInvalidOrDriftedContract,
            "the immutable basal receipt did not verify cleanly, so this substructure certificate remains refused"
                .to_string(),
        );
    }
    classify_basal_receipt_contract(&verified.contract, true)
}

fn substructure_certificate_summary(
    result: LivePackageSubstructureCertificateResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageSubstructureCertificateResult::RefusedNowByStage3 => {
            "Substructure certificate outcome: the immutable culminated chain remains refused now by Stage 3; archive this final signet seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageSubstructureCertificateResult::RefusedNowByPreActivationGate => {
            "Substructure certificate outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final signet seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageSubstructureCertificateResult::RefusedNowByInvalidOrDriftedContract => {
            "Substructure certificate outcome: the immutable culminated chain is invalid or drifted; archive this final signet seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackageSubstructureCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Substructure certificate outcome: this immutable signet seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn substructure_certificate_paths(session_dir: &Path) -> PackageSubstructureCertificatePaths {
    PackageSubstructureCertificatePaths {
        ...
        basal_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_substructure_certificate.basal_receipt.report.json",
        ),
    }
}

#[cfg(test)]
fn basal_receipt_paths(session_dir: &Path) -> PackageSubstructureCertificatePaths {
    substructure_certificate_paths(session_dir)
}

fn ensure_clean_substructure_certificate_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing substructure-certificate session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating substructure-certificate session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageSubstructureCertificateSession {
    PackageSubstructureCertificateSession {
        session_version: SUBSTRUCTURE_CERTIFICATE_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        basal_receipt_session_dir: config.basal_receipt_session_dir.display().to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_basal_receipt_command_summary: verify_basal_receipt_command_summary(
            &config.basal_receipt_session_dir,
        ),
        ...
        substructure_certificate_sha256: substructure_certificate_sha256.to_string(),
        substructure_certificate_algorithm: SUBSTRUCTURE_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: SUBSTRUCTURE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    basal_receipt_step: PackageSubstructureCertificateStepArtifact,
    substructure_certificate_sha256: &str,
) -> PackageSubstructureCertificateStatus {
    PackageSubstructureCertificateStatus {
        status_version: SUBSTRUCTURE_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        basal_receipt_step: Some(basal_receipt_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageSubstructureCertificatePaths,
    contract: &package_basal_receipt_substructure_certificate::LivePackageBasalReceiptContractView,
    verdict: TinyLivePackageSubstructureCertificateVerdict,
    status: &PackageSubstructureCertificateStatus,
) -> PackageSubstructureCertificateReport {
    PackageSubstructureCertificateReport {
        generated_at: Utc::now(),
        mode: if verdict == TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateVerifyOk
        {
            "verify_live_package_substructure_certificate".to_string()
        } else {
            "run_live_package_substructure_certificate".to_string()
        },
        verdict,
        reason: status.reason.clone(),
        basal_receipt_session_dir: config
            .basal_receipt_session_dir
            .display()
            .to_string(),
        bedrock_certificate_session_dir: Some(
        ...
        session_dir: Some(session_dir.display().to_string()),
        substructure_certificate_session_path: Some(paths.session_path.display().to_string()),
        substructure_certificate_status_path: Some(paths.status_path.display().to_string()),
        archived_basal_receipt_report_path: Some(
            paths.basal_receipt_report_path.display().to_string(),
        ),
        ...
        basal_receipt_step: status.basal_receipt_step.clone(),
        verify_basal_receipt_command_summary: Some(verify_basal_receipt_command_summary(
            &config.basal_receipt_session_dir,
        )),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
        ...
        explicit_statement: if verdict
            == TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateVerifyOk
        {
            SUBSTRUCTURE_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageSubstructureCertificateReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageSubstructureCertificate => {
                "plan_live_package_substructure_certificate".to_string()
            }
            Mode::RenderLivePackageSubstructureCertificate => {
                "render_live_package_substructure_certificate".to_string()
            }
            Mode::RunLivePackageSubstructureCertificate => {
                "run_live_package_substructure_certificate".to_string()
            }
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable substructure certificate refused before persisting because the basal-receipt artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "basal-receipt-native substructure-certificate verification failed".to_string()
    });
    PackageSubstructureCertificateReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_substructure_certificate".to_string(),
        verdict: TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateVerifyInvalid,
        reason,
        basal_receipt_session_dir: config
            .basal_receipt_session_dir
            .display()
        ...
        explicit_statement: SUBSTRUCTURE_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageSubstructureCertificateStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from substructure-certificate status"
        ));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageSubstructureCertificateStepArtifact {
    PackageSubstructureCertificateStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageSubstructureCertificateStepArtifact>,
    expected: &PackageSubstructureCertificateStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_substructure_certificate_sha256(
    result: &str,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing substructure-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackageSubstructureCertificateReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.substructure_certificate_sha256 {
        lines.push(format!("substructure_certificate_sha256={value}"));
    }
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_substructure_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify =
            verify_live_package_substructure_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateVerifyOk
        );
    ...
    #[test]
    fn drifted_basal_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            "basal_receipt_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_basal_receipt",
                "verdict": "tiny_live_package_basal_receipt_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        let error = run_live_package_substructure_certificate_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
    ...
    #[test]
    fn verify_rejects_tampered_basal_receipt_step_path() {
        let fixture = fake_command_fixture(
            "basal_receipt_tampered_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_substructure_certificate_report(&fixture.run_config()).unwrap();
        let paths = basal_receipt_paths(&fixture.basal_receipt_session_dir);
        let mut status: PackageSubstructureCertificateStatus =
            load_json(&paths.status_path).unwrap();
        status.basal_receipt_step.as_mut().unwrap().path =
            "/tmp/foreign.basal-receipt.report.json".to_string();
        ...
        assert_eq!(
            verify.verdict,
            TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateVerifyInvalid
        );
    }

    #[test]
    fn verify_rejects_tampered_foundation_receipt_identity_text_in_substructure_session() {
        let fixture = fake_command_fixture(
            "substructure_certificate_tampered_foundation_identity_text",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        run_live_package_substructure_certificate_report(&fixture.run_config()).unwrap();
        let paths = basal_receipt_paths(&fixture.basal_receipt_session_dir);
        let mut session: PackageSubstructureCertificateSession =
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let mut status: PackageSubstructureCertificateStatus =
            load_json(&paths.status_path).unwrap();
        status.result = LivePackageSubstructureCertificateResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_basal_receipt_report_content() {
        let fixture = fake_command_fixture(
            "basal_receipt_tampered_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        run_live_package_substructure_certificate_report(&fixture.run_config()).unwrap();
        let paths = basal_receipt_paths(&fixture.basal_receipt_session_dir);
        let mut report: serde_json::Value = load_json(&paths.basal_receipt_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested basal-receipt reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_basal_receipt_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "basal_receipt_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut status: PackageSubstructureCertificateStatus =
            load_json(&paths.status_path).unwrap();
        status.basal_receipt_step.as_mut().unwrap().generated_at = forged_generated_at;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    fn verify_rejects_tampered_chain_ledger_registry_filing_archive_summit_closure_finality_or_consummation_identity_fields(
    ) {
        {
            ...
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_substructure_certificate_report(&fixture.verify_config())
                    .unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageSubstructureCertificateVerdict::TinyLivePackageSubstructureCertificateVerifyInvalid
    ...
    #[test]
    fn verify_rejects_tampered_substructure_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "substructure_certificate_tampered_substructure_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "substructure_certificate_tampered_substructure_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "substructure_certificate_tampered_substructure_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        basal_receipt_session_dir: PathBuf,
        input_basal_receipt_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageSubstructureCertificate,
                basal_receipt_session_dir: self.input_basal_receipt_session_dir.clone(),
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageSubstructureCertificate,
                basal_receipt_session_dir: self.input_basal_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let basal_receipt_session_dir = fixture_dir.join("substructure-certificate-session");
        let input_basal_receipt_session_dir = fixture_dir.join("basal-receipt-session");
        ...
        for path in [
            &input_basal_receipt_session_dir,
            &bedrock_certificate_session_dir,
            &foundation_receipt_session_dir,
        ...
        let verify_path = fixture_dir.join("basal-receipt.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_basal_receipt_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_basal_receipt"),
            &verify_path,
            &bedrock_certificate_session_dir,
            &decision_packet_session_dir,
            &input_basal_receipt_session_dir,
    ...
    fn bind_bedrock_certificate_report_to_fixture(
        ...
        object.insert(
            "verify_basal_receipt_command_summary".to_string(),
            json!(format!(
        ...
        object.insert(
            ...
            json!("basal-receipt verify statement"),
        );
    }
    ...
    fn bedrock_certificate_verify_report(
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_basal_receipt",
            "verdict": "tiny_live_package_basal_receipt_verify_ok",
            "reason": "basal-receipt chain is coherent",
            "result": result,
