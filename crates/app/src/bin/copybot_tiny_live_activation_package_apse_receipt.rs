...
use copybot_app::tiny_live_activation::package_chancel_certificate_apse_receipt;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_apse_receipt --chancel-certificate-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-apse-receipt | --render-live-package-apse-receipt | --run-live-package-apse-receipt | --verify-live-package-apse-receipt)";
const APSE_RECEIPT_SESSION_VERSION: &str = "1";
const APSE_RECEIPT_STATUS_VERSION: &str = "1";
const APSE_RECEIPT_ALGORITHM: &str = "sha256";
const APSE_RECEIPT_SESSION_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    chancel_certificate_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackageApseReceipt,
    RenderLivePackageApseReceipt,
    RunLivePackageApseReceipt,
    VerifyLivePackageApseReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageApseReceiptVerdict {
    TinyLivePackageApseReceiptPlanReady,
    TinyLivePackageApseReceiptRendered,
    TinyLivePackageApseReceiptRefusedNowByStage3,
    TinyLivePackageApseReceiptRefusedNowByPreActivationGate,
    TinyLivePackageApseReceiptRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageApseReceiptReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageApseReceiptVerifyOk,
    TinyLivePackageApseReceiptVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageApseReceiptResult {
    RefusedNowByStage3,
...
struct PackageApseReceiptStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageApseReceiptSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageApseReceiptStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackageApseReceiptResult,
    reason: String,
    ...
    chancel_certificate_step: Option<PackageApseReceiptStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageApseReceiptPaths {
...
struct PackageApseReceiptReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageApseReceiptVerdict,
    reason: String,
    chancel_certificate_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut chancel_certificate_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--chancel-certificate-session-dir" => {
                chancel_certificate_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--chancel-certificate-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--plan-live-package-apse-receipt" => {
                set_mode(&mut mode, Mode::PlanLivePackageApseReceipt, arg.as_str())?
            }
            "--render-live-package-apse-receipt" => {
                set_mode(&mut mode, Mode::RenderLivePackageApseReceipt, arg.as_str())?
            }
            "--run-live-package-apse-receipt" => {
                set_mode(&mut mode, Mode::RunLivePackageApseReceipt, arg.as_str())?
            }
            "--verify-live-package-apse-receipt" => {
                set_mode(&mut mode, Mode::VerifyLivePackageApseReceipt, arg.as_str())?
            }
    ...
    if matches!(mode, Mode::RenderLivePackageApseReceipt) && output_path.is_none() {
        bail!("missing --output for --render-live-package-apse-receipt");
    }
    if matches!(
        mode,
        Mode::RunLivePackageApseReceipt | Mode::VerifyLivePackageApseReceipt
    ) && session_dir.is_none()
    {
    ...
    let chancel_certificate_session_dir = chancel_certificate_session_dir
        .ok_or_else(|| anyhow!("missing --chancel-certificate-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageApseReceipt => plan_live_package_apse_receipt_report(&config)?,
        Mode::RenderLivePackageApseReceipt => render_live_package_apse_receipt_report(&config)?,
        Mode::RunLivePackageApseReceipt => run_live_package_apse_receipt_report(&config)?,
        Mode::VerifyLivePackageApseReceipt => verify_live_package_apse_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_live_package_apse_receipt_report(config: &Config) -> Result<PackageApseReceiptReport> {
    let (chancel_certificate_contract, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_chancel_certificate_apse_receipt::verify_live_package_chancel_certificate_for_apse_receipt(
                    &config.chancel_certificate_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_chancel_certificate(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_chancel_certificate_apse_receipt::load_live_package_chancel_certificate_contract_for_apse_receipt(
                            &config.chancel_certificate_session_dir,
                        )?;
                        let classification =
                            classify_chancel_certificate_contract(&raw_contract, false);
    ...
    let session_dir = config
        .session_dir
        .clone()
        .unwrap_or_else(|| config.chancel_certificate_session_dir.join("apse-receipt"));
    let result = serialize_enum(&classification.0);
    let apse_receipt_sha256 = compute_apse_receipt_sha256(
        &result,
        chancel_certificate_contract
        ...
        chancel_certificate_contract
            ...
            .as_deref(),
        &chancel_certificate_contract.reviewed_frozen_live_cutover_controller_command_summary,
        &chancel_certificate_contract.chain_fingerprint_sha256,
        &chancel_certificate_contract.chain_fingerprint_algorithm,
        &chancel_certificate_contract.release_capsule_digest_manifest_sha256,
        chancel_certificate_contract.release_capsule_digest_manifest_entry_count,
        &chancel_certificate_contract.release_capsule_digest_algorithm,
        &chancel_certificate_contract.ledger_seal_sha256,
        &chancel_certificate_contract.ledger_seal_algorithm,
        &chancel_certificate_contract.registry_entry_sha256,
        &chancel_certificate_contract.registry_entry_algorithm,
        &chancel_certificate_contract.filing_certificate_sha256,
        &chancel_certificate_contract.filing_certificate_algorithm,
        &chancel_certificate_contract.archive_receipt_sha256,
        &chancel_certificate_contract.archive_receipt_algorithm,
        &chancel_certificate_contract.closure_certificate_summary,
        &chancel_certificate_contract.closure_certificate_sha256,
        &chancel_certificate_contract.closure_certificate_algorithm,
        &chancel_certificate_contract.finality_receipt_sha256,
        &chancel_certificate_contract.finality_receipt_algorithm,
        &chancel_certificate_contract.consummation_record_sha256,
        &chancel_certificate_contract.consummation_record_algorithm,
        &chancel_certificate_contract.completion_certificate_summary,
        &chancel_certificate_contract.completion_certificate_sha256,
        &chancel_certificate_contract.completion_certificate_algorithm,
        &chancel_certificate_contract.summit_certificate_summary,
        &chancel_certificate_contract.summit_certificate_sha256,
        &chancel_certificate_contract.summit_certificate_algorithm,
        &chancel_certificate_contract.culmination_receipt_summary,
        &chancel_certificate_contract.culmination_receipt_sha256,
        &chancel_certificate_contract.culmination_receipt_algorithm,
        &chancel_certificate_contract.pinnacle_receipt_summary,
        &chancel_certificate_contract.pinnacle_receipt_sha256,
        &chancel_certificate_contract.pinnacle_receipt_algorithm,
        &chancel_certificate_contract.capstone_certificate_summary,
        &chancel_certificate_contract.capstone_certificate_sha256,
        &chancel_certificate_contract.capstone_certificate_algorithm,
        &chancel_certificate_contract.keystone_receipt_summary,
        &chancel_certificate_contract.keystone_receipt_sha256,
        &chancel_certificate_contract.keystone_receipt_algorithm,
        &chancel_certificate_contract.cornerstone_certificate_summary,
        &chancel_certificate_contract.cornerstone_certificate_sha256,
        &chancel_certificate_contract.cornerstone_certificate_algorithm,
        &chancel_certificate_contract.foundation_receipt_summary,
        &chancel_certificate_contract.foundation_receipt_sha256,
        &chancel_certificate_contract.foundation_receipt_algorithm,
        &chancel_certificate_contract.bedrock_certificate_summary,
        &chancel_certificate_contract.bedrock_certificate_sha256,
        &chancel_certificate_contract.bedrock_certificate_algorithm,
        &chancel_certificate_contract.basal_receipt_summary,
        &chancel_certificate_contract.basal_receipt_sha256,
        &chancel_certificate_contract.basal_receipt_algorithm,
        &chancel_certificate_contract.substructure_certificate_summary,
        &chancel_certificate_contract.substructure_certificate_sha256,
        &chancel_certificate_contract.substructure_certificate_algorithm,
        &chancel_certificate_contract.plinth_receipt_summary,
        &chancel_certificate_contract.plinth_receipt_sha256,
        &chancel_certificate_contract.plinth_receipt_algorithm,
        &chancel_certificate_contract.pedestal_certificate_summary,
        &chancel_certificate_contract.pedestal_certificate_sha256,
        &chancel_certificate_contract.pedestal_certificate_algorithm,
        &chancel_certificate_contract.dais_receipt_summary,
        &chancel_certificate_contract.dais_receipt_sha256,
        &chancel_certificate_contract.dais_receipt_algorithm,
        &chancel_certificate_contract.rostrum_certificate_summary,
        &chancel_certificate_contract.rostrum_certificate_sha256,
        &chancel_certificate_contract.rostrum_certificate_algorithm,
        &chancel_certificate_contract.podium_receipt_summary,
        &chancel_certificate_contract.podium_receipt_sha256,
        &chancel_certificate_contract.podium_receipt_algorithm,
        &chancel_certificate_contract.lectern_certificate_summary,
        &chancel_certificate_contract.lectern_certificate_sha256,
        &chancel_certificate_contract.lectern_certificate_algorithm,
        &chancel_certificate_contract.pulpit_receipt_summary,
        &chancel_certificate_contract.pulpit_receipt_sha256,
        &chancel_certificate_contract.pulpit_receipt_algorithm,
        &chancel_certificate_contract.chancel_certificate_summary,
        &chancel_certificate_contract.chancel_certificate_sha256,
        &chancel_certificate_contract.chancel_certificate_algorithm,
    )?;

    let lectern_certificate_contract = chancel_certificate_contract.clone();
    Ok(PackageApseReceiptReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_apse_receipt".to_string(),
        verdict:
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptPlanReady,
        reason: format!(
            "chancel-certificate-native apse receipt is explicit for verified chancel-certificate session {}; this command stays archival and read-only while freezing one final standard-seal identity over the fully culminated chain",
            config.chancel_certificate_session_dir.display()
        ),
        chancel_certificate_session_dir: config.chancel_certificate_session_dir.display().to_string(),
        ...
        archived_chancel_certificate_report_path: None,
        result: Some(result),
        ...
        verify_chancel_certificate_command_summary: Some(verify_chancel_certificate_command_summary(
            &config.chancel_certificate_session_dir,
        )),
        ...
        chancel_certificate_summary: Some(
            chancel_certificate_contract.chancel_certificate_summary.clone(),
        ),
        chancel_certificate_sha256: Some(
            chancel_certificate_contract.chancel_certificate_sha256.clone(),
        ),
        chancel_certificate_algorithm: Some(
            chancel_certificate_contract.chancel_certificate_algorithm.clone(),
        ),
        apse_receipt_summary: Some(apse_receipt_summary(
            classification.0,
            &chancel_certificate_contract.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        apse_receipt_sha256: Some(apse_receipt_sha256),
        apse_receipt_algorithm: Some(APSE_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: APSE_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_apse_receipt_report(config: &Config) -> Result<PackageApseReceiptReport> {
    let plan = plan_live_package_apse_receipt_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_apse_receipt --chancel-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-apse-receipt --json\ncopybot_tiny_live_activation_package_apse_receipt --chancel-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-apse-receipt --json\n",
        shell_escape_path(&config.chancel_certificate_session_dir),
    ...
    Ok(PackageApseReceiptReport {
        generated_at: Utc::now(),
        mode: "render_live_package_apse_receipt".to_string(),
        verdict:
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptRendered,
        reason: format!(
            "rendered apse-receipt run/verify script to {}; the primary input remains the immutable chancel-certificate session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: APSE_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_apse_receipt_report(config: &Config) -> Result<PackageApseReceiptReport> {
    let session_dir = config
        .session_dir
        .as_ref()
        .ok_or_else(|| anyhow!("missing --session-dir for run mode"))?;
    let verified_lectern_certificate = match package_chancel_certificate_apse_receipt::verify_live_package_chancel_certificate_for_apse_receipt(
        &config.chancel_certificate_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify chancel-certificate session under {}: {error}",
                    config.chancel_certificate_session_dir.display()
                ),
            ))
        }
    ...
    .with_context(|| {
        format!(
            "apse-receipt session dir {} overlaps the managed live target surface derived from verified chancel-certificate install_root {}",
            session_dir.display(),
            verified_lectern_certificate.contract.install_root.display()
        )
    })?;
    ensure_clean_apse_receipt_session_dir(session_dir)?;
    let paths = apse_receipt_paths(session_dir);
    let classification = classify_verified_chancel_certificate(&verified_lectern_certificate);
    ...
    let chancel_certificate_step = Some(step_artifact(
        &paths.chancel_certificate_report_path,
        "verify_live_package_chancel_certificate",
        &verified_lectern_certificate.verdict,
        &verified_lectern_certificate.reason,
        verified_lectern_certificate.generated_at,
    ));
    let status = PackageApseReceiptStatus {
        status_version: APSE_RECEIPT_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        chancel_certificate_session_dir: config
            .chancel_certificate_session_dir
            .display()
            .to_string(),
        ...
        chancel_certificate_algorithm: verified_lectern_certificate
            ...
            .chancel_certificate_algorithm
            .clone(),
        apse_receipt_summary: apse_receipt_summary(
            result,
            &verified_lectern_certificate
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        apse_receipt_sha256: apse_receipt_sha256.clone(),
        apse_receipt_algorithm: APSE_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: APSE_RECEIPT_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_apse_receipt_report(config: &Config) -> Result<PackageApseReceiptReport> {
    let session_dir = config
        .session_dir
        .as_ref()
        .ok_or_else(|| anyhow!("missing --session-dir for verify mode"))?;
    let paths = apse_receipt_paths(session_dir);
    let session: PackageApseReceiptSession = load_json(&paths.session_path)?;
    let status: PackageApseReceiptStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.chancel_certificate_session_dir,
        &config.chancel_certificate_session_dir.display().to_string(),
        "chancel-certificate chancel_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
    ...
    compare_string(
        &status.chancel_certificate_session_dir,
        &config.chancel_certificate_session_dir.display().to_string(),
        "chancel-certificate status chancel_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "apse-receipt substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "apse-receipt status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "apse-receipt bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "apse-receipt status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "apse-receipt foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "apse-receipt status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(stored_lectern_certificate_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested chancel-certificate report {}",
                paths.chancel_certificate_report_path.display()
            )
        })?;
    let verified_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(verified_lectern_certificate.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested chancel-certificate report for {}",
                    config.chancel_certificate_session_dir.display()
                )
            },
        )?;
        ...
        != verified_lectern_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "apse-receipt archived chancel-certificate report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_lectern_certificate_report_view.generated_at,
            verified_lectern_certificate_report_view.generated_at
        ));
    }
    let expected_chancel_certificate_step = step_artifact(
        &paths.chancel_certificate_report_path,
        &verified_lectern_certificate_report_view.mode,
        &verified_lectern_certificate_report_view.verdict,
        &verified_lectern_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_chancel_certificate_step,
        "apse-receipt chancel_certificate_step",
        &mut mismatches,
    );
    if verified_lectern_certificate.verdict != "tiny_live_package_chancel_certificate_verify_ok" {
        mismatches.push(format!(
            "apse-receipt nested chancel-certificate verification is non-green: {}",
            verified_lectern_certificate.reason
        ));
    }

    let classification = classify_verified_chancel_certificate(&verified_lectern_certificate);
    let expected_apse_receipt_sha256 = compute_apse_receipt_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_lectern_certificate.contract,
        classification.0,
        &expected_apse_receipt_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "apse-receipt session contract does not match freshly verified chancel-certificate truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_lectern_certificate.contract,
        classification,
        expected_chancel_certificate_step.clone(),
        &expected_apse_receipt_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "apse-receipt status contract does not match freshly verified chancel-certificate truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_lectern_certificate.contract,
        TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptVerifyOk,
        &expected_status,
    ))
}

fn verify_chancel_certificate_command_summary(chancel_certificate_session_dir: &Path) -> String {
    format!(
...
fn classify_chancel_certificate_contract(
    contract: &package_chancel_certificate_apse_receipt::LivePackageChancelCertificateContractView,
    assume_verified: bool,
) -> (
    LivePackageApseReceiptResult,
    TinyLivePackageApseReceiptVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageApseReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptRefusedNowByInvalidOrDriftedContract,
            "the chancel-certificate chain is not verified, so this apse receipt remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageApseReceiptResult::RefusedNowByStage3,
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptRefusedNowByStage3,
            "the immutable chancel certificate remains refused now by Stage 3, so this apse receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageApseReceiptResult::RefusedNowByPreActivationGate,
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptRefusedNowByPreActivationGate,
            "the immutable chancel certificate remains refused now by the pre-activation gate, so this apse receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageApseReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable chancel certificate is coherent and this apse receipt is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageApseReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptRefusedNowByInvalidOrDriftedContract,
            "the verified chancel-certificate chain is invalid, drifted, or non-runnable, so this apse receipt remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_chancel_certificate(
    verified: &package_chancel_certificate_apse_receipt::VerifiedLivePackageChancelCertificateApseReceiptStep,
) -> (
    LivePackageApseReceiptResult,
    TinyLivePackageApseReceiptVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_chancel_certificate_verify_ok" {
        return (
            LivePackageApseReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptRefusedNowByInvalidOrDriftedContract,
            "the immutable chancel certificate did not verify cleanly, so this apse receipt remains refused"
                .to_string(),
        );
    }
    classify_chancel_certificate_contract(&verified.contract, true)
}

fn apse_receipt_summary(
    result: LivePackageApseReceiptResult,
    ...
    match result {
        ...
        LivePackageApseReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
...
fn apse_receipt_paths(session_dir: &Path) -> PackageApseReceiptPaths {
    PackageApseReceiptPaths {
        session_path: session_dir.join("tiny_live_activation_package_apse_receipt.session.json"),
        status_path: session_dir.join("tiny_live_activation_package_apse_receipt.status.json"),
        chancel_certificate_report_path: session_dir
            .join("tiny_live_activation_package_apse_receipt.chancel_certificate.report.json"),
    }
}

fn ensure_clean_apse_receipt_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing apse-receipt session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating apse-receipt session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageApseReceiptSession {
    PackageApseReceiptSession {
        session_version: APSE_RECEIPT_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        chancel_certificate_session_dir: config
            .chancel_certificate_session_dir
            .display()
        ...
        session_dir: session_dir.display().to_string(),
        verify_chancel_certificate_command_summary: verify_chancel_certificate_command_summary(
            &config.chancel_certificate_session_dir,
        ),
        ...
        apse_receipt_sha256: apse_receipt_sha256.to_string(),
        apse_receipt_algorithm: APSE_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: APSE_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_session_from_verified(
    ...
    PackageApseReceiptSession {
        planned_at,
        ..planned_session(config, session_dir, contract, result, apse_receipt_sha256)
    }
}
...
fn expected_status_from_verified(
    ...
    chancel_certificate_step: PackageApseReceiptStepArtifact,
    apse_receipt_sha256: &str,
) -> PackageApseReceiptStatus {
    PackageApseReceiptStatus {
        status_version: APSE_RECEIPT_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        chancel_certificate_step: Some(chancel_certificate_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageApseReceiptPaths,
    contract: &package_chancel_certificate_apse_receipt::LivePackageChancelCertificateContractView,
    verdict: TinyLivePackageApseReceiptVerdict,
    status: &PackageApseReceiptStatus,
) -> PackageApseReceiptReport {
    PackageApseReceiptReport {
        generated_at: Utc::now(),
        mode: if verdict == TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptVerifyOk {
            "verify_live_package_apse_receipt".to_string()
        } else {
            "run_live_package_apse_receipt".to_string()
        },
        verdict,
        ...
        archived_chancel_certificate_report_path: Some(
        ...
        chancel_certificate_step: status.chancel_certificate_step.clone(),
        verify_chancel_certificate_command_summary: Some(
            verify_chancel_certificate_command_summary(&config.chancel_certificate_session_dir),
        ),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
            contract
                .reviewed_frozen_live_cutover_controller_command_summary
        ...
        explicit_statement: if verdict
            == TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptVerifyOk
        {
            APSE_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageApseReceiptReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageApseReceipt => {
                "plan_live_package_apse_receipt".to_string()
            }
            Mode::RenderLivePackageApseReceipt => {
                "render_live_package_apse_receipt".to_string()
            }
            Mode::RunLivePackageApseReceipt => {
                "run_live_package_apse_receipt".to_string()
            }
            Mode::VerifyLivePackageApseReceipt => {
                "verify_live_package_apse_receipt".to_string()
            }
        },
        verdict,
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable apse receipt refused before persisting because the chancel-certificate artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "chancel-certificate-native apse-receipt verification failed".to_string()
    });
    PackageApseReceiptReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_apse_receipt".to_string(),
        verdict: TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptVerifyInvalid,
        reason,
        chancel_certificate_session_dir: config
            .chancel_certificate_session_dir
            .display()
        ...
        explicit_statement: APSE_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageApseReceiptStepArtifact>,
    expected_path: &Path,
    label: &str,
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageApseReceiptStepArtifact {
    PackageApseReceiptStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageApseReceiptStepArtifact>,
    expected: &PackageApseReceiptStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_apse_receipt_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&ApseReceiptPayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn render_report_lines(report: &PackageApseReceiptReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "chancel_certificate_session_dir={}",
            report.chancel_certificate_session_dir
        ),
    ];
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_apse_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify = verify_live_package_apse_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptVerifyOk
        );
    ...
    #[test]
    fn drifted_chancel_certificate_contract_is_refused() {
        let fixture = fake_command_fixture(
            "chancel_certificate_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_chancel_certificate",
                "verdict": "tiny_live_package_chancel_certificate_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        .unwrap();
        let overlap_session_dir = managed_paths.runtime_dir.join("apse-receipt-session");
        ...
        let error = run_live_package_apse_receipt_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = apse_receipt_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_chancel_certificate_step_path() {
        let fixture = fake_command_fixture(
            "chancel_certificate_tampered_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_apse_receipt_report(&fixture.run_config()).unwrap();
        let paths = apse_receipt_paths(&fixture.chancel_certificate_session_dir);
        let mut status: PackageApseReceiptStatus = load_json(&paths.status_path).unwrap();
        status.chancel_certificate_step.as_mut().unwrap().path =
            "/tmp/foreign.chancel-certificate.report.json".to_string();
        persist_json(&paths.status_path, &status).unwrap();

        let verify = verify_live_package_apse_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageApseReceiptVerdict::TinyLivePackageApseReceiptVerifyInvalid
        );
    }
    ...
    fn verify_rejects_tampered_rostrum_certificate_sha256_summary_or_algorithm() {
        {
            ...
            run_live_package_apse_receipt_report(&fixture.run_config()).unwrap();
            let paths = apse_receipt_paths(&fixture.chancel_certificate_session_dir);
            let mut session: PackageApseReceiptSession = load_json(&paths.session_path).unwrap();
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = apse_receipt_paths(&fixture.chancel_certificate_session_dir);
        let mut status: PackageApseReceiptStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageApseReceiptResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_chancel_certificate_report_content() {
        let fixture = fake_command_fixture(
            "chancel_certificate_tampered_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut report: serde_json::Value =
            load_json(&paths.chancel_certificate_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested chancel-certificate reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_chancel_certificate_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "chancel_certificate_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        chancel_certificate_session_dir: PathBuf,
        input_chancel_certificate_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageApseReceipt,
                chancel_certificate_session_dir: self.input_chancel_certificate_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.chancel_certificate_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageApseReceipt,
                chancel_certificate_session_dir: self.input_chancel_certificate_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let chancel_certificate_session_dir = fixture_dir.join("apse-receipt-session");
        let input_chancel_certificate_session_dir = fixture_dir.join("chancel-certificate-session");
        let lectern_certificate_session_dir = fixture_dir.join("lectern-certificate-session");
        let pulpit_receipt_session_dir = fixture_dir.join("pulpit-receipt-session");
        ...
        for path in [
            &input_chancel_certificate_session_dir,
        ...
        persist_json(
            ...
            &stored_chancel_certificate_session_fixture(
                &lectern_certificate_session_dir,
                &pulpit_receipt_session_dir,
                &rostrum_certificate_session_dir,
                &pedestal_certificate_session_dir,
        ...
        persist_json(
            ...
            &stored_chancel_certificate_status_fixture(
                &lectern_certificate_session_dir,
                &pulpit_receipt_session_dir,
                &rostrum_certificate_session_dir,
                &pedestal_certificate_session_dir,
        ...
        .unwrap();
        persist_json(
            &input_chancel_certificate_session_dir.join(
                "tiny_live_activation_package_chancel_certificate.pulpit_receipt.report.json",
            ),
            &json!({
        ...
        bind_chancel_certificate_report_to_fixture(
            &mut verify_report,
            &lectern_certificate_session_dir,
            &pulpit_receipt_session_dir,
            &rostrum_certificate_session_dir,
        ...
        let verify_path = fixture_dir.join("chancel-certificate.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_chancel_certificate_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_chancel_certificate"),
            &verify_path,
            &pulpit_receipt_session_dir,
            &decision_packet_session_dir,
            &input_chancel_certificate_session_dir,
    ...
    fn bind_chancel_certificate_report_to_fixture(
        ...
        object.insert(
            ...
            json!(pulpit_receipt_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            "verify_pulpit_receipt_command_summary".to_string(),
            json!(format!(
        ...
        object.insert(
            ...
            json!("chancel-certificate verify statement"),
        );
    }

    fn chancel_certificate_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_chancel_certificate",
            "verdict": "tiny_live_package_chancel_certificate_verify_ok",
            "reason": "chancel-certificate chain is coherent",
            "result": result,
    ...
    fn bedrock_certificate_verify_report(
        ...
        chancel_certificate_verify_report(
            result,
            handoff_bundle_result,
            decision_packet_result,
            execute_frozen_result,
    ...
    fn stored_chancel_certificate_session_fixture(
        lectern_certificate_session_dir: &Path,
        pulpit_receipt_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("chancel-certificate statement"),
        );
        Value::Object(object)
    }

    fn stored_chancel_certificate_status_fixture(
        lectern_certificate_session_dir: &Path,
        pulpit_receipt_session_dir: &Path,
        _rostrum_certificate_session_dir: &Path,
        pedestal_certificate_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("chancel-certificate status statement"),
        );
        Value::Object(object)
    }
}
