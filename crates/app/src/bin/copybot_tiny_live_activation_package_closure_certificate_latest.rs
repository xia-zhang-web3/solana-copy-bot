...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_closure_certificate_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-closure-certificate | --render-latest-closure-certificate-script | --run-latest-closure-certificate | --verify-latest-closure-certificate)";
const CLOSURE_CERTIFICATE_LATEST_SESSION_VERSION: &str = "1";
const CLOSURE_CERTIFICATE_LATEST_STATUS_VERSION: &str = "1";
const ARCHIVE_RECEIPT_LATEST_BIN: &str =
    "copybot_tiny_live_activation_package_archive_receipt_latest";
const CLOSURE_CERTIFICATE_BIN: &str = "copybot_tiny_live_activation_package_closure_certificate";
const ARCHIVE_RECEIPT_LATEST_PLAN_READY: &str =
    "tiny_live_package_archive_receipt_latest_plan_ready";
const ARCHIVE_RECEIPT_LATEST_VERIFY_OK: &str = "tiny_live_package_archive_receipt_latest_verify_ok";
const CLOSURE_CERTIFICATE_VERIFY_OK: &str = "tiny_live_package_closure_certificate_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-closure-certificate handoff binds the latest persisted immutable package chain to the accepted closure-certificate contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestClosureCertificate,
    RenderLatestClosureCertificateScript,
    RunLatestClosureCertificate,
    VerifyLatestClosureCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageClosureCertificateLatestVerdict {
    TinyLivePackageClosureCertificateLatestPlanReady,
    TinyLivePackageClosureCertificateLatestScriptRendered,
    TinyLivePackageClosureCertificateLatestRefusedByMissingLatestChain,
    TinyLivePackageClosureCertificateLatestRefusedByInvalidLatestChain,
    TinyLivePackageClosureCertificateLatestRefusedByDriftedClosureCertificateContract,
    TinyLivePackageClosureCertificateLatestRefusedNowByStage3,
    TinyLivePackageClosureCertificateLatestRefusedNowByPreActivationGate,
    TinyLivePackageClosureCertificateLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageClosureCertificateLatestVerifyOk,
    TinyLivePackageClosureCertificateLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageClosureCertificateLatestResult {
    RefusedByMissingLatestChain,
    RefusedByInvalidLatestChain,
    RefusedByDriftedClosureCertificateContract,
    RefusedNowByStage3,
...
struct PackageClosureCertificateLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageClosureCertificateLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_archive_receipt_latest_session_dir: String,
    nested_closure_certificate_session_dir: String,
    ...
    resolve_latest_archive_receipt_command_summary: String,
    run_latest_archive_receipt_command_summary: String,
    verify_latest_archive_receipt_command_summary: String,
    downstream_closure_certificate_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageClosureCertificateLatestStatus {
    ...
    nested_closure_certificate_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackageClosureCertificateLatestResult,
    reason: String,
    latest_archive_receipt_plan_verdict: Option<String>,
    latest_archive_receipt_verdict: Option<String>,
    closure_certificate_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_archive_receipt_plan_step: Option<PackageClosureCertificateLatestStepArtifact>,
    latest_archive_receipt_step: Option<PackageClosureCertificateLatestStepArtifact>,
    closure_certificate_step: Option<PackageClosureCertificateLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackageClosureCertificateLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_archive_receipt_plan_report_path: PathBuf,
    latest_archive_receipt_report_path: PathBuf,
    closure_certificate_report_path: PathBuf,
    nested_archive_receipt_latest_session_dir: PathBuf,
    nested_closure_certificate_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageClosureCertificateLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageClosureCertificateLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    closure_certificate_latest_session_path: Option<String>,
    closure_certificate_latest_status_path: Option<String>,
    closure_certificate_latest_report_path: Option<String>,
    latest_archive_receipt_plan_report_path: Option<String>,
    latest_archive_receipt_report_path: Option<String>,
    closure_certificate_report_path: Option<String>,
    nested_archive_receipt_latest_session_dir: Option<String>,
    nested_closure_certificate_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_archive_receipt_command_summary: Option<String>,
    run_latest_archive_receipt_command_summary: Option<String>,
    verify_latest_archive_receipt_command_summary: Option<String>,
    downstream_closure_certificate_command_summary: Option<String>,
...
#[derive(Debug, Clone)]
struct LatestArchiveReceiptSnapshot {
...
#[derive(Debug, Clone)]
struct ResolvedLatestArchiveReceipt {
    snapshot: LatestArchiveReceiptSnapshot,
    latest_archive_receipt_plan_raw: Value,
    latest_archive_receipt_plan: ArchiveReceiptLatestReportView,
}
...
enum ResolutionFailureKind {
    MissingLatestChain,
    InvalidLatestChain,
    DriftedClosureCertificateContract,
}

#[derive(Debug, Clone)]
struct LatestArchiveReceiptResolutionFailure {
    kind: ResolutionFailureKind,
    reason: String,
    snapshot: Option<LatestArchiveReceiptSnapshot>,
    latest_archive_receipt_plan_raw: Option<Value>,
    latest_archive_receipt_plan_verdict: Option<String>,
    latest_archive_receipt_plan_reason: Option<String>,
    latest_archive_receipt_plan_generated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone, Deserialize)]
struct ArchiveReceiptLatestReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Deserialize)]
struct ClosureCertificateReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    archive_receipt_step: Option<PackageClosureCertificateLatestStepArtifact>,
    verify_archive_receipt_command_summary: Option<String>,
    reviewed_frozen_live_cutover_controller_command_summary: Option<String>,
...
#[derive(Debug, Clone, Deserialize)]
struct ClosureCertificateSessionView {
    archive_receipt_session_dir: String,
...
#[derive(Debug, Clone, Deserialize)]
struct ClosureCertificateStatusView {
    session_dir: String,
    archive_receipt_session_dir: String,
    result: String,
    reason: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-closure-certificate" => {
                set_mode(&mut mode, Mode::PlanLatestClosureCertificate, arg.as_str())?
            }
            "--render-latest-closure-certificate-script" => set_mode(
                &mut mode,
                Mode::RenderLatestClosureCertificateScript,
                arg.as_str(),
            )?,
            "--run-latest-closure-certificate" => {
                set_mode(&mut mode, Mode::RunLatestClosureCertificate, arg.as_str())?
            }
            "--verify-latest-closure-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLatestClosureCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(
        mode,
        Mode::PlanLatestClosureCertificate
            | Mode::RenderLatestClosureCertificateScript
            | Mode::RunLatestClosureCertificate
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestClosureCertificateScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestClosureCertificate | Mode::VerifyLatestClosureCertificate
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestClosureCertificate => plan_latest_closure_certificate_report(&config)?,
        Mode::RenderLatestClosureCertificateScript => {
            render_latest_closure_certificate_script_report(&config)?
        }
        Mode::RunLatestClosureCertificate => run_latest_closure_certificate_report(&config)?,
        Mode::VerifyLatestClosureCertificate => verify_latest_closure_certificate_report(&config)?,
    };
    if config.json {
...
fn plan_latest_closure_certificate_report(
    config: &Config,
) -> Result<PackageClosureCertificateLatestReport> {
    let root = config
        .root
        .as_ref()
    ...
    let paths = archive_receipt_latest_paths(&suggested_session_dir);

    match resolve_latest_archive_receipt(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            "plan_latest_closure_certificate",
            TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-archive-receipt {} lineage, exact decision-packet confirmation anchor {}, and the accepted closure-certificate contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_archive_receipt_latest_session_dir.display(),
    ...
    match resolve_latest_archive_receipt(root, &paths) {
        Ok(resolved) => {
            write_script(
                ...
                &format!(
                    ...
                    run_latest = run_latest_archive_receipt_command_summary(
                        root,
                        &paths.nested_archive_receipt_latest_session_dir,
                    ),
                    verify_latest = verify_latest_archive_receipt_command_summary(
                        &paths.nested_archive_receipt_latest_session_dir,
                    ),
                    run_handoff_bundle = downstream_closure_certificate_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_archive_receipt_session_dir(&paths),
                        &paths.nested_closure_certificate_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                ...
                TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestScriptRendered,
                format!(
                    "rendered latest-closure-certificate handoff script to {} using the accepted latest-archive-receipt and closure-certificate contracts",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_closure_certificate_report(
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-closure-certificate session dir {}",
            session_dir.display()
        );
    }
    let paths = archive_receipt_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-closure-certificate")?;

    let resolution = resolve_latest_archive_receipt(root, &paths);
    match resolution {
        Ok(resolved) => {
...
fn verify_latest_closure_certificate_report(
    ...
    let session: PackageClosureCertificateLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_archive_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-closure-certificate session artifact {}: {error}",
    ...
    let status: PackageClosureCertificateLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_archive_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-closure-certificate status artifact {}: {error}",
    ...
    let stored_report: PackageClosureCertificateLatestReport = match load_json(&paths.report_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_archive_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-closure-certificate report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-closure-certificate root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-closure-certificate session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-closure-certificate status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-closure-certificate report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_archive_receipt_latest_session_dir,
        &paths
            .nested_archive_receipt_latest_session_dir
            .display()
            .to_string(),
        "latest-closure-certificate session nested_archive_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_archive_receipt_latest_session_dir,
        &paths
            .nested_archive_receipt_latest_session_dir
            .display()
            .to_string(),
        "latest-closure-certificate status nested_archive_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-closure-certificate report nested_archive_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_closure_certificate_session_dir,
        &paths
            .nested_closure_certificate_session_dir
            .display()
            .to_string(),
        "latest-closure-certificate session nested_closure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_closure_certificate_session_dir,
        &paths
            .nested_closure_certificate_session_dir
            .display()
            .to_string(),
        "latest-closure-certificate status nested_closure_certificate_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-closure-certificate report nested_closure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-closure-certificate session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-closure-certificate status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-closure-certificate report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .closure_certificate_latest_session_path
            .as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-closure-certificate report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .closure_certificate_latest_status_path
            .as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-closure-certificate report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .closure_certificate_latest_report_path
            .as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-closure-certificate report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-closure-certificate session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-closure-certificate status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-closure-certificate report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_closure_certificate",
        "latest-closure-certificate stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-closure-certificate stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-closure-certificate stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-closure-certificate stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-closure-certificate stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report.latest_archive_receipt_plan_verdict.as_deref(),
        status.latest_archive_receipt_plan_verdict.as_deref(),
        "stored report latest_archive_receipt_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        stored_report.closure_certificate_verdict.as_deref(),
        status.closure_certificate_verdict.as_deref(),
        "stored report closure_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: ArchiveReceiptLatestReportView = load_required_step_json(
                &status.latest_archive_receipt_plan_step,
                &paths.latest_archive_receipt_plan_report_path,
                "latest_archive_receipt_plan_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_archive_receipt_plan_step.as_ref(),
                &paths.latest_archive_receipt_plan_report_path,
                "plan_latest_archive_receipt",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_archive_receipt_plan_step",
                &mut mismatches,
            );
            compare_archive_receipt_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_archive_receipt_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_archive_receipt_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_archive_receipt_plan_verdict.as_deref(),
            ...
            let stored_latest_run: ArchiveReceiptLatestReportView = load_required_step_json(
            ...
            compare_archive_receipt_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_archive_receipt_latest_session_dir,
                &mut mismatches,
            ...
            let actual_latest_run: ArchiveReceiptLatestReportView = match load_json(
                &nested_archive_receipt_latest_report_path(
                    &paths.nested_archive_receipt_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed loading actual nested latest-archive-receipt report under {}: {error}",
                        paths.nested_archive_receipt_latest_session_dir.display()
                    ));
                    fallback_archive_receipt_latest_copy()
                }
            };
            compare_archive_receipt_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-archive-receipt session under {}: {error}",
                        paths.nested_archive_receipt_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: ArchiveReceiptLatestReportView =
                    serde_json::from_value(latest_verify_raw)
                        .context("failed parsing nested latest-archive-receipt verify json")?;
                if latest_verify.verdict != ARCHIVE_RECEIPT_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-archive-receipt verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: ClosureCertificateReportView = load_required_step_json(
            ...
            compare_option_string(
                ...
                "stored status closure_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.closure_certificate_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: ClosureCertificateSessionView =
                match load_json(&nested_closure_certificate_session_path(
                    &paths.nested_closure_certificate_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                            "failed reading nested closure-certificate session under {}: {error}",
                            paths.nested_closure_certificate_session_dir.display()
                        ));
            ...
            let nested_handoff_bundle_status: ClosureCertificateStatusView =
                match load_json(&nested_closure_certificate_status_path(
                    &paths.nested_closure_certificate_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                            "failed reading nested closure-certificate status under {}: {error}",
                            paths.nested_closure_certificate_session_dir.display()
                        ));
            ...
            let handoff_bundle_verify_raw = match run_json_command(
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested closure-certificate session under {}: {error}",
                        paths.nested_closure_certificate_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: ClosureCertificateReportView =
                    serde_json::from_value(handoff_bundle_verify_raw)
                        .context("failed parsing nested closure-certificate verify json")?;
                compare_option_string(
                    stored_handoff_bundle.registry_entry_session_dir.as_deref(),
                    handoff_bundle_verify.registry_entry_session_dir.as_deref(),
                    "stored closure-certificate copy registry_entry_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored closure-certificate copy notarization_receipt_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .reviewed_frozen_live_cutover_controller_command_summary
                        .as_deref(),
                    "stored closure-certificate copy reviewed_frozen_live_cutover_controller_command_summary",
                    &mut mismatches,
                );
                if handoff_bundle_verify.verdict != CLOSURE_CERTIFICATE_VERIFY_OK {
                    mismatches.push(format!(
            ...
            .unwrap_or(
                LivePackageClosureCertificateLatestResult::RefusedByDriftedClosureCertificateContract,
            );
            if status.result != expected_result {
                mismatches.push(format!(
                    "stored latest-closure-certificate result {} does not match nested closure-certificate result {}",
            ...
            if status.activation_authorized {
                mismatches.push(
                    "stored status activation_authorized must remain false for latest-closure-certificate"
                        .to_string(),
                );
            }
            compare_option_string(
        ...
        Err(failure) => {
            ...
            if failure.latest_archive_receipt_plan_raw.is_some() {
                let stored_latest_plan: ArchiveReceiptLatestReportView = load_required_step_json(
                    &status.latest_archive_receipt_plan_step,
                    &paths.latest_archive_receipt_plan_report_path,
                    "latest_archive_receipt_plan_step",
                ...
                compare_option_string(
                    status.latest_archive_receipt_plan_verdict.as_deref(),
                    failure.latest_archive_receipt_plan_verdict.as_deref(),
                    "stored status latest_archive_receipt_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report.latest_archive_receipt_plan_verdict.as_deref(),
                    failure.latest_archive_receipt_plan_verdict.as_deref(),
                    "stored report latest_archive_receipt_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
            ...
            compare_option_string(
                status.closure_certificate_verdict.as_deref(),
                None,
                "stored status closure_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.closure_certificate_verdict.as_deref(),
                None,
                "stored report closure_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestVerifyOk
    } else {
        TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-closure-certificate session under {} is coherent with the current latest immutable chain and the accepted closure-certificate contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-closure-certificate verification failed".to_string())
    };

    Ok(PackageClosureCertificateLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_closure_certificate".to_string(),
        verdict,
        reason,
        ...
        closure_certificate_latest_session_path: Some(paths.session_path.display().to_string()),
        closure_certificate_latest_status_path: Some(paths.status_path.display().to_string()),
        closure_certificate_latest_report_path: Some(paths.report_path.display().to_string()),
        latest_archive_receipt_plan_report_path: Some(
            paths
                .latest_archive_receipt_plan_report_path
                .display()
                .to_string(),
        ),
        ...
        closure_certificate_report_path: Some(paths.closure_certificate_report_path.display().to_string()),
        nested_archive_receipt_latest_session_dir: Some(
            paths
                .nested_archive_receipt_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_closure_certificate_session_dir: Some(
            paths.nested_closure_certificate_session_dir.display().to_string(),
        ),
        ...
        latest_archive_receipt_plan_verdict: status.latest_archive_receipt_plan_verdict.clone(),
        latest_archive_receipt_verdict: status.latest_archive_receipt_verdict.clone(),
        closure_certificate_verdict: status.closure_certificate_verdict.clone(),
        ...
        resolve_latest_archive_receipt_command_summary: Some(
            session.resolve_latest_archive_receipt_command_summary.clone(),
        ),
        ...
        downstream_closure_certificate_command_summary: Some(
            session.downstream_closure_certificate_command_summary.clone(),
        ),
        ...
        explicit_statement:
            "this verification proves only whether the latest immutable chain still resolves cleanly into the accepted closure-certificate contract; it never runs the frozen controller"
                .to_string(),
    })
}
...
fn run_latest_closure_certificate_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackageClosureCertificateLatestPaths,
    resolved: &ResolvedLatestArchiveReceipt,
) -> Result<PackageClosureCertificateLatestReport> {
    let session = planned_session(root, session_dir, paths, Some(resolved), None);
    persist_json(&paths.session_path, &session)?;
    persist_json(
        &paths.latest_archive_receipt_plan_report_path,
        &resolved.latest_archive_receipt_plan_raw,
    )?;

    let latest_archive_receipt_plan_step = Some(step_artifact(
        &paths.latest_archive_receipt_plan_report_path,
        "plan_latest_archive_receipt",
        &resolved.latest_archive_receipt_plan.verdict,
        &resolved.latest_archive_receipt_plan.reason,
        resolved.latest_archive_receipt_plan.generated_at,
    ));

    let latest_archive_receipt_run_raw = match run_json_command(
        ARCHIVE_RECEIPT_LATEST_BIN,
        &latest_archive_receipt_run_args(root, &paths.nested_archive_receipt_latest_session_dir),
    ) {
        Ok(value) => value,
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageClosureCertificateLatestResult::RefusedByDriftedClosureCertificateContract,
                format!(
                    "failed running nested latest-archive-receipt handoff under {}: {error}",
                    paths.nested_archive_receipt_latest_session_dir.display()
                ),
                latest_archive_receipt_plan_step,
                None,
    ...
    let latest_archive_receipt_run: ArchiveReceiptLatestReportView =
        serde_json::from_value(latest_archive_receipt_run_raw)
            .context("failed parsing nested latest-archive-receipt run json")?;
    ...
    if let Some(terminal_failure) =
        latest_archive_receipt_terminal_failure(&latest_archive_receipt_run)
    {
        let status = refusal_status(
            root,
            session_dir,
    ...
    let latest_archive_receipt_verify: ArchiveReceiptLatestReportView = match run_json_command(
        ARCHIVE_RECEIPT_LATEST_BIN,
        &latest_archive_receipt_verify_args(&paths.nested_archive_receipt_latest_session_dir),
    ) {
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageClosureCertificateLatestResult::RefusedByDriftedClosureCertificateContract,
                format!(
                    "failed verifying nested latest-archive-receipt handoff under {}: {error}",
                    paths.nested_archive_receipt_latest_session_dir.display()
                ),
                latest_archive_receipt_plan_step,
                latest_archive_receipt_step,
    ...
    if latest_archive_receipt_verify.verdict != ARCHIVE_RECEIPT_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    let provenance_certificate_run: ClosureCertificateReportView =
        serde_json::from_value(release_capsule_run_raw)
            .context("failed parsing nested closure-certificate run json")?;
    ...
    let provenance_certificate_verify: ClosureCertificateReportView = match run_json_command(
        CLOSURE_CERTIFICATE_BIN,
        &archive_receipt_verify_args(
            &resolved.snapshot,
            &expected_downstream_archive_receipt_session_dir(paths),
    ...
    if provenance_certificate_verify.verdict != CLOSURE_CERTIFICATE_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    let status = PackageClosureCertificateLatestStatus {
        status_version: CLOSURE_CERTIFICATE_LATEST_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nested_archive_receipt_latest_session_dir: paths
            .nested_archive_receipt_latest_session_dir
            .display()
            .to_string(),
        nested_closure_certificate_session_dir: paths
            .nested_closure_certificate_session_dir
            .display()
            .to_string(),
        ...
        latest_archive_receipt_plan_verdict: Some(
            resolved.latest_archive_receipt_plan.verdict.clone(),
        ),
        latest_archive_receipt_verdict: Some(latest_archive_receipt_run.verdict.clone()),
        closure_certificate_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_closure_certificate_failure(
    ...
    session_dir: &Path,
    paths: &PackageClosureCertificateLatestPaths,
    failure: &LatestArchiveReceiptResolutionFailure,
) -> Result<PackageClosureCertificateLatestReport> {
    let session = planned_session(root, session_dir, paths, None, Some(failure));
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_archive_receipt_plan_raw {
        persist_json(&paths.latest_archive_receipt_plan_report_path, value)?;
    }
...
fn resolve_latest_archive_receipt(
    root: &Path,
    paths: &PackageClosureCertificateLatestPaths,
) -> std::result::Result<ResolvedLatestArchiveReceipt, LatestArchiveReceiptResolutionFailure> {
    let latest_archive_receipt_plan_raw = match run_json_command(
        ARCHIVE_RECEIPT_LATEST_BIN,
        &latest_archive_receipt_plan_args(root, &paths.nested_archive_receipt_latest_session_dir),
    ) {
        Ok(value) => value,
        Err(error) => {
            return Err(LatestArchiveReceiptResolutionFailure {
                ...
                snapshot: None,
                latest_archive_receipt_plan_raw: None,
                latest_archive_receipt_plan_verdict: None,
                latest_archive_receipt_plan_reason: None,
                latest_archive_receipt_plan_generated_at: None,
            });
        }
    };
    let latest_archive_receipt_plan: ArchiveReceiptLatestReportView =
        serde_json::from_value(latest_archive_receipt_plan_raw.clone()).map_err(|error| {
            LatestArchiveReceiptResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-archive-receipt plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_archive_receipt_plan_raw: Some(latest_archive_receipt_plan_raw.clone()),
                latest_archive_receipt_plan_verdict: None,
                latest_archive_receipt_plan_reason: None,
                latest_archive_receipt_plan_generated_at: None,
            }
    ...
    let snapshot = parse_snapshot_from_archive_receipt_latest(&latest_archive_receipt_plan).ok();
    if latest_archive_receipt_plan.verdict != ARCHIVE_RECEIPT_LATEST_PLAN_READY {
        return Err(LatestArchiveReceiptResolutionFailure {
            kind: map_archive_receipt_latest_failure_kind(&latest_archive_receipt_plan.verdict),
            reason: latest_archive_receipt_plan.reason.clone(),
            snapshot,
            latest_archive_receipt_plan_raw: Some(latest_archive_receipt_plan_raw),
            latest_archive_receipt_plan_verdict: Some(latest_archive_receipt_plan.verdict),
            latest_archive_receipt_plan_reason: Some(latest_archive_receipt_plan.reason),
            latest_archive_receipt_plan_generated_at: Some(
                latest_archive_receipt_plan.generated_at,
            ),
        });
    }

    let snapshot = parse_snapshot_from_archive_receipt_latest(&latest_archive_receipt_plan)
        .map_err(|error| LatestArchiveReceiptResolutionFailure {
            kind: ResolutionFailureKind::InvalidLatestChain,
            reason: error.to_string(),
            snapshot: None,
            latest_archive_receipt_plan_raw: Some(latest_archive_receipt_plan_raw.clone()),
            latest_archive_receipt_plan_verdict: Some(latest_archive_receipt_plan.verdict.clone()),
            latest_archive_receipt_plan_reason: Some(latest_archive_receipt_plan.reason.clone()),
            latest_archive_receipt_plan_generated_at: Some(
                latest_archive_receipt_plan.generated_at,
            ),
        })?;
    ...
    if !mismatches.is_empty() {
        return Err(LatestArchiveReceiptResolutionFailure {
            kind: ResolutionFailureKind::DriftedClosureCertificateContract,
    ...
    Ok(ResolvedLatestArchiveReceipt {
        snapshot,
        latest_archive_receipt_plan_raw,
        latest_archive_receipt_plan,
    })
}

fn parse_snapshot_from_archive_receipt_latest(
    view: &ArchiveReceiptLatestReportView,
) -> Result<LatestArchiveReceiptSnapshot> {
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-archive-receipt plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-archive-receipt plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestArchiveReceiptSnapshot {
        latest_top_step_name,
        ...
        wrapper_timeout_ms: view
            .wrapper_timeout_ms
            .ok_or_else(|| anyhow!("latest-archive-receipt plan is missing wrapper_timeout_ms"))?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!("latest-archive-receipt plan is missing service_status_max_staleness_ms")
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackageClosureCertificateLatestPaths,
    resolved: &ResolvedLatestArchiveReceipt,
) -> PackageClosureCertificateLatestReport {
    PackageClosureCertificateLatestReport {
        generated_at: Utc::now(),
        ...
        closure_certificate_report_path: Some(
        ...
        downstream_closure_certificate_command_summary: Some(
            downstream_closure_certificate_command_summary(
                &resolved.snapshot,
                &expected_downstream_archive_receipt_session_dir(paths),
                &paths.nested_closure_certificate_session_dir,
            ),
...
fn report_from_resolution_failure(
    ...
    PackageClosureCertificateLatestReport {
        ...
        latest_archive_receipt_plan_verdict: failure.latest_archive_receipt_plan_verdict.clone(),
        latest_archive_receipt_verdict: None,
        closure_certificate_verdict: None,
        ...
        downstream_closure_certificate_command_summary: snapshot.map(|value| {
            downstream_closure_certificate_command_summary(
                value,
                &expected_downstream_archive_receipt_session_dir(paths),
                &paths.nested_closure_certificate_session_dir,
...
fn planned_session(
    ...
    session_dir: &Path,
    paths: &PackageClosureCertificateLatestPaths,
    resolved: Option<&ResolvedLatestArchiveReceipt>,
    failure: Option<&LatestArchiveReceiptResolutionFailure>,
) -> PackageClosureCertificateLatestSession {
    let snapshot = resolved
        .map(|value| &value.snapshot)
        .or_else(|| failure.and_then(|value| value.snapshot.as_ref()));
    PackageClosureCertificateLatestSession {
        session_version: CLOSURE_CERTIFICATE_LATEST_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        root: root.display().to_string(),
        session_dir: session_dir.display().to_string(),
        nested_archive_receipt_latest_session_dir: paths
        ...
        downstream_closure_certificate_command_summary: snapshot
            .map(|value| {
                downstream_closure_certificate_command_summary(
                    value,
                    &expected_downstream_archive_receipt_session_dir(paths),
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackageClosureCertificateLatestPaths,
    session: &PackageClosureCertificateLatestSession,
    status: &PackageClosureCertificateLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackageClosureCertificateLatestReport {
    PackageClosureCertificateLatestReport {
        generated_at: Utc::now(),
        ...
        nested_archive_receipt_latest_session_dir: Some(
            session.nested_archive_receipt_latest_session_dir.clone(),
        ),
        nested_closure_certificate_session_dir: Some(
            session.nested_closure_certificate_session_dir.clone(),
        ),
        downstream_decision_packet_session_dir: Some(
...
fn archive_receipt_latest_paths(session_dir: &Path) -> PackageClosureCertificateLatestPaths {
    PackageClosureCertificateLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_closure_certificate_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_closure_certificate_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_closure_certificate_latest.report.json"),
        latest_archive_receipt_plan_report_path: session_dir.join(
            "tiny_live_activation_package_closure_certificate_latest.archive_receipt_latest_plan.report.json",
        ),
        latest_archive_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_closure_certificate_latest.archive_receipt_latest.report.json",
        ),
        closure_certificate_report_path: session_dir.join(
            "tiny_live_activation_package_closure_certificate_latest.closure_certificate.report.json",
        ),
        nested_archive_receipt_latest_session_dir: session_dir
            .join("tiny_live_activation_package_closure_certificate_latest.archive_receipt_latest_session"),
        nested_closure_certificate_session_dir: session_dir.join(
            "tiny_live_activation_package_closure_certificate_latest.closure_certificate_session",
        ),
    }
}
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_archive_receipt_latest_session_dir
        .join("tiny_live_activation_package_archive_receipt_latest.decision_packet_session")
}

fn expected_downstream_archive_receipt_session_dir(
    paths: &PackageClosureCertificateLatestPaths,
) -> PathBuf {
    paths
        .nested_archive_receipt_latest_session_dir
        .join("tiny_live_activation_package_archive_receipt_latest.archive_receipt_session")
}
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_archive_receipt_latest_session_dir
        .join("tiny_live_activation_package_archive_receipt_latest.review_receipt_session")
}

fn latest_archive_receipt_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-archive-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn latest_archive_receipt_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-archive-receipt".to_string(),
        "--json".to_string(),
    ]
}
...
fn latest_archive_receipt_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-archive-receipt".to_string(),
        "--json".to_string(),
    ]
}
...
fn registry_entry_run_args(
    ...
    vec![
        "--archive-receipt-session-dir".to_string(),
        archive_receipt_session_dir.display().to_string(),
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-closure-certificate".to_string(),
        "--json".to_string(),
    ]
}
...
fn archive_receipt_verify_args(
    ...
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-live-package-closure-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn resolve_latest_archive_receipt_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{ARCHIVE_RECEIPT_LATEST_BIN} --root {} --session-dir {} --plan-latest-archive-receipt --json",
...
fn run_latest_archive_receipt_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{ARCHIVE_RECEIPT_LATEST_BIN} --root {} --session-dir {} --run-latest-archive-receipt --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}
...
fn verify_latest_archive_receipt_command_summary(session_dir: &Path) -> String {
    format!(
        "{ARCHIVE_RECEIPT_LATEST_BIN} --session-dir {} --verify-latest-archive-receipt --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_closure_certificate_command_summary(
    snapshot: &LatestArchiveReceiptSnapshot,
    archive_receipt_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{CLOSURE_CERTIFICATE_BIN} --archive-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-closure-certificate --json",
...
fn verify_archive_receipt_command_summary(
    ...
    format!(
        "{CLOSURE_CERTIFICATE_BIN} --archive-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-closure-certificate --json",
        shell_escape_path(archive_receipt_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
...
fn nested_archive_receipt_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_archive_receipt_latest.report.json")
}

fn nested_closure_certificate_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_closure_certificate.session.json")
}

fn nested_closure_certificate_status_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_closure_certificate.status.json")
}
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-closure-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageClosureCertificateLatestStepArtifact {
    PackageClosureCertificateLatestStepArtifact {
...
fn canonical_wrapper_mode(mode: &str) -> &str {
    match mode {
        "plan_latest_closure_certificate" => "plan_latest_closure_certificate",
        "render_latest_closure_certificate_script" => "render_latest_closure_certificate_script",
        "run_latest_closure_certificate" => "run_latest_closure_certificate",
        "verify_latest_closure_certificate" => "verify_latest_closure_certificate",
        other => other,
    }
}

fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackageClosureCertificateLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-closure-certificate status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-closure-certificate status"
        ));
        return;
    };
    compare_string(
...
fn compare_archive_receipt_latest_plan_against_snapshot(
    stored: &ArchiveReceiptLatestReportView,
    snapshot: &LatestArchiveReceiptSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        ...
        Some(ARCHIVE_RECEIPT_LATEST_PLAN_READY),
        "stored latest-archive-receipt plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_archive_receipt",
        "stored latest-archive-receipt plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-archive-receipt plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-archive-receipt plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-archive-receipt plan session_dir",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        ...
        "stored latest-archive-receipt plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-archive-receipt plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-archive-receipt plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-archive-receipt plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_archive_receipt_latest_run_against_snapshot(
    stored: &ArchiveReceiptLatestReportView,
    snapshot: &LatestArchiveReceiptSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_archive_receipt",
        "stored latest-archive-receipt run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-archive-receipt run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-archive-receipt run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-archive-receipt run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-archive-receipt run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-archive-receipt run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-archive-receipt run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-archive-receipt run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-archive-receipt run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-archive-receipt run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-archive-receipt run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-archive-receipt run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-archive-receipt run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-archive-receipt run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-archive-receipt run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_archive_receipt_latest_copy_matches_actual(
    stored: &ArchiveReceiptLatestReportView,
    actual: &ArchiveReceiptLatestReportView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        Some(stored.verdict.as_str()),
        Some(actual.verdict.as_str()),
        "stored latest-archive-receipt copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-archive-receipt copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-archive-receipt copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-archive-receipt copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-archive-receipt copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-archive-receipt copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-archive-receipt copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-archive-receipt copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-archive-receipt copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-archive-receipt copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-archive-receipt copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-archive-receipt copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_archive_receipt_run_against_snapshot(
    stored: &ClosureCertificateReportView,
    snapshot: &LatestArchiveReceiptSnapshot,
    archive_receipt_session_dir: &Path,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_live_package_closure_certificate",
        "stored closure-certificate run mode",
        mismatches,
    );
    compare_string(
        &stored.archive_receipt_session_dir,
        &archive_receipt_session_dir.display().to_string(),
        "stored closure-certificate run archive_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored closure-certificate run decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored closure-certificate run execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored closure-certificate run turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored closure-certificate run launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored closure-certificate run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored closure-certificate run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored closure-certificate run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored closure-certificate run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored closure-certificate run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored closure-certificate run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored closure-certificate run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.closure_certificate_session_path.as_deref(),
        Some(
            nested_closure_certificate_session_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored closure-certificate run closure_certificate_session_path",
        mismatches,
    );
    compare_option_string(
        stored.closure_certificate_status_path.as_deref(),
        Some(
            nested_closure_certificate_status_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored closure-certificate run closure_certificate_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_archive_receipt_report_path.as_deref(),
        Some(
            session_dir
                .join(
        ...
        "stored closure-certificate run archived_archive_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        stored.verify_archive_receipt_command_summary.as_deref(),
        Some(
            archive_receipt_verify_archive_receipt_command_summary(archive_receipt_session_dir)
                .as_str(),
        ),
        "stored closure-certificate run verify_archive_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored closure-certificate run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}
...
fn compare_archive_receipt_copy_against_nested_truth(
    ...
    compare_option_string(
        ...
        "stored closure-certificate copy verify_archive_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored closure-certificate copy release_capsule_digest_manifest_sha256",
        mismatches,
    );
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_archive_receipt_command_summary,
        &resolve_latest_archive_receipt_command_summary(
            root,
            &paths.nested_archive_receipt_latest_session_dir,
        ),
        "stored session resolve_latest_archive_receipt_command_summary",
        mismatches,
    );
    compare_string(
    ...
    compare_string(
        &session.downstream_closure_certificate_command_summary,
        &downstream_closure_certificate_command_summary(
            snapshot,
            &expected_downstream_archive_receipt_session_dir(paths),
            &paths.nested_closure_certificate_session_dir,
        ),
        "stored session downstream_closure_certificate_command_summary",
        mismatches,
    );
    compare_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_archive_receipt_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_closure_certificate_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_closure_certificate_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackageClosureCertificateLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackageClosureCertificateLatestPaths,
    snapshot: &LatestArchiveReceiptSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_archive_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        ...
        "stored report downstream_closure_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        report.closure_certificate_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "stored report closure_certificate_latest_session_path",
        mismatches,
    );
    compare_option_string(
        report.closure_certificate_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "stored report closure_certificate_latest_status_path",
        mismatches,
    );
    compare_option_string(
        report.closure_certificate_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "stored report closure_certificate_latest_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_archive_receipt_plan_report_path.as_deref(),
        Some(
            paths
                .latest_archive_receipt_plan_report_path
                .display()
        ...
        "stored report latest_archive_receipt_plan_report_path",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        report.closure_certificate_report_path.as_deref(),
        Some(
            paths
                .closure_certificate_report_path
                .display()
                .to_string()
                .as_str(),
        ),
        "stored report closure_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        report.nested_archive_receipt_latest_session_dir.as_deref(),
        Some(
            paths
                .nested_archive_receipt_latest_session_dir
                .display()
        ...
        "stored report nested_archive_receipt_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_closure_certificate_session_dir.as_deref(),
        Some(
            paths
                .nested_closure_certificate_session_dir
                .display()
        ...
        "stored report nested_closure_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_archive_receipt_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_closure_certificate_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_closure_certificate_session_dir on failure",
        mismatches,
    );
...
fn map_archive_receipt_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_archive_receipt_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_archive_receipt_latest_refused_by_drifted_archive_receipt_contract" => {
            ResolutionFailureKind::DriftedClosureCertificateContract
        }
        _ => ResolutionFailureKind::InvalidLatestChain,
    }
}

fn latest_archive_receipt_terminal_failure(
    report: &ArchiveReceiptLatestReportView,
) -> Option<LivePackageClosureCertificateLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackageClosureCertificateLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackageClosureCertificateLatestResult::RefusedByInvalidLatestChain)
        }
        Some("refused_by_drifted_archive_receipt_contract") => Some(
            LivePackageClosureCertificateLatestResult::RefusedByDriftedClosureCertificateContract,
        ),
        _ => None,
    }
...
fn closure_certificate_latest_result_from_nested_status(
    ...
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => Some(
            LivePackageClosureCertificateLatestResult::ReadyForManualExecutionWhenGateTurnsGreen,
        ),
        "refused_now_by_stage3" => {
            Some(LivePackageClosureCertificateLatestResult::RefusedNowByStage3)
        }
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackageClosureCertificateLatestResult::RefusedNowByPreActivationGate)
        }
...
fn closure_certificate_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => Some(
            "tiny_live_package_closure_certificate_ready_for_manual_execution_when_gate_turns_green",
        ),
        "refused_now_by_stage3" => {
            Some("tiny_live_package_closure_certificate_refused_now_by_stage3")
        }
        "refused_now_by_pre_activation_gate" => {
            Some("tiny_live_package_closure_certificate_refused_now_by_pre_activation_gate")
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_closure_certificate_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}
...
fn result_from_failure_kind(
    kind: ResolutionFailureKind,
) -> LivePackageClosureCertificateLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackageClosureCertificateLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackageClosureCertificateLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedClosureCertificateContract => {
            LivePackageClosureCertificateLatestResult::RefusedByDriftedClosureCertificateContract
        }
...
fn verdict_for_result(
    result: LivePackageClosureCertificateLatestResult,
) -> TinyLivePackageClosureCertificateLatestVerdict {
    match result {
        LivePackageClosureCertificateLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestRefusedByMissingLatestChain
        }
        LivePackageClosureCertificateLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestRefusedByInvalidLatestChain
        }
        LivePackageClosureCertificateLatestResult::RefusedByDriftedClosureCertificateContract => {
            TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestRefusedByDriftedClosureCertificateContract
        }
        LivePackageClosureCertificateLatestResult::RefusedNowByStage3 => {
            TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestRefusedNowByStage3
        }
        LivePackageClosureCertificateLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestRefusedNowByPreActivationGate
        }
        LivePackageClosureCertificateLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackageClosureCertificateLatestResult) -> String {
    match result {
        LivePackageClosureCertificateLatestResult::RefusedByMissingLatestChain => {
        ...
        LivePackageClosureCertificateLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-archive-receipt / closure-certificate path".to_string()
        }
        LivePackageClosureCertificateLatestResult::RefusedByDriftedClosureCertificateContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-archive-receipt / closure-certificate contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackageClosureCertificateLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-closure-certificate refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackageClosureCertificateLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-closure-certificate refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackageClosureCertificateLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            "the latest immutable chain now resolves into the accepted closure-certificate contract and one bounded latest closure-certificate handoff is ready for manual execution when gate truth turns green; this wrapper still did not run or authorize the frozen controller".to_string()
        }
    }
}
...
fn archive_receipt_verify_archive_receipt_command_summary(
    archive_receipt_session_dir: &Path,
) -> String {
    format!(
...
fn refusal_status(
    ...
    latest_archive_receipt_step: Option<PackageClosureCertificateLatestStepArtifact>,
    closure_certificate_step: Option<PackageClosureCertificateLatestStepArtifact>,
) -> PackageClosureCertificateLatestStatus {
    ...
    PackageClosureCertificateLatestStatus {
        ...
        result,
        reason,
        latest_archive_receipt_plan_verdict: resolved
            .map(|value| value.latest_archive_receipt_plan.verdict.clone()),
        latest_archive_receipt_verdict: latest_archive_receipt_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        closure_certificate_verdict: closure_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackageClosureCertificateLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_archive_receipt_plan_step: failure.latest_archive_receipt_plan_raw.as_ref().map(
            |_| {
                step_artifact(
                    &paths.latest_archive_receipt_plan_report_path,
                    "plan_latest_archive_receipt",
                    failure
                        .latest_archive_receipt_plan_verdict
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_archive_receipt_plan_reason
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_archive_receipt_plan_generated_at
...
fn verify_invalid_archive_receipt_latest_report_without_session(
    ...
    PackageClosureCertificateLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_closure_certificate".to_string(),
        verdict: TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestVerifyInvalid,
        reason,
        root: None,
        ...
        session_dir: Some(session_dir.display().to_string()),
        closure_certificate_latest_session_path: Some(
            archive_receipt_latest_paths(session_dir)
                .session_path
                .display()
                .to_string(),
        ),
        closure_certificate_latest_status_path: Some(
            archive_receipt_latest_paths(session_dir)
                .status_path
                .display()
                .to_string(),
        ),
        closure_certificate_latest_report_path: Some(
            archive_receipt_latest_paths(session_dir)
                .report_path
                .display()
                .to_string(),
        ),
        latest_archive_receipt_plan_report_path: None,
        latest_archive_receipt_report_path: None,
        closure_certificate_report_path: None,
        nested_archive_receipt_latest_session_dir: None,
        nested_closure_certificate_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_archive_receipt_command_summary: None,
        run_latest_archive_receipt_command_summary: None,
        verify_latest_archive_receipt_command_summary: None,
        downstream_closure_certificate_command_summary: None,
...
fn fallback_archive_receipt_latest_copy() -> ArchiveReceiptLatestReportView {
    ArchiveReceiptLatestReportView {
        generated_at: Utc::now(),
...
fn fallback_notarization_receipt_session(
    snapshot: &LatestArchiveReceiptSnapshot,
    session_dir: &Path,
) -> ClosureCertificateSessionView {
    ClosureCertificateSessionView {
        archive_receipt_session_dir: "<missing>".to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_archive_receipt_command_summary:
            archive_receipt_verify_archive_receipt_command_summary(Path::new("<missing>")),
...
fn fallback_notarization_receipt_status(
    _snapshot: &LatestArchiveReceiptSnapshot,
    session_dir: &Path,
) -> ClosureCertificateStatusView {
    ClosureCertificateStatusView {
        session_dir: session_dir.display().to_string(),
        archive_receipt_session_dir: "<missing>".to_string(),
...
fn render_report_lines(report: &PackageClosureCertificateLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_archive_receipt_latest_session_dir {
        lines.push(format!("nested_archive_receipt_latest_session_dir={value}"));
    }
    ...
    if let Some(value) = &report.nested_closure_certificate_session_dir {
        lines.push(format!("nested_closure_certificate_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_closure_certificate_command_summary {
        lines.push(format!(
            "downstream_closure_certificate_command_summary={value}"
        ));
    }
...
mod tests {
    ...
    impl FakeNotarizationReceiptLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestClosureCertificate,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestClosureCertificateScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestClosureCertificate,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestClosureCertificate,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_archive_receipt_lineage(
        ...
        let paths = archive_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert_eq!(
            report.verdict,
            TinyLivePackageClosureCertificateLatestVerdict::TinyLivePackageClosureCertificateLatestPlanReady
        );
        ...
        assert!(report
            ...
            .as_deref()
            .unwrap()
            .contains(CLOSURE_CERTIFICATE_BIN));
    }
    ...
    fn render_mode_emits_exact_downstream_provenance_certificate_contract_not_second_controller_path(
        ...
        assert!(script.contains(ARCHIVE_RECEIPT_LATEST_BIN));
        assert!(script.contains(CLOSURE_CERTIFICATE_BIN));
        assert!(script.contains("--archive-receipt-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-closure-certificate"));
        assert!(script.contains("--verify-live-package-closure-certificate"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            ...
            (
                ...
                "tiny_live_package_archive_receipt_latest_refused_by_invalid_latest_chain",
    ...
    fn plan_mode_maps_real_latest_archive_receipt_drift_verdict_to_local_drift_refusal() {
        ...
        plan["reason"] = json!("real accepted latest-archive-receipt drift");
        persist_json(&fixture.archive_receipt_latest_plan_json_path, &plan).unwrap();
    ...
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_archive_receipt_reports_real_drift()
    {
        let fixture = fake_archive_receipt_latest_fixture(
            ...
            "refused_by_drifted_archive_receipt_contract",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert_eq!(report.closure_certificate_verdict.as_deref(), None);
    }
    ...
    fn run_mode_refuses_when_nested_release_capsule_lineage_drifts_from_latest_immutable_chain() {
        ...
        assert!(report
            .reason
            .contains("stored latest-archive-receipt run package_dir"));
    }
    ...
    fn verify_mode_fails_when_persisted_latest_archive_receipt_artifacts_are_tampered_or_missing_nested_lineage_truth(
    ) {
        {
            ...
            let paths = archive_receipt_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.closure_certificate_report_path).unwrap();
...

        {
            ...
            let paths = archive_receipt_latest_paths(&fixture.wrapper_session_dir);
            fs::remove_file(nested_archive_receipt_latest_report_path(
                &paths.nested_archive_receipt_latest_session_dir,
            ))
            .unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains("tiny_live_activation_package_archive_receipt_latest.report.json")
            }));
        }
    }
    ...
    fn verify_fails_when_copied_nested_latest_archive_receipt_explicit_statement_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored latest-archive-receipt copy explicit_statement")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy decision_packet_result")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy attestation_seal_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_closure_certificate_registry_entry_session_dir_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy registry_entry_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_closure_certificate_notarization_receipt_session_dir_drifts()
    {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy notarization_receipt_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy chain_fingerprint_summary")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy chain_fingerprint_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy chain_fingerprint_algorithm")
        }));
    }
    ...
    fn verify_fails_when_copied_native_closure_certificate_filing_certificate_sha256_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy filing_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_closure_certificate_archive_receipt_sha256_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy archive_receipt_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_closure_certificate_closure_certificate_sha256_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy closure_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy decision_packet_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored closure-certificate copy execute_frozen_result")
        }));
    }

    #[test]
    fn verify_fails_when_copied_nested_closure_certificate_verdict_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| entry.contains("stored closure-certificate copy verdict")));
    }
    ...
    fn fake_archive_receipt_latest_fixture(
        ...
        write_fake_archive_receipt_latest_command(
            &fixture.bin_dir.join(ARCHIVE_RECEIPT_LATEST_BIN),
            &fixture.archive_receipt_latest_plan_json_path,
            &fixture.archive_receipt_latest_run_json_path,
            &fixture.archive_receipt_latest_verify_json_path,
            &fixture.root,
        ...
        write_fake_release_capsule_command(
            &fixture.bin_dir.join(CLOSURE_CERTIFICATE_BIN),
            &fixture.handoff_bundle_run_json_path,
            &fixture.handoff_bundle_verify_json_path,
            &fixture.handoff_bundle_session_json_path,
            &fixture.handoff_bundle_status_json_path,
    ...
    fn default_archive_receipt_latest_plan_report(
        ...
        json!({
            ...
            "verdict": ARCHIVE_RECEIPT_LATEST_PLAN_READY,
            "reason": "latest immutable chain resolves cleanly into accepted latest-archive-receipt contract",
            ...
            "session_dir": paths.nested_archive_receipt_latest_session_dir.display().to_string(),
    ...
    fn default_archive_receipt_latest_run_report(
        ...
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_archive_receipt_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_archive_receipt_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_archive_receipt_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
    ...
    fn default_archive_receipt_latest_verify_report(
        ...
        report["verdict"] = json!(ARCHIVE_RECEIPT_LATEST_VERIFY_OK);
    ...
    fn default_provenance_certificate_session(
        ...
        let paths = archive_receipt_latest_paths(&fixture.wrapper_session_dir);
        let archive_receipt_session_dir = expected_downstream_archive_receipt_session_dir(&paths);
        ...
        let snapshot = LatestArchiveReceiptSnapshot {
        ...
        json!({
            "session_version": "1",
            "planned_at": Utc::now(),
            "archive_receipt_session_dir": archive_receipt_session_dir.display().to_string(),
            ...
            "release_capsule_session_dir": archive_receipt_session_dir.display().to_string(),
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": archive_receipt_latest_paths(&fixture.wrapper_session_dir).nested_closure_certificate_session_dir.display().to_string(),
            "verify_archive_receipt_command_summary": archive_receipt_verify_archive_receipt_command_summary(&archive_receipt_session_dir),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
            ...
            "explicit_statement": "the verified archive-receipt session is the primary input here; this immutable closure certificate freezes one final terminal identity over the fully archived chain without enabling production execution"
        })
    }
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_closure_certificate_session_dir.display().to_string(),
            "archive_receipt_session_dir": expected_downstream_archive_receipt_session_dir(&paths).display().to_string(),
            "result": result,
            "reason": reason,
            ...
            "current_pre_activation_gate_verdict": gate_verdict,
            "current_pre_activation_gate_reason": gate_reason,
            "archive_receipt_step": Value::Null,
            ...
            "closure_certificate_sha256": "fixture-closure-certificate-sha256",
            "closure_certificate_algorithm": "sha256",
            "explicit_statement": "this closure certificate is archival and read-only; it never executes or enables the frozen controller"
        })
    }
    ...
    fn default_provenance_certificate_run_report(
        ...
        let closure_certificate_verdict = match result {
            "ready_for_manual_execution_when_gate_turns_green" => {
                "tiny_live_package_closure_certificate_ready_for_manual_execution_when_gate_turns_green"
            }
            "refused_now_by_stage3" => "tiny_live_package_closure_certificate_refused_now_by_stage3",
            "refused_now_by_pre_activation_gate" => {
                "tiny_live_package_closure_certificate_refused_now_by_pre_activation_gate"
            }
            _ => "tiny_live_package_closure_certificate_refused_now_by_invalid_or_drifted_contract",
        };
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_closure_certificate",
            "verdict": closure_certificate_verdict,
            ...
            "release_capsule_session_dir": expected_downstream_archive_receipt_session_dir(&paths).display().to_string(),
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_closure_certificate_session_dir.display().to_string(),
            "closure_certificate_session_path": nested_closure_certificate_session_path(&paths.nested_closure_certificate_session_dir).display().to_string(),
            "closure_certificate_status_path": nested_closure_certificate_status_path(&paths.nested_closure_certificate_session_dir).display().to_string(),
            "archived_archive_receipt_report_path": paths.nested_closure_certificate_session_dir.join("tiny_live_activation_package_closure_certificate.archive_receipt.report.json").display().to_string(),
            "result": result,
            ...
            "archive_receipt_step": Value::Null,
            "verify_archive_receipt_command_summary": archive_receipt_verify_archive_receipt_command_summary(&expected_downstream_archive_receipt_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn default_provenance_certificate_verify_report(
        ...
        report["mode"] = json!("verify_live_package_closure_certificate");
        report["verdict"] = json!(CLOSURE_CERTIFICATE_VERIFY_OK);
        report["reason"] = json!("verify ok");
        report["explicit_statement"] = json!("this verification binds one immutable closure certificate to verified archive-receipt truth, the exact reviewed frozen controller summary, the canonical chain fingerprint, the ledger seal, the registry entry, the filing-certificate identity, and the archive-receipt identity");
        report
    }
    ...
    fn write_fake_archive_receipt_latest_command(
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-archive-receipt session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-archive-receipt \"*) mode='plan';;\n  *\" --run-latest-archive-receipt \"*) mode='run';;\n  *\" --verify-latest-archive-receipt \"*) mode='verify';;\n  *) echo 'unexpected latest-archive-receipt mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_archive_receipt_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
