...
use copybot_app::tiny_live_activation::package_substructure_certificate_plinth_receipt;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_plinth_receipt --substructure-certificate-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-plinth-receipt | --render-live-package-plinth-receipt | --run-live-package-plinth-receipt | --verify-live-package-plinth-receipt)";
const PLINTH_RECEIPT_SESSION_VERSION: &str = "1";
const PLINTH_RECEIPT_STATUS_VERSION: &str = "1";
const PLINTH_RECEIPT_ALGORITHM: &str = "sha256";
const PLINTH_RECEIPT_SESSION_EXPLICIT_STATEMENT: &str =
...
const PLINTH_RECEIPT_STATUS_EXPLICIT_STATEMENT: &str =
    "this plinth receipt is archival and read-only; it never executes or enables the frozen controller";
const PLINTH_RECEIPT_VERIFY_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    substructure_certificate_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackagePlinthReceipt,
    RenderLivePackagePlinthReceipt,
    RunLivePackagePlinthReceipt,
    VerifyLivePackagePlinthReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackagePlinthReceiptVerdict {
    TinyLivePackagePlinthReceiptPlanReady,
    TinyLivePackagePlinthReceiptRendered,
    TinyLivePackagePlinthReceiptRefusedNowByStage3,
    TinyLivePackagePlinthReceiptRefusedNowByPreActivationGate,
    TinyLivePackagePlinthReceiptRefusedNowByInvalidOrDriftedContract,
    TinyLivePackagePlinthReceiptReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackagePlinthReceiptVerifyOk,
    TinyLivePackagePlinthReceiptVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackagePlinthReceiptResult {
    RefusedNowByStage3,
...
struct PackagePlinthReceiptStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackagePlinthReceiptSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackagePlinthReceiptStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackagePlinthReceiptResult,
    reason: String,
    ...
    substructure_certificate_step: Option<PackagePlinthReceiptStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackagePlinthReceiptPaths {
...
struct PackagePlinthReceiptReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackagePlinthReceiptVerdict,
    reason: String,
    substructure_certificate_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut substructure_certificate_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--substructure-certificate-session-dir" => {
                substructure_certificate_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--substructure-certificate-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--plan-live-package-plinth-receipt" => {
                set_mode(&mut mode, Mode::PlanLivePackagePlinthReceipt, arg.as_str())?
            }
            "--render-live-package-plinth-receipt" => set_mode(
                &mut mode,
                Mode::RenderLivePackagePlinthReceipt,
                arg.as_str(),
            )?,
            "--run-live-package-plinth-receipt" => {
                set_mode(&mut mode, Mode::RunLivePackagePlinthReceipt, arg.as_str())?
            }
            "--verify-live-package-plinth-receipt" => set_mode(
                &mut mode,
                Mode::VerifyLivePackagePlinthReceipt,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackagePlinthReceipt) && output_path.is_none() {
        bail!("missing --output for --render-live-package-plinth-receipt");
    }
    if matches!(
        mode,
        Mode::RunLivePackagePlinthReceipt | Mode::VerifyLivePackagePlinthReceipt
    ) && session_dir.is_none()
    {
    ...
    let substructure_certificate_session_dir = substructure_certificate_session_dir
        .ok_or_else(|| anyhow!("missing --substructure-certificate-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackagePlinthReceipt => plan_live_package_plinth_receipt_report(&config)?,
        Mode::RenderLivePackagePlinthReceipt => render_live_package_plinth_receipt_report(&config)?,
        Mode::RunLivePackagePlinthReceipt => run_live_package_plinth_receipt_report(&config)?,
        Mode::VerifyLivePackagePlinthReceipt => verify_live_package_plinth_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_live_package_plinth_receipt_report(config: &Config) -> Result<PackagePlinthReceiptReport> {
    let (basal_receipt, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_substructure_certificate_plinth_receipt::verify_live_package_substructure_certificate_for_plinth_receipt(
                    &config.substructure_certificate_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification =
                            classify_verified_substructure_certificate(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_substructure_certificate_plinth_receipt::load_live_package_substructure_certificate_contract_for_plinth_receipt(
                            &config.substructure_certificate_session_dir,
                        )?;
                        let classification =
                            classify_substructure_certificate_contract(&raw_contract, false);
    ...
    let plinth_receipt_sha256 = compute_plinth_receipt_sha256(
        &result,
    ...
    Ok(PackagePlinthReceiptReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_plinth_receipt".to_string(),
        verdict:
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptPlanReady,
        reason: format!(
            "substructure-certificate-native plinth receipt is explicit for verified substructure-certificate session {}; this command stays archival and read-only while freezing one final cachet identity over the fully culminated chain",
            config.substructure_certificate_session_dir.display()
        ),
        substructure_certificate_session_dir: config.substructure_certificate_session_dir.display().to_string(),
        ...
        plinth_receipt_session_path: None,
        plinth_receipt_status_path: None,
        archived_substructure_certificate_report_path: None,
        result: Some(result),
        ...
        verify_substructure_certificate_command_summary: Some(verify_substructure_certificate_command_summary(
            &config.substructure_certificate_session_dir,
        )),
        ...
        plinth_receipt_summary: Some(plinth_receipt_summary(
            classification.0,
            &basal_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        plinth_receipt_sha256: Some(plinth_receipt_sha256),
        plinth_receipt_algorithm: Some(PLINTH_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: PLINTH_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_plinth_receipt_report(
    config: &Config,
) -> Result<PackagePlinthReceiptReport> {
    let plan = plan_live_package_plinth_receipt_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_plinth_receipt --substructure-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-plinth-receipt --json\ncopybot_tiny_live_activation_package_plinth_receipt --substructure-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-plinth-receipt --json\n",
        shell_escape_path(&config.substructure_certificate_session_dir),
    ...
    Ok(PackagePlinthReceiptReport {
        generated_at: Utc::now(),
        mode: "render_live_package_plinth_receipt".to_string(),
        verdict:
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptRendered,
        reason: format!(
            "rendered plinth-receipt run/verify script to {}; the primary input remains the immutable substructure-certificate session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: PLINTH_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_plinth_receipt_report(config: &Config) -> Result<PackagePlinthReceiptReport> {
    let session_dir = config
        .session_dir
        .as_ref()
        .ok_or_else(|| anyhow!("missing --session-dir for run mode"))?;
    let verified_basal_receipt = match package_substructure_certificate_plinth_receipt::verify_live_package_substructure_certificate_for_plinth_receipt(
        &config.substructure_certificate_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify substructure-certificate session under {}: {error}",
                    config.substructure_certificate_session_dir.display()
                ),
            ))
        }
    ...
    .with_context(|| {
        format!(
            "plinth-receipt session dir {} overlaps the managed live target surface derived from verified substructure-certificate install_root {}",
            session_dir.display(),
            verified_basal_receipt.contract.install_root.display()
        )
    })?;
    ensure_clean_plinth_receipt_session_dir(session_dir)?;
    let paths = plinth_receipt_paths(session_dir);
    let classification = classify_verified_substructure_certificate(&verified_basal_receipt);
    ...
    let substructure_certificate_step = Some(step_artifact(
        &paths.substructure_certificate_report_path,
        "verify_live_package_substructure_certificate",
        &verified_basal_receipt.verdict,
        &verified_basal_receipt.reason,
        verified_basal_receipt.generated_at,
    ));
    let status = PackagePlinthReceiptStatus {
        status_version: PLINTH_RECEIPT_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        substructure_certificate_session_dir: config
            .substructure_certificate_session_dir
            .display()
            .to_string(),
        basal_receipt_session_dir: verified_basal_receipt
            .contract
            .basal_receipt_session_dir
            .display()
            .to_string(),
        ...
        basal_receipt_algorithm: verified_basal_receipt
            ...
            .clone(),
        substructure_certificate_summary: verified_basal_receipt
            .contract
            .substructure_certificate_summary
            .clone(),
        substructure_certificate_sha256: verified_basal_receipt
            .contract
            .substructure_certificate_sha256
            .clone(),
        substructure_certificate_algorithm: verified_basal_receipt
            .contract
            .substructure_certificate_algorithm
            .clone(),
        plinth_receipt_summary: plinth_receipt_summary(
            result,
            &verified_basal_receipt
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        plinth_receipt_sha256: plinth_receipt_sha256.clone(),
        plinth_receipt_algorithm: PLINTH_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: PLINTH_RECEIPT_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_plinth_receipt_report(
    config: &Config,
) -> Result<PackagePlinthReceiptReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = plinth_receipt_paths(session_dir);
    let session: PackagePlinthReceiptSession = load_json(&paths.session_path)?;
    let status: PackagePlinthReceiptStatus = load_json(&paths.status_path)?;
    ...
    compare_string(
        ...
        &config
            ...
            .display()
            .to_string(),
        "plinth-receipt substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "plinth-receipt session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "plinth-receipt status session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &config
            ...
            .display()
            .to_string(),
        "plinth-receipt status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_basal_receipt
            ...
            .display()
            .to_string(),
        "plinth-receipt bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_basal_receipt
            ...
            .display()
            .to_string(),
        "plinth-receipt status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_basal_receipt
            ...
            .display()
            .to_string(),
        "plinth-receipt foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_basal_receipt
            ...
            .display()
            .to_string(),
        "plinth-receipt status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_substructure_certificate_report_view: NestedSubstructureCertificateReportView =
        serde_json::from_value(stored_basal_receipt_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested substructure-certificate report {}",
                paths.substructure_certificate_report_path.display()
            )
        })?;
    let verified_substructure_certificate_report_view: NestedSubstructureCertificateReportView =
        serde_json::from_value(verified_basal_receipt.report_json.clone()).with_context(|| {
            format!(
                "failed parsing freshly verified nested substructure-certificate report for {}",
                config.substructure_certificate_session_dir.display()
            )
        })?;
        ...
        != verified_substructure_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "plinth-receipt archived substructure-certificate report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_substructure_certificate_report_view.generated_at,
            verified_substructure_certificate_report_view.generated_at
        ));
    }
    let expected_substructure_certificate_step = step_artifact(
        &paths.substructure_certificate_report_path,
        &verified_substructure_certificate_report_view.mode,
        &verified_substructure_certificate_report_view.verdict,
        &verified_substructure_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_substructure_certificate_step,
        "plinth-receipt substructure_certificate_step",
        &mut mismatches,
    );
    if verified_basal_receipt.verdict != "tiny_live_package_substructure_certificate_verify_ok" {
        mismatches.push(format!(
            "plinth-receipt nested substructure-certificate verification is non-green: {}",
            verified_basal_receipt.reason
        ));
    }

    let classification = classify_verified_substructure_certificate(&verified_basal_receipt);
    let expected_plinth_receipt_sha256 = compute_plinth_receipt_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_basal_receipt.contract,
        classification.0,
        &expected_plinth_receipt_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "plinth-receipt session contract does not match freshly verified substructure-certificate truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_basal_receipt.contract,
        classification,
        expected_substructure_certificate_step.clone(),
        &expected_plinth_receipt_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "plinth-receipt status contract does not match freshly verified substructure-certificate truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_basal_receipt.contract,
        TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptVerifyOk,
        &expected_status,
    ))
}
...
fn verify_substructure_certificate_command_summary(
    ...
    format!(
        "verify immutable substructure-certificate session under {} before freezing the final plinth receipt",
        substructure_certificate_session_dir.display()
    )
}

fn classify_substructure_certificate_contract(
    contract: &package_substructure_certificate_plinth_receipt::LivePackageSubstructureCertificateContractView,
    assume_verified: bool,
) -> (
    LivePackagePlinthReceiptResult,
    TinyLivePackagePlinthReceiptVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackagePlinthReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptRefusedNowByInvalidOrDriftedContract,
            "the substructure-certificate chain is not verified, so this plinth receipt remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackagePlinthReceiptResult::RefusedNowByStage3,
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptRefusedNowByStage3,
            "the immutable substructure certificate remains refused now by Stage 3, so this plinth receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackagePlinthReceiptResult::RefusedNowByPreActivationGate,
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptRefusedNowByPreActivationGate,
            "the immutable substructure certificate remains refused now by the pre-activation gate, so this plinth receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackagePlinthReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable substructure certificate is coherent and this plinth receipt is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackagePlinthReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptRefusedNowByInvalidOrDriftedContract,
            "the verified substructure-certificate chain is invalid, drifted, or non-runnable, so this plinth receipt remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_substructure_certificate(
    verified: &package_substructure_certificate_plinth_receipt::VerifiedLivePackageSubstructureCertificatePlinthReceiptStep,
) -> (
    LivePackagePlinthReceiptResult,
    TinyLivePackagePlinthReceiptVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_substructure_certificate_verify_ok" {
        return (
            LivePackagePlinthReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptRefusedNowByInvalidOrDriftedContract,
            "the immutable substructure certificate did not verify cleanly, so this plinth receipt remains refused"
                .to_string(),
        );
    }
    classify_substructure_certificate_contract(&verified.contract, true)
}

fn plinth_receipt_summary(
    result: LivePackagePlinthReceiptResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackagePlinthReceiptResult::RefusedNowByStage3 => {
            "Plinth receipt outcome: the immutable culminated chain remains refused now by Stage 3; archive this final cachet seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackagePlinthReceiptResult::RefusedNowByPreActivationGate => {
            "Plinth receipt outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final cachet seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackagePlinthReceiptResult::RefusedNowByInvalidOrDriftedContract => {
            "Plinth receipt outcome: the immutable culminated chain is invalid or drifted; archive this final cachet seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackagePlinthReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Plinth receipt outcome: this immutable cachet seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn plinth_receipt_paths(session_dir: &Path) -> PackagePlinthReceiptPaths {
    PackagePlinthReceiptPaths {
        session_path: session_dir.join("tiny_live_activation_package_plinth_receipt.session.json"),
        status_path: session_dir.join("tiny_live_activation_package_plinth_receipt.status.json"),
...
#[cfg(test)]
fn substructure_certificate_paths(session_dir: &Path) -> PackagePlinthReceiptPaths {
    plinth_receipt_paths(session_dir)
}

fn ensure_clean_plinth_receipt_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing plinth-receipt session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating plinth-receipt session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackagePlinthReceiptSession {
    PackagePlinthReceiptSession {
        session_version: PLINTH_RECEIPT_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        substructure_certificate_session_dir: config
            .substructure_certificate_session_dir
            .display()
        ...
        plinth_receipt_sha256: plinth_receipt_sha256.to_string(),
        plinth_receipt_algorithm: PLINTH_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: PLINTH_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_session_from_verified(
    ...
    PackagePlinthReceiptSession {
        planned_at,
        ..planned_session(config, session_dir, contract, result, plinth_receipt_sha256)
    }
}
...
fn expected_status_from_verified(
    ...
    substructure_certificate_step: PackagePlinthReceiptStepArtifact,
    plinth_receipt_sha256: &str,
) -> PackagePlinthReceiptStatus {
    PackagePlinthReceiptStatus {
        status_version: PLINTH_RECEIPT_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        substructure_certificate_step: Some(substructure_certificate_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackagePlinthReceiptPaths,
    contract: &package_substructure_certificate_plinth_receipt::LivePackageSubstructureCertificateContractView,
    verdict: TinyLivePackagePlinthReceiptVerdict,
    status: &PackagePlinthReceiptStatus,
) -> PackagePlinthReceiptReport {
    PackagePlinthReceiptReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptVerifyOk
        {
            "verify_live_package_plinth_receipt".to_string()
        } else {
            "run_live_package_plinth_receipt".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        plinth_receipt_session_path: Some(paths.session_path.display().to_string()),
        plinth_receipt_status_path: Some(paths.status_path.display().to_string()),
        archived_substructure_certificate_report_path: Some(
            paths
        ...
        substructure_certificate_step: status.substructure_certificate_step.clone(),
        ...
        explicit_statement: if verdict
            == TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptVerifyOk
        {
            PLINTH_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackagePlinthReceiptReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackagePlinthReceipt => {
                "plan_live_package_plinth_receipt".to_string()
            }
            Mode::RenderLivePackagePlinthReceipt => {
                "render_live_package_plinth_receipt".to_string()
            }
            Mode::RunLivePackagePlinthReceipt => {
                "run_live_package_plinth_receipt".to_string()
            }
            Mode::VerifyLivePackagePlinthReceipt => {
                "verify_live_package_plinth_receipt".to_string()
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable plinth receipt refused before persisting because the substructure-certificate artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "substructure-certificate-native plinth-receipt verification failed".to_string()
    });
    PackagePlinthReceiptReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_plinth_receipt".to_string(),
        verdict: TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptVerifyInvalid,
        reason,
        substructure_certificate_session_dir: config
            .substructure_certificate_session_dir
            .display()
        ...
        explicit_statement: PLINTH_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackagePlinthReceiptStepArtifact>,
    expected_path: &Path,
    label: &str,
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackagePlinthReceiptStepArtifact {
    PackagePlinthReceiptStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackagePlinthReceiptStepArtifact>,
    expected: &PackagePlinthReceiptStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_plinth_receipt_sha256(
    result: &str,
...
fn render_report_lines(report: &PackagePlinthReceiptReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.plinth_receipt_sha256 {
        lines.push(format!("plinth_receipt_sha256={value}"));
    }
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_plinth_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify = verify_live_package_plinth_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptVerifyOk
        );
    ...
    #[test]
    fn drifted_substructure_certificate_contract_is_refused() {
        let fixture = fake_command_fixture(
            "substructure_certificate_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_substructure_certificate",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        let error = run_live_package_plinth_receipt_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = substructure_certificate_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_substructure_certificate_step_path() {
        let fixture = fake_command_fixture(
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_plinth_receipt_report(&fixture.run_config()).unwrap();
        let paths = substructure_certificate_paths(&fixture.substructure_certificate_session_dir);
        let mut status: PackagePlinthReceiptStatus = load_json(&paths.status_path).unwrap();
        status.substructure_certificate_step.as_mut().unwrap().path =
        ...
        assert_eq!(
            verify.verdict,
            TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptVerifyInvalid
        );
    }

    #[test]
    fn verify_rejects_tampered_foundation_receipt_identity_text_in_plinth_session() {
        let fixture = fake_command_fixture(
            "plinth_receipt_tampered_foundation_identity_text",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        run_live_package_plinth_receipt_report(&fixture.run_config()).unwrap();
        let paths = substructure_certificate_paths(&fixture.substructure_certificate_session_dir);
        let mut session: PackagePlinthReceiptSession = load_json(&paths.session_path).unwrap();
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = substructure_certificate_paths(&fixture.substructure_certificate_session_dir);
        let mut status: PackagePlinthReceiptStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackagePlinthReceiptResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_substructure_certificate_report_content() {
        let fixture = fake_command_fixture(
            "substructure_certificate_tampered_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut report: serde_json::Value =
            load_json(&paths.substructure_certificate_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested substructure-certificate reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_substructure_certificate_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "substructure_certificate_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_chain_ledger_registry_filing_archive_summit_closure_finality_or_consummation_identity_fields(
    ) {
        {
            ...
            run_live_package_plinth_receipt_report(&fixture.run_config()).unwrap();
            let paths =
                substructure_certificate_paths(&fixture.substructure_certificate_session_dir);
            let mut status: PackagePlinthReceiptStatus = load_json(&paths.status_path).unwrap();
            status.chain_fingerprint_sha256 = "tampered-chain".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_plinth_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackagePlinthReceiptVerdict::TinyLivePackagePlinthReceiptVerifyInvalid
            );
    ...
    fn verify_rejects_tampered_substructure_certificate_sha256() {
        let fixture = fake_command_fixture(
            "plinth_receipt_tampered_substructure_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "plinth_receipt_tampered_substructure_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "plinth_receipt_tampered_substructure_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_plinth_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "plinth_receipt_tampered_plinth_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "plinth_receipt_tampered_plinth_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "plinth_receipt_tampered_plinth_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        substructure_certificate_session_dir: PathBuf,
        input_substructure_certificate_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackagePlinthReceipt,
                substructure_certificate_session_dir: self
                    .input_substructure_certificate_session_dir
                    .clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.substructure_certificate_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackagePlinthReceipt,
                substructure_certificate_session_dir: self
                    .input_substructure_certificate_session_dir
                    .clone(),
                confirmed_decision_packet_session_dir: Some(
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let substructure_certificate_session_dir = fixture_dir.join("plinth-receipt-session");
        let input_substructure_certificate_session_dir =
        ...
        persist_json(
            ...
            &stored_substructure_certificate_status_fixture(&verify_report),
        )
        .unwrap();
        persist_json(
            &input_substructure_certificate_session_dir.join(
        ...
        let verify_path = fixture_dir.join("substructure-certificate.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_substructure_certificate_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_substructure_certificate"),
            &verify_path,
            &basal_receipt_session_dir,
            &decision_packet_session_dir,
            &input_substructure_certificate_session_dir,
    ...
    fn bind_substructure_certificate_report_to_fixture(
        ...
        object.insert(
            "verify_substructure_certificate_command_summary".to_string(),
            json!(format!(
        ...
        object.insert(
            ...
            json!("substructure-certificate verify statement"),
        );
    }
    ...
    fn substructure_certificate_verify_report(
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_substructure_certificate",
            "verdict": "tiny_live_package_substructure_certificate_verify_ok",
            "reason": "substructure-certificate chain is coherent",
            "result": result,
    ...
    fn stored_substructure_certificate_session_fixture(
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("substructure-certificate statement"),
        );
        Value::Object(object)
    }

    fn stored_substructure_certificate_status_fixture(verify_report: &Value) -> Value {
        let mut object = Map::new();
        object.insert(
            "result".to_string(),
            json!(verify_report
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("substructure-certificate status statement"),
        );
        Value::Object(object)
    }
}
