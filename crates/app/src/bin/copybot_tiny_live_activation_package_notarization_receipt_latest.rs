...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_notarization_receipt_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-notarization-receipt | --render-latest-notarization-receipt-script | --run-latest-notarization-receipt | --verify-latest-notarization-receipt)";
const NOTARIZATION_RECEIPT_LATEST_SESSION_VERSION: &str = "1";
const NOTARIZATION_RECEIPT_LATEST_STATUS_VERSION: &str = "1";
const PROVENANCE_CERTIFICATE_LATEST_BIN: &str =
...
const PROVENANCE_CERTIFICATE_LATEST_PLAN_READY: &str =
    "tiny_live_package_provenance_certificate_latest_plan_ready";
const PROVENANCE_CERTIFICATE_LATEST_VERIFY_OK: &str =
    "tiny_live_package_provenance_certificate_latest_verify_ok";
const NOTARIZATION_RECEIPT_VERIFY_OK: &str = "tiny_live_package_notarization_receipt_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-notarization-receipt handoff binds the latest persisted immutable package chain to the accepted notarization-receipt contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestNotarizationReceipt,
    RenderLatestNotarizationReceiptScript,
    RunLatestNotarizationReceipt,
    VerifyLatestNotarizationReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageNotarizationReceiptLatestVerdict {
    TinyLivePackageNotarizationReceiptLatestPlanReady,
    TinyLivePackageNotarizationReceiptLatestScriptRendered,
    TinyLivePackageNotarizationReceiptLatestRefusedByMissingLatestChain,
    TinyLivePackageNotarizationReceiptLatestRefusedByInvalidLatestChain,
    TinyLivePackageNotarizationReceiptLatestRefusedByDriftedProvenanceCertificateContract,
    TinyLivePackageNotarizationReceiptLatestRefusedNowByStage3,
    TinyLivePackageNotarizationReceiptLatestRefusedNowByPreActivationGate,
    TinyLivePackageNotarizationReceiptLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageNotarizationReceiptLatestVerifyOk,
    TinyLivePackageNotarizationReceiptLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageNotarizationReceiptLatestResult {
...
struct PackageNotarizationReceiptLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageNotarizationReceiptLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_provenance_certificate_latest_session_dir: String,
    nested_notarization_receipt_session_dir: String,
    ...
    resolve_latest_provenance_certificate_command_summary: String,
    run_latest_provenance_certificate_command_summary: String,
    verify_latest_provenance_certificate_command_summary: String,
    downstream_notarization_receipt_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageNotarizationReceiptLatestStatus {
    ...
    nested_notarization_receipt_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackageNotarizationReceiptLatestResult,
    reason: String,
    latest_provenance_certificate_plan_verdict: Option<String>,
    latest_provenance_certificate_verdict: Option<String>,
    notarization_receipt_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_provenance_certificate_plan_step: Option<PackageNotarizationReceiptLatestStepArtifact>,
    latest_provenance_certificate_step: Option<PackageNotarizationReceiptLatestStepArtifact>,
    notarization_receipt_step: Option<PackageNotarizationReceiptLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackageNotarizationReceiptLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_provenance_certificate_plan_report_path: PathBuf,
    latest_provenance_certificate_report_path: PathBuf,
    notarization_receipt_report_path: PathBuf,
    nested_provenance_certificate_latest_session_dir: PathBuf,
    nested_notarization_receipt_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageNotarizationReceiptLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageNotarizationReceiptLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    notarization_receipt_latest_session_path: Option<String>,
    notarization_receipt_latest_status_path: Option<String>,
    notarization_receipt_latest_report_path: Option<String>,
    latest_provenance_certificate_plan_report_path: Option<String>,
    latest_provenance_certificate_report_path: Option<String>,
    notarization_receipt_report_path: Option<String>,
    nested_provenance_certificate_latest_session_dir: Option<String>,
    nested_notarization_receipt_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_provenance_certificate_command_summary: Option<String>,
    run_latest_provenance_certificate_command_summary: Option<String>,
    verify_latest_provenance_certificate_command_summary: Option<String>,
    downstream_notarization_receipt_command_summary: Option<String>,
...
#[derive(Debug, Clone)]
struct LatestNotarizationReceiptSnapshot {
...
#[derive(Debug, Clone)]
struct ResolvedLatestNotarizationReceipt {
    snapshot: LatestNotarizationReceiptSnapshot,
    latest_provenance_certificate_plan_raw: Value,
    latest_provenance_certificate_plan: ProvenanceCertificateLatestReportView,
}
...
#[derive(Debug, Clone)]
struct LatestNotarizationReceiptResolutionFailure {
    kind: ResolutionFailureKind,
    reason: String,
    snapshot: Option<LatestNotarizationReceiptSnapshot>,
    latest_provenance_certificate_plan_raw: Option<Value>,
    latest_provenance_certificate_plan_verdict: Option<String>,
    latest_provenance_certificate_plan_reason: Option<String>,
    latest_provenance_certificate_plan_generated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone, Deserialize)]
struct ProvenanceCertificateLatestReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Deserialize)]
struct NotarizationReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    ...
    provenance_certificate_step: Option<PackageNotarizationReceiptLatestStepArtifact>,
...
#[derive(Debug, Clone, Deserialize)]
struct NotarizationReceiptSessionView {
    provenance_certificate_session_dir: String,
...
#[derive(Debug, Clone, Deserialize)]
struct NotarizationReceiptStatusView {
    session_dir: String,
    provenance_certificate_session_dir: String,
    result: String,
    reason: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-notarization-receipt" => {
                set_mode(&mut mode, Mode::PlanLatestNotarizationReceipt, arg.as_str())?
            }
            "--render-latest-notarization-receipt-script" => set_mode(
                &mut mode,
                Mode::RenderLatestNotarizationReceiptScript,
                arg.as_str(),
            )?,
            "--run-latest-notarization-receipt" => {
                set_mode(&mut mode, Mode::RunLatestNotarizationReceipt, arg.as_str())?
            }
            "--verify-latest-notarization-receipt" => set_mode(
                &mut mode,
                Mode::VerifyLatestNotarizationReceipt,
                arg.as_str(),
            )?,
    ...
    if matches!(
        mode,
        Mode::PlanLatestNotarizationReceipt
            | Mode::RenderLatestNotarizationReceiptScript
            | Mode::RunLatestNotarizationReceipt
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestNotarizationReceiptScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestNotarizationReceipt | Mode::VerifyLatestNotarizationReceipt
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestNotarizationReceipt => plan_latest_provenance_certificate_report(&config)?,
        Mode::RenderLatestNotarizationReceiptScript => {
            render_latest_provenance_certificate_script_report(&config)?
        }
        Mode::RunLatestNotarizationReceipt => run_latest_provenance_certificate_report(&config)?,
        Mode::VerifyLatestNotarizationReceipt => {
...
fn plan_latest_provenance_certificate_report(
    config: &Config,
) -> Result<PackageNotarizationReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
    ...
    let paths = notarization_receipt_latest_paths(&suggested_session_dir);
    ...
    match resolve_latest_provenance_certificate(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            ...
            TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-provenance-certificate {} lineage, exact decision-packet confirmation anchor {}, and the accepted notarization-receipt contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_provenance_certificate_latest_session_dir.display(),
        ...
        Ok(resolved) => {
            write_script(
                ...
                &format!(
                    ...
                    run_latest = run_latest_provenance_certificate_command_summary(
                        root,
                        &paths.nested_provenance_certificate_latest_session_dir,
                    ),
                    verify_latest = verify_latest_provenance_certificate_command_summary(
                        &paths.nested_provenance_certificate_latest_session_dir,
                    ),
                    run_handoff_bundle = downstream_notarization_receipt_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_provenance_certificate_session_dir(&paths),
                        &paths.nested_notarization_receipt_session_dir,
                    ),
                    verify_handoff_bundle = verify_notarization_receipt_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_provenance_certificate_session_dir(&paths),
                        &paths.nested_notarization_receipt_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                ...
                TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestScriptRendered,
                format!(
                    "rendered latest-notarization-receipt handoff script to {} using the accepted latest-provenance-certificate and notarization-receipt contracts",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_provenance_certificate_report(
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-notarization-receipt session dir {}",
            session_dir.display()
        );
    }
    let paths = notarization_receipt_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-notarization-receipt")?;
...
fn verify_latest_provenance_certificate_report(
    ...
    let session: PackageNotarizationReceiptLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_provenance_certificate_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-notarization-receipt session artifact {}: {error}",
    ...
    let status: PackageNotarizationReceiptLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_provenance_certificate_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-notarization-receipt status artifact {}: {error}",
    ...
    let stored_report: PackageNotarizationReceiptLatestReport = match load_json(&paths.report_path)
    {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_provenance_certificate_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-notarization-receipt report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-notarization-receipt root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-notarization-receipt session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-notarization-receipt status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-notarization-receipt report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_provenance_certificate_latest_session_dir,
        &paths
            .nested_provenance_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-notarization-receipt session nested_provenance_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_provenance_certificate_latest_session_dir,
        &paths
            .nested_provenance_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-notarization-receipt status nested_provenance_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-notarization-receipt report nested_provenance_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_notarization_receipt_session_dir,
        &paths
            .nested_notarization_receipt_session_dir
            .display()
            .to_string(),
        "latest-notarization-receipt session nested_notarization_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_notarization_receipt_session_dir,
        &paths
            .nested_notarization_receipt_session_dir
            .display()
            .to_string(),
        "latest-notarization-receipt status nested_notarization_receipt_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-notarization-receipt report nested_notarization_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-notarization-receipt session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-notarization-receipt status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-notarization-receipt report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .notarization_receipt_latest_session_path
            .as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-notarization-receipt report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .notarization_receipt_latest_status_path
            .as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-notarization-receipt report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .notarization_receipt_latest_report_path
            .as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-notarization-receipt report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-notarization-receipt session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-notarization-receipt status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-notarization-receipt report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_provenance_certificate",
        "latest-notarization-receipt stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-notarization-receipt stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-notarization-receipt stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-notarization-receipt stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-notarization-receipt stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report
            .latest_provenance_certificate_plan_verdict
            .as_deref(),
        status.latest_provenance_certificate_plan_verdict.as_deref(),
        "stored report latest_provenance_certificate_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .latest_provenance_certificate_verdict
            .as_deref(),
        status.latest_provenance_certificate_verdict.as_deref(),
        "stored report latest_provenance_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.notarization_receipt_verdict.as_deref(),
        status.notarization_receipt_verdict.as_deref(),
        "stored report notarization_receipt_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: ProvenanceCertificateLatestReportView =
                load_required_step_json(
                    &status.latest_provenance_certificate_plan_step,
                    &paths.latest_provenance_certificate_plan_report_path,
                    "latest_provenance_certificate_plan_step",
                    &mut mismatches,
                )?;
            compare_step_artifact_against_report_metadata(
                status.latest_provenance_certificate_plan_step.as_ref(),
                &paths.latest_provenance_certificate_plan_report_path,
                "plan_latest_provenance_certificate",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_provenance_certificate_plan_step",
                &mut mismatches,
            );
            compare_provenance_certificate_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_provenance_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_provenance_certificate_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let stored_latest_run: ProvenanceCertificateLatestReportView = load_required_step_json(
                &status.latest_provenance_certificate_step,
                &paths.latest_provenance_certificate_report_path,
                "latest_provenance_certificate_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_provenance_certificate_step.as_ref(),
                &paths.latest_provenance_certificate_report_path,
                "run_latest_provenance_certificate",
                &stored_latest_run.verdict,
                &stored_latest_run.reason,
                stored_latest_run.generated_at,
                "stored latest_provenance_certificate_step",
                &mut mismatches,
            );
            compare_provenance_certificate_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_provenance_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_provenance_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let actual_latest_run: ProvenanceCertificateLatestReportView = match load_json(
                &nested_provenance_certificate_latest_report_path(
                    &paths.nested_provenance_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed loading actual nested latest-provenance-certificate report under {}: {error}",
                        paths.nested_provenance_certificate_latest_session_dir.display()
                    ));
                    fallback_provenance_certificate_latest_copy()
                }
            };
            compare_provenance_certificate_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                PROVENANCE_CERTIFICATE_LATEST_BIN,
                &latest_provenance_certificate_verify_args(
                    &paths.nested_provenance_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-provenance-certificate session under {}: {error}",
                        paths.nested_provenance_certificate_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: ProvenanceCertificateLatestReportView = serde_json::from_value(
                    latest_verify_raw,
                )
                .context("failed parsing nested latest-provenance-certificate verify json")?;
                if latest_verify.verdict != PROVENANCE_CERTIFICATE_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-provenance-certificate verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: NotarizationReceiptReportView = load_required_step_json(
            ...
            compare_step_artifact_against_report_metadata(
                ...
                "stored notarization_receipt_step",
                &mut mismatches,
            );
            compare_notarization_receipt_run_against_snapshot(
            ...
            compare_option_string(
                ...
                "stored status notarization_receipt_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.notarization_receipt_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: NotarizationReceiptSessionView =
                match load_json(&nested_notarization_receipt_session_path(
                    &paths.nested_notarization_receipt_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        ...
                        paths.nested_notarization_receipt_session_dir.display()
                    ));
            ...
            let nested_handoff_bundle_status: NotarizationReceiptStatusView =
                match load_json(&nested_notarization_receipt_status_path(
                    &paths.nested_notarization_receipt_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
            ...
            compare_notarization_receipt_copy_against_nested_truth(
                &stored_handoff_bundle,
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: NotarizationReceiptReportView =
                    serde_json::from_value(handoff_bundle_verify_raw)
                        .context("failed parsing nested provenance-certificate verify json")?;
                compare_option_string(
                    stored_handoff_bundle.attestation_seal_session_dir.as_deref(),
                    handoff_bundle_verify.attestation_seal_session_dir.as_deref(),
                    "stored provenance-certificate copy attestation_seal_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.chain_fingerprint_summary.as_deref(),
                    handoff_bundle_verify.chain_fingerprint_summary.as_deref(),
                    "stored provenance-certificate copy chain_fingerprint_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.chain_fingerprint_sha256.as_deref(),
                    handoff_bundle_verify.chain_fingerprint_sha256.as_deref(),
                    "stored provenance-certificate copy chain_fingerprint_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.chain_fingerprint_algorithm.as_deref(),
                    handoff_bundle_verify.chain_fingerprint_algorithm.as_deref(),
                    "stored provenance-certificate copy chain_fingerprint_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_json(
                    stored_handoff_bundle.release_capsule_digest_manifest.as_ref(),
                    handoff_bundle_verify.release_capsule_digest_manifest.as_ref(),
                    "stored provenance-certificate copy release_capsule_digest_manifest against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                if handoff_bundle_verify.verdict != NOTARIZATION_RECEIPT_VERIFY_OK {
                    mismatches.push(format!(
            ...
            let expected_result = notarization_receipt_latest_result_from_notarization_receipt(
                nested_handoff_bundle_status.result.as_str(),
            )
            .unwrap_or(
                LivePackageNotarizationReceiptLatestResult::RefusedByDriftedProvenanceCertificateContract,
            );
            if status.result != expected_result {
                mismatches.push(format!(
                    "stored latest-notarization-receipt result {} does not match nested provenance-certificate result {}",
            ...
            if status.activation_authorized {
                mismatches.push(
                    "stored status activation_authorized must remain false for latest-notarization-receipt"
                        .to_string(),
                );
            }
            compare_option_string(
        ...
        Err(failure) => {
            ...
            if failure.latest_provenance_certificate_plan_raw.is_some() {
                let stored_latest_plan: ProvenanceCertificateLatestReportView =
                    load_required_step_json(
                        &status.latest_provenance_certificate_plan_step,
                        &paths.latest_provenance_certificate_plan_report_path,
                ...
                compare_option_string(
                    ...
                    failure
                        .latest_provenance_certificate_plan_verdict
                        .as_deref(),
                    "stored status latest_provenance_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report
                    ...
                    failure
                        .latest_provenance_certificate_plan_verdict
                        .as_deref(),
                    "stored report latest_provenance_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
                status.latest_provenance_certificate_verdict.as_deref(),
                None,
                "stored status latest_provenance_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
                    ...
                    .as_deref(),
                None,
                "stored report latest_provenance_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.notarization_receipt_verdict.as_deref(),
                None,
                "stored status notarization_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.notarization_receipt_verdict.as_deref(),
                None,
                "stored report notarization_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestVerifyOk
    } else {
        TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-notarization-receipt session under {} is coherent with the current latest immutable chain and the accepted notarization-receipt contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-notarization-receipt verification failed".to_string())
    };

    Ok(PackageNotarizationReceiptLatestReport {
        ...
        notarization_receipt_latest_session_path: Some(paths.session_path.display().to_string()),
        notarization_receipt_latest_status_path: Some(paths.status_path.display().to_string()),
        notarization_receipt_latest_report_path: Some(paths.report_path.display().to_string()),
        latest_provenance_certificate_plan_report_path: Some(
            paths
                .latest_provenance_certificate_plan_report_path
                .display()
                .to_string(),
        ),
        latest_provenance_certificate_report_path: Some(
            paths.latest_provenance_certificate_report_path.display().to_string(),
        ),
        notarization_receipt_report_path: Some(paths.notarization_receipt_report_path.display().to_string()),
        nested_provenance_certificate_latest_session_dir: Some(
            paths
                .nested_provenance_certificate_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_notarization_receipt_session_dir: Some(
            paths.nested_notarization_receipt_session_dir.display().to_string(),
        ),
        ...
        latest_provenance_certificate_plan_verdict: status.latest_provenance_certificate_plan_verdict.clone(),
        latest_provenance_certificate_verdict: status.latest_provenance_certificate_verdict.clone(),
        notarization_receipt_verdict: status.notarization_receipt_verdict.clone(),
        ...
        resolve_latest_provenance_certificate_command_summary: Some(
            session.resolve_latest_provenance_certificate_command_summary.clone(),
        ),
        run_latest_provenance_certificate_command_summary: Some(
            session.run_latest_provenance_certificate_command_summary.clone(),
        ),
        verify_latest_provenance_certificate_command_summary: Some(
            session.verify_latest_provenance_certificate_command_summary.clone(),
        ),
        downstream_notarization_receipt_command_summary: Some(
            session.downstream_notarization_receipt_command_summary.clone(),
        ),
        verify_notarization_receipt_command_summary: Some(session.verify_notarization_receipt_command_summary.clone()),
        ...
        explicit_statement:
            "this verification proves only whether the latest immutable chain still resolves cleanly into the accepted notarization-receipt contract; it never runs the frozen controller"
                .to_string(),
    })
}
...
fn run_latest_provenance_certificate_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackageNotarizationReceiptLatestPaths,
    resolved: &ResolvedLatestNotarizationReceipt,
) -> Result<PackageNotarizationReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, Some(resolved), None);
    persist_json(&paths.session_path, &session)?;
    persist_json(
        &paths.latest_provenance_certificate_plan_report_path,
        &resolved.latest_provenance_certificate_plan_raw,
    )?;

    let latest_provenance_certificate_plan_step = Some(step_artifact(
        &paths.latest_provenance_certificate_plan_report_path,
        "plan_latest_provenance_certificate",
        &resolved.latest_provenance_certificate_plan.verdict,
        &resolved.latest_provenance_certificate_plan.reason,
        resolved.latest_provenance_certificate_plan.generated_at,
    ));

    let latest_provenance_certificate_run_raw = match run_json_command(
        PROVENANCE_CERTIFICATE_LATEST_BIN,
        &latest_provenance_certificate_run_args(
            root,
            &paths.nested_provenance_certificate_latest_session_dir,
        ),
    ) {
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageNotarizationReceiptLatestResult::RefusedByDriftedProvenanceCertificateContract,
                format!(
                    "failed running nested latest-provenance-certificate handoff under {}: {error}",
                    paths.nested_provenance_certificate_latest_session_dir.display()
                ),
                latest_provenance_certificate_plan_step,
                None,
    ...
    persist_json(
        &paths.latest_provenance_certificate_report_path,
        &latest_provenance_certificate_run_raw,
    )?;
    let latest_provenance_certificate_run: ProvenanceCertificateLatestReportView =
        serde_json::from_value(latest_provenance_certificate_run_raw)
            .context("failed parsing nested latest-provenance-certificate run json")?;
    let latest_provenance_certificate_step = Some(step_artifact(
        &paths.latest_provenance_certificate_report_path,
        "run_latest_provenance_certificate",
        &latest_provenance_certificate_run.verdict,
        &latest_provenance_certificate_run.reason,
        latest_provenance_certificate_run.generated_at,
    ));

    if let Some(terminal_failure) =
        latest_provenance_certificate_terminal_failure(&latest_provenance_certificate_run)
    {
        let status = refusal_status(
            root,
            session_dir,
            ...
            Some(resolved),
            terminal_failure,
            latest_provenance_certificate_run.reason.clone(),
            latest_provenance_certificate_plan_step,
            latest_provenance_certificate_step,
            None,
        );
    ...
    compare_provenance_certificate_latest_run_against_snapshot(
        &latest_provenance_certificate_run,
        &resolved.snapshot,
        &paths.nested_provenance_certificate_latest_session_dir,
        &mut run_mismatches,
    );
    ...
    let latest_provenance_certificate_verify: ProvenanceCertificateLatestReportView =
        match run_json_command(
            ...
            Err(error) => {
                ...
                LivePackageNotarizationReceiptLatestResult::RefusedByDriftedProvenanceCertificateContract,
                format!(
                    "failed verifying nested latest-provenance-certificate handoff under {}: {error}",
                    paths.nested_provenance_certificate_latest_session_dir.display()
                ),
                latest_provenance_certificate_plan_step,
                latest_provenance_certificate_step,
    ...
    if latest_provenance_certificate_verify.verdict != PROVENANCE_CERTIFICATE_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            LivePackageNotarizationReceiptLatestResult::RefusedByDriftedProvenanceCertificateContract,
            latest_provenance_certificate_verify.reason,
            latest_provenance_certificate_plan_step,
            latest_provenance_certificate_step,
            None,
        );
    ...
    let provenance_certificate_run: NotarizationReceiptReportView =
    ...
    let provenance_certificate_verify: NotarizationReceiptReportView = match run_json_command(
        NOTARIZATION_RECEIPT_BIN,
        &notarization_receipt_verify_args(
            &resolved.snapshot,
            &expected_downstream_provenance_certificate_session_dir(paths),
    ...
    if provenance_certificate_verify.verdict != NOTARIZATION_RECEIPT_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    let result = notarization_receipt_latest_result_from_notarization_receipt(
        provenance_certificate_run
            .result
            .as_deref()
            .unwrap_or_default(),
    ...
    let status = PackageNotarizationReceiptLatestStatus {
        status_version: NOTARIZATION_RECEIPT_LATEST_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nested_provenance_certificate_latest_session_dir: paths
            .nested_provenance_certificate_latest_session_dir
            .display()
            .to_string(),
        nested_notarization_receipt_session_dir: paths
            .nested_notarization_receipt_session_dir
            .display()
            .to_string(),
        ...
        latest_provenance_certificate_plan_verdict: Some(
            resolved.latest_provenance_certificate_plan.verdict.clone(),
        ),
        latest_provenance_certificate_verdict: Some(
            latest_provenance_certificate_run.verdict.clone(),
        ),
        notarization_receipt_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_provenance_certificate_failure(
    ...
    session_dir: &Path,
    paths: &PackageNotarizationReceiptLatestPaths,
    failure: &LatestNotarizationReceiptResolutionFailure,
) -> Result<PackageNotarizationReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, None, Some(failure));
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_provenance_certificate_plan_raw {
        persist_json(&paths.latest_provenance_certificate_plan_report_path, value)?;
    }
...
fn resolve_latest_provenance_certificate(
    ...
    ResolvedLatestNotarizationReceipt,
    LatestNotarizationReceiptResolutionFailure,
> {
    let latest_provenance_certificate_plan_raw = match run_json_command(
        PROVENANCE_CERTIFICATE_LATEST_BIN,
        &latest_provenance_certificate_plan_args(
            root,
            &paths.nested_provenance_certificate_latest_session_dir,
        ),
    ) {
        Ok(value) => value,
        Err(error) => {
            return Err(LatestNotarizationReceiptResolutionFailure {
                ...
                snapshot: None,
                latest_provenance_certificate_plan_raw: None,
                latest_provenance_certificate_plan_verdict: None,
                latest_provenance_certificate_plan_reason: None,
                latest_provenance_certificate_plan_generated_at: None,
            });
        }
    };
    let latest_provenance_certificate_plan: ProvenanceCertificateLatestReportView =
        serde_json::from_value(latest_provenance_certificate_plan_raw.clone()).map_err(
            |error| LatestNotarizationReceiptResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-provenance-certificate plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_provenance_certificate_plan_raw: Some(
                    latest_provenance_certificate_plan_raw.clone(),
                ),
                latest_provenance_certificate_plan_verdict: None,
                latest_provenance_certificate_plan_reason: None,
                latest_provenance_certificate_plan_generated_at: None,
    ...
    let snapshot =
        parse_snapshot_from_provenance_certificate_latest(&latest_provenance_certificate_plan).ok();
    if latest_provenance_certificate_plan.verdict != PROVENANCE_CERTIFICATE_LATEST_PLAN_READY {
        return Err(LatestNotarizationReceiptResolutionFailure {
            kind: map_provenance_certificate_latest_failure_kind(
                &latest_provenance_certificate_plan.verdict,
            ),
            reason: latest_provenance_certificate_plan.reason.clone(),
            snapshot,
            latest_provenance_certificate_plan_raw: Some(latest_provenance_certificate_plan_raw),
            latest_provenance_certificate_plan_verdict: Some(
                latest_provenance_certificate_plan.verdict,
            ),
            latest_provenance_certificate_plan_reason: Some(
                latest_provenance_certificate_plan.reason,
            ),
            latest_provenance_certificate_plan_generated_at: Some(
                latest_provenance_certificate_plan.generated_at,
            ),
        });
    }

    let snapshot =
        parse_snapshot_from_provenance_certificate_latest(&latest_provenance_certificate_plan)
            .map_err(|error| LatestNotarizationReceiptResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                ...
                latest_provenance_certificate_plan_verdict: Some(
                    latest_provenance_certificate_plan.verdict.clone(),
                ),
                latest_provenance_certificate_plan_reason: Some(
                    latest_provenance_certificate_plan.reason.clone(),
                ),
                latest_provenance_certificate_plan_generated_at: Some(
                    latest_provenance_certificate_plan.generated_at,
                ),
    ...
    let mut mismatches = Vec::new();
    compare_provenance_certificate_latest_plan_against_snapshot(
        &latest_provenance_certificate_plan,
        &snapshot,
        &paths.nested_provenance_certificate_latest_session_dir,
        &mut mismatches,
    );
    ...
    Ok(ResolvedLatestNotarizationReceipt {
        snapshot,
        latest_provenance_certificate_plan_raw,
        latest_provenance_certificate_plan,
    })
}

fn parse_snapshot_from_provenance_certificate_latest(
    view: &ProvenanceCertificateLatestReportView,
) -> Result<LatestNotarizationReceiptSnapshot> {
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-provenance-certificate plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-provenance-certificate plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestNotarizationReceiptSnapshot {
        latest_top_step_name,
        ...
        wrapper_timeout_ms: view.wrapper_timeout_ms.ok_or_else(|| {
            anyhow!("latest-provenance-certificate plan is missing wrapper_timeout_ms")
        })?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!("latest-provenance-certificate plan is missing service_status_max_staleness_ms")
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackageNotarizationReceiptLatestPaths,
    resolved: &ResolvedLatestNotarizationReceipt,
) -> PackageNotarizationReceiptLatestReport {
    PackageNotarizationReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        latest_provenance_certificate_report_path: Some(
            paths
                .latest_provenance_certificate_report_path
                .display()
                .to_string(),
        ),
        notarization_receipt_report_path: Some(
        ...
        latest_provenance_certificate_verdict: None,
        notarization_receipt_verdict: None,
        result: None,
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_provenance_certificate_command_summary: Some(
            resolve_latest_provenance_certificate_command_summary(
                root,
                &paths.nested_provenance_certificate_latest_session_dir,
            ),
        ),
        run_latest_provenance_certificate_command_summary: Some(
            run_latest_provenance_certificate_command_summary(
                root,
                &paths.nested_provenance_certificate_latest_session_dir,
            ),
        ),
        verify_latest_provenance_certificate_command_summary: Some(
            verify_latest_provenance_certificate_command_summary(
                &paths.nested_provenance_certificate_latest_session_dir,
            ),
        ),
        downstream_notarization_receipt_command_summary: Some(
            downstream_notarization_receipt_command_summary(
                &resolved.snapshot,
                &expected_downstream_provenance_certificate_session_dir(paths),
                &paths.nested_notarization_receipt_session_dir,
            ),
        ...
        verify_notarization_receipt_command_summary: Some(
            verify_notarization_receipt_command_summary(
                &resolved.snapshot,
                &expected_downstream_provenance_certificate_session_dir(paths),
                &paths.nested_notarization_receipt_session_dir,
            ),
...
fn report_from_resolution_failure(
    ...
    PackageNotarizationReceiptLatestReport {
        ...
        latest_provenance_certificate_plan_verdict: failure
            .latest_provenance_certificate_plan_verdict
            .clone(),
        latest_provenance_certificate_verdict: None,
        notarization_receipt_verdict: None,
        ...
        downstream_notarization_receipt_command_summary: snapshot.map(|value| {
            downstream_notarization_receipt_command_summary(
                value,
                &expected_downstream_provenance_certificate_session_dir(paths),
                &paths.nested_notarization_receipt_session_dir,
        ...
        verify_notarization_receipt_command_summary: snapshot.map(|value| {
            verify_notarization_receipt_command_summary(
                value,
                &expected_downstream_provenance_certificate_session_dir(paths),
                &paths.nested_notarization_receipt_session_dir,
...
fn planned_session(
    ...
    session_dir: &Path,
    paths: &PackageNotarizationReceiptLatestPaths,
    resolved: Option<&ResolvedLatestNotarizationReceipt>,
    failure: Option<&LatestNotarizationReceiptResolutionFailure>,
) -> PackageNotarizationReceiptLatestSession {
    let snapshot = resolved
        .map(|value| &value.snapshot)
        .or_else(|| failure.and_then(|value| value.snapshot.as_ref()));
    PackageNotarizationReceiptLatestSession {
        session_version: NOTARIZATION_RECEIPT_LATEST_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        root: root.display().to_string(),
        session_dir: session_dir.display().to_string(),
        nested_provenance_certificate_latest_session_dir: paths
        ...
        downstream_notarization_receipt_command_summary: snapshot
            .map(|value| {
                downstream_notarization_receipt_command_summary(
                    value,
                    &expected_downstream_provenance_certificate_session_dir(paths),
        ...
        verify_notarization_receipt_command_summary: snapshot
            .map(|value| {
                verify_notarization_receipt_command_summary(
                    value,
                    &expected_downstream_provenance_certificate_session_dir(paths),
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackageNotarizationReceiptLatestPaths,
    session: &PackageNotarizationReceiptLatestSession,
    status: &PackageNotarizationReceiptLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackageNotarizationReceiptLatestReport {
    PackageNotarizationReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        nested_notarization_receipt_session_dir: Some(
            session.nested_notarization_receipt_session_dir.clone(),
        ),
        downstream_decision_packet_session_dir: Some(
            session.downstream_decision_packet_session_dir.clone(),
        ),
        latest_provenance_certificate_plan_verdict: status
            .latest_provenance_certificate_plan_verdict
            .clone(),
        latest_provenance_certificate_verdict: status.latest_provenance_certificate_verdict.clone(),
        notarization_receipt_verdict: status.notarization_receipt_verdict.clone(),
        ...
        verify_notarization_receipt_command_summary: Some(
            session.verify_notarization_receipt_command_summary.clone(),
        ),
        reviewed_frozen_live_cutover_controller_command_summary: session
            .reviewed_frozen_live_cutover_controller_command_summary
            .clone(),
...
fn notarization_receipt_latest_paths(session_dir: &Path) -> PackageNotarizationReceiptLatestPaths {
    PackageNotarizationReceiptLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_notarization_receipt_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_notarization_receipt_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_notarization_receipt_latest.report.json"),
        latest_provenance_certificate_plan_report_path: session_dir.join(
            "tiny_live_activation_package_notarization_receipt_latest.provenance_certificate_latest_plan.report.json",
        ),
        latest_provenance_certificate_report_path: session_dir.join(
            "tiny_live_activation_package_notarization_receipt_latest.provenance_certificate_latest.report.json",
        ),
        notarization_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_notarization_receipt_latest.notarization_receipt.report.json",
        ),
        nested_provenance_certificate_latest_session_dir: session_dir
            .join("tiny_live_activation_package_notarization_receipt_latest.provenance_certificate_latest_session"),
        nested_notarization_receipt_session_dir: session_dir.join(
            "tiny_live_activation_package_notarization_receipt_latest.notarization_receipt_session",
        ),
    }
}
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_provenance_certificate_latest_session_dir
        .join("tiny_live_activation_package_provenance_certificate_latest.decision_packet_session")
}

fn expected_downstream_provenance_certificate_session_dir(
    paths: &PackageNotarizationReceiptLatestPaths,
) -> PathBuf {
    paths.nested_provenance_certificate_latest_session_dir.join(
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_provenance_certificate_latest_session_dir
        .join("tiny_live_activation_package_provenance_certificate_latest.review_receipt_session")
}

fn latest_provenance_certificate_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-provenance-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn latest_provenance_certificate_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-provenance-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn latest_provenance_certificate_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-provenance-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn notarization_receipt_run_args(
    snapshot: &LatestNotarizationReceiptSnapshot,
    ...
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-notarization-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn notarization_receipt_verify_args(
    snapshot: &LatestNotarizationReceiptSnapshot,
    provenance_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
...
fn resolve_latest_provenance_certificate_command_summary(
    root: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{PROVENANCE_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --plan-latest-provenance-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn run_latest_provenance_certificate_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{PROVENANCE_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --run-latest-provenance-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn verify_latest_provenance_certificate_command_summary(session_dir: &Path) -> String {
    format!(
        "{PROVENANCE_CERTIFICATE_LATEST_BIN} --session-dir {} --verify-latest-provenance-certificate --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_notarization_receipt_command_summary(
    snapshot: &LatestNotarizationReceiptSnapshot,
    provenance_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{NOTARIZATION_RECEIPT_BIN} --provenance-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-notarization-receipt --json",
...
fn verify_notarization_receipt_command_summary(
    snapshot: &LatestNotarizationReceiptSnapshot,
    provenance_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{NOTARIZATION_RECEIPT_BIN} --provenance-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-notarization-receipt --json",
        shell_escape_path(provenance_certificate_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
...
fn nested_provenance_certificate_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_provenance_certificate_latest.report.json")
}

fn nested_notarization_receipt_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_notarization_receipt.session.json")
}

fn nested_notarization_receipt_status_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_notarization_receipt.status.json")
}
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-notarization-receipt script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageNotarizationReceiptLatestStepArtifact {
    PackageNotarizationReceiptLatestStepArtifact {
...
fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackageNotarizationReceiptLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-notarization-receipt status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-notarization-receipt status"
        ));
        return;
    };
    compare_string(
...
fn compare_provenance_certificate_latest_plan_against_snapshot(
    stored: &ProvenanceCertificateLatestReportView,
    snapshot: &LatestNotarizationReceiptSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        ...
        Some(PROVENANCE_CERTIFICATE_LATEST_PLAN_READY),
        "stored latest-provenance-certificate plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_provenance_certificate",
        "stored latest-provenance-certificate plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-provenance-certificate plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-provenance-certificate plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-provenance-certificate plan session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-provenance-certificate plan downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-provenance-certificate plan package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-provenance-certificate plan install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-provenance-certificate plan target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-provenance-certificate plan backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-provenance-certificate plan wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-provenance-certificate plan service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-provenance-certificate plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-provenance-certificate plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-provenance-certificate plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-provenance-certificate plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_provenance_certificate_latest_run_against_snapshot(
    stored: &ProvenanceCertificateLatestReportView,
    snapshot: &LatestNotarizationReceiptSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_provenance_certificate",
        "stored latest-provenance-certificate run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-provenance-certificate run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-provenance-certificate run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-provenance-certificate run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-provenance-certificate run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-provenance-certificate run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-provenance-certificate run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-provenance-certificate run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-provenance-certificate run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-provenance-certificate run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-provenance-certificate run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-provenance-certificate run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-provenance-certificate run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-provenance-certificate run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-provenance-certificate run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_provenance_certificate_latest_copy_matches_actual(
    stored: &ProvenanceCertificateLatestReportView,
    actual: &ProvenanceCertificateLatestReportView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        Some(stored.verdict.as_str()),
        Some(actual.verdict.as_str()),
        "stored latest-provenance-certificate copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-provenance-certificate copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-provenance-certificate copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-provenance-certificate copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-provenance-certificate copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-provenance-certificate copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-provenance-certificate copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-provenance-certificate copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-provenance-certificate copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-provenance-certificate copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-provenance-certificate copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-provenance-certificate copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_notarization_receipt_run_against_snapshot(
    stored: &NotarizationReceiptReportView,
    snapshot: &LatestNotarizationReceiptSnapshot,
    provenance_certificate_session_dir: &Path,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.provenance_certificate_session_dir,
        &provenance_certificate_session_dir.display().to_string(),
        "stored provenance-certificate run provenance_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        stored.notarization_receipt_session_path.as_deref(),
        Some(
            nested_notarization_receipt_session_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored provenance-certificate run notarization_receipt_session_path",
        mismatches,
    );
    compare_option_string(
        stored.notarization_receipt_status_path.as_deref(),
        Some(
            nested_notarization_receipt_status_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored provenance-certificate run notarization_receipt_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_provenance_certificate_report_path.as_deref(),
        Some(
            session_dir
                .join("tiny_live_activation_package_notarization_receipt.provenance_certificate.report.json")
                .display()
                .to_string()
                .as_str(),
        ),
        "stored notarization-receipt run archived_provenance_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        ...
        "stored notarization-receipt run archived_release_capsule_digest_manifest_path",
        mismatches,
    );
    compare_option_string(
        stored
            ...
            .as_deref(),
        Some(
            notarization_receipt_verify_provenance_certificate_command_summary(
                provenance_certificate_session_dir,
            )
            .as_str(),
        ),
        "stored notarization-receipt run verify_provenance_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
...
fn compare_notarization_receipt_copy_against_nested_truth(
    stored: &NotarizationReceiptReportView,
    nested_session: &NotarizationReceiptSessionView,
    nested_status: &NotarizationReceiptStatusView,
    mismatches: &mut Vec<String>,
) {
    compare_string(
        &stored.provenance_certificate_session_dir,
        &nested_session.provenance_certificate_session_dir,
        "stored provenance-certificate copy provenance_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        Some(stored.verdict.as_str()),
        notarization_receipt_run_verdict_for_result(&nested_status.result),
    ...
    compare_option_string(
        ...
        "stored notarization-receipt copy verify_provenance_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored provenance-certificate copy notarization_receipt_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        ...
        "stored provenance-certificate copy ledger_seal_summary",
        mismatches,
    );
    compare_option_string(
        stored.ledger_seal_sha256.as_deref(),
        Some(nested_status.ledger_seal_sha256.as_str()),
        "stored provenance-certificate copy ledger_seal_sha256",
        mismatches,
    );
    compare_option_string(
        stored.ledger_seal_algorithm.as_deref(),
        Some(nested_status.ledger_seal_algorithm.as_str()),
        "stored provenance-certificate copy ledger_seal_algorithm",
        mismatches,
    );
    compare_string(
        &stored.reason,
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_provenance_certificate_command_summary,
        &resolve_latest_provenance_certificate_command_summary(
            root,
            &paths.nested_provenance_certificate_latest_session_dir,
        ),
        "stored session resolve_latest_provenance_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.run_latest_provenance_certificate_command_summary,
        &run_latest_provenance_certificate_command_summary(
            root,
            &paths.nested_provenance_certificate_latest_session_dir,
        ),
        "stored session run_latest_provenance_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_latest_provenance_certificate_command_summary,
        &verify_latest_provenance_certificate_command_summary(
            &paths.nested_provenance_certificate_latest_session_dir,
        ),
        "stored session verify_latest_provenance_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.downstream_notarization_receipt_command_summary,
        &downstream_notarization_receipt_command_summary(
            snapshot,
            &expected_downstream_provenance_certificate_session_dir(paths),
            &paths.nested_notarization_receipt_session_dir,
        ),
        "stored session downstream_notarization_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_notarization_receipt_command_summary,
        &verify_notarization_receipt_command_summary(
            snapshot,
            &expected_downstream_provenance_certificate_session_dir(paths),
            &paths.nested_notarization_receipt_session_dir,
        ),
        "stored session verify_notarization_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_provenance_certificate_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_notarization_receipt_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_notarization_receipt_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackageNotarizationReceiptLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackageNotarizationReceiptLatestPaths,
    snapshot: &LatestNotarizationReceiptSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_provenance_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report run_latest_provenance_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report verify_latest_provenance_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report downstream_notarization_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report verify_notarization_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
    ...
    compare_option_string(
        report.notarization_receipt_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "stored report notarization_receipt_latest_session_path",
        mismatches,
    );
    compare_option_string(
        report.notarization_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "stored report notarization_receipt_latest_status_path",
        mismatches,
    );
    compare_option_string(
        report.notarization_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "stored report notarization_receipt_latest_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report latest_provenance_certificate_plan_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_provenance_certificate_report_path.as_deref(),
        Some(
            paths
                .latest_provenance_certificate_report_path
                .display()
        ...
        "stored report latest_provenance_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        report.notarization_receipt_report_path.as_deref(),
        Some(
            paths
                .notarization_receipt_report_path
                .display()
                .to_string()
                .as_str(),
        ),
        "stored report notarization_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report nested_provenance_certificate_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_notarization_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_notarization_receipt_session_dir
                .display()
        ...
        "stored report nested_notarization_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_provenance_certificate_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_notarization_receipt_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_notarization_receipt_session_dir on failure",
        mismatches,
    );
...
fn map_provenance_certificate_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_provenance_certificate_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_provenance_certificate_latest_refused_by_drifted_provenance_certificate_contract" => {
...
fn latest_provenance_certificate_terminal_failure(
    report: &ProvenanceCertificateLatestReportView,
) -> Option<LivePackageNotarizationReceiptLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackageNotarizationReceiptLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackageNotarizationReceiptLatestResult::RefusedByInvalidLatestChain)
        }
...
fn notarization_receipt_latest_result_from_notarization_receipt(
    result: &str,
) -> Option<LivePackageNotarizationReceiptLatestResult> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => {
            Some(LivePackageNotarizationReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen)
        }
        "refused_now_by_stage3" => Some(LivePackageNotarizationReceiptLatestResult::RefusedNowByStage3),
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackageNotarizationReceiptLatestResult::RefusedNowByPreActivationGate)
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some(LivePackageNotarizationReceiptLatestResult::RefusedByDriftedProvenanceCertificateContract)
        }
        _ => None,
    }
}

fn notarization_receipt_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        ...
        "refused_now_by_stage3" => {
            Some("tiny_live_package_notarization_receipt_refused_now_by_stage3")
        }
        "refused_now_by_pre_activation_gate" => {
            Some("tiny_live_package_notarization_receipt_refused_now_by_pre_activation_gate")
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_notarization_receipt_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}
...
fn result_from_failure_kind(
    kind: ResolutionFailureKind,
) -> LivePackageNotarizationReceiptLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackageNotarizationReceiptLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackageNotarizationReceiptLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedProvenanceCertificateContract => {
            LivePackageNotarizationReceiptLatestResult::RefusedByDriftedProvenanceCertificateContract
        }
...
fn verdict_for_result(
    result: LivePackageNotarizationReceiptLatestResult,
) -> TinyLivePackageNotarizationReceiptLatestVerdict {
    match result {
        LivePackageNotarizationReceiptLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestRefusedByMissingLatestChain
        }
        LivePackageNotarizationReceiptLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestRefusedByInvalidLatestChain
        }
        LivePackageNotarizationReceiptLatestResult::RefusedByDriftedProvenanceCertificateContract => {
            TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestRefusedByDriftedProvenanceCertificateContract
        }
        LivePackageNotarizationReceiptLatestResult::RefusedNowByStage3 => {
            TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestRefusedNowByStage3
        }
        LivePackageNotarizationReceiptLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestRefusedNowByPreActivationGate
        }
        LivePackageNotarizationReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackageNotarizationReceiptLatestResult) -> String {
    match result {
        LivePackageNotarizationReceiptLatestResult::RefusedByMissingLatestChain => {
        ...
        LivePackageNotarizationReceiptLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-provenance-certificate / provenance-certificate path".to_string()
        }
        LivePackageNotarizationReceiptLatestResult::RefusedByDriftedProvenanceCertificateContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-provenance-certificate / notarization-receipt contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackageNotarizationReceiptLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-notarization-receipt refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackageNotarizationReceiptLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-notarization-receipt refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackageNotarizationReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            "the latest immutable chain now resolves into the accepted notarization-receipt contract and one bounded latest notarization-receipt handoff is ready for manual execution when gate truth turns green; this wrapper still did not run or authorize the frozen controller".to_string()
        }
    }
}
...
fn notarization_receipt_verify_provenance_certificate_command_summary(
    provenance_certificate_session_dir: &Path,
) -> String {
    format!(
...
fn refusal_status(
    ...
    latest_provenance_certificate_step: Option<PackageNotarizationReceiptLatestStepArtifact>,
    notarization_receipt_step: Option<PackageNotarizationReceiptLatestStepArtifact>,
) -> PackageNotarizationReceiptLatestStatus {
    ...
    PackageNotarizationReceiptLatestStatus {
        ...
        result,
        reason,
        latest_provenance_certificate_plan_verdict: resolved
            .map(|value| value.latest_provenance_certificate_plan.verdict.clone()),
        latest_provenance_certificate_verdict: latest_provenance_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        notarization_receipt_verdict: notarization_receipt_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackageNotarizationReceiptLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_provenance_certificate_plan_step: failure
            .latest_provenance_certificate_plan_raw
            .as_ref()
            .map(|_| {
                step_artifact(
                    ...
                    failure
                        ...
                        .unwrap_or("<missing>"),
                    failure
                        .latest_provenance_certificate_plan_reason
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_provenance_certificate_plan_generated_at
                        .unwrap_or_else(Utc::now),
                )
            }),
        latest_provenance_certificate_step: None,
...
fn verify_invalid_provenance_certificate_latest_report_without_session(
    ...
    PackageNotarizationReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_provenance_certificate".to_string(),
        verdict: TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestVerifyInvalid,
        reason,
        root: None,
        ...
        session_dir: Some(session_dir.display().to_string()),
        notarization_receipt_latest_session_path: Some(
            notarization_receipt_latest_paths(session_dir)
                .session_path
                .display()
                .to_string(),
        ),
        notarization_receipt_latest_status_path: Some(
            notarization_receipt_latest_paths(session_dir)
                .status_path
                .display()
                .to_string(),
        ),
        notarization_receipt_latest_report_path: Some(
            notarization_receipt_latest_paths(session_dir)
                .report_path
                .display()
                .to_string(),
        ),
        latest_provenance_certificate_plan_report_path: None,
        latest_provenance_certificate_report_path: None,
        notarization_receipt_report_path: None,
        nested_provenance_certificate_latest_session_dir: None,
        nested_notarization_receipt_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_provenance_certificate_command_summary: None,
        run_latest_provenance_certificate_command_summary: None,
        verify_latest_provenance_certificate_command_summary: None,
        downstream_notarization_receipt_command_summary: None,
        verify_notarization_receipt_command_summary: None,
...
fn fallback_provenance_certificate_latest_copy() -> ProvenanceCertificateLatestReportView {
    ProvenanceCertificateLatestReportView {
        generated_at: Utc::now(),
...
fn fallback_notarization_receipt_session(
    snapshot: &LatestNotarizationReceiptSnapshot,
    session_dir: &Path,
) -> NotarizationReceiptSessionView {
    NotarizationReceiptSessionView {
        ...
        session_dir: session_dir.display().to_string(),
        verify_provenance_certificate_command_summary:
            notarization_receipt_verify_provenance_certificate_command_summary(Path::new("<missing>")),
...
fn fallback_notarization_receipt_status(
    _snapshot: &LatestNotarizationReceiptSnapshot,
    session_dir: &Path,
) -> NotarizationReceiptStatusView {
    NotarizationReceiptStatusView {
        session_dir: session_dir.display().to_string(),
        provenance_certificate_session_dir: "<missing>".to_string(),
...
fn render_report_lines(report: &PackageNotarizationReceiptLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_provenance_certificate_latest_session_dir {
        lines.push(format!(
            "nested_provenance_certificate_latest_session_dir={value}"
        ));
    }
    ...
    if let Some(value) = &report.nested_notarization_receipt_session_dir {
        lines.push(format!("nested_notarization_receipt_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_notarization_receipt_command_summary {
        lines.push(format!(
            "downstream_notarization_receipt_command_summary={value}"
        ));
    }
...
mod tests {
    ...
    struct FakeProvenanceCertificateLatestFixture {
        temp_dir: PathBuf,
        root: PathBuf,
        ...
        provenance_certificate_latest_plan_json_path: PathBuf,
        provenance_certificate_latest_run_json_path: PathBuf,
        provenance_certificate_latest_verify_json_path: PathBuf,
    ...
    impl FakeProvenanceCertificateLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestNotarizationReceipt,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestNotarizationReceiptScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestNotarizationReceipt,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestNotarizationReceipt,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    #[test]
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_provenance_certificate_lineage(
    ) {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_plan_ok",
        ...
        let paths = notarization_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert_eq!(
            report.verdict,
            TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestPlanReady
        );
        ...
        assert!(report
            ...
            .as_deref()
            .unwrap()
            .contains(NOTARIZATION_RECEIPT_BIN));
    }
    ...
    fn render_mode_emits_exact_downstream_provenance_certificate_contract_not_second_controller_path(
        ...
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_render_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(script.contains(PROVENANCE_CERTIFICATE_LATEST_BIN));
        assert!(script.contains(NOTARIZATION_RECEIPT_BIN));
        assert!(script.contains("--provenance-certificate-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-notarization-receipt"));
        assert!(script.contains("--verify-live-package-notarization-receipt"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            (
                "provenance_certificate_latest_missing",
                "tiny_live_package_provenance_certificate_latest_refused_by_missing_latest_chain",
                "no persisted immutable chain exists",
                TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestRefusedByMissingLatestChain,
            ),
            (
                "provenance_certificate_latest_invalid",
                "tiny_live_package_provenance_certificate_latest_refused_by_invalid_latest_chain",
                "latest immutable chain is incomplete",
                TinyLivePackageNotarizationReceiptLatestVerdict::TinyLivePackageNotarizationReceiptLatestRefusedByInvalidLatestChain,
            ),
            (
                "provenance_certificate_latest_below",
                "tiny_live_package_provenance_certificate_latest_refused_by_invalid_latest_chain",
            ...
            let mut plan: Value = load_json(&fixture.provenance_certificate_latest_plan_json_path).unwrap();
            plan["verdict"] = json!(verdict);
            plan["reason"] = json!(reason);
            if label.ends_with("below") {
                plan["latest_top_step_name"] = json!("choir_receipt");
            }
            persist_json(&fixture.provenance_certificate_latest_plan_json_path, &plan).unwrap();
    ...
    #[test]
    fn plan_mode_maps_real_latest_provenance_certificate_drift_verdict_to_local_drift_refusal() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_plan_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value =
            load_json(&fixture.provenance_certificate_latest_plan_json_path).unwrap();
        plan["verdict"] =
            json!("tiny_live_package_provenance_certificate_latest_refused_by_drifted_provenance_certificate_contract");
        plan["reason"] = json!("real accepted latest-provenance-certificate drift");
        persist_json(&fixture.provenance_certificate_latest_plan_json_path, &plan).unwrap();
    ...
    #[test]
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_provenance_certificate_reports_real_drift(
    ) {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_run_drift",
            "refused_by_drifted_provenance_certificate_contract",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert_eq!(report.notarization_receipt_verdict.as_deref(), None);
    }

    #[test]
    fn run_mode_refuses_when_nested_release_capsule_lineage_drifts_from_latest_immutable_chain() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_handoff_bundle_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut run: Value =
            load_json(&fixture.provenance_certificate_latest_run_json_path).unwrap();
        run["package_dir"] = json!("/tampered/package");
        persist_json(&fixture.provenance_certificate_latest_run_json_path, &run).unwrap();
        ...
        assert!(report
            .reason
            .contains("stored latest-provenance-certificate run package_dir"));
    }

    #[test]
    fn verify_mode_fails_when_persisted_latest_provenance_certificate_artifacts_are_tampered_or_missing_nested_lineage_truth(
    ) {
        {
            let fixture = fake_provenance_certificate_latest_fixture(
                "provenance_certificate_latest_verify_tampered_copy",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = notarization_receipt_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.notarization_receipt_report_path).unwrap();
        ...
        {
            let fixture = fake_provenance_certificate_latest_fixture(
                "provenance_certificate_latest_verify_missing_nested_truth",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = notarization_receipt_latest_paths(&fixture.wrapper_session_dir);
            fs::remove_file(nested_provenance_certificate_latest_report_path(
                &paths.nested_provenance_certificate_latest_session_dir,
            ))
            .unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains(
                    "tiny_live_activation_package_provenance_certificate_latest.report.json",
                )
            }));
        }
    }
    ...
    #[test]
    fn verify_fails_when_copied_nested_latest_provenance_certificate_explicit_statement_drifts() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_verify_launch_packet_explicit_statement_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut report: Value =
            load_json(&paths.latest_provenance_certificate_report_path).unwrap();
        report["explicit_statement"] =
            json!("tampered nested latest release capsule explicit statement");
        persist_json(&paths.latest_provenance_certificate_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored latest-provenance-certificate copy explicit_statement")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_verify_decision_packet_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_verify_attestation_seal_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored provenance-certificate copy attestation_seal_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_verify_chain_fingerprint_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored provenance-certificate copy chain_fingerprint_summary")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_verify_chain_fingerprint_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored provenance-certificate copy chain_fingerprint_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_verify_chain_fingerprint_algorithm_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored provenance-certificate copy chain_fingerprint_algorithm")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_verify_decision_packet_anchor_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_verify_current_authorization_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    #[test]
    fn verify_fails_when_copied_nested_notarization_receipt_verdict_drifts() {
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_verify_release_capsule_verdict_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_current_live_cutover_controller_summary_drifts(
        ...
        let fixture = fake_provenance_certificate_latest_fixture(
            "provenance_certificate_latest_verify_current_live_cutover_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn all_modes_preserve_stage3_and_pre_activation_refusal_semantics_and_do_not_imply_activation_authorization_by_themselves(
        ...
        {
            let fixture = fake_provenance_certificate_latest_fixture(
                "provenance_certificate_latest_stage3",
                "refused_now_by_stage3",
                "refused_now_by_stage3",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        {
            let fixture = fake_provenance_certificate_latest_fixture(
                "provenance_certificate_latest_pre_activation",
                "refused_now_by_pre_activation_gate",
                "refused_now_by_pre_activation_gate",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        {
            let fixture = fake_provenance_certificate_latest_fixture(
                "provenance_certificate_latest_green",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn fake_provenance_certificate_latest_fixture(
        label: &str,
        provenance_certificate_latest_run_result: &str,
        handoff_bundle_result: &str,
    ) -> FakeProvenanceCertificateLatestFixture {
        let temp_dir = temp_dir(label);
        let root = temp_dir.join("root");
        let wrapper_session_dir = temp_dir.join("provenance-certificate-latest-session");
        let output_script_path = temp_dir.join("provenance-certificate-latest.sh");
        ...
        let provenance_certificate_latest_plan_json_path =
            temp_dir.join("provenance_certificate_latest.plan.json");
        let provenance_certificate_latest_run_json_path =
            temp_dir.join("provenance_certificate_latest.run.json");
        let provenance_certificate_latest_verify_json_path =
            temp_dir.join("provenance_certificate_latest.verify.json");
        ...
        let fixture = FakeProvenanceCertificateLatestFixture {
            temp_dir,
            root,
            wrapper_session_dir,
            output_script_path,
            ...
            install_root,
            backend_command,
            provenance_certificate_latest_plan_json_path,
            provenance_certificate_latest_run_json_path,
            provenance_certificate_latest_verify_json_path,
        ...
        persist_json(
            &fixture.provenance_certificate_latest_plan_json_path,
            &default_provenance_certificate_latest_plan_report(&fixture),
        )
        .unwrap();
        persist_json(
            &fixture.provenance_certificate_latest_run_json_path,
            &default_provenance_certificate_latest_run_report(
                &fixture,
                provenance_certificate_latest_run_result,
            ),
        )
        .unwrap();
        persist_json(
            &fixture.provenance_certificate_latest_verify_json_path,
            &default_provenance_certificate_latest_verify_report(
                &fixture,
                provenance_certificate_latest_run_result,
            ),
        )
        ...
        let paths = notarization_receipt_latest_paths(&fixture.wrapper_session_dir);
        write_fake_provenance_certificate_latest_command(
            &fixture.bin_dir.join(PROVENANCE_CERTIFICATE_LATEST_BIN),
            &fixture.provenance_certificate_latest_plan_json_path,
            &fixture.provenance_certificate_latest_run_json_path,
            &fixture.provenance_certificate_latest_verify_json_path,
            &fixture.root,
        ...
        write_fake_release_capsule_command(
            &fixture.bin_dir.join(NOTARIZATION_RECEIPT_BIN),
            &fixture.handoff_bundle_run_json_path,
            &fixture.handoff_bundle_verify_json_path,
            &fixture.handoff_bundle_session_json_path,
            &fixture.handoff_bundle_status_json_path,
    ...
    fn default_provenance_certificate_latest_plan_report(
        fixture: &FakeProvenanceCertificateLatestFixture,
    ) -> Value {
        let paths = notarization_receipt_latest_paths(&fixture.wrapper_session_dir);
        json!({
            "generated_at": Utc::now(),
            "mode": "plan_latest_provenance_certificate",
            "verdict": PROVENANCE_CERTIFICATE_LATEST_PLAN_READY,
            "reason": "latest immutable chain resolves cleanly into accepted latest-provenance-certificate contract",
            ...
            "session_dir": paths.nested_provenance_certificate_latest_session_dir.display().to_string(),
    ...
    fn canonical_provenance_certificate_latest_result(result: &str) -> &str {
        match result {
            "runnable_when_gate_truth_turns_green" => {
                "ready_for_manual_execution_when_gate_turns_green"
            }
    ...
    fn default_provenance_certificate_latest_run_report(
        fixture: &FakeProvenanceCertificateLatestFixture,
        result: &str,
    ) -> Value {
        let result = canonical_provenance_certificate_latest_result(result);
        let paths = notarization_receipt_latest_paths(&fixture.wrapper_session_dir);
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_provenance_certificate_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_provenance_certificate_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_provenance_certificate_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_latest_provenance_certificate",
            "verdict": verdict,
    ...
    fn default_provenance_certificate_latest_verify_report(
        fixture: &FakeProvenanceCertificateLatestFixture,
        result: &str,
    ) -> Value {
        let mut report = default_provenance_certificate_latest_run_report(fixture, result);
        report["mode"] = json!("verify_latest_provenance_certificate");
        report["verdict"] = json!(PROVENANCE_CERTIFICATE_LATEST_VERIFY_OK);
    ...
    fn default_provenance_certificate_session(
        ...
        let paths = notarization_receipt_latest_paths(&fixture.wrapper_session_dir);
        let provenance_certificate_session_dir =
            expected_downstream_provenance_certificate_session_dir(&paths);
        ...
        let snapshot = LatestNotarizationReceiptSnapshot {
        ...
        json!({
            ...
            "release_capsule_session_dir": provenance_certificate_session_dir.display().to_string(),
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": notarization_receipt_latest_paths(&fixture.wrapper_session_dir).nested_notarization_receipt_session_dir.display().to_string(),
            "verify_provenance_certificate_command_summary": notarization_receipt_verify_provenance_certificate_command_summary(&provenance_certificate_session_dir),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_notarization_receipt_session_dir.display().to_string(),
            "provenance_certificate_session_dir": expected_downstream_provenance_certificate_session_dir(&paths).display().to_string(),
            "result": result,
            "reason": reason,
            ...
            "current_pre_activation_gate_verdict": gate_verdict,
            "current_pre_activation_gate_reason": gate_reason,
            "provenance_certificate_step": Value::Null,
            ...
            "ledger_seal_sha256": "fixture-notarization-ledger-seal-sha256",
            "ledger_seal_algorithm": "sha256",
            "explicit_statement": "this notarization receipt is archival and read-only; it never executes or enables the frozen controller"
        })
    }
    ...
    fn default_provenance_certificate_run_report(
        ...
        let provenance_verdict = match result {
            ...
            _ => "tiny_live_package_notarization_receipt_refused_now_by_invalid_or_drifted_contract",
        };
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_notarization_receipt",
            ...
            "release_capsule_session_dir": expected_downstream_provenance_certificate_session_dir(&paths).display().to_string(),
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_notarization_receipt_session_dir.display().to_string(),
            "notarization_receipt_session_path": nested_notarization_receipt_session_path(&paths.nested_notarization_receipt_session_dir).display().to_string(),
            "notarization_receipt_status_path": nested_notarization_receipt_status_path(&paths.nested_notarization_receipt_session_dir).display().to_string(),
            "archived_provenance_certificate_report_path": paths.nested_notarization_receipt_session_dir.join("tiny_live_activation_package_notarization_receipt.provenance_certificate.report.json").display().to_string(),
            "archived_release_capsule_digest_manifest_path": paths.nested_notarization_receipt_session_dir.join("tiny_live_activation_package_notarization_receipt.release_capsule.digest_manifest.json").display().to_string(),
            "result": result,
            ...
            "provenance_certificate_step": Value::Null,
            "verify_provenance_certificate_command_summary": notarization_receipt_verify_provenance_certificate_command_summary(&expected_downstream_provenance_certificate_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn default_provenance_certificate_verify_report(
        ...
        report["mode"] = json!("verify_live_package_notarization_receipt");
        report["verdict"] = json!(NOTARIZATION_RECEIPT_VERIFY_OK);
        report["reason"] = json!("verify ok");
        report["explicit_statement"] = json!("this verification binds one immutable notarization receipt to verified provenance-certificate truth, the exact reviewed frozen controller summary, and one final ledger-seal identity over the canonical nested archival chain fingerprint");
        report
    }

    fn write_fake_provenance_certificate_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-provenance-certificate session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-provenance-certificate \"*) mode='plan';;\n  *\" --run-latest-provenance-certificate \"*) mode='run';;\n  *\" --verify-latest-provenance-certificate \"*) mode='verify';;\n  *) echo 'unexpected latest-provenance-certificate mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_provenance_certificate_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
    ...
    fn write_fake_release_capsule_command(
        ...
        let nested_session_path = nested_notarization_receipt_session_path(session_dir);
        let nested_status_path = nested_notarization_receipt_status_path(session_dir);
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --provenance-certificate-session-dir {} \"*) : ;;\n  *) echo 'unexpected notarization-receipt provenance-certificate-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected notarization-receipt confirm-decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected notarization-receipt session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --run-live-package-notarization-receipt \"*) mode='run';;\n  *\" --verify-live-package-notarization-receipt \"*) mode='verify';;\n  *) echo 'unexpected notarization-receipt mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  run) mkdir -p '{}'; cp '{}' '{}'; cp '{}' '{}'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
