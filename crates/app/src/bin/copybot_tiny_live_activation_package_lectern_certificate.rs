...
use copybot_app::tiny_live_activation::package_podium_receipt_lectern_certificate;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_lectern_certificate --podium-receipt-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-lectern-certificate | --render-live-package-lectern-certificate | --run-live-package-lectern-certificate | --verify-live-package-lectern-certificate)";
const LECTERN_CERTIFICATE_SESSION_VERSION: &str = "1";
const LECTERN_CERTIFICATE_STATUS_VERSION: &str = "1";
const LECTERN_CERTIFICATE_ALGORITHM: &str = "sha256";
const LECTERN_CERTIFICATE_SESSION_EXPLICIT_STATEMENT: &str =
...
const LECTERN_CERTIFICATE_STATUS_EXPLICIT_STATEMENT: &str =
    "this lectern certificate is archival and read-only; it never executes or enables the frozen controller";
const LECTERN_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    podium_receipt_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackageLecternCertificate,
    RenderLivePackageLecternCertificate,
    RunLivePackageLecternCertificate,
    VerifyLivePackageLecternCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageLecternCertificateVerdict {
    TinyLivePackageLecternCertificatePlanReady,
    TinyLivePackageLecternCertificateRendered,
    TinyLivePackageLecternCertificateRefusedNowByStage3,
    TinyLivePackageLecternCertificateRefusedNowByPreActivationGate,
    TinyLivePackageLecternCertificateRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageLecternCertificateReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageLecternCertificateVerifyOk,
    TinyLivePackageLecternCertificateVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageLecternCertificateResult {
    RefusedNowByStage3,
...
struct PackageLecternCertificateStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageLecternCertificateSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageLecternCertificateStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackageLecternCertificateResult,
    reason: String,
    ...
    podium_receipt_step: Option<PackageLecternCertificateStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageLecternCertificatePaths {
...
struct PackageLecternCertificateReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageLecternCertificateVerdict,
    reason: String,
    podium_receipt_session_dir: String,
...
struct NestedPodiumReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
struct LecternCertificatePayload<'a> {
    result: &'a str,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut podium_receipt_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--podium-receipt-session-dir" => {
                podium_receipt_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--podium-receipt-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-lectern-certificate" => set_mode(
                &mut mode,
                Mode::PlanLivePackageLecternCertificate,
                arg.as_str(),
            )?,
            "--render-live-package-lectern-certificate" => set_mode(
                &mut mode,
                Mode::RenderLivePackageLecternCertificate,
                arg.as_str(),
            )?,
            "--run-live-package-lectern-certificate" => set_mode(
                &mut mode,
                Mode::RunLivePackageLecternCertificate,
                arg.as_str(),
            )?,
            "--verify-live-package-lectern-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLivePackageLecternCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackageLecternCertificate) && output_path.is_none() {
        bail!("missing --output for --render-live-package-lectern-certificate");
    }
    if matches!(
        mode,
        Mode::RunLivePackageLecternCertificate | Mode::VerifyLivePackageLecternCertificate
    ) && session_dir.is_none()
    {
    ...
    let podium_receipt_session_dir = podium_receipt_session_dir
        .ok_or_else(|| anyhow!("missing --podium-receipt-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageLecternCertificate => {
            plan_live_package_lectern_certificate_report(&config)?
        }
        Mode::RenderLivePackageLecternCertificate => {
            render_live_package_lectern_certificate_report(&config)?
        }
        Mode::RunLivePackageLecternCertificate => {
            run_live_package_lectern_certificate_report(&config)?
        }
        Mode::VerifyLivePackageLecternCertificate => {
            verify_live_package_lectern_certificate_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_lectern_certificate_report(
    config: &Config,
) -> Result<PackageLecternCertificateReport> {
    let (podium_receipt_contract, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_podium_receipt_lectern_certificate::verify_live_package_podium_receipt_for_lectern_certificate(
                    &config.podium_receipt_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification =
                            classify_verified_podium_receipt(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_podium_receipt_lectern_certificate::load_live_package_podium_receipt_contract_for_lectern_certificate(
                            &config.podium_receipt_session_dir,
                        )?;
                        let classification =
                            classify_podium_receipt_contract(&raw_contract, false);
    ...
    let session_dir = config.session_dir.clone().unwrap_or_else(|| {
        config
            .podium_receipt_session_dir
            .join("lectern-certificate")
    });
    let result = serialize_enum(&classification.0);
    let lectern_certificate_sha256 = compute_lectern_certificate_sha256(
        &result,
    ...
    Ok(PackageLecternCertificateReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_lectern_certificate".to_string(),
        verdict:
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificatePlanReady,
        reason: format!(
            "podium-receipt-native lectern certificate is explicit for verified podium-receipt session {}; this command stays archival and read-only while freezing one final armorial identity over the fully culminated chain",
            config.podium_receipt_session_dir.display()
        ),
        podium_receipt_session_dir: config.podium_receipt_session_dir.display().to_string(),
        ...
        lectern_certificate_session_path: None,
        lectern_certificate_status_path: None,
        archived_podium_receipt_report_path: None,
        result: Some(result),
        ...
        lectern_certificate_summary: Some(lectern_certificate_summary(
            classification.0,
            &podium_receipt_contract.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        lectern_certificate_sha256: Some(lectern_certificate_sha256),
        lectern_certificate_algorithm: Some(LECTERN_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: LECTERN_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_lectern_certificate_report(
    config: &Config,
) -> Result<PackageLecternCertificateReport> {
    let plan = plan_live_package_lectern_certificate_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_lectern_certificate --podium-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-lectern-certificate --json\ncopybot_tiny_live_activation_package_lectern_certificate --podium-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-lectern-certificate --json\n",
        shell_escape_path(&config.podium_receipt_session_dir),
    ...
    Ok(PackageLecternCertificateReport {
        generated_at: Utc::now(),
        mode: "render_live_package_lectern_certificate".to_string(),
        verdict:
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateRendered,
        reason: format!(
            "rendered lectern-certificate run/verify script to {}; the primary input remains the immutable podium-receipt session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: LECTERN_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_lectern_certificate_report(
    config: &Config,
) -> Result<PackageLecternCertificateReport> {
    ...
    let verified_podium_receipt = match package_podium_receipt_lectern_certificate::verify_live_package_podium_receipt_for_lectern_certificate(
        &config.podium_receipt_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify podium-receipt session under {}: {error}",
                    config.podium_receipt_session_dir.display()
                ),
            ))
        }
    ...
    .with_context(|| {
        format!(
            "lectern-certificate session dir {} overlaps the managed live target surface derived from verified podium-receipt install_root {}",
            session_dir.display(),
            verified_podium_receipt.contract.install_root.display()
        )
    })?;
    ensure_clean_lectern_certificate_session_dir(session_dir)?;
    let paths = lectern_certificate_paths(session_dir);
    let classification = classify_verified_podium_receipt(&verified_podium_receipt);
    ...
    let podium_receipt_step = Some(step_artifact(
        &paths.podium_receipt_report_path,
        "verify_live_package_podium_receipt",
        &verified_podium_receipt.verdict,
        &verified_podium_receipt.reason,
        verified_podium_receipt.generated_at,
    ));
    let status = PackageLecternCertificateStatus {
        status_version: LECTERN_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        podium_receipt_algorithm: verified_podium_receipt
            ...
            .clone(),
        lectern_certificate_summary: lectern_certificate_summary(
            result,
            &verified_podium_receipt
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        lectern_certificate_sha256: lectern_certificate_sha256.clone(),
        lectern_certificate_algorithm: LECTERN_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: LECTERN_CERTIFICATE_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_lectern_certificate_report(
    config: &Config,
) -> Result<PackageLecternCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = lectern_certificate_paths(session_dir);
    let session: PackageLecternCertificateSession = load_json(&paths.session_path)?;
    let status: PackageLecternCertificateStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.podium_receipt_session_dir,
        &config.podium_receipt_session_dir.display().to_string(),
        "podium-receipt podium_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "podium-receipt session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "podium-receipt status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.podium_receipt_session_dir,
        &config.podium_receipt_session_dir.display().to_string(),
        "podium-receipt status podium_receipt_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "podium-receipt substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "podium-receipt status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "podium-receipt bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "podium-receipt status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "podium-receipt foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "podium-receipt status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_podium_receipt_report_view: NestedPodiumReceiptReportView =
        serde_json::from_value(stored_podium_receipt_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested podium-receipt report {}",
                paths.podium_receipt_report_path.display()
            )
        })?;
    let verified_podium_receipt_report_view: NestedPodiumReceiptReportView =
        serde_json::from_value(verified_podium_receipt.report_json.clone()).with_context(|| {
            format!(
                "failed parsing freshly verified nested podium-receipt report for {}",
                config.podium_receipt_session_dir.display()
            )
        })?;
        ...
        != verified_podium_receipt_report_view.generated_at
    {
        mismatches.push(format!(
            "lectern-certificate archived podium-receipt report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_podium_receipt_report_view.generated_at,
            verified_podium_receipt_report_view.generated_at
        ));
    }
    let expected_podium_receipt_step = step_artifact(
        &paths.podium_receipt_report_path,
        &verified_podium_receipt_report_view.mode,
        &verified_podium_receipt_report_view.verdict,
        &verified_podium_receipt_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_podium_receipt_step,
        "podium-receipt podium_receipt_step",
        &mut mismatches,
    );
    if verified_podium_receipt.verdict != "tiny_live_package_podium_receipt_verify_ok" {
        mismatches.push(format!(
            "lectern-certificate nested podium-receipt verification is non-green: {}",
            verified_podium_receipt.reason
        ));
    }

    let classification = classify_verified_podium_receipt(&verified_podium_receipt);
    let expected_lectern_certificate_sha256 = compute_lectern_certificate_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_podium_receipt.contract,
        classification.0,
        &expected_lectern_certificate_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "lectern-certificate session contract does not match freshly verified podium-receipt truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_podium_receipt.contract,
        classification,
        expected_podium_receipt_step.clone(),
        &expected_lectern_certificate_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "lectern-certificate status contract does not match freshly verified podium-receipt truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_podium_receipt.contract,
        TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateVerifyOk,
        &expected_status,
    ))
}

fn verify_podium_receipt_command_summary(podium_receipt_session_dir: &Path) -> String {
    format!(
...
fn classify_podium_receipt_contract(
    contract: &package_podium_receipt_lectern_certificate::LivePackagePodiumReceiptContractView,
    assume_verified: bool,
) -> (
    LivePackageLecternCertificateResult,
    TinyLivePackageLecternCertificateVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageLecternCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateRefusedNowByInvalidOrDriftedContract,
            "the podium-receipt chain is not verified, so this lectern certificate remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageLecternCertificateResult::RefusedNowByStage3,
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateRefusedNowByStage3,
            "the immutable podium receipt remains refused now by Stage 3, so this lectern certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageLecternCertificateResult::RefusedNowByPreActivationGate,
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateRefusedNowByPreActivationGate,
            "the immutable podium receipt remains refused now by the pre-activation gate, so this lectern certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageLecternCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable podium receipt is coherent and this lectern certificate is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageLecternCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateRefusedNowByInvalidOrDriftedContract,
            "the verified podium-receipt chain is invalid, drifted, or non-runnable, so this lectern certificate remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_podium_receipt(
    verified: &package_podium_receipt_lectern_certificate::VerifiedLivePackagePodiumReceiptLecternCertificateStep,
) -> (
    LivePackageLecternCertificateResult,
    TinyLivePackageLecternCertificateVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_podium_receipt_verify_ok" {
        return (
            LivePackageLecternCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateRefusedNowByInvalidOrDriftedContract,
            "the immutable podium receipt did not verify cleanly, so this lectern certificate remains refused"
                .to_string(),
        );
    }
    classify_podium_receipt_contract(&verified.contract, true)
}

fn lectern_certificate_summary(
    result: LivePackageLecternCertificateResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageLecternCertificateResult::RefusedNowByStage3 => {
            "Lectern certificate outcome: the immutable culminated chain remains refused now by Stage 3; archive this final armorial seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageLecternCertificateResult::RefusedNowByPreActivationGate => {
            "Lectern certificate outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final armorial seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageLecternCertificateResult::RefusedNowByInvalidOrDriftedContract => {
            "Lectern certificate outcome: the immutable culminated chain is invalid or drifted; archive this final armorial seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackageLecternCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Lectern certificate outcome: this immutable armorial seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn lectern_certificate_paths(session_dir: &Path) -> PackageLecternCertificatePaths {
    PackageLecternCertificatePaths {
        ...
        podium_receipt_report_path: session_dir
            .join("tiny_live_activation_package_lectern_certificate.podium_receipt.report.json"),
    }
}

fn ensure_clean_lectern_certificate_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing lectern-certificate session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating lectern-certificate session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageLecternCertificateSession {
    PackageLecternCertificateSession {
        session_version: LECTERN_CERTIFICATE_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        podium_receipt_session_dir: config.podium_receipt_session_dir.display().to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_podium_receipt_command_summary: verify_podium_receipt_command_summary(
            &config.podium_receipt_session_dir,
        ),
        ...
        lectern_certificate_sha256: lectern_certificate_sha256.to_string(),
        lectern_certificate_algorithm: LECTERN_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: LECTERN_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    podium_receipt_step: PackageLecternCertificateStepArtifact,
    lectern_certificate_sha256: &str,
) -> PackageLecternCertificateStatus {
    PackageLecternCertificateStatus {
        status_version: LECTERN_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        podium_receipt_step: Some(podium_receipt_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageLecternCertificatePaths,
    contract: &package_podium_receipt_lectern_certificate::LivePackagePodiumReceiptContractView,
    verdict: TinyLivePackageLecternCertificateVerdict,
    status: &PackageLecternCertificateStatus,
) -> PackageLecternCertificateReport {
    PackageLecternCertificateReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateVerifyOk
        {
            "verify_live_package_lectern_certificate".to_string()
        } else {
            "run_live_package_lectern_certificate".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        lectern_certificate_session_path: Some(paths.session_path.display().to_string()),
        lectern_certificate_status_path: Some(paths.status_path.display().to_string()),
        archived_podium_receipt_report_path: Some(
            paths.podium_receipt_report_path.display().to_string(),
        ),
        ...
        podium_receipt_step: status.podium_receipt_step.clone(),
        verify_podium_receipt_command_summary: Some(verify_podium_receipt_command_summary(
            &config.podium_receipt_session_dir,
        )),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
        ...
        explicit_statement: if verdict
            == TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateVerifyOk
        {
            LECTERN_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageLecternCertificateReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageLecternCertificate => {
                "plan_live_package_lectern_certificate".to_string()
            }
            Mode::RenderLivePackageLecternCertificate => {
                "render_live_package_lectern_certificate".to_string()
            }
            Mode::RunLivePackageLecternCertificate => {
                "run_live_package_lectern_certificate".to_string()
            }
        ...
        verdict,
        reason,
        podium_receipt_session_dir: config
            .podium_receipt_session_dir
            .display()
            .to_string(),
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable lectern certificate refused before persisting because the podium-receipt artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "podium-receipt-native lectern-certificate verification failed".to_string()
    });
    PackageLecternCertificateReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_lectern_certificate".to_string(),
        verdict:
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateVerifyInvalid,
        reason,
        podium_receipt_session_dir: config.podium_receipt_session_dir.display().to_string(),
        dais_receipt_session_dir: Some(contract.dais_receipt_session_dir.display().to_string()),
        pedestal_certificate_session_dir: Some(
        ...
        explicit_statement: LECTERN_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageLecternCertificateStepArtifact>,
    expected_path: &Path,
    label: &str,
    mismatches: &mut Vec<String>,
) -> Result<serde_json::Value> {
    let Some(step) = step else {
        mismatches.push(format!("{label} is missing from podium-receipt status"));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageLecternCertificateStepArtifact {
    PackageLecternCertificateStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageLecternCertificateStepArtifact>,
    expected: &PackageLecternCertificateStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_lectern_certificate_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&LecternCertificatePayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn render_report_lines(report: &PackageLecternCertificateReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "podium_receipt_session_dir={}",
            report.podium_receipt_session_dir
        ),
    ];
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_lectern_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify =
            verify_live_package_lectern_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageLecternCertificateVerdict::TinyLivePackageLecternCertificateVerifyOk
        );
    ...
    #[test]
    fn drifted_podium_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            "podium_receipt_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_podium_receipt",
                "verdict": "tiny_live_package_podium_receipt_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        let overlap_session_dir = managed_paths
            .runtime_dir
            .join("lectern-certificate-session");
        ...
        let error = run_live_package_lectern_certificate_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = lectern_certificate_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_podium_receipt_step_path() {
        let fixture = fake_command_fixture(
            "podium_receipt_tampered_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_lectern_certificate_report(&fixture.run_config()).unwrap();
        let paths = lectern_certificate_paths(&fixture.lectern_certificate_session_dir);
        let mut status: PackageLecternCertificateStatus = load_json(&paths.status_path).unwrap();
        status.podium_receipt_step.as_mut().unwrap().path =
            "/tmp/foreign.podium-receipt.report.json".to_string();
    ...
    fn verify_rejects_tampered_rostrum_certificate_sha256_summary_or_algorithm() {
        {
            ...
            run_live_package_lectern_certificate_report(&fixture.run_config()).unwrap();
            let paths = lectern_certificate_paths(&fixture.lectern_certificate_session_dir);
            let mut session: PackageLecternCertificateSession =
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = lectern_certificate_paths(&fixture.lectern_certificate_session_dir);
        let mut status: PackageLecternCertificateStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageLecternCertificateResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_podium_receipt_report_content() {
        let fixture = fake_command_fixture(
            "podium_receipt_tampered_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        run_live_package_lectern_certificate_report(&fixture.run_config()).unwrap();
        let paths = lectern_certificate_paths(&fixture.lectern_certificate_session_dir);
        let mut report: serde_json::Value = load_json(&paths.podium_receipt_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested podium-receipt reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_podium_receipt_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "podium_receipt_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut status: PackageLecternCertificateStatus = load_json(&paths.status_path).unwrap();
        status.podium_receipt_step.as_mut().unwrap().generated_at = forged_generated_at;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    fn verify_rejects_tampered_chain_ledger_registry_filing_archive_summit_closure_finality_or_consummation_identity_fields(
    ) {
        {
            ...
            run_live_package_lectern_certificate_report(&fixture.run_config()).unwrap();
            let paths = lectern_certificate_paths(&fixture.lectern_certificate_session_dir);
            let mut status: PackageLecternCertificateStatus =
    ...
    struct FakeCommandFixture {
        ...
        input_podium_receipt_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageLecternCertificate,
                podium_receipt_session_dir: self.input_podium_receipt_session_dir.clone(),
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageLecternCertificate,
                podium_receipt_session_dir: self.input_podium_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let input_podium_receipt_session_dir = fixture_dir.join("podium-receipt-session");
        ...
        for path in [
            &input_podium_receipt_session_dir,
        ...
        .unwrap();
        persist_json(
            &input_podium_receipt_session_dir.join(
                "tiny_live_activation_package_podium_receipt.rostrum_certificate.report.json",
            ),
            &json!({
        ...
        let verify_path = fixture_dir.join("podium-receipt.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_rostrum_certificate_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_podium_receipt"),
            &verify_path,
            &rostrum_certificate_session_dir,
            &decision_packet_session_dir,
            &input_podium_receipt_session_dir,
    ...
    fn bind_rostrum_certificate_report_to_fixture(
        ...
        object.insert(
            ...
            json!("podium-receipt verify statement"),
        );
    }
    ...
    fn rostrum_certificate_verify_report(
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_podium_receipt",
            "verdict": "tiny_live_package_podium_receipt_verify_ok",
            "reason": "podium-receipt chain is coherent",
            "result": result,
    ...
    fn stored_rostrum_certificate_status_fixture(
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("podium-receipt status statement"),
        );
        Value::Object(object)
    }
}
