...
use copybot_app::tiny_live_activation::package_rostrum_certificate_podium_receipt;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_podium_receipt --rostrum-certificate-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-podium-receipt | --render-live-package-podium-receipt | --run-live-package-podium-receipt | --verify-live-package-podium-receipt)";
const PODIUM_RECEIPT_SESSION_VERSION: &str = "1";
const PODIUM_RECEIPT_STATUS_VERSION: &str = "1";
const PODIUM_RECEIPT_ALGORITHM: &str = "sha256";
const PODIUM_RECEIPT_SESSION_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    rostrum_certificate_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackagePodiumReceipt,
    RenderLivePackagePodiumReceipt,
    RunLivePackagePodiumReceipt,
    VerifyLivePackagePodiumReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackagePodiumReceiptVerdict {
    TinyLivePackagePodiumReceiptPlanReady,
    TinyLivePackagePodiumReceiptRendered,
    TinyLivePackagePodiumReceiptRefusedNowByStage3,
    TinyLivePackagePodiumReceiptRefusedNowByPreActivationGate,
    TinyLivePackagePodiumReceiptRefusedNowByInvalidOrDriftedContract,
    TinyLivePackagePodiumReceiptReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackagePodiumReceiptVerifyOk,
    TinyLivePackagePodiumReceiptVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackagePodiumReceiptResult {
    RefusedNowByStage3,
...
struct PackagePodiumReceiptStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackagePodiumReceiptSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackagePodiumReceiptStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackagePodiumReceiptResult,
    reason: String,
    ...
    rostrum_certificate_step: Option<PackagePodiumReceiptStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackagePodiumReceiptPaths {
...
struct PackagePodiumReceiptReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackagePodiumReceiptVerdict,
    reason: String,
    rostrum_certificate_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut rostrum_certificate_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--rostrum-certificate-session-dir" => {
                rostrum_certificate_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--rostrum-certificate-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--plan-live-package-podium-receipt" => {
                set_mode(&mut mode, Mode::PlanLivePackagePodiumReceipt, arg.as_str())?
            }
            "--render-live-package-podium-receipt" => set_mode(
                &mut mode,
                Mode::RenderLivePackagePodiumReceipt,
                arg.as_str(),
            )?,
            "--run-live-package-podium-receipt" => {
                set_mode(&mut mode, Mode::RunLivePackagePodiumReceipt, arg.as_str())?
            }
            "--verify-live-package-podium-receipt" => set_mode(
                &mut mode,
                Mode::VerifyLivePackagePodiumReceipt,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackagePodiumReceipt) && output_path.is_none() {
        bail!("missing --output for --render-live-package-podium-receipt");
    }
    if matches!(
        mode,
        Mode::RunLivePackagePodiumReceipt | Mode::VerifyLivePackagePodiumReceipt
    ) && session_dir.is_none()
    {
    ...
    let rostrum_certificate_session_dir = rostrum_certificate_session_dir
        .ok_or_else(|| anyhow!("missing --rostrum-certificate-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackagePodiumReceipt => plan_live_package_podium_receipt_report(&config)?,
        Mode::RenderLivePackagePodiumReceipt => render_live_package_podium_receipt_report(&config)?,
        Mode::RunLivePackagePodiumReceipt => run_live_package_podium_receipt_report(&config)?,
        Mode::VerifyLivePackagePodiumReceipt => verify_live_package_podium_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_live_package_podium_receipt_report(config: &Config) -> Result<PackagePodiumReceiptReport> {
    let (basal_receipt, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_rostrum_certificate_podium_receipt::verify_live_package_rostrum_certificate_for_podium_receipt(
                    &config.rostrum_certificate_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification =
                            classify_verified_rostrum_certificate(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_rostrum_certificate_podium_receipt::load_live_package_rostrum_certificate_contract_for_podium_receipt(
                            &config.rostrum_certificate_session_dir,
                        )?;
                        let classification =
                            classify_rostrum_certificate_contract(&raw_contract, false);
    ...
    let podium_receipt_sha256 = compute_podium_receipt_sha256(
        &result,
        ...
        &basal_receipt.pedestal_certificate_summary,
        &basal_receipt.pedestal_certificate_sha256,
        &basal_receipt.pedestal_certificate_algorithm,
        &basal_receipt.dais_receipt_summary,
        &basal_receipt.dais_receipt_sha256,
        &basal_receipt.dais_receipt_algorithm,
        &basal_receipt.rostrum_certificate_summary,
        &basal_receipt.rostrum_certificate_sha256,
        &basal_receipt.rostrum_certificate_algorithm,
    )?;

    Ok(PackagePodiumReceiptReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_podium_receipt".to_string(),
        verdict:
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptPlanReady,
        reason: format!(
            "rostrum-certificate-native podium receipt is explicit for verified rostrum-certificate session {}; this command stays archival and read-only while freezing one final blazon identity over the fully culminated chain",
            config.rostrum_certificate_session_dir.display()
        ),
        rostrum_certificate_session_dir: config.rostrum_certificate_session_dir.display().to_string(),
        dais_receipt_session_dir: Some(
            basal_receipt.dais_receipt_session_dir.display().to_string(),
        ),
        pedestal_certificate_session_dir: Some(
            basal_receipt.pedestal_certificate_session_dir.display().to_string(),
        ),
        plinth_receipt_session_dir: Some(
            basal_receipt.plinth_receipt_session_dir.display().to_string(),
        ),
        ...
        podium_receipt_session_path: None,
        podium_receipt_status_path: None,
        archived_rostrum_certificate_report_path: None,
        result: Some(result),
        ...
        verify_rostrum_certificate_command_summary: Some(verify_rostrum_certificate_command_summary(
            &config.rostrum_certificate_session_dir,
        )),
        ...
        pedestal_certificate_summary: Some(basal_receipt.pedestal_certificate_summary.clone()),
        pedestal_certificate_sha256: Some(basal_receipt.pedestal_certificate_sha256.clone()),
        pedestal_certificate_algorithm: Some(basal_receipt.pedestal_certificate_algorithm.clone()),
        dais_receipt_summary: Some(basal_receipt.dais_receipt_summary.clone()),
        dais_receipt_sha256: Some(basal_receipt.dais_receipt_sha256.clone()),
        dais_receipt_algorithm: Some(basal_receipt.dais_receipt_algorithm.clone()),
        rostrum_certificate_summary: Some(basal_receipt.rostrum_certificate_summary.clone()),
        rostrum_certificate_sha256: Some(basal_receipt.rostrum_certificate_sha256.clone()),
        rostrum_certificate_algorithm: Some(basal_receipt.rostrum_certificate_algorithm.clone()),
        podium_receipt_summary: Some(podium_receipt_summary(
            classification.0,
            &basal_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        podium_receipt_sha256: Some(podium_receipt_sha256),
        podium_receipt_algorithm: Some(PODIUM_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: PODIUM_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_podium_receipt_report(
    config: &Config,
) -> Result<PackagePodiumReceiptReport> {
    let plan = plan_live_package_podium_receipt_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_podium_receipt --rostrum-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-podium-receipt --json\ncopybot_tiny_live_activation_package_podium_receipt --rostrum-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-podium-receipt --json\n",
        shell_escape_path(&config.rostrum_certificate_session_dir),
    ...
    Ok(PackagePodiumReceiptReport {
        generated_at: Utc::now(),
        mode: "render_live_package_podium_receipt".to_string(),
        verdict:
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptRendered,
        reason: format!(
            "rendered podium-receipt run/verify script to {}; the primary input remains the immutable rostrum-certificate session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: PODIUM_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_podium_receipt_report(config: &Config) -> Result<PackagePodiumReceiptReport> {
    let session_dir = config
        .session_dir
        .as_ref()
        .ok_or_else(|| anyhow!("missing --session-dir for run mode"))?;
    let verified_rostrum_certificate = match package_rostrum_certificate_podium_receipt::verify_live_package_rostrum_certificate_for_podium_receipt(
        &config.rostrum_certificate_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify rostrum-certificate session under {}: {error}",
                    config.rostrum_certificate_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_rostrum_certificate.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "podium-receipt session dir {} overlaps the managed live target surface derived from verified rostrum-certificate install_root {}",
            session_dir.display(),
            verified_rostrum_certificate.contract.install_root.display()
        )
    })?;
    ensure_clean_podium_receipt_session_dir(session_dir)?;
    let paths = podium_receipt_paths(session_dir);
    let classification = classify_verified_rostrum_certificate(&verified_rostrum_certificate);
    let result = classification.0;
    let podium_receipt_sha256 = compute_podium_receipt_sha256(
        &serialize_enum(&result),
        verified_rostrum_certificate
            .contract
        ...
        &verified_rostrum_certificate.contract.ledger_seal_sha256,
        &verified_rostrum_certificate.contract.ledger_seal_algorithm,
        &verified_rostrum_certificate.contract.registry_entry_sha256,
        &verified_rostrum_certificate
            .contract
        ...
        &verified_rostrum_certificate.contract.archive_receipt_sha256,
        &verified_rostrum_certificate
            .contract
        ...
        &verified_rostrum_certificate.contract.basal_receipt_summary,
        &verified_rostrum_certificate.contract.basal_receipt_sha256,
        &verified_rostrum_certificate
            .contract
        ...
        &verified_rostrum_certificate.contract.plinth_receipt_summary,
        &verified_rostrum_certificate.contract.plinth_receipt_sha256,
        &verified_rostrum_certificate
            .contract
        ...
        &verified_rostrum_certificate.contract.dais_receipt_summary,
        &verified_rostrum_certificate.contract.dais_receipt_sha256,
        &verified_rostrum_certificate.contract.dais_receipt_algorithm,
        &verified_rostrum_certificate
            .contract
    ...
    let session = planned_session(
        config,
        session_dir,
        &verified_rostrum_certificate.contract,
        result,
    ...
    persist_json(
        ...
        &verified_rostrum_certificate.report_json,
    )?;
    let rostrum_certificate_step = Some(step_artifact(
        &paths.rostrum_certificate_report_path,
        "verify_live_package_rostrum_certificate",
        &verified_rostrum_certificate.verdict,
        &verified_rostrum_certificate.reason,
        verified_rostrum_certificate.generated_at,
    ));
    let status = PackagePodiumReceiptStatus {
        status_version: PODIUM_RECEIPT_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        rostrum_certificate_session_dir: config
            .rostrum_certificate_session_dir
            .display()
            .to_string(),
        dais_receipt_session_dir: verified_rostrum_certificate
            .contract
            .dais_receipt_session_dir
            .display()
            .to_string(),
        pedestal_certificate_session_dir: verified_rostrum_certificate
            .contract
            .pedestal_certificate_session_dir
            .display()
            .to_string(),
        plinth_receipt_session_dir: verified_rostrum_certificate
            .contract
            .plinth_receipt_session_dir
            .display()
            .to_string(),
        substructure_certificate_session_dir: verified_rostrum_certificate
            .contract
            .substructure_certificate_session_dir
            .display()
            .to_string(),
        basal_receipt_session_dir: verified_rostrum_certificate
            .contract
            .basal_receipt_session_dir
            .display()
            .to_string(),
        bedrock_certificate_session_dir: verified_rostrum_certificate
            .contract
            .bedrock_certificate_session_dir
            .display()
            .to_string(),
        foundation_receipt_session_dir: verified_rostrum_certificate
            .contract
            .foundation_receipt_session_dir
            .display()
            .to_string(),
        ...
        handoff_bundle_result: verified_rostrum_certificate
            .contract
            .handoff_bundle_result
            .clone(),
        decision_packet_result: verified_rostrum_certificate
            .contract
            .decision_packet_result
            .clone(),
        execute_frozen_result: verified_rostrum_certificate
            .contract
            .execute_frozen_result
            .clone(),
        current_pre_activation_gate_verdict: verified_rostrum_certificate
            .contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_rostrum_certificate
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        rostrum_certificate_step,
        provenance_certificate_summary: verified_rostrum_certificate
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_rostrum_certificate
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_rostrum_certificate
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_rostrum_certificate
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_rostrum_certificate
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_rostrum_certificate
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_rostrum_certificate
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_rostrum_certificate
            .contract
            .notarization_receipt_summary
            .clone(),
        ledger_seal_summary: verified_rostrum_certificate
            .contract
            .ledger_seal_summary
            .clone(),
        ledger_seal_sha256: verified_rostrum_certificate
            .contract
            .ledger_seal_sha256
            .clone(),
        ledger_seal_algorithm: verified_rostrum_certificate
            .contract
            .ledger_seal_algorithm
            .clone(),
        registry_entry_summary: verified_rostrum_certificate
            .contract
            .registry_entry_summary
            .clone(),
        registry_entry_sha256: verified_rostrum_certificate
            .contract
            .registry_entry_sha256
            .clone(),
        registry_entry_algorithm: verified_rostrum_certificate
            .contract
            .registry_entry_algorithm
            .clone(),
        filing_certificate_summary: verified_rostrum_certificate
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_rostrum_certificate
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_rostrum_certificate
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_rostrum_certificate
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_rostrum_certificate
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_rostrum_certificate
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_rostrum_certificate
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_rostrum_certificate
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_rostrum_certificate
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_rostrum_certificate
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_rostrum_certificate
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_rostrum_certificate
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_rostrum_certificate
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_rostrum_certificate
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_rostrum_certificate
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_rostrum_certificate
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_rostrum_certificate
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_rostrum_certificate
            .contract
            .completion_certificate_algorithm
            .clone(),
        summit_certificate_summary: verified_rostrum_certificate
            .contract
            .summit_certificate_summary
            .clone(),
        summit_certificate_sha256: verified_rostrum_certificate
            .contract
            .summit_certificate_sha256
            .clone(),
        summit_certificate_algorithm: verified_rostrum_certificate
            .contract
            .summit_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_rostrum_certificate
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_rostrum_certificate
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_rostrum_certificate
            .contract
            .culmination_receipt_algorithm
            .clone(),
        pinnacle_receipt_summary: verified_rostrum_certificate
            .contract
            .pinnacle_receipt_summary
            .clone(),
        pinnacle_receipt_sha256: verified_rostrum_certificate
            .contract
            .pinnacle_receipt_sha256
            .clone(),
        pinnacle_receipt_algorithm: verified_rostrum_certificate
            .contract
            .pinnacle_receipt_algorithm
            .clone(),
        capstone_certificate_summary: verified_rostrum_certificate
            .contract
            .capstone_certificate_summary
            .clone(),
        capstone_certificate_sha256: verified_rostrum_certificate
            .contract
            .capstone_certificate_sha256
            .clone(),
        capstone_certificate_algorithm: verified_rostrum_certificate
            .contract
            .capstone_certificate_algorithm
            .clone(),
        keystone_receipt_summary: verified_rostrum_certificate
            .contract
            .keystone_receipt_summary
            .clone(),
        keystone_receipt_sha256: verified_rostrum_certificate
            .contract
            .keystone_receipt_sha256
            .clone(),
        keystone_receipt_algorithm: verified_rostrum_certificate
            .contract
            .keystone_receipt_algorithm
            .clone(),
        cornerstone_certificate_summary: verified_rostrum_certificate
            .contract
            .cornerstone_certificate_summary
            .clone(),
        cornerstone_certificate_sha256: verified_rostrum_certificate
            .contract
            .cornerstone_certificate_sha256
            .clone(),
        cornerstone_certificate_algorithm: verified_rostrum_certificate
            .contract
            .cornerstone_certificate_algorithm
            .clone(),
        foundation_receipt_summary: verified_rostrum_certificate
            .contract
            .foundation_receipt_summary
            .clone(),
        foundation_receipt_sha256: verified_rostrum_certificate
            .contract
            .foundation_receipt_sha256
            .clone(),
        foundation_receipt_algorithm: verified_rostrum_certificate
            .contract
            .foundation_receipt_algorithm
            .clone(),
        bedrock_certificate_summary: verified_rostrum_certificate
            .contract
            .bedrock_certificate_summary
            .clone(),
        bedrock_certificate_sha256: verified_rostrum_certificate
            .contract
            .bedrock_certificate_sha256
            .clone(),
        bedrock_certificate_algorithm: verified_rostrum_certificate
            .contract
            .bedrock_certificate_algorithm
            .clone(),
        basal_receipt_summary: verified_rostrum_certificate
            .contract
            .basal_receipt_summary
            .clone(),
        basal_receipt_sha256: verified_rostrum_certificate
            .contract
            .basal_receipt_sha256
            .clone(),
        basal_receipt_algorithm: verified_rostrum_certificate
            .contract
            .basal_receipt_algorithm
            .clone(),
        substructure_certificate_summary: verified_rostrum_certificate
            .contract
            .substructure_certificate_summary
            .clone(),
        substructure_certificate_sha256: verified_rostrum_certificate
            .contract
            .substructure_certificate_sha256
            .clone(),
        substructure_certificate_algorithm: verified_rostrum_certificate
            .contract
            .substructure_certificate_algorithm
            .clone(),
        plinth_receipt_summary: verified_rostrum_certificate
            .contract
            .plinth_receipt_summary
            .clone(),
        plinth_receipt_sha256: verified_rostrum_certificate
            .contract
            .plinth_receipt_sha256
            .clone(),
        plinth_receipt_algorithm: verified_rostrum_certificate
            .contract
            .plinth_receipt_algorithm
            .clone(),
        pedestal_certificate_summary: verified_rostrum_certificate
            .contract
            .pedestal_certificate_summary
            .clone(),
        pedestal_certificate_sha256: verified_rostrum_certificate
            .contract
            .pedestal_certificate_sha256
            .clone(),
        pedestal_certificate_algorithm: verified_rostrum_certificate
            .contract
            .pedestal_certificate_algorithm
            .clone(),
        dais_receipt_summary: verified_rostrum_certificate
            .contract
            .dais_receipt_summary
            .clone(),
        dais_receipt_sha256: verified_rostrum_certificate
            .contract
            .dais_receipt_sha256
            .clone(),
        dais_receipt_algorithm: verified_rostrum_certificate
            .contract
            .dais_receipt_algorithm
            .clone(),
        rostrum_certificate_summary: verified_rostrum_certificate
            .contract
            .rostrum_certificate_summary
            .clone(),
        rostrum_certificate_sha256: verified_rostrum_certificate
            .contract
            .rostrum_certificate_sha256
            .clone(),
        rostrum_certificate_algorithm: verified_rostrum_certificate
            .contract
            .rostrum_certificate_algorithm
            .clone(),
        podium_receipt_summary: podium_receipt_summary(
            result,
            &verified_rostrum_certificate
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        podium_receipt_sha256: podium_receipt_sha256.clone(),
        podium_receipt_algorithm: PODIUM_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: PODIUM_RECEIPT_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_podium_receipt_report(
    config: &Config,
) -> Result<PackagePodiumReceiptReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = podium_receipt_paths(session_dir);
    let session: PackagePodiumReceiptSession = load_json(&paths.session_path)?;
    let status: PackagePodiumReceiptStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.rostrum_certificate_session_dir,
        &config.rostrum_certificate_session_dir.display().to_string(),
        "rostrum-certificate rostrum_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "rostrum-certificate session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "rostrum-certificate status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.rostrum_certificate_session_dir,
        &config.rostrum_certificate_session_dir.display().to_string(),
        "rostrum-certificate status rostrum_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_rostrum_certificate
            ...
            .display()
            .to_string(),
        "rostrum-certificate substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_rostrum_certificate
            ...
            .display()
            .to_string(),
        "rostrum-certificate status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_rostrum_certificate
            ...
            .display()
            .to_string(),
        "rostrum-certificate bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_rostrum_certificate
            ...
            .display()
            .to_string(),
        "rostrum-certificate status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_rostrum_certificate
            ...
            .display()
            .to_string(),
        "rostrum-certificate foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_rostrum_certificate
            ...
            .display()
            .to_string(),
        "rostrum-certificate status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_substructure_certificate_report_view: NestedSubstructureCertificateReportView =
        serde_json::from_value(stored_basal_receipt_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested rostrum-certificate report {}",
                paths.rostrum_certificate_report_path.display()
            )
        })?;
    let verified_substructure_certificate_report_view: NestedSubstructureCertificateReportView =
        serde_json::from_value(verified_rostrum_certificate.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested rostrum-certificate report for {}",
                    config.rostrum_certificate_session_dir.display()
                )
            },
        )?;
        ...
        != verified_substructure_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "podium-receipt archived rostrum-certificate report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_substructure_certificate_report_view.generated_at,
            verified_substructure_certificate_report_view.generated_at
        ));
    }
    let expected_rostrum_certificate_step = step_artifact(
        &paths.rostrum_certificate_report_path,
        &verified_substructure_certificate_report_view.mode,
        &verified_substructure_certificate_report_view.verdict,
        &verified_substructure_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_rostrum_certificate_step,
        "rostrum-certificate rostrum_certificate_step",
        &mut mismatches,
    );
    if verified_rostrum_certificate.verdict != "tiny_live_package_rostrum_certificate_verify_ok" {
        mismatches.push(format!(
            "podium-receipt nested rostrum-certificate verification is non-green: {}",
            verified_rostrum_certificate.reason
        ));
    }

    let classification = classify_verified_rostrum_certificate(&verified_rostrum_certificate);
    let expected_podium_receipt_sha256 = compute_podium_receipt_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_rostrum_certificate.contract,
        classification.0,
        &expected_podium_receipt_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "podium-receipt session contract does not match freshly verified rostrum-certificate truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_rostrum_certificate.contract,
        classification,
        expected_rostrum_certificate_step.clone(),
        &expected_podium_receipt_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "podium-receipt status contract does not match freshly verified rostrum-certificate truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_rostrum_certificate.contract,
        TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptVerifyOk,
        &expected_status,
    ))
}

fn verify_rostrum_certificate_command_summary(rostrum_certificate_session_dir: &Path) -> String {
    format!(
...
fn classify_rostrum_certificate_contract(
    contract: &package_rostrum_certificate_podium_receipt::LivePackageRostrumCertificateContractView,
    assume_verified: bool,
) -> (
    LivePackagePodiumReceiptResult,
    TinyLivePackagePodiumReceiptVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackagePodiumReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptRefusedNowByInvalidOrDriftedContract,
            "the rostrum-certificate chain is not verified, so this podium receipt remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackagePodiumReceiptResult::RefusedNowByStage3,
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptRefusedNowByStage3,
            "the immutable rostrum certificate remains refused now by Stage 3, so this podium receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackagePodiumReceiptResult::RefusedNowByPreActivationGate,
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptRefusedNowByPreActivationGate,
            "the immutable rostrum certificate remains refused now by the pre-activation gate, so this podium receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackagePodiumReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable rostrum certificate is coherent and this podium receipt is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackagePodiumReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptRefusedNowByInvalidOrDriftedContract,
            "the verified rostrum-certificate chain is invalid, drifted, or non-runnable, so this podium receipt remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_rostrum_certificate(
    verified: &package_rostrum_certificate_podium_receipt::VerifiedLivePackageRostrumCertificatePodiumReceiptStep,
) -> (
    LivePackagePodiumReceiptResult,
    TinyLivePackagePodiumReceiptVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_rostrum_certificate_verify_ok" {
        return (
            LivePackagePodiumReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptRefusedNowByInvalidOrDriftedContract,
            "the immutable rostrum certificate did not verify cleanly, so this podium receipt remains refused"
                .to_string(),
        );
    }
    classify_rostrum_certificate_contract(&verified.contract, true)
}

fn podium_receipt_summary(
    result: LivePackagePodiumReceiptResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackagePodiumReceiptResult::RefusedNowByStage3 => {
            "Podium receipt outcome: the immutable culminated chain remains refused now by Stage 3; archive this final blazon seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackagePodiumReceiptResult::RefusedNowByPreActivationGate => {
            "Podium receipt outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final blazon seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackagePodiumReceiptResult::RefusedNowByInvalidOrDriftedContract => {
            "Podium receipt outcome: the immutable culminated chain is invalid or drifted; archive this final blazon seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackagePodiumReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Podium receipt outcome: this immutable blazon seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn podium_receipt_paths(session_dir: &Path) -> PackagePodiumReceiptPaths {
    PackagePodiumReceiptPaths {
...
fn ensure_clean_podium_receipt_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing podium-receipt session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating podium-receipt session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackagePodiumReceiptSession {
    PackagePodiumReceiptSession {
        session_version: PODIUM_RECEIPT_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        rostrum_certificate_session_dir: config
            .rostrum_certificate_session_dir
            .display()
        ...
        session_dir: session_dir.display().to_string(),
        verify_rostrum_certificate_command_summary: verify_rostrum_certificate_command_summary(
            &config.rostrum_certificate_session_dir,
        ),
        ...
        podium_receipt_sha256: podium_receipt_sha256.to_string(),
        podium_receipt_algorithm: PODIUM_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: PODIUM_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_session_from_verified(
    ...
    PackagePodiumReceiptSession {
        planned_at,
        ..planned_session(config, session_dir, contract, result, podium_receipt_sha256)
    }
}
...
fn expected_status_from_verified(
    ...
    rostrum_certificate_step: PackagePodiumReceiptStepArtifact,
    podium_receipt_sha256: &str,
) -> PackagePodiumReceiptStatus {
    PackagePodiumReceiptStatus {
        status_version: PODIUM_RECEIPT_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        rostrum_certificate_step: Some(rostrum_certificate_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackagePodiumReceiptPaths,
    contract: &package_rostrum_certificate_podium_receipt::LivePackageRostrumCertificateContractView,
    verdict: TinyLivePackagePodiumReceiptVerdict,
    status: &PackagePodiumReceiptStatus,
) -> PackagePodiumReceiptReport {
    PackagePodiumReceiptReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptVerifyOk
        {
            "verify_live_package_podium_receipt".to_string()
        } else {
            "run_live_package_podium_receipt".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        podium_receipt_session_path: Some(paths.session_path.display().to_string()),
        podium_receipt_status_path: Some(paths.status_path.display().to_string()),
        archived_rostrum_certificate_report_path: Some(
        ...
        rostrum_certificate_step: status.rostrum_certificate_step.clone(),
        verify_rostrum_certificate_command_summary: Some(
            verify_rostrum_certificate_command_summary(&config.rostrum_certificate_session_dir),
        ),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
            contract
                .reviewed_frozen_live_cutover_controller_command_summary
        ...
        explicit_statement: if verdict
            == TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptVerifyOk
        {
            PODIUM_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackagePodiumReceiptReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackagePodiumReceipt => {
                "plan_live_package_podium_receipt".to_string()
            }
            Mode::RenderLivePackagePodiumReceipt => {
                "render_live_package_podium_receipt".to_string()
            }
            Mode::RunLivePackagePodiumReceipt => {
                "run_live_package_podium_receipt".to_string()
            }
            Mode::VerifyLivePackagePodiumReceipt => {
                "verify_live_package_podium_receipt".to_string()
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable podium receipt refused before persisting because the rostrum-certificate artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "rostrum-certificate-native podium-receipt verification failed".to_string()
    });
    PackagePodiumReceiptReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_podium_receipt".to_string(),
        verdict: TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptVerifyInvalid,
        reason,
        rostrum_certificate_session_dir: config
            .rostrum_certificate_session_dir
            .display()
        ...
        explicit_statement: PODIUM_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackagePodiumReceiptStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from rostrum-certificate status"
        ));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackagePodiumReceiptStepArtifact {
    PackagePodiumReceiptStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackagePodiumReceiptStepArtifact>,
    expected: &PackagePodiumReceiptStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_podium_receipt_sha256(
    result: &str,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents)
        .with_context(|| format!("failed writing podium-receipt script to {}", path.display()))?;
    #[cfg(unix)]
    {
...
fn render_report_lines(report: &PackagePodiumReceiptReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "rostrum_certificate_session_dir={}",
            report.rostrum_certificate_session_dir
        ),
    ];
    ...
    if let Some(value) = &report.podium_receipt_sha256 {
        lines.push(format!("podium_receipt_sha256={value}"));
    }
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_podium_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify = verify_live_package_podium_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptVerifyOk
        );
    ...
    #[test]
    fn drifted_rostrum_certificate_contract_is_refused() {
        let fixture = fake_command_fixture(
            "rostrum_certificate_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_rostrum_certificate",
                "verdict": "tiny_live_package_rostrum_certificate_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        let fixture = fake_command_fixture(
            "podium_receipt_managed_overlap",
        ...
        .unwrap();
        let overlap_session_dir = managed_paths.runtime_dir.join("podium-receipt-session");
        ...
        let error = run_live_package_podium_receipt_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = podium_receipt_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_rostrum_certificate_step_path() {
        let fixture = fake_command_fixture(
            "rostrum_certificate_tampered_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_podium_receipt_report(&fixture.run_config()).unwrap();
        let paths = podium_receipt_paths(&fixture.podium_receipt_session_dir);
        let mut status: PackagePodiumReceiptStatus = load_json(&paths.status_path).unwrap();
        status.rostrum_certificate_step.as_mut().unwrap().path =
            "/tmp/foreign.rostrum-certificate.report.json".to_string();
        persist_json(&paths.status_path, &status).unwrap();

        let verify = verify_live_package_podium_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptVerifyInvalid
        );
    }
    ...
    fn verify_rejects_tampered_rostrum_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_rostrum_certificate_sha256",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            run_live_package_podium_receipt_report(&fixture.run_config()).unwrap();
            let paths = podium_receipt_paths(&fixture.podium_receipt_session_dir);
            let mut session: PackagePodiumReceiptSession = load_json(&paths.session_path).unwrap();
            ...
            let verify =
                verify_live_package_podium_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackagePodiumReceiptVerdict::TinyLivePackagePodiumReceiptVerifyInvalid
            );
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_rostrum_certificate_summary",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_rostrum_certificate_algorithm",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = podium_receipt_paths(&fixture.podium_receipt_session_dir);
        let mut status: PackagePodiumReceiptStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackagePodiumReceiptResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_rostrum_certificate_report_content() {
        let fixture = fake_command_fixture(
            "rostrum_certificate_tampered_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut report: serde_json::Value =
            load_json(&paths.rostrum_certificate_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested rostrum-certificate reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_rostrum_certificate_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "rostrum_certificate_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_capstone_keystone_cornerstone_or_foundation_identity_fields()
    {
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_bedrock_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_capstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_keystone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_cornerstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_foundation_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_basal_receipt_sha256() {
        let fixture = fake_command_fixture(
            "podium_receipt_tampered_basal_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_sha256() {
        let fixture = fake_command_fixture(
            "podium_receipt_tampered_substructure_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_substructure_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_substructure_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_plinth_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_plinth_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_plinth_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_plinth_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pedestal_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_pedestal_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_pedestal_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_pedestal_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_bedrock_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_bedrock_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_podium_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_podium_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = podium_receipt_paths(&fixture.podium_receipt_session_dir);
            let mut status: PackagePodiumReceiptStatus = load_json(&paths.status_path).unwrap();
            status.podium_receipt_sha256 = "tampered-podium".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_podium_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_podium_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = podium_receipt_paths(&fixture.podium_receipt_session_dir);
            let mut status: PackagePodiumReceiptStatus = load_json(&paths.status_path).unwrap();
            status.podium_receipt_summary = "tampered podium summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_podium_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "podium_receipt_tampered_podium_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = podium_receipt_paths(&fixture.podium_receipt_session_dir);
            let mut status: PackagePodiumReceiptStatus = load_json(&paths.status_path).unwrap();
            status.podium_receipt_algorithm = "tampered-podium-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_podium_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
    ...
    struct FakeCommandFixture {
        ...
        input_rostrum_certificate_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackagePodiumReceipt,
                rostrum_certificate_session_dir: self.input_rostrum_certificate_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.podium_receipt_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackagePodiumReceipt,
                rostrum_certificate_session_dir: self.input_rostrum_certificate_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let input_rostrum_certificate_session_dir = fixture_dir.join("rostrum-certificate-session");
        ...
        for path in [
            &input_rostrum_certificate_session_dir,
            &dais_receipt_session_dir,
            &pedestal_certificate_session_dir,
            &plinth_receipt_session_dir,
            &substructure_certificate_session_dir,
        ...
        let verify_path = fixture_dir.join("rostrum-certificate.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_rostrum_certificate_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_rostrum_certificate"),
            &verify_path,
            &dais_receipt_session_dir,
            &decision_packet_session_dir,
            &input_rostrum_certificate_session_dir,
    ...
    fn bind_rostrum_certificate_report_to_fixture(
        ...
        object.insert(
            "verify_rostrum_certificate_command_summary".to_string(),
            json!(format!(
                "verify immutable rostrum-certificate session under {} before freezing the final podium receipt",
                rostrum_certificate_session_dir.display()
            )),
        ...
        object.insert(
            ...
            json!("rostrum-certificate verify statement"),
        );
    }
    ...
    fn rostrum_certificate_verify_report(
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_rostrum_certificate",
            "verdict": "tiny_live_package_rostrum_certificate_verify_ok",
            "reason": "rostrum-certificate chain is coherent",
            "result": result,
    ...
    fn stored_rostrum_certificate_session_fixture(
        ...
        object.insert(
            "dais_receipt_session_dir".to_string(),
            json!(rostrum_certificate_session_dir
                .parent()
                .unwrap()
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("rostrum-certificate statement"),
        );
        Value::Object(object)
    }
    ...
    fn stored_rostrum_certificate_status_fixture(
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("rostrum-certificate status statement"),
        );
        Value::Object(object)
    }
}
