...
use copybot_app::tiny_live_activation::package_lectern_certificate_pulpit_receipt;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_pulpit_receipt --lectern-certificate-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-pulpit-receipt | --render-live-package-pulpit-receipt | --run-live-package-pulpit-receipt | --verify-live-package-pulpit-receipt)";
const PULPIT_RECEIPT_SESSION_VERSION: &str = "1";
const PULPIT_RECEIPT_STATUS_VERSION: &str = "1";
const PULPIT_RECEIPT_ALGORITHM: &str = "sha256";
const PULPIT_RECEIPT_SESSION_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    lectern_certificate_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackagePulpitReceipt,
    RenderLivePackagePulpitReceipt,
    RunLivePackagePulpitReceipt,
    VerifyLivePackagePulpitReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackagePulpitReceiptVerdict {
    TinyLivePackagePulpitReceiptPlanReady,
    TinyLivePackagePulpitReceiptRendered,
    TinyLivePackagePulpitReceiptRefusedNowByStage3,
    TinyLivePackagePulpitReceiptRefusedNowByPreActivationGate,
    TinyLivePackagePulpitReceiptRefusedNowByInvalidOrDriftedContract,
    TinyLivePackagePulpitReceiptReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackagePulpitReceiptVerifyOk,
    TinyLivePackagePulpitReceiptVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackagePulpitReceiptResult {
    RefusedNowByStage3,
...
struct PackagePulpitReceiptStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackagePulpitReceiptSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackagePulpitReceiptStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackagePulpitReceiptResult,
    reason: String,
    ...
    lectern_certificate_step: Option<PackagePulpitReceiptStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackagePulpitReceiptPaths {
...
struct PackagePulpitReceiptReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackagePulpitReceiptVerdict,
    reason: String,
    lectern_certificate_session_dir: String,
    podium_receipt_session_dir: Option<String>,
...
struct NestedLecternCertificateReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut lectern_certificate_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--lectern-certificate-session-dir" => {
                lectern_certificate_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--lectern-certificate-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--plan-live-package-pulpit-receipt" => {
                set_mode(&mut mode, Mode::PlanLivePackagePulpitReceipt, arg.as_str())?
            }
            "--render-live-package-pulpit-receipt" => set_mode(
                &mut mode,
                Mode::RenderLivePackagePulpitReceipt,
                arg.as_str(),
            )?,
            "--run-live-package-pulpit-receipt" => {
                set_mode(&mut mode, Mode::RunLivePackagePulpitReceipt, arg.as_str())?
            }
            "--verify-live-package-pulpit-receipt" => set_mode(
                &mut mode,
                Mode::VerifyLivePackagePulpitReceipt,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackagePulpitReceipt) && output_path.is_none() {
        bail!("missing --output for --render-live-package-pulpit-receipt");
    }
    if matches!(
        mode,
        Mode::RunLivePackagePulpitReceipt | Mode::VerifyLivePackagePulpitReceipt
    ) && session_dir.is_none()
    {
    ...
    let lectern_certificate_session_dir = lectern_certificate_session_dir
        .ok_or_else(|| anyhow!("missing --lectern-certificate-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackagePulpitReceipt => plan_live_package_pulpit_receipt_report(&config)?,
        Mode::RenderLivePackagePulpitReceipt => render_live_package_pulpit_receipt_report(&config)?,
        Mode::RunLivePackagePulpitReceipt => run_live_package_pulpit_receipt_report(&config)?,
        Mode::VerifyLivePackagePulpitReceipt => verify_live_package_pulpit_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_live_package_pulpit_receipt_report(config: &Config) -> Result<PackagePulpitReceiptReport> {
    let (podium_receipt_contract, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_lectern_certificate_pulpit_receipt::verify_live_package_lectern_certificate_for_pulpit_receipt(
                    &config.lectern_certificate_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification =
                            classify_verified_lectern_certificate(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_lectern_certificate_pulpit_receipt::load_live_package_lectern_certificate_contract_for_pulpit_receipt(
                            &config.lectern_certificate_session_dir,
                        )?;
                        let classification =
                            classify_lectern_certificate_contract(&raw_contract, false);
    ...
    let pulpit_receipt_sha256 = compute_pulpit_receipt_sha256(
        &result,
        podium_receipt_contract.handoff_bundle_result.as_deref(),
        podium_receipt_contract.decision_packet_result.as_deref(),
        podium_receipt_contract.execute_frozen_result.as_deref(),
        podium_receipt_contract
            .current_pre_activation_gate_verdict
            .as_deref(),
        podium_receipt_contract
            .current_pre_activation_gate_reason
            .as_deref(),
        &podium_receipt_contract.reviewed_frozen_live_cutover_controller_command_summary,
        &podium_receipt_contract.chain_fingerprint_sha256,
        &podium_receipt_contract.chain_fingerprint_algorithm,
        &podium_receipt_contract.release_capsule_digest_manifest_sha256,
        podium_receipt_contract.release_capsule_digest_manifest_entry_count,
        &podium_receipt_contract.release_capsule_digest_algorithm,
        &podium_receipt_contract.ledger_seal_sha256,
        &podium_receipt_contract.ledger_seal_algorithm,
        &podium_receipt_contract.registry_entry_sha256,
        &podium_receipt_contract.registry_entry_algorithm,
        &podium_receipt_contract.filing_certificate_sha256,
        &podium_receipt_contract.filing_certificate_algorithm,
        &podium_receipt_contract.archive_receipt_sha256,
        &podium_receipt_contract.archive_receipt_algorithm,
        &podium_receipt_contract.closure_certificate_summary,
        &podium_receipt_contract.closure_certificate_sha256,
        &podium_receipt_contract.closure_certificate_algorithm,
        &podium_receipt_contract.finality_receipt_sha256,
        &podium_receipt_contract.finality_receipt_algorithm,
        &podium_receipt_contract.consummation_record_sha256,
        &podium_receipt_contract.consummation_record_algorithm,
        &podium_receipt_contract.completion_certificate_summary,
        &podium_receipt_contract.completion_certificate_sha256,
        &podium_receipt_contract.completion_certificate_algorithm,
        &podium_receipt_contract.summit_certificate_summary,
        &podium_receipt_contract.summit_certificate_sha256,
        &podium_receipt_contract.summit_certificate_algorithm,
        &podium_receipt_contract.culmination_receipt_summary,
        &podium_receipt_contract.culmination_receipt_sha256,
        &podium_receipt_contract.culmination_receipt_algorithm,
        &podium_receipt_contract.pinnacle_receipt_summary,
        &podium_receipt_contract.pinnacle_receipt_sha256,
        &podium_receipt_contract.pinnacle_receipt_algorithm,
        &podium_receipt_contract.capstone_certificate_summary,
        &podium_receipt_contract.capstone_certificate_sha256,
        &podium_receipt_contract.capstone_certificate_algorithm,
        &podium_receipt_contract.keystone_receipt_summary,
        &podium_receipt_contract.keystone_receipt_sha256,
        &podium_receipt_contract.keystone_receipt_algorithm,
        &podium_receipt_contract.cornerstone_certificate_summary,
        &podium_receipt_contract.cornerstone_certificate_sha256,
        &podium_receipt_contract.cornerstone_certificate_algorithm,
        &podium_receipt_contract.foundation_receipt_summary,
        &podium_receipt_contract.foundation_receipt_sha256,
        &podium_receipt_contract.foundation_receipt_algorithm,
        &podium_receipt_contract.bedrock_certificate_summary,
        &podium_receipt_contract.bedrock_certificate_sha256,
        &podium_receipt_contract.bedrock_certificate_algorithm,
        &podium_receipt_contract.basal_receipt_summary,
        &podium_receipt_contract.basal_receipt_sha256,
        &podium_receipt_contract.basal_receipt_algorithm,
        &podium_receipt_contract.substructure_certificate_summary,
        &podium_receipt_contract.substructure_certificate_sha256,
        &podium_receipt_contract.substructure_certificate_algorithm,
        &podium_receipt_contract.plinth_receipt_summary,
        &podium_receipt_contract.plinth_receipt_sha256,
        &podium_receipt_contract.plinth_receipt_algorithm,
        &podium_receipt_contract.pedestal_certificate_summary,
        &podium_receipt_contract.pedestal_certificate_sha256,
        &podium_receipt_contract.pedestal_certificate_algorithm,
        &podium_receipt_contract.dais_receipt_summary,
        &podium_receipt_contract.dais_receipt_sha256,
        &podium_receipt_contract.dais_receipt_algorithm,
        &podium_receipt_contract.rostrum_certificate_summary,
        &podium_receipt_contract.rostrum_certificate_sha256,
        &podium_receipt_contract.rostrum_certificate_algorithm,
        &podium_receipt_contract.podium_receipt_summary,
        &podium_receipt_contract.podium_receipt_sha256,
        &podium_receipt_contract.podium_receipt_algorithm,
        &podium_receipt_contract.lectern_certificate_summary,
        &podium_receipt_contract.lectern_certificate_sha256,
        &podium_receipt_contract.lectern_certificate_algorithm,
    )?;

    Ok(PackagePulpitReceiptReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_pulpit_receipt".to_string(),
        verdict:
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptPlanReady,
        reason: format!(
            "lectern-certificate-native pulpit receipt is explicit for verified lectern-certificate session {}; this command stays archival and read-only while freezing one final crest identity over the fully culminated chain",
            config.lectern_certificate_session_dir.display()
        ),
        lectern_certificate_session_dir: config.lectern_certificate_session_dir.display().to_string(),
        podium_receipt_session_dir: Some(
            podium_receipt_contract.podium_receipt_session_dir.display().to_string(),
        ),
        dais_receipt_session_dir: Some(
            podium_receipt_contract.dais_receipt_session_dir.display().to_string(),
        ),
        pedestal_certificate_session_dir: Some(
            podium_receipt_contract.pedestal_certificate_session_dir.display().to_string(),
        ),
        plinth_receipt_session_dir: Some(
            podium_receipt_contract.plinth_receipt_session_dir.display().to_string(),
        ),
        ...
        registry_entry_session_dir: Some(
            podium_receipt_contract.registry_entry_session_dir.display().to_string(),
        ),
        ...
        review_receipt_session_dir: Some(
            podium_receipt_contract.review_receipt_session_dir.display().to_string(),
        ),
        handoff_bundle_session_dir: Some(
            podium_receipt_contract.handoff_bundle_session_dir.display().to_string(),
        ),
        ...
        turn_green_session_dir: Some(
            podium_receipt_contract.turn_green_session_dir.display().to_string(),
        ),
        launch_packet_session_dir: Some(
            podium_receipt_contract.launch_packet_session_dir.display().to_string(),
        ),
        package_dir: Some(podium_receipt_contract.package_dir.display().to_string()),
        install_root: Some(podium_receipt_contract.install_root.display().to_string()),
        target_service_name: Some(podium_receipt_contract.target_service_name.clone()),
        backend_command: Some(podium_receipt_contract.backend_command.clone()),
        wrapper_timeout_ms: Some(podium_receipt_contract.wrapper_timeout_ms),
        service_status_max_staleness_ms: Some(podium_receipt_contract.service_status_max_staleness_ms),
        session_dir: Some(session_dir.display().to_string()),
        pulpit_receipt_session_path: None,
        pulpit_receipt_status_path: None,
        archived_lectern_certificate_report_path: None,
        result: Some(result),
        handoff_bundle_result: podium_receipt_contract.handoff_bundle_result.clone(),
        decision_packet_result: podium_receipt_contract.decision_packet_result.clone(),
        execute_frozen_result: podium_receipt_contract.execute_frozen_result.clone(),
        current_pre_activation_gate_verdict: podium_receipt_contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: podium_receipt_contract
            .current_pre_activation_gate_reason
            .clone(),
        lectern_certificate_step: None,
        verify_lectern_certificate_command_summary: Some(verify_lectern_certificate_command_summary(
            &config.lectern_certificate_session_dir,
        )),
        ...
        provenance_certificate_summary: Some(podium_receipt_contract.provenance_certificate_summary.clone()),
        chain_fingerprint_summary: Some(podium_receipt_contract.chain_fingerprint_summary.clone()),
        chain_fingerprint_sha256: Some(podium_receipt_contract.chain_fingerprint_sha256.clone()),
        chain_fingerprint_algorithm: Some(podium_receipt_contract.chain_fingerprint_algorithm.clone()),
        release_capsule_digest_manifest_sha256: Some(
            podium_receipt_contract.release_capsule_digest_manifest_sha256.clone(),
        ),
        release_capsule_digest_manifest_entry_count: Some(
            podium_receipt_contract.release_capsule_digest_manifest_entry_count,
        ),
        release_capsule_digest_algorithm: Some(
            podium_receipt_contract.release_capsule_digest_algorithm.clone(),
        ),
        notarization_receipt_summary: Some(podium_receipt_contract.notarization_receipt_summary.clone()),
        ledger_seal_summary: Some(podium_receipt_contract.ledger_seal_summary.clone()),
        ledger_seal_sha256: Some(podium_receipt_contract.ledger_seal_sha256.clone()),
        ledger_seal_algorithm: Some(podium_receipt_contract.ledger_seal_algorithm.clone()),
        registry_entry_summary: Some(podium_receipt_contract.registry_entry_summary.clone()),
        registry_entry_sha256: Some(podium_receipt_contract.registry_entry_sha256.clone()),
        registry_entry_algorithm: Some(podium_receipt_contract.registry_entry_algorithm.clone()),
        filing_certificate_summary: Some(podium_receipt_contract.filing_certificate_summary.clone()),
        filing_certificate_sha256: Some(podium_receipt_contract.filing_certificate_sha256.clone()),
        filing_certificate_algorithm: Some(podium_receipt_contract.filing_certificate_algorithm.clone()),
        archive_receipt_summary: Some(podium_receipt_contract.archive_receipt_summary.clone()),
        archive_receipt_sha256: Some(podium_receipt_contract.archive_receipt_sha256.clone()),
        archive_receipt_algorithm: Some(podium_receipt_contract.archive_receipt_algorithm.clone()),
        closure_certificate_summary: Some(podium_receipt_contract.closure_certificate_summary.clone()),
        closure_certificate_sha256: Some(podium_receipt_contract.closure_certificate_sha256.clone()),
        closure_certificate_algorithm: Some(
            podium_receipt_contract.closure_certificate_algorithm.clone(),
        ),
        finality_receipt_summary: Some(podium_receipt_contract.finality_receipt_summary.clone()),
        finality_receipt_sha256: Some(podium_receipt_contract.finality_receipt_sha256.clone()),
        finality_receipt_algorithm: Some(podium_receipt_contract.finality_receipt_algorithm.clone()),
        consummation_record_summary: Some(podium_receipt_contract.consummation_record_summary.clone()),
        consummation_record_sha256: Some(podium_receipt_contract.consummation_record_sha256.clone()),
        consummation_record_algorithm: Some(podium_receipt_contract.consummation_record_algorithm.clone()),
        completion_certificate_summary: Some(
            podium_receipt_contract.completion_certificate_summary.clone(),
        ),
        completion_certificate_sha256: Some(
            podium_receipt_contract.completion_certificate_sha256.clone(),
        ),
        completion_certificate_algorithm: Some(
            podium_receipt_contract.completion_certificate_algorithm.clone(),
        ),
        summit_certificate_summary: Some(podium_receipt_contract.summit_certificate_summary.clone()),
        summit_certificate_sha256: Some(podium_receipt_contract.summit_certificate_sha256.clone()),
        summit_certificate_algorithm: Some(
            podium_receipt_contract.summit_certificate_algorithm.clone(),
        ),
        culmination_receipt_summary: Some(
            podium_receipt_contract.culmination_receipt_summary.clone(),
        ),
        culmination_receipt_sha256: Some(
            podium_receipt_contract.culmination_receipt_sha256.clone(),
        ),
        culmination_receipt_algorithm: Some(
            podium_receipt_contract.culmination_receipt_algorithm.clone(),
        ),
        pinnacle_receipt_summary: Some(podium_receipt_contract.pinnacle_receipt_summary.clone()),
        pinnacle_receipt_sha256: Some(podium_receipt_contract.pinnacle_receipt_sha256.clone()),
        pinnacle_receipt_algorithm: Some(
            podium_receipt_contract.pinnacle_receipt_algorithm.clone(),
        ),
        capstone_certificate_summary: Some(
            podium_receipt_contract.capstone_certificate_summary.clone(),
        ),
        capstone_certificate_sha256: Some(
            podium_receipt_contract.capstone_certificate_sha256.clone(),
        ),
        capstone_certificate_algorithm: Some(
            podium_receipt_contract.capstone_certificate_algorithm.clone(),
        ),
        keystone_receipt_summary: Some(podium_receipt_contract.keystone_receipt_summary.clone()),
        keystone_receipt_sha256: Some(podium_receipt_contract.keystone_receipt_sha256.clone()),
        keystone_receipt_algorithm: Some(
            podium_receipt_contract.keystone_receipt_algorithm.clone(),
        ),
        cornerstone_certificate_summary: Some(podium_receipt_contract.cornerstone_certificate_summary.clone()),
        cornerstone_certificate_sha256: Some(podium_receipt_contract.cornerstone_certificate_sha256.clone()),
        cornerstone_certificate_algorithm: Some(podium_receipt_contract.cornerstone_certificate_algorithm.clone()),
        foundation_receipt_summary: Some(podium_receipt_contract.foundation_receipt_summary.clone()),
        foundation_receipt_sha256: Some(podium_receipt_contract.foundation_receipt_sha256.clone()),
        foundation_receipt_algorithm: Some(podium_receipt_contract.foundation_receipt_algorithm.clone()),
        bedrock_certificate_summary: Some(podium_receipt_contract.bedrock_certificate_summary.clone()),
        bedrock_certificate_sha256: Some(podium_receipt_contract.bedrock_certificate_sha256.clone()),
        bedrock_certificate_algorithm: Some(
            podium_receipt_contract.bedrock_certificate_algorithm.clone(),
        ),
        basal_receipt_summary: Some(podium_receipt_contract.basal_receipt_summary.clone()),
        basal_receipt_sha256: Some(podium_receipt_contract.basal_receipt_sha256.clone()),
        basal_receipt_algorithm: Some(
            podium_receipt_contract.basal_receipt_algorithm.clone(),
        ),
        substructure_certificate_summary: Some(
            podium_receipt_contract.substructure_certificate_summary.clone(),
        ),
        substructure_certificate_sha256: Some(
            podium_receipt_contract.substructure_certificate_sha256.clone(),
        ),
        substructure_certificate_algorithm: Some(
            podium_receipt_contract.substructure_certificate_algorithm.clone(),
        ),
        plinth_receipt_summary: Some(podium_receipt_contract.plinth_receipt_summary.clone()),
        plinth_receipt_sha256: Some(podium_receipt_contract.plinth_receipt_sha256.clone()),
        plinth_receipt_algorithm: Some(podium_receipt_contract.plinth_receipt_algorithm.clone()),
        pedestal_certificate_summary: Some(podium_receipt_contract.pedestal_certificate_summary.clone()),
        pedestal_certificate_sha256: Some(podium_receipt_contract.pedestal_certificate_sha256.clone()),
        pedestal_certificate_algorithm: Some(podium_receipt_contract.pedestal_certificate_algorithm.clone()),
        dais_receipt_summary: Some(podium_receipt_contract.dais_receipt_summary.clone()),
        dais_receipt_sha256: Some(podium_receipt_contract.dais_receipt_sha256.clone()),
        dais_receipt_algorithm: Some(podium_receipt_contract.dais_receipt_algorithm.clone()),
        rostrum_certificate_summary: Some(podium_receipt_contract.rostrum_certificate_summary.clone()),
        rostrum_certificate_sha256: Some(podium_receipt_contract.rostrum_certificate_sha256.clone()),
        rostrum_certificate_algorithm: Some(podium_receipt_contract.rostrum_certificate_algorithm.clone()),
        podium_receipt_summary: Some(podium_receipt_contract.podium_receipt_summary.clone()),
        podium_receipt_sha256: Some(podium_receipt_contract.podium_receipt_sha256.clone()),
        podium_receipt_algorithm: Some(podium_receipt_contract.podium_receipt_algorithm.clone()),
        lectern_certificate_summary: Some(
            podium_receipt_contract.lectern_certificate_summary.clone(),
        ),
        lectern_certificate_sha256: Some(
            podium_receipt_contract.lectern_certificate_sha256.clone(),
        ),
        lectern_certificate_algorithm: Some(
            podium_receipt_contract.lectern_certificate_algorithm.clone(),
        ),
        pulpit_receipt_summary: Some(pulpit_receipt_summary(
            classification.0,
            &podium_receipt_contract.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        pulpit_receipt_sha256: Some(pulpit_receipt_sha256),
        pulpit_receipt_algorithm: Some(PULPIT_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: PULPIT_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_pulpit_receipt_report(
    config: &Config,
) -> Result<PackagePulpitReceiptReport> {
    let plan = plan_live_package_pulpit_receipt_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_pulpit_receipt --lectern-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-pulpit-receipt --json\ncopybot_tiny_live_activation_package_pulpit_receipt --lectern-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-pulpit-receipt --json\n",
        shell_escape_path(&config.lectern_certificate_session_dir),
    ...
    Ok(PackagePulpitReceiptReport {
        generated_at: Utc::now(),
        mode: "render_live_package_pulpit_receipt".to_string(),
        verdict:
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptRendered,
        reason: format!(
            "rendered pulpit-receipt run/verify script to {}; the primary input remains the immutable lectern-certificate session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: PULPIT_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_pulpit_receipt_report(config: &Config) -> Result<PackagePulpitReceiptReport> {
    let session_dir = config
        .session_dir
        .as_ref()
        .ok_or_else(|| anyhow!("missing --session-dir for run mode"))?;
    let verified_podium_receipt = match package_lectern_certificate_pulpit_receipt::verify_live_package_lectern_certificate_for_pulpit_receipt(
        &config.lectern_certificate_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify lectern-certificate session under {}: {error}",
                    config.lectern_certificate_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_podium_receipt.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "pulpit-receipt session dir {} overlaps the managed live target surface derived from verified lectern-certificate install_root {}",
            session_dir.display(),
            verified_podium_receipt.contract.install_root.display()
        )
    })?;
    ensure_clean_pulpit_receipt_session_dir(session_dir)?;
    let paths = pulpit_receipt_paths(session_dir);
    let classification = classify_verified_lectern_certificate(&verified_podium_receipt);
    let result = classification.0;
    let pulpit_receipt_sha256 = compute_pulpit_receipt_sha256(
        &serialize_enum(&result),
        verified_podium_receipt
            .contract
        ...
        &verified_podium_receipt
            .contract
            .reviewed_frozen_live_cutover_controller_command_summary,
        &verified_podium_receipt.contract.chain_fingerprint_sha256,
        &verified_podium_receipt.contract.chain_fingerprint_algorithm,
        &verified_podium_receipt
            .contract
        ...
        &verified_podium_receipt.contract.ledger_seal_sha256,
        &verified_podium_receipt.contract.ledger_seal_algorithm,
        &verified_podium_receipt.contract.registry_entry_sha256,
        &verified_podium_receipt.contract.registry_entry_algorithm,
        &verified_podium_receipt.contract.filing_certificate_sha256,
        &verified_podium_receipt
            .contract
            .filing_certificate_algorithm,
        &verified_podium_receipt.contract.archive_receipt_sha256,
        &verified_podium_receipt.contract.archive_receipt_algorithm,
        &verified_podium_receipt.contract.closure_certificate_summary,
        &verified_podium_receipt.contract.closure_certificate_sha256,
        &verified_podium_receipt
            .contract
            .closure_certificate_algorithm,
        &verified_podium_receipt.contract.finality_receipt_sha256,
        &verified_podium_receipt.contract.finality_receipt_algorithm,
        &verified_podium_receipt.contract.consummation_record_sha256,
        &verified_podium_receipt
            .contract
        ...
        &verified_podium_receipt.contract.summit_certificate_summary,
        &verified_podium_receipt.contract.summit_certificate_sha256,
        &verified_podium_receipt
            .contract
            .summit_certificate_algorithm,
        &verified_podium_receipt.contract.culmination_receipt_summary,
        &verified_podium_receipt.contract.culmination_receipt_sha256,
        &verified_podium_receipt
            .contract
            .culmination_receipt_algorithm,
        &verified_podium_receipt.contract.pinnacle_receipt_summary,
        &verified_podium_receipt.contract.pinnacle_receipt_sha256,
        &verified_podium_receipt.contract.pinnacle_receipt_algorithm,
        &verified_podium_receipt
            .contract
            .capstone_certificate_summary,
        &verified_podium_receipt.contract.capstone_certificate_sha256,
        &verified_podium_receipt
            .contract
            .capstone_certificate_algorithm,
        &verified_podium_receipt.contract.keystone_receipt_summary,
        &verified_podium_receipt.contract.keystone_receipt_sha256,
        &verified_podium_receipt.contract.keystone_receipt_algorithm,
        &verified_podium_receipt
            .contract
        ...
        &verified_podium_receipt.contract.foundation_receipt_summary,
        &verified_podium_receipt.contract.foundation_receipt_sha256,
        &verified_podium_receipt
            .contract
            .foundation_receipt_algorithm,
        &verified_podium_receipt.contract.bedrock_certificate_summary,
        &verified_podium_receipt.contract.bedrock_certificate_sha256,
        &verified_podium_receipt
            .contract
            .bedrock_certificate_algorithm,
        &verified_podium_receipt.contract.basal_receipt_summary,
        &verified_podium_receipt.contract.basal_receipt_sha256,
        &verified_podium_receipt.contract.basal_receipt_algorithm,
        &verified_podium_receipt
            .contract
        ...
        &verified_podium_receipt.contract.plinth_receipt_summary,
        &verified_podium_receipt.contract.plinth_receipt_sha256,
        &verified_podium_receipt.contract.plinth_receipt_algorithm,
        &verified_podium_receipt
            .contract
            .pedestal_certificate_summary,
        &verified_podium_receipt.contract.pedestal_certificate_sha256,
        &verified_podium_receipt
            .contract
            .pedestal_certificate_algorithm,
        &verified_podium_receipt.contract.dais_receipt_summary,
        &verified_podium_receipt.contract.dais_receipt_sha256,
        &verified_podium_receipt.contract.dais_receipt_algorithm,
        &verified_podium_receipt.contract.rostrum_certificate_summary,
        &verified_podium_receipt.contract.rostrum_certificate_sha256,
        &verified_podium_receipt
            .contract
            .rostrum_certificate_algorithm,
        &verified_podium_receipt.contract.podium_receipt_summary,
        &verified_podium_receipt.contract.podium_receipt_sha256,
        &verified_podium_receipt.contract.podium_receipt_algorithm,
        &verified_podium_receipt.contract.lectern_certificate_summary,
        &verified_podium_receipt.contract.lectern_certificate_sha256,
        &verified_podium_receipt
            .contract
    ...
    let session = planned_session(
        config,
        session_dir,
        &verified_podium_receipt.contract,
        result,
    ...
    persist_json(
        ...
        &verified_podium_receipt.report_json,
    )?;
    let lectern_certificate_step = Some(step_artifact(
        &paths.lectern_certificate_report_path,
        "verify_live_package_lectern_certificate",
        &verified_podium_receipt.verdict,
        &verified_podium_receipt.reason,
        verified_podium_receipt.generated_at,
    ));
    let status = PackagePulpitReceiptStatus {
        status_version: PULPIT_RECEIPT_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        lectern_certificate_session_dir: config
            .lectern_certificate_session_dir
            .display()
            .to_string(),
        podium_receipt_session_dir: verified_podium_receipt
            .contract
            .podium_receipt_session_dir
            .display()
            .to_string(),
        dais_receipt_session_dir: verified_podium_receipt
            .contract
            .dais_receipt_session_dir
            .display()
            .to_string(),
        pedestal_certificate_session_dir: verified_podium_receipt
            .contract
            .pedestal_certificate_session_dir
            .display()
            .to_string(),
        plinth_receipt_session_dir: verified_podium_receipt
            .contract
            .plinth_receipt_session_dir
            .display()
            .to_string(),
        substructure_certificate_session_dir: verified_podium_receipt
            .contract
            .substructure_certificate_session_dir
            .display()
            .to_string(),
        basal_receipt_session_dir: verified_podium_receipt
            .contract
            .basal_receipt_session_dir
            .display()
            .to_string(),
        bedrock_certificate_session_dir: verified_podium_receipt
            .contract
            .bedrock_certificate_session_dir
            .display()
            .to_string(),
        foundation_receipt_session_dir: verified_podium_receipt
            .contract
            .foundation_receipt_session_dir
            .display()
            .to_string(),
        ...
        handoff_bundle_result: verified_podium_receipt
            .contract
            .handoff_bundle_result
            .clone(),
        decision_packet_result: verified_podium_receipt
            .contract
            .decision_packet_result
            .clone(),
        execute_frozen_result: verified_podium_receipt
            .contract
            .execute_frozen_result
            .clone(),
        current_pre_activation_gate_verdict: verified_podium_receipt
            .contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_podium_receipt
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        lectern_certificate_step,
        provenance_certificate_summary: verified_podium_receipt
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_podium_receipt
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_podium_receipt
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_podium_receipt
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_podium_receipt
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_podium_receipt
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_podium_receipt
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_podium_receipt
            .contract
            .notarization_receipt_summary
            .clone(),
        ledger_seal_summary: verified_podium_receipt.contract.ledger_seal_summary.clone(),
        ledger_seal_sha256: verified_podium_receipt.contract.ledger_seal_sha256.clone(),
        ledger_seal_algorithm: verified_podium_receipt
            .contract
            .ledger_seal_algorithm
            .clone(),
        registry_entry_summary: verified_podium_receipt
            .contract
            .registry_entry_summary
            .clone(),
        registry_entry_sha256: verified_podium_receipt
            .contract
            .registry_entry_sha256
            .clone(),
        registry_entry_algorithm: verified_podium_receipt
            .contract
            .registry_entry_algorithm
            .clone(),
        filing_certificate_summary: verified_podium_receipt
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_podium_receipt
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_podium_receipt
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_podium_receipt
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_podium_receipt
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_podium_receipt
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_podium_receipt
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_podium_receipt
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_podium_receipt
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_podium_receipt
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_podium_receipt
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_podium_receipt
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_podium_receipt
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_podium_receipt
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_podium_receipt
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_podium_receipt
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_podium_receipt
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_podium_receipt
            .contract
            .completion_certificate_algorithm
            .clone(),
        summit_certificate_summary: verified_podium_receipt
            .contract
            .summit_certificate_summary
            .clone(),
        summit_certificate_sha256: verified_podium_receipt
            .contract
            .summit_certificate_sha256
            .clone(),
        summit_certificate_algorithm: verified_podium_receipt
            .contract
            .summit_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_podium_receipt
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_podium_receipt
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_podium_receipt
            .contract
            .culmination_receipt_algorithm
            .clone(),
        pinnacle_receipt_summary: verified_podium_receipt
            .contract
            .pinnacle_receipt_summary
            .clone(),
        pinnacle_receipt_sha256: verified_podium_receipt
            .contract
            .pinnacle_receipt_sha256
            .clone(),
        pinnacle_receipt_algorithm: verified_podium_receipt
            .contract
            .pinnacle_receipt_algorithm
            .clone(),
        capstone_certificate_summary: verified_podium_receipt
            .contract
            .capstone_certificate_summary
            .clone(),
        capstone_certificate_sha256: verified_podium_receipt
            .contract
            .capstone_certificate_sha256
            .clone(),
        capstone_certificate_algorithm: verified_podium_receipt
            .contract
            .capstone_certificate_algorithm
            .clone(),
        keystone_receipt_summary: verified_podium_receipt
            .contract
            .keystone_receipt_summary
            .clone(),
        keystone_receipt_sha256: verified_podium_receipt
            .contract
            .keystone_receipt_sha256
            .clone(),
        keystone_receipt_algorithm: verified_podium_receipt
            .contract
            .keystone_receipt_algorithm
            .clone(),
        cornerstone_certificate_summary: verified_podium_receipt
            .contract
            .cornerstone_certificate_summary
            .clone(),
        cornerstone_certificate_sha256: verified_podium_receipt
            .contract
            .cornerstone_certificate_sha256
            .clone(),
        cornerstone_certificate_algorithm: verified_podium_receipt
            .contract
            .cornerstone_certificate_algorithm
            .clone(),
        foundation_receipt_summary: verified_podium_receipt
            .contract
            .foundation_receipt_summary
            .clone(),
        foundation_receipt_sha256: verified_podium_receipt
            .contract
            .foundation_receipt_sha256
            .clone(),
        foundation_receipt_algorithm: verified_podium_receipt
            .contract
            .foundation_receipt_algorithm
            .clone(),
        bedrock_certificate_summary: verified_podium_receipt
            .contract
            .bedrock_certificate_summary
            .clone(),
        bedrock_certificate_sha256: verified_podium_receipt
            .contract
            .bedrock_certificate_sha256
            .clone(),
        bedrock_certificate_algorithm: verified_podium_receipt
            .contract
            .bedrock_certificate_algorithm
            .clone(),
        basal_receipt_summary: verified_podium_receipt
            .contract
            .basal_receipt_summary
            .clone(),
        basal_receipt_sha256: verified_podium_receipt
            .contract
            .basal_receipt_sha256
            .clone(),
        basal_receipt_algorithm: verified_podium_receipt
            .contract
            .basal_receipt_algorithm
            .clone(),
        substructure_certificate_summary: verified_podium_receipt
            .contract
            .substructure_certificate_summary
            .clone(),
        substructure_certificate_sha256: verified_podium_receipt
            .contract
            .substructure_certificate_sha256
            .clone(),
        substructure_certificate_algorithm: verified_podium_receipt
            .contract
            .substructure_certificate_algorithm
            .clone(),
        plinth_receipt_summary: verified_podium_receipt
            .contract
            .plinth_receipt_summary
            .clone(),
        plinth_receipt_sha256: verified_podium_receipt
            .contract
            .plinth_receipt_sha256
            .clone(),
        plinth_receipt_algorithm: verified_podium_receipt
            .contract
            .plinth_receipt_algorithm
            .clone(),
        pedestal_certificate_summary: verified_podium_receipt
            .contract
            .pedestal_certificate_summary
            .clone(),
        pedestal_certificate_sha256: verified_podium_receipt
            .contract
            .pedestal_certificate_sha256
            .clone(),
        pedestal_certificate_algorithm: verified_podium_receipt
            .contract
            .pedestal_certificate_algorithm
            .clone(),
        dais_receipt_summary: verified_podium_receipt
            .contract
            .dais_receipt_summary
            .clone(),
        dais_receipt_sha256: verified_podium_receipt.contract.dais_receipt_sha256.clone(),
        dais_receipt_algorithm: verified_podium_receipt
            .contract
            .dais_receipt_algorithm
            .clone(),
        rostrum_certificate_summary: verified_podium_receipt
            .contract
            .rostrum_certificate_summary
            .clone(),
        rostrum_certificate_sha256: verified_podium_receipt
            .contract
            .rostrum_certificate_sha256
            .clone(),
        rostrum_certificate_algorithm: verified_podium_receipt
            .contract
            .rostrum_certificate_algorithm
            .clone(),
        podium_receipt_summary: verified_podium_receipt
            .contract
            .podium_receipt_summary
            .clone(),
        podium_receipt_sha256: verified_podium_receipt
            .contract
            .podium_receipt_sha256
            .clone(),
        podium_receipt_algorithm: verified_podium_receipt
            .contract
            .podium_receipt_algorithm
            .clone(),
        lectern_certificate_summary: verified_podium_receipt
            .contract
            .lectern_certificate_summary
            .clone(),
        lectern_certificate_sha256: verified_podium_receipt
            .contract
            .lectern_certificate_sha256
            .clone(),
        lectern_certificate_algorithm: verified_podium_receipt
            .contract
            .lectern_certificate_algorithm
            .clone(),
        pulpit_receipt_summary: pulpit_receipt_summary(
            result,
            &verified_podium_receipt
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        pulpit_receipt_sha256: pulpit_receipt_sha256.clone(),
        pulpit_receipt_algorithm: PULPIT_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: PULPIT_RECEIPT_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_pulpit_receipt_report(
    config: &Config,
) -> Result<PackagePulpitReceiptReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = pulpit_receipt_paths(session_dir);
    let session: PackagePulpitReceiptSession = load_json(&paths.session_path)?;
    let status: PackagePulpitReceiptStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.lectern_certificate_session_dir,
        &config.lectern_certificate_session_dir.display().to_string(),
        "pulpit-receipt lectern_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "pulpit-receipt session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "pulpit-receipt status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.lectern_certificate_session_dir,
        &config.lectern_certificate_session_dir.display().to_string(),
        "pulpit-receipt status lectern_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "pulpit-receipt substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "pulpit-receipt status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "pulpit-receipt bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "pulpit-receipt status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "pulpit-receipt foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_podium_receipt
            ...
            .display()
            .to_string(),
        "pulpit-receipt status foundation_receipt_session_dir",
        &mut mismatches,
    );

    let stored_podium_receipt_report: serde_json::Value = load_required_step_json(
        &status.lectern_certificate_step,
        &paths.lectern_certificate_report_path,
        "lectern_certificate_step",
        &mut mismatches,
    )?;
    let stored_podium_receipt_report_view: NestedLecternCertificateReportView =
        serde_json::from_value(stored_podium_receipt_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested lectern-certificate report {}",
                paths.lectern_certificate_report_path.display()
            )
        })?;
    let verified_podium_receipt_report_view: NestedLecternCertificateReportView =
        serde_json::from_value(verified_podium_receipt.report_json.clone()).with_context(|| {
            format!(
                "failed parsing freshly verified nested lectern-certificate report for {}",
                config.lectern_certificate_session_dir.display()
            )
        })?;
    compare_json_ignoring_generated_at(
        &stored_podium_receipt_report,
        &verified_podium_receipt.report_json,
        "pulpit-receipt archived lectern-certificate report json",
        &mut mismatches,
    );
    compare_string(
        &stored_podium_receipt_report_view.mode,
        &verified_podium_receipt_report_view.mode,
        "pulpit-receipt archived lectern-certificate report mode",
        &mut mismatches,
    );
    compare_string(
        &stored_podium_receipt_report_view.verdict,
        &verified_podium_receipt_report_view.verdict,
        "pulpit-receipt archived lectern-certificate report verdict",
        &mut mismatches,
    );
    compare_string(
        &stored_podium_receipt_report_view.reason,
        &verified_podium_receipt_report_view.reason,
        "pulpit-receipt archived lectern-certificate report reason",
        &mut mismatches,
    );
    if stored_podium_receipt_report_view.generated_at
        != verified_podium_receipt_report_view.generated_at
    {
        mismatches.push(format!(
            "pulpit-receipt archived lectern-certificate report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_podium_receipt_report_view.generated_at,
            verified_podium_receipt_report_view.generated_at
        ));
    }
    let expected_lectern_certificate_step = step_artifact(
        &paths.lectern_certificate_report_path,
        &verified_podium_receipt_report_view.mode,
        &verified_podium_receipt_report_view.verdict,
        &verified_podium_receipt_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_lectern_certificate_step,
        "pulpit-receipt lectern_certificate_step",
        &mut mismatches,
    );
    if verified_podium_receipt.verdict != "tiny_live_package_lectern_certificate_verify_ok" {
        mismatches.push(format!(
            "pulpit-receipt nested lectern-certificate verification is non-green: {}",
            verified_podium_receipt.reason
        ));
    }

    let classification = classify_verified_lectern_certificate(&verified_podium_receipt);
    let expected_pulpit_receipt_sha256 = compute_pulpit_receipt_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_podium_receipt.contract,
        classification.0,
        &expected_pulpit_receipt_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "pulpit-receipt session contract does not match freshly verified lectern-certificate truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_podium_receipt.contract,
        classification,
        expected_lectern_certificate_step.clone(),
        &expected_pulpit_receipt_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "pulpit-receipt status contract does not match freshly verified lectern-certificate truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_podium_receipt.contract,
        TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptVerifyOk,
        &expected_status,
    ))
}

fn verify_lectern_certificate_command_summary(lectern_certificate_session_dir: &Path) -> String {
    format!(
...
fn classify_lectern_certificate_contract(
    contract: &package_lectern_certificate_pulpit_receipt::LivePackageLecternCertificateContractView,
    assume_verified: bool,
) -> (
    LivePackagePulpitReceiptResult,
    TinyLivePackagePulpitReceiptVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackagePulpitReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptRefusedNowByInvalidOrDriftedContract,
            "the lectern-certificate chain is not verified, so this pulpit receipt remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackagePulpitReceiptResult::RefusedNowByStage3,
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptRefusedNowByStage3,
            "the immutable lectern certificate remains refused now by Stage 3, so this pulpit receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackagePulpitReceiptResult::RefusedNowByPreActivationGate,
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptRefusedNowByPreActivationGate,
            "the immutable lectern certificate remains refused now by the pre-activation gate, so this pulpit receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackagePulpitReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable lectern certificate is coherent and this pulpit receipt is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackagePulpitReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptRefusedNowByInvalidOrDriftedContract,
            "the verified lectern-certificate chain is invalid, drifted, or non-runnable, so this pulpit receipt remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_lectern_certificate(
    verified: &package_lectern_certificate_pulpit_receipt::VerifiedLivePackageLecternCertificatePulpitReceiptStep,
) -> (
    LivePackagePulpitReceiptResult,
    TinyLivePackagePulpitReceiptVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_lectern_certificate_verify_ok" {
        return (
            LivePackagePulpitReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptRefusedNowByInvalidOrDriftedContract,
            "the immutable lectern certificate did not verify cleanly, so this pulpit receipt remains refused"
                .to_string(),
        );
    }
    classify_lectern_certificate_contract(&verified.contract, true)
}

fn pulpit_receipt_summary(
    result: LivePackagePulpitReceiptResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackagePulpitReceiptResult::RefusedNowByStage3 => {
            "Pulpit receipt outcome: the immutable culminated chain remains refused now by Stage 3; archive this final crest seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackagePulpitReceiptResult::RefusedNowByPreActivationGate => {
            "Pulpit receipt outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final crest seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackagePulpitReceiptResult::RefusedNowByInvalidOrDriftedContract => {
            "Pulpit receipt outcome: the immutable culminated chain is invalid or drifted; archive this final crest seal for audit only and mint a new coherent receipt before any later execution handoff.".to_string()
        }
        LivePackagePulpitReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Pulpit receipt outcome: this immutable crest seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn pulpit_receipt_paths(session_dir: &Path) -> PackagePulpitReceiptPaths {
    PackagePulpitReceiptPaths {
        session_path: session_dir.join("tiny_live_activation_package_pulpit_receipt.session.json"),
        status_path: session_dir.join("tiny_live_activation_package_pulpit_receipt.status.json"),
        lectern_certificate_report_path: session_dir
            .join("tiny_live_activation_package_pulpit_receipt.lectern_certificate.report.json"),
    }
}

fn ensure_clean_pulpit_receipt_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing pulpit-receipt session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating pulpit-receipt session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackagePulpitReceiptSession {
    PackagePulpitReceiptSession {
        session_version: PULPIT_RECEIPT_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        lectern_certificate_session_dir: config
            .lectern_certificate_session_dir
            .display()
            .to_string(),
        podium_receipt_session_dir: contract.podium_receipt_session_dir.display().to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_lectern_certificate_command_summary: verify_lectern_certificate_command_summary(
            &config.lectern_certificate_session_dir,
        ),
        ...
        pulpit_receipt_sha256: pulpit_receipt_sha256.to_string(),
        pulpit_receipt_algorithm: PULPIT_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: PULPIT_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_session_from_verified(
    ...
    PackagePulpitReceiptSession {
        planned_at,
        ..planned_session(config, session_dir, contract, result, pulpit_receipt_sha256)
    }
}
...
fn expected_status_from_verified(
    ...
    lectern_certificate_step: PackagePulpitReceiptStepArtifact,
    pulpit_receipt_sha256: &str,
) -> PackagePulpitReceiptStatus {
    PackagePulpitReceiptStatus {
        status_version: PULPIT_RECEIPT_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        lectern_certificate_step: Some(lectern_certificate_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackagePulpitReceiptPaths,
    contract: &package_lectern_certificate_pulpit_receipt::LivePackageLecternCertificateContractView,
    verdict: TinyLivePackagePulpitReceiptVerdict,
    status: &PackagePulpitReceiptStatus,
) -> PackagePulpitReceiptReport {
    PackagePulpitReceiptReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptVerifyOk
        {
            "verify_live_package_pulpit_receipt".to_string()
        } else {
            "run_live_package_pulpit_receipt".to_string()
        },
        verdict,
        ...
        lectern_certificate_session_dir: config
            ...
            .display()
            .to_string(),
        podium_receipt_session_dir: Some(contract.podium_receipt_session_dir.display().to_string()),
        ...
        session_dir: Some(session_dir.display().to_string()),
        pulpit_receipt_session_path: Some(paths.session_path.display().to_string()),
        pulpit_receipt_status_path: Some(paths.status_path.display().to_string()),
        archived_lectern_certificate_report_path: Some(
        ...
        lectern_certificate_step: status.lectern_certificate_step.clone(),
        verify_lectern_certificate_command_summary: Some(
            verify_lectern_certificate_command_summary(&config.lectern_certificate_session_dir),
        ),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
            contract
                .reviewed_frozen_live_cutover_controller_command_summary
        ...
        explicit_statement: if verdict
            == TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptVerifyOk
        {
            PULPIT_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackagePulpitReceiptReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackagePulpitReceipt => {
                "plan_live_package_pulpit_receipt".to_string()
            }
            Mode::RenderLivePackagePulpitReceipt => {
                "render_live_package_pulpit_receipt".to_string()
            }
            Mode::RunLivePackagePulpitReceipt => {
                "run_live_package_pulpit_receipt".to_string()
            }
            Mode::VerifyLivePackagePulpitReceipt => {
                "verify_live_package_pulpit_receipt".to_string()
        ...
        lectern_certificate_session_dir: config
            ...
            .display()
            .to_string(),
        podium_receipt_session_dir: None,
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable pulpit receipt refused before persisting because the lectern-certificate artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "lectern-certificate-native pulpit-receipt verification failed".to_string()
    });
    PackagePulpitReceiptReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_pulpit_receipt".to_string(),
        verdict: TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptVerifyInvalid,
        reason,
        lectern_certificate_session_dir: config
            .lectern_certificate_session_dir
            .display()
        ...
        explicit_statement: PULPIT_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackagePulpitReceiptStepArtifact>,
    expected_path: &Path,
    label: &str,
    mismatches: &mut Vec<String>,
) -> Result<serde_json::Value> {
    let Some(step) = step else {
        mismatches.push(format!("{label} is missing from pulpit-receipt status"));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackagePulpitReceiptStepArtifact {
    PackagePulpitReceiptStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackagePulpitReceiptStepArtifact>,
    expected: &PackagePulpitReceiptStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_pulpit_receipt_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&PulpitReceiptPayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing lectern-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackagePulpitReceiptReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.lectern_certificate_sha256 {
        lines.push(format!("lectern_certificate_sha256={value}"));
    }
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_pulpit_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify = verify_live_package_pulpit_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptVerifyOk
        );
    ...
    #[test]
    fn drifted_lectern_certificate_contract_is_refused() {
        let fixture = fake_command_fixture(
            "lectern_certificate_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_lectern_certificate",
                "verdict": "tiny_live_package_lectern_certificate_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        let fixture = fake_command_fixture(
            "lectern_certificate_managed_overlap",
        ...
        .unwrap();
        let overlap_session_dir = managed_paths.runtime_dir.join("pulpit-receipt-session");
        ...
        let error = run_live_package_pulpit_receipt_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = pulpit_receipt_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_lectern_certificate_step_path() {
        let fixture = fake_command_fixture(
            "lectern_certificate_tampered_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_pulpit_receipt_report(&fixture.run_config()).unwrap();
        let paths = pulpit_receipt_paths(&fixture.lectern_certificate_session_dir);
        let mut status: PackagePulpitReceiptStatus = load_json(&paths.status_path).unwrap();
        status.lectern_certificate_step.as_mut().unwrap().path =
            "/tmp/foreign.lectern-certificate.report.json".to_string();
        persist_json(&paths.status_path, &status).unwrap();

        let verify = verify_live_package_pulpit_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptVerifyInvalid
        );
    }
    ...
    fn verify_rejects_tampered_rostrum_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_rostrum_certificate_sha256",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            run_live_package_pulpit_receipt_report(&fixture.run_config()).unwrap();
            let paths = pulpit_receipt_paths(&fixture.lectern_certificate_session_dir);
            let mut session: PackagePulpitReceiptSession = load_json(&paths.session_path).unwrap();
            ...
            let verify =
                verify_live_package_pulpit_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackagePulpitReceiptVerdict::TinyLivePackagePulpitReceiptVerifyInvalid
            );
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_rostrum_certificate_summary",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_rostrum_certificate_algorithm",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = pulpit_receipt_paths(&fixture.lectern_certificate_session_dir);
        let mut status: PackagePulpitReceiptStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackagePulpitReceiptResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_lectern_certificate_report_content() {
        let fixture = fake_command_fixture(
            "lectern_certificate_tampered_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut report: serde_json::Value =
            load_json(&paths.lectern_certificate_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested lectern-certificate reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_lectern_certificate_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "lectern_certificate_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_capstone_keystone_cornerstone_or_foundation_identity_fields()
    {
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_bedrock_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_capstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_keystone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_cornerstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_foundation_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_basal_receipt_sha256() {
        let fixture = fake_command_fixture(
            "lectern_certificate_tampered_basal_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_sha256() {
        let fixture = fake_command_fixture(
            "lectern_certificate_tampered_substructure_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_substructure_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_substructure_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_plinth_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_plinth_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_plinth_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_plinth_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pedestal_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_pedestal_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_pedestal_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_pedestal_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    #[test]
    fn verify_rejects_tampered_podium_receipt_sha256() {
        let fixture = fake_command_fixture(
            "lectern_certificate_tampered_podium_receipt_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    #[test]
    fn verify_rejects_tampered_podium_receipt_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_podium_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_podium_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_bedrock_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_bedrock_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    #[test]
    fn verify_rejects_tampered_lectern_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_lectern_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = pulpit_receipt_paths(&fixture.lectern_certificate_session_dir);
            let mut status: PackagePulpitReceiptStatus = load_json(&paths.status_path).unwrap();
            status.lectern_certificate_sha256 = "tampered-lectern".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_pulpit_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_lectern_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = pulpit_receipt_paths(&fixture.lectern_certificate_session_dir);
            let mut status: PackagePulpitReceiptStatus = load_json(&paths.status_path).unwrap();
            status.lectern_certificate_summary = "tampered lectern summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_pulpit_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "lectern_certificate_tampered_lectern_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = pulpit_receipt_paths(&fixture.lectern_certificate_session_dir);
            let mut status: PackagePulpitReceiptStatus = load_json(&paths.status_path).unwrap();
            status.lectern_certificate_algorithm = "tampered-lectern-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_pulpit_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
    ...
    fn verify_rejects_tampered_pulpit_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_pulpit_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = pulpit_receipt_paths(&fixture.lectern_certificate_session_dir);
            let mut status: PackagePulpitReceiptStatus = load_json(&paths.status_path).unwrap();
            status.pulpit_receipt_sha256 = "tampered-pulpit-sha256".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_pulpit_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_pulpit_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_pulpit_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        lectern_certificate_session_dir: PathBuf,
        input_lectern_certificate_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackagePulpitReceipt,
                lectern_certificate_session_dir: self.input_lectern_certificate_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.lectern_certificate_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackagePulpitReceipt,
                lectern_certificate_session_dir: self.input_lectern_certificate_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let lectern_certificate_session_dir = fixture_dir.join("pulpit-receipt-session");
        let input_lectern_certificate_session_dir = fixture_dir.join("lectern-certificate-session");
        let podium_receipt_session_dir = fixture_dir.join("podium-receipt-session");
        ...
        for path in [
            &input_lectern_certificate_session_dir,
            &podium_receipt_session_dir,
        ...
        persist_json(
            ...
            &stored_rostrum_certificate_session_fixture(
                &podium_receipt_session_dir,
                &rostrum_certificate_session_dir,
                &pedestal_certificate_session_dir,
                &plinth_receipt_session_dir,
        ...
        persist_json(
            ...
            &stored_rostrum_certificate_status_fixture(
                &podium_receipt_session_dir,
                &rostrum_certificate_session_dir,
                &pedestal_certificate_session_dir,
                &plinth_receipt_session_dir,
        ...
        .unwrap();
        persist_json(
            &input_lectern_certificate_session_dir.join(
                "tiny_live_activation_package_lectern_certificate.podium_receipt.report.json",
            ),
            &json!({
        ...
        bind_rostrum_certificate_report_to_fixture(
            &mut verify_report,
            &podium_receipt_session_dir,
            &rostrum_certificate_session_dir,
            &dais_receipt_session_dir,
        ...
        let verify_path = fixture_dir.join("lectern-certificate.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_rostrum_certificate_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_lectern_certificate"),
            &verify_path,
            &podium_receipt_session_dir,
            &decision_packet_session_dir,
            &input_lectern_certificate_session_dir,
    ...
    fn bind_rostrum_certificate_report_to_fixture(
        ...
        object.insert(
            "podium_receipt_session_dir".to_string(),
            json!(podium_receipt_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            "verify_podium_receipt_command_summary".to_string(),
            json!(format!(
        ...
        object.insert(
            ...
            json!("lectern-certificate verify statement"),
        );
    }

    fn rostrum_certificate_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_lectern_certificate",
            "verdict": "tiny_live_package_lectern_certificate_verify_ok",
            "reason": "lectern-certificate chain is coherent",
            "result": result,
    ...
    fn bedrock_certificate_verify_report(
        ...
        rostrum_certificate_verify_report(
            result,
            handoff_bundle_result,
            decision_packet_result,
            execute_frozen_result,
    ...
    fn write_fake_rostrum_certificate_verify_command(
        path: &Path,
        verify_path: &Path,
        expected_podium_receipt_session_dir: &Path,
        expected_decision_packet_session_dir: &Path,
        expected_lectern_certificate_session_dir: &Path,
    ) {
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --podium-receipt-session-dir {} \"*) : ;;\n  *) echo 'unexpected podium-receipt-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected lectern-certificate session dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --verify-live-package-lectern-certificate \"*) : ;;\n  *) echo 'missing lectern-certificate verify mode' >&2; exit 1;;\nesac\ncat '{}'\n",
            shell_escape_path(expected_podium_receipt_session_dir),
            shell_escape_path(expected_decision_packet_session_dir),
            shell_escape_path(expected_lectern_certificate_session_dir),
            verify_path.display(),
        );
    ...
    fn stored_rostrum_certificate_session_fixture(
        podium_receipt_session_dir: &Path,
        rostrum_certificate_session_dir: &Path,
        pedestal_certificate_session_dir: &Path,
        plinth_receipt_session_dir: &Path,
        ...
        object.insert(
            "dais_receipt_session_dir".to_string(),
            json!(podium_receipt_session_dir
                .parent()
                .unwrap()
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("lectern-certificate statement"),
        );
        Value::Object(object)
    }

    fn stored_rostrum_certificate_status_fixture(
        podium_receipt_session_dir: &Path,
        rostrum_certificate_session_dir: &Path,
        pedestal_certificate_session_dir: &Path,
        plinth_receipt_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("lectern-certificate status statement"),
        );
        Value::Object(object)
    }
}
