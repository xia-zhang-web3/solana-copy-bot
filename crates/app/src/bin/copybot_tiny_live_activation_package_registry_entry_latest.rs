...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_registry_entry_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-registry-entry | --render-latest-registry-entry-script | --run-latest-registry-entry | --verify-latest-registry-entry)";
const REGISTRY_ENTRY_LATEST_SESSION_VERSION: &str = "1";
const REGISTRY_ENTRY_LATEST_STATUS_VERSION: &str = "1";
const NOTARIZATION_RECEIPT_LATEST_BIN: &str =
    "copybot_tiny_live_activation_package_notarization_receipt_latest";
const REGISTRY_ENTRY_BIN: &str = "copybot_tiny_live_activation_package_registry_entry";
const NOTARIZATION_RECEIPT_LATEST_PLAN_READY: &str =
    "tiny_live_package_notarization_receipt_latest_plan_ready";
const NOTARIZATION_RECEIPT_LATEST_VERIFY_OK: &str =
    "tiny_live_package_notarization_receipt_latest_verify_ok";
const REGISTRY_ENTRY_VERIFY_OK: &str = "tiny_live_package_registry_entry_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-registry-entry handoff binds the latest persisted immutable package chain to the accepted registry-entry contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestRegistryEntry,
    RenderLatestRegistryEntryScript,
    RunLatestRegistryEntry,
    VerifyLatestRegistryEntry,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageRegistryEntryLatestVerdict {
    TinyLivePackageRegistryEntryLatestPlanReady,
    TinyLivePackageRegistryEntryLatestScriptRendered,
    TinyLivePackageRegistryEntryLatestRefusedByMissingLatestChain,
    TinyLivePackageRegistryEntryLatestRefusedByInvalidLatestChain,
    TinyLivePackageRegistryEntryLatestRefusedByDriftedRegistryEntryContract,
    TinyLivePackageRegistryEntryLatestRefusedNowByStage3,
    TinyLivePackageRegistryEntryLatestRefusedNowByPreActivationGate,
    TinyLivePackageRegistryEntryLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageRegistryEntryLatestVerifyOk,
    TinyLivePackageRegistryEntryLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageRegistryEntryLatestResult {
    RefusedByMissingLatestChain,
    RefusedByInvalidLatestChain,
    RefusedByDriftedRegistryEntryContract,
    RefusedNowByStage3,
...
struct PackageRegistryEntryLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageRegistryEntryLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_notarization_receipt_latest_session_dir: String,
    nested_registry_entry_session_dir: String,
    ...
    resolve_latest_notarization_receipt_command_summary: String,
    run_latest_registry_entry_command_summary: String,
    verify_latest_registry_entry_command_summary: String,
    downstream_registry_entry_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageRegistryEntryLatestStatus {
    ...
    nested_registry_entry_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackageRegistryEntryLatestResult,
    reason: String,
    latest_notarization_receipt_plan_verdict: Option<String>,
    latest_notarization_receipt_verdict: Option<String>,
    registry_entry_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_notarization_receipt_plan_step: Option<PackageRegistryEntryLatestStepArtifact>,
    latest_notarization_receipt_step: Option<PackageRegistryEntryLatestStepArtifact>,
    registry_entry_step: Option<PackageRegistryEntryLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackageRegistryEntryLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_notarization_receipt_plan_report_path: PathBuf,
    latest_notarization_receipt_report_path: PathBuf,
    registry_entry_report_path: PathBuf,
    nested_notarization_receipt_latest_session_dir: PathBuf,
    nested_registry_entry_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageRegistryEntryLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageRegistryEntryLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    latest_notarization_receipt_plan_report_path: Option<String>,
    latest_notarization_receipt_report_path: Option<String>,
    registry_entry_report_path: Option<String>,
    nested_notarization_receipt_latest_session_dir: Option<String>,
    nested_registry_entry_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_notarization_receipt_command_summary: Option<String>,
    run_latest_registry_entry_command_summary: Option<String>,
    verify_latest_registry_entry_command_summary: Option<String>,
    downstream_registry_entry_command_summary: Option<String>,
...
struct ResolvedLatestNotarizationReceipt {
    ...
    latest_notarization_receipt_plan_raw: Value,
    latest_notarization_receipt_plan: NotarizationReceiptLatestReportView,
}
...
struct LatestNotarizationReceiptResolutionFailure {
    ...
    latest_notarization_receipt_plan_raw: Option<Value>,
    latest_notarization_receipt_plan_verdict: Option<String>,
    latest_notarization_receipt_plan_reason: Option<String>,
    latest_notarization_receipt_plan_generated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone, Deserialize)]
struct NotarizationReceiptLatestReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Deserialize)]
struct RegistryEntryReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    notarization_receipt_step: Option<PackageRegistryEntryLatestStepArtifact>,
...
#[derive(Debug, Clone, Deserialize)]
struct RegistryEntrySessionView {
    notarization_receipt_session_dir: String,
...
#[derive(Debug, Clone, Deserialize)]
struct RegistryEntryStatusView {
    session_dir: String,
    notarization_receipt_session_dir: String,
    result: String,
    reason: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-registry-entry" => {
                set_mode(&mut mode, Mode::PlanLatestRegistryEntry, arg.as_str())?
            }
            "--render-latest-registry-entry-script" => set_mode(
                &mut mode,
                Mode::RenderLatestRegistryEntryScript,
                arg.as_str(),
            )?,
            "--run-latest-registry-entry" => {
                set_mode(&mut mode, Mode::RunLatestRegistryEntry, arg.as_str())?
            }
            "--verify-latest-registry-entry" => {
                set_mode(&mut mode, Mode::VerifyLatestRegistryEntry, arg.as_str())?
            }
    ...
    if matches!(
        mode,
        Mode::PlanLatestRegistryEntry
            | Mode::RenderLatestRegistryEntryScript
            | Mode::RunLatestRegistryEntry
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestRegistryEntryScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestRegistryEntry | Mode::VerifyLatestRegistryEntry
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestRegistryEntry => plan_latest_registry_entry_report(&config)?,
        Mode::RenderLatestRegistryEntryScript => {
            render_latest_registry_entry_script_report(&config)?
        }
        Mode::RunLatestRegistryEntry => run_latest_registry_entry_report(&config)?,
        Mode::VerifyLatestRegistryEntry => verify_latest_registry_entry_report(&config)?,
    };
    if config.json {
...
fn plan_latest_registry_entry_report(config: &Config) -> Result<PackageRegistryEntryLatestReport> {
    let root = config
        .root
        .as_ref()
    ...
    let suggested_session_dir = config
        .session_dir
        .clone()
        .unwrap_or_else(|| root.join("tiny_live_activation_package_registry_entry_latest.session"));
    let paths = registry_entry_latest_paths(&suggested_session_dir);

    match resolve_latest_notarization_receipt(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            "plan_latest_registry_entry",
            TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-notarization-receipt {} lineage, exact decision-packet confirmation anchor {}, and the accepted registry-entry contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_notarization_receipt_latest_session_dir.display(),
...
fn render_latest_registry_entry_script_report(
    config: &Config,
) -> Result<PackageRegistryEntryLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
        ...
        Ok(resolved) => {
            write_script(
                ...
                &format!(
                    ...
                    run_latest = run_latest_registry_entry_command_summary(
                        root,
                        &paths.nested_notarization_receipt_latest_session_dir,
                    ),
                    verify_latest = verify_latest_registry_entry_command_summary(
                        &paths.nested_notarization_receipt_latest_session_dir,
                    ),
                    run_handoff_bundle = downstream_registry_entry_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_notarization_receipt_session_dir(&paths),
                        &paths.nested_registry_entry_session_dir,
                    ),
                    verify_handoff_bundle = verify_registry_entry_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_notarization_receipt_session_dir(&paths),
                        &paths.nested_registry_entry_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                ...
                TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestScriptRendered,
                format!(
                    "rendered latest-registry-entry handoff script to {} using the accepted latest-notarization-receipt and registry-entry contracts",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_registry_entry_report(config: &Config) -> Result<PackageRegistryEntryLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-registry-entry session dir {}",
            session_dir.display()
        );
    }
    let paths = registry_entry_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-registry-entry")?;

    let resolution = resolve_latest_notarization_receipt(root, &paths);
    match resolution {
        Ok(resolved) => {
...
fn verify_latest_registry_entry_report(
    config: &Config,
) -> Result<PackageRegistryEntryLatestReport> {
    let session_dir = config
        .session_dir
    ...
    let session: PackageRegistryEntryLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_registry_entry_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-registry-entry session artifact {}: {error}",
                    paths.session_path.display()
                )],
            ))
        }
    };
    let status: PackageRegistryEntryLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_registry_entry_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-registry-entry status artifact {}: {error}",
                    paths.status_path.display()
                )],
            ))
        }
    };
    let stored_report: PackageRegistryEntryLatestReport = match load_json(&paths.report_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_registry_entry_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-registry-entry report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-registry-entry root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-registry-entry session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-registry-entry status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-registry-entry report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_notarization_receipt_latest_session_dir,
        &paths
            .nested_notarization_receipt_latest_session_dir
            .display()
            .to_string(),
        "latest-registry-entry session nested_notarization_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_notarization_receipt_latest_session_dir,
        &paths
            .nested_notarization_receipt_latest_session_dir
            .display()
            .to_string(),
        "latest-registry-entry status nested_notarization_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-registry-entry report nested_notarization_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_registry_entry_session_dir,
        &paths
            .nested_registry_entry_session_dir
            .display()
            .to_string(),
        "latest-registry-entry session nested_registry_entry_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_registry_entry_session_dir,
        &paths
            .nested_registry_entry_session_dir
            .display()
            .to_string(),
        "latest-registry-entry status nested_registry_entry_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.nested_registry_entry_session_dir.as_deref(),
        Some(
            paths
                .nested_registry_entry_session_dir
                .display()
        ...
        "latest-registry-entry report nested_registry_entry_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-registry-entry session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-registry-entry status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-registry-entry report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.registry_entry_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-registry-entry report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.registry_entry_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-registry-entry report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.registry_entry_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-registry-entry report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-registry-entry session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-registry-entry status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-registry-entry report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_notarization_receipt",
        "latest-registry-entry stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-registry-entry stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-registry-entry stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-registry-entry stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-registry-entry stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report
            .latest_notarization_receipt_plan_verdict
            .as_deref(),
        status.latest_notarization_receipt_plan_verdict.as_deref(),
        "stored report latest_notarization_receipt_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.latest_notarization_receipt_verdict.as_deref(),
        status.latest_notarization_receipt_verdict.as_deref(),
        "stored report latest_notarization_receipt_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.registry_entry_verdict.as_deref(),
        status.registry_entry_verdict.as_deref(),
        "stored report registry_entry_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: NotarizationReceiptLatestReportView = load_required_step_json(
                &status.latest_notarization_receipt_plan_step,
                &paths.latest_notarization_receipt_plan_report_path,
                "latest_notarization_receipt_plan_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_notarization_receipt_plan_step.as_ref(),
                &paths.latest_notarization_receipt_plan_report_path,
                "plan_latest_notarization_receipt",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_notarization_receipt_plan_step",
                &mut mismatches,
            );
            compare_notarization_receipt_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_notarization_receipt_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_notarization_receipt_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let stored_latest_run: NotarizationReceiptLatestReportView = load_required_step_json(
                &status.latest_notarization_receipt_step,
                &paths.latest_notarization_receipt_report_path,
                "latest_notarization_receipt_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_notarization_receipt_step.as_ref(),
                &paths.latest_notarization_receipt_report_path,
                "run_latest_notarization_receipt",
                &stored_latest_run.verdict,
                &stored_latest_run.reason,
                stored_latest_run.generated_at,
                "stored latest_notarization_receipt_step",
                &mut mismatches,
            );
            compare_notarization_receipt_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_notarization_receipt_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_notarization_receipt_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_notarization_receipt_verdict.as_deref(),
            ...
            let actual_latest_run: NotarizationReceiptLatestReportView = match load_json(
                &nested_notarization_receipt_latest_report_path(
                    &paths.nested_notarization_receipt_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed loading actual nested latest-notarization-receipt report under {}: {error}",
                        paths.nested_notarization_receipt_latest_session_dir.display()
                    ));
                    fallback_notarization_receipt_latest_copy()
                }
            };
            compare_notarization_receipt_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                NOTARIZATION_RECEIPT_LATEST_BIN,
                &latest_notarization_receipt_verify_args(
                    &paths.nested_notarization_receipt_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-notarization-receipt session under {}: {error}",
                        paths.nested_notarization_receipt_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: NotarizationReceiptLatestReportView =
                    serde_json::from_value(latest_verify_raw)
                        .context("failed parsing nested latest-notarization-receipt verify json")?;
                if latest_verify.verdict != NOTARIZATION_RECEIPT_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-notarization-receipt verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: RegistryEntryReportView = load_required_step_json(
            ...
            compare_step_artifact_against_report_metadata(
                ...
                "stored registry_entry_step",
                &mut mismatches,
            );
            compare_registry_entry_run_against_snapshot(
            ...
            compare_option_string(
                ...
                "stored status registry_entry_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.registry_entry_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: RegistryEntrySessionView = match load_json(
                &nested_registry_entry_session_path(&paths.nested_registry_entry_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed reading nested provenance-certificate session under {}: {error}",
                        paths.nested_registry_entry_session_dir.display()
                    ));
            ...
            let nested_handoff_bundle_status: RegistryEntryStatusView = match load_json(
                &nested_registry_entry_status_path(&paths.nested_registry_entry_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
            ...
            compare_registry_entry_copy_against_nested_truth(
                &stored_handoff_bundle,
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: RegistryEntryReportView =
                ...
                if handoff_bundle_verify.verdict != REGISTRY_ENTRY_VERIFY_OK {
                    mismatches.push(format!(
            ...
            .unwrap_or(LivePackageRegistryEntryLatestResult::RefusedByDriftedRegistryEntryContract);
            if status.result != expected_result {
                mismatches.push(format!(
                    "stored latest-registry-entry result {} does not match nested provenance-certificate result {}",
            ...
            if status.activation_authorized {
                mismatches.push(
                    "stored status activation_authorized must remain false for latest-registry-entry"
                        .to_string(),
                );
            }
            compare_option_string(
        ...
        Err(failure) => {
            ...
            if failure.latest_notarization_receipt_plan_raw.is_some() {
                let stored_latest_plan: NotarizationReceiptLatestReportView =
                ...
                compare_option_string(
                    status.latest_notarization_receipt_plan_verdict.as_deref(),
                    failure.latest_notarization_receipt_plan_verdict.as_deref(),
                    "stored status latest_notarization_receipt_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report
                        ...
                        .as_deref(),
                    failure.latest_notarization_receipt_plan_verdict.as_deref(),
                    "stored report latest_notarization_receipt_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
                status.latest_notarization_receipt_verdict.as_deref(),
                None,
                "stored status latest_notarization_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_notarization_receipt_verdict.as_deref(),
                None,
                "stored report latest_notarization_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.registry_entry_verdict.as_deref(),
                None,
                "stored status registry_entry_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.registry_entry_verdict.as_deref(),
                None,
                "stored report registry_entry_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestVerifyOk
    } else {
        TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-registry-entry session under {} is coherent with the current latest immutable chain and the accepted notarization-receipt contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-registry-entry verification failed".to_string())
    };

    Ok(PackageRegistryEntryLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_notarization_receipt".to_string(),
        verdict,
        reason,
        ...
        latest_notarization_receipt_plan_report_path: Some(
            paths
                .latest_notarization_receipt_plan_report_path
                .display()
                .to_string(),
        ),
        latest_notarization_receipt_report_path: Some(
            paths.latest_notarization_receipt_report_path.display().to_string(),
        ),
        registry_entry_report_path: Some(paths.registry_entry_report_path.display().to_string()),
        nested_notarization_receipt_latest_session_dir: Some(
            paths
                .nested_notarization_receipt_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_registry_entry_session_dir: Some(
            paths.nested_registry_entry_session_dir.display().to_string(),
        ),
        ...
        latest_notarization_receipt_plan_verdict: status.latest_notarization_receipt_plan_verdict.clone(),
        latest_notarization_receipt_verdict: status.latest_notarization_receipt_verdict.clone(),
        registry_entry_verdict: status.registry_entry_verdict.clone(),
        ...
        resolve_latest_notarization_receipt_command_summary: Some(
            session.resolve_latest_notarization_receipt_command_summary.clone(),
        ),
        run_latest_registry_entry_command_summary: Some(
            session.run_latest_registry_entry_command_summary.clone(),
        ),
        verify_latest_registry_entry_command_summary: Some(
            session.verify_latest_registry_entry_command_summary.clone(),
        ),
        downstream_registry_entry_command_summary: Some(
            session.downstream_registry_entry_command_summary.clone(),
        ),
        verify_registry_entry_command_summary: Some(session.verify_registry_entry_command_summary.clone()),
...
fn run_latest_registry_entry_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackageRegistryEntryLatestPaths,
    ...
    persist_json(
        &paths.latest_notarization_receipt_plan_report_path,
        &resolved.latest_notarization_receipt_plan_raw,
    )?;

    let latest_notarization_receipt_plan_step = Some(step_artifact(
        &paths.latest_notarization_receipt_plan_report_path,
        "plan_latest_notarization_receipt",
        &resolved.latest_notarization_receipt_plan.verdict,
        &resolved.latest_notarization_receipt_plan.reason,
        resolved.latest_notarization_receipt_plan.generated_at,
    ));

    let latest_notarization_receipt_run_raw = match run_json_command(
        NOTARIZATION_RECEIPT_LATEST_BIN,
        &latest_notarization_receipt_run_args(
            root,
            &paths.nested_notarization_receipt_latest_session_dir,
        ),
    ) {
        ...
        Err(error) => {
            let status =
                refusal_status(
                    ...
                    paths,
                    Some(resolved),
                    LivePackageRegistryEntryLatestResult::RefusedByDriftedRegistryEntryContract,
                    format!(
                    "failed running nested latest-notarization-receipt handoff under {}: {error}",
                    paths.nested_notarization_receipt_latest_session_dir.display()
                ),
                    latest_notarization_receipt_plan_step,
                    None,
    ...
    persist_json(
        &paths.latest_notarization_receipt_report_path,
        &latest_notarization_receipt_run_raw,
    )?;
    let latest_notarization_receipt_run: NotarizationReceiptLatestReportView =
        serde_json::from_value(latest_notarization_receipt_run_raw)
            .context("failed parsing nested latest-notarization-receipt run json")?;
    let latest_notarization_receipt_step = Some(step_artifact(
        &paths.latest_notarization_receipt_report_path,
        "run_latest_notarization_receipt",
        &latest_notarization_receipt_run.verdict,
        &latest_notarization_receipt_run.reason,
        latest_notarization_receipt_run.generated_at,
    ));

    if let Some(terminal_failure) =
        latest_notarization_receipt_terminal_failure(&latest_notarization_receipt_run)
    {
        let status = refusal_status(
            ...
            Some(resolved),
            terminal_failure,
            latest_notarization_receipt_run.reason.clone(),
            latest_notarization_receipt_plan_step,
            latest_notarization_receipt_step,
            None,
        );
    ...
    compare_notarization_receipt_latest_run_against_snapshot(
        &latest_notarization_receipt_run,
        &resolved.snapshot,
        &paths.nested_notarization_receipt_latest_session_dir,
        &mut run_mismatches,
    );
    ...
    let latest_notarization_receipt_verify: NotarizationReceiptLatestReportView =
        match run_json_command(
            ...
            Err(error) => {
                let status = refusal_status(
                    ...
                    LivePackageRegistryEntryLatestResult::RefusedByDriftedRegistryEntryContract,
                    format!(
                    "failed verifying nested latest-notarization-receipt handoff under {}: {error}",
                    paths.nested_notarization_receipt_latest_session_dir.display()
                ),
                    latest_notarization_receipt_plan_step,
                    latest_notarization_receipt_step,
    ...
    if latest_notarization_receipt_verify.verdict != NOTARIZATION_RECEIPT_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            LivePackageRegistryEntryLatestResult::RefusedByDriftedRegistryEntryContract,
            latest_notarization_receipt_verify.reason,
            latest_notarization_receipt_plan_step,
            latest_notarization_receipt_step,
            None,
        );
    ...
    persist_json(&paths.registry_entry_report_path, &release_capsule_run_raw)?;
    let provenance_certificate_run: RegistryEntryReportView =
    ...
    let provenance_certificate_verify: RegistryEntryReportView = match run_json_command(
        REGISTRY_ENTRY_BIN,
        &registry_entry_verify_args(
            &resolved.snapshot,
            &expected_downstream_notarization_receipt_session_dir(paths),
    ...
    if provenance_certificate_verify.verdict != REGISTRY_ENTRY_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    .unwrap_or(LivePackageRegistryEntryLatestResult::RefusedByDriftedRegistryEntryContract);
    let status = PackageRegistryEntryLatestStatus {
        status_version: REGISTRY_ENTRY_LATEST_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nested_notarization_receipt_latest_session_dir: paths
            .nested_notarization_receipt_latest_session_dir
            .display()
            .to_string(),
        nested_registry_entry_session_dir: paths
            .nested_registry_entry_session_dir
            .display()
            .to_string(),
        ...
        latest_notarization_receipt_plan_verdict: Some(
            resolved.latest_notarization_receipt_plan.verdict.clone(),
        ),
        latest_notarization_receipt_verdict: Some(latest_notarization_receipt_run.verdict.clone()),
        registry_entry_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_registry_entry_failure(
    ...
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_notarization_receipt_plan_raw {
        persist_json(&paths.latest_notarization_receipt_plan_report_path, value)?;
    }
...
fn resolve_latest_notarization_receipt(
    root: &Path,
    paths: &PackageRegistryEntryLatestPaths,
) -> std::result::Result<
...
> {
    let latest_notarization_receipt_plan_raw = match run_json_command(
        NOTARIZATION_RECEIPT_LATEST_BIN,
        &latest_notarization_receipt_plan_args(
            root,
            &paths.nested_notarization_receipt_latest_session_dir,
        ),
    ) {
        ...
        Err(error) => {
            return Err(LatestNotarizationReceiptResolutionFailure {
                ...
                snapshot: None,
                latest_notarization_receipt_plan_raw: None,
                latest_notarization_receipt_plan_verdict: None,
                latest_notarization_receipt_plan_reason: None,
                latest_notarization_receipt_plan_generated_at: None,
            });
        }
    };
    let latest_notarization_receipt_plan: NotarizationReceiptLatestReportView =
        serde_json::from_value(latest_notarization_receipt_plan_raw.clone()).map_err(|error| {
            LatestNotarizationReceiptResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-notarization-receipt plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_notarization_receipt_plan_raw: Some(
                    latest_notarization_receipt_plan_raw.clone(),
                ),
                latest_notarization_receipt_plan_verdict: None,
                latest_notarization_receipt_plan_reason: None,
                latest_notarization_receipt_plan_generated_at: None,
    ...
    let snapshot =
        parse_snapshot_from_notarization_receipt_latest(&latest_notarization_receipt_plan).ok();
    if latest_notarization_receipt_plan.verdict != NOTARIZATION_RECEIPT_LATEST_PLAN_READY {
        return Err(LatestNotarizationReceiptResolutionFailure {
            kind: map_notarization_receipt_latest_failure_kind(
                &latest_notarization_receipt_plan.verdict,
            ),
            reason: latest_notarization_receipt_plan.reason.clone(),
            snapshot,
            latest_notarization_receipt_plan_raw: Some(latest_notarization_receipt_plan_raw),
            latest_notarization_receipt_plan_verdict: Some(
                latest_notarization_receipt_plan.verdict,
            ),
            latest_notarization_receipt_plan_reason: Some(latest_notarization_receipt_plan.reason),
            latest_notarization_receipt_plan_generated_at: Some(
                latest_notarization_receipt_plan.generated_at,
            ),
        });
    }

    let snapshot =
        parse_snapshot_from_notarization_receipt_latest(&latest_notarization_receipt_plan)
            .map_err(|error| LatestNotarizationReceiptResolutionFailure {
                ...
                latest_notarization_receipt_plan_verdict: Some(
                    latest_notarization_receipt_plan.verdict.clone(),
                ),
                latest_notarization_receipt_plan_reason: Some(
                    latest_notarization_receipt_plan.reason.clone(),
                ),
                latest_notarization_receipt_plan_generated_at: Some(
                    latest_notarization_receipt_plan.generated_at,
                ),
    ...
    let mut mismatches = Vec::new();
    compare_notarization_receipt_latest_plan_against_snapshot(
        &latest_notarization_receipt_plan,
        &snapshot,
        &paths.nested_notarization_receipt_latest_session_dir,
        &mut mismatches,
    );
    ...
    Ok(ResolvedLatestNotarizationReceipt {
        snapshot,
        latest_notarization_receipt_plan_raw,
        latest_notarization_receipt_plan,
    })
}

fn parse_snapshot_from_notarization_receipt_latest(
    view: &NotarizationReceiptLatestReportView,
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-notarization-receipt plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-notarization-receipt plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestNotarizationReceiptSnapshot {
        ...
        wrapper_timeout_ms: view.wrapper_timeout_ms.ok_or_else(|| {
            anyhow!("latest-notarization-receipt plan is missing wrapper_timeout_ms")
        })?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!("latest-notarization-receipt plan is missing service_status_max_staleness_ms")
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackageRegistryEntryLatestPaths,
    resolved: &ResolvedLatestNotarizationReceipt,
) -> PackageRegistryEntryLatestReport {
    PackageRegistryEntryLatestReport {
        generated_at: Utc::now(),
        ...
        latest_notarization_receipt_report_path: Some(
            paths
                .latest_notarization_receipt_report_path
                .display()
                .to_string(),
        ),
        registry_entry_report_path: Some(paths.registry_entry_report_path.display().to_string()),
        ...
        latest_notarization_receipt_verdict: None,
        registry_entry_verdict: None,
        result: None,
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_notarization_receipt_command_summary: Some(
            resolve_latest_notarization_receipt_command_summary(
                root,
                &paths.nested_notarization_receipt_latest_session_dir,
            ),
        ),
        run_latest_registry_entry_command_summary: Some(run_latest_registry_entry_command_summary(
            root,
            &paths.nested_notarization_receipt_latest_session_dir,
        )),
        verify_latest_registry_entry_command_summary: Some(
            verify_latest_registry_entry_command_summary(
                &paths.nested_notarization_receipt_latest_session_dir,
            ),
        ),
        downstream_registry_entry_command_summary: Some(downstream_registry_entry_command_summary(
            &resolved.snapshot,
            &expected_downstream_notarization_receipt_session_dir(paths),
            &paths.nested_registry_entry_session_dir,
        )),
...
fn report_from_resolution_failure(
    ...
    PackageRegistryEntryLatestReport {
        ...
        latest_notarization_receipt_plan_verdict: failure
            .latest_notarization_receipt_plan_verdict
            .clone(),
        latest_notarization_receipt_verdict: None,
        registry_entry_verdict: None,
        ...
        downstream_registry_entry_command_summary: snapshot.map(|value| {
            downstream_registry_entry_command_summary(
                value,
                &expected_downstream_notarization_receipt_session_dir(paths),
                &paths.nested_registry_entry_session_dir,
            )
        }),
        verify_registry_entry_command_summary: snapshot.map(|value| {
...
) -> PackageRegistryEntryLatestSession {
    let snapshot = resolved
        .map(|value| &value.snapshot)
        .or_else(|| failure.and_then(|value| value.snapshot.as_ref()));
    PackageRegistryEntryLatestSession {
        session_version: REGISTRY_ENTRY_LATEST_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        root: root.display().to_string(),
        session_dir: session_dir.display().to_string(),
        nested_notarization_receipt_latest_session_dir: paths
        ...
        run_latest_registry_entry_command_summary: run_latest_registry_entry_command_summary(
            root,
            &paths.nested_notarization_receipt_latest_session_dir,
        ),
        verify_latest_registry_entry_command_summary: verify_latest_registry_entry_command_summary(
            &paths.nested_notarization_receipt_latest_session_dir,
        ),
        downstream_registry_entry_command_summary: snapshot
            .map(|value| {
                downstream_registry_entry_command_summary(
                    value,
                    &expected_downstream_notarization_receipt_session_dir(paths),
        ...
        verify_registry_entry_command_summary: snapshot
            .map(|value| {
                verify_registry_entry_command_summary(
                    value,
                    &expected_downstream_notarization_receipt_session_dir(paths),
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackageRegistryEntryLatestPaths,
    session: &PackageRegistryEntryLatestSession,
    status: &PackageRegistryEntryLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackageRegistryEntryLatestReport {
    PackageRegistryEntryLatestReport {
        generated_at: Utc::now(),
        ...
        nested_registry_entry_session_dir: Some(session.nested_registry_entry_session_dir.clone()),
        downstream_decision_packet_session_dir: Some(
            session.downstream_decision_packet_session_dir.clone(),
        ),
        latest_notarization_receipt_plan_verdict: status
            .latest_notarization_receipt_plan_verdict
            .clone(),
        latest_notarization_receipt_verdict: status.latest_notarization_receipt_verdict.clone(),
        registry_entry_verdict: status.registry_entry_verdict.clone(),
        ...
        verify_registry_entry_command_summary: Some(
            session.verify_registry_entry_command_summary.clone(),
        ),
        reviewed_frozen_live_cutover_controller_command_summary: session
            .reviewed_frozen_live_cutover_controller_command_summary
            .clone(),
...
fn registry_entry_latest_paths(session_dir: &Path) -> PackageRegistryEntryLatestPaths {
    PackageRegistryEntryLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_registry_entry_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_registry_entry_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_registry_entry_latest.report.json"),
        latest_notarization_receipt_plan_report_path: session_dir.join(
            "tiny_live_activation_package_registry_entry_latest.notarization_receipt_latest_plan.report.json",
        ),
        latest_notarization_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_registry_entry_latest.notarization_receipt_latest.report.json",
        ),
        registry_entry_report_path: session_dir.join(
            "tiny_live_activation_package_registry_entry_latest.registry_entry.report.json",
        ),
        nested_notarization_receipt_latest_session_dir: session_dir
            .join("tiny_live_activation_package_registry_entry_latest.notarization_receipt_latest_session"),
        nested_registry_entry_session_dir: session_dir.join(
            "tiny_live_activation_package_registry_entry_latest.registry_entry_session",
        ),
    }
}
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_notarization_receipt_latest_session_dir
        .join("tiny_live_activation_package_notarization_receipt_latest.decision_packet_session")
}

fn expected_downstream_notarization_receipt_session_dir(
    paths: &PackageRegistryEntryLatestPaths,
) -> PathBuf {
    paths.nested_notarization_receipt_latest_session_dir.join(
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_notarization_receipt_latest_session_dir
        .join("tiny_live_activation_package_notarization_receipt_latest.review_receipt_session")
}

fn latest_notarization_receipt_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-notarization-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn latest_notarization_receipt_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-notarization-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn latest_notarization_receipt_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-notarization-receipt".to_string(),
        "--json".to_string(),
    ]
}
...
fn registry_entry_run_args(
    ...
    vec![
        "--notarization-receipt-session-dir".to_string(),
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-registry-entry".to_string(),
        "--json".to_string(),
    ]
}

fn registry_entry_verify_args(
    snapshot: &LatestNotarizationReceiptSnapshot,
    notarization_receipt_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-live-package-registry-entry".to_string(),
        "--json".to_string(),
    ]
}

fn resolve_latest_notarization_receipt_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{NOTARIZATION_RECEIPT_LATEST_BIN} --root {} --session-dir {} --plan-latest-notarization-receipt --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn run_latest_registry_entry_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{NOTARIZATION_RECEIPT_LATEST_BIN} --root {} --session-dir {} --run-latest-notarization-receipt --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn verify_latest_registry_entry_command_summary(session_dir: &Path) -> String {
    format!(
        "{NOTARIZATION_RECEIPT_LATEST_BIN} --session-dir {} --verify-latest-notarization-receipt --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_registry_entry_command_summary(
    snapshot: &LatestNotarizationReceiptSnapshot,
    notarization_receipt_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{REGISTRY_ENTRY_BIN} --notarization-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-registry-entry --json",
...
fn verify_registry_entry_command_summary(
    snapshot: &LatestNotarizationReceiptSnapshot,
    notarization_receipt_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{REGISTRY_ENTRY_BIN} --notarization-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-registry-entry --json",
        shell_escape_path(notarization_receipt_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
...
fn nested_notarization_receipt_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_notarization_receipt_latest.report.json")
}

fn nested_registry_entry_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_registry_entry.session.json")
}

fn nested_registry_entry_status_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_registry_entry.status.json")
}
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-registry-entry script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageRegistryEntryLatestStepArtifact {
    PackageRegistryEntryLatestStepArtifact {
...
fn canonical_wrapper_mode(mode: &str) -> &str {
    match mode {
        "plan_latest_notarization_receipt" => "plan_latest_notarization_receipt",
        "render_latest_notarization_receipt_script" => "render_latest_notarization_receipt_script",
        "run_latest_notarization_receipt" => "run_latest_notarization_receipt",
        "verify_latest_notarization_receipt" => "verify_latest_notarization_receipt",
        other => other,
    }
}

fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackageRegistryEntryLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-registry-entry status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-registry-entry status"
        ));
        return;
    };
    compare_string(
...
fn compare_notarization_receipt_latest_plan_against_snapshot(
    stored: &NotarizationReceiptLatestReportView,
    snapshot: &LatestNotarizationReceiptSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        ...
        Some(NOTARIZATION_RECEIPT_LATEST_PLAN_READY),
        "stored latest-notarization-receipt plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_notarization_receipt",
        "stored latest-notarization-receipt plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-notarization-receipt plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-notarization-receipt plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-notarization-receipt plan session_dir",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        ...
        "stored latest-notarization-receipt plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-notarization-receipt plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-notarization-receipt plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-notarization-receipt plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_notarization_receipt_latest_run_against_snapshot(
    stored: &NotarizationReceiptLatestReportView,
    snapshot: &LatestNotarizationReceiptSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_notarization_receipt",
        "stored latest-notarization-receipt run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-notarization-receipt run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-notarization-receipt run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-notarization-receipt run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-notarization-receipt run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-notarization-receipt run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-notarization-receipt run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-notarization-receipt run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-notarization-receipt run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-notarization-receipt run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-notarization-receipt run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-notarization-receipt run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-notarization-receipt run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-notarization-receipt run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-notarization-receipt run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_notarization_receipt_latest_copy_matches_actual(
    stored: &NotarizationReceiptLatestReportView,
    actual: &NotarizationReceiptLatestReportView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        Some(stored.verdict.as_str()),
        Some(actual.verdict.as_str()),
        "stored latest-notarization-receipt copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-notarization-receipt copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-notarization-receipt copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-notarization-receipt copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-notarization-receipt copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-notarization-receipt copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-notarization-receipt copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-notarization-receipt copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-notarization-receipt copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-notarization-receipt copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-notarization-receipt copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-notarization-receipt copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_registry_entry_run_against_snapshot(
    stored: &RegistryEntryReportView,
    snapshot: &LatestNotarizationReceiptSnapshot,
    notarization_receipt_session_dir: &Path,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_live_package_registry_entry",
        "stored registry-entry run mode",
        mismatches,
    );
    compare_string(
        &stored.notarization_receipt_session_dir,
        &notarization_receipt_session_dir.display().to_string(),
        "stored registry-entry run notarization_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored registry-entry run decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored registry-entry run execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored registry-entry run turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored registry-entry run launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored registry-entry run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored registry-entry run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored registry-entry run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored registry-entry run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored registry-entry run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored registry-entry run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored registry-entry run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.registry_entry_session_path.as_deref(),
        Some(
            nested_registry_entry_session_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored registry-entry run registry_entry_session_path",
        mismatches,
    );
    compare_option_string(
        stored.registry_entry_status_path.as_deref(),
        Some(
            nested_registry_entry_status_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored registry-entry run registry_entry_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_notarization_receipt_report_path.as_deref(),
        Some(
            session_dir
                .join(
        ...
        "stored registry-entry run archived_notarization_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        stored
            ...
            .as_deref(),
        Some(
            registry_entry_verify_notarization_receipt_command_summary(
                notarization_receipt_session_dir,
            )
            .as_str(),
        ),
        "stored registry-entry run verify_notarization_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
...
fn compare_registry_entry_copy_against_nested_truth(
    ...
    compare_option_string(
        ...
        "stored registry-entry copy verify_notarization_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored registry-entry copy release_capsule_digest_manifest_sha256",
        mismatches,
    );
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_notarization_receipt_command_summary,
        &resolve_latest_notarization_receipt_command_summary(
            root,
            &paths.nested_notarization_receipt_latest_session_dir,
        ),
        "stored session resolve_latest_notarization_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.run_latest_registry_entry_command_summary,
        &run_latest_registry_entry_command_summary(
            root,
            &paths.nested_notarization_receipt_latest_session_dir,
        ),
        "stored session run_latest_registry_entry_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_latest_registry_entry_command_summary,
        &verify_latest_registry_entry_command_summary(
            &paths.nested_notarization_receipt_latest_session_dir,
        ),
        "stored session verify_latest_registry_entry_command_summary",
        mismatches,
    );
    compare_string(
        &session.downstream_registry_entry_command_summary,
        &downstream_registry_entry_command_summary(
            snapshot,
            &expected_downstream_notarization_receipt_session_dir(paths),
            &paths.nested_registry_entry_session_dir,
        ),
        "stored session downstream_registry_entry_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_registry_entry_command_summary,
        &verify_registry_entry_command_summary(
            snapshot,
            &expected_downstream_notarization_receipt_session_dir(paths),
            &paths.nested_registry_entry_session_dir,
        ),
        "stored session verify_registry_entry_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_notarization_receipt_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_registry_entry_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_registry_entry_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackageRegistryEntryLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackageRegistryEntryLatestPaths,
    snapshot: &LatestNotarizationReceiptSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_notarization_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report.run_latest_registry_entry_command_summary.as_deref(),
        Some(
            run_latest_registry_entry_command_summary(
                root,
                &paths.nested_notarization_receipt_latest_session_dir,
        ...
        "stored report run_latest_registry_entry_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report verify_latest_registry_entry_command_summary",
        mismatches,
    );
    compare_option_string(
        report.downstream_registry_entry_command_summary.as_deref(),
        Some(
            downstream_registry_entry_command_summary(
                snapshot,
                &expected_downstream_notarization_receipt_session_dir(paths),
        ...
        "stored report downstream_registry_entry_command_summary",
        mismatches,
    );
    compare_option_string(
        report.verify_registry_entry_command_summary.as_deref(),
        Some(
            verify_registry_entry_command_summary(
                snapshot,
                &expected_downstream_notarization_receipt_session_dir(paths),
        ...
        "stored report verify_registry_entry_command_summary",
        mismatches,
    );
    compare_option_string(
        report
    ...
    compare_option_string(
        ...
        "stored report latest_notarization_receipt_plan_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_notarization_receipt_report_path.as_deref(),
        Some(
            paths
                .latest_notarization_receipt_report_path
                .display()
        ...
        "stored report latest_notarization_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        report.registry_entry_report_path.as_deref(),
        Some(
            paths
                .registry_entry_report_path
                .display()
                .to_string()
                .as_str(),
        ),
        "stored report registry_entry_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report nested_notarization_receipt_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_registry_entry_session_dir.as_deref(),
        Some(
            paths
                .nested_registry_entry_session_dir
                .display()
        ...
        "stored report nested_registry_entry_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_notarization_receipt_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_registry_entry_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_registry_entry_session_dir on failure",
        mismatches,
    );
...
fn map_notarization_receipt_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_notarization_receipt_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_notarization_receipt_latest_refused_by_drifted_provenance_certificate_contract" => {
...
fn latest_notarization_receipt_terminal_failure(
    report: &NotarizationReceiptLatestReportView,
) -> Option<LivePackageRegistryEntryLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackageRegistryEntryLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackageRegistryEntryLatestResult::RefusedByInvalidLatestChain)
        }
        Some("refused_by_drifted_provenance_certificate_contract") => {
            Some(LivePackageRegistryEntryLatestResult::RefusedByDriftedRegistryEntryContract)
        }
        _ => None,
    }
}
...
fn registry_entry_latest_result_from_notarization_receipt(
    ...
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => {
            Some(LivePackageRegistryEntryLatestResult::ReadyForManualExecutionWhenGateTurnsGreen)
        }
        "refused_now_by_stage3" => Some(LivePackageRegistryEntryLatestResult::RefusedNowByStage3),
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackageRegistryEntryLatestResult::RefusedNowByPreActivationGate)
        }
...
fn notarization_receipt_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        ...
        "refused_now_by_stage3" => Some("tiny_live_package_registry_entry_refused_now_by_stage3"),
        "refused_now_by_pre_activation_gate" => {
            Some("tiny_live_package_registry_entry_refused_now_by_pre_activation_gate")
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_registry_entry_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}

fn result_from_failure_kind(kind: ResolutionFailureKind) -> LivePackageRegistryEntryLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackageRegistryEntryLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackageRegistryEntryLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedRegistryEntryContract => {
            LivePackageRegistryEntryLatestResult::RefusedByDriftedRegistryEntryContract
        }
...
fn verdict_for_result(
    result: LivePackageRegistryEntryLatestResult,
) -> TinyLivePackageRegistryEntryLatestVerdict {
    match result {
        LivePackageRegistryEntryLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestRefusedByMissingLatestChain
        }
        LivePackageRegistryEntryLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestRefusedByInvalidLatestChain
        }
        LivePackageRegistryEntryLatestResult::RefusedByDriftedRegistryEntryContract => {
            TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestRefusedByDriftedRegistryEntryContract
        }
        LivePackageRegistryEntryLatestResult::RefusedNowByStage3 => {
            TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestRefusedNowByStage3
        }
        LivePackageRegistryEntryLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestRefusedNowByPreActivationGate
        }
        LivePackageRegistryEntryLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackageRegistryEntryLatestResult) -> String {
    match result {
        LivePackageRegistryEntryLatestResult::RefusedByMissingLatestChain => {
        ...
        LivePackageRegistryEntryLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-notarization-receipt / provenance-certificate path".to_string()
        }
        LivePackageRegistryEntryLatestResult::RefusedByDriftedRegistryEntryContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-notarization-receipt / notarization-receipt contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackageRegistryEntryLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-registry-entry refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackageRegistryEntryLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-registry-entry refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackageRegistryEntryLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
...
fn registry_entry_verify_notarization_receipt_command_summary(
    notarization_receipt_session_dir: &Path,
) -> String {
    format!(
...
fn refusal_status(
    ...
    latest_notarization_receipt_step: Option<PackageRegistryEntryLatestStepArtifact>,
    registry_entry_step: Option<PackageRegistryEntryLatestStepArtifact>,
) -> PackageRegistryEntryLatestStatus {
    ...
    PackageRegistryEntryLatestStatus {
        ...
        result,
        reason,
        latest_notarization_receipt_plan_verdict: resolved
            .map(|value| value.latest_notarization_receipt_plan.verdict.clone()),
        latest_notarization_receipt_verdict: latest_notarization_receipt_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        registry_entry_verdict: registry_entry_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackageRegistryEntryLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_notarization_receipt_plan_step: failure
            .latest_notarization_receipt_plan_raw
            .as_ref()
            .map(|_| {
                step_artifact(
                    ...
                    failure
                        ...
                        .unwrap_or("<missing>"),
                    failure
                        .latest_notarization_receipt_plan_reason
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_notarization_receipt_plan_generated_at
                        .unwrap_or_else(Utc::now),
                )
            }),
        latest_notarization_receipt_step: None,
...
fn verify_invalid_registry_entry_latest_report_without_session(
    ...
    PackageRegistryEntryLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_notarization_receipt".to_string(),
        verdict: TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestVerifyInvalid,
        reason,
        root: None,
        ...
        latest_notarization_receipt_plan_report_path: None,
        latest_notarization_receipt_report_path: None,
        registry_entry_report_path: None,
        nested_notarization_receipt_latest_session_dir: None,
        nested_registry_entry_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_notarization_receipt_command_summary: None,
        run_latest_registry_entry_command_summary: None,
        verify_latest_registry_entry_command_summary: None,
        downstream_registry_entry_command_summary: None,
        verify_registry_entry_command_summary: None,
...
fn fallback_notarization_receipt_latest_copy() -> NotarizationReceiptLatestReportView {
    NotarizationReceiptLatestReportView {
        generated_at: Utc::now(),
...
fn fallback_notarization_receipt_session(
    ...
    RegistryEntrySessionView {
        ...
        session_dir: session_dir.display().to_string(),
        verify_notarization_receipt_command_summary:
            registry_entry_verify_notarization_receipt_command_summary(Path::new("<missing>")),
...
fn render_report_lines(report: &PackageRegistryEntryLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_notarization_receipt_latest_session_dir {
        lines.push(format!(
            "nested_notarization_receipt_latest_session_dir={value}"
        ));
    }
    ...
    if let Some(value) = &report.nested_registry_entry_session_dir {
        lines.push(format!("nested_registry_entry_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_registry_entry_command_summary {
        lines.push(format!("downstream_registry_entry_command_summary={value}"));
    }
...
mod tests {
    ...
    struct FakeNotarizationReceiptLatestFixture {
        ...
        notarization_receipt_latest_plan_json_path: PathBuf,
        notarization_receipt_latest_run_json_path: PathBuf,
        notarization_receipt_latest_verify_json_path: PathBuf,
    ...
    impl FakeNotarizationReceiptLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestRegistryEntry,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestRegistryEntryScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestRegistryEntry,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestRegistryEntry,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    #[test]
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_notarization_receipt_lineage(
    ) {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_plan_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = plan_latest_registry_entry_report(&fixture.plan_config()).unwrap();
    ...
    fn render_mode_emits_exact_downstream_provenance_certificate_contract_not_second_controller_path(
        ...
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_render_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = render_latest_registry_entry_script_report(&fixture.render_config()).unwrap();
        assert_eq!(
            report.verdict,
            TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestScriptRendered
        );
        let script = fs::read_to_string(&fixture.output_script_path).unwrap();
        assert!(script.contains(NOTARIZATION_RECEIPT_LATEST_BIN));
        assert!(script.contains(REGISTRY_ENTRY_BIN));
        assert!(script.contains("--notarization-receipt-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-registry-entry"));
        assert!(script.contains("--verify-live-package-registry-entry"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            (
                "notarization_receipt_latest_missing",
                "tiny_live_package_notarization_receipt_latest_refused_by_missing_latest_chain",
                "no persisted immutable chain exists",
                TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestRefusedByMissingLatestChain,
            ),
            (
                "notarization_receipt_latest_invalid",
                "tiny_live_package_notarization_receipt_latest_refused_by_invalid_latest_chain",
                "latest immutable chain is incomplete",
                TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestRefusedByInvalidLatestChain,
            ),
            (
                "notarization_receipt_latest_below",
                "tiny_live_package_notarization_receipt_latest_refused_by_invalid_latest_chain",
            ...
            let mut plan: Value = load_json(&fixture.notarization_receipt_latest_plan_json_path).unwrap();
            plan["verdict"] = json!(verdict);
            plan["reason"] = json!(reason);
            if label.ends_with("below") {
                plan["latest_top_step_name"] = json!("choir_receipt");
            }
            persist_json(&fixture.notarization_receipt_latest_plan_json_path, &plan).unwrap();

            let report = run_latest_registry_entry_report(&fixture.run_config()).unwrap();
            assert_eq!(report.verdict, expected);
        }
    }

    #[test]
    fn plan_mode_maps_real_latest_notarization_receipt_drift_verdict_to_local_drift_refusal() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_plan_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value =
            load_json(&fixture.notarization_receipt_latest_plan_json_path).unwrap();
        plan["verdict"] =
            json!("tiny_live_package_notarization_receipt_latest_refused_by_drifted_provenance_certificate_contract");
        plan["reason"] = json!("real accepted latest-notarization-receipt drift");
        persist_json(&fixture.notarization_receipt_latest_plan_json_path, &plan).unwrap();
    ...
    #[test]
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_notarization_receipt_reports_real_drift(
    ) {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_run_drift",
        ...
        assert_eq!(report.registry_entry_verdict.as_deref(), None);
    }

    #[test]
    fn run_mode_refuses_when_nested_release_capsule_lineage_drifts_from_latest_immutable_chain() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_handoff_bundle_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let mut run: Value = load_json(&fixture.notarization_receipt_latest_run_json_path).unwrap();
        run["package_dir"] = json!("/tampered/package");
        persist_json(&fixture.notarization_receipt_latest_run_json_path, &run).unwrap();
        ...
        assert!(report
            .reason
            .contains("stored latest-notarization-receipt run package_dir"));
    }

    #[test]
    fn verify_mode_fails_when_persisted_latest_notarization_receipt_artifacts_are_tampered_or_missing_nested_lineage_truth(
    ) {
        {
            let fixture = fake_notarization_receipt_latest_fixture(
                "notarization_receipt_latest_verify_tampered_copy",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            run_latest_registry_entry_report(&fixture.run_config()).unwrap();

            let paths = registry_entry_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.registry_entry_report_path).unwrap();
            ...
            let verify = verify_latest_registry_entry_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestVerifyInvalid
            );
        ...
        {
            let fixture = fake_notarization_receipt_latest_fixture(
                "notarization_receipt_latest_verify_missing_nested_truth",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = registry_entry_latest_paths(&fixture.wrapper_session_dir);
            fs::remove_file(nested_notarization_receipt_latest_report_path(
                &paths.nested_notarization_receipt_latest_session_dir,
            ))
            .unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains(
                    "tiny_live_activation_package_notarization_receipt_latest.report.json",
                )
            }));
        }
    }
    ...
    #[test]
    fn verify_fails_when_copied_nested_latest_notarization_receipt_explicit_statement_drifts() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_verify_launch_packet_explicit_statement_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = registry_entry_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.latest_notarization_receipt_report_path).unwrap();
        report["explicit_statement"] =
            json!("tampered nested latest release capsule explicit statement");
        persist_json(&paths.latest_notarization_receipt_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored latest-notarization-receipt copy explicit_statement")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_verify_decision_packet_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored registry-entry copy decision_packet_result") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_verify_attestation_seal_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored registry-entry copy attestation_seal_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_verify_chain_fingerprint_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored registry-entry copy chain_fingerprint_summary")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_verify_chain_fingerprint_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored registry-entry copy chain_fingerprint_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_verify_chain_fingerprint_algorithm_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored registry-entry copy chain_fingerprint_algorithm")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_verify_decision_packet_anchor_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored registry-entry copy decision_packet_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_verify_current_authorization_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored registry-entry copy execute_frozen_result") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_registry_entry_verdict_drifts() {
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_verify_release_capsule_verdict_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| entry.contains("stored registry-entry copy verdict")));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_current_live_cutover_controller_summary_drifts(
        ...
        let fixture = fake_notarization_receipt_latest_fixture(
            "notarization_receipt_latest_verify_current_live_cutover_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn all_modes_preserve_stage3_and_pre_activation_refusal_semantics_and_do_not_imply_activation_authorization_by_themselves(
        ...
        {
            let fixture = fake_notarization_receipt_latest_fixture(
                "notarization_receipt_latest_stage3",
                "refused_now_by_stage3",
                "refused_now_by_stage3",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);

            let plan = plan_latest_registry_entry_report(&fixture.plan_config()).unwrap();
            assert_eq!(
                plan.verdict,
                TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestPlanReady
            );
            ...
            let run = run_latest_registry_entry_report(&fixture.run_config()).unwrap();
            assert_eq!(
                run.verdict,
                TinyLivePackageRegistryEntryLatestVerdict::TinyLivePackageRegistryEntryLatestRefusedNowByStage3
            );
        ...
        {
            let fixture = fake_notarization_receipt_latest_fixture(
                "notarization_receipt_latest_pre_activation",
                "refused_now_by_pre_activation_gate",
                "refused_now_by_pre_activation_gate",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        {
            let fixture = fake_notarization_receipt_latest_fixture(
                "notarization_receipt_latest_green",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn fake_notarization_receipt_latest_fixture(
        label: &str,
        notarization_receipt_latest_run_result: &str,
        ...
        let notarization_receipt_latest_plan_json_path =
            temp_dir.join("notarization_receipt_latest.plan.json");
        let notarization_receipt_latest_run_json_path =
            temp_dir.join("notarization_receipt_latest.run.json");
        let notarization_receipt_latest_verify_json_path =
            temp_dir.join("notarization_receipt_latest.verify.json");
        ...
        let fixture = FakeNotarizationReceiptLatestFixture {
            ...
            install_root,
            backend_command,
            notarization_receipt_latest_plan_json_path,
            notarization_receipt_latest_run_json_path,
            notarization_receipt_latest_verify_json_path,
        ...
        persist_json(
            &fixture.notarization_receipt_latest_plan_json_path,
            &default_notarization_receipt_latest_plan_report(&fixture),
        )
        .unwrap();
        persist_json(
            &fixture.notarization_receipt_latest_run_json_path,
            &default_notarization_receipt_latest_run_report(
                &fixture,
                notarization_receipt_latest_run_result,
            ),
        )
        .unwrap();
        persist_json(
            &fixture.notarization_receipt_latest_verify_json_path,
            &default_notarization_receipt_latest_verify_report(
                &fixture,
                notarization_receipt_latest_run_result,
            ),
        )
        ...
        let paths = registry_entry_latest_paths(&fixture.wrapper_session_dir);
        write_fake_notarization_receipt_latest_command(
            &fixture.bin_dir.join(NOTARIZATION_RECEIPT_LATEST_BIN),
            &fixture.notarization_receipt_latest_plan_json_path,
            &fixture.notarization_receipt_latest_run_json_path,
            &fixture.notarization_receipt_latest_verify_json_path,
            &fixture.root,
        ...
        write_fake_release_capsule_command(
            &fixture.bin_dir.join(REGISTRY_ENTRY_BIN),
            &fixture.handoff_bundle_run_json_path,
            &fixture.handoff_bundle_verify_json_path,
            &fixture.handoff_bundle_session_json_path,
            &fixture.handoff_bundle_status_json_path,
    ...
    fn default_notarization_receipt_latest_plan_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
    ) -> Value {
        let paths = registry_entry_latest_paths(&fixture.wrapper_session_dir);
        json!({
            "generated_at": Utc::now(),
            "mode": "plan_latest_notarization_receipt",
            "verdict": NOTARIZATION_RECEIPT_LATEST_PLAN_READY,
            "reason": "latest immutable chain resolves cleanly into accepted latest-notarization-receipt contract",
            ...
            "session_dir": paths.nested_notarization_receipt_latest_session_dir.display().to_string(),
    ...
    fn canonical_notarization_receipt_latest_result(result: &str) -> &str {
        match result {
            "runnable_when_gate_truth_turns_green" => {
                "ready_for_manual_execution_when_gate_turns_green"
            }
    ...
    fn default_notarization_receipt_latest_run_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let result = canonical_notarization_receipt_latest_result(result);
        let paths = registry_entry_latest_paths(&fixture.wrapper_session_dir);
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_notarization_receipt_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_notarization_receipt_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_notarization_receipt_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_latest_notarization_receipt",
            "verdict": verdict,
    ...
    fn default_notarization_receipt_latest_verify_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let mut report = default_notarization_receipt_latest_run_report(fixture, result);
        report["mode"] = json!("verify_latest_notarization_receipt");
        report["verdict"] = json!(NOTARIZATION_RECEIPT_LATEST_VERIFY_OK);
    ...
    fn default_provenance_certificate_session(
        ...
        let paths = registry_entry_latest_paths(&fixture.wrapper_session_dir);
        let notarization_receipt_session_dir =
            expected_downstream_notarization_receipt_session_dir(&paths);
        ...
        json!({
            ...
            "release_capsule_session_dir": notarization_receipt_session_dir.display().to_string(),
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": registry_entry_latest_paths(&fixture.wrapper_session_dir).nested_registry_entry_session_dir.display().to_string(),
            "verify_notarization_receipt_command_summary": registry_entry_verify_notarization_receipt_command_summary(&notarization_receipt_session_dir),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_registry_entry_session_dir.display().to_string(),
            "notarization_receipt_session_dir": expected_downstream_notarization_receipt_session_dir(&paths).display().to_string(),
            "result": result,
            "reason": reason,
            ...
            "current_pre_activation_gate_verdict": gate_verdict,
            "current_pre_activation_gate_reason": gate_reason,
            "notarization_receipt_step": Value::Null,
    ...
    fn default_provenance_certificate_run_report(
        ...
        let registry_entry_verdict = match result {
            "ready_for_manual_execution_when_gate_turns_green" => {
                "tiny_live_package_registry_entry_ready_for_manual_execution_when_gate_turns_green"
            }
            "refused_now_by_stage3" => "tiny_live_package_registry_entry_refused_now_by_stage3",
            "refused_now_by_pre_activation_gate" => {
                "tiny_live_package_registry_entry_refused_now_by_pre_activation_gate"
            }
            _ => "tiny_live_package_registry_entry_refused_now_by_invalid_or_drifted_contract",
        };
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_registry_entry",
            "verdict": registry_entry_verdict,
            ...
            "release_capsule_session_dir": expected_downstream_notarization_receipt_session_dir(&paths).display().to_string(),
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_registry_entry_session_dir.display().to_string(),
            "registry_entry_session_path": nested_registry_entry_session_path(&paths.nested_registry_entry_session_dir).display().to_string(),
            "registry_entry_status_path": nested_registry_entry_status_path(&paths.nested_registry_entry_session_dir).display().to_string(),
            "archived_notarization_receipt_report_path": paths.nested_registry_entry_session_dir.join("tiny_live_activation_package_registry_entry.notarization_receipt.report.json").display().to_string(),
            "result": result,
            ...
            "notarization_receipt_step": Value::Null,
            "verify_notarization_receipt_command_summary": registry_entry_verify_notarization_receipt_command_summary(&expected_downstream_notarization_receipt_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn default_provenance_certificate_verify_report(
        ...
        report["mode"] = json!("verify_live_package_registry_entry");
        report["verdict"] = json!(REGISTRY_ENTRY_VERIFY_OK);
        report["reason"] = json!("verify ok");
    ...
    fn write_fake_notarization_receipt_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-notarization-receipt session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-notarization-receipt \"*) mode='plan';;\n  *\" --run-latest-notarization-receipt \"*) mode='run';;\n  *\" --verify-latest-notarization-receipt \"*) mode='verify';;\n  *) echo 'unexpected latest-notarization-receipt mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_notarization_receipt_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
