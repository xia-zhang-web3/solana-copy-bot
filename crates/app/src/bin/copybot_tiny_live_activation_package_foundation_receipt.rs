...
use copybot_app::tiny_live_activation::package_cornerstone_certificate_foundation_receipt;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_foundation_receipt --cornerstone-certificate-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-foundation-receipt | --render-live-package-foundation-receipt | --run-live-package-foundation-receipt | --verify-live-package-foundation-receipt)";
const FOUNDATION_RECEIPT_SESSION_VERSION: &str = "1";
const FOUNDATION_RECEIPT_STATUS_VERSION: &str = "1";
const FOUNDATION_RECEIPT_ALGORITHM: &str = "sha256";
const FOUNDATION_RECEIPT_SESSION_EXPLICIT_STATEMENT: &str =
...
const FOUNDATION_RECEIPT_STATUS_EXPLICIT_STATEMENT: &str =
    "this foundation receipt is archival and read-only; it never executes or enables the frozen controller";
const FOUNDATION_RECEIPT_VERIFY_EXPLICIT_STATEMENT: &str =
...
enum Mode {
    PlanLivePackageFoundationReceipt,
    RenderLivePackageFoundationReceipt,
    RunLivePackageFoundationReceipt,
    VerifyLivePackageFoundationReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageFoundationReceiptVerdict {
    TinyLivePackageFoundationReceiptPlanReady,
    TinyLivePackageFoundationReceiptRendered,
    TinyLivePackageFoundationReceiptRefusedNowByStage3,
    TinyLivePackageFoundationReceiptRefusedNowByPreActivationGate,
    TinyLivePackageFoundationReceiptRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageFoundationReceiptReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageFoundationReceiptVerifyOk,
    TinyLivePackageFoundationReceiptVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageFoundationReceiptResult {
    RefusedNowByStage3,
...
struct PackageFoundationReceiptStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageFoundationReceiptSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageFoundationReceiptStatus {
    status_version: String,
    updated_at: DateTime<Utc>,
    session_dir: String,
    cornerstone_certificate_session_dir: String,
    result: LivePackageFoundationReceiptResult,
    reason: String,
    ...
    cornerstone_certificate_step: Option<PackageFoundationReceiptStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageFoundationReceiptPaths {
...
struct PackageFoundationReceiptReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageFoundationReceiptVerdict,
    reason: String,
    cornerstone_certificate_session_dir: String,
...
struct NestedCornerstoneCertificateReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
struct FoundationReceiptPayload<'a> {
    result: &'a str,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut cornerstone_certificate_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--cornerstone-certificate-session-dir" => {
                cornerstone_certificate_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--cornerstone-certificate-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-foundation-receipt" => set_mode(
                &mut mode,
                Mode::PlanLivePackageFoundationReceipt,
                arg.as_str(),
            )?,
            "--render-live-package-foundation-receipt" => set_mode(
                &mut mode,
                Mode::RenderLivePackageFoundationReceipt,
                arg.as_str(),
            )?,
            "--run-live-package-foundation-receipt" => set_mode(
                &mut mode,
                Mode::RunLivePackageFoundationReceipt,
                arg.as_str(),
            )?,
            "--verify-live-package-foundation-receipt" => set_mode(
                &mut mode,
                Mode::VerifyLivePackageFoundationReceipt,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackageFoundationReceipt) && output_path.is_none() {
        bail!("missing --output for --render-live-package-foundation-receipt");
    }
    if matches!(
        mode,
        Mode::RunLivePackageFoundationReceipt | Mode::VerifyLivePackageFoundationReceipt
    ) && session_dir.is_none()
    {
    ...
    let cornerstone_certificate_session_dir = cornerstone_certificate_session_dir
        .ok_or_else(|| anyhow!("missing --cornerstone-certificate-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageFoundationReceipt => {
            plan_live_package_foundation_receipt_report(&config)?
        }
        Mode::RenderLivePackageFoundationReceipt => {
            render_live_package_foundation_receipt_report(&config)?
        }
        Mode::RunLivePackageFoundationReceipt => {
            run_live_package_foundation_receipt_report(&config)?
        }
        Mode::VerifyLivePackageFoundationReceipt => {
            verify_live_package_foundation_receipt_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_foundation_receipt_report(
    config: &Config,
) -> Result<PackageFoundationReceiptReport> {
    let (cornerstone_certificate, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_cornerstone_certificate_foundation_receipt::verify_live_package_cornerstone_certificate_for_foundation_receipt(
                    &config.cornerstone_certificate_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_cornerstone_certificate(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_cornerstone_certificate_foundation_receipt::load_live_package_cornerstone_certificate_contract_for_foundation_receipt(
                            &config.cornerstone_certificate_session_dir,
                        )?;
                        let classification = classify_cornerstone_certificate_contract(&raw_contract, false);
    ...
    let session_dir = config.session_dir.clone().unwrap_or_else(|| {
        config
            .cornerstone_certificate_session_dir
            .join("foundation-receipt")
    });
    let result = serialize_enum(&classification.0);
    let foundation_receipt_sha256 = compute_foundation_receipt_sha256(
        &result,
    ...
    Ok(PackageFoundationReceiptReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_foundation_receipt".to_string(),
        verdict:
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptPlanReady,
        reason: format!(
            "foundation-receipt-native foundation receipt is explicit for verified cornerstone-certificate session {}; this command stays archival and read-only while freezing one final diadem identity over the fully culminated chain",
            config.cornerstone_certificate_session_dir.display()
        ),
        cornerstone_certificate_session_dir: config.cornerstone_certificate_session_dir.display().to_string(),
        ...
        foundation_receipt_session_path: None,
        foundation_receipt_status_path: None,
        archived_cornerstone_certificate_report_path: None,
        result: Some(result),
        ...
        verify_cornerstone_certificate_command_summary: Some(verify_cornerstone_certificate_command_summary(
            &config.cornerstone_certificate_session_dir,
        )),
        ...
        foundation_receipt_summary: Some(foundation_receipt_summary(
            classification.0,
            &cornerstone_certificate.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        foundation_receipt_sha256: Some(foundation_receipt_sha256),
        foundation_receipt_algorithm: Some(FOUNDATION_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: FOUNDATION_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_foundation_receipt_report(
    config: &Config,
) -> Result<PackageFoundationReceiptReport> {
    let plan = plan_live_package_foundation_receipt_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_foundation_receipt --cornerstone-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-foundation-receipt --json\ncopybot_tiny_live_activation_package_foundation_receipt --cornerstone-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-foundation-receipt --json\n",
        shell_escape_path(&config.cornerstone_certificate_session_dir),
    ...
    Ok(PackageFoundationReceiptReport {
        generated_at: Utc::now(),
        mode: "render_live_package_foundation_receipt".to_string(),
        verdict:
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptRendered,
        reason: format!(
            "rendered foundation-receipt run/verify script to {}; the primary input remains the immutable cornerstone-certificate session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: FOUNDATION_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_foundation_receipt_report(
    config: &Config,
) -> Result<PackageFoundationReceiptReport> {
    ...
    let verified_cornerstone_certificate = match package_cornerstone_certificate_foundation_receipt::verify_live_package_cornerstone_certificate_for_foundation_receipt(
        &config.cornerstone_certificate_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify cornerstone-certificate session under {}: {error}",
                    config.cornerstone_certificate_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_cornerstone_certificate.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "foundation-receipt session dir {} overlaps the managed live target surface derived from verified cornerstone-certificate install_root {}",
            session_dir.display(),
            verified_cornerstone_certificate.contract.install_root.display()
        )
    })?;
    ensure_clean_foundation_receipt_session_dir(session_dir)?;
    let paths = foundation_receipt_paths(session_dir);
    let classification =
        classify_verified_cornerstone_certificate(&verified_cornerstone_certificate);
    let result = classification.0;
    let foundation_receipt_sha256 = compute_foundation_receipt_sha256(
        &serialize_enum(&result),
        verified_cornerstone_certificate
            .contract
        ...
        &verified_cornerstone_certificate.contract.ledger_seal_sha256,
        &verified_cornerstone_certificate
            .contract
    ...
    let session = planned_session(
        config,
        session_dir,
        &verified_cornerstone_certificate.contract,
        result,
    ...
    persist_json(
        ...
        &verified_cornerstone_certificate.report_json,
    )?;
    let cornerstone_certificate_step = Some(step_artifact(
        &paths.cornerstone_certificate_report_path,
        "verify_live_package_cornerstone_certificate",
        &verified_cornerstone_certificate.verdict,
        &verified_cornerstone_certificate.reason,
        verified_cornerstone_certificate.generated_at,
    ));
    let status = PackageFoundationReceiptStatus {
        status_version: FOUNDATION_RECEIPT_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        cornerstone_certificate_session_dir: config
            .cornerstone_certificate_session_dir
            .display()
            .to_string(),
        result,
        reason: classification.2.clone(),
        handoff_bundle_result: verified_cornerstone_certificate
            .contract
            .handoff_bundle_result
            .clone(),
        decision_packet_result: verified_cornerstone_certificate
            .contract
            .decision_packet_result
            .clone(),
        execute_frozen_result: verified_cornerstone_certificate
            .contract
            .execute_frozen_result
            .clone(),
        current_pre_activation_gate_verdict: verified_cornerstone_certificate
            .contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_cornerstone_certificate
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        cornerstone_certificate_step: cornerstone_certificate_step.clone(),
        provenance_certificate_summary: verified_cornerstone_certificate
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_cornerstone_certificate
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_cornerstone_certificate
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_cornerstone_certificate
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_cornerstone_certificate
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_cornerstone_certificate
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_cornerstone_certificate
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_cornerstone_certificate
            .contract
            .notarization_receipt_summary
            .clone(),
        ledger_seal_summary: verified_cornerstone_certificate
            .contract
            .ledger_seal_summary
            .clone(),
        ledger_seal_sha256: verified_cornerstone_certificate
            .contract
            .ledger_seal_sha256
            .clone(),
        ledger_seal_algorithm: verified_cornerstone_certificate
            .contract
            .ledger_seal_algorithm
            .clone(),
        registry_entry_summary: verified_cornerstone_certificate
            .contract
            .registry_entry_summary
            .clone(),
        registry_entry_sha256: verified_cornerstone_certificate
            .contract
            .registry_entry_sha256
            .clone(),
        registry_entry_algorithm: verified_cornerstone_certificate
            .contract
            .registry_entry_algorithm
            .clone(),
        filing_certificate_summary: verified_cornerstone_certificate
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_cornerstone_certificate
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_cornerstone_certificate
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_cornerstone_certificate
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_cornerstone_certificate
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_cornerstone_certificate
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_cornerstone_certificate
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_cornerstone_certificate
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_cornerstone_certificate
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_cornerstone_certificate
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_cornerstone_certificate
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_cornerstone_certificate
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_cornerstone_certificate
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_cornerstone_certificate
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_cornerstone_certificate
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_cornerstone_certificate
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_cornerstone_certificate
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_cornerstone_certificate
            .contract
            .completion_certificate_algorithm
            .clone(),
        summit_certificate_summary: verified_cornerstone_certificate
            .contract
            .summit_certificate_summary
            .clone(),
        summit_certificate_sha256: verified_cornerstone_certificate
            .contract
            .summit_certificate_sha256
            .clone(),
        summit_certificate_algorithm: verified_cornerstone_certificate
            .contract
            .summit_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_cornerstone_certificate
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_cornerstone_certificate
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_cornerstone_certificate
            .contract
            .culmination_receipt_algorithm
            .clone(),
        pinnacle_receipt_summary: verified_cornerstone_certificate
            .contract
            .pinnacle_receipt_summary
            .clone(),
        pinnacle_receipt_sha256: verified_cornerstone_certificate
            .contract
            .pinnacle_receipt_sha256
            .clone(),
        pinnacle_receipt_algorithm: verified_cornerstone_certificate
            .contract
            .pinnacle_receipt_algorithm
            .clone(),
        capstone_certificate_summary: verified_cornerstone_certificate
            .contract
            .capstone_certificate_summary
            .clone(),
        capstone_certificate_sha256: verified_cornerstone_certificate
            .contract
            .capstone_certificate_sha256
            .clone(),
        capstone_certificate_algorithm: verified_cornerstone_certificate
            .contract
            .capstone_certificate_algorithm
            .clone(),
        keystone_receipt_summary: verified_cornerstone_certificate
            .contract
            .keystone_receipt_summary
            .clone(),
        keystone_receipt_sha256: verified_cornerstone_certificate
            .contract
            .keystone_receipt_sha256
            .clone(),
        keystone_receipt_algorithm: verified_cornerstone_certificate
            .contract
            .keystone_receipt_algorithm
            .clone(),
        cornerstone_certificate_summary: verified_cornerstone_certificate
            .contract
            .cornerstone_certificate_summary
            .clone(),
        cornerstone_certificate_sha256: verified_cornerstone_certificate
            .contract
            .cornerstone_certificate_sha256
            .clone(),
        cornerstone_certificate_algorithm: verified_cornerstone_certificate
            .contract
            .cornerstone_certificate_algorithm
            .clone(),
        foundation_receipt_summary: foundation_receipt_summary(
            result,
            &verified_cornerstone_certificate
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        foundation_receipt_sha256: foundation_receipt_sha256.clone(),
        foundation_receipt_algorithm: FOUNDATION_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: FOUNDATION_RECEIPT_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_foundation_receipt_report(
    config: &Config,
) -> Result<PackageFoundationReceiptReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = foundation_receipt_paths(session_dir);
    let session: PackageFoundationReceiptSession = load_json(&paths.session_path)?;
    let status: PackageFoundationReceiptStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.cornerstone_certificate_session_dir,
        &config
            .cornerstone_certificate_session_dir
            .display()
            .to_string(),
        "foundation-receipt cornerstone_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "foundation-receipt session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "foundation-receipt status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.cornerstone_certificate_session_dir,
        &config
            .cornerstone_certificate_session_dir
            .display()
            .to_string(),
        "foundation-receipt status cornerstone_certificate_session_dir",
        &mut mismatches,
    );
    ...
    let stored_cornerstone_certificate_report: serde_json::Value = load_required_step_json(
        &status.cornerstone_certificate_step,
        &paths.cornerstone_certificate_report_path,
        "cornerstone_certificate_step",
        &mut mismatches,
    )?;
    let stored_cornerstone_certificate_report_view: NestedCornerstoneCertificateReportView =
        serde_json::from_value(stored_cornerstone_certificate_report.clone()).with_context(
            || {
                format!(
                    "failed parsing archived nested cornerstone-certificate report {}",
                    paths.cornerstone_certificate_report_path.display()
                )
            },
        )?;
    let verified_cornerstone_certificate_report_view: NestedCornerstoneCertificateReportView =
        serde_json::from_value(verified_cornerstone_certificate.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested cornerstone-certificate report for {}",
                    config.cornerstone_certificate_session_dir.display()
                )
            },
        )?;
    compare_json_ignoring_generated_at(
        &stored_cornerstone_certificate_report,
        &verified_cornerstone_certificate.report_json,
        "foundation-receipt archived cornerstone-certificate report json",
        &mut mismatches,
    );
    compare_string(
        &stored_cornerstone_certificate_report_view.mode,
        &verified_cornerstone_certificate_report_view.mode,
        "foundation-receipt archived cornerstone-certificate report mode",
        &mut mismatches,
    );
    compare_string(
        &stored_cornerstone_certificate_report_view.verdict,
        &verified_cornerstone_certificate_report_view.verdict,
        "foundation-receipt archived cornerstone-certificate report verdict",
        &mut mismatches,
    );
    compare_string(
        &stored_cornerstone_certificate_report_view.reason,
        &verified_cornerstone_certificate_report_view.reason,
        "foundation-receipt archived cornerstone-certificate report reason",
        &mut mismatches,
    );
    if stored_cornerstone_certificate_report_view.generated_at
        != verified_cornerstone_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "foundation-receipt archived cornerstone-certificate report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_cornerstone_certificate_report_view.generated_at,
            verified_cornerstone_certificate_report_view.generated_at
        ));
    }
    let expected_cornerstone_certificate_step = step_artifact(
        &paths.cornerstone_certificate_report_path,
        &verified_cornerstone_certificate_report_view.mode,
        &verified_cornerstone_certificate_report_view.verdict,
        &verified_cornerstone_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_cornerstone_certificate_step,
        "foundation-receipt cornerstone_certificate_step",
        &mut mismatches,
    );
    if verified_cornerstone_certificate.verdict
        != "tiny_live_package_cornerstone_certificate_verify_ok"
    {
        mismatches.push(format!(
            "foundation-receipt nested cornerstone-certificate verification is non-green: {}",
            verified_cornerstone_certificate.reason
        ));
    }
    ...
    let classification =
        classify_verified_cornerstone_certificate(&verified_cornerstone_certificate);
    let expected_foundation_receipt_sha256 = compute_foundation_receipt_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_cornerstone_certificate.contract,
        classification.0,
        &expected_foundation_receipt_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "foundation-receipt session contract does not match freshly verified cornerstone-certificate truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_cornerstone_certificate.contract,
        classification,
        expected_cornerstone_certificate_step.clone(),
        &expected_foundation_receipt_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "foundation-receipt status contract does not match freshly verified cornerstone-certificate truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_cornerstone_certificate.contract,
        TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptVerifyOk,
        &expected_status,
    ))
}
...
fn classify_cornerstone_certificate_contract(
    contract: &package_cornerstone_certificate_foundation_receipt::LivePackageCornerstoneCertificateContractView,
    assume_verified: bool,
) -> (
    LivePackageFoundationReceiptResult,
    TinyLivePackageFoundationReceiptVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageFoundationReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptRefusedNowByInvalidOrDriftedContract,
            "the cornerstone-certificate chain is not verified, so this foundation receipt remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageFoundationReceiptResult::RefusedNowByStage3,
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptRefusedNowByStage3,
            "the immutable cornerstone certificate remains refused now by Stage 3, so this foundation receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageFoundationReceiptResult::RefusedNowByPreActivationGate,
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptRefusedNowByPreActivationGate,
            "the immutable cornerstone certificate remains refused now by the pre-activation gate, so this foundation receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageFoundationReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable cornerstone certificate is coherent and this foundation receipt is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageFoundationReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptRefusedNowByInvalidOrDriftedContract,
            "the verified cornerstone-certificate chain is invalid, drifted, or non-runnable, so this foundation receipt remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_cornerstone_certificate(
    verified: &package_cornerstone_certificate_foundation_receipt::VerifiedLivePackageCornerstoneCertificateFoundationReceiptStep,
) -> (
    LivePackageFoundationReceiptResult,
    TinyLivePackageFoundationReceiptVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_cornerstone_certificate_verify_ok" {
        return (
            LivePackageFoundationReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptRefusedNowByInvalidOrDriftedContract,
            "the immutable cornerstone certificate did not verify cleanly, so this foundation receipt remains refused"
                .to_string(),
        );
    }
    classify_cornerstone_certificate_contract(&verified.contract, true)
}

fn foundation_receipt_summary(
    result: LivePackageFoundationReceiptResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageFoundationReceiptResult::RefusedNowByStage3 => {
            "Foundation receipt outcome: the immutable culminated chain remains refused now by Stage 3; archive this final diadem seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageFoundationReceiptResult::RefusedNowByPreActivationGate => {
            "Foundation receipt outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final diadem seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageFoundationReceiptResult::RefusedNowByInvalidOrDriftedContract => {
            "Foundation receipt outcome: the immutable culminated chain is invalid or drifted; archive this final diadem seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackageFoundationReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Foundation receipt outcome: this immutable diadem seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn foundation_receipt_paths(session_dir: &Path) -> PackageFoundationReceiptPaths {
    PackageFoundationReceiptPaths {
...
fn ensure_clean_foundation_receipt_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing foundation-receipt session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating foundation-receipt session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageFoundationReceiptSession {
    PackageFoundationReceiptSession {
        session_version: FOUNDATION_RECEIPT_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        cornerstone_certificate_session_dir: config
            .cornerstone_certificate_session_dir
            .display()
        ...
        foundation_receipt_sha256: foundation_receipt_sha256.to_string(),
        foundation_receipt_algorithm: FOUNDATION_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: FOUNDATION_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    cornerstone_certificate_step: PackageFoundationReceiptStepArtifact,
    foundation_receipt_sha256: &str,
) -> PackageFoundationReceiptStatus {
    PackageFoundationReceiptStatus {
        status_version: FOUNDATION_RECEIPT_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        cornerstone_certificate_step: Some(cornerstone_certificate_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageFoundationReceiptPaths,
    contract: &package_cornerstone_certificate_foundation_receipt::LivePackageCornerstoneCertificateContractView,
    verdict: TinyLivePackageFoundationReceiptVerdict,
    status: &PackageFoundationReceiptStatus,
) -> PackageFoundationReceiptReport {
    PackageFoundationReceiptReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptVerifyOk
        {
            "verify_live_package_foundation_receipt".to_string()
        } else {
            "run_live_package_foundation_receipt".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        foundation_receipt_session_path: Some(paths.session_path.display().to_string()),
        foundation_receipt_status_path: Some(paths.status_path.display().to_string()),
        archived_cornerstone_certificate_report_path: Some(
            paths
        ...
        cornerstone_certificate_step: status.cornerstone_certificate_step.clone(),
        ...
        explicit_statement: if verdict
            == TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptVerifyOk
        {
            FOUNDATION_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageFoundationReceiptReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageFoundationReceipt => {
                "plan_live_package_foundation_receipt".to_string()
            }
            Mode::RenderLivePackageFoundationReceipt => {
                "render_live_package_foundation_receipt".to_string()
            }
            Mode::RunLivePackageFoundationReceipt => {
                "run_live_package_foundation_receipt".to_string()
            }
        ...
        cornerstone_certificate_algorithm: Some(FOUNDATION_RECEIPT_ALGORITHM.to_string()),
        foundation_receipt_summary: None,
        foundation_receipt_sha256: None,
        foundation_receipt_algorithm: Some(FOUNDATION_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable foundation receipt refused before persisting because the cornerstone-certificate artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "foundation-receipt-native foundation-receipt verification failed".to_string()
    });
    PackageFoundationReceiptReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_foundation_receipt".to_string(),
        verdict:
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptVerifyInvalid,
        reason,
        cornerstone_certificate_session_dir: config
            .cornerstone_certificate_session_dir
            .display()
        ...
        explicit_statement: FOUNDATION_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageFoundationReceiptStepArtifact>,
    expected_path: &Path,
    label: &str,
    mismatches: &mut Vec<String>,
) -> Result<serde_json::Value> {
    let Some(step) = step else {
        mismatches.push(format!("{label} is missing from foundation-receipt status"));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageFoundationReceiptStepArtifact {
    PackageFoundationReceiptStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageFoundationReceiptStepArtifact>,
    expected: &PackageFoundationReceiptStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_foundation_receipt_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&FoundationReceiptPayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing foundation-receipt script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackageFoundationReceiptReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "cornerstone_certificate_session_dir={}",
            report.cornerstone_certificate_session_dir
        ),
    ];
    ...
    if let Some(value) = &report.foundation_receipt_sha256 {
        lines.push(format!("foundation_receipt_sha256={value}"));
    }
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_foundation_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify =
            verify_live_package_foundation_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageFoundationReceiptVerdict::TinyLivePackageFoundationReceiptVerifyOk
        );
    ...
    #[test]
    fn drifted_cornerstone_certificate_contract_is_refused() {
        let fixture = fake_command_fixture(
            "cornerstone_certificate_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_cornerstone_certificate",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        let overlap_session_dir = managed_paths
            .runtime_dir
            .join("cornerstone-certificate-session");
        ...
        let error = run_live_package_foundation_receipt_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = foundation_receipt_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_cornerstone_certificate_step_path() {
        let fixture = fake_command_fixture(
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_foundation_receipt_report(&fixture.run_config()).unwrap();
        let paths = foundation_receipt_paths(&fixture.foundation_receipt_session_dir);
        let mut status: PackageFoundationReceiptStatus = load_json(&paths.status_path).unwrap();
        status.cornerstone_certificate_step.as_mut().unwrap().path =
    ...
    #[test]
    fn verify_rejects_tampered_cornerstone_certificate_text() {
        let fixture = fake_command_fixture(
            "cornerstone_certificate_tampered_text",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        run_live_package_foundation_receipt_report(&fixture.run_config()).unwrap();
        let paths = foundation_receipt_paths(&fixture.foundation_receipt_session_dir);
        let mut session: PackageFoundationReceiptSession = load_json(&paths.session_path).unwrap();
        session.cornerstone_certificate_summary = "tampered cornerstone summary".to_string();
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = foundation_receipt_paths(&fixture.foundation_receipt_session_dir);
        let mut status: PackageFoundationReceiptStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageFoundationReceiptResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_cornerstone_certificate_report_content() {
        let fixture = fake_command_fixture(
            "cornerstone_certificate_tampered_nested_report",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut report: serde_json::Value =
            load_json(&paths.cornerstone_certificate_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested cornerstone-certificate reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_cornerstone_certificate_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "cornerstone_certificate_retimed_nested_report",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_capstone_keystone_cornerstone_or_foundation_identity_fields() {
        {
            let fixture = fake_command_fixture(
                "foundation_receipt_tampered_capstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "foundation_receipt_tampered_keystone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "foundation_receipt_tampered_cornerstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "foundation_receipt_tampered_foundation_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    #[test]
    fn verify_rejects_tampered_foundation_receipt_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "foundation_receipt_tampered_foundation_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = foundation_receipt_paths(&fixture.foundation_receipt_session_dir);
            let mut status: PackageFoundationReceiptStatus = load_json(&paths.status_path).unwrap();
            status.foundation_receipt_summary = "tampered foundation summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_foundation_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "foundation_receipt_tampered_foundation_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = foundation_receipt_paths(&fixture.foundation_receipt_session_dir);
            let mut status: PackageFoundationReceiptStatus = load_json(&paths.status_path).unwrap();
            status.foundation_receipt_algorithm = "tampered-foundation-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_foundation_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageFoundationReceipt,
                cornerstone_certificate_session_dir: self
                    .cornerstone_certificate_session_dir
                    .clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.foundation_receipt_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageFoundationReceipt,
                cornerstone_certificate_session_dir: self
                    .cornerstone_certificate_session_dir
                    .clone(),
                confirmed_decision_packet_session_dir: Some(
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        persist_json(
            ...
            &stored_cornerstone_certificate_session_fixture(
                &keystone_receipt_session_dir,
                &finality_receipt_session_dir,
                &archive_receipt_session_dir,
                &registry_entry_session_dir,
        ...
        persist_json(
            ...
            &stored_cornerstone_certificate_status_fixture(&verify_report),
        )
        .unwrap();
        persist_json(
            &cornerstone_certificate_session_dir.join(
        ...
        bind_cornerstone_certificate_report_to_fixture(
            &mut verify_report,
            &keystone_receipt_session_dir,
            &finality_receipt_session_dir,
            &archive_receipt_session_dir,
        ...
        let verify_path = fixture_dir.join("cornerstone-certificate.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_cornerstone_certificate_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_cornerstone_certificate"),
            &verify_path,
            &keystone_receipt_session_dir,
            &decision_packet_session_dir,
            &cornerstone_certificate_session_dir,
    ...
    fn bind_cornerstone_certificate_report_to_fixture(
        ...
        _cornerstone_certificate_session_dir: &Path,
        package_dir: &Path,
        install_root: &Path,
        ...
        object.insert(
            "verify_keystone_receipt_command_summary".to_string(),
            json!(format!(
        ...
        object.insert(
            ...
            json!("cornerstone-certificate verify statement"),
        );
    }

    fn cornerstone_certificate_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_cornerstone_certificate",
            "verdict": "tiny_live_package_cornerstone_certificate_verify_ok",
            "reason": "cornerstone-certificate chain is coherent",
            "result": result,
    ...
    fn capstone_fixture_verify_report(
        ...
        cornerstone_certificate_verify_report(
            result,
            handoff_bundle_result,
            decision_packet_result,
            execute_frozen_result,
    ...
    fn write_fake_cornerstone_certificate_verify_command(
        path: &Path,
        verify_path: &Path,
        expected_keystone_receipt_session_dir: &Path,
        expected_decision_packet_session_dir: &Path,
        cornerstone_certificate_session_dir: &Path,
    ) {
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --keystone-receipt-session-dir {} \"*) : ;;\n  *) echo 'unexpected keystone-receipt-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected cornerstone-certificate session dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --verify-live-package-cornerstone-certificate \"*) : ;;\n  *) echo 'missing cornerstone verify mode' >&2; exit 1;;\nesac\ncat '{}'\n",
            shell_escape_path(expected_keystone_receipt_session_dir),
    ...
    fn stored_cornerstone_certificate_session_fixture(
        keystone_receipt_session_dir: &Path,
        finality_receipt_session_dir: &Path,
        archive_receipt_session_dir: &Path,
        registry_entry_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("cornerstone-certificate statement"),
        );
        Value::Object(object)
    }

    fn stored_cornerstone_certificate_status_fixture(verify_report: &Value) -> Value {
        let mut object = Map::new();
        object.insert(
            "result".to_string(),
            json!(verify_report
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("cornerstone-certificate status statement"),
        );
        Value::Object(object)
    }
}
