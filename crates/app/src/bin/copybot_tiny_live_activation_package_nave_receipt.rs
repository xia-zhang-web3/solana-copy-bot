...
use copybot_app::tiny_live_activation::package_sanctuary_certificate_nave_receipt;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_nave_receipt --sanctuary-certificate-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-nave-receipt | --render-live-package-nave-receipt | --run-live-package-nave-receipt | --verify-live-package-nave-receipt)";
const NAVE_RECEIPT_SESSION_VERSION: &str = "1";
const NAVE_RECEIPT_STATUS_VERSION: &str = "1";
const NAVE_RECEIPT_ALGORITHM: &str = "sha256";
const NAVE_RECEIPT_SESSION_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    sanctuary_certificate_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackageNaveReceipt,
    RenderLivePackageNaveReceipt,
    RunLivePackageNaveReceipt,
    VerifyLivePackageNaveReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageNaveReceiptVerdict {
    TinyLivePackageNaveReceiptPlanReady,
    TinyLivePackageNaveReceiptRendered,
    TinyLivePackageNaveReceiptRefusedNowByStage3,
    TinyLivePackageNaveReceiptRefusedNowByPreActivationGate,
    TinyLivePackageNaveReceiptRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageNaveReceiptReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageNaveReceiptVerifyOk,
    TinyLivePackageNaveReceiptVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageNaveReceiptResult {
    RefusedNowByStage3,
...
struct PackageNaveReceiptStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageNaveReceiptSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageNaveReceiptStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackageNaveReceiptResult,
    reason: String,
    ...
    sanctuary_certificate_step: Option<PackageNaveReceiptStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageNaveReceiptPaths {
...
struct PackageNaveReceiptReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageNaveReceiptVerdict,
    reason: String,
    sanctuary_certificate_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut sanctuary_certificate_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--sanctuary-certificate-session-dir" => {
                sanctuary_certificate_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--sanctuary-certificate-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--plan-live-package-nave-receipt" => {
                set_mode(&mut mode, Mode::PlanLivePackageNaveReceipt, arg.as_str())?
            }
            "--render-live-package-nave-receipt" => {
                set_mode(&mut mode, Mode::RenderLivePackageNaveReceipt, arg.as_str())?
            }
            "--run-live-package-nave-receipt" => {
                set_mode(&mut mode, Mode::RunLivePackageNaveReceipt, arg.as_str())?
            }
            "--verify-live-package-nave-receipt" => {
                set_mode(&mut mode, Mode::VerifyLivePackageNaveReceipt, arg.as_str())?
            }
    ...
    if matches!(mode, Mode::RenderLivePackageNaveReceipt) && output_path.is_none() {
        bail!("missing --output for --render-live-package-nave-receipt");
    }
    if matches!(
        mode,
        Mode::RunLivePackageNaveReceipt | Mode::VerifyLivePackageNaveReceipt
    ) && session_dir.is_none()
    {
    ...
    let sanctuary_certificate_session_dir = sanctuary_certificate_session_dir
        .ok_or_else(|| anyhow!("missing --sanctuary-certificate-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageNaveReceipt => plan_live_package_nave_receipt_report(&config)?,
        Mode::RenderLivePackageNaveReceipt => render_live_package_nave_receipt_report(&config)?,
        Mode::RunLivePackageNaveReceipt => run_live_package_nave_receipt_report(&config)?,
        Mode::VerifyLivePackageNaveReceipt => verify_live_package_nave_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_live_package_nave_receipt_report(config: &Config) -> Result<PackageNaveReceiptReport> {
    let (sanctuary_certificate_contract, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_sanctuary_certificate_nave_receipt::verify_live_package_sanctuary_certificate_for_nave_receipt(
                    &config.sanctuary_certificate_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_sanctuary_certificate(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_sanctuary_certificate_nave_receipt::load_live_package_sanctuary_certificate_contract_for_nave_receipt(
                            &config.sanctuary_certificate_session_dir,
                        )?;
                        let classification =
                            classify_sanctuary_certificate_contract(&raw_contract, false);
    ...
    let nave_receipt_sha256 = compute_nave_receipt_sha256(
        &result,
        sanctuary_certificate_contract
        ...
        sanctuary_certificate_contract
            ...
            .as_deref(),
        &sanctuary_certificate_contract.reviewed_frozen_live_cutover_controller_command_summary,
        &sanctuary_certificate_contract.chain_fingerprint_sha256,
        &sanctuary_certificate_contract.chain_fingerprint_algorithm,
        &sanctuary_certificate_contract.release_capsule_digest_manifest_sha256,
        sanctuary_certificate_contract.release_capsule_digest_manifest_entry_count,
        &sanctuary_certificate_contract.release_capsule_digest_algorithm,
        &sanctuary_certificate_contract.ledger_seal_sha256,
        &sanctuary_certificate_contract.ledger_seal_algorithm,
        &sanctuary_certificate_contract.registry_entry_sha256,
        &sanctuary_certificate_contract.registry_entry_algorithm,
        &sanctuary_certificate_contract.filing_certificate_sha256,
        &sanctuary_certificate_contract.filing_certificate_algorithm,
        &sanctuary_certificate_contract.archive_receipt_sha256,
        &sanctuary_certificate_contract.archive_receipt_algorithm,
        &sanctuary_certificate_contract.closure_certificate_summary,
        &sanctuary_certificate_contract.closure_certificate_sha256,
        &sanctuary_certificate_contract.closure_certificate_algorithm,
        &sanctuary_certificate_contract.finality_receipt_sha256,
        &sanctuary_certificate_contract.finality_receipt_algorithm,
        &sanctuary_certificate_contract.consummation_record_sha256,
        &sanctuary_certificate_contract.consummation_record_algorithm,
        &sanctuary_certificate_contract.completion_certificate_summary,
        &sanctuary_certificate_contract.completion_certificate_sha256,
        &sanctuary_certificate_contract.completion_certificate_algorithm,
        &sanctuary_certificate_contract.summit_certificate_summary,
        &sanctuary_certificate_contract.summit_certificate_sha256,
        &sanctuary_certificate_contract.summit_certificate_algorithm,
        &sanctuary_certificate_contract.culmination_receipt_summary,
        &sanctuary_certificate_contract.culmination_receipt_sha256,
        &sanctuary_certificate_contract.culmination_receipt_algorithm,
        &sanctuary_certificate_contract.pinnacle_receipt_summary,
        &sanctuary_certificate_contract.pinnacle_receipt_sha256,
        &sanctuary_certificate_contract.pinnacle_receipt_algorithm,
        &sanctuary_certificate_contract.capstone_certificate_summary,
        &sanctuary_certificate_contract.capstone_certificate_sha256,
        &sanctuary_certificate_contract.capstone_certificate_algorithm,
        &sanctuary_certificate_contract.keystone_receipt_summary,
        &sanctuary_certificate_contract.keystone_receipt_sha256,
        &sanctuary_certificate_contract.keystone_receipt_algorithm,
        &sanctuary_certificate_contract.cornerstone_certificate_summary,
        &sanctuary_certificate_contract.cornerstone_certificate_sha256,
        &sanctuary_certificate_contract.cornerstone_certificate_algorithm,
        &sanctuary_certificate_contract.foundation_receipt_summary,
        &sanctuary_certificate_contract.foundation_receipt_sha256,
        &sanctuary_certificate_contract.foundation_receipt_algorithm,
        &sanctuary_certificate_contract.bedrock_certificate_summary,
        &sanctuary_certificate_contract.bedrock_certificate_sha256,
        &sanctuary_certificate_contract.bedrock_certificate_algorithm,
        &sanctuary_certificate_contract.basal_receipt_summary,
        &sanctuary_certificate_contract.basal_receipt_sha256,
        &sanctuary_certificate_contract.basal_receipt_algorithm,
        &sanctuary_certificate_contract.substructure_certificate_summary,
        &sanctuary_certificate_contract.substructure_certificate_sha256,
        &sanctuary_certificate_contract.substructure_certificate_algorithm,
        &sanctuary_certificate_contract.plinth_receipt_summary,
        &sanctuary_certificate_contract.plinth_receipt_sha256,
        &sanctuary_certificate_contract.plinth_receipt_algorithm,
        &sanctuary_certificate_contract.pedestal_certificate_summary,
        &sanctuary_certificate_contract.pedestal_certificate_sha256,
        &sanctuary_certificate_contract.pedestal_certificate_algorithm,
        &sanctuary_certificate_contract.dais_receipt_summary,
        &sanctuary_certificate_contract.dais_receipt_sha256,
        &sanctuary_certificate_contract.dais_receipt_algorithm,
        &sanctuary_certificate_contract.rostrum_certificate_summary,
        &sanctuary_certificate_contract.rostrum_certificate_sha256,
        &sanctuary_certificate_contract.rostrum_certificate_algorithm,
        &sanctuary_certificate_contract.podium_receipt_summary,
        &sanctuary_certificate_contract.podium_receipt_sha256,
        &sanctuary_certificate_contract.podium_receipt_algorithm,
        &sanctuary_certificate_contract.lectern_certificate_summary,
        &sanctuary_certificate_contract.lectern_certificate_sha256,
        &sanctuary_certificate_contract.lectern_certificate_algorithm,
        &sanctuary_certificate_contract.pulpit_receipt_summary,
        &sanctuary_certificate_contract.pulpit_receipt_sha256,
        &sanctuary_certificate_contract.pulpit_receipt_algorithm,
        &sanctuary_certificate_contract.chancel_certificate_summary,
        &sanctuary_certificate_contract.chancel_certificate_sha256,
        &sanctuary_certificate_contract.chancel_certificate_algorithm,
        &sanctuary_certificate_contract.apse_receipt_summary,
        &sanctuary_certificate_contract.apse_receipt_sha256,
        &sanctuary_certificate_contract.apse_receipt_algorithm,
        &sanctuary_certificate_contract.sanctuary_certificate_summary,
        &sanctuary_certificate_contract.sanctuary_certificate_sha256,
        &sanctuary_certificate_contract.sanctuary_certificate_algorithm,
    )?;

    let lectern_certificate_contract = sanctuary_certificate_contract.clone();
    Ok(PackageNaveReceiptReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_nave_receipt".to_string(),
        verdict:
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptPlanReady,
        reason: format!(
            "sanctuary-certificate-native nave receipt is explicit for verified sanctuary-certificate session {}; this command stays archival and read-only while freezing one final pennant-seal identity over the fully culminated chain",
            config.sanctuary_certificate_session_dir.display()
        ),
        sanctuary_certificate_session_dir: config.sanctuary_certificate_session_dir.display().to_string(),
        ...
        sanctuary_certificate_session_path: None,
        sanctuary_certificate_status_path: None,
        archived_sanctuary_certificate_report_path: None,
        result: Some(result),
        ...
        verify_sanctuary_certificate_command_summary: Some(verify_sanctuary_certificate_command_summary(
            &config.sanctuary_certificate_session_dir,
        )),
        ...
        chancel_certificate_summary: Some(
            sanctuary_certificate_contract.chancel_certificate_summary.clone(),
        ),
        chancel_certificate_sha256: Some(
            sanctuary_certificate_contract.chancel_certificate_sha256.clone(),
        ),
        chancel_certificate_algorithm: Some(
            sanctuary_certificate_contract.chancel_certificate_algorithm.clone(),
        ),
        apse_receipt_summary: Some(
            sanctuary_certificate_contract.apse_receipt_summary.clone(),
        ),
        apse_receipt_sha256: Some(
            sanctuary_certificate_contract.apse_receipt_sha256.clone(),
        ),
        apse_receipt_algorithm: Some(
            sanctuary_certificate_contract.apse_receipt_algorithm.clone(),
        ),
        sanctuary_certificate_summary: Some(
            sanctuary_certificate_contract.sanctuary_certificate_summary.clone(),
        ),
        sanctuary_certificate_sha256: Some(
            sanctuary_certificate_contract.sanctuary_certificate_sha256.clone(),
        ),
        sanctuary_certificate_algorithm: Some(
            sanctuary_certificate_contract.sanctuary_certificate_algorithm.clone(),
        ),
        nave_receipt_summary: Some(nave_receipt_summary(
            classification.0,
            &sanctuary_certificate_contract.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        nave_receipt_sha256: Some(nave_receipt_sha256),
        nave_receipt_algorithm: Some(NAVE_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: NAVE_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_nave_receipt_report(config: &Config) -> Result<PackageNaveReceiptReport> {
    let plan = plan_live_package_nave_receipt_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_nave_receipt --sanctuary-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-nave-receipt --json\ncopybot_tiny_live_activation_package_nave_receipt --sanctuary-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-nave-receipt --json\n",
        shell_escape_path(&config.sanctuary_certificate_session_dir),
    ...
    Ok(PackageNaveReceiptReport {
        generated_at: Utc::now(),
        mode: "render_live_package_nave_receipt".to_string(),
        verdict:
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptRendered,
        reason: format!(
            "rendered nave-receipt run/verify script to {}; the primary input remains the immutable sanctuary-certificate session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: NAVE_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_nave_receipt_report(config: &Config) -> Result<PackageNaveReceiptReport> {
    let session_dir = config
        .session_dir
        .as_ref()
        .ok_or_else(|| anyhow!("missing --session-dir for run mode"))?;
    let verified_lectern_certificate = match package_sanctuary_certificate_nave_receipt::verify_live_package_sanctuary_certificate_for_nave_receipt(
        &config.sanctuary_certificate_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify sanctuary-certificate session under {}: {error}",
                    config.sanctuary_certificate_session_dir.display()
                ),
            ))
        }
    ...
    .with_context(|| {
        format!(
            "nave-receipt session dir {} overlaps the managed live target surface derived from verified sanctuary-certificate install_root {}",
            session_dir.display(),
            verified_lectern_certificate.contract.install_root.display()
        )
    })?;
    ensure_clean_nave_receipt_session_dir(session_dir)?;
    let paths = nave_receipt_paths(session_dir);
    let classification = classify_verified_sanctuary_certificate(&verified_lectern_certificate);
    ...
    let sanctuary_certificate_step = Some(step_artifact(
        &paths.sanctuary_certificate_report_path,
        "verify_live_package_sanctuary_certificate",
        &verified_lectern_certificate.verdict,
        &verified_lectern_certificate.reason,
        verified_lectern_certificate.generated_at,
    ));
    let status = PackageNaveReceiptStatus {
        status_version: NAVE_RECEIPT_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        sanctuary_certificate_session_dir: config
            .sanctuary_certificate_session_dir
            .display()
            .to_string(),
        ...
        sanctuary_certificate_algorithm: verified_lectern_certificate
            ...
            .sanctuary_certificate_algorithm
            .clone(),
        nave_receipt_summary: nave_receipt_summary(
            result,
            &verified_lectern_certificate
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        nave_receipt_sha256: nave_receipt_sha256.clone(),
        nave_receipt_algorithm: NAVE_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: NAVE_RECEIPT_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_nave_receipt_report(config: &Config) -> Result<PackageNaveReceiptReport> {
    let session_dir = config
        .session_dir
        .as_ref()
        .ok_or_else(|| anyhow!("missing --session-dir for verify mode"))?;
    let paths = nave_receipt_paths(session_dir);
    let session: PackageNaveReceiptSession = load_json(&paths.session_path)?;
    let status: PackageNaveReceiptStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.sanctuary_certificate_session_dir,
        &config
            .sanctuary_certificate_session_dir
            .display()
            .to_string(),
        "nave-receipt sanctuary_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "nave-receipt session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "nave-receipt status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.sanctuary_certificate_session_dir,
        &config
            .sanctuary_certificate_session_dir
            .display()
            .to_string(),
        "nave-receipt status sanctuary_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "nave-receipt substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "nave-receipt status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "nave-receipt bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "nave-receipt status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "nave-receipt foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "nave-receipt status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(stored_lectern_certificate_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested sanctuary-certificate report {}",
                paths.sanctuary_certificate_report_path.display()
            )
        })?;
    let verified_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(verified_lectern_certificate.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested sanctuary-certificate report for {}",
                    config.sanctuary_certificate_session_dir.display()
                )
            },
        )?;
        ...
        != verified_lectern_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "nave-receipt archived sanctuary-certificate report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_lectern_certificate_report_view.generated_at,
            verified_lectern_certificate_report_view.generated_at
        ));
    }
    let expected_sanctuary_certificate_step = step_artifact(
        &paths.sanctuary_certificate_report_path,
        &verified_lectern_certificate_report_view.mode,
        &verified_lectern_certificate_report_view.verdict,
        &verified_lectern_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_sanctuary_certificate_step,
        "nave-receipt sanctuary_certificate_step",
        &mut mismatches,
    );
    if verified_lectern_certificate.verdict != "tiny_live_package_sanctuary_certificate_verify_ok" {
        mismatches.push(format!(
            "nave-receipt nested sanctuary-certificate verification is non-green: {}",
            verified_lectern_certificate.reason
        ));
    }

    let classification = classify_verified_sanctuary_certificate(&verified_lectern_certificate);
    let expected_nave_receipt_sha256 = compute_nave_receipt_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_lectern_certificate.contract,
        classification.0,
        &expected_nave_receipt_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "nave-receipt session contract does not match freshly verified sanctuary-certificate truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_lectern_certificate.contract,
        classification,
        expected_sanctuary_certificate_step.clone(),
        &expected_nave_receipt_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "nave-receipt status contract does not match freshly verified sanctuary-certificate truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_lectern_certificate.contract,
        TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptVerifyOk,
        &expected_status,
    ))
}
...
fn classify_sanctuary_certificate_contract(
    contract: &package_sanctuary_certificate_nave_receipt::LivePackageSanctuaryCertificateContractView,
    assume_verified: bool,
) -> (
    LivePackageNaveReceiptResult,
    TinyLivePackageNaveReceiptVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageNaveReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptRefusedNowByInvalidOrDriftedContract,
            "the sanctuary-certificate chain is not verified, so this nave receipt remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageNaveReceiptResult::RefusedNowByStage3,
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptRefusedNowByStage3,
            "the immutable sanctuary certificate remains refused now by Stage 3, so this nave receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageNaveReceiptResult::RefusedNowByPreActivationGate,
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptRefusedNowByPreActivationGate,
            "the immutable sanctuary certificate remains refused now by the pre-activation gate, so this nave receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageNaveReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable sanctuary certificate is coherent and this nave receipt is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageNaveReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptRefusedNowByInvalidOrDriftedContract,
            "the verified sanctuary-certificate chain is invalid, drifted, or non-runnable, so this nave receipt remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_sanctuary_certificate(
    verified: &package_sanctuary_certificate_nave_receipt::VerifiedLivePackageSanctuaryCertificateNaveReceiptStep,
) -> (
    LivePackageNaveReceiptResult,
    TinyLivePackageNaveReceiptVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_sanctuary_certificate_verify_ok" {
        return (
            LivePackageNaveReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptRefusedNowByInvalidOrDriftedContract,
            "the immutable sanctuary certificate did not verify cleanly, so this nave receipt remains refused"
                .to_string(),
        );
    }
    classify_sanctuary_certificate_contract(&verified.contract, true)
}

fn nave_receipt_summary(
    result: LivePackageNaveReceiptResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageNaveReceiptResult::RefusedNowByStage3 => {
            "Sanctuary certificate outcome: the immutable culminated chain remains refused now by Stage 3; archive this final standard seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageNaveReceiptResult::RefusedNowByPreActivationGate => {
            "Sanctuary certificate outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final standard seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageNaveReceiptResult::RefusedNowByInvalidOrDriftedContract => {
            "Sanctuary certificate outcome: the immutable culminated chain is invalid or drifted; archive this final standard seal for audit only and mint a new coherent receipt before any later execution handoff.".to_string()
        }
        LivePackageNaveReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Sanctuary certificate outcome: this immutable standard seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn nave_receipt_paths(session_dir: &Path) -> PackageNaveReceiptPaths {
    PackageNaveReceiptPaths {
        session_path: session_dir.join("tiny_live_activation_package_nave_receipt.session.json"),
        status_path: session_dir.join("tiny_live_activation_package_nave_receipt.status.json"),
        sanctuary_certificate_report_path: session_dir
            .join("tiny_live_activation_package_nave_receipt.sanctuary_certificate.report.json"),
    }
}

fn ensure_clean_nave_receipt_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing nave-receipt session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating nave-receipt session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageNaveReceiptSession {
    PackageNaveReceiptSession {
        session_version: NAVE_RECEIPT_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        sanctuary_certificate_session_dir: config
            .sanctuary_certificate_session_dir
            .display()
        ...
        session_dir: session_dir.display().to_string(),
        verify_sanctuary_certificate_command_summary: verify_sanctuary_certificate_command_summary(
            &config.sanctuary_certificate_session_dir,
        ),
        ...
        nave_receipt_sha256: nave_receipt_sha256.to_string(),
        nave_receipt_algorithm: NAVE_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: NAVE_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_session_from_verified(
    ...
    PackageNaveReceiptSession {
        planned_at,
        ..planned_session(config, session_dir, contract, result, nave_receipt_sha256)
    }
}
...
fn expected_status_from_verified(
    ...
    sanctuary_certificate_step: PackageNaveReceiptStepArtifact,
    nave_receipt_sha256: &str,
) -> PackageNaveReceiptStatus {
    PackageNaveReceiptStatus {
        status_version: NAVE_RECEIPT_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        sanctuary_certificate_step: Some(sanctuary_certificate_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageNaveReceiptPaths,
    contract: &package_sanctuary_certificate_nave_receipt::LivePackageSanctuaryCertificateContractView,
    verdict: TinyLivePackageNaveReceiptVerdict,
    status: &PackageNaveReceiptStatus,
) -> PackageNaveReceiptReport {
    PackageNaveReceiptReport {
        generated_at: Utc::now(),
        mode: if verdict == TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptVerifyOk {
            "verify_live_package_nave_receipt".to_string()
        } else {
            "run_live_package_nave_receipt".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        sanctuary_certificate_session_path: Some(paths.session_path.display().to_string()),
        sanctuary_certificate_status_path: Some(paths.status_path.display().to_string()),
        archived_sanctuary_certificate_report_path: Some(
            paths
        ...
        sanctuary_certificate_step: status.sanctuary_certificate_step.clone(),
        verify_sanctuary_certificate_command_summary: Some(
            verify_sanctuary_certificate_command_summary(&config.sanctuary_certificate_session_dir),
        ),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
            contract
                .reviewed_frozen_live_cutover_controller_command_summary
        ...
        explicit_statement: if verdict
            == TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptVerifyOk
        {
            NAVE_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageNaveReceiptReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageNaveReceipt => {
                "plan_live_package_nave_receipt".to_string()
            }
            Mode::RenderLivePackageNaveReceipt => {
                "render_live_package_nave_receipt".to_string()
            }
            Mode::RunLivePackageNaveReceipt => {
                "run_live_package_nave_receipt".to_string()
            }
            Mode::VerifyLivePackageNaveReceipt => {
                "verify_live_package_nave_receipt".to_string()
            }
        },
        verdict,
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable nave receipt refused before persisting because the sanctuary-certificate artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "sanctuary-certificate-native nave-receipt verification failed".to_string()
    });
    PackageNaveReceiptReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_nave_receipt".to_string(),
        verdict: TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptVerifyInvalid,
        reason,
        sanctuary_certificate_session_dir: config
            .sanctuary_certificate_session_dir
            .display()
        ...
        explicit_statement: NAVE_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageNaveReceiptStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from sanctuary-certificate status"
        ));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageNaveReceiptStepArtifact {
    PackageNaveReceiptStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageNaveReceiptStepArtifact>,
    expected: &PackageNaveReceiptStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_nave_receipt_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&NaveReceiptPayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents)
        .with_context(|| format!("failed writing nave-receipt script to {}", path.display()))?;
    #[cfg(unix)]
    {
...
fn render_report_lines(report: &PackageNaveReceiptReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "sanctuary_certificate_session_dir={}",
            report.sanctuary_certificate_session_dir
        ),
    ];
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_nave_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify = verify_live_package_nave_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptVerifyOk
        );
    ...
    #[test]
    fn drifted_sanctuary_certificate_contract_is_refused() {
        let fixture = fake_command_fixture(
            "sanctuary_certificate_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_sanctuary_certificate",
                "verdict": "tiny_live_package_sanctuary_certificate_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        .unwrap();
        let overlap_session_dir = managed_paths.runtime_dir.join("nave-receipt-session");
        ...
        let error = run_live_package_nave_receipt_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = nave_receipt_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_sanctuary_certificate_step_path() {
        let fixture = fake_command_fixture(
            "nave_receipt_tampered_sanctuary_certificate_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_nave_receipt_report(&fixture.run_config()).unwrap();
        let paths = nave_receipt_paths(&fixture.sanctuary_certificate_session_dir);
        let mut status: PackageNaveReceiptStatus = load_json(&paths.status_path).unwrap();
        status.sanctuary_certificate_step.as_mut().unwrap().path =
            "/tmp/foreign.sanctuary-certificate.report.json".to_string();
        persist_json(&paths.status_path, &status).unwrap();

        let verify = verify_live_package_nave_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageNaveReceiptVerdict::TinyLivePackageNaveReceiptVerifyInvalid
        );
    }
    ...
    fn verify_rejects_tampered_rostrum_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_rostrum_certificate_sha256",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            run_live_package_nave_receipt_report(&fixture.run_config()).unwrap();
            let paths = nave_receipt_paths(&fixture.sanctuary_certificate_session_dir);
            let mut session: PackageNaveReceiptSession = load_json(&paths.session_path).unwrap();
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_rostrum_certificate_summary",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_rostrum_certificate_algorithm",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = nave_receipt_paths(&fixture.sanctuary_certificate_session_dir);
        let mut status: PackageNaveReceiptStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageNaveReceiptResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_sanctuary_certificate_report_content() {
        let fixture = fake_command_fixture(
            "nave_receipt_tampered_nested_sanctuary_certificate_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut report: serde_json::Value =
            load_json(&paths.sanctuary_certificate_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested sanctuary-certificate reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_sanctuary_certificate_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "sanctuary_certificate_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_capstone_keystone_cornerstone_or_foundation_identity_fields()
    {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_bedrock_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_capstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_keystone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_cornerstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_foundation_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_basal_receipt_sha256() {
        let fixture = fake_command_fixture(
            "nave_receipt_tampered_basal_receipt_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_sha256() {
        let fixture = fake_command_fixture(
            "nave_receipt_tampered_substructure_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_substructure_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_substructure_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_plinth_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_plinth_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_plinth_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_plinth_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pedestal_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_pedestal_certificate_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_pedestal_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_pedestal_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_podium_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_podium_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_podium_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_podium_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_lectern_certificate_sha256() {
        let fixture = fake_command_fixture(
            "nave_receipt_tampered_lectern_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_lectern_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_lectern_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_lectern_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_bedrock_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_bedrock_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pulpit_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_pulpit_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_pulpit_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_pulpit_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_apse_receipt_sha256() {
        let fixture = fake_command_fixture(
            "nave_receipt_tampered_apse_receipt_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_chancel_certificate_sha256() {
        let fixture = fake_command_fixture(
            "nave_receipt_tampered_chancel_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_chancel_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_chancel_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_chancel_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_apse_receipt_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_apse_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_apse_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_sanctuary_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_sanctuary_certificate_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_sanctuary_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_sanctuary_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_nave_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_nave_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_nave_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "nave_receipt_tampered_nave_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        sanctuary_certificate_session_dir: PathBuf,
        input_sanctuary_certificate_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageNaveReceipt,
                sanctuary_certificate_session_dir: self
                    .input_sanctuary_certificate_session_dir
                    .clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.sanctuary_certificate_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageNaveReceipt,
                sanctuary_certificate_session_dir: self
                    .input_sanctuary_certificate_session_dir
                    .clone(),
                confirmed_decision_packet_session_dir: Some(
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let sanctuary_certificate_session_dir = fixture_dir.join("nave-receipt-session");
        let input_sanctuary_certificate_session_dir =
            fixture_dir.join("sanctuary-certificate-session");
        ...
        persist_json(
            ...
            &stored_sanctuary_certificate_session_fixture(
                &lectern_certificate_session_dir,
                &apse_receipt_session_dir,
                &rostrum_certificate_session_dir,
                &pedestal_certificate_session_dir,
        ...
        persist_json(
            ...
            &stored_sanctuary_certificate_status_fixture(
                &lectern_certificate_session_dir,
                &apse_receipt_session_dir,
                &rostrum_certificate_session_dir,
                &pedestal_certificate_session_dir,
        ...
        .unwrap();
        persist_json(
            &input_sanctuary_certificate_session_dir.join(
                "tiny_live_activation_package_sanctuary_certificate.apse_receipt.report.json",
            ),
            &json!({
        ...
        bind_sanctuary_certificate_report_to_fixture(
            &mut verify_report,
            &lectern_certificate_session_dir,
            &apse_receipt_session_dir,
            &rostrum_certificate_session_dir,
        ...
        let verify_path = fixture_dir.join("sanctuary-certificate.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_sanctuary_certificate_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_sanctuary_certificate"),
            &verify_path,
            &apse_receipt_session_dir,
            &decision_packet_session_dir,
            &input_sanctuary_certificate_session_dir,
    ...
    fn bind_sanctuary_certificate_report_to_fixture(
        ...
        object.insert(
            "apse_receipt_session_dir".to_string(),
        ...
        object.insert(
            "verify_apse_receipt_command_summary".to_string(),
            json!(format!(
        ...
        object.insert(
            ...
            json!("sanctuary-certificate verify statement"),
        );
    }

    fn sanctuary_certificate_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_sanctuary_certificate",
            "verdict": "tiny_live_package_sanctuary_certificate_verify_ok",
            "reason": "sanctuary-certificate chain is coherent",
            "result": result,
    ...
    fn bedrock_certificate_verify_report(
        ...
        sanctuary_certificate_verify_report(
            result,
            handoff_bundle_result,
            decision_packet_result,
            execute_frozen_result,
    ...
    fn stored_sanctuary_certificate_session_fixture(
        lectern_certificate_session_dir: &Path,
        apse_receipt_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("sanctuary-certificate statement"),
        );
        Value::Object(object)
    }

    fn stored_sanctuary_certificate_status_fixture(
        lectern_certificate_session_dir: &Path,
        apse_receipt_session_dir: &Path,
        _rostrum_certificate_session_dir: &Path,
        pedestal_certificate_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("sanctuary-certificate status statement"),
        );
        Value::Object(object)
    }
}
