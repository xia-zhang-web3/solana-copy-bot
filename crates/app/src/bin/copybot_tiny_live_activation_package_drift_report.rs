...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_drift_report --root <path> [--session-dir <path>] [--json] (--latest-vs-previous | --session-vs-previous | --verify-latest-vs-previous)";
...
const NOT_AUTHORIZED_SUMMARY: &str =
    "This tiny-live package drift-report surface only audits persisted package session/status/report artifacts. It stays planning-safe, does not authorize activation, and does not override the Stage 3 production gate.";
...
enum Mode {
    LatestVsPrevious,
    SessionVsPrevious,
    VerifyLatestVsPrevious,
}

#[derive(Debug, Clone, Copy, Serialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageDriftReportVerdict {
    TinyLivePackageDriftReportAvailable,
    TinyLivePackageDriftReportEmpty,
    TinyLivePackageDriftReportInvalid,
    TinyLivePackageDriftReportVerifyOk,
    TinyLivePackageDriftReportVerifyInvalid,
}
...
#[derive(Debug, Clone, Serialize)]
struct PackageDriftReport {
    mode: String,
    verdict: TinyLivePackageDriftReportVerdict,
    reason: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--latest-vs-previous" => {
                set_mode(&mut mode, Mode::LatestVsPrevious, "--latest-vs-previous")?
            }
            "--session-vs-previous" => {
                set_mode(&mut mode, Mode::SessionVsPrevious, "--session-vs-previous")?
            }
            "--verify-latest-vs-previous" => set_mode(
                &mut mode,
                Mode::VerifyLatestVsPrevious,
                "--verify-latest-vs-previous",
            )?,
            "--json" => json = true,
    ...
    let Some(mode) = mode else {
        bail!(
            "exactly one of --latest-vs-previous, --session-vs-previous, or --verify-latest-vs-previous is required"
        );
    };

    match mode {
        Mode::LatestVsPrevious | Mode::VerifyLatestVsPrevious => {
            if root.is_none() {
                bail!("missing required --root");
            }
            if session_dir.is_some() {
                bail!("--session-dir is only valid with --session-vs-previous");
            }
        }
        Mode::SessionVsPrevious => {
            if root.is_none() {
                bail!("missing required --root");
            }
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::LatestVsPrevious => build_latest_vs_previous_report(&config)?,
        Mode::SessionVsPrevious => build_session_vs_previous_report(&config)?,
        Mode::VerifyLatestVsPrevious => build_verify_latest_vs_previous_report(&config)?,
    };
    if config.json {
        serde_json::to_string_pretty(&report)
            .context("failed serializing tiny-live package drift report json")
    } else {
        Ok(render_drift_report_human(&report))
    }
}

fn build_latest_vs_previous_report(config: &Config) -> Result<PackageDriftReport> {
...
fn build_session_vs_previous_report(config: &Config) -> Result<PackageDriftReport> {
    let root = config.root.as_ref().expect("validated root");
    let sessions = scan_package_sessions(root)?;
...
fn build_verify_latest_vs_previous_report(config: &Config) -> Result<PackageDriftReport> {
    let root = config.root.as_ref().expect("validated root");
    let sessions = scan_package_sessions(root)?;
    let Some(latest) = select_latest_top_chain_candidate(&sessions) else {
        return Ok(empty_drift_report(
            "verify_latest_vs_previous",
            format!(
                "no persisted immutable tiny-live package sessions were found under {}",
                root.display()
            ),
...
fn empty_drift_report(mode: &str, reason: String) -> PackageDriftReport {
    PackageDriftReport {
        mode: mode.to_string(),
        verdict: TinyLivePackageDriftReportVerdict::TinyLivePackageDriftReportEmpty,
        reason,
        subject_chain: None,
        previous_chain: None,
...
fn missing_previous_report(
    ...
    let verdict = if verify_mode {
        TinyLivePackageDriftReportVerdict::TinyLivePackageDriftReportVerifyInvalid
    } else {
        TinyLivePackageDriftReportVerdict::TinyLivePackageDriftReportInvalid
    };
...
fn compare_sides(
    ...
    let comparable =
        subject_side.invalid_reasons.is_empty() && previous_side.invalid_reasons.is_empty();
    ...
    if verify_mode {
        if subject_side.top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        ...
        if previous_side.top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
            invalid_reasons.push(format!(
    ...
    let (verdict, reason) = if invalid_reasons.is_empty() {
        if verify_mode {
            (
                TinyLivePackageDriftReportVerdict::TinyLivePackageDriftReportVerifyOk,
                "the latest subject chain and the immediately previous comparable persisted top chain are comparable and show no top-identity, lineage, or manifest drift".to_string(),
            )
        } else if !top_identity_drift && !lineage_drift && !manifest_drift {
            (
                TinyLivePackageDriftReportVerdict::TinyLivePackageDriftReportAvailable,
                "the subject chain and the immediately previous comparable persisted top chain are identical at the bounded artifact level".to_string(),
            )
        } else {
            (
                TinyLivePackageDriftReportVerdict::TinyLivePackageDriftReportAvailable,
                "the subject chain and the immediately previous comparable persisted top chain are comparable but drift exists in top identity, lineage, or artifact manifest state".to_string(),
            )
        }
    } else if verify_mode {
        (
...
fn render_drift_report_human(report: &PackageDriftReport) -> String {
    let mut lines = vec![
        "event=copybot_tiny_live_activation_package_drift_report".to_string(),
...
mod tests {
    ...
    #[test]
    fn latest_vs_previous_reports_identical_chains_when_latest_and_previous_top_chains_match_exactly(
    ) {
        let mut fixture = DriftFixture::new("tiny_live_package_drift_identical");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);

        let report = build_latest_vs_previous_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::LatestVsPrevious,
        })
        .expect("latest-vs-previous report");
        ...
        assert_eq!(
            report
                .previous_chain
                .as_ref()
                .expect("previous chain")
                .top_session_path,
            fixture
                .step_dir("previous", TOP_ACCEPTED_PACKAGE_STEP)
                .display()
                .to_string()
        );
    }
    ...
    #[test]
    fn latest_vs_previous_reports_top_identity_drift_when_newest_top_identity_differs_from_previous(
    ) {
        let mut fixture = DriftFixture::new("tiny_live_package_drift_top_identity");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);
    ...
    #[test]
    fn latest_vs_previous_reports_lineage_drift_when_newest_nested_continuity_differs_from_previous(
    ) {
        let mut fixture = DriftFixture::new("tiny_live_package_drift_lineage");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);
    ...
    #[test]
    fn latest_vs_previous_reports_manifest_drift_when_newest_required_artifact_digest_differs_from_previous(
    ) {
        let mut fixture = DriftFixture::new("tiny_live_package_drift_manifest");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);
    ...
    #[test]
    fn verify_latest_vs_previous_fails_when_there_is_no_previous_comparable_top_chain() {
        let mut fixture = DriftFixture::new("tiny_live_package_drift_no_previous");
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);

        let report = build_verify_latest_vs_previous_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::VerifyLatestVsPrevious,
        })
        .expect("verify-latest-vs-previous report");
    ...
    #[test]
    fn verify_latest_vs_previous_fails_when_either_compared_chain_is_below_clerestory_certificate()
    {
        let mut fixture = DriftFixture::new("tiny_live_package_drift_below_clerestory");
    ...
    fn all_modes_stay_planning_safe_and_do_not_imply_activation_authorization() {
        let mut fixture = DriftFixture::new("tiny_live_package_drift_planning_safe");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);

        let latest = build_latest_vs_previous_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::LatestVsPrevious,
        ...
        .expect("latest-vs-previous report");
        let session = build_session_vs_previous_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: Some(
                fixture
                    .step_dir("latest", TOP_ACCEPTED_PACKAGE_STEP)
            ...
            json: true,
            mode: Mode::SessionVsPrevious,
        })
        .expect("session-vs-previous report");
        let verify = build_verify_latest_vs_previous_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::VerifyLatestVsPrevious,
