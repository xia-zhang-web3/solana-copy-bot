...
use copybot_app::tiny_live_activation::package_transept_certificate_choir_receipt;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_choir_receipt --transept-certificate-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-choir-receipt | --render-live-package-choir-receipt | --run-live-package-choir-receipt | --verify-live-package-choir-receipt)";
const CHOIR_RECEIPT_SESSION_VERSION: &str = "1";
const CHOIR_RECEIPT_STATUS_VERSION: &str = "1";
const CHOIR_RECEIPT_ALGORITHM: &str = "sha256";
const CHOIR_RECEIPT_SESSION_EXPLICIT_STATEMENT: &str =
...
const CHOIR_RECEIPT_STATUS_EXPLICIT_STATEMENT: &str =
    "this choir receipt is archival and read-only; it never executes or enables the frozen controller";
const CHOIR_RECEIPT_VERIFY_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    transept_certificate_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackageChoirReceipt,
    RenderLivePackageChoirReceipt,
    RunLivePackageChoirReceipt,
    VerifyLivePackageChoirReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageChoirReceiptVerdict {
    TinyLivePackageChoirReceiptPlanReady,
    TinyLivePackageChoirReceiptRendered,
    TinyLivePackageChoirReceiptRefusedNowByStage3,
    TinyLivePackageChoirReceiptRefusedNowByPreActivationGate,
    TinyLivePackageChoirReceiptRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageChoirReceiptReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageChoirReceiptVerifyOk,
    TinyLivePackageChoirReceiptVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageChoirReceiptResult {
    RefusedNowByStage3,
...
struct PackageChoirReceiptStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageChoirReceiptSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageChoirReceiptStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackageChoirReceiptResult,
    reason: String,
    ...
    transept_certificate_step: Option<PackageChoirReceiptStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageChoirReceiptPaths {
...
struct PackageChoirReceiptReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageChoirReceiptVerdict,
    reason: String,
    transept_certificate_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut transept_certificate_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--transept-certificate-session-dir" => {
                transept_certificate_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--transept-certificate-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--plan-live-package-choir-receipt" => {
                set_mode(&mut mode, Mode::PlanLivePackageChoirReceipt, arg.as_str())?
            }
            "--render-live-package-choir-receipt" => {
                set_mode(&mut mode, Mode::RenderLivePackageChoirReceipt, arg.as_str())?
            }
            "--run-live-package-choir-receipt" => {
                set_mode(&mut mode, Mode::RunLivePackageChoirReceipt, arg.as_str())?
            }
            "--verify-live-package-choir-receipt" => {
                set_mode(&mut mode, Mode::VerifyLivePackageChoirReceipt, arg.as_str())?
            }
    ...
    if matches!(mode, Mode::RenderLivePackageChoirReceipt) && output_path.is_none() {
        bail!("missing --output for --render-live-package-choir-receipt");
    }
    if matches!(
        mode,
        Mode::RunLivePackageChoirReceipt | Mode::VerifyLivePackageChoirReceipt
    ) && session_dir.is_none()
    {
    ...
    let transept_certificate_session_dir = transept_certificate_session_dir
        .ok_or_else(|| anyhow!("missing --transept-certificate-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageChoirReceipt => plan_live_package_choir_receipt_report(&config)?,
        Mode::RenderLivePackageChoirReceipt => render_live_package_choir_receipt_report(&config)?,
        Mode::RunLivePackageChoirReceipt => run_live_package_choir_receipt_report(&config)?,
        Mode::VerifyLivePackageChoirReceipt => verify_live_package_choir_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_live_package_choir_receipt_report(config: &Config) -> Result<PackageChoirReceiptReport> {
    let (nave_receipt_contract, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_transept_certificate_choir_receipt::verify_live_package_transept_certificate_for_choir_receipt(
                    &config.transept_certificate_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_transept_certificate(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_transept_certificate_choir_receipt::load_live_package_transept_certificate_contract_for_choir_receipt(
                            &config.transept_certificate_session_dir,
                        )?;
                        let classification =
                            classify_transept_certificate_contract(&raw_contract, false);
    ...
    let choir_receipt_sha256 = compute_choir_receipt_sha256(
        &result,
        nave_receipt_contract.handoff_bundle_result.as_deref(),
        nave_receipt_contract.decision_packet_result.as_deref(),
        nave_receipt_contract.execute_frozen_result.as_deref(),
        nave_receipt_contract
            .current_pre_activation_gate_verdict
            .as_deref(),
        nave_receipt_contract
            .current_pre_activation_gate_reason
            .as_deref(),
        &nave_receipt_contract.reviewed_frozen_live_cutover_controller_command_summary,
        &nave_receipt_contract.chain_fingerprint_sha256,
        &nave_receipt_contract.chain_fingerprint_algorithm,
        &nave_receipt_contract.release_capsule_digest_manifest_sha256,
        nave_receipt_contract.release_capsule_digest_manifest_entry_count,
        &nave_receipt_contract.release_capsule_digest_algorithm,
        &nave_receipt_contract.ledger_seal_sha256,
        &nave_receipt_contract.ledger_seal_algorithm,
        &nave_receipt_contract.registry_entry_sha256,
        &nave_receipt_contract.registry_entry_algorithm,
        &nave_receipt_contract.filing_certificate_sha256,
        &nave_receipt_contract.filing_certificate_algorithm,
        &nave_receipt_contract.archive_receipt_sha256,
        &nave_receipt_contract.archive_receipt_algorithm,
        &nave_receipt_contract.closure_certificate_summary,
        &nave_receipt_contract.closure_certificate_sha256,
        &nave_receipt_contract.closure_certificate_algorithm,
        &nave_receipt_contract.finality_receipt_sha256,
        &nave_receipt_contract.finality_receipt_algorithm,
        &nave_receipt_contract.consummation_record_sha256,
        &nave_receipt_contract.consummation_record_algorithm,
        &nave_receipt_contract.completion_certificate_summary,
        &nave_receipt_contract.completion_certificate_sha256,
        &nave_receipt_contract.completion_certificate_algorithm,
        &nave_receipt_contract.summit_certificate_summary,
        &nave_receipt_contract.summit_certificate_sha256,
        &nave_receipt_contract.summit_certificate_algorithm,
        &nave_receipt_contract.culmination_receipt_summary,
        &nave_receipt_contract.culmination_receipt_sha256,
        &nave_receipt_contract.culmination_receipt_algorithm,
        &nave_receipt_contract.pinnacle_receipt_summary,
        &nave_receipt_contract.pinnacle_receipt_sha256,
        &nave_receipt_contract.pinnacle_receipt_algorithm,
        &nave_receipt_contract.capstone_certificate_summary,
        &nave_receipt_contract.capstone_certificate_sha256,
        &nave_receipt_contract.capstone_certificate_algorithm,
        &nave_receipt_contract.keystone_receipt_summary,
        &nave_receipt_contract.keystone_receipt_sha256,
        &nave_receipt_contract.keystone_receipt_algorithm,
        &nave_receipt_contract.cornerstone_certificate_summary,
        &nave_receipt_contract.cornerstone_certificate_sha256,
        &nave_receipt_contract.cornerstone_certificate_algorithm,
        &nave_receipt_contract.foundation_receipt_summary,
        &nave_receipt_contract.foundation_receipt_sha256,
        &nave_receipt_contract.foundation_receipt_algorithm,
        &nave_receipt_contract.bedrock_certificate_summary,
        &nave_receipt_contract.bedrock_certificate_sha256,
        &nave_receipt_contract.bedrock_certificate_algorithm,
        &nave_receipt_contract.basal_receipt_summary,
        &nave_receipt_contract.basal_receipt_sha256,
        &nave_receipt_contract.basal_receipt_algorithm,
        &nave_receipt_contract.substructure_certificate_summary,
        &nave_receipt_contract.substructure_certificate_sha256,
        &nave_receipt_contract.substructure_certificate_algorithm,
        &nave_receipt_contract.plinth_receipt_summary,
        &nave_receipt_contract.plinth_receipt_sha256,
        &nave_receipt_contract.plinth_receipt_algorithm,
        &nave_receipt_contract.pedestal_certificate_summary,
        &nave_receipt_contract.pedestal_certificate_sha256,
        &nave_receipt_contract.pedestal_certificate_algorithm,
        &nave_receipt_contract.dais_receipt_summary,
        &nave_receipt_contract.dais_receipt_sha256,
        &nave_receipt_contract.dais_receipt_algorithm,
        &nave_receipt_contract.rostrum_certificate_summary,
        &nave_receipt_contract.rostrum_certificate_sha256,
        &nave_receipt_contract.rostrum_certificate_algorithm,
        &nave_receipt_contract.podium_receipt_summary,
        &nave_receipt_contract.podium_receipt_sha256,
        &nave_receipt_contract.podium_receipt_algorithm,
        &nave_receipt_contract.lectern_certificate_summary,
        &nave_receipt_contract.lectern_certificate_sha256,
        &nave_receipt_contract.lectern_certificate_algorithm,
        &nave_receipt_contract.pulpit_receipt_summary,
        &nave_receipt_contract.pulpit_receipt_sha256,
        &nave_receipt_contract.pulpit_receipt_algorithm,
        &nave_receipt_contract.chancel_certificate_summary,
        &nave_receipt_contract.chancel_certificate_sha256,
        &nave_receipt_contract.chancel_certificate_algorithm,
        &nave_receipt_contract.apse_receipt_summary,
        &nave_receipt_contract.apse_receipt_sha256,
        &nave_receipt_contract.apse_receipt_algorithm,
        &nave_receipt_contract.sanctuary_certificate_summary,
        &nave_receipt_contract.sanctuary_certificate_sha256,
        &nave_receipt_contract.sanctuary_certificate_algorithm,
        &nave_receipt_contract.nave_receipt_summary,
        &nave_receipt_contract.nave_receipt_sha256,
        &nave_receipt_contract.nave_receipt_algorithm,
        &nave_receipt_contract.transept_certificate_summary,
        &nave_receipt_contract.transept_certificate_sha256,
        &nave_receipt_contract.transept_certificate_algorithm,
    )?;

    let lectern_certificate_contract = nave_receipt_contract.clone();
    Ok(PackageChoirReceiptReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_choir_receipt".to_string(),
        verdict:
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptPlanReady,
        reason: format!(
            "transept-certificate-native choir receipt is explicit for verified transept-certificate session {}; this command stays archival and read-only while freezing one final ensign-seal identity over the fully culminated chain",
            config.transept_certificate_session_dir.display()
        ),
        transept_certificate_session_dir: config.transept_certificate_session_dir.display().to_string(),
        ...
        transept_certificate_session_path: None,
        transept_certificate_status_path: None,
        archived_transept_certificate_report_path: None,
        result: Some(result),
        ...
        verify_transept_certificate_command_summary: Some(verify_transept_certificate_command_summary(
            &config.transept_certificate_session_dir,
        )),
        ...
        chancel_certificate_summary: Some(
            nave_receipt_contract.chancel_certificate_summary.clone(),
        ),
        chancel_certificate_sha256: Some(
            nave_receipt_contract.chancel_certificate_sha256.clone(),
        ),
        chancel_certificate_algorithm: Some(
            nave_receipt_contract.chancel_certificate_algorithm.clone(),
        ),
        apse_receipt_summary: Some(
            nave_receipt_contract.apse_receipt_summary.clone(),
        ),
        apse_receipt_sha256: Some(
            nave_receipt_contract.apse_receipt_sha256.clone(),
        ),
        apse_receipt_algorithm: Some(
            nave_receipt_contract.apse_receipt_algorithm.clone(),
        ),
        sanctuary_certificate_summary: Some(
            nave_receipt_contract.sanctuary_certificate_summary.clone(),
        ),
        sanctuary_certificate_sha256: Some(
            nave_receipt_contract.sanctuary_certificate_sha256.clone(),
        ),
        sanctuary_certificate_algorithm: Some(
            nave_receipt_contract.sanctuary_certificate_algorithm.clone(),
        ),
        nave_receipt_summary: Some(
            nave_receipt_contract.nave_receipt_summary.clone(),
        ),
        nave_receipt_sha256: Some(
            nave_receipt_contract.nave_receipt_sha256.clone(),
        ),
        nave_receipt_algorithm: Some(
            nave_receipt_contract.nave_receipt_algorithm.clone(),
        ),
        transept_certificate_summary: Some(
            nave_receipt_contract.transept_certificate_summary.clone(),
        ),
        transept_certificate_sha256: Some(
            nave_receipt_contract.transept_certificate_sha256.clone(),
        ),
        transept_certificate_algorithm: Some(
            nave_receipt_contract.transept_certificate_algorithm.clone(),
        ),
        choir_receipt_summary: Some(choir_receipt_summary(
            classification.0,
            &nave_receipt_contract.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        choir_receipt_sha256: Some(choir_receipt_sha256),
        choir_receipt_algorithm: Some(CHOIR_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: CHOIR_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_choir_receipt_report(config: &Config) -> Result<PackageChoirReceiptReport> {
    let plan = plan_live_package_choir_receipt_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_choir_receipt --transept-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-choir-receipt --json\ncopybot_tiny_live_activation_package_choir_receipt --transept-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-choir-receipt --json\n",
        shell_escape_path(&config.transept_certificate_session_dir),
    ...
    Ok(PackageChoirReceiptReport {
        generated_at: Utc::now(),
        mode: "render_live_package_choir_receipt".to_string(),
        verdict:
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptRendered,
        reason: format!(
            "rendered choir-receipt run/verify script to {}; the primary input remains the immutable transept-certificate session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: CHOIR_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_choir_receipt_report(config: &Config) -> Result<PackageChoirReceiptReport> {
    let session_dir = config
        .session_dir
        .as_ref()
        .ok_or_else(|| anyhow!("missing --session-dir for run mode"))?;
    let verified_lectern_certificate = match package_transept_certificate_choir_receipt::verify_live_package_transept_certificate_for_choir_receipt(
        &config.transept_certificate_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify transept-certificate session under {}: {error}",
                    config.transept_certificate_session_dir.display()
                ),
            ))
        }
    ...
    .with_context(|| {
        format!(
            "choir-receipt session dir {} overlaps the managed live target surface derived from verified transept-certificate install_root {}",
            session_dir.display(),
            verified_lectern_certificate.contract.install_root.display()
        )
    })?;
    ensure_clean_choir_receipt_session_dir(session_dir)?;
    let paths = choir_receipt_paths(session_dir);
    let classification = classify_verified_transept_certificate(&verified_lectern_certificate);
    ...
    let choir_receipt_sha256 = compute_choir_receipt_sha256(
        ...
        &verified_lectern_certificate.contract.nave_receipt_summary,
        &verified_lectern_certificate.contract.nave_receipt_sha256,
        &verified_lectern_certificate.contract.nave_receipt_algorithm,
        &verified_lectern_certificate
            .contract
    ...
    let transept_certificate_step = Some(step_artifact(
        &paths.transept_certificate_report_path,
        "verify_live_package_transept_certificate",
        &verified_lectern_certificate.verdict,
        &verified_lectern_certificate.reason,
        verified_lectern_certificate.generated_at,
    ));
    let status = PackageChoirReceiptStatus {
        status_version: CHOIR_RECEIPT_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        transept_certificate_session_dir: config
            .transept_certificate_session_dir
            .display()
            .to_string(),
        ...
        apse_receipt_algorithm: verified_lectern_certificate
            ...
            .clone(),
        sanctuary_certificate_summary: verified_lectern_certificate
            .contract
            .sanctuary_certificate_summary
            .clone(),
        sanctuary_certificate_sha256: verified_lectern_certificate
            .contract
            .sanctuary_certificate_sha256
            .clone(),
        sanctuary_certificate_algorithm: verified_lectern_certificate
            .contract
            .sanctuary_certificate_algorithm
            .clone(),
        nave_receipt_summary: verified_lectern_certificate
            .contract
            .nave_receipt_summary
            .clone(),
        nave_receipt_sha256: verified_lectern_certificate
            .contract
            .nave_receipt_sha256
            .clone(),
        nave_receipt_algorithm: verified_lectern_certificate
            .contract
            .nave_receipt_algorithm
            .clone(),
        transept_certificate_summary: verified_lectern_certificate
            .contract
            .transept_certificate_summary
            .clone(),
        transept_certificate_sha256: verified_lectern_certificate
            .contract
            .transept_certificate_sha256
            .clone(),
        transept_certificate_algorithm: verified_lectern_certificate
            .contract
            .transept_certificate_algorithm
            .clone(),
        choir_receipt_summary: choir_receipt_summary(
            result,
            &verified_lectern_certificate
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        choir_receipt_sha256: choir_receipt_sha256.clone(),
        choir_receipt_algorithm: CHOIR_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: CHOIR_RECEIPT_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_choir_receipt_report(config: &Config) -> Result<PackageChoirReceiptReport> {
    let session_dir = config
        .session_dir
        .as_ref()
        .ok_or_else(|| anyhow!("missing --session-dir for verify mode"))?;
    let paths = choir_receipt_paths(session_dir);
    let session: PackageChoirReceiptSession = load_json(&paths.session_path)?;
    let status: PackageChoirReceiptStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.transept_certificate_session_dir,
        &config
            .transept_certificate_session_dir
            .display()
            .to_string(),
        "choir-receipt transept_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "choir-receipt session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "choir-receipt status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.transept_certificate_session_dir,
        &config
            .transept_certificate_session_dir
            .display()
            .to_string(),
        "choir-receipt status transept_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "choir-receipt substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "choir-receipt status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "choir-receipt bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "choir-receipt status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "choir-receipt foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "choir-receipt status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(stored_lectern_certificate_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested transept-certificate report {}",
                paths.transept_certificate_report_path.display()
            )
        })?;
    let verified_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(verified_lectern_certificate.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested transept-certificate report for {}",
                    config.transept_certificate_session_dir.display()
                )
            },
        )?;
        ...
        != verified_lectern_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "choir-receipt archived transept-certificate report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_lectern_certificate_report_view.generated_at,
            verified_lectern_certificate_report_view.generated_at
        ));
    }
    let expected_transept_certificate_step = step_artifact(
        &paths.transept_certificate_report_path,
        &verified_lectern_certificate_report_view.mode,
        &verified_lectern_certificate_report_view.verdict,
        &verified_lectern_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_transept_certificate_step,
        "choir-receipt transept_certificate_step",
        &mut mismatches,
    );
    if verified_lectern_certificate.verdict != "tiny_live_package_transept_certificate_verify_ok" {
        mismatches.push(format!(
            "choir-receipt nested transept-certificate verification is non-green: {}",
            verified_lectern_certificate.reason
        ));
    }

    let classification = classify_verified_transept_certificate(&verified_lectern_certificate);
    let expected_choir_receipt_sha256 = compute_choir_receipt_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_lectern_certificate.contract,
        classification.0,
        &expected_choir_receipt_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "choir-receipt session contract does not match freshly verified transept-certificate truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_lectern_certificate.contract,
        classification,
        expected_transept_certificate_step.clone(),
        &expected_choir_receipt_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "choir-receipt status contract does not match freshly verified transept-certificate truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_lectern_certificate.contract,
        TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptVerifyOk,
        &expected_status,
    ))
}

fn verify_transept_certificate_command_summary(transept_certificate_session_dir: &Path) -> String {
    format!(
...
fn classify_transept_certificate_contract(
    contract: &package_transept_certificate_choir_receipt::LivePackageTranseptCertificateContractView,
    assume_verified: bool,
) -> (
    LivePackageChoirReceiptResult,
    TinyLivePackageChoirReceiptVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageChoirReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptRefusedNowByInvalidOrDriftedContract,
            "the transept-certificate chain is not verified, so this choir receipt remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageChoirReceiptResult::RefusedNowByStage3,
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptRefusedNowByStage3,
            "the immutable transept certificate remains refused now by Stage 3, so this choir receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageChoirReceiptResult::RefusedNowByPreActivationGate,
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptRefusedNowByPreActivationGate,
            "the immutable transept certificate remains refused now by the pre-activation gate, so this choir receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageChoirReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable transept certificate is coherent and this choir receipt is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageChoirReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptRefusedNowByInvalidOrDriftedContract,
            "the verified transept-certificate chain is invalid, drifted, or non-runnable, so this choir receipt remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_transept_certificate(
    verified: &package_transept_certificate_choir_receipt::VerifiedLivePackageTranseptCertificateChoirReceiptStep,
) -> (
    LivePackageChoirReceiptResult,
    TinyLivePackageChoirReceiptVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_transept_certificate_verify_ok" {
        return (
            LivePackageChoirReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptRefusedNowByInvalidOrDriftedContract,
            "the immutable transept certificate did not verify cleanly, so this choir receipt remains refused"
                .to_string(),
        );
    }
    classify_transept_certificate_contract(&verified.contract, true)
}

fn choir_receipt_summary(
    result: LivePackageChoirReceiptResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageChoirReceiptResult::RefusedNowByStage3 => {
            "Choir receipt outcome: the immutable culminated chain remains refused now by Stage 3; archive this final ensign seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageChoirReceiptResult::RefusedNowByPreActivationGate => {
            "Choir receipt outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final ensign seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageChoirReceiptResult::RefusedNowByInvalidOrDriftedContract => {
            "Choir receipt outcome: the immutable culminated chain is invalid or drifted; archive this final ensign seal for audit only and mint a new coherent receipt before any later execution handoff.".to_string()
        }
        LivePackageChoirReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Choir receipt outcome: this immutable ensign seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn choir_receipt_paths(session_dir: &Path) -> PackageChoirReceiptPaths {
    PackageChoirReceiptPaths {
        session_path: session_dir.join("tiny_live_activation_package_choir_receipt.session.json"),
        status_path: session_dir.join("tiny_live_activation_package_choir_receipt.status.json"),
        transept_certificate_report_path: session_dir
            .join("tiny_live_activation_package_choir_receipt.transept_certificate.report.json"),
    }
}

fn ensure_clean_choir_receipt_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing choir-receipt session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating choir-receipt session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageChoirReceiptSession {
    PackageChoirReceiptSession {
        session_version: CHOIR_RECEIPT_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        transept_certificate_session_dir: config
            .transept_certificate_session_dir
            .display()
        ...
        session_dir: session_dir.display().to_string(),
        verify_transept_certificate_command_summary: verify_transept_certificate_command_summary(
            &config.transept_certificate_session_dir,
        ),
        ...
        choir_receipt_sha256: choir_receipt_sha256.to_string(),
        choir_receipt_algorithm: CHOIR_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: CHOIR_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_session_from_verified(
    ...
    PackageChoirReceiptSession {
        planned_at,
        ..planned_session(config, session_dir, contract, result, choir_receipt_sha256)
    }
}
...
fn expected_status_from_verified(
    ...
    transept_certificate_step: PackageChoirReceiptStepArtifact,
    choir_receipt_sha256: &str,
) -> PackageChoirReceiptStatus {
    PackageChoirReceiptStatus {
        status_version: CHOIR_RECEIPT_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        transept_certificate_step: Some(transept_certificate_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageChoirReceiptPaths,
    contract: &package_transept_certificate_choir_receipt::LivePackageTranseptCertificateContractView,
    verdict: TinyLivePackageChoirReceiptVerdict,
    status: &PackageChoirReceiptStatus,
) -> PackageChoirReceiptReport {
    PackageChoirReceiptReport {
        generated_at: Utc::now(),
        mode: if verdict == TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptVerifyOk
        {
            "verify_live_package_choir_receipt".to_string()
        } else {
            "run_live_package_choir_receipt".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        transept_certificate_session_path: Some(paths.session_path.display().to_string()),
        transept_certificate_status_path: Some(paths.status_path.display().to_string()),
        archived_transept_certificate_report_path: Some(
        ...
        transept_certificate_step: status.transept_certificate_step.clone(),
        verify_transept_certificate_command_summary: Some(
            verify_transept_certificate_command_summary(&config.transept_certificate_session_dir),
        ),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
            contract
                .reviewed_frozen_live_cutover_controller_command_summary
        ...
        explicit_statement: if verdict
            == TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptVerifyOk
        {
            CHOIR_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageChoirReceiptReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageChoirReceipt => {
                "plan_live_package_choir_receipt".to_string()
            }
            Mode::RenderLivePackageChoirReceipt => {
                "render_live_package_choir_receipt".to_string()
            }
            Mode::RunLivePackageChoirReceipt => {
                "run_live_package_choir_receipt".to_string()
            }
            Mode::VerifyLivePackageChoirReceipt => {
                "verify_live_package_choir_receipt".to_string()
            }
        },
        verdict,
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable choir receipt refused before persisting because the transept-certificate artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "transept-certificate-native choir-receipt verification failed".to_string()
    });
    PackageChoirReceiptReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_choir_receipt".to_string(),
        verdict: TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptVerifyInvalid,
        reason,
        transept_certificate_session_dir: config
            .transept_certificate_session_dir
            .display()
        ...
        explicit_statement: CHOIR_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageChoirReceiptStepArtifact>,
    expected_path: &Path,
    label: &str,
    mismatches: &mut Vec<String>,
) -> Result<serde_json::Value> {
    let Some(step) = step else {
        mismatches.push(format!("{label} is missing from choir-receipt status"));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageChoirReceiptStepArtifact {
    PackageChoirReceiptStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageChoirReceiptStepArtifact>,
    expected: &PackageChoirReceiptStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_choir_receipt_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&ChoirReceiptPayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents)
        .with_context(|| format!("failed writing choir-receipt script to {}", path.display()))?;
    #[cfg(unix)]
    {
...
fn render_report_lines(report: &PackageChoirReceiptReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "transept_certificate_session_dir={}",
            report.transept_certificate_session_dir
        ),
    ];
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_choir_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify = verify_live_package_choir_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptVerifyOk
        );
    ...
    #[test]
    fn drifted_transept_certificate_contract_is_refused() {
        let fixture = fake_command_fixture(
            "nave_receipt_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_transept_certificate",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        .unwrap();
        let overlap_session_dir = managed_paths.runtime_dir.join("choir-receipt-session");
        ...
        let error = run_live_package_choir_receipt_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = choir_receipt_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_transept_certificate_step_path() {
        let fixture = fake_command_fixture(
            "choir_receipt_tampered_transept_certificate_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_choir_receipt_report(&fixture.run_config()).unwrap();
        let paths = choir_receipt_paths(&fixture.transept_certificate_session_dir);
        let mut status: PackageChoirReceiptStatus = load_json(&paths.status_path).unwrap();
        status.transept_certificate_step.as_mut().unwrap().path =
        ...
        assert_eq!(
            verify.verdict,
            TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptVerifyInvalid
        );
    }
    ...
    fn verify_rejects_tampered_rostrum_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_rostrum_certificate_sha256",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            run_live_package_choir_receipt_report(&fixture.run_config()).unwrap();
            let paths = choir_receipt_paths(&fixture.transept_certificate_session_dir);
            let mut session: PackageChoirReceiptSession = load_json(&paths.session_path).unwrap();
            ...
            let verify =
                verify_live_package_choir_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageChoirReceiptVerdict::TinyLivePackageChoirReceiptVerifyInvalid
            );
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_rostrum_certificate_summary",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_rostrum_certificate_algorithm",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = choir_receipt_paths(&fixture.transept_certificate_session_dir);
        let mut status: PackageChoirReceiptStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageChoirReceiptResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_transept_certificate_report_content() {
        let fixture = fake_command_fixture(
            "choir_receipt_tampered_nested_nave_receipt_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut report: serde_json::Value =
            load_json(&paths.transept_certificate_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested transept-certificate reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_transept_certificate_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
    ...
    fn verify_rejects_tampered_bedrock_capstone_keystone_cornerstone_or_foundation_identity_fields()
    {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_bedrock_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_capstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_keystone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_cornerstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_foundation_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_basal_receipt_sha256() {
        let fixture = fake_command_fixture(
            "choir_receipt_tampered_basal_receipt_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_sha256() {
        let fixture = fake_command_fixture(
            "choir_receipt_tampered_substructure_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_substructure_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_substructure_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_plinth_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_plinth_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_plinth_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_plinth_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pedestal_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_pedestal_certificate_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_pedestal_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_pedestal_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_podium_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_podium_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_podium_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_podium_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_lectern_certificate_sha256() {
        let fixture = fake_command_fixture(
            "choir_receipt_tampered_lectern_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_lectern_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_lectern_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_lectern_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_bedrock_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_bedrock_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pulpit_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_pulpit_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_pulpit_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_pulpit_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_apse_receipt_sha256() {
        let fixture = fake_command_fixture(
            "choir_receipt_tampered_apse_receipt_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_chancel_certificate_sha256() {
        let fixture = fake_command_fixture(
            "choir_receipt_tampered_chancel_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_chancel_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_chancel_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_chancel_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_apse_receipt_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_apse_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_apse_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    #[test]
    fn verify_rejects_tampered_transept_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_transept_certificate_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = choir_receipt_paths(&fixture.transept_certificate_session_dir);
            let mut status: PackageChoirReceiptStatus = load_json(&paths.status_path).unwrap();
            status.transept_certificate_sha256 = "tampered-transept-certificate-sha256".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_choir_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_transept_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = choir_receipt_paths(&fixture.transept_certificate_session_dir);
            let mut status: PackageChoirReceiptStatus = load_json(&paths.status_path).unwrap();
            status.transept_certificate_summary =
                "tampered transept certificate summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_choir_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_transept_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let paths = choir_receipt_paths(&fixture.transept_certificate_session_dir);
            let mut status: PackageChoirReceiptStatus = load_json(&paths.status_path).unwrap();
            status.transept_certificate_algorithm =
                "tampered-transept-certificate-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_choir_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
    ...
    fn verify_rejects_tampered_nave_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_nave_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_nave_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_nave_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_choir_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_choir_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_choir_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "choir_receipt_tampered_choir_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        transept_certificate_session_dir: PathBuf,
        input_transept_certificate_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageChoirReceipt,
                transept_certificate_session_dir: self
                    .input_transept_certificate_session_dir
                    .clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.transept_certificate_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageChoirReceipt,
                transept_certificate_session_dir: self
                    .input_transept_certificate_session_dir
                    .clone(),
                confirmed_decision_packet_session_dir: Some(
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let transept_certificate_session_dir = fixture_dir.join("choir-receipt-session");
        let input_transept_certificate_session_dir =
            fixture_dir.join("transept-certificate-session");
        let nave_receipt_session_dir = fixture_dir.join("nave-receipt-session");
        let lectern_certificate_session_dir = fixture_dir.join("lectern-certificate-session");
        let apse_receipt_session_dir = fixture_dir.join("apse-receipt-session");
        ...
        persist_json(
            ...
            &stored_transept_certificate_session_fixture(
                &nave_receipt_session_dir,
                &lectern_certificate_session_dir,
                &apse_receipt_session_dir,
                &rostrum_certificate_session_dir,
        ...
        persist_json(
            ...
            &stored_transept_certificate_status_fixture(
                &nave_receipt_session_dir,
                &lectern_certificate_session_dir,
                &apse_receipt_session_dir,
                &rostrum_certificate_session_dir,
        ...
        bind_transept_certificate_report_to_fixture(
            &mut verify_report,
            &nave_receipt_session_dir,
            &lectern_certificate_session_dir,
            &apse_receipt_session_dir,
        ...
        let verify_path = fixture_dir.join("transept-certificate.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_transept_certificate_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_transept_certificate"),
            &verify_path,
            &nave_receipt_session_dir,
            &decision_packet_session_dir,
            &input_transept_certificate_session_dir,
    ...
    fn bind_transept_certificate_report_to_fixture(
        ...
        object.insert(
            "nave_receipt_session_dir".to_string(),
            json!(nave_receipt_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            ...
            json!(apse_receipt_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            "verify_transept_certificate_command_summary".to_string(),
            json!(format!(
                "verify immutable transept-certificate session under {} before freezing the final choir receipt",
                transept_certificate_session_dir.display()
            )),
        ...
        object.insert(
            ...
            json!("transept-certificate verify statement"),
        );
    }

    fn transept_certificate_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_transept_certificate",
            "verdict": "tiny_live_package_transept_certificate_verify_ok",
            "reason": "transept-certificate chain is coherent",
            "result": result,
    ...
    fn bedrock_certificate_verify_report(
        ...
        transept_certificate_verify_report(
            result,
            handoff_bundle_result,
            decision_packet_result,
            execute_frozen_result,
    ...
    fn stored_transept_certificate_session_fixture(
        nave_receipt_session_dir: &Path,
        lectern_certificate_session_dir: &Path,
        apse_receipt_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("transept-certificate session statement"),
        );
        Value::Object(object)
    }

    fn stored_transept_certificate_status_fixture(
        nave_receipt_session_dir: &Path,
        lectern_certificate_session_dir: &Path,
        apse_receipt_session_dir: &Path,
        _rostrum_certificate_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("transept-certificate status statement"),
        );
        Value::Object(object)
    }
}
