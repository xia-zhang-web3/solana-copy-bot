...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_sanctuary_certificate_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-sanctuary-certificate | --render-latest-sanctuary-certificate-script | --run-latest-sanctuary-certificate | --verify-latest-sanctuary-certificate)";
const SANCTUARY_CERTIFICATE_LATEST_SESSION_VERSION: &str = "1";
const SANCTUARY_CERTIFICATE_LATEST_STATUS_VERSION: &str = "1";
...
const SANCTUARY_CERTIFICATE_BIN: &str =
    "copybot_tiny_live_activation_package_sanctuary_certificate";
const APSE_RECEIPT_LATEST_PLAN_READY: &str = "tiny_live_package_apse_receipt_latest_plan_ready";
const APSE_RECEIPT_LATEST_VERIFY_OK: &str = "tiny_live_package_apse_receipt_latest_verify_ok";
const SANCTUARY_CERTIFICATE_VERIFY_OK: &str = "tiny_live_package_sanctuary_certificate_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-sanctuary-certificate handoff binds the latest persisted immutable package chain to the accepted sanctuary-certificate contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestSanctuaryCertificate,
    RenderLatestSanctuaryCertificateScript,
    RunLatestSanctuaryCertificate,
    VerifyLatestSanctuaryCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageSanctuaryCertificateLatestVerdict {
    TinyLivePackageSanctuaryCertificateLatestPlanReady,
    TinyLivePackageSanctuaryCertificateLatestScriptRendered,
    TinyLivePackageSanctuaryCertificateLatestRefusedByMissingLatestChain,
    TinyLivePackageSanctuaryCertificateLatestRefusedByInvalidLatestChain,
    TinyLivePackageSanctuaryCertificateLatestRefusedByDriftedSanctuaryCertificateContract,
    TinyLivePackageSanctuaryCertificateLatestRefusedNowByStage3,
    TinyLivePackageSanctuaryCertificateLatestRefusedNowByPreActivationGate,
    TinyLivePackageSanctuaryCertificateLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageSanctuaryCertificateLatestVerifyOk,
    TinyLivePackageSanctuaryCertificateLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageSanctuaryCertificateLatestResult {
    RefusedByMissingLatestChain,
    RefusedByInvalidLatestChain,
    RefusedByDriftedSanctuaryCertificateContract,
    RefusedNowByStage3,
...
struct PackageSanctuaryCertificateLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageSanctuaryCertificateLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_apse_receipt_latest_session_dir: String,
    nested_sanctuary_certificate_session_dir: String,
    ...
    resolve_latest_apse_receipt_command_summary: String,
    run_latest_sanctuary_certificate_command_summary: String,
    verify_latest_sanctuary_certificate_command_summary: String,
    downstream_sanctuary_certificate_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageSanctuaryCertificateLatestStatus {
    ...
    nested_sanctuary_certificate_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackageSanctuaryCertificateLatestResult,
    reason: String,
    latest_apse_receipt_plan_verdict: Option<String>,
    latest_apse_receipt_verdict: Option<String>,
    sanctuary_certificate_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_apse_receipt_plan_step: Option<PackageSanctuaryCertificateLatestStepArtifact>,
    latest_apse_receipt_step: Option<PackageSanctuaryCertificateLatestStepArtifact>,
    sanctuary_certificate_step: Option<PackageSanctuaryCertificateLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackageSanctuaryCertificateLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_apse_receipt_plan_report_path: PathBuf,
    latest_apse_receipt_report_path: PathBuf,
    sanctuary_certificate_report_path: PathBuf,
    nested_apse_receipt_latest_session_dir: PathBuf,
    nested_sanctuary_certificate_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageSanctuaryCertificateLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageSanctuaryCertificateLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    sanctuary_certificate_latest_session_path: Option<String>,
    sanctuary_certificate_latest_status_path: Option<String>,
    sanctuary_certificate_latest_report_path: Option<String>,
    latest_apse_receipt_plan_report_path: Option<String>,
    latest_apse_receipt_report_path: Option<String>,
    sanctuary_certificate_report_path: Option<String>,
    nested_apse_receipt_latest_session_dir: Option<String>,
    nested_sanctuary_certificate_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_apse_receipt_command_summary: Option<String>,
    run_latest_sanctuary_certificate_command_summary: Option<String>,
    verify_latest_sanctuary_certificate_command_summary: Option<String>,
    downstream_sanctuary_certificate_command_summary: Option<String>,
...
#[derive(Debug, Clone)]
struct LatestApseReceiptSnapshot {
...
#[derive(Debug, Clone)]
struct ResolvedLatestApseReceipt {
    snapshot: LatestApseReceiptSnapshot,
    latest_apse_receipt_plan_raw: Value,
    latest_apse_receipt_plan: ApseReceiptLatestReportView,
}
...
enum ResolutionFailureKind {
    MissingLatestChain,
    InvalidLatestChain,
    DriftedApseReceiptContract,
}

#[derive(Debug, Clone)]
struct LatestApseReceiptResolutionFailure {
    kind: ResolutionFailureKind,
    reason: String,
    snapshot: Option<LatestApseReceiptSnapshot>,
    latest_apse_receipt_plan_raw: Option<Value>,
    latest_apse_receipt_plan_verdict: Option<String>,
    latest_apse_receipt_plan_reason: Option<String>,
    latest_apse_receipt_plan_generated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone, Deserialize)]
struct ApseReceiptLatestReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Deserialize)]
struct SanctuaryCertificateReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    ...
    service_status_max_staleness_ms: Option<u64>,
    session_dir: Option<String>,
    sanctuary_certificate_session_path: Option<String>,
    sanctuary_certificate_status_path: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    apse_receipt_step: Option<PackageSanctuaryCertificateLatestStepArtifact>,
...
#[derive(Debug, Clone, Deserialize)]
struct SanctuaryCertificateSessionView {
    apse_receipt_session_dir: String,
...
#[derive(Debug, Clone, Deserialize)]
struct SanctuaryCertificateStatusView {
    session_dir: String,
    apse_receipt_session_dir: String,
    pedestal_certificate_session_dir: String,
    plinth_receipt_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-sanctuary-certificate" => set_mode(
                &mut mode,
                Mode::PlanLatestSanctuaryCertificate,
                arg.as_str(),
            )?,
            "--render-latest-sanctuary-certificate-script" => set_mode(
                &mut mode,
                Mode::RenderLatestSanctuaryCertificateScript,
                arg.as_str(),
            )?,
            "--run-latest-sanctuary-certificate" => {
                set_mode(&mut mode, Mode::RunLatestSanctuaryCertificate, arg.as_str())?
            }
            "--verify-latest-sanctuary-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLatestSanctuaryCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(
        ...
        Mode::PlanLatestSanctuaryCertificate
            | Mode::RenderLatestSanctuaryCertificateScript
            | Mode::RunLatestSanctuaryCertificate
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestSanctuaryCertificateScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestSanctuaryCertificate | Mode::VerifyLatestSanctuaryCertificate
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestSanctuaryCertificate => plan_latest_sanctuary_certificate_report(&config)?,
        Mode::RenderLatestSanctuaryCertificateScript => {
            render_latest_sanctuary_certificate_script_report(&config)?
        }
        Mode::RunLatestSanctuaryCertificate => run_latest_sanctuary_certificate_report(&config)?,
        Mode::VerifyLatestSanctuaryCertificate => {
            verify_latest_sanctuary_certificate_report(&config)?
        }
    };
    if config.json {
...
fn plan_latest_sanctuary_certificate_report(
    config: &Config,
) -> Result<PackageSanctuaryCertificateLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    let suggested_session_dir = config.session_dir.clone().unwrap_or_else(|| {
        root.join("tiny_live_activation_package_sanctuary_certificate_latest.session")
    });
    let paths = sanctuary_certificate_latest_paths(&suggested_session_dir);

    match resolve_latest_apse_receipt(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            "plan_latest_sanctuary_certificate",
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-apse-receipt lineage via accepted latest-apse-receipt session {}, exact decision-packet confirmation anchor {}, and the accepted sanctuary-certificate contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_apse_receipt_latest_session_dir.display(),
...
fn render_latest_sanctuary_certificate_script_report(
    config: &Config,
) -> Result<PackageSanctuaryCertificateLatestReport> {
    let root = config
        .root
    ...
    match resolve_latest_apse_receipt(root, &paths) {
        Ok(resolved) => {
            write_script(
                output_path,
                &format!(
                    "#!/usr/bin/env bash\nset -euo pipefail\n\n{run_latest}\n{verify_latest}\n{run_sanctuary_certificate}\n{verify_sanctuary_certificate}\n",
                    run_latest = run_latest_sanctuary_certificate_command_summary(
                        root,
                        &suggested_session_dir,
                    ),
                    verify_latest =
                        verify_latest_sanctuary_certificate_command_summary(&suggested_session_dir),
                    run_sanctuary_certificate = downstream_sanctuary_certificate_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_apse_receipt_session_dir(&paths),
                        &paths.nested_sanctuary_certificate_session_dir,
                    ),
                    verify_sanctuary_certificate = verify_sanctuary_certificate_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_apse_receipt_session_dir(&paths),
                        &paths.nested_sanctuary_certificate_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                "render_latest_sanctuary_certificate_script",
                TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestScriptRendered,
                format!(
                    "rendered latest-sanctuary-certificate handoff script to {} using the accepted latest-apse-receipt lineage and the accepted sanctuary-certificate contract",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_sanctuary_certificate_report(
    config: &Config,
) -> Result<PackageSanctuaryCertificateLatestReport> {
    let root = config
        .root
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-sanctuary-certificate session dir {}",
            session_dir.display()
        );
    }
    let paths = sanctuary_certificate_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-sanctuary-certificate")?;

    let resolution = resolve_latest_apse_receipt(root, &paths);
    match resolution {
        Ok(resolved) => {
            run_latest_sanctuary_certificate_with_resolution(root, session_dir, &paths, &resolved)
        }
        Err(failure) => {
            run_latest_sanctuary_certificate_failure(root, session_dir, &paths, &failure)
        }
    }
}

fn verify_latest_sanctuary_certificate_report(
    config: &Config,
) -> Result<PackageSanctuaryCertificateLatestReport> {
    let session_dir = config
        .session_dir
    ...
    let session: PackageSanctuaryCertificateLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_sanctuary_certificate_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-sanctuary-certificate session artifact {}: {error}",
    ...
    let status: PackageSanctuaryCertificateLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_sanctuary_certificate_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-sanctuary-certificate status artifact {}: {error}",
    ...
    let stored_report: PackageSanctuaryCertificateLatestReport = match load_json(&paths.report_path)
    {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_sanctuary_certificate_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-sanctuary-certificate report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-sanctuary-certificate root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-sanctuary-certificate session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-sanctuary-certificate status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-sanctuary-certificate report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_apse_receipt_latest_session_dir,
        &paths
            .nested_apse_receipt_latest_session_dir
            .display()
            .to_string(),
        "latest-sanctuary-certificate session nested_apse_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_apse_receipt_latest_session_dir,
        &paths
            .nested_apse_receipt_latest_session_dir
            .display()
            .to_string(),
        "latest-sanctuary-certificate status nested_apse_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-sanctuary-certificate report nested_apse_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_sanctuary_certificate_session_dir,
        &paths
            .nested_sanctuary_certificate_session_dir
            .display()
            .to_string(),
        "latest-sanctuary-certificate session nested_sanctuary_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_sanctuary_certificate_session_dir,
        &paths
            .nested_sanctuary_certificate_session_dir
            .display()
            .to_string(),
        "latest-sanctuary-certificate status nested_sanctuary_certificate_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-sanctuary-certificate report nested_sanctuary_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-sanctuary-certificate session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-sanctuary-certificate status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-sanctuary-certificate report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .sanctuary_certificate_latest_session_path
            .as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-sanctuary-certificate report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .sanctuary_certificate_latest_status_path
            .as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-sanctuary-certificate report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .sanctuary_certificate_latest_report_path
            .as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-sanctuary-certificate report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-sanctuary-certificate session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-sanctuary-certificate status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-sanctuary-certificate report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_sanctuary_certificate",
        "latest-sanctuary-certificate stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-sanctuary-certificate stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-sanctuary-certificate stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-sanctuary-certificate stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-sanctuary-certificate stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report.latest_apse_receipt_plan_verdict.as_deref(),
        status.latest_apse_receipt_plan_verdict.as_deref(),
        "stored report latest_apse_receipt_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.latest_apse_receipt_verdict.as_deref(),
        status.latest_apse_receipt_verdict.as_deref(),
        "stored report latest_apse_receipt_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.sanctuary_certificate_verdict.as_deref(),
        status.sanctuary_certificate_verdict.as_deref(),
        "stored report sanctuary_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: ApseReceiptLatestReportView = load_required_step_json(
                &status.latest_apse_receipt_plan_step,
                &paths.latest_apse_receipt_plan_report_path,
                "latest_apse_receipt_plan_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_apse_receipt_plan_step.as_ref(),
                &paths.latest_apse_receipt_plan_report_path,
                "plan_latest_apse_receipt",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_apse_receipt_plan_step",
                &mut mismatches,
            );
            compare_apse_receipt_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_apse_receipt_latest_session_dir,
                &mut mismatches,
            );
            compare_option_string(
                status.latest_apse_receipt_plan_verdict.as_deref(),
                Some(stored_latest_plan.verdict.as_str()),
                "stored status latest_apse_receipt_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_apse_receipt_plan_verdict.as_deref(),
            ...
            let stored_latest_run: ApseReceiptLatestReportView = load_required_step_json(
                &status.latest_apse_receipt_step,
                &paths.latest_apse_receipt_report_path,
                "latest_apse_receipt_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_apse_receipt_step.as_ref(),
                &paths.latest_apse_receipt_report_path,
                "run_latest_apse_receipt",
                &stored_latest_run.verdict,
                &stored_latest_run.reason,
                stored_latest_run.generated_at,
                "stored latest_apse_receipt_step",
                &mut mismatches,
            );
            compare_apse_receipt_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_apse_receipt_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_apse_receipt_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_apse_receipt_verdict.as_deref(),
            ...
            let actual_latest_run: ApseReceiptLatestReportView =
                match load_json(&nested_apse_receipt_latest_report_path(
                    &paths.nested_apse_receipt_latest_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                        "failed loading actual nested latest-apse-receipt report under {}: {error}",
                        paths.nested_apse_receipt_latest_session_dir.display()
                    ));
                        fallback_apse_receipt_latest_copy()
                    }
                };
            compare_apse_receipt_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                APSE_RECEIPT_LATEST_BIN,
                &latest_apse_receipt_verify_args(&paths.nested_apse_receipt_latest_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-apse-receipt session under {}: {error}",
                        paths.nested_apse_receipt_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: ApseReceiptLatestReportView =
                    serde_json::from_value(latest_verify_raw)
                        .context("failed parsing nested latest-apse-receipt verify json")?;
                if latest_verify.verdict != APSE_RECEIPT_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-apse-receipt verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: SanctuaryCertificateReportView = load_required_step_json(
                &status.sanctuary_certificate_step,
                &paths.sanctuary_certificate_report_path,
                "sanctuary_certificate_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.sanctuary_certificate_step.as_ref(),
                &paths.sanctuary_certificate_report_path,
                "run_live_package_sanctuary_certificate",
                &stored_handoff_bundle.verdict,
                &stored_handoff_bundle.reason,
                stored_handoff_bundle.generated_at,
                "stored sanctuary_certificate_step",
                &mut mismatches,
            );
            compare_sanctuary_certificate_run_against_snapshot(
            ...
            compare_option_string(
                ...
                "stored status sanctuary_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.sanctuary_certificate_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: SanctuaryCertificateSessionView =
                match load_json(&nested_sanctuary_certificate_session_path(
                    &paths.nested_sanctuary_certificate_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                            "failed reading nested sanctuary-certificate session under {}: {error}",
                            paths.nested_sanctuary_certificate_session_dir.display()
                        ));
            ...
            let nested_handoff_bundle_status: SanctuaryCertificateStatusView =
                match load_json(&nested_sanctuary_certificate_status_path(
                    &paths.nested_sanctuary_certificate_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                            "failed reading nested sanctuary-certificate status under {}: {error}",
                            paths.nested_sanctuary_certificate_session_dir.display()
                        ));
            ...
            compare_sanctuary_certificate_copy_against_nested_truth(
                &stored_handoff_bundle,
            ...
            let handoff_bundle_verify_raw = match run_json_command(
                SANCTUARY_CERTIFICATE_BIN,
                &sanctuary_certificate_verify_args(
                    &resolved.snapshot,
                    &expected_downstream_apse_receipt_session_dir(&paths),
                    &paths.nested_sanctuary_certificate_session_dir,
                ),
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested sanctuary-certificate session under {}: {error}",
                        paths.nested_sanctuary_certificate_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: SanctuaryCertificateReportView =
                    serde_json::from_value(handoff_bundle_verify_raw)
                        .context("failed parsing nested sanctuary-certificate verify json")?;
                compare_option_string(
                    stored_handoff_bundle.registry_entry_session_dir.as_deref(),
                    handoff_bundle_verify.registry_entry_session_dir.as_deref(),
                    "stored sanctuary-certificate copy registry_entry_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored sanctuary-certificate copy notarization_receipt_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .consummation_record_algorithm
                        .as_deref(),
                    "stored sanctuary-certificate copy consummation_record_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .completion_certificate_summary
                        .as_deref(),
                    "stored sanctuary-certificate copy completion_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .completion_certificate_sha256
                        .as_deref(),
                    "stored sanctuary-certificate copy completion_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored sanctuary-certificate copy completion_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.culmination_receipt_summary.as_deref(),
                    handoff_bundle_verify.culmination_receipt_summary.as_deref(),
                    "stored sanctuary-certificate copy culmination_receipt_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.culmination_receipt_sha256.as_deref(),
                    handoff_bundle_verify.culmination_receipt_sha256.as_deref(),
                    "stored sanctuary-certificate copy culmination_receipt_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .culmination_receipt_algorithm
                        .as_deref(),
                    "stored sanctuary-certificate copy culmination_receipt_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.summit_certificate_summary.as_deref(),
                    handoff_bundle_verify.summit_certificate_summary.as_deref(),
                    "stored sanctuary-certificate copy summit_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.summit_certificate_sha256.as_deref(),
                    handoff_bundle_verify.summit_certificate_sha256.as_deref(),
                    "stored sanctuary-certificate copy summit_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_summary
                        .as_deref(),
                    "stored sanctuary-certificate copy cornerstone_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_sha256
                        .as_deref(),
                    "stored sanctuary-certificate copy cornerstone_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_algorithm
                        .as_deref(),
                    "stored sanctuary-certificate copy cornerstone_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .capstone_certificate_summary
                        .as_deref(),
                    "stored sanctuary-certificate copy capstone_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.capstone_certificate_sha256.as_deref(),
                    handoff_bundle_verify.capstone_certificate_sha256.as_deref(),
                    "stored sanctuary-certificate copy capstone_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .capstone_certificate_algorithm
                        .as_deref(),
                    "stored sanctuary-certificate copy capstone_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.foundation_receipt_summary.as_deref(),
                    handoff_bundle_verify.foundation_receipt_summary.as_deref(),
                    "stored sanctuary-certificate copy foundation_receipt_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.foundation_receipt_sha256.as_deref(),
                    handoff_bundle_verify.foundation_receipt_sha256.as_deref(),
                    "stored sanctuary-certificate copy foundation_receipt_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .foundation_receipt_algorithm
                        .as_deref(),
                    "stored sanctuary-certificate copy foundation_receipt_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.bedrock_certificate_summary.as_deref(),
                    handoff_bundle_verify.bedrock_certificate_summary.as_deref(),
                    "stored sanctuary-certificate copy bedrock_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.bedrock_certificate_sha256.as_deref(),
                    handoff_bundle_verify.bedrock_certificate_sha256.as_deref(),
                    "stored sanctuary-certificate copy bedrock_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .substructure_certificate_summary
                        .as_deref(),
                    "stored sanctuary-certificate copy substructure_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .substructure_certificate_sha256
                        .as_deref(),
                    "stored sanctuary-certificate copy substructure_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .substructure_certificate_algorithm
                        .as_deref(),
                    "stored sanctuary-certificate copy substructure_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.plinth_receipt_summary.as_deref(),
                    handoff_bundle_verify.plinth_receipt_summary.as_deref(),
                    "stored sanctuary-certificate copy plinth_receipt_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.plinth_receipt_sha256.as_deref(),
                    handoff_bundle_verify.plinth_receipt_sha256.as_deref(),
                    "stored sanctuary-certificate copy plinth_receipt_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.plinth_receipt_algorithm.as_deref(),
                    handoff_bundle_verify.plinth_receipt_algorithm.as_deref(),
                    "stored sanctuary-certificate copy plinth_receipt_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .pedestal_certificate_summary
                        .as_deref(),
                    "stored sanctuary-certificate copy pedestal_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.pedestal_certificate_sha256.as_deref(),
                    handoff_bundle_verify.pedestal_certificate_sha256.as_deref(),
                    "stored sanctuary-certificate copy pedestal_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .pedestal_certificate_algorithm
                        .as_deref(),
                    "stored sanctuary-certificate copy pedestal_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.apse_receipt_summary.as_deref(),
                    handoff_bundle_verify.apse_receipt_summary.as_deref(),
                    "stored sanctuary-certificate copy apse_receipt_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.apse_receipt_sha256.as_deref(),
                    handoff_bundle_verify.apse_receipt_sha256.as_deref(),
                    "stored sanctuary-certificate copy apse_receipt_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.apse_receipt_algorithm.as_deref(),
                    handoff_bundle_verify.apse_receipt_algorithm.as_deref(),
                    "stored sanctuary-certificate copy apse_receipt_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .reviewed_frozen_live_cutover_controller_command_summary
                        .as_deref(),
                    "stored sanctuary-certificate copy reviewed_frozen_live_cutover_controller_command_summary",
                    &mut mismatches,
                );
                if handoff_bundle_verify.verdict != SANCTUARY_CERTIFICATE_VERIFY_OK {
                    mismatches.push(format!(
                        "nested sanctuary-certificate verification is non-green: {}",
                        handoff_bundle_verify.reason
                    ));
                }
            }

            let expected_result = apse_receipt_latest_result_from_nested_status(
                nested_handoff_bundle_status.result.as_str(),
            )
            .unwrap_or(LivePackageSanctuaryCertificateLatestResult::RefusedByDriftedSanctuaryCertificateContract);
            if status.result != expected_result {
                mismatches.push(format!(
                    "stored latest-sanctuary-certificate result {} does not match nested sanctuary-certificate result {}",
        ...
        Err(failure) => {
            ...
            if failure.latest_apse_receipt_plan_raw.is_some() {
                let stored_latest_plan: ApseReceiptLatestReportView = load_required_step_json(
                    &status.latest_apse_receipt_plan_step,
                    &paths.latest_apse_receipt_plan_report_path,
                    "latest_apse_receipt_plan_step",
                ...
                compare_option_string(
                    status.latest_apse_receipt_plan_verdict.as_deref(),
                    failure.latest_apse_receipt_plan_verdict.as_deref(),
                    "stored status latest_apse_receipt_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report.latest_apse_receipt_plan_verdict.as_deref(),
                    failure.latest_apse_receipt_plan_verdict.as_deref(),
                    "stored report latest_apse_receipt_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
                status.latest_apse_receipt_verdict.as_deref(),
                None,
                "stored status latest_apse_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_apse_receipt_verdict.as_deref(),
                None,
                "stored report latest_apse_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.sanctuary_certificate_verdict.as_deref(),
                None,
                "stored status sanctuary_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.sanctuary_certificate_verdict.as_deref(),
                None,
                "stored report sanctuary_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestVerifyOk
    } else {
        TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-sanctuary-certificate session under {} is coherent with the current latest immutable chain and the accepted sanctuary-certificate contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-sanctuary-certificate verification failed".to_string())
    };

    Ok(PackageSanctuaryCertificateLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_sanctuary_certificate".to_string(),
        verdict,
        reason,
        ...
        sanctuary_certificate_latest_session_path: Some(paths.session_path.display().to_string()),
        sanctuary_certificate_latest_status_path: Some(paths.status_path.display().to_string()),
        sanctuary_certificate_latest_report_path: Some(paths.report_path.display().to_string()),
        latest_apse_receipt_plan_report_path: Some(
            paths
                .latest_apse_receipt_plan_report_path
                .display()
                .to_string(),
        ),
        latest_apse_receipt_report_path: Some(
            paths.latest_apse_receipt_report_path.display().to_string(),
        ),
        sanctuary_certificate_report_path: Some(paths.sanctuary_certificate_report_path.display().to_string()),
        nested_apse_receipt_latest_session_dir: Some(
            paths
                .nested_apse_receipt_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_sanctuary_certificate_session_dir: Some(
            paths.nested_sanctuary_certificate_session_dir.display().to_string(),
        ),
        ...
        latest_apse_receipt_plan_verdict: status.latest_apse_receipt_plan_verdict.clone(),
        latest_apse_receipt_verdict: status.latest_apse_receipt_verdict.clone(),
        sanctuary_certificate_verdict: status.sanctuary_certificate_verdict.clone(),
        ...
        resolve_latest_apse_receipt_command_summary: Some(
            session.resolve_latest_apse_receipt_command_summary.clone(),
        ),
        run_latest_sanctuary_certificate_command_summary: Some(
            session.run_latest_sanctuary_certificate_command_summary.clone(),
        ),
        verify_latest_sanctuary_certificate_command_summary: Some(
            session.verify_latest_sanctuary_certificate_command_summary.clone(),
        ),
        downstream_sanctuary_certificate_command_summary: Some(
            session.downstream_sanctuary_certificate_command_summary.clone(),
        ),
        verify_sanctuary_certificate_command_summary: Some(
            session.verify_sanctuary_certificate_command_summary.clone(),
        ),
        ...
        explicit_statement:
            "this verification proves only whether the latest immutable chain still resolves cleanly into the accepted sanctuary-certificate contract; it never runs the frozen controller"
                .to_string(),
    })
}

fn run_latest_sanctuary_certificate_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackageSanctuaryCertificateLatestPaths,
    resolved: &ResolvedLatestApseReceipt,
) -> Result<PackageSanctuaryCertificateLatestReport> {
    let session = planned_session(root, session_dir, paths, Some(resolved), None);
    persist_json(&paths.session_path, &session)?;
    persist_json(
        &paths.latest_apse_receipt_plan_report_path,
        &resolved.latest_apse_receipt_plan_raw,
    )?;

    let latest_apse_receipt_plan_step = Some(step_artifact(
        &paths.latest_apse_receipt_plan_report_path,
        "plan_latest_apse_receipt",
        &resolved.latest_apse_receipt_plan.verdict,
        &resolved.latest_apse_receipt_plan.reason,
        resolved.latest_apse_receipt_plan.generated_at,
    ));

    let latest_apse_receipt_run_raw = match run_json_command(
        APSE_RECEIPT_LATEST_BIN,
        &latest_apse_receipt_run_args(root, &paths.nested_apse_receipt_latest_session_dir),
    ) {
        Ok(value) => value,
        Err(error) => {
            let status = refusal_status(
                ...
                paths,
                Some(resolved),
                LivePackageSanctuaryCertificateLatestResult::RefusedByDriftedSanctuaryCertificateContract,
                format!(
                    "failed running nested latest-apse-receipt handoff under {}: {error}",
                    paths
                        .nested_apse_receipt_latest_session_dir
                        .display()
                ),
    ...
    persist_json(
        &paths.latest_apse_receipt_report_path,
        &latest_apse_receipt_run_raw,
    )?;
    let latest_apse_receipt_run: ApseReceiptLatestReportView =
        serde_json::from_value(latest_apse_receipt_run_raw)
            .context("failed parsing nested latest-apse-receipt run json")?;
    let latest_apse_receipt_step = Some(step_artifact(
        &paths.latest_apse_receipt_report_path,
        "run_latest_apse_receipt",
        &latest_apse_receipt_run.verdict,
        &latest_apse_receipt_run.reason,
        latest_apse_receipt_run.generated_at,
    ));

    if let Some(terminal_failure) = latest_apse_receipt_terminal_failure(&latest_apse_receipt_run) {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            terminal_failure,
            latest_apse_receipt_run.reason.clone(),
            latest_apse_receipt_plan_step,
            latest_apse_receipt_step,
            None,
        );
    ...
    let latest_apse_receipt_verify: ApseReceiptLatestReportView = match run_json_command(
        APSE_RECEIPT_LATEST_BIN,
        &latest_apse_receipt_verify_args(&paths.nested_apse_receipt_latest_session_dir),
    ) {
        ...
        Err(error) => {
            let status = refusal_status(
                    ...
                    LivePackageSanctuaryCertificateLatestResult::RefusedByDriftedSanctuaryCertificateContract,
                    format!(
                    "failed verifying nested latest-apse-receipt handoff under {}: {error}",
                    paths.nested_apse_receipt_latest_session_dir.display()
                ),
                    latest_apse_receipt_plan_step,
                    latest_apse_receipt_step,
    ...
    if latest_apse_receipt_verify.verdict != APSE_RECEIPT_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            LivePackageSanctuaryCertificateLatestResult::RefusedByDriftedSanctuaryCertificateContract,
            latest_apse_receipt_verify.reason,
            latest_apse_receipt_plan_step,
            latest_apse_receipt_step,
            None,
        );
    ...
    let release_capsule_run_raw = match run_json_command(
        SANCTUARY_CERTIFICATE_BIN,
        &sanctuary_certificate_run_args(
            &resolved.snapshot,
            &expected_downstream_apse_receipt_session_dir(paths),
            &paths.nested_sanctuary_certificate_session_dir,
        ),
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageSanctuaryCertificateLatestResult::RefusedByDriftedSanctuaryCertificateContract,
                format!(
                    "failed running downstream sanctuary-certificate contract under {}: {error}",
                    paths.nested_sanctuary_certificate_session_dir.display()
                ),
                latest_apse_receipt_plan_step,
                latest_apse_receipt_step,
    ...
    let provenance_certificate_run: SanctuaryCertificateReportView =
        serde_json::from_value(release_capsule_run_raw)
            .context("failed parsing nested sanctuary-certificate run json")?;
    let sanctuary_certificate_step = Some(step_artifact(
        &paths.sanctuary_certificate_report_path,
        "run_live_package_sanctuary_certificate",
    ...
    let provenance_certificate_verify: SanctuaryCertificateReportView = match run_json_command(
        SANCTUARY_CERTIFICATE_BIN,
        &sanctuary_certificate_verify_args(
            &resolved.snapshot,
            &expected_downstream_apse_receipt_session_dir(paths),
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageSanctuaryCertificateLatestResult::RefusedByDriftedSanctuaryCertificateContract,
                format!(
                    "failed verifying downstream sanctuary-certificate contract under {}: {error}",
                    paths.nested_sanctuary_certificate_session_dir.display()
                ),
                latest_apse_receipt_plan_step,
                latest_apse_receipt_step,
    ...
    if provenance_certificate_verify.verdict != SANCTUARY_CERTIFICATE_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    let result = apse_receipt_latest_result_from_nested_status(
        provenance_certificate_run
            .result
            .as_deref()
            .unwrap_or_default(),
    ...
    let status = PackageSanctuaryCertificateLatestStatus {
        status_version: SANCTUARY_CERTIFICATE_LATEST_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nested_apse_receipt_latest_session_dir: paths
            .nested_apse_receipt_latest_session_dir
            .display()
            .to_string(),
        nested_sanctuary_certificate_session_dir: paths
            .nested_sanctuary_certificate_session_dir
            .display()
            .to_string(),
        ...
        latest_apse_receipt_plan_verdict: Some(resolved.latest_apse_receipt_plan.verdict.clone()),
        latest_apse_receipt_verdict: Some(latest_apse_receipt_run.verdict.clone()),
        sanctuary_certificate_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_sanctuary_certificate_failure(
    root: &Path,
    session_dir: &Path,
    paths: &PackageSanctuaryCertificateLatestPaths,
    failure: &LatestApseReceiptResolutionFailure,
) -> Result<PackageSanctuaryCertificateLatestReport> {
    let session = planned_session(root, session_dir, paths, None, Some(failure));
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_apse_receipt_plan_raw {
        persist_json(&paths.latest_apse_receipt_plan_report_path, value)?;
    }
...
fn resolve_latest_apse_receipt(
    root: &Path,
    paths: &PackageSanctuaryCertificateLatestPaths,
) -> std::result::Result<ResolvedLatestApseReceipt, LatestApseReceiptResolutionFailure> {
    let latest_apse_receipt_plan_raw = match run_json_command(
        APSE_RECEIPT_LATEST_BIN,
        &latest_apse_receipt_plan_args(root, &paths.nested_apse_receipt_latest_session_dir),
    ) {
        Ok(value) => value,
        Err(error) => {
            return Err(LatestApseReceiptResolutionFailure {
                ...
                snapshot: None,
                latest_apse_receipt_plan_raw: None,
                latest_apse_receipt_plan_verdict: None,
                latest_apse_receipt_plan_reason: None,
                latest_apse_receipt_plan_generated_at: None,
            });
        }
    };
    let latest_apse_receipt_plan: ApseReceiptLatestReportView =
        serde_json::from_value(latest_apse_receipt_plan_raw.clone()).map_err(
            |error| LatestApseReceiptResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-apse-receipt upstream latest-apse-receipt plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_apse_receipt_plan_raw: Some(
                    latest_apse_receipt_plan_raw.clone(),
                ),
                latest_apse_receipt_plan_verdict: None,
                latest_apse_receipt_plan_reason: None,
                latest_apse_receipt_plan_generated_at: None,
    ...
    let snapshot = parse_snapshot_from_latest_apse_receipt(&latest_apse_receipt_plan).ok();
    if latest_apse_receipt_plan.verdict != APSE_RECEIPT_LATEST_PLAN_READY {
        return Err(LatestApseReceiptResolutionFailure {
            kind: map_apse_receipt_latest_failure_kind(
                &latest_apse_receipt_plan.verdict,
            ),
            reason: format!(
                "latest-apse-receipt resolution refused because nested latest-apse-receipt reported: {}",
                latest_apse_receipt_plan.reason
            ),
            snapshot,
            latest_apse_receipt_plan_raw: Some(latest_apse_receipt_plan_raw),
            latest_apse_receipt_plan_verdict: Some(
                latest_apse_receipt_plan.verdict,
            ),
            latest_apse_receipt_plan_reason: Some(
                latest_apse_receipt_plan.reason,
            ),
            latest_apse_receipt_plan_generated_at: Some(
                latest_apse_receipt_plan.generated_at,
            ),
        });
    }

    let snapshot =
        parse_snapshot_from_latest_apse_receipt(&latest_apse_receipt_plan).map_err(|error| {
            LatestApseReceiptResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!("latest-apse-receipt resolution failed: {error}"),
                snapshot: None,
                latest_apse_receipt_plan_raw: Some(latest_apse_receipt_plan_raw.clone()),
                latest_apse_receipt_plan_verdict: Some(latest_apse_receipt_plan.verdict.clone()),
                latest_apse_receipt_plan_reason: Some(latest_apse_receipt_plan.reason.clone()),
                latest_apse_receipt_plan_generated_at: Some(latest_apse_receipt_plan.generated_at),
            }
        })?;

    let mut mismatches = Vec::new();
    compare_apse_receipt_latest_plan_against_snapshot(
        &latest_apse_receipt_plan,
        &snapshot,
        &paths.nested_apse_receipt_latest_session_dir,
        &mut mismatches,
    );
    if !mismatches.is_empty() {
        return Err(LatestApseReceiptResolutionFailure {
            kind: ResolutionFailureKind::DriftedApseReceiptContract,
            reason: format!("latest-apse-receipt drift detected: {}", mismatches[0]),
            snapshot: Some(snapshot),
            latest_apse_receipt_plan_raw: Some(latest_apse_receipt_plan_raw),
            latest_apse_receipt_plan_verdict: Some(latest_apse_receipt_plan.verdict),
            latest_apse_receipt_plan_reason: Some(latest_apse_receipt_plan.reason),
            latest_apse_receipt_plan_generated_at: Some(latest_apse_receipt_plan.generated_at),
        });
    }

    Ok(ResolvedLatestApseReceipt {
        snapshot,
        latest_apse_receipt_plan_raw,
        latest_apse_receipt_plan,
    })
}

fn parse_snapshot_from_latest_apse_receipt(
    view: &ApseReceiptLatestReportView,
) -> Result<LatestApseReceiptSnapshot> {
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-apse-receipt nested latest-apse-receipt plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-apse-receipt nested latest-apse-receipt plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestApseReceiptSnapshot {
        latest_top_step_name,
        ...
        wrapper_timeout_ms: view.wrapper_timeout_ms.ok_or_else(|| {
            anyhow!(
                "latest-apse-receipt nested latest-apse-receipt plan is missing wrapper_timeout_ms"
            )
        })?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!(
                "latest-apse-receipt nested latest-apse-receipt plan is missing service_status_max_staleness_ms"
            )
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackageSanctuaryCertificateLatestPaths,
    resolved: &ResolvedLatestApseReceipt,
) -> PackageSanctuaryCertificateLatestReport {
    PackageSanctuaryCertificateLatestReport {
        generated_at: Utc::now(),
        ...
        sanctuary_certificate_report_path: Some(
            paths
                .sanctuary_certificate_report_path
                .display()
                .to_string(),
        ),
        nested_apse_receipt_latest_session_dir: Some(
        ...
        latest_apse_receipt_plan_verdict: Some(resolved.latest_apse_receipt_plan.verdict.clone()),
        latest_apse_receipt_verdict: None,
        sanctuary_certificate_verdict: None,
        result: None,
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_apse_receipt_command_summary: Some(
            resolve_latest_apse_receipt_command_summary(
                root,
                &paths.nested_apse_receipt_latest_session_dir,
            ),
        ),
        run_latest_sanctuary_certificate_command_summary: Some(
            run_latest_sanctuary_certificate_command_summary(root, session_dir),
        ),
        verify_latest_sanctuary_certificate_command_summary: Some(
            verify_latest_sanctuary_certificate_command_summary(session_dir),
        ),
        downstream_sanctuary_certificate_command_summary: Some(
            downstream_sanctuary_certificate_command_summary(
                &resolved.snapshot,
                &expected_downstream_apse_receipt_session_dir(paths),
                &paths.nested_sanctuary_certificate_session_dir,
            ),
...
fn report_from_resolution_failure(
    ...
    PackageSanctuaryCertificateLatestReport {
        ...
        latest_apse_receipt_plan_verdict: failure.latest_apse_receipt_plan_verdict.clone(),
        latest_apse_receipt_verdict: None,
        sanctuary_certificate_verdict: None,
        ...
        downstream_sanctuary_certificate_command_summary: snapshot.map(|value| {
            downstream_sanctuary_certificate_command_summary(
                value,
                &expected_downstream_apse_receipt_session_dir(paths),
                &paths.nested_sanctuary_certificate_session_dir,
        ...
        verify_sanctuary_certificate_command_summary: snapshot.map(|value| {
            verify_sanctuary_certificate_command_summary(
                value,
                &expected_downstream_apse_receipt_session_dir(paths),
                &paths.nested_sanctuary_certificate_session_dir,
...
fn planned_session(
    ...
    session_dir: &Path,
    paths: &PackageSanctuaryCertificateLatestPaths,
    resolved: Option<&ResolvedLatestApseReceipt>,
    failure: Option<&LatestApseReceiptResolutionFailure>,
) -> PackageSanctuaryCertificateLatestSession {
    let snapshot = resolved
        .map(|value| &value.snapshot)
        .or_else(|| failure.and_then(|value| value.snapshot.as_ref()));
    PackageSanctuaryCertificateLatestSession {
        session_version: SANCTUARY_CERTIFICATE_LATEST_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        root: root.display().to_string(),
        session_dir: session_dir.display().to_string(),
        nested_apse_receipt_latest_session_dir: paths
        ...
        service_status_max_staleness_ms: snapshot
            .map(|value| value.service_status_max_staleness_ms),
        resolve_latest_apse_receipt_command_summary: resolve_latest_apse_receipt_command_summary(
            root,
            &paths.nested_apse_receipt_latest_session_dir,
        ),
        run_latest_sanctuary_certificate_command_summary:
            run_latest_sanctuary_certificate_command_summary(root, session_dir),
        verify_latest_sanctuary_certificate_command_summary:
            verify_latest_sanctuary_certificate_command_summary(session_dir),
        downstream_sanctuary_certificate_command_summary: snapshot
            .map(|value| {
                downstream_sanctuary_certificate_command_summary(
                    value,
                    &expected_downstream_apse_receipt_session_dir(paths),
            ...
            .unwrap_or_else(|| "<sanctuary-certificate-run-unavailable>".to_string()),
        verify_sanctuary_certificate_command_summary: snapshot
            .map(|value| {
                verify_sanctuary_certificate_command_summary(
                    value,
                    &expected_downstream_apse_receipt_session_dir(paths),
            ...
            .unwrap_or_else(|| "<sanctuary-certificate-verify-unavailable>".to_string()),
        reviewed_frozen_live_cutover_controller_command_summary: snapshot.map(|value| {
            value
                .reviewed_frozen_live_cutover_controller_command_summary
                .clone()
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackageSanctuaryCertificateLatestPaths,
    session: &PackageSanctuaryCertificateLatestSession,
    status: &PackageSanctuaryCertificateLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackageSanctuaryCertificateLatestReport {
    PackageSanctuaryCertificateLatestReport {
        generated_at: Utc::now(),
        ...
        nested_apse_receipt_latest_session_dir: Some(
            session.nested_apse_receipt_latest_session_dir.clone(),
        ),
        nested_sanctuary_certificate_session_dir: Some(
            session.nested_sanctuary_certificate_session_dir.clone(),
        ),
        downstream_decision_packet_session_dir: Some(
...
fn sanctuary_certificate_latest_paths(
    session_dir: &Path,
) -> PackageSanctuaryCertificateLatestPaths {
    PackageSanctuaryCertificateLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_sanctuary_certificate_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_sanctuary_certificate_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_sanctuary_certificate_latest.report.json"),
        latest_apse_receipt_plan_report_path: session_dir.join(
            "tiny_live_activation_package_sanctuary_certificate_latest.apse_receipt_latest_plan.report.json",
        ),
        latest_apse_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_sanctuary_certificate_latest.apse_receipt_latest.report.json",
        ),
        sanctuary_certificate_report_path: session_dir.join(
            "tiny_live_activation_package_sanctuary_certificate_latest.sanctuary_certificate.report.json",
        ),
        nested_apse_receipt_latest_session_dir: session_dir.join(
            "tiny_live_activation_package_sanctuary_certificate_latest.apse_receipt_latest_session",
        ),
        nested_sanctuary_certificate_session_dir: session_dir.join(
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_apse_receipt_latest_session_dir
        .join("tiny_live_activation_package_apse_receipt_latest.decision_packet_session")
}

fn expected_downstream_apse_receipt_session_dir(
    paths: &PackageSanctuaryCertificateLatestPaths,
) -> PathBuf {
    paths
        .nested_apse_receipt_latest_session_dir
        .join("tiny_live_activation_package_apse_receipt_latest.apse_receipt_session")
}
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_apse_receipt_latest_session_dir
        .join("tiny_live_activation_package_apse_receipt_latest.review_receipt_session")
}

fn latest_apse_receipt_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-apse-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn latest_apse_receipt_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-apse-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn latest_apse_receipt_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-apse-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn sanctuary_certificate_run_args(
    snapshot: &LatestApseReceiptSnapshot,
    apse_receipt_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        "--apse-receipt-session-dir".to_string(),
        apse_receipt_session_dir.display().to_string(),
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-sanctuary-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn sanctuary_certificate_verify_args(
    snapshot: &LatestApseReceiptSnapshot,
    apse_receipt_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-live-package-sanctuary-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn resolve_latest_apse_receipt_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{APSE_RECEIPT_LATEST_BIN} --root {} --session-dir {} --plan-latest-apse-receipt --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn run_latest_sanctuary_certificate_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{SANCTUARY_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --run-latest-sanctuary-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn verify_latest_sanctuary_certificate_command_summary(session_dir: &Path) -> String {
    format!(
        "{SANCTUARY_CERTIFICATE_LATEST_BIN} --session-dir {} --verify-latest-sanctuary-certificate --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_sanctuary_certificate_command_summary(
    snapshot: &LatestApseReceiptSnapshot,
    apse_receipt_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{SANCTUARY_CERTIFICATE_BIN} --apse-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-sanctuary-certificate --json",
        shell_escape_path(apse_receipt_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
}

fn verify_sanctuary_certificate_command_summary(
    snapshot: &LatestApseReceiptSnapshot,
    apse_receipt_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{SANCTUARY_CERTIFICATE_BIN} --apse-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-sanctuary-certificate --json",
        shell_escape_path(apse_receipt_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
...
fn nested_apse_receipt_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_apse_receipt_latest.report.json")
}

fn nested_sanctuary_certificate_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_sanctuary_certificate.session.json")
}

fn nested_sanctuary_certificate_status_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_sanctuary_certificate.status.json")
}
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-sanctuary-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageSanctuaryCertificateLatestStepArtifact {
    PackageSanctuaryCertificateLatestStepArtifact {
...
fn canonical_wrapper_mode(mode: &str) -> &str {
    match mode {
        "plan_latest_sanctuary_certificate" => "plan_latest_sanctuary_certificate",
        "render_latest_sanctuary_certificate_script" => {
            "render_latest_sanctuary_certificate_script"
        }
        "run_latest_sanctuary_certificate" => "run_latest_sanctuary_certificate",
        "verify_latest_sanctuary_certificate" => "verify_latest_sanctuary_certificate",
        other => other,
    }
}

fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackageSanctuaryCertificateLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-sanctuary-certificate status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-sanctuary-certificate status"
        ));
        return;
    };
    compare_string(
...
fn compare_apse_receipt_latest_plan_against_snapshot(
    stored: &ApseReceiptLatestReportView,
    snapshot: &LatestApseReceiptSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        ...
        Some(APSE_RECEIPT_LATEST_PLAN_READY),
        "stored latest-apse-receipt plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_apse_receipt",
        "stored latest-apse-receipt plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-apse-receipt plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-apse-receipt plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-apse-receipt plan session_dir",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        ...
        "stored latest-apse-receipt plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-apse-receipt plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-apse-receipt plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-apse-receipt plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_apse_receipt_latest_run_against_snapshot(
    stored: &ApseReceiptLatestReportView,
    snapshot: &LatestApseReceiptSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_apse_receipt",
        "stored latest-apse-receipt run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-apse-receipt run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-apse-receipt run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-apse-receipt run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-apse-receipt run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-apse-receipt run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-apse-receipt run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-apse-receipt run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-apse-receipt run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-apse-receipt run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-apse-receipt run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-apse-receipt run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-apse-receipt run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-apse-receipt run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-apse-receipt run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_apse_receipt_latest_copy_matches_actual(
    stored: &ApseReceiptLatestReportView,
    actual: &ApseReceiptLatestReportView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        Some(stored.verdict.as_str()),
        Some(actual.verdict.as_str()),
        "stored latest-apse-receipt copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-apse-receipt copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-apse-receipt copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-apse-receipt copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-apse-receipt copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-apse-receipt copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-apse-receipt copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-apse-receipt copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-apse-receipt copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-apse-receipt copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-apse-receipt copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-apse-receipt copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_sanctuary_certificate_run_against_snapshot(
    stored: &SanctuaryCertificateReportView,
    snapshot: &LatestApseReceiptSnapshot,
    apse_receipt_session_dir: &Path,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_live_package_sanctuary_certificate",
        "stored sanctuary-certificate run mode",
        mismatches,
    );
    compare_string(
        &stored.apse_receipt_session_dir,
        &apse_receipt_session_dir.display().to_string(),
        "stored sanctuary-certificate run apse_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored sanctuary-certificate run decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored sanctuary-certificate run execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored sanctuary-certificate run turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored sanctuary-certificate run launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored sanctuary-certificate run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored sanctuary-certificate run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored sanctuary-certificate run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored sanctuary-certificate run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored sanctuary-certificate run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored sanctuary-certificate run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored sanctuary-certificate run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.sanctuary_certificate_session_path.as_deref(),
        Some(
            nested_sanctuary_certificate_session_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored sanctuary-certificate run sanctuary_certificate_session_path",
        mismatches,
    );
    compare_option_string(
        stored.sanctuary_certificate_status_path.as_deref(),
        Some(
            nested_sanctuary_certificate_status_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored sanctuary-certificate run sanctuary_certificate_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_apse_receipt_report_path.as_deref(),
        Some(
            session_dir
        ...
        "stored sanctuary-certificate run archived_apse_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        stored.verify_apse_receipt_command_summary.as_deref(),
        Some(verify_sanctuary_certificate_input_command_summary(apse_receipt_session_dir).as_str()),
        "stored sanctuary-certificate run verify_apse_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored sanctuary-certificate run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_sanctuary_certificate_copy_against_nested_truth(
    stored: &SanctuaryCertificateReportView,
    nested_session: &SanctuaryCertificateSessionView,
    nested_status: &SanctuaryCertificateStatusView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        stored.pedestal_certificate_session_dir.as_deref(),
        Some(nested_session.pedestal_certificate_session_dir.as_str()),
        "stored sanctuary-certificate copy pedestal_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.plinth_receipt_session_dir.as_deref(),
        Some(nested_session.plinth_receipt_session_dir.as_str()),
        "stored sanctuary-certificate copy plinth_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.registry_entry_session_dir.as_deref(),
        Some(nested_session.registry_entry_session_dir.as_str()),
        "stored sanctuary-certificate copy registry_entry_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.notarization_receipt_session_dir.as_deref(),
        Some(nested_session.notarization_receipt_session_dir.as_str()),
        "stored sanctuary-certificate copy notarization_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.provenance_certificate_session_dir.as_deref(),
        Some(nested_session.provenance_certificate_session_dir.as_str()),
        "stored sanctuary-certificate copy provenance_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.attestation_seal_session_dir.as_deref(),
        Some(nested_session.attestation_seal_session_dir.as_str()),
        "stored sanctuary-certificate copy attestation_seal_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.release_capsule_session_dir.as_deref(),
        Some(nested_session.release_capsule_session_dir.as_str()),
        "stored sanctuary-certificate copy release_capsule_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.activation_ticket_session_dir.as_deref(),
        Some(nested_session.activation_ticket_session_dir.as_str()),
        "stored sanctuary-certificate copy activation_ticket_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.review_receipt_session_dir.as_deref(),
        Some(nested_session.review_receipt_session_dir.as_str()),
        "stored sanctuary-certificate copy review_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.handoff_bundle_session_dir.as_deref(),
        Some(nested_session.handoff_bundle_session_dir.as_str()),
        "stored sanctuary-certificate copy handoff_bundle_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.decision_packet_session_dir.as_deref(),
        Some(nested_session.decision_packet_session_dir.as_str()),
        "stored sanctuary-certificate copy decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.execute_frozen_session_dir.as_deref(),
        Some(nested_session.execute_frozen_session_dir.as_str()),
        "stored sanctuary-certificate copy execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.turn_green_session_dir.as_deref(),
        Some(nested_session.turn_green_session_dir.as_str()),
        "stored sanctuary-certificate copy turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.launch_packet_session_dir.as_deref(),
        Some(nested_session.launch_packet_session_dir.as_str()),
        "stored sanctuary-certificate copy launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(nested_session.session_dir.as_str()),
        "stored sanctuary-certificate copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        Some(nested_status.result.as_str()),
        "stored sanctuary-certificate copy result",
        mismatches,
    );
    compare_option_string(
        Some(stored.verdict.as_str()),
        sanctuary_certificate_run_verdict_for_result(&nested_status.result),
        "stored sanctuary-certificate copy verdict",
        mismatches,
    );
    compare_option_string(
        stored.handoff_bundle_result.as_deref(),
        nested_status.handoff_bundle_result.as_deref(),
        "stored sanctuary-certificate copy handoff_bundle_result",
        mismatches,
    );
    compare_option_string(
        stored.decision_packet_result.as_deref(),
        nested_status.decision_packet_result.as_deref(),
        "stored sanctuary-certificate copy decision_packet_result",
        mismatches,
    );
    compare_option_string(
        stored.execute_frozen_result.as_deref(),
        nested_status.execute_frozen_result.as_deref(),
        "stored sanctuary-certificate copy execute_frozen_result",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        nested_status.current_pre_activation_gate_verdict.as_deref(),
        "stored sanctuary-certificate copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        nested_status.current_pre_activation_gate_reason.as_deref(),
        "stored sanctuary-certificate copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored.verify_apse_receipt_command_summary.as_deref(),
        Some(nested_session.verify_apse_receipt_command_summary.as_str()),
        "stored sanctuary-certificate copy verify_apse_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored sanctuary-certificate copy release_capsule_digest_manifest_sha256",
        mismatches,
    );
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_apse_receipt_command_summary,
        &resolve_latest_apse_receipt_command_summary(
            root,
            &paths.nested_apse_receipt_latest_session_dir,
        ),
        "stored session resolve_latest_apse_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.run_latest_sanctuary_certificate_command_summary,
        &run_latest_sanctuary_certificate_command_summary(root, session_dir),
        "stored session run_latest_sanctuary_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_latest_sanctuary_certificate_command_summary,
        &verify_latest_sanctuary_certificate_command_summary(session_dir),
        "stored session verify_latest_sanctuary_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.downstream_sanctuary_certificate_command_summary,
        &downstream_sanctuary_certificate_command_summary(
            snapshot,
            &expected_downstream_apse_receipt_session_dir(paths),
            &paths.nested_sanctuary_certificate_session_dir,
        ),
        "stored session downstream_sanctuary_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_sanctuary_certificate_command_summary,
        &verify_sanctuary_certificate_command_summary(
            snapshot,
            &expected_downstream_apse_receipt_session_dir(paths),
            &paths.nested_sanctuary_certificate_session_dir,
        ),
        "stored session verify_sanctuary_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_apse_receipt_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_sanctuary_certificate_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_sanctuary_certificate_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackageSanctuaryCertificateLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackageSanctuaryCertificateLatestPaths,
    snapshot: &LatestApseReceiptSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_apse_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
            .run_latest_sanctuary_certificate_command_summary
            .as_deref(),
        Some(run_latest_sanctuary_certificate_command_summary(root, session_dir).as_str()),
        "stored report run_latest_sanctuary_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
            .verify_latest_sanctuary_certificate_command_summary
            .as_deref(),
        Some(verify_latest_sanctuary_certificate_command_summary(session_dir).as_str()),
        "stored report verify_latest_sanctuary_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report downstream_sanctuary_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report verify_sanctuary_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
    ...
    compare_option_string(
        report.sanctuary_certificate_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "stored report sanctuary_certificate_latest_session_path",
        mismatches,
    );
    compare_option_string(
        report.sanctuary_certificate_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "stored report sanctuary_certificate_latest_status_path",
        mismatches,
    );
    compare_option_string(
        report.sanctuary_certificate_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "stored report sanctuary_certificate_latest_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_apse_receipt_plan_report_path.as_deref(),
        Some(
            paths
                .latest_apse_receipt_plan_report_path
                .display()
        ...
        "stored report latest_apse_receipt_plan_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_apse_receipt_report_path.as_deref(),
        Some(
            paths
                .latest_apse_receipt_report_path
                .display()
                .to_string()
                .as_str(),
        ),
        "stored report latest_apse_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        report.sanctuary_certificate_report_path.as_deref(),
        Some(
            paths
                .sanctuary_certificate_report_path
                .display()
        ...
        "stored report sanctuary_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        report.nested_apse_receipt_latest_session_dir.as_deref(),
        Some(
            paths
                .nested_apse_receipt_latest_session_dir
                .display()
        ...
        "stored report nested_apse_receipt_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_sanctuary_certificate_session_dir.as_deref(),
        Some(
            paths
                .nested_sanctuary_certificate_session_dir
                .display()
        ...
        "stored report nested_sanctuary_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_apse_receipt_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_sanctuary_certificate_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_sanctuary_certificate_session_dir on failure",
        mismatches,
    );
...
fn map_apse_receipt_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_apse_receipt_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_apse_receipt_latest_refused_by_drifted_apse_receipt_contract" => {
            ResolutionFailureKind::DriftedApseReceiptContract
        }
        _ => ResolutionFailureKind::InvalidLatestChain,
    }
}

fn latest_apse_receipt_terminal_failure(
    report: &ApseReceiptLatestReportView,
) -> Option<LivePackageSanctuaryCertificateLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackageSanctuaryCertificateLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackageSanctuaryCertificateLatestResult::RefusedByInvalidLatestChain)
        }
        Some("refused_by_drifted_apse_receipt_contract") => {
            Some(LivePackageSanctuaryCertificateLatestResult::RefusedByDriftedSanctuaryCertificateContract)
        }
        _ => None,
    }
}

fn apse_receipt_latest_result_from_nested_status(
    result: &str,
) -> Option<LivePackageSanctuaryCertificateLatestResult> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => {
            Some(LivePackageSanctuaryCertificateLatestResult::ReadyForManualExecutionWhenGateTurnsGreen)
        }
        "refused_now_by_stage3" => Some(LivePackageSanctuaryCertificateLatestResult::RefusedNowByStage3),
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackageSanctuaryCertificateLatestResult::RefusedNowByPreActivationGate)
        }
...
fn sanctuary_certificate_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => {
            Some("tiny_live_package_sanctuary_certificate_ready_for_manual_execution_when_gate_turns_green")
        }
        "refused_now_by_stage3" => Some("tiny_live_package_sanctuary_certificate_refused_now_by_stage3"),
        "refused_now_by_pre_activation_gate" => {
            Some("tiny_live_package_sanctuary_certificate_refused_now_by_pre_activation_gate")
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_sanctuary_certificate_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}
...
fn result_from_failure_kind(
    kind: ResolutionFailureKind,
) -> LivePackageSanctuaryCertificateLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackageSanctuaryCertificateLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackageSanctuaryCertificateLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedApseReceiptContract => {
            LivePackageSanctuaryCertificateLatestResult::RefusedByDriftedSanctuaryCertificateContract
        }
...
fn verdict_for_result(
    result: LivePackageSanctuaryCertificateLatestResult,
) -> TinyLivePackageSanctuaryCertificateLatestVerdict {
    match result {
        LivePackageSanctuaryCertificateLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestRefusedByMissingLatestChain
        }
        LivePackageSanctuaryCertificateLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestRefusedByInvalidLatestChain
        }
        LivePackageSanctuaryCertificateLatestResult::RefusedByDriftedSanctuaryCertificateContract => {
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestRefusedByDriftedSanctuaryCertificateContract
        }
        LivePackageSanctuaryCertificateLatestResult::RefusedNowByStage3 => {
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestRefusedNowByStage3
        }
        LivePackageSanctuaryCertificateLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestRefusedNowByPreActivationGate
        }
        LivePackageSanctuaryCertificateLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackageSanctuaryCertificateLatestResult) -> String {
    match result {
        LivePackageSanctuaryCertificateLatestResult::RefusedByMissingLatestChain => {
            "no immutable clerestory chain is available yet; do not manually hunt latest-apse-receipt or sanctuary-certificate lineage by hand".to_string()
        }
        LivePackageSanctuaryCertificateLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-apse-receipt / sanctuary-certificate path".to_string()
        }
        LivePackageSanctuaryCertificateLatestResult::RefusedByDriftedSanctuaryCertificateContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-apse-receipt / sanctuary-certificate contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackageSanctuaryCertificateLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-sanctuary-certificate refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackageSanctuaryCertificateLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-sanctuary-certificate refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackageSanctuaryCertificateLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            "the latest immutable chain now resolves into the accepted sanctuary-certificate contract and one bounded latest-sanctuary-certificate handoff is ready for manual execution when gate truth turns green; this wrapper still did not run or authorize the frozen controller".to_string()
        }
    }
}
...
fn verify_sanctuary_certificate_input_command_summary(apse_receipt_session_dir: &Path) -> String {
    format!(
...
fn refusal_status(
    ...
    latest_apse_receipt_step: Option<PackageSanctuaryCertificateLatestStepArtifact>,
    sanctuary_certificate_step: Option<PackageSanctuaryCertificateLatestStepArtifact>,
) -> PackageSanctuaryCertificateLatestStatus {
    ...
    PackageSanctuaryCertificateLatestStatus {
        ...
        result,
        reason,
        latest_apse_receipt_plan_verdict: resolved
            .map(|value| value.latest_apse_receipt_plan.verdict.clone()),
        latest_apse_receipt_verdict: latest_apse_receipt_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        sanctuary_certificate_verdict: sanctuary_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackageSanctuaryCertificateLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_apse_receipt_plan_step: failure.latest_apse_receipt_plan_raw.as_ref().map(|_| {
            step_artifact(
                &paths.latest_apse_receipt_plan_report_path,
                "plan_latest_apse_receipt",
                failure
                    .latest_apse_receipt_plan_verdict
                    .as_deref()
                    .unwrap_or("<missing>"),
                failure
                    .latest_apse_receipt_plan_reason
                    .as_deref()
                    .unwrap_or("<missing>"),
                failure
                    .latest_apse_receipt_plan_generated_at
                    .unwrap_or_else(Utc::now),
            )
        }),
        latest_apse_receipt_step: None,
...
fn verify_invalid_sanctuary_certificate_latest_report_without_session(
    session_dir: &Path,
    verification_mismatches: Vec<String>,
) -> PackageSanctuaryCertificateLatestReport {
    ...
    PackageSanctuaryCertificateLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_sanctuary_certificate".to_string(),
        verdict: TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestVerifyInvalid,
        reason,
        root: None,
        ...
        session_dir: Some(session_dir.display().to_string()),
        sanctuary_certificate_latest_session_path: Some(
            sanctuary_certificate_latest_paths(session_dir)
                .session_path
                .display()
                .to_string(),
        ),
        sanctuary_certificate_latest_status_path: Some(
            sanctuary_certificate_latest_paths(session_dir)
                .status_path
                .display()
                .to_string(),
        ),
        sanctuary_certificate_latest_report_path: Some(
            sanctuary_certificate_latest_paths(session_dir)
                .report_path
                .display()
                .to_string(),
        ),
        latest_apse_receipt_plan_report_path: None,
        latest_apse_receipt_report_path: None,
        sanctuary_certificate_report_path: None,
        nested_apse_receipt_latest_session_dir: None,
        nested_sanctuary_certificate_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_apse_receipt_command_summary: None,
        run_latest_sanctuary_certificate_command_summary: None,
        verify_latest_sanctuary_certificate_command_summary: None,
        downstream_sanctuary_certificate_command_summary: None,
        verify_sanctuary_certificate_command_summary: None,
...
fn fallback_apse_receipt_latest_copy() -> ApseReceiptLatestReportView {
    ApseReceiptLatestReportView {
        generated_at: Utc::now(),
...
fn fallback_notarization_receipt_session(
    snapshot: &LatestApseReceiptSnapshot,
    session_dir: &Path,
) -> SanctuaryCertificateSessionView {
    SanctuaryCertificateSessionView {
        apse_receipt_session_dir: "<missing>".to_string(),
        ...
        verify_apse_receipt_command_summary:
            verify_sanctuary_certificate_input_command_summary(Path::new("<missing>")),
...
fn fallback_notarization_receipt_status(
    _snapshot: &LatestApseReceiptSnapshot,
    session_dir: &Path,
) -> SanctuaryCertificateStatusView {
    SanctuaryCertificateStatusView {
        session_dir: session_dir.display().to_string(),
        apse_receipt_session_dir: "<missing>".to_string(),
        pedestal_certificate_session_dir: "<missing>".to_string(),
...
fn render_report_lines(report: &PackageSanctuaryCertificateLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_apse_receipt_latest_session_dir {
        lines.push(format!("nested_apse_receipt_latest_session_dir={value}"));
    }
    ...
    if let Some(value) = &report.nested_sanctuary_certificate_session_dir {
        lines.push(format!("nested_sanctuary_certificate_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_sanctuary_certificate_command_summary {
        lines.push(format!(
            "downstream_sanctuary_certificate_command_summary={value}"
        ));
    }
...
mod tests {
    ...
    struct FakeApseReceiptLatestFixture {
        temp_dir: PathBuf,
        root: PathBuf,
        ...
        apse_receipt_latest_plan_json_path: PathBuf,
        apse_receipt_latest_run_json_path: PathBuf,
        apse_receipt_latest_verify_json_path: PathBuf,
    ...
    impl FakeApseReceiptLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestSanctuaryCertificate,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestSanctuaryCertificateScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestSanctuaryCertificate,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestSanctuaryCertificate,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    #[test]
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_apse_receipt_lineage(
    ) {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_plan_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = plan_latest_sanctuary_certificate_report(&fixture.plan_config()).unwrap();
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        assert_eq!(
            report.verdict,
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestPlanReady
        );
        assert_eq!(report.mode, "plan_latest_sanctuary_certificate");
        assert_eq!(
            report.latest_top_step_name.as_deref(),
        ...
        assert!(report
            ...
            .as_deref()
            .unwrap()
            .contains(SANCTUARY_CERTIFICATE_BIN));
    }

    #[test]
    fn plan_mode_without_session_dir_uses_sanctuary_latest_default_wrapper_session_dir() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_plan_default_session_dir",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        ...
        let stale_apse_wrapper_session_dir = fixture
            .root
            .join("tiny_live_activation_package_apse_receipt_latest.session");
        let default_paths = sanctuary_certificate_latest_paths(&default_wrapper_session_dir);
        let mut plan_json = default_apse_receipt_latest_plan_report(&fixture);
        ...
        persist_json(&fixture.apse_receipt_latest_plan_json_path, &plan_json).unwrap();
        write_fake_apse_receipt_latest_command(
            &fixture.bin_dir.join(APSE_RECEIPT_LATEST_BIN),
            &fixture.apse_receipt_latest_plan_json_path,
            &fixture.apse_receipt_latest_run_json_path,
            &fixture.apse_receipt_latest_verify_json_path,
            &fixture.root,
            &default_paths.nested_apse_receipt_latest_session_dir,
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let report = plan_latest_sanctuary_certificate_report(&config).unwrap();
        let default_wrapper_session_dir = default_wrapper_session_dir.display().to_string();
        let stale_apse_wrapper_session_dir = stale_apse_wrapper_session_dir.display().to_string();
        ...
        for summary in [
            report
                .run_latest_sanctuary_certificate_command_summary
                .as_deref()
                .expect("missing run_latest_sanctuary_certificate_command_summary"),
            report
                .verify_latest_sanctuary_certificate_command_summary
                .as_deref()
                .expect("missing verify_latest_sanctuary_certificate_command_summary"),
        ] {
            assert!(
                summary.contains(default_wrapper_session_dir.as_str()),
                "self command summary must use sanctuary latest default session dir: {summary}"
            );
            assert!(
                !summary.contains(stale_apse_wrapper_session_dir.as_str()),
                "self command summary must not use apse latest default wrapper session dir: {summary}"
            );
        }
        assert_eq!(
            report.nested_apse_receipt_latest_session_dir.as_deref(),
    ...
    #[test]
    fn render_mode_emits_exact_downstream_sanctuary_certificate_contract_not_second_controller_path(
    ) {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_render_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let report =
            render_latest_sanctuary_certificate_script_report(&fixture.render_config()).unwrap();
        assert_eq!(
            report.verdict,
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestScriptRendered
        );
        assert_eq!(report.mode, "render_latest_sanctuary_certificate_script");
        ...
        assert!(script.contains(SANCTUARY_CERTIFICATE_BIN));
        ...
        assert!(script.contains("--apse-receipt-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-sanctuary-certificate"));
        assert!(script.contains("--verify-live-package-sanctuary-certificate"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            (
                "apse_receipt_latest_missing",
                "tiny_live_package_apse_receipt_latest_refused_by_missing_latest_chain",
                "no persisted immutable chain exists",
                TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestRefusedByMissingLatestChain,
            ),
            (
                "apse_receipt_latest_invalid",
                "tiny_live_package_apse_receipt_latest_refused_by_invalid_latest_chain",
                "latest immutable chain is incomplete",
                TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestRefusedByInvalidLatestChain,
            ),
            (
                "apse_receipt_latest_below",
                "tiny_live_package_apse_receipt_latest_refused_by_invalid_latest_chain",
            ...
            let mut plan: Value = load_json(&fixture.apse_receipt_latest_plan_json_path).unwrap();
            plan["verdict"] = json!(verdict);
            plan["reason"] = json!(reason);
            if label.ends_with("below") {
                plan["latest_top_step_name"] = json!("choir_receipt");
            }
            persist_json(&fixture.apse_receipt_latest_plan_json_path, &plan).unwrap();

            let report = run_latest_sanctuary_certificate_report(&fixture.run_config()).unwrap();
            assert_eq!(report.verdict, expected);
        }
    }

    #[test]
    fn plan_mode_maps_real_latest_apse_receipt_drift_verdict_to_local_drift_refusal() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_plan_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value = load_json(&fixture.apse_receipt_latest_plan_json_path).unwrap();
        plan["verdict"] =
            json!("tiny_live_package_apse_receipt_latest_refused_by_drifted_apse_receipt_contract");
        plan["reason"] = json!("real accepted latest-apse-receipt drift");
        persist_json(&fixture.apse_receipt_latest_plan_json_path, &plan).unwrap();
    ...
    #[test]
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_apse_receipt_reports_real_drift() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_run_drift",
            "refused_by_drifted_apse_receipt_contract",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert_eq!(report.sanctuary_certificate_verdict.as_deref(), None);
    }

    #[test]
    fn run_mode_refuses_when_nested_latest_apse_receipt_lineage_drifts_from_latest_immutable_chain()
    {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_handoff_bundle_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let mut run: Value = load_json(&fixture.apse_receipt_latest_run_json_path).unwrap();
        run["package_dir"] = json!("/tampered/package");
        persist_json(&fixture.apse_receipt_latest_run_json_path, &run).unwrap();
        ...
        assert!(report
            .reason
            .contains("stored latest-apse-receipt run package_dir"));
    }

    #[test]
    fn valid_nested_latest_apse_modes_are_accepted_by_latest_sanctuary_plan_run_verify() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_accepts_real_nested_apse_modes",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let plan = plan_latest_sanctuary_certificate_report(&fixture.plan_config()).unwrap();
        assert_eq!(
            plan.verdict,
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestPlanReady
        );

        let run = run_latest_sanctuary_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestReadyForManualExecutionWhenGateTurnsGreen
        );

        let verify = verify_latest_sanctuary_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestVerifyOk,
            "{:#?}",
        ...
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        let stored_plan: Value = load_json(&paths.latest_apse_receipt_plan_report_path).unwrap();
        let stored_run: Value = load_json(&paths.latest_apse_receipt_report_path).unwrap();
        let status: Value = load_json(&paths.status_path).unwrap();

        assert_eq!(stored_plan["mode"], json!("plan_latest_apse_receipt"));
        assert_eq!(stored_run["mode"], json!("run_latest_apse_receipt"));
        assert_eq!(
            status["latest_apse_receipt_plan_step"]["mode"],
            json!("plan_latest_apse_receipt")
        );
        assert_eq!(
            status["latest_apse_receipt_step"]["mode"],
            json!("run_latest_apse_receipt")
        );
    }

    #[test]
    fn plan_rejects_stale_parent_mode_in_nested_latest_apse_plan_payload() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_rejects_parent_mode_in_nested_apse_plan",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value = load_json(&fixture.apse_receipt_latest_plan_json_path).unwrap();
        plan["mode"] = json!("plan_latest_sanctuary_certificate");
        persist_json(&fixture.apse_receipt_latest_plan_json_path, &plan).unwrap();
        ...
        assert!(report.reason.contains(
            "stored latest-apse-receipt plan mode \"plan_latest_sanctuary_certificate\" does not match expected \"plan_latest_apse_receipt\""
        ));
    }

    #[test]
    fn run_rejects_stale_parent_mode_in_nested_latest_apse_run_payload() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_rejects_parent_mode_in_nested_apse_run",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut run: Value = load_json(&fixture.apse_receipt_latest_run_json_path).unwrap();
        run["mode"] = json!("run_latest_sanctuary_certificate");
        persist_json(&fixture.apse_receipt_latest_run_json_path, &run).unwrap();
        ...
        assert!(report.reason.contains(
            "stored latest-apse-receipt run mode \"run_latest_sanctuary_certificate\" does not match expected \"run_latest_apse_receipt\""
        ));
    }

    #[test]
    fn verify_rejects_stale_parent_modes_in_copied_nested_latest_apse_reports() {
        {
            let fixture = fake_apse_receipt_latest_fixture(
                "sanctuary_certificate_latest_rejects_parent_mode_in_copied_apse_plan",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            run_latest_sanctuary_certificate_report(&fixture.run_config()).unwrap();

            let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
            let mut plan: Value = load_json(&paths.latest_apse_receipt_plan_report_path).unwrap();
            plan["mode"] = json!("plan_latest_sanctuary_certificate");
            persist_json(&paths.latest_apse_receipt_plan_report_path, &plan).unwrap();

            let verify =
                verify_latest_sanctuary_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageSanctuaryCertificateLatestVerdict::TinyLivePackageSanctuaryCertificateLatestVerifyInvalid
            );
        ...
        {
            let fixture = fake_apse_receipt_latest_fixture(
                "sanctuary_certificate_latest_rejects_parent_mode_in_copied_apse_run",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
            let mut run: Value = load_json(&paths.latest_apse_receipt_report_path).unwrap();
            run["mode"] = json!("run_latest_sanctuary_certificate");
            persist_json(&paths.latest_apse_receipt_report_path, &run).unwrap();
    ...
    #[test]
    fn verify_mode_fails_when_persisted_latest_apse_receipt_artifacts_are_tampered_or_missing_nested_lineage_truth(
    ) {
        {
            let fixture = fake_apse_receipt_latest_fixture(
                "apse_receipt_latest_verify_tampered_copy",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.sanctuary_certificate_report_path).unwrap();
            report["decision_packet_session_dir"] = json!("/tampered/run-decision-packet-session");
            persist_json(&paths.sanctuary_certificate_report_path, &report).unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains("stored sanctuary-certificate run decision_packet_session_dir")
            }));
        }

        {
            let fixture = fake_apse_receipt_latest_fixture(
                "apse_receipt_latest_verify_missing_nested_truth",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
            fs::remove_file(nested_apse_receipt_latest_report_path(
                &paths.nested_apse_receipt_latest_session_dir,
            ))
            .unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains("tiny_live_activation_package_apse_receipt_latest.report.json")
            }));
        }
    }

    #[test]
    fn verify_accepts_real_shaped_native_apse_and_sanctuary_identity_layers_on_happy_path() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_verify_real_shaped_rostrum_paths_green",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    #[test]
    fn fake_native_sanctuary_payload_uses_banner_seal_and_apse_standard_seal_input_wording() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_fixture_identity_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        let snapshot = LatestApseReceiptSnapshot {
        ...
        for (label, payload) in payloads {
            let sanctuary_summary = payload["sanctuary_certificate_summary"]
                .as_str()
                .unwrap_or_else(|| panic!("{label} sanctuary_certificate_summary missing"));
            assert!(
                sanctuary_summary.contains("banner-seal"),
                "{label} sanctuary summary must use accepted banner-seal wording: {sanctuary_summary}"
            );
            assert!(
                !sanctuary_summary.contains("standard-seal"),
                "{label} sanctuary summary must not use upstream apse standard-seal wording: {sanctuary_summary}"
            );
            assert!(
                !sanctuary_summary.contains("herald"),
                "{label} sanctuary summary must not use upstream chancel herald wording: {sanctuary_summary}"
            );
            assert!(
                !sanctuary_summary.contains("crest"),
                "{label} sanctuary summary must not use upstream pulpit crest wording: {sanctuary_summary}"
            );
            assert!(
                !sanctuary_summary.contains("hallmark"),
                "{label} sanctuary summary must not use stale hallmark wording: {sanctuary_summary}"
            );
            assert!(
                !sanctuary_summary.contains("escutcheon"),
                "{label} sanctuary summary must not use upstream rostrum escutcheon wording: {sanctuary_summary}"
            );
            assert!(
                !sanctuary_summary.contains("blazon"),
                "{label} sanctuary summary must not use upstream podium blazon wording: {sanctuary_summary}"
            );
            assert!(
                !sanctuary_summary.contains("armorial"),
                "{label} sanctuary summary must not use upstream lectern armorial wording: {sanctuary_summary}"
            );
            ...
            if explicit_statement.contains("freezes one final") {
                assert!(
                    ...
                    "{label} sanctuary session/run explicit statement must use banner-seal wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("standard-seal"),
                    "{label} sanctuary session/run explicit statement must not use upstream apse standard-seal wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("herald"),
                    "{label} sanctuary session/run explicit statement must not use upstream chancel herald wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("crest"),
                    "{label} sanctuary session/run explicit statement must not use upstream pulpit crest wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("escutcheon"),
                    "{label} sanctuary session/run explicit statement must not use upstream rostrum escutcheon wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("blazon"),
                    "{label} sanctuary session/run explicit statement must not use upstream podium blazon wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("armorial"),
                    "{label} sanctuary session/run explicit statement must not use upstream lectern armorial wording: {explicit_statement}"
                );
            }

            let apse_input_summary = payload["apse_receipt_summary"]
                .as_str()
                .unwrap_or_else(|| panic!("{label} apse_receipt_summary missing"));
            assert!(
                apse_input_summary.contains("standard-seal"),
                "{label} apse input summary must use accepted standard-seal wording: {apse_input_summary}"
            );
            assert!(
                apse_input_summary.contains("chancel-certificate"),
                "{label} apse input summary must bind to chancel-certificate truth: {apse_input_summary}"
            );
            assert!(
                !apse_input_summary.contains("herald"),
                "{label} apse input summary must not use upstream chancel herald wording: {apse_input_summary}"
            );
            assert!(
                !apse_input_summary.contains("crest"),
                "{label} apse input summary must not use upstream pulpit crest wording: {apse_input_summary}"
            );
            assert!(
                !apse_input_summary.contains("hallmark"),
                "{label} apse input summary must not use stale hallmark wording: {apse_input_summary}"
            );
            assert!(
                !apse_input_summary.contains("escutcheon"),
                "{label} apse input summary must not use upstream rostrum escutcheon wording: {apse_input_summary}"
            );
            assert!(
                !apse_input_summary.contains("blazon"),
                "{label} apse input summary must not use upstream podium blazon wording: {apse_input_summary}"
            );
            assert!(
                !apse_input_summary.contains("armorial"),
                "{label} apse input summary must not use upstream lectern armorial wording: {apse_input_summary}"
            );
            assert!(
                !apse_input_summary.contains("rostrum-certificate"),
                "{label} apse input summary must not bind directly to rostrum-certificate truth: {apse_input_summary}"
            );
            assert!(
                !apse_input_summary.contains("dais-receipt"),
                "{label} apse input summary must not bind directly to dais-receipt truth: {apse_input_summary}"
            );
            assert!(
                !apse_input_summary.contains("plinth-receipt"),
                "{label} apse input summary must not bind directly to plinth-receipt truth: {apse_input_summary}"
            );
        }
    }

    #[test]
    fn run_and_verify_reject_stale_foreign_wrapper_schema_for_downstream_sanctuary_copy() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_verify_rejects_stale_keystone_wrapper_schema",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        assert!(paths.sanctuary_certificate_report_path.ends_with(
            "tiny_live_activation_package_sanctuary_certificate_latest.sanctuary_certificate.report.json"
        ));
        assert!(paths.sanctuary_certificate_report_path.exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_sanctuary_certificate_latest.apse_receipt.report.json"
            )
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_sanctuary_certificate_latest.pedestal_certificate.report.json"
            )
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_sanctuary_certificate_latest.plinth_receipt.report.json")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_sanctuary_certificate_latest.substructure_certificate.report.json"
            )
            .exists());
        assert!(paths
            .nested_sanctuary_certificate_session_dir
            .ends_with("tiny_live_activation_package_sanctuary_certificate_latest.sanctuary_certificate_session"));
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_sanctuary_certificate_latest.apse_receipt_session")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_sanctuary_certificate_latest.pedestal_certificate_session")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_sanctuary_certificate_latest.plinth_receipt_session"
            )
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_sanctuary_certificate_latest.substructure_certificate_session"
            )
            .exists());

        let mut status: Value = load_json(&paths.status_path).unwrap();
        let stored_step = status
            .get("sanctuary_certificate_step")
            .cloned()
            .expect("sanctuary_certificate_step should be present");
        assert!(status.get("keystone_receipt_step").is_none());
        status["keystone_receipt_step"] = stored_step;
        if let Some(object) = status.as_object_mut() {
            object.remove("sanctuary_certificate_step");
        }
        persist_json(&paths.status_path, &status).unwrap();

        let error =
            verify_latest_sanctuary_certificate_report(&fixture.verify_config()).unwrap_err();
        assert!(error.to_string().contains("sanctuary_certificate_step"));
    }

    #[test]
    fn verify_names_sanctuary_certificate_when_nested_native_session_load_breaks() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_nested_dais_session_load_break",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        fs::remove_file(nested_sanctuary_certificate_session_path(
            &paths.nested_sanctuary_certificate_session_dir,
        ))
        .unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("failed reading nested sanctuary-certificate session under")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_sanctuary_certificate_session_path_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_dais_session_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.sanctuary_certificate_report_path).unwrap();
        report["sanctuary_certificate_session_path"] =
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate run sanctuary_certificate_session_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_sanctuary_certificate_status_path_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_dais_status_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.sanctuary_certificate_report_path).unwrap();
        report["sanctuary_certificate_status_path"] =
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate run sanctuary_certificate_status_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_sanctuary_pedestal_certificate_identity_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_verify_pedestal_identity_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy pedestal_certificate_sha256")
        }));
    }

    #[test]
    fn run_names_latest_apse_receipt_when_nested_latest_wrapper_run_fails() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_nested_run_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        overwrite_fake_apse_receipt_latest_command(
            &fixture.bin_dir.join(APSE_RECEIPT_LATEST_BIN),
            &fixture.apse_receipt_latest_plan_json_path,
            &fixture.apse_receipt_latest_run_json_path,
            &fixture.apse_receipt_latest_verify_json_path,
            None,
            Some("forced nested latest-apse-receipt run failure"),
            None,
            &fixture.root,
            &paths.nested_apse_receipt_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed running nested latest-apse-receipt handoff under"));
    }

    #[test]
    fn run_names_latest_apse_receipt_when_nested_latest_wrapper_verify_fails() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_nested_verify_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_apse_receipt_latest_command(
            ...
            None,
            None,
            Some("forced nested latest-apse-receipt verify failure"),
            &fixture.root,
            &paths.nested_apse_receipt_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed verifying nested latest-apse-receipt handoff under"));
    }

    #[test]
    fn run_names_sanctuary_certificate_when_downstream_native_run_fails() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_downstream_run_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_release_capsule_command(
            &fixture.bin_dir.join(SANCTUARY_CERTIFICATE_BIN),
            ...
            Some("forced downstream native sanctuary run failure"),
            None,
            &expected_downstream_apse_receipt_session_dir(&paths),
            &expected_downstream_decision_packet_session_dir(&paths),
            &paths.nested_sanctuary_certificate_session_dir,
    ...
    #[test]
    fn run_names_sanctuary_certificate_when_downstream_native_verify_fails() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_downstream_verify_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_release_capsule_command(
            ...
            &fixture.handoff_bundle_status_json_path,
            None,
            Some("forced downstream native sanctuary verify failure"),
            &expected_downstream_apse_receipt_session_dir(&paths),
            &expected_downstream_decision_packet_session_dir(&paths),
            &paths.nested_sanctuary_certificate_session_dir,
        );
    ...
    #[test]
    fn run_uses_sanctuary_placeholders_when_missing_snapshot_blocks_nested_handoff_summaries() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_missing_snapshot_placeholders",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_apse_receipt_latest_command(
            ...
            &fixture.apse_receipt_latest_run_json_path,
            &fixture.apse_receipt_latest_verify_json_path,
            Some("forced latest-apse-receipt plan failure without snapshot"),
            None,
            None,
            &fixture.root,
            &paths.nested_apse_receipt_latest_session_dir,
        ...
        assert_eq!(
            report
                .downstream_sanctuary_certificate_command_summary
                .as_deref(),
            Some("<sanctuary-certificate-run-unavailable>")
        );
        assert_eq!(
            report
                .verify_sanctuary_certificate_command_summary
                .as_deref(),
            Some("<sanctuary-certificate-verify-unavailable>")
        );

        let session: PackageSanctuaryCertificateLatestSession =
            load_json(&paths.session_path).unwrap();
        assert_eq!(
            session.downstream_sanctuary_certificate_command_summary,
            "<sanctuary-certificate-run-unavailable>"
        );
        assert_eq!(
            session.verify_sanctuary_certificate_command_summary,
            "<sanctuary-certificate-verify-unavailable>"
        );
    }

    #[test]
    fn verify_names_sanctuary_certificate_when_nested_native_verify_is_non_green() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_verify_non_green_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        verify_json["verdict"] = json!("tiny_live_package_sanctuary_certificate_verify_invalid");
        verify_json["reason"] = json!("forced nested sanctuary-certificate verify non-green");
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("nested sanctuary-certificate verification is non-green")
        }));
        assert!(!verify.verification_mismatches.iter().any(|entry| {
    ...
    #[test]
    fn verify_names_latest_sanctuary_certificate_when_wrapper_result_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_wrapper_result_mismatch_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    #[test]
    fn verify_names_latest_sanctuary_certificate_when_activation_authorized_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_activation_authorized_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn verify_fails_when_copied_native_pinnacle_receipt_sha256_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_pinnacle_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy pinnacle_receipt_sha256")
        }));
    }

    #[test]
    fn verify_fails_when_copied_nested_latest_apse_receipt_explicit_statement_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_launch_packet_explicit_statement_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.latest_apse_receipt_report_path).unwrap();
        report["explicit_statement"] =
            json!("tampered nested latest release capsule explicit statement");
        persist_json(&paths.latest_apse_receipt_report_path, &report).unwrap();
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored latest-apse-receipt copy explicit_statement") }));
    }

    #[test]
    fn verify_rejects_tampered_stale_parent_wrapper_mode_string() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_verify_wrapper_mode_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("latest-sanctuary-certificate stored report mode") }));
    }

    #[test]
    fn wrapper_self_modes_and_self_command_summaries_use_sanctuary_latest_contract() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_self_mode_and_summary_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let plan = plan_latest_sanctuary_certificate_report(&fixture.plan_config()).unwrap();
        assert_eq!(plan.mode, "plan_latest_sanctuary_certificate");
        assert!(plan
            .run_latest_sanctuary_certificate_command_summary
            .as_deref()
            .unwrap()
            .contains(SANCTUARY_CERTIFICATE_LATEST_BIN));
        assert!(plan
            .run_latest_sanctuary_certificate_command_summary
            .as_deref()
            .unwrap()
        ...
        let run = run_latest_sanctuary_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(run.mode, "run_latest_sanctuary_certificate");

        let verify = verify_latest_sanctuary_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(verify.mode, "verify_latest_sanctuary_certificate");

        let session: PackageSanctuaryCertificateLatestSession = load_json(
            &sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir).session_path,
        )
        .unwrap();
        assert!(session
            .run_latest_sanctuary_certificate_command_summary
    ...
    #[test]
    fn native_sanctuary_input_uses_real_latest_apse_receipt_session_path() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_native_input_session_dir_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);

        let downstream = expected_downstream_apse_receipt_session_dir(&paths);
        let expected = paths
            .nested_apse_receipt_latest_session_dir
            .join("tiny_live_activation_package_apse_receipt_latest.apse_receipt_session");
    ...
    #[test]
    fn run_persists_copied_native_sanctuary_session_and_status_under_real_sanctuary_filenames() {
        let fixture = fake_apse_receipt_latest_fixture(
            "sanctuary_certificate_latest_native_sanctuary_filenames",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        let real_session = nested_sanctuary_certificate_session_path(
            &paths.nested_sanctuary_certificate_session_dir,
        );
        let real_status = nested_sanctuary_certificate_status_path(
            &paths.nested_sanctuary_certificate_session_dir,
        );
        ...
        let stale_apse_session = paths
            .nested_sanctuary_certificate_session_dir
            .join("tiny_live_activation_package_apse_receipt.session.json");
        let stale_apse_status = paths
            .nested_sanctuary_certificate_session_dir
            .join("tiny_live_activation_package_apse_receipt.status.json");

        assert!(
            real_session.exists(),
            "missing real native sanctuary session path"
        );
        assert!(
            real_status.exists(),
            "missing real native sanctuary status path"
        );
        assert!(
        ...
        assert!(
            !stale_apse_session.exists(),
            "stale native apse session filename should not be used"
        );
        assert!(
            !stale_apse_status.exists(),
            "stale native apse status filename should not be used"
        );
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_decision_packet_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy decision_packet_result")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_attestation_seal_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy attestation_seal_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_registry_entry_session_dir_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_registry_entry_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy registry_entry_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_notarization_receipt_session_dir_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_notarization_receipt_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy notarization_receipt_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_chain_fingerprint_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy chain_fingerprint_summary")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_chain_fingerprint_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy chain_fingerprint_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_chain_fingerprint_algorithm_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy chain_fingerprint_algorithm")
        }));
    }
    ...
    fn verify_fails_when_copied_native_completion_certificate_closure_certificate_sha256_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_closure_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy closure_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_archive_receipt_sha256_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_archive_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy archive_receipt_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_finality_receipt_sha256_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_finality_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy finality_receipt_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_consummation_record_sha256_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_consummation_record_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy consummation_record_sha256")
        }));
    }
    ...
    {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_completion_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy completion_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_decision_packet_anchor_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy decision_packet_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_current_authorization_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored sanctuary-certificate copy execute_frozen_result")
        }));
    }

    #[test]
    fn verify_fails_when_copied_nested_sanctuary_certificate_verdict_drifts() {
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_release_capsule_verdict_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| entry.contains("stored sanctuary-certificate copy verdict")));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_current_live_cutover_controller_summary_drifts(
        ...
        let fixture = fake_apse_receipt_latest_fixture(
            "apse_receipt_latest_verify_current_live_cutover_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn all_modes_preserve_stage3_and_pre_activation_refusal_semantics_and_do_not_imply_activation_authorization_by_themselves(
        ...
        {
            let fixture = fake_apse_receipt_latest_fixture(
                "apse_receipt_latest_pre_activation",
                "refused_now_by_pre_activation_gate",
                "refused_now_by_pre_activation_gate",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn fake_apse_receipt_latest_fixture(
        label: &str,
        apse_receipt_latest_run_result: &str,
        handoff_bundle_result: &str,
    ) -> FakeApseReceiptLatestFixture {
        ...
        let apse_receipt_latest_plan_json_path = temp_dir.join("apse_receipt_latest.plan.json");
        let apse_receipt_latest_run_json_path = temp_dir.join("apse_receipt_latest.run.json");
        let apse_receipt_latest_verify_json_path = temp_dir.join("apse_receipt_latest.verify.json");
        ...
        let fixture = FakeApseReceiptLatestFixture {
            temp_dir,
            root,
            wrapper_session_dir,
            output_script_path,
            ...
            install_root,
            backend_command,
            apse_receipt_latest_plan_json_path,
            apse_receipt_latest_run_json_path,
            apse_receipt_latest_verify_json_path,
        ...
        persist_json(
            &fixture.apse_receipt_latest_plan_json_path,
            &default_apse_receipt_latest_plan_report(&fixture),
        )
        .unwrap();
        persist_json(
            &fixture.apse_receipt_latest_run_json_path,
            &default_apse_receipt_latest_run_report(&fixture, apse_receipt_latest_run_result),
        )
        .unwrap();
        persist_json(
            &fixture.apse_receipt_latest_verify_json_path,
            &default_apse_receipt_latest_verify_report(&fixture, apse_receipt_latest_run_result),
        )
        .unwrap();
        persist_json(
            &fixture.handoff_bundle_session_json_path,
    ...
    fn default_apse_receipt_latest_plan_report(fixture: &FakeApseReceiptLatestFixture) -> Value {
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        json!({
            "generated_at": Utc::now(),
            "mode": "plan_latest_apse_receipt",
            "verdict": APSE_RECEIPT_LATEST_PLAN_READY,
            "reason": "latest immutable chain resolves cleanly into accepted latest-apse-receipt contract",
            ...
            "wrapper_timeout_ms": 1200,
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_apse_receipt_latest_session_dir.display().to_string(),
            ...
            "activation_authorized": false,
            "explicit_statement": "latest apse-receipt wrapper plan"
        })
    }

    fn canonical_apse_receipt_latest_result(result: &str) -> &str {
        match result {
            "runnable_when_gate_truth_turns_green" => {
                "ready_for_manual_execution_when_gate_turns_green"
            }
    ...
    fn default_apse_receipt_latest_run_report(
        fixture: &FakeApseReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let result = canonical_apse_receipt_latest_result(result);
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_apse_receipt_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_apse_receipt_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_apse_receipt_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_latest_apse_receipt",
            "verdict": verdict,
            "reason": format!("latest-apse-receipt {result}"),
            "latest_top_step_name": "clerestory_certificate",
            "latest_top_session_dir": fixture.latest_top_session_dir.display().to_string(),
            "historical_execute_frozen_session_dir": fixture.historical_execute_frozen_session_dir.display().to_string(),
            "turn_green_session_dir": fixture.turn_green_session_dir.display().to_string(),
            ...
            "clerestory_certificate_algorithm": "sha256",
            "activation_authorized": false,
            "explicit_statement": "latest apse-receipt wrapper run"
        })
    }

    fn default_apse_receipt_latest_verify_report(
        fixture: &FakeApseReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let mut report = default_apse_receipt_latest_run_report(fixture, result);
        report["mode"] = json!("verify_latest_apse_receipt");
        report["verdict"] = json!(APSE_RECEIPT_LATEST_VERIFY_OK);
    ...
    fn default_provenance_certificate_session(fixture: &FakeApseReceiptLatestFixture) -> Value {
        let paths = sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir);
        let apse_receipt_session_dir = expected_downstream_apse_receipt_session_dir(&paths);
        ...
        let pedestal_certificate_session_dir = fixture
            .temp_dir
            .join("downstream-apse-receipt-pedestal-certificate-session");
        ...
        json!({
            "session_version": "1",
            "planned_at": Utc::now(),
            "apse_receipt_session_dir": apse_receipt_session_dir.display().to_string(),
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": sanctuary_certificate_latest_paths(&fixture.wrapper_session_dir).nested_sanctuary_certificate_session_dir.display().to_string(),
            "verify_apse_receipt_command_summary": verify_sanctuary_certificate_input_command_summary(&apse_receipt_session_dir),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
            ...
            "explicit_statement": "the verified apse-receipt session is the primary input here; this immutable sanctuary certificate freezes one final banner-seal identity over the fully culminated chain without enabling production execution"
        })
    }
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_sanctuary_certificate_session_dir.display().to_string(),
            "apse_receipt_session_dir": expected_downstream_apse_receipt_session_dir(&paths).display().to_string(),
            "pedestal_certificate_session_dir": pedestal_certificate_session_dir.display().to_string(),
            "plinth_receipt_session_dir": plinth_receipt_session_dir.display().to_string(),
            "substructure_certificate_session_dir": substructure_certificate_session_dir.display().to_string(),
            "basal_receipt_session_dir": basal_receipt_session_dir.display().to_string(),
            ...
            "sanctuary_certificate_sha256": "fixture-sanctuary-certificate-sha256",
            "sanctuary_certificate_algorithm": "sha256",
            "explicit_statement": "this sanctuary certificate is archival and read-only; it never executes or enables the frozen controller"
        })
    }
    ...
    fn default_provenance_certificate_run_report(
        ...
        let sanctuary_certificate_verdict = match result {
            "ready_for_manual_execution_when_gate_turns_green" => {
                "tiny_live_package_sanctuary_certificate_ready_for_manual_execution_when_gate_turns_green"
            }
            "refused_now_by_stage3" => "tiny_live_package_sanctuary_certificate_refused_now_by_stage3",
            "refused_now_by_pre_activation_gate" => {
                "tiny_live_package_sanctuary_certificate_refused_now_by_pre_activation_gate"
            }
            _ => "tiny_live_package_sanctuary_certificate_refused_now_by_invalid_or_drifted_contract",
        };
        let substructure_certificate_session_dir = fixture
            .temp_dir
            .join("downstream-basal-receipt-substructure-certificate-session");
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_sanctuary_certificate",
            "verdict": sanctuary_certificate_verdict,
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_sanctuary_certificate_session_dir.display().to_string(),
            "sanctuary_certificate_session_path": nested_sanctuary_certificate_session_path(&paths.nested_sanctuary_certificate_session_dir).display().to_string(),
            "sanctuary_certificate_status_path": nested_sanctuary_certificate_status_path(&paths.nested_sanctuary_certificate_session_dir).display().to_string(),
            "archived_apse_receipt_report_path": paths.nested_sanctuary_certificate_session_dir.join("tiny_live_activation_package_sanctuary_certificate.apse_receipt.report.json").display().to_string(),
            "result": result,
            ...
            "apse_receipt_step": status["apse_receipt_step"],
            "verify_apse_receipt_command_summary": verify_sanctuary_certificate_input_command_summary(&expected_downstream_apse_receipt_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn write_fake_apse_receipt_latest_command(
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-apse-receipt session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-apse-receipt \"*) mode='plan';;\n  *\" --run-latest-apse-receipt \"*) mode='run';;\n  *\" --verify-latest-apse-receipt \"*) mode='verify';;\n  *) echo 'unexpected latest-apse-receipt mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_apse_receipt_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
    ...
    fn overwrite_fake_apse_receipt_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        verify_json_path: &Path,
    ...
    fn write_fake_release_capsule_command(
        ...
        let nested_session_path = nested_sanctuary_certificate_session_path(session_dir);
        let nested_status_path = nested_sanctuary_certificate_status_path(session_dir);
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --apse-receipt-session-dir {} \"*) : ;;\n  *) echo 'unexpected apse-receipt-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected confirm-decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected sanctuary-certificate session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --run-live-package-sanctuary-certificate \"*) mode='run';;\n  *\" --verify-live-package-sanctuary-certificate \"*) mode='verify';;\n  *) echo 'unexpected sanctuary-certificate mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  run) mkdir -p '{}'; cp '{}' '{}'; cp '{}' '{}'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
