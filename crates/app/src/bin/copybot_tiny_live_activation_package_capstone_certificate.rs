...
use copybot_app::tiny_live_activation::package_pinnacle_receipt_capstone_certificate;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_capstone_certificate --pinnacle-receipt-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-capstone-certificate | --render-live-package-capstone-certificate | --run-live-package-capstone-certificate | --verify-live-package-capstone-certificate)";
const CAPSTONE_CERTIFICATE_SESSION_VERSION: &str = "1";
const CAPSTONE_CERTIFICATE_STATUS_VERSION: &str = "1";
const CAPSTONE_CERTIFICATE_ALGORITHM: &str = "sha256";
const CAPSTONE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT: &str =
...
const CAPSTONE_CERTIFICATE_STATUS_EXPLICIT_STATEMENT: &str =
    "this capstone certificate is archival and read-only; it never executes or enables the frozen controller";
const CAPSTONE_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT: &str =
...
enum Mode {
    PlanLivePackageCapstoneCertificate,
    RenderLivePackageCapstoneCertificate,
    RunLivePackageCapstoneCertificate,
    VerifyLivePackageCapstoneCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageCapstoneCertificateVerdict {
    TinyLivePackageCapstoneCertificatePlanReady,
    TinyLivePackageCapstoneCertificateRendered,
    TinyLivePackageCapstoneCertificateRefusedNowByStage3,
    TinyLivePackageCapstoneCertificateRefusedNowByPreActivationGate,
    TinyLivePackageCapstoneCertificateRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageCapstoneCertificateReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageCapstoneCertificateVerifyOk,
    TinyLivePackageCapstoneCertificateVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageCapstoneCertificateResult {
    RefusedNowByStage3,
...
struct PackageCapstoneCertificateStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageCapstoneCertificateSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageCapstoneCertificateStatus {
    status_version: String,
    updated_at: DateTime<Utc>,
    session_dir: String,
    pinnacle_receipt_session_dir: String,
    result: LivePackageCapstoneCertificateResult,
    reason: String,
    ...
    pinnacle_receipt_step: Option<PackageCapstoneCertificateStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageCapstoneCertificatePaths {
...
struct PackageCapstoneCertificateReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageCapstoneCertificateVerdict,
    reason: String,
    pinnacle_receipt_session_dir: String,
...
struct NestedPinnacleReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
struct CapstoneCertificatePayload<'a> {
    result: &'a str,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut pinnacle_receipt_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--pinnacle-receipt-session-dir" => {
                pinnacle_receipt_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--pinnacle-receipt-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-capstone-certificate" => set_mode(
                &mut mode,
                Mode::PlanLivePackageCapstoneCertificate,
                arg.as_str(),
            )?,
            "--render-live-package-capstone-certificate" => set_mode(
                &mut mode,
    ...
    if matches!(mode, Mode::RenderLivePackageCapstoneCertificate) && output_path.is_none() {
        bail!("missing --output for --render-live-package-capstone-certificate");
    }
    if matches!(
        mode,
        Mode::RunLivePackageCapstoneCertificate | Mode::VerifyLivePackageCapstoneCertificate
    ) && session_dir.is_none()
    {
    ...
    let pinnacle_receipt_session_dir = pinnacle_receipt_session_dir
        .ok_or_else(|| anyhow!("missing --pinnacle-receipt-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn plan_live_package_capstone_certificate_report(
    config: &Config,
) -> Result<PackageCapstoneCertificateReport> {
    let (pinnacle_receipt, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_pinnacle_receipt_capstone_certificate::verify_live_package_pinnacle_receipt_for_capstone_certificate(
                    &config.pinnacle_receipt_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_pinnacle_receipt(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_pinnacle_receipt_capstone_certificate::load_live_package_pinnacle_receipt_contract_for_capstone_certificate(
                            &config.pinnacle_receipt_session_dir,
                        )?;
                        let classification = classify_pinnacle_receipt_contract(&raw_contract, false);
    ...
    let session_dir = config.session_dir.clone().unwrap_or_else(|| {
        config
            .pinnacle_receipt_session_dir
            .join("capstone-certificate")
    });
    let result = serialize_enum(&classification.0);
    let capstone_certificate_sha256 = compute_capstone_certificate_sha256(
        &result,
        pinnacle_receipt.handoff_bundle_result.as_deref(),
        pinnacle_receipt.decision_packet_result.as_deref(),
        pinnacle_receipt.execute_frozen_result.as_deref(),
        pinnacle_receipt
        ...
        pinnacle_receipt
            ...
            .as_deref(),
        &pinnacle_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        &pinnacle_receipt.chain_fingerprint_sha256,
        &pinnacle_receipt.chain_fingerprint_algorithm,
        &pinnacle_receipt.release_capsule_digest_manifest_sha256,
        pinnacle_receipt.release_capsule_digest_manifest_entry_count,
        &pinnacle_receipt.release_capsule_digest_algorithm,
        &pinnacle_receipt.ledger_seal_sha256,
        &pinnacle_receipt.ledger_seal_algorithm,
        &pinnacle_receipt.registry_entry_sha256,
        &pinnacle_receipt.registry_entry_algorithm,
        &pinnacle_receipt.filing_certificate_sha256,
        &pinnacle_receipt.filing_certificate_algorithm,
        &pinnacle_receipt.archive_receipt_sha256,
        &pinnacle_receipt.archive_receipt_algorithm,
        &pinnacle_receipt.closure_certificate_summary,
        &pinnacle_receipt.closure_certificate_sha256,
        &pinnacle_receipt.closure_certificate_algorithm,
        &pinnacle_receipt.finality_receipt_sha256,
        &pinnacle_receipt.finality_receipt_algorithm,
        &pinnacle_receipt.consummation_record_sha256,
        &pinnacle_receipt.consummation_record_algorithm,
        &pinnacle_receipt.completion_certificate_summary,
        &pinnacle_receipt.completion_certificate_sha256,
        &pinnacle_receipt.completion_certificate_algorithm,
        &pinnacle_receipt.summit_certificate_summary,
        &pinnacle_receipt.summit_certificate_sha256,
        &pinnacle_receipt.summit_certificate_algorithm,
        &pinnacle_receipt.culmination_receipt_summary,
        &pinnacle_receipt.culmination_receipt_sha256,
        &pinnacle_receipt.culmination_receipt_algorithm,
        &pinnacle_receipt.pinnacle_receipt_summary,
        &pinnacle_receipt.pinnacle_receipt_sha256,
        &pinnacle_receipt.pinnacle_receipt_algorithm,
    )?;

    Ok(PackageCapstoneCertificateReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_capstone_certificate".to_string(),
        verdict:
            TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificatePlanReady,
        reason: format!(
            "capstone-certificate-native capstone certificate is explicit for verified pinnacle-receipt session {}; this command stays archival and read-only while freezing one final sovereign identity over the fully culminated chain",
            config.pinnacle_receipt_session_dir.display()
        ),
        pinnacle_receipt_session_dir: config.pinnacle_receipt_session_dir.display().to_string(),
        registry_entry_session_dir: Some(
            pinnacle_receipt.registry_entry_session_dir.display().to_string(),
        ),
        ...
        review_receipt_session_dir: Some(
            pinnacle_receipt.review_receipt_session_dir.display().to_string(),
        ),
        handoff_bundle_session_dir: Some(
            pinnacle_receipt.handoff_bundle_session_dir.display().to_string(),
        ),
        ...
        turn_green_session_dir: Some(
            pinnacle_receipt.turn_green_session_dir.display().to_string(),
        ),
        launch_packet_session_dir: Some(
            pinnacle_receipt.launch_packet_session_dir.display().to_string(),
        ),
        package_dir: Some(pinnacle_receipt.package_dir.display().to_string()),
        install_root: Some(pinnacle_receipt.install_root.display().to_string()),
        target_service_name: Some(pinnacle_receipt.target_service_name.clone()),
        backend_command: Some(pinnacle_receipt.backend_command.clone()),
        wrapper_timeout_ms: Some(pinnacle_receipt.wrapper_timeout_ms),
        service_status_max_staleness_ms: Some(pinnacle_receipt.service_status_max_staleness_ms),
        session_dir: Some(session_dir.display().to_string()),
        capstone_certificate_session_path: None,
        capstone_certificate_status_path: None,
        archived_pinnacle_receipt_report_path: None,
        result: Some(result),
        handoff_bundle_result: pinnacle_receipt.handoff_bundle_result.clone(),
        decision_packet_result: pinnacle_receipt.decision_packet_result.clone(),
        execute_frozen_result: pinnacle_receipt.execute_frozen_result.clone(),
        current_pre_activation_gate_verdict: pinnacle_receipt
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: pinnacle_receipt
            .current_pre_activation_gate_reason
            .clone(),
        ...
        provenance_certificate_summary: Some(pinnacle_receipt.provenance_certificate_summary.clone()),
        chain_fingerprint_summary: Some(pinnacle_receipt.chain_fingerprint_summary.clone()),
        chain_fingerprint_sha256: Some(pinnacle_receipt.chain_fingerprint_sha256.clone()),
        chain_fingerprint_algorithm: Some(pinnacle_receipt.chain_fingerprint_algorithm.clone()),
        release_capsule_digest_manifest_sha256: Some(
            pinnacle_receipt.release_capsule_digest_manifest_sha256.clone(),
        ),
        release_capsule_digest_manifest_entry_count: Some(
            pinnacle_receipt.release_capsule_digest_manifest_entry_count,
        ),
        release_capsule_digest_algorithm: Some(
            pinnacle_receipt.release_capsule_digest_algorithm.clone(),
        ),
        notarization_receipt_summary: Some(pinnacle_receipt.notarization_receipt_summary.clone()),
        ledger_seal_summary: Some(pinnacle_receipt.ledger_seal_summary.clone()),
        ledger_seal_sha256: Some(pinnacle_receipt.ledger_seal_sha256.clone()),
        ledger_seal_algorithm: Some(pinnacle_receipt.ledger_seal_algorithm.clone()),
        registry_entry_summary: Some(pinnacle_receipt.registry_entry_summary.clone()),
        registry_entry_sha256: Some(pinnacle_receipt.registry_entry_sha256.clone()),
        registry_entry_algorithm: Some(pinnacle_receipt.registry_entry_algorithm.clone()),
        filing_certificate_summary: Some(pinnacle_receipt.filing_certificate_summary.clone()),
        filing_certificate_sha256: Some(pinnacle_receipt.filing_certificate_sha256.clone()),
        filing_certificate_algorithm: Some(pinnacle_receipt.filing_certificate_algorithm.clone()),
        archive_receipt_summary: Some(pinnacle_receipt.archive_receipt_summary.clone()),
        archive_receipt_sha256: Some(pinnacle_receipt.archive_receipt_sha256.clone()),
        archive_receipt_algorithm: Some(pinnacle_receipt.archive_receipt_algorithm.clone()),
        closure_certificate_summary: Some(pinnacle_receipt.closure_certificate_summary.clone()),
        closure_certificate_sha256: Some(pinnacle_receipt.closure_certificate_sha256.clone()),
        closure_certificate_algorithm: Some(
            pinnacle_receipt.closure_certificate_algorithm.clone(),
        ),
        finality_receipt_summary: Some(pinnacle_receipt.finality_receipt_summary.clone()),
        finality_receipt_sha256: Some(pinnacle_receipt.finality_receipt_sha256.clone()),
        finality_receipt_algorithm: Some(pinnacle_receipt.finality_receipt_algorithm.clone()),
        consummation_record_summary: Some(pinnacle_receipt.consummation_record_summary.clone()),
        consummation_record_sha256: Some(pinnacle_receipt.consummation_record_sha256.clone()),
        consummation_record_algorithm: Some(pinnacle_receipt.consummation_record_algorithm.clone()),
        completion_certificate_summary: Some(
            pinnacle_receipt.completion_certificate_summary.clone(),
        ),
        completion_certificate_sha256: Some(
            pinnacle_receipt.completion_certificate_sha256.clone(),
        ),
        completion_certificate_algorithm: Some(
            pinnacle_receipt.completion_certificate_algorithm.clone(),
        ),
        summit_certificate_summary: Some(pinnacle_receipt.summit_certificate_summary.clone()),
        summit_certificate_sha256: Some(pinnacle_receipt.summit_certificate_sha256.clone()),
        summit_certificate_algorithm: Some(
            pinnacle_receipt.summit_certificate_algorithm.clone(),
        ),
        culmination_receipt_summary: Some(
            pinnacle_receipt.culmination_receipt_summary.clone(),
        ),
        culmination_receipt_sha256: Some(
            pinnacle_receipt.culmination_receipt_sha256.clone(),
        ),
        culmination_receipt_algorithm: Some(
            pinnacle_receipt.culmination_receipt_algorithm.clone(),
        ),
        pinnacle_receipt_summary: Some(pinnacle_receipt.pinnacle_receipt_summary.clone()),
        pinnacle_receipt_sha256: Some(pinnacle_receipt.pinnacle_receipt_sha256.clone()),
        pinnacle_receipt_algorithm: Some(pinnacle_receipt.pinnacle_receipt_algorithm.clone()),
        capstone_certificate_summary: Some(capstone_certificate_summary(
            classification.0,
            &pinnacle_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        capstone_certificate_sha256: Some(capstone_certificate_sha256),
        capstone_certificate_algorithm: Some(CAPSTONE_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: CAPSTONE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_capstone_certificate_report(
    config: &Config,
) -> Result<PackageCapstoneCertificateReport> {
    let plan = plan_live_package_capstone_certificate_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_capstone_certificate --pinnacle-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-capstone-certificate --json\ncopybot_tiny_live_activation_package_capstone_certificate --pinnacle-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-capstone-certificate --json\n",
        shell_escape_path(&config.pinnacle_receipt_session_dir),
    ...
    Ok(PackageCapstoneCertificateReport {
        generated_at: Utc::now(),
        mode: "render_live_package_capstone_certificate".to_string(),
        verdict:
            TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateRendered,
        reason: format!(
            "rendered capstone-certificate run/verify script to {}; the primary input remains the immutable pinnacle-receipt session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: CAPSTONE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_capstone_certificate_report(
    config: &Config,
) -> Result<PackageCapstoneCertificateReport> {
    ...
    let verified_pinnacle_receipt = match package_pinnacle_receipt_capstone_certificate::verify_live_package_pinnacle_receipt_for_capstone_certificate(
        &config.pinnacle_receipt_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify pinnacle-receipt session under {}: {error}",
                    config.pinnacle_receipt_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_pinnacle_receipt.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "capstone-certificate session dir {} overlaps the managed live target surface derived from verified pinnacle-receipt install_root {}",
            session_dir.display(),
            verified_pinnacle_receipt.contract.install_root.display()
        )
    })?;
    ensure_clean_capstone_certificate_session_dir(session_dir)?;
    let paths = capstone_certificate_paths(session_dir);
    let classification = classify_verified_pinnacle_receipt(&verified_pinnacle_receipt);
    ...
    let capstone_certificate_sha256 = compute_capstone_certificate_sha256(
        ...
        &verified_pinnacle_receipt.contract.finality_receipt_sha256,
        &verified_pinnacle_receipt
            .contract
    ...
    let session = planned_session(
        config,
        session_dir,
        &verified_pinnacle_receipt.contract,
        result,
    ...
    persist_json(
        ...
        &verified_pinnacle_receipt.report_json,
    )?;
    let pinnacle_receipt_step = Some(step_artifact(
        &paths.pinnacle_receipt_report_path,
        "verify_live_package_pinnacle_receipt",
        &verified_pinnacle_receipt.verdict,
        &verified_pinnacle_receipt.reason,
        verified_pinnacle_receipt.generated_at,
    ));
    let status = PackageCapstoneCertificateStatus {
        status_version: CAPSTONE_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        pinnacle_receipt_session_dir: config.pinnacle_receipt_session_dir.display().to_string(),
        result,
        reason: classification.2.clone(),
        handoff_bundle_result: verified_pinnacle_receipt
            .contract
            .handoff_bundle_result
            .clone(),
        decision_packet_result: verified_pinnacle_receipt
            .contract
            .decision_packet_result
            .clone(),
        execute_frozen_result: verified_pinnacle_receipt
            .contract
            .execute_frozen_result
            .clone(),
        current_pre_activation_gate_verdict: verified_pinnacle_receipt
            .contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_pinnacle_receipt
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        pinnacle_receipt_step: pinnacle_receipt_step.clone(),
        provenance_certificate_summary: verified_pinnacle_receipt
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_pinnacle_receipt
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_pinnacle_receipt
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_pinnacle_receipt
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_pinnacle_receipt
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_pinnacle_receipt
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_pinnacle_receipt
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_pinnacle_receipt
            .contract
            .notarization_receipt_summary
            .clone(),
        ledger_seal_summary: verified_pinnacle_receipt
            .contract
            .ledger_seal_summary
            .clone(),
        ledger_seal_sha256: verified_pinnacle_receipt
            .contract
            .ledger_seal_sha256
            .clone(),
        ledger_seal_algorithm: verified_pinnacle_receipt
            .contract
            .ledger_seal_algorithm
            .clone(),
        registry_entry_summary: verified_pinnacle_receipt
            .contract
            .registry_entry_summary
            .clone(),
        registry_entry_sha256: verified_pinnacle_receipt
            .contract
            .registry_entry_sha256
            .clone(),
        registry_entry_algorithm: verified_pinnacle_receipt
            .contract
            .registry_entry_algorithm
            .clone(),
        filing_certificate_summary: verified_pinnacle_receipt
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_pinnacle_receipt
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_pinnacle_receipt
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_pinnacle_receipt
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_pinnacle_receipt
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_pinnacle_receipt
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_pinnacle_receipt
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_pinnacle_receipt
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_pinnacle_receipt
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_pinnacle_receipt
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_pinnacle_receipt
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_pinnacle_receipt
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_pinnacle_receipt
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_pinnacle_receipt
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_pinnacle_receipt
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_pinnacle_receipt
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_pinnacle_receipt
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_pinnacle_receipt
            .contract
            .completion_certificate_algorithm
            .clone(),
        summit_certificate_summary: verified_pinnacle_receipt
            .contract
            .summit_certificate_summary
            .clone(),
        summit_certificate_sha256: verified_pinnacle_receipt
            .contract
            .summit_certificate_sha256
            .clone(),
        summit_certificate_algorithm: verified_pinnacle_receipt
            .contract
            .summit_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_pinnacle_receipt
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_pinnacle_receipt
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_pinnacle_receipt
            .contract
            .culmination_receipt_algorithm
            .clone(),
        pinnacle_receipt_summary: verified_pinnacle_receipt
            .contract
            .pinnacle_receipt_summary
            .clone(),
        pinnacle_receipt_sha256: verified_pinnacle_receipt
            .contract
            .pinnacle_receipt_sha256
            .clone(),
        pinnacle_receipt_algorithm: verified_pinnacle_receipt
            .contract
            .pinnacle_receipt_algorithm
            .clone(),
        capstone_certificate_summary: capstone_certificate_summary(
            result,
            &verified_pinnacle_receipt
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        capstone_certificate_sha256: capstone_certificate_sha256.clone(),
        capstone_certificate_algorithm: CAPSTONE_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: CAPSTONE_CERTIFICATE_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_capstone_certificate_report(
    config: &Config,
) -> Result<PackageCapstoneCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = capstone_certificate_paths(session_dir);
    let session: PackageCapstoneCertificateSession = load_json(&paths.session_path)?;
    let status: PackageCapstoneCertificateStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.pinnacle_receipt_session_dir,
        &config.pinnacle_receipt_session_dir.display().to_string(),
        "pinnacle-receipt pinnacle_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
    ...
    compare_string(
        &status.pinnacle_receipt_session_dir,
        &config.pinnacle_receipt_session_dir.display().to_string(),
        "capstone-certificate status pinnacle_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_pinnacle_receipt_report: serde_json::Value = load_required_step_json(
        &status.pinnacle_receipt_step,
        &paths.pinnacle_receipt_report_path,
        "pinnacle_receipt_step",
        &mut mismatches,
    )?;
    let stored_pinnacle_receipt_report_view: NestedPinnacleReceiptReportView =
        serde_json::from_value(stored_pinnacle_receipt_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested pinnacle-receipt report {}",
                paths.pinnacle_receipt_report_path.display()
            )
        })?;
    let verified_pinnacle_receipt_report_view: NestedPinnacleReceiptReportView =
        serde_json::from_value(verified_pinnacle_receipt.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested pinnacle-receipt report for {}",
                    config.pinnacle_receipt_session_dir.display()
                )
            },
        )?;
    compare_json_ignoring_generated_at(
        &stored_pinnacle_receipt_report,
        &verified_pinnacle_receipt.report_json,
        "capstone-certificate archived pinnacle-receipt report json",
        &mut mismatches,
    );
    compare_string(
        &stored_pinnacle_receipt_report_view.mode,
        &verified_pinnacle_receipt_report_view.mode,
        "capstone-certificate archived pinnacle-receipt report mode",
        &mut mismatches,
    );
    compare_string(
        &stored_pinnacle_receipt_report_view.verdict,
        &verified_pinnacle_receipt_report_view.verdict,
        "capstone-certificate archived pinnacle-receipt report verdict",
        &mut mismatches,
    );
    compare_string(
        &stored_pinnacle_receipt_report_view.reason,
        &verified_pinnacle_receipt_report_view.reason,
        "capstone-certificate archived pinnacle-receipt report reason",
        &mut mismatches,
    );
    if stored_pinnacle_receipt_report_view.generated_at
        != verified_pinnacle_receipt_report_view.generated_at
    {
        mismatches.push(format!(
            "capstone-certificate archived pinnacle-receipt report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_pinnacle_receipt_report_view.generated_at,
            verified_pinnacle_receipt_report_view.generated_at
        ));
    }
    let expected_pinnacle_receipt_step = step_artifact(
        &paths.pinnacle_receipt_report_path,
        &verified_pinnacle_receipt_report_view.mode,
        &verified_pinnacle_receipt_report_view.verdict,
        &verified_pinnacle_receipt_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_pinnacle_receipt_step,
        "pinnacle-receipt pinnacle_receipt_step",
        &mut mismatches,
    );
    if verified_pinnacle_receipt.verdict != "tiny_live_package_pinnacle_receipt_verify_ok" {
        mismatches.push(format!(
    ...
    let classification = classify_verified_pinnacle_receipt(&verified_pinnacle_receipt);
    let expected_capstone_certificate_sha256 = compute_capstone_certificate_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_pinnacle_receipt.contract,
        classification.0,
        &expected_capstone_certificate_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "capstone-certificate session contract does not match freshly verified pinnacle-receipt truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_pinnacle_receipt.contract,
        classification,
        expected_pinnacle_receipt_step.clone(),
        &expected_capstone_certificate_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "capstone-certificate status contract does not match freshly verified pinnacle-receipt truth"
                .to_string(),
        );
    }
...
fn verify_pinnacle_receipt_command_summary(pinnacle_receipt_session_dir: &Path) -> String {
    format!(
...
fn classify_pinnacle_receipt_contract(
    contract: &package_pinnacle_receipt_capstone_certificate::LivePackagePinnacleReceiptContractView,
    assume_verified: bool,
) -> (
    LivePackageCapstoneCertificateResult,
    TinyLivePackageCapstoneCertificateVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageCapstoneCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateRefusedNowByInvalidOrDriftedContract,
            "the pinnacle-receipt chain is not verified, so this capstone certificate remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageCapstoneCertificateResult::RefusedNowByStage3,
            TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateRefusedNowByStage3,
            "the immutable pinnacle receipt remains refused now by Stage 3, so this capstone certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageCapstoneCertificateResult::RefusedNowByPreActivationGate,
            TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateRefusedNowByPreActivationGate,
            "the immutable pinnacle receipt remains refused now by the pre-activation gate, so this capstone certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageCapstoneCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable pinnacle receipt is coherent and this capstone certificate is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageCapstoneCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateRefusedNowByInvalidOrDriftedContract,
            "the verified pinnacle-receipt chain is invalid, drifted, or non-runnable, so this capstone certificate remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_pinnacle_receipt(
    verified: &package_pinnacle_receipt_capstone_certificate::VerifiedLivePackagePinnacleReceiptCapstoneCertificateStep,
) -> (
    LivePackageCapstoneCertificateResult,
    TinyLivePackageCapstoneCertificateVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_pinnacle_receipt_verify_ok" {
        return (
            LivePackageCapstoneCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateRefusedNowByInvalidOrDriftedContract,
            "the immutable pinnacle receipt did not verify cleanly, so this capstone certificate remains refused"
                .to_string(),
        );
    }
    classify_pinnacle_receipt_contract(&verified.contract, true)
}

fn capstone_certificate_summary(
    result: LivePackageCapstoneCertificateResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageCapstoneCertificateResult::RefusedNowByStage3 => {
            "Capstone certificate outcome: the immutable culminated chain remains refused now by Stage 3; archive this final sovereign seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageCapstoneCertificateResult::RefusedNowByPreActivationGate => {
            "Capstone certificate outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final sovereign seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageCapstoneCertificateResult::RefusedNowByInvalidOrDriftedContract => {
            "Capstone certificate outcome: the immutable culminated chain is invalid or drifted; archive this final sovereign seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackageCapstoneCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Capstone certificate outcome: this immutable sovereign seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn capstone_certificate_paths(session_dir: &Path) -> PackageCapstoneCertificatePaths {
...
fn ensure_clean_capstone_certificate_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing capstone-certificate session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating capstone-certificate session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageCapstoneCertificateSession {
    PackageCapstoneCertificateSession {
        session_version: CAPSTONE_CERTIFICATE_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        pinnacle_receipt_session_dir: config.pinnacle_receipt_session_dir.display().to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_pinnacle_receipt_command_summary: verify_pinnacle_receipt_command_summary(
            &config.pinnacle_receipt_session_dir,
        ),
        ...
        capstone_certificate_sha256: capstone_certificate_sha256.to_string(),
        capstone_certificate_algorithm: CAPSTONE_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: CAPSTONE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    pinnacle_receipt_step: PackageCapstoneCertificateStepArtifact,
    capstone_certificate_sha256: &str,
) -> PackageCapstoneCertificateStatus {
    PackageCapstoneCertificateStatus {
        status_version: CAPSTONE_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        pinnacle_receipt_step: Some(pinnacle_receipt_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageCapstoneCertificatePaths,
    contract: &package_pinnacle_receipt_capstone_certificate::LivePackagePinnacleReceiptContractView,
    verdict: TinyLivePackageCapstoneCertificateVerdict,
    status: &PackageCapstoneCertificateStatus,
) -> PackageCapstoneCertificateReport {
    PackageCapstoneCertificateReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateVerifyOk
        {
            "verify_live_package_capstone_certificate".to_string()
        } else {
            "run_live_package_capstone_certificate".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        capstone_certificate_session_path: Some(paths.session_path.display().to_string()),
        capstone_certificate_status_path: Some(paths.status_path.display().to_string()),
        archived_pinnacle_receipt_report_path: Some(
        ...
        pinnacle_receipt_step: status.pinnacle_receipt_step.clone(),
        verify_pinnacle_receipt_command_summary: Some(verify_pinnacle_receipt_command_summary(
            &config.pinnacle_receipt_session_dir,
        )),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
        ...
        explicit_statement: if verdict
            == TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateVerifyOk
        {
            CAPSTONE_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageCapstoneCertificateReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageCapstoneCertificate => {
                "plan_live_package_capstone_certificate".to_string()
            }
            Mode::RenderLivePackageCapstoneCertificate => {
                "render_live_package_capstone_certificate".to_string()
            }
            Mode::RunLivePackageCapstoneCertificate => {
                "run_live_package_capstone_certificate".to_string()
            }
        ...
        verdict,
        reason,
        pinnacle_receipt_session_dir: config
            .pinnacle_receipt_session_dir
            .display()
            .to_string(),
        ...
        pinnacle_receipt_algorithm: Some(CAPSTONE_CERTIFICATE_ALGORITHM.to_string()),
        capstone_certificate_summary: None,
        capstone_certificate_sha256: None,
        capstone_certificate_algorithm: Some(CAPSTONE_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable capstone certificate refused before persisting because the pinnacle-receipt artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "capstone-certificate-native capstone-certificate verification failed".to_string()
    });
    PackageCapstoneCertificateReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_capstone_certificate".to_string(),
        verdict: TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateVerifyInvalid,
        reason,
        pinnacle_receipt_session_dir: config.pinnacle_receipt_session_dir.display().to_string(),
        registry_entry_session_dir: Some(contract.registry_entry_session_dir.display().to_string()),
        notarization_receipt_session_dir: Some(
        ...
        explicit_statement: CAPSTONE_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageCapstoneCertificateStepArtifact>,
    expected_path: &Path,
    label: &str,
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageCapstoneCertificateStepArtifact {
    PackageCapstoneCertificateStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageCapstoneCertificateStepArtifact>,
    expected: &PackageCapstoneCertificateStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_capstone_certificate_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&CapstoneCertificatePayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing capstone-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackageCapstoneCertificateReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "pinnacle_receipt_session_dir={}",
            report.pinnacle_receipt_session_dir
        ),
    ];
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_capstone_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageCapstoneCertificateVerdict::TinyLivePackageCapstoneCertificateReadyForManualExecutionWhenGateTurnsGreen
        );
    ...
    fn drifted_finality_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            ...
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_pinnacle_receipt",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        let error = run_live_package_capstone_certificate_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = capstone_certificate_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_pinnacle_receipt_step_path() {
        let fixture = fake_command_fixture(
        ...
        status.pinnacle_receipt_step.as_mut().unwrap().path =
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageCapstoneCertificate,
                pinnacle_receipt_session_dir: self.pinnacle_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.capstone_certificate_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageCapstoneCertificate,
                pinnacle_receipt_session_dir: self.pinnacle_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        .unwrap();
        persist_json(
            &pinnacle_receipt_session_dir.join(
        ...
        let verify_path = fixture_dir.join("pinnacle-receipt.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_culmination_receipt_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_pinnacle_receipt"),
            &verify_path,
            &summit_certificate_session_dir,
            &decision_packet_session_dir,
            &pinnacle_receipt_session_dir,
    ...
    fn bind_culmination_receipt_report_to_fixture(
        ...
        _pinnacle_receipt_session_dir: &Path,
        package_dir: &Path,
        install_root: &Path,
        ...
        object.insert(
            ...
            json!(summit_certificate_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            ...
            json!("pinnacle-receipt verify statement"),
        );
    }
    ...
    fn culmination_receipt_verify_report(
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_pinnacle_receipt",
            "verdict": "tiny_live_package_pinnacle_receipt_verify_ok",
            "reason": "pinnacle-receipt chain is coherent",
            "result": result,
    ...
    fn stored_culmination_receipt_session_fixture(
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("pinnacle-receipt statement"),
        );
        Value::Object(object)
    }
    ...
    fn stored_culmination_receipt_status_fixture(verify_report: &Value) -> Value {
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("pinnacle-receipt status statement"),
        );
        Value::Object(object)
    }
}
