...
use copybot_app::tiny_live_activation::package_choir_receipt_clerestory_certificate;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_clerestory_certificate --choir-receipt-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-clerestory-certificate | --render-live-package-clerestory-certificate | --run-live-package-clerestory-certificate | --verify-live-package-clerestory-certificate)";
const CLERESTORY_CERTIFICATE_SESSION_VERSION: &str = "1";
const CLERESTORY_CERTIFICATE_STATUS_VERSION: &str = "1";
const CLERESTORY_CERTIFICATE_ALGORITHM: &str = "sha256";
const CLERESTORY_CERTIFICATE_SESSION_EXPLICIT_STATEMENT: &str =
...
enum Mode {
    PlanLivePackageClerestoryCertificate,
    RenderLivePackageClerestoryCertificate,
    RunLivePackageClerestoryCertificate,
    VerifyLivePackageClerestoryCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageClerestoryCertificateVerdict {
    TinyLivePackageClerestoryCertificatePlanReady,
    TinyLivePackageClerestoryCertificateRendered,
    TinyLivePackageClerestoryCertificateRefusedNowByStage3,
    TinyLivePackageClerestoryCertificateRefusedNowByPreActivationGate,
    TinyLivePackageClerestoryCertificateRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageClerestoryCertificateReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageClerestoryCertificateVerifyOk,
    TinyLivePackageClerestoryCertificateVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageClerestoryCertificateResult {
    RefusedNowByStage3,
...
struct PackageClerestoryCertificateStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageClerestoryCertificateSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageClerestoryCertificateStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackageClerestoryCertificateResult,
    reason: String,
    ...
    choir_receipt_step: Option<PackageClerestoryCertificateStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageClerestoryCertificatePaths {
...
struct PackageClerestoryCertificateReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageClerestoryCertificateVerdict,
    reason: String,
    choir_receipt_session_dir: String,
...
struct ClerestoryCertificatePayload<'a> {
    result: &'a str,
    ...
    sanctuary_certificate_summary: &'a str,
    sanctuary_certificate_sha256: &'a str,
    sanctuary_certificate_algorithm: &'a str,
    nave_receipt_summary: &'a str,
    nave_receipt_sha256: &'a str,
    nave_receipt_algorithm: &'a str,
    transept_certificate_summary: &'a str,
    transept_certificate_sha256: &'a str,
    transept_certificate_algorithm: &'a str,
    choir_receipt_summary: &'a str,
    choir_receipt_sha256: &'a str,
    choir_receipt_algorithm: &'a str,
}
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut choir_receipt_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--choir-receipt-session-dir" => {
                choir_receipt_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--choir-receipt-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-clerestory-certificate" => set_mode(
                &mut mode,
                Mode::PlanLivePackageClerestoryCertificate,
                arg.as_str(),
            )?,
            "--render-live-package-clerestory-certificate" => set_mode(
                &mut mode,
                Mode::RenderLivePackageClerestoryCertificate,
                arg.as_str(),
            )?,
            "--run-live-package-clerestory-certificate" => set_mode(
                &mut mode,
                Mode::RunLivePackageClerestoryCertificate,
                arg.as_str(),
            )?,
            "--verify-live-package-clerestory-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLivePackageClerestoryCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackageClerestoryCertificate) && output_path.is_none() {
        bail!("missing --output for --render-live-package-clerestory-certificate");
    }
    if matches!(
        mode,
        Mode::RunLivePackageClerestoryCertificate | Mode::VerifyLivePackageClerestoryCertificate
    ) && session_dir.is_none()
    {
    ...
    let choir_receipt_session_dir =
        choir_receipt_session_dir.ok_or_else(|| anyhow!("missing --choir-receipt-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageClerestoryCertificate => {
            plan_live_package_clerestory_certificate_report(&config)?
        }
        Mode::RenderLivePackageClerestoryCertificate => {
            render_live_package_clerestory_certificate_report(&config)?
        }
        Mode::RunLivePackageClerestoryCertificate => {
            run_live_package_clerestory_certificate_report(&config)?
        }
        Mode::VerifyLivePackageClerestoryCertificate => {
            verify_live_package_clerestory_certificate_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_clerestory_certificate_report(
    config: &Config,
) -> Result<PackageClerestoryCertificateReport> {
    let (choir_receipt_contract, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_choir_receipt_clerestory_certificate::verify_live_package_choir_receipt_for_clerestory_certificate(
                    &config.choir_receipt_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_choir_receipt(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_choir_receipt_clerestory_certificate::load_live_package_choir_receipt_contract_for_clerestory_certificate(
                            &config.choir_receipt_session_dir,
                        )?;
                        let classification =
                            classify_choir_receipt_contract(&raw_contract, false);
    ...
    let session_dir = config.session_dir.clone().unwrap_or_else(|| {
        config
            .choir_receipt_session_dir
            .join("clerestory-certificate")
    });
    let result = serialize_enum(&classification.0);
    let clerestory_certificate_sha256 = compute_clerestory_certificate_sha256(
        &result,
        choir_receipt_contract.handoff_bundle_result.as_deref(),
        choir_receipt_contract.decision_packet_result.as_deref(),
        choir_receipt_contract.execute_frozen_result.as_deref(),
        choir_receipt_contract
            .current_pre_activation_gate_verdict
            .as_deref(),
        choir_receipt_contract
            .current_pre_activation_gate_reason
            .as_deref(),
        &choir_receipt_contract.reviewed_frozen_live_cutover_controller_command_summary,
        &choir_receipt_contract.chain_fingerprint_sha256,
        &choir_receipt_contract.chain_fingerprint_algorithm,
        &choir_receipt_contract.release_capsule_digest_manifest_sha256,
        choir_receipt_contract.release_capsule_digest_manifest_entry_count,
        &choir_receipt_contract.release_capsule_digest_algorithm,
        &choir_receipt_contract.ledger_seal_sha256,
        &choir_receipt_contract.ledger_seal_algorithm,
        &choir_receipt_contract.registry_entry_sha256,
        &choir_receipt_contract.registry_entry_algorithm,
        &choir_receipt_contract.filing_certificate_sha256,
        &choir_receipt_contract.filing_certificate_algorithm,
        &choir_receipt_contract.archive_receipt_sha256,
        &choir_receipt_contract.archive_receipt_algorithm,
        &choir_receipt_contract.closure_certificate_summary,
        &choir_receipt_contract.closure_certificate_sha256,
        &choir_receipt_contract.closure_certificate_algorithm,
        &choir_receipt_contract.finality_receipt_sha256,
        &choir_receipt_contract.finality_receipt_algorithm,
        &choir_receipt_contract.consummation_record_sha256,
        &choir_receipt_contract.consummation_record_algorithm,
        &choir_receipt_contract.completion_certificate_summary,
        &choir_receipt_contract.completion_certificate_sha256,
        &choir_receipt_contract.completion_certificate_algorithm,
        &choir_receipt_contract.summit_certificate_summary,
        &choir_receipt_contract.summit_certificate_sha256,
        &choir_receipt_contract.summit_certificate_algorithm,
        &choir_receipt_contract.culmination_receipt_summary,
        &choir_receipt_contract.culmination_receipt_sha256,
        &choir_receipt_contract.culmination_receipt_algorithm,
        &choir_receipt_contract.pinnacle_receipt_summary,
        &choir_receipt_contract.pinnacle_receipt_sha256,
        &choir_receipt_contract.pinnacle_receipt_algorithm,
        &choir_receipt_contract.capstone_certificate_summary,
        &choir_receipt_contract.capstone_certificate_sha256,
        &choir_receipt_contract.capstone_certificate_algorithm,
        &choir_receipt_contract.keystone_receipt_summary,
        &choir_receipt_contract.keystone_receipt_sha256,
        &choir_receipt_contract.keystone_receipt_algorithm,
        &choir_receipt_contract.cornerstone_certificate_summary,
        &choir_receipt_contract.cornerstone_certificate_sha256,
        &choir_receipt_contract.cornerstone_certificate_algorithm,
        &choir_receipt_contract.foundation_receipt_summary,
        &choir_receipt_contract.foundation_receipt_sha256,
        &choir_receipt_contract.foundation_receipt_algorithm,
        &choir_receipt_contract.bedrock_certificate_summary,
        &choir_receipt_contract.bedrock_certificate_sha256,
        &choir_receipt_contract.bedrock_certificate_algorithm,
        &choir_receipt_contract.basal_receipt_summary,
        &choir_receipt_contract.basal_receipt_sha256,
        &choir_receipt_contract.basal_receipt_algorithm,
        &choir_receipt_contract.substructure_certificate_summary,
        &choir_receipt_contract.substructure_certificate_sha256,
        &choir_receipt_contract.substructure_certificate_algorithm,
        &choir_receipt_contract.plinth_receipt_summary,
        &choir_receipt_contract.plinth_receipt_sha256,
        &choir_receipt_contract.plinth_receipt_algorithm,
        &choir_receipt_contract.pedestal_certificate_summary,
        &choir_receipt_contract.pedestal_certificate_sha256,
        &choir_receipt_contract.pedestal_certificate_algorithm,
        &choir_receipt_contract.dais_receipt_summary,
        &choir_receipt_contract.dais_receipt_sha256,
        &choir_receipt_contract.dais_receipt_algorithm,
        &choir_receipt_contract.rostrum_certificate_summary,
        &choir_receipt_contract.rostrum_certificate_sha256,
        &choir_receipt_contract.rostrum_certificate_algorithm,
        &choir_receipt_contract.podium_receipt_summary,
        &choir_receipt_contract.podium_receipt_sha256,
        &choir_receipt_contract.podium_receipt_algorithm,
        &choir_receipt_contract.lectern_certificate_summary,
        &choir_receipt_contract.lectern_certificate_sha256,
        &choir_receipt_contract.lectern_certificate_algorithm,
        &choir_receipt_contract.pulpit_receipt_summary,
        &choir_receipt_contract.pulpit_receipt_sha256,
        &choir_receipt_contract.pulpit_receipt_algorithm,
        &choir_receipt_contract.chancel_certificate_summary,
        &choir_receipt_contract.chancel_certificate_sha256,
        &choir_receipt_contract.chancel_certificate_algorithm,
        &choir_receipt_contract.apse_receipt_summary,
        &choir_receipt_contract.apse_receipt_sha256,
        &choir_receipt_contract.apse_receipt_algorithm,
        &choir_receipt_contract.sanctuary_certificate_summary,
        &choir_receipt_contract.sanctuary_certificate_sha256,
        &choir_receipt_contract.sanctuary_certificate_algorithm,
        &choir_receipt_contract.nave_receipt_summary,
        &choir_receipt_contract.nave_receipt_sha256,
        &choir_receipt_contract.nave_receipt_algorithm,
        &choir_receipt_contract.transept_certificate_summary,
        &choir_receipt_contract.transept_certificate_sha256,
        &choir_receipt_contract.transept_certificate_algorithm,
        &choir_receipt_contract.choir_receipt_summary,
        &choir_receipt_contract.choir_receipt_sha256,
        &choir_receipt_contract.choir_receipt_algorithm,
    )?;

    let choir_receipt_contract_snapshot = choir_receipt_contract.clone();
    Ok(PackageClerestoryCertificateReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_clerestory_certificate".to_string(),
        verdict:
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificatePlanReady,
        reason: format!(
            "choir-receipt-native clerestory certificate is explicit for verified choir-receipt session {}; this command stays archival and read-only while freezing one final gonfalon-seal identity over the fully culminated chain",
            config.choir_receipt_session_dir.display()
        ),
        choir_receipt_session_dir: config.choir_receipt_session_dir.display().to_string(),
        lectern_certificate_session_dir: Some(
            choir_receipt_contract_snapshot.lectern_certificate_session_dir.display().to_string(),
        ),
        dais_receipt_session_dir: Some(
            choir_receipt_contract_snapshot.dais_receipt_session_dir.display().to_string(),
        ),
        pedestal_certificate_session_dir: Some(
            choir_receipt_contract_snapshot.pedestal_certificate_session_dir.display().to_string(),
        ),
        plinth_receipt_session_dir: Some(
            choir_receipt_contract_snapshot.plinth_receipt_session_dir.display().to_string(),
        ),
        substructure_certificate_session_dir: Some(
            choir_receipt_contract_snapshot
        ...
        registry_entry_session_dir: Some(
            choir_receipt_contract_snapshot.registry_entry_session_dir.display().to_string(),
        ),
        ...
        review_receipt_session_dir: Some(
            choir_receipt_contract_snapshot.review_receipt_session_dir.display().to_string(),
        ),
        handoff_bundle_session_dir: Some(
            choir_receipt_contract_snapshot.handoff_bundle_session_dir.display().to_string(),
        ),
        ...
        turn_green_session_dir: Some(
            choir_receipt_contract_snapshot.turn_green_session_dir.display().to_string(),
        ),
        launch_packet_session_dir: Some(
            choir_receipt_contract_snapshot.launch_packet_session_dir.display().to_string(),
        ),
        package_dir: Some(choir_receipt_contract_snapshot.package_dir.display().to_string()),
        install_root: Some(choir_receipt_contract_snapshot.install_root.display().to_string()),
        target_service_name: Some(choir_receipt_contract_snapshot.target_service_name.clone()),
        backend_command: Some(choir_receipt_contract_snapshot.backend_command.clone()),
        wrapper_timeout_ms: Some(choir_receipt_contract_snapshot.wrapper_timeout_ms),
        service_status_max_staleness_ms: Some(choir_receipt_contract_snapshot.service_status_max_staleness_ms),
        session_dir: Some(session_dir.display().to_string()),
        choir_receipt_session_path: None,
        choir_receipt_status_path: None,
        archived_choir_receipt_report_path: None,
        result: Some(result),
        handoff_bundle_result: choir_receipt_contract_snapshot.handoff_bundle_result.clone(),
        decision_packet_result: choir_receipt_contract_snapshot.decision_packet_result.clone(),
        execute_frozen_result: choir_receipt_contract_snapshot.execute_frozen_result.clone(),
        current_pre_activation_gate_verdict: choir_receipt_contract_snapshot
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: choir_receipt_contract_snapshot
            .current_pre_activation_gate_reason
            .clone(),
        ...
        provenance_certificate_summary: Some(choir_receipt_contract_snapshot.provenance_certificate_summary.clone()),
        chain_fingerprint_summary: Some(choir_receipt_contract_snapshot.chain_fingerprint_summary.clone()),
        chain_fingerprint_sha256: Some(choir_receipt_contract_snapshot.chain_fingerprint_sha256.clone()),
        chain_fingerprint_algorithm: Some(choir_receipt_contract_snapshot.chain_fingerprint_algorithm.clone()),
        release_capsule_digest_manifest_sha256: Some(
            choir_receipt_contract_snapshot.release_capsule_digest_manifest_sha256.clone(),
        ),
        release_capsule_digest_manifest_entry_count: Some(
            choir_receipt_contract_snapshot.release_capsule_digest_manifest_entry_count,
        ),
        release_capsule_digest_algorithm: Some(
            choir_receipt_contract_snapshot.release_capsule_digest_algorithm.clone(),
        ),
        notarization_receipt_summary: Some(choir_receipt_contract_snapshot.notarization_receipt_summary.clone()),
        ledger_seal_summary: Some(choir_receipt_contract_snapshot.ledger_seal_summary.clone()),
        ledger_seal_sha256: Some(choir_receipt_contract_snapshot.ledger_seal_sha256.clone()),
        ledger_seal_algorithm: Some(choir_receipt_contract_snapshot.ledger_seal_algorithm.clone()),
        registry_entry_summary: Some(choir_receipt_contract_snapshot.registry_entry_summary.clone()),
        registry_entry_sha256: Some(choir_receipt_contract_snapshot.registry_entry_sha256.clone()),
        registry_entry_algorithm: Some(choir_receipt_contract_snapshot.registry_entry_algorithm.clone()),
        filing_certificate_summary: Some(choir_receipt_contract_snapshot.filing_certificate_summary.clone()),
        filing_certificate_sha256: Some(choir_receipt_contract_snapshot.filing_certificate_sha256.clone()),
        filing_certificate_algorithm: Some(choir_receipt_contract_snapshot.filing_certificate_algorithm.clone()),
        archive_receipt_summary: Some(choir_receipt_contract_snapshot.archive_receipt_summary.clone()),
        archive_receipt_sha256: Some(choir_receipt_contract_snapshot.archive_receipt_sha256.clone()),
        archive_receipt_algorithm: Some(choir_receipt_contract_snapshot.archive_receipt_algorithm.clone()),
        closure_certificate_summary: Some(choir_receipt_contract_snapshot.closure_certificate_summary.clone()),
        closure_certificate_sha256: Some(choir_receipt_contract_snapshot.closure_certificate_sha256.clone()),
        closure_certificate_algorithm: Some(
            choir_receipt_contract_snapshot.closure_certificate_algorithm.clone(),
        ),
        finality_receipt_summary: Some(choir_receipt_contract_snapshot.finality_receipt_summary.clone()),
        finality_receipt_sha256: Some(choir_receipt_contract_snapshot.finality_receipt_sha256.clone()),
        finality_receipt_algorithm: Some(choir_receipt_contract_snapshot.finality_receipt_algorithm.clone()),
        consummation_record_summary: Some(choir_receipt_contract_snapshot.consummation_record_summary.clone()),
        consummation_record_sha256: Some(choir_receipt_contract_snapshot.consummation_record_sha256.clone()),
        consummation_record_algorithm: Some(choir_receipt_contract_snapshot.consummation_record_algorithm.clone()),
        completion_certificate_summary: Some(
            choir_receipt_contract_snapshot.completion_certificate_summary.clone(),
        ),
        completion_certificate_sha256: Some(
            choir_receipt_contract_snapshot.completion_certificate_sha256.clone(),
        ),
        completion_certificate_algorithm: Some(
            choir_receipt_contract_snapshot.completion_certificate_algorithm.clone(),
        ),
        summit_certificate_summary: Some(choir_receipt_contract_snapshot.summit_certificate_summary.clone()),
        summit_certificate_sha256: Some(choir_receipt_contract_snapshot.summit_certificate_sha256.clone()),
        summit_certificate_algorithm: Some(
            choir_receipt_contract_snapshot.summit_certificate_algorithm.clone(),
        ),
        culmination_receipt_summary: Some(
            choir_receipt_contract_snapshot.culmination_receipt_summary.clone(),
        ),
        culmination_receipt_sha256: Some(
            choir_receipt_contract_snapshot.culmination_receipt_sha256.clone(),
        ),
        culmination_receipt_algorithm: Some(
            choir_receipt_contract_snapshot.culmination_receipt_algorithm.clone(),
        ),
        pinnacle_receipt_summary: Some(choir_receipt_contract_snapshot.pinnacle_receipt_summary.clone()),
        pinnacle_receipt_sha256: Some(choir_receipt_contract_snapshot.pinnacle_receipt_sha256.clone()),
        pinnacle_receipt_algorithm: Some(
            choir_receipt_contract_snapshot.pinnacle_receipt_algorithm.clone(),
        ),
        capstone_certificate_summary: Some(
            choir_receipt_contract_snapshot.capstone_certificate_summary.clone(),
        ),
        capstone_certificate_sha256: Some(
            choir_receipt_contract_snapshot.capstone_certificate_sha256.clone(),
        ),
        capstone_certificate_algorithm: Some(
            choir_receipt_contract_snapshot.capstone_certificate_algorithm.clone(),
        ),
        keystone_receipt_summary: Some(choir_receipt_contract_snapshot.keystone_receipt_summary.clone()),
        keystone_receipt_sha256: Some(choir_receipt_contract_snapshot.keystone_receipt_sha256.clone()),
        keystone_receipt_algorithm: Some(
            choir_receipt_contract_snapshot.keystone_receipt_algorithm.clone(),
        ),
        cornerstone_certificate_summary: Some(choir_receipt_contract_snapshot.cornerstone_certificate_summary.clone()),
        cornerstone_certificate_sha256: Some(choir_receipt_contract_snapshot.cornerstone_certificate_sha256.clone()),
        cornerstone_certificate_algorithm: Some(choir_receipt_contract_snapshot.cornerstone_certificate_algorithm.clone()),
        foundation_receipt_summary: Some(choir_receipt_contract_snapshot.foundation_receipt_summary.clone()),
        foundation_receipt_sha256: Some(choir_receipt_contract_snapshot.foundation_receipt_sha256.clone()),
        foundation_receipt_algorithm: Some(choir_receipt_contract_snapshot.foundation_receipt_algorithm.clone()),
        bedrock_certificate_summary: Some(choir_receipt_contract_snapshot.bedrock_certificate_summary.clone()),
        bedrock_certificate_sha256: Some(choir_receipt_contract_snapshot.bedrock_certificate_sha256.clone()),
        bedrock_certificate_algorithm: Some(
            choir_receipt_contract_snapshot.bedrock_certificate_algorithm.clone(),
        ),
        basal_receipt_summary: Some(choir_receipt_contract_snapshot.basal_receipt_summary.clone()),
        basal_receipt_sha256: Some(choir_receipt_contract_snapshot.basal_receipt_sha256.clone()),
        basal_receipt_algorithm: Some(
            choir_receipt_contract_snapshot.basal_receipt_algorithm.clone(),
        ),
        substructure_certificate_summary: Some(
            choir_receipt_contract_snapshot.substructure_certificate_summary.clone(),
        ),
        substructure_certificate_sha256: Some(
            choir_receipt_contract_snapshot.substructure_certificate_sha256.clone(),
        ),
        substructure_certificate_algorithm: Some(
            choir_receipt_contract_snapshot.substructure_certificate_algorithm.clone(),
        ),
        plinth_receipt_summary: Some(choir_receipt_contract_snapshot.plinth_receipt_summary.clone()),
        plinth_receipt_sha256: Some(choir_receipt_contract_snapshot.plinth_receipt_sha256.clone()),
        plinth_receipt_algorithm: Some(choir_receipt_contract_snapshot.plinth_receipt_algorithm.clone()),
        pedestal_certificate_summary: Some(choir_receipt_contract_snapshot.pedestal_certificate_summary.clone()),
        pedestal_certificate_sha256: Some(choir_receipt_contract_snapshot.pedestal_certificate_sha256.clone()),
        pedestal_certificate_algorithm: Some(choir_receipt_contract_snapshot.pedestal_certificate_algorithm.clone()),
        dais_receipt_summary: Some(choir_receipt_contract_snapshot.dais_receipt_summary.clone()),
        dais_receipt_sha256: Some(choir_receipt_contract_snapshot.dais_receipt_sha256.clone()),
        dais_receipt_algorithm: Some(choir_receipt_contract_snapshot.dais_receipt_algorithm.clone()),
        rostrum_certificate_summary: Some(choir_receipt_contract_snapshot.rostrum_certificate_summary.clone()),
        rostrum_certificate_sha256: Some(choir_receipt_contract_snapshot.rostrum_certificate_sha256.clone()),
        rostrum_certificate_algorithm: Some(choir_receipt_contract_snapshot.rostrum_certificate_algorithm.clone()),
        podium_receipt_summary: Some(choir_receipt_contract_snapshot.podium_receipt_summary.clone()),
        podium_receipt_sha256: Some(choir_receipt_contract_snapshot.podium_receipt_sha256.clone()),
        podium_receipt_algorithm: Some(choir_receipt_contract_snapshot.podium_receipt_algorithm.clone()),
        lectern_certificate_summary: Some(choir_receipt_contract_snapshot.lectern_certificate_summary.clone()),
        lectern_certificate_sha256: Some(choir_receipt_contract_snapshot.lectern_certificate_sha256.clone()),
        lectern_certificate_algorithm: Some(choir_receipt_contract_snapshot.lectern_certificate_algorithm.clone()),
        pulpit_receipt_summary: Some(
            choir_receipt_contract_snapshot.pulpit_receipt_summary.clone(),
        ),
        pulpit_receipt_sha256: Some(
            choir_receipt_contract_snapshot.pulpit_receipt_sha256.clone(),
        ),
        pulpit_receipt_algorithm: Some(
            choir_receipt_contract_snapshot.pulpit_receipt_algorithm.clone(),
        ),
        chancel_certificate_summary: Some(
            choir_receipt_contract.chancel_certificate_summary.clone(),
        ),
        chancel_certificate_sha256: Some(
            choir_receipt_contract.chancel_certificate_sha256.clone(),
        ),
        chancel_certificate_algorithm: Some(
            choir_receipt_contract.chancel_certificate_algorithm.clone(),
        ),
        apse_receipt_summary: Some(
            choir_receipt_contract.apse_receipt_summary.clone(),
        ),
        apse_receipt_sha256: Some(
            choir_receipt_contract.apse_receipt_sha256.clone(),
        ),
        apse_receipt_algorithm: Some(
            choir_receipt_contract.apse_receipt_algorithm.clone(),
        ),
        sanctuary_certificate_summary: Some(
            choir_receipt_contract.sanctuary_certificate_summary.clone(),
        ),
        sanctuary_certificate_sha256: Some(
            choir_receipt_contract.sanctuary_certificate_sha256.clone(),
        ),
        sanctuary_certificate_algorithm: Some(
            choir_receipt_contract.sanctuary_certificate_algorithm.clone(),
        ),
        nave_receipt_summary: Some(
            choir_receipt_contract.nave_receipt_summary.clone(),
        ),
        nave_receipt_sha256: Some(
            choir_receipt_contract.nave_receipt_sha256.clone(),
        ),
        nave_receipt_algorithm: Some(
            choir_receipt_contract.nave_receipt_algorithm.clone(),
        ),
        transept_certificate_summary: Some(
            choir_receipt_contract.transept_certificate_summary.clone(),
        ),
        transept_certificate_sha256: Some(
            choir_receipt_contract.transept_certificate_sha256.clone(),
        ),
        transept_certificate_algorithm: Some(
            choir_receipt_contract.transept_certificate_algorithm.clone(),
        ),
        choir_receipt_summary: Some(
            choir_receipt_contract.choir_receipt_summary.clone(),
        ),
        choir_receipt_sha256: Some(
            choir_receipt_contract.choir_receipt_sha256.clone(),
        ),
        choir_receipt_algorithm: Some(
            choir_receipt_contract.choir_receipt_algorithm.clone(),
        ),
        clerestory_certificate_summary: Some(clerestory_certificate_summary(
            classification.0,
            &choir_receipt_contract.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        clerestory_certificate_sha256: Some(clerestory_certificate_sha256),
        clerestory_certificate_algorithm: Some(CLERESTORY_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: CLERESTORY_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_clerestory_certificate_report(
    config: &Config,
) -> Result<PackageClerestoryCertificateReport> {
    let plan = plan_live_package_clerestory_certificate_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_clerestory_certificate --choir-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-clerestory-certificate --json\ncopybot_tiny_live_activation_package_clerestory_certificate --choir-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-clerestory-certificate --json\n",
        shell_escape_path(&config.choir_receipt_session_dir),
    ...
    Ok(PackageClerestoryCertificateReport {
        generated_at: Utc::now(),
        mode: "render_live_package_clerestory_certificate".to_string(),
        verdict:
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateRendered,
        reason: format!(
            "rendered clerestory-certificate run/verify script to {}; the primary input remains the immutable choir-receipt session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: CLERESTORY_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_clerestory_certificate_report(
    config: &Config,
) -> Result<PackageClerestoryCertificateReport> {
    ...
    let verified_choir_receipt = match package_choir_receipt_clerestory_certificate::verify_live_package_choir_receipt_for_clerestory_certificate(
        &config.choir_receipt_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify choir-receipt session under {}: {error}",
                    config.choir_receipt_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_choir_receipt.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "clerestory-certificate session dir {} overlaps the managed live target surface derived from verified choir-receipt install_root {}",
            session_dir.display(),
            verified_choir_receipt.contract.install_root.display()
        )
    })?;
    ensure_clean_clerestory_certificate_session_dir(session_dir)?;
    let paths = clerestory_certificate_paths(session_dir);
    let classification = classify_verified_choir_receipt(&verified_choir_receipt);
    let result = classification.0;
    let clerestory_certificate_sha256 = compute_clerestory_certificate_sha256(
        &serialize_enum(&result),
        verified_choir_receipt
            .contract
        ...
        &verified_choir_receipt
            .contract
            .reviewed_frozen_live_cutover_controller_command_summary,
        &verified_choir_receipt.contract.chain_fingerprint_sha256,
        &verified_choir_receipt.contract.chain_fingerprint_algorithm,
        &verified_choir_receipt
            .contract
        ...
        &verified_choir_receipt.contract.ledger_seal_sha256,
        &verified_choir_receipt.contract.ledger_seal_algorithm,
        &verified_choir_receipt.contract.registry_entry_sha256,
        &verified_choir_receipt.contract.registry_entry_algorithm,
        &verified_choir_receipt.contract.filing_certificate_sha256,
        &verified_choir_receipt.contract.filing_certificate_algorithm,
        &verified_choir_receipt.contract.archive_receipt_sha256,
        &verified_choir_receipt.contract.archive_receipt_algorithm,
        &verified_choir_receipt.contract.closure_certificate_summary,
        &verified_choir_receipt.contract.closure_certificate_sha256,
        &verified_choir_receipt
            .contract
            .closure_certificate_algorithm,
        &verified_choir_receipt.contract.finality_receipt_sha256,
        &verified_choir_receipt.contract.finality_receipt_algorithm,
        &verified_choir_receipt.contract.consummation_record_sha256,
        &verified_choir_receipt
            .contract
        ...
        &verified_choir_receipt.contract.summit_certificate_summary,
        &verified_choir_receipt.contract.summit_certificate_sha256,
        &verified_choir_receipt.contract.summit_certificate_algorithm,
        &verified_choir_receipt.contract.culmination_receipt_summary,
        &verified_choir_receipt.contract.culmination_receipt_sha256,
        &verified_choir_receipt
            .contract
            .culmination_receipt_algorithm,
        &verified_choir_receipt.contract.pinnacle_receipt_summary,
        &verified_choir_receipt.contract.pinnacle_receipt_sha256,
        &verified_choir_receipt.contract.pinnacle_receipt_algorithm,
        &verified_choir_receipt.contract.capstone_certificate_summary,
        &verified_choir_receipt.contract.capstone_certificate_sha256,
        &verified_choir_receipt
            .contract
            .capstone_certificate_algorithm,
        &verified_choir_receipt.contract.keystone_receipt_summary,
        &verified_choir_receipt.contract.keystone_receipt_sha256,
        &verified_choir_receipt.contract.keystone_receipt_algorithm,
        &verified_choir_receipt
            .contract
        ...
        &verified_choir_receipt.contract.foundation_receipt_summary,
        &verified_choir_receipt.contract.foundation_receipt_sha256,
        &verified_choir_receipt.contract.foundation_receipt_algorithm,
        &verified_choir_receipt.contract.bedrock_certificate_summary,
        &verified_choir_receipt.contract.bedrock_certificate_sha256,
        &verified_choir_receipt
            .contract
            .bedrock_certificate_algorithm,
        &verified_choir_receipt.contract.basal_receipt_summary,
        &verified_choir_receipt.contract.basal_receipt_sha256,
        &verified_choir_receipt.contract.basal_receipt_algorithm,
        &verified_choir_receipt
            .contract
        ...
        &verified_choir_receipt.contract.plinth_receipt_summary,
        &verified_choir_receipt.contract.plinth_receipt_sha256,
        &verified_choir_receipt.contract.plinth_receipt_algorithm,
        &verified_choir_receipt.contract.pedestal_certificate_summary,
        &verified_choir_receipt.contract.pedestal_certificate_sha256,
        &verified_choir_receipt
            .contract
            .pedestal_certificate_algorithm,
        &verified_choir_receipt.contract.dais_receipt_summary,
        &verified_choir_receipt.contract.dais_receipt_sha256,
        &verified_choir_receipt.contract.dais_receipt_algorithm,
        &verified_choir_receipt.contract.rostrum_certificate_summary,
        &verified_choir_receipt.contract.rostrum_certificate_sha256,
        &verified_choir_receipt
            .contract
            .rostrum_certificate_algorithm,
        &verified_choir_receipt.contract.podium_receipt_summary,
        &verified_choir_receipt.contract.podium_receipt_sha256,
        &verified_choir_receipt.contract.podium_receipt_algorithm,
        &verified_choir_receipt.contract.lectern_certificate_summary,
        &verified_choir_receipt.contract.lectern_certificate_sha256,
        &verified_choir_receipt
            .contract
            .lectern_certificate_algorithm,
        &verified_choir_receipt.contract.pulpit_receipt_summary,
        &verified_choir_receipt.contract.pulpit_receipt_sha256,
        &verified_choir_receipt.contract.pulpit_receipt_algorithm,
        &verified_choir_receipt.contract.chancel_certificate_summary,
        &verified_choir_receipt.contract.chancel_certificate_sha256,
        &verified_choir_receipt
            .contract
            .chancel_certificate_algorithm,
        &verified_choir_receipt.contract.apse_receipt_summary,
        &verified_choir_receipt.contract.apse_receipt_sha256,
        &verified_choir_receipt.contract.apse_receipt_algorithm,
        &verified_choir_receipt
            .contract
            .sanctuary_certificate_summary,
        &verified_choir_receipt.contract.sanctuary_certificate_sha256,
        &verified_choir_receipt
            .contract
            .sanctuary_certificate_algorithm,
        &verified_choir_receipt.contract.nave_receipt_summary,
        &verified_choir_receipt.contract.nave_receipt_sha256,
        &verified_choir_receipt.contract.nave_receipt_algorithm,
        &verified_choir_receipt.contract.transept_certificate_summary,
        &verified_choir_receipt.contract.transept_certificate_sha256,
        &verified_choir_receipt
            .contract
            .transept_certificate_algorithm,
        &verified_choir_receipt.contract.choir_receipt_summary,
        &verified_choir_receipt.contract.choir_receipt_sha256,
        &verified_choir_receipt.contract.choir_receipt_algorithm,
    )?;
    let session = planned_session(
        config,
        session_dir,
        &verified_choir_receipt.contract,
        result,
    ...
    persist_json(
        ...
        &verified_choir_receipt.report_json,
    )?;
    let choir_receipt_step = Some(step_artifact(
        &paths.choir_receipt_report_path,
        "verify_live_package_choir_receipt",
        &verified_choir_receipt.verdict,
        &verified_choir_receipt.reason,
        verified_choir_receipt.generated_at,
    ));
    let status = PackageClerestoryCertificateStatus {
        status_version: CLERESTORY_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        choir_receipt_session_dir: config.choir_receipt_session_dir.display().to_string(),
        lectern_certificate_session_dir: verified_choir_receipt
            .contract
            .lectern_certificate_session_dir
            .display()
            .to_string(),
        dais_receipt_session_dir: verified_choir_receipt
            .contract
            .dais_receipt_session_dir
            .display()
            .to_string(),
        pedestal_certificate_session_dir: verified_choir_receipt
            .contract
            .pedestal_certificate_session_dir
            .display()
            .to_string(),
        plinth_receipt_session_dir: verified_choir_receipt
            .contract
            .plinth_receipt_session_dir
            .display()
            .to_string(),
        substructure_certificate_session_dir: verified_choir_receipt
            .contract
            .substructure_certificate_session_dir
            .display()
            .to_string(),
        basal_receipt_session_dir: verified_choir_receipt
            .contract
            .basal_receipt_session_dir
            .display()
            .to_string(),
        bedrock_certificate_session_dir: verified_choir_receipt
            .contract
            .bedrock_certificate_session_dir
            .display()
            .to_string(),
        foundation_receipt_session_dir: verified_choir_receipt
            .contract
            .foundation_receipt_session_dir
            .display()
            .to_string(),
        ...
        handoff_bundle_result: verified_choir_receipt
            .contract
            .handoff_bundle_result
            .clone(),
        decision_packet_result: verified_choir_receipt
            .contract
            .decision_packet_result
            .clone(),
        execute_frozen_result: verified_choir_receipt
            .contract
            .execute_frozen_result
            .clone(),
        current_pre_activation_gate_verdict: verified_choir_receipt
            .contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_choir_receipt
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        choir_receipt_step,
        provenance_certificate_summary: verified_choir_receipt
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_choir_receipt
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_choir_receipt
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_choir_receipt
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_choir_receipt
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_choir_receipt
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_choir_receipt
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_choir_receipt
            .contract
            .notarization_receipt_summary
            .clone(),
        ledger_seal_summary: verified_choir_receipt.contract.ledger_seal_summary.clone(),
        ledger_seal_sha256: verified_choir_receipt.contract.ledger_seal_sha256.clone(),
        ledger_seal_algorithm: verified_choir_receipt
            .contract
            .ledger_seal_algorithm
            .clone(),
        registry_entry_summary: verified_choir_receipt
            .contract
            .registry_entry_summary
            .clone(),
        registry_entry_sha256: verified_choir_receipt
            .contract
            .registry_entry_sha256
            .clone(),
        registry_entry_algorithm: verified_choir_receipt
            .contract
            .registry_entry_algorithm
            .clone(),
        filing_certificate_summary: verified_choir_receipt
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_choir_receipt
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_choir_receipt
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_choir_receipt
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_choir_receipt
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_choir_receipt
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_choir_receipt
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_choir_receipt
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_choir_receipt
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_choir_receipt
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_choir_receipt
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_choir_receipt
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_choir_receipt
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_choir_receipt
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_choir_receipt
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_choir_receipt
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_choir_receipt
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_choir_receipt
            .contract
            .completion_certificate_algorithm
            .clone(),
        summit_certificate_summary: verified_choir_receipt
            .contract
            .summit_certificate_summary
            .clone(),
        summit_certificate_sha256: verified_choir_receipt
            .contract
            .summit_certificate_sha256
            .clone(),
        summit_certificate_algorithm: verified_choir_receipt
            .contract
            .summit_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_choir_receipt
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_choir_receipt
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_choir_receipt
            .contract
            .culmination_receipt_algorithm
            .clone(),
        pinnacle_receipt_summary: verified_choir_receipt
            .contract
            .pinnacle_receipt_summary
            .clone(),
        pinnacle_receipt_sha256: verified_choir_receipt
            .contract
            .pinnacle_receipt_sha256
            .clone(),
        pinnacle_receipt_algorithm: verified_choir_receipt
            .contract
            .pinnacle_receipt_algorithm
            .clone(),
        capstone_certificate_summary: verified_choir_receipt
            .contract
            .capstone_certificate_summary
            .clone(),
        capstone_certificate_sha256: verified_choir_receipt
            .contract
            .capstone_certificate_sha256
            .clone(),
        capstone_certificate_algorithm: verified_choir_receipt
            .contract
            .capstone_certificate_algorithm
            .clone(),
        keystone_receipt_summary: verified_choir_receipt
            .contract
            .keystone_receipt_summary
            .clone(),
        keystone_receipt_sha256: verified_choir_receipt
            .contract
            .keystone_receipt_sha256
            .clone(),
        keystone_receipt_algorithm: verified_choir_receipt
            .contract
            .keystone_receipt_algorithm
            .clone(),
        cornerstone_certificate_summary: verified_choir_receipt
            .contract
            .cornerstone_certificate_summary
            .clone(),
        cornerstone_certificate_sha256: verified_choir_receipt
            .contract
            .cornerstone_certificate_sha256
            .clone(),
        cornerstone_certificate_algorithm: verified_choir_receipt
            .contract
            .cornerstone_certificate_algorithm
            .clone(),
        foundation_receipt_summary: verified_choir_receipt
            .contract
            .foundation_receipt_summary
            .clone(),
        foundation_receipt_sha256: verified_choir_receipt
            .contract
            .foundation_receipt_sha256
            .clone(),
        foundation_receipt_algorithm: verified_choir_receipt
            .contract
            .foundation_receipt_algorithm
            .clone(),
        bedrock_certificate_summary: verified_choir_receipt
            .contract
            .bedrock_certificate_summary
            .clone(),
        bedrock_certificate_sha256: verified_choir_receipt
            .contract
            .bedrock_certificate_sha256
            .clone(),
        bedrock_certificate_algorithm: verified_choir_receipt
            .contract
            .bedrock_certificate_algorithm
            .clone(),
        basal_receipt_summary: verified_choir_receipt
            .contract
            .basal_receipt_summary
            .clone(),
        basal_receipt_sha256: verified_choir_receipt.contract.basal_receipt_sha256.clone(),
        basal_receipt_algorithm: verified_choir_receipt
            .contract
            .basal_receipt_algorithm
            .clone(),
        substructure_certificate_summary: verified_choir_receipt
            .contract
            .substructure_certificate_summary
            .clone(),
        substructure_certificate_sha256: verified_choir_receipt
            .contract
            .substructure_certificate_sha256
            .clone(),
        substructure_certificate_algorithm: verified_choir_receipt
            .contract
            .substructure_certificate_algorithm
            .clone(),
        plinth_receipt_summary: verified_choir_receipt
            .contract
            .plinth_receipt_summary
            .clone(),
        plinth_receipt_sha256: verified_choir_receipt
            .contract
            .plinth_receipt_sha256
            .clone(),
        plinth_receipt_algorithm: verified_choir_receipt
            .contract
            .plinth_receipt_algorithm
            .clone(),
        pedestal_certificate_summary: verified_choir_receipt
            .contract
            .pedestal_certificate_summary
            .clone(),
        pedestal_certificate_sha256: verified_choir_receipt
            .contract
            .pedestal_certificate_sha256
            .clone(),
        pedestal_certificate_algorithm: verified_choir_receipt
            .contract
            .pedestal_certificate_algorithm
            .clone(),
        dais_receipt_summary: verified_choir_receipt.contract.dais_receipt_summary.clone(),
        dais_receipt_sha256: verified_choir_receipt.contract.dais_receipt_sha256.clone(),
        dais_receipt_algorithm: verified_choir_receipt
            .contract
            .dais_receipt_algorithm
            .clone(),
        rostrum_certificate_summary: verified_choir_receipt
            .contract
            .rostrum_certificate_summary
            .clone(),
        rostrum_certificate_sha256: verified_choir_receipt
            .contract
            .rostrum_certificate_sha256
            .clone(),
        rostrum_certificate_algorithm: verified_choir_receipt
            .contract
            .rostrum_certificate_algorithm
            .clone(),
        podium_receipt_summary: verified_choir_receipt
            .contract
            .podium_receipt_summary
            .clone(),
        podium_receipt_sha256: verified_choir_receipt
            .contract
            .podium_receipt_sha256
            .clone(),
        podium_receipt_algorithm: verified_choir_receipt
            .contract
            .podium_receipt_algorithm
            .clone(),
        lectern_certificate_summary: verified_choir_receipt
            .contract
            .lectern_certificate_summary
            .clone(),
        lectern_certificate_sha256: verified_choir_receipt
            .contract
            .lectern_certificate_sha256
            .clone(),
        lectern_certificate_algorithm: verified_choir_receipt
            .contract
            .lectern_certificate_algorithm
            .clone(),
        pulpit_receipt_summary: verified_choir_receipt
            .contract
            .pulpit_receipt_summary
            .clone(),
        pulpit_receipt_sha256: verified_choir_receipt
            .contract
            .pulpit_receipt_sha256
            .clone(),
        pulpit_receipt_algorithm: verified_choir_receipt
            .contract
            .pulpit_receipt_algorithm
            .clone(),
        chancel_certificate_summary: verified_choir_receipt
            .contract
            .chancel_certificate_summary
            .clone(),
        chancel_certificate_sha256: verified_choir_receipt
            .contract
            .chancel_certificate_sha256
            .clone(),
        chancel_certificate_algorithm: verified_choir_receipt
            .contract
            .chancel_certificate_algorithm
            .clone(),
        apse_receipt_summary: verified_choir_receipt.contract.apse_receipt_summary.clone(),
        apse_receipt_sha256: verified_choir_receipt.contract.apse_receipt_sha256.clone(),
        apse_receipt_algorithm: verified_choir_receipt
            .contract
            .apse_receipt_algorithm
            .clone(),
        sanctuary_certificate_summary: verified_choir_receipt
            .contract
            .sanctuary_certificate_summary
            .clone(),
        sanctuary_certificate_sha256: verified_choir_receipt
            .contract
            .sanctuary_certificate_sha256
            .clone(),
        sanctuary_certificate_algorithm: verified_choir_receipt
            .contract
            .sanctuary_certificate_algorithm
            .clone(),
        nave_receipt_summary: verified_choir_receipt.contract.nave_receipt_summary.clone(),
        nave_receipt_sha256: verified_choir_receipt.contract.nave_receipt_sha256.clone(),
        nave_receipt_algorithm: verified_choir_receipt
            .contract
            .nave_receipt_algorithm
            .clone(),
        transept_certificate_summary: verified_choir_receipt
            .contract
            .transept_certificate_summary
            .clone(),
        transept_certificate_sha256: verified_choir_receipt
            .contract
            .transept_certificate_sha256
            .clone(),
        transept_certificate_algorithm: verified_choir_receipt
            .contract
            .transept_certificate_algorithm
            .clone(),
        choir_receipt_summary: verified_choir_receipt
            .contract
            .choir_receipt_summary
            .clone(),
        choir_receipt_sha256: verified_choir_receipt.contract.choir_receipt_sha256.clone(),
        choir_receipt_algorithm: verified_choir_receipt
            .contract
            .choir_receipt_algorithm
            .clone(),
        clerestory_certificate_summary: clerestory_certificate_summary(
            result,
            &verified_choir_receipt
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        clerestory_certificate_sha256: clerestory_certificate_sha256.clone(),
        clerestory_certificate_algorithm: CLERESTORY_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: CLERESTORY_CERTIFICATE_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_clerestory_certificate_report(
    config: &Config,
) -> Result<PackageClerestoryCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = clerestory_certificate_paths(session_dir);
    let session: PackageClerestoryCertificateSession = load_json(&paths.session_path)?;
    let status: PackageClerestoryCertificateStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.choir_receipt_session_dir,
        &config.choir_receipt_session_dir.display().to_string(),
        "clerestory-certificate choir_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "clerestory-certificate session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "clerestory-certificate status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.choir_receipt_session_dir,
        &config.choir_receipt_session_dir.display().to_string(),
        "clerestory-certificate status choir_receipt_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_choir_receipt
            ...
            .display()
            .to_string(),
        "clerestory-certificate substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_choir_receipt
            ...
            .display()
            .to_string(),
        "clerestory-certificate status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_choir_receipt
            ...
            .display()
            .to_string(),
        "clerestory-certificate bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_choir_receipt
            ...
            .display()
            .to_string(),
        "clerestory-certificate status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_choir_receipt
            ...
            .display()
            .to_string(),
        "clerestory-certificate foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_choir_receipt
            ...
            .display()
            .to_string(),
        "clerestory-certificate status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(stored_lectern_certificate_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested choir-receipt report {}",
                paths.choir_receipt_report_path.display()
            )
        })?;
    let verified_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(verified_choir_receipt.report_json.clone()).with_context(|| {
            format!(
                "failed parsing freshly verified nested choir-receipt report for {}",
                config.choir_receipt_session_dir.display()
            )
        })?;
        ...
        != verified_lectern_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "clerestory-certificate archived choir-receipt report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_lectern_certificate_report_view.generated_at,
            verified_lectern_certificate_report_view.generated_at
        ));
    }
    let expected_choir_receipt_step = step_artifact(
        &paths.choir_receipt_report_path,
        &verified_lectern_certificate_report_view.mode,
        &verified_lectern_certificate_report_view.verdict,
        &verified_lectern_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_choir_receipt_step,
        "clerestory-certificate choir_receipt_step",
        &mut mismatches,
    );
    if verified_choir_receipt.verdict != "tiny_live_package_choir_receipt_verify_ok" {
        mismatches.push(format!(
            "clerestory-certificate nested choir-receipt verification is non-green: {}",
            verified_choir_receipt.reason
        ));
    }

    let classification = classify_verified_choir_receipt(&verified_choir_receipt);
    let expected_clerestory_certificate_sha256 = compute_clerestory_certificate_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_choir_receipt.contract,
        classification.0,
        &expected_clerestory_certificate_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "clerestory-certificate session contract does not match freshly verified choir-receipt truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_choir_receipt.contract,
        classification,
        expected_choir_receipt_step.clone(),
        &expected_clerestory_certificate_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "clerestory-certificate status contract does not match freshly verified choir-receipt truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_choir_receipt.contract,
        TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateVerifyOk,
        &expected_status,
    ))
}

fn verify_choir_receipt_command_summary(choir_receipt_session_dir: &Path) -> String {
    format!(
...
fn classify_choir_receipt_contract(
    contract: &package_choir_receipt_clerestory_certificate::LivePackageChoirReceiptContractView,
    assume_verified: bool,
) -> (
    LivePackageClerestoryCertificateResult,
    TinyLivePackageClerestoryCertificateVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageClerestoryCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateRefusedNowByInvalidOrDriftedContract,
            "the choir-receipt chain is not verified, so this clerestory certificate remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageClerestoryCertificateResult::RefusedNowByStage3,
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateRefusedNowByStage3,
            "the immutable choir receipt remains refused now by Stage 3, so this clerestory certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageClerestoryCertificateResult::RefusedNowByPreActivationGate,
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateRefusedNowByPreActivationGate,
            "the immutable choir receipt remains refused now by the pre-activation gate, so this clerestory certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageClerestoryCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable choir receipt is coherent and this clerestory certificate is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageClerestoryCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateRefusedNowByInvalidOrDriftedContract,
            "the verified choir-receipt chain is invalid, drifted, or non-runnable, so this clerestory certificate remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_choir_receipt(
    verified: &package_choir_receipt_clerestory_certificate::VerifiedLivePackageChoirReceiptClerestoryCertificateStep,
) -> (
    LivePackageClerestoryCertificateResult,
    TinyLivePackageClerestoryCertificateVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_choir_receipt_verify_ok" {
        return (
            LivePackageClerestoryCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateRefusedNowByInvalidOrDriftedContract,
            "the immutable choir receipt did not verify cleanly, so this clerestory certificate remains refused"
                .to_string(),
        );
    }
    classify_choir_receipt_contract(&verified.contract, true)
}

fn clerestory_certificate_summary(
    result: LivePackageClerestoryCertificateResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageClerestoryCertificateResult::RefusedNowByStage3 => {
            "Clerestory certificate outcome: the immutable culminated chain remains refused now by Stage 3; archive this final gonfalon seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageClerestoryCertificateResult::RefusedNowByPreActivationGate => {
            "Clerestory certificate outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final gonfalon seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageClerestoryCertificateResult::RefusedNowByInvalidOrDriftedContract => {
            "Clerestory certificate outcome: the immutable culminated chain is invalid or drifted; archive this final gonfalon seal for audit only and mint a new coherent receipt before any later execution handoff.".to_string()
        }
        LivePackageClerestoryCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Clerestory certificate outcome: this immutable gonfalon seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn clerestory_certificate_paths(session_dir: &Path) -> PackageClerestoryCertificatePaths {
    PackageClerestoryCertificatePaths {
        ...
        choir_receipt_report_path: session_dir
            .join("tiny_live_activation_package_clerestory_certificate.choir_receipt.report.json"),
    }
}

fn ensure_clean_clerestory_certificate_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing clerestory-certificate session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating clerestory-certificate session dir {}",
            session_dir.display()
        )
    })
}
...
fn planned_session(
    ...
    contract: &package_choir_receipt_clerestory_certificate::LivePackageChoirReceiptContractView,
    result: LivePackageClerestoryCertificateResult,
    clerestory_certificate_sha256: &str,
) -> PackageClerestoryCertificateSession {
    PackageClerestoryCertificateSession {
        session_version: CLERESTORY_CERTIFICATE_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        choir_receipt_session_dir: config.choir_receipt_session_dir.display().to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_choir_receipt_command_summary: verify_choir_receipt_command_summary(
            &config.choir_receipt_session_dir,
        ),
        ...
        sanctuary_certificate_summary: contract.sanctuary_certificate_summary.clone(),
        sanctuary_certificate_sha256: contract.sanctuary_certificate_sha256.clone(),
        sanctuary_certificate_algorithm: contract.sanctuary_certificate_algorithm.clone(),
        nave_receipt_summary: contract.nave_receipt_summary.clone(),
        nave_receipt_sha256: contract.nave_receipt_sha256.clone(),
        nave_receipt_algorithm: contract.nave_receipt_algorithm.clone(),
        transept_certificate_summary: contract.transept_certificate_summary.clone(),
        transept_certificate_sha256: contract.transept_certificate_sha256.clone(),
        transept_certificate_algorithm: contract.transept_certificate_algorithm.clone(),
        choir_receipt_summary: contract.choir_receipt_summary.clone(),
        choir_receipt_sha256: contract.choir_receipt_sha256.clone(),
        choir_receipt_algorithm: contract.choir_receipt_algorithm.clone(),
        clerestory_certificate_summary: clerestory_certificate_summary(
            result,
            &contract.reviewed_frozen_live_cutover_controller_command_summary,
        ),
        clerestory_certificate_sha256: clerestory_certificate_sha256.to_string(),
        clerestory_certificate_algorithm: CLERESTORY_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: CLERESTORY_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    choir_receipt_step: PackageClerestoryCertificateStepArtifact,
    clerestory_certificate_sha256: &str,
) -> PackageClerestoryCertificateStatus {
    PackageClerestoryCertificateStatus {
        status_version: CLERESTORY_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        choir_receipt_step: Some(choir_receipt_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageClerestoryCertificatePaths,
    contract: &package_choir_receipt_clerestory_certificate::LivePackageChoirReceiptContractView,
    verdict: TinyLivePackageClerestoryCertificateVerdict,
    status: &PackageClerestoryCertificateStatus,
) -> PackageClerestoryCertificateReport {
    PackageClerestoryCertificateReport {
        generated_at: Utc::now(),
        mode: if verdict == TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateVerifyOk
        {
            "verify_live_package_clerestory_certificate".to_string()
        } else {
            "run_live_package_clerestory_certificate".to_string()
        },
        verdict,
        reason: status.reason.clone(),
        choir_receipt_session_dir: config
            .choir_receipt_session_dir
            .display()
            .to_string(),
        lectern_certificate_session_dir: Some(
        ...
        session_dir: Some(session_dir.display().to_string()),
        choir_receipt_session_path: Some(paths.session_path.display().to_string()),
        choir_receipt_status_path: Some(paths.status_path.display().to_string()),
        archived_choir_receipt_report_path: Some(
            paths.choir_receipt_report_path.display().to_string(),
        ),
        ...
        choir_receipt_step: status.choir_receipt_step.clone(),
        verify_choir_receipt_command_summary: Some(
            verify_choir_receipt_command_summary(&config.choir_receipt_session_dir),
        ),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
            contract
                .reviewed_frozen_live_cutover_controller_command_summary
        ...
        nave_receipt_summary: Some(status.nave_receipt_summary.clone()),
        nave_receipt_sha256: Some(status.nave_receipt_sha256.clone()),
        nave_receipt_algorithm: Some(status.nave_receipt_algorithm.clone()),
        transept_certificate_summary: Some(status.transept_certificate_summary.clone()),
        transept_certificate_sha256: Some(status.transept_certificate_sha256.clone()),
        transept_certificate_algorithm: Some(status.transept_certificate_algorithm.clone()),
        choir_receipt_summary: Some(status.choir_receipt_summary.clone()),
        choir_receipt_sha256: Some(status.choir_receipt_sha256.clone()),
        choir_receipt_algorithm: Some(status.choir_receipt_algorithm.clone()),
        clerestory_certificate_summary: Some(status.clerestory_certificate_summary.clone()),
        clerestory_certificate_sha256: Some(status.clerestory_certificate_sha256.clone()),
        clerestory_certificate_algorithm: Some(status.clerestory_certificate_algorithm.clone()),
        verification_mismatches: Vec::new(),
        explicit_statement: if verdict
            == TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateVerifyOk
        {
            CLERESTORY_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageClerestoryCertificateReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageClerestoryCertificate => {
                "plan_live_package_clerestory_certificate".to_string()
            }
            Mode::RenderLivePackageClerestoryCertificate => {
                "render_live_package_clerestory_certificate".to_string()
            }
            Mode::RunLivePackageClerestoryCertificate => {
                "run_live_package_clerestory_certificate".to_string()
            }
        ...
        sanctuary_certificate_algorithm: None,
        nave_receipt_summary: None,
        nave_receipt_sha256: None,
        nave_receipt_algorithm: None,
        transept_certificate_summary: None,
        transept_certificate_sha256: None,
        transept_certificate_algorithm: None,
        choir_receipt_summary: None,
        choir_receipt_sha256: None,
        choir_receipt_algorithm: None,
        clerestory_certificate_summary: None,
        clerestory_certificate_sha256: None,
        clerestory_certificate_algorithm: Some(CLERESTORY_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable clerestory certificate refused before persisting because the choir-receipt artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "choir-receipt-native clerestory-certificate verification failed".to_string()
    });
    PackageClerestoryCertificateReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_clerestory_certificate".to_string(),
        verdict: TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateVerifyInvalid,
        reason,
        choir_receipt_session_dir: config
            .choir_receipt_session_dir
            .display()
        ...
        sanctuary_certificate_summary: Some(contract.sanctuary_certificate_summary.clone()),
        sanctuary_certificate_sha256: Some(contract.sanctuary_certificate_sha256.clone()),
        sanctuary_certificate_algorithm: Some(contract.sanctuary_certificate_algorithm.clone()),
        nave_receipt_summary: Some(contract.nave_receipt_summary.clone()),
        nave_receipt_sha256: Some(contract.nave_receipt_sha256.clone()),
        nave_receipt_algorithm: Some(contract.nave_receipt_algorithm.clone()),
        transept_certificate_summary: Some(contract.transept_certificate_summary.clone()),
        transept_certificate_sha256: Some(contract.transept_certificate_sha256.clone()),
        transept_certificate_algorithm: Some(contract.transept_certificate_algorithm.clone()),
        choir_receipt_summary: Some(contract.choir_receipt_summary.clone()),
        choir_receipt_sha256: Some(contract.choir_receipt_sha256.clone()),
        choir_receipt_algorithm: Some(contract.choir_receipt_algorithm.clone()),
        clerestory_certificate_summary: None,
        clerestory_certificate_sha256: None,
        clerestory_certificate_algorithm: Some(CLERESTORY_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: mismatches,
        explicit_statement: CLERESTORY_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageClerestoryCertificateStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from clerestory-certificate status"
        ));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageClerestoryCertificateStepArtifact {
    PackageClerestoryCertificateStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageClerestoryCertificateStepArtifact>,
    expected: &PackageClerestoryCertificateStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_clerestory_certificate_sha256(
    result: &str,
    ...
    sanctuary_certificate_summary: &str,
    sanctuary_certificate_sha256: &str,
    sanctuary_certificate_algorithm: &str,
    nave_receipt_summary: &str,
    nave_receipt_sha256: &str,
    nave_receipt_algorithm: &str,
    transept_certificate_summary: &str,
    transept_certificate_sha256: &str,
    transept_certificate_algorithm: &str,
    choir_receipt_summary: &str,
    choir_receipt_sha256: &str,
    choir_receipt_algorithm: &str,
) -> Result<String> {
    canonical_sha256_hex(&ClerestoryCertificatePayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing clerestory-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackageClerestoryCertificateReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "choir_receipt_session_dir={}",
            report.choir_receipt_session_dir
        ),
    ];
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_clerestory_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify =
            verify_live_package_clerestory_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateVerifyOk
        );
    ...
    #[test]
    fn drifted_choir_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            "nave_receipt_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_choir_receipt",
                "verdict": "tiny_live_package_nave_receipt_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        let fixture = fake_command_fixture(
            "clerestory_certificate_managed_overlap",
        ...
        let overlap_session_dir = managed_paths
            .runtime_dir
            .join("clerestory-certificate-session");
        ...
        let error = run_live_package_clerestory_certificate_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = clerestory_certificate_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_choir_receipt_step_path() {
        let fixture = fake_command_fixture(
            "clerestory_certificate_tampered_choir_receipt_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_clerestory_certificate_report(&fixture.run_config()).unwrap();
        let paths = clerestory_certificate_paths(&fixture.choir_receipt_session_dir);
        let mut status: PackageClerestoryCertificateStatus = load_json(&paths.status_path).unwrap();
        status.choir_receipt_step.as_mut().unwrap().path =
            "/tmp/foreign.nave-receipt.report.json".to_string();
        ...
        assert_eq!(
            verify.verdict,
            TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateVerifyInvalid
        );
    }
    ...
    fn verify_rejects_tampered_rostrum_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_rostrum_certificate_sha256",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            run_live_package_clerestory_certificate_report(&fixture.run_config()).unwrap();
            let paths = clerestory_certificate_paths(&fixture.choir_receipt_session_dir);
            let mut session: PackageClerestoryCertificateSession =
            ...
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageClerestoryCertificateVerdict::TinyLivePackageClerestoryCertificateVerifyInvalid
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_rostrum_certificate_summary",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_rostrum_certificate_algorithm",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = clerestory_certificate_paths(&fixture.choir_receipt_session_dir);
        let mut status: PackageClerestoryCertificateStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageClerestoryCertificateResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_choir_receipt_report_content() {
        let fixture = fake_command_fixture(
            "clerestory_certificate_tampered_nested_nave_receipt_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        run_live_package_clerestory_certificate_report(&fixture.run_config()).unwrap();
        let paths = clerestory_certificate_paths(&fixture.choir_receipt_session_dir);
        let mut report: serde_json::Value = load_json(&paths.choir_receipt_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested choir-receipt reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_choir_receipt_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "nave_receipt_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut status: PackageClerestoryCertificateStatus = load_json(&paths.status_path).unwrap();
        status.choir_receipt_step.as_mut().unwrap().generated_at = forged_generated_at;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    fn verify_rejects_tampered_chain_ledger_registry_filing_archive_summit_closure_finality_or_consummation_identity_fields(
    ) {
        {
            ...
            run_live_package_clerestory_certificate_report(&fixture.run_config()).unwrap();
            let paths = clerestory_certificate_paths(&fixture.choir_receipt_session_dir);
            let mut status: PackageClerestoryCertificateStatus =
    ...
    fn verify_rejects_tampered_bedrock_capstone_keystone_cornerstone_or_foundation_identity_fields()
    {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_bedrock_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_capstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_keystone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_cornerstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_foundation_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_basal_receipt_sha256() {
        let fixture = fake_command_fixture(
            "clerestory_certificate_tampered_basal_receipt_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_sha256() {
        let fixture = fake_command_fixture(
            "clerestory_certificate_tampered_substructure_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_substructure_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_substructure_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_plinth_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_plinth_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_plinth_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_plinth_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pedestal_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_pedestal_certificate_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_pedestal_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_pedestal_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_podium_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_podium_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_podium_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_podium_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_lectern_certificate_sha256() {
        let fixture = fake_command_fixture(
            "clerestory_certificate_tampered_lectern_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_lectern_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_lectern_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_lectern_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_bedrock_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_bedrock_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pulpit_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_pulpit_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_pulpit_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_pulpit_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    #[test]
    fn verify_rejects_tampered_apse_receipt_sha256() {
        let fixture = fake_command_fixture(
            "clerestory_certificate_tampered_apse_receipt_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let paths = clerestory_certificate_paths(&fixture.choir_receipt_session_dir);
        let mut status: PackageClerestoryCertificateStatus = load_json(&paths.status_path).unwrap();
        status.apse_receipt_sha256 = "tampered-apse-receipt-sha256".to_string();
        persist_json(&paths.status_path, &status).unwrap();
        let verify =
            verify_live_package_clerestory_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
    ...
    fn verify_rejects_tampered_chancel_certificate_sha256() {
        let fixture = fake_command_fixture(
            "clerestory_certificate_tampered_chancel_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_chancel_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_chancel_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_chancel_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    #[test]
    fn verify_rejects_tampered_apse_receipt_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_apse_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.apse_receipt_summary = "tampered apse receipt summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_apse_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.apse_receipt_algorithm = "tampered-apse-receipt-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_choir_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_choir_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.choir_receipt_sha256 = "tampered-choir-receipt-sha256".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_choir_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.choir_receipt_summary = "tampered choir receipt summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_choir_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.choir_receipt_algorithm = "tampered-choir-receipt-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nave_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_nave_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.nave_receipt_sha256 = "tampered-nave-receipt-sha256".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_nave_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.nave_receipt_summary = "tampered nave receipt summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_nave_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.nave_receipt_algorithm = "tampered-nave-receipt-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_clerestory_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_clerestory_certificate_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.clerestory_certificate_sha256 =
                "tampered-clerestory-certificate-sha256".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_clerestory_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.clerestory_certificate_summary =
                "tampered clerestory certificate summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
        ...
        {
            let fixture = fake_command_fixture(
                "clerestory_certificate_tampered_clerestory_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageClerestoryCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.clerestory_certificate_algorithm =
                "tampered-clerestory-certificate-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_clerestory_certificate_report(&fixture.verify_config())
                    .unwrap();
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        choir_receipt_session_dir: PathBuf,
        input_choir_receipt_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageClerestoryCertificate,
                choir_receipt_session_dir: self.input_choir_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.choir_receipt_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageClerestoryCertificate,
                choir_receipt_session_dir: self.input_choir_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let choir_receipt_session_dir = fixture_dir.join("clerestory-certificate-session");
        let input_choir_receipt_session_dir = fixture_dir.join("choir-receipt-session");
        let transept_certificate_session_dir = fixture_dir.join("transept-certificate-session");
        let lectern_certificate_session_dir = fixture_dir.join("lectern-certificate-session");
        let sanctuary_certificate_session_dir = fixture_dir.join("sanctuary-certificate-session");
        ...
        for path in [
            &input_choir_receipt_session_dir,
            &transept_certificate_session_dir,
            &sanctuary_certificate_session_dir,
            &lectern_certificate_session_dir,
        ...
        persist_json(
            ...
            &stored_choir_receipt_session_fixture(
                &transept_certificate_session_dir,
                &lectern_certificate_session_dir,
                &sanctuary_certificate_session_dir,
                &rostrum_certificate_session_dir,
        ...
        persist_json(
            ...
            &stored_choir_receipt_status_fixture(
                &transept_certificate_session_dir,
                &lectern_certificate_session_dir,
                &sanctuary_certificate_session_dir,
                &rostrum_certificate_session_dir,
        ...
        .unwrap();
        persist_json(
            &input_choir_receipt_session_dir.join(
                "tiny_live_activation_package_choir_receipt.transept_certificate.report.json",
            ),
            &json!({
        ...
        bind_choir_receipt_report_to_fixture(
            &mut verify_report,
            &transept_certificate_session_dir,
            &lectern_certificate_session_dir,
            &sanctuary_certificate_session_dir,
        ...
        let verify_path = fixture_dir.join("choir-receipt.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_choir_receipt_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_choir_receipt"),
            &verify_path,
            &transept_certificate_session_dir,
            &decision_packet_session_dir,
            &input_choir_receipt_session_dir,
    ...
    fn bind_choir_receipt_report_to_fixture(
        ...
        object.insert(
            "transept_certificate_session_dir".to_string(),
            json!(transept_certificate_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            "sanctuary_certificate_session_dir".to_string(),
            json!(sanctuary_certificate_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            "verify_choir_receipt_command_summary".to_string(),
            json!(format!(
                "verify immutable choir-receipt session under {} before freezing the final clerestory certificate",
                choir_receipt_session_dir.display()
            )),
        ...
        object.insert(
            "apse_receipt_sha256".to_string(),
            json!("herald-certificate-sha256"),
        );
        object.insert("apse_receipt_algorithm".to_string(), json!("sha256"));
        object.insert(
            "sanctuary_certificate_summary".to_string(),
            json!("Sanctuary certificate summary."),
        );
        object.insert(
            "sanctuary_certificate_sha256".to_string(),
            json!("banner-sha256"),
        );
        object.insert(
            "sanctuary_certificate_algorithm".to_string(),
            json!("sha256"),
        );
        object.insert(
            "nave_receipt_summary".to_string(),
            json!("Nave receipt summary."),
        );
        object.insert("nave_receipt_sha256".to_string(), json!("pennant-sha256"));
        object.insert("nave_receipt_algorithm".to_string(), json!("sha256"));
        object.insert(
            "transept_certificate_summary".to_string(),
            json!("Transept certificate summary."),
        );
        object.insert(
            "transept_certificate_sha256".to_string(),
            json!("transept-certificate-sha256"),
        );
        object.insert(
            "transept_certificate_algorithm".to_string(),
            json!("sha256"),
        );
        object.insert(
            "choir_receipt_summary".to_string(),
            json!("Choir receipt summary."),
        );
        object.insert("choir_receipt_sha256".to_string(), json!("guidon-sha256"));
        object.insert("choir_receipt_algorithm".to_string(), json!("sha256"));
        object.insert(
            "explicit_statement".to_string(),
            json!("choir-receipt verify statement"),
        );
    }

    fn choir_receipt_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_choir_receipt",
            "verdict": "tiny_live_package_choir_receipt_verify_ok",
            "reason": "choir-receipt chain is coherent",
            "result": result,
    ...
    fn write_fake_choir_receipt_verify_command(
        path: &Path,
        verify_path: &Path,
        expected_transept_certificate_session_dir: &Path,
        expected_decision_packet_session_dir: &Path,
        expected_choir_receipt_session_dir: &Path,
    ) {
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --transept-certificate-session-dir {} \"*) : ;;\n  *) echo 'unexpected transept-certificate-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected choir-receipt session dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --verify-live-package-choir-receipt \"*) : ;;\n  *) echo 'missing choir-receipt verify mode' >&2; exit 1;;\nesac\ncat '{}'\n",
            shell_escape_path(expected_transept_certificate_session_dir),
            shell_escape_path(expected_decision_packet_session_dir),
            shell_escape_path(expected_choir_receipt_session_dir),
            verify_path.display(),
        );
    ...
    fn stored_choir_receipt_session_fixture(
        transept_certificate_session_dir: &Path,
        lectern_certificate_session_dir: &Path,
        sanctuary_certificate_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("choir-receipt session statement"),
        );
        Value::Object(object)
    }

    fn stored_choir_receipt_status_fixture(
        transept_certificate_session_dir: &Path,
        lectern_certificate_session_dir: &Path,
        sanctuary_certificate_session_dir: &Path,
        _rostrum_certificate_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("choir-receipt status statement"),
        );
        Value::Object(object)
    }
}
