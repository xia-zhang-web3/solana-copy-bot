...
use copybot_app::tiny_live_activation::package_pedestal_certificate_dais_receipt;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_dais_receipt --pedestal-certificate-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-dais-receipt | --render-live-package-dais-receipt | --run-live-package-dais-receipt | --verify-live-package-dais-receipt)";
const DAIS_RECEIPT_SESSION_VERSION: &str = "1";
const DAIS_RECEIPT_STATUS_VERSION: &str = "1";
const DAIS_RECEIPT_ALGORITHM: &str = "sha256";
const DAIS_RECEIPT_SESSION_EXPLICIT_STATEMENT: &str =
...
const DAIS_RECEIPT_STATUS_EXPLICIT_STATEMENT: &str =
    "this dais receipt is archival and read-only; it never executes or enables the frozen controller";
const DAIS_RECEIPT_VERIFY_EXPLICIT_STATEMENT: &str =
...
enum Mode {
    PlanLivePackageDaisReceipt,
    RenderLivePackageDaisReceipt,
    RunLivePackageDaisReceipt,
    VerifyLivePackageDaisReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageDaisReceiptVerdict {
    TinyLivePackageDaisReceiptPlanReady,
    TinyLivePackageDaisReceiptRendered,
    TinyLivePackageDaisReceiptRefusedNowByStage3,
    TinyLivePackageDaisReceiptRefusedNowByPreActivationGate,
    TinyLivePackageDaisReceiptRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageDaisReceiptReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageDaisReceiptVerifyOk,
    TinyLivePackageDaisReceiptVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageDaisReceiptResult {
    RefusedNowByStage3,
...
struct PackageDaisReceiptStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageDaisReceiptSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageDaisReceiptStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackageDaisReceiptResult,
    reason: String,
    ...
    pedestal_certificate_step: Option<PackageDaisReceiptStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageDaisReceiptPaths {
...
struct PackageDaisReceiptReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageDaisReceiptVerdict,
    reason: String,
    pedestal_certificate_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut pedestal_certificate_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--pedestal-certificate-session-dir" => {
                pedestal_certificate_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--pedestal-certificate-session-dir",
                    args.next(),
                )?))
            }
    ...
    if matches!(mode, Mode::RenderLivePackageDaisReceipt) && output_path.is_none() {
        bail!("missing --output for --render-live-package-dais-receipt");
    }
    if matches!(
        mode,
        Mode::RunLivePackageDaisReceipt | Mode::VerifyLivePackageDaisReceipt
    ) && session_dir.is_none()
    {
    ...
    let pedestal_certificate_session_dir = pedestal_certificate_session_dir
        .ok_or_else(|| anyhow!("missing --pedestal-certificate-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn plan_live_package_dais_receipt_report(
    ...
    let (basal_receipt, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_pedestal_certificate_dais_receipt::verify_live_package_pedestal_certificate_for_dais_receipt(
                    &config.pedestal_certificate_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification =
                            classify_verified_pedestal_certificate(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_pedestal_certificate_dais_receipt::load_live_package_pedestal_certificate_contract_for_dais_receipt(
                            &config.pedestal_certificate_session_dir,
                        )?;
                        let classification =
                            classify_pedestal_certificate_contract(&raw_contract, false);
    ...
    Ok(PackageDaisReceiptReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_dais_receipt".to_string(),
        verdict:
            TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptPlanReady,
        reason: format!(
            "pedestal-certificate-native dais receipt is explicit for verified pedestal-certificate session {}; this command stays archival and read-only while freezing one final hallmark identity over the fully culminated chain",
        ...
        dais_receipt_session_path: None,
        dais_receipt_status_path: None,
        archived_pedestal_certificate_report_path: None,
        result: Some(result),
        ...
        dais_receipt_summary: Some(dais_receipt_summary(
            classification.0,
            &basal_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        dais_receipt_sha256: Some(dais_receipt_sha256),
        dais_receipt_algorithm: Some(DAIS_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: DAIS_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}
...
fn render_live_package_dais_receipt_report(
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_dais_receipt --pedestal-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-dais-receipt --json\ncopybot_tiny_live_activation_package_dais_receipt --pedestal-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-dais-receipt --json\n",
        shell_escape_path(&config.pedestal_certificate_session_dir),
    ...
    Ok(PackageDaisReceiptReport {
        generated_at: Utc::now(),
        mode: "render_live_package_dais_receipt".to_string(),
        verdict:
            TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptRendered,
        reason: format!(
            "rendered dais-receipt run/verify script to {}; the primary input remains the immutable pedestal-certificate session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: DAIS_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_dais_receipt_report(
    ...
    let verified_pedestal_certificate = match package_pedestal_certificate_dais_receipt::verify_live_package_pedestal_certificate_for_dais_receipt(
        &config.pedestal_certificate_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify pedestal-certificate session under {}: {error}",
                    config.pedestal_certificate_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_pedestal_certificate.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "dais-receipt session dir {} overlaps the managed live target surface derived from verified pedestal-certificate install_root {}",
            session_dir.display(),
            verified_pedestal_certificate.contract.install_root.display()
        )
    })?;
    ensure_clean_dais_receipt_session_dir(session_dir)?;
    let paths = dais_receipt_paths(session_dir);
    let classification = classify_verified_pedestal_certificate(&verified_pedestal_certificate);
    ...
    let session = planned_session(
        config,
        session_dir,
        &verified_pedestal_certificate.contract,
        result,
        &dais_receipt_sha256,
    );
    ...
    let pedestal_certificate_step = Some(step_artifact(
        &paths.pedestal_certificate_report_path,
        "verify_live_package_pedestal_certificate",
        &verified_pedestal_certificate.verdict,
        &verified_pedestal_certificate.reason,
        verified_pedestal_certificate.generated_at,
    ));
    let status = PackageDaisReceiptStatus {
        ...
        plinth_receipt_session_dir: verified_pedestal_certificate
            ...
            .display()
            .to_string(),
        substructure_certificate_session_dir: verified_pedestal_certificate
            .contract
            .substructure_certificate_session_dir
            .display()
            .to_string(),
        basal_receipt_session_dir: verified_pedestal_certificate
            .contract
            .basal_receipt_session_dir
            .display()
            .to_string(),
        bedrock_certificate_session_dir: verified_pedestal_certificate
            .contract
            .bedrock_certificate_session_dir
            .display()
            .to_string(),
        foundation_receipt_session_dir: verified_pedestal_certificate
            .contract
            .foundation_receipt_session_dir
            .display()
            .to_string(),
        ...
        handoff_bundle_result: verified_pedestal_certificate
            .contract
            .handoff_bundle_result
            .clone(),
        decision_packet_result: verified_pedestal_certificate
            .contract
            .decision_packet_result
            .clone(),
        execute_frozen_result: verified_pedestal_certificate
            .contract
            .execute_frozen_result
            .clone(),
        current_pre_activation_gate_verdict: verified_pedestal_certificate
            .contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_pedestal_certificate
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        pedestal_certificate_step,
        provenance_certificate_summary: verified_pedestal_certificate
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_pedestal_certificate
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_pedestal_certificate
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_pedestal_certificate
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_pedestal_certificate
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_pedestal_certificate
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_pedestal_certificate
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_pedestal_certificate
            .contract
        ...
        ledger_seal_algorithm: verified_pedestal_certificate
            ...
            .clone(),
        registry_entry_summary: verified_pedestal_certificate
            .contract
            .registry_entry_summary
            .clone(),
        registry_entry_sha256: verified_pedestal_certificate
            .contract
            .registry_entry_sha256
            .clone(),
        registry_entry_algorithm: verified_pedestal_certificate
            .contract
            .registry_entry_algorithm
            .clone(),
        filing_certificate_summary: verified_pedestal_certificate
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_pedestal_certificate
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_pedestal_certificate
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_pedestal_certificate
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_pedestal_certificate
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_pedestal_certificate
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_pedestal_certificate
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_pedestal_certificate
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_pedestal_certificate
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_pedestal_certificate
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_pedestal_certificate
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_pedestal_certificate
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_pedestal_certificate
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_pedestal_certificate
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_pedestal_certificate
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_pedestal_certificate
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_pedestal_certificate
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_pedestal_certificate
            .contract
            .completion_certificate_algorithm
            .clone(),
        summit_certificate_summary: verified_pedestal_certificate
            .contract
            .summit_certificate_summary
            .clone(),
        summit_certificate_sha256: verified_pedestal_certificate
            .contract
            .summit_certificate_sha256
            .clone(),
        summit_certificate_algorithm: verified_pedestal_certificate
            .contract
            .summit_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_pedestal_certificate
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_pedestal_certificate
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_pedestal_certificate
            .contract
            .culmination_receipt_algorithm
            .clone(),
        pinnacle_receipt_summary: verified_pedestal_certificate
            .contract
            .pinnacle_receipt_summary
            .clone(),
        pinnacle_receipt_sha256: verified_pedestal_certificate
            .contract
            .pinnacle_receipt_sha256
            .clone(),
        pinnacle_receipt_algorithm: verified_pedestal_certificate
            .contract
            .pinnacle_receipt_algorithm
            .clone(),
        capstone_certificate_summary: verified_pedestal_certificate
            .contract
            .capstone_certificate_summary
            .clone(),
        capstone_certificate_sha256: verified_pedestal_certificate
            .contract
            .capstone_certificate_sha256
            .clone(),
        capstone_certificate_algorithm: verified_pedestal_certificate
            .contract
            .capstone_certificate_algorithm
            .clone(),
        keystone_receipt_summary: verified_pedestal_certificate
            .contract
            .keystone_receipt_summary
            .clone(),
        keystone_receipt_sha256: verified_pedestal_certificate
            .contract
            .keystone_receipt_sha256
            .clone(),
        keystone_receipt_algorithm: verified_pedestal_certificate
            .contract
            .keystone_receipt_algorithm
            .clone(),
        cornerstone_certificate_summary: verified_pedestal_certificate
            .contract
            .cornerstone_certificate_summary
            .clone(),
        cornerstone_certificate_sha256: verified_pedestal_certificate
            .contract
            .cornerstone_certificate_sha256
            .clone(),
        cornerstone_certificate_algorithm: verified_pedestal_certificate
            .contract
            .cornerstone_certificate_algorithm
            .clone(),
        foundation_receipt_summary: verified_pedestal_certificate
            .contract
            .foundation_receipt_summary
            .clone(),
        foundation_receipt_sha256: verified_pedestal_certificate
            .contract
            .foundation_receipt_sha256
            .clone(),
        foundation_receipt_algorithm: verified_pedestal_certificate
            .contract
            .foundation_receipt_algorithm
            .clone(),
        bedrock_certificate_summary: verified_pedestal_certificate
            .contract
            .bedrock_certificate_summary
            .clone(),
        bedrock_certificate_sha256: verified_pedestal_certificate
            .contract
            .bedrock_certificate_sha256
            .clone(),
        bedrock_certificate_algorithm: verified_pedestal_certificate
            .contract
            .bedrock_certificate_algorithm
            .clone(),
        basal_receipt_summary: verified_pedestal_certificate
            .contract
            .basal_receipt_summary
            .clone(),
        basal_receipt_sha256: verified_pedestal_certificate
            .contract
            .basal_receipt_sha256
            .clone(),
        basal_receipt_algorithm: verified_pedestal_certificate
            .contract
            .basal_receipt_algorithm
            .clone(),
        substructure_certificate_summary: verified_pedestal_certificate
            .contract
            .substructure_certificate_summary
            .clone(),
        substructure_certificate_sha256: verified_pedestal_certificate
            .contract
            .substructure_certificate_sha256
            .clone(),
        substructure_certificate_algorithm: verified_pedestal_certificate
            .contract
            .substructure_certificate_algorithm
            .clone(),
        plinth_receipt_summary: verified_pedestal_certificate
            .contract
            .plinth_receipt_summary
            .clone(),
        plinth_receipt_sha256: verified_pedestal_certificate
            .contract
            .plinth_receipt_sha256
            .clone(),
        plinth_receipt_algorithm: verified_pedestal_certificate
            .contract
            .plinth_receipt_algorithm
            .clone(),
        pedestal_certificate_summary: verified_pedestal_certificate
            .contract
            .pedestal_certificate_summary
            .clone(),
        pedestal_certificate_sha256: verified_pedestal_certificate
            .contract
            .pedestal_certificate_sha256
            .clone(),
        pedestal_certificate_algorithm: verified_pedestal_certificate
            .contract
            .pedestal_certificate_algorithm
            .clone(),
        dais_receipt_summary: dais_receipt_summary(
            result,
            &verified_pedestal_certificate
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        dais_receipt_sha256: dais_receipt_sha256.clone(),
        dais_receipt_algorithm: DAIS_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: DAIS_RECEIPT_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_dais_receipt_report(
    ...
    let paths = dais_receipt_paths(session_dir);
    let session: PackageDaisReceiptSession = load_json(&paths.session_path)?;
    let status: PackageDaisReceiptStatus = load_json(&paths.status_path)?;
    ...
    compare_string(
        ...
        "dais-receipt session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "dais-receipt status session_dir",
        &mut mismatches,
    );
    compare_string(
    ...
    compare_string(
        ...
        &verified_pedestal_certificate
            ...
            .display()
            .to_string(),
        "dais-receipt substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_pedestal_certificate
            ...
            .display()
            .to_string(),
        "dais-receipt status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_pedestal_certificate
            ...
            .display()
            .to_string(),
        "dais-receipt bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_pedestal_certificate
            ...
            .display()
            .to_string(),
        "dais-receipt status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_pedestal_certificate
            ...
            .display()
            .to_string(),
        "dais-receipt foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_pedestal_certificate
            ...
            .display()
            .to_string(),
        "dais-receipt status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_substructure_certificate_report_view: NestedSubstructureCertificateReportView =
        serde_json::from_value(stored_basal_receipt_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested pedestal-certificate report {}",
                paths.pedestal_certificate_report_path.display()
            )
        })?;
        ...
        != verified_substructure_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "dais-receipt archived pedestal-certificate report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_substructure_certificate_report_view.generated_at,
            verified_substructure_certificate_report_view.generated_at
        ));
    }
    let expected_pedestal_certificate_step = step_artifact(
        &paths.pedestal_certificate_report_path,
        &verified_substructure_certificate_report_view.mode,
        &verified_substructure_certificate_report_view.verdict,
        &verified_substructure_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_pedestal_certificate_step,
        "dais-receipt pedestal_certificate_step",
        &mut mismatches,
    );
    if verified_pedestal_certificate.verdict != "tiny_live_package_pedestal_certificate_verify_ok" {
        mismatches.push(format!(
            "dais-receipt nested pedestal-certificate verification is non-green: {}",
            verified_pedestal_certificate.reason
        ));
    }

    let classification = classify_verified_pedestal_certificate(&verified_pedestal_certificate);
    let expected_dais_receipt_sha256 = compute_dais_receipt_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_pedestal_certificate.contract,
        classification.0,
        &expected_dais_receipt_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "dais-receipt session contract does not match freshly verified pedestal-certificate truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_pedestal_certificate.contract,
        classification,
        expected_pedestal_certificate_step.clone(),
        &expected_dais_receipt_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "dais-receipt status contract does not match freshly verified pedestal-certificate truth"
                .to_string(),
        );
    }
...
fn verify_pedestal_certificate_command_summary(pedestal_certificate_session_dir: &Path) -> String {
    format!(
...
fn classify_pedestal_certificate_contract(
    contract: &package_pedestal_certificate_dais_receipt::LivePackagePedestalCertificateContractView,
    assume_verified: bool,
) -> (
    LivePackageDaisReceiptResult,
    TinyLivePackageDaisReceiptVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageDaisReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptRefusedNowByInvalidOrDriftedContract,
            "the pedestal-certificate chain is not verified, so this dais receipt remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageDaisReceiptResult::RefusedNowByStage3,
            TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptRefusedNowByStage3,
            "the immutable pedestal certificate remains refused now by Stage 3, so this dais receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageDaisReceiptResult::RefusedNowByPreActivationGate,
            TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptRefusedNowByPreActivationGate,
            "the immutable pedestal certificate remains refused now by the pre-activation gate, so this dais receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageDaisReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable pedestal certificate is coherent and this dais receipt is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageDaisReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptRefusedNowByInvalidOrDriftedContract,
            "the verified pedestal-certificate chain is invalid, drifted, or non-runnable, so this dais receipt remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_pedestal_certificate(
    verified: &package_pedestal_certificate_dais_receipt::VerifiedLivePackagePedestalCertificateDaisReceiptStep,
) -> (
    LivePackageDaisReceiptResult,
    TinyLivePackageDaisReceiptVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_pedestal_certificate_verify_ok" {
        return (
            LivePackageDaisReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptRefusedNowByInvalidOrDriftedContract,
            "the immutable pedestal certificate did not verify cleanly, so this dais receipt remains refused"
                .to_string(),
        );
    }
    classify_pedestal_certificate_contract(&verified.contract, true)
}

fn dais_receipt_summary(
    result: LivePackageDaisReceiptResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageDaisReceiptResult::RefusedNowByStage3 => {
            "Pedestal certificate outcome: the immutable culminated chain remains refused now by Stage 3; archive this final hallmark seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageDaisReceiptResult::RefusedNowByPreActivationGate => {
            "Pedestal certificate outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final hallmark seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageDaisReceiptResult::RefusedNowByInvalidOrDriftedContract => {
            "Pedestal certificate outcome: the immutable culminated chain is invalid or drifted; archive this final hallmark seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackageDaisReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Pedestal certificate outcome: this immutable hallmark seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}
...
fn ensure_clean_dais_receipt_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing dais-receipt session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating dais-receipt session dir {}",
            session_dir.display()
        )
    })
}
...
fn planned_session(
    ...
    PackageDaisReceiptSession {
        ...
        session_dir: session_dir.display().to_string(),
        verify_pedestal_certificate_command_summary: verify_pedestal_certificate_command_summary(
            &config.pedestal_certificate_session_dir,
        ),
        ...
        dais_receipt_sha256: dais_receipt_sha256.to_string(),
        dais_receipt_algorithm: DAIS_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: DAIS_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    pedestal_certificate_step: PackageDaisReceiptStepArtifact,
    dais_receipt_sha256: &str,
) -> PackageDaisReceiptStatus {
    PackageDaisReceiptStatus {
        status_version: DAIS_RECEIPT_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        pedestal_certificate_step: Some(pedestal_certificate_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageDaisReceiptPaths,
    contract: &package_pedestal_certificate_dais_receipt::LivePackagePedestalCertificateContractView,
    verdict: TinyLivePackageDaisReceiptVerdict,
    status: &PackageDaisReceiptStatus,
    ...
    PackageDaisReceiptReport {
        ...
        explicit_statement: if verdict
            == TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptVerifyOk
        {
            DAIS_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageDaisReceiptReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageDaisReceipt => {
                "plan_live_package_dais_receipt".to_string()
            }
            Mode::RenderLivePackageDaisReceipt => {
                "render_live_package_dais_receipt".to_string()
            }
            Mode::RunLivePackageDaisReceipt => {
                "run_live_package_dais_receipt".to_string()
            }
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable dais receipt refused before persisting because the pedestal-certificate artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "pedestal-certificate-native dais-receipt verification failed".to_string()
    });
    PackageDaisReceiptReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_dais_receipt".to_string(),
        verdict: TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptVerifyInvalid,
        reason,
        pedestal_certificate_session_dir: config
            .pedestal_certificate_session_dir
            .display()
        ...
        explicit_statement: DAIS_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageDaisReceiptStepArtifact>,
    expected_path: &Path,
    label: &str,
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageDaisReceiptStepArtifact {
    PackageDaisReceiptStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageDaisReceiptStepArtifact>,
    expected: &PackageDaisReceiptStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_dais_receipt_sha256(
    result: &str,
...
fn render_report_lines(report: &PackageDaisReceiptReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.dais_receipt_sha256 {
        lines.push(format!("dais_receipt_sha256={value}"));
    }
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_dais_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageDaisReceiptVerdict::TinyLivePackageDaisReceiptReadyForManualExecutionWhenGateTurnsGreen
        );
    ...
    #[test]
    fn drifted_pedestal_certificate_contract_is_refused() {
        let fixture = fake_command_fixture(
            "pedestal_certificate_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_pedestal_certificate",
                "verdict": "tiny_live_package_pedestal_certificate_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        let error = run_live_package_dais_receipt_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = dais_receipt_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_pedestal_certificate_step_path() {
        let fixture = fake_command_fixture(
            "pedestal_certificate_tampered_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        status.pedestal_certificate_step.as_mut().unwrap().path =
    ...
    fn verify_rejects_tampered_pedestal_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_pedestal_certificate_sha256",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_pedestal_certificate_summary",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_pedestal_certificate_algorithm",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    #[test]
    fn verify_rejects_tampered_nested_pedestal_certificate_report_content() {
        let fixture = fake_command_fixture(
            "pedestal_certificate_tampered_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    #[test]
    fn verify_rejects_retimed_nested_pedestal_certificate_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "pedestal_certificate_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_capstone_keystone_cornerstone_or_foundation_identity_fields()
    {
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_bedrock_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_capstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_keystone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_cornerstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_foundation_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_basal_receipt_sha256() {
        let fixture = fake_command_fixture(
            "dais_receipt_tampered_basal_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_sha256() {
        let fixture = fake_command_fixture(
            "dais_receipt_tampered_substructure_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_substructure_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_substructure_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_plinth_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_plinth_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_plinth_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_plinth_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_bedrock_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_bedrock_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_dais_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_dais_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_dais_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "dais_receipt_tampered_dais_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    struct FakeCommandFixture {
        ...
        input_pedestal_certificate_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                ...
                session_dir: Some(self.dais_receipt_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        persist_json(
            ...
            &stored_pedestal_certificate_session_fixture(
                &input_pedestal_certificate_session_dir,
                &plinth_receipt_session_dir,
                &substructure_certificate_session_dir,
                &basal_receipt_session_dir,
        ...
        persist_json(
            ...
            &stored_pedestal_certificate_status_fixture(
                &plinth_receipt_session_dir,
                &substructure_certificate_session_dir,
                &verify_report,
            ),
        ...
        .unwrap();
        persist_json(
            &input_pedestal_certificate_session_dir.join(
        ...
        bind_pedestal_certificate_report_to_fixture(
            &mut verify_report,
            &input_pedestal_certificate_session_dir,
            &plinth_receipt_session_dir,
            &substructure_certificate_session_dir,
        ...
        let verify_path = fixture_dir.join("pedestal-certificate.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_pedestal_certificate_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_pedestal_certificate"),
            &verify_path,
            &plinth_receipt_session_dir,
            &decision_packet_session_dir,
            &input_pedestal_certificate_session_dir,
    ...
    fn bind_pedestal_certificate_report_to_fixture(
        ...
        object.insert(
            "verify_pedestal_certificate_command_summary".to_string(),
            json!(format!(
                "verify immutable pedestal-certificate session under {} before freezing the final dais receipt",
                pedestal_certificate_session_dir.display()
            )),
    ...
    fn pedestal_certificate_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_pedestal_certificate",
            "verdict": "tiny_live_package_pedestal_certificate_verify_ok",
            "reason": "pedestal-certificate chain is coherent",
            "result": result,
    ...
    fn bedrock_certificate_verify_report(
        ...
        pedestal_certificate_verify_report(
            result,
            handoff_bundle_result,
            decision_packet_result,
            execute_frozen_result,
    ...
    fn write_fake_pedestal_certificate_verify_command(
        path: &Path,
        verify_path: &Path,
    ...
    fn stored_pedestal_certificate_session_fixture(
        pedestal_certificate_session_dir: &Path,
        plinth_receipt_session_dir: &Path,
        substructure_certificate_session_dir: &Path,
        basal_receipt_session_dir: &Path,
    ...
    fn stored_pedestal_certificate_status_fixture(
        plinth_receipt_session_dir: &Path,
        substructure_certificate_session_dir: &Path,
        verify_report: &Value,
    ) -> Value {
