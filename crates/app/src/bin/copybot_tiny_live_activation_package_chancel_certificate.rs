...
use copybot_app::tiny_live_activation::package_pulpit_receipt_chancel_certificate;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_chancel_certificate --pulpit-receipt-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-chancel-certificate | --render-live-package-chancel-certificate | --run-live-package-chancel-certificate | --verify-live-package-chancel-certificate)";
const CHANCEL_CERTIFICATE_SESSION_VERSION: &str = "1";
const CHANCEL_CERTIFICATE_STATUS_VERSION: &str = "1";
const CHANCEL_CERTIFICATE_ALGORITHM: &str = "sha256";
const CHANCEL_CERTIFICATE_SESSION_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    pulpit_receipt_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackageChancelCertificate,
    RenderLivePackageChancelCertificate,
    RunLivePackageChancelCertificate,
    VerifyLivePackageChancelCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageChancelCertificateVerdict {
    TinyLivePackageChancelCertificatePlanReady,
    TinyLivePackageChancelCertificateRendered,
    TinyLivePackageChancelCertificateRefusedNowByStage3,
    TinyLivePackageChancelCertificateRefusedNowByPreActivationGate,
    TinyLivePackageChancelCertificateRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageChancelCertificateReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageChancelCertificateVerifyOk,
    TinyLivePackageChancelCertificateVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageChancelCertificateResult {
    RefusedNowByStage3,
...
struct PackageChancelCertificateStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageChancelCertificateSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageChancelCertificateStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackageChancelCertificateResult,
    reason: String,
    ...
    pulpit_receipt_step: Option<PackageChancelCertificateStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageChancelCertificatePaths {
...
struct PackageChancelCertificateReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageChancelCertificateVerdict,
    reason: String,
    pulpit_receipt_session_dir: String,
...
struct ChancelCertificatePayload<'a> {
    result: &'a str,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut pulpit_receipt_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--pulpit-receipt-session-dir" => {
                pulpit_receipt_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--pulpit-receipt-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-chancel-certificate" => set_mode(
                &mut mode,
                Mode::PlanLivePackageChancelCertificate,
                arg.as_str(),
            )?,
            "--render-live-package-chancel-certificate" => set_mode(
                &mut mode,
                Mode::RenderLivePackageChancelCertificate,
                arg.as_str(),
            )?,
            "--run-live-package-chancel-certificate" => set_mode(
                &mut mode,
                Mode::RunLivePackageChancelCertificate,
                arg.as_str(),
            )?,
            "--verify-live-package-chancel-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLivePackageChancelCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackageChancelCertificate) && output_path.is_none() {
        bail!("missing --output for --render-live-package-chancel-certificate");
    }
    if matches!(
        mode,
        Mode::RunLivePackageChancelCertificate | Mode::VerifyLivePackageChancelCertificate
    ) && session_dir.is_none()
    {
    ...
    let pulpit_receipt_session_dir = pulpit_receipt_session_dir
        .ok_or_else(|| anyhow!("missing --pulpit-receipt-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageChancelCertificate => {
            plan_live_package_chancel_certificate_report(&config)?
        }
        Mode::RenderLivePackageChancelCertificate => {
            render_live_package_chancel_certificate_report(&config)?
        }
        Mode::RunLivePackageChancelCertificate => {
            run_live_package_chancel_certificate_report(&config)?
        }
        Mode::VerifyLivePackageChancelCertificate => {
            verify_live_package_chancel_certificate_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_chancel_certificate_report(
    config: &Config,
) -> Result<PackageChancelCertificateReport> {
    let (lectern_certificate_contract, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_pulpit_receipt_chancel_certificate::verify_live_package_pulpit_receipt_for_chancel_certificate(
                    &config.pulpit_receipt_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification =
                            classify_verified_pulpit_receipt(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_pulpit_receipt_chancel_certificate::load_live_package_pulpit_receipt_contract_for_chancel_certificate(
                            &config.pulpit_receipt_session_dir,
                        )?;
                        let classification =
                            classify_pulpit_receipt_contract(&raw_contract, false);
    ...
    let session_dir = config.session_dir.clone().unwrap_or_else(|| {
        config
            .pulpit_receipt_session_dir
            .join("chancel-certificate")
    });
    let result = serialize_enum(&classification.0);
    let chancel_certificate_sha256 = compute_chancel_certificate_sha256(
        &result,
        ...
        lectern_certificate_contract
            ...
            .as_deref(),
        &lectern_certificate_contract.reviewed_frozen_live_cutover_controller_command_summary,
        &lectern_certificate_contract.chain_fingerprint_sha256,
        &lectern_certificate_contract.chain_fingerprint_algorithm,
        &lectern_certificate_contract.release_capsule_digest_manifest_sha256,
        lectern_certificate_contract.release_capsule_digest_manifest_entry_count,
        &lectern_certificate_contract.release_capsule_digest_algorithm,
        &lectern_certificate_contract.ledger_seal_sha256,
        &lectern_certificate_contract.ledger_seal_algorithm,
        &lectern_certificate_contract.registry_entry_sha256,
        &lectern_certificate_contract.registry_entry_algorithm,
        &lectern_certificate_contract.filing_certificate_sha256,
        &lectern_certificate_contract.filing_certificate_algorithm,
        &lectern_certificate_contract.archive_receipt_sha256,
        &lectern_certificate_contract.archive_receipt_algorithm,
        &lectern_certificate_contract.closure_certificate_summary,
        &lectern_certificate_contract.closure_certificate_sha256,
        &lectern_certificate_contract.closure_certificate_algorithm,
        &lectern_certificate_contract.finality_receipt_sha256,
        &lectern_certificate_contract.finality_receipt_algorithm,
        &lectern_certificate_contract.consummation_record_sha256,
        &lectern_certificate_contract.consummation_record_algorithm,
        &lectern_certificate_contract.completion_certificate_summary,
        &lectern_certificate_contract.completion_certificate_sha256,
        &lectern_certificate_contract.completion_certificate_algorithm,
        &lectern_certificate_contract.summit_certificate_summary,
        &lectern_certificate_contract.summit_certificate_sha256,
        &lectern_certificate_contract.summit_certificate_algorithm,
        &lectern_certificate_contract.culmination_receipt_summary,
        &lectern_certificate_contract.culmination_receipt_sha256,
        &lectern_certificate_contract.culmination_receipt_algorithm,
        &lectern_certificate_contract.pinnacle_receipt_summary,
        &lectern_certificate_contract.pinnacle_receipt_sha256,
        &lectern_certificate_contract.pinnacle_receipt_algorithm,
        &lectern_certificate_contract.capstone_certificate_summary,
        &lectern_certificate_contract.capstone_certificate_sha256,
        &lectern_certificate_contract.capstone_certificate_algorithm,
        &lectern_certificate_contract.keystone_receipt_summary,
        &lectern_certificate_contract.keystone_receipt_sha256,
        &lectern_certificate_contract.keystone_receipt_algorithm,
        &lectern_certificate_contract.cornerstone_certificate_summary,
        &lectern_certificate_contract.cornerstone_certificate_sha256,
        &lectern_certificate_contract.cornerstone_certificate_algorithm,
        &lectern_certificate_contract.foundation_receipt_summary,
        &lectern_certificate_contract.foundation_receipt_sha256,
        &lectern_certificate_contract.foundation_receipt_algorithm,
        &lectern_certificate_contract.bedrock_certificate_summary,
        &lectern_certificate_contract.bedrock_certificate_sha256,
        &lectern_certificate_contract.bedrock_certificate_algorithm,
        &lectern_certificate_contract.basal_receipt_summary,
        &lectern_certificate_contract.basal_receipt_sha256,
        &lectern_certificate_contract.basal_receipt_algorithm,
        &lectern_certificate_contract.substructure_certificate_summary,
        &lectern_certificate_contract.substructure_certificate_sha256,
        &lectern_certificate_contract.substructure_certificate_algorithm,
        &lectern_certificate_contract.plinth_receipt_summary,
        &lectern_certificate_contract.plinth_receipt_sha256,
        &lectern_certificate_contract.plinth_receipt_algorithm,
        &lectern_certificate_contract.pedestal_certificate_summary,
        &lectern_certificate_contract.pedestal_certificate_sha256,
        &lectern_certificate_contract.pedestal_certificate_algorithm,
        &lectern_certificate_contract.dais_receipt_summary,
        &lectern_certificate_contract.dais_receipt_sha256,
        &lectern_certificate_contract.dais_receipt_algorithm,
        &lectern_certificate_contract.rostrum_certificate_summary,
        &lectern_certificate_contract.rostrum_certificate_sha256,
        &lectern_certificate_contract.rostrum_certificate_algorithm,
        &lectern_certificate_contract.podium_receipt_summary,
        &lectern_certificate_contract.podium_receipt_sha256,
        &lectern_certificate_contract.podium_receipt_algorithm,
        &lectern_certificate_contract.lectern_certificate_summary,
        &lectern_certificate_contract.lectern_certificate_sha256,
        &lectern_certificate_contract.lectern_certificate_algorithm,
        &lectern_certificate_contract.pulpit_receipt_summary,
        &lectern_certificate_contract.pulpit_receipt_sha256,
        &lectern_certificate_contract.pulpit_receipt_algorithm,
    )?;

    Ok(PackageChancelCertificateReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_chancel_certificate".to_string(),
        verdict:
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificatePlanReady,
        reason: format!(
            "pulpit-receipt-native chancel certificate is explicit for verified pulpit-receipt session {}; this command stays archival and read-only while freezing one final herald identity over the fully culminated chain",
            config.pulpit_receipt_session_dir.display()
        ),
        pulpit_receipt_session_dir: config.pulpit_receipt_session_dir.display().to_string(),
        ...
        chancel_certificate_session_path: None,
        chancel_certificate_status_path: None,
        archived_pulpit_receipt_report_path: None,
        result: Some(result),
        ...
        chancel_certificate_summary: Some(chancel_certificate_summary(
            classification.0,
            &lectern_certificate_contract.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        chancel_certificate_sha256: Some(chancel_certificate_sha256),
        chancel_certificate_algorithm: Some(CHANCEL_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: CHANCEL_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_chancel_certificate_report(
    config: &Config,
) -> Result<PackageChancelCertificateReport> {
    let plan = plan_live_package_chancel_certificate_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_chancel_certificate --pulpit-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-chancel-certificate --json\ncopybot_tiny_live_activation_package_chancel_certificate --pulpit-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-chancel-certificate --json\n",
        shell_escape_path(&config.pulpit_receipt_session_dir),
    ...
    Ok(PackageChancelCertificateReport {
        generated_at: Utc::now(),
        mode: "render_live_package_chancel_certificate".to_string(),
        verdict:
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateRendered,
        reason: format!(
            "rendered chancel-certificate run/verify script to {}; the primary input remains the immutable pulpit-receipt session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: CHANCEL_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_chancel_certificate_report(
    config: &Config,
) -> Result<PackageChancelCertificateReport> {
    ...
    let verified_lectern_certificate = match package_pulpit_receipt_chancel_certificate::verify_live_package_pulpit_receipt_for_chancel_certificate(
        &config.pulpit_receipt_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify pulpit-receipt session under {}: {error}",
                    config.pulpit_receipt_session_dir.display()
                ),
            ))
        }
    ...
    .with_context(|| {
        format!(
            "chancel-certificate session dir {} overlaps the managed live target surface derived from verified pulpit-receipt install_root {}",
            session_dir.display(),
            verified_lectern_certificate.contract.install_root.display()
        )
    })?;
    ensure_clean_chancel_certificate_session_dir(session_dir)?;
    let paths = chancel_certificate_paths(session_dir);
    let classification = classify_verified_pulpit_receipt(&verified_lectern_certificate);
    ...
    let pulpit_receipt_step = Some(step_artifact(
        &paths.pulpit_receipt_report_path,
        "verify_live_package_pulpit_receipt",
        &verified_lectern_certificate.verdict,
        &verified_lectern_certificate.reason,
        verified_lectern_certificate.generated_at,
    ));
    let status = PackageChancelCertificateStatus {
        status_version: CHANCEL_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        pulpit_receipt_algorithm: verified_lectern_certificate
            ...
            .pulpit_receipt_algorithm
            .clone(),
        chancel_certificate_summary: chancel_certificate_summary(
            result,
            &verified_lectern_certificate
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        chancel_certificate_sha256: chancel_certificate_sha256.clone(),
        chancel_certificate_algorithm: CHANCEL_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: CHANCEL_CERTIFICATE_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_chancel_certificate_report(
    config: &Config,
) -> Result<PackageChancelCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = chancel_certificate_paths(session_dir);
    let session: PackageChancelCertificateSession = load_json(&paths.session_path)?;
    let status: PackageChancelCertificateStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.pulpit_receipt_session_dir,
        &config.pulpit_receipt_session_dir.display().to_string(),
        "chancel-certificate pulpit_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "chancel-certificate session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "chancel-certificate status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.pulpit_receipt_session_dir,
        &config.pulpit_receipt_session_dir.display().to_string(),
        "chancel-certificate status pulpit_receipt_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "chancel-certificate substructure_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "chancel-certificate status substructure_certificate_session_dir",
        &mut mismatches,
    );
    ...
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "chancel-certificate bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "chancel-certificate status bedrock_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "chancel-certificate foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &verified_lectern_certificate
            ...
            .display()
            .to_string(),
        "chancel-certificate status foundation_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(stored_lectern_certificate_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested pulpit-receipt report {}",
                paths.pulpit_receipt_report_path.display()
            )
        })?;
    let verified_lectern_certificate_report_view: NestedPulpitReceiptReportView =
        serde_json::from_value(verified_lectern_certificate.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested pulpit-receipt report for {}",
                    config.pulpit_receipt_session_dir.display()
                )
            },
        )?;
        ...
        != verified_lectern_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "chancel-certificate archived pulpit-receipt report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_lectern_certificate_report_view.generated_at,
            verified_lectern_certificate_report_view.generated_at
        ));
    }
    let expected_pulpit_receipt_step = step_artifact(
        &paths.pulpit_receipt_report_path,
        &verified_lectern_certificate_report_view.mode,
        &verified_lectern_certificate_report_view.verdict,
        &verified_lectern_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_pulpit_receipt_step,
        "chancel-certificate pulpit_receipt_step",
        &mut mismatches,
    );
    if verified_lectern_certificate.verdict != "tiny_live_package_pulpit_receipt_verify_ok" {
        mismatches.push(format!(
            "chancel-certificate nested pulpit-receipt verification is non-green: {}",
            verified_lectern_certificate.reason
        ));
    }

    let classification = classify_verified_pulpit_receipt(&verified_lectern_certificate);
    let expected_chancel_certificate_sha256 = compute_chancel_certificate_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_lectern_certificate.contract,
        classification.0,
        &expected_chancel_certificate_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "chancel-certificate session contract does not match freshly verified pulpit-receipt truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_lectern_certificate.contract,
        classification,
        expected_pulpit_receipt_step.clone(),
        &expected_chancel_certificate_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "chancel-certificate status contract does not match freshly verified pulpit-receipt truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_lectern_certificate.contract,
        TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateVerifyOk,
        &expected_status,
    ))
}

fn verify_pulpit_receipt_command_summary(pulpit_receipt_session_dir: &Path) -> String {
    format!(
...
fn classify_pulpit_receipt_contract(
    contract: &package_pulpit_receipt_chancel_certificate::LivePackagePulpitReceiptContractView,
    assume_verified: bool,
) -> (
    LivePackageChancelCertificateResult,
    TinyLivePackageChancelCertificateVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageChancelCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateRefusedNowByInvalidOrDriftedContract,
            "the pulpit-receipt chain is not verified, so this chancel certificate remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageChancelCertificateResult::RefusedNowByStage3,
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateRefusedNowByStage3,
            "the immutable pulpit receipt remains refused now by Stage 3, so this chancel certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageChancelCertificateResult::RefusedNowByPreActivationGate,
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateRefusedNowByPreActivationGate,
            "the immutable pulpit receipt remains refused now by the pre-activation gate, so this chancel certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageChancelCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable pulpit receipt is coherent and this chancel certificate is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageChancelCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateRefusedNowByInvalidOrDriftedContract,
            "the verified pulpit-receipt chain is invalid, drifted, or non-runnable, so this chancel certificate remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_pulpit_receipt(
    verified: &package_pulpit_receipt_chancel_certificate::VerifiedLivePackagePulpitReceiptChancelCertificateStep,
) -> (
    LivePackageChancelCertificateResult,
    TinyLivePackageChancelCertificateVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_pulpit_receipt_verify_ok" {
        return (
            LivePackageChancelCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateRefusedNowByInvalidOrDriftedContract,
            "the immutable pulpit receipt did not verify cleanly, so this chancel certificate remains refused"
                .to_string(),
        );
    }
    classify_pulpit_receipt_contract(&verified.contract, true)
}

fn chancel_certificate_summary(
    result: LivePackageChancelCertificateResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageChancelCertificateResult::RefusedNowByStage3 => {
            "Pulpit receipt outcome: the immutable culminated chain remains refused now by Stage 3; archive this final herald seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageChancelCertificateResult::RefusedNowByPreActivationGate => {
            "Pulpit receipt outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final herald seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageChancelCertificateResult::RefusedNowByInvalidOrDriftedContract => {
            "Pulpit receipt outcome: the immutable culminated chain is invalid or drifted; archive this final herald seal for audit only and mint a new coherent receipt before any later execution handoff.".to_string()
        }
        LivePackageChancelCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Pulpit receipt outcome: this immutable herald seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn chancel_certificate_paths(session_dir: &Path) -> PackageChancelCertificatePaths {
    PackageChancelCertificatePaths {
...
fn ensure_clean_chancel_certificate_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing chancel-certificate session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating chancel-certificate session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageChancelCertificateSession {
    PackageChancelCertificateSession {
        session_version: CHANCEL_CERTIFICATE_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        pulpit_receipt_session_dir: config.pulpit_receipt_session_dir.display().to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_pulpit_receipt_command_summary: verify_pulpit_receipt_command_summary(
            &config.pulpit_receipt_session_dir,
        ),
        ...
        chancel_certificate_sha256: chancel_certificate_sha256.to_string(),
        chancel_certificate_algorithm: CHANCEL_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: CHANCEL_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    pulpit_receipt_step: PackageChancelCertificateStepArtifact,
    chancel_certificate_sha256: &str,
) -> PackageChancelCertificateStatus {
    PackageChancelCertificateStatus {
        status_version: CHANCEL_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        pulpit_receipt_step: Some(pulpit_receipt_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageChancelCertificatePaths,
    contract: &package_pulpit_receipt_chancel_certificate::LivePackagePulpitReceiptContractView,
    verdict: TinyLivePackageChancelCertificateVerdict,
    status: &PackageChancelCertificateStatus,
) -> PackageChancelCertificateReport {
    PackageChancelCertificateReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateVerifyOk
        {
            "verify_live_package_chancel_certificate".to_string()
        } else {
            "run_live_package_chancel_certificate".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        chancel_certificate_session_path: Some(paths.session_path.display().to_string()),
        chancel_certificate_status_path: Some(paths.status_path.display().to_string()),
        archived_pulpit_receipt_report_path: Some(
            paths.pulpit_receipt_report_path.display().to_string(),
        ),
        ...
        pulpit_receipt_step: status.pulpit_receipt_step.clone(),
        verify_pulpit_receipt_command_summary: Some(verify_pulpit_receipt_command_summary(
            &config.pulpit_receipt_session_dir,
        )),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
        ...
        explicit_statement: if verdict
            == TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateVerifyOk
        {
            CHANCEL_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageChancelCertificateReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageChancelCertificate => {
                "plan_live_package_chancel_certificate".to_string()
            }
            Mode::RenderLivePackageChancelCertificate => {
                "render_live_package_chancel_certificate".to_string()
            }
            Mode::RunLivePackageChancelCertificate => {
                "run_live_package_chancel_certificate".to_string()
            }
        ...
        verdict,
        reason,
        pulpit_receipt_session_dir: config
            .pulpit_receipt_session_dir
            .display()
            .to_string(),
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable chancel certificate refused before persisting because the pulpit-receipt artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "pulpit-receipt-native chancel-certificate verification failed".to_string()
    });
    PackageChancelCertificateReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_chancel_certificate".to_string(),
        verdict:
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateVerifyInvalid,
        reason,
        pulpit_receipt_session_dir: config.pulpit_receipt_session_dir.display().to_string(),
        lectern_certificate_session_dir: Some(
            contract
        ...
        explicit_statement: CHANCEL_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageChancelCertificateStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from chancel-certificate status"
        ));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageChancelCertificateStepArtifact {
    PackageChancelCertificateStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageChancelCertificateStepArtifact>,
    expected: &PackageChancelCertificateStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_chancel_certificate_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&ChancelCertificatePayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents)
        .with_context(|| format!("failed writing pulpit-receipt script to {}", path.display()))?;
    #[cfg(unix)]
    {
...
fn render_report_lines(report: &PackageChancelCertificateReport) -> String {
    let mut lines = vec![
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_chancel_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify =
            verify_live_package_chancel_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageChancelCertificateVerdict::TinyLivePackageChancelCertificateVerifyOk
        );
    ...
    #[test]
    fn drifted_pulpit_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            "pulpit_receipt_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_pulpit_receipt",
                "verdict": "tiny_live_package_pulpit_receipt_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        let fixture = fake_command_fixture(
            "pulpit_receipt_managed_overlap",
        ...
        let overlap_session_dir = managed_paths
            .runtime_dir
            .join("chancel-certificate-session");
        ...
        let error = run_live_package_chancel_certificate_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = chancel_certificate_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_pulpit_receipt_step_path() {
        let fixture = fake_command_fixture(
            "pulpit_receipt_tampered_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_chancel_certificate_report(&fixture.run_config()).unwrap();
        let paths = chancel_certificate_paths(&fixture.pulpit_receipt_session_dir);
        let mut status: PackageChancelCertificateStatus = load_json(&paths.status_path).unwrap();
        status.pulpit_receipt_step.as_mut().unwrap().path =
            "/tmp/foreign.pulpit-receipt.report.json".to_string();
    ...
    fn verify_rejects_tampered_rostrum_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_rostrum_certificate_sha256",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            run_live_package_chancel_certificate_report(&fixture.run_config()).unwrap();
            let paths = chancel_certificate_paths(&fixture.pulpit_receipt_session_dir);
            let mut session: PackageChancelCertificateSession =
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_rostrum_certificate_summary",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_rostrum_certificate_algorithm",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = chancel_certificate_paths(&fixture.pulpit_receipt_session_dir);
        let mut status: PackageChancelCertificateStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageChancelCertificateResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_pulpit_receipt_report_content() {
        let fixture = fake_command_fixture(
            "pulpit_receipt_tampered_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        run_live_package_chancel_certificate_report(&fixture.run_config()).unwrap();
        let paths = chancel_certificate_paths(&fixture.pulpit_receipt_session_dir);
        let mut report: serde_json::Value = load_json(&paths.pulpit_receipt_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested pulpit-receipt reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_pulpit_receipt_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "pulpit_receipt_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut status: PackageChancelCertificateStatus = load_json(&paths.status_path).unwrap();
        status.pulpit_receipt_step.as_mut().unwrap().generated_at = forged_generated_at;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    fn verify_rejects_tampered_chain_ledger_registry_filing_archive_summit_closure_finality_or_consummation_identity_fields(
    ) {
        {
            ...
            run_live_package_chancel_certificate_report(&fixture.run_config()).unwrap();
            let paths = chancel_certificate_paths(&fixture.pulpit_receipt_session_dir);
            let mut status: PackageChancelCertificateStatus =
    ...
    fn verify_rejects_tampered_bedrock_capstone_keystone_cornerstone_or_foundation_identity_fields()
    {
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_bedrock_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_capstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_keystone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_cornerstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_foundation_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_basal_receipt_sha256() {
        let fixture = fake_command_fixture(
            "pulpit_receipt_tampered_basal_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_sha256() {
        let fixture = fake_command_fixture(
            "pulpit_receipt_tampered_substructure_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_substructure_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_substructure_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_plinth_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_plinth_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_plinth_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_plinth_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pedestal_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "chancel_certificate_tampered_pedestal_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "chancel_certificate_tampered_pedestal_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "chancel_certificate_tampered_pedestal_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_podium_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_podium_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_podium_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_podium_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_lectern_certificate_sha256() {
        let fixture = fake_command_fixture(
            "pulpit_receipt_tampered_lectern_certificate_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_lectern_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_lectern_certificate_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_lectern_certificate_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_bedrock_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "pulpit_receipt_tampered_bedrock_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    #[test]
    fn verify_rejects_tampered_chancel_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "chancel_certificate_tampered_chancel_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageChancelCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.chancel_certificate_sha256 = "tampered-chancel-sha256".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_chancel_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "chancel_certificate_tampered_chancel_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageChancelCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.chancel_certificate_summary = "tampered chancel summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_chancel_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "chancel_certificate_tampered_chancel_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageChancelCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.chancel_certificate_algorithm = "tampered-chancel-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_chancel_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        pulpit_receipt_session_dir: PathBuf,
        input_pulpit_receipt_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageChancelCertificate,
                pulpit_receipt_session_dir: self.input_pulpit_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.pulpit_receipt_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageChancelCertificate,
                pulpit_receipt_session_dir: self.input_pulpit_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let pulpit_receipt_session_dir = fixture_dir.join("chancel-certificate-session");
        let input_pulpit_receipt_session_dir = fixture_dir.join("pulpit-receipt-session");
        ...
        for path in [
            &input_pulpit_receipt_session_dir,
            &lectern_certificate_session_dir,
        ...
        persist_json(
            ...
            &stored_pulpit_receipt_session_fixture(
                &lectern_certificate_session_dir,
                &rostrum_certificate_session_dir,
                &pedestal_certificate_session_dir,
                &plinth_receipt_session_dir,
        ...
        persist_json(
            ...
            &stored_pulpit_receipt_status_fixture(
                &lectern_certificate_session_dir,
                &rostrum_certificate_session_dir,
                &pedestal_certificate_session_dir,
                &plinth_receipt_session_dir,
        ...
        .unwrap();
        persist_json(
            &input_pulpit_receipt_session_dir.join(
                "tiny_live_activation_package_pulpit_receipt.lectern_certificate.report.json",
            ),
            &json!({
        ...
        bind_pulpit_receipt_report_to_fixture(
            &mut verify_report,
            &lectern_certificate_session_dir,
            &rostrum_certificate_session_dir,
            &dais_receipt_session_dir,
        ...
        let verify_path = fixture_dir.join("pulpit-receipt.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_pulpit_receipt_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_pulpit_receipt"),
            &verify_path,
            &lectern_certificate_session_dir,
            &decision_packet_session_dir,
            &input_pulpit_receipt_session_dir,
    ...
    fn bind_pulpit_receipt_report_to_fixture(
        ...
        object.insert(
            "verify_lectern_certificate_command_summary".to_string(),
            json!(format!(
        ...
        object.insert(
            ...
            json!("pulpit-receipt verify statement"),
        );
    }

    fn pulpit_receipt_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_pulpit_receipt",
            "verdict": "tiny_live_package_pulpit_receipt_verify_ok",
            "reason": "pulpit-receipt chain is coherent",
            "result": result,
    ...
    fn stored_pulpit_receipt_session_fixture(
        lectern_certificate_session_dir: &Path,
        rostrum_certificate_session_dir: &Path,
        pedestal_certificate_session_dir: &Path,
        plinth_receipt_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("pulpit-receipt statement"),
        );
        Value::Object(object)
    }

    fn stored_pulpit_receipt_status_fixture(
        lectern_certificate_session_dir: &Path,
        rostrum_certificate_session_dir: &Path,
        pedestal_certificate_session_dir: &Path,
        plinth_receipt_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("pulpit-receipt status statement"),
        );
        Value::Object(object)
    }
}
