...
use copybot_app::tiny_live_activation::package_keystone_receipt_cornerstone_certificate;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_cornerstone_certificate --keystone-receipt-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-cornerstone-certificate | --render-live-package-cornerstone-certificate | --run-live-package-cornerstone-certificate | --verify-live-package-cornerstone-certificate)";
const CORNERSTONE_CERTIFICATE_SESSION_VERSION: &str = "1";
const CORNERSTONE_CERTIFICATE_STATUS_VERSION: &str = "1";
const CORNERSTONE_CERTIFICATE_ALGORITHM: &str = "sha256";
const CORNERSTONE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT: &str =
...
enum Mode {
    PlanLivePackageCornerstoneCertificate,
    RenderLivePackageCornerstoneCertificate,
    RunLivePackageCornerstoneCertificate,
    VerifyLivePackageCornerstoneCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageCornerstoneCertificateVerdict {
    TinyLivePackageCornerstoneCertificatePlanReady,
    TinyLivePackageCornerstoneCertificateRendered,
    TinyLivePackageCornerstoneCertificateRefusedNowByStage3,
    TinyLivePackageCornerstoneCertificateRefusedNowByPreActivationGate,
    TinyLivePackageCornerstoneCertificateRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageCornerstoneCertificateReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageCornerstoneCertificateVerifyOk,
    TinyLivePackageCornerstoneCertificateVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageCornerstoneCertificateResult {
    RefusedNowByStage3,
...
struct PackageCornerstoneCertificateStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageCornerstoneCertificateSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageCornerstoneCertificateStatus {
    status_version: String,
    updated_at: DateTime<Utc>,
    session_dir: String,
    keystone_receipt_session_dir: String,
    result: LivePackageCornerstoneCertificateResult,
    reason: String,
    ...
    keystone_receipt_step: Option<PackageCornerstoneCertificateStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageCornerstoneCertificatePaths {
...
struct PackageCornerstoneCertificateReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageCornerstoneCertificateVerdict,
    reason: String,
    keystone_receipt_session_dir: String,
...
struct NestedKeystoneReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
struct CornerstoneCertificatePayload<'a> {
    result: &'a str,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut keystone_receipt_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--keystone-receipt-session-dir" => {
                keystone_receipt_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--keystone-receipt-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-cornerstone-certificate" => set_mode(
                &mut mode,
                Mode::PlanLivePackageCornerstoneCertificate,
                arg.as_str(),
            )?,
            "--render-live-package-cornerstone-certificate" => set_mode(
                &mut mode,
    ...
    if matches!(mode, Mode::RenderLivePackageCornerstoneCertificate) && output_path.is_none() {
        bail!("missing --output for --render-live-package-cornerstone-certificate");
    }
    if matches!(
        mode,
        Mode::RunLivePackageCornerstoneCertificate | Mode::VerifyLivePackageCornerstoneCertificate
    ) && session_dir.is_none()
    {
    ...
    let keystone_receipt_session_dir = keystone_receipt_session_dir
        .ok_or_else(|| anyhow!("missing --keystone-receipt-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn plan_live_package_cornerstone_certificate_report(
    config: &Config,
) -> Result<PackageCornerstoneCertificateReport> {
    let (keystone_receipt, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_keystone_receipt_cornerstone_certificate::verify_live_package_keystone_receipt_for_cornerstone_certificate(
                    &config.keystone_receipt_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_keystone_receipt(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_keystone_receipt_cornerstone_certificate::load_live_package_keystone_receipt_contract_for_cornerstone_certificate(
                            &config.keystone_receipt_session_dir,
                        )?;
                        let classification = classify_keystone_receipt_contract(&raw_contract, false);
    ...
    let session_dir = config.session_dir.clone().unwrap_or_else(|| {
        config
            .keystone_receipt_session_dir
            .join("cornerstone-certificate")
    });
    let result = serialize_enum(&classification.0);
    let cornerstone_certificate_sha256 = compute_cornerstone_certificate_sha256(
        &result,
        keystone_receipt.handoff_bundle_result.as_deref(),
        keystone_receipt.decision_packet_result.as_deref(),
        keystone_receipt.execute_frozen_result.as_deref(),
        keystone_receipt
        ...
        keystone_receipt
            ...
            .as_deref(),
        &keystone_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        &keystone_receipt.chain_fingerprint_sha256,
        &keystone_receipt.chain_fingerprint_algorithm,
        &keystone_receipt.release_capsule_digest_manifest_sha256,
        keystone_receipt.release_capsule_digest_manifest_entry_count,
        &keystone_receipt.release_capsule_digest_algorithm,
        &keystone_receipt.ledger_seal_sha256,
        &keystone_receipt.ledger_seal_algorithm,
        &keystone_receipt.registry_entry_sha256,
        &keystone_receipt.registry_entry_algorithm,
        &keystone_receipt.filing_certificate_sha256,
        &keystone_receipt.filing_certificate_algorithm,
        &keystone_receipt.archive_receipt_sha256,
        &keystone_receipt.archive_receipt_algorithm,
        &keystone_receipt.closure_certificate_summary,
        &keystone_receipt.closure_certificate_sha256,
        &keystone_receipt.closure_certificate_algorithm,
        &keystone_receipt.finality_receipt_sha256,
        &keystone_receipt.finality_receipt_algorithm,
        &keystone_receipt.consummation_record_sha256,
        &keystone_receipt.consummation_record_algorithm,
        &keystone_receipt.completion_certificate_summary,
        &keystone_receipt.completion_certificate_sha256,
        &keystone_receipt.completion_certificate_algorithm,
        &keystone_receipt.summit_certificate_summary,
        &keystone_receipt.summit_certificate_sha256,
        &keystone_receipt.summit_certificate_algorithm,
        &keystone_receipt.culmination_receipt_summary,
        &keystone_receipt.culmination_receipt_sha256,
        &keystone_receipt.culmination_receipt_algorithm,
        &keystone_receipt.pinnacle_receipt_summary,
        &keystone_receipt.pinnacle_receipt_sha256,
        &keystone_receipt.pinnacle_receipt_algorithm,
        &keystone_receipt.capstone_certificate_summary,
        &keystone_receipt.capstone_certificate_sha256,
        &keystone_receipt.capstone_certificate_algorithm,
        &keystone_receipt.keystone_receipt_summary,
        &keystone_receipt.keystone_receipt_sha256,
        &keystone_receipt.keystone_receipt_algorithm,
    )?;

    Ok(PackageCornerstoneCertificateReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_cornerstone_certificate".to_string(),
        verdict:
            TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificatePlanReady,
        reason: format!(
            "cornerstone-certificate-native cornerstone certificate is explicit for verified keystone-receipt session {}; this command stays archival and read-only while freezing one final regalia identity over the fully culminated chain",
            config.keystone_receipt_session_dir.display()
        ),
        keystone_receipt_session_dir: config.keystone_receipt_session_dir.display().to_string(),
        registry_entry_session_dir: Some(
            keystone_receipt.registry_entry_session_dir.display().to_string(),
        ),
        ...
        review_receipt_session_dir: Some(
            keystone_receipt.review_receipt_session_dir.display().to_string(),
        ),
        handoff_bundle_session_dir: Some(
            keystone_receipt.handoff_bundle_session_dir.display().to_string(),
        ),
        ...
        turn_green_session_dir: Some(
            keystone_receipt.turn_green_session_dir.display().to_string(),
        ),
        launch_packet_session_dir: Some(
            keystone_receipt.launch_packet_session_dir.display().to_string(),
        ),
        package_dir: Some(keystone_receipt.package_dir.display().to_string()),
        install_root: Some(keystone_receipt.install_root.display().to_string()),
        target_service_name: Some(keystone_receipt.target_service_name.clone()),
        backend_command: Some(keystone_receipt.backend_command.clone()),
        wrapper_timeout_ms: Some(keystone_receipt.wrapper_timeout_ms),
        service_status_max_staleness_ms: Some(keystone_receipt.service_status_max_staleness_ms),
        session_dir: Some(session_dir.display().to_string()),
        cornerstone_certificate_session_path: None,
        cornerstone_certificate_status_path: None,
        archived_keystone_receipt_report_path: None,
        result: Some(result),
        handoff_bundle_result: keystone_receipt.handoff_bundle_result.clone(),
        decision_packet_result: keystone_receipt.decision_packet_result.clone(),
        execute_frozen_result: keystone_receipt.execute_frozen_result.clone(),
        current_pre_activation_gate_verdict: keystone_receipt
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: keystone_receipt
            .current_pre_activation_gate_reason
            .clone(),
        keystone_receipt_step: None,
        verify_keystone_receipt_command_summary: Some(verify_keystone_receipt_command_summary(
            &config.keystone_receipt_session_dir,
        )),
        ...
        provenance_certificate_summary: Some(keystone_receipt.provenance_certificate_summary.clone()),
        chain_fingerprint_summary: Some(keystone_receipt.chain_fingerprint_summary.clone()),
        chain_fingerprint_sha256: Some(keystone_receipt.chain_fingerprint_sha256.clone()),
        chain_fingerprint_algorithm: Some(keystone_receipt.chain_fingerprint_algorithm.clone()),
        release_capsule_digest_manifest_sha256: Some(
            keystone_receipt.release_capsule_digest_manifest_sha256.clone(),
        ),
        release_capsule_digest_manifest_entry_count: Some(
            keystone_receipt.release_capsule_digest_manifest_entry_count,
        ),
        release_capsule_digest_algorithm: Some(
            keystone_receipt.release_capsule_digest_algorithm.clone(),
        ),
        notarization_receipt_summary: Some(keystone_receipt.notarization_receipt_summary.clone()),
        ledger_seal_summary: Some(keystone_receipt.ledger_seal_summary.clone()),
        ledger_seal_sha256: Some(keystone_receipt.ledger_seal_sha256.clone()),
        ledger_seal_algorithm: Some(keystone_receipt.ledger_seal_algorithm.clone()),
        registry_entry_summary: Some(keystone_receipt.registry_entry_summary.clone()),
        registry_entry_sha256: Some(keystone_receipt.registry_entry_sha256.clone()),
        registry_entry_algorithm: Some(keystone_receipt.registry_entry_algorithm.clone()),
        filing_certificate_summary: Some(keystone_receipt.filing_certificate_summary.clone()),
        filing_certificate_sha256: Some(keystone_receipt.filing_certificate_sha256.clone()),
        filing_certificate_algorithm: Some(keystone_receipt.filing_certificate_algorithm.clone()),
        archive_receipt_summary: Some(keystone_receipt.archive_receipt_summary.clone()),
        archive_receipt_sha256: Some(keystone_receipt.archive_receipt_sha256.clone()),
        archive_receipt_algorithm: Some(keystone_receipt.archive_receipt_algorithm.clone()),
        closure_certificate_summary: Some(keystone_receipt.closure_certificate_summary.clone()),
        closure_certificate_sha256: Some(keystone_receipt.closure_certificate_sha256.clone()),
        closure_certificate_algorithm: Some(
            keystone_receipt.closure_certificate_algorithm.clone(),
        ),
        finality_receipt_summary: Some(keystone_receipt.finality_receipt_summary.clone()),
        finality_receipt_sha256: Some(keystone_receipt.finality_receipt_sha256.clone()),
        finality_receipt_algorithm: Some(keystone_receipt.finality_receipt_algorithm.clone()),
        consummation_record_summary: Some(keystone_receipt.consummation_record_summary.clone()),
        consummation_record_sha256: Some(keystone_receipt.consummation_record_sha256.clone()),
        consummation_record_algorithm: Some(keystone_receipt.consummation_record_algorithm.clone()),
        completion_certificate_summary: Some(
            keystone_receipt.completion_certificate_summary.clone(),
        ),
        completion_certificate_sha256: Some(
            keystone_receipt.completion_certificate_sha256.clone(),
        ),
        completion_certificate_algorithm: Some(
            keystone_receipt.completion_certificate_algorithm.clone(),
        ),
        summit_certificate_summary: Some(keystone_receipt.summit_certificate_summary.clone()),
        summit_certificate_sha256: Some(keystone_receipt.summit_certificate_sha256.clone()),
        summit_certificate_algorithm: Some(
            keystone_receipt.summit_certificate_algorithm.clone(),
        ),
        culmination_receipt_summary: Some(
            keystone_receipt.culmination_receipt_summary.clone(),
        ),
        culmination_receipt_sha256: Some(
            keystone_receipt.culmination_receipt_sha256.clone(),
        ),
        culmination_receipt_algorithm: Some(
            keystone_receipt.culmination_receipt_algorithm.clone(),
        ),
        pinnacle_receipt_summary: Some(keystone_receipt.pinnacle_receipt_summary.clone()),
        pinnacle_receipt_sha256: Some(keystone_receipt.pinnacle_receipt_sha256.clone()),
        pinnacle_receipt_algorithm: Some(
            keystone_receipt.pinnacle_receipt_algorithm.clone(),
        ),
        capstone_certificate_summary: Some(
            keystone_receipt.capstone_certificate_summary.clone(),
        ),
        capstone_certificate_sha256: Some(
            keystone_receipt.capstone_certificate_sha256.clone(),
        ),
        capstone_certificate_algorithm: Some(
            keystone_receipt.capstone_certificate_algorithm.clone(),
        ),
        keystone_receipt_summary: Some(keystone_receipt.keystone_receipt_summary.clone()),
        keystone_receipt_sha256: Some(keystone_receipt.keystone_receipt_sha256.clone()),
        keystone_receipt_algorithm: Some(keystone_receipt.keystone_receipt_algorithm.clone()),
        cornerstone_certificate_summary: Some(cornerstone_certificate_summary(
            classification.0,
            &keystone_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        cornerstone_certificate_sha256: Some(cornerstone_certificate_sha256),
        cornerstone_certificate_algorithm: Some(CORNERSTONE_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: CORNERSTONE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_cornerstone_certificate_report(
    config: &Config,
) -> Result<PackageCornerstoneCertificateReport> {
    let plan = plan_live_package_cornerstone_certificate_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_cornerstone_certificate --keystone-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-cornerstone-certificate --json\ncopybot_tiny_live_activation_package_cornerstone_certificate --keystone-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-cornerstone-certificate --json\n",
        shell_escape_path(&config.keystone_receipt_session_dir),
    ...
    Ok(PackageCornerstoneCertificateReport {
        generated_at: Utc::now(),
        mode: "render_live_package_cornerstone_certificate".to_string(),
        verdict:
            TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateRendered,
        reason: format!(
            "rendered cornerstone-certificate run/verify script to {}; the primary input remains the immutable keystone-receipt session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: CORNERSTONE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_cornerstone_certificate_report(
    config: &Config,
) -> Result<PackageCornerstoneCertificateReport> {
    ...
    let verified_keystone_receipt = match package_keystone_receipt_cornerstone_certificate::verify_live_package_keystone_receipt_for_cornerstone_certificate(
        &config.keystone_receipt_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify keystone-receipt session under {}: {error}",
                    config.keystone_receipt_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_keystone_receipt.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "cornerstone-certificate session dir {} overlaps the managed live target surface derived from verified keystone-receipt install_root {}",
            session_dir.display(),
            verified_keystone_receipt.contract.install_root.display()
        )
    })?;
    ensure_clean_cornerstone_certificate_session_dir(session_dir)?;
    let paths = cornerstone_certificate_paths(session_dir);
    let classification = classify_verified_keystone_receipt(&verified_keystone_receipt);
    ...
    let cornerstone_certificate_sha256 = compute_cornerstone_certificate_sha256(
        ...
        &verified_keystone_receipt.contract.finality_receipt_sha256,
        &verified_keystone_receipt
            .contract
    ...
    let session = planned_session(
        config,
        session_dir,
        &verified_keystone_receipt.contract,
        result,
    ...
    persist_json(
        ...
        &verified_keystone_receipt.report_json,
    )?;
    let keystone_receipt_step = Some(step_artifact(
        &paths.keystone_receipt_report_path,
        "verify_live_package_keystone_receipt",
        &verified_keystone_receipt.verdict,
        &verified_keystone_receipt.reason,
        verified_keystone_receipt.generated_at,
    ));
    let status = PackageCornerstoneCertificateStatus {
        status_version: CORNERSTONE_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        keystone_receipt_session_dir: config.keystone_receipt_session_dir.display().to_string(),
        result,
        reason: classification.2.clone(),
        handoff_bundle_result: verified_keystone_receipt
            .contract
            .handoff_bundle_result
            .clone(),
        decision_packet_result: verified_keystone_receipt
            .contract
            .decision_packet_result
            .clone(),
        execute_frozen_result: verified_keystone_receipt
            .contract
            .execute_frozen_result
            .clone(),
        current_pre_activation_gate_verdict: verified_keystone_receipt
            .contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_keystone_receipt
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        keystone_receipt_step: keystone_receipt_step.clone(),
        provenance_certificate_summary: verified_keystone_receipt
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_keystone_receipt
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_keystone_receipt
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_keystone_receipt
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_keystone_receipt
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_keystone_receipt
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_keystone_receipt
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_keystone_receipt
            .contract
            .notarization_receipt_summary
            .clone(),
        ledger_seal_summary: verified_keystone_receipt
            .contract
            .ledger_seal_summary
            .clone(),
        ledger_seal_sha256: verified_keystone_receipt
            .contract
            .ledger_seal_sha256
            .clone(),
        ledger_seal_algorithm: verified_keystone_receipt
            .contract
            .ledger_seal_algorithm
            .clone(),
        registry_entry_summary: verified_keystone_receipt
            .contract
            .registry_entry_summary
            .clone(),
        registry_entry_sha256: verified_keystone_receipt
            .contract
            .registry_entry_sha256
            .clone(),
        registry_entry_algorithm: verified_keystone_receipt
            .contract
            .registry_entry_algorithm
            .clone(),
        filing_certificate_summary: verified_keystone_receipt
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_keystone_receipt
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_keystone_receipt
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_keystone_receipt
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_keystone_receipt
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_keystone_receipt
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_keystone_receipt
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_keystone_receipt
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_keystone_receipt
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_keystone_receipt
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_keystone_receipt
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_keystone_receipt
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_keystone_receipt
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_keystone_receipt
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_keystone_receipt
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_keystone_receipt
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_keystone_receipt
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_keystone_receipt
            .contract
            .completion_certificate_algorithm
            .clone(),
        summit_certificate_summary: verified_keystone_receipt
            .contract
            .summit_certificate_summary
            .clone(),
        summit_certificate_sha256: verified_keystone_receipt
            .contract
            .summit_certificate_sha256
            .clone(),
        summit_certificate_algorithm: verified_keystone_receipt
            .contract
            .summit_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_keystone_receipt
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_keystone_receipt
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_keystone_receipt
            .contract
            .culmination_receipt_algorithm
            .clone(),
        pinnacle_receipt_summary: verified_keystone_receipt
            .contract
            .pinnacle_receipt_summary
            .clone(),
        pinnacle_receipt_sha256: verified_keystone_receipt
            .contract
            .pinnacle_receipt_sha256
            .clone(),
        pinnacle_receipt_algorithm: verified_keystone_receipt
            .contract
            .pinnacle_receipt_algorithm
            .clone(),
        capstone_certificate_summary: verified_keystone_receipt
            .contract
            .capstone_certificate_summary
            .clone(),
        capstone_certificate_sha256: verified_keystone_receipt
            .contract
            .capstone_certificate_sha256
            .clone(),
        capstone_certificate_algorithm: verified_keystone_receipt
            .contract
            .capstone_certificate_algorithm
            .clone(),
        keystone_receipt_summary: verified_keystone_receipt
            .contract
            .keystone_receipt_summary
            .clone(),
        keystone_receipt_sha256: verified_keystone_receipt
            .contract
            .keystone_receipt_sha256
            .clone(),
        keystone_receipt_algorithm: verified_keystone_receipt
            .contract
            .keystone_receipt_algorithm
            .clone(),
        cornerstone_certificate_summary: cornerstone_certificate_summary(
            result,
            &verified_keystone_receipt
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        cornerstone_certificate_sha256: cornerstone_certificate_sha256.clone(),
        cornerstone_certificate_algorithm: CORNERSTONE_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: CORNERSTONE_CERTIFICATE_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_cornerstone_certificate_report(
    config: &Config,
) -> Result<PackageCornerstoneCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = cornerstone_certificate_paths(session_dir);
    let session: PackageCornerstoneCertificateSession = load_json(&paths.session_path)?;
    let status: PackageCornerstoneCertificateStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.keystone_receipt_session_dir,
        &config.keystone_receipt_session_dir.display().to_string(),
        "keystone-receipt keystone_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "keystone-receipt session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "keystone-receipt status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.keystone_receipt_session_dir,
        &config.keystone_receipt_session_dir.display().to_string(),
        "keystone-receipt status keystone_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_keystone_receipt_report: serde_json::Value = load_required_step_json(
        &status.keystone_receipt_step,
        &paths.keystone_receipt_report_path,
        "keystone_receipt_step",
        &mut mismatches,
    )?;
    let stored_keystone_receipt_report_view: NestedKeystoneReceiptReportView =
        serde_json::from_value(stored_keystone_receipt_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested keystone-receipt report {}",
                paths.keystone_receipt_report_path.display()
            )
        })?;
    let verified_keystone_receipt_report_view: NestedKeystoneReceiptReportView =
        serde_json::from_value(verified_keystone_receipt.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested keystone-receipt report for {}",
                    config.keystone_receipt_session_dir.display()
                )
            },
        )?;
    compare_json_ignoring_generated_at(
        &stored_keystone_receipt_report,
        &verified_keystone_receipt.report_json,
        "keystone-receipt archived keystone-receipt report json",
        &mut mismatches,
    );
    compare_string(
        &stored_keystone_receipt_report_view.mode,
        &verified_keystone_receipt_report_view.mode,
        "keystone-receipt archived keystone-receipt report mode",
        &mut mismatches,
    );
    compare_string(
        &stored_keystone_receipt_report_view.verdict,
        &verified_keystone_receipt_report_view.verdict,
        "keystone-receipt archived keystone-receipt report verdict",
        &mut mismatches,
    );
    compare_string(
        &stored_keystone_receipt_report_view.reason,
        &verified_keystone_receipt_report_view.reason,
        "keystone-receipt archived keystone-receipt report reason",
        &mut mismatches,
    );
    if stored_keystone_receipt_report_view.generated_at
        != verified_keystone_receipt_report_view.generated_at
    {
        mismatches.push(format!(
            "keystone-receipt archived keystone-receipt report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_keystone_receipt_report_view.generated_at,
            verified_keystone_receipt_report_view.generated_at
        ));
    }
    let expected_keystone_receipt_step = step_artifact(
        &paths.keystone_receipt_report_path,
        &verified_keystone_receipt_report_view.mode,
        &verified_keystone_receipt_report_view.verdict,
        &verified_keystone_receipt_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_keystone_receipt_step,
        "keystone-receipt keystone_receipt_step",
        &mut mismatches,
    );
    if verified_keystone_receipt.verdict != "tiny_live_package_keystone_receipt_verify_ok" {
        mismatches.push(format!(
    ...
    let classification = classify_verified_keystone_receipt(&verified_keystone_receipt);
    let expected_cornerstone_certificate_sha256 = compute_cornerstone_certificate_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_keystone_receipt.contract,
        classification.0,
        &expected_cornerstone_certificate_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "cornerstone-certificate session contract does not match freshly verified keystone-receipt truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_keystone_receipt.contract,
        classification,
        expected_keystone_receipt_step.clone(),
        &expected_cornerstone_certificate_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "cornerstone-certificate status contract does not match freshly verified keystone-receipt truth"
                .to_string(),
        );
    }
...
fn verify_keystone_receipt_command_summary(keystone_receipt_session_dir: &Path) -> String {
    format!(
...
fn classify_keystone_receipt_contract(
    contract: &package_keystone_receipt_cornerstone_certificate::LivePackageKeystoneReceiptContractView,
    assume_verified: bool,
) -> (
    LivePackageCornerstoneCertificateResult,
    TinyLivePackageCornerstoneCertificateVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageCornerstoneCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateRefusedNowByInvalidOrDriftedContract,
            "the keystone-receipt chain is not verified, so this cornerstone certificate remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageCornerstoneCertificateResult::RefusedNowByStage3,
            TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateRefusedNowByStage3,
            "the immutable keystone receipt remains refused now by Stage 3, so this cornerstone certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageCornerstoneCertificateResult::RefusedNowByPreActivationGate,
            TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateRefusedNowByPreActivationGate,
            "the immutable keystone receipt remains refused now by the pre-activation gate, so this cornerstone certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageCornerstoneCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen,
            ...
            "the immutable keystone receipt is coherent and this cornerstone certificate is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageCornerstoneCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateRefusedNowByInvalidOrDriftedContract,
            "the verified keystone-receipt chain is invalid, drifted, or non-runnable, so this cornerstone certificate remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_keystone_receipt(
    verified: &package_keystone_receipt_cornerstone_certificate::VerifiedLivePackageKeystoneReceiptCornerstoneCertificateStep,
) -> (
    LivePackageCornerstoneCertificateResult,
    TinyLivePackageCornerstoneCertificateVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_keystone_receipt_verify_ok" {
        return (
            LivePackageCornerstoneCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateRefusedNowByInvalidOrDriftedContract,
            "the immutable keystone receipt did not verify cleanly, so this cornerstone certificate remains refused"
                .to_string(),
        );
    }
    classify_keystone_receipt_contract(&verified.contract, true)
}

fn cornerstone_certificate_summary(
    result: LivePackageCornerstoneCertificateResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageCornerstoneCertificateResult::RefusedNowByStage3 => {
            "Cornerstone certificate outcome: the immutable culminated chain remains refused now by Stage 3; archive this final regalia seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageCornerstoneCertificateResult::RefusedNowByPreActivationGate => {
            "Cornerstone certificate outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final regalia seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageCornerstoneCertificateResult::RefusedNowByInvalidOrDriftedContract => {
            "Cornerstone certificate outcome: the immutable culminated chain is invalid or drifted; archive this final regalia seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackageCornerstoneCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Cornerstone certificate outcome: this immutable regalia seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn cornerstone_certificate_paths(session_dir: &Path) -> PackageCornerstoneCertificatePaths {
...
fn ensure_clean_cornerstone_certificate_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing cornerstone-certificate session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating cornerstone-certificate session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageCornerstoneCertificateSession {
    PackageCornerstoneCertificateSession {
        session_version: CORNERSTONE_CERTIFICATE_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        keystone_receipt_session_dir: config.keystone_receipt_session_dir.display().to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_keystone_receipt_command_summary: verify_keystone_receipt_command_summary(
            &config.keystone_receipt_session_dir,
        ),
        ...
        cornerstone_certificate_sha256: cornerstone_certificate_sha256.to_string(),
        cornerstone_certificate_algorithm: CORNERSTONE_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: CORNERSTONE_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    keystone_receipt_step: PackageCornerstoneCertificateStepArtifact,
    cornerstone_certificate_sha256: &str,
) -> PackageCornerstoneCertificateStatus {
    PackageCornerstoneCertificateStatus {
        status_version: CORNERSTONE_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        keystone_receipt_step: Some(keystone_receipt_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageCornerstoneCertificatePaths,
    contract: &package_keystone_receipt_cornerstone_certificate::LivePackageKeystoneReceiptContractView,
    verdict: TinyLivePackageCornerstoneCertificateVerdict,
    status: &PackageCornerstoneCertificateStatus,
) -> PackageCornerstoneCertificateReport {
    PackageCornerstoneCertificateReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateVerifyOk
        {
            "verify_live_package_cornerstone_certificate".to_string()
        } else {
            "run_live_package_cornerstone_certificate".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        cornerstone_certificate_session_path: Some(paths.session_path.display().to_string()),
        cornerstone_certificate_status_path: Some(paths.status_path.display().to_string()),
        archived_keystone_receipt_report_path: Some(
        ...
        keystone_receipt_step: status.keystone_receipt_step.clone(),
        verify_keystone_receipt_command_summary: Some(verify_keystone_receipt_command_summary(
            &config.keystone_receipt_session_dir,
        )),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
        ...
        explicit_statement: if verdict
            == TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateVerifyOk
        {
            CORNERSTONE_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageCornerstoneCertificateReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageCornerstoneCertificate => {
                "plan_live_package_cornerstone_certificate".to_string()
            }
            Mode::RenderLivePackageCornerstoneCertificate => {
                "render_live_package_cornerstone_certificate".to_string()
            }
        ...
        verdict,
        reason,
        keystone_receipt_session_dir: config
            .keystone_receipt_session_dir
            .display()
            .to_string(),
        ...
        keystone_receipt_algorithm: Some(CORNERSTONE_CERTIFICATE_ALGORITHM.to_string()),
        cornerstone_certificate_summary: None,
        cornerstone_certificate_sha256: None,
        cornerstone_certificate_algorithm: Some(CORNERSTONE_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable cornerstone certificate refused before persisting because the keystone-receipt artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "cornerstone-certificate-native cornerstone-certificate verification failed".to_string()
    });
    PackageCornerstoneCertificateReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_cornerstone_certificate".to_string(),
        verdict: TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateVerifyInvalid,
        reason,
        keystone_receipt_session_dir: config.keystone_receipt_session_dir.display().to_string(),
        registry_entry_session_dir: Some(contract.registry_entry_session_dir.display().to_string()),
        notarization_receipt_session_dir: Some(
        ...
        explicit_statement: CORNERSTONE_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageCornerstoneCertificateStepArtifact>,
    expected_path: &Path,
    label: &str,
    mismatches: &mut Vec<String>,
) -> Result<serde_json::Value> {
    let Some(step) = step else {
        mismatches.push(format!("{label} is missing from keystone-receipt status"));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageCornerstoneCertificateStepArtifact {
    PackageCornerstoneCertificateStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageCornerstoneCertificateStepArtifact>,
    expected: &PackageCornerstoneCertificateStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_cornerstone_certificate_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&CornerstoneCertificatePayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing cornerstone-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackageCornerstoneCertificateReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "keystone_receipt_session_dir={}",
            report.keystone_receipt_session_dir
        ),
    ];
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_cornerstone_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageCornerstoneCertificateVerdict::TinyLivePackageCornerstoneCertificateReadyForManualExecutionWhenGateTurnsGreen
        );
    ...
    fn drifted_finality_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            ...
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_keystone_receipt",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        .unwrap();
        let overlap_session_dir = managed_paths.runtime_dir.join("keystone-receipt-session");
        ...
        let error = run_live_package_cornerstone_certificate_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = cornerstone_certificate_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_keystone_receipt_step_path() {
        let fixture = fake_command_fixture(
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        cornerstone_certificate_session_dir: PathBuf,
        keystone_receipt_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageCornerstoneCertificate,
                keystone_receipt_session_dir: self.keystone_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.cornerstone_certificate_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageCornerstoneCertificate,
                keystone_receipt_session_dir: self.keystone_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        persist_json(
            ...
            &stored_keystone_receipt_session_fixture(
                &capstone_certificate_session_dir,
                &finality_receipt_session_dir,
                &archive_receipt_session_dir,
                &registry_entry_session_dir,
        ...
        persist_json(
            ...
            &stored_keystone_receipt_status_fixture(&verify_report),
        )
        .unwrap();
        persist_json(
            &keystone_receipt_session_dir.join(
                "tiny_live_activation_package_keystone_receipt.capstone_certificate.report.json",
            ),
            &json!({
        ...
        bind_keystone_receipt_report_to_fixture(
            &mut verify_report,
            &capstone_certificate_session_dir,
            &finality_receipt_session_dir,
            &archive_receipt_session_dir,
        ...
        let verify_path = fixture_dir.join("keystone-receipt.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_keystone_receipt_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_keystone_receipt"),
            &verify_path,
            &capstone_certificate_session_dir,
            &decision_packet_session_dir,
            &keystone_receipt_session_dir,
    ...
    fn bind_keystone_receipt_report_to_fixture(
        ...
        _keystone_receipt_session_dir: &Path,
        package_dir: &Path,
        install_root: &Path,
        ...
        object.insert(
            "capstone_certificate_session_dir".to_string(),
            json!(capstone_certificate_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            "verify_capstone_certificate_command_summary".to_string(),
            json!(format!(
        ...
        object.insert(
            ...
            json!("keystone-receipt verify statement"),
        );
    }

    fn keystone_receipt_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_keystone_receipt",
            "verdict": "tiny_live_package_keystone_receipt_verify_ok",
            "reason": "keystone-receipt chain is coherent",
            "result": result,
    ...
    fn capstone_fixture_verify_report(
        ...
        keystone_receipt_verify_report(
            result,
            handoff_bundle_result,
            decision_packet_result,
            execute_frozen_result,
    ...
    fn stored_keystone_receipt_session_fixture(
        capstone_certificate_session_dir: &Path,
        finality_receipt_session_dir: &Path,
        archive_receipt_session_dir: &Path,
        registry_entry_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("keystone-receipt statement"),
        );
        Value::Object(object)
    }

    fn stored_keystone_receipt_status_fixture(verify_report: &Value) -> Value {
        let mut object = Map::new();
        object.insert(
            "result".to_string(),
            json!(verify_report
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("keystone-receipt status statement"),
        );
        Value::Object(object)
    }
}
