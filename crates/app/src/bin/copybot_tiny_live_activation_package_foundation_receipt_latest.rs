...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_foundation_receipt_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-foundation-receipt | --render-latest-foundation-receipt-script | --run-latest-foundation-receipt | --verify-latest-foundation-receipt)";
...
const FOUNDATION_RECEIPT_BIN: &str = "copybot_tiny_live_activation_package_foundation_receipt";
const CORNERSTONE_CERTIFICATE_LATEST_PLAN_READY: &str =
    "tiny_live_package_cornerstone_certificate_latest_plan_ready";
const CORNERSTONE_CERTIFICATE_LATEST_VERIFY_OK: &str =
    "tiny_live_package_cornerstone_certificate_latest_verify_ok";
const FOUNDATION_RECEIPT_VERIFY_OK: &str = "tiny_live_package_foundation_receipt_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-foundation-receipt handoff binds the latest persisted immutable package chain to the accepted foundation-receipt contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestFoundationReceipt,
    RenderLatestFoundationReceiptScript,
    RunLatestFoundationReceipt,
    VerifyLatestFoundationReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageFoundationReceiptLatestVerdict {
    TinyLivePackageFoundationReceiptLatestPlanReady,
    TinyLivePackageFoundationReceiptLatestScriptRendered,
    TinyLivePackageFoundationReceiptLatestRefusedByMissingLatestChain,
    TinyLivePackageFoundationReceiptLatestRefusedByInvalidLatestChain,
    TinyLivePackageFoundationReceiptLatestRefusedByDriftedFoundationReceiptContract,
    TinyLivePackageFoundationReceiptLatestRefusedNowByStage3,
    TinyLivePackageFoundationReceiptLatestRefusedNowByPreActivationGate,
    TinyLivePackageFoundationReceiptLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageFoundationReceiptLatestVerifyOk,
    TinyLivePackageFoundationReceiptLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageFoundationReceiptLatestResult {
    RefusedByMissingLatestChain,
    RefusedByInvalidLatestChain,
    RefusedByDriftedFoundationReceiptContract,
    RefusedNowByStage3,
...
struct PackageFoundationReceiptLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageFoundationReceiptLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_cornerstone_certificate_latest_session_dir: String,
    nested_foundation_receipt_session_dir: String,
    ...
    resolve_latest_cornerstone_certificate_command_summary: String,
    run_latest_foundation_receipt_command_summary: String,
    verify_latest_foundation_receipt_command_summary: String,
    downstream_foundation_receipt_command_summary: String,
    verify_cornerstone_certificate_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageFoundationReceiptLatestStatus {
    ...
    nested_foundation_receipt_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackageFoundationReceiptLatestResult,
    reason: String,
    latest_cornerstone_certificate_plan_verdict: Option<String>,
    latest_cornerstone_certificate_verdict: Option<String>,
    foundation_receipt_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_cornerstone_certificate_plan_step: Option<PackageFoundationReceiptLatestStepArtifact>,
    latest_cornerstone_certificate_step: Option<PackageFoundationReceiptLatestStepArtifact>,
    foundation_receipt_step: Option<PackageFoundationReceiptLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackageFoundationReceiptLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_cornerstone_certificate_plan_report_path: PathBuf,
    latest_cornerstone_certificate_report_path: PathBuf,
    foundation_receipt_report_path: PathBuf,
    nested_cornerstone_certificate_latest_session_dir: PathBuf,
    nested_foundation_receipt_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageFoundationReceiptLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageFoundationReceiptLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    foundation_receipt_latest_session_path: Option<String>,
    foundation_receipt_latest_status_path: Option<String>,
    foundation_receipt_latest_report_path: Option<String>,
    latest_cornerstone_certificate_plan_report_path: Option<String>,
    latest_cornerstone_certificate_report_path: Option<String>,
    foundation_receipt_report_path: Option<String>,
    nested_cornerstone_certificate_latest_session_dir: Option<String>,
    nested_foundation_receipt_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_cornerstone_certificate_command_summary: Option<String>,
    run_latest_foundation_receipt_command_summary: Option<String>,
    verify_latest_foundation_receipt_command_summary: Option<String>,
    downstream_foundation_receipt_command_summary: Option<String>,
    verify_cornerstone_certificate_command_summary: Option<String>,
    reviewed_frozen_live_cutover_controller_command_summary: Option<String>,
    clerestory_certificate_summary: Option<String>,
    clerestory_certificate_sha256: Option<String>,
    clerestory_certificate_algorithm: Option<String>,
...
#[derive(Debug, Clone)]
struct LatestCornerstoneCertificateSnapshot {
...
#[derive(Debug, Clone)]
struct ResolvedLatestCornerstoneCertificate {
    snapshot: LatestCornerstoneCertificateSnapshot,
    latest_cornerstone_certificate_plan_raw: Value,
    latest_cornerstone_certificate_plan: CornerstoneCertificateLatestReportView,
}
...
enum ResolutionFailureKind {
    MissingLatestChain,
    InvalidLatestChain,
    DriftedCornerstoneCertificateContract,
}

#[derive(Debug, Clone)]
struct LatestCornerstoneCertificateResolutionFailure {
    kind: ResolutionFailureKind,
    reason: String,
    snapshot: Option<LatestCornerstoneCertificateSnapshot>,
    latest_cornerstone_certificate_plan_raw: Option<Value>,
    latest_cornerstone_certificate_plan_verdict: Option<String>,
    latest_cornerstone_certificate_plan_reason: Option<String>,
    latest_cornerstone_certificate_plan_generated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone, Deserialize)]
struct CornerstoneCertificateLatestReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Deserialize)]
struct FoundationReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    cornerstone_certificate_session_dir: String,
    ...
    service_status_max_staleness_ms: Option<u64>,
    session_dir: Option<String>,
    foundation_receipt_session_path: Option<String>,
    foundation_receipt_status_path: Option<String>,
    archived_cornerstone_certificate_report_path: Option<String>,
    result: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    cornerstone_certificate_step: Option<PackageFoundationReceiptLatestStepArtifact>,
    verify_cornerstone_certificate_command_summary: Option<String>,
    reviewed_frozen_live_cutover_controller_command_summary: Option<String>,
...
#[derive(Debug, Clone, Deserialize)]
struct FoundationReceiptSessionView {
    cornerstone_certificate_session_dir: String,
...
#[derive(Debug, Clone, Deserialize)]
struct FoundationReceiptStatusView {
    session_dir: String,
    cornerstone_certificate_session_dir: String,
    result: String,
    reason: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-foundation-receipt" => {
                set_mode(&mut mode, Mode::PlanLatestFoundationReceipt, arg.as_str())?
            }
            "--render-latest-foundation-receipt-script" => set_mode(
                &mut mode,
                Mode::RenderLatestFoundationReceiptScript,
                arg.as_str(),
            )?,
            "--run-latest-foundation-receipt" => {
                set_mode(&mut mode, Mode::RunLatestFoundationReceipt, arg.as_str())?
            }
            "--verify-latest-foundation-receipt" => {
                set_mode(&mut mode, Mode::VerifyLatestFoundationReceipt, arg.as_str())?
            }
    ...
    if matches!(
        mode,
        Mode::PlanLatestFoundationReceipt
            | Mode::RenderLatestFoundationReceiptScript
            | Mode::RunLatestFoundationReceipt
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestFoundationReceiptScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestFoundationReceipt | Mode::VerifyLatestFoundationReceipt
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestFoundationReceipt => plan_latest_foundation_receipt_report(&config)?,
        Mode::RenderLatestFoundationReceiptScript => {
            render_latest_foundation_receipt_script_report(&config)?
        }
        Mode::RunLatestFoundationReceipt => run_latest_foundation_receipt_report(&config)?,
        Mode::VerifyLatestFoundationReceipt => verify_latest_foundation_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_latest_foundation_receipt_report(
    config: &Config,
) -> Result<PackageFoundationReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    let suggested_session_dir = config.session_dir.clone().unwrap_or_else(|| {
        root.join("tiny_live_activation_package_foundation_receipt_latest.session")
    });
    let paths = foundation_receipt_latest_paths(&suggested_session_dir);

    match resolve_latest_cornerstone_certificate(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            "plan_latest_foundation_receipt",
            TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-cornerstone-certificate lineage via accepted latest-cornerstone-certificate session {}, exact decision-packet confirmation anchor {}, and the accepted latest-foundation-receipt contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_cornerstone_certificate_latest_session_dir.display(),
...
fn render_latest_foundation_receipt_script_report(
    config: &Config,
) -> Result<PackageFoundationReceiptLatestReport> {
    let root = config
        .root
        ...
        Ok(resolved) => {
            write_script(
                ...
                &format!(
                    ...
                    run_latest = run_latest_foundation_receipt_command_summary(
                        root,
                        &paths.nested_cornerstone_certificate_latest_session_dir,
                    ),
                    verify_latest = verify_latest_foundation_receipt_command_summary(
                        &paths.nested_cornerstone_certificate_latest_session_dir,
                    ),
                    run_handoff_bundle = downstream_foundation_receipt_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_foundation_receipt_session_dir(&paths),
                        &paths.nested_foundation_receipt_session_dir,
                    ),
                    verify_handoff_bundle = verify_cornerstone_certificate_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_foundation_receipt_session_dir(&paths),
                        &paths.nested_foundation_receipt_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                "render_latest_foundation_receipt_script",
                TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestScriptRendered,
                format!(
                    "rendered latest-foundation-receipt handoff script to {} using the accepted latest-cornerstone-certificate lineage and the accepted foundation-receipt contract",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_foundation_receipt_report(
    config: &Config,
) -> Result<PackageFoundationReceiptLatestReport> {
    let root = config
        .root
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-foundation-receipt session dir {}",
            session_dir.display()
        );
    }
    let paths = foundation_receipt_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-foundation-receipt")?;

    let resolution = resolve_latest_cornerstone_certificate(root, &paths);
    match resolution {
        Ok(resolved) => {
            run_latest_foundation_receipt_with_resolution(root, session_dir, &paths, &resolved)
        }
        Err(failure) => run_latest_foundation_receipt_failure(root, session_dir, &paths, &failure),
    }
}

fn verify_latest_foundation_receipt_report(
    config: &Config,
) -> Result<PackageFoundationReceiptLatestReport> {
    let session_dir = config
        .session_dir
    ...
    let session: PackageFoundationReceiptLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_foundation_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-foundation-receipt session artifact {}: {error}",
    ...
    let status: PackageFoundationReceiptLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_foundation_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-foundation-receipt status artifact {}: {error}",
    ...
    let stored_report: PackageFoundationReceiptLatestReport = match load_json(&paths.report_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_foundation_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-foundation-receipt report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-foundation-receipt root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-foundation-receipt session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-foundation-receipt status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-foundation-receipt report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_cornerstone_certificate_latest_session_dir,
        &paths
            .nested_cornerstone_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-foundation-receipt session nested_cornerstone_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_cornerstone_certificate_latest_session_dir,
        &paths
            .nested_cornerstone_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-foundation-receipt status nested_cornerstone_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-foundation-receipt report nested_cornerstone_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_foundation_receipt_session_dir,
        &paths
            .nested_foundation_receipt_session_dir
            .display()
            .to_string(),
        "latest-foundation-receipt session nested_foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_foundation_receipt_session_dir,
        &paths
            .nested_foundation_receipt_session_dir
            .display()
            .to_string(),
        "latest-foundation-receipt status nested_foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-foundation-receipt report nested_foundation_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-foundation-receipt session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-foundation-receipt status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-foundation-receipt report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .foundation_receipt_latest_session_path
            .as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-foundation-receipt report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .foundation_receipt_latest_status_path
            .as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-foundation-receipt report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .foundation_receipt_latest_report_path
            .as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-foundation-receipt report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-foundation-receipt session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-foundation-receipt status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-foundation-receipt report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_foundation_receipt",
        "latest-foundation-receipt stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-foundation-receipt stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-foundation-receipt stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-foundation-receipt stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-foundation-receipt stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report
            .latest_cornerstone_certificate_plan_verdict
            .as_deref(),
        status
            .latest_cornerstone_certificate_plan_verdict
            .as_deref(),
        "stored report latest_cornerstone_certificate_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .latest_cornerstone_certificate_verdict
            .as_deref(),
        status.latest_cornerstone_certificate_verdict.as_deref(),
        "stored report latest_cornerstone_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.foundation_receipt_verdict.as_deref(),
        status.foundation_receipt_verdict.as_deref(),
        "stored report foundation_receipt_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: CornerstoneCertificateLatestReportView =
                load_required_step_json(
                    &status.latest_cornerstone_certificate_plan_step,
                    &paths.latest_cornerstone_certificate_plan_report_path,
                    "latest_cornerstone_certificate_plan_step",
                    &mut mismatches,
                )?;
            compare_step_artifact_against_report_metadata(
                status.latest_cornerstone_certificate_plan_step.as_ref(),
                &paths.latest_cornerstone_certificate_plan_report_path,
                "plan_latest_foundation_receipt",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_cornerstone_certificate_plan_step",
                &mut mismatches,
            );
            compare_cornerstone_certificate_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_cornerstone_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_cornerstone_certificate_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let stored_latest_run: CornerstoneCertificateLatestReportView =
                load_required_step_json(
                    &status.latest_cornerstone_certificate_step,
                    &paths.latest_cornerstone_certificate_report_path,
                    "latest_cornerstone_certificate_step",
                    &mut mismatches,
                )?;
            compare_step_artifact_against_report_metadata(
                status.latest_cornerstone_certificate_step.as_ref(),
                &paths.latest_cornerstone_certificate_report_path,
                "run_latest_foundation_receipt",
                &stored_latest_run.verdict,
                &stored_latest_run.reason,
                stored_latest_run.generated_at,
                "stored latest_cornerstone_certificate_step",
                &mut mismatches,
            );
            compare_cornerstone_certificate_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_cornerstone_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_cornerstone_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let actual_latest_run: CornerstoneCertificateLatestReportView = match load_json(
                &nested_cornerstone_certificate_latest_report_path(
                    &paths.nested_cornerstone_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed loading actual nested latest-cornerstone-certificate report under {}: {error}",
                        paths.nested_cornerstone_certificate_latest_session_dir.display()
                    ));
                    fallback_cornerstone_certificate_latest_copy()
                }
            };
            compare_cornerstone_certificate_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                ...
                &latest_cornerstone_certificate_verify_args(
                    &paths.nested_cornerstone_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-cornerstone-certificate session under {}: {error}",
                        paths.nested_cornerstone_certificate_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: CornerstoneCertificateLatestReportView = serde_json::from_value(
                    latest_verify_raw,
                )
                .context("failed parsing nested latest-cornerstone-certificate verify json")?;
                if latest_verify.verdict != CORNERSTONE_CERTIFICATE_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-cornerstone-certificate verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: FoundationReceiptReportView = load_required_step_json(
            ...
            compare_step_artifact_against_report_metadata(
                ...
                &paths.foundation_receipt_report_path,
                "run_live_package_foundation_receipt",
                &stored_handoff_bundle.verdict,
                &stored_handoff_bundle.reason,
                stored_handoff_bundle.generated_at,
                "stored foundation_receipt_step",
                &mut mismatches,
            );
            compare_keystone_receipt_run_against_snapshot(
            ...
            compare_option_string(
                ...
                "stored status foundation_receipt_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.foundation_receipt_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: FoundationReceiptSessionView =
                match load_json(&nested_foundation_receipt_session_path(
                    &paths.nested_foundation_receipt_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                            "failed reading nested foundation-receipt session under {}: {error}",
                            paths.nested_foundation_receipt_session_dir.display()
                        ));
            ...
            let nested_handoff_bundle_status: FoundationReceiptStatusView =
                match load_json(&nested_foundation_receipt_status_path(
                    &paths.nested_foundation_receipt_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                            "failed reading nested foundation-receipt status under {}: {error}",
                            paths.nested_foundation_receipt_session_dir.display()
                        ));
            ...
            compare_keystone_receipt_copy_against_nested_truth(
                &stored_handoff_bundle,
            ...
            let handoff_bundle_verify_raw = match run_json_command(
                ...
                &cornerstone_certificate_verify_args(
                    &resolved.snapshot,
                    &expected_downstream_foundation_receipt_session_dir(&paths),
                    &paths.nested_foundation_receipt_session_dir,
                ),
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested foundation-receipt session under {}: {error}",
                        paths.nested_foundation_receipt_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: FoundationReceiptReportView =
                    serde_json::from_value(handoff_bundle_verify_raw)
                        .context("failed parsing nested foundation-receipt verify json")?;
                compare_option_string(
                    stored_handoff_bundle.registry_entry_session_dir.as_deref(),
                    handoff_bundle_verify.registry_entry_session_dir.as_deref(),
                    "stored foundation-receipt copy registry_entry_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored foundation-receipt copy notarization_receipt_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .consummation_record_algorithm
                        .as_deref(),
                    "stored foundation-receipt copy consummation_record_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored foundation-receipt copy completion_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.completion_certificate_sha256.as_deref(),
                    handoff_bundle_verify.completion_certificate_sha256.as_deref(),
                    "stored foundation-receipt copy completion_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                ...
                compare_option_string(
                    ...
                    "stored foundation-receipt copy capstone_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_summary
                        .as_deref(),
                    "stored foundation-receipt copy cornerstone_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_sha256
                        .as_deref(),
                    "stored foundation-receipt copy cornerstone_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_algorithm
                        .as_deref(),
                    "stored foundation-receipt copy cornerstone_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.foundation_receipt_summary.as_deref(),
                    handoff_bundle_verify.foundation_receipt_summary.as_deref(),
                    "stored foundation-receipt copy foundation_receipt_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.foundation_receipt_sha256.as_deref(),
                    handoff_bundle_verify.foundation_receipt_sha256.as_deref(),
                    "stored foundation-receipt copy foundation_receipt_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.foundation_receipt_algorithm.as_deref(),
                    handoff_bundle_verify.foundation_receipt_algorithm.as_deref(),
                    "stored foundation-receipt copy foundation_receipt_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .reviewed_frozen_live_cutover_controller_command_summary
                        .as_deref(),
                    "stored foundation-receipt copy reviewed_frozen_live_cutover_controller_command_summary",
                    &mut mismatches,
                );
                if handoff_bundle_verify.verdict != FOUNDATION_RECEIPT_VERIFY_OK {
                    mismatches.push(format!(
                        "nested foundation-receipt verification is non-green: {}",
                        handoff_bundle_verify.reason
                    ));
                }
            }

            let expected_result = cornerstone_certificate_latest_result_from_nested_status(
                nested_handoff_bundle_status.result.as_str(),
            )
            .unwrap_or(
                LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract,
            );
            if status.result != expected_result {
                mismatches.push(format!(
                    "stored latest-foundation-receipt result {} does not match nested foundation-receipt result {}",
            ...
            if status.activation_authorized {
                mismatches.push(
                    "stored status activation_authorized must remain false for latest-foundation-receipt"
                        .to_string(),
                );
            }
            compare_option_string(
        ...
        Err(failure) => {
            ...
            if failure.latest_cornerstone_certificate_plan_raw.is_some() {
                let stored_latest_plan: CornerstoneCertificateLatestReportView =
                    load_required_step_json(
                        &status.latest_cornerstone_certificate_plan_step,
                        &paths.latest_cornerstone_certificate_plan_report_path,
                ...
                compare_option_string(
                    ...
                    failure
                        .latest_cornerstone_certificate_plan_verdict
                        .as_deref(),
                    "stored status latest_cornerstone_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report
                    ...
                    failure
                        .latest_cornerstone_certificate_plan_verdict
                        .as_deref(),
                    "stored report latest_cornerstone_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
                status.latest_cornerstone_certificate_verdict.as_deref(),
                None,
                "stored status latest_cornerstone_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
                    ...
                    .as_deref(),
                None,
                "stored report latest_cornerstone_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.foundation_receipt_verdict.as_deref(),
                None,
                "stored status foundation_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.foundation_receipt_verdict.as_deref(),
                None,
                "stored report foundation_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestVerifyOk
    } else {
        TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-foundation-receipt session under {} is coherent with the current latest immutable chain and the accepted foundation-receipt contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-foundation-receipt verification failed".to_string())
    };

    Ok(PackageFoundationReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_foundation_receipt".to_string(),
        verdict,
        reason,
        ...
        foundation_receipt_latest_session_path: Some(paths.session_path.display().to_string()),
        foundation_receipt_latest_status_path: Some(paths.status_path.display().to_string()),
        foundation_receipt_latest_report_path: Some(paths.report_path.display().to_string()),
        latest_cornerstone_certificate_plan_report_path: Some(
            paths
                .latest_cornerstone_certificate_plan_report_path
                .display()
                .to_string(),
        ),
        latest_cornerstone_certificate_report_path: Some(
            paths.latest_cornerstone_certificate_report_path.display().to_string(),
        ),
        foundation_receipt_report_path: Some(paths.foundation_receipt_report_path.display().to_string()),
        nested_cornerstone_certificate_latest_session_dir: Some(
            paths
                .nested_cornerstone_certificate_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_foundation_receipt_session_dir: Some(
            paths.nested_foundation_receipt_session_dir.display().to_string(),
        ),
        ...
        latest_cornerstone_certificate_plan_verdict: status.latest_cornerstone_certificate_plan_verdict.clone(),
        latest_cornerstone_certificate_verdict: status.latest_cornerstone_certificate_verdict.clone(),
        foundation_receipt_verdict: status.foundation_receipt_verdict.clone(),
        ...
        resolve_latest_cornerstone_certificate_command_summary: Some(
            session.resolve_latest_cornerstone_certificate_command_summary.clone(),
        ),
        run_latest_foundation_receipt_command_summary: Some(
            session.run_latest_foundation_receipt_command_summary.clone(),
        ),
        verify_latest_foundation_receipt_command_summary: Some(
            session.verify_latest_foundation_receipt_command_summary.clone(),
        ),
        downstream_foundation_receipt_command_summary: Some(
            session.downstream_foundation_receipt_command_summary.clone(),
        ),
        verify_cornerstone_certificate_command_summary: Some(
            session.verify_cornerstone_certificate_command_summary.clone(),
        ),
        ...
        explicit_statement:
            "this verification proves only whether the latest immutable chain still resolves cleanly into the accepted foundation-receipt contract; it never runs the frozen controller"
                .to_string(),
    })
}

fn run_latest_foundation_receipt_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackageFoundationReceiptLatestPaths,
    resolved: &ResolvedLatestCornerstoneCertificate,
) -> Result<PackageFoundationReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, Some(resolved), None);
    persist_json(&paths.session_path, &session)?;
    persist_json(
        &paths.latest_cornerstone_certificate_plan_report_path,
        &resolved.latest_cornerstone_certificate_plan_raw,
    )?;

    let latest_cornerstone_certificate_plan_step = Some(step_artifact(
        &paths.latest_cornerstone_certificate_plan_report_path,
        "plan_latest_foundation_receipt",
        &resolved.latest_cornerstone_certificate_plan.verdict,
        &resolved.latest_cornerstone_certificate_plan.reason,
        resolved.latest_cornerstone_certificate_plan.generated_at,
    ));

    let latest_cornerstone_certificate_run_raw = match run_json_command(
        CORNERSTONE_CERTIFICATE_LATEST_BIN,
        &latest_cornerstone_certificate_run_args(
            root,
            &paths.nested_cornerstone_certificate_latest_session_dir,
        ),
    ) {
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract,
                format!(
                    "failed running nested latest-cornerstone-certificate handoff under {}: {error}",
                    paths
                        .nested_cornerstone_certificate_latest_session_dir
                        .display()
                ),
    ...
    persist_json(
        &paths.latest_cornerstone_certificate_report_path,
        &latest_cornerstone_certificate_run_raw,
    )?;
    let latest_cornerstone_certificate_run: CornerstoneCertificateLatestReportView =
        serde_json::from_value(latest_cornerstone_certificate_run_raw)
            .context("failed parsing nested latest-cornerstone-certificate run json")?;
    let latest_cornerstone_certificate_step = Some(step_artifact(
        &paths.latest_cornerstone_certificate_report_path,
        "run_latest_foundation_receipt",
        &latest_cornerstone_certificate_run.verdict,
        &latest_cornerstone_certificate_run.reason,
        latest_cornerstone_certificate_run.generated_at,
    ));

    if let Some(terminal_failure) =
        latest_cornerstone_certificate_terminal_failure(&latest_cornerstone_certificate_run)
    {
        let status = refusal_status(
            root,
            session_dir,
            ...
            Some(resolved),
            terminal_failure,
            latest_cornerstone_certificate_run.reason.clone(),
            latest_cornerstone_certificate_plan_step,
            latest_cornerstone_certificate_step,
            None,
        );
    ...
    compare_cornerstone_certificate_latest_run_against_snapshot(
        &latest_cornerstone_certificate_run,
        &resolved.snapshot,
        &paths.nested_cornerstone_certificate_latest_session_dir,
        &mut run_mismatches,
    );
    ...
    let latest_cornerstone_certificate_verify: CornerstoneCertificateLatestReportView =
        match run_json_command(
            ...
            Err(error) => {
                ...
                refusal_status(
                    ...
                    LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract,
                    format!(
                    "failed verifying nested latest-cornerstone-certificate handoff under {}: {error}",
                    paths.nested_cornerstone_certificate_latest_session_dir.display()
                ),
                    latest_cornerstone_certificate_plan_step,
                    latest_cornerstone_certificate_step,
    ...
    if latest_cornerstone_certificate_verify.verdict != CORNERSTONE_CERTIFICATE_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract,
            latest_cornerstone_certificate_verify.reason,
            latest_cornerstone_certificate_plan_step,
            latest_cornerstone_certificate_step,
            None,
        );
    ...
    let release_capsule_run_raw = match run_json_command(
        FOUNDATION_RECEIPT_BIN,
        &cornerstone_certificate_run_args(
            &resolved.snapshot,
            &expected_downstream_foundation_receipt_session_dir(paths),
            &paths.nested_foundation_receipt_session_dir,
        ),
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract,
                format!(
                    "failed running downstream foundation-receipt contract under {}: {error}",
                    paths.nested_foundation_receipt_session_dir.display()
                ),
                latest_cornerstone_certificate_plan_step,
                latest_cornerstone_certificate_step,
    ...
    let provenance_certificate_run: FoundationReceiptReportView =
        serde_json::from_value(release_capsule_run_raw)
            .context("failed parsing nested foundation-receipt run json")?;
    ...
    let provenance_certificate_verify: FoundationReceiptReportView = match run_json_command(
        FOUNDATION_RECEIPT_BIN,
        &cornerstone_certificate_verify_args(
            &resolved.snapshot,
            &expected_downstream_foundation_receipt_session_dir(paths),
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract,
                format!(
                    "failed verifying downstream foundation-receipt contract under {}: {error}",
                    paths.nested_foundation_receipt_session_dir.display()
                ),
                latest_cornerstone_certificate_plan_step,
                latest_cornerstone_certificate_step,
    ...
    let result = cornerstone_certificate_latest_result_from_nested_status(
        provenance_certificate_run
            .result
            .as_deref()
            .unwrap_or_default(),
    )
    .unwrap_or(LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract);
    let status = PackageFoundationReceiptLatestStatus {
        ...
        nested_cornerstone_certificate_latest_session_dir: paths
            .nested_cornerstone_certificate_latest_session_dir
            .display()
            .to_string(),
        nested_foundation_receipt_session_dir: paths
            .nested_foundation_receipt_session_dir
            .display()
            .to_string(),
        ...
        latest_cornerstone_certificate_plan_verdict: Some(
            resolved.latest_cornerstone_certificate_plan.verdict.clone(),
        ),
        latest_cornerstone_certificate_verdict: Some(
            latest_cornerstone_certificate_run.verdict.clone(),
        ),
        foundation_receipt_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_foundation_receipt_failure(
    root: &Path,
    session_dir: &Path,
    paths: &PackageFoundationReceiptLatestPaths,
    failure: &LatestCornerstoneCertificateResolutionFailure,
) -> Result<PackageFoundationReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, None, Some(failure));
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_cornerstone_certificate_plan_raw {
        persist_json(
            &paths.latest_cornerstone_certificate_plan_report_path,
            value,
        )?;
...
fn resolve_latest_cornerstone_certificate(
    root: &Path,
    paths: &PackageFoundationReceiptLatestPaths,
) -> std::result::Result<
    ResolvedLatestCornerstoneCertificate,
    LatestCornerstoneCertificateResolutionFailure,
> {
    let latest_cornerstone_certificate_plan_raw = match run_json_command(
        CORNERSTONE_CERTIFICATE_LATEST_BIN,
        &latest_cornerstone_certificate_plan_args(
            root,
            &paths.nested_cornerstone_certificate_latest_session_dir,
        ),
    ) {
        Ok(value) => value,
        Err(error) => {
            return Err(LatestCornerstoneCertificateResolutionFailure {
                ...
                snapshot: None,
                latest_cornerstone_certificate_plan_raw: None,
                latest_cornerstone_certificate_plan_verdict: None,
                latest_cornerstone_certificate_plan_reason: None,
                latest_cornerstone_certificate_plan_generated_at: None,
            });
        }
    };
    let latest_cornerstone_certificate_plan: CornerstoneCertificateLatestReportView =
        serde_json::from_value(latest_cornerstone_certificate_plan_raw.clone()).map_err(
            |error| LatestCornerstoneCertificateResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-foundation-receipt upstream latest-cornerstone-certificate plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_cornerstone_certificate_plan_raw: Some(
                    latest_cornerstone_certificate_plan_raw.clone(),
                ),
                latest_cornerstone_certificate_plan_verdict: None,
                latest_cornerstone_certificate_plan_reason: None,
                latest_cornerstone_certificate_plan_generated_at: None,
    ...
    let snapshot =
        parse_snapshot_from_latest_cornerstone_certificate(&latest_cornerstone_certificate_plan)
            .ok();
    if latest_cornerstone_certificate_plan.verdict != CORNERSTONE_CERTIFICATE_LATEST_PLAN_READY {
        return Err(LatestCornerstoneCertificateResolutionFailure {
            kind: map_cornerstone_certificate_latest_failure_kind(
                &latest_cornerstone_certificate_plan.verdict,
            ),
            reason: format!(
                "latest-foundation-receipt resolution refused because nested latest-cornerstone-certificate reported: {}",
                latest_cornerstone_certificate_plan.reason
            ),
            snapshot,
            latest_cornerstone_certificate_plan_raw: Some(latest_cornerstone_certificate_plan_raw),
            latest_cornerstone_certificate_plan_verdict: Some(
                latest_cornerstone_certificate_plan.verdict,
            ),
            latest_cornerstone_certificate_plan_reason: Some(
                latest_cornerstone_certificate_plan.reason,
            ),
            latest_cornerstone_certificate_plan_generated_at: Some(
                latest_cornerstone_certificate_plan.generated_at,
            ),
        });
    }
    ...
    let snapshot =
        parse_snapshot_from_latest_cornerstone_certificate(&latest_cornerstone_certificate_plan)
            .map_err(|error| LatestCornerstoneCertificateResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!("latest-foundation-receipt resolution failed: {error}"),
                snapshot: None,
                latest_cornerstone_certificate_plan_raw: Some(
                    latest_cornerstone_certificate_plan_raw.clone(),
                ),
                latest_cornerstone_certificate_plan_verdict: Some(
                    latest_cornerstone_certificate_plan.verdict.clone(),
                ),
                latest_cornerstone_certificate_plan_reason: Some(
                    latest_cornerstone_certificate_plan.reason.clone(),
                ),
                latest_cornerstone_certificate_plan_generated_at: Some(
                    latest_cornerstone_certificate_plan.generated_at,
                ),
    ...
    let mut mismatches = Vec::new();
    compare_cornerstone_certificate_latest_plan_against_snapshot(
        &latest_cornerstone_certificate_plan,
        &snapshot,
        &paths.nested_cornerstone_certificate_latest_session_dir,
        &mut mismatches,
    );
    if !mismatches.is_empty() {
        return Err(LatestCornerstoneCertificateResolutionFailure {
            kind: ResolutionFailureKind::DriftedCornerstoneCertificateContract,
            reason: format!(
                "latest-foundation-receipt drift detected: {}",
                mismatches[0]
            ),
    ...
    Ok(ResolvedLatestCornerstoneCertificate {
        snapshot,
        latest_cornerstone_certificate_plan_raw,
        latest_cornerstone_certificate_plan,
    })
}

fn parse_snapshot_from_latest_cornerstone_certificate(
    view: &CornerstoneCertificateLatestReportView,
) -> Result<LatestCornerstoneCertificateSnapshot> {
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-foundation-receipt nested latest-cornerstone-certificate plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-foundation-receipt nested latest-cornerstone-certificate plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestCornerstoneCertificateSnapshot {
        latest_top_step_name,
        ...
        wrapper_timeout_ms: view.wrapper_timeout_ms.ok_or_else(|| {
            anyhow!(
                "latest-foundation-receipt nested latest-cornerstone-certificate plan is missing wrapper_timeout_ms"
            )
        })?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!(
                "latest-foundation-receipt nested latest-cornerstone-certificate plan is missing service_status_max_staleness_ms"
            )
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackageFoundationReceiptLatestPaths,
    resolved: &ResolvedLatestCornerstoneCertificate,
) -> PackageFoundationReceiptLatestReport {
    PackageFoundationReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        latest_cornerstone_certificate_report_path: Some(
            paths
                .latest_cornerstone_certificate_report_path
                .display()
                .to_string(),
        ),
        foundation_receipt_report_path: Some(
        ...
        latest_cornerstone_certificate_verdict: None,
        foundation_receipt_verdict: None,
        result: None,
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_cornerstone_certificate_command_summary: Some(
            resolve_latest_cornerstone_certificate_command_summary(
                root,
                &paths.nested_cornerstone_certificate_latest_session_dir,
            ),
        ),
        run_latest_foundation_receipt_command_summary: Some(
            run_latest_foundation_receipt_command_summary(
                root,
                &paths.nested_cornerstone_certificate_latest_session_dir,
            ),
        ),
        verify_latest_foundation_receipt_command_summary: Some(
            verify_latest_foundation_receipt_command_summary(
                &paths.nested_cornerstone_certificate_latest_session_dir,
            ),
        ),
        downstream_foundation_receipt_command_summary: Some(
            downstream_foundation_receipt_command_summary(
                &resolved.snapshot,
                &expected_downstream_foundation_receipt_session_dir(paths),
                &paths.nested_foundation_receipt_session_dir,
            ),
        ...
        verify_cornerstone_certificate_command_summary: Some(
            verify_cornerstone_certificate_command_summary(
                &resolved.snapshot,
                &expected_downstream_foundation_receipt_session_dir(paths),
                &paths.nested_foundation_receipt_session_dir,
            ),
...
fn report_from_resolution_failure(
    ...
    PackageFoundationReceiptLatestReport {
        ...
        latest_cornerstone_certificate_plan_verdict: failure
            .latest_cornerstone_certificate_plan_verdict
            .clone(),
        latest_cornerstone_certificate_verdict: None,
        foundation_receipt_verdict: None,
        ...
        downstream_foundation_receipt_command_summary: snapshot.map(|value| {
            downstream_foundation_receipt_command_summary(
                value,
                &expected_downstream_foundation_receipt_session_dir(paths),
                &paths.nested_foundation_receipt_session_dir,
        ...
        verify_cornerstone_certificate_command_summary: snapshot.map(|value| {
            verify_cornerstone_certificate_command_summary(
                value,
                &expected_downstream_foundation_receipt_session_dir(paths),
                &paths.nested_foundation_receipt_session_dir,
...
fn planned_session(
    ...
    session_dir: &Path,
    paths: &PackageFoundationReceiptLatestPaths,
    resolved: Option<&ResolvedLatestCornerstoneCertificate>,
    failure: Option<&LatestCornerstoneCertificateResolutionFailure>,
) -> PackageFoundationReceiptLatestSession {
    ...
    PackageFoundationReceiptLatestSession {
        ...
        downstream_foundation_receipt_command_summary: snapshot
            .map(|value| {
                downstream_foundation_receipt_command_summary(
                    value,
                    &expected_downstream_foundation_receipt_session_dir(paths),
            ...
            .unwrap_or_else(|| "<foundation-receipt-run-unavailable>".to_string()),
        verify_cornerstone_certificate_command_summary: snapshot
            .map(|value| {
                verify_cornerstone_certificate_command_summary(
                    value,
                    &expected_downstream_foundation_receipt_session_dir(paths),
            ...
            .unwrap_or_else(|| "<foundation-receipt-verify-unavailable>".to_string()),
        reviewed_frozen_live_cutover_controller_command_summary: snapshot.map(|value| {
            value
                .reviewed_frozen_live_cutover_controller_command_summary
                .clone()
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackageFoundationReceiptLatestPaths,
    session: &PackageFoundationReceiptLatestSession,
    status: &PackageFoundationReceiptLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackageFoundationReceiptLatestReport {
    PackageFoundationReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        nested_foundation_receipt_session_dir: Some(
            session.nested_foundation_receipt_session_dir.clone(),
        ),
        downstream_decision_packet_session_dir: Some(
            session.downstream_decision_packet_session_dir.clone(),
        ),
        latest_cornerstone_certificate_plan_verdict: status
            .latest_cornerstone_certificate_plan_verdict
            .clone(),
        latest_cornerstone_certificate_verdict: status
            .latest_cornerstone_certificate_verdict
            .clone(),
        foundation_receipt_verdict: status.foundation_receipt_verdict.clone(),
        result: Some(serialize_enum(&status.result)),
...
fn foundation_receipt_latest_paths(session_dir: &Path) -> PackageFoundationReceiptLatestPaths {
    PackageFoundationReceiptLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_foundation_receipt_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_foundation_receipt_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_foundation_receipt_latest.report.json"),
        latest_cornerstone_certificate_plan_report_path: session_dir.join(
            "tiny_live_activation_package_foundation_receipt_latest.cornerstone_certificate_latest_plan.report.json",
        ),
        latest_cornerstone_certificate_report_path: session_dir.join(
            "tiny_live_activation_package_foundation_receipt_latest.cornerstone_certificate_latest.report.json",
        ),
        foundation_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_foundation_receipt_latest.foundation_receipt.report.json",
        ),
        nested_cornerstone_certificate_latest_session_dir: session_dir
            .join("tiny_live_activation_package_foundation_receipt_latest.cornerstone_certificate_latest_session"),
        nested_foundation_receipt_session_dir: session_dir.join(
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_cornerstone_certificate_latest_session_dir
        .join("tiny_live_activation_package_cornerstone_certificate_latest.decision_packet_session")
}
...
fn expected_downstream_foundation_receipt_session_dir(
    ...
    paths
        .nested_cornerstone_certificate_latest_session_dir
        .join(
            "tiny_live_activation_package_cornerstone_certificate_latest.cornerstone_certificate_session",
        )
}
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_cornerstone_certificate_latest_session_dir
        .join("tiny_live_activation_package_cornerstone_certificate_latest.review_receipt_session")
}

fn latest_cornerstone_certificate_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-cornerstone-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn latest_cornerstone_certificate_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-cornerstone-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn latest_cornerstone_certificate_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-cornerstone-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn cornerstone_certificate_run_args(
    snapshot: &LatestCornerstoneCertificateSnapshot,
    cornerstone_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        "--cornerstone-certificate-session-dir".to_string(),
        cornerstone_certificate_session_dir.display().to_string(),
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-foundation-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn cornerstone_certificate_verify_args(
    snapshot: &LatestCornerstoneCertificateSnapshot,
    cornerstone_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-live-package-foundation-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn resolve_latest_cornerstone_certificate_command_summary(
    root: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{CORNERSTONE_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --plan-latest-cornerstone-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn run_latest_foundation_receipt_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{CORNERSTONE_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --run-latest-cornerstone-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn verify_latest_foundation_receipt_command_summary(session_dir: &Path) -> String {
    format!(
        "{CORNERSTONE_CERTIFICATE_LATEST_BIN} --session-dir {} --verify-latest-cornerstone-certificate --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_foundation_receipt_command_summary(
    snapshot: &LatestCornerstoneCertificateSnapshot,
    cornerstone_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{FOUNDATION_RECEIPT_BIN} --cornerstone-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-foundation-receipt --json",
        shell_escape_path(cornerstone_certificate_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
}

fn verify_cornerstone_certificate_command_summary(
    snapshot: &LatestCornerstoneCertificateSnapshot,
    cornerstone_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{FOUNDATION_RECEIPT_BIN} --cornerstone-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-foundation-receipt --json",
        shell_escape_path(cornerstone_certificate_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
...
fn nested_cornerstone_certificate_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_cornerstone_certificate_latest.report.json")
}

fn nested_foundation_receipt_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_foundation_receipt.session.json")
}

fn nested_foundation_receipt_status_path(session_dir: &Path) -> PathBuf {
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-foundation-receipt script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageFoundationReceiptLatestStepArtifact {
    PackageFoundationReceiptLatestStepArtifact {
...
fn canonical_wrapper_mode(mode: &str) -> &str {
    match mode {
        "plan_latest_foundation_receipt" => "plan_latest_foundation_receipt",
        "render_latest_foundation_receipt_script" => "render_latest_foundation_receipt_script",
        "run_latest_foundation_receipt" => "run_latest_foundation_receipt",
        "verify_latest_foundation_receipt" => "verify_latest_foundation_receipt",
        other => other,
    }
}

fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackageFoundationReceiptLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-foundation-receipt status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-foundation-receipt status"
        ));
        return;
    };
    compare_string(
...
fn compare_cornerstone_certificate_latest_plan_against_snapshot(
    stored: &CornerstoneCertificateLatestReportView,
    snapshot: &LatestCornerstoneCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        ...
        "stored latest-cornerstone-certificate plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_foundation_receipt",
        "stored latest-cornerstone-certificate plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-cornerstone-certificate plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-cornerstone-certificate plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-cornerstone-certificate plan session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-cornerstone-certificate plan downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-cornerstone-certificate plan package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-cornerstone-certificate plan install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-cornerstone-certificate plan target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-cornerstone-certificate plan backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-cornerstone-certificate plan wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-cornerstone-certificate plan service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-cornerstone-certificate plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-cornerstone-certificate plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-cornerstone-certificate plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-cornerstone-certificate plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_cornerstone_certificate_latest_run_against_snapshot(
    stored: &CornerstoneCertificateLatestReportView,
    snapshot: &LatestCornerstoneCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_foundation_receipt",
        "stored latest-cornerstone-certificate run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-cornerstone-certificate run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-cornerstone-certificate run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-cornerstone-certificate run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-cornerstone-certificate run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-cornerstone-certificate run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-cornerstone-certificate run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-cornerstone-certificate run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-cornerstone-certificate run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-cornerstone-certificate run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-cornerstone-certificate run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-cornerstone-certificate run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-cornerstone-certificate run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-cornerstone-certificate run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-cornerstone-certificate run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_cornerstone_certificate_latest_copy_matches_actual(
    stored: &CornerstoneCertificateLatestReportView,
    actual: &CornerstoneCertificateLatestReportView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        Some(stored.verdict.as_str()),
        Some(actual.verdict.as_str()),
        "stored latest-cornerstone-certificate copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-cornerstone-certificate copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-cornerstone-certificate copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-cornerstone-certificate copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-cornerstone-certificate copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-cornerstone-certificate copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-cornerstone-certificate copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-cornerstone-certificate copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-cornerstone-certificate copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-cornerstone-certificate copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-cornerstone-certificate copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-cornerstone-certificate copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_keystone_receipt_run_against_snapshot(
    stored: &FoundationReceiptReportView,
    snapshot: &LatestCornerstoneCertificateSnapshot,
    cornerstone_certificate_session_dir: &Path,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_live_package_foundation_receipt",
        "stored foundation-receipt run mode",
        mismatches,
    );
    compare_string(
        &stored.cornerstone_certificate_session_dir,
        &cornerstone_certificate_session_dir.display().to_string(),
        "stored foundation-receipt run cornerstone_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored foundation-receipt run decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored foundation-receipt run execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored foundation-receipt run turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored foundation-receipt run launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored foundation-receipt run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored foundation-receipt run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored foundation-receipt run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored foundation-receipt run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored foundation-receipt run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored foundation-receipt run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored foundation-receipt run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.foundation_receipt_session_path.as_deref(),
        Some(
            nested_foundation_receipt_session_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored foundation-receipt run foundation_receipt_session_path",
        mismatches,
    );
    compare_option_string(
        stored.foundation_receipt_status_path.as_deref(),
        Some(
            nested_foundation_receipt_status_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored foundation-receipt run foundation_receipt_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_cornerstone_certificate_report_path.as_deref(),
        Some(
            session_dir
        ...
        "stored foundation-receipt run archived_cornerstone_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        stored
            ...
            .as_deref(),
        Some(
            cornerstone_certificate_verify_cornerstone_certificate_command_summary(
                cornerstone_certificate_session_dir,
            )
            .as_str(),
        ),
        "stored foundation-receipt run verify_cornerstone_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored foundation-receipt run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_keystone_receipt_copy_against_nested_truth(
    stored: &FoundationReceiptReportView,
    nested_session: &FoundationReceiptSessionView,
    nested_status: &FoundationReceiptStatusView,
    mismatches: &mut Vec<String>,
) {
    compare_string(
        &stored.cornerstone_certificate_session_dir,
        &nested_session.cornerstone_certificate_session_dir,
        "stored foundation-receipt copy cornerstone_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.registry_entry_session_dir.as_deref(),
        Some(nested_session.registry_entry_session_dir.as_str()),
        "stored foundation-receipt copy registry_entry_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.notarization_receipt_session_dir.as_deref(),
        Some(nested_session.notarization_receipt_session_dir.as_str()),
        "stored foundation-receipt copy notarization_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.provenance_certificate_session_dir.as_deref(),
        Some(nested_session.provenance_certificate_session_dir.as_str()),
        "stored foundation-receipt copy provenance_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.attestation_seal_session_dir.as_deref(),
        Some(nested_session.attestation_seal_session_dir.as_str()),
        "stored foundation-receipt copy attestation_seal_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.release_capsule_session_dir.as_deref(),
        Some(nested_session.release_capsule_session_dir.as_str()),
        "stored foundation-receipt copy release_capsule_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.activation_ticket_session_dir.as_deref(),
        Some(nested_session.activation_ticket_session_dir.as_str()),
        "stored foundation-receipt copy activation_ticket_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.review_receipt_session_dir.as_deref(),
        Some(nested_session.review_receipt_session_dir.as_str()),
        "stored foundation-receipt copy review_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.handoff_bundle_session_dir.as_deref(),
        Some(nested_session.handoff_bundle_session_dir.as_str()),
        "stored foundation-receipt copy handoff_bundle_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.decision_packet_session_dir.as_deref(),
        Some(nested_session.decision_packet_session_dir.as_str()),
        "stored foundation-receipt copy decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.execute_frozen_session_dir.as_deref(),
        Some(nested_session.execute_frozen_session_dir.as_str()),
        "stored foundation-receipt copy execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.turn_green_session_dir.as_deref(),
        Some(nested_session.turn_green_session_dir.as_str()),
        "stored foundation-receipt copy turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.launch_packet_session_dir.as_deref(),
        Some(nested_session.launch_packet_session_dir.as_str()),
        "stored foundation-receipt copy launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(nested_session.session_dir.as_str()),
        "stored foundation-receipt copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        Some(nested_status.result.as_str()),
        "stored foundation-receipt copy result",
        mismatches,
    );
    compare_option_string(
        Some(stored.verdict.as_str()),
        cornerstone_certificate_run_verdict_for_result(&nested_status.result),
        "stored foundation-receipt copy verdict",
        mismatches,
    );
    compare_option_string(
        stored.handoff_bundle_result.as_deref(),
        nested_status.handoff_bundle_result.as_deref(),
        "stored foundation-receipt copy handoff_bundle_result",
        mismatches,
    );
    compare_option_string(
        stored.decision_packet_result.as_deref(),
        nested_status.decision_packet_result.as_deref(),
        "stored foundation-receipt copy decision_packet_result",
        mismatches,
    );
    compare_option_string(
        stored.execute_frozen_result.as_deref(),
        nested_status.execute_frozen_result.as_deref(),
        "stored foundation-receipt copy execute_frozen_result",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        nested_status.current_pre_activation_gate_verdict.as_deref(),
        "stored foundation-receipt copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        nested_status.current_pre_activation_gate_reason.as_deref(),
        "stored foundation-receipt copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored foundation-receipt copy verify_cornerstone_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored foundation-receipt copy release_capsule_digest_manifest_sha256",
        mismatches,
    );
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_cornerstone_certificate_command_summary,
        &resolve_latest_cornerstone_certificate_command_summary(
            root,
            &paths.nested_cornerstone_certificate_latest_session_dir,
        ),
        "stored session resolve_latest_cornerstone_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.run_latest_foundation_receipt_command_summary,
        &run_latest_foundation_receipt_command_summary(
            root,
            &paths.nested_cornerstone_certificate_latest_session_dir,
        ),
        "stored session run_latest_foundation_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_latest_foundation_receipt_command_summary,
        &verify_latest_foundation_receipt_command_summary(
            &paths.nested_cornerstone_certificate_latest_session_dir,
        ),
        "stored session verify_latest_foundation_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.downstream_foundation_receipt_command_summary,
        &downstream_foundation_receipt_command_summary(
            snapshot,
            &expected_downstream_foundation_receipt_session_dir(paths),
            &paths.nested_foundation_receipt_session_dir,
        ),
        "stored session downstream_foundation_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_cornerstone_certificate_command_summary,
        &verify_cornerstone_certificate_command_summary(
            snapshot,
            &expected_downstream_foundation_receipt_session_dir(paths),
            &paths.nested_foundation_receipt_session_dir,
        ),
        "stored session verify_cornerstone_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_cornerstone_certificate_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_foundation_receipt_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_foundation_receipt_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackageFoundationReceiptLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackageFoundationReceiptLatestPaths,
    snapshot: &LatestCornerstoneCertificateSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_cornerstone_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report run_latest_foundation_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report verify_latest_foundation_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report downstream_foundation_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report verify_cornerstone_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
    ...
    compare_option_string(
        report.foundation_receipt_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "stored report foundation_receipt_latest_session_path",
        mismatches,
    );
    compare_option_string(
        report.foundation_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "stored report foundation_receipt_latest_status_path",
        mismatches,
    );
    compare_option_string(
        report.foundation_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "stored report foundation_receipt_latest_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report latest_cornerstone_certificate_plan_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_cornerstone_certificate_report_path.as_deref(),
        Some(
            paths
                .latest_cornerstone_certificate_report_path
                .display()
        ...
        "stored report latest_cornerstone_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        report.foundation_receipt_report_path.as_deref(),
        Some(
            paths
                .foundation_receipt_report_path
                .display()
                .to_string()
                .as_str(),
        ),
        "stored report foundation_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report nested_cornerstone_certificate_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_foundation_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_foundation_receipt_session_dir
                .display()
        ...
        "stored report nested_foundation_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_cornerstone_certificate_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_foundation_receipt_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_foundation_receipt_session_dir on failure",
        mismatches,
    );
...
fn map_cornerstone_certificate_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_cornerstone_certificate_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_cornerstone_certificate_latest_refused_by_drifted_cornerstone_certificate_contract" => {
            ResolutionFailureKind::DriftedCornerstoneCertificateContract
        }
        _ => ResolutionFailureKind::InvalidLatestChain,
    }
}

fn latest_cornerstone_certificate_terminal_failure(
    report: &CornerstoneCertificateLatestReportView,
) -> Option<LivePackageFoundationReceiptLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackageFoundationReceiptLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackageFoundationReceiptLatestResult::RefusedByInvalidLatestChain)
        }
        Some("refused_by_drifted_cornerstone_certificate_contract") => Some(
            LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract,
        ),
        _ => None,
    }
...
fn cornerstone_certificate_latest_result_from_nested_status(
    result: &str,
) -> Option<LivePackageFoundationReceiptLatestResult> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => Some(
            LivePackageFoundationReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen,
        ),
        "refused_now_by_stage3" => {
            Some(LivePackageFoundationReceiptLatestResult::RefusedNowByStage3)
        }
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackageFoundationReceiptLatestResult::RefusedNowByPreActivationGate)
        }
...
fn cornerstone_certificate_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => Some(
            "tiny_live_package_foundation_receipt_ready_for_manual_execution_when_gate_turns_green",
        ),
        "refused_now_by_stage3" => {
            Some("tiny_live_package_foundation_receipt_refused_now_by_stage3")
        }
        "refused_now_by_pre_activation_gate" => {
            Some("tiny_live_package_foundation_receipt_refused_now_by_pre_activation_gate")
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_foundation_receipt_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}
...
fn result_from_failure_kind(
    kind: ResolutionFailureKind,
) -> LivePackageFoundationReceiptLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackageFoundationReceiptLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackageFoundationReceiptLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedCornerstoneCertificateContract => {
            LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract
        }
...
fn verdict_for_result(
    result: LivePackageFoundationReceiptLatestResult,
) -> TinyLivePackageFoundationReceiptLatestVerdict {
    match result {
        LivePackageFoundationReceiptLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestRefusedByMissingLatestChain
        }
        LivePackageFoundationReceiptLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestRefusedByInvalidLatestChain
        }
        LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract => {
            TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestRefusedByDriftedFoundationReceiptContract
        }
        LivePackageFoundationReceiptLatestResult::RefusedNowByStage3 => {
            TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestRefusedNowByStage3
        }
        LivePackageFoundationReceiptLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestRefusedNowByPreActivationGate
        }
        LivePackageFoundationReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackageFoundationReceiptLatestResult) -> String {
    match result {
        LivePackageFoundationReceiptLatestResult::RefusedByMissingLatestChain => {
            "no immutable clerestory chain is available yet; do not manually hunt foundation-receipt or cornerstone-certificate lineage by hand".to_string()
        }
        LivePackageFoundationReceiptLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-cornerstone-certificate / foundation-receipt path".to_string()
        }
        LivePackageFoundationReceiptLatestResult::RefusedByDriftedFoundationReceiptContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-cornerstone-certificate / foundation-receipt contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackageFoundationReceiptLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-foundation-receipt refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackageFoundationReceiptLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-foundation-receipt refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackageFoundationReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            "the latest immutable chain now resolves into the accepted foundation-receipt contract and one bounded latest-foundation-receipt handoff is ready for manual execution when gate truth turns green; this wrapper still did not run or authorize the frozen controller".to_string()
        }
    }
}
...
fn cornerstone_certificate_verify_cornerstone_certificate_command_summary(
    cornerstone_certificate_session_dir: &Path,
) -> String {
    format!(
        "verify immutable keystone-receipt session under {} before freezing the final cornerstone certificate",
...
fn refusal_status(
    ...
    latest_cornerstone_certificate_step: Option<PackageFoundationReceiptLatestStepArtifact>,
    foundation_receipt_step: Option<PackageFoundationReceiptLatestStepArtifact>,
) -> PackageFoundationReceiptLatestStatus {
    ...
    PackageFoundationReceiptLatestStatus {
        ...
        result,
        reason,
        latest_cornerstone_certificate_plan_verdict: resolved
            .map(|value| value.latest_cornerstone_certificate_plan.verdict.clone()),
        latest_cornerstone_certificate_verdict: latest_cornerstone_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        foundation_receipt_verdict: foundation_receipt_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackageFoundationReceiptLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_cornerstone_certificate_plan_step: failure
            .latest_cornerstone_certificate_plan_raw
            .as_ref()
            .map(|_| {
                step_artifact(
                    ...
                    failure
                        ...
                        .unwrap_or("<missing>"),
                    failure
                        .latest_cornerstone_certificate_plan_reason
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_cornerstone_certificate_plan_generated_at
                        .unwrap_or_else(Utc::now),
                )
            }),
        latest_cornerstone_certificate_step: None,
...
fn verify_invalid_foundation_receipt_latest_report_without_session(
    session_dir: &Path,
    verification_mismatches: Vec<String>,
) -> PackageFoundationReceiptLatestReport {
    ...
    PackageFoundationReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_foundation_receipt".to_string(),
        verdict: TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestVerifyInvalid,
        reason,
        root: None,
        ...
        session_dir: Some(session_dir.display().to_string()),
        foundation_receipt_latest_session_path: Some(
            foundation_receipt_latest_paths(session_dir)
                .session_path
                .display()
                .to_string(),
        ),
        foundation_receipt_latest_status_path: Some(
            foundation_receipt_latest_paths(session_dir)
                .status_path
                .display()
                .to_string(),
        ),
        foundation_receipt_latest_report_path: Some(
            foundation_receipt_latest_paths(session_dir)
                .report_path
                .display()
                .to_string(),
        ),
        latest_cornerstone_certificate_plan_report_path: None,
        latest_cornerstone_certificate_report_path: None,
        foundation_receipt_report_path: None,
        nested_cornerstone_certificate_latest_session_dir: None,
        nested_foundation_receipt_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_cornerstone_certificate_command_summary: None,
        run_latest_foundation_receipt_command_summary: None,
        verify_latest_foundation_receipt_command_summary: None,
        downstream_foundation_receipt_command_summary: None,
        verify_cornerstone_certificate_command_summary: None,
...
fn fallback_cornerstone_certificate_latest_copy() -> CornerstoneCertificateLatestReportView {
    CornerstoneCertificateLatestReportView {
        generated_at: Utc::now(),
...
fn fallback_notarization_receipt_session(
    snapshot: &LatestCornerstoneCertificateSnapshot,
    session_dir: &Path,
) -> FoundationReceiptSessionView {
    FoundationReceiptSessionView {
        cornerstone_certificate_session_dir: "<missing>".to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_cornerstone_certificate_command_summary:
            cornerstone_certificate_verify_cornerstone_certificate_command_summary(Path::new("<missing>")),
        ...
        cornerstone_certificate_summary:
            "Cornerstone certificate outcome: fallback session is synthetic and must not verify green."
                .to_string(),
        ...
        foundation_receipt_summary:
            "Foundation receipt outcome: fallback session is synthetic and must not verify green."
                .to_string(),
        foundation_receipt_sha256: "<missing>".to_string(),
        foundation_receipt_algorithm: "sha256".to_string(),
        explicit_statement:
            "the verified cornerstone-certificate session is the primary input here; this immutable foundation receipt freezes one final diadem identity over the fully culminated chain without enabling production execution"
                .to_string(),
    }
}

fn fallback_notarization_receipt_status(
    _snapshot: &LatestCornerstoneCertificateSnapshot,
    session_dir: &Path,
) -> FoundationReceiptStatusView {
    FoundationReceiptStatusView {
        session_dir: session_dir.display().to_string(),
        cornerstone_certificate_session_dir: "<missing>".to_string(),
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        cornerstone_certificate_step: None,
        provenance_certificate_summary:
        ...
        keystone_receipt_algorithm: "sha256".to_string(),
        cornerstone_certificate_summary:
            "Cornerstone certificate outcome: fallback status is synthetic and must not verify green."
                .to_string(),
        cornerstone_certificate_sha256: "<missing>".to_string(),
        cornerstone_certificate_algorithm: "sha256".to_string(),
        foundation_receipt_summary:
            "Foundation receipt outcome: fallback status is synthetic and must not verify green."
                .to_string(),
        foundation_receipt_sha256: "<missing>".to_string(),
        foundation_receipt_algorithm: "sha256".to_string(),
        explicit_statement:
            "this foundation receipt is archival and read-only; it never executes or enables the frozen controller"
                .to_string(),
    }
}

fn render_report_lines(report: &PackageFoundationReceiptLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_cornerstone_certificate_latest_session_dir {
        lines.push(format!(
            "nested_cornerstone_certificate_latest_session_dir={value}"
        ));
    }
    ...
    if let Some(value) = &report.nested_foundation_receipt_session_dir {
        lines.push(format!("nested_foundation_receipt_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_foundation_receipt_command_summary {
        lines.push(format!(
            "downstream_foundation_receipt_command_summary={value}"
        ));
    }
...
mod tests {
    ...
    struct FakeNotarizationReceiptLatestFixture {
        ...
        cornerstone_certificate_latest_plan_json_path: PathBuf,
        cornerstone_certificate_latest_run_json_path: PathBuf,
        cornerstone_certificate_latest_verify_json_path: PathBuf,
    ...
    impl FakeNotarizationReceiptLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestFoundationReceipt,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestFoundationReceiptScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestFoundationReceipt,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestFoundationReceipt,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    #[test]
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_cornerstone_certificate_lineage(
    ) {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_plan_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = plan_latest_foundation_receipt_report(&fixture.plan_config()).unwrap();
        let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert_eq!(
            report.verdict,
            TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestPlanReady
        );
        assert_eq!(report.mode, "plan_latest_foundation_receipt");
        assert_eq!(
            report.latest_top_step_name.as_deref(),
    ...
    #[test]
    fn render_mode_emits_exact_downstream_foundation_receipt_contract_not_second_controller_path() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_render_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let report =
            render_latest_foundation_receipt_script_report(&fixture.render_config()).unwrap();
        assert_eq!(
            report.verdict,
            TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestScriptRendered
        );
        assert_eq!(report.mode, "render_latest_foundation_receipt_script");
        ...
        assert!(script.contains("--cornerstone-certificate-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-foundation-receipt"));
        assert!(script.contains("--verify-live-package-foundation-receipt"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            (
                "cornerstone_certificate_latest_missing",
                "tiny_live_package_cornerstone_certificate_latest_refused_by_missing_latest_chain",
                "no persisted immutable chain exists",
                TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestRefusedByMissingLatestChain,
            ),
            (
                "cornerstone_certificate_latest_invalid",
                "tiny_live_package_cornerstone_certificate_latest_refused_by_invalid_latest_chain",
                "latest immutable chain is incomplete",
                TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestRefusedByInvalidLatestChain,
            ),
            (
                "cornerstone_certificate_latest_below",
                "tiny_live_package_cornerstone_certificate_latest_refused_by_invalid_latest_chain",
            ...
            let mut plan: Value = load_json(&fixture.cornerstone_certificate_latest_plan_json_path).unwrap();
            plan["verdict"] = json!(verdict);
            plan["reason"] = json!(reason);
            if label.ends_with("below") {
                plan["latest_top_step_name"] = json!("choir_receipt");
            }
            persist_json(&fixture.cornerstone_certificate_latest_plan_json_path, &plan).unwrap();

            let report = run_latest_foundation_receipt_report(&fixture.run_config()).unwrap();
            assert_eq!(report.verdict, expected);
        }
    }

    #[test]
    fn plan_mode_maps_real_latest_cornerstone_certificate_drift_verdict_to_local_drift_refusal() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_plan_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value =
            load_json(&fixture.cornerstone_certificate_latest_plan_json_path).unwrap();
        plan["verdict"] = json!(
            "tiny_live_package_cornerstone_certificate_latest_refused_by_drifted_cornerstone_certificate_contract"
        );
        plan["reason"] = json!("real accepted latest-cornerstone-certificate drift");
        persist_json(
            &fixture.cornerstone_certificate_latest_plan_json_path,
            &plan,
        )
        .unwrap();
        ...
        assert_eq!(
            report.result.as_deref(),
            Some("refused_by_drifted_foundation_receipt_contract")
        );
    }

    #[test]
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_cornerstone_certificate_reports_real_drift(
    ) {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_run_drift",
            "refused_by_drifted_cornerstone_certificate_contract",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert_eq!(report.foundation_receipt_verdict.as_deref(), None);
    }

    #[test]
    fn run_mode_refuses_when_nested_release_capsule_lineage_drifts_from_latest_immutable_chain() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_handoff_bundle_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut run: Value =
            load_json(&fixture.cornerstone_certificate_latest_run_json_path).unwrap();
        run["package_dir"] = json!("/tampered/package");
        persist_json(&fixture.cornerstone_certificate_latest_run_json_path, &run).unwrap();
        ...
        assert!(report
            .reason
            .contains("stored latest-cornerstone-certificate run package_dir"));
    }

    #[test]
    fn verify_mode_fails_when_persisted_latest_cornerstone_certificate_artifacts_are_tampered_or_missing_nested_lineage_truth(
    ) {
        {
            let fixture = fake_cornerstone_certificate_latest_fixture(
                "cornerstone_certificate_latest_verify_tampered_copy",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            run_latest_foundation_receipt_report(&fixture.run_config()).unwrap();

            let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.foundation_receipt_report_path).unwrap();
            ...
            let verify = verify_latest_foundation_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestVerifyInvalid
            );
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains("stored foundation-receipt run decision_packet_session_dir")
            }));
        }

        {
            let fixture = fake_cornerstone_certificate_latest_fixture(
                "cornerstone_certificate_latest_verify_missing_nested_truth",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
            fs::remove_file(nested_cornerstone_certificate_latest_report_path(
                &paths.nested_cornerstone_certificate_latest_session_dir,
            ))
            .unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains(
                    "tiny_live_activation_package_cornerstone_certificate_latest.report.json",
                )
            }));
        }
    }
    ...
    fn verify_accepts_real_shaped_native_foundation_identity_layers_on_happy_path() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_real_shaped_foundation_paths_green",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn run_and_verify_reject_stale_keystone_wrapper_schema_for_downstream_foundation_copy() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_rejects_stale_keystone_wrapper_schema",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert!(paths.foundation_receipt_report_path.ends_with(
            "tiny_live_activation_package_foundation_receipt_latest.foundation_receipt.report.json"
        ));
        assert!(paths.foundation_receipt_report_path.exists());
        assert!(
            !fixture
                .wrapper_session_dir
                .join(
                    "tiny_live_activation_package_foundation_receipt_latest.keystone_receipt.report.json"
                )
                .exists()
        );
        ...
        let stored_step = status
            .get("foundation_receipt_step")
            .cloned()
            .expect("foundation_receipt_step should be present");
        assert!(status.get("keystone_receipt_step").is_none());
        status["keystone_receipt_step"] = stored_step;
        if let Some(object) = status.as_object_mut() {
            object.remove("foundation_receipt_step");
        }
        persist_json(&paths.status_path, &status).unwrap();

        let error = verify_latest_foundation_receipt_report(&fixture.verify_config()).unwrap_err();
        assert!(error.to_string().contains("foundation_receipt_step"));
    }

    #[test]
    fn verify_names_foundation_receipt_when_nested_native_session_load_breaks() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_nested_foundation_session_load_break",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
        fs::remove_file(nested_foundation_receipt_session_path(
            &paths.nested_foundation_receipt_session_dir,
        ))
        .unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("failed reading nested foundation-receipt session under")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_foundation_receipt_session_path_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_foundation_session_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.foundation_receipt_report_path).unwrap();
        report["foundation_receipt_session_path"] =
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt run foundation_receipt_session_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_foundation_receipt_status_path_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_foundation_status_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.foundation_receipt_report_path).unwrap();
        report["foundation_receipt_status_path"] = json!("/tampered/finality-receipt-status.json");
        persist_json(&paths.foundation_receipt_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt run foundation_receipt_status_path")
        }));
    }

    #[test]
    fn run_names_latest_cornerstone_certificate_when_nested_latest_wrapper_run_fails() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_nested_run_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
        overwrite_fake_cornerstone_certificate_latest_command(
            &fixture.bin_dir.join(CORNERSTONE_CERTIFICATE_LATEST_BIN),
            &fixture.cornerstone_certificate_latest_plan_json_path,
            &fixture.cornerstone_certificate_latest_run_json_path,
            &fixture.cornerstone_certificate_latest_verify_json_path,
            None,
            Some("forced nested latest-cornerstone-certificate run failure"),
            None,
            &fixture.root,
            &paths.nested_cornerstone_certificate_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed running nested latest-cornerstone-certificate handoff under"));
    }

    #[test]
    fn run_names_latest_cornerstone_certificate_when_nested_latest_wrapper_verify_fails() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_nested_verify_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_cornerstone_certificate_latest_command(
            ...
            None,
            None,
            Some("forced nested latest-cornerstone-certificate verify failure"),
            &fixture.root,
            &paths.nested_cornerstone_certificate_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed verifying nested latest-cornerstone-certificate handoff under"));
    }

    #[test]
    fn run_uses_foundation_placeholders_when_missing_snapshot_blocks_nested_handoff_summaries() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_missing_snapshot_foundation_placeholders",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_cornerstone_certificate_latest_command(
            ...
            &fixture.cornerstone_certificate_latest_run_json_path,
            &fixture.cornerstone_certificate_latest_verify_json_path,
            Some("forced latest-cornerstone-certificate plan failure without snapshot"),
            None,
            None,
            &fixture.root,
            &paths.nested_cornerstone_certificate_latest_session_dir,
        ...
        assert_eq!(
            report
                .downstream_foundation_receipt_command_summary
                .as_deref(),
            Some("<foundation-receipt-run-unavailable>")
        );
        assert_eq!(
            report
                .verify_cornerstone_certificate_command_summary
                .as_deref(),
            Some("<foundation-receipt-verify-unavailable>")
        );

        let session: PackageFoundationReceiptLatestSession =
            load_json(&paths.session_path).unwrap();
        assert_eq!(
            session.downstream_foundation_receipt_command_summary,
            "<foundation-receipt-run-unavailable>"
        );
        assert_eq!(
            session.verify_cornerstone_certificate_command_summary,
            "<foundation-receipt-verify-unavailable>"
        );
    }
    ...
    fn verify_fails_when_copied_native_pinnacle_receipt_sha256_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_pinnacle_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy pinnacle_receipt_sha256")
        }));
    }

    #[test]
    fn verify_fails_when_copied_nested_latest_cornerstone_certificate_explicit_statement_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_launch_packet_explicit_statement_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut report: Value =
            load_json(&paths.latest_cornerstone_certificate_report_path).unwrap();
        report["explicit_statement"] =
            json!("tampered nested latest release capsule explicit statement");
        persist_json(&paths.latest_cornerstone_certificate_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored latest-cornerstone-certificate copy explicit_statement")
        }));
    }
    ...
    fn verify_rejects_tampered_stale_keystone_receipt_wrapper_mode_string() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_wrapper_mode_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        report["mode"] = json!("run_latest_cornerstone_certificate");
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("latest-foundation-receipt stored report mode") }));
    }

    #[test]
    fn downstream_native_foundation_session_dir_uses_real_cornerstone_certificate_session_path() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_downstream_session_dir_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
        ...
        let expected = paths
            .nested_cornerstone_certificate_latest_session_dir
            .join("tiny_live_activation_package_cornerstone_certificate_latest.cornerstone_certificate_session");
        ...
        assert!(downstream
            ...
            .unwrap()
            .to_string_lossy()
            .contains("cornerstone_certificate_session"));
        assert!(!downstream
            .file_name()
            .unwrap()
            .to_string_lossy()
    ...
    {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_native_cornerstone_filenames",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
        let real_session =
            nested_foundation_receipt_session_path(&paths.nested_foundation_receipt_session_dir);
        let real_status =
            nested_foundation_receipt_status_path(&paths.nested_foundation_receipt_session_dir);
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_decision_packet_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy decision_packet_result")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_attestation_seal_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy attestation_seal_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_registry_entry_session_dir_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_registry_entry_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy registry_entry_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_notarization_receipt_session_dir_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_notarization_receipt_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy notarization_receipt_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_chain_fingerprint_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy chain_fingerprint_summary")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_chain_fingerprint_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy chain_fingerprint_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_chain_fingerprint_algorithm_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy chain_fingerprint_algorithm")
        }));
    }
    ...
    fn verify_fails_when_copied_native_completion_certificate_closure_certificate_sha256_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_closure_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy closure_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_archive_receipt_sha256_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_archive_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy archive_receipt_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_finality_receipt_sha256_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_finality_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy finality_receipt_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_consummation_record_sha256_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_consummation_record_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy consummation_record_sha256")
        }));
    }
    ...
    {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_completion_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy completion_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_decision_packet_anchor_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy decision_packet_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_current_authorization_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored foundation-receipt copy execute_frozen_result")
        }));
    }

    #[test]
    fn verify_fails_when_copied_nested_foundation_receipt_verdict_drifts() {
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_release_capsule_verdict_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| entry.contains("stored foundation-receipt copy verdict")));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_current_live_cutover_controller_summary_drifts(
        ...
        let fixture = fake_cornerstone_certificate_latest_fixture(
            "cornerstone_certificate_latest_verify_current_live_cutover_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn all_modes_preserve_stage3_and_pre_activation_refusal_semantics_and_do_not_imply_activation_authorization_by_themselves(
        ...
        {
            let fixture = fake_cornerstone_certificate_latest_fixture(
                "cornerstone_certificate_latest_stage3",
                "refused_now_by_stage3",
                "refused_now_by_stage3",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);

            let plan = plan_latest_foundation_receipt_report(&fixture.plan_config()).unwrap();
            assert_eq!(
                plan.verdict,
                TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestPlanReady
            );
            assert_eq!(plan.mode, "plan_latest_foundation_receipt");
            assert!(!plan.activation_authorized);

            let run = run_latest_foundation_receipt_report(&fixture.run_config()).unwrap();
            assert_eq!(
                run.verdict,
                TinyLivePackageFoundationReceiptLatestVerdict::TinyLivePackageFoundationReceiptLatestRefusedNowByStage3
            );
            assert_eq!(run.mode, "run_latest_foundation_receipt");
            ...
            assert_eq!(verify.mode, "verify_latest_foundation_receipt");
            assert!(!verify.activation_authorized);
        }

        {
            let fixture = fake_cornerstone_certificate_latest_fixture(
                "cornerstone_certificate_latest_pre_activation",
                "refused_now_by_pre_activation_gate",
                "refused_now_by_pre_activation_gate",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        {
            let fixture = fake_cornerstone_certificate_latest_fixture(
                "cornerstone_certificate_latest_green",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn fake_cornerstone_certificate_latest_fixture(
        label: &str,
        cornerstone_certificate_latest_run_result: &str,
        ...
        let cornerstone_certificate_latest_plan_json_path =
            temp_dir.join("cornerstone_certificate_latest.plan.json");
        let cornerstone_certificate_latest_run_json_path =
            temp_dir.join("cornerstone_certificate_latest.run.json");
        let cornerstone_certificate_latest_verify_json_path =
            temp_dir.join("cornerstone_certificate_latest.verify.json");
        ...
        let fixture = FakeNotarizationReceiptLatestFixture {
            ...
            install_root,
            backend_command,
            cornerstone_certificate_latest_plan_json_path,
            cornerstone_certificate_latest_run_json_path,
            cornerstone_certificate_latest_verify_json_path,
        ...
        persist_json(
            &fixture.cornerstone_certificate_latest_plan_json_path,
            &default_cornerstone_certificate_latest_plan_report(&fixture),
        )
        .unwrap();
        persist_json(
            &fixture.cornerstone_certificate_latest_run_json_path,
            &default_cornerstone_certificate_latest_run_report(
                &fixture,
                cornerstone_certificate_latest_run_result,
            ),
        )
        .unwrap();
        persist_json(
            &fixture.cornerstone_certificate_latest_verify_json_path,
            &default_cornerstone_certificate_latest_verify_report(
                &fixture,
                cornerstone_certificate_latest_run_result,
            ),
        )
        ...
        let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
        write_fake_cornerstone_certificate_latest_command(
            &fixture.bin_dir.join(CORNERSTONE_CERTIFICATE_LATEST_BIN),
            &fixture.cornerstone_certificate_latest_plan_json_path,
            &fixture.cornerstone_certificate_latest_run_json_path,
            &fixture.cornerstone_certificate_latest_verify_json_path,
    ...
    fn default_cornerstone_certificate_latest_plan_report(
        ...
        json!({
            ...
            "session_dir": paths.nested_cornerstone_certificate_latest_session_dir.display().to_string(),
            ...
            "activation_authorized": false,
            "explicit_statement": "latest cornerstone-certificate wrapper plan"
        })
    }

    fn canonical_cornerstone_certificate_latest_result(result: &str) -> &str {
        match result {
            "runnable_when_gate_truth_turns_green" => {
    ...
    fn default_cornerstone_certificate_latest_run_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let result = canonical_cornerstone_certificate_latest_result(result);
        let paths = foundation_receipt_latest_paths(&fixture.wrapper_session_dir);
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_cornerstone_certificate_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_cornerstone_certificate_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_cornerstone_certificate_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
        ...
        json!({
            ...
            "verdict": verdict,
            "reason": format!("latest cornerstone-certificate {result}"),
            "latest_top_step_name": "clerestory_certificate",
            "latest_top_session_dir": fixture.latest_top_session_dir.display().to_string(),
            "historical_execute_frozen_session_dir": fixture.historical_execute_frozen_session_dir.display().to_string(),
            "turn_green_session_dir": fixture.turn_green_session_dir.display().to_string(),
            ...
            "clerestory_certificate_algorithm": "sha256",
            "activation_authorized": false,
            "explicit_statement": "latest cornerstone-certificate wrapper run"
        })
    }

    fn default_cornerstone_certificate_latest_verify_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let mut report = default_cornerstone_certificate_latest_run_report(fixture, result);
        report["mode"] = json!("verify_latest_cornerstone_certificate");
    ...
    fn default_provenance_certificate_session(
        ...
        let snapshot = LatestCornerstoneCertificateSnapshot {
        ...
        json!({
            "session_version": "1",
            "planned_at": Utc::now(),
            "cornerstone_certificate_session_dir": cornerstone_certificate_session_dir.display().to_string(),
            ...
            "release_capsule_session_dir": cornerstone_certificate_session_dir.display().to_string(),
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": foundation_receipt_latest_paths(&fixture.wrapper_session_dir).nested_foundation_receipt_session_dir.display().to_string(),
            "verify_cornerstone_certificate_command_summary": cornerstone_certificate_verify_cornerstone_certificate_command_summary(&cornerstone_certificate_session_dir),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
            ...
            "cornerstone_certificate_summary": "Cornerstone certificate identity: fixture immutable imperial seal over verified keystone-receipt truth.",
            ...
            "explicit_statement": "the verified cornerstone-certificate session is the primary input here; this immutable foundation receipt freezes one final diadem identity over the fully culminated chain without enabling production execution"
        })
    }
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_foundation_receipt_session_dir.display().to_string(),
            "cornerstone_certificate_session_dir": expected_downstream_foundation_receipt_session_dir(&paths).display().to_string(),
            "result": result,
            "reason": reason,
            ...
            "current_pre_activation_gate_verdict": gate_verdict,
            "current_pre_activation_gate_reason": gate_reason,
            "cornerstone_certificate_step": Value::Null,
            ...
            "keystone_receipt_sha256": "fixture-keystone-receipt-sha256",
            "keystone_receipt_algorithm": "sha256",
            "cornerstone_certificate_summary": "Cornerstone certificate identity: fixture status immutable imperial seal over verified keystone-receipt truth.",
            "cornerstone_certificate_sha256": "fixture-cornerstone-certificate-sha256",
            "cornerstone_certificate_algorithm": "sha256",
            ...
            "foundation_receipt_sha256": "fixture-foundation-receipt-sha256",
            "foundation_receipt_algorithm": "sha256",
            "explicit_statement": "this foundation receipt is archival and read-only; it never executes or enables the frozen controller"
        })
    }
    ...
    fn default_provenance_certificate_run_report(
        ...
        let foundation_receipt_verdict = match result {
            "ready_for_manual_execution_when_gate_turns_green" => {
                "tiny_live_package_foundation_receipt_ready_for_manual_execution_when_gate_turns_green"
            }
            "refused_now_by_stage3" => "tiny_live_package_foundation_receipt_refused_now_by_stage3",
            "refused_now_by_pre_activation_gate" => {
                "tiny_live_package_foundation_receipt_refused_now_by_pre_activation_gate"
            }
            _ => "tiny_live_package_foundation_receipt_refused_now_by_invalid_or_drifted_contract",
        };
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_foundation_receipt",
            "verdict": foundation_receipt_verdict,
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_foundation_receipt_session_dir.display().to_string(),
            "foundation_receipt_session_path": nested_foundation_receipt_session_path(&paths.nested_foundation_receipt_session_dir).display().to_string(),
            "foundation_receipt_status_path": nested_foundation_receipt_status_path(&paths.nested_foundation_receipt_session_dir).display().to_string(),
            "archived_cornerstone_certificate_report_path": paths.nested_foundation_receipt_session_dir.join("tiny_live_activation_package_foundation_receipt.cornerstone_certificate.report.json").display().to_string(),
            "result": result,
            ...
            "cornerstone_certificate_step": Value::Null,
            "verify_cornerstone_certificate_command_summary": cornerstone_certificate_verify_cornerstone_certificate_command_summary(&expected_downstream_foundation_receipt_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn write_fake_cornerstone_certificate_latest_command(
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-cornerstone-certificate session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-cornerstone-certificate \"*) mode='plan';;\n  *\" --run-latest-cornerstone-certificate \"*) mode='run';;\n  *\" --verify-latest-cornerstone-certificate \"*) mode='verify';;\n  *) echo 'unexpected latest-cornerstone-certificate mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_cornerstone_certificate_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
    ...
    fn overwrite_fake_cornerstone_certificate_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        verify_json_path: &Path,
    ...
    fn write_fake_release_capsule_command(
        ...
        let nested_session_path = nested_foundation_receipt_session_path(session_dir);
        let nested_status_path = nested_foundation_receipt_status_path(session_dir);
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --cornerstone-certificate-session-dir {} \"*) : ;;\n  *) echo 'unexpected cornerstone-certificate-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected confirm-decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected foundation-receipt session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --run-live-package-foundation-receipt \"*) mode='run';;\n  *\" --verify-live-package-foundation-receipt \"*) mode='verify';;\n  *) echo 'unexpected foundation-receipt mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  run) mkdir -p '{}'; cp '{}' '{}'; cp '{}' '{}'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
