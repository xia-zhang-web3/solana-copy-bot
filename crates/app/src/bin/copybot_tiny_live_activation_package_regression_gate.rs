...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_regression_gate --root <path> [--session-dir <path>] [--json] (--latest-gate | --session-gate | --verify-gate)";
...
const NOT_AUTHORIZED_SUMMARY: &str =
    "This tiny-live package regression-gate surface only audits persisted package session/status/report artifacts. It stays planning-safe, does not authorize activation, and does not override the Stage 3 production gate.";
...
enum TinyLivePackageRegressionGateVerdict {
    TinyLivePackageRegressionGateAvailable,
    TinyLivePackageRegressionGateEmpty,
    TinyLivePackageRegressionGateInvalid,
    TinyLivePackageRegressionGateVerifyOk,
    TinyLivePackageRegressionGateVerifyInvalid,
}
...
#[derive(Debug, Clone, Serialize)]
struct PackageRegressionGateReport {
    mode: String,
    verdict: TinyLivePackageRegressionGateVerdict,
    reason: String,
    subject_chain: Option<DriftChainSummary>,
    previous_chain: Option<DriftChainSummary>,
    subject_chain_valid: bool,
    previous_chain_available: bool,
    previous_chain_valid: bool,
    subject_chain_at_top_accepted_layer: bool,
    previous_chain_at_top_accepted_layer: bool,
    comparable: bool,
    top_identity_drift: bool,
    lineage_drift: bool,
    manifest_drift: bool,
    gate_clean: bool,
    byte_identical_at_bounded_artifact_level: bool,
    diff_entries: Vec<DriftEntry>,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--latest-gate" => set_mode(&mut mode, Mode::LatestGate, "--latest-gate")?,
            "--session-gate" => set_mode(&mut mode, Mode::SessionGate, "--session-gate")?,
            "--verify-gate" => set_mode(&mut mode, Mode::VerifyGate, "--verify-gate")?,
            "--json" => json = true,
    ...
    let Some(mode) = mode else {
        bail!("exactly one of --latest-gate, --session-gate, or --verify-gate is required");
    };

    match mode {
        Mode::LatestGate | Mode::VerifyGate => {
            if root.is_none() {
                bail!("missing required --root");
            }
            if session_dir.is_some() {
                bail!("--session-dir is only valid with --session-gate");
            }
        }
        Mode::SessionGate => {
            if root.is_none() {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::LatestGate => build_latest_gate_report(&config)?,
        Mode::SessionGate => build_session_gate_report(&config)?,
        Mode::VerifyGate => build_verify_gate_report(&config)?,
    };
    if config.json {
        serde_json::to_string_pretty(&report)
            .context("failed serializing tiny-live package regression gate json")
    } else {
        Ok(render_regression_gate_human(&report))
    }
}

fn build_latest_gate_report(config: &Config) -> Result<PackageRegressionGateReport> {
    let root = config.root.as_ref().expect("validated root");
    let sessions = scan_package_sessions(root)?;
    let Some(latest) = select_latest_top_chain_candidate(&sessions) else {
        return Ok(missing_latest_report(
            "latest_gate",
            format!(
    ...
    let subject_side = resolve_candidate_side(latest, "latest_root".to_string())?;
    let Some(previous) = select_previous_comparable_top_chain_candidate(&sessions, latest) else {
        return Ok(missing_previous_report(
            "latest_gate",
            subject_side,
            format!(
                "no previous comparable persisted top chain was found under {} for top step {}",
                root.display(),
    ...
    let previous_side = resolve_candidate_side(previous, "previous_under_root".to_string())?;
    Ok(compare_sides(
        "latest_gate",
        subject_side,
        Some(previous_side),
...
fn build_session_gate_report(config: &Config) -> Result<PackageRegressionGateReport> {
    let root = config.root.as_ref().expect("validated root");
    let sessions = scan_package_sessions(root)?;
    let subject_session_dir = config.session_dir.as_ref().expect("validated session dir");
    let subject_side = resolve_session_side(
        subject_session_dir,
        format!("session {}", subject_session_dir.display()),
    )?;
    let subject_artifact =
        inspect_package_artifact(&subject_side.top_session_dir, &subject_side.top_step_name)?;
    if !sessions.iter().any(|candidate| {
        candidate.package_step_name == subject_artifact.package_step_name
            && candidate.session_dir == subject_artifact.session_dir
    }) {
        return Ok(missing_previous_report(
            "session_gate",
            subject_side,
            format!(
                "session {} is not present under root {} as a persisted comparable top chain",
                subject_artifact.session_dir.display(),
                root.display()
            ),
            false,
        ));
    ...
    let Some(previous) =
        select_previous_comparable_top_chain_candidate(&sessions, &subject_artifact)
    else {
        return Ok(missing_previous_report(
            "session_gate",
            subject_side,
            format!(
                "no previous comparable persisted top chain was found under {} before {}",
                root.display(),
                subject_artifact.session_dir.display()
            ),
            false,
...
fn build_verify_gate_report(config: &Config) -> Result<PackageRegressionGateReport> {
    let root = config.root.as_ref().expect("validated root");
    let sessions = scan_package_sessions(root)?;
    let Some(latest) = select_latest_top_chain_candidate(&sessions) else {
        return Ok(missing_latest_report(
...
fn missing_latest_report(
    mode: &str,
    reason: String,
    verify_mode: bool,
) -> PackageRegressionGateReport {
    let verdict = if verify_mode {
        TinyLivePackageRegressionGateVerdict::TinyLivePackageRegressionGateVerifyInvalid
    } else {
        TinyLivePackageRegressionGateVerdict::TinyLivePackageRegressionGateEmpty
    };
    ...
    PackageRegressionGateReport {
        ...
        subject_chain: None,
        previous_chain: None,
        subject_chain_valid: false,
        previous_chain_available: false,
        previous_chain_valid: false,
        subject_chain_at_top_accepted_layer: false,
        previous_chain_at_top_accepted_layer: false,
        comparable: false,
        top_identity_drift: false,
        lineage_drift: false,
...
fn missing_previous_report(
    mode: &str,
    subject_side: ComparisonSide,
    reason: String,
    verify_mode: bool,
) -> PackageRegressionGateReport {
    let verdict = if verify_mode {
        ...
        TinyLivePackageRegressionGateVerdict::TinyLivePackageRegressionGateInvalid
    };
    let subject_chain_valid = subject_side.invalid_reasons.is_empty();
    let subject_chain_at_top_accepted_layer =
        subject_side.top_step_name == TOP_ACCEPTED_PACKAGE_STEP;

    PackageRegressionGateReport {
        mode: mode.to_string(),
        verdict,
        reason: reason.clone(),
        subject_chain: Some(summarize_side(&subject_side)),
        previous_chain: None,
        subject_chain_valid,
        previous_chain_available: false,
        previous_chain_valid: false,
        subject_chain_at_top_accepted_layer,
        previous_chain_at_top_accepted_layer: false,
        comparable: false,
        top_identity_drift: false,
        lineage_drift: false,
        ...
        byte_identical_at_bounded_artifact_level: false,
        diff_entries: vec![DriftEntry {
            category: "previous_missing".to_string(),
            key: "previous_chain".to_string(),
            subject_value: Some(subject_side.top_session_dir.display().to_string()),
            previous_value: None,
            reason,
        }],
        planning_safe_only: true,
...
fn resolve_candidate_side(
    candidate: &PackageSessionArtifact,
    source_label: String,
...
fn compare_sides(
    mode: &str,
    subject_side: ComparisonSide,
    previous_side: Option<ComparisonSide>,
    verify_mode: bool,
) -> PackageRegressionGateReport {
    let previous_side = previous_side.expect("validated previous side");
    let subject_summary = summarize_side(&subject_side);
    let previous_summary = summarize_side(&previous_side);
    let subject_chain_valid = subject_side.invalid_reasons.is_empty();
    let previous_chain_valid = previous_side.invalid_reasons.is_empty();
    let subject_chain_at_top_accepted_layer =
        subject_side.top_step_name == TOP_ACCEPTED_PACKAGE_STEP;
    let previous_chain_at_top_accepted_layer =
        previous_side.top_step_name == TOP_ACCEPTED_PACKAGE_STEP;

    let mut diff_entries = Vec::new();
    let comparable = subject_chain_valid && previous_chain_valid;

    if !subject_side.invalid_reasons.is_empty() {
        diff_entries.extend(
            subject_side
                .invalid_reasons
                .iter()
                .map(|reason| DriftEntry {
                    category: "subject_invalid".to_string(),
                    key: "subject_chain".to_string(),
                    subject_value: Some(reason.clone()),
                    previous_value: None,
                    reason: format!("subject chain is not comparable: {reason}"),
                }),
        );
    }
    if !previous_side.invalid_reasons.is_empty() {
        diff_entries.extend(
            previous_side
                .invalid_reasons
                .iter()
                .map(|reason| DriftEntry {
                    category: "previous_invalid".to_string(),
                    key: "previous_chain".to_string(),
                    subject_value: None,
                    previous_value: Some(reason.clone()),
                    reason: format!("previous chain is not comparable: {reason}"),
                }),
        );
    }

    let mut top_identity_drift = false;
    let mut lineage_drift = false;
    let mut manifest_drift = false;

    if comparable {
        diff_entries.extend(compare_top_identity(&subject_side, &previous_side));
        let top_identity_entry_count = diff_entries.len();
        diff_entries.extend(compare_lineage(&subject_side, &previous_side));
        let lineage_entry_count = diff_entries.len();
        diff_entries.extend(compare_manifest(&subject_side, &previous_side));
        top_identity_drift = top_identity_entry_count > 0;
        lineage_drift = lineage_entry_count > top_identity_entry_count;
        manifest_drift = diff_entries.len() > lineage_entry_count;
    }

    let byte_identical_at_bounded_artifact_level = comparable && !manifest_drift;

    let mut invalid_reasons = Vec::new();
    if !subject_chain_at_top_accepted_layer {
        invalid_reasons.push(format!(
            "subject chain stops at {} below the current top accepted layer {}",
            subject_side.top_step_name, TOP_ACCEPTED_PACKAGE_STEP
        ));
    }
    if !previous_chain_at_top_accepted_layer {
        invalid_reasons.push(format!(
            "previous chain stops at {} below the current top accepted layer {}",
            previous_side.top_step_name, TOP_ACCEPTED_PACKAGE_STEP
        ));
    }
    if !comparable {
        invalid_reasons.extend(subject_side.invalid_reasons.iter().cloned());
        invalid_reasons.extend(previous_side.invalid_reasons.iter().cloned());
    }
    if comparable && top_identity_drift {
        invalid_reasons
            .push("top identity drift detected between compared persisted chains".to_string());
    }
    if comparable && lineage_drift {
        invalid_reasons
            .push("lineage drift detected between compared persisted chains".to_string());
    }
    if comparable && manifest_drift {
        invalid_reasons
            .push("artifact manifest drift detected between compared persisted chains".to_string());
    }
    invalid_reasons.dedup();
    let gate_clean = invalid_reasons.is_empty();

    let (verdict, reason) = if gate_clean {
        if verify_mode {
            (
                TinyLivePackageRegressionGateVerdict::TinyLivePackageRegressionGateVerifyOk,
                "the regression gate is clean: the subject chain and the immediately previous comparable top chain are valid, top-layer complete, and drift-free".to_string(),
            )
        } else {
            (
                TinyLivePackageRegressionGateVerdict::TinyLivePackageRegressionGateAvailable,
                "the regression gate is clean: the subject chain and the immediately previous comparable top chain are valid, top-layer complete, and drift-free".to_string(),
            )
        }
    } else if verify_mode {
    ...
    PackageRegressionGateReport {
        ...
        verdict,
        reason,
        subject_chain: Some(subject_summary),
        previous_chain: Some(previous_summary),
        subject_chain_valid,
        previous_chain_available: true,
        previous_chain_valid,
        subject_chain_at_top_accepted_layer,
        previous_chain_at_top_accepted_layer,
        comparable,
        top_identity_drift,
        lineage_drift,
        manifest_drift,
...
fn compare_top_identity(left: &ComparisonSide, right: &ComparisonSide) -> Vec<DriftEntry> {
...
fn compare_lineage(left: &ComparisonSide, right: &ComparisonSide) -> Vec<DriftEntry> {
    let mut entries = Vec::new();
    push_diff_if_mismatch(
        &mut entries,
        "lineage",
    ...
    for index in 0..max_len {
        ...
        if left_entry.is_none() || right_entry.is_none() {
            entries.push(DriftEntry {
                ...
                subject_value: left_entry.map(render_lineage_entry),
                previous_value: right_entry.map(render_lineage_entry),
...
fn compare_manifest(left: &ComparisonSide, right: &ComparisonSide) -> Vec<DriftEntry> {
    let mut entries = Vec::new();
    push_diff_if_mismatch(
        &mut entries,
        "manifest",
    ...
    for key in keys {
        ...
        if left_entry.is_none() || right_entry.is_none() {
            entries.push(DriftEntry {
                ...
                key,
                subject_value: left_entry.map(|entry| render_manifest_entry(entry)),
                previous_value: right_entry.map(|entry| render_manifest_entry(entry)),
...
fn push_diff_if_mismatch(
    ...
    if subject_value == previous_value {
        return;
    }
    entries.push(DriftEntry {
        category: category.to_string(),
        key: key.to_string(),
        subject_value: subject_value.map(ToOwned::to_owned),
        previous_value: previous_value.map(ToOwned::to_owned),
...
fn summarize_side(side: &ComparisonSide) -> DriftChainSummary {
    DriftChainSummary {
...
fn inspect_manifest_artifact(artifact_key: String, path: &Path) -> ManifestArtifactEntry {
    ...
    let mut size_bytes = fs::metadata(path).ok().map(|metadata| metadata.len());
    ...
    if !exists {
        ...
        match fs::read(path) {
            Ok(bytes) => match serde_json::from_slice::<Value>(&bytes) {
                Ok(value) => {
                    parseable = true;
                    let canonical = canonical_manifest_bytes(&canonicalize_manifest_value(value));
                    size_bytes = Some(canonical.len() as u64);
                    sha256_digest = Some(sha256_hex(&canonical));
                }
                Err(_) => {
...
fn canonicalize_manifest_value(value: Value) -> Value {
    match value {
            ...
            let filtered = map
                ...
                .filter(|(key, _)| {
                    key != "planned_at"
                        && key != "updated_at"
                        && key != "session_dir"
                        && !key.ends_with("_session_dir")
                })
                .map(|(key, value)| (key, canonicalize_manifest_value(value)))
                .collect();
        ...
        Value::Array(values) => Value::Array(
            values
                .into_iter()
                .map(canonicalize_manifest_value)
                .collect::<Vec<_>>(),
        ),
        other => other,
    }
...
fn canonical_manifest_bytes(value: &Value) -> Vec<u8> {
    serde_json::to_vec(value).expect("canonical manifest json serialization")
}
...
fn select_previous_comparable_top_chain_candidate<'a>(
    sessions: &'a [PackageSessionArtifact],
    current: &PackageSessionArtifact,
) -> Option<&'a PackageSessionArtifact> {
    let mut comparable = sessions
        .iter()
        .filter(|candidate| candidate.package_step_name == current.package_step_name)
        .collect::<Vec<_>>();
    ...
    comparable
        .into_iter()
        .skip_while(|candidate| candidate.session_dir != current.session_dir)
        .nth(1)
}
...
fn render_regression_gate_human(report: &PackageRegressionGateReport) -> String {
    let mut lines = vec![
        "event=copybot_tiny_live_activation_package_regression_gate".to_string(),
        format!("mode={}", report.mode),
        format!("verdict={}", serialize_enum(&report.verdict)),
        format!("reason={}", report.reason),
        format!("subject_chain_valid={}", report.subject_chain_valid),
        format!(
            "previous_chain_available={}",
            report.previous_chain_available
        ),
        format!("previous_chain_valid={}", report.previous_chain_valid),
        format!(
            "subject_chain_at_top_accepted_layer={}",
            report.subject_chain_at_top_accepted_layer
        ),
        format!(
            "previous_chain_at_top_accepted_layer={}",
            report.previous_chain_at_top_accepted_layer
        ),
        format!("comparable={}", report.comparable),
        format!("top_identity_drift={}", report.top_identity_drift),
        format!("lineage_drift={}", report.lineage_drift),
        format!("manifest_drift={}", report.manifest_drift),
        format!("gate_clean={}", report.gate_clean),
        format!(
            "byte_identical_at_bounded_artifact_level={}",
    ...
    if let Some(subject) = &report.subject_chain {
        lines.push(format!("subject.source={}", subject.source));
        lines.push(format!("subject.top_step_name={}", subject.top_step_name));
        lines.push(format!(
            "subject.top_session_path={}",
            subject.top_session_path
        ));
    }
    if let Some(previous) = &report.previous_chain {
        lines.push(format!("previous.source={}", previous.source));
        lines.push(format!("previous.top_step_name={}", previous.top_step_name));
        lines.push(format!(
            "previous.top_session_path={}",
            previous.top_session_path
        ));
    }
...
mod tests {
    ...
    #[test]
    fn latest_gate_returns_a_clean_gate_when_latest_and_previous_comparable_chains_are_valid_and_drift_free(
    ) {
        let mut fixture = DriftFixture::new("tiny_live_package_regression_gate_clean");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);

        let report = build_latest_gate_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::LatestGate,
        })
        .expect("latest-gate report");

        assert_eq!(
            report.verdict,
            TinyLivePackageRegressionGateVerdict::TinyLivePackageRegressionGateAvailable
        );
        assert!(report.subject_chain_valid);
        assert!(report.previous_chain_available);
        assert!(report.previous_chain_valid);
        assert!(report.subject_chain_at_top_accepted_layer);
        assert!(report.previous_chain_at_top_accepted_layer);
        assert!(report.comparable);
        assert!(!report.top_identity_drift);
        assert!(!report.lineage_drift);
    ...
    #[test]
    fn latest_gate_reports_failure_when_previous_comparable_chain_is_missing() {
        let mut fixture = DriftFixture::new("tiny_live_package_regression_gate_no_previous");
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);
        ...
        assert!(report
            .reason
            .contains("no previous comparable persisted top chain"));
    }

    #[test]
    fn latest_gate_reports_failure_when_latest_chain_is_incomplete_or_invalid() {
        let mut fixture = DriftFixture::new("tiny_live_package_regression_gate_latest_invalid");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);

        fs::remove_file(
            fixture
                .step_dir("latest", "decision_packet")
                .join("tiny_live_activation_package_decision_packet.status.json"),
        )
        .expect("remove latest invalid status");
        ...
        assert!(report.previous_chain_valid);
        assert!(!report.gate_clean);
        assert!(report.reason.contains("missing status artifact"));
    }

    #[test]
    fn latest_gate_reports_failure_when_latest_vs_previous_top_identity_drifts() {
        let mut fixture = DriftFixture::new("tiny_live_package_regression_gate_top_identity");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);

        set_json_string(
            fixture
                .step_dir("latest", TOP_ACCEPTED_PACKAGE_STEP)
    ...
    #[test]
    fn latest_gate_reports_failure_when_latest_vs_previous_lineage_drifts() {
        let mut fixture = DriftFixture::new("tiny_live_package_regression_gate_lineage");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);
        ...
        set_json_string(
            fixture
                .step_dir("latest", "execute_frozen")
    ...
    #[test]
    fn latest_gate_reports_failure_when_latest_vs_previous_manifest_drifts() {
        let mut fixture = DriftFixture::new("tiny_live_package_regression_gate_manifest");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);
    ...
    #[test]
    fn verify_gate_fails_when_either_side_is_below_clerestory_certificate() {
        let mut fixture = DriftFixture::new("tiny_live_package_regression_gate_below_clerestory");
        fixture.add_chain("previous", "choir_receipt", 0);
        fixture.add_chain("latest", "choir_receipt", 180);

        let report = build_verify_gate_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::VerifyGate,
        })
        .expect("verify-gate report");
    ...
    fn all_modes_stay_planning_safe_and_do_not_imply_activation_authorization() {
        let mut fixture = DriftFixture::new("tiny_live_package_regression_gate_planning_safe");
        fixture.add_chain("previous", TOP_ACCEPTED_PACKAGE_STEP, 0);
        fixture.add_chain("latest", TOP_ACCEPTED_PACKAGE_STEP, 180);

        let latest = build_latest_gate_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::LatestGate,
        ...
        .expect("latest-gate report");
        let session = build_session_gate_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: Some(
                fixture
                    .step_dir("latest", TOP_ACCEPTED_PACKAGE_STEP)
            ...
            json: true,
            mode: Mode::SessionGate,
        })
        .expect("session-gate report");
        let verify = build_verify_gate_report(&Config {
            root: Some(fixture.root.clone()),
            session_dir: None,
            json: true,
            mode: Mode::VerifyGate,
    ...
    struct DriftFixture {
        root: PathBuf,
        chains: BTreeMap<String, BTreeMap<String, PathBuf>>,
    }
    ...
    impl DriftFixture {
        ...
        fn add_chain(&mut self, chain_label: &str, top_step: &str, base_minute: usize) {
            let chain_root = self.root.join(chain_label);
            ...
            for index in 0..top_rank {
                ...
                let step_dir = chain_root.join(format!("{:02}-{step_name}", index + 1));
                ...
                write_step_session(
                    &step_dir,
                    step_name,
                    &timestamp(base_minute + index, 0),
                    &timestamp(base_minute + index, 30),
                    &identity,
                    lower.as_ref(),
                );
                lower = Some(identity);
            ...
            self.chains.insert(chain_label.to_string(), step_dirs);
        }

        fn step_dir(&self, chain_label: &str, step_name: &str) -> &Path {
            self.chains
                .get(chain_label)
                .and_then(|steps| steps.get(step_name))
                .map(PathBuf::as_path)
                .unwrap_or_else(|| panic!("missing fixture step dir for {chain_label}/{step_name}"))
        }
    }
        ...
        if let Some(lower) = lower {
            ...
            fs::write(
                ...
                serde_json::to_vec_pretty(&json!({
                    ...
                    "reason": "nested drift continuity ok"
                }))
                .unwrap(),
            )
            .unwrap();
    ...
    fn timestamp(total_minutes: usize, seconds: u8) -> String {
        let hours = total_minutes / 60;
        let minutes = total_minutes % 60;
        format!("2026-04-11T{:02}:{:02}:{:02}Z", hours, minutes, seconds)
    }
