...
use copybot_app::tiny_live_activation::package_dais_receipt_rostrum_certificate;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_rostrum_certificate --dais-receipt-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-rostrum-certificate | --render-live-package-rostrum-certificate | --run-live-package-rostrum-certificate | --verify-live-package-rostrum-certificate)";
const ROSTRUM_CERTIFICATE_SESSION_VERSION: &str = "1";
const ROSTRUM_CERTIFICATE_STATUS_VERSION: &str = "1";
const ROSTRUM_CERTIFICATE_ALGORITHM: &str = "sha256";
const ROSTRUM_CERTIFICATE_SESSION_EXPLICIT_STATEMENT: &str =
...
const ROSTRUM_CERTIFICATE_STATUS_EXPLICIT_STATEMENT: &str =
    "this rostrum certificate is archival and read-only; it never executes or enables the frozen controller";
const ROSTRUM_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    dais_receipt_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackageRostrumCertificate,
    RenderLivePackageRostrumCertificate,
    RunLivePackageRostrumCertificate,
    VerifyLivePackageRostrumCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageRostrumCertificateVerdict {
    TinyLivePackageRostrumCertificatePlanReady,
    TinyLivePackageRostrumCertificateRendered,
    TinyLivePackageRostrumCertificateRefusedNowByStage3,
    TinyLivePackageRostrumCertificateRefusedNowByPreActivationGate,
    TinyLivePackageRostrumCertificateRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageRostrumCertificateReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageRostrumCertificateVerifyOk,
    TinyLivePackageRostrumCertificateVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageRostrumCertificateResult {
    RefusedNowByStage3,
...
struct PackageRostrumCertificateStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageRostrumCertificateSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageRostrumCertificateStatus {
    ...
    bedrock_certificate_session_dir: String,
    foundation_receipt_session_dir: String,
    result: LivePackageRostrumCertificateResult,
    reason: String,
    ...
    dais_receipt_step: Option<PackageRostrumCertificateStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageRostrumCertificatePaths {
...
struct PackageRostrumCertificateReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageRostrumCertificateVerdict,
    reason: String,
    dais_receipt_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut dais_receipt_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--dais-receipt-session-dir" => {
                dais_receipt_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--dais-receipt-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-rostrum-certificate" => set_mode(
                &mut mode,
                Mode::PlanLivePackageRostrumCertificate,
                arg.as_str(),
            )?,
            "--render-live-package-rostrum-certificate" => set_mode(
                &mut mode,
                Mode::RenderLivePackageRostrumCertificate,
                arg.as_str(),
            )?,
            "--run-live-package-rostrum-certificate" => set_mode(
                &mut mode,
                Mode::RunLivePackageRostrumCertificate,
                arg.as_str(),
            )?,
            "--verify-live-package-rostrum-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLivePackageRostrumCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackageRostrumCertificate) && output_path.is_none() {
        bail!("missing --output for --render-live-package-rostrum-certificate");
    }
    if matches!(
        mode,
        Mode::RunLivePackageRostrumCertificate | Mode::VerifyLivePackageRostrumCertificate
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageRostrumCertificate => {
            plan_live_package_rostrum_certificate_report(&config)?
        }
        Mode::RenderLivePackageRostrumCertificate => {
            render_live_package_rostrum_certificate_report(&config)?
        }
        Mode::RunLivePackageRostrumCertificate => {
            run_live_package_rostrum_certificate_report(&config)?
        }
        Mode::VerifyLivePackageRostrumCertificate => {
            verify_live_package_rostrum_certificate_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_rostrum_certificate_report(
    config: &Config,
) -> Result<PackageRostrumCertificateReport> {
    let (basal_receipt, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_dais_receipt_rostrum_certificate::verify_live_package_dais_receipt_for_rostrum_certificate(
                    &config.dais_receipt_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification =
                            classify_verified_dais_receipt(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_dais_receipt_rostrum_certificate::load_live_package_dais_receipt_contract_for_rostrum_certificate(
                            &config.dais_receipt_session_dir,
                        )?;
                        let classification =
                            classify_dais_receipt_contract(&raw_contract, false);
    ...
    Ok(PackageRostrumCertificateReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_rostrum_certificate".to_string(),
        verdict:
            TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificatePlanReady,
        reason: format!(
            "dais-receipt-native rostrum certificate is explicit for verified dais-receipt session {}; this command stays archival and read-only while freezing one final escutcheon identity over the fully culminated chain",
            config.dais_receipt_session_dir.display()
        ),
        ...
        rostrum_certificate_session_path: None,
        rostrum_certificate_status_path: None,
        archived_dais_receipt_report_path: None,
        result: Some(result),
        ...
        rostrum_certificate_summary: Some(rostrum_certificate_summary(
            classification.0,
            &basal_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        rostrum_certificate_sha256: Some(rostrum_certificate_sha256),
        rostrum_certificate_algorithm: Some(ROSTRUM_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: ROSTRUM_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_rostrum_certificate_report(
    config: &Config,
) -> Result<PackageRostrumCertificateReport> {
    let plan = plan_live_package_rostrum_certificate_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_rostrum_certificate --dais-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-rostrum-certificate --json\ncopybot_tiny_live_activation_package_rostrum_certificate --dais-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-rostrum-certificate --json\n",
        shell_escape_path(&config.dais_receipt_session_dir),
    ...
    Ok(PackageRostrumCertificateReport {
        generated_at: Utc::now(),
        mode: "render_live_package_rostrum_certificate".to_string(),
        verdict:
            TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateRendered,
        reason: format!(
            "rendered rostrum-certificate run/verify script to {}; the primary input remains the immutable dais-receipt session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: ROSTRUM_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_rostrum_certificate_report(
    config: &Config,
) -> Result<PackageRostrumCertificateReport> {
    ...
    let verified_dais_receipt = match package_dais_receipt_rostrum_certificate::verify_live_package_dais_receipt_for_rostrum_certificate(
        &config.dais_receipt_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify dais-receipt session under {}: {error}",
                    config.dais_receipt_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_dais_receipt.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "rostrum-certificate session dir {} overlaps the managed live target surface derived from verified dais-receipt install_root {}",
            session_dir.display(),
            verified_dais_receipt.contract.install_root.display()
        )
    })?;
    ensure_clean_rostrum_certificate_session_dir(session_dir)?;
    let paths = rostrum_certificate_paths(session_dir);
    let classification = classify_verified_dais_receipt(&verified_dais_receipt);
    ...
    let rostrum_certificate_sha256 = compute_rostrum_certificate_sha256(
        ...
        &verified_dais_receipt
            .contract
            .reviewed_frozen_live_cutover_controller_command_summary,
        &verified_dais_receipt.contract.chain_fingerprint_sha256,
        &verified_dais_receipt.contract.chain_fingerprint_algorithm,
        &verified_dais_receipt
            .contract
        ...
        &verified_dais_receipt.contract.ledger_seal_sha256,
        &verified_dais_receipt.contract.ledger_seal_algorithm,
        ...
        &verified_dais_receipt.contract.keystone_receipt_summary,
        &verified_dais_receipt.contract.keystone_receipt_sha256,
        &verified_dais_receipt.contract.keystone_receipt_algorithm,
        &verified_dais_receipt
            .contract
        ...
        &verified_dais_receipt.contract.plinth_receipt_summary,
        &verified_dais_receipt.contract.plinth_receipt_sha256,
    ...
    let session = planned_session(
        config,
        session_dir,
        &verified_dais_receipt.contract,
        result,
    ...
    persist_json(
        ...
        &verified_dais_receipt.report_json,
    )?;
    let dais_receipt_step = Some(step_artifact(
        &paths.dais_receipt_report_path,
        "verify_live_package_dais_receipt",
        &verified_dais_receipt.verdict,
        &verified_dais_receipt.reason,
        verified_dais_receipt.generated_at,
    ));
    let status = PackageRostrumCertificateStatus {
        status_version: ROSTRUM_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        dais_receipt_session_dir: config.dais_receipt_session_dir.display().to_string(),
        pedestal_certificate_session_dir: verified_dais_receipt
            .contract
            .pedestal_certificate_session_dir
            .display()
            .to_string(),
        plinth_receipt_session_dir: verified_dais_receipt
            .contract
            .plinth_receipt_session_dir
            .display()
            .to_string(),
        substructure_certificate_session_dir: verified_dais_receipt
            .contract
            .substructure_certificate_session_dir
            .display()
            .to_string(),
        basal_receipt_session_dir: verified_dais_receipt
            .contract
            .basal_receipt_session_dir
            .display()
            .to_string(),
        bedrock_certificate_session_dir: verified_dais_receipt
            .contract
            .bedrock_certificate_session_dir
            .display()
            .to_string(),
        foundation_receipt_session_dir: verified_dais_receipt
            .contract
            .foundation_receipt_session_dir
            .display()
            .to_string(),
        ...
        current_pre_activation_gate_verdict: verified_dais_receipt
            ...
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_dais_receipt
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        dais_receipt_step,
        provenance_certificate_summary: verified_dais_receipt
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_dais_receipt
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_dais_receipt
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_dais_receipt
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_dais_receipt
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_dais_receipt
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_dais_receipt
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_dais_receipt
            .contract
        ...
        registry_entry_algorithm: verified_dais_receipt
            ...
            .clone(),
        filing_certificate_summary: verified_dais_receipt
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_dais_receipt
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_dais_receipt
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_dais_receipt
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_dais_receipt
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_dais_receipt
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_dais_receipt
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_dais_receipt
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_dais_receipt
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_dais_receipt
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_dais_receipt
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_dais_receipt
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_dais_receipt
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_dais_receipt
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_dais_receipt
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_dais_receipt
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_dais_receipt
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_dais_receipt
            .contract
            .completion_certificate_algorithm
            .clone(),
        summit_certificate_summary: verified_dais_receipt
            .contract
            .summit_certificate_summary
            .clone(),
        summit_certificate_sha256: verified_dais_receipt
            .contract
            .summit_certificate_sha256
            .clone(),
        summit_certificate_algorithm: verified_dais_receipt
            .contract
            .summit_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_dais_receipt
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_dais_receipt
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_dais_receipt
            .contract
            .culmination_receipt_algorithm
            .clone(),
        pinnacle_receipt_summary: verified_dais_receipt
            .contract
            .pinnacle_receipt_summary
            .clone(),
        pinnacle_receipt_sha256: verified_dais_receipt
            .contract
            .pinnacle_receipt_sha256
            .clone(),
        pinnacle_receipt_algorithm: verified_dais_receipt
            .contract
            .pinnacle_receipt_algorithm
            .clone(),
        capstone_certificate_summary: verified_dais_receipt
            .contract
            .capstone_certificate_summary
            .clone(),
        capstone_certificate_sha256: verified_dais_receipt
            .contract
            .capstone_certificate_sha256
            .clone(),
        capstone_certificate_algorithm: verified_dais_receipt
            .contract
            .capstone_certificate_algorithm
            .clone(),
        keystone_receipt_summary: verified_dais_receipt
            .contract
            .keystone_receipt_summary
            .clone(),
        keystone_receipt_sha256: verified_dais_receipt
            .contract
            .keystone_receipt_sha256
            .clone(),
        keystone_receipt_algorithm: verified_dais_receipt
            .contract
            .keystone_receipt_algorithm
            .clone(),
        cornerstone_certificate_summary: verified_dais_receipt
            .contract
            .cornerstone_certificate_summary
            .clone(),
        cornerstone_certificate_sha256: verified_dais_receipt
            .contract
            .cornerstone_certificate_sha256
            .clone(),
        cornerstone_certificate_algorithm: verified_dais_receipt
            .contract
            .cornerstone_certificate_algorithm
            .clone(),
        foundation_receipt_summary: verified_dais_receipt
            .contract
            .foundation_receipt_summary
            .clone(),
        foundation_receipt_sha256: verified_dais_receipt
            .contract
            .foundation_receipt_sha256
            .clone(),
        foundation_receipt_algorithm: verified_dais_receipt
            .contract
            .foundation_receipt_algorithm
            .clone(),
        bedrock_certificate_summary: verified_dais_receipt
            .contract
            .bedrock_certificate_summary
            .clone(),
        bedrock_certificate_sha256: verified_dais_receipt
            .contract
            .bedrock_certificate_sha256
            .clone(),
        bedrock_certificate_algorithm: verified_dais_receipt
            .contract
            .bedrock_certificate_algorithm
            .clone(),
        ...
        basal_receipt_algorithm: verified_dais_receipt
            ...
            .clone(),
        substructure_certificate_summary: verified_dais_receipt
            .contract
            .substructure_certificate_summary
            .clone(),
        substructure_certificate_sha256: verified_dais_receipt
            .contract
            .substructure_certificate_sha256
            .clone(),
        substructure_certificate_algorithm: verified_dais_receipt
            .contract
            .substructure_certificate_algorithm
            .clone(),
        plinth_receipt_summary: verified_dais_receipt
            .contract
        ...
        plinth_receipt_algorithm: verified_dais_receipt
            ...
            .clone(),
        pedestal_certificate_summary: verified_dais_receipt
            .contract
            .pedestal_certificate_summary
            .clone(),
        pedestal_certificate_sha256: verified_dais_receipt
            .contract
            .pedestal_certificate_sha256
            .clone(),
        pedestal_certificate_algorithm: verified_dais_receipt
            .contract
        ...
        dais_receipt_algorithm: verified_dais_receipt
            ...
            .clone(),
        rostrum_certificate_summary: rostrum_certificate_summary(
            result,
            &verified_dais_receipt
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        rostrum_certificate_sha256: rostrum_certificate_sha256.clone(),
        rostrum_certificate_algorithm: ROSTRUM_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: ROSTRUM_CERTIFICATE_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_rostrum_certificate_report(
    config: &Config,
) -> Result<PackageRostrumCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = rostrum_certificate_paths(session_dir);
    let session: PackageRostrumCertificateSession = load_json(&paths.session_path)?;
    let status: PackageRostrumCertificateStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.dais_receipt_session_dir,
        &config.dais_receipt_session_dir.display().to_string(),
        "rostrum-certificate dais_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
    ...
    compare_string(
        &status.dais_receipt_session_dir,
        &config.dais_receipt_session_dir.display().to_string(),
        "rostrum-certificate status dais_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_substructure_certificate_report_view: NestedSubstructureCertificateReportView =
        serde_json::from_value(stored_basal_receipt_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested dais-receipt report {}",
                paths.dais_receipt_report_path.display()
            )
        })?;
    let verified_substructure_certificate_report_view: NestedSubstructureCertificateReportView =
        serde_json::from_value(verified_dais_receipt.report_json.clone()).with_context(|| {
            format!(
                "failed parsing freshly verified nested dais-receipt report for {}",
                config.dais_receipt_session_dir.display()
            )
        })?;
        ...
        != verified_substructure_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "rostrum-certificate archived dais-receipt report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_substructure_certificate_report_view.generated_at,
            verified_substructure_certificate_report_view.generated_at
        ));
    }
    let expected_dais_receipt_step = step_artifact(
        &paths.dais_receipt_report_path,
        &verified_substructure_certificate_report_view.mode,
        &verified_substructure_certificate_report_view.verdict,
        &verified_substructure_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_dais_receipt_step,
        "rostrum-certificate dais_receipt_step",
        &mut mismatches,
    );
    if verified_dais_receipt.verdict != "tiny_live_package_dais_receipt_verify_ok" {
        mismatches.push(format!(
            "rostrum-certificate nested dais-receipt verification is non-green: {}",
            verified_dais_receipt.reason
        ));
    }

    let classification = classify_verified_dais_receipt(&verified_dais_receipt);
    let expected_rostrum_certificate_sha256 = compute_rostrum_certificate_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_dais_receipt.contract,
        classification.0,
        &expected_rostrum_certificate_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "rostrum-certificate session contract does not match freshly verified dais-receipt truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_dais_receipt.contract,
        classification,
        expected_dais_receipt_step.clone(),
        &expected_rostrum_certificate_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "rostrum-certificate status contract does not match freshly verified dais-receipt truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_dais_receipt.contract,
        TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateVerifyOk,
        &expected_status,
    ))
}

fn verify_dais_receipt_command_summary(dais_receipt_session_dir: &Path) -> String {
    format!(
...
fn classify_dais_receipt_contract(
    contract: &package_dais_receipt_rostrum_certificate::LivePackageDaisReceiptContractView,
    assume_verified: bool,
) -> (
    LivePackageRostrumCertificateResult,
    TinyLivePackageRostrumCertificateVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageRostrumCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateRefusedNowByInvalidOrDriftedContract,
            "the dais-receipt chain is not verified, so this rostrum certificate remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageRostrumCertificateResult::RefusedNowByStage3,
            TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateRefusedNowByStage3,
            "the immutable dais receipt remains refused now by Stage 3, so this rostrum certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageRostrumCertificateResult::RefusedNowByPreActivationGate,
            TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateRefusedNowByPreActivationGate,
            "the immutable dais receipt remains refused now by the pre-activation gate, so this rostrum certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageRostrumCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable dais receipt is coherent and this rostrum certificate is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageRostrumCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateRefusedNowByInvalidOrDriftedContract,
            "the verified dais-receipt chain is invalid, drifted, or non-runnable, so this rostrum certificate remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_dais_receipt(
    verified: &package_dais_receipt_rostrum_certificate::VerifiedLivePackageDaisReceiptRostrumCertificateStep,
) -> (
    LivePackageRostrumCertificateResult,
    TinyLivePackageRostrumCertificateVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_dais_receipt_verify_ok" {
        return (
            LivePackageRostrumCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateRefusedNowByInvalidOrDriftedContract,
            "the immutable dais receipt did not verify cleanly, so this rostrum certificate remains refused"
                .to_string(),
        );
    }
    classify_dais_receipt_contract(&verified.contract, true)
}

fn rostrum_certificate_summary(
    result: LivePackageRostrumCertificateResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageRostrumCertificateResult::RefusedNowByStage3 => {
            "Pedestal certificate outcome: the immutable culminated chain remains refused now by Stage 3; archive this final escutcheon seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageRostrumCertificateResult::RefusedNowByPreActivationGate => {
            "Pedestal certificate outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final escutcheon seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageRostrumCertificateResult::RefusedNowByInvalidOrDriftedContract => {
            "Pedestal certificate outcome: the immutable culminated chain is invalid or drifted; archive this final escutcheon seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackageRostrumCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Pedestal certificate outcome: this immutable escutcheon seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn rostrum_certificate_paths(session_dir: &Path) -> PackageRostrumCertificatePaths {
    PackageRostrumCertificatePaths {
        ...
        dais_receipt_report_path: session_dir
            .join("tiny_live_activation_package_rostrum_certificate.dais_receipt.report.json"),
    }
}

fn ensure_clean_rostrum_certificate_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing rostrum-certificate session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating rostrum-certificate session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageRostrumCertificateSession {
    PackageRostrumCertificateSession {
        ...
        session_dir: session_dir.display().to_string(),
        verify_dais_receipt_command_summary: verify_dais_receipt_command_summary(
            &config.dais_receipt_session_dir,
        ),
        ...
        rostrum_certificate_sha256: rostrum_certificate_sha256.to_string(),
        rostrum_certificate_algorithm: ROSTRUM_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: ROSTRUM_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    dais_receipt_step: PackageRostrumCertificateStepArtifact,
    rostrum_certificate_sha256: &str,
) -> PackageRostrumCertificateStatus {
    PackageRostrumCertificateStatus {
        status_version: ROSTRUM_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        dais_receipt_step: Some(dais_receipt_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageRostrumCertificatePaths,
    contract: &package_dais_receipt_rostrum_certificate::LivePackageDaisReceiptContractView,
    verdict: TinyLivePackageRostrumCertificateVerdict,
    status: &PackageRostrumCertificateStatus,
    ...
    PackageRostrumCertificateReport {
        ...
        mode: if verdict
            == TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateVerifyOk
        {
            "verify_live_package_rostrum_certificate".to_string()
        } else {
            "run_live_package_rostrum_certificate".to_string()
        },
        verdict,
        ...
        dais_receipt_step: status.dais_receipt_step.clone(),
        verify_dais_receipt_command_summary: Some(verify_dais_receipt_command_summary(
            &config.dais_receipt_session_dir,
        )),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
        ...
        explicit_statement: if verdict
            == TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateVerifyOk
        {
            ROSTRUM_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageRostrumCertificateReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageRostrumCertificate => {
                "plan_live_package_rostrum_certificate".to_string()
            }
            Mode::RenderLivePackageRostrumCertificate => {
                "render_live_package_rostrum_certificate".to_string()
            }
            Mode::RunLivePackageRostrumCertificate => {
                "run_live_package_rostrum_certificate".to_string()
            }
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable rostrum certificate refused before persisting because the dais-receipt artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "dais-receipt-native rostrum-certificate verification failed".to_string()
    });
    PackageRostrumCertificateReport {
        generated_at: Utc::now(),
        ...
        explicit_statement: ROSTRUM_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageRostrumCertificateStepArtifact>,
    expected_path: &Path,
    label: &str,
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageRostrumCertificateStepArtifact {
    PackageRostrumCertificateStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageRostrumCertificateStepArtifact>,
    expected: &PackageRostrumCertificateStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_rostrum_certificate_sha256(
    result: &str,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing rostrum-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackageRostrumCertificateReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.rostrum_certificate_sha256 {
        lines.push(format!("rostrum_certificate_sha256={value}"));
    }
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_rostrum_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify =
            verify_live_package_rostrum_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageRostrumCertificateVerdict::TinyLivePackageRostrumCertificateVerifyOk
        );
    ...
    #[test]
    fn drifted_dais_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            "dais_receipt_drifted_contract",
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_dais_receipt",
                "verdict": "tiny_live_package_dais_receipt_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        let fixture = fake_command_fixture(
            "rostrum_certificate_managed_overlap",
        ...
        let error = run_live_package_rostrum_certificate_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = rostrum_certificate_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_dais_receipt_step_path() {
        let fixture = fake_command_fixture(
            "dais_receipt_tampered_step_path",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_rostrum_certificate_report(&fixture.run_config()).unwrap();
        let paths = rostrum_certificate_paths(&fixture.rostrum_certificate_session_dir);
        let mut status: PackageRostrumCertificateStatus = load_json(&paths.status_path).unwrap();
        status.dais_receipt_step.as_mut().unwrap().path =
            "/tmp/foreign.dais-receipt.report.json".to_string();
    ...
    #[test]
    fn verify_rejects_tampered_dais_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_dais_receipt_sha256",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_dais_receipt_summary",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut session: PackageRostrumCertificateSession =
                load_json(&paths.session_path).unwrap();
            session.dais_receipt_summary = "tampered dais receipt summary".to_string();
            persist_json(&paths.session_path, &session).unwrap();
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_dais_receipt_algorithm",
                bedrock_certificate_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut session: PackageRostrumCertificateSession =
                load_json(&paths.session_path).unwrap();
            session.dais_receipt_algorithm = "tampered-dais-receipt-algorithm".to_string();
            persist_json(&paths.session_path, &session).unwrap();
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = rostrum_certificate_paths(&fixture.rostrum_certificate_session_dir);
        let mut status: PackageRostrumCertificateStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageRostrumCertificateResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    #[test]
    fn verify_rejects_tampered_nested_dais_receipt_report_content() {
        let fixture = fake_command_fixture(
            "dais_receipt_tampered_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        run_live_package_rostrum_certificate_report(&fixture.run_config()).unwrap();
        let paths = rostrum_certificate_paths(&fixture.rostrum_certificate_session_dir);
        let mut report: serde_json::Value = load_json(&paths.dais_receipt_report_path).unwrap();
        report.as_object_mut().unwrap().insert(
            "reason".to_string(),
            json!("tampered nested dais-receipt reason"),
        );
    ...
    #[test]
    fn verify_rejects_retimed_nested_dais_receipt_report_and_step_generated_at() {
        let fixture = fake_command_fixture(
            "dais_receipt_retimed_nested_report",
            bedrock_certificate_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
        ...
        let mut status: PackageRostrumCertificateStatus = load_json(&paths.status_path).unwrap();
        status.dais_receipt_step.as_mut().unwrap().generated_at = forged_generated_at;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    fn verify_rejects_tampered_chain_ledger_registry_filing_archive_summit_closure_finality_or_consummation_identity_fields(
    ) {
        {
            ...
            run_live_package_rostrum_certificate_report(&fixture.run_config()).unwrap();
            let paths = rostrum_certificate_paths(&fixture.rostrum_certificate_session_dir);
            let mut status: PackageRostrumCertificateStatus =
    ...
    fn verify_rejects_tampered_bedrock_capstone_keystone_cornerstone_or_foundation_identity_fields()
    {
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_bedrock_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_capstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_keystone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_cornerstone_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_foundation_identity",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_basal_receipt_sha256() {
        let fixture = fake_command_fixture(
            "rostrum_certificate_tampered_basal_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_sha256() {
        let fixture = fake_command_fixture(
            "rostrum_certificate_tampered_substructure_sha256",
            consummation_record_verify_report(
                "ready_for_manual_execution_when_gate_turns_green",
                "ready_for_manual_go_live_review",
                "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_substructure_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_substructure_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_substructure_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_plinth_receipt_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_plinth_receipt_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_plinth_receipt_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_plinth_receipt_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_pedestal_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_pedestal_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_pedestal_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_pedestal_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_bedrock_certificate_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_bedrock_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_bedrock_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
    ...
    fn verify_rejects_tampered_rostrum_certificate_sha256_summary_or_algorithm() {
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_rostrum_sha256",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageRostrumCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.rostrum_certificate_sha256 = "tampered-rostrum".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_rostrum_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_rostrum_summary",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageRostrumCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.rostrum_certificate_summary = "tampered rostrum summary".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_rostrum_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
        ...
        {
            let fixture = fake_command_fixture(
                "rostrum_certificate_tampered_rostrum_algorithm",
                consummation_record_verify_report(
                    "ready_for_manual_execution_when_gate_turns_green",
                    "ready_for_manual_go_live_review",
                    "runnable_when_gate_truth_turns_green",
            ...
            let mut status: PackageRostrumCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.rostrum_certificate_algorithm = "tampered-rostrum-algorithm".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_rostrum_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
    ...
    struct FakeCommandFixture {
        ...
        input_dais_receipt_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageRostrumCertificate,
                dais_receipt_session_dir: self.input_dais_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.rostrum_certificate_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageRostrumCertificate,
                dais_receipt_session_dir: self.input_dais_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let input_dais_receipt_session_dir = fixture_dir.join("dais-receipt-session");
        ...
        persist_json(
            ...
            &stored_dais_receipt_session_fixture(
                &input_dais_receipt_session_dir,
                &pedestal_certificate_session_dir,
                &plinth_receipt_session_dir,
                &substructure_certificate_session_dir,
        ...
        persist_json(
            ...
            &stored_dais_receipt_status_fixture(
                &pedestal_certificate_session_dir,
                &plinth_receipt_session_dir,
                &substructure_certificate_session_dir,
                &verify_report,
        ...
        bind_dais_receipt_report_to_fixture(
            &mut verify_report,
            &input_dais_receipt_session_dir,
            &pedestal_certificate_session_dir,
            &plinth_receipt_session_dir,
        ...
        let verify_path = fixture_dir.join("dais-receipt.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_dais_receipt_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_dais_receipt"),
            &verify_path,
            &pedestal_certificate_session_dir,
            &decision_packet_session_dir,
            &input_dais_receipt_session_dir,
    ...
    fn bind_dais_receipt_report_to_fixture(
        ...
        object.insert(
            ...
            json!("dais-receipt verify statement"),
        );
    }

    fn dais_receipt_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_dais_receipt",
            "verdict": "tiny_live_package_dais_receipt_verify_ok",
            "reason": "dais-receipt chain is coherent",
            "result": result,
    ...
    fn stored_dais_receipt_session_fixture(
        dais_receipt_session_dir: &Path,
        pedestal_certificate_session_dir: &Path,
        plinth_receipt_session_dir: &Path,
        substructure_certificate_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("dais-receipt statement"),
        );
        Value::Object(object)
    }

    fn stored_dais_receipt_status_fixture(
        pedestal_certificate_session_dir: &Path,
        plinth_receipt_session_dir: &Path,
        substructure_certificate_session_dir: &Path,
        verify_report: &Value,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("dais-receipt status statement"),
        );
        Value::Object(object)
    }
}
