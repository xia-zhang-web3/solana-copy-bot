...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_dais_receipt_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-dais-receipt | --render-latest-dais-receipt-script | --run-latest-dais-receipt | --verify-latest-dais-receipt)";
const DAIS_RECEIPT_LATEST_SESSION_VERSION: &str = "1";
const DAIS_RECEIPT_LATEST_STATUS_VERSION: &str = "1";
...
const DAIS_RECEIPT_BIN: &str = "copybot_tiny_live_activation_package_dais_receipt";
const PEDESTAL_CERTIFICATE_LATEST_PLAN_READY: &str =
    "tiny_live_package_pedestal_certificate_latest_plan_ready";
const PEDESTAL_CERTIFICATE_LATEST_VERIFY_OK: &str =
    "tiny_live_package_pedestal_certificate_latest_verify_ok";
const DAIS_RECEIPT_VERIFY_OK: &str = "tiny_live_package_dais_receipt_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-dais-receipt handoff binds the latest persisted immutable package chain to the accepted dais-receipt contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestDaisReceipt,
    RenderLatestDaisReceiptScript,
    RunLatestDaisReceipt,
    VerifyLatestDaisReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageDaisReceiptLatestVerdict {
    TinyLivePackageDaisReceiptLatestPlanReady,
    TinyLivePackageDaisReceiptLatestScriptRendered,
    TinyLivePackageDaisReceiptLatestRefusedByMissingLatestChain,
    TinyLivePackageDaisReceiptLatestRefusedByInvalidLatestChain,
    TinyLivePackageDaisReceiptLatestRefusedByDriftedDaisReceiptContract,
    TinyLivePackageDaisReceiptLatestRefusedNowByStage3,
    TinyLivePackageDaisReceiptLatestRefusedNowByPreActivationGate,
    TinyLivePackageDaisReceiptLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageDaisReceiptLatestVerifyOk,
    TinyLivePackageDaisReceiptLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageDaisReceiptLatestResult {
    RefusedByMissingLatestChain,
    RefusedByInvalidLatestChain,
    RefusedByDriftedDaisReceiptContract,
    RefusedNowByStage3,
...
struct PackageDaisReceiptLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageDaisReceiptLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_pedestal_certificate_latest_session_dir: String,
    nested_dais_receipt_session_dir: String,
    ...
    resolve_latest_pedestal_certificate_command_summary: String,
    run_latest_dais_receipt_command_summary: String,
    verify_latest_dais_receipt_command_summary: String,
    downstream_dais_receipt_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageDaisReceiptLatestStatus {
    ...
    nested_dais_receipt_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackageDaisReceiptLatestResult,
    reason: String,
    latest_pedestal_certificate_plan_verdict: Option<String>,
    latest_pedestal_certificate_verdict: Option<String>,
    dais_receipt_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_pedestal_certificate_plan_step: Option<PackageDaisReceiptLatestStepArtifact>,
    latest_pedestal_certificate_step: Option<PackageDaisReceiptLatestStepArtifact>,
    dais_receipt_step: Option<PackageDaisReceiptLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackageDaisReceiptLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_pedestal_certificate_plan_report_path: PathBuf,
    latest_pedestal_certificate_report_path: PathBuf,
    dais_receipt_report_path: PathBuf,
    nested_pedestal_certificate_latest_session_dir: PathBuf,
    nested_dais_receipt_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageDaisReceiptLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageDaisReceiptLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    dais_receipt_latest_session_path: Option<String>,
    dais_receipt_latest_status_path: Option<String>,
    dais_receipt_latest_report_path: Option<String>,
    latest_pedestal_certificate_plan_report_path: Option<String>,
    latest_pedestal_certificate_report_path: Option<String>,
    dais_receipt_report_path: Option<String>,
    nested_pedestal_certificate_latest_session_dir: Option<String>,
    nested_dais_receipt_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_pedestal_certificate_command_summary: Option<String>,
    run_latest_dais_receipt_command_summary: Option<String>,
    verify_latest_dais_receipt_command_summary: Option<String>,
    downstream_dais_receipt_command_summary: Option<String>,
...
#[derive(Debug, Clone)]
struct LatestPedestalCertificateSnapshot {
...
#[derive(Debug, Clone)]
struct ResolvedLatestPedestalCertificate {
    snapshot: LatestPedestalCertificateSnapshot,
    latest_pedestal_certificate_plan_raw: Value,
    latest_pedestal_certificate_plan: SubstructureCertificateLatestReportView,
}
...
enum ResolutionFailureKind {
    MissingLatestChain,
    InvalidLatestChain,
    DriftedPedestalCertificateContract,
}

#[derive(Debug, Clone)]
struct LatestPedestalCertificateResolutionFailure {
    kind: ResolutionFailureKind,
    reason: String,
    snapshot: Option<LatestPedestalCertificateSnapshot>,
    latest_pedestal_certificate_plan_raw: Option<Value>,
    latest_pedestal_certificate_plan_verdict: Option<String>,
    latest_pedestal_certificate_plan_reason: Option<String>,
    latest_pedestal_certificate_plan_generated_at: Option<DateTime<Utc>>,
}
...
struct DaisReceiptReportView {
    ...
    service_status_max_staleness_ms: Option<u64>,
    session_dir: Option<String>,
    dais_receipt_session_path: Option<String>,
    dais_receipt_status_path: Option<String>,
    archived_pedestal_certificate_report_path: Option<String>,
    result: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    pedestal_certificate_step: Option<PackageDaisReceiptLatestStepArtifact>,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-dais-receipt" => {
                set_mode(&mut mode, Mode::PlanLatestDaisReceipt, arg.as_str())?
            }
            "--render-latest-dais-receipt-script" => {
                set_mode(&mut mode, Mode::RenderLatestDaisReceiptScript, arg.as_str())?
            }
            "--run-latest-dais-receipt" => {
                set_mode(&mut mode, Mode::RunLatestDaisReceipt, arg.as_str())?
            }
            "--verify-latest-dais-receipt" => {
                set_mode(&mut mode, Mode::VerifyLatestDaisReceipt, arg.as_str())?
            }
    ...
    if matches!(
        mode,
        Mode::PlanLatestDaisReceipt
            | Mode::RenderLatestDaisReceiptScript
            | Mode::RunLatestDaisReceipt
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestDaisReceiptScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestDaisReceipt | Mode::VerifyLatestDaisReceipt
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestDaisReceipt => plan_latest_dais_receipt_report(&config)?,
        Mode::RenderLatestDaisReceiptScript => render_latest_dais_receipt_script_report(&config)?,
        Mode::RunLatestDaisReceipt => run_latest_dais_receipt_report(&config)?,
        Mode::VerifyLatestDaisReceipt => verify_latest_dais_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_latest_dais_receipt_report(config: &Config) -> Result<PackageDaisReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    let suggested_session_dir = config.session_dir.clone().unwrap_or_else(|| {
        root.join("tiny_live_activation_package_pedestal_certificate_latest.session")
    });
    let paths = dais_receipt_latest_paths(&suggested_session_dir);

    match resolve_latest_pedestal_certificate(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            "plan_latest_dais_receipt",
            TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-pedestal-certificate lineage via accepted latest-pedestal-certificate session {}, exact decision-packet confirmation anchor {}, and the accepted dais-receipt contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_pedestal_certificate_latest_session_dir.display(),
...
fn render_latest_dais_receipt_script_report(
    config: &Config,
) -> Result<PackageDaisReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    ...
    match resolve_latest_pedestal_certificate(root, &paths) {
        Ok(resolved) => {
            write_script(
                ...
                &format!(
                    ...
                    run_latest = command_summary(
                        PEDESTAL_CERTIFICATE_LATEST_BIN,
                        &latest_dais_receipt_run_args(
                            root,
                            &paths.nested_pedestal_certificate_latest_session_dir,
                        ),
                    ),
                    verify_latest = command_summary(
                        PEDESTAL_CERTIFICATE_LATEST_BIN,
                        &latest_dais_receipt_verify_args(
                            &paths.nested_pedestal_certificate_latest_session_dir,
                        ),
                    ),
                    run_handoff_bundle = downstream_dais_receipt_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_pedestal_certificate_session_dir(&paths),
                        &paths.nested_dais_receipt_session_dir,
                    ),
                    verify_handoff_bundle = verify_dais_receipt_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_pedestal_certificate_session_dir(&paths),
                        &paths.nested_dais_receipt_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                "render_latest_dais_receipt_script",
                TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestScriptRendered,
                format!(
                    "rendered latest-dais-receipt handoff script to {} using the accepted latest-pedestal-certificate lineage and the accepted dais-receipt contract",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_dais_receipt_report(config: &Config) -> Result<PackageDaisReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-dais-receipt session dir {}",
            session_dir.display()
        );
    }
    let paths = dais_receipt_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-dais-receipt")?;

    let resolution = resolve_latest_pedestal_certificate(root, &paths);
    match resolution {
        Ok(resolved) => {
            run_latest_dais_receipt_with_resolution(root, session_dir, &paths, &resolved)
        }
        Err(failure) => run_latest_dais_receipt_failure(root, session_dir, &paths, &failure),
    }
}

fn verify_latest_dais_receipt_report(config: &Config) -> Result<PackageDaisReceiptLatestReport> {
    let session_dir = config
        .session_dir
        .as_ref()
    ...
    let session: PackageDaisReceiptLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_dais_receipt_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-dais-receipt session artifact {}: {error}",
                    paths.session_path.display()
                )],
            ))
        }
    };
    let status: PackageDaisReceiptLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_dais_receipt_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-dais-receipt status artifact {}: {error}",
                    paths.status_path.display()
                )],
            ))
        }
    };
    let stored_report: PackageDaisReceiptLatestReport = match load_json(&paths.report_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_dais_receipt_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-dais-receipt report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-dais-receipt root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-dais-receipt session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-dais-receipt status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-dais-receipt report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_pedestal_certificate_latest_session_dir,
        &paths
            .nested_pedestal_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-dais-receipt session nested_pedestal_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_pedestal_certificate_latest_session_dir,
        &paths
            .nested_pedestal_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-dais-receipt status nested_pedestal_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-dais-receipt report nested_pedestal_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_dais_receipt_session_dir,
        &paths.nested_dais_receipt_session_dir.display().to_string(),
        "latest-dais-receipt session nested_dais_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_dais_receipt_session_dir,
        &paths.nested_dais_receipt_session_dir.display().to_string(),
        "latest-dais-receipt status nested_dais_receipt_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.nested_dais_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_dais_receipt_session_dir
                .display()
                .to_string()
                .as_str(),
        ),
        "latest-dais-receipt report nested_dais_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-dais-receipt session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-dais-receipt status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-dais-receipt report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.dais_receipt_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-dais-receipt report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.dais_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-dais-receipt report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.dais_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-dais-receipt report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-dais-receipt session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-dais-receipt status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-dais-receipt report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_dais_receipt",
        "latest-dais-receipt stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-dais-receipt stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-dais-receipt stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-dais-receipt stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-dais-receipt stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report
            .latest_pedestal_certificate_plan_verdict
            .as_deref(),
        status.latest_pedestal_certificate_plan_verdict.as_deref(),
        "stored report latest_pedestal_certificate_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.latest_pedestal_certificate_verdict.as_deref(),
        status.latest_pedestal_certificate_verdict.as_deref(),
        "stored report latest_pedestal_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.dais_receipt_verdict.as_deref(),
        status.dais_receipt_verdict.as_deref(),
        "stored report dais_receipt_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: SubstructureCertificateLatestReportView =
                load_required_step_json(
                    &status.latest_pedestal_certificate_plan_step,
                    &paths.latest_pedestal_certificate_plan_report_path,
                    "latest_pedestal_certificate_plan_step",
                    &mut mismatches,
                )?;
            compare_step_artifact_against_report_metadata(
                status.latest_pedestal_certificate_plan_step.as_ref(),
                &paths.latest_pedestal_certificate_plan_report_path,
                "plan_latest_dais_receipt",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_pedestal_certificate_plan_step",
                &mut mismatches,
            );
            compare_pedestal_certificate_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_pedestal_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_pedestal_certificate_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let stored_latest_run: SubstructureCertificateLatestReportView =
                load_required_step_json(
                    &status.latest_pedestal_certificate_step,
                    &paths.latest_pedestal_certificate_report_path,
                    "latest_pedestal_certificate_step",
                    &mut mismatches,
                )?;
            compare_step_artifact_against_report_metadata(
                status.latest_pedestal_certificate_step.as_ref(),
                &paths.latest_pedestal_certificate_report_path,
                "run_latest_dais_receipt",
                &stored_latest_run.verdict,
                &stored_latest_run.reason,
                stored_latest_run.generated_at,
                "stored latest_pedestal_certificate_step",
                &mut mismatches,
            );
            compare_pedestal_certificate_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_pedestal_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_pedestal_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_pedestal_certificate_verdict.as_deref(),
            ...
            let actual_latest_run: SubstructureCertificateLatestReportView = match load_json(
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed loading actual nested latest-pedestal-certificate report under {}: {error}",
                        paths.nested_pedestal_certificate_latest_session_dir.display()
                    ));
                    fallback_pedestal_certificate_latest_copy()
                }
            };
            compare_pedestal_certificate_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-pedestal-certificate session under {}: {error}",
                        paths.nested_pedestal_certificate_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: SubstructureCertificateLatestReportView =
                    serde_json::from_value(latest_verify_raw)
                        .context("failed parsing nested latest-pedestal-certificate verify json")?;
                if latest_verify.verdict != PEDESTAL_CERTIFICATE_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-pedestal-certificate verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: DaisReceiptReportView = load_required_step_json(
                &status.dais_receipt_step,
                &paths.dais_receipt_report_path,
                "dais_receipt_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.dais_receipt_step.as_ref(),
                &paths.dais_receipt_report_path,
                "run_live_package_dais_receipt",
                &stored_handoff_bundle.verdict,
                &stored_handoff_bundle.reason,
                stored_handoff_bundle.generated_at,
                "stored dais_receipt_step",
                &mut mismatches,
            );
            compare_pedestal_certificate_run_against_snapshot(
            ...
            compare_option_string(
                ...
                "stored status dais_receipt_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.dais_receipt_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: DaisReceiptSessionView = match load_json(
                &nested_dais_receipt_session_path(&paths.nested_dais_receipt_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed reading nested dais-receipt session under {}: {error}",
                        paths.nested_dais_receipt_session_dir.display()
                    ));
            ...
            let nested_handoff_bundle_status: DaisReceiptStatusView = match load_json(
                &nested_dais_receipt_status_path(&paths.nested_dais_receipt_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed reading nested dais-receipt status under {}: {error}",
                        paths.nested_dais_receipt_session_dir.display()
                    ));
            ...
            compare_pedestal_certificate_copy_against_nested_truth(
                &stored_handoff_bundle,
            ...
            let handoff_bundle_verify_raw = match run_json_command(
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested dais-receipt session under {}: {error}",
                        paths.nested_dais_receipt_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: DaisReceiptReportView =
                    serde_json::from_value(handoff_bundle_verify_raw)
                        .context("failed parsing nested dais-receipt verify json")?;
                compare_option_string(
                    stored_handoff_bundle.registry_entry_session_dir.as_deref(),
                    handoff_bundle_verify.registry_entry_session_dir.as_deref(),
                    "stored dais-receipt copy registry_entry_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored dais-receipt copy notarization_receipt_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .consummation_record_algorithm
                        .as_deref(),
                    "stored dais-receipt copy consummation_record_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .completion_certificate_summary
                        .as_deref(),
                    "stored dais-receipt copy completion_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .completion_certificate_sha256
                        .as_deref(),
                    "stored dais-receipt copy completion_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_summary
                        .as_deref(),
                    "stored dais-receipt copy cornerstone_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_sha256
                        .as_deref(),
                    "stored dais-receipt copy cornerstone_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_algorithm
                        .as_deref(),
                    "stored dais-receipt copy cornerstone_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .capstone_certificate_summary
                        .as_deref(),
                    "stored dais-receipt copy capstone_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.capstone_certificate_sha256.as_deref(),
                    handoff_bundle_verify.capstone_certificate_sha256.as_deref(),
                    "stored dais-receipt copy capstone_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .substructure_certificate_summary
                        .as_deref(),
                    "stored dais-receipt copy substructure_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .substructure_certificate_sha256
                        .as_deref(),
                    "stored dais-receipt copy substructure_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .pedestal_certificate_summary
                        .as_deref(),
                    "stored dais-receipt copy pedestal_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.pedestal_certificate_sha256.as_deref(),
                    handoff_bundle_verify.pedestal_certificate_sha256.as_deref(),
                    "stored dais-receipt copy pedestal_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .pedestal_certificate_algorithm
                        .as_deref(),
                    "stored dais-receipt copy pedestal_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .reviewed_frozen_live_cutover_controller_command_summary
                        .as_deref(),
                    "stored dais-receipt copy reviewed_frozen_live_cutover_controller_command_summary",
                    &mut mismatches,
                );
                if handoff_bundle_verify.verdict != DAIS_RECEIPT_VERIFY_OK {
                    mismatches.push(format!(
                        "nested dais-receipt verification is non-green: {}",
                        handoff_bundle_verify.reason
                    ));
                }
            }

            let expected_result = pedestal_certificate_latest_result_from_nested_status(
                nested_handoff_bundle_status.result.as_str(),
            )
            .unwrap_or(LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract);
            if status.result != expected_result {
                mismatches.push(format!(
                    "stored latest-dais-receipt result {} does not match nested dais-receipt result {}",
        ...
        Err(failure) => {
            ...
            if failure.latest_pedestal_certificate_plan_raw.is_some() {
                let stored_latest_plan: SubstructureCertificateLatestReportView =
                    load_required_step_json(
                        &status.latest_pedestal_certificate_plan_step,
                        &paths.latest_pedestal_certificate_plan_report_path,
                ...
                compare_option_string(
                    status.latest_pedestal_certificate_plan_verdict.as_deref(),
                    failure.latest_pedestal_certificate_plan_verdict.as_deref(),
                    "stored status latest_pedestal_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report
                        ...
                        .as_deref(),
                    failure.latest_pedestal_certificate_plan_verdict.as_deref(),
                    "stored report latest_pedestal_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
                status.latest_pedestal_certificate_verdict.as_deref(),
                None,
                "stored status latest_pedestal_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_pedestal_certificate_verdict.as_deref(),
                None,
                "stored report latest_pedestal_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.dais_receipt_verdict.as_deref(),
                None,
                "stored status dais_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.dais_receipt_verdict.as_deref(),
                None,
                "stored report dais_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestVerifyOk
    } else {
        TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-dais-receipt session under {} is coherent with the current latest immutable chain and the accepted dais-receipt contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-dais-receipt verification failed".to_string())
    };

    Ok(PackageDaisReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_dais_receipt".to_string(),
        verdict,
        reason,
        ...
        dais_receipt_latest_session_path: Some(paths.session_path.display().to_string()),
        dais_receipt_latest_status_path: Some(paths.status_path.display().to_string()),
        dais_receipt_latest_report_path: Some(paths.report_path.display().to_string()),
        latest_pedestal_certificate_plan_report_path: Some(
            paths
                .latest_pedestal_certificate_plan_report_path
                .display()
                .to_string(),
        ),
        latest_pedestal_certificate_report_path: Some(
            paths.latest_pedestal_certificate_report_path.display().to_string(),
        ),
        dais_receipt_report_path: Some(paths.dais_receipt_report_path.display().to_string()),
        nested_pedestal_certificate_latest_session_dir: Some(
            paths
                .nested_pedestal_certificate_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_dais_receipt_session_dir: Some(
            paths.nested_dais_receipt_session_dir.display().to_string(),
        ),
        ...
        latest_pedestal_certificate_plan_verdict: status.latest_pedestal_certificate_plan_verdict.clone(),
        latest_pedestal_certificate_verdict: status.latest_pedestal_certificate_verdict.clone(),
        dais_receipt_verdict: status.dais_receipt_verdict.clone(),
        ...
        resolve_latest_pedestal_certificate_command_summary: Some(
            session.resolve_latest_pedestal_certificate_command_summary.clone(),
        ),
        run_latest_dais_receipt_command_summary: Some(
            session.run_latest_dais_receipt_command_summary.clone(),
        ),
        verify_latest_dais_receipt_command_summary: Some(
            session.verify_latest_dais_receipt_command_summary.clone(),
        ),
        downstream_dais_receipt_command_summary: Some(
            session.downstream_dais_receipt_command_summary.clone(),
        ),
        verify_dais_receipt_command_summary: Some(
            session.verify_dais_receipt_command_summary.clone(),
        ),
        ...
        explicit_statement:
            "this verification proves only whether the latest immutable chain still resolves cleanly into the accepted dais-receipt contract; it never runs the frozen controller"
                .to_string(),
    })
}

fn run_latest_dais_receipt_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackageDaisReceiptLatestPaths,
    resolved: &ResolvedLatestPedestalCertificate,
) -> Result<PackageDaisReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, Some(resolved), None);
    persist_json(&paths.session_path, &session)?;
    persist_json(
        &paths.latest_pedestal_certificate_plan_report_path,
        &resolved.latest_pedestal_certificate_plan_raw,
    )?;

    let latest_pedestal_certificate_plan_step = Some(step_artifact(
        &paths.latest_pedestal_certificate_plan_report_path,
        "plan_latest_dais_receipt",
        &resolved.latest_pedestal_certificate_plan.verdict,
        &resolved.latest_pedestal_certificate_plan.reason,
        resolved.latest_pedestal_certificate_plan.generated_at,
    ));

    let latest_pedestal_certificate_run_raw = match run_json_command(
        PEDESTAL_CERTIFICATE_LATEST_BIN,
        &latest_dais_receipt_run_args(root, &paths.nested_pedestal_certificate_latest_session_dir),
    ) {
        Ok(value) => value,
        Err(error) => {
            let status = refusal_status(
                ...
                paths,
                Some(resolved),
                LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract,
                format!(
                    "failed running nested latest-pedestal-certificate handoff under {}: {error}",
                    paths
                        .nested_pedestal_certificate_latest_session_dir
                        .display()
                ),
    ...
    persist_json(
        &paths.latest_pedestal_certificate_report_path,
        &latest_pedestal_certificate_run_raw,
    )?;
    let latest_pedestal_certificate_run: SubstructureCertificateLatestReportView =
        serde_json::from_value(latest_pedestal_certificate_run_raw)
            .context("failed parsing nested latest-pedestal-certificate run json")?;
    let latest_pedestal_certificate_step = Some(step_artifact(
        &paths.latest_pedestal_certificate_report_path,
        "run_latest_dais_receipt",
        &latest_pedestal_certificate_run.verdict,
        &latest_pedestal_certificate_run.reason,
        latest_pedestal_certificate_run.generated_at,
    ));

    if let Some(terminal_failure) =
        latest_pedestal_certificate_terminal_failure(&latest_pedestal_certificate_run)
    {
        let status = refusal_status(
            root,
            session_dir,
            ...
            Some(resolved),
            terminal_failure,
            latest_pedestal_certificate_run.reason.clone(),
            latest_pedestal_certificate_plan_step,
            latest_pedestal_certificate_step,
            None,
        );
    ...
    compare_pedestal_certificate_latest_run_against_snapshot(
        &latest_pedestal_certificate_run,
        &resolved.snapshot,
        &paths.nested_pedestal_certificate_latest_session_dir,
        &mut run_mismatches,
    );
    ...
    let latest_pedestal_certificate_verify: SubstructureCertificateLatestReportView =
        match run_json_command(
            PEDESTAL_CERTIFICATE_LATEST_BIN,
            &latest_dais_receipt_verify_args(&paths.nested_pedestal_certificate_latest_session_dir),
        ) {
            ...
            Err(error) => {
                let status = refusal_status(
                    ...
                    LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract,
                    format!(
                    "failed verifying nested latest-pedestal-certificate handoff under {}: {error}",
                    paths.nested_pedestal_certificate_latest_session_dir.display()
                ),
                    latest_pedestal_certificate_plan_step,
                    latest_pedestal_certificate_step,
    ...
    if latest_pedestal_certificate_verify.verdict != PEDESTAL_CERTIFICATE_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract,
            latest_pedestal_certificate_verify.reason,
            latest_pedestal_certificate_plan_step,
            latest_pedestal_certificate_step,
            None,
        );
    ...
    let release_capsule_run_raw = match run_json_command(
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract,
                format!(
                    "failed running downstream dais-receipt contract under {}: {error}",
                    paths.nested_dais_receipt_session_dir.display()
                ),
                latest_pedestal_certificate_plan_step,
                latest_pedestal_certificate_step,
    ...
    persist_json(&paths.dais_receipt_report_path, &release_capsule_run_raw)?;
    let provenance_certificate_run: DaisReceiptReportView =
        serde_json::from_value(release_capsule_run_raw)
            .context("failed parsing nested dais-receipt run json")?;
    let dais_receipt_step = Some(step_artifact(
        &paths.dais_receipt_report_path,
        "run_live_package_dais_receipt",
    ...
    let provenance_certificate_verify: DaisReceiptReportView = match run_json_command(
        DAIS_RECEIPT_BIN,
        &dais_receipt_verify_args(
            &resolved.snapshot,
            &expected_downstream_pedestal_certificate_session_dir(paths),
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract,
                format!(
                    "failed verifying downstream dais-receipt contract under {}: {error}",
                    paths.nested_dais_receipt_session_dir.display()
                ),
                latest_pedestal_certificate_plan_step,
                latest_pedestal_certificate_step,
    ...
    if provenance_certificate_verify.verdict != DAIS_RECEIPT_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    let result = pedestal_certificate_latest_result_from_nested_status(
        provenance_certificate_run
            .result
            .as_deref()
            .unwrap_or_default(),
    ...
    .unwrap_or(LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract);
    let status = PackageDaisReceiptLatestStatus {
        status_version: DAIS_RECEIPT_LATEST_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nested_pedestal_certificate_latest_session_dir: paths
            .nested_pedestal_certificate_latest_session_dir
            .display()
            .to_string(),
        nested_dais_receipt_session_dir: paths
            .nested_dais_receipt_session_dir
            .display()
            .to_string(),
        ...
        latest_pedestal_certificate_plan_verdict: Some(
            resolved.latest_pedestal_certificate_plan.verdict.clone(),
        ),
        latest_pedestal_certificate_verdict: Some(latest_pedestal_certificate_run.verdict.clone()),
        dais_receipt_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_dais_receipt_failure(
    root: &Path,
    session_dir: &Path,
    paths: &PackageDaisReceiptLatestPaths,
    failure: &LatestPedestalCertificateResolutionFailure,
) -> Result<PackageDaisReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, None, Some(failure));
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_pedestal_certificate_plan_raw {
        persist_json(&paths.latest_pedestal_certificate_plan_report_path, value)?;
    }
...
fn resolve_latest_pedestal_certificate(
    root: &Path,
    paths: &PackageDaisReceiptLatestPaths,
) -> std::result::Result<
    ResolvedLatestPedestalCertificate,
    LatestPedestalCertificateResolutionFailure,
> {
    let latest_pedestal_certificate_plan_raw = match run_json_command(
        PEDESTAL_CERTIFICATE_LATEST_BIN,
        &latest_pedestal_certificate_plan_args(
            root,
            &paths.nested_pedestal_certificate_latest_session_dir,
        ),
    ) {
        Ok(value) => value,
        Err(error) => {
            return Err(LatestPedestalCertificateResolutionFailure {
                ...
                snapshot: None,
                latest_pedestal_certificate_plan_raw: None,
                latest_pedestal_certificate_plan_verdict: None,
                latest_pedestal_certificate_plan_reason: None,
                latest_pedestal_certificate_plan_generated_at: None,
            });
        }
    };
    let latest_pedestal_certificate_plan: SubstructureCertificateLatestReportView =
        serde_json::from_value(latest_pedestal_certificate_plan_raw.clone()).map_err(
            |error| LatestPedestalCertificateResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-pedestal-certificate upstream latest-pedestal-certificate plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_pedestal_certificate_plan_raw: Some(
                    latest_pedestal_certificate_plan_raw.clone(),
                ),
                latest_pedestal_certificate_plan_verdict: None,
                latest_pedestal_certificate_plan_reason: None,
                latest_pedestal_certificate_plan_generated_at: None,
    ...
    let snapshot =
        parse_snapshot_from_latest_pedestal_certificate(&latest_pedestal_certificate_plan).ok();
    if latest_pedestal_certificate_plan.verdict != PEDESTAL_CERTIFICATE_LATEST_PLAN_READY {
        return Err(LatestPedestalCertificateResolutionFailure {
            kind: map_pedestal_certificate_latest_failure_kind(
                &latest_pedestal_certificate_plan.verdict,
            ),
            reason: format!(
                "latest-pedestal-certificate resolution refused because nested latest-pedestal-certificate reported: {}",
                latest_pedestal_certificate_plan.reason
            ),
            snapshot,
            latest_pedestal_certificate_plan_raw: Some(latest_pedestal_certificate_plan_raw),
            latest_pedestal_certificate_plan_verdict: Some(
                latest_pedestal_certificate_plan.verdict,
            ),
            latest_pedestal_certificate_plan_reason: Some(
                latest_pedestal_certificate_plan.reason,
            ),
            latest_pedestal_certificate_plan_generated_at: Some(
                latest_pedestal_certificate_plan.generated_at,
            ),
        });
    }

    let snapshot =
        parse_snapshot_from_latest_pedestal_certificate(&latest_pedestal_certificate_plan)
            .map_err(|error| LatestPedestalCertificateResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!("latest-pedestal-certificate resolution failed: {error}"),
                snapshot: None,
                latest_pedestal_certificate_plan_raw: Some(
                    latest_pedestal_certificate_plan_raw.clone(),
                ),
                latest_pedestal_certificate_plan_verdict: Some(
                    latest_pedestal_certificate_plan.verdict.clone(),
                ),
                latest_pedestal_certificate_plan_reason: Some(
                    latest_pedestal_certificate_plan.reason.clone(),
                ),
                latest_pedestal_certificate_plan_generated_at: Some(
                    latest_pedestal_certificate_plan.generated_at,
                ),
    ...
    let mut mismatches = Vec::new();
    compare_pedestal_certificate_latest_plan_against_snapshot(
        &latest_pedestal_certificate_plan,
        &snapshot,
        &paths.nested_pedestal_certificate_latest_session_dir,
        &mut mismatches,
    );
    if !mismatches.is_empty() {
        return Err(LatestPedestalCertificateResolutionFailure {
            kind: ResolutionFailureKind::DriftedPedestalCertificateContract,
            reason: format!(
                "latest-pedestal-certificate drift detected: {}",
                mismatches[0]
            ),
            ...
            latest_pedestal_certificate_plan_reason: Some(latest_pedestal_certificate_plan.reason),
            latest_pedestal_certificate_plan_generated_at: Some(
                latest_pedestal_certificate_plan.generated_at,
            ),
        });
    ...
    Ok(ResolvedLatestPedestalCertificate {
        snapshot,
        latest_pedestal_certificate_plan_raw,
        latest_pedestal_certificate_plan,
    })
}

fn parse_snapshot_from_latest_pedestal_certificate(
    view: &SubstructureCertificateLatestReportView,
) -> Result<LatestPedestalCertificateSnapshot> {
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-pedestal-certificate nested latest-pedestal-certificate plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-pedestal-certificate nested latest-pedestal-certificate plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestPedestalCertificateSnapshot {
        latest_top_step_name,
        ...
        wrapper_timeout_ms: view.wrapper_timeout_ms.ok_or_else(|| {
            anyhow!(
                "latest-pedestal-certificate nested latest-pedestal-certificate plan is missing wrapper_timeout_ms"
            )
        })?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!(
                "latest-pedestal-certificate nested latest-pedestal-certificate plan is missing service_status_max_staleness_ms"
            )
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackageDaisReceiptLatestPaths,
    resolved: &ResolvedLatestPedestalCertificate,
) -> PackageDaisReceiptLatestReport {
    PackageDaisReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        latest_pedestal_certificate_report_path: Some(
            paths
                .latest_pedestal_certificate_report_path
                .display()
                .to_string(),
        ),
        dais_receipt_report_path: Some(paths.dais_receipt_report_path.display().to_string()),
        ...
        latest_pedestal_certificate_verdict: None,
        dais_receipt_verdict: None,
        result: None,
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_pedestal_certificate_command_summary: Some(
            resolve_latest_pedestal_certificate_command_summary(
                root,
                &paths.nested_pedestal_certificate_latest_session_dir,
            ),
        ),
        run_latest_dais_receipt_command_summary: Some(run_latest_dais_receipt_command_summary(
            root,
            session_dir,
        )),
        verify_latest_dais_receipt_command_summary: Some(
            verify_latest_dais_receipt_command_summary(session_dir),
        ),
        downstream_dais_receipt_command_summary: Some(downstream_dais_receipt_command_summary(
            &resolved.snapshot,
            &expected_downstream_pedestal_certificate_session_dir(paths),
            &paths.nested_dais_receipt_session_dir,
        )),
...
fn report_from_resolution_failure(
    ...
    PackageDaisReceiptLatestReport {
        ...
        latest_pedestal_certificate_plan_verdict: failure
            .latest_pedestal_certificate_plan_verdict
            .clone(),
        latest_pedestal_certificate_verdict: None,
        dais_receipt_verdict: None,
        ...
        downstream_dais_receipt_command_summary: snapshot.map(|value| {
            downstream_dais_receipt_command_summary(
                value,
                &expected_downstream_pedestal_certificate_session_dir(paths),
                &paths.nested_dais_receipt_session_dir,
            )
        }),
        verify_dais_receipt_command_summary: snapshot.map(|value| {
...
fn planned_session(
    ...
    session_dir: &Path,
    paths: &PackageDaisReceiptLatestPaths,
    resolved: Option<&ResolvedLatestPedestalCertificate>,
    failure: Option<&LatestPedestalCertificateResolutionFailure>,
) -> PackageDaisReceiptLatestSession {
    let snapshot = resolved
        .map(|value| &value.snapshot)
        .or_else(|| failure.and_then(|value| value.snapshot.as_ref()));
    PackageDaisReceiptLatestSession {
        session_version: DAIS_RECEIPT_LATEST_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        root: root.display().to_string(),
        session_dir: session_dir.display().to_string(),
        nested_pedestal_certificate_latest_session_dir: paths
        ...
        run_latest_dais_receipt_command_summary: run_latest_dais_receipt_command_summary(
            root,
            session_dir,
        ),
        verify_latest_dais_receipt_command_summary: verify_latest_dais_receipt_command_summary(
            session_dir,
        ),
        downstream_dais_receipt_command_summary: snapshot
            .map(|value| {
                downstream_dais_receipt_command_summary(
                    value,
                    &expected_downstream_pedestal_certificate_session_dir(paths),
            ...
            .unwrap_or_else(|| "<dais-receipt-run-unavailable>".to_string()),
        verify_dais_receipt_command_summary: snapshot
            .map(|value| {
                verify_dais_receipt_command_summary(
                    value,
                    &expected_downstream_pedestal_certificate_session_dir(paths),
            ...
            .unwrap_or_else(|| "<dais-receipt-verify-unavailable>".to_string()),
        reviewed_frozen_live_cutover_controller_command_summary: snapshot.map(|value| {
            value
                .reviewed_frozen_live_cutover_controller_command_summary
                .clone()
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackageDaisReceiptLatestPaths,
    session: &PackageDaisReceiptLatestSession,
    status: &PackageDaisReceiptLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackageDaisReceiptLatestReport {
    PackageDaisReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        nested_dais_receipt_session_dir: Some(session.nested_dais_receipt_session_dir.clone()),
        downstream_decision_packet_session_dir: Some(
            session.downstream_decision_packet_session_dir.clone(),
        ),
        latest_pedestal_certificate_plan_verdict: status
            .latest_pedestal_certificate_plan_verdict
            .clone(),
        latest_pedestal_certificate_verdict: status.latest_pedestal_certificate_verdict.clone(),
        dais_receipt_verdict: status.dais_receipt_verdict.clone(),
...
fn dais_receipt_latest_paths(session_dir: &Path) -> PackageDaisReceiptLatestPaths {
    PackageDaisReceiptLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_dais_receipt_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_dais_receipt_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_dais_receipt_latest.report.json"),
        latest_pedestal_certificate_plan_report_path: session_dir.join(
            "tiny_live_activation_package_dais_receipt_latest.pedestal_certificate_latest_plan.report.json",
        ),
        latest_pedestal_certificate_report_path: session_dir.join(
            "tiny_live_activation_package_dais_receipt_latest.pedestal_certificate_latest.report.json",
        ),
        dais_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_dais_receipt_latest.dais_receipt.report.json",
        ),
        nested_pedestal_certificate_latest_session_dir: session_dir.join(
            "tiny_live_activation_package_dais_receipt_latest.pedestal_certificate_latest_session",
        ),
        nested_dais_receipt_session_dir: session_dir.join(
            "tiny_live_activation_package_dais_receipt_latest.dais_receipt_session",
        ),
    }
}
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_pedestal_certificate_latest_session_dir
        .join("tiny_live_activation_package_pedestal_certificate_latest.decision_packet_session")
}

fn expected_downstream_pedestal_certificate_session_dir(
    paths: &PackageDaisReceiptLatestPaths,
) -> PathBuf {
    paths.nested_pedestal_certificate_latest_session_dir.join(
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_pedestal_certificate_latest_session_dir
        .join("tiny_live_activation_package_pedestal_certificate_latest.review_receipt_session")
}

fn latest_pedestal_certificate_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-pedestal-certificate".to_string(),
        "--json".to_string(),
    ]
}
...
fn latest_dais_receipt_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-pedestal-certificate".to_string(),
        "--json".to_string(),
    ]
}
...
fn latest_dais_receipt_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-pedestal-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn dais_receipt_run_args(
    snapshot: &LatestPedestalCertificateSnapshot,
    pedestal_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        "--pedestal-certificate-session-dir".to_string(),
        pedestal_certificate_session_dir.display().to_string(),
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-dais-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn dais_receipt_verify_args(
    snapshot: &LatestPedestalCertificateSnapshot,
    pedestal_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-live-package-dais-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn resolve_latest_pedestal_certificate_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{PEDESTAL_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --plan-latest-pedestal-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn run_latest_dais_receipt_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{DAIS_RECEIPT_LATEST_BIN} --root {} --session-dir {} --run-latest-dais-receipt --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn verify_latest_dais_receipt_command_summary(session_dir: &Path) -> String {
    format!(
        "{DAIS_RECEIPT_LATEST_BIN} --session-dir {} --verify-latest-dais-receipt --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_dais_receipt_command_summary(
    snapshot: &LatestPedestalCertificateSnapshot,
    plinth_receipt_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{DAIS_RECEIPT_BIN} --pedestal-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-dais-receipt --json",
...
fn verify_dais_receipt_command_summary(
    snapshot: &LatestPedestalCertificateSnapshot,
    pedestal_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{DAIS_RECEIPT_BIN} --pedestal-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-dais-receipt --json",
        shell_escape_path(pedestal_certificate_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
}
...
fn nested_pedestal_certificate_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_pedestal_certificate_latest.report.json")
}

fn nested_dais_receipt_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_dais_receipt.session.json")
}

fn nested_dais_receipt_status_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_dais_receipt.status.json")
}
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-dais-receipt script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageDaisReceiptLatestStepArtifact {
    PackageDaisReceiptLatestStepArtifact {
...
fn canonical_wrapper_mode(mode: &str) -> &str {
    match mode {
        "plan_latest_dais_receipt" => "plan_latest_dais_receipt",
        "render_latest_dais_receipt_script" => "render_latest_dais_receipt_script",
        "run_latest_dais_receipt" => "run_latest_dais_receipt",
        "verify_latest_dais_receipt" => "verify_latest_dais_receipt",
        other => other,
    }
}

fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackageDaisReceiptLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-dais-receipt status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-dais-receipt status"
        ));
        return;
    };
    compare_string(
...
fn compare_pedestal_certificate_latest_plan_against_snapshot(
    ...
    compare_option_string(
        ...
        Some(PEDESTAL_CERTIFICATE_LATEST_PLAN_READY),
        "stored latest-pedestal-certificate plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_dais_receipt",
        "stored latest-pedestal-certificate plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-pedestal-certificate plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-pedestal-certificate plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-pedestal-certificate plan session_dir",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        ...
        "stored latest-pedestal-certificate plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-pedestal-certificate plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-pedestal-certificate plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-pedestal-certificate plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_pedestal_certificate_latest_run_against_snapshot(
    stored: &SubstructureCertificateLatestReportView,
    snapshot: &LatestPedestalCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_dais_receipt",
        "stored latest-pedestal-certificate run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-pedestal-certificate run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-pedestal-certificate run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-pedestal-certificate run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-pedestal-certificate run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-pedestal-certificate run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-pedestal-certificate run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-pedestal-certificate run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-pedestal-certificate run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-pedestal-certificate run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-pedestal-certificate run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-pedestal-certificate run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-pedestal-certificate run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-pedestal-certificate run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-pedestal-certificate run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_pedestal_certificate_latest_copy_matches_actual(
    stored: &SubstructureCertificateLatestReportView,
    ...
    compare_option_string(
        ...
        "stored latest-pedestal-certificate copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-pedestal-certificate copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-pedestal-certificate copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-pedestal-certificate copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-pedestal-certificate copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-pedestal-certificate copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-pedestal-certificate copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-pedestal-certificate copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-pedestal-certificate copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-pedestal-certificate copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-pedestal-certificate copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-pedestal-certificate copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_pedestal_certificate_run_against_snapshot(
    stored: &DaisReceiptReportView,
    snapshot: &LatestPedestalCertificateSnapshot,
    pedestal_certificate_session_dir: &Path,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_live_package_dais_receipt",
        "stored dais-receipt run mode",
        mismatches,
    );
    compare_string(
        &stored.pedestal_certificate_session_dir,
        &pedestal_certificate_session_dir.display().to_string(),
        "stored dais-receipt run pedestal_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored dais-receipt run decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored dais-receipt run execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored dais-receipt run turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored dais-receipt run launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored dais-receipt run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored dais-receipt run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored dais-receipt run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored dais-receipt run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored dais-receipt run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored dais-receipt run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored dais-receipt run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.dais_receipt_session_path.as_deref(),
        Some(
            nested_dais_receipt_session_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored dais-receipt run dais_receipt_session_path",
        mismatches,
    );
    compare_option_string(
        stored.dais_receipt_status_path.as_deref(),
        Some(
            nested_dais_receipt_status_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored dais-receipt run dais_receipt_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_pedestal_certificate_report_path.as_deref(),
        Some(
            session_dir
        ...
        "stored dais-receipt run archived_pedestal_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        stored
            ...
            .as_deref(),
        Some(verify_dais_receipt_input_command_summary(pedestal_certificate_session_dir).as_str()),
        "stored dais-receipt run verify_pedestal_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
...
fn compare_pedestal_certificate_copy_against_nested_truth(
    ...
    compare_option_string(
        ...
        "stored dais-receipt copy verify_pedestal_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored dais-receipt copy release_capsule_digest_manifest_sha256",
        mismatches,
    );
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_pedestal_certificate_command_summary,
        &resolve_latest_pedestal_certificate_command_summary(
            root,
            &paths.nested_pedestal_certificate_latest_session_dir,
        ),
        "stored session resolve_latest_pedestal_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.run_latest_dais_receipt_command_summary,
        &run_latest_dais_receipt_command_summary(root, session_dir),
        "stored session run_latest_dais_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_latest_dais_receipt_command_summary,
        &verify_latest_dais_receipt_command_summary(session_dir),
        "stored session verify_latest_dais_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.downstream_dais_receipt_command_summary,
        &downstream_dais_receipt_command_summary(
            snapshot,
            &expected_downstream_pedestal_certificate_session_dir(paths),
            &paths.nested_dais_receipt_session_dir,
        ),
        "stored session downstream_dais_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_dais_receipt_command_summary,
        &verify_dais_receipt_command_summary(
            snapshot,
            &expected_downstream_pedestal_certificate_session_dir(paths),
            &paths.nested_dais_receipt_session_dir,
        ),
        "stored session verify_dais_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_pedestal_certificate_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_dais_receipt_session_dir,
        &paths.nested_dais_receipt_session_dir.display().to_string(),
        "stored session nested_dais_receipt_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackageDaisReceiptLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackageDaisReceiptLatestPaths,
    snapshot: &LatestPedestalCertificateSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_pedestal_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report.run_latest_dais_receipt_command_summary.as_deref(),
        Some(run_latest_dais_receipt_command_summary(root, session_dir).as_str()),
        "stored report run_latest_dais_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report.verify_latest_dais_receipt_command_summary.as_deref(),
        Some(verify_latest_dais_receipt_command_summary(session_dir).as_str()),
        "stored report verify_latest_dais_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report.downstream_dais_receipt_command_summary.as_deref(),
        Some(
            downstream_dais_receipt_command_summary(
                snapshot,
                &expected_downstream_pedestal_certificate_session_dir(paths),
        ...
        "stored report downstream_dais_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report.verify_dais_receipt_command_summary.as_deref(),
        Some(
            verify_dais_receipt_command_summary(
                snapshot,
                &expected_downstream_pedestal_certificate_session_dir(paths),
        ...
        "stored report verify_dais_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
    ...
    compare_option_string(
        report.dais_receipt_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "stored report dais_receipt_latest_session_path",
        mismatches,
    );
    compare_option_string(
        report.dais_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "stored report dais_receipt_latest_status_path",
        mismatches,
    );
    compare_option_string(
        report.dais_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "stored report dais_receipt_latest_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report latest_pedestal_certificate_plan_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_pedestal_certificate_report_path.as_deref(),
        Some(
            paths
                .latest_pedestal_certificate_report_path
                .display()
        ...
        "stored report latest_pedestal_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        report.dais_receipt_report_path.as_deref(),
        Some(
            paths
        ...
        "stored report dais_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report nested_pedestal_certificate_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_dais_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_dais_receipt_session_dir
                .display()
        ...
        "stored report nested_dais_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_pedestal_certificate_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_dais_receipt_session_dir,
        &paths.nested_dais_receipt_session_dir.display().to_string(),
        "stored session nested_dais_receipt_session_dir on failure",
        mismatches,
    );
...
fn map_pedestal_certificate_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_pedestal_certificate_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_pedestal_certificate_latest_refused_by_drifted_pedestal_certificate_contract" => {
            ResolutionFailureKind::DriftedPedestalCertificateContract
        }
        _ => ResolutionFailureKind::InvalidLatestChain,
    }
}

fn latest_pedestal_certificate_terminal_failure(
    report: &SubstructureCertificateLatestReportView,
) -> Option<LivePackageDaisReceiptLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackageDaisReceiptLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackageDaisReceiptLatestResult::RefusedByInvalidLatestChain)
        }
        Some("refused_by_drifted_pedestal_certificate_contract") => {
            Some(LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract)
        }
        _ => None,
    }
}

fn pedestal_certificate_latest_result_from_nested_status(
    result: &str,
) -> Option<LivePackageDaisReceiptLatestResult> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => {
            Some(LivePackageDaisReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen)
        }
        "refused_now_by_stage3" => Some(LivePackageDaisReceiptLatestResult::RefusedNowByStage3),
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackageDaisReceiptLatestResult::RefusedNowByPreActivationGate)
        }
...
fn dais_receipt_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => {
            Some("tiny_live_package_dais_receipt_ready_for_manual_execution_when_gate_turns_green")
        }
        "refused_now_by_stage3" => Some("tiny_live_package_dais_receipt_refused_now_by_stage3"),
        "refused_now_by_pre_activation_gate" => {
            Some("tiny_live_package_dais_receipt_refused_now_by_pre_activation_gate")
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_dais_receipt_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}

fn result_from_failure_kind(kind: ResolutionFailureKind) -> LivePackageDaisReceiptLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackageDaisReceiptLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackageDaisReceiptLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedPedestalCertificateContract => {
            LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract
        }
...
fn verdict_for_result(
    result: LivePackageDaisReceiptLatestResult,
) -> TinyLivePackageDaisReceiptLatestVerdict {
    match result {
        LivePackageDaisReceiptLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestRefusedByMissingLatestChain
        }
        LivePackageDaisReceiptLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestRefusedByInvalidLatestChain
        }
        LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract => {
            TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestRefusedByDriftedDaisReceiptContract
        }
        LivePackageDaisReceiptLatestResult::RefusedNowByStage3 => {
            TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestRefusedNowByStage3
        }
        LivePackageDaisReceiptLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestRefusedNowByPreActivationGate
        }
        LivePackageDaisReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackageDaisReceiptLatestResult) -> String {
    match result {
        LivePackageDaisReceiptLatestResult::RefusedByMissingLatestChain => {
        ...
        LivePackageDaisReceiptLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-pedestal-certificate / dais-receipt path".to_string()
        }
        LivePackageDaisReceiptLatestResult::RefusedByDriftedDaisReceiptContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-pedestal-certificate / dais-receipt contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackageDaisReceiptLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-dais-receipt refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackageDaisReceiptLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-dais-receipt refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackageDaisReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            "the latest immutable chain now resolves into the accepted dais-receipt contract and one bounded latest-dais-receipt handoff is ready for manual execution when gate truth turns green; this wrapper still did not run or authorize the frozen controller".to_string()
        }
    }
}
...
fn verify_dais_receipt_input_command_summary(pedestal_certificate_session_dir: &Path) -> String {
    format!(
        "verify immutable pedestal-certificate session under {} before freezing the final dais receipt",
        pedestal_certificate_session_dir.display()
    )
}
...
fn refusal_status(
    ...
    latest_pedestal_certificate_step: Option<PackageDaisReceiptLatestStepArtifact>,
    dais_receipt_step: Option<PackageDaisReceiptLatestStepArtifact>,
) -> PackageDaisReceiptLatestStatus {
    ...
    PackageDaisReceiptLatestStatus {
        ...
        result,
        reason,
        latest_pedestal_certificate_plan_verdict: resolved
            .map(|value| value.latest_pedestal_certificate_plan.verdict.clone()),
        latest_pedestal_certificate_verdict: latest_pedestal_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        dais_receipt_verdict: dais_receipt_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackageDaisReceiptLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_pedestal_certificate_plan_step: failure
            .latest_pedestal_certificate_plan_raw
            .as_ref()
            .map(|_| {
                step_artifact(
                    ...
                    failure
                        ...
                        .unwrap_or("<missing>"),
                    failure
                        .latest_pedestal_certificate_plan_reason
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_pedestal_certificate_plan_generated_at
                        .unwrap_or_else(Utc::now),
                )
            }),
        latest_pedestal_certificate_step: None,
...
fn verify_invalid_dais_receipt_latest_report_without_session(
    session_dir: &Path,
    verification_mismatches: Vec<String>,
) -> PackageDaisReceiptLatestReport {
    ...
    PackageDaisReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_dais_receipt".to_string(),
        verdict: TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestVerifyInvalid,
        reason,
        root: None,
        ...
        session_dir: Some(session_dir.display().to_string()),
        dais_receipt_latest_session_path: Some(
            dais_receipt_latest_paths(session_dir)
                .session_path
                .display()
                .to_string(),
        ),
        dais_receipt_latest_status_path: Some(
            dais_receipt_latest_paths(session_dir)
                .status_path
                .display()
                .to_string(),
        ),
        dais_receipt_latest_report_path: Some(
            dais_receipt_latest_paths(session_dir)
                .report_path
                .display()
                .to_string(),
        ),
        latest_pedestal_certificate_plan_report_path: None,
        latest_pedestal_certificate_report_path: None,
        dais_receipt_report_path: None,
        nested_pedestal_certificate_latest_session_dir: None,
        nested_dais_receipt_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_pedestal_certificate_command_summary: None,
        run_latest_dais_receipt_command_summary: None,
        verify_latest_dais_receipt_command_summary: None,
        downstream_dais_receipt_command_summary: None,
        verify_dais_receipt_command_summary: None,
...
fn fallback_pedestal_certificate_latest_copy() -> SubstructureCertificateLatestReportView {
...
fn fallback_notarization_receipt_session(
    ...
    DaisReceiptSessionView {
        ...
        session_dir: session_dir.display().to_string(),
        verify_pedestal_certificate_command_summary:
            verify_dais_receipt_input_command_summary(Path::new("<missing>")),
        ...
        explicit_statement:
            "the verified pedestal-certificate session is the primary input here; this immutable dais receipt freezes one final hallmark identity over the fully culminated chain without enabling production execution"
                .to_string(),
    }
}

fn fallback_notarization_receipt_status(
    _snapshot: &LatestPedestalCertificateSnapshot,
    session_dir: &Path,
) -> DaisReceiptStatusView {
    DaisReceiptStatusView {
        session_dir: session_dir.display().to_string(),
        pedestal_certificate_session_dir: "<missing>".to_string(),
        plinth_receipt_session_dir: "<missing>".to_string(),
        ...
        dais_receipt_algorithm: "sha256".to_string(),
        explicit_statement:
            "this dais receipt is archival and read-only; it never executes or enables the frozen controller"
                .to_string(),
    }
}

fn render_report_lines(report: &PackageDaisReceiptLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_pedestal_certificate_latest_session_dir {
        lines.push(format!(
            "nested_pedestal_certificate_latest_session_dir={value}"
        ));
    }
    ...
    if let Some(value) = &report.nested_dais_receipt_session_dir {
        lines.push(format!("nested_dais_receipt_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_dais_receipt_command_summary {
        lines.push(format!("downstream_dais_receipt_command_summary={value}"));
    }
...
mod tests {
    ...
    struct FakePedestalCertificateLatestFixture {
        temp_dir: PathBuf,
        root: PathBuf,
        ...
        pedestal_certificate_latest_plan_json_path: PathBuf,
        pedestal_certificate_latest_run_json_path: PathBuf,
        pedestal_certificate_latest_verify_json_path: PathBuf,
    ...
    impl FakePedestalCertificateLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestDaisReceipt,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestDaisReceiptScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestDaisReceipt,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestDaisReceipt,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    #[test]
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_pedestal_certificate_lineage(
    ) {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_plan_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = plan_latest_dais_receipt_report(&fixture.plan_config()).unwrap();
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert_eq!(
            report.verdict,
            TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestPlanReady
        );
        assert_eq!(report.mode, "plan_latest_dais_receipt");
        assert_eq!(
            report.latest_top_step_name.as_deref(),
        ...
        assert!(report
            ...
            .as_deref()
            .unwrap()
            .contains(DAIS_RECEIPT_BIN));
    }

    #[test]
    fn render_mode_emits_exact_downstream_dais_receipt_contract_not_second_controller_path() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_render_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = render_latest_dais_receipt_script_report(&fixture.render_config()).unwrap();
        assert_eq!(
            report.verdict,
            TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestScriptRendered
        );
        assert_eq!(report.mode, "render_latest_dais_receipt_script");
        let script = fs::read_to_string(&fixture.output_script_path).unwrap();
        assert!(script.contains(PEDESTAL_CERTIFICATE_LATEST_BIN));
        assert!(script.contains(DAIS_RECEIPT_BIN));
        assert!(script.contains("--pedestal-certificate-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-dais-receipt"));
        assert!(script.contains("--verify-live-package-dais-receipt"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            (
                "pedestal_certificate_latest_missing",
                "tiny_live_package_pedestal_certificate_latest_refused_by_missing_latest_chain",
                "no persisted immutable chain exists",
                TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestRefusedByMissingLatestChain,
            ),
            (
                "pedestal_certificate_latest_invalid",
                "tiny_live_package_pedestal_certificate_latest_refused_by_invalid_latest_chain",
                "latest immutable chain is incomplete",
                TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestRefusedByInvalidLatestChain,
            ),
            (
                "pedestal_certificate_latest_below",
                "tiny_live_package_pedestal_certificate_latest_refused_by_invalid_latest_chain",
            ...
            let mut plan: Value = load_json(&fixture.pedestal_certificate_latest_plan_json_path).unwrap();
            plan["verdict"] = json!(verdict);
            plan["reason"] = json!(reason);
            if label.ends_with("below") {
                plan["latest_top_step_name"] = json!("choir_receipt");
            }
            persist_json(&fixture.pedestal_certificate_latest_plan_json_path, &plan).unwrap();

            let report = run_latest_dais_receipt_report(&fixture.run_config()).unwrap();
            assert_eq!(report.verdict, expected);
        }
    }

    #[test]
    fn plan_mode_maps_real_latest_pedestal_certificate_drift_verdict_to_local_drift_refusal() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_plan_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value =
            load_json(&fixture.pedestal_certificate_latest_plan_json_path).unwrap();
        plan["verdict"] = json!(
            "tiny_live_package_pedestal_certificate_latest_refused_by_drifted_pedestal_certificate_contract"
        );
        plan["reason"] = json!("real accepted latest-pedestal-certificate drift");
        persist_json(&fixture.pedestal_certificate_latest_plan_json_path, &plan).unwrap();
        ...
        assert_eq!(
            report.result.as_deref(),
            Some("refused_by_drifted_dais_receipt_contract")
        );
    }

    #[test]
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_pedestal_certificate_reports_real_drift(
    ) {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_run_drift",
            "refused_by_drifted_pedestal_certificate_contract",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert_eq!(report.dais_receipt_verdict.as_deref(), None);
    }

    #[test]
    fn run_mode_refuses_when_nested_release_capsule_lineage_drifts_from_latest_immutable_chain() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_handoff_bundle_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let mut run: Value = load_json(&fixture.pedestal_certificate_latest_run_json_path).unwrap();
        run["package_dir"] = json!("/tampered/package");
        persist_json(&fixture.pedestal_certificate_latest_run_json_path, &run).unwrap();
        ...
        assert!(report
            .reason
            .contains("stored latest-pedestal-certificate run package_dir"));
    }

    #[test]
    fn verify_mode_fails_when_persisted_latest_pedestal_certificate_artifacts_are_tampered_or_missing_nested_lineage_truth(
    ) {
        {
            let fixture = fake_pedestal_certificate_latest_fixture(
                "pedestal_certificate_latest_verify_tampered_copy",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            run_latest_dais_receipt_report(&fixture.run_config()).unwrap();

            let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.dais_receipt_report_path).unwrap();
            report["decision_packet_session_dir"] = json!("/tampered/run-decision-packet-session");
            persist_json(&paths.dais_receipt_report_path, &report).unwrap();

            let verify = verify_latest_dais_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageDaisReceiptLatestVerdict::TinyLivePackageDaisReceiptLatestVerifyInvalid
            );
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains("stored dais-receipt run decision_packet_session_dir")
            }));
        }

        {
            let fixture = fake_pedestal_certificate_latest_fixture(
                "pedestal_certificate_latest_verify_missing_nested_truth",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains(
                    "tiny_live_activation_package_pedestal_certificate_latest.report.json",
                )
            }));
        }
    }
    ...
    #[test]
    fn verify_accepts_real_shaped_native_dais_identity_layers_on_happy_path() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_real_shaped_dais_paths_green",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    #[test]
    fn run_and_verify_reject_stale_foreign_wrapper_schema_for_downstream_dais_copy() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_rejects_stale_keystone_wrapper_schema",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert!(paths.dais_receipt_report_path.ends_with(
            "tiny_live_activation_package_dais_receipt_latest.dais_receipt.report.json"
        ));
        assert!(paths.dais_receipt_report_path.exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_dais_receipt_latest.pedestal_certificate.report.json"
            )
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_dais_receipt_latest.plinth_receipt.report.json")
            .exists());

        let mut status: Value = load_json(&paths.status_path).unwrap();
        let stored_step = status
            .get("dais_receipt_step")
            .cloned()
            .expect("dais_receipt_step should be present");
        assert!(status.get("keystone_receipt_step").is_none());
        status["keystone_receipt_step"] = stored_step;
        if let Some(object) = status.as_object_mut() {
            object.remove("dais_receipt_step");
        }
        persist_json(&paths.status_path, &status).unwrap();

        let error = verify_latest_dais_receipt_report(&fixture.verify_config()).unwrap_err();
        assert!(error.to_string().contains("dais_receipt_step"));
    }

    #[test]
    fn verify_names_dais_receipt_when_nested_native_session_load_breaks() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_nested_dais_session_load_break",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        fs::remove_file(nested_dais_receipt_session_path(
            &paths.nested_dais_receipt_session_dir,
        ))
        .unwrap();
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("failed reading nested dais-receipt session under") }));
    }

    #[test]
    fn verify_fails_when_copied_native_dais_receipt_session_path_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_dais_session_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.dais_receipt_report_path).unwrap();
        report["dais_receipt_session_path"] = json!("/tampered/finality-receipt-session.json");
        persist_json(&paths.dais_receipt_report_path, &report).unwrap();
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored dais-receipt run dais_receipt_session_path") }));
    }

    #[test]
    fn verify_fails_when_copied_native_dais_receipt_status_path_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_dais_status_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.dais_receipt_report_path).unwrap();
        report["dais_receipt_status_path"] = json!("/tampered/finality-receipt-status.json");
        persist_json(&paths.dais_receipt_report_path, &report).unwrap();
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored dais-receipt run dais_receipt_status_path") }));
    }

    #[test]
    fn run_names_latest_pedestal_certificate_when_nested_latest_wrapper_run_fails() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_nested_run_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        overwrite_fake_pedestal_certificate_latest_command(
            &fixture.bin_dir.join(PEDESTAL_CERTIFICATE_LATEST_BIN),
            &fixture.pedestal_certificate_latest_plan_json_path,
            &fixture.pedestal_certificate_latest_run_json_path,
            &fixture.pedestal_certificate_latest_verify_json_path,
            None,
            Some("forced nested latest-pedestal-certificate run failure"),
            None,
            &fixture.root,
            &paths.nested_pedestal_certificate_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed running nested latest-pedestal-certificate handoff under"));
    }

    #[test]
    fn run_names_latest_pedestal_certificate_when_nested_latest_wrapper_verify_fails() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_nested_verify_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_pedestal_certificate_latest_command(
            ...
            None,
            None,
            Some("forced nested latest-pedestal-certificate verify failure"),
            &fixture.root,
            &paths.nested_pedestal_certificate_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed verifying nested latest-pedestal-certificate handoff under"));
    }

    #[test]
    fn run_names_dais_receipt_when_downstream_native_run_fails() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "dais_receipt_downstream_run_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_release_capsule_command(
            &fixture.bin_dir.join(DAIS_RECEIPT_BIN),
            ...
            Some("forced downstream native dais run failure"),
            None,
            &expected_downstream_pedestal_certificate_session_dir(&paths),
            &expected_downstream_decision_packet_session_dir(&paths),
            &paths.nested_dais_receipt_session_dir,
    ...
    #[test]
    fn run_names_dais_receipt_when_downstream_native_verify_fails() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "dais_receipt_downstream_verify_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_release_capsule_command(
            ...
            &fixture.handoff_bundle_status_json_path,
            None,
            Some("forced downstream native dais verify failure"),
            &expected_downstream_pedestal_certificate_session_dir(&paths),
            &expected_downstream_decision_packet_session_dir(&paths),
            &paths.nested_dais_receipt_session_dir,
        );
    ...
    #[test]
    fn run_uses_dais_placeholders_when_missing_snapshot_blocks_nested_handoff_summaries() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_missing_snapshot_dais_placeholders",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_pedestal_certificate_latest_command(
            ...
            &fixture.pedestal_certificate_latest_run_json_path,
            &fixture.pedestal_certificate_latest_verify_json_path,
            Some("forced latest-pedestal-certificate plan failure without snapshot"),
            None,
            None,
            &fixture.root,
            &paths.nested_pedestal_certificate_latest_session_dir,
        ...
        assert_eq!(
            report.downstream_dais_receipt_command_summary.as_deref(),
            Some("<dais-receipt-run-unavailable>")
        );
        assert_eq!(
            report.verify_dais_receipt_command_summary.as_deref(),
            Some("<dais-receipt-verify-unavailable>")
        );

        let session: PackageDaisReceiptLatestSession = load_json(&paths.session_path).unwrap();
        assert_eq!(
            session.downstream_dais_receipt_command_summary,
            "<dais-receipt-run-unavailable>"
        );
        assert_eq!(
            session.verify_dais_receipt_command_summary,
            "<dais-receipt-verify-unavailable>"
        );
    }

    #[test]
    fn verify_names_dais_receipt_when_nested_native_verify_is_non_green() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "dais_receipt_verify_non_green_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        verify_json["verdict"] = json!("tiny_live_package_dais_receipt_verify_invalid");
        verify_json["reason"] = json!("forced nested dais-receipt verify non-green");
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("nested dais-receipt verification is non-green") }));
        assert!(!verify.verification_mismatches.iter().any(|entry| {
    ...
    #[test]
    fn verify_names_latest_dais_receipt_when_wrapper_result_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "dais_receipt_wrapper_result_mismatch_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    #[test]
    fn verify_names_latest_dais_receipt_when_activation_authorized_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "dais_receipt_activation_authorized_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn verify_fails_when_copied_native_pinnacle_receipt_sha256_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_pinnacle_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored dais-receipt copy pinnacle_receipt_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_latest_pedestal_certificate_explicit_statement_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_launch_packet_explicit_statement_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.latest_pedestal_certificate_report_path).unwrap();
        report["explicit_statement"] =
            json!("tampered nested latest release capsule explicit statement");
        persist_json(&paths.latest_pedestal_certificate_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored latest-pedestal-certificate copy explicit_statement")
        }));
    }
    ...
    fn verify_rejects_tampered_stale_substructure_wrapper_mode_string() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_wrapper_mode_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("latest-dais-receipt stored report mode") }));
    }

    #[test]
    fn wrapper_self_modes_and_self_command_summaries_use_pedestal_latest_contract() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_self_mode_and_summary_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let plan = plan_latest_dais_receipt_report(&fixture.plan_config()).unwrap();
        assert_eq!(plan.mode, "plan_latest_dais_receipt");
        assert!(plan
            .run_latest_dais_receipt_command_summary
            .as_deref()
            .unwrap()
            .contains(DAIS_RECEIPT_LATEST_BIN));
        assert!(plan
            .run_latest_dais_receipt_command_summary
            .as_deref()
            .unwrap()
        ...
        assert!(plan
            .verify_latest_dais_receipt_command_summary
            .as_deref()
            .unwrap()
            .contains(DAIS_RECEIPT_LATEST_BIN));
        assert!(plan
        ...
        let run = run_latest_dais_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(run.mode, "run_latest_dais_receipt");

        let verify = verify_latest_dais_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(verify.mode, "verify_latest_dais_receipt");

        let session: PackageDaisReceiptLatestSession =
            load_json(&dais_receipt_latest_paths(&fixture.wrapper_session_dir).session_path)
                .unwrap();
        assert!(session
            .run_latest_dais_receipt_command_summary
            .contains(DAIS_RECEIPT_LATEST_BIN));
    ...
    #[test]
    fn native_dais_input_uses_real_pedestal_certificate_session_path() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "dais_receipt_native_input_session_dir_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);

        let downstream = expected_downstream_pedestal_certificate_session_dir(&paths);
        let expected = paths.nested_pedestal_certificate_latest_session_dir.join(
            "tiny_live_activation_package_pedestal_certificate_latest.pedestal_certificate_session",
        );
    ...
    #[test]
    fn run_persists_copied_native_dais_session_and_status_under_real_dais_filenames() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_native_dais_filenames",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        let real_session = nested_dais_receipt_session_path(&paths.nested_dais_receipt_session_dir);
        let real_status = nested_dais_receipt_status_path(&paths.nested_dais_receipt_session_dir);
        ...
        let stale_pedestal_session = paths
            .nested_dais_receipt_session_dir
            .join("tiny_live_activation_package_pedestal_certificate.session.json");
        let stale_pedestal_status = paths
            .nested_dais_receipt_session_dir
            .join("tiny_live_activation_package_pedestal_certificate.status.json");

        assert!(
            real_session.exists(),
            "missing real native dais session path"
        );
        assert!(real_status.exists(), "missing real native dais status path");
        assert!(
        ...
        assert!(
            !stale_pedestal_session.exists(),
            "stale native pedestal session filename should not be used"
        );
        assert!(
            !stale_pedestal_status.exists(),
            "stale native pedestal status filename should not be used"
        );
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_decision_packet_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored dais-receipt copy decision_packet_result") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_attestation_seal_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored dais-receipt copy attestation_seal_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_registry_entry_session_dir_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_registry_entry_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored dais-receipt copy registry_entry_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_notarization_receipt_session_dir_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_notarization_receipt_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored dais-receipt copy notarization_receipt_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_chain_fingerprint_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored dais-receipt copy chain_fingerprint_summary") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_chain_fingerprint_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored dais-receipt copy chain_fingerprint_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_chain_fingerprint_algorithm_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored dais-receipt copy chain_fingerprint_algorithm")
        }));
    }
    ...
    fn verify_fails_when_copied_native_completion_certificate_closure_certificate_sha256_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_closure_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored dais-receipt copy closure_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_archive_receipt_sha256_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_archive_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored dais-receipt copy archive_receipt_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_native_finality_receipt_finality_receipt_sha256_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_finality_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored dais-receipt copy finality_receipt_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_native_consummation_record_sha256_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_consummation_record_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored dais-receipt copy consummation_record_sha256")
        }));
    }
    ...
    {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_completion_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored dais-receipt copy completion_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_decision_packet_anchor_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored dais-receipt copy decision_packet_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_current_authorization_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored dais-receipt copy execute_frozen_result") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_dais_receipt_verdict_drifts() {
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_release_capsule_verdict_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| entry.contains("stored dais-receipt copy verdict")));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_current_live_cutover_controller_summary_drifts(
        ...
        let fixture = fake_pedestal_certificate_latest_fixture(
            "pedestal_certificate_latest_verify_current_live_cutover_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn all_modes_preserve_stage3_and_pre_activation_refusal_semantics_and_do_not_imply_activation_authorization_by_themselves(
        ...
        {
            let fixture = fake_pedestal_certificate_latest_fixture(
                "pedestal_certificate_latest_stage3",
        ...
        {
            let fixture = fake_pedestal_certificate_latest_fixture(
                "pedestal_certificate_latest_pre_activation",
                "refused_now_by_pre_activation_gate",
                "refused_now_by_pre_activation_gate",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        {
            let fixture = fake_pedestal_certificate_latest_fixture(
                "pedestal_certificate_latest_green",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn fake_pedestal_certificate_latest_fixture(
        label: &str,
        pedestal_certificate_latest_run_result: &str,
        handoff_bundle_result: &str,
    ) -> FakePedestalCertificateLatestFixture {
        ...
        let pedestal_certificate_latest_plan_json_path =
            temp_dir.join("pedestal_certificate_latest.plan.json");
        let pedestal_certificate_latest_run_json_path =
            temp_dir.join("pedestal_certificate_latest.run.json");
        let pedestal_certificate_latest_verify_json_path =
            temp_dir.join("pedestal_certificate_latest.verify.json");
        ...
        let fixture = FakePedestalCertificateLatestFixture {
            temp_dir,
            root,
            wrapper_session_dir,
            output_script_path,
            ...
            install_root,
            backend_command,
            pedestal_certificate_latest_plan_json_path,
            pedestal_certificate_latest_run_json_path,
            pedestal_certificate_latest_verify_json_path,
        ...
        persist_json(
            &fixture.pedestal_certificate_latest_plan_json_path,
            &default_pedestal_certificate_latest_plan_report(&fixture),
        )
        .unwrap();
        persist_json(
            &fixture.pedestal_certificate_latest_run_json_path,
            &default_pedestal_certificate_latest_run_report(
                &fixture,
                pedestal_certificate_latest_run_result,
            ),
        )
        .unwrap();
        persist_json(
            &fixture.pedestal_certificate_latest_verify_json_path,
            &default_pedestal_certificate_latest_verify_report(
                &fixture,
                pedestal_certificate_latest_run_result,
            ),
        )
        ...
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        write_fake_pedestal_certificate_latest_command(
            &fixture.bin_dir.join(PEDESTAL_CERTIFICATE_LATEST_BIN),
            &fixture.pedestal_certificate_latest_plan_json_path,
            &fixture.pedestal_certificate_latest_run_json_path,
            &fixture.pedestal_certificate_latest_verify_json_path,
    ...
    fn default_pedestal_certificate_latest_plan_report(
        fixture: &FakePedestalCertificateLatestFixture,
    ) -> Value {
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        json!({
            "generated_at": Utc::now(),
            "mode": "plan_latest_dais_receipt",
            "verdict": PEDESTAL_CERTIFICATE_LATEST_PLAN_READY,
            "reason": "latest immutable chain resolves cleanly into accepted latest-pedestal-certificate contract",
            ...
            "session_dir": paths.nested_pedestal_certificate_latest_session_dir.display().to_string(),
            ...
            "activation_authorized": false,
            "explicit_statement": "latest dais-receipt wrapper plan"
        })
    }

    fn canonical_pedestal_certificate_latest_result(result: &str) -> &str {
        match result {
            "runnable_when_gate_truth_turns_green" => {
                "ready_for_manual_execution_when_gate_turns_green"
            }
    ...
    fn default_pedestal_certificate_latest_run_report(
        fixture: &FakePedestalCertificateLatestFixture,
        result: &str,
    ) -> Value {
        let result = canonical_pedestal_certificate_latest_result(result);
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_pedestal_certificate_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_pedestal_certificate_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_pedestal_certificate_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_latest_dais_receipt",
            "verdict": verdict,
            "reason": format!("latest-pedestal-certificate {result}"),
            "latest_top_step_name": "clerestory_certificate",
            "latest_top_session_dir": fixture.latest_top_session_dir.display().to_string(),
            "historical_execute_frozen_session_dir": fixture.historical_execute_frozen_session_dir.display().to_string(),
            "turn_green_session_dir": fixture.turn_green_session_dir.display().to_string(),
            ...
            "clerestory_certificate_algorithm": "sha256",
            "activation_authorized": false,
            "explicit_statement": "latest dais-receipt wrapper run"
        })
    }

    fn default_pedestal_certificate_latest_verify_report(
        fixture: &FakePedestalCertificateLatestFixture,
        result: &str,
    ) -> Value {
        let mut report = default_pedestal_certificate_latest_run_report(fixture, result);
        report["mode"] = json!("verify_latest_dais_receipt");
        report["verdict"] = json!(PEDESTAL_CERTIFICATE_LATEST_VERIFY_OK);
    ...
    fn default_provenance_certificate_session(
        ...
        let paths = dais_receipt_latest_paths(&fixture.wrapper_session_dir);
        let plinth_receipt_session_dir =
            expected_downstream_pedestal_certificate_session_dir(&paths);
        ...
        let snapshot = LatestPedestalCertificateSnapshot {
        ...
        json!({
            "session_version": "1",
            "planned_at": Utc::now(),
            "pedestal_certificate_session_dir": plinth_receipt_session_dir.display().to_string(),
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": dais_receipt_latest_paths(&fixture.wrapper_session_dir).nested_dais_receipt_session_dir.display().to_string(),
            "verify_pedestal_certificate_command_summary": verify_dais_receipt_input_command_summary(&plinth_receipt_session_dir),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
            ...
            "dais_receipt_summary": "Dais receipt identity: fixture immutable hallmark seal over verified pedestal-certificate truth.",
            "dais_receipt_sha256": "fixture-dais-receipt-sha256",
            "dais_receipt_algorithm": "sha256",
            "explicit_statement": "the verified pedestal-certificate session is the primary input here; this immutable dais receipt freezes one final hallmark identity over the fully culminated chain without enabling production execution"
        })
    }
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_dais_receipt_session_dir.display().to_string(),
            "pedestal_certificate_session_dir": expected_downstream_pedestal_certificate_session_dir(&paths).display().to_string(),
            "plinth_receipt_session_dir": expected_downstream_pedestal_certificate_session_dir(&paths).display().to_string(),
            "substructure_certificate_session_dir": substructure_certificate_session_dir.display().to_string(),
            "basal_receipt_session_dir": basal_receipt_session_dir.display().to_string(),
            "bedrock_certificate_session_dir": bedrock_certificate_session_dir.display().to_string(),
            "foundation_receipt_session_dir": foundation_receipt_session_dir.display().to_string(),
            ...
            "current_pre_activation_gate_verdict": gate_verdict,
            "current_pre_activation_gate_reason": gate_reason,
            "pedestal_certificate_step": Value::Null,
            ...
            "pedestal_certificate_sha256": "fixture-pedestal-certificate-sha256",
            "pedestal_certificate_algorithm": "sha256",
            "dais_receipt_summary": "Dais receipt identity: fixture status immutable hallmark seal over verified pedestal-certificate truth.",
            "dais_receipt_sha256": "fixture-dais-receipt-sha256",
            "dais_receipt_algorithm": "sha256",
            "explicit_statement": "this dais receipt is archival and read-only; it never executes or enables the frozen controller"
        })
    }
    ...
    fn default_provenance_certificate_run_report(
        ...
        let dais_receipt_verdict = match result {
            "ready_for_manual_execution_when_gate_turns_green" => {
                "tiny_live_package_dais_receipt_ready_for_manual_execution_when_gate_turns_green"
            }
            "refused_now_by_stage3" => "tiny_live_package_dais_receipt_refused_now_by_stage3",
            "refused_now_by_pre_activation_gate" => {
                "tiny_live_package_dais_receipt_refused_now_by_pre_activation_gate"
            }
            _ => "tiny_live_package_dais_receipt_refused_now_by_invalid_or_drifted_contract",
        };
        let substructure_certificate_session_dir = fixture
            .temp_dir
            .join("downstream-basal-receipt-substructure-certificate-session");
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_dais_receipt",
            "verdict": dais_receipt_verdict,
            ...
            "release_capsule_session_dir": expected_downstream_pedestal_certificate_session_dir(&paths).display().to_string(),
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_dais_receipt_session_dir.display().to_string(),
            "dais_receipt_session_path": nested_dais_receipt_session_path(&paths.nested_dais_receipt_session_dir).display().to_string(),
            "dais_receipt_status_path": nested_dais_receipt_status_path(&paths.nested_dais_receipt_session_dir).display().to_string(),
            "archived_pedestal_certificate_report_path": paths.nested_dais_receipt_session_dir.join("tiny_live_activation_package_dais_receipt.pedestal_certificate.report.json").display().to_string(),
            "result": result,
            ...
            "pedestal_certificate_step": status["pedestal_certificate_step"],
            "verify_pedestal_certificate_command_summary": verify_dais_receipt_input_command_summary(&expected_downstream_pedestal_certificate_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn write_fake_pedestal_certificate_latest_command(
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-pedestal-certificate session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-pedestal-certificate \"*) mode='plan';;\n  *\" --run-latest-pedestal-certificate \"*) mode='run';;\n  *\" --verify-latest-pedestal-certificate \"*) mode='verify';;\n  *) echo 'unexpected latest-pedestal-certificate mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_pedestal_certificate_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
    ...
    fn overwrite_fake_pedestal_certificate_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        verify_json_path: &Path,
    ...
    fn write_fake_release_capsule_command(
        ...
        let nested_session_path = nested_dais_receipt_session_path(session_dir);
        let nested_status_path = nested_dais_receipt_status_path(session_dir);
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --pedestal-certificate-session-dir {} \"*) : ;;\n  *) echo 'unexpected pedestal-certificate-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected confirm-decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected dais-receipt session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --run-live-package-dais-receipt \"*) mode='run';;\n  *\" --verify-live-package-dais-receipt \"*) mode='verify';;\n  *) echo 'unexpected dais-receipt mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  run) mkdir -p '{}'; cp '{}' '{}'; cp '{}' '{}'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
