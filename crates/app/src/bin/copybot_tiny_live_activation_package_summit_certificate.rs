...
use copybot_app::tiny_live_activation::package_culmination_receipt_summit_certificate;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_summit_certificate --culmination-receipt-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-summit-certificate | --render-live-package-summit-certificate | --run-live-package-summit-certificate | --verify-live-package-summit-certificate)";
const SUMMIT_CERTIFICATE_SESSION_VERSION: &str = "1";
const SUMMIT_CERTIFICATE_STATUS_VERSION: &str = "1";
const SUMMIT_CERTIFICATE_ALGORITHM: &str = "sha256";
const SUMMIT_CERTIFICATE_SESSION_EXPLICIT_STATEMENT: &str =
...
const SUMMIT_CERTIFICATE_STATUS_EXPLICIT_STATEMENT: &str =
    "this summit certificate is archival and read-only; it never executes or enables the frozen controller";
const SUMMIT_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    culmination_receipt_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackageSummitCertificate,
    RenderLivePackageSummitCertificate,
    RunLivePackageSummitCertificate,
    VerifyLivePackageSummitCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageSummitCertificateVerdict {
    TinyLivePackageSummitCertificatePlanReady,
    TinyLivePackageSummitCertificateRendered,
    TinyLivePackageSummitCertificateRefusedNowByStage3,
    TinyLivePackageSummitCertificateRefusedNowByPreActivationGate,
    TinyLivePackageSummitCertificateRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageSummitCertificateReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageSummitCertificateVerifyOk,
    TinyLivePackageSummitCertificateVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageSummitCertificateResult {
    RefusedNowByStage3,
...
struct PackageSummitCertificateStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageSummitCertificateSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageSummitCertificateStatus {
    status_version: String,
    updated_at: DateTime<Utc>,
    session_dir: String,
    culmination_receipt_session_dir: String,
    result: LivePackageSummitCertificateResult,
    reason: String,
    ...
    culmination_receipt_step: Option<PackageSummitCertificateStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageSummitCertificatePaths {
...
struct PackageSummitCertificateReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageSummitCertificateVerdict,
    reason: String,
    culmination_receipt_session_dir: String,
...
struct NestedCulminationReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
struct SummitCertificatePayload<'a> {
    result: &'a str,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut culmination_receipt_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--culmination-receipt-session-dir" => {
                culmination_receipt_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--culmination-receipt-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-summit-certificate" => set_mode(
                &mut mode,
                Mode::PlanLivePackageSummitCertificate,
                arg.as_str(),
            )?,
            "--render-live-package-summit-certificate" => set_mode(
                &mut mode,
                Mode::RenderLivePackageSummitCertificate,
                arg.as_str(),
            )?,
            "--run-live-package-summit-certificate" => set_mode(
                &mut mode,
                Mode::RunLivePackageSummitCertificate,
                arg.as_str(),
            )?,
            "--verify-live-package-summit-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLivePackageSummitCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackageSummitCertificate) && output_path.is_none() {
        bail!("missing --output for --render-live-package-summit-certificate");
    }
    if matches!(
        mode,
        Mode::RunLivePackageSummitCertificate | Mode::VerifyLivePackageSummitCertificate
    ) && session_dir.is_none()
    {
    ...
    let culmination_receipt_session_dir = culmination_receipt_session_dir
        .ok_or_else(|| anyhow!("missing --culmination-receipt-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageSummitCertificate => {
            plan_live_package_summit_certificate_report(&config)?
        }
        Mode::RenderLivePackageSummitCertificate => {
            render_live_package_summit_certificate_report(&config)?
        }
        Mode::RunLivePackageSummitCertificate => {
            run_live_package_summit_certificate_report(&config)?
        }
        Mode::VerifyLivePackageSummitCertificate => {
            verify_live_package_summit_certificate_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_summit_certificate_report(
    config: &Config,
) -> Result<PackageSummitCertificateReport> {
    let (culmination_receipt, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_culmination_receipt_summit_certificate::verify_live_package_culmination_receipt_for_summit_certificate(
                    &config.culmination_receipt_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_consummation_record(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_culmination_receipt_summit_certificate::load_live_package_culmination_receipt_contract_for_summit_certificate(
                            &config.culmination_receipt_session_dir,
                        )?;
                        let classification = classify_consummation_contract(&raw_contract, false);
    ...
    let session_dir = config.session_dir.clone().unwrap_or_else(|| {
        config
            .culmination_receipt_session_dir
            .join("summit-certificate")
    });
    let result = serialize_enum(&classification.0);
    let summit_certificate_sha256 = compute_summit_certificate_sha256(
        &result,
        culmination_receipt.handoff_bundle_result.as_deref(),
        culmination_receipt.decision_packet_result.as_deref(),
        culmination_receipt.execute_frozen_result.as_deref(),
        culmination_receipt
        ...
        culmination_receipt
            ...
            .as_deref(),
        &culmination_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        &culmination_receipt.chain_fingerprint_sha256,
        &culmination_receipt.chain_fingerprint_algorithm,
        &culmination_receipt.release_capsule_digest_manifest_sha256,
        culmination_receipt.release_capsule_digest_manifest_entry_count,
        &culmination_receipt.release_capsule_digest_algorithm,
        &culmination_receipt.ledger_seal_sha256,
        &culmination_receipt.ledger_seal_algorithm,
        &culmination_receipt.registry_entry_sha256,
        &culmination_receipt.registry_entry_algorithm,
        &culmination_receipt.filing_certificate_sha256,
        &culmination_receipt.filing_certificate_algorithm,
        &culmination_receipt.archive_receipt_sha256,
        &culmination_receipt.archive_receipt_algorithm,
        &culmination_receipt.closure_certificate_summary,
        &culmination_receipt.closure_certificate_sha256,
        &culmination_receipt.closure_certificate_algorithm,
        &culmination_receipt.finality_receipt_sha256,
        &culmination_receipt.finality_receipt_algorithm,
        &culmination_receipt.consummation_record_sha256,
        &culmination_receipt.consummation_record_algorithm,
        &culmination_receipt.completion_certificate_summary,
        &culmination_receipt.completion_certificate_sha256,
        &culmination_receipt.completion_certificate_algorithm,
        &culmination_receipt.culmination_receipt_summary,
        &culmination_receipt.culmination_receipt_sha256,
        &culmination_receipt.culmination_receipt_algorithm,
    )?;

    Ok(PackageSummitCertificateReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_summit_certificate".to_string(),
        verdict:
            TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificatePlanReady,
        reason: format!(
            "summit-certificate-native summit certificate is explicit for verified culmination-receipt session {}; this command stays archival and read-only while freezing one final zenith identity over the fully culminated chain",
            config.culmination_receipt_session_dir.display()
        ),
        culmination_receipt_session_dir: config.culmination_receipt_session_dir.display().to_string(),
        registry_entry_session_dir: Some(
            culmination_receipt.registry_entry_session_dir.display().to_string(),
        ),
        ...
        review_receipt_session_dir: Some(
            culmination_receipt.review_receipt_session_dir.display().to_string(),
        ),
        handoff_bundle_session_dir: Some(
            culmination_receipt.handoff_bundle_session_dir.display().to_string(),
        ),
        ...
        turn_green_session_dir: Some(
            culmination_receipt.turn_green_session_dir.display().to_string(),
        ),
        launch_packet_session_dir: Some(
            culmination_receipt.launch_packet_session_dir.display().to_string(),
        ),
        package_dir: Some(culmination_receipt.package_dir.display().to_string()),
        install_root: Some(culmination_receipt.install_root.display().to_string()),
        target_service_name: Some(culmination_receipt.target_service_name.clone()),
        backend_command: Some(culmination_receipt.backend_command.clone()),
        wrapper_timeout_ms: Some(culmination_receipt.wrapper_timeout_ms),
        service_status_max_staleness_ms: Some(culmination_receipt.service_status_max_staleness_ms),
        session_dir: Some(session_dir.display().to_string()),
        summit_certificate_session_path: None,
        summit_certificate_status_path: None,
        archived_culmination_receipt_report_path: None,
        result: Some(result),
        handoff_bundle_result: culmination_receipt.handoff_bundle_result.clone(),
        decision_packet_result: culmination_receipt.decision_packet_result.clone(),
        execute_frozen_result: culmination_receipt.execute_frozen_result.clone(),
        current_pre_activation_gate_verdict: culmination_receipt
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: culmination_receipt
            .current_pre_activation_gate_reason
            .clone(),
        ...
        provenance_certificate_summary: Some(culmination_receipt.provenance_certificate_summary.clone()),
        chain_fingerprint_summary: Some(culmination_receipt.chain_fingerprint_summary.clone()),
        chain_fingerprint_sha256: Some(culmination_receipt.chain_fingerprint_sha256.clone()),
        chain_fingerprint_algorithm: Some(culmination_receipt.chain_fingerprint_algorithm.clone()),
        release_capsule_digest_manifest_sha256: Some(
            culmination_receipt.release_capsule_digest_manifest_sha256.clone(),
        ),
        release_capsule_digest_manifest_entry_count: Some(
            culmination_receipt.release_capsule_digest_manifest_entry_count,
        ),
        release_capsule_digest_algorithm: Some(
            culmination_receipt.release_capsule_digest_algorithm.clone(),
        ),
        notarization_receipt_summary: Some(culmination_receipt.notarization_receipt_summary.clone()),
        ledger_seal_summary: Some(culmination_receipt.ledger_seal_summary.clone()),
        ledger_seal_sha256: Some(culmination_receipt.ledger_seal_sha256.clone()),
        ledger_seal_algorithm: Some(culmination_receipt.ledger_seal_algorithm.clone()),
        registry_entry_summary: Some(culmination_receipt.registry_entry_summary.clone()),
        registry_entry_sha256: Some(culmination_receipt.registry_entry_sha256.clone()),
        registry_entry_algorithm: Some(culmination_receipt.registry_entry_algorithm.clone()),
        filing_certificate_summary: Some(culmination_receipt.filing_certificate_summary.clone()),
        filing_certificate_sha256: Some(culmination_receipt.filing_certificate_sha256.clone()),
        filing_certificate_algorithm: Some(culmination_receipt.filing_certificate_algorithm.clone()),
        archive_receipt_summary: Some(culmination_receipt.archive_receipt_summary.clone()),
        archive_receipt_sha256: Some(culmination_receipt.archive_receipt_sha256.clone()),
        archive_receipt_algorithm: Some(culmination_receipt.archive_receipt_algorithm.clone()),
        closure_certificate_summary: Some(culmination_receipt.closure_certificate_summary.clone()),
        closure_certificate_sha256: Some(culmination_receipt.closure_certificate_sha256.clone()),
        closure_certificate_algorithm: Some(
            culmination_receipt.closure_certificate_algorithm.clone(),
        ),
        finality_receipt_summary: Some(culmination_receipt.finality_receipt_summary.clone()),
        finality_receipt_sha256: Some(culmination_receipt.finality_receipt_sha256.clone()),
        finality_receipt_algorithm: Some(culmination_receipt.finality_receipt_algorithm.clone()),
        consummation_record_summary: Some(culmination_receipt.consummation_record_summary.clone()),
        consummation_record_sha256: Some(culmination_receipt.consummation_record_sha256.clone()),
        consummation_record_algorithm: Some(culmination_receipt.consummation_record_algorithm.clone()),
        completion_certificate_summary: Some(
            culmination_receipt.completion_certificate_summary.clone(),
        ),
        completion_certificate_sha256: Some(
            culmination_receipt.completion_certificate_sha256.clone(),
        ),
        completion_certificate_algorithm: Some(
            culmination_receipt.completion_certificate_algorithm.clone(),
        ),
        culmination_receipt_summary: Some(
            culmination_receipt.culmination_receipt_summary.clone(),
        ),
        culmination_receipt_sha256: Some(
            culmination_receipt.culmination_receipt_sha256.clone(),
        ),
        culmination_receipt_algorithm: Some(
            culmination_receipt.culmination_receipt_algorithm.clone(),
        ),
        summit_certificate_summary: Some(summit_certificate_summary(
            classification.0,
            &culmination_receipt.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        summit_certificate_sha256: Some(summit_certificate_sha256),
        summit_certificate_algorithm: Some(SUMMIT_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: SUMMIT_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_summit_certificate_report(
    config: &Config,
) -> Result<PackageSummitCertificateReport> {
    let plan = plan_live_package_summit_certificate_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_summit_certificate --culmination-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-summit-certificate --json\ncopybot_tiny_live_activation_package_summit_certificate --culmination-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-summit-certificate --json\n",
        shell_escape_path(&config.culmination_receipt_session_dir),
    ...
    Ok(PackageSummitCertificateReport {
        generated_at: Utc::now(),
        mode: "render_live_package_summit_certificate".to_string(),
        verdict:
            TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateRendered,
        reason: format!(
            "rendered summit-certificate run/verify script to {}; the primary input remains the immutable culmination-receipt session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: SUMMIT_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_summit_certificate_report(
    config: &Config,
) -> Result<PackageSummitCertificateReport> {
    ...
    let verified_consummation_record = match package_culmination_receipt_summit_certificate::verify_live_package_culmination_receipt_for_summit_certificate(
        &config.culmination_receipt_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify culmination-receipt session under {}: {error}",
                    config.culmination_receipt_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_consummation_record.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "summit-certificate session dir {} overlaps the managed live target surface derived from verified culmination-receipt install_root {}",
            session_dir.display(),
            verified_consummation_record.contract.install_root.display()
        )
    })?;
    ensure_clean_summit_certificate_session_dir(session_dir)?;
    let paths = summit_certificate_paths(session_dir);
    let classification = classify_verified_consummation_record(&verified_consummation_record);
    let result = classification.0;
    let summit_certificate_sha256 = compute_summit_certificate_sha256(
        &serialize_enum(&result),
        verified_consummation_record
            .contract
        ...
        &verified_consummation_record.contract.ledger_seal_sha256,
        &verified_consummation_record.contract.ledger_seal_algorithm,
        &verified_consummation_record.contract.registry_entry_sha256,
        &verified_consummation_record
            .contract
        ...
        &verified_consummation_record.contract.archive_receipt_sha256,
        &verified_consummation_record
            .contract
    ...
    let session = planned_session(
        config,
        session_dir,
        &verified_consummation_record.contract,
        result,
    ...
    persist_json(
        ...
        &verified_consummation_record.report_json,
    )?;
    let culmination_receipt_step = Some(step_artifact(
        &paths.culmination_receipt_report_path,
        "verify_live_package_culmination_receipt",
        &verified_consummation_record.verdict,
        &verified_consummation_record.reason,
        verified_consummation_record.generated_at,
    ));
    let status = PackageSummitCertificateStatus {
        status_version: SUMMIT_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        handoff_bundle_result: verified_consummation_record
            .contract
            .handoff_bundle_result
            .clone(),
        decision_packet_result: verified_consummation_record
            .contract
            .decision_packet_result
            .clone(),
        execute_frozen_result: verified_consummation_record
            .contract
            .execute_frozen_result
            .clone(),
        current_pre_activation_gate_verdict: verified_consummation_record
            .contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_consummation_record
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        culmination_receipt_step: culmination_receipt_step.clone(),
        provenance_certificate_summary: verified_consummation_record
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_consummation_record
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_consummation_record
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_consummation_record
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_consummation_record
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_consummation_record
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_consummation_record
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_consummation_record
            .contract
            .notarization_receipt_summary
            .clone(),
        ledger_seal_summary: verified_consummation_record
            .contract
            .ledger_seal_summary
            .clone(),
        ledger_seal_sha256: verified_consummation_record
            .contract
            .ledger_seal_sha256
            .clone(),
        ledger_seal_algorithm: verified_consummation_record
            .contract
            .ledger_seal_algorithm
            .clone(),
        registry_entry_summary: verified_consummation_record
            .contract
            .registry_entry_summary
            .clone(),
        registry_entry_sha256: verified_consummation_record
            .contract
            .registry_entry_sha256
            .clone(),
        registry_entry_algorithm: verified_consummation_record
            .contract
            .registry_entry_algorithm
            .clone(),
        filing_certificate_summary: verified_consummation_record
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_consummation_record
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_consummation_record
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_consummation_record
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_consummation_record
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_consummation_record
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_consummation_record
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_consummation_record
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_consummation_record
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_consummation_record
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_consummation_record
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_consummation_record
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_consummation_record
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_consummation_record
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_consummation_record
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_consummation_record
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_consummation_record
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_consummation_record
            .contract
            .completion_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_consummation_record
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_consummation_record
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_consummation_record
            .contract
            .culmination_receipt_algorithm
            .clone(),
        summit_certificate_summary: summit_certificate_summary(
            result,
            &verified_consummation_record
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        summit_certificate_sha256: summit_certificate_sha256.clone(),
        summit_certificate_algorithm: SUMMIT_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: SUMMIT_CERTIFICATE_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_summit_certificate_report(
    config: &Config,
) -> Result<PackageSummitCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = summit_certificate_paths(session_dir);
    let session: PackageSummitCertificateSession = load_json(&paths.session_path)?;
    let status: PackageSummitCertificateStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.culmination_receipt_session_dir,
        &config.culmination_receipt_session_dir.display().to_string(),
        "culmination-receipt culmination_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "summit-certificate session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "summit-certificate status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.culmination_receipt_session_dir,
        &config.culmination_receipt_session_dir.display().to_string(),
        "summit-certificate status culmination_receipt_session_dir",
        &mut mismatches,
    );
    ...
    let stored_culmination_receipt_report: serde_json::Value = load_required_step_json(
        &status.culmination_receipt_step,
        &paths.culmination_receipt_report_path,
        "culmination_receipt_step",
        &mut mismatches,
    )?;
    let stored_culmination_receipt_report_view: NestedCulminationReceiptReportView =
        serde_json::from_value(stored_culmination_receipt_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested culmination-receipt report {}",
                paths.culmination_receipt_report_path.display()
            )
        })?;
    let verified_culmination_receipt_report_view: NestedCulminationReceiptReportView =
        serde_json::from_value(verified_consummation_record.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested culmination-receipt report for {}",
                    config.culmination_receipt_session_dir.display()
                )
            },
        )?;
    compare_json_ignoring_generated_at(
        &stored_culmination_receipt_report,
        &verified_consummation_record.report_json,
        "summit-certificate archived culmination-receipt report json",
        &mut mismatches,
    );
    compare_string(
        &stored_culmination_receipt_report_view.mode,
        &verified_culmination_receipt_report_view.mode,
        "summit-certificate archived culmination-receipt report mode",
        &mut mismatches,
    );
    compare_string(
        &stored_culmination_receipt_report_view.verdict,
        &verified_culmination_receipt_report_view.verdict,
        "summit-certificate archived culmination-receipt report verdict",
        &mut mismatches,
    );
    compare_string(
        &stored_culmination_receipt_report_view.reason,
        &verified_culmination_receipt_report_view.reason,
        "summit-certificate archived culmination-receipt report reason",
        &mut mismatches,
    );
    if stored_culmination_receipt_report_view.generated_at
        != verified_culmination_receipt_report_view.generated_at
    {
        mismatches.push(format!(
            "summit-certificate archived culmination-receipt report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_culmination_receipt_report_view.generated_at,
            verified_culmination_receipt_report_view.generated_at
        ));
    }
    let expected_culmination_receipt_step = step_artifact(
        &paths.culmination_receipt_report_path,
        &verified_culmination_receipt_report_view.mode,
        &verified_culmination_receipt_report_view.verdict,
        &verified_culmination_receipt_report_view.reason,
    ...
    compare_step_artifact(
        status.culmination_receipt_step.as_ref(),
        &expected_culmination_receipt_step,
        "culmination-receipt culmination_receipt_step",
        &mut mismatches,
    );
    if verified_consummation_record.verdict != "tiny_live_package_culmination_receipt_verify_ok" {
        mismatches.push(format!(
    ...
    let classification = classify_verified_consummation_record(&verified_consummation_record);
    let expected_summit_certificate_sha256 = compute_summit_certificate_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_consummation_record.contract,
        classification.0,
        &expected_summit_certificate_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "summit-certificate session contract does not match freshly verified culmination-receipt truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_consummation_record.contract,
        classification,
        expected_culmination_receipt_step.clone(),
        &expected_summit_certificate_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "summit-certificate status contract does not match freshly verified culmination-receipt truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_consummation_record.contract,
        TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateVerifyOk,
        &expected_status,
    ))
}

fn verify_culmination_receipt_command_summary(culmination_receipt_session_dir: &Path) -> String {
    format!(
...
fn classify_consummation_contract(
    contract: &package_culmination_receipt_summit_certificate::LivePackageCulminationReceiptContractView,
    assume_verified: bool,
) -> (
    LivePackageSummitCertificateResult,
    TinyLivePackageSummitCertificateVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageSummitCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateRefusedNowByInvalidOrDriftedContract,
            "the culmination-receipt chain is not verified, so this summit certificate remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageSummitCertificateResult::RefusedNowByStage3,
            TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateRefusedNowByStage3,
            "the immutable culmination receipt remains refused now by Stage 3, so this summit certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageSummitCertificateResult::RefusedNowByPreActivationGate,
            TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateRefusedNowByPreActivationGate,
            "the immutable culmination receipt remains refused now by the pre-activation gate, so this summit certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageSummitCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable culmination receipt is coherent and this summit certificate is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageSummitCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateRefusedNowByInvalidOrDriftedContract,
            "the verified culmination-receipt chain is invalid, drifted, or non-runnable, so this summit certificate remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_consummation_record(
    verified: &package_culmination_receipt_summit_certificate::VerifiedLivePackageCulminationReceiptSummitCertificateStep,
) -> (
    LivePackageSummitCertificateResult,
    TinyLivePackageSummitCertificateVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_culmination_receipt_verify_ok" {
        return (
            LivePackageSummitCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateRefusedNowByInvalidOrDriftedContract,
            "the immutable culmination receipt did not verify cleanly, so this summit certificate remains refused"
                .to_string(),
        );
    }
    classify_consummation_contract(&verified.contract, true)
}

fn summit_certificate_summary(
    result: LivePackageSummitCertificateResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageSummitCertificateResult::RefusedNowByStage3 => {
            "Summit certificate outcome: the immutable completed chain remains refused now by Stage 3; archive this final zenith seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageSummitCertificateResult::RefusedNowByPreActivationGate => {
            "Summit certificate outcome: the immutable completed chain remains refused now by the pre-activation gate; archive this final zenith seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageSummitCertificateResult::RefusedNowByInvalidOrDriftedContract => {
            "Summit certificate outcome: the immutable completed chain is invalid or drifted; archive this final zenith seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackageSummitCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Summit certificate outcome: this immutable zenith seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn summit_certificate_paths(session_dir: &Path) -> PackageSummitCertificatePaths {
    PackageSummitCertificatePaths {
...
fn ensure_clean_summit_certificate_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing summit-certificate session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating summit-certificate session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageSummitCertificateSession {
    PackageSummitCertificateSession {
        session_version: SUMMIT_CERTIFICATE_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        culmination_receipt_session_dir: config
            .culmination_receipt_session_dir
            .display()
        ...
        session_dir: session_dir.display().to_string(),
        verify_culmination_receipt_command_summary: verify_culmination_receipt_command_summary(
            &config.culmination_receipt_session_dir,
        ),
        ...
        summit_certificate_sha256: summit_certificate_sha256.to_string(),
        summit_certificate_algorithm: SUMMIT_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: SUMMIT_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    culmination_receipt_step: PackageSummitCertificateStepArtifact,
    summit_certificate_sha256: &str,
) -> PackageSummitCertificateStatus {
    PackageSummitCertificateStatus {
        status_version: SUMMIT_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        culmination_receipt_step: Some(culmination_receipt_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageSummitCertificatePaths,
    contract: &package_culmination_receipt_summit_certificate::LivePackageCulminationReceiptContractView,
    verdict: TinyLivePackageSummitCertificateVerdict,
    status: &PackageSummitCertificateStatus,
) -> PackageSummitCertificateReport {
    PackageSummitCertificateReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateVerifyOk
        {
            "verify_live_package_summit_certificate".to_string()
        } else {
            "run_live_package_summit_certificate".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        summit_certificate_session_path: Some(paths.session_path.display().to_string()),
        summit_certificate_status_path: Some(paths.status_path.display().to_string()),
        archived_culmination_receipt_report_path: Some(
        ...
        explicit_statement: if verdict
            == TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateVerifyOk
        {
            SUMMIT_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageSummitCertificateReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageSummitCertificate => {
                "plan_live_package_summit_certificate".to_string()
            }
            Mode::RenderLivePackageSummitCertificate => {
                "render_live_package_summit_certificate".to_string()
            }
            Mode::RunLivePackageSummitCertificate => {
                "run_live_package_summit_certificate".to_string()
            }
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable summit certificate refused before persisting because the culmination-receipt artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "summit-certificate-native summit-certificate verification failed".to_string()
    });
    PackageSummitCertificateReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_summit_certificate".to_string(),
        verdict:
        ...
        explicit_statement: SUMMIT_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageSummitCertificateStepArtifact>,
    expected_path: &Path,
    label: &str,
    mismatches: &mut Vec<String>,
) -> Result<serde_json::Value> {
    let Some(step) = step else {
        mismatches.push(format!("{label} is missing from summit-certificate status"));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageSummitCertificateStepArtifact {
    PackageSummitCertificateStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageSummitCertificateStepArtifact>,
    expected: &PackageSummitCertificateStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_summit_certificate_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&SummitCertificatePayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing summit-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackageSummitCertificateReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "culmination_receipt_session_dir={}",
            report.culmination_receipt_session_dir
        ),
    ];
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_summit_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageSummitCertificateVerdict::TinyLivePackageSummitCertificateReadyForManualExecutionWhenGateTurnsGreen
        );
    ...
    fn drifted_finality_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            ...
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_culmination_receipt",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        let error = run_live_package_summit_certificate_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = summit_certificate_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_culmination_receipt_step_path() {
        let fixture = fake_command_fixture(
        ...
        status.culmination_receipt_step.as_mut().unwrap().path =
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = summit_certificate_paths(&fixture.summit_certificate_session_dir);
        let mut status: PackageSummitCertificateStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageSummitCertificateResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    fn verify_rejects_tampered_nested_finality_receipt_report_content() {
        ...
        let mut report: serde_json::Value =
            load_json(&paths.culmination_receipt_report_path).unwrap();
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageSummitCertificate,
                culmination_receipt_session_dir: self.culmination_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.summit_certificate_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageSummitCertificate,
                culmination_receipt_session_dir: self.culmination_receipt_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        let completion_certificate_session_dir = fixture_dir.join("completion-certificate-session");
        ...
        persist_json(
            &culmination_receipt_session_dir
                .join("tiny_live_activation_package_culmination_receipt.session.json"),
        ...
        persist_json(
            &culmination_receipt_session_dir
                .join("tiny_live_activation_package_culmination_receipt.status.json"),
            &stored_culmination_receipt_status_fixture(&verify_report),
        )
        .unwrap();
        persist_json(
            &culmination_receipt_session_dir.join(
        ...
        let verify_path = fixture_dir.join("culmination-receipt.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_culmination_receipt_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_culmination_receipt"),
            &verify_path,
            &completion_certificate_session_dir,
            &decision_packet_session_dir,
            &culmination_receipt_session_dir,
    ...
    fn bind_culmination_receipt_report_to_fixture(
        ...
        object.insert(
            "completion_certificate_session_dir".to_string(),
            json!(completion_certificate_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            "verify_completion_certificate_command_summary".to_string(),
            json!(format!(
                "verify immutable culmination-receipt session under {} before freezing the final summit certificate",
                culmination_receipt_session_dir.display()
            )),
    ...
    fn culmination_receipt_verify_report(
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_culmination_receipt",
            "verdict": "tiny_live_package_culmination_receipt_verify_ok",
            "reason": "culmination-receipt chain is coherent",
            "result": result,
    ...
    fn write_fake_culmination_receipt_verify_command(
        path: &Path,
        verify_path: &Path,
        expected_completion_certificate_session_dir: &Path,
        expected_decision_packet_session_dir: &Path,
        culmination_receipt_session_dir: &Path,
    ) {
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --completion-certificate-session-dir {} \"*) : ;;\n  *) echo 'unexpected completion-certificate-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected culmination-receipt session dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --verify-live-package-culmination-receipt \"*) : ;;\n  *) echo 'missing culmination verify mode' >&2; exit 1;;\nesac\ncat '{}'\n",
            shell_escape_path(expected_completion_certificate_session_dir),
