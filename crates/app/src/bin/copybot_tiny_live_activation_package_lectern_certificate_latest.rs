...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_lectern_certificate_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-lectern-certificate | --render-latest-lectern-certificate-script | --run-latest-lectern-certificate | --verify-latest-lectern-certificate)";
const LECTERN_CERTIFICATE_LATEST_SESSION_VERSION: &str = "1";
const LECTERN_CERTIFICATE_LATEST_STATUS_VERSION: &str = "1";
const LECTERN_CERTIFICATE_LATEST_BIN: &str =
    "copybot_tiny_live_activation_package_lectern_certificate_latest";
const PODIUM_RECEIPT_LATEST_BIN: &str =
    "copybot_tiny_live_activation_package_podium_receipt_latest";
const LECTERN_CERTIFICATE_BIN: &str = "copybot_tiny_live_activation_package_lectern_certificate";
const PODIUM_RECEIPT_LATEST_PLAN_READY: &str = "tiny_live_package_podium_receipt_latest_plan_ready";
const PODIUM_RECEIPT_LATEST_VERIFY_OK: &str = "tiny_live_package_podium_receipt_latest_verify_ok";
const LECTERN_CERTIFICATE_VERIFY_OK: &str = "tiny_live_package_lectern_certificate_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-lectern-certificate handoff binds the latest persisted immutable package chain to the accepted lectern-certificate contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestLecternCertificate,
    RenderLatestLecternCertificateScript,
    RunLatestLecternCertificate,
    VerifyLatestLecternCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageLecternCertificateLatestVerdict {
    TinyLivePackageLecternCertificateLatestPlanReady,
    TinyLivePackageLecternCertificateLatestScriptRendered,
    TinyLivePackageLecternCertificateLatestRefusedByMissingLatestChain,
    TinyLivePackageLecternCertificateLatestRefusedByInvalidLatestChain,
    TinyLivePackageLecternCertificateLatestRefusedByDriftedLecternCertificateContract,
    TinyLivePackageLecternCertificateLatestRefusedNowByStage3,
    TinyLivePackageLecternCertificateLatestRefusedNowByPreActivationGate,
    TinyLivePackageLecternCertificateLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageLecternCertificateLatestVerifyOk,
    TinyLivePackageLecternCertificateLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageLecternCertificateLatestResult {
    RefusedByMissingLatestChain,
    RefusedByInvalidLatestChain,
    RefusedByDriftedLecternCertificateContract,
    RefusedNowByStage3,
...
struct PackageLecternCertificateLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageLecternCertificateLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_podium_receipt_latest_session_dir: String,
    nested_lectern_certificate_session_dir: String,
    ...
    resolve_latest_podium_receipt_command_summary: String,
    run_latest_lectern_certificate_command_summary: String,
    verify_latest_lectern_certificate_command_summary: String,
    downstream_lectern_certificate_command_summary: String,
    verify_lectern_certificate_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageLecternCertificateLatestStatus {
    ...
    nested_lectern_certificate_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackageLecternCertificateLatestResult,
    reason: String,
    latest_podium_receipt_plan_verdict: Option<String>,
    latest_podium_receipt_verdict: Option<String>,
    lectern_certificate_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_podium_receipt_plan_step: Option<PackageLecternCertificateLatestStepArtifact>,
    latest_podium_receipt_step: Option<PackageLecternCertificateLatestStepArtifact>,
    lectern_certificate_step: Option<PackageLecternCertificateLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackageLecternCertificateLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_podium_receipt_plan_report_path: PathBuf,
    latest_podium_receipt_report_path: PathBuf,
    lectern_certificate_report_path: PathBuf,
    nested_podium_receipt_latest_session_dir: PathBuf,
    nested_lectern_certificate_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageLecternCertificateLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageLecternCertificateLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    lectern_certificate_latest_session_path: Option<String>,
    lectern_certificate_latest_status_path: Option<String>,
    lectern_certificate_latest_report_path: Option<String>,
    latest_podium_receipt_plan_report_path: Option<String>,
    latest_podium_receipt_report_path: Option<String>,
    lectern_certificate_report_path: Option<String>,
    nested_podium_receipt_latest_session_dir: Option<String>,
    nested_lectern_certificate_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_podium_receipt_command_summary: Option<String>,
    run_latest_lectern_certificate_command_summary: Option<String>,
    verify_latest_lectern_certificate_command_summary: Option<String>,
    downstream_lectern_certificate_command_summary: Option<String>,
    verify_lectern_certificate_command_summary: Option<String>,
    reviewed_frozen_live_cutover_controller_command_summary: Option<String>,
    clerestory_certificate_summary: Option<String>,
    clerestory_certificate_sha256: Option<String>,
    clerestory_certificate_algorithm: Option<String>,
...
#[derive(Debug, Clone)]
struct LatestPodiumReceiptSnapshot {
...
#[derive(Debug, Clone)]
struct ResolvedLatestPodiumReceipt {
    snapshot: LatestPodiumReceiptSnapshot,
    latest_podium_receipt_plan_raw: Value,
    latest_podium_receipt_plan: PodiumReceiptLatestReportView,
}
...
enum ResolutionFailureKind {
    MissingLatestChain,
    InvalidLatestChain,
    DriftedPodiumReceiptContract,
}

#[derive(Debug, Clone)]
struct LatestPodiumReceiptResolutionFailure {
    kind: ResolutionFailureKind,
    reason: String,
    snapshot: Option<LatestPodiumReceiptSnapshot>,
    latest_podium_receipt_plan_raw: Option<Value>,
    latest_podium_receipt_plan_verdict: Option<String>,
    latest_podium_receipt_plan_reason: Option<String>,
    latest_podium_receipt_plan_generated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone, Deserialize)]
struct PodiumReceiptLatestReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Deserialize)]
struct LecternCertificateReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    podium_receipt_session_dir: String,
    ...
    service_status_max_staleness_ms: Option<u64>,
    session_dir: Option<String>,
    lectern_certificate_session_path: Option<String>,
    lectern_certificate_status_path: Option<String>,
    archived_podium_receipt_report_path: Option<String>,
    result: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    podium_receipt_step: Option<PackageLecternCertificateLatestStepArtifact>,
...
#[derive(Debug, Clone, Deserialize)]
struct LecternCertificateSessionView {
    podium_receipt_session_dir: String,
...
#[derive(Debug, Clone, Deserialize)]
struct LecternCertificateStatusView {
    session_dir: String,
    podium_receipt_session_dir: String,
    pedestal_certificate_session_dir: String,
    plinth_receipt_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-lectern-certificate" => {
                set_mode(&mut mode, Mode::PlanLatestLecternCertificate, arg.as_str())?
            }
            "--render-latest-lectern-certificate-script" => set_mode(
                &mut mode,
                Mode::RenderLatestLecternCertificateScript,
                arg.as_str(),
            )?,
            "--run-latest-lectern-certificate" => {
                set_mode(&mut mode, Mode::RunLatestLecternCertificate, arg.as_str())?
            }
            "--verify-latest-lectern-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLatestLecternCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(
        mode,
        Mode::PlanLatestLecternCertificate
            | Mode::RenderLatestLecternCertificateScript
            | Mode::RunLatestLecternCertificate
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestLecternCertificateScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestLecternCertificate | Mode::VerifyLatestLecternCertificate
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestLecternCertificate => plan_latest_lectern_certificate_report(&config)?,
        Mode::RenderLatestLecternCertificateScript => {
            render_latest_lectern_certificate_script_report(&config)?
        }
        Mode::RunLatestLecternCertificate => run_latest_lectern_certificate_report(&config)?,
        Mode::VerifyLatestLecternCertificate => verify_latest_lectern_certificate_report(&config)?,
    };
    if config.json {
...
fn plan_latest_lectern_certificate_report(
    config: &Config,
) -> Result<PackageLecternCertificateLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    let suggested_session_dir = config.session_dir.clone().unwrap_or_else(|| {
        root.join("tiny_live_activation_package_lectern_certificate_latest.session")
    });
    let paths = lectern_certificate_latest_paths(&suggested_session_dir);

    match resolve_latest_podium_receipt(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            "plan_latest_lectern_certificate",
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-podium-receipt lineage via accepted latest-podium-receipt session {}, exact decision-packet confirmation anchor {}, and the accepted lectern-certificate contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_podium_receipt_latest_session_dir.display(),
...
fn render_latest_lectern_certificate_script_report(
    config: &Config,
) -> Result<PackageLecternCertificateLatestReport> {
    let root = config
        .root
    ...
    match resolve_latest_podium_receipt(root, &paths) {
        Ok(resolved) => {
            write_script(
                output_path,
                &format!(
                    "#!/usr/bin/env bash\nset -euo pipefail\n\n{run_latest}\n{verify_latest}\n{run_lectern_certificate}\n{verify_lectern_certificate}\n",
                    run_latest = run_latest_lectern_certificate_command_summary(
                        root,
                        &suggested_session_dir,
                    ),
                    verify_latest =
                        verify_latest_lectern_certificate_command_summary(&suggested_session_dir),
                    run_lectern_certificate = downstream_lectern_certificate_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_podium_receipt_session_dir(&paths),
                        &paths.nested_lectern_certificate_session_dir,
                    ),
                    verify_lectern_certificate = verify_lectern_certificate_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_podium_receipt_session_dir(&paths),
                        &paths.nested_lectern_certificate_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                "render_latest_lectern_certificate_script",
                TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestScriptRendered,
                format!(
                    "rendered latest-lectern-certificate handoff script to {} using the accepted latest-podium-receipt lineage and the accepted lectern-certificate contract",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_lectern_certificate_report(
    config: &Config,
) -> Result<PackageLecternCertificateLatestReport> {
    let root = config
        .root
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-lectern-certificate session dir {}",
            session_dir.display()
        );
    }
    let paths = lectern_certificate_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-lectern-certificate")?;

    let resolution = resolve_latest_podium_receipt(root, &paths);
    match resolution {
        Ok(resolved) => {
            run_latest_lectern_certificate_with_resolution(root, session_dir, &paths, &resolved)
        }
        Err(failure) => run_latest_lectern_certificate_failure(root, session_dir, &paths, &failure),
    }
}

fn verify_latest_lectern_certificate_report(
    config: &Config,
) -> Result<PackageLecternCertificateLatestReport> {
    let session_dir = config
        .session_dir
    ...
    let session: PackageLecternCertificateLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_lectern_certificate_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-lectern-certificate session artifact {}: {error}",
    ...
    let status: PackageLecternCertificateLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_lectern_certificate_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-lectern-certificate status artifact {}: {error}",
    ...
    let stored_report: PackageLecternCertificateLatestReport = match load_json(&paths.report_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_lectern_certificate_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-lectern-certificate report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-lectern-certificate root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-lectern-certificate session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-lectern-certificate status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-lectern-certificate report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_podium_receipt_latest_session_dir,
        &paths
            .nested_podium_receipt_latest_session_dir
            .display()
            .to_string(),
        "latest-lectern-certificate session nested_podium_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_podium_receipt_latest_session_dir,
        &paths
            .nested_podium_receipt_latest_session_dir
            .display()
            .to_string(),
        "latest-lectern-certificate status nested_podium_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-lectern-certificate report nested_podium_receipt_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_lectern_certificate_session_dir,
        &paths
            .nested_lectern_certificate_session_dir
            .display()
            .to_string(),
        "latest-lectern-certificate session nested_lectern_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_lectern_certificate_session_dir,
        &paths
            .nested_lectern_certificate_session_dir
            .display()
            .to_string(),
        "latest-lectern-certificate status nested_lectern_certificate_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-lectern-certificate report nested_lectern_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-lectern-certificate session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-lectern-certificate status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-lectern-certificate report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .lectern_certificate_latest_session_path
            .as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-lectern-certificate report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .lectern_certificate_latest_status_path
            .as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-lectern-certificate report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .lectern_certificate_latest_report_path
            .as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-lectern-certificate report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-lectern-certificate session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-lectern-certificate status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-lectern-certificate report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_lectern_certificate",
        "latest-lectern-certificate stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-lectern-certificate stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-lectern-certificate stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-lectern-certificate stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-lectern-certificate stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report.latest_podium_receipt_plan_verdict.as_deref(),
        status.latest_podium_receipt_plan_verdict.as_deref(),
        "stored report latest_podium_receipt_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.latest_podium_receipt_verdict.as_deref(),
        status.latest_podium_receipt_verdict.as_deref(),
        "stored report latest_podium_receipt_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.lectern_certificate_verdict.as_deref(),
        status.lectern_certificate_verdict.as_deref(),
        "stored report lectern_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: PodiumReceiptLatestReportView = load_required_step_json(
                &status.latest_podium_receipt_plan_step,
                &paths.latest_podium_receipt_plan_report_path,
                "latest_podium_receipt_plan_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_podium_receipt_plan_step.as_ref(),
                &paths.latest_podium_receipt_plan_report_path,
                "plan_latest_podium_receipt",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_podium_receipt_plan_step",
                &mut mismatches,
            );
            compare_podium_receipt_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_podium_receipt_latest_session_dir,
                &mut mismatches,
            );
            compare_option_string(
                status.latest_podium_receipt_plan_verdict.as_deref(),
                Some(stored_latest_plan.verdict.as_str()),
                "stored status latest_podium_receipt_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_podium_receipt_plan_verdict.as_deref(),
            ...
            let stored_latest_run: PodiumReceiptLatestReportView = load_required_step_json(
                &status.latest_podium_receipt_step,
                &paths.latest_podium_receipt_report_path,
                "latest_podium_receipt_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_podium_receipt_step.as_ref(),
                &paths.latest_podium_receipt_report_path,
                "run_latest_podium_receipt",
                &stored_latest_run.verdict,
                &stored_latest_run.reason,
                stored_latest_run.generated_at,
                "stored latest_podium_receipt_step",
                &mut mismatches,
            );
            compare_podium_receipt_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_podium_receipt_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_podium_receipt_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_podium_receipt_verdict.as_deref(),
            ...
            let actual_latest_run: PodiumReceiptLatestReportView = match load_json(
                &nested_podium_receipt_latest_report_path(
                    &paths.nested_podium_receipt_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed loading actual nested latest-podium-receipt report under {}: {error}",
                        paths.nested_podium_receipt_latest_session_dir.display()
                    ));
                    fallback_podium_receipt_latest_copy()
                }
            };
            compare_podium_receipt_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                PODIUM_RECEIPT_LATEST_BIN,
                &latest_podium_receipt_verify_args(&paths.nested_podium_receipt_latest_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-podium-receipt session under {}: {error}",
                        paths.nested_podium_receipt_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: PodiumReceiptLatestReportView =
                    serde_json::from_value(latest_verify_raw)
                        .context("failed parsing nested latest-podium-receipt verify json")?;
                if latest_verify.verdict != PODIUM_RECEIPT_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-podium-receipt verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: LecternCertificateReportView = load_required_step_json(
                &status.lectern_certificate_step,
                &paths.lectern_certificate_report_path,
                "lectern_certificate_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.lectern_certificate_step.as_ref(),
                &paths.lectern_certificate_report_path,
                "run_live_package_lectern_certificate",
                &stored_handoff_bundle.verdict,
                &stored_handoff_bundle.reason,
                stored_handoff_bundle.generated_at,
                "stored lectern_certificate_step",
                &mut mismatches,
            );
            compare_lectern_certificate_run_against_snapshot(
            ...
            compare_option_string(
                ...
                "stored status lectern_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.lectern_certificate_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: LecternCertificateSessionView =
                match load_json(&nested_lectern_certificate_session_path(
                    &paths.nested_lectern_certificate_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                            "failed reading nested lectern-certificate session under {}: {error}",
                            paths.nested_lectern_certificate_session_dir.display()
                        ));
            ...
            let nested_handoff_bundle_status: LecternCertificateStatusView =
                match load_json(&nested_lectern_certificate_status_path(
                    &paths.nested_lectern_certificate_session_dir,
                )) {
                    Ok(value) => value,
                    Err(error) => {
                        mismatches.push(format!(
                            "failed reading nested lectern-certificate status under {}: {error}",
                            paths.nested_lectern_certificate_session_dir.display()
                        ));
            ...
            compare_lectern_certificate_copy_against_nested_truth(
                &stored_handoff_bundle,
            ...
            let handoff_bundle_verify_raw = match run_json_command(
                LECTERN_CERTIFICATE_BIN,
                &lectern_certificate_verify_args(
                    &resolved.snapshot,
                    &expected_downstream_podium_receipt_session_dir(&paths),
                    &paths.nested_lectern_certificate_session_dir,
                ),
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested lectern-certificate session under {}: {error}",
                        paths.nested_lectern_certificate_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: LecternCertificateReportView =
                    serde_json::from_value(handoff_bundle_verify_raw)
                        .context("failed parsing nested lectern-certificate verify json")?;
                compare_option_string(
                    stored_handoff_bundle.registry_entry_session_dir.as_deref(),
                    handoff_bundle_verify.registry_entry_session_dir.as_deref(),
                    "stored lectern-certificate copy registry_entry_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored lectern-certificate copy notarization_receipt_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .consummation_record_algorithm
                        .as_deref(),
                    "stored lectern-certificate copy consummation_record_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .completion_certificate_summary
                        .as_deref(),
                    "stored lectern-certificate copy completion_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .completion_certificate_sha256
                        .as_deref(),
                    "stored lectern-certificate copy completion_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored lectern-certificate copy completion_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.culmination_receipt_summary.as_deref(),
                    handoff_bundle_verify.culmination_receipt_summary.as_deref(),
                    "stored lectern-certificate copy culmination_receipt_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.culmination_receipt_sha256.as_deref(),
                    handoff_bundle_verify.culmination_receipt_sha256.as_deref(),
                    "stored lectern-certificate copy culmination_receipt_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .culmination_receipt_algorithm
                        .as_deref(),
                    "stored lectern-certificate copy culmination_receipt_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.summit_certificate_summary.as_deref(),
                    handoff_bundle_verify.summit_certificate_summary.as_deref(),
                    "stored lectern-certificate copy summit_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.summit_certificate_sha256.as_deref(),
                    handoff_bundle_verify.summit_certificate_sha256.as_deref(),
                    "stored lectern-certificate copy summit_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_summary
                        .as_deref(),
                    "stored lectern-certificate copy cornerstone_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_sha256
                        .as_deref(),
                    "stored lectern-certificate copy cornerstone_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .cornerstone_certificate_algorithm
                        .as_deref(),
                    "stored lectern-certificate copy cornerstone_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .capstone_certificate_summary
                        .as_deref(),
                    "stored lectern-certificate copy capstone_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.capstone_certificate_sha256.as_deref(),
                    handoff_bundle_verify.capstone_certificate_sha256.as_deref(),
                    "stored lectern-certificate copy capstone_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .capstone_certificate_algorithm
                        .as_deref(),
                    "stored lectern-certificate copy capstone_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.foundation_receipt_summary.as_deref(),
                    handoff_bundle_verify.foundation_receipt_summary.as_deref(),
                    "stored lectern-certificate copy foundation_receipt_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.foundation_receipt_sha256.as_deref(),
                    handoff_bundle_verify.foundation_receipt_sha256.as_deref(),
                    "stored lectern-certificate copy foundation_receipt_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .foundation_receipt_algorithm
                        .as_deref(),
                    "stored lectern-certificate copy foundation_receipt_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.bedrock_certificate_summary.as_deref(),
                    handoff_bundle_verify.bedrock_certificate_summary.as_deref(),
                    "stored lectern-certificate copy bedrock_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.bedrock_certificate_sha256.as_deref(),
                    handoff_bundle_verify.bedrock_certificate_sha256.as_deref(),
                    "stored lectern-certificate copy bedrock_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .substructure_certificate_summary
                        .as_deref(),
                    "stored lectern-certificate copy substructure_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .substructure_certificate_sha256
                        .as_deref(),
                    "stored lectern-certificate copy substructure_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .pedestal_certificate_summary
                        .as_deref(),
                    "stored lectern-certificate copy pedestal_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.pedestal_certificate_sha256.as_deref(),
                    handoff_bundle_verify.pedestal_certificate_sha256.as_deref(),
                    "stored lectern-certificate copy pedestal_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .pedestal_certificate_algorithm
                        .as_deref(),
                    "stored lectern-certificate copy pedestal_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.podium_receipt_summary.as_deref(),
                    handoff_bundle_verify.podium_receipt_summary.as_deref(),
                    "stored lectern-certificate copy podium_receipt_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.podium_receipt_sha256.as_deref(),
                    handoff_bundle_verify.podium_receipt_sha256.as_deref(),
                    "stored lectern-certificate copy podium_receipt_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.podium_receipt_algorithm.as_deref(),
                    handoff_bundle_verify.podium_receipt_algorithm.as_deref(),
                    "stored lectern-certificate copy podium_receipt_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .reviewed_frozen_live_cutover_controller_command_summary
                        .as_deref(),
                    "stored lectern-certificate copy reviewed_frozen_live_cutover_controller_command_summary",
                    &mut mismatches,
                );
                if handoff_bundle_verify.verdict != LECTERN_CERTIFICATE_VERIFY_OK {
                    mismatches.push(format!(
                        "nested lectern-certificate verification is non-green: {}",
                        handoff_bundle_verify.reason
                    ));
                }
            }

            let expected_result = podium_receipt_latest_result_from_nested_status(
                nested_handoff_bundle_status.result.as_str(),
            )
            .unwrap_or(LivePackageLecternCertificateLatestResult::RefusedByDriftedLecternCertificateContract);
            if status.result != expected_result {
                mismatches.push(format!(
                    "stored latest-lectern-certificate result {} does not match nested lectern-certificate result {}",
            ...
            if status.activation_authorized {
                mismatches.push(
                    "stored status activation_authorized must remain false for latest-lectern-certificate"
                        .to_string(),
                );
            }
            compare_option_string(
        ...
        Err(failure) => {
            ...
            if failure.latest_podium_receipt_plan_raw.is_some() {
                let stored_latest_plan: PodiumReceiptLatestReportView = load_required_step_json(
                    &status.latest_podium_receipt_plan_step,
                    &paths.latest_podium_receipt_plan_report_path,
                    "latest_podium_receipt_plan_step",
                ...
                compare_option_string(
                    status.latest_podium_receipt_plan_verdict.as_deref(),
                    failure.latest_podium_receipt_plan_verdict.as_deref(),
                    "stored status latest_podium_receipt_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report.latest_podium_receipt_plan_verdict.as_deref(),
                    failure.latest_podium_receipt_plan_verdict.as_deref(),
                    "stored report latest_podium_receipt_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
                status.latest_podium_receipt_verdict.as_deref(),
                None,
                "stored status latest_podium_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_podium_receipt_verdict.as_deref(),
                None,
                "stored report latest_podium_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.lectern_certificate_verdict.as_deref(),
                None,
                "stored status lectern_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.lectern_certificate_verdict.as_deref(),
                None,
                "stored report lectern_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestVerifyOk
    } else {
        TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-lectern-certificate session under {} is coherent with the current latest immutable chain and the accepted lectern-certificate contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-lectern-certificate verification failed".to_string())
    };

    Ok(PackageLecternCertificateLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_lectern_certificate".to_string(),
        verdict,
        reason,
        ...
        lectern_certificate_latest_session_path: Some(paths.session_path.display().to_string()),
        lectern_certificate_latest_status_path: Some(paths.status_path.display().to_string()),
        lectern_certificate_latest_report_path: Some(paths.report_path.display().to_string()),
        latest_podium_receipt_plan_report_path: Some(
            paths
                .latest_podium_receipt_plan_report_path
                .display()
                .to_string(),
        ),
        latest_podium_receipt_report_path: Some(
            paths.latest_podium_receipt_report_path.display().to_string(),
        ),
        lectern_certificate_report_path: Some(paths.lectern_certificate_report_path.display().to_string()),
        nested_podium_receipt_latest_session_dir: Some(
            paths
                .nested_podium_receipt_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_lectern_certificate_session_dir: Some(
            paths.nested_lectern_certificate_session_dir.display().to_string(),
        ),
        ...
        latest_podium_receipt_plan_verdict: status.latest_podium_receipt_plan_verdict.clone(),
        latest_podium_receipt_verdict: status.latest_podium_receipt_verdict.clone(),
        lectern_certificate_verdict: status.lectern_certificate_verdict.clone(),
        ...
        resolve_latest_podium_receipt_command_summary: Some(
            session.resolve_latest_podium_receipt_command_summary.clone(),
        ),
        run_latest_lectern_certificate_command_summary: Some(
            session.run_latest_lectern_certificate_command_summary.clone(),
        ),
        verify_latest_lectern_certificate_command_summary: Some(
            session.verify_latest_lectern_certificate_command_summary.clone(),
        ),
        downstream_lectern_certificate_command_summary: Some(
            session.downstream_lectern_certificate_command_summary.clone(),
        ),
        verify_lectern_certificate_command_summary: Some(
            session.verify_lectern_certificate_command_summary.clone(),
        ),
        ...
        explicit_statement:
            "this verification proves only whether the latest immutable chain still resolves cleanly into the accepted lectern-certificate contract; it never runs the frozen controller"
                .to_string(),
    })
}

fn run_latest_lectern_certificate_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackageLecternCertificateLatestPaths,
    resolved: &ResolvedLatestPodiumReceipt,
) -> Result<PackageLecternCertificateLatestReport> {
    let session = planned_session(root, session_dir, paths, Some(resolved), None);
    persist_json(&paths.session_path, &session)?;
    persist_json(
        &paths.latest_podium_receipt_plan_report_path,
        &resolved.latest_podium_receipt_plan_raw,
    )?;

    let latest_podium_receipt_plan_step = Some(step_artifact(
        &paths.latest_podium_receipt_plan_report_path,
        "plan_latest_podium_receipt",
        &resolved.latest_podium_receipt_plan.verdict,
        &resolved.latest_podium_receipt_plan.reason,
        resolved.latest_podium_receipt_plan.generated_at,
    ));

    let latest_podium_receipt_run_raw = match run_json_command(
        PODIUM_RECEIPT_LATEST_BIN,
        &latest_podium_receipt_run_args(root, &paths.nested_podium_receipt_latest_session_dir),
    ) {
        Ok(value) => value,
        Err(error) => {
            let status = refusal_status(
                ...
                paths,
                Some(resolved),
                LivePackageLecternCertificateLatestResult::RefusedByDriftedLecternCertificateContract,
                format!(
                    "failed running nested latest-podium-receipt handoff under {}: {error}",
                    paths
                        .nested_podium_receipt_latest_session_dir
                        .display()
                ),
    ...
    persist_json(
        &paths.latest_podium_receipt_report_path,
        &latest_podium_receipt_run_raw,
    )?;
    let latest_podium_receipt_run: PodiumReceiptLatestReportView =
        serde_json::from_value(latest_podium_receipt_run_raw)
            .context("failed parsing nested latest-podium-receipt run json")?;
    let latest_podium_receipt_step = Some(step_artifact(
        &paths.latest_podium_receipt_report_path,
        "run_latest_podium_receipt",
        &latest_podium_receipt_run.verdict,
        &latest_podium_receipt_run.reason,
        latest_podium_receipt_run.generated_at,
    ));

    if let Some(terminal_failure) =
        latest_podium_receipt_terminal_failure(&latest_podium_receipt_run)
    {
        let status = refusal_status(
            root,
            session_dir,
            ...
            Some(resolved),
            terminal_failure,
            latest_podium_receipt_run.reason.clone(),
            latest_podium_receipt_plan_step,
            latest_podium_receipt_step,
            None,
        );
    ...
    let latest_podium_receipt_verify: PodiumReceiptLatestReportView = match run_json_command(
        PODIUM_RECEIPT_LATEST_BIN,
        &latest_podium_receipt_verify_args(&paths.nested_podium_receipt_latest_session_dir),
    ) {
        ...
        Err(error) => {
            let status = refusal_status(
                    ...
                    LivePackageLecternCertificateLatestResult::RefusedByDriftedLecternCertificateContract,
                    format!(
                    "failed verifying nested latest-podium-receipt handoff under {}: {error}",
                    paths.nested_podium_receipt_latest_session_dir.display()
                ),
                    latest_podium_receipt_plan_step,
                    latest_podium_receipt_step,
    ...
    if latest_podium_receipt_verify.verdict != PODIUM_RECEIPT_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            LivePackageLecternCertificateLatestResult::RefusedByDriftedLecternCertificateContract,
            latest_podium_receipt_verify.reason,
            latest_podium_receipt_plan_step,
            latest_podium_receipt_step,
            None,
        );
    ...
    let release_capsule_run_raw = match run_json_command(
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageLecternCertificateLatestResult::RefusedByDriftedLecternCertificateContract,
                format!(
                    "failed running downstream lectern-certificate contract under {}: {error}",
                    paths.nested_lectern_certificate_session_dir.display()
                ),
                latest_podium_receipt_plan_step,
                latest_podium_receipt_step,
    ...
    let provenance_certificate_run: LecternCertificateReportView =
        serde_json::from_value(release_capsule_run_raw)
            .context("failed parsing nested lectern-certificate run json")?;
    let lectern_certificate_step = Some(step_artifact(
        &paths.lectern_certificate_report_path,
        "run_live_package_lectern_certificate",
    ...
    let provenance_certificate_verify: LecternCertificateReportView = match run_json_command(
        LECTERN_CERTIFICATE_BIN,
        &lectern_certificate_verify_args(
            &resolved.snapshot,
            &expected_downstream_podium_receipt_session_dir(paths),
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageLecternCertificateLatestResult::RefusedByDriftedLecternCertificateContract,
                format!(
                    "failed verifying downstream lectern-certificate contract under {}: {error}",
                    paths.nested_lectern_certificate_session_dir.display()
                ),
                latest_podium_receipt_plan_step,
                latest_podium_receipt_step,
    ...
    if provenance_certificate_verify.verdict != LECTERN_CERTIFICATE_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    let result = podium_receipt_latest_result_from_nested_status(
        provenance_certificate_run
            .result
            .as_deref()
            .unwrap_or_default(),
    ...
    let status = PackageLecternCertificateLatestStatus {
        status_version: LECTERN_CERTIFICATE_LATEST_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nested_podium_receipt_latest_session_dir: paths
            .nested_podium_receipt_latest_session_dir
            .display()
            .to_string(),
        nested_lectern_certificate_session_dir: paths
            .nested_lectern_certificate_session_dir
            .display()
            .to_string(),
        ...
        latest_podium_receipt_plan_verdict: Some(
            resolved.latest_podium_receipt_plan.verdict.clone(),
        ),
        latest_podium_receipt_verdict: Some(latest_podium_receipt_run.verdict.clone()),
        lectern_certificate_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_lectern_certificate_failure(
    root: &Path,
    session_dir: &Path,
    paths: &PackageLecternCertificateLatestPaths,
    failure: &LatestPodiumReceiptResolutionFailure,
) -> Result<PackageLecternCertificateLatestReport> {
    let session = planned_session(root, session_dir, paths, None, Some(failure));
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_podium_receipt_plan_raw {
        persist_json(&paths.latest_podium_receipt_plan_report_path, value)?;
    }
...
fn resolve_latest_podium_receipt(
    root: &Path,
    paths: &PackageLecternCertificateLatestPaths,
) -> std::result::Result<ResolvedLatestPodiumReceipt, LatestPodiumReceiptResolutionFailure> {
    let latest_podium_receipt_plan_raw = match run_json_command(
        PODIUM_RECEIPT_LATEST_BIN,
        &latest_podium_receipt_plan_args(root, &paths.nested_podium_receipt_latest_session_dir),
    ) {
        Ok(value) => value,
        Err(error) => {
            return Err(LatestPodiumReceiptResolutionFailure {
                ...
                snapshot: None,
                latest_podium_receipt_plan_raw: None,
                latest_podium_receipt_plan_verdict: None,
                latest_podium_receipt_plan_reason: None,
                latest_podium_receipt_plan_generated_at: None,
            });
        }
    };
    let latest_podium_receipt_plan: PodiumReceiptLatestReportView =
        serde_json::from_value(latest_podium_receipt_plan_raw.clone()).map_err(
            |error| LatestPodiumReceiptResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-podium-receipt upstream latest-podium-receipt plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_podium_receipt_plan_raw: Some(
                    latest_podium_receipt_plan_raw.clone(),
                ),
                latest_podium_receipt_plan_verdict: None,
                latest_podium_receipt_plan_reason: None,
                latest_podium_receipt_plan_generated_at: None,
    ...
    let snapshot = parse_snapshot_from_latest_podium_receipt(&latest_podium_receipt_plan).ok();
    if latest_podium_receipt_plan.verdict != PODIUM_RECEIPT_LATEST_PLAN_READY {
        return Err(LatestPodiumReceiptResolutionFailure {
            kind: map_podium_receipt_latest_failure_kind(
                &latest_podium_receipt_plan.verdict,
            ),
            reason: format!(
                "latest-podium-receipt resolution refused because nested latest-podium-receipt reported: {}",
                latest_podium_receipt_plan.reason
            ),
            snapshot,
            latest_podium_receipt_plan_raw: Some(latest_podium_receipt_plan_raw),
            latest_podium_receipt_plan_verdict: Some(
                latest_podium_receipt_plan.verdict,
            ),
            latest_podium_receipt_plan_reason: Some(
                latest_podium_receipt_plan.reason,
            ),
            latest_podium_receipt_plan_generated_at: Some(
                latest_podium_receipt_plan.generated_at,
            ),
        });
    }

    let snapshot = parse_snapshot_from_latest_podium_receipt(&latest_podium_receipt_plan).map_err(
        |error| LatestPodiumReceiptResolutionFailure {
            kind: ResolutionFailureKind::InvalidLatestChain,
            reason: format!("latest-podium-receipt resolution failed: {error}"),
            snapshot: None,
            latest_podium_receipt_plan_raw: Some(latest_podium_receipt_plan_raw.clone()),
            latest_podium_receipt_plan_verdict: Some(latest_podium_receipt_plan.verdict.clone()),
            latest_podium_receipt_plan_reason: Some(latest_podium_receipt_plan.reason.clone()),
            latest_podium_receipt_plan_generated_at: Some(latest_podium_receipt_plan.generated_at),
        },
    )?;

    let mut mismatches = Vec::new();
    compare_podium_receipt_latest_plan_against_snapshot(
        &latest_podium_receipt_plan,
        &snapshot,
        &paths.nested_podium_receipt_latest_session_dir,
        &mut mismatches,
    );
    if !mismatches.is_empty() {
        return Err(LatestPodiumReceiptResolutionFailure {
            kind: ResolutionFailureKind::DriftedPodiumReceiptContract,
            reason: format!("latest-podium-receipt drift detected: {}", mismatches[0]),
            snapshot: Some(snapshot),
            latest_podium_receipt_plan_raw: Some(latest_podium_receipt_plan_raw),
            latest_podium_receipt_plan_verdict: Some(latest_podium_receipt_plan.verdict),
            latest_podium_receipt_plan_reason: Some(latest_podium_receipt_plan.reason),
            latest_podium_receipt_plan_generated_at: Some(latest_podium_receipt_plan.generated_at),
        });
    }

    Ok(ResolvedLatestPodiumReceipt {
        snapshot,
        latest_podium_receipt_plan_raw,
        latest_podium_receipt_plan,
    })
}

fn parse_snapshot_from_latest_podium_receipt(
    view: &PodiumReceiptLatestReportView,
) -> Result<LatestPodiumReceiptSnapshot> {
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-podium-receipt nested latest-podium-receipt plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-podium-receipt nested latest-podium-receipt plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestPodiumReceiptSnapshot {
        latest_top_step_name,
        ...
        wrapper_timeout_ms: view.wrapper_timeout_ms.ok_or_else(|| {
            anyhow!(
                "latest-podium-receipt nested latest-podium-receipt plan is missing wrapper_timeout_ms"
            )
        })?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!(
                "latest-podium-receipt nested latest-podium-receipt plan is missing service_status_max_staleness_ms"
            )
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackageLecternCertificateLatestPaths,
    resolved: &ResolvedLatestPodiumReceipt,
) -> PackageLecternCertificateLatestReport {
    PackageLecternCertificateLatestReport {
        generated_at: Utc::now(),
        ...
        latest_podium_receipt_report_path: Some(
            paths
                .latest_podium_receipt_report_path
                .display()
                .to_string(),
        ),
        lectern_certificate_report_path: Some(
            paths.lectern_certificate_report_path.display().to_string(),
        ),
        nested_podium_receipt_latest_session_dir: Some(
            paths
                .nested_podium_receipt_latest_session_dir
        ...
        latest_podium_receipt_verdict: None,
        lectern_certificate_verdict: None,
        result: None,
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_podium_receipt_command_summary: Some(
            resolve_latest_podium_receipt_command_summary(
                root,
                &paths.nested_podium_receipt_latest_session_dir,
            ),
        ),
        run_latest_lectern_certificate_command_summary: Some(
            run_latest_lectern_certificate_command_summary(root, session_dir),
        ),
        verify_latest_lectern_certificate_command_summary: Some(
            verify_latest_lectern_certificate_command_summary(session_dir),
        ),
        downstream_lectern_certificate_command_summary: Some(
            downstream_lectern_certificate_command_summary(
                &resolved.snapshot,
                &expected_downstream_podium_receipt_session_dir(paths),
                &paths.nested_lectern_certificate_session_dir,
            ),
        ...
        verify_lectern_certificate_command_summary: Some(
            verify_lectern_certificate_command_summary(
                &resolved.snapshot,
                &expected_downstream_podium_receipt_session_dir(paths),
                &paths.nested_lectern_certificate_session_dir,
            ),
...
fn report_from_resolution_failure(
    ...
    PackageLecternCertificateLatestReport {
        ...
        latest_podium_receipt_plan_verdict: failure.latest_podium_receipt_plan_verdict.clone(),
        latest_podium_receipt_verdict: None,
        lectern_certificate_verdict: None,
        ...
        downstream_lectern_certificate_command_summary: snapshot.map(|value| {
            downstream_lectern_certificate_command_summary(
                value,
                &expected_downstream_podium_receipt_session_dir(paths),
                &paths.nested_lectern_certificate_session_dir,
        ...
        verify_lectern_certificate_command_summary: snapshot.map(|value| {
            verify_lectern_certificate_command_summary(
                value,
                &expected_downstream_podium_receipt_session_dir(paths),
                &paths.nested_lectern_certificate_session_dir,
...
fn planned_session(
    ...
    session_dir: &Path,
    paths: &PackageLecternCertificateLatestPaths,
    resolved: Option<&ResolvedLatestPodiumReceipt>,
    failure: Option<&LatestPodiumReceiptResolutionFailure>,
) -> PackageLecternCertificateLatestSession {
    let snapshot = resolved
        .map(|value| &value.snapshot)
        .or_else(|| failure.and_then(|value| value.snapshot.as_ref()));
    PackageLecternCertificateLatestSession {
        session_version: LECTERN_CERTIFICATE_LATEST_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        root: root.display().to_string(),
        session_dir: session_dir.display().to_string(),
        nested_podium_receipt_latest_session_dir: paths
        ...
        run_latest_lectern_certificate_command_summary:
            run_latest_lectern_certificate_command_summary(root, session_dir),
        verify_latest_lectern_certificate_command_summary:
            verify_latest_lectern_certificate_command_summary(session_dir),
        downstream_lectern_certificate_command_summary: snapshot
            .map(|value| {
                downstream_lectern_certificate_command_summary(
                    value,
                    &expected_downstream_podium_receipt_session_dir(paths),
            ...
            .unwrap_or_else(|| "<lectern-certificate-run-unavailable>".to_string()),
        verify_lectern_certificate_command_summary: snapshot
            .map(|value| {
                verify_lectern_certificate_command_summary(
                    value,
                    &expected_downstream_podium_receipt_session_dir(paths),
            ...
            .unwrap_or_else(|| "<lectern-certificate-verify-unavailable>".to_string()),
        reviewed_frozen_live_cutover_controller_command_summary: snapshot.map(|value| {
            value
                .reviewed_frozen_live_cutover_controller_command_summary
                .clone()
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackageLecternCertificateLatestPaths,
    session: &PackageLecternCertificateLatestSession,
    status: &PackageLecternCertificateLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackageLecternCertificateLatestReport {
    PackageLecternCertificateLatestReport {
        generated_at: Utc::now(),
        ...
        nested_podium_receipt_latest_session_dir: Some(
            session.nested_podium_receipt_latest_session_dir.clone(),
        ),
        nested_lectern_certificate_session_dir: Some(
            session.nested_lectern_certificate_session_dir.clone(),
        ),
        downstream_decision_packet_session_dir: Some(
...
fn lectern_certificate_latest_paths(session_dir: &Path) -> PackageLecternCertificateLatestPaths {
    PackageLecternCertificateLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_lectern_certificate_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_lectern_certificate_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_lectern_certificate_latest.report.json"),
        latest_podium_receipt_plan_report_path: session_dir.join(
            "tiny_live_activation_package_lectern_certificate_latest.podium_receipt_latest_plan.report.json",
        ),
        latest_podium_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_lectern_certificate_latest.podium_receipt_latest.report.json",
        ),
        lectern_certificate_report_path: session_dir.join(
            "tiny_live_activation_package_lectern_certificate_latest.lectern_certificate.report.json",
        ),
        nested_podium_receipt_latest_session_dir: session_dir.join(
            "tiny_live_activation_package_lectern_certificate_latest.podium_receipt_latest_session",
        ),
        nested_lectern_certificate_session_dir: session_dir.join(
            "tiny_live_activation_package_lectern_certificate_latest.lectern_certificate_session",
        ),
    }
}
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_podium_receipt_latest_session_dir
        .join("tiny_live_activation_package_podium_receipt_latest.decision_packet_session")
}

fn expected_downstream_podium_receipt_session_dir(
    paths: &PackageLecternCertificateLatestPaths,
) -> PathBuf {
    paths
        .nested_podium_receipt_latest_session_dir
        .join("tiny_live_activation_package_podium_receipt_latest.podium_receipt_session")
}
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_podium_receipt_latest_session_dir
        .join("tiny_live_activation_package_podium_receipt_latest.review_receipt_session")
}

fn latest_podium_receipt_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-podium-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn latest_podium_receipt_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-podium-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn latest_podium_receipt_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-podium-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn lectern_certificate_run_args(
    snapshot: &LatestPodiumReceiptSnapshot,
    podium_receipt_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        "--podium-receipt-session-dir".to_string(),
        podium_receipt_session_dir.display().to_string(),
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-lectern-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn lectern_certificate_verify_args(
    snapshot: &LatestPodiumReceiptSnapshot,
    podium_receipt_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-live-package-lectern-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn resolve_latest_podium_receipt_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{PODIUM_RECEIPT_LATEST_BIN} --root {} --session-dir {} --plan-latest-podium-receipt --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn run_latest_lectern_certificate_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{LECTERN_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --run-latest-lectern-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn verify_latest_lectern_certificate_command_summary(session_dir: &Path) -> String {
    format!(
        "{LECTERN_CERTIFICATE_LATEST_BIN} --session-dir {} --verify-latest-lectern-certificate --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_lectern_certificate_command_summary(
    snapshot: &LatestPodiumReceiptSnapshot,
    podium_receipt_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{LECTERN_CERTIFICATE_BIN} --podium-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-lectern-certificate --json",
        shell_escape_path(podium_receipt_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
}

fn verify_lectern_certificate_command_summary(
    snapshot: &LatestPodiumReceiptSnapshot,
    podium_receipt_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{LECTERN_CERTIFICATE_BIN} --podium-receipt-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-lectern-certificate --json",
        shell_escape_path(podium_receipt_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
...
fn nested_podium_receipt_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_podium_receipt_latest.report.json")
}

fn nested_lectern_certificate_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_lectern_certificate.session.json")
}

fn nested_lectern_certificate_status_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_lectern_certificate.status.json")
}
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-lectern-certificate script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageLecternCertificateLatestStepArtifact {
    PackageLecternCertificateLatestStepArtifact {
...
fn canonical_wrapper_mode(mode: &str) -> &str {
    match mode {
        "plan_latest_lectern_certificate" => "plan_latest_lectern_certificate",
        "render_latest_lectern_certificate_script" => "render_latest_lectern_certificate_script",
        "run_latest_lectern_certificate" => "run_latest_lectern_certificate",
        "verify_latest_lectern_certificate" => "verify_latest_lectern_certificate",
        other => other,
    }
}

fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackageLecternCertificateLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-lectern-certificate status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-lectern-certificate status"
        ));
        return;
    };
    compare_string(
...
fn compare_podium_receipt_latest_plan_against_snapshot(
    stored: &PodiumReceiptLatestReportView,
    snapshot: &LatestPodiumReceiptSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        ...
        Some(PODIUM_RECEIPT_LATEST_PLAN_READY),
        "stored latest-podium-receipt plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_podium_receipt",
        "stored latest-podium-receipt plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-podium-receipt plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-podium-receipt plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-podium-receipt plan session_dir",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        ...
        "stored latest-podium-receipt plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-podium-receipt plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-podium-receipt plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-podium-receipt plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_podium_receipt_latest_run_against_snapshot(
    stored: &PodiumReceiptLatestReportView,
    snapshot: &LatestPodiumReceiptSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_podium_receipt",
        "stored latest-podium-receipt run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-podium-receipt run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-podium-receipt run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-podium-receipt run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-podium-receipt run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-podium-receipt run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-podium-receipt run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-podium-receipt run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-podium-receipt run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-podium-receipt run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-podium-receipt run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-podium-receipt run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-podium-receipt run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-podium-receipt run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-podium-receipt run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_podium_receipt_latest_copy_matches_actual(
    stored: &PodiumReceiptLatestReportView,
    actual: &PodiumReceiptLatestReportView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        Some(stored.verdict.as_str()),
        Some(actual.verdict.as_str()),
        "stored latest-podium-receipt copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-podium-receipt copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-podium-receipt copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-podium-receipt copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-podium-receipt copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-podium-receipt copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-podium-receipt copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-podium-receipt copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-podium-receipt copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-podium-receipt copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-podium-receipt copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-podium-receipt copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_lectern_certificate_run_against_snapshot(
    stored: &LecternCertificateReportView,
    snapshot: &LatestPodiumReceiptSnapshot,
    podium_receipt_session_dir: &Path,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_live_package_lectern_certificate",
        "stored lectern-certificate run mode",
        mismatches,
    );
    compare_string(
        &stored.podium_receipt_session_dir,
        &podium_receipt_session_dir.display().to_string(),
        "stored lectern-certificate run podium_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored lectern-certificate run decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored lectern-certificate run execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored lectern-certificate run turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored lectern-certificate run launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored lectern-certificate run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored lectern-certificate run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored lectern-certificate run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored lectern-certificate run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored lectern-certificate run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored lectern-certificate run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored lectern-certificate run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.lectern_certificate_session_path.as_deref(),
        Some(
            nested_lectern_certificate_session_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored lectern-certificate run lectern_certificate_session_path",
        mismatches,
    );
    compare_option_string(
        stored.lectern_certificate_status_path.as_deref(),
        Some(
            nested_lectern_certificate_status_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored lectern-certificate run lectern_certificate_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_podium_receipt_report_path.as_deref(),
        Some(
            session_dir
                .join("tiny_live_activation_package_lectern_certificate.podium_receipt.report.json")
                .display()
                .to_string()
                .as_str(),
        ),
        "stored lectern-certificate run archived_podium_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        stored.verify_podium_receipt_command_summary.as_deref(),
        Some(verify_lectern_certificate_input_command_summary(podium_receipt_session_dir).as_str()),
        "stored lectern-certificate run verify_podium_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
...
fn compare_lectern_certificate_copy_against_nested_truth(
    ...
    compare_option_string(
        ...
        "stored lectern-certificate copy verify_podium_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored lectern-certificate copy release_capsule_digest_manifest_sha256",
        mismatches,
    );
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_podium_receipt_command_summary,
        &resolve_latest_podium_receipt_command_summary(
            root,
            &paths.nested_podium_receipt_latest_session_dir,
        ),
        "stored session resolve_latest_podium_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.run_latest_lectern_certificate_command_summary,
        &run_latest_lectern_certificate_command_summary(root, session_dir),
        "stored session run_latest_lectern_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_latest_lectern_certificate_command_summary,
        &verify_latest_lectern_certificate_command_summary(session_dir),
        "stored session verify_latest_lectern_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.downstream_lectern_certificate_command_summary,
        &downstream_lectern_certificate_command_summary(
            snapshot,
            &expected_downstream_podium_receipt_session_dir(paths),
            &paths.nested_lectern_certificate_session_dir,
        ),
        "stored session downstream_lectern_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_lectern_certificate_command_summary,
        &verify_lectern_certificate_command_summary(
            snapshot,
            &expected_downstream_podium_receipt_session_dir(paths),
            &paths.nested_lectern_certificate_session_dir,
        ),
        "stored session verify_lectern_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_podium_receipt_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_lectern_certificate_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_lectern_certificate_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackageLecternCertificateLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackageLecternCertificateLatestPaths,
    snapshot: &LatestPodiumReceiptSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_podium_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
            .run_latest_lectern_certificate_command_summary
            .as_deref(),
        Some(run_latest_lectern_certificate_command_summary(root, session_dir).as_str()),
        "stored report run_latest_lectern_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
            .verify_latest_lectern_certificate_command_summary
            .as_deref(),
        Some(verify_latest_lectern_certificate_command_summary(session_dir).as_str()),
        "stored report verify_latest_lectern_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report downstream_lectern_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report.verify_lectern_certificate_command_summary.as_deref(),
        Some(
            verify_lectern_certificate_command_summary(
                snapshot,
                &expected_downstream_podium_receipt_session_dir(paths),
        ...
        "stored report verify_lectern_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
    ...
    compare_option_string(
        report.lectern_certificate_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "stored report lectern_certificate_latest_session_path",
        mismatches,
    );
    compare_option_string(
        report.lectern_certificate_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "stored report lectern_certificate_latest_status_path",
        mismatches,
    );
    compare_option_string(
        report.lectern_certificate_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "stored report lectern_certificate_latest_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_podium_receipt_plan_report_path.as_deref(),
        Some(
            paths
                .latest_podium_receipt_plan_report_path
                .display()
        ...
        "stored report latest_podium_receipt_plan_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_podium_receipt_report_path.as_deref(),
        Some(
            paths
                .latest_podium_receipt_report_path
                .display()
        ...
        "stored report latest_podium_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        report.lectern_certificate_report_path.as_deref(),
        Some(
            paths
                .lectern_certificate_report_path
                .display()
                .to_string()
                .as_str(),
        ),
        "stored report lectern_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        report.nested_podium_receipt_latest_session_dir.as_deref(),
        Some(
            paths
                .nested_podium_receipt_latest_session_dir
                .display()
        ...
        "stored report nested_podium_receipt_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_lectern_certificate_session_dir.as_deref(),
        Some(
            paths
                .nested_lectern_certificate_session_dir
                .display()
        ...
        "stored report nested_lectern_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_podium_receipt_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_lectern_certificate_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_lectern_certificate_session_dir on failure",
        mismatches,
    );
...
fn map_podium_receipt_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_podium_receipt_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_podium_receipt_latest_refused_by_drifted_podium_receipt_contract" => {
            ResolutionFailureKind::DriftedPodiumReceiptContract
        }
        _ => ResolutionFailureKind::InvalidLatestChain,
    }
}

fn latest_podium_receipt_terminal_failure(
    report: &PodiumReceiptLatestReportView,
) -> Option<LivePackageLecternCertificateLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackageLecternCertificateLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackageLecternCertificateLatestResult::RefusedByInvalidLatestChain)
        }
        Some("refused_by_drifted_podium_receipt_contract") => Some(
            LivePackageLecternCertificateLatestResult::RefusedByDriftedLecternCertificateContract,
        ),
        _ => None,
    }
...
fn podium_receipt_latest_result_from_nested_status(
    result: &str,
) -> Option<LivePackageLecternCertificateLatestResult> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => Some(
            LivePackageLecternCertificateLatestResult::ReadyForManualExecutionWhenGateTurnsGreen,
        ),
        "refused_now_by_stage3" => {
            Some(LivePackageLecternCertificateLatestResult::RefusedNowByStage3)
        }
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackageLecternCertificateLatestResult::RefusedNowByPreActivationGate)
        }
...
fn lectern_certificate_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => Some(
            "tiny_live_package_lectern_certificate_ready_for_manual_execution_when_gate_turns_green",
        ),
        "refused_now_by_stage3" => Some("tiny_live_package_lectern_certificate_refused_now_by_stage3"),
        "refused_now_by_pre_activation_gate" => {
            Some("tiny_live_package_lectern_certificate_refused_now_by_pre_activation_gate")
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_lectern_certificate_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}
...
fn result_from_failure_kind(
    kind: ResolutionFailureKind,
) -> LivePackageLecternCertificateLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackageLecternCertificateLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackageLecternCertificateLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedPodiumReceiptContract => {
            LivePackageLecternCertificateLatestResult::RefusedByDriftedLecternCertificateContract
        }
...
fn verdict_for_result(
    result: LivePackageLecternCertificateLatestResult,
) -> TinyLivePackageLecternCertificateLatestVerdict {
    match result {
        LivePackageLecternCertificateLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestRefusedByMissingLatestChain
        }
        LivePackageLecternCertificateLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestRefusedByInvalidLatestChain
        }
        LivePackageLecternCertificateLatestResult::RefusedByDriftedLecternCertificateContract => {
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestRefusedByDriftedLecternCertificateContract
        }
        LivePackageLecternCertificateLatestResult::RefusedNowByStage3 => {
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestRefusedNowByStage3
        }
        LivePackageLecternCertificateLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestRefusedNowByPreActivationGate
        }
        LivePackageLecternCertificateLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackageLecternCertificateLatestResult) -> String {
    match result {
        LivePackageLecternCertificateLatestResult::RefusedByMissingLatestChain => {
            "no immutable clerestory chain is available yet; do not manually hunt latest-podium-receipt or lectern-certificate lineage by hand".to_string()
        }
        LivePackageLecternCertificateLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-podium-receipt / lectern-certificate path".to_string()
        }
        LivePackageLecternCertificateLatestResult::RefusedByDriftedLecternCertificateContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-podium-receipt / lectern-certificate contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackageLecternCertificateLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-lectern-certificate refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackageLecternCertificateLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-lectern-certificate refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackageLecternCertificateLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            "the latest immutable chain now resolves into the accepted lectern-certificate contract and one bounded latest-lectern-certificate handoff is ready for manual execution when gate truth turns green; this wrapper still did not run or authorize the frozen controller".to_string()
        }
    }
}
...
fn verify_lectern_certificate_input_command_summary(podium_receipt_session_dir: &Path) -> String {
    format!(
        "verify immutable podium-receipt session under {} before freezing the final lectern certificate",
        podium_receipt_session_dir.display()
    )
}
...
fn refusal_status(
    ...
    latest_podium_receipt_step: Option<PackageLecternCertificateLatestStepArtifact>,
    lectern_certificate_step: Option<PackageLecternCertificateLatestStepArtifact>,
) -> PackageLecternCertificateLatestStatus {
    ...
    PackageLecternCertificateLatestStatus {
        ...
        result,
        reason,
        latest_podium_receipt_plan_verdict: resolved
            .map(|value| value.latest_podium_receipt_plan.verdict.clone()),
        latest_podium_receipt_verdict: latest_podium_receipt_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        lectern_certificate_verdict: lectern_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackageLecternCertificateLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_podium_receipt_plan_step: failure.latest_podium_receipt_plan_raw.as_ref().map(
            |_| {
                step_artifact(
                    &paths.latest_podium_receipt_plan_report_path,
                    "plan_latest_podium_receipt",
                    failure
                        .latest_podium_receipt_plan_verdict
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_podium_receipt_plan_reason
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_podium_receipt_plan_generated_at
                        .unwrap_or_else(Utc::now),
                )
            },
        ),
        latest_podium_receipt_step: None,
        lectern_certificate_step: None,
...
fn verify_invalid_lectern_certificate_latest_report_without_session(
    session_dir: &Path,
    verification_mismatches: Vec<String>,
) -> PackageLecternCertificateLatestReport {
    ...
    PackageLecternCertificateLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_lectern_certificate".to_string(),
        verdict: TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestVerifyInvalid,
        reason,
        root: None,
        ...
        session_dir: Some(session_dir.display().to_string()),
        lectern_certificate_latest_session_path: Some(
            lectern_certificate_latest_paths(session_dir)
                .session_path
                .display()
                .to_string(),
        ),
        lectern_certificate_latest_status_path: Some(
            lectern_certificate_latest_paths(session_dir)
                .status_path
                .display()
                .to_string(),
        ),
        lectern_certificate_latest_report_path: Some(
            lectern_certificate_latest_paths(session_dir)
                .report_path
                .display()
                .to_string(),
        ),
        latest_podium_receipt_plan_report_path: None,
        latest_podium_receipt_report_path: None,
        lectern_certificate_report_path: None,
        nested_podium_receipt_latest_session_dir: None,
        nested_lectern_certificate_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_podium_receipt_command_summary: None,
        run_latest_lectern_certificate_command_summary: None,
        verify_latest_lectern_certificate_command_summary: None,
        downstream_lectern_certificate_command_summary: None,
        verify_lectern_certificate_command_summary: None,
...
fn fallback_podium_receipt_latest_copy() -> PodiumReceiptLatestReportView {
    PodiumReceiptLatestReportView {
        generated_at: Utc::now(),
...
fn fallback_notarization_receipt_session(
    snapshot: &LatestPodiumReceiptSnapshot,
    session_dir: &Path,
) -> LecternCertificateSessionView {
    LecternCertificateSessionView {
        podium_receipt_session_dir: "<missing>".to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_podium_receipt_command_summary:
            verify_lectern_certificate_input_command_summary(Path::new("<missing>")),
        ...
        lectern_certificate_summary:
            "Lectern certificate identity: fallback session is synthetic and must not verify green."
                .to_string(),
        lectern_certificate_sha256: "<missing>".to_string(),
        lectern_certificate_algorithm: "sha256".to_string(),
        explicit_statement:
            "the verified podium-receipt session is the primary input here; this immutable lectern certificate freezes one final armorial identity over the fully culminated chain without enabling production execution"
                .to_string(),
    }
}

fn fallback_notarization_receipt_status(
    _snapshot: &LatestPodiumReceiptSnapshot,
    session_dir: &Path,
) -> LecternCertificateStatusView {
    LecternCertificateStatusView {
        session_dir: session_dir.display().to_string(),
        podium_receipt_session_dir: "<missing>".to_string(),
        pedestal_certificate_session_dir: "<missing>".to_string(),
        ...
        podium_receipt_algorithm: "sha256".to_string(),
        lectern_certificate_summary:
            "Lectern certificate identity: fallback status is synthetic and must not verify green."
                .to_string(),
        lectern_certificate_sha256: "<missing>".to_string(),
        lectern_certificate_algorithm: "sha256".to_string(),
        explicit_statement:
            "this lectern certificate is archival and read-only; it never executes or enables the frozen controller"
                .to_string(),
    }
}

fn render_report_lines(report: &PackageLecternCertificateLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_podium_receipt_latest_session_dir {
        lines.push(format!("nested_podium_receipt_latest_session_dir={value}"));
    }
    ...
    if let Some(value) = &report.nested_lectern_certificate_session_dir {
        lines.push(format!("nested_lectern_certificate_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_lectern_certificate_command_summary {
        lines.push(format!(
            "downstream_lectern_certificate_command_summary={value}"
        ));
    }
...
mod tests {
    ...
    struct FakePodiumReceiptLatestFixture {
        temp_dir: PathBuf,
        root: PathBuf,
        ...
        podium_receipt_latest_plan_json_path: PathBuf,
        podium_receipt_latest_run_json_path: PathBuf,
        podium_receipt_latest_verify_json_path: PathBuf,
    ...
    impl FakePodiumReceiptLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestLecternCertificate,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestLecternCertificateScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestLecternCertificate,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestLecternCertificate,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    #[test]
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_podium_receipt_lineage(
    ) {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_plan_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = plan_latest_lectern_certificate_report(&fixture.plan_config()).unwrap();
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        assert_eq!(
            report.verdict,
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestPlanReady
        );
        assert_eq!(report.mode, "plan_latest_lectern_certificate");
        assert_eq!(
            report.latest_top_step_name.as_deref(),
        ...
        assert!(report
            ...
            .as_deref()
            .unwrap()
            .contains(LECTERN_CERTIFICATE_BIN));
    }

    #[test]
    fn plan_mode_without_session_dir_uses_lectern_latest_default_wrapper_session_dir() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_latest_plan_default_session_dir",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let default_wrapper_session_dir = fixture
            .root
            .join("tiny_live_activation_package_lectern_certificate_latest.session");
        let stale_podium_wrapper_session_dir = fixture
            .root
            .join("tiny_live_activation_package_podium_receipt_latest.session");
        let default_paths = lectern_certificate_latest_paths(&default_wrapper_session_dir);
        let mut plan_json = default_podium_receipt_latest_plan_report(&fixture);
        ...
        persist_json(&fixture.podium_receipt_latest_plan_json_path, &plan_json).unwrap();
        write_fake_podium_receipt_latest_command(
            &fixture.bin_dir.join(PODIUM_RECEIPT_LATEST_BIN),
            &fixture.podium_receipt_latest_plan_json_path,
            &fixture.podium_receipt_latest_run_json_path,
            &fixture.podium_receipt_latest_verify_json_path,
            &fixture.root,
            &default_paths.nested_podium_receipt_latest_session_dir,
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let report = plan_latest_lectern_certificate_report(&config).unwrap();
        let default_wrapper_session_dir = default_wrapper_session_dir.display().to_string();
        let stale_podium_wrapper_session_dir =
            stale_podium_wrapper_session_dir.display().to_string();
        ...
        for summary in [
            report
                .run_latest_lectern_certificate_command_summary
                .as_deref()
                .expect("missing run_latest_lectern_certificate_command_summary"),
            report
                .verify_latest_lectern_certificate_command_summary
                .as_deref()
                .expect("missing verify_latest_lectern_certificate_command_summary"),
        ] {
            assert!(
                summary.contains(default_wrapper_session_dir.as_str()),
                "self command summary must use lectern latest default session dir: {summary}"
            );
            assert!(
                !summary.contains(stale_podium_wrapper_session_dir.as_str()),
                "self command summary must not use podium latest default wrapper session dir: {summary}"
            );
        }
        assert_eq!(
            report.nested_podium_receipt_latest_session_dir.as_deref(),
    ...
    #[test]
    fn render_mode_emits_exact_downstream_lectern_certificate_contract_not_second_controller_path()
    {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_render_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let report =
            render_latest_lectern_certificate_script_report(&fixture.render_config()).unwrap();
        assert_eq!(
            report.verdict,
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestScriptRendered
        );
        assert_eq!(report.mode, "render_latest_lectern_certificate_script");
        let script = fs::read_to_string(&fixture.output_script_path).unwrap();
        assert!(script.contains(LECTERN_CERTIFICATE_LATEST_BIN));
        assert!(!script.contains(PODIUM_RECEIPT_LATEST_BIN));
        assert!(script.contains(LECTERN_CERTIFICATE_BIN));
        assert!(script.contains("--run-latest-lectern-certificate"));
        assert!(script.contains("--verify-latest-lectern-certificate"));
        assert!(!script.contains("--run-latest-podium-receipt"));
        assert!(!script.contains("--verify-latest-podium-receipt"));
        assert!(script.contains("--podium-receipt-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-lectern-certificate"));
        assert!(script.contains("--verify-live-package-lectern-certificate"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            (
                "podium_receipt_latest_missing",
                "tiny_live_package_podium_receipt_latest_refused_by_missing_latest_chain",
                "no persisted immutable chain exists",
                TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestRefusedByMissingLatestChain,
            ),
            (
                "podium_receipt_latest_invalid",
                "tiny_live_package_podium_receipt_latest_refused_by_invalid_latest_chain",
                "latest immutable chain is incomplete",
                TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestRefusedByInvalidLatestChain,
            ),
            (
                "podium_receipt_latest_below",
                "tiny_live_package_podium_receipt_latest_refused_by_invalid_latest_chain",
            ...
            let mut plan: Value = load_json(&fixture.podium_receipt_latest_plan_json_path).unwrap();
            plan["verdict"] = json!(verdict);
            plan["reason"] = json!(reason);
            if label.ends_with("below") {
                plan["latest_top_step_name"] = json!("choir_receipt");
            }
            persist_json(&fixture.podium_receipt_latest_plan_json_path, &plan).unwrap();

            let report = run_latest_lectern_certificate_report(&fixture.run_config()).unwrap();
            assert_eq!(report.verdict, expected);
        }
    }

    #[test]
    fn plan_mode_maps_real_latest_podium_receipt_drift_verdict_to_local_drift_refusal() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_plan_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        plan["reason"] = json!("real accepted latest-podium-receipt drift");
        persist_json(&fixture.podium_receipt_latest_plan_json_path, &plan).unwrap();
        ...
        assert_eq!(
            report.result.as_deref(),
            Some("refused_by_drifted_lectern_certificate_contract")
        );
    }

    #[test]
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_podium_receipt_reports_real_drift()
    {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_run_drift",
            "refused_by_drifted_podium_receipt_contract",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert_eq!(report.lectern_certificate_verdict.as_deref(), None);
    }

    #[test]
    fn run_mode_refuses_when_nested_latest_podium_receipt_lineage_drifts_from_latest_immutable_chain(
    ) {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_handoff_bundle_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let mut run: Value = load_json(&fixture.podium_receipt_latest_run_json_path).unwrap();
        run["package_dir"] = json!("/tampered/package");
        persist_json(&fixture.podium_receipt_latest_run_json_path, &run).unwrap();
        ...
        assert!(report
            .reason
            .contains("stored latest-podium-receipt run package_dir"));
    }

    #[test]
    fn valid_nested_latest_podium_modes_are_accepted_by_latest_lectern_plan_run_verify() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_latest_accepts_real_nested_podium_modes",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let plan = plan_latest_lectern_certificate_report(&fixture.plan_config()).unwrap();
        assert_eq!(
            plan.verdict,
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestPlanReady
        );

        let run = run_latest_lectern_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestReadyForManualExecutionWhenGateTurnsGreen
        );

        let verify = verify_latest_lectern_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestVerifyOk,
            "{:#?}",
        ...
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        let stored_plan: Value = load_json(&paths.latest_podium_receipt_plan_report_path).unwrap();
        let stored_run: Value = load_json(&paths.latest_podium_receipt_report_path).unwrap();
        let status: Value = load_json(&paths.status_path).unwrap();

        assert_eq!(stored_plan["mode"], json!("plan_latest_podium_receipt"));
        assert_eq!(stored_run["mode"], json!("run_latest_podium_receipt"));
        assert_eq!(
            status["latest_podium_receipt_plan_step"]["mode"],
            json!("plan_latest_podium_receipt")
        );
        assert_eq!(
            status["latest_podium_receipt_step"]["mode"],
            json!("run_latest_podium_receipt")
        );
    }

    #[test]
    fn plan_rejects_stale_parent_mode_in_nested_latest_podium_plan_payload() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_latest_rejects_parent_mode_in_nested_podium_plan",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value = load_json(&fixture.podium_receipt_latest_plan_json_path).unwrap();
        plan["mode"] = json!("plan_latest_lectern_certificate");
        persist_json(&fixture.podium_receipt_latest_plan_json_path, &plan).unwrap();
        ...
        assert!(report.reason.contains(
            "stored latest-podium-receipt plan mode \"plan_latest_lectern_certificate\" does not match expected \"plan_latest_podium_receipt\""
        ));
    }

    #[test]
    fn run_rejects_stale_parent_mode_in_nested_latest_podium_run_payload() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_latest_rejects_parent_mode_in_nested_podium_run",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut run: Value = load_json(&fixture.podium_receipt_latest_run_json_path).unwrap();
        run["mode"] = json!("run_latest_lectern_certificate");
        persist_json(&fixture.podium_receipt_latest_run_json_path, &run).unwrap();
        ...
        assert!(report.reason.contains(
            "stored latest-podium-receipt run mode \"run_latest_lectern_certificate\" does not match expected \"run_latest_podium_receipt\""
        ));
    }

    #[test]
    fn verify_rejects_stale_parent_modes_in_copied_nested_latest_podium_reports() {
        {
            let fixture = fake_podium_receipt_latest_fixture(
                "lectern_certificate_latest_rejects_parent_mode_in_copied_podium_plan",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            run_latest_lectern_certificate_report(&fixture.run_config()).unwrap();

            let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
            let mut plan: Value = load_json(&paths.latest_podium_receipt_plan_report_path).unwrap();
            plan["mode"] = json!("plan_latest_lectern_certificate");
            persist_json(&paths.latest_podium_receipt_plan_report_path, &plan).unwrap();

            let verify =
                verify_latest_lectern_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageLecternCertificateLatestVerdict::TinyLivePackageLecternCertificateLatestVerifyInvalid
            );
        ...
        {
            let fixture = fake_podium_receipt_latest_fixture(
                "lectern_certificate_latest_rejects_parent_mode_in_copied_podium_run",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
            let mut run: Value = load_json(&paths.latest_podium_receipt_report_path).unwrap();
            run["mode"] = json!("run_latest_lectern_certificate");
            persist_json(&paths.latest_podium_receipt_report_path, &run).unwrap();
    ...
    #[test]
    fn verify_mode_fails_when_persisted_latest_podium_receipt_artifacts_are_tampered_or_missing_nested_lineage_truth(
    ) {
        {
            let fixture = fake_podium_receipt_latest_fixture(
                "podium_receipt_latest_verify_tampered_copy",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.lectern_certificate_report_path).unwrap();
            report["decision_packet_session_dir"] = json!("/tampered/run-decision-packet-session");
            persist_json(&paths.lectern_certificate_report_path, &report).unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains("stored lectern-certificate run decision_packet_session_dir")
            }));
        }

        {
            let fixture = fake_podium_receipt_latest_fixture(
                "podium_receipt_latest_verify_missing_nested_truth",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
            fs::remove_file(nested_podium_receipt_latest_report_path(
                &paths.nested_podium_receipt_latest_session_dir,
            ))
            .unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains("tiny_live_activation_package_podium_receipt_latest.report.json")
            }));
        }
    }

    #[test]
    fn verify_accepts_real_shaped_native_podium_and_lectern_identity_layers_on_happy_path() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_latest_verify_real_shaped_rostrum_paths_green",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    #[test]
    fn fake_native_lectern_payload_uses_armorial_and_podium_blazon_input_wording() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_latest_fixture_identity_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        let snapshot = LatestPodiumReceiptSnapshot {
        ...
        for (label, payload) in payloads {
            let lectern_summary = payload["lectern_certificate_summary"]
                .as_str()
                .unwrap_or_else(|| panic!("{label} lectern_certificate_summary missing"));
            assert!(
                lectern_summary.contains("armorial"),
                "{label} lectern summary must use accepted armorial wording: {lectern_summary}"
            );
            assert!(
                !lectern_summary.contains("hallmark"),
                "{label} lectern summary must not use stale hallmark wording: {lectern_summary}"
            );
            assert!(
                !lectern_summary.contains("escutcheon"),
                "{label} lectern summary must not use upstream rostrum escutcheon wording: {lectern_summary}"
            );
            assert!(
                !lectern_summary.contains("blazon"),
                "{label} lectern summary must not use upstream podium blazon wording: {lectern_summary}"
            );
            ...
            if explicit_statement.contains("freezes one final") {
                assert!(
                    ...
                    "{label} lectern session/run explicit statement must use armorial wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("escutcheon"),
                    "{label} lectern session/run explicit statement must not use upstream rostrum escutcheon wording: {explicit_statement}"
                );
                assert!(
                    !explicit_statement.contains("blazon"),
                    "{label} lectern session/run explicit statement must not use upstream podium blazon wording: {explicit_statement}"
                );
            }
            ...
            assert!(
                ...
                "{label} podium input summary must use accepted blazon wording: {podium_summary}"
            );
            assert!(
                podium_summary.contains("rostrum-certificate"),
                "{label} podium input summary must bind to rostrum-certificate truth: {podium_summary}"
            );
            assert!(
                !podium_summary.contains("hallmark"),
                "{label} podium input summary must not use stale hallmark wording: {podium_summary}"
            );
            assert!(
                !podium_summary.contains("escutcheon"),
                "{label} podium input summary must not use upstream rostrum escutcheon wording: {podium_summary}"
            );
            assert!(
                !podium_summary.contains("dais-receipt"),
                "{label} podium input summary must not bind directly to dais-receipt truth: {podium_summary}"
            );
            assert!(
                !podium_summary.contains("plinth-receipt"),
                "{label} podium input summary must not bind directly to plinth-receipt truth: {podium_summary}"
            );
        }
    }

    #[test]
    fn run_and_verify_reject_stale_foreign_wrapper_schema_for_downstream_lectern_copy() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_rejects_stale_keystone_wrapper_schema",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        assert!(paths.lectern_certificate_report_path.ends_with(
            "tiny_live_activation_package_lectern_certificate_latest.lectern_certificate.report.json"
        ));
        assert!(paths.lectern_certificate_report_path.exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_lectern_certificate_latest.podium_receipt.report.json"
            )
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_lectern_certificate_latest.pedestal_certificate.report.json"
            )
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_lectern_certificate_latest.plinth_receipt.report.json")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_lectern_certificate_latest.substructure_certificate.report.json"
            )
            .exists());
        assert!(paths.nested_lectern_certificate_session_dir.ends_with(
            "tiny_live_activation_package_lectern_certificate_latest.lectern_certificate_session"
        ));
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_lectern_certificate_latest.podium_receipt_session")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_lectern_certificate_latest.pedestal_certificate_session")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_lectern_certificate_latest.plinth_receipt_session")
            .exists());
        assert!(!fixture
            .wrapper_session_dir
            .join("tiny_live_activation_package_lectern_certificate_latest.substructure_certificate_session")
            .exists());

        let mut status: Value = load_json(&paths.status_path).unwrap();
        let stored_step = status
            .get("lectern_certificate_step")
            .cloned()
            .expect("lectern_certificate_step should be present");
        assert!(status.get("keystone_receipt_step").is_none());
        status["keystone_receipt_step"] = stored_step;
        if let Some(object) = status.as_object_mut() {
            object.remove("lectern_certificate_step");
        }
        persist_json(&paths.status_path, &status).unwrap();

        let error = verify_latest_lectern_certificate_report(&fixture.verify_config()).unwrap_err();
        assert!(error.to_string().contains("lectern_certificate_step"));
    }

    #[test]
    fn verify_names_lectern_certificate_when_nested_native_session_load_breaks() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_nested_dais_session_load_break",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        fs::remove_file(nested_lectern_certificate_session_path(
            &paths.nested_lectern_certificate_session_dir,
        ))
        .unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("failed reading nested lectern-certificate session under")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_lectern_certificate_session_path_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_dais_session_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.lectern_certificate_report_path).unwrap();
        report["lectern_certificate_session_path"] =
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate run lectern_certificate_session_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_lectern_certificate_status_path_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_dais_status_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.lectern_certificate_report_path).unwrap();
        report["lectern_certificate_status_path"] = json!("/tampered/finality-receipt-status.json");
        persist_json(&paths.lectern_certificate_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate run lectern_certificate_status_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_lectern_pedestal_certificate_identity_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_verify_pedestal_identity_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy pedestal_certificate_sha256")
        }));
    }

    #[test]
    fn run_names_latest_podium_receipt_when_nested_latest_wrapper_run_fails() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_nested_run_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        overwrite_fake_podium_receipt_latest_command(
            &fixture.bin_dir.join(PODIUM_RECEIPT_LATEST_BIN),
            &fixture.podium_receipt_latest_plan_json_path,
            &fixture.podium_receipt_latest_run_json_path,
            &fixture.podium_receipt_latest_verify_json_path,
            None,
            Some("forced nested latest-podium-receipt run failure"),
            None,
            &fixture.root,
            &paths.nested_podium_receipt_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed running nested latest-podium-receipt handoff under"));
    }

    #[test]
    fn run_names_latest_podium_receipt_when_nested_latest_wrapper_verify_fails() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_nested_verify_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_podium_receipt_latest_command(
            ...
            None,
            None,
            Some("forced nested latest-podium-receipt verify failure"),
            &fixture.root,
            &paths.nested_podium_receipt_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed verifying nested latest-podium-receipt handoff under"));
    }

    #[test]
    fn run_names_lectern_certificate_when_downstream_native_run_fails() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_downstream_run_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_release_capsule_command(
            &fixture.bin_dir.join(LECTERN_CERTIFICATE_BIN),
            ...
            Some("forced downstream native lectern run failure"),
            None,
            &expected_downstream_podium_receipt_session_dir(&paths),
            &expected_downstream_decision_packet_session_dir(&paths),
            &paths.nested_lectern_certificate_session_dir,
        ...
        assert!(report
            .reason
            .contains("failed running downstream lectern-certificate contract under"));
        assert!(!report
            .reason
        ...
        assert!(!report.reason.contains("native podium"));
    }

    #[test]
    fn run_names_lectern_certificate_when_downstream_native_verify_fails() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_downstream_verify_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_release_capsule_command(
            ...
            &fixture.handoff_bundle_status_json_path,
            None,
            Some("forced downstream native lectern verify failure"),
            &expected_downstream_podium_receipt_session_dir(&paths),
            &expected_downstream_decision_packet_session_dir(&paths),
            &paths.nested_lectern_certificate_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed verifying downstream lectern-certificate contract under"));
        assert!(!report
            .reason
    ...
    #[test]
    fn run_uses_lectern_placeholders_when_missing_snapshot_blocks_nested_handoff_summaries() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_latest_missing_snapshot_placeholders",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_podium_receipt_latest_command(
            ...
            &fixture.podium_receipt_latest_run_json_path,
            &fixture.podium_receipt_latest_verify_json_path,
            Some("forced latest-podium-receipt plan failure without snapshot"),
            None,
            None,
            &fixture.root,
            &paths.nested_podium_receipt_latest_session_dir,
        ...
        assert_eq!(
            report
                .downstream_lectern_certificate_command_summary
                .as_deref(),
            Some("<lectern-certificate-run-unavailable>")
        );
        assert_eq!(
            report.verify_lectern_certificate_command_summary.as_deref(),
            Some("<lectern-certificate-verify-unavailable>")
        );

        let session: PackageLecternCertificateLatestSession =
            load_json(&paths.session_path).unwrap();
        assert_eq!(
            session.downstream_lectern_certificate_command_summary,
            "<lectern-certificate-run-unavailable>"
        );
        assert_eq!(
            session.verify_lectern_certificate_command_summary,
            "<lectern-certificate-verify-unavailable>"
        );
    }

    #[test]
    fn verify_names_lectern_certificate_when_nested_native_verify_is_non_green() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_verify_non_green_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        verify_json["verdict"] = json!("tiny_live_package_lectern_certificate_verify_invalid");
        verify_json["reason"] = json!("forced nested lectern-certificate verify non-green");
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("nested lectern-certificate verification is non-green")
        }));
        assert!(!verify.verification_mismatches.iter().any(|entry| {
    ...
    #[test]
    fn verify_names_latest_lectern_certificate_when_wrapper_result_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_wrapper_result_mismatch_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored latest-lectern-certificate result") }));
        assert!(!verify
            .verification_mismatches
            .iter()
    ...
    #[test]
    fn verify_names_latest_lectern_certificate_when_activation_authorized_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_activation_authorized_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn verify_fails_when_copied_native_pinnacle_receipt_sha256_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_pinnacle_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy pinnacle_receipt_sha256")
        }));
    }

    #[test]
    fn verify_fails_when_copied_nested_latest_podium_receipt_explicit_statement_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_launch_packet_explicit_statement_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.latest_podium_receipt_report_path).unwrap();
        report["explicit_statement"] =
            json!("tampered nested latest release capsule explicit statement");
        persist_json(&paths.latest_podium_receipt_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored latest-podium-receipt copy explicit_statement")
        }));
    }
    ...
    fn verify_rejects_tampered_stale_substructure_wrapper_mode_string() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_wrapper_mode_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("latest-lectern-certificate stored report mode") }));
    }

    #[test]
    fn wrapper_self_modes_and_self_command_summaries_use_lectern_latest_contract() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_self_mode_and_summary_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let plan = plan_latest_lectern_certificate_report(&fixture.plan_config()).unwrap();
        assert_eq!(plan.mode, "plan_latest_lectern_certificate");
        assert!(plan
            .run_latest_lectern_certificate_command_summary
            .as_deref()
            .unwrap()
            .contains(LECTERN_CERTIFICATE_LATEST_BIN));
        assert!(plan
            .run_latest_lectern_certificate_command_summary
            .as_deref()
            .unwrap()
            .contains("--run-latest-lectern-certificate"));
        assert!(!plan
            .run_latest_lectern_certificate_command_summary
            .as_deref()
            .unwrap()
        ...
        assert!(plan
            ...
            .as_deref()
            .unwrap()
            .contains("--verify-latest-lectern-certificate"));
        assert!(!plan
            .verify_latest_lectern_certificate_command_summary
            .as_deref()
            .unwrap()
        ...
        let run = run_latest_lectern_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(run.mode, "run_latest_lectern_certificate");

        let verify = verify_latest_lectern_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(verify.mode, "verify_latest_lectern_certificate");

        let session: PackageLecternCertificateLatestSession =
            load_json(&lectern_certificate_latest_paths(&fixture.wrapper_session_dir).session_path)
                .unwrap();
        assert!(session
            .run_latest_lectern_certificate_command_summary
            .contains(LECTERN_CERTIFICATE_LATEST_BIN));
        ...
        assert!(session_json
            .get("verify_lectern_certificate_command_summary")
            .is_some());
        assert!(session_json
    ...
    #[test]
    fn native_lectern_input_uses_real_latest_podium_receipt_session_path() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_native_input_session_dir_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);

        let downstream = expected_downstream_podium_receipt_session_dir(&paths);
        let expected = paths
            .nested_podium_receipt_latest_session_dir
            .join("tiny_live_activation_package_podium_receipt_latest.podium_receipt_session");
        ...
        assert!(paths
            ...
            .unwrap()
            .to_string_lossy()
            .contains("lectern_certificate_session"));
        assert!(!paths
            .nested_lectern_certificate_session_dir
            .file_name()
            .unwrap()
    ...
    #[test]
    fn run_persists_copied_native_lectern_session_and_status_under_real_lectern_filenames() {
        let fixture = fake_podium_receipt_latest_fixture(
            "lectern_certificate_latest_native_lectern_filenames",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        let real_session =
            nested_lectern_certificate_session_path(&paths.nested_lectern_certificate_session_dir);
        let real_status =
            nested_lectern_certificate_status_path(&paths.nested_lectern_certificate_session_dir);
        ...
        let stale_podium_session = paths
            .nested_lectern_certificate_session_dir
            .join("tiny_live_activation_package_podium_receipt.session.json");
        let stale_podium_status = paths
            .nested_lectern_certificate_session_dir
            .join("tiny_live_activation_package_podium_receipt.status.json");

        assert!(
            real_session.exists(),
            "missing real native lectern session path"
        );
        assert!(
            real_status.exists(),
            "missing real native lectern status path"
        );
        assert!(
        ...
        assert!(
            !stale_podium_session.exists(),
            "stale native podium session filename should not be used"
        );
        assert!(
            !stale_podium_status.exists(),
            "stale native podium status filename should not be used"
        );
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_decision_packet_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy decision_packet_result")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_attestation_seal_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy attestation_seal_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_registry_entry_session_dir_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_registry_entry_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy registry_entry_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_notarization_receipt_session_dir_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_notarization_receipt_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy notarization_receipt_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_chain_fingerprint_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy chain_fingerprint_summary")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_chain_fingerprint_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy chain_fingerprint_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_chain_fingerprint_algorithm_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy chain_fingerprint_algorithm")
        }));
    }
    ...
    fn verify_fails_when_copied_native_completion_certificate_closure_certificate_sha256_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_closure_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy closure_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_archive_receipt_sha256_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_archive_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy archive_receipt_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_finality_receipt_sha256_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_finality_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy finality_receipt_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_consummation_record_sha256_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_consummation_record_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy consummation_record_sha256")
        }));
    }
    ...
    {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_completion_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy completion_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_decision_packet_anchor_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy decision_packet_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_current_authorization_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored lectern-certificate copy execute_frozen_result")
        }));
    }

    #[test]
    fn verify_fails_when_copied_nested_lectern_certificate_verdict_drifts() {
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_release_capsule_verdict_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| entry.contains("stored lectern-certificate copy verdict")));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_current_live_cutover_controller_summary_drifts(
        ...
        let fixture = fake_podium_receipt_latest_fixture(
            "podium_receipt_latest_verify_current_live_cutover_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn all_modes_preserve_stage3_and_pre_activation_refusal_semantics_and_do_not_imply_activation_authorization_by_themselves(
        ...
        {
            let fixture = fake_podium_receipt_latest_fixture(
                "podium_receipt_latest_pre_activation",
                "refused_now_by_pre_activation_gate",
                "refused_now_by_pre_activation_gate",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn fake_podium_receipt_latest_fixture(
        label: &str,
        podium_receipt_latest_run_result: &str,
        handoff_bundle_result: &str,
    ) -> FakePodiumReceiptLatestFixture {
        ...
        let podium_receipt_latest_plan_json_path = temp_dir.join("podium_receipt_latest.plan.json");
        let podium_receipt_latest_run_json_path = temp_dir.join("podium_receipt_latest.run.json");
        let podium_receipt_latest_verify_json_path =
            temp_dir.join("podium_receipt_latest.verify.json");
        ...
        let fixture = FakePodiumReceiptLatestFixture {
            temp_dir,
            root,
            wrapper_session_dir,
            output_script_path,
            ...
            install_root,
            backend_command,
            podium_receipt_latest_plan_json_path,
            podium_receipt_latest_run_json_path,
            podium_receipt_latest_verify_json_path,
        ...
        persist_json(
            &fixture.podium_receipt_latest_plan_json_path,
            &default_podium_receipt_latest_plan_report(&fixture),
        )
        .unwrap();
        persist_json(
            &fixture.podium_receipt_latest_run_json_path,
            &default_podium_receipt_latest_run_report(&fixture, podium_receipt_latest_run_result),
        )
        .unwrap();
        persist_json(
            &fixture.podium_receipt_latest_verify_json_path,
            &default_podium_receipt_latest_verify_report(
                &fixture,
                podium_receipt_latest_run_result,
            ),
        )
        .unwrap();
        persist_json(
    ...
    fn default_podium_receipt_latest_plan_report(
        fixture: &FakePodiumReceiptLatestFixture,
    ) -> Value {
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        json!({
            "generated_at": Utc::now(),
            "mode": "plan_latest_podium_receipt",
            "verdict": PODIUM_RECEIPT_LATEST_PLAN_READY,
            "reason": "latest immutable chain resolves cleanly into accepted latest-podium-receipt contract",
            ...
            "wrapper_timeout_ms": 1200,
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_podium_receipt_latest_session_dir.display().to_string(),
    ...
    fn canonical_podium_receipt_latest_result(result: &str) -> &str {
        match result {
            "runnable_when_gate_truth_turns_green" => {
                "ready_for_manual_execution_when_gate_turns_green"
            }
    ...
    fn default_podium_receipt_latest_run_report(
        fixture: &FakePodiumReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let result = canonical_podium_receipt_latest_result(result);
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_podium_receipt_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_podium_receipt_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_podium_receipt_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
        ...
        json!({
            ...
            "verdict": verdict,
            "reason": format!("latest-podium-receipt {result}"),
            "latest_top_step_name": "clerestory_certificate",
            "latest_top_session_dir": fixture.latest_top_session_dir.display().to_string(),
            "historical_execute_frozen_session_dir": fixture.historical_execute_frozen_session_dir.display().to_string(),
            "turn_green_session_dir": fixture.turn_green_session_dir.display().to_string(),
    ...
    fn default_podium_receipt_latest_verify_report(
        fixture: &FakePodiumReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let mut report = default_podium_receipt_latest_run_report(fixture, result);
        report["mode"] = json!("verify_latest_podium_receipt");
        report["verdict"] = json!(PODIUM_RECEIPT_LATEST_VERIFY_OK);
    ...
    fn default_provenance_certificate_session(fixture: &FakePodiumReceiptLatestFixture) -> Value {
        let paths = lectern_certificate_latest_paths(&fixture.wrapper_session_dir);
        let podium_receipt_session_dir = expected_downstream_podium_receipt_session_dir(&paths);
        ...
        let pedestal_certificate_session_dir = fixture
            .temp_dir
            .join("downstream-podium-receipt-pedestal-certificate-session");
        ...
        json!({
            "session_version": "1",
            "planned_at": Utc::now(),
            "podium_receipt_session_dir": podium_receipt_session_dir.display().to_string(),
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": lectern_certificate_latest_paths(&fixture.wrapper_session_dir).nested_lectern_certificate_session_dir.display().to_string(),
            "verify_podium_receipt_command_summary": verify_lectern_certificate_input_command_summary(&podium_receipt_session_dir),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
            ...
            "lectern_certificate_summary": "Lectern certificate identity: fixture immutable armorial seal over verified podium-receipt truth.",
            "lectern_certificate_sha256": "fixture-lectern-certificate-sha256",
            "lectern_certificate_algorithm": "sha256",
            "explicit_statement": "the verified podium-receipt session is the primary input here; this immutable lectern certificate freezes one final armorial identity over the fully culminated chain without enabling production execution"
        })
    }
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_lectern_certificate_session_dir.display().to_string(),
            "podium_receipt_session_dir": expected_downstream_podium_receipt_session_dir(&paths).display().to_string(),
            "pedestal_certificate_session_dir": pedestal_certificate_session_dir.display().to_string(),
            "plinth_receipt_session_dir": plinth_receipt_session_dir.display().to_string(),
            "substructure_certificate_session_dir": substructure_certificate_session_dir.display().to_string(),
            "basal_receipt_session_dir": basal_receipt_session_dir.display().to_string(),
            ...
            "current_pre_activation_gate_verdict": gate_verdict,
            "current_pre_activation_gate_reason": gate_reason,
            "podium_receipt_step": Value::Null,
            ...
            "podium_receipt_sha256": "fixture-podium-receipt-sha256",
            "podium_receipt_algorithm": "sha256",
            "lectern_certificate_summary": "Lectern certificate identity: fixture status immutable armorial seal over verified podium-receipt truth.",
            "lectern_certificate_sha256": "fixture-lectern-certificate-sha256",
            "lectern_certificate_algorithm": "sha256",
            "explicit_statement": "this lectern certificate is archival and read-only; it never executes or enables the frozen controller"
        })
    }
    ...
    fn default_provenance_certificate_run_report(
        ...
        let lectern_certificate_verdict = match result {
            "ready_for_manual_execution_when_gate_turns_green" => {
                "tiny_live_package_lectern_certificate_ready_for_manual_execution_when_gate_turns_green"
            }
            "refused_now_by_stage3" => "tiny_live_package_lectern_certificate_refused_now_by_stage3",
            "refused_now_by_pre_activation_gate" => {
                "tiny_live_package_lectern_certificate_refused_now_by_pre_activation_gate"
            }
            _ => "tiny_live_package_lectern_certificate_refused_now_by_invalid_or_drifted_contract",
        };
        let substructure_certificate_session_dir = fixture
            .temp_dir
            .join("downstream-basal-receipt-substructure-certificate-session");
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_lectern_certificate",
            "verdict": lectern_certificate_verdict,
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_lectern_certificate_session_dir.display().to_string(),
            "lectern_certificate_session_path": nested_lectern_certificate_session_path(&paths.nested_lectern_certificate_session_dir).display().to_string(),
            "lectern_certificate_status_path": nested_lectern_certificate_status_path(&paths.nested_lectern_certificate_session_dir).display().to_string(),
            "archived_podium_receipt_report_path": paths.nested_lectern_certificate_session_dir.join("tiny_live_activation_package_lectern_certificate.podium_receipt.report.json").display().to_string(),
            "result": result,
            ...
            "podium_receipt_step": status["podium_receipt_step"],
            "verify_podium_receipt_command_summary": verify_lectern_certificate_input_command_summary(&expected_downstream_podium_receipt_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
            ...
            "lectern_certificate_summary": status["lectern_certificate_summary"],
            "lectern_certificate_sha256": status["lectern_certificate_sha256"],
            "lectern_certificate_algorithm": status["lectern_certificate_algorithm"],
    ...
    fn write_fake_podium_receipt_latest_command(
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-podium-receipt session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-podium-receipt \"*) mode='plan';;\n  *\" --run-latest-podium-receipt \"*) mode='run';;\n  *\" --verify-latest-podium-receipt \"*) mode='verify';;\n  *) echo 'unexpected latest-podium-receipt mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_podium_receipt_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
    ...
    fn overwrite_fake_podium_receipt_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        verify_json_path: &Path,
    ...
    fn write_fake_release_capsule_command(
        ...
        let nested_session_path = nested_lectern_certificate_session_path(session_dir);
        let nested_status_path = nested_lectern_certificate_status_path(session_dir);
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --podium-receipt-session-dir {} \"*) : ;;\n  *) echo 'unexpected podium-receipt-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --confirm-decision-packet-session-dir {} \"*) : ;;\n  *) echo 'unexpected confirm-decision-packet-session-dir' >&2; exit 1;;\nesac\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected lectern-certificate session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --run-live-package-lectern-certificate \"*) mode='run';;\n  *\" --verify-live-package-lectern-certificate \"*) mode='verify';;\n  *) echo 'unexpected lectern-certificate mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  run) mkdir -p '{}'; cp '{}' '{}'; cp '{}' '{}'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
