...
use copybot_app::tiny_live_activation::package_consummation_record_completion_certificate;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_completion_certificate --consummation-record-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-completion-certificate | --render-live-package-completion-certificate | --run-live-package-completion-certificate | --verify-live-package-completion-certificate)";
const COMPLETION_CERTIFICATE_SESSION_VERSION: &str = "1";
const COMPLETION_CERTIFICATE_STATUS_VERSION: &str = "1";
const COMPLETION_CERTIFICATE_ALGORITHM: &str = "sha256";
const COMPLETION_CERTIFICATE_SESSION_EXPLICIT_STATEMENT: &str =
...
const COMPLETION_CERTIFICATE_STATUS_EXPLICIT_STATEMENT: &str =
    "this completion certificate is archival and read-only; it never executes or enables the frozen controller";
const COMPLETION_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT: &str =
...
enum Mode {
    PlanLivePackageCompletionCertificate,
    RenderLivePackageCompletionCertificate,
    RunLivePackageCompletionCertificate,
    VerifyLivePackageCompletionCertificate,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageCompletionCertificateVerdict {
    TinyLivePackageCompletionCertificatePlanReady,
    TinyLivePackageCompletionCertificateRendered,
    TinyLivePackageCompletionCertificateRefusedNowByStage3,
    TinyLivePackageCompletionCertificateRefusedNowByPreActivationGate,
    TinyLivePackageCompletionCertificateRefusedNowByInvalidOrDriftedContract,
    TinyLivePackageCompletionCertificateReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageCompletionCertificateVerifyOk,
    TinyLivePackageCompletionCertificateVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageCompletionCertificateResult {
    RefusedNowByStage3,
...
struct PackageCompletionCertificateStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageCompletionCertificateSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackageCompletionCertificateStatus {
    status_version: String,
    updated_at: DateTime<Utc>,
    session_dir: String,
    consummation_record_session_dir: String,
    result: LivePackageCompletionCertificateResult,
    reason: String,
    ...
    consummation_record_step: Option<PackageCompletionCertificateStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackageCompletionCertificatePaths {
...
struct PackageCompletionCertificateReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageCompletionCertificateVerdict,
    reason: String,
    consummation_record_session_dir: String,
...
struct NestedFinalityReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
struct CompletionCertificatePayload<'a> {
    result: &'a str,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut consummation_record_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--consummation-record-session-dir" => {
                consummation_record_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--consummation-record-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-completion-certificate" => set_mode(
                &mut mode,
                Mode::PlanLivePackageCompletionCertificate,
                arg.as_str(),
            )?,
            "--render-live-package-completion-certificate" => set_mode(
                &mut mode,
                Mode::RenderLivePackageCompletionCertificate,
                arg.as_str(),
            )?,
            "--run-live-package-completion-certificate" => set_mode(
                &mut mode,
                Mode::RunLivePackageCompletionCertificate,
                arg.as_str(),
            )?,
            "--verify-live-package-completion-certificate" => set_mode(
                &mut mode,
                Mode::VerifyLivePackageCompletionCertificate,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackageCompletionCertificate) && output_path.is_none() {
        bail!("missing --output for --render-live-package-completion-certificate");
    }
    if matches!(
        mode,
        Mode::RunLivePackageCompletionCertificate | Mode::VerifyLivePackageCompletionCertificate
    ) && session_dir.is_none()
    {
    ...
    let consummation_record_session_dir = consummation_record_session_dir
        .ok_or_else(|| anyhow!("missing --consummation-record-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackageCompletionCertificate => {
            plan_live_package_completion_certificate_report(&config)?
        }
        Mode::RenderLivePackageCompletionCertificate => {
            render_live_package_completion_certificate_report(&config)?
        }
        Mode::RunLivePackageCompletionCertificate => {
            run_live_package_completion_certificate_report(&config)?
        }
        Mode::VerifyLivePackageCompletionCertificate => {
            verify_live_package_completion_certificate_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_completion_certificate_report(
    config: &Config,
) -> Result<PackageCompletionCertificateReport> {
    let (consummation_record, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_consummation_record_completion_certificate::verify_live_package_consummation_record_for_completion_certificate(
                    &config.consummation_record_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                    ...
                    Err(_) => {
                        let raw_contract = package_consummation_record_completion_certificate::load_live_package_consummation_record_contract_for_completion_certificate(
                            &config.consummation_record_session_dir,
                        )?;
    ...
    let session_dir = config.session_dir.clone().unwrap_or_else(|| {
        config
            .consummation_record_session_dir
            .join("completion-certificate")
    });
    let result = serialize_enum(&classification.0);
    let completion_certificate_sha256 = compute_completion_certificate_sha256(
        &result,
        consummation_record.handoff_bundle_result.as_deref(),
        consummation_record.decision_packet_result.as_deref(),
        consummation_record.execute_frozen_result.as_deref(),
        consummation_record
        ...
        consummation_record
            ...
            .as_deref(),
        &consummation_record.reviewed_frozen_live_cutover_controller_command_summary,
        &consummation_record.chain_fingerprint_sha256,
        &consummation_record.chain_fingerprint_algorithm,
        &consummation_record.release_capsule_digest_manifest_sha256,
        consummation_record.release_capsule_digest_manifest_entry_count,
        &consummation_record.release_capsule_digest_algorithm,
        &consummation_record.ledger_seal_sha256,
        &consummation_record.ledger_seal_algorithm,
        &consummation_record.registry_entry_sha256,
        &consummation_record.registry_entry_algorithm,
        &consummation_record.filing_certificate_sha256,
        &consummation_record.filing_certificate_algorithm,
        &consummation_record.archive_receipt_sha256,
        &consummation_record.archive_receipt_algorithm,
        &consummation_record.closure_certificate_summary,
        &consummation_record.closure_certificate_sha256,
        &consummation_record.closure_certificate_algorithm,
        &consummation_record.finality_receipt_sha256,
        &consummation_record.finality_receipt_algorithm,
        &consummation_record.consummation_record_sha256,
        &consummation_record.consummation_record_algorithm,
    )?;

    Ok(PackageCompletionCertificateReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_completion_certificate".to_string(),
        verdict:
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificatePlanReady,
        reason: format!(
            "consummation-record-native completion certificate is explicit for verified consummation-record session {}; this command stays archival and read-only while freezing one final omega identity over the fully consummated chain",
            config.consummation_record_session_dir.display()
        ),
        consummation_record_session_dir: config.consummation_record_session_dir.display().to_string(),
        registry_entry_session_dir: Some(
            consummation_record.registry_entry_session_dir.display().to_string(),
        ),
        ...
        review_receipt_session_dir: Some(
            consummation_record.review_receipt_session_dir.display().to_string(),
        ),
        handoff_bundle_session_dir: Some(
            consummation_record.handoff_bundle_session_dir.display().to_string(),
        ),
        ...
        turn_green_session_dir: Some(
            consummation_record.turn_green_session_dir.display().to_string(),
        ),
        launch_packet_session_dir: Some(
            consummation_record.launch_packet_session_dir.display().to_string(),
        ),
        package_dir: Some(consummation_record.package_dir.display().to_string()),
        install_root: Some(consummation_record.install_root.display().to_string()),
        target_service_name: Some(consummation_record.target_service_name.clone()),
        backend_command: Some(consummation_record.backend_command.clone()),
        wrapper_timeout_ms: Some(consummation_record.wrapper_timeout_ms),
        service_status_max_staleness_ms: Some(consummation_record.service_status_max_staleness_ms),
        session_dir: Some(session_dir.display().to_string()),
        completion_certificate_session_path: None,
        completion_certificate_status_path: None,
        archived_consummation_record_report_path: None,
        result: Some(result),
        handoff_bundle_result: consummation_record.handoff_bundle_result.clone(),
        decision_packet_result: consummation_record.decision_packet_result.clone(),
        execute_frozen_result: consummation_record.execute_frozen_result.clone(),
        current_pre_activation_gate_verdict: consummation_record
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: consummation_record
            .current_pre_activation_gate_reason
            .clone(),
        consummation_record_step: None,
        verify_consummation_record_command_summary: Some(verify_consummation_record_command_summary(
            &config.consummation_record_session_dir,
        )),
        ...
        provenance_certificate_summary: Some(consummation_record.provenance_certificate_summary.clone()),
        chain_fingerprint_summary: Some(consummation_record.chain_fingerprint_summary.clone()),
        chain_fingerprint_sha256: Some(consummation_record.chain_fingerprint_sha256.clone()),
        chain_fingerprint_algorithm: Some(consummation_record.chain_fingerprint_algorithm.clone()),
        release_capsule_digest_manifest_sha256: Some(
            consummation_record.release_capsule_digest_manifest_sha256.clone(),
        ),
        release_capsule_digest_manifest_entry_count: Some(
            consummation_record.release_capsule_digest_manifest_entry_count,
        ),
        release_capsule_digest_algorithm: Some(
            consummation_record.release_capsule_digest_algorithm.clone(),
        ),
        notarization_receipt_summary: Some(consummation_record.notarization_receipt_summary.clone()),
        ledger_seal_summary: Some(consummation_record.ledger_seal_summary.clone()),
        ledger_seal_sha256: Some(consummation_record.ledger_seal_sha256.clone()),
        ledger_seal_algorithm: Some(consummation_record.ledger_seal_algorithm.clone()),
        registry_entry_summary: Some(consummation_record.registry_entry_summary.clone()),
        registry_entry_sha256: Some(consummation_record.registry_entry_sha256.clone()),
        registry_entry_algorithm: Some(consummation_record.registry_entry_algorithm.clone()),
        filing_certificate_summary: Some(consummation_record.filing_certificate_summary.clone()),
        filing_certificate_sha256: Some(consummation_record.filing_certificate_sha256.clone()),
        filing_certificate_algorithm: Some(consummation_record.filing_certificate_algorithm.clone()),
        archive_receipt_summary: Some(consummation_record.archive_receipt_summary.clone()),
        archive_receipt_sha256: Some(consummation_record.archive_receipt_sha256.clone()),
        archive_receipt_algorithm: Some(consummation_record.archive_receipt_algorithm.clone()),
        closure_certificate_summary: Some(consummation_record.closure_certificate_summary.clone()),
        closure_certificate_sha256: Some(consummation_record.closure_certificate_sha256.clone()),
        closure_certificate_algorithm: Some(
            consummation_record.closure_certificate_algorithm.clone(),
        ),
        finality_receipt_summary: Some(consummation_record.finality_receipt_summary.clone()),
        finality_receipt_sha256: Some(consummation_record.finality_receipt_sha256.clone()),
        finality_receipt_algorithm: Some(consummation_record.finality_receipt_algorithm.clone()),
        consummation_record_summary: Some(consummation_record.consummation_record_summary.clone()),
        consummation_record_sha256: Some(consummation_record.consummation_record_sha256.clone()),
        consummation_record_algorithm: Some(consummation_record.consummation_record_algorithm.clone()),
        completion_certificate_summary: Some(completion_certificate_summary(
            classification.0,
            &consummation_record.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        completion_certificate_sha256: Some(completion_certificate_sha256),
        completion_certificate_algorithm: Some(COMPLETION_CERTIFICATE_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: COMPLETION_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_completion_certificate_report(
    config: &Config,
) -> Result<PackageCompletionCertificateReport> {
    let plan = plan_live_package_completion_certificate_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_completion_certificate --consummation-record-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-completion-certificate --json\ncopybot_tiny_live_activation_package_completion_certificate --consummation-record-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-completion-certificate --json\n",
        shell_escape_path(&config.consummation_record_session_dir),
    ...
    Ok(PackageCompletionCertificateReport {
        generated_at: Utc::now(),
        mode: "render_live_package_completion_certificate".to_string(),
        verdict:
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateRendered,
        reason: format!(
            "rendered completion-certificate run/verify script to {}; the primary input remains the immutable consummation-record session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: COMPLETION_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_completion_certificate_report(
    config: &Config,
) -> Result<PackageCompletionCertificateReport> {
    ...
    let verified_consummation_record = match package_consummation_record_completion_certificate::verify_live_package_consummation_record_for_completion_certificate(
        &config.consummation_record_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify consummation-record session under {}: {error}",
                    config.consummation_record_session_dir.display()
                ),
            ))
        }
    ...
    .with_context(|| {
        format!(
            "completion-certificate session dir {} overlaps the managed live target surface derived from verified consummation-record install_root {}",
            session_dir.display(),
            verified_consummation_record.contract.install_root.display()
        )
    })?;
    ensure_clean_completion_certificate_session_dir(session_dir)?;
    let paths = completion_certificate_paths(session_dir);
    ...
    let consummation_record_step = Some(step_artifact(
        &paths.consummation_record_report_path,
        "verify_live_package_consummation_record",
        &verified_consummation_record.verdict,
        &verified_consummation_record.reason,
        verified_consummation_record.generated_at,
    ));
    let status = PackageCompletionCertificateStatus {
        status_version: COMPLETION_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        consummation_record_session_dir: config
            .consummation_record_session_dir
            .display()
            .to_string(),
        result,
        ...
        current_pre_activation_gate_reason: verified_consummation_record
            ...
            .current_pre_activation_gate_reason
            .clone(),
        consummation_record_step: consummation_record_step.clone(),
        ...
        consummation_record_algorithm: verified_consummation_record
            ...
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: completion_certificate_summary(
            result,
            &verified_consummation_record
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        completion_certificate_sha256: completion_certificate_sha256.clone(),
        completion_certificate_algorithm: COMPLETION_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: COMPLETION_CERTIFICATE_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_completion_certificate_report(
    config: &Config,
) -> Result<PackageCompletionCertificateReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = completion_certificate_paths(session_dir);
    let session: PackageCompletionCertificateSession = load_json(&paths.session_path)?;
    let status: PackageCompletionCertificateStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.consummation_record_session_dir,
        &config.consummation_record_session_dir.display().to_string(),
        "consummation-record consummation_record_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
    ...
    compare_string(
        &status.consummation_record_session_dir,
        &config.consummation_record_session_dir.display().to_string(),
        "consummation-record status consummation_record_session_dir",
        &mut mismatches,
    );
    ...
    let stored_consummation_record_report: serde_json::Value = load_required_step_json(
        &status.consummation_record_step,
        &paths.consummation_record_report_path,
        "consummation_record_step",
        &mut mismatches,
    )?;
    let stored_consummation_record_report_view: NestedFinalityReceiptReportView =
        serde_json::from_value(stored_consummation_record_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested consummation-record report {}",
                paths.consummation_record_report_path.display()
            )
        })?;
    let verified_consummation_record_report_view: NestedFinalityReceiptReportView =
        serde_json::from_value(verified_consummation_record.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested consummation-record report for {}",
                    config.consummation_record_session_dir.display()
                )
            },
        )?;
    compare_json_ignoring_generated_at(
        &stored_consummation_record_report,
        &verified_consummation_record.report_json,
        "completion-certificate archived consummation-record report json",
        &mut mismatches,
    );
    compare_string(
        &stored_consummation_record_report_view.mode,
        &verified_consummation_record_report_view.mode,
        "completion-certificate archived consummation-record report mode",
        &mut mismatches,
    );
    compare_string(
        &stored_consummation_record_report_view.verdict,
        &verified_consummation_record_report_view.verdict,
        "completion-certificate archived consummation-record report verdict",
        &mut mismatches,
    );
    compare_string(
        &stored_consummation_record_report_view.reason,
        &verified_consummation_record_report_view.reason,
        "completion-certificate archived consummation-record report reason",
        &mut mismatches,
    );
    if stored_consummation_record_report_view.generated_at
        != verified_consummation_record_report_view.generated_at
    {
        mismatches.push(format!(
            "completion-certificate archived consummation-record report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_consummation_record_report_view.generated_at,
            verified_consummation_record_report_view.generated_at
        ));
    }
    let expected_consummation_record_step = step_artifact(
        &paths.consummation_record_report_path,
        &verified_consummation_record_report_view.mode,
        &verified_consummation_record_report_view.verdict,
        &verified_consummation_record_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_consummation_record_step,
        "consummation-record consummation_record_step",
        &mut mismatches,
    );
    if verified_consummation_record.verdict != "tiny_live_package_consummation_record_verify_ok" {
        mismatches.push(format!(
            "nested consummation-record verification is non-green: {}",
            verified_consummation_record.reason
        ));
    }

    let classification = classify_verified_consummation_record(&verified_consummation_record);
    let expected_completion_certificate_sha256 = compute_completion_certificate_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_consummation_record.contract,
        classification.0,
        &expected_completion_certificate_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "completion-certificate session contract does not match freshly verified consummation-record truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_consummation_record.contract,
        classification,
        expected_consummation_record_step.clone(),
        &expected_completion_certificate_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "completion-certificate status contract does not match freshly verified consummation-record truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_consummation_record.contract,
        TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateVerifyOk,
        &expected_status,
    ))
}

fn verify_consummation_record_command_summary(consummation_record_session_dir: &Path) -> String {
    format!(
...
fn classify_consummation_contract(
    contract: &package_consummation_record_completion_certificate::LivePackageConsummationRecordContractView,
    assume_verified: bool,
) -> (
    LivePackageCompletionCertificateResult,
    TinyLivePackageCompletionCertificateVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackageCompletionCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateRefusedNowByInvalidOrDriftedContract,
            "the consummation-record chain is not verified, so this completion certificate remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackageCompletionCertificateResult::RefusedNowByStage3,
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateRefusedNowByStage3,
            "the immutable consummation record remains refused now by Stage 3, so this completion certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackageCompletionCertificateResult::RefusedNowByPreActivationGate,
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateRefusedNowByPreActivationGate,
            "the immutable consummation record remains refused now by the pre-activation gate, so this completion certificate remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackageCompletionCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable consummation record is coherent and this completion certificate is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackageCompletionCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateRefusedNowByInvalidOrDriftedContract,
            "the verified consummation-record chain is invalid, drifted, or non-runnable, so this completion certificate remains refused"
                .to_string(),
        ),
    }
}
...
fn classify_verified_consummation_record(
    verified: &package_consummation_record_completion_certificate::VerifiedLivePackageConsummationRecordCompletionCertificateStep,
) -> (
    LivePackageCompletionCertificateResult,
    TinyLivePackageCompletionCertificateVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_consummation_record_verify_ok" {
        return (
            LivePackageCompletionCertificateResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateRefusedNowByInvalidOrDriftedContract,
            "the immutable consummation record did not verify cleanly, so this completion certificate remains refused"
                .to_string(),
        );
    }
...
fn completion_certificate_summary(
    result: LivePackageCompletionCertificateResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackageCompletionCertificateResult::RefusedNowByStage3 => {
            "Completion certificate outcome: the immutable consummated chain remains refused now by Stage 3; archive this final omega seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageCompletionCertificateResult::RefusedNowByPreActivationGate => {
            "Completion certificate outcome: the immutable consummated chain remains refused now by the pre-activation gate; archive this final omega seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackageCompletionCertificateResult::RefusedNowByInvalidOrDriftedContract => {
            "Completion certificate outcome: the immutable consummated chain is invalid or drifted; archive this final omega seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackageCompletionCertificateResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Completion certificate outcome: this immutable omega seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn completion_certificate_paths(session_dir: &Path) -> PackageCompletionCertificatePaths {
    PackageCompletionCertificatePaths {
        session_path: session_dir
            .join("tiny_live_activation_package_completion_certificate.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_completion_certificate.status.json"),
...
fn ensure_clean_completion_certificate_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing completion-certificate session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating completion-certificate session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackageCompletionCertificateSession {
    PackageCompletionCertificateSession {
        session_version: COMPLETION_CERTIFICATE_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        consummation_record_session_dir: config
            .consummation_record_session_dir
            .display()
        ...
        session_dir: session_dir.display().to_string(),
        verify_consummation_record_command_summary: verify_consummation_record_command_summary(
            &config.consummation_record_session_dir,
        ),
        ...
        completion_certificate_sha256: completion_certificate_sha256.to_string(),
        completion_certificate_algorithm: COMPLETION_CERTIFICATE_ALGORITHM.to_string(),
        explicit_statement: COMPLETION_CERTIFICATE_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    consummation_record_step: PackageCompletionCertificateStepArtifact,
    completion_certificate_sha256: &str,
) -> PackageCompletionCertificateStatus {
    PackageCompletionCertificateStatus {
        status_version: COMPLETION_CERTIFICATE_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        consummation_record_step: Some(consummation_record_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackageCompletionCertificatePaths,
    contract: &package_consummation_record_completion_certificate::LivePackageConsummationRecordContractView,
    verdict: TinyLivePackageCompletionCertificateVerdict,
    status: &PackageCompletionCertificateStatus,
) -> PackageCompletionCertificateReport {
    PackageCompletionCertificateReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateVerifyOk
        {
            "verify_live_package_completion_certificate".to_string()
        } else {
            "run_live_package_completion_certificate".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        completion_certificate_session_path: Some(paths.session_path.display().to_string()),
        completion_certificate_status_path: Some(paths.status_path.display().to_string()),
        archived_consummation_record_report_path: Some(
        ...
        consummation_record_step: status.consummation_record_step.clone(),
        verify_consummation_record_command_summary: Some(verify_consummation_record_command_summary(
            &config.consummation_record_session_dir,
        )),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
        ...
        explicit_statement: if verdict
            == TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateVerifyOk
        {
            COMPLETION_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackageCompletionCertificateReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackageCompletionCertificate => {
                "plan_live_package_completion_certificate".to_string()
            }
            Mode::RenderLivePackageCompletionCertificate => {
                "render_live_package_completion_certificate".to_string()
            }
            Mode::RunLivePackageCompletionCertificate => {
                "run_live_package_completion_certificate".to_string()
            }
        ...
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable completion certificate refused before persisting because the consummation-record artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "consummation-record-native completion-certificate verification failed".to_string()
    });
    PackageCompletionCertificateReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_completion_certificate".to_string(),
        verdict:
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateVerifyInvalid,
        reason,
        consummation_record_session_dir: config.consummation_record_session_dir.display().to_string(),
        registry_entry_session_dir: Some(contract.registry_entry_session_dir.display().to_string()),
        notarization_receipt_session_dir: Some(
        ...
        explicit_statement: COMPLETION_CERTIFICATE_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackageCompletionCertificateStepArtifact>,
    expected_path: &Path,
    label: &str,
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageCompletionCertificateStepArtifact {
    PackageCompletionCertificateStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackageCompletionCertificateStepArtifact>,
    expected: &PackageCompletionCertificateStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_completion_certificate_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&CompletionCertificatePayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn render_report_lines(report: &PackageCompletionCertificateReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "consummation_record_session_dir={}",
            report.consummation_record_session_dir
        ),
    ];
    ...
    if let Some(value) = &report.completion_certificate_sha256 {
        lines.push(format!("completion_certificate_sha256={value}"));
    }
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_completion_certificate_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify =
            verify_live_package_completion_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateVerifyOk
        );
    ...
    fn drifted_finality_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            ...
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_consummation_record",
                "verdict": "tiny_live_package_consummation_record_verify_invalid",
                "reason": "drifted",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        let error = run_live_package_completion_certificate_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = completion_certificate_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_consummation_record_step_path() {
        let fixture = fake_command_fixture(
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_completion_certificate_report(&fixture.run_config()).unwrap();
        let paths = completion_certificate_paths(&fixture.completion_certificate_session_dir);
        let mut status: PackageCompletionCertificateStatus = load_json(&paths.status_path).unwrap();
        status.consummation_record_step.as_mut().unwrap().path =
    ...
    fn verify_rejects_tampered_consummation_record_text() {
        ...
        run_live_package_completion_certificate_report(&fixture.run_config()).unwrap();
        let paths = completion_certificate_paths(&fixture.completion_certificate_session_dir);
        let mut session: PackageCompletionCertificateSession =
            load_json(&paths.session_path).unwrap();
        session.completion_certificate_summary = "tampered consummation summary".to_string();
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = completion_certificate_paths(&fixture.completion_certificate_session_dir);
        let mut status: PackageCompletionCertificateStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackageCompletionCertificateResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    fn verify_rejects_tampered_nested_finality_receipt_report_content() {
        ...
        let mut report: serde_json::Value =
            load_json(&paths.consummation_record_report_path).unwrap();
    ...
    fn verify_rejects_tampered_chain_ledger_registry_filing_archive_closure_finality_or_consummation_identity_fields(
    ) {
        {
            ...
            run_live_package_completion_certificate_report(&fixture.run_config()).unwrap();
            let paths = completion_certificate_paths(&fixture.completion_certificate_session_dir);
            let mut status: PackageCompletionCertificateStatus =
                load_json(&paths.status_path).unwrap();
            status.chain_fingerprint_sha256 = "tampered-chain".to_string();
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_completion_certificate_report(&fixture.verify_config())
                    .unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageCompletionCertificateVerdict::TinyLivePackageCompletionCertificateVerifyInvalid
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        completion_certificate_session_dir: PathBuf,
        consummation_record_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackageCompletionCertificate,
                consummation_record_session_dir: self.consummation_record_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.completion_certificate_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackageCompletionCertificate,
                consummation_record_session_dir: self.consummation_record_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        persist_json(
            ...
            &stored_consummation_record_session_fixture(
                &finality_receipt_session_dir,
                &archive_receipt_session_dir,
                &registry_entry_session_dir,
                &notarization_receipt_session_dir,
        ...
        persist_json(
            ...
            &stored_consummation_record_status_fixture(&verify_report),
        )
        .unwrap();
        persist_json(
            &consummation_record_session_dir.join(
        ...
        bind_consummation_record_report_to_fixture(
            &mut verify_report,
            &finality_receipt_session_dir,
            &archive_receipt_session_dir,
            &registry_entry_session_dir,
        ...
        let verify_path = fixture_dir.join("consummation-record.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_consummation_record_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_consummation_record"),
            &verify_path,
            &finality_receipt_session_dir,
            &decision_packet_session_dir,
            &consummation_record_session_dir,
    ...
    fn bind_consummation_record_report_to_fixture(
        ...
        object.insert(
            "verify_finality_receipt_command_summary".to_string(),
            json!(format!(
        ...
        object.insert(
            ...
            json!("consummation-record verify statement"),
        );
    }
    ...
    fn consummation_record_verify_report(
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_consummation_record",
            "verdict": "tiny_live_package_consummation_record_verify_ok",
            "reason": "consummation-record chain is coherent",
            "result": result,
    ...
    fn stored_consummation_record_session_fixture(
        finality_receipt_session_dir: &Path,
        archive_receipt_session_dir: &Path,
        registry_entry_session_dir: &Path,
        notarization_receipt_session_dir: &Path,
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("consummation-record statement"),
        );
        Value::Object(object)
    }

    fn stored_consummation_record_status_fixture(verify_report: &Value) -> Value {
        let mut object = Map::new();
        object.insert(
            "result".to_string(),
            json!(verify_report
        ...
        object.insert(
            "explicit_statement".to_string(),
            json!("consummation-record status statement"),
        );
        Value::Object(object)
    }
}
