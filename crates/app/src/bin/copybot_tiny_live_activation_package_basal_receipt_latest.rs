...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_basal_receipt_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-basal-receipt | --render-latest-basal-receipt-script | --run-latest-basal-receipt | --verify-latest-basal-receipt)";
const BASAL_RECEIPT_LATEST_SESSION_VERSION: &str = "1";
const BASAL_RECEIPT_LATEST_STATUS_VERSION: &str = "1";
const BASAL_RECEIPT_LATEST_BIN: &str = "copybot_tiny_live_activation_package_basal_receipt_latest";
const BEDROCK_CERTIFICATE_LATEST_BIN: &str =
    "copybot_tiny_live_activation_package_bedrock_certificate_latest";
const BASAL_RECEIPT_BIN: &str = "copybot_tiny_live_activation_package_basal_receipt";
const BEDROCK_CERTIFICATE_LATEST_PLAN_READY: &str =
    "tiny_live_package_bedrock_certificate_latest_plan_ready";
const BEDROCK_CERTIFICATE_LATEST_VERIFY_OK: &str =
    "tiny_live_package_bedrock_certificate_latest_verify_ok";
const BASAL_RECEIPT_VERIFY_OK: &str = "tiny_live_package_basal_receipt_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-basal-receipt handoff binds the latest persisted immutable package chain to the accepted basal-receipt contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestBasalReceipt,
    RenderLatestBasalReceiptScript,
    RunLatestBasalReceipt,
    VerifyLatestBasalReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageBasalReceiptLatestVerdict {
    TinyLivePackageBasalReceiptLatestPlanReady,
    TinyLivePackageBasalReceiptLatestScriptRendered,
    TinyLivePackageBasalReceiptLatestRefusedByMissingLatestChain,
    TinyLivePackageBasalReceiptLatestRefusedByInvalidLatestChain,
    TinyLivePackageBasalReceiptLatestRefusedByDriftedBasalReceiptContract,
    TinyLivePackageBasalReceiptLatestRefusedNowByStage3,
    TinyLivePackageBasalReceiptLatestRefusedNowByPreActivationGate,
    TinyLivePackageBasalReceiptLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageBasalReceiptLatestVerifyOk,
    TinyLivePackageBasalReceiptLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageBasalReceiptLatestResult {
    RefusedByMissingLatestChain,
    RefusedByInvalidLatestChain,
    RefusedByDriftedBasalReceiptContract,
    RefusedNowByStage3,
...
struct PackageBasalReceiptLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageBasalReceiptLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_bedrock_certificate_latest_session_dir: String,
    nested_basal_receipt_session_dir: String,
    ...
    resolve_latest_bedrock_certificate_command_summary: String,
    run_latest_basal_receipt_command_summary: String,
    verify_latest_basal_receipt_command_summary: String,
    downstream_basal_receipt_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageBasalReceiptLatestStatus {
    ...
    nested_basal_receipt_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackageBasalReceiptLatestResult,
    reason: String,
    latest_bedrock_certificate_plan_verdict: Option<String>,
    latest_bedrock_certificate_verdict: Option<String>,
    basal_receipt_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_bedrock_certificate_plan_step: Option<PackageBasalReceiptLatestStepArtifact>,
    latest_bedrock_certificate_step: Option<PackageBasalReceiptLatestStepArtifact>,
    basal_receipt_step: Option<PackageBasalReceiptLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackageBasalReceiptLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_bedrock_certificate_plan_report_path: PathBuf,
    latest_bedrock_certificate_report_path: PathBuf,
    basal_receipt_report_path: PathBuf,
    nested_bedrock_certificate_latest_session_dir: PathBuf,
    nested_basal_receipt_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageBasalReceiptLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageBasalReceiptLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    basal_receipt_latest_session_path: Option<String>,
    basal_receipt_latest_status_path: Option<String>,
    basal_receipt_latest_report_path: Option<String>,
    latest_bedrock_certificate_plan_report_path: Option<String>,
    latest_bedrock_certificate_report_path: Option<String>,
    basal_receipt_report_path: Option<String>,
    nested_bedrock_certificate_latest_session_dir: Option<String>,
    nested_basal_receipt_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_bedrock_certificate_command_summary: Option<String>,
    run_latest_basal_receipt_command_summary: Option<String>,
    verify_latest_basal_receipt_command_summary: Option<String>,
    downstream_basal_receipt_command_summary: Option<String>,
...
#[derive(Debug, Clone)]
struct LatestBedrockCertificateSnapshot {
...
struct ResolvedLatestBedrockCertificate {
    snapshot: LatestBedrockCertificateSnapshot,
    latest_bedrock_certificate_plan_raw: Value,
    latest_bedrock_certificate_plan: BedrockCertificateLatestReportView,
}
...
enum ResolutionFailureKind {
    MissingLatestChain,
    InvalidLatestChain,
    DriftedBedrockCertificateContract,
}

#[derive(Debug, Clone)]
struct LatestBedrockCertificateResolutionFailure {
    kind: ResolutionFailureKind,
    reason: String,
    snapshot: Option<LatestBedrockCertificateSnapshot>,
    latest_bedrock_certificate_plan_raw: Option<Value>,
    latest_bedrock_certificate_plan_verdict: Option<String>,
    latest_bedrock_certificate_plan_reason: Option<String>,
    latest_bedrock_certificate_plan_generated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone, Deserialize)]
struct BedrockCertificateLatestReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Deserialize)]
struct BasalReceiptSessionView {
    foundation_receipt_session_dir: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-basal-receipt" => {
                set_mode(&mut mode, Mode::PlanLatestBasalReceipt, arg.as_str())?
            }
            "--render-latest-basal-receipt-script" => set_mode(
                &mut mode,
                Mode::RenderLatestBasalReceiptScript,
                arg.as_str(),
            )?,
            "--run-latest-basal-receipt" => {
                set_mode(&mut mode, Mode::RunLatestBasalReceipt, arg.as_str())?
            }
            "--verify-latest-basal-receipt" => {
                set_mode(&mut mode, Mode::VerifyLatestBasalReceipt, arg.as_str())?
            }
    ...
    if matches!(
        mode,
        Mode::PlanLatestBasalReceipt
            | Mode::RenderLatestBasalReceiptScript
            | Mode::RunLatestBasalReceipt
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestBasalReceiptScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestBasalReceipt | Mode::VerifyLatestBasalReceipt
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestBasalReceipt => plan_latest_basal_receipt_report(&config)?,
        Mode::RenderLatestBasalReceiptScript => render_latest_basal_receipt_script_report(&config)?,
        Mode::RunLatestBasalReceipt => run_latest_basal_receipt_report(&config)?,
        Mode::VerifyLatestBasalReceipt => verify_latest_basal_receipt_report(&config)?,
    };
    if config.json {
...
fn plan_latest_basal_receipt_report(config: &Config) -> Result<PackageBasalReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
    ...
    let suggested_session_dir = config
        .session_dir
        .clone()
        .unwrap_or_else(|| root.join("tiny_live_activation_package_basal_receipt_latest.session"));
    let paths = basal_receipt_latest_paths(&suggested_session_dir);

    match resolve_latest_bedrock_certificate(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            "plan_latest_basal_receipt",
            TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-bedrock-certificate lineage via accepted latest-bedrock-certificate session {}, exact decision-packet confirmation anchor {}, and the accepted latest-basal-receipt contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_bedrock_certificate_latest_session_dir.display(),
...
fn render_latest_basal_receipt_script_report(
    config: &Config,
) -> Result<PackageBasalReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    ...
    match resolve_latest_bedrock_certificate(root, &paths) {
        Ok(resolved) => {
            write_script(
                ...
                &format!(
                    ...
                    run_latest = command_summary(
                        BEDROCK_CERTIFICATE_LATEST_BIN,
                        &latest_bedrock_certificate_run_args(
                            root,
                            &paths.nested_bedrock_certificate_latest_session_dir,
                        ),
                    ),
                    verify_latest = command_summary(
                        BEDROCK_CERTIFICATE_LATEST_BIN,
                        &latest_bedrock_certificate_verify_args(
                            &paths.nested_bedrock_certificate_latest_session_dir,
                        ),
                    ),
                    run_handoff_bundle = downstream_basal_receipt_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_bedrock_certificate_session_dir(&paths),
                        &paths.nested_basal_receipt_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                "render_latest_basal_receipt_script",
                TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestScriptRendered,
                format!(
                    "rendered latest-basal-receipt handoff script to {} using the accepted latest-bedrock-certificate lineage and the accepted basal-receipt contract",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_basal_receipt_report(config: &Config) -> Result<PackageBasalReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-basal-receipt session dir {}",
            session_dir.display()
        );
    }
    let paths = basal_receipt_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-basal-receipt")?;

    let resolution = resolve_latest_bedrock_certificate(root, &paths);
    match resolution {
        Ok(resolved) => {
            run_latest_basal_receipt_with_resolution(root, session_dir, &paths, &resolved)
        }
        Err(failure) => run_latest_basal_receipt_failure(root, session_dir, &paths, &failure),
    }
}

fn verify_latest_basal_receipt_report(config: &Config) -> Result<PackageBasalReceiptLatestReport> {
    let session_dir = config
        .session_dir
        .as_ref()
    ...
    let session: PackageBasalReceiptLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_basal_receipt_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-basal-receipt session artifact {}: {error}",
                    paths.session_path.display()
                )],
            ))
        }
    };
    let status: PackageBasalReceiptLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_basal_receipt_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-basal-receipt status artifact {}: {error}",
                    paths.status_path.display()
                )],
            ))
        }
    };
    let stored_report: PackageBasalReceiptLatestReport = match load_json(&paths.report_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(verify_invalid_basal_receipt_latest_report_without_session(
                session_dir,
                vec![format!(
                    "failed reading latest-basal-receipt report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-basal-receipt root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-basal-receipt session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-basal-receipt status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-basal-receipt report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_bedrock_certificate_latest_session_dir,
        &paths
            .nested_bedrock_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-basal-receipt session nested_bedrock_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_bedrock_certificate_latest_session_dir,
        &paths
            .nested_bedrock_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-basal-receipt status nested_bedrock_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-basal-receipt report nested_bedrock_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_basal_receipt_session_dir,
        &paths.nested_basal_receipt_session_dir.display().to_string(),
        "latest-basal-receipt session nested_basal_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_basal_receipt_session_dir,
        &paths.nested_basal_receipt_session_dir.display().to_string(),
        "latest-basal-receipt status nested_basal_receipt_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.nested_basal_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_basal_receipt_session_dir
                .display()
                .to_string()
                .as_str(),
        ),
        "latest-basal-receipt report nested_basal_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-basal-receipt session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-basal-receipt status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-basal-receipt report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.basal_receipt_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-basal-receipt report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.basal_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-basal-receipt report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.basal_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-basal-receipt report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-basal-receipt session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-basal-receipt status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-basal-receipt report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_basal_receipt",
        "latest-basal-receipt stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-basal-receipt stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-basal-receipt stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-basal-receipt stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-basal-receipt stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report
            .latest_bedrock_certificate_plan_verdict
            .as_deref(),
        status.latest_bedrock_certificate_plan_verdict.as_deref(),
        "stored report latest_bedrock_certificate_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.latest_bedrock_certificate_verdict.as_deref(),
        status.latest_bedrock_certificate_verdict.as_deref(),
        "stored report latest_bedrock_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.basal_receipt_verdict.as_deref(),
        status.basal_receipt_verdict.as_deref(),
        "stored report basal_receipt_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: BedrockCertificateLatestReportView = load_required_step_json(
                &status.latest_bedrock_certificate_plan_step,
                &paths.latest_bedrock_certificate_plan_report_path,
                "latest_bedrock_certificate_plan_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_bedrock_certificate_plan_step.as_ref(),
                &paths.latest_bedrock_certificate_plan_report_path,
                "plan_latest_bedrock_certificate",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_bedrock_certificate_plan_step",
                &mut mismatches,
            );
            compare_bedrock_certificate_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_bedrock_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_bedrock_certificate_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let stored_latest_run: BedrockCertificateLatestReportView = load_required_step_json(
                &status.latest_bedrock_certificate_step,
                &paths.latest_bedrock_certificate_report_path,
                "latest_bedrock_certificate_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_bedrock_certificate_step.as_ref(),
                &paths.latest_bedrock_certificate_report_path,
                "run_latest_bedrock_certificate",
                &stored_latest_run.verdict,
                &stored_latest_run.reason,
                stored_latest_run.generated_at,
                "stored latest_bedrock_certificate_step",
                &mut mismatches,
            );
            compare_bedrock_certificate_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_bedrock_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_bedrock_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_bedrock_certificate_verdict.as_deref(),
            ...
            let actual_latest_run: BedrockCertificateLatestReportView = match load_json(
                &nested_bedrock_certificate_latest_report_path(
                    &paths.nested_bedrock_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed loading actual nested latest-bedrock-certificate report under {}: {error}",
                        paths.nested_bedrock_certificate_latest_session_dir.display()
                    ));
                    fallback_bedrock_certificate_latest_copy()
                }
            };
            compare_bedrock_certificate_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-bedrock-certificate session under {}: {error}",
                        paths.nested_bedrock_certificate_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: BedrockCertificateLatestReportView =
                    serde_json::from_value(latest_verify_raw)
                        .context("failed parsing nested latest-bedrock-certificate verify json")?;
                if latest_verify.verdict != BEDROCK_CERTIFICATE_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-bedrock-certificate verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: BasalReceiptReportView = load_required_step_json(
                &status.basal_receipt_step,
                &paths.basal_receipt_report_path,
                "basal_receipt_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.basal_receipt_step.as_ref(),
                &paths.basal_receipt_report_path,
                "run_live_package_basal_receipt",
                &stored_handoff_bundle.verdict,
                &stored_handoff_bundle.reason,
                stored_handoff_bundle.generated_at,
                "stored basal_receipt_step",
                &mut mismatches,
            );
            ...
            compare_option_string(
                ...
                "stored status basal_receipt_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.basal_receipt_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: BasalReceiptSessionView = match load_json(
                &nested_bedrock_certificate_session_path(&paths.nested_basal_receipt_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed reading nested basal-receipt session under {}: {error}",
                        paths.nested_basal_receipt_session_dir.display()
                    ));
            ...
            let nested_handoff_bundle_status: BasalReceiptStatusView = match load_json(
                &nested_bedrock_certificate_status_path(&paths.nested_basal_receipt_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed reading nested basal-receipt status under {}: {error}",
                        paths.nested_basal_receipt_session_dir.display()
                    ));
            ...
            let handoff_bundle_verify_raw = match run_json_command(
                BASAL_RECEIPT_BIN,
                &bedrock_certificate_verify_args(
                    &resolved.snapshot,
                    &expected_downstream_bedrock_certificate_session_dir(&paths),
                    &paths.nested_basal_receipt_session_dir,
                ),
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested basal-receipt session under {}: {error}",
                        paths.nested_basal_receipt_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: BasalReceiptReportView =
                    serde_json::from_value(handoff_bundle_verify_raw)
                        .context("failed parsing nested basal-receipt verify json")?;
                compare_option_string(
                    stored_handoff_bundle.registry_entry_session_dir.as_deref(),
                    handoff_bundle_verify.registry_entry_session_dir.as_deref(),
                    "stored basal-receipt copy registry_entry_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored basal-receipt copy notarization_receipt_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .consummation_record_algorithm
                        .as_deref(),
                    "stored basal-receipt copy consummation_record_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .completion_certificate_summary
                        .as_deref(),
                    "stored basal-receipt copy completion_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .completion_certificate_sha256
                        .as_deref(),
                    "stored basal-receipt copy completion_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .capstone_certificate_summary
                        .as_deref(),
                    "stored basal-receipt copy capstone_certificate_summary against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle.capstone_certificate_sha256.as_deref(),
                    handoff_bundle_verify.capstone_certificate_sha256.as_deref(),
                    "stored basal-receipt copy capstone_certificate_sha256 against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .bedrock_certificate_algorithm
                        .as_deref(),
                    "stored basal-receipt copy bedrock_certificate_algorithm against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                    ...
                    handoff_bundle_verify
                        .reviewed_frozen_live_cutover_controller_command_summary
                        .as_deref(),
                    "stored basal-receipt copy reviewed_frozen_live_cutover_controller_command_summary",
                    &mut mismatches,
                );
                if handoff_bundle_verify.verdict != BASAL_RECEIPT_VERIFY_OK {
                    mismatches.push(format!(
            ...
            let expected_result = bedrock_certificate_latest_result_from_nested_status(
                nested_handoff_bundle_status.result.as_str(),
            )
            .unwrap_or(LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract);
        ...
        Err(failure) => {
            ...
            if failure.latest_bedrock_certificate_plan_raw.is_some() {
                let stored_latest_plan: BedrockCertificateLatestReportView =
                ...
                compare_option_string(
                    status.latest_bedrock_certificate_plan_verdict.as_deref(),
                    failure.latest_bedrock_certificate_plan_verdict.as_deref(),
                    "stored status latest_bedrock_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report
                        ...
                        .as_deref(),
                    failure.latest_bedrock_certificate_plan_verdict.as_deref(),
                    "stored report latest_bedrock_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
                status.latest_bedrock_certificate_verdict.as_deref(),
                None,
                "stored status latest_bedrock_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_bedrock_certificate_verdict.as_deref(),
                None,
                "stored report latest_bedrock_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.basal_receipt_verdict.as_deref(),
                None,
                "stored status basal_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.basal_receipt_verdict.as_deref(),
                None,
                "stored report basal_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestVerifyOk
    } else {
        TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-basal-receipt session under {} is coherent with the current latest immutable chain and the accepted basal-receipt contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-basal-receipt verification failed".to_string())
    };

    Ok(PackageBasalReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_basal_receipt".to_string(),
        verdict,
        reason,
        ...
        basal_receipt_latest_session_path: Some(paths.session_path.display().to_string()),
        basal_receipt_latest_status_path: Some(paths.status_path.display().to_string()),
        basal_receipt_latest_report_path: Some(paths.report_path.display().to_string()),
        latest_bedrock_certificate_plan_report_path: Some(
            paths
                .latest_bedrock_certificate_plan_report_path
                .display()
                .to_string(),
        ),
        latest_bedrock_certificate_report_path: Some(
            paths.latest_bedrock_certificate_report_path.display().to_string(),
        ),
        basal_receipt_report_path: Some(paths.basal_receipt_report_path.display().to_string()),
        nested_bedrock_certificate_latest_session_dir: Some(
            paths
                .nested_bedrock_certificate_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_basal_receipt_session_dir: Some(
            paths.nested_basal_receipt_session_dir.display().to_string(),
        ),
        ...
        latest_bedrock_certificate_plan_verdict: status.latest_bedrock_certificate_plan_verdict.clone(),
        latest_bedrock_certificate_verdict: status.latest_bedrock_certificate_verdict.clone(),
        basal_receipt_verdict: status.basal_receipt_verdict.clone(),
        ...
        resolve_latest_bedrock_certificate_command_summary: Some(
            session.resolve_latest_bedrock_certificate_command_summary.clone(),
        ),
        run_latest_basal_receipt_command_summary: Some(
            session.run_latest_basal_receipt_command_summary.clone(),
        ),
        verify_latest_basal_receipt_command_summary: Some(
            session.verify_latest_basal_receipt_command_summary.clone(),
        ),
        downstream_basal_receipt_command_summary: Some(
            session.downstream_basal_receipt_command_summary.clone(),
        ),
        ...
        explicit_statement:
            "this verification proves only whether the latest immutable chain still resolves cleanly into the accepted basal-receipt contract; it never runs the frozen controller"
                .to_string(),
    })
}

fn run_latest_basal_receipt_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackageBasalReceiptLatestPaths,
    ...
    persist_json(
        &paths.latest_bedrock_certificate_plan_report_path,
        &resolved.latest_bedrock_certificate_plan_raw,
    )?;

    let latest_bedrock_certificate_plan_step = Some(step_artifact(
        &paths.latest_bedrock_certificate_plan_report_path,
        "plan_latest_bedrock_certificate",
        &resolved.latest_bedrock_certificate_plan.verdict,
        &resolved.latest_bedrock_certificate_plan.reason,
        resolved.latest_bedrock_certificate_plan.generated_at,
    ));

    let latest_bedrock_certificate_run_raw = match run_json_command(
        BEDROCK_CERTIFICATE_LATEST_BIN,
        &latest_bedrock_certificate_run_args(
            root,
            &paths.nested_bedrock_certificate_latest_session_dir,
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                paths,
                Some(resolved),
                LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract,
                format!(
                    "failed running nested latest-bedrock-certificate handoff under {}: {error}",
                    paths
                        .nested_bedrock_certificate_latest_session_dir
                        .display()
                ),
    ...
    persist_json(
        &paths.latest_bedrock_certificate_report_path,
        &latest_bedrock_certificate_run_raw,
    )?;
    let latest_bedrock_certificate_run: BedrockCertificateLatestReportView =
        serde_json::from_value(latest_bedrock_certificate_run_raw)
            .context("failed parsing nested latest-bedrock-certificate run json")?;
    let latest_bedrock_certificate_step = Some(step_artifact(
        &paths.latest_bedrock_certificate_report_path,
        "run_latest_bedrock_certificate",
        &latest_bedrock_certificate_run.verdict,
        &latest_bedrock_certificate_run.reason,
        latest_bedrock_certificate_run.generated_at,
    ));

    if let Some(terminal_failure) =
        latest_bedrock_certificate_terminal_failure(&latest_bedrock_certificate_run)
    {
        let status = refusal_status(
            root,
            session_dir,
            ...
            Some(resolved),
            terminal_failure,
            latest_bedrock_certificate_run.reason.clone(),
            latest_bedrock_certificate_plan_step,
            latest_bedrock_certificate_step,
            None,
        );
    ...
    compare_bedrock_certificate_latest_run_against_snapshot(
        &latest_bedrock_certificate_run,
        &resolved.snapshot,
        &paths.nested_bedrock_certificate_latest_session_dir,
        &mut run_mismatches,
    );
    ...
    let latest_bedrock_certificate_verify: BedrockCertificateLatestReportView =
        match run_json_command(
            ...
            Err(error) => {
                let status = refusal_status(
                    ...
                    LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract,
                    format!(
                    "failed verifying nested latest-bedrock-certificate handoff under {}: {error}",
                    paths.nested_bedrock_certificate_latest_session_dir.display()
                ),
                    latest_bedrock_certificate_plan_step,
                    latest_bedrock_certificate_step,
    ...
    if latest_bedrock_certificate_verify.verdict != BEDROCK_CERTIFICATE_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract,
            latest_bedrock_certificate_verify.reason,
            latest_bedrock_certificate_plan_step,
            latest_bedrock_certificate_step,
            None,
        );
    ...
    let release_capsule_run_raw = match run_json_command(
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract,
                format!(
                    "failed running downstream basal-receipt contract under {}: {error}",
                    paths.nested_basal_receipt_session_dir.display()
                ),
                latest_bedrock_certificate_plan_step,
                latest_bedrock_certificate_step,
    ...
    persist_json(&paths.basal_receipt_report_path, &release_capsule_run_raw)?;
    let provenance_certificate_run: BasalReceiptReportView =
        serde_json::from_value(release_capsule_run_raw)
            .context("failed parsing nested basal-receipt run json")?;
    let basal_receipt_step = Some(step_artifact(
        &paths.basal_receipt_report_path,
        "run_live_package_basal_receipt",
    ...
    let provenance_certificate_verify: BasalReceiptReportView = match run_json_command(
        BASAL_RECEIPT_BIN,
        &bedrock_certificate_verify_args(
            &resolved.snapshot,
            &expected_downstream_bedrock_certificate_session_dir(paths),
        ...
        Err(error) => {
            let status = refusal_status(
                ...
                LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract,
                format!(
                    "failed verifying downstream basal-receipt contract under {}: {error}",
                    paths.nested_basal_receipt_session_dir.display()
                ),
                latest_bedrock_certificate_plan_step,
                latest_bedrock_certificate_step,
    ...
    if provenance_certificate_verify.verdict != BASAL_RECEIPT_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    let result = bedrock_certificate_latest_result_from_nested_status(
        provenance_certificate_run
            .result
            .as_deref()
            .unwrap_or_default(),
    ...
    .unwrap_or(LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract);
    let status = PackageBasalReceiptLatestStatus {
        status_version: BASAL_RECEIPT_LATEST_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nested_bedrock_certificate_latest_session_dir: paths
            .nested_bedrock_certificate_latest_session_dir
            .display()
            .to_string(),
        nested_basal_receipt_session_dir: paths
            .nested_basal_receipt_session_dir
            .display()
            .to_string(),
        ...
        latest_bedrock_certificate_plan_verdict: Some(
            resolved.latest_bedrock_certificate_plan.verdict.clone(),
        ),
        latest_bedrock_certificate_verdict: Some(latest_bedrock_certificate_run.verdict.clone()),
        basal_receipt_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_basal_receipt_failure(
    root: &Path,
    session_dir: &Path,
    paths: &PackageBasalReceiptLatestPaths,
    failure: &LatestBedrockCertificateResolutionFailure,
) -> Result<PackageBasalReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, None, Some(failure));
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_bedrock_certificate_plan_raw {
        persist_json(&paths.latest_bedrock_certificate_plan_report_path, value)?;
    }
...
fn resolve_latest_bedrock_certificate(
    root: &Path,
    paths: &PackageBasalReceiptLatestPaths,
) -> std::result::Result<ResolvedLatestBedrockCertificate, LatestBedrockCertificateResolutionFailure>
{
    let latest_bedrock_certificate_plan_raw = match run_json_command(
        BEDROCK_CERTIFICATE_LATEST_BIN,
        &latest_bedrock_certificate_plan_args(
            root,
            &paths.nested_bedrock_certificate_latest_session_dir,
        ),
    ) {
        Ok(value) => value,
        Err(error) => {
            return Err(LatestBedrockCertificateResolutionFailure {
                ...
                snapshot: None,
                latest_bedrock_certificate_plan_raw: None,
                latest_bedrock_certificate_plan_verdict: None,
                latest_bedrock_certificate_plan_reason: None,
                latest_bedrock_certificate_plan_generated_at: None,
            });
        }
    };
    let latest_bedrock_certificate_plan: BedrockCertificateLatestReportView =
        serde_json::from_value(latest_bedrock_certificate_plan_raw.clone()).map_err(
            |error| LatestBedrockCertificateResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-basal-receipt upstream latest-bedrock-certificate plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_bedrock_certificate_plan_raw: Some(
                    latest_bedrock_certificate_plan_raw.clone(),
                ),
                latest_bedrock_certificate_plan_verdict: None,
                latest_bedrock_certificate_plan_reason: None,
                latest_bedrock_certificate_plan_generated_at: None,
    ...
    let snapshot =
        parse_snapshot_from_latest_bedrock_certificate(&latest_bedrock_certificate_plan).ok();
    if latest_bedrock_certificate_plan.verdict != BEDROCK_CERTIFICATE_LATEST_PLAN_READY {
        return Err(LatestBedrockCertificateResolutionFailure {
            kind: map_bedrock_certificate_latest_failure_kind(
                &latest_bedrock_certificate_plan.verdict,
            ),
            reason: format!(
                "latest-basal-receipt resolution refused because nested latest-bedrock-certificate reported: {}",
                latest_bedrock_certificate_plan.reason
            ),
            snapshot,
            latest_bedrock_certificate_plan_raw: Some(latest_bedrock_certificate_plan_raw),
            latest_bedrock_certificate_plan_verdict: Some(
                latest_bedrock_certificate_plan.verdict,
            ),
            latest_bedrock_certificate_plan_reason: Some(
                latest_bedrock_certificate_plan.reason,
            ),
            latest_bedrock_certificate_plan_generated_at: Some(
                latest_bedrock_certificate_plan.generated_at,
            ),
        });
    }

    let snapshot = parse_snapshot_from_latest_bedrock_certificate(&latest_bedrock_certificate_plan)
        .map_err(|error| LatestBedrockCertificateResolutionFailure {
            kind: ResolutionFailureKind::InvalidLatestChain,
            reason: format!("latest-basal-receipt resolution failed: {error}"),
            snapshot: None,
            latest_bedrock_certificate_plan_raw: Some(latest_bedrock_certificate_plan_raw.clone()),
            latest_bedrock_certificate_plan_verdict: Some(
                latest_bedrock_certificate_plan.verdict.clone(),
            ),
            latest_bedrock_certificate_plan_reason: Some(
                latest_bedrock_certificate_plan.reason.clone(),
            ),
            latest_bedrock_certificate_plan_generated_at: Some(
                latest_bedrock_certificate_plan.generated_at,
            ),
    ...
    let mut mismatches = Vec::new();
    compare_bedrock_certificate_latest_plan_against_snapshot(
        &latest_bedrock_certificate_plan,
        &snapshot,
        &paths.nested_bedrock_certificate_latest_session_dir,
        &mut mismatches,
    );
    if !mismatches.is_empty() {
        return Err(LatestBedrockCertificateResolutionFailure {
            kind: ResolutionFailureKind::DriftedBedrockCertificateContract,
            reason: format!("latest-basal-receipt drift detected: {}", mismatches[0]),
            snapshot: Some(snapshot),
            latest_bedrock_certificate_plan_raw: Some(latest_bedrock_certificate_plan_raw),
            latest_bedrock_certificate_plan_verdict: Some(latest_bedrock_certificate_plan.verdict),
            latest_bedrock_certificate_plan_reason: Some(latest_bedrock_certificate_plan.reason),
            latest_bedrock_certificate_plan_generated_at: Some(
                latest_bedrock_certificate_plan.generated_at,
            ),
        });
    ...
    Ok(ResolvedLatestBedrockCertificate {
        snapshot,
        latest_bedrock_certificate_plan_raw,
        latest_bedrock_certificate_plan,
    })
}

fn parse_snapshot_from_latest_bedrock_certificate(
    view: &BedrockCertificateLatestReportView,
) -> Result<LatestBedrockCertificateSnapshot> {
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-basal-receipt nested latest-bedrock-certificate plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-basal-receipt nested latest-bedrock-certificate plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestBedrockCertificateSnapshot {
        latest_top_step_name,
        ...
        wrapper_timeout_ms: view.wrapper_timeout_ms.ok_or_else(|| {
            anyhow!(
                "latest-basal-receipt nested latest-bedrock-certificate plan is missing wrapper_timeout_ms"
            )
        })?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!(
                "latest-basal-receipt nested latest-bedrock-certificate plan is missing service_status_max_staleness_ms"
            )
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackageBasalReceiptLatestPaths,
    resolved: &ResolvedLatestBedrockCertificate,
) -> PackageBasalReceiptLatestReport {
    PackageBasalReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        latest_bedrock_certificate_report_path: Some(
            paths
                .latest_bedrock_certificate_report_path
                .display()
                .to_string(),
        ),
        basal_receipt_report_path: Some(paths.basal_receipt_report_path.display().to_string()),
        ...
        latest_bedrock_certificate_verdict: None,
        basal_receipt_verdict: None,
        result: None,
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_bedrock_certificate_command_summary: Some(
            resolve_latest_bedrock_certificate_command_summary(
                root,
                &paths.nested_bedrock_certificate_latest_session_dir,
            ),
        ),
        run_latest_basal_receipt_command_summary: Some(run_latest_basal_receipt_command_summary(
            root,
            session_dir,
        )),
        verify_latest_basal_receipt_command_summary: Some(
            verify_latest_basal_receipt_command_summary(session_dir),
        ),
        downstream_basal_receipt_command_summary: Some(downstream_basal_receipt_command_summary(
            &resolved.snapshot,
            &expected_downstream_bedrock_certificate_session_dir(paths),
            &paths.nested_basal_receipt_session_dir,
        )),
...
fn report_from_resolution_failure(
    ...
    PackageBasalReceiptLatestReport {
        ...
        latest_bedrock_certificate_plan_verdict: failure
            .latest_bedrock_certificate_plan_verdict
            .clone(),
        latest_bedrock_certificate_verdict: None,
        basal_receipt_verdict: None,
        ...
        downstream_basal_receipt_command_summary: snapshot.map(|value| {
            downstream_basal_receipt_command_summary(
                value,
                &expected_downstream_bedrock_certificate_session_dir(paths),
                &paths.nested_basal_receipt_session_dir,
            )
...
fn planned_session(
    ...
    failure: Option<&LatestBedrockCertificateResolutionFailure>,
) -> PackageBasalReceiptLatestSession {
    let snapshot = resolved
        .map(|value| &value.snapshot)
        .or_else(|| failure.and_then(|value| value.snapshot.as_ref()));
    PackageBasalReceiptLatestSession {
        session_version: BASAL_RECEIPT_LATEST_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        root: root.display().to_string(),
        session_dir: session_dir.display().to_string(),
        nested_bedrock_certificate_latest_session_dir: paths
        ...
        run_latest_basal_receipt_command_summary: run_latest_basal_receipt_command_summary(
            root,
            session_dir,
        ),
        verify_latest_basal_receipt_command_summary: verify_latest_basal_receipt_command_summary(
            session_dir,
        ),
        downstream_basal_receipt_command_summary: snapshot
            .map(|value| {
                downstream_basal_receipt_command_summary(
                    value,
                    &expected_downstream_bedrock_certificate_session_dir(paths),
            ...
            .unwrap_or_else(|| "<basal-receipt-run-unavailable>".to_string()),
        verify_foundation_receipt_command_summary: snapshot
            ...
            .unwrap_or_else(|| "<basal-receipt-verify-unavailable>".to_string()),
        reviewed_frozen_live_cutover_controller_command_summary: snapshot.map(|value| {
            value
                .reviewed_frozen_live_cutover_controller_command_summary
                .clone()
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackageBasalReceiptLatestPaths,
    session: &PackageBasalReceiptLatestSession,
    status: &PackageBasalReceiptLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackageBasalReceiptLatestReport {
    PackageBasalReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        nested_basal_receipt_session_dir: Some(session.nested_basal_receipt_session_dir.clone()),
        downstream_decision_packet_session_dir: Some(
            session.downstream_decision_packet_session_dir.clone(),
        ),
        latest_bedrock_certificate_plan_verdict: status
            .latest_bedrock_certificate_plan_verdict
            .clone(),
        latest_bedrock_certificate_verdict: status.latest_bedrock_certificate_verdict.clone(),
        basal_receipt_verdict: status.basal_receipt_verdict.clone(),
...
fn basal_receipt_latest_paths(session_dir: &Path) -> PackageBasalReceiptLatestPaths {
    PackageBasalReceiptLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_basal_receipt_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_basal_receipt_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_basal_receipt_latest.report.json"),
        latest_bedrock_certificate_plan_report_path: session_dir.join(
            "tiny_live_activation_package_basal_receipt_latest.bedrock_certificate_latest_plan.report.json",
        ),
        latest_bedrock_certificate_report_path: session_dir.join(
            "tiny_live_activation_package_basal_receipt_latest.bedrock_certificate_latest.report.json",
        ),
        basal_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_basal_receipt_latest.basal_receipt.report.json",
        ),
        nested_bedrock_certificate_latest_session_dir: session_dir
            .join("tiny_live_activation_package_basal_receipt_latest.bedrock_certificate_latest_session"),
        nested_basal_receipt_session_dir: session_dir.join(
            "tiny_live_activation_package_basal_receipt_latest.basal_receipt_session",
        ),
    }
}
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_bedrock_certificate_latest_session_dir
        .join("tiny_live_activation_package_bedrock_certificate_latest.decision_packet_session")
}

fn expected_downstream_bedrock_certificate_session_dir(
    paths: &PackageBasalReceiptLatestPaths,
) -> PathBuf {
    paths
        .nested_bedrock_certificate_latest_session_dir
        .join("tiny_live_activation_package_bedrock_certificate_latest.bedrock_certificate_session")
}
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_bedrock_certificate_latest_session_dir
        .join("tiny_live_activation_package_bedrock_certificate_latest.review_receipt_session")
}

fn latest_bedrock_certificate_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-bedrock-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn latest_bedrock_certificate_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-bedrock-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn latest_bedrock_certificate_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-bedrock-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn bedrock_certificate_run_args(
    snapshot: &LatestBedrockCertificateSnapshot,
    bedrock_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        "--bedrock-certificate-session-dir".to_string(),
        bedrock_certificate_session_dir.display().to_string(),
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-basal-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn bedrock_certificate_verify_args(
    snapshot: &LatestBedrockCertificateSnapshot,
    bedrock_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-live-package-basal-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn resolve_latest_bedrock_certificate_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{BEDROCK_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --plan-latest-bedrock-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn run_latest_basal_receipt_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{BASAL_RECEIPT_LATEST_BIN} --root {} --session-dir {} --run-latest-basal-receipt --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn verify_latest_basal_receipt_command_summary(session_dir: &Path) -> String {
    format!(
        "{BASAL_RECEIPT_LATEST_BIN} --session-dir {} --verify-latest-basal-receipt --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_basal_receipt_command_summary(
    snapshot: &LatestBedrockCertificateSnapshot,
    bedrock_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{BASAL_RECEIPT_BIN} --bedrock-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-basal-receipt --json",
...
fn verify_foundation_receipt_command_summary(
    ...
    format!(
        "{BASAL_RECEIPT_BIN} --bedrock-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-basal-receipt --json",
        shell_escape_path(bedrock_certificate_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
...
fn nested_bedrock_certificate_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_bedrock_certificate_latest.report.json")
}

fn nested_bedrock_certificate_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_basal_receipt.session.json")
}

fn nested_bedrock_certificate_status_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_basal_receipt.status.json")
}
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-basal-receipt script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageBasalReceiptLatestStepArtifact {
    PackageBasalReceiptLatestStepArtifact {
...
fn canonical_wrapper_mode(mode: &str) -> &str {
    match mode {
        "plan_latest_basal_receipt" => "plan_latest_basal_receipt",
        "render_latest_basal_receipt_script" => "render_latest_basal_receipt_script",
        "run_latest_basal_receipt" => "run_latest_basal_receipt",
        "verify_latest_basal_receipt" => "verify_latest_basal_receipt",
        other => other,
    }
}

fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackageBasalReceiptLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-basal-receipt status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-basal-receipt status"
        ));
        return;
    };
    compare_string(
...
fn compare_bedrock_certificate_latest_plan_against_snapshot(
    stored: &BedrockCertificateLatestReportView,
    snapshot: &LatestBedrockCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        ...
        Some(BEDROCK_CERTIFICATE_LATEST_PLAN_READY),
        "stored latest-bedrock-certificate plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_bedrock_certificate",
        "stored latest-bedrock-certificate plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-bedrock-certificate plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-bedrock-certificate plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-bedrock-certificate plan session_dir",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        ...
        "stored latest-bedrock-certificate plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-bedrock-certificate plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-bedrock-certificate plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-bedrock-certificate plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_bedrock_certificate_latest_run_against_snapshot(
    stored: &BedrockCertificateLatestReportView,
    snapshot: &LatestBedrockCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_bedrock_certificate",
        "stored latest-bedrock-certificate run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-bedrock-certificate run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-bedrock-certificate run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-bedrock-certificate run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-bedrock-certificate run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-bedrock-certificate run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-bedrock-certificate run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-bedrock-certificate run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-bedrock-certificate run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-bedrock-certificate run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-bedrock-certificate run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-bedrock-certificate run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-bedrock-certificate run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-bedrock-certificate run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-bedrock-certificate run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_bedrock_certificate_latest_copy_matches_actual(
    stored: &BedrockCertificateLatestReportView,
    actual: &BedrockCertificateLatestReportView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        Some(stored.verdict.as_str()),
        Some(actual.verdict.as_str()),
        "stored latest-bedrock-certificate copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-bedrock-certificate copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-bedrock-certificate copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-bedrock-certificate copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-bedrock-certificate copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-bedrock-certificate copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-bedrock-certificate copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-bedrock-certificate copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-bedrock-certificate copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-bedrock-certificate copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-bedrock-certificate copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-bedrock-certificate copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_bedrock_certificate_run_against_snapshot(
    stored: &BasalReceiptReportView,
    snapshot: &LatestBedrockCertificateSnapshot,
    ...
    compare_option_string(
        ...
        "stored basal-receipt run decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored basal-receipt run execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored basal-receipt run turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored basal-receipt run launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored basal-receipt run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored basal-receipt run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored basal-receipt run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored basal-receipt run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored basal-receipt run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored basal-receipt run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored basal-receipt run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.basal_receipt_session_path.as_deref(),
        Some(
        ...
        "stored basal-receipt run basal_receipt_session_path",
        mismatches,
    );
    compare_option_string(
        stored.basal_receipt_status_path.as_deref(),
        Some(
        ...
        "stored basal-receipt run basal_receipt_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_bedrock_certificate_report_path.as_deref(),
        Some(
            session_dir
                .join("tiny_live_activation_package_basal_receipt.bedrock_certificate.report.json")
                .display()
                .to_string()
                .as_str(),
        ),
        "stored basal-receipt run archived_bedrock_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        ...
        "stored basal-receipt run verify_foundation_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored basal-receipt run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}
...
fn compare_bedrock_certificate_copy_against_nested_truth(
    ...
    compare_option_string(
        ...
        "stored basal-receipt copy verify_foundation_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored basal-receipt copy release_capsule_digest_manifest_sha256",
        mismatches,
    );
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_bedrock_certificate_command_summary,
        &resolve_latest_bedrock_certificate_command_summary(
            root,
            &paths.nested_bedrock_certificate_latest_session_dir,
        ),
        "stored session resolve_latest_bedrock_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.run_latest_basal_receipt_command_summary,
        &run_latest_basal_receipt_command_summary(root, session_dir),
        "stored session run_latest_basal_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_latest_basal_receipt_command_summary,
        &verify_latest_basal_receipt_command_summary(session_dir),
        "stored session verify_latest_basal_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.downstream_basal_receipt_command_summary,
        &downstream_basal_receipt_command_summary(
            snapshot,
            &expected_downstream_bedrock_certificate_session_dir(paths),
            &paths.nested_basal_receipt_session_dir,
        ),
        "stored session downstream_basal_receipt_command_summary",
        mismatches,
    );
    compare_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_bedrock_certificate_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_basal_receipt_session_dir,
        &paths.nested_basal_receipt_session_dir.display().to_string(),
        "stored session nested_basal_receipt_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackageBasalReceiptLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackageBasalReceiptLatestPaths,
    snapshot: &LatestBedrockCertificateSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_bedrock_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report.run_latest_basal_receipt_command_summary.as_deref(),
        Some(run_latest_basal_receipt_command_summary(root, session_dir).as_str()),
        "stored report run_latest_basal_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report
            .verify_latest_basal_receipt_command_summary
            .as_deref(),
        Some(verify_latest_basal_receipt_command_summary(session_dir).as_str()),
        "stored report verify_latest_basal_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report.downstream_basal_receipt_command_summary.as_deref(),
        Some(
            downstream_basal_receipt_command_summary(
                snapshot,
                &expected_downstream_bedrock_certificate_session_dir(paths),
        ...
        "stored report downstream_basal_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        report.basal_receipt_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "stored report basal_receipt_latest_session_path",
        mismatches,
    );
    compare_option_string(
        report.basal_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "stored report basal_receipt_latest_status_path",
        mismatches,
    );
    compare_option_string(
        report.basal_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "stored report basal_receipt_latest_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report latest_bedrock_certificate_plan_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_bedrock_certificate_report_path.as_deref(),
        Some(
            paths
                .latest_bedrock_certificate_report_path
                .display()
        ...
        "stored report latest_bedrock_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        report.basal_receipt_report_path.as_deref(),
        Some(
            paths
                .basal_receipt_report_path
                .display()
                .to_string()
                .as_str(),
        ),
        "stored report basal_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report nested_bedrock_certificate_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_basal_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_basal_receipt_session_dir
                .display()
        ...
        "stored report nested_basal_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_bedrock_certificate_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_basal_receipt_session_dir,
        &paths.nested_basal_receipt_session_dir.display().to_string(),
        "stored session nested_basal_receipt_session_dir on failure",
        mismatches,
    );
...
fn map_bedrock_certificate_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_bedrock_certificate_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_bedrock_certificate_latest_refused_by_drifted_bedrock_certificate_contract" => {
            ResolutionFailureKind::DriftedBedrockCertificateContract
        }
        _ => ResolutionFailureKind::InvalidLatestChain,
    }
}

fn latest_bedrock_certificate_terminal_failure(
    report: &BedrockCertificateLatestReportView,
) -> Option<LivePackageBasalReceiptLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackageBasalReceiptLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackageBasalReceiptLatestResult::RefusedByInvalidLatestChain)
        }
        Some("refused_by_drifted_bedrock_certificate_contract") => {
            Some(LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract)
        }
        _ => None,
    }
}

fn bedrock_certificate_latest_result_from_nested_status(
    result: &str,
) -> Option<LivePackageBasalReceiptLatestResult> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => {
            Some(LivePackageBasalReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen)
        }
        "refused_now_by_stage3" => Some(LivePackageBasalReceiptLatestResult::RefusedNowByStage3),
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackageBasalReceiptLatestResult::RefusedNowByPreActivationGate)
        }
...
fn foundation_receipt_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        ...
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_basal_receipt_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}

fn result_from_failure_kind(kind: ResolutionFailureKind) -> LivePackageBasalReceiptLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackageBasalReceiptLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackageBasalReceiptLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedBedrockCertificateContract => {
            LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract
        }
...
fn verdict_for_result(
    result: LivePackageBasalReceiptLatestResult,
) -> TinyLivePackageBasalReceiptLatestVerdict {
    match result {
        LivePackageBasalReceiptLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestRefusedByMissingLatestChain
        }
        LivePackageBasalReceiptLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestRefusedByInvalidLatestChain
        }
        LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract => {
            TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestRefusedByDriftedBasalReceiptContract
        }
        LivePackageBasalReceiptLatestResult::RefusedNowByStage3 => {
            TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestRefusedNowByStage3
        }
        LivePackageBasalReceiptLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestRefusedNowByPreActivationGate
        }
        LivePackageBasalReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackageBasalReceiptLatestResult) -> String {
    match result {
        LivePackageBasalReceiptLatestResult::RefusedByMissingLatestChain => {
        ...
        LivePackageBasalReceiptLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-bedrock-certificate / basal-receipt path".to_string()
        }
        LivePackageBasalReceiptLatestResult::RefusedByDriftedBasalReceiptContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-bedrock-certificate / basal-receipt contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackageBasalReceiptLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-basal-receipt refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackageBasalReceiptLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-basal-receipt refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackageBasalReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            "the latest immutable chain now resolves into the accepted basal-receipt contract and one bounded latest-basal-receipt handoff is ready for manual execution when gate truth turns green; this wrapper still did not run or authorize the frozen controller".to_string()
        }
    }
}
...
fn refusal_status(
    ...
    latest_bedrock_certificate_step: Option<PackageBasalReceiptLatestStepArtifact>,
    basal_receipt_step: Option<PackageBasalReceiptLatestStepArtifact>,
) -> PackageBasalReceiptLatestStatus {
    ...
    PackageBasalReceiptLatestStatus {
        ...
        result,
        reason,
        latest_bedrock_certificate_plan_verdict: resolved
            .map(|value| value.latest_bedrock_certificate_plan.verdict.clone()),
        latest_bedrock_certificate_verdict: latest_bedrock_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        basal_receipt_verdict: basal_receipt_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackageBasalReceiptLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_bedrock_certificate_plan_step: failure
            .latest_bedrock_certificate_plan_raw
            .as_ref()
            .map(|_| {
                step_artifact(
                    ...
                    failure
                        ...
                        .unwrap_or("<missing>"),
                    failure
                        .latest_bedrock_certificate_plan_reason
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_bedrock_certificate_plan_generated_at
                        .unwrap_or_else(Utc::now),
                )
            }),
        latest_bedrock_certificate_step: None,
...
fn verify_invalid_basal_receipt_latest_report_without_session(
    ...
    PackageBasalReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_basal_receipt".to_string(),
        verdict: TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestVerifyInvalid,
        reason,
        root: None,
        ...
        session_dir: Some(session_dir.display().to_string()),
        basal_receipt_latest_session_path: Some(
            basal_receipt_latest_paths(session_dir)
                .session_path
                .display()
                .to_string(),
        ),
        basal_receipt_latest_status_path: Some(
            basal_receipt_latest_paths(session_dir)
                .status_path
                .display()
                .to_string(),
        ),
        basal_receipt_latest_report_path: Some(
            basal_receipt_latest_paths(session_dir)
                .report_path
                .display()
                .to_string(),
        ),
        latest_bedrock_certificate_plan_report_path: None,
        latest_bedrock_certificate_report_path: None,
        basal_receipt_report_path: None,
        nested_bedrock_certificate_latest_session_dir: None,
        nested_basal_receipt_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_bedrock_certificate_command_summary: None,
        run_latest_basal_receipt_command_summary: None,
        verify_latest_basal_receipt_command_summary: None,
        downstream_basal_receipt_command_summary: None,
...
fn fallback_bedrock_certificate_latest_copy() -> BedrockCertificateLatestReportView {
    BedrockCertificateLatestReportView {
        generated_at: Utc::now(),
...
fn fallback_notarization_receipt_status(
    _snapshot: &LatestBedrockCertificateSnapshot,
    session_dir: &Path,
) -> BasalReceiptStatusView {
    BasalReceiptStatusView {
        session_dir: session_dir.display().to_string(),
        foundation_receipt_session_dir: "<missing>".to_string(),
...
fn render_report_lines(report: &PackageBasalReceiptLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_bedrock_certificate_latest_session_dir {
        lines.push(format!(
            "nested_bedrock_certificate_latest_session_dir={value}"
        ));
    }
    ...
    if let Some(value) = &report.nested_basal_receipt_session_dir {
        lines.push(format!("nested_basal_receipt_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_basal_receipt_command_summary {
        lines.push(format!("downstream_basal_receipt_command_summary={value}"));
    }
...
mod tests {
    ...
    struct FakeNotarizationReceiptLatestFixture {
        ...
        bedrock_certificate_latest_plan_json_path: PathBuf,
        bedrock_certificate_latest_run_json_path: PathBuf,
        bedrock_certificate_latest_verify_json_path: PathBuf,
    ...
    impl FakeNotarizationReceiptLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestBasalReceipt,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestBasalReceiptScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestBasalReceipt,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestBasalReceipt,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_foundation_receipt_lineage(
        ...
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_plan_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = plan_latest_basal_receipt_report(&fixture.plan_config()).unwrap();
        let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert_eq!(
            report.verdict,
            TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestPlanReady
        );
        assert_eq!(report.mode, "plan_latest_basal_receipt");
        assert_eq!(
            report.latest_top_step_name.as_deref(),
    ...
    {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_render_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = render_latest_basal_receipt_script_report(&fixture.render_config()).unwrap();
        assert_eq!(
            report.verdict,
            TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestScriptRendered
        );
        assert_eq!(report.mode, "render_latest_basal_receipt_script");
        let script = fs::read_to_string(&fixture.output_script_path).unwrap();
        assert!(script.contains(BEDROCK_CERTIFICATE_LATEST_BIN));
        assert!(script.contains(BASAL_RECEIPT_BIN));
        assert!(script.contains("--bedrock-certificate-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-basal-receipt"));
        assert!(script.contains("--verify-live-package-basal-receipt"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            (
                "bedrock_certificate_latest_missing",
                "tiny_live_package_bedrock_certificate_latest_refused_by_missing_latest_chain",
                "no persisted immutable chain exists",
                TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestRefusedByMissingLatestChain,
            ),
            (
                "bedrock_certificate_latest_invalid",
                "tiny_live_package_bedrock_certificate_latest_refused_by_invalid_latest_chain",
                "latest immutable chain is incomplete",
                TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestRefusedByInvalidLatestChain,
            ),
            (
                "bedrock_certificate_latest_below",
                "tiny_live_package_bedrock_certificate_latest_refused_by_invalid_latest_chain",
            ...
            let mut plan: Value = load_json(&fixture.bedrock_certificate_latest_plan_json_path).unwrap();
            plan["verdict"] = json!(verdict);
            plan["reason"] = json!(reason);
            if label.ends_with("below") {
                plan["latest_top_step_name"] = json!("choir_receipt");
            }
            persist_json(&fixture.bedrock_certificate_latest_plan_json_path, &plan).unwrap();

            let report = run_latest_basal_receipt_report(&fixture.run_config()).unwrap();
    ...
    fn plan_mode_maps_real_latest_foundation_receipt_drift_verdict_to_local_drift_refusal() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_plan_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let mut plan: Value =
            load_json(&fixture.bedrock_certificate_latest_plan_json_path).unwrap();
        plan["verdict"] = json!(
            "tiny_live_package_bedrock_certificate_latest_refused_by_drifted_bedrock_certificate_contract"
        );
        plan["reason"] = json!("real accepted latest-bedrock-certificate drift");
        persist_json(&fixture.bedrock_certificate_latest_plan_json_path, &plan).unwrap();
    ...
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_foundation_receipt_reports_real_drift(
        ...
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_run_drift",
            "refused_by_drifted_bedrock_certificate_contract",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert_eq!(report.basal_receipt_verdict.as_deref(), None);
    }

    #[test]
    fn run_mode_refuses_when_nested_release_capsule_lineage_drifts_from_latest_immutable_chain() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_handoff_bundle_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let mut run: Value = load_json(&fixture.bedrock_certificate_latest_run_json_path).unwrap();
        run["package_dir"] = json!("/tampered/package");
        persist_json(&fixture.bedrock_certificate_latest_run_json_path, &run).unwrap();
        ...
        assert!(report
            .reason
            .contains("stored latest-bedrock-certificate run package_dir"));
    }
    ...
    fn verify_mode_fails_when_persisted_latest_foundation_receipt_artifacts_are_tampered_or_missing_nested_lineage_truth(
        ...
        {
            let fixture = fake_bedrock_certificate_latest_fixture(
                "bedrock_certificate_latest_verify_tampered_copy",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            run_latest_basal_receipt_report(&fixture.run_config()).unwrap();

            let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.basal_receipt_report_path).unwrap();
            report["decision_packet_session_dir"] = json!("/tampered/run-decision-packet-session");
            persist_json(&paths.basal_receipt_report_path, &report).unwrap();

            let verify = verify_latest_basal_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageBasalReceiptLatestVerdict::TinyLivePackageBasalReceiptLatestVerifyInvalid
            );
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry.contains("stored basal-receipt run decision_packet_session_dir")
            }));
        }

        {
            let fixture = fake_bedrock_certificate_latest_fixture(
                "bedrock_certificate_latest_verify_missing_nested_truth",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
            fs::remove_file(nested_bedrock_certificate_latest_report_path(
                &paths.nested_bedrock_certificate_latest_session_dir,
            ))
            .unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry
                    .contains("tiny_live_activation_package_bedrock_certificate_latest.report.json")
            }));
        }
    }
    ...
    fn verify_accepts_real_shaped_native_foundation_identity_layers_on_happy_path() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_real_shaped_foundation_paths_green",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn run_and_verify_reject_stale_keystone_wrapper_schema_for_downstream_foundation_copy() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_rejects_stale_keystone_wrapper_schema",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert!(paths.basal_receipt_report_path.ends_with(
            "tiny_live_activation_package_basal_receipt_latest.basal_receipt.report.json"
        ));
        assert!(paths.basal_receipt_report_path.exists());
        assert!(!fixture
            .wrapper_session_dir
            .join(
                "tiny_live_activation_package_basal_receipt_latest.bedrock_certificate.report.json"
            )
            .exists());

        let mut status: Value = load_json(&paths.status_path).unwrap();
        let stored_step = status
            .get("basal_receipt_step")
            .cloned()
            .expect("basal_receipt_step should be present");
        assert!(status.get("keystone_receipt_step").is_none());
        status["keystone_receipt_step"] = stored_step;
        if let Some(object) = status.as_object_mut() {
            object.remove("basal_receipt_step");
        }
        persist_json(&paths.status_path, &status).unwrap();

        let error = verify_latest_basal_receipt_report(&fixture.verify_config()).unwrap_err();
        assert!(error.to_string().contains("basal_receipt_step"));
    }

    #[test]
    fn verify_names_bedrock_certificate_when_nested_native_session_load_breaks() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_nested_foundation_session_load_break",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("failed reading nested basal-receipt session under") }));
    }

    #[test]
    fn verify_fails_when_copied_native_basal_receipt_session_path_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_foundation_session_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.basal_receipt_report_path).unwrap();
        report["basal_receipt_session_path"] = json!("/tampered/finality-receipt-session.json");
        persist_json(&paths.basal_receipt_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored basal-receipt run basal_receipt_session_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_basal_receipt_status_path_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_foundation_status_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.basal_receipt_report_path).unwrap();
        report["basal_receipt_status_path"] = json!("/tampered/finality-receipt-status.json");
        persist_json(&paths.basal_receipt_report_path, &report).unwrap();
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored basal-receipt run basal_receipt_status_path") }));
    }

    #[test]
    fn run_names_latest_foundation_receipt_when_nested_latest_wrapper_run_fails() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_nested_run_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
        overwrite_fake_bedrock_certificate_latest_command(
            &fixture.bin_dir.join(BEDROCK_CERTIFICATE_LATEST_BIN),
            &fixture.bedrock_certificate_latest_plan_json_path,
            &fixture.bedrock_certificate_latest_run_json_path,
            &fixture.bedrock_certificate_latest_verify_json_path,
            None,
            Some("forced nested latest-bedrock-certificate run failure"),
            None,
            &fixture.root,
            &paths.nested_bedrock_certificate_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed running nested latest-bedrock-certificate handoff under"));
    }

    #[test]
    fn run_names_latest_foundation_receipt_when_nested_latest_wrapper_verify_fails() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_nested_verify_failure_wording",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_bedrock_certificate_latest_command(
            ...
            None,
            None,
            Some("forced nested latest-bedrock-certificate verify failure"),
            &fixture.root,
            &paths.nested_bedrock_certificate_latest_session_dir,
        );
        ...
        assert!(report
            .reason
            .contains("failed verifying nested latest-bedrock-certificate handoff under"));
    }

    #[test]
    fn run_uses_foundation_placeholders_when_missing_snapshot_blocks_nested_handoff_summaries() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_missing_snapshot_foundation_placeholders",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        overwrite_fake_bedrock_certificate_latest_command(
            ...
            &fixture.bedrock_certificate_latest_run_json_path,
            &fixture.bedrock_certificate_latest_verify_json_path,
            Some("forced latest-bedrock-certificate plan failure without snapshot"),
            None,
            None,
            &fixture.root,
            &paths.nested_bedrock_certificate_latest_session_dir,
        ...
        assert_eq!(
            report.downstream_basal_receipt_command_summary.as_deref(),
            Some("<basal-receipt-run-unavailable>")
        );
        assert_eq!(
            report.verify_foundation_receipt_command_summary.as_deref(),
            Some("<basal-receipt-verify-unavailable>")
        );

        let session: PackageBasalReceiptLatestSession = load_json(&paths.session_path).unwrap();
        assert_eq!(
            session.downstream_basal_receipt_command_summary,
            "<basal-receipt-run-unavailable>"
        );
        assert_eq!(
            session.verify_foundation_receipt_command_summary,
            "<basal-receipt-verify-unavailable>"
        );
    }
    ...
    fn verify_fails_when_copied_native_pinnacle_receipt_sha256_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_pinnacle_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored basal-receipt copy pinnacle_receipt_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_latest_foundation_receipt_explicit_statement_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_launch_packet_explicit_statement_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.latest_bedrock_certificate_report_path).unwrap();
        report["explicit_statement"] =
            json!("tampered nested latest release capsule explicit statement");
        persist_json(&paths.latest_bedrock_certificate_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored latest-bedrock-certificate copy explicit_statement")
        }));
    }
    ...
    fn verify_rejects_tampered_stale_bedrock_wrapper_mode_string() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_wrapper_mode_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        report["mode"] = json!("run_latest_bedrock_certificate");
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("latest-basal-receipt stored report mode") }));
    }

    #[test]
    fn wrapper_self_modes_and_self_command_summaries_use_basal_latest_contract() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_self_mode_and_summary_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let plan = plan_latest_basal_receipt_report(&fixture.plan_config()).unwrap();
        assert_eq!(plan.mode, "plan_latest_basal_receipt");
        assert!(plan
            .run_latest_basal_receipt_command_summary
            .as_deref()
            .unwrap()
            .contains(BASAL_RECEIPT_LATEST_BIN));
        assert!(plan
            .run_latest_basal_receipt_command_summary
            .as_deref()
            .unwrap()
        ...
        assert!(!plan
            ...
            .as_deref()
            .unwrap()
            .contains("--run-latest-bedrock-certificate"));
        assert!(plan
            .verify_latest_basal_receipt_command_summary
            .as_deref()
            .unwrap()
        ...
        assert!(!plan
            ...
            .as_deref()
            .unwrap()
            .contains("--verify-latest-bedrock-certificate"));

        let run = run_latest_basal_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(run.mode, "run_latest_basal_receipt");

        let verify = verify_latest_basal_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(verify.mode, "verify_latest_basal_receipt");

        let session: PackageBasalReceiptLatestSession =
            load_json(&basal_receipt_latest_paths(&fixture.wrapper_session_dir).session_path)
                .unwrap();
        assert!(session
            .run_latest_basal_receipt_command_summary
            .contains(BASAL_RECEIPT_LATEST_BIN));
    ...
    fn downstream_native_bedrock_session_dir_uses_real_bedrock_certificate_session_path() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_downstream_session_dir_shape",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);

        let downstream = expected_downstream_bedrock_certificate_session_dir(&paths);
        let expected = paths.nested_bedrock_certificate_latest_session_dir.join(
    ...
    {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_native_cornerstone_filenames",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(
            real_session.exists(),
            "missing real native basal session path"
        );
        assert!(
            real_status.exists(),
            "missing real native basal status path"
        );
        assert!(
            !stale_session.exists(),
            "stale native bedrock session filename should not be used"
        );
        assert!(
            !stale_status.exists(),
            "stale native bedrock status filename should not be used"
        );
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_decision_packet_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored basal-receipt copy decision_packet_result") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_attestation_seal_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored basal-receipt copy attestation_seal_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_registry_entry_session_dir_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_registry_entry_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored basal-receipt copy registry_entry_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_notarization_receipt_session_dir_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_notarization_receipt_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored basal-receipt copy notarization_receipt_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_chain_fingerprint_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored basal-receipt copy chain_fingerprint_summary")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_chain_fingerprint_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored basal-receipt copy chain_fingerprint_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_chain_fingerprint_algorithm_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored basal-receipt copy chain_fingerprint_algorithm")
        }));
    }
    ...
    fn verify_fails_when_copied_native_completion_certificate_closure_certificate_sha256_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_closure_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored basal-receipt copy closure_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_native_finality_receipt_archive_receipt_sha256_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_archive_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored basal-receipt copy archive_receipt_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_native_finality_receipt_finality_receipt_sha256_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_finality_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored basal-receipt copy finality_receipt_sha256") }));
    }

    #[test]
    fn verify_fails_when_copied_native_consummation_record_sha256_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_consummation_record_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored basal-receipt copy consummation_record_sha256")
        }));
    }
    ...
    {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_completion_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored basal-receipt copy completion_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_decision_packet_anchor_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored basal-receipt copy decision_packet_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_current_authorization_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored basal-receipt copy execute_frozen_result") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_bedrock_certificate_verdict_drifts() {
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_release_capsule_verdict_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| entry.contains("stored basal-receipt copy verdict")));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_current_live_cutover_controller_summary_drifts(
        ...
        let fixture = fake_bedrock_certificate_latest_fixture(
            "bedrock_certificate_latest_verify_current_live_cutover_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn all_modes_preserve_stage3_and_pre_activation_refusal_semantics_and_do_not_imply_activation_authorization_by_themselves(
        ...
        {
            let fixture = fake_bedrock_certificate_latest_fixture(
                "bedrock_certificate_latest_stage3",
                "refused_now_by_stage3",
                "refused_now_by_stage3",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        {
            let fixture = fake_bedrock_certificate_latest_fixture(
                "bedrock_certificate_latest_pre_activation",
                "refused_now_by_pre_activation_gate",
                "refused_now_by_pre_activation_gate",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        {
            let fixture = fake_bedrock_certificate_latest_fixture(
                "bedrock_certificate_latest_green",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn fake_bedrock_certificate_latest_fixture(
        label: &str,
        bedrock_certificate_latest_run_result: &str,
        ...
        let bedrock_certificate_latest_plan_json_path =
            temp_dir.join("bedrock_certificate_latest.plan.json");
        let bedrock_certificate_latest_run_json_path =
            temp_dir.join("bedrock_certificate_latest.run.json");
        let bedrock_certificate_latest_verify_json_path =
            temp_dir.join("bedrock_certificate_latest.verify.json");
        ...
        let fixture = FakeNotarizationReceiptLatestFixture {
            ...
            install_root,
            backend_command,
            bedrock_certificate_latest_plan_json_path,
            bedrock_certificate_latest_run_json_path,
            bedrock_certificate_latest_verify_json_path,
        ...
        persist_json(
            &fixture.bedrock_certificate_latest_plan_json_path,
            &default_bedrock_certificate_latest_plan_report(&fixture),
        )
        .unwrap();
        persist_json(
            &fixture.bedrock_certificate_latest_run_json_path,
            &default_bedrock_certificate_latest_run_report(
                &fixture,
                bedrock_certificate_latest_run_result,
            ),
        )
        .unwrap();
        persist_json(
            &fixture.bedrock_certificate_latest_verify_json_path,
            &default_bedrock_certificate_latest_verify_report(
                &fixture,
                bedrock_certificate_latest_run_result,
            ),
        )
        ...
        let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
        write_fake_bedrock_certificate_latest_command(
            &fixture.bin_dir.join(BEDROCK_CERTIFICATE_LATEST_BIN),
            &fixture.bedrock_certificate_latest_plan_json_path,
            &fixture.bedrock_certificate_latest_run_json_path,
            &fixture.bedrock_certificate_latest_verify_json_path,
        ...
        write_fake_release_capsule_command(
            &fixture.bin_dir.join(BASAL_RECEIPT_BIN),
            &fixture.handoff_bundle_run_json_path,
            &fixture.handoff_bundle_verify_json_path,
            &fixture.handoff_bundle_session_json_path,
            &fixture.handoff_bundle_status_json_path,
    ...
    fn default_bedrock_certificate_latest_plan_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
    ) -> Value {
        let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
        json!({
            "generated_at": Utc::now(),
            "mode": "plan_latest_bedrock_certificate",
            "verdict": BEDROCK_CERTIFICATE_LATEST_PLAN_READY,
            "reason": "latest immutable chain resolves cleanly into accepted latest-bedrock-certificate contract",
            ...
            "session_dir": paths.nested_bedrock_certificate_latest_session_dir.display().to_string(),
    ...
    fn canonical_bedrock_certificate_latest_result(result: &str) -> &str {
        match result {
            "runnable_when_gate_truth_turns_green" => {
                "ready_for_manual_execution_when_gate_turns_green"
            }
    ...
    fn default_bedrock_certificate_latest_run_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let result = canonical_bedrock_certificate_latest_result(result);
        let paths = basal_receipt_latest_paths(&fixture.wrapper_session_dir);
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_bedrock_certificate_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_bedrock_certificate_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_bedrock_certificate_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_latest_bedrock_certificate",
            "verdict": verdict,
    ...
    fn default_bedrock_certificate_latest_verify_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let mut report = default_bedrock_certificate_latest_run_report(fixture, result);
        report["mode"] = json!("verify_latest_bedrock_certificate");
        report["verdict"] = json!(BEDROCK_CERTIFICATE_LATEST_VERIFY_OK);
    ...
    fn default_provenance_certificate_session(
        ...
        let snapshot = LatestBedrockCertificateSnapshot {
        ...
        json!({
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": basal_receipt_latest_paths(&fixture.wrapper_session_dir).nested_basal_receipt_session_dir.display().to_string(),
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_basal_receipt_session_dir.display().to_string(),
            "foundation_receipt_session_dir": expected_downstream_bedrock_certificate_session_dir(&paths).display().to_string(),
            "result": result,
            "reason": reason,
    ...
    fn default_provenance_certificate_run_report(
        ...
        let basal_receipt_verdict = match result {
            "ready_for_manual_execution_when_gate_turns_green" => {
                "tiny_live_package_basal_receipt_ready_for_manual_execution_when_gate_turns_green"
            }
            "refused_now_by_stage3" => "tiny_live_package_basal_receipt_refused_now_by_stage3",
            "refused_now_by_pre_activation_gate" => {
                "tiny_live_package_basal_receipt_refused_now_by_pre_activation_gate"
            }
            _ => "tiny_live_package_basal_receipt_refused_now_by_invalid_or_drifted_contract",
        };
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_basal_receipt",
            "verdict": basal_receipt_verdict,
            ...
            "release_capsule_session_dir": expected_downstream_bedrock_certificate_session_dir(&paths).display().to_string(),
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_basal_receipt_session_dir.display().to_string(),
            "basal_receipt_session_path": nested_bedrock_certificate_session_path(&paths.nested_basal_receipt_session_dir).display().to_string(),
            "basal_receipt_status_path": nested_bedrock_certificate_status_path(&paths.nested_basal_receipt_session_dir).display().to_string(),
            "archived_bedrock_certificate_report_path": paths.nested_basal_receipt_session_dir.join("tiny_live_activation_package_basal_receipt.bedrock_certificate.report.json").display().to_string(),
            "result": result,
            ...
            "bedrock_certificate_step": Value::Null,
            "verify_foundation_receipt_command_summary": foundation_receipt_verify_command_summary(&expected_downstream_bedrock_certificate_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn default_provenance_certificate_verify_report(
        ...
        report["mode"] = json!("verify_live_package_basal_receipt");
        report["verdict"] = json!(BASAL_RECEIPT_VERIFY_OK);
        report["reason"] = json!("verify ok");
    ...
    fn write_fake_bedrock_certificate_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-bedrock-certificate session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-bedrock-certificate \"*) mode='plan';;\n  *\" --run-latest-bedrock-certificate \"*) mode='run';;\n  *\" --verify-latest-bedrock-certificate \"*) mode='verify';;\n  *) echo 'unexpected latest-bedrock-certificate mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_bedrock_certificate_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
    ...
    fn overwrite_fake_bedrock_certificate_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        verify_json_path: &Path,
