...
use copybot_app::tiny_live_activation::package_summit_certificate_pinnacle_receipt;
...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_pinnacle_receipt --summit-certificate-session-dir <path> [--confirm-decision-packet-session-dir <path>] [--session-dir <path>] [--output <path>] [--json] (--plan-live-package-pinnacle-receipt | --render-live-package-pinnacle-receipt | --run-live-package-pinnacle-receipt | --verify-live-package-pinnacle-receipt)";
const PINNACLE_RECEIPT_SESSION_VERSION: &str = "1";
const PINNACLE_RECEIPT_STATUS_VERSION: &str = "1";
const PINNACLE_RECEIPT_ALGORITHM: &str = "sha256";
const PINNACLE_RECEIPT_SESSION_EXPLICIT_STATEMENT: &str =
...
const PINNACLE_RECEIPT_STATUS_EXPLICIT_STATEMENT: &str =
    "this pinnacle receipt is archival and read-only; it never executes or enables the frozen controller";
const PINNACLE_RECEIPT_VERIFY_EXPLICIT_STATEMENT: &str =
...
struct Config {
    mode: Mode,
    summit_certificate_session_dir: PathBuf,
...
enum Mode {
    PlanLivePackagePinnacleReceipt,
    RenderLivePackagePinnacleReceipt,
    RunLivePackagePinnacleReceipt,
    VerifyLivePackagePinnacleReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackagePinnacleReceiptVerdict {
    TinyLivePackagePinnacleReceiptPlanReady,
    TinyLivePackagePinnacleReceiptRendered,
    TinyLivePackagePinnacleReceiptRefusedNowByStage3,
    TinyLivePackagePinnacleReceiptRefusedNowByPreActivationGate,
    TinyLivePackagePinnacleReceiptRefusedNowByInvalidOrDriftedContract,
    TinyLivePackagePinnacleReceiptReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackagePinnacleReceiptVerifyOk,
    TinyLivePackagePinnacleReceiptVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackagePinnacleReceiptResult {
    RefusedNowByStage3,
...
struct PackagePinnacleReceiptStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackagePinnacleReceiptSession {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct PackagePinnacleReceiptStatus {
    status_version: String,
    updated_at: DateTime<Utc>,
    session_dir: String,
    summit_certificate_session_dir: String,
    result: LivePackagePinnacleReceiptResult,
    reason: String,
    ...
    summit_certificate_step: Option<PackagePinnacleReceiptStepArtifact>,
    provenance_certificate_summary: String,
    chain_fingerprint_summary: String,
    chain_fingerprint_sha256: String,
    chain_fingerprint_algorithm: String,
...
#[derive(Debug, Clone)]
struct PackagePinnacleReceiptPaths {
...
struct PackagePinnacleReceiptReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackagePinnacleReceiptVerdict,
    reason: String,
    summit_certificate_session_dir: String,
...
struct NestedSummitCertificateReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
struct PinnacleReceiptPayload<'a> {
    result: &'a str,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut summit_certificate_session_dir: Option<PathBuf> = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--summit-certificate-session-dir" => {
                summit_certificate_session_dir = Some(PathBuf::from(parse_string_arg(
                    "--summit-certificate-session-dir",
                    args.next(),
                )?))
            }
            ...
            "--json" => json = true,
            "--plan-live-package-pinnacle-receipt" => set_mode(
                &mut mode,
                Mode::PlanLivePackagePinnacleReceipt,
                arg.as_str(),
            )?,
            "--render-live-package-pinnacle-receipt" => set_mode(
                &mut mode,
                Mode::RenderLivePackagePinnacleReceipt,
                arg.as_str(),
            )?,
            "--run-live-package-pinnacle-receipt" => {
                set_mode(&mut mode, Mode::RunLivePackagePinnacleReceipt, arg.as_str())?
            }
            "--verify-live-package-pinnacle-receipt" => set_mode(
                &mut mode,
                Mode::VerifyLivePackagePinnacleReceipt,
                arg.as_str(),
            )?,
    ...
    if matches!(mode, Mode::RenderLivePackagePinnacleReceipt) && output_path.is_none() {
        bail!("missing --output for --render-live-package-pinnacle-receipt");
    }
    if matches!(
        mode,
        Mode::RunLivePackagePinnacleReceipt | Mode::VerifyLivePackagePinnacleReceipt
    ) && session_dir.is_none()
    {
    ...
    let summit_certificate_session_dir = summit_certificate_session_dir
        .ok_or_else(|| anyhow!("missing --summit-certificate-session-dir"))?;
    Ok(Some(Config {
        mode,
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLivePackagePinnacleReceipt => plan_live_package_pinnacle_receipt_report(&config)?,
        Mode::RenderLivePackagePinnacleReceipt => {
            render_live_package_pinnacle_receipt_report(&config)?
        }
        Mode::RunLivePackagePinnacleReceipt => run_live_package_pinnacle_receipt_report(&config)?,
        Mode::VerifyLivePackagePinnacleReceipt => {
            verify_live_package_pinnacle_receipt_report(&config)?
        }
    };
    if config.json {
...
fn plan_live_package_pinnacle_receipt_report(
    config: &Config,
) -> Result<PackagePinnacleReceiptReport> {
    let (summit_certificate, classification) =
        match config.confirmed_decision_packet_session_dir.as_deref() {
            Some(confirmed_decision_packet_session_dir) => {
                match package_summit_certificate_pinnacle_receipt::verify_live_package_summit_certificate_for_pinnacle_receipt(
                    &config.summit_certificate_session_dir,
                    confirmed_decision_packet_session_dir,
                ) {
                    Ok(verified) => {
                        let classification = classify_verified_summit_certificate(&verified);
                        (verified.contract, classification)
                    }
                    Err(_) => {
                        let raw_contract = package_summit_certificate_pinnacle_receipt::load_live_package_summit_certificate_contract_for_pinnacle_receipt(
                            &config.summit_certificate_session_dir,
                        )?;
                        let classification = classify_summit_certificate_contract(&raw_contract, false);
    ...
    let session_dir = config.session_dir.clone().unwrap_or_else(|| {
        config
            .summit_certificate_session_dir
            .join("pinnacle-receipt")
    });
    let result = serialize_enum(&classification.0);
    let pinnacle_receipt_sha256 = compute_pinnacle_receipt_sha256(
        &result,
        summit_certificate.handoff_bundle_result.as_deref(),
        summit_certificate.decision_packet_result.as_deref(),
        summit_certificate.execute_frozen_result.as_deref(),
        summit_certificate
        ...
        summit_certificate
            ...
            .as_deref(),
        &summit_certificate.reviewed_frozen_live_cutover_controller_command_summary,
        &summit_certificate.chain_fingerprint_sha256,
        &summit_certificate.chain_fingerprint_algorithm,
        &summit_certificate.release_capsule_digest_manifest_sha256,
        summit_certificate.release_capsule_digest_manifest_entry_count,
        &summit_certificate.release_capsule_digest_algorithm,
        &summit_certificate.ledger_seal_sha256,
        &summit_certificate.ledger_seal_algorithm,
        &summit_certificate.registry_entry_sha256,
        &summit_certificate.registry_entry_algorithm,
        &summit_certificate.filing_certificate_sha256,
        &summit_certificate.filing_certificate_algorithm,
        &summit_certificate.archive_receipt_sha256,
        &summit_certificate.archive_receipt_algorithm,
        &summit_certificate.closure_certificate_summary,
        &summit_certificate.closure_certificate_sha256,
        &summit_certificate.closure_certificate_algorithm,
        &summit_certificate.finality_receipt_sha256,
        &summit_certificate.finality_receipt_algorithm,
        &summit_certificate.consummation_record_sha256,
        &summit_certificate.consummation_record_algorithm,
        &summit_certificate.completion_certificate_summary,
        &summit_certificate.completion_certificate_sha256,
        &summit_certificate.completion_certificate_algorithm,
        &summit_certificate.culmination_receipt_summary,
        &summit_certificate.culmination_receipt_sha256,
        &summit_certificate.culmination_receipt_algorithm,
        &summit_certificate.summit_certificate_summary,
        &summit_certificate.summit_certificate_sha256,
        &summit_certificate.summit_certificate_algorithm,
    )?;

    Ok(PackagePinnacleReceiptReport {
        generated_at: Utc::now(),
        mode: "plan_live_package_pinnacle_receipt".to_string(),
        verdict:
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptPlanReady,
        reason: format!(
            "pinnacle-receipt-native pinnacle receipt is explicit for verified summit-certificate session {}; this command stays archival and read-only while freezing one final crown identity over the fully culminated chain",
            config.summit_certificate_session_dir.display()
        ),
        summit_certificate_session_dir: config.summit_certificate_session_dir.display().to_string(),
        registry_entry_session_dir: Some(
            summit_certificate.registry_entry_session_dir.display().to_string(),
        ),
        ...
        review_receipt_session_dir: Some(
            summit_certificate.review_receipt_session_dir.display().to_string(),
        ),
        handoff_bundle_session_dir: Some(
            summit_certificate.handoff_bundle_session_dir.display().to_string(),
        ),
        ...
        turn_green_session_dir: Some(
            summit_certificate.turn_green_session_dir.display().to_string(),
        ),
        launch_packet_session_dir: Some(
            summit_certificate.launch_packet_session_dir.display().to_string(),
        ),
        package_dir: Some(summit_certificate.package_dir.display().to_string()),
        install_root: Some(summit_certificate.install_root.display().to_string()),
        target_service_name: Some(summit_certificate.target_service_name.clone()),
        backend_command: Some(summit_certificate.backend_command.clone()),
        wrapper_timeout_ms: Some(summit_certificate.wrapper_timeout_ms),
        service_status_max_staleness_ms: Some(summit_certificate.service_status_max_staleness_ms),
        session_dir: Some(session_dir.display().to_string()),
        pinnacle_receipt_session_path: None,
        pinnacle_receipt_status_path: None,
        archived_summit_certificate_report_path: None,
        result: Some(result),
        handoff_bundle_result: summit_certificate.handoff_bundle_result.clone(),
        decision_packet_result: summit_certificate.decision_packet_result.clone(),
        execute_frozen_result: summit_certificate.execute_frozen_result.clone(),
        current_pre_activation_gate_verdict: summit_certificate
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: summit_certificate
            .current_pre_activation_gate_reason
            .clone(),
        ...
        provenance_certificate_summary: Some(summit_certificate.provenance_certificate_summary.clone()),
        chain_fingerprint_summary: Some(summit_certificate.chain_fingerprint_summary.clone()),
        chain_fingerprint_sha256: Some(summit_certificate.chain_fingerprint_sha256.clone()),
        chain_fingerprint_algorithm: Some(summit_certificate.chain_fingerprint_algorithm.clone()),
        release_capsule_digest_manifest_sha256: Some(
            summit_certificate.release_capsule_digest_manifest_sha256.clone(),
        ),
        release_capsule_digest_manifest_entry_count: Some(
            summit_certificate.release_capsule_digest_manifest_entry_count,
        ),
        release_capsule_digest_algorithm: Some(
            summit_certificate.release_capsule_digest_algorithm.clone(),
        ),
        notarization_receipt_summary: Some(summit_certificate.notarization_receipt_summary.clone()),
        ledger_seal_summary: Some(summit_certificate.ledger_seal_summary.clone()),
        ledger_seal_sha256: Some(summit_certificate.ledger_seal_sha256.clone()),
        ledger_seal_algorithm: Some(summit_certificate.ledger_seal_algorithm.clone()),
        registry_entry_summary: Some(summit_certificate.registry_entry_summary.clone()),
        registry_entry_sha256: Some(summit_certificate.registry_entry_sha256.clone()),
        registry_entry_algorithm: Some(summit_certificate.registry_entry_algorithm.clone()),
        filing_certificate_summary: Some(summit_certificate.filing_certificate_summary.clone()),
        filing_certificate_sha256: Some(summit_certificate.filing_certificate_sha256.clone()),
        filing_certificate_algorithm: Some(summit_certificate.filing_certificate_algorithm.clone()),
        archive_receipt_summary: Some(summit_certificate.archive_receipt_summary.clone()),
        archive_receipt_sha256: Some(summit_certificate.archive_receipt_sha256.clone()),
        archive_receipt_algorithm: Some(summit_certificate.archive_receipt_algorithm.clone()),
        closure_certificate_summary: Some(summit_certificate.closure_certificate_summary.clone()),
        closure_certificate_sha256: Some(summit_certificate.closure_certificate_sha256.clone()),
        closure_certificate_algorithm: Some(
            summit_certificate.closure_certificate_algorithm.clone(),
        ),
        finality_receipt_summary: Some(summit_certificate.finality_receipt_summary.clone()),
        finality_receipt_sha256: Some(summit_certificate.finality_receipt_sha256.clone()),
        finality_receipt_algorithm: Some(summit_certificate.finality_receipt_algorithm.clone()),
        consummation_record_summary: Some(summit_certificate.consummation_record_summary.clone()),
        consummation_record_sha256: Some(summit_certificate.consummation_record_sha256.clone()),
        consummation_record_algorithm: Some(summit_certificate.consummation_record_algorithm.clone()),
        completion_certificate_summary: Some(
            summit_certificate.completion_certificate_summary.clone(),
        ),
        completion_certificate_sha256: Some(
            summit_certificate.completion_certificate_sha256.clone(),
        ),
        completion_certificate_algorithm: Some(
            summit_certificate.completion_certificate_algorithm.clone(),
        ),
        culmination_receipt_summary: Some(
            summit_certificate.culmination_receipt_summary.clone(),
        ),
        culmination_receipt_sha256: Some(
            summit_certificate.culmination_receipt_sha256.clone(),
        ),
        culmination_receipt_algorithm: Some(
            summit_certificate.culmination_receipt_algorithm.clone(),
        ),
        summit_certificate_summary: Some(summit_certificate.summit_certificate_summary.clone()),
        summit_certificate_sha256: Some(summit_certificate.summit_certificate_sha256.clone()),
        summit_certificate_algorithm: Some(summit_certificate.summit_certificate_algorithm.clone()),
        pinnacle_receipt_summary: Some(pinnacle_receipt_summary(
            classification.0,
            &summit_certificate.reviewed_frozen_live_cutover_controller_command_summary,
        )),
        pinnacle_receipt_sha256: Some(pinnacle_receipt_sha256),
        pinnacle_receipt_algorithm: Some(PINNACLE_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement: PINNACLE_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    })
}

fn render_live_package_pinnacle_receipt_report(
    config: &Config,
) -> Result<PackagePinnacleReceiptReport> {
    let plan = plan_live_package_pinnacle_receipt_report(config)?;
    ...
    let script = format!(
        "#!/bin/sh\nset -eu\ncopybot_tiny_live_activation_package_pinnacle_receipt --summit-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-pinnacle-receipt --json\ncopybot_tiny_live_activation_package_pinnacle_receipt --summit-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-pinnacle-receipt --json\n",
        shell_escape_path(&config.summit_certificate_session_dir),
    ...
    Ok(PackagePinnacleReceiptReport {
        generated_at: Utc::now(),
        mode: "render_live_package_pinnacle_receipt".to_string(),
        verdict:
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptRendered,
        reason: format!(
            "rendered pinnacle-receipt run/verify script to {}; the primary input remains the immutable summit-certificate session and run/verify still require the decision-packet confirmation anchor",
            output_path.display()
        ),
        explicit_statement: PINNACLE_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
        ..plan
...
fn run_live_package_pinnacle_receipt_report(
    config: &Config,
) -> Result<PackagePinnacleReceiptReport> {
    ...
    let verified_summit_certificate = match package_summit_certificate_pinnacle_receipt::verify_live_package_summit_certificate_for_pinnacle_receipt(
        &config.summit_certificate_session_dir,
        config
            .confirmed_decision_packet_session_dir
            .as_deref()
        ...
        Err(error) => {
            return Ok(failure_report_without_contract(
                config,
                TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptRefusedNowByInvalidOrDriftedContract,
                format!(
                    "failed to verify summit-certificate session under {}: {error}",
                    config.summit_certificate_session_dir.display()
                ),
            ))
        }
    ...
    validate_managed_surface_session_dir(
        &verified_summit_certificate.contract.install_root,
        session_dir,
    )
    .with_context(|| {
        format!(
            "pinnacle-receipt session dir {} overlaps the managed live target surface derived from verified summit-certificate install_root {}",
            session_dir.display(),
            verified_summit_certificate.contract.install_root.display()
        )
    })?;
    ensure_clean_pinnacle_receipt_session_dir(session_dir)?;
    let paths = pinnacle_receipt_paths(session_dir);
    let classification = classify_verified_summit_certificate(&verified_summit_certificate);
    let result = classification.0;
    let pinnacle_receipt_sha256 = compute_pinnacle_receipt_sha256(
        &serialize_enum(&result),
        verified_summit_certificate
            .contract
        ...
        &verified_summit_certificate.contract.ledger_seal_sha256,
        &verified_summit_certificate.contract.ledger_seal_algorithm,
        &verified_summit_certificate.contract.registry_entry_sha256,
        &verified_summit_certificate
            .contract
        ...
        &verified_summit_certificate.contract.archive_receipt_sha256,
        &verified_summit_certificate
            .contract
        ...
        &verified_summit_certificate.contract.finality_receipt_sha256,
        &verified_summit_certificate
            .contract
    ...
    let session = planned_session(
        config,
        session_dir,
        &verified_summit_certificate.contract,
        result,
    ...
    persist_json(
        ...
        &verified_summit_certificate.report_json,
    )?;
    let summit_certificate_step = Some(step_artifact(
        &paths.summit_certificate_report_path,
        "verify_live_package_summit_certificate",
        &verified_summit_certificate.verdict,
        &verified_summit_certificate.reason,
        verified_summit_certificate.generated_at,
    ));
    let status = PackagePinnacleReceiptStatus {
        status_version: PINNACLE_RECEIPT_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        session_dir: session_dir.display().to_string(),
        summit_certificate_session_dir: config.summit_certificate_session_dir.display().to_string(),
        result,
        reason: classification.2.clone(),
        handoff_bundle_result: verified_summit_certificate
            .contract
            .handoff_bundle_result
            .clone(),
        decision_packet_result: verified_summit_certificate
            .contract
            .decision_packet_result
            .clone(),
        execute_frozen_result: verified_summit_certificate
            .contract
            .execute_frozen_result
            .clone(),
        current_pre_activation_gate_verdict: verified_summit_certificate
            .contract
            .current_pre_activation_gate_verdict
            .clone(),
        current_pre_activation_gate_reason: verified_summit_certificate
            .contract
            .current_pre_activation_gate_reason
            .clone(),
        summit_certificate_step: summit_certificate_step.clone(),
        provenance_certificate_summary: verified_summit_certificate
            .contract
            .provenance_certificate_summary
            .clone(),
        chain_fingerprint_summary: verified_summit_certificate
            .contract
            .chain_fingerprint_summary
            .clone(),
        chain_fingerprint_sha256: verified_summit_certificate
            .contract
            .chain_fingerprint_sha256
            .clone(),
        chain_fingerprint_algorithm: verified_summit_certificate
            .contract
            .chain_fingerprint_algorithm
            .clone(),
        release_capsule_digest_manifest_sha256: verified_summit_certificate
            .contract
            .release_capsule_digest_manifest_sha256
            .clone(),
        release_capsule_digest_manifest_entry_count: verified_summit_certificate
            .contract
            .release_capsule_digest_manifest_entry_count,
        release_capsule_digest_algorithm: verified_summit_certificate
            .contract
            .release_capsule_digest_algorithm
            .clone(),
        notarization_receipt_summary: verified_summit_certificate
            .contract
            .notarization_receipt_summary
            .clone(),
        ledger_seal_summary: verified_summit_certificate
            .contract
            .ledger_seal_summary
            .clone(),
        ledger_seal_sha256: verified_summit_certificate
            .contract
            .ledger_seal_sha256
            .clone(),
        ledger_seal_algorithm: verified_summit_certificate
            .contract
            .ledger_seal_algorithm
            .clone(),
        registry_entry_summary: verified_summit_certificate
            .contract
            .registry_entry_summary
            .clone(),
        registry_entry_sha256: verified_summit_certificate
            .contract
            .registry_entry_sha256
            .clone(),
        registry_entry_algorithm: verified_summit_certificate
            .contract
            .registry_entry_algorithm
            .clone(),
        filing_certificate_summary: verified_summit_certificate
            .contract
            .filing_certificate_summary
            .clone(),
        filing_certificate_sha256: verified_summit_certificate
            .contract
            .filing_certificate_sha256
            .clone(),
        filing_certificate_algorithm: verified_summit_certificate
            .contract
            .filing_certificate_algorithm
            .clone(),
        archive_receipt_summary: verified_summit_certificate
            .contract
            .archive_receipt_summary
            .clone(),
        archive_receipt_sha256: verified_summit_certificate
            .contract
            .archive_receipt_sha256
            .clone(),
        archive_receipt_algorithm: verified_summit_certificate
            .contract
            .archive_receipt_algorithm
            .clone(),
        closure_certificate_summary: verified_summit_certificate
            .contract
            .closure_certificate_summary
            .clone(),
        closure_certificate_sha256: verified_summit_certificate
            .contract
            .closure_certificate_sha256
            .clone(),
        closure_certificate_algorithm: verified_summit_certificate
            .contract
            .closure_certificate_algorithm
            .clone(),
        finality_receipt_summary: verified_summit_certificate
            .contract
            .finality_receipt_summary
            .clone(),
        finality_receipt_sha256: verified_summit_certificate
            .contract
            .finality_receipt_sha256
            .clone(),
        finality_receipt_algorithm: verified_summit_certificate
            .contract
            .finality_receipt_algorithm
            .clone(),
        consummation_record_summary: verified_summit_certificate
            .contract
            .consummation_record_summary
            .clone(),
        consummation_record_sha256: verified_summit_certificate
            .contract
            .consummation_record_sha256
            .clone(),
        consummation_record_algorithm: verified_summit_certificate
            .contract
            .consummation_record_algorithm
            .clone(),
        completion_certificate_summary: verified_summit_certificate
            .contract
            .completion_certificate_summary
            .clone(),
        completion_certificate_sha256: verified_summit_certificate
            .contract
            .completion_certificate_sha256
            .clone(),
        completion_certificate_algorithm: verified_summit_certificate
            .contract
            .completion_certificate_algorithm
            .clone(),
        culmination_receipt_summary: verified_summit_certificate
            .contract
            .culmination_receipt_summary
            .clone(),
        culmination_receipt_sha256: verified_summit_certificate
            .contract
            .culmination_receipt_sha256
            .clone(),
        culmination_receipt_algorithm: verified_summit_certificate
            .contract
            .culmination_receipt_algorithm
            .clone(),
        summit_certificate_summary: verified_summit_certificate
            .contract
            .summit_certificate_summary
            .clone(),
        summit_certificate_sha256: verified_summit_certificate
            .contract
            .summit_certificate_sha256
            .clone(),
        summit_certificate_algorithm: verified_summit_certificate
            .contract
            .summit_certificate_algorithm
            .clone(),
        pinnacle_receipt_summary: pinnacle_receipt_summary(
            result,
            &verified_summit_certificate
                .contract
                .reviewed_frozen_live_cutover_controller_command_summary,
        ),
        pinnacle_receipt_sha256: pinnacle_receipt_sha256.clone(),
        pinnacle_receipt_algorithm: PINNACLE_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: PINNACLE_RECEIPT_STATUS_EXPLICIT_STATEMENT.to_string(),
    };
...
fn verify_live_package_pinnacle_receipt_report(
    config: &Config,
) -> Result<PackagePinnacleReceiptReport> {
    let session_dir = config
        .session_dir
    ...
    let paths = pinnacle_receipt_paths(session_dir);
    let session: PackagePinnacleReceiptSession = load_json(&paths.session_path)?;
    let status: PackagePinnacleReceiptStatus = load_json(&paths.status_path)?;
    let mut mismatches = Vec::new();

    compare_string(
        &session.summit_certificate_session_dir,
        &config.summit_certificate_session_dir.display().to_string(),
        "summit-certificate summit_certificate_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "pinnacle-receipt session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "pinnacle-receipt status session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.summit_certificate_session_dir,
        &config.summit_certificate_session_dir.display().to_string(),
        "pinnacle-receipt status summit_certificate_session_dir",
        &mut mismatches,
    );
    ...
    let stored_summit_certificate_report: serde_json::Value = load_required_step_json(
        &status.summit_certificate_step,
        &paths.summit_certificate_report_path,
        "summit_certificate_step",
        &mut mismatches,
    )?;
    let stored_summit_certificate_report_view: NestedSummitCertificateReportView =
        serde_json::from_value(stored_summit_certificate_report.clone()).with_context(|| {
            format!(
                "failed parsing archived nested summit-certificate report {}",
                paths.summit_certificate_report_path.display()
            )
        })?;
    let verified_summit_certificate_report_view: NestedSummitCertificateReportView =
        serde_json::from_value(verified_summit_certificate.report_json.clone()).with_context(
            || {
                format!(
                    "failed parsing freshly verified nested summit-certificate report for {}",
                    config.summit_certificate_session_dir.display()
                )
            },
        )?;
    compare_json_ignoring_generated_at(
        &stored_summit_certificate_report,
        &verified_summit_certificate.report_json,
        "pinnacle-receipt archived summit-certificate report json",
        &mut mismatches,
    );
    compare_string(
        &stored_summit_certificate_report_view.mode,
        &verified_summit_certificate_report_view.mode,
        "pinnacle-receipt archived summit-certificate report mode",
        &mut mismatches,
    );
    compare_string(
        &stored_summit_certificate_report_view.verdict,
        &verified_summit_certificate_report_view.verdict,
        "pinnacle-receipt archived summit-certificate report verdict",
        &mut mismatches,
    );
    compare_string(
        &stored_summit_certificate_report_view.reason,
        &verified_summit_certificate_report_view.reason,
        "pinnacle-receipt archived summit-certificate report reason",
        &mut mismatches,
    );
    if stored_summit_certificate_report_view.generated_at
        != verified_summit_certificate_report_view.generated_at
    {
        mismatches.push(format!(
            "pinnacle-receipt archived summit-certificate report generated_at {:?} does not match freshly verified nested truth {:?}",
            stored_summit_certificate_report_view.generated_at,
            verified_summit_certificate_report_view.generated_at
        ));
    }
    let expected_summit_certificate_step = step_artifact(
        &paths.summit_certificate_report_path,
        &verified_summit_certificate_report_view.mode,
        &verified_summit_certificate_report_view.verdict,
        &verified_summit_certificate_report_view.reason,
    ...
    compare_step_artifact(
        ...
        &expected_summit_certificate_step,
        "summit-certificate summit_certificate_step",
        &mut mismatches,
    );
    if verified_summit_certificate.verdict != "tiny_live_package_summit_certificate_verify_ok" {
        mismatches.push(format!(
    ...
    let classification = classify_verified_summit_certificate(&verified_summit_certificate);
    let expected_pinnacle_receipt_sha256 = compute_pinnacle_receipt_sha256(
    ...
    let expected_session = expected_session_from_verified(
        ...
        &verified_summit_certificate.contract,
        classification.0,
        &expected_pinnacle_receipt_sha256,
    );
    if session != expected_session {
        mismatches.push(
            "pinnacle-receipt session contract does not match freshly verified summit-certificate truth"
                .to_string(),
        );
    }
    let expected_status = expected_status_from_verified(
        ...
        &verified_summit_certificate.contract,
        classification,
        expected_summit_certificate_step.clone(),
        &expected_pinnacle_receipt_sha256,
    );
    if status != expected_status {
        mismatches.push(
            "pinnacle-receipt status contract does not match freshly verified summit-certificate truth"
                .to_string(),
        );
    }
    ...
    Ok(report_from_status(
        ...
        &paths,
        &verified_summit_certificate.contract,
        TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptVerifyOk,
        &expected_status,
    ))
}

fn verify_summit_certificate_command_summary(summit_certificate_session_dir: &Path) -> String {
    format!(
        "verify immutable summit-certificate session under {} before freezing the final pinnacle receipt",
        summit_certificate_session_dir.display()
    )
}

fn classify_summit_certificate_contract(
    contract: &package_summit_certificate_pinnacle_receipt::LivePackageSummitCertificateContractView,
    assume_verified: bool,
) -> (
    LivePackagePinnacleReceiptResult,
    TinyLivePackagePinnacleReceiptVerdict,
    String,
) {
    if !assume_verified {
        return (
            LivePackagePinnacleReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptRefusedNowByInvalidOrDriftedContract,
            "the summit-certificate chain is not verified, so this pinnacle receipt remains refused"
                .to_string(),
        );
    }
    match contract.result.as_deref() {
        Some("refused_now_by_stage3") => (
            LivePackagePinnacleReceiptResult::RefusedNowByStage3,
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptRefusedNowByStage3,
            "the immutable summit certificate remains refused now by Stage 3, so this pinnacle receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("refused_now_by_pre_activation_gate") => (
            LivePackagePinnacleReceiptResult::RefusedNowByPreActivationGate,
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptRefusedNowByPreActivationGate,
            "the immutable summit certificate remains refused now by the pre-activation gate, so this pinnacle receipt remains an explicit refusal record"
                .to_string(),
        ),
        Some("ready_for_manual_execution_when_gate_turns_green") => (
            LivePackagePinnacleReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen,
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptReadyForManualExecutionWhenGateTurnsGreen,
            "the immutable summit certificate is coherent and this pinnacle receipt is ready for manual execution when gate truth turns green"
                .to_string(),
        ),
        _ => (
            LivePackagePinnacleReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptRefusedNowByInvalidOrDriftedContract,
            "the verified summit-certificate chain is invalid, drifted, or non-runnable, so this pinnacle receipt remains refused"
                .to_string(),
        ),
    }
}

fn classify_verified_summit_certificate(
    verified: &package_summit_certificate_pinnacle_receipt::VerifiedLivePackageSummitCertificatePinnacleReceiptStep,
) -> (
    LivePackagePinnacleReceiptResult,
    TinyLivePackagePinnacleReceiptVerdict,
    String,
) {
    if verified.verdict != "tiny_live_package_summit_certificate_verify_ok" {
        return (
            LivePackagePinnacleReceiptResult::RefusedNowByInvalidOrDriftedContract,
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptRefusedNowByInvalidOrDriftedContract,
            "the immutable summit certificate did not verify cleanly, so this pinnacle receipt remains refused"
                .to_string(),
        );
    }
    classify_summit_certificate_contract(&verified.contract, true)
}

fn pinnacle_receipt_summary(
    result: LivePackagePinnacleReceiptResult,
    reviewed_frozen_live_cutover_controller_command_summary: &str,
) -> String {
    match result {
        LivePackagePinnacleReceiptResult::RefusedNowByStage3 => {
            "Pinnacle receipt outcome: the immutable culminated chain remains refused now by Stage 3; archive this final crown seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackagePinnacleReceiptResult::RefusedNowByPreActivationGate => {
            "Pinnacle receipt outcome: the immutable culminated chain remains refused now by the pre-activation gate; archive this final crown seal as refusal evidence only and do not run the frozen controller.".to_string()
        }
        LivePackagePinnacleReceiptResult::RefusedNowByInvalidOrDriftedContract => {
            "Pinnacle receipt outcome: the immutable culminated chain is invalid or drifted; archive this final crown seal for audit only and mint a new coherent certificate before any later execution handoff.".to_string()
        }
        LivePackagePinnacleReceiptResult::ReadyForManualExecutionWhenGateTurnsGreen => format!(
            "Pinnacle receipt outcome: this immutable crown seal is ready for manual execution when gate truth turns green; the exact reviewed frozen controller command remains: {reviewed_frozen_live_cutover_controller_command_summary}"
        ),
    }
}

fn pinnacle_receipt_paths(session_dir: &Path) -> PackagePinnacleReceiptPaths {
    PackagePinnacleReceiptPaths {
...
fn ensure_clean_pinnacle_receipt_session_dir(session_dir: &Path) -> Result<()> {
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing pinnacle-receipt session dir {}",
            session_dir.display()
        );
    }
    fs::create_dir_all(session_dir).with_context(|| {
        format!(
            "failed creating pinnacle-receipt session dir {}",
            session_dir.display()
        )
    })
}
...
) -> PackagePinnacleReceiptSession {
    PackagePinnacleReceiptSession {
        session_version: PINNACLE_RECEIPT_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        summit_certificate_session_dir: config.summit_certificate_session_dir.display().to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_summit_certificate_command_summary: verify_summit_certificate_command_summary(
            &config.summit_certificate_session_dir,
        ),
        ...
        pinnacle_receipt_sha256: pinnacle_receipt_sha256.to_string(),
        pinnacle_receipt_algorithm: PINNACLE_RECEIPT_ALGORITHM.to_string(),
        explicit_statement: PINNACLE_RECEIPT_SESSION_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn expected_status_from_verified(
    ...
    summit_certificate_step: PackagePinnacleReceiptStepArtifact,
    pinnacle_receipt_sha256: &str,
) -> PackagePinnacleReceiptStatus {
    PackagePinnacleReceiptStatus {
        status_version: PINNACLE_RECEIPT_STATUS_VERSION.to_string(),
        updated_at,
        session_dir: session_dir.display().to_string(),
        ...
        summit_certificate_step: Some(summit_certificate_step),
        provenance_certificate_summary: contract.provenance_certificate_summary.clone(),
        chain_fingerprint_summary: contract.chain_fingerprint_summary.clone(),
        chain_fingerprint_sha256: contract.chain_fingerprint_sha256.clone(),
        chain_fingerprint_algorithm: contract.chain_fingerprint_algorithm.clone(),
...
fn report_from_status(
    config: &Config,
    session_dir: &Path,
    paths: &PackagePinnacleReceiptPaths,
    contract: &package_summit_certificate_pinnacle_receipt::LivePackageSummitCertificateContractView,
    verdict: TinyLivePackagePinnacleReceiptVerdict,
    status: &PackagePinnacleReceiptStatus,
) -> PackagePinnacleReceiptReport {
    PackagePinnacleReceiptReport {
        generated_at: Utc::now(),
        mode: if verdict
            == TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptVerifyOk
        {
            "verify_live_package_pinnacle_receipt".to_string()
        } else {
            "run_live_package_pinnacle_receipt".to_string()
        },
        verdict,
        ...
        session_dir: Some(session_dir.display().to_string()),
        pinnacle_receipt_session_path: Some(paths.session_path.display().to_string()),
        pinnacle_receipt_status_path: Some(paths.status_path.display().to_string()),
        archived_summit_certificate_report_path: Some(
        ...
        summit_certificate_step: status.summit_certificate_step.clone(),
        verify_summit_certificate_command_summary: Some(verify_summit_certificate_command_summary(
            &config.summit_certificate_session_dir,
        )),
        reviewed_frozen_live_cutover_controller_command_summary: Some(
        ...
        explicit_statement: if verdict
            == TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptVerifyOk
        {
            PINNACLE_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string()
        } else {
...
fn failure_report_without_contract(
    ...
    PackagePinnacleReceiptReport {
        ...
        mode: match config.mode {
            Mode::PlanLivePackagePinnacleReceipt => {
                "plan_live_package_pinnacle_receipt".to_string()
            }
            Mode::RenderLivePackagePinnacleReceipt => {
                "render_live_package_pinnacle_receipt".to_string()
            }
            Mode::RunLivePackagePinnacleReceipt => {
                "run_live_package_pinnacle_receipt".to_string()
            }
            Mode::VerifyLivePackagePinnacleReceipt => {
                "verify_live_package_pinnacle_receipt".to_string()
        ...
        verdict,
        reason,
        summit_certificate_session_dir: config
            .summit_certificate_session_dir
            .display()
            .to_string(),
        ...
        summit_certificate_algorithm: Some(PINNACLE_RECEIPT_ALGORITHM.to_string()),
        pinnacle_receipt_summary: None,
        pinnacle_receipt_sha256: None,
        pinnacle_receipt_algorithm: Some(PINNACLE_RECEIPT_ALGORITHM.to_string()),
        verification_mismatches: Vec::new(),
        explicit_statement:
            "this immutable pinnacle receipt refused before persisting because the summit-certificate artifact could not be verified coherently"
                .to_string(),
    }
}
...
fn failure_report_for_verify(
    ...
    let reason = mismatches.first().cloned().unwrap_or_else(|| {
        "pinnacle-receipt-native pinnacle-receipt verification failed".to_string()
    });
    PackagePinnacleReceiptReport {
        generated_at: Utc::now(),
        mode: "verify_live_package_pinnacle_receipt".to_string(),
        verdict: TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptVerifyInvalid,
        reason,
        summit_certificate_session_dir: config.summit_certificate_session_dir.display().to_string(),
        registry_entry_session_dir: Some(contract.registry_entry_session_dir.display().to_string()),
        notarization_receipt_session_dir: Some(
        ...
        explicit_statement: PINNACLE_RECEIPT_VERIFY_EXPLICIT_STATEMENT.to_string(),
    }
}
...
fn load_required_step_json(
    step: &Option<PackagePinnacleReceiptStepArtifact>,
    expected_path: &Path,
    label: &str,
    mismatches: &mut Vec<String>,
) -> Result<serde_json::Value> {
    let Some(step) = step else {
        mismatches.push(format!("{label} is missing from pinnacle-receipt status"));
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackagePinnacleReceiptStepArtifact {
    PackagePinnacleReceiptStepArtifact {
...
fn compare_step_artifact(
    actual: Option<&PackagePinnacleReceiptStepArtifact>,
    expected: &PackagePinnacleReceiptStepArtifact,
    label: &str,
    mismatches: &mut Vec<String>,
) {
...
fn compute_pinnacle_receipt_sha256(
    result: &str,
    ...
    canonical_sha256_hex(&PinnacleReceiptPayload {
        result,
        handoff_bundle_result,
        decision_packet_result,
        execute_frozen_result,
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing pinnacle-receipt script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn render_report_lines(report: &PackagePinnacleReceiptReport) -> String {
    let mut lines = vec![
        ...
        format!(
            "summit_certificate_session_dir={}",
            report.summit_certificate_session_dir
        ),
    ];
    ...
    if let Some(value) = &report.pinnacle_receipt_sha256 {
        lines.push(format!("pinnacle_receipt_sha256={value}"));
    }
...
mod tests {
    ...
    fn run_ready_for_manual_execution_when_gate_turns_green_then_verify_stays_green() {
        ...
        let run = run_live_package_pinnacle_receipt_report(&fixture.run_config()).unwrap();
        assert_eq!(
            run.verdict,
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptReadyForManualExecutionWhenGateTurnsGreen
        );
        let verify = verify_live_package_pinnacle_receipt_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptVerifyOk
        );
    ...
    fn drifted_finality_receipt_contract_is_refused() {
        let fixture = fake_command_fixture(
            ...
            json!({
                "generated_at": Utc::now(),
                "mode": "verify_live_package_summit_certificate",
    ...
    fn run_refuses_managed_surface_overlap_before_writing_any_artifacts() {
        ...
        .unwrap();
        let overlap_session_dir = managed_paths.runtime_dir.join("pinnacle-receipt-session");
        ...
        let error = run_live_package_pinnacle_receipt_report(&config).unwrap_err();
        assert!(
            error
                .to_string()
        ...
        let paths = pinnacle_receipt_paths(&overlap_session_dir);
        assert!(!paths.session_path.exists());
        assert!(!paths.status_path.exists());
    }

    #[test]
    fn verify_rejects_tampered_summit_certificate_step_path() {
        let fixture = fake_command_fixture(
        ...
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        run_live_package_pinnacle_receipt_report(&fixture.run_config()).unwrap();
        let paths = pinnacle_receipt_paths(&fixture.pinnacle_receipt_session_dir);
        let mut status: PackagePinnacleReceiptStatus = load_json(&paths.status_path).unwrap();
        status.summit_certificate_step.as_mut().unwrap().path =
        ...
        assert_eq!(
            verify.verdict,
            TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptVerifyInvalid
        );
    }
    ...
    fn verify_rejects_tampered_consummation_record_text() {
        ...
        run_live_package_pinnacle_receipt_report(&fixture.run_config()).unwrap();
        let paths = pinnacle_receipt_paths(&fixture.pinnacle_receipt_session_dir);
        let mut session: PackagePinnacleReceiptSession = load_json(&paths.session_path).unwrap();
    ...
    fn verify_rejects_tampered_status_result() {
        ...
        let paths = pinnacle_receipt_paths(&fixture.pinnacle_receipt_session_dir);
        let mut status: PackagePinnacleReceiptStatus = load_json(&paths.status_path).unwrap();
        status.result = LivePackagePinnacleReceiptResult::RefusedNowByStage3;
        persist_json(&paths.status_path, &status).unwrap();
    ...
    fn verify_rejects_tampered_nested_finality_receipt_report_content() {
        ...
        let mut report: serde_json::Value =
            load_json(&paths.summit_certificate_report_path).unwrap();
    ...
    fn verify_rejects_tampered_chain_ledger_registry_filing_archive_closure_finality_or_consummation_identity_fields(
    ) {
        {
            ...
            persist_json(&paths.status_path, &status).unwrap();
            let verify =
                verify_live_package_pinnacle_receipt_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackagePinnacleReceiptVerdict::TinyLivePackagePinnacleReceiptVerifyInvalid
            );
    ...
    struct FakeCommandFixture {
        bin_dir: PathBuf,
        pinnacle_receipt_session_dir: PathBuf,
        summit_certificate_session_dir: PathBuf,
    ...
    impl FakeCommandFixture {
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLivePackagePinnacleReceipt,
                summit_certificate_session_dir: self.summit_certificate_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
                session_dir: Some(self.pinnacle_receipt_session_dir.clone()),
                output_path: None,
                json: true,
            }
        }
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLivePackagePinnacleReceipt,
                summit_certificate_session_dir: self.summit_certificate_session_dir.clone(),
                confirmed_decision_packet_session_dir: Some(
                    self.decision_packet_session_dir.clone(),
                ),
    ...
    fn fake_command_fixture(label: &str, verify_report: serde_json::Value) -> FakeCommandFixture {
        ...
        persist_json(
            &summit_certificate_session_dir
                .join("tiny_live_activation_package_summit_certificate.session.json"),
            &stored_culmination_receipt_session_fixture(
                &culmination_receipt_session_dir,
                &finality_receipt_session_dir,
                &archive_receipt_session_dir,
                &registry_entry_session_dir,
        ...
        persist_json(
            &summit_certificate_session_dir
                .join("tiny_live_activation_package_summit_certificate.status.json"),
            &stored_culmination_receipt_status_fixture(&verify_report),
        )
        .unwrap();
        persist_json(
            &summit_certificate_session_dir.join(
        ...
        bind_culmination_receipt_report_to_fixture(
            &mut verify_report,
            &culmination_receipt_session_dir,
            &finality_receipt_session_dir,
            &archive_receipt_session_dir,
        ...
        let verify_path = fixture_dir.join("summit-certificate.verify.json");
        persist_json(&verify_path, &verify_report).unwrap();
        write_fake_culmination_receipt_verify_command(
            &bin_dir.join("copybot_tiny_live_activation_package_summit_certificate"),
            &verify_path,
            &culmination_receipt_session_dir,
            &decision_packet_session_dir,
            &summit_certificate_session_dir,
    ...
    fn bind_culmination_receipt_report_to_fixture(
        ...
        _summit_certificate_session_dir: &Path,
        package_dir: &Path,
        install_root: &Path,
        ...
        object.insert(
            ...
            json!(culmination_receipt_session_dir.display().to_string()),
        );
        object.insert(
        ...
        object.insert(
            "verify_culmination_receipt_command_summary".to_string(),
            json!(format!(
                "verify immutable culmination-receipt session under {} before freezing the final summit certificate",
        ...
        object.insert(
            ...
            json!("summit-certificate verify statement"),
        );
    }

    fn culmination_receipt_verify_report(
        result: &str,
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "verify_live_package_summit_certificate",
            "verdict": "tiny_live_package_summit_certificate_verify_ok",
            "reason": "summit-certificate chain is coherent",
            "result": result,
    ...
    fn summit_fixture_verify_report(
        result: &str,
        handoff_bundle_result: &str,
        decision_packet_result: &str,
        execute_frozen_result: &str,
        ...
        culmination_receipt_verify_report(
            result,
            handoff_bundle_result,
            decision_packet_result,
            execute_frozen_result,
    ...
    fn stored_culmination_receipt_session_fixture(
        culmination_receipt_session_dir: &Path,
        finality_receipt_session_dir: &Path,
        archive_receipt_session_dir: &Path,
        registry_entry_session_dir: &Path,
    ...
    fn stored_culmination_receipt_status_fixture(verify_report: &Value) -> Value {
        let mut object = Map::new();
        object.insert(
            "result".to_string(),
            json!(verify_report
