...
const USAGE: &str = "usage: copybot_tiny_live_activation_package_finality_receipt_latest --root <path> [--session-dir <path>] [--output <path>] [--json] (--plan-latest-finality-receipt | --render-latest-finality-receipt-script | --run-latest-finality-receipt | --verify-latest-finality-receipt)";
const FINALITY_RECEIPT_LATEST_SESSION_VERSION: &str = "1";
const FINALITY_RECEIPT_LATEST_STATUS_VERSION: &str = "1";
const CLOSURE_CERTIFICATE_LATEST_BIN: &str =
    "copybot_tiny_live_activation_package_closure_certificate_latest";
const FINALITY_RECEIPT_BIN: &str = "copybot_tiny_live_activation_package_finality_receipt";
const CLOSURE_CERTIFICATE_LATEST_PLAN_READY: &str =
    "tiny_live_package_closure_certificate_latest_plan_ready";
const CLOSURE_CERTIFICATE_LATEST_VERIFY_OK: &str =
    "tiny_live_package_closure_certificate_latest_verify_ok";
const FINALITY_RECEIPT_VERIFY_OK: &str = "tiny_live_package_finality_receipt_verify_ok";
const TOP_ACCEPTED_PACKAGE_STEP: &str = "clerestory_certificate";
const PLANNING_SAFE_STATEMENT: &str =
    "this latest-finality-receipt handoff binds the latest persisted immutable package chain to the accepted finality-receipt contract, but it never overrides Stage 3 or pre-activation refusal truth, it stays archival and read-only, it never runs the frozen live cutover controller by itself, and activation_authorized remains false";
...
enum Mode {
    PlanLatestFinalityReceipt,
    RenderLatestFinalityReceiptScript,
    RunLatestFinalityReceipt,
    VerifyLatestFinalityReceipt,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum TinyLivePackageFinalityReceiptLatestVerdict {
    TinyLivePackageFinalityReceiptLatestPlanReady,
    TinyLivePackageFinalityReceiptLatestScriptRendered,
    TinyLivePackageFinalityReceiptLatestRefusedByMissingLatestChain,
    TinyLivePackageFinalityReceiptLatestRefusedByInvalidLatestChain,
    TinyLivePackageFinalityReceiptLatestRefusedByDriftedFinalityReceiptContract,
    TinyLivePackageFinalityReceiptLatestRefusedNowByStage3,
    TinyLivePackageFinalityReceiptLatestRefusedNowByPreActivationGate,
    TinyLivePackageFinalityReceiptLatestReadyForManualExecutionWhenGateTurnsGreen,
    TinyLivePackageFinalityReceiptLatestVerifyOk,
    TinyLivePackageFinalityReceiptLatestVerifyInvalid,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum LivePackageFinalityReceiptLatestResult {
    RefusedByMissingLatestChain,
    RefusedByInvalidLatestChain,
    RefusedByDriftedFinalityReceiptContract,
    RefusedNowByStage3,
...
struct PackageFinalityReceiptLatestStepArtifact {
    path: String,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageFinalityReceiptLatestSession {
    session_version: String,
    planned_at: DateTime<Utc>,
    root: String,
    session_dir: String,
    nested_closure_certificate_latest_session_dir: String,
    nested_finality_receipt_session_dir: String,
    ...
    resolve_latest_closure_certificate_command_summary: String,
    run_latest_closure_certificate_command_summary: String,
    verify_latest_closure_certificate_command_summary: String,
    downstream_finality_receipt_command_summary: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageFinalityReceiptLatestStatus {
    ...
    nested_finality_receipt_session_dir: String,
    downstream_decision_packet_session_dir: String,
    result: LivePackageFinalityReceiptLatestResult,
    reason: String,
    latest_closure_certificate_plan_verdict: Option<String>,
    latest_closure_certificate_verdict: Option<String>,
    finality_receipt_verdict: Option<String>,
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    latest_closure_certificate_plan_step: Option<PackageFinalityReceiptLatestStepArtifact>,
    latest_closure_certificate_step: Option<PackageFinalityReceiptLatestStepArtifact>,
    finality_receipt_step: Option<PackageFinalityReceiptLatestStepArtifact>,
...
#[derive(Debug, Clone)]
struct PackageFinalityReceiptLatestPaths {
    session_path: PathBuf,
    status_path: PathBuf,
    report_path: PathBuf,
    latest_closure_certificate_plan_report_path: PathBuf,
    latest_closure_certificate_report_path: PathBuf,
    finality_receipt_report_path: PathBuf,
    nested_closure_certificate_latest_session_dir: PathBuf,
    nested_finality_receipt_session_dir: PathBuf,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct PackageFinalityReceiptLatestReport {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: TinyLivePackageFinalityReceiptLatestVerdict,
    reason: String,
    root: Option<String>,
    latest_top_step_name: Option<String>,
    latest_top_session_dir: Option<String>,
    ...
    finality_receipt_latest_session_path: Option<String>,
    finality_receipt_latest_status_path: Option<String>,
    finality_receipt_latest_report_path: Option<String>,
    latest_closure_certificate_plan_report_path: Option<String>,
    latest_closure_certificate_report_path: Option<String>,
    finality_receipt_report_path: Option<String>,
    nested_closure_certificate_latest_session_dir: Option<String>,
    nested_finality_receipt_session_dir: Option<String>,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    resolve_latest_closure_certificate_command_summary: Option<String>,
    run_latest_closure_certificate_command_summary: Option<String>,
    verify_latest_closure_certificate_command_summary: Option<String>,
    downstream_finality_receipt_command_summary: Option<String>,
...
#[derive(Debug, Clone)]
struct LatestClosureCertificateSnapshot {
...
#[derive(Debug, Clone)]
struct ResolvedLatestClosureCertificate {
    snapshot: LatestClosureCertificateSnapshot,
    latest_closure_certificate_plan_raw: Value,
    latest_closure_certificate_plan: ClosureCertificateLatestReportView,
}
...
enum ResolutionFailureKind {
    MissingLatestChain,
    InvalidLatestChain,
    DriftedFinalityReceiptContract,
}

#[derive(Debug, Clone)]
struct LatestClosureCertificateResolutionFailure {
    kind: ResolutionFailureKind,
    reason: String,
    snapshot: Option<LatestClosureCertificateSnapshot>,
    latest_closure_certificate_plan_raw: Option<Value>,
    latest_closure_certificate_plan_verdict: Option<String>,
    latest_closure_certificate_plan_reason: Option<String>,
    latest_closure_certificate_plan_generated_at: Option<DateTime<Utc>>,
}

#[derive(Debug, Clone, Deserialize)]
struct ClosureCertificateLatestReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
...
#[derive(Debug, Clone, Deserialize)]
struct FinalityReceiptReportView {
    generated_at: DateTime<Utc>,
    mode: String,
    verdict: String,
    reason: String,
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    closure_certificate_step: Option<PackageFinalityReceiptLatestStepArtifact>,
    verify_closure_certificate_command_summary: Option<String>,
    reviewed_frozen_live_cutover_controller_command_summary: Option<String>,
...
#[derive(Debug, Clone, Deserialize)]
struct FinalityReceiptSessionView {
    closure_certificate_session_dir: String,
...
#[derive(Debug, Clone, Deserialize)]
struct FinalityReceiptStatusView {
    session_dir: String,
    closure_certificate_session_dir: String,
    result: String,
    reason: String,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--json" => json = true,
            "--plan-latest-finality-receipt" => {
                set_mode(&mut mode, Mode::PlanLatestFinalityReceipt, arg.as_str())?
            }
            "--render-latest-finality-receipt-script" => set_mode(
                &mut mode,
                Mode::RenderLatestFinalityReceiptScript,
                arg.as_str(),
            )?,
            "--run-latest-finality-receipt" => {
                set_mode(&mut mode, Mode::RunLatestFinalityReceipt, arg.as_str())?
            }
            "--verify-latest-finality-receipt" => {
                set_mode(&mut mode, Mode::VerifyLatestFinalityReceipt, arg.as_str())?
            }
    ...
    if matches!(
        mode,
        Mode::PlanLatestFinalityReceipt
            | Mode::RenderLatestFinalityReceiptScript
            | Mode::RunLatestFinalityReceipt
    ) && root.is_none()
    {
        bail!("missing --root");
    }
    if matches!(mode, Mode::RenderLatestFinalityReceiptScript) && output_path.is_none() {
        bail!("missing --output for render mode");
    }
    if matches!(
        mode,
        Mode::RunLatestFinalityReceipt | Mode::VerifyLatestFinalityReceipt
    ) && session_dir.is_none()
    {
...
fn run(config: Config) -> Result<String> {
    let report = match config.mode {
        Mode::PlanLatestFinalityReceipt => plan_latest_closure_certificate_report(&config)?,
        Mode::RenderLatestFinalityReceiptScript => {
            render_latest_closure_certificate_script_report(&config)?
        }
        Mode::RunLatestFinalityReceipt => run_latest_closure_certificate_report(&config)?,
        Mode::VerifyLatestFinalityReceipt => verify_latest_closure_certificate_report(&config)?,
    };
    if config.json {
...
fn plan_latest_closure_certificate_report(
    config: &Config,
) -> Result<PackageFinalityReceiptLatestReport> {
    let root = config
        .root
        .as_ref()
        .ok_or_else(|| anyhow!("missing --root"))?;
    let suggested_session_dir = config.session_dir.clone().unwrap_or_else(|| {
        root.join("tiny_live_activation_package_closure_certificate_latest.session")
    });
    let paths = finality_receipt_latest_paths(&suggested_session_dir);

    match resolve_latest_closure_certificate(root, &paths) {
        Ok(resolved) => Ok(plan_report_from_resolution(
            "plan_latest_finality_receipt",
            TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestPlanReady,
            format!(
                "latest immutable {} chain under {} resolves to exact latest-closure-certificate {} lineage, exact decision-packet confirmation anchor {}, and the accepted finality-receipt contract remains current",
                resolved.snapshot.latest_top_step_name,
                root.display(),
                paths.nested_closure_certificate_latest_session_dir.display(),
...
fn render_latest_closure_certificate_script_report(
    config: &Config,
) -> Result<PackageFinalityReceiptLatestReport> {
    let root = config
        .root
        ...
        Ok(resolved) => {
            write_script(
                ...
                &format!(
                    ...
                    run_latest = run_latest_closure_certificate_command_summary(
                        root,
                        &paths.nested_closure_certificate_latest_session_dir,
                    ),
                    verify_latest = verify_latest_closure_certificate_command_summary(
                        &paths.nested_closure_certificate_latest_session_dir,
                    ),
                    run_handoff_bundle = downstream_finality_receipt_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_closure_certificate_session_dir(&paths),
                        &paths.nested_finality_receipt_session_dir,
                    ),
                    verify_handoff_bundle = verify_closure_certificate_command_summary(
                        &resolved.snapshot,
                        &expected_downstream_closure_certificate_session_dir(&paths),
                        &paths.nested_finality_receipt_session_dir,
                    ),
            ...
            Ok(plan_report_from_resolution(
                "render_latest_closure_certificate_script",
                TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestScriptRendered,
                format!(
                    "rendered latest-finality-receipt handoff script to {} using the accepted latest-closure-certificate and closure-certificate contracts",
                    output_path.display()
                ),
                root,
                &suggested_session_dir,
...
fn run_latest_closure_certificate_report(
    config: &Config,
) -> Result<PackageFinalityReceiptLatestReport> {
    let root = config
        .root
    ...
    if session_dir.exists() {
        bail!(
            "refusing to overwrite existing latest-finality-receipt session dir {}",
            session_dir.display()
        );
    }
    let paths = finality_receipt_latest_paths(session_dir);
    create_clean_session_dir(session_dir, "latest-finality-receipt")?;

    let resolution = resolve_latest_closure_certificate(root, &paths);
    match resolution {
        Ok(resolved) => {
            run_latest_closure_certificate_with_resolution(root, session_dir, &paths, &resolved)
        }
        Err(failure) => run_latest_closure_certificate_failure(root, session_dir, &paths, &failure),
    }
}

fn verify_latest_closure_certificate_report(
    config: &Config,
) -> Result<PackageFinalityReceiptLatestReport> {
    let session_dir = config
        .session_dir
    ...
    let session: PackageFinalityReceiptLatestSession = match load_json(&paths.session_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_archive_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-finality-receipt session artifact {}: {error}",
    ...
    let status: PackageFinalityReceiptLatestStatus = match load_json(&paths.status_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_archive_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-finality-receipt status artifact {}: {error}",
    ...
    let stored_report: PackageFinalityReceiptLatestReport = match load_json(&paths.report_path) {
        Ok(value) => value,
        Err(error) => {
            return Ok(
                verify_invalid_archive_receipt_latest_report_without_session(
                    session_dir,
                    vec![format!(
                        "failed reading latest-finality-receipt report artifact {}: {error}",
    ...
    compare_string(
        &session.root,
        &status.root,
        "latest-finality-receipt root consistency",
        &mut mismatches,
    );
    compare_string(
        &session.session_dir,
        &session_dir.display().to_string(),
        "latest-finality-receipt session session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.session_dir,
        &session_dir.display().to_string(),
        "latest-finality-receipt status session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "latest-finality-receipt report session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_closure_certificate_latest_session_dir,
        &paths
            .nested_closure_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-finality-receipt session nested_closure_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_closure_certificate_latest_session_dir,
        &paths
            .nested_closure_certificate_latest_session_dir
            .display()
            .to_string(),
        "latest-finality-receipt status nested_closure_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-finality-receipt report nested_closure_certificate_latest_session_dir",
        &mut mismatches,
    );
    compare_string(
        &session.nested_finality_receipt_session_dir,
        &paths
            .nested_finality_receipt_session_dir
            .display()
            .to_string(),
        "latest-finality-receipt session nested_finality_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        &status.nested_finality_receipt_session_dir,
        &paths
            .nested_finality_receipt_session_dir
            .display()
            .to_string(),
        "latest-finality-receipt status nested_finality_receipt_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.nested_finality_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_finality_receipt_session_dir
                .display()
        ...
        "latest-finality-receipt report nested_finality_receipt_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-finality-receipt session downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_string(
        ...
        &expected_downstream_decision_packet_session_dir(&paths)
            .display()
            .to_string(),
        "latest-finality-receipt status downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
        ...
        "latest-finality-receipt report downstream_decision_packet_session_dir",
        &mut mismatches,
    );
    compare_option_string(
        stored_report
            .finality_receipt_latest_session_path
            .as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "latest-finality-receipt report session path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.finality_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "latest-finality-receipt report status path",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.finality_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "latest-finality-receipt report report path",
        &mut mismatches,
    );
    compare_string(
        &session.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-finality-receipt session explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &status.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-finality-receipt status explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.explicit_statement,
        PLANNING_SAFE_STATEMENT,
        "latest-finality-receipt report explicit_statement",
        &mut mismatches,
    );
    compare_string(
        &stored_report.mode,
        "run_latest_finality_receipt",
        "latest-finality-receipt stored report mode",
        &mut mismatches,
    );
    compare_string(
        &serialize_enum(&stored_report.verdict),
        &serialize_enum(&verdict_for_result(status.result)),
        "latest-finality-receipt stored report verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.result.as_deref(),
        Some(serialize_enum(&status.result).as_str()),
        "latest-finality-receipt stored report result consistency",
        &mut mismatches,
    );
    compare_string(
        &stored_report.reason,
        &status.reason,
        "latest-finality-receipt stored report reason consistency",
        &mut mismatches,
    );
    if stored_report.activation_authorized != status.activation_authorized {
        mismatches.push(format!(
            "latest-finality-receipt stored report activation_authorized {} does not match status {}",
            stored_report.activation_authorized, status.activation_authorized
        ));
    }
    compare_option_string(
        stored_report
            .latest_closure_certificate_plan_verdict
            .as_deref(),
        status.latest_closure_certificate_plan_verdict.as_deref(),
        "stored report latest_closure_certificate_plan_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.latest_closure_certificate_verdict.as_deref(),
        status.latest_closure_certificate_verdict.as_deref(),
        "stored report latest_closure_certificate_verdict",
        &mut mismatches,
    );
    compare_option_string(
        stored_report.finality_receipt_verdict.as_deref(),
        status.finality_receipt_verdict.as_deref(),
        "stored report finality_receipt_verdict",
        &mut mismatches,
    );
    compare_option_string(
    ...
    match resolution {
        Ok(resolved) => {
            ...
            let stored_latest_plan: ClosureCertificateLatestReportView = load_required_step_json(
                &status.latest_closure_certificate_plan_step,
                &paths.latest_closure_certificate_plan_report_path,
                "latest_closure_certificate_plan_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_closure_certificate_plan_step.as_ref(),
                &paths.latest_closure_certificate_plan_report_path,
                "plan_latest_archive_receipt",
                &stored_latest_plan.verdict,
                &stored_latest_plan.reason,
                stored_latest_plan.generated_at,
                "stored latest_closure_certificate_plan_step",
                &mut mismatches,
            );
            compare_closure_certificate_latest_plan_against_snapshot(
                &stored_latest_plan,
                &resolved.snapshot,
                &paths.nested_closure_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_closure_certificate_plan_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report
            ...
            let stored_latest_run: ClosureCertificateLatestReportView = load_required_step_json(
                &status.latest_closure_certificate_step,
                &paths.latest_closure_certificate_report_path,
                "latest_closure_certificate_step",
                &mut mismatches,
            )?;
            compare_step_artifact_against_report_metadata(
                status.latest_closure_certificate_step.as_ref(),
                &paths.latest_closure_certificate_report_path,
                "run_latest_archive_receipt",
                &stored_latest_run.verdict,
                &stored_latest_run.reason,
                stored_latest_run.generated_at,
                "stored latest_closure_certificate_step",
                &mut mismatches,
            );
            compare_closure_certificate_latest_run_against_snapshot(
                &stored_latest_run,
                &resolved.snapshot,
                &paths.nested_closure_certificate_latest_session_dir,
                &mut mismatches,
            ...
            compare_option_string(
                ...
                "stored status latest_closure_certificate_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_closure_certificate_verdict.as_deref(),
            ...
            let actual_latest_run: ClosureCertificateLatestReportView = match load_json(
                &nested_closure_certificate_latest_report_path(
                    &paths.nested_closure_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed loading actual nested latest-closure-certificate report under {}: {error}",
                        paths.nested_closure_certificate_latest_session_dir.display()
                    ));
                    fallback_closure_certificate_latest_copy()
                }
            };
            compare_closure_certificate_latest_copy_matches_actual(
                &stored_latest_run,
                &actual_latest_run,
                &mut mismatches,
            );
            ...
            let latest_verify_raw = match run_json_command(
                CLOSURE_CERTIFICATE_LATEST_BIN,
                &latest_closure_certificate_verify_args(
                    &paths.nested_closure_certificate_latest_session_dir,
                ),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested latest-closure-certificate session under {}: {error}",
                        paths.nested_closure_certificate_latest_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !latest_verify_raw.is_null() {
                let latest_verify: ClosureCertificateLatestReportView =
                    serde_json::from_value(latest_verify_raw)
                        .context("failed parsing nested latest-closure-certificate verify json")?;
                if latest_verify.verdict != CLOSURE_CERTIFICATE_LATEST_VERIFY_OK {
                    mismatches.push(format!(
                        "nested latest-closure-certificate verification is non-green: {}",
                        latest_verify.reason
                    ));
                }
            }

            let stored_handoff_bundle: FinalityReceiptReportView = load_required_step_json(
            ...
            compare_step_artifact_against_report_metadata(
                ...
                &paths.finality_receipt_report_path,
                "run_live_package_finality_receipt",
                &stored_handoff_bundle.verdict,
                &stored_handoff_bundle.reason,
                stored_handoff_bundle.generated_at,
                "stored finality_receipt_step",
                &mut mismatches,
            );
            compare_finality_receipt_run_against_snapshot(
            ...
            compare_option_string(
                ...
                "stored status finality_receipt_verdict",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.finality_receipt_verdict.as_deref(),
            ...
            let nested_handoff_bundle_session: FinalityReceiptSessionView = match load_json(
                &nested_finality_receipt_session_path(&paths.nested_finality_receipt_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
                        "failed reading nested finality-receipt session under {}: {error}",
                        paths.nested_finality_receipt_session_dir.display()
                    ));
            ...
            let nested_handoff_bundle_status: FinalityReceiptStatusView = match load_json(
                &nested_finality_receipt_status_path(&paths.nested_finality_receipt_session_dir),
            ) {
                Ok(value) => value,
                Err(error) => {
                    mismatches.push(format!(
            ...
            compare_finality_receipt_copy_against_nested_truth(
                &stored_handoff_bundle,
            ...
            let handoff_bundle_verify_raw = match run_json_command(
                FINALITY_RECEIPT_BIN,
                &closure_certificate_verify_args(
                    &resolved.snapshot,
                    &expected_downstream_closure_certificate_session_dir(&paths),
                    &paths.nested_finality_receipt_session_dir,
                ),
                ...
                Err(error) => {
                    mismatches.push(format!(
                        "failed verifying nested finality-receipt session under {}: {error}",
                        paths.nested_finality_receipt_session_dir.display()
                    ));
                    Value::Null
                }
            ...
            if !handoff_bundle_verify_raw.is_null() {
                let handoff_bundle_verify: FinalityReceiptReportView =
                ...
                compare_option_string(
                    ...
                    "stored finality-receipt copy registry_entry_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    ...
                    "stored finality-receipt copy notarization_receipt_session_dir against verify truth",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_handoff_bundle
                ...
                compare_option_string(
                    ...
                    handoff_bundle_verify
                        .reviewed_frozen_live_cutover_controller_command_summary
                        .as_deref(),
                    "stored finality-receipt copy reviewed_frozen_live_cutover_controller_command_summary",
                    &mut mismatches,
                );
                if handoff_bundle_verify.verdict != FINALITY_RECEIPT_VERIFY_OK {
                    mismatches.push(format!(
            ...
            let expected_result = closure_certificate_latest_result_from_nested_status(
                nested_handoff_bundle_status.result.as_str(),
            )
            .unwrap_or(
                LivePackageFinalityReceiptLatestResult::RefusedByDriftedFinalityReceiptContract,
            );
            if status.result != expected_result {
                mismatches.push(format!(
                    "stored latest-finality-receipt result {} does not match nested closure-certificate result {}",
            ...
            if status.activation_authorized {
                mismatches.push(
                    "stored status activation_authorized must remain false for latest-finality-receipt"
                        .to_string(),
                );
            }
            compare_option_string(
        ...
        Err(failure) => {
            ...
            if failure.latest_closure_certificate_plan_raw.is_some() {
                let stored_latest_plan: ClosureCertificateLatestReportView =
                ...
                compare_option_string(
                    status.latest_closure_certificate_plan_verdict.as_deref(),
                    failure.latest_closure_certificate_plan_verdict.as_deref(),
                    "stored status latest_closure_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
                compare_option_string(
                    stored_report
                        ...
                        .as_deref(),
                    failure.latest_closure_certificate_plan_verdict.as_deref(),
                    "stored report latest_closure_certificate_plan_verdict on failure",
                    &mut mismatches,
                );
            }
            compare_option_string(
                status.latest_closure_certificate_verdict.as_deref(),
                None,
                "stored status latest_closure_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.latest_closure_certificate_verdict.as_deref(),
                None,
                "stored report latest_closure_certificate_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.finality_receipt_verdict.as_deref(),
                None,
                "stored status finality_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                stored_report.finality_receipt_verdict.as_deref(),
                None,
                "stored report finality_receipt_verdict on failure",
                &mut mismatches,
            );
            compare_option_string(
                status.current_pre_activation_gate_verdict.as_deref(),
    ...
    let verdict = if mismatches.is_empty() {
        TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestVerifyOk
    } else {
        TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestVerifyInvalid
    };
    let reason = if mismatches.is_empty() {
        format!(
            "latest-finality-receipt session under {} is coherent with the current latest immutable chain and the accepted finality-receipt contract",
            session_dir.display()
        )
    } else {
        mismatches
            .first()
            .cloned()
            .unwrap_or_else(|| "latest-finality-receipt verification failed".to_string())
    };

    Ok(PackageFinalityReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_finality_receipt".to_string(),
        verdict,
        reason,
        ...
        finality_receipt_latest_session_path: Some(paths.session_path.display().to_string()),
        finality_receipt_latest_status_path: Some(paths.status_path.display().to_string()),
        finality_receipt_latest_report_path: Some(paths.report_path.display().to_string()),
        latest_closure_certificate_plan_report_path: Some(
            paths
                .latest_closure_certificate_plan_report_path
                .display()
                .to_string(),
        ),
        latest_closure_certificate_report_path: Some(
            paths.latest_closure_certificate_report_path.display().to_string(),
        ),
        finality_receipt_report_path: Some(paths.finality_receipt_report_path.display().to_string()),
        nested_closure_certificate_latest_session_dir: Some(
            paths
                .nested_closure_certificate_latest_session_dir
                .display()
                .to_string(),
        ),
        nested_finality_receipt_session_dir: Some(
            paths.nested_finality_receipt_session_dir.display().to_string(),
        ),
        ...
        latest_closure_certificate_plan_verdict: status.latest_closure_certificate_plan_verdict.clone(),
        latest_closure_certificate_verdict: status.latest_closure_certificate_verdict.clone(),
        finality_receipt_verdict: status.finality_receipt_verdict.clone(),
        ...
        resolve_latest_closure_certificate_command_summary: Some(
            session.resolve_latest_closure_certificate_command_summary.clone(),
        ),
        run_latest_closure_certificate_command_summary: Some(
            session.run_latest_closure_certificate_command_summary.clone(),
        ),
        verify_latest_closure_certificate_command_summary: Some(
            session.verify_latest_closure_certificate_command_summary.clone(),
        ),
        downstream_finality_receipt_command_summary: Some(
            session.downstream_finality_receipt_command_summary.clone(),
        ),
        verify_closure_certificate_command_summary: Some(session.verify_closure_certificate_command_summary.clone()),
        ...
        explicit_statement:
            "this verification proves only whether the latest immutable chain still resolves cleanly into the accepted finality-receipt contract; it never runs the frozen controller"
                .to_string(),
    })
}

fn run_latest_closure_certificate_with_resolution(
    root: &Path,
    session_dir: &Path,
    paths: &PackageFinalityReceiptLatestPaths,
    resolved: &ResolvedLatestClosureCertificate,
) -> Result<PackageFinalityReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, Some(resolved), None);
    persist_json(&paths.session_path, &session)?;
    persist_json(
        &paths.latest_closure_certificate_plan_report_path,
        &resolved.latest_closure_certificate_plan_raw,
    )?;

    let latest_closure_certificate_plan_step = Some(step_artifact(
        &paths.latest_closure_certificate_plan_report_path,
        "plan_latest_archive_receipt",
        &resolved.latest_closure_certificate_plan.verdict,
        &resolved.latest_closure_certificate_plan.reason,
        resolved.latest_closure_certificate_plan.generated_at,
    ));

    let latest_archive_receipt_run_raw = match run_json_command(
        CLOSURE_CERTIFICATE_LATEST_BIN,
        &latest_closure_certificate_run_args(
            root,
            &paths.nested_closure_certificate_latest_session_dir,
        ),
    ) {
        ...
        Err(error) => {
            let status =
                refusal_status(
                    ...
                    LivePackageFinalityReceiptLatestResult::RefusedByDriftedFinalityReceiptContract,
                    format!(
                    "failed running nested latest-closure-certificate handoff under {}: {error}",
                    paths.nested_closure_certificate_latest_session_dir.display()
                ),
                    latest_closure_certificate_plan_step,
                    None,
    ...
    persist_json(
        &paths.latest_closure_certificate_report_path,
        &latest_archive_receipt_run_raw,
    )?;
    let latest_archive_receipt_run: ClosureCertificateLatestReportView =
        serde_json::from_value(latest_archive_receipt_run_raw)
            .context("failed parsing nested latest-closure-certificate run json")?;
    let latest_closure_certificate_step = Some(step_artifact(
        &paths.latest_closure_certificate_report_path,
        "run_latest_archive_receipt",
        &latest_archive_receipt_run.verdict,
        &latest_archive_receipt_run.reason,
        latest_archive_receipt_run.generated_at,
    ));

    if let Some(terminal_failure) =
        latest_closure_certificate_terminal_failure(&latest_archive_receipt_run)
    {
        let status = refusal_status(
            ...
            Some(resolved),
            terminal_failure,
            latest_archive_receipt_run.reason.clone(),
            latest_closure_certificate_plan_step,
            latest_closure_certificate_step,
            None,
        );
    ...
    let latest_archive_receipt_verify: ClosureCertificateLatestReportView = match run_json_command(
        CLOSURE_CERTIFICATE_LATEST_BIN,
        &latest_closure_certificate_verify_args(
            &paths.nested_closure_certificate_latest_session_dir,
        ),
        ...
        Err(error) => {
            let status =
                refusal_status(
                    ...
                    LivePackageFinalityReceiptLatestResult::RefusedByDriftedFinalityReceiptContract,
                    format!(
                    "failed verifying nested latest-closure-certificate handoff under {}: {error}",
                    paths.nested_closure_certificate_latest_session_dir.display()
                ),
                    latest_closure_certificate_plan_step,
                    latest_closure_certificate_step,
    ...
    if latest_archive_receipt_verify.verdict != CLOSURE_CERTIFICATE_LATEST_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
            Some(resolved),
            LivePackageFinalityReceiptLatestResult::RefusedByDriftedFinalityReceiptContract,
            latest_archive_receipt_verify.reason,
            latest_closure_certificate_plan_step,
            latest_closure_certificate_step,
            None,
        );
    ...
    let provenance_certificate_run: FinalityReceiptReportView =
    ...
    let provenance_certificate_verify: FinalityReceiptReportView = match run_json_command(
        FINALITY_RECEIPT_BIN,
        &closure_certificate_verify_args(
            &resolved.snapshot,
            &expected_downstream_closure_certificate_session_dir(paths),
    ...
    if provenance_certificate_verify.verdict != FINALITY_RECEIPT_VERIFY_OK {
        let status = refusal_status(
            root,
            session_dir,
            paths,
    ...
    let result = closure_certificate_latest_result_from_nested_status(
        provenance_certificate_run
            .result
            .as_deref()
            .unwrap_or_default(),
    )
    .unwrap_or(LivePackageFinalityReceiptLatestResult::RefusedByDriftedFinalityReceiptContract);
    let status = PackageFinalityReceiptLatestStatus {
        status_version: FINALITY_RECEIPT_LATEST_STATUS_VERSION.to_string(),
        updated_at: Utc::now(),
        ...
        nested_closure_certificate_latest_session_dir: paths
            .nested_closure_certificate_latest_session_dir
            .display()
            .to_string(),
        nested_finality_receipt_session_dir: paths
            .nested_finality_receipt_session_dir
            .display()
            .to_string(),
        ...
        latest_closure_certificate_plan_verdict: Some(
            resolved.latest_closure_certificate_plan.verdict.clone(),
        ),
        latest_closure_certificate_verdict: Some(latest_archive_receipt_run.verdict.clone()),
        finality_receipt_verdict: Some(provenance_certificate_run.verdict.clone()),
...
fn run_latest_closure_certificate_failure(
    root: &Path,
    session_dir: &Path,
    paths: &PackageFinalityReceiptLatestPaths,
    failure: &LatestClosureCertificateResolutionFailure,
) -> Result<PackageFinalityReceiptLatestReport> {
    let session = planned_session(root, session_dir, paths, None, Some(failure));
    persist_json(&paths.session_path, &session)?;
    if let Some(value) = &failure.latest_closure_certificate_plan_raw {
        persist_json(&paths.latest_closure_certificate_plan_report_path, value)?;
    }
...
fn resolve_latest_closure_certificate(
    root: &Path,
    paths: &PackageFinalityReceiptLatestPaths,
) -> std::result::Result<ResolvedLatestClosureCertificate, LatestClosureCertificateResolutionFailure>
{
    let latest_closure_certificate_plan_raw = match run_json_command(
        CLOSURE_CERTIFICATE_LATEST_BIN,
        &latest_closure_certificate_plan_args(
            root,
            &paths.nested_closure_certificate_latest_session_dir,
        ),
    ) {
        Ok(value) => value,
        Err(error) => {
            return Err(LatestClosureCertificateResolutionFailure {
                ...
                snapshot: None,
                latest_closure_certificate_plan_raw: None,
                latest_closure_certificate_plan_verdict: None,
                latest_closure_certificate_plan_reason: None,
                latest_closure_certificate_plan_generated_at: None,
            });
        }
    };
    let latest_closure_certificate_plan: ClosureCertificateLatestReportView =
        serde_json::from_value(latest_closure_certificate_plan_raw.clone()).map_err(|error| {
            LatestClosureCertificateResolutionFailure {
                kind: ResolutionFailureKind::InvalidLatestChain,
                reason: format!(
                    "failed parsing latest-closure-certificate plan json for {}: {error}",
                    root.display()
                ),
                snapshot: None,
                latest_closure_certificate_plan_raw: Some(
                    latest_closure_certificate_plan_raw.clone(),
                ),
                latest_closure_certificate_plan_verdict: None,
                latest_closure_certificate_plan_reason: None,
                latest_closure_certificate_plan_generated_at: None,
    ...
    let snapshot =
        parse_snapshot_from_archive_receipt_latest(&latest_closure_certificate_plan).ok();
    if latest_closure_certificate_plan.verdict != CLOSURE_CERTIFICATE_LATEST_PLAN_READY {
        return Err(LatestClosureCertificateResolutionFailure {
            kind: map_closure_certificate_latest_failure_kind(
                &latest_closure_certificate_plan.verdict,
            ),
            reason: latest_closure_certificate_plan.reason.clone(),
            snapshot,
            latest_closure_certificate_plan_raw: Some(latest_closure_certificate_plan_raw),
            latest_closure_certificate_plan_verdict: Some(latest_closure_certificate_plan.verdict),
            latest_closure_certificate_plan_reason: Some(latest_closure_certificate_plan.reason),
            latest_closure_certificate_plan_generated_at: Some(
                latest_closure_certificate_plan.generated_at,
            ),
        });
    }

    let snapshot = parse_snapshot_from_archive_receipt_latest(&latest_closure_certificate_plan)
        .map_err(|error| LatestClosureCertificateResolutionFailure {
            kind: ResolutionFailureKind::InvalidLatestChain,
            reason: error.to_string(),
            snapshot: None,
            latest_closure_certificate_plan_raw: Some(latest_closure_certificate_plan_raw.clone()),
            latest_closure_certificate_plan_verdict: Some(
                latest_closure_certificate_plan.verdict.clone(),
            ),
            latest_closure_certificate_plan_reason: Some(
                latest_closure_certificate_plan.reason.clone(),
            ),
            latest_closure_certificate_plan_generated_at: Some(
                latest_closure_certificate_plan.generated_at,
            ),
    ...
    let mut mismatches = Vec::new();
    compare_closure_certificate_latest_plan_against_snapshot(
        &latest_closure_certificate_plan,
        &snapshot,
        &paths.nested_closure_certificate_latest_session_dir,
        &mut mismatches,
    );
    if !mismatches.is_empty() {
        return Err(LatestClosureCertificateResolutionFailure {
            kind: ResolutionFailureKind::DriftedFinalityReceiptContract,
    ...
    Ok(ResolvedLatestClosureCertificate {
        snapshot,
        latest_closure_certificate_plan_raw,
        latest_closure_certificate_plan,
    })
}

fn parse_snapshot_from_archive_receipt_latest(
    view: &ClosureCertificateLatestReportView,
) -> Result<LatestClosureCertificateSnapshot> {
    ...
    if latest_top_step_name != TOP_ACCEPTED_PACKAGE_STEP {
        bail!(
            "latest-closure-certificate plan top accepted step is {} instead of {}",
    ...
    if view.activation_authorized {
        bail!(
            "latest-closure-certificate plan must remain planning-safe with activation_authorized=false"
        );
    }
    Ok(LatestClosureCertificateSnapshot {
        latest_top_step_name,
        ...
        wrapper_timeout_ms: view.wrapper_timeout_ms.ok_or_else(|| {
            anyhow!("latest-closure-certificate plan is missing wrapper_timeout_ms")
        })?,
        service_status_max_staleness_ms: view.service_status_max_staleness_ms.ok_or_else(|| {
            anyhow!("latest-closure-certificate plan is missing service_status_max_staleness_ms")
        })?,
...
fn plan_report_from_resolution(
    ...
    paths: &PackageFinalityReceiptLatestPaths,
    resolved: &ResolvedLatestClosureCertificate,
) -> PackageFinalityReceiptLatestReport {
    PackageFinalityReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        latest_closure_certificate_report_path: Some(
            paths
                .latest_closure_certificate_report_path
                .display()
                .to_string(),
        ),
        finality_receipt_report_path: Some(
        ...
        latest_closure_certificate_verdict: None,
        finality_receipt_verdict: None,
        result: None,
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_closure_certificate_command_summary: Some(
            resolve_latest_closure_certificate_command_summary(
                root,
                &paths.nested_closure_certificate_latest_session_dir,
            ),
        ),
        run_latest_closure_certificate_command_summary: Some(
            run_latest_closure_certificate_command_summary(
                root,
                &paths.nested_closure_certificate_latest_session_dir,
            ),
        ),
        verify_latest_closure_certificate_command_summary: Some(
            verify_latest_closure_certificate_command_summary(
                &paths.nested_closure_certificate_latest_session_dir,
            ),
        ),
        downstream_finality_receipt_command_summary: Some(
            downstream_finality_receipt_command_summary(
                &resolved.snapshot,
                &expected_downstream_closure_certificate_session_dir(paths),
                &paths.nested_finality_receipt_session_dir,
            ),
        ...
        verify_closure_certificate_command_summary: Some(
            verify_closure_certificate_command_summary(
                &resolved.snapshot,
                &expected_downstream_closure_certificate_session_dir(paths),
                &paths.nested_finality_receipt_session_dir,
            ),
...
fn report_from_resolution_failure(
    ...
    PackageFinalityReceiptLatestReport {
        ...
        latest_closure_certificate_plan_verdict: failure
            .latest_closure_certificate_plan_verdict
            .clone(),
        latest_closure_certificate_verdict: None,
        finality_receipt_verdict: None,
        ...
        downstream_finality_receipt_command_summary: snapshot.map(|value| {
            downstream_finality_receipt_command_summary(
                value,
                &expected_downstream_closure_certificate_session_dir(paths),
                &paths.nested_finality_receipt_session_dir,
        ...
        verify_closure_certificate_command_summary: snapshot.map(|value| {
            verify_closure_certificate_command_summary(
                value,
                &expected_downstream_closure_certificate_session_dir(paths),
                &paths.nested_finality_receipt_session_dir,
...
fn planned_session(
    ...
    session_dir: &Path,
    paths: &PackageFinalityReceiptLatestPaths,
    resolved: Option<&ResolvedLatestClosureCertificate>,
    failure: Option<&LatestClosureCertificateResolutionFailure>,
) -> PackageFinalityReceiptLatestSession {
    let snapshot = resolved
        .map(|value| &value.snapshot)
        .or_else(|| failure.and_then(|value| value.snapshot.as_ref()));
    PackageFinalityReceiptLatestSession {
        session_version: FINALITY_RECEIPT_LATEST_SESSION_VERSION.to_string(),
        planned_at: Utc::now(),
        root: root.display().to_string(),
        session_dir: session_dir.display().to_string(),
        nested_closure_certificate_latest_session_dir: paths
        ...
        downstream_finality_receipt_command_summary: snapshot
            .map(|value| {
                downstream_finality_receipt_command_summary(
                    value,
                    &expected_downstream_closure_certificate_session_dir(paths),
        ...
        verify_closure_certificate_command_summary: snapshot
            .map(|value| {
                verify_closure_certificate_command_summary(
                    value,
                    &expected_downstream_closure_certificate_session_dir(paths),
...
fn report_from_status(
    ...
    session_dir: &Path,
    paths: &PackageFinalityReceiptLatestPaths,
    session: &PackageFinalityReceiptLatestSession,
    status: &PackageFinalityReceiptLatestStatus,
    verification_mismatches: Vec<String>,
) -> PackageFinalityReceiptLatestReport {
    PackageFinalityReceiptLatestReport {
        generated_at: Utc::now(),
        ...
        nested_finality_receipt_session_dir: Some(
            session.nested_finality_receipt_session_dir.clone(),
        ),
        downstream_decision_packet_session_dir: Some(
            session.downstream_decision_packet_session_dir.clone(),
        ),
        latest_closure_certificate_plan_verdict: status
            .latest_closure_certificate_plan_verdict
            .clone(),
        latest_closure_certificate_verdict: status.latest_closure_certificate_verdict.clone(),
        finality_receipt_verdict: status.finality_receipt_verdict.clone(),
        ...
        verify_closure_certificate_command_summary: Some(
            session.verify_closure_certificate_command_summary.clone(),
        ),
        reviewed_frozen_live_cutover_controller_command_summary: session
            .reviewed_frozen_live_cutover_controller_command_summary
            .clone(),
...
fn finality_receipt_latest_paths(session_dir: &Path) -> PackageFinalityReceiptLatestPaths {
    PackageFinalityReceiptLatestPaths {
        session_path: session_dir
            .join("tiny_live_activation_package_finality_receipt_latest.session.json"),
        status_path: session_dir
            .join("tiny_live_activation_package_finality_receipt_latest.status.json"),
        report_path: session_dir
            .join("tiny_live_activation_package_finality_receipt_latest.report.json"),
        latest_closure_certificate_plan_report_path: session_dir.join(
            "tiny_live_activation_package_finality_receipt_latest.closure_certificate_latest_plan.report.json",
        ),
        latest_closure_certificate_report_path: session_dir.join(
            "tiny_live_activation_package_finality_receipt_latest.closure_certificate_latest.report.json",
        ),
        finality_receipt_report_path: session_dir.join(
            "tiny_live_activation_package_finality_receipt_latest.finality_receipt.report.json",
        ),
        nested_closure_certificate_latest_session_dir: session_dir
            .join("tiny_live_activation_package_finality_receipt_latest.closure_certificate_latest_session"),
        nested_finality_receipt_session_dir: session_dir.join(
            "tiny_live_activation_package_finality_receipt_latest.finality_receipt_session",
        ),
    }
}
...
fn expected_downstream_decision_packet_session_dir(
    ...
    paths
        .nested_closure_certificate_latest_session_dir
        .join("tiny_live_activation_package_closure_certificate_latest.decision_packet_session")
}

fn expected_downstream_closure_certificate_session_dir(
    paths: &PackageFinalityReceiptLatestPaths,
) -> PathBuf {
    paths
        .nested_closure_certificate_latest_session_dir
        .join("tiny_live_activation_package_closure_certificate_latest.closure_certificate_session")
}
...
fn expected_downstream_review_receipt_session_dir(
    ...
    paths
        .nested_closure_certificate_latest_session_dir
        .join("tiny_live_activation_package_closure_certificate_latest.review_receipt_session")
}

fn latest_closure_certificate_plan_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--plan-latest-closure-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn latest_closure_certificate_run_args(root: &Path, session_dir: &Path) -> Vec<String> {
    vec![
        "--root".to_string(),
        root.display().to_string(),
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-latest-closure-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn latest_closure_certificate_verify_args(session_dir: &Path) -> Vec<String> {
    vec![
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-latest-closure-certificate".to_string(),
        "--json".to_string(),
    ]
}

fn finality_receipt_run_args(
    snapshot: &LatestClosureCertificateSnapshot,
    closure_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        "--closure-certificate-session-dir".to_string(),
        closure_certificate_session_dir.display().to_string(),
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--run-live-package-finality-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn closure_certificate_verify_args(
    snapshot: &LatestClosureCertificateSnapshot,
    closure_certificate_session_dir: &Path,
    session_dir: &Path,
) -> Vec<String> {
    vec![
        ...
        "--session-dir".to_string(),
        session_dir.display().to_string(),
        "--verify-live-package-finality-receipt".to_string(),
        "--json".to_string(),
    ]
}

fn resolve_latest_closure_certificate_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{CLOSURE_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --plan-latest-closure-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn run_latest_closure_certificate_command_summary(root: &Path, session_dir: &Path) -> String {
    format!(
        "{CLOSURE_CERTIFICATE_LATEST_BIN} --root {} --session-dir {} --run-latest-closure-certificate --json",
        shell_escape_path(root),
        shell_escape_path(session_dir),
    )
}

fn verify_latest_closure_certificate_command_summary(session_dir: &Path) -> String {
    format!(
        "{CLOSURE_CERTIFICATE_LATEST_BIN} --session-dir {} --verify-latest-closure-certificate --json",
        shell_escape_path(session_dir),
    )
}

fn downstream_finality_receipt_command_summary(
    snapshot: &LatestClosureCertificateSnapshot,
    closure_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{FINALITY_RECEIPT_BIN} --closure-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --run-live-package-finality-receipt --json",
...
fn verify_closure_certificate_command_summary(
    snapshot: &LatestClosureCertificateSnapshot,
    closure_certificate_session_dir: &Path,
    session_dir: &Path,
) -> String {
    format!(
        "{FINALITY_RECEIPT_BIN} --closure-certificate-session-dir {} --confirm-decision-packet-session-dir {} --session-dir {} --verify-live-package-finality-receipt --json",
        shell_escape_path(closure_certificate_session_dir),
        shell_escape_path(&snapshot.downstream_decision_packet_session_dir),
        shell_escape_path(session_dir),
    )
...
fn nested_closure_certificate_latest_report_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_closure_certificate_latest.report.json")
}

fn nested_finality_receipt_session_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_finality_receipt.session.json")
}

fn nested_finality_receipt_status_path(session_dir: &Path) -> PathBuf {
    session_dir.join("tiny_live_activation_package_finality_receipt.status.json")
}
...
fn write_script(path: &Path, contents: &str) -> Result<()> {
    ...
    fs::write(path, contents).with_context(|| {
        format!(
            "failed writing latest-finality-receipt script to {}",
            path.display()
        )
    })?;
    #[cfg(unix)]
...
fn step_artifact(
    ...
    reason: &str,
    generated_at: DateTime<Utc>,
) -> PackageFinalityReceiptLatestStepArtifact {
    PackageFinalityReceiptLatestStepArtifact {
...
fn canonical_wrapper_mode(mode: &str) -> &str {
    match mode {
        "plan_latest_finality_receipt" => "plan_latest_finality_receipt",
        "render_latest_closure_certificate_script" => "render_latest_closure_certificate_script",
        "run_latest_finality_receipt" => "run_latest_finality_receipt",
        "verify_latest_finality_receipt" => "verify_latest_finality_receipt",
        other => other,
    }
}

fn load_required_step_json<T: for<'de> Deserialize<'de>>(
    step: &Option<PackageFinalityReceiptLatestStepArtifact>,
    expected_path: &Path,
    label: &str,
    ...
    let step = step
        .as_ref()
        .ok_or_else(|| anyhow!("{label} is missing from latest-finality-receipt status"))?;
    compare_string(
        &step.path,
...
fn compare_step_artifact_against_report_metadata(
    ...
    let Some(step) = step else {
        mismatches.push(format!(
            "{label} is missing from latest-finality-receipt status"
        ));
        return;
    };
    compare_string(
...
fn compare_closure_certificate_latest_plan_against_snapshot(
    stored: &ClosureCertificateLatestReportView,
    snapshot: &LatestClosureCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        ...
        Some(CLOSURE_CERTIFICATE_LATEST_PLAN_READY),
        "stored latest-closure-certificate plan verdict",
        mismatches,
    );
    compare_string(
        &stored.mode,
        "plan_latest_archive_receipt",
        "stored latest-closure-certificate plan mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-closure-certificate plan latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        ...
        "stored latest-closure-certificate plan latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-closure-certificate plan session_dir",
        mismatches,
    );
    compare_option_string(
    ...
    compare_option_string(
        ...
        "stored latest-closure-certificate plan reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-closure-certificate plan clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-closure-certificate plan clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-closure-certificate plan clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_closure_certificate_latest_run_against_snapshot(
    stored: &ClosureCertificateLatestReportView,
    snapshot: &LatestClosureCertificateSnapshot,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_latest_archive_receipt",
        "stored latest-closure-certificate run mode",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_step_name.as_deref(),
        Some(snapshot.latest_top_step_name.as_str()),
        "stored latest-closure-certificate run latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        ...
        "stored latest-closure-certificate run latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored latest-closure-certificate run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        ...
        "stored latest-closure-certificate run downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored latest-closure-certificate run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored latest-closure-certificate run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored latest-closure-certificate run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored latest-closure-certificate run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored latest-closure-certificate run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored latest-closure-certificate run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored latest-closure-certificate run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_summary.as_deref(),
        Some(snapshot.clerestory_certificate_summary.as_str()),
        "stored latest-closure-certificate run clerestory_certificate_summary",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_sha256.as_deref(),
        Some(snapshot.clerestory_certificate_sha256.as_str()),
        "stored latest-closure-certificate run clerestory_certificate_sha256",
        mismatches,
    );
    compare_option_string(
        stored.clerestory_certificate_algorithm.as_deref(),
        Some(snapshot.clerestory_certificate_algorithm.as_str()),
        "stored latest-closure-certificate run clerestory_certificate_algorithm",
        mismatches,
    );
}

fn compare_closure_certificate_latest_copy_matches_actual(
    stored: &ClosureCertificateLatestReportView,
    actual: &ClosureCertificateLatestReportView,
    mismatches: &mut Vec<String>,
) {
    compare_option_string(
        Some(stored.verdict.as_str()),
        Some(actual.verdict.as_str()),
        "stored latest-closure-certificate copy verdict",
        mismatches,
    );
    compare_option_string(
        Some(stored.reason.as_str()),
        Some(actual.reason.as_str()),
        "stored latest-closure-certificate copy reason",
        mismatches,
    );
    if stored.generated_at != actual.generated_at {
        mismatches.push(format!(
            "stored latest-closure-certificate copy generated_at {:?} does not match actual {:?}",
    ...
    compare_option_string(
        ...
        "stored latest-closure-certificate copy latest_top_step_name",
        mismatches,
    );
    compare_option_string(
        stored.latest_top_session_dir.as_deref(),
        actual.latest_top_session_dir.as_deref(),
        "stored latest-closure-certificate copy latest_top_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        actual.session_dir.as_deref(),
        "stored latest-closure-certificate copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.downstream_decision_packet_session_dir.as_deref(),
        actual.downstream_decision_packet_session_dir.as_deref(),
        "stored latest-closure-certificate copy downstream_decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        actual.result.as_deref(),
        "stored latest-closure-certificate copy result",
        mismatches,
    );
    compare_option_string(
        stored.explicit_statement.as_deref(),
        actual.explicit_statement.as_deref(),
        "stored latest-closure-certificate copy explicit_statement",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        actual.current_pre_activation_gate_verdict.as_deref(),
        "stored latest-closure-certificate copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        actual.current_pre_activation_gate_reason.as_deref(),
        "stored latest-closure-certificate copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        actual
            .reviewed_frozen_live_cutover_controller_command_summary
            .as_deref(),
        "stored latest-closure-certificate copy reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_finality_receipt_run_against_snapshot(
    stored: &FinalityReceiptReportView,
    snapshot: &LatestClosureCertificateSnapshot,
    closure_certificate_session_dir: &Path,
    session_dir: &Path,
    mismatches: &mut Vec<String>,
    ...
    compare_string(
        &stored.mode,
        "run_live_package_finality_receipt",
        "stored finality-receipt run mode",
        mismatches,
    );
    compare_string(
        &stored.closure_certificate_session_dir,
        &closure_certificate_session_dir.display().to_string(),
        "stored finality-receipt run closure_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored finality-receipt run decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored finality-receipt run execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored finality-receipt run turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        ...
        "stored finality-receipt run launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.package_dir.as_deref(),
        Some(snapshot.package_dir.display().to_string().as_str()),
        "stored finality-receipt run package_dir",
        mismatches,
    );
    compare_option_string(
        stored.install_root.as_deref(),
        Some(snapshot.install_root.display().to_string().as_str()),
        "stored finality-receipt run install_root",
        mismatches,
    );
    compare_option_string(
        stored.target_service_name.as_deref(),
        Some(snapshot.target_service_name.as_str()),
        "stored finality-receipt run target_service_name",
        mismatches,
    );
    compare_option_string(
        stored.backend_command.as_deref(),
        Some(snapshot.backend_command.as_str()),
        "stored finality-receipt run backend_command",
        mismatches,
    );
    compare_option_u64(
        stored.wrapper_timeout_ms,
        Some(snapshot.wrapper_timeout_ms),
        "stored finality-receipt run wrapper_timeout_ms",
        mismatches,
    );
    compare_option_u64(
        stored.service_status_max_staleness_ms,
        Some(snapshot.service_status_max_staleness_ms),
        "stored finality-receipt run service_status_max_staleness_ms",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(session_dir.display().to_string().as_str()),
        "stored finality-receipt run session_dir",
        mismatches,
    );
    compare_option_string(
        stored.finality_receipt_session_path.as_deref(),
        Some(
            nested_finality_receipt_session_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored finality-receipt run finality_receipt_session_path",
        mismatches,
    );
    compare_option_string(
        stored.finality_receipt_status_path.as_deref(),
        Some(
            nested_finality_receipt_status_path(session_dir)
                .display()
                .to_string()
                .as_str(),
        ),
        "stored finality-receipt run finality_receipt_status_path",
        mismatches,
    );
    compare_option_string(
        stored.archived_closure_certificate_report_path.as_deref(),
        Some(
            session_dir
                .join(
        ...
        "stored finality-receipt run archived_closure_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        stored.verify_closure_certificate_command_summary.as_deref(),
        Some(
            finality_receipt_verify_closure_certificate_command_summary(
                closure_certificate_session_dir,
            )
            .as_str(),
        ),
        "stored finality-receipt run verify_closure_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
        ...
        "stored finality-receipt run reviewed_frozen_live_cutover_controller_command_summary",
        mismatches,
    );
}

fn compare_finality_receipt_copy_against_nested_truth(
    stored: &FinalityReceiptReportView,
    nested_session: &FinalityReceiptSessionView,
    nested_status: &FinalityReceiptStatusView,
    mismatches: &mut Vec<String>,
) {
    compare_string(
        &stored.closure_certificate_session_dir,
        &nested_session.closure_certificate_session_dir,
        "stored finality-receipt copy closure_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.registry_entry_session_dir.as_deref(),
        Some(nested_session.registry_entry_session_dir.as_str()),
        "stored finality-receipt copy registry_entry_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.notarization_receipt_session_dir.as_deref(),
        Some(nested_session.notarization_receipt_session_dir.as_str()),
        "stored finality-receipt copy notarization_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.provenance_certificate_session_dir.as_deref(),
        Some(nested_session.provenance_certificate_session_dir.as_str()),
        "stored finality-receipt copy provenance_certificate_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.attestation_seal_session_dir.as_deref(),
        Some(nested_session.attestation_seal_session_dir.as_str()),
        "stored finality-receipt copy attestation_seal_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.release_capsule_session_dir.as_deref(),
        Some(nested_session.release_capsule_session_dir.as_str()),
        "stored finality-receipt copy release_capsule_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.activation_ticket_session_dir.as_deref(),
        Some(nested_session.activation_ticket_session_dir.as_str()),
        "stored finality-receipt copy activation_ticket_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.review_receipt_session_dir.as_deref(),
        Some(nested_session.review_receipt_session_dir.as_str()),
        "stored finality-receipt copy review_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.handoff_bundle_session_dir.as_deref(),
        Some(nested_session.handoff_bundle_session_dir.as_str()),
        "stored finality-receipt copy handoff_bundle_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.decision_packet_session_dir.as_deref(),
        Some(nested_session.decision_packet_session_dir.as_str()),
        "stored finality-receipt copy decision_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.execute_frozen_session_dir.as_deref(),
        Some(nested_session.execute_frozen_session_dir.as_str()),
        "stored finality-receipt copy execute_frozen_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.turn_green_session_dir.as_deref(),
        Some(nested_session.turn_green_session_dir.as_str()),
        "stored finality-receipt copy turn_green_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.launch_packet_session_dir.as_deref(),
        Some(nested_session.launch_packet_session_dir.as_str()),
        "stored finality-receipt copy launch_packet_session_dir",
        mismatches,
    );
    compare_option_string(
        stored.session_dir.as_deref(),
        Some(nested_session.session_dir.as_str()),
        "stored finality-receipt copy session_dir",
        mismatches,
    );
    compare_option_string(
        stored.result.as_deref(),
        Some(nested_status.result.as_str()),
        "stored finality-receipt copy result",
        mismatches,
    );
    compare_option_string(
        Some(stored.verdict.as_str()),
        finality_receipt_run_verdict_for_result(&nested_status.result),
        "stored finality-receipt copy verdict",
        mismatches,
    );
    compare_option_string(
        stored.handoff_bundle_result.as_deref(),
        nested_status.handoff_bundle_result.as_deref(),
        "stored finality-receipt copy handoff_bundle_result",
        mismatches,
    );
    compare_option_string(
        stored.decision_packet_result.as_deref(),
        nested_status.decision_packet_result.as_deref(),
        "stored finality-receipt copy decision_packet_result",
        mismatches,
    );
    compare_option_string(
        stored.execute_frozen_result.as_deref(),
        nested_status.execute_frozen_result.as_deref(),
        "stored finality-receipt copy execute_frozen_result",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_verdict.as_deref(),
        nested_status.current_pre_activation_gate_verdict.as_deref(),
        "stored finality-receipt copy current_pre_activation_gate_verdict",
        mismatches,
    );
    compare_option_string(
        stored.current_pre_activation_gate_reason.as_deref(),
        nested_status.current_pre_activation_gate_reason.as_deref(),
        "stored finality-receipt copy current_pre_activation_gate_reason",
        mismatches,
    );
    compare_option_string(
        stored.verify_closure_certificate_command_summary.as_deref(),
        ...
        "stored finality-receipt copy verify_closure_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        stored
    ...
    compare_option_string(
        ...
        "stored finality-receipt copy release_capsule_digest_manifest_sha256",
        mismatches,
    );
...
fn compare_session_against_snapshot(
    ...
    compare_string(
        &session.resolve_latest_closure_certificate_command_summary,
        &resolve_latest_closure_certificate_command_summary(
            root,
            &paths.nested_closure_certificate_latest_session_dir,
        ),
        "stored session resolve_latest_closure_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.run_latest_closure_certificate_command_summary,
        &run_latest_closure_certificate_command_summary(
            root,
            &paths.nested_closure_certificate_latest_session_dir,
        ),
        "stored session run_latest_closure_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_latest_closure_certificate_command_summary,
        &verify_latest_closure_certificate_command_summary(
            &paths.nested_closure_certificate_latest_session_dir,
        ),
        "stored session verify_latest_closure_certificate_command_summary",
        mismatches,
    );
    compare_string(
        &session.downstream_finality_receipt_command_summary,
        &downstream_finality_receipt_command_summary(
            snapshot,
            &expected_downstream_closure_certificate_session_dir(paths),
            &paths.nested_finality_receipt_session_dir,
        ),
        "stored session downstream_finality_receipt_command_summary",
        mismatches,
    );
    compare_string(
        &session.verify_closure_certificate_command_summary,
        &verify_closure_certificate_command_summary(
            snapshot,
            &expected_downstream_closure_certificate_session_dir(paths),
            &paths.nested_finality_receipt_session_dir,
        ),
        "stored session verify_closure_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_closure_certificate_latest_session_dir",
        mismatches,
    );
    compare_string(
        &session.nested_finality_receipt_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_finality_receipt_session_dir",
        mismatches,
    );
    compare_string(
        &session.downstream_decision_packet_session_dir,
...
fn compare_report_against_snapshot(
    report: &PackageFinalityReceiptLatestReport,
    root: &Path,
    session_dir: &Path,
    paths: &PackageFinalityReceiptLatestPaths,
    snapshot: &LatestClosureCertificateSnapshot,
    ...
    compare_option_string(
        ...
        "stored report resolve_latest_closure_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report run_latest_closure_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report verify_latest_closure_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report downstream_finality_receipt_command_summary",
        mismatches,
    );
    compare_option_string(
        report.verify_closure_certificate_command_summary.as_deref(),
        Some(
            verify_closure_certificate_command_summary(
                snapshot,
                &expected_downstream_closure_certificate_session_dir(paths),
        ...
        "stored report verify_closure_certificate_command_summary",
        mismatches,
    );
    compare_option_string(
        report
    ...
    compare_option_string(
        report.finality_receipt_latest_session_path.as_deref(),
        Some(paths.session_path.display().to_string().as_str()),
        "stored report finality_receipt_latest_session_path",
        mismatches,
    );
    compare_option_string(
        report.finality_receipt_latest_status_path.as_deref(),
        Some(paths.status_path.display().to_string().as_str()),
        "stored report finality_receipt_latest_status_path",
        mismatches,
    );
    compare_option_string(
        report.finality_receipt_latest_report_path.as_deref(),
        Some(paths.report_path.display().to_string().as_str()),
        "stored report finality_receipt_latest_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report latest_closure_certificate_plan_report_path",
        mismatches,
    );
    compare_option_string(
        report.latest_closure_certificate_report_path.as_deref(),
        Some(
            paths
                .latest_closure_certificate_report_path
                .display()
        ...
        "stored report latest_closure_certificate_report_path",
        mismatches,
    );
    compare_option_string(
        report.finality_receipt_report_path.as_deref(),
        Some(
            paths
                .finality_receipt_report_path
                .display()
                .to_string()
                .as_str(),
        ),
        "stored report finality_receipt_report_path",
        mismatches,
    );
    compare_option_string(
        report
        ...
        "stored report nested_closure_certificate_latest_session_dir",
        mismatches,
    );
    compare_option_string(
        report.nested_finality_receipt_session_dir.as_deref(),
        Some(
            paths
                .nested_finality_receipt_session_dir
                .display()
        ...
        "stored report nested_finality_receipt_session_dir",
        mismatches,
    );
    compare_option_string(
...
fn compare_session_against_failure(
    ...
    compare_string(
        ...
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_closure_certificate_latest_session_dir on failure",
        mismatches,
    );
    compare_string(
        &session.nested_finality_receipt_session_dir,
        &paths
            ...
            .display()
            .to_string(),
        "stored session nested_finality_receipt_session_dir on failure",
        mismatches,
    );
...
fn map_closure_certificate_latest_failure_kind(verdict: &str) -> ResolutionFailureKind {
    match verdict {
        "tiny_live_package_closure_certificate_latest_refused_by_missing_latest_chain" => {
            ResolutionFailureKind::MissingLatestChain
        }
        "tiny_live_package_closure_certificate_latest_refused_by_drifted_closure_certificate_contract" => {
            ResolutionFailureKind::DriftedFinalityReceiptContract
        }
        _ => ResolutionFailureKind::InvalidLatestChain,
    }
}

fn latest_closure_certificate_terminal_failure(
    report: &ClosureCertificateLatestReportView,
) -> Option<LivePackageFinalityReceiptLatestResult> {
    match report.result.as_deref() {
        Some("refused_by_missing_latest_chain") => {
            Some(LivePackageFinalityReceiptLatestResult::RefusedByMissingLatestChain)
        }
        Some("refused_by_invalid_latest_chain") => {
            Some(LivePackageFinalityReceiptLatestResult::RefusedByInvalidLatestChain)
        }
        Some("refused_by_drifted_closure_certificate_contract") => {
            Some(LivePackageFinalityReceiptLatestResult::RefusedByDriftedFinalityReceiptContract)
        }
        _ => None,
    }
}

fn closure_certificate_latest_result_from_nested_status(
    result: &str,
) -> Option<LivePackageFinalityReceiptLatestResult> {
    match result {
        "ready_for_manual_execution_when_gate_turns_green" => {
            Some(LivePackageFinalityReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen)
        }
        "refused_now_by_stage3" => Some(LivePackageFinalityReceiptLatestResult::RefusedNowByStage3),
        "refused_now_by_pre_activation_gate" => {
            Some(LivePackageFinalityReceiptLatestResult::RefusedNowByPreActivationGate)
        }
...
fn finality_receipt_run_verdict_for_result(result: &str) -> Option<&'static str> {
    match result {
        ...
        "refused_now_by_stage3" => Some("tiny_live_package_finality_receipt_refused_now_by_stage3"),
        "refused_now_by_pre_activation_gate" => {
            Some("tiny_live_package_finality_receipt_refused_now_by_pre_activation_gate")
        }
        "refused_now_by_invalid_or_drifted_contract" => {
            Some("tiny_live_package_finality_receipt_refused_now_by_invalid_or_drifted_contract")
        }
        _ => None,
    }
}

fn result_from_failure_kind(kind: ResolutionFailureKind) -> LivePackageFinalityReceiptLatestResult {
    match kind {
        ResolutionFailureKind::MissingLatestChain => {
            LivePackageFinalityReceiptLatestResult::RefusedByMissingLatestChain
        }
        ResolutionFailureKind::InvalidLatestChain => {
            LivePackageFinalityReceiptLatestResult::RefusedByInvalidLatestChain
        }
        ResolutionFailureKind::DriftedFinalityReceiptContract => {
            LivePackageFinalityReceiptLatestResult::RefusedByDriftedFinalityReceiptContract
        }
...
fn verdict_for_result(
    result: LivePackageFinalityReceiptLatestResult,
) -> TinyLivePackageFinalityReceiptLatestVerdict {
    match result {
        LivePackageFinalityReceiptLatestResult::RefusedByMissingLatestChain => {
            TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestRefusedByMissingLatestChain
        }
        LivePackageFinalityReceiptLatestResult::RefusedByInvalidLatestChain => {
            TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestRefusedByInvalidLatestChain
        }
        LivePackageFinalityReceiptLatestResult::RefusedByDriftedFinalityReceiptContract => {
            TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestRefusedByDriftedFinalityReceiptContract
        }
        LivePackageFinalityReceiptLatestResult::RefusedNowByStage3 => {
            TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestRefusedNowByStage3
        }
        LivePackageFinalityReceiptLatestResult::RefusedNowByPreActivationGate => {
            TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestRefusedNowByPreActivationGate
        }
        LivePackageFinalityReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestReadyForManualExecutionWhenGateTurnsGreen
        }
    }
}
...
fn operator_next_action_summary(result: LivePackageFinalityReceiptLatestResult) -> String {
    match result {
        LivePackageFinalityReceiptLatestResult::RefusedByMissingLatestChain => {
        ...
        LivePackageFinalityReceiptLatestResult::RefusedByInvalidLatestChain => {
            "the latest immutable chain is incomplete or below the accepted clerestory layer; mint a fresh coherent clerestory chain instead of bypassing the accepted latest-closure-certificate / closure-certificate path".to_string()
        }
        LivePackageFinalityReceiptLatestResult::RefusedByDriftedFinalityReceiptContract => {
            "the latest chain no longer resolves cleanly into the accepted latest-closure-certificate / closure-certificate contract; repair that drift instead of bypassing the bounded handoff path".to_string()
        }
        LivePackageFinalityReceiptLatestResult::RefusedNowByStage3 => {
            "the frozen controller stays refused now by Stage 3; rerun this latest-finality-receipt refresh after current Stage 3 truth turns green".to_string()
        }
        LivePackageFinalityReceiptLatestResult::RefusedNowByPreActivationGate => {
            "the frozen controller stays refused now by the pre-activation gate; rerun this latest-finality-receipt refresh after the current pre-activation gate turns green".to_string()
        }
        LivePackageFinalityReceiptLatestResult::ReadyForManualExecutionWhenGateTurnsGreen => {
            "the latest immutable chain now resolves into the accepted finality-receipt contract and one bounded latest finality-receipt handoff is ready for manual execution when gate truth turns green; this wrapper still did not run or authorize the frozen controller".to_string()
        }
    }
}
...
fn finality_receipt_verify_closure_certificate_command_summary(
    closure_certificate_session_dir: &Path,
) -> String {
    format!(
        "verify immutable filing-certificate session under {} before freezing the final closure certificate",
...
fn refusal_status(
    ...
    latest_closure_certificate_step: Option<PackageFinalityReceiptLatestStepArtifact>,
    finality_receipt_step: Option<PackageFinalityReceiptLatestStepArtifact>,
) -> PackageFinalityReceiptLatestStatus {
    ...
    PackageFinalityReceiptLatestStatus {
        ...
        result,
        reason,
        latest_closure_certificate_plan_verdict: resolved
            .map(|value| value.latest_closure_certificate_plan.verdict.clone()),
        latest_closure_certificate_verdict: latest_closure_certificate_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        finality_receipt_verdict: finality_receipt_step
            .as_ref()
            .map(|value| value.verdict.clone()),
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
...
fn refusal_status_from_resolution_failure(
    ...
    PackageFinalityReceiptLatestStatus {
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        latest_closure_certificate_plan_step: failure
            .latest_closure_certificate_plan_raw
            .as_ref()
            .map(|_| {
                step_artifact(
                    ...
                    failure
                        ...
                        .unwrap_or("<missing>"),
                    failure
                        .latest_closure_certificate_plan_reason
                        .as_deref()
                        .unwrap_or("<missing>"),
                    failure
                        .latest_closure_certificate_plan_generated_at
                        .unwrap_or_else(Utc::now),
                )
            }),
        latest_closure_certificate_step: None,
...
fn verify_invalid_archive_receipt_latest_report_without_session(
    session_dir: &Path,
    verification_mismatches: Vec<String>,
) -> PackageFinalityReceiptLatestReport {
    ...
    PackageFinalityReceiptLatestReport {
        generated_at: Utc::now(),
        mode: "verify_latest_finality_receipt".to_string(),
        verdict: TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestVerifyInvalid,
        reason,
        root: None,
        ...
        session_dir: Some(session_dir.display().to_string()),
        finality_receipt_latest_session_path: Some(
            finality_receipt_latest_paths(session_dir)
                .session_path
                .display()
                .to_string(),
        ),
        finality_receipt_latest_status_path: Some(
            finality_receipt_latest_paths(session_dir)
                .status_path
                .display()
                .to_string(),
        ),
        finality_receipt_latest_report_path: Some(
            finality_receipt_latest_paths(session_dir)
                .report_path
                .display()
                .to_string(),
        ),
        latest_closure_certificate_plan_report_path: None,
        latest_closure_certificate_report_path: None,
        finality_receipt_report_path: None,
        nested_closure_certificate_latest_session_dir: None,
        nested_finality_receipt_session_dir: None,
        ...
        current_pre_activation_gate_verdict: None,
        current_pre_activation_gate_reason: None,
        resolve_latest_closure_certificate_command_summary: None,
        run_latest_closure_certificate_command_summary: None,
        verify_latest_closure_certificate_command_summary: None,
        downstream_finality_receipt_command_summary: None,
        verify_closure_certificate_command_summary: None,
...
fn fallback_closure_certificate_latest_copy() -> ClosureCertificateLatestReportView {
    ClosureCertificateLatestReportView {
        generated_at: Utc::now(),
...
fn fallback_notarization_receipt_session(
    snapshot: &LatestClosureCertificateSnapshot,
    session_dir: &Path,
) -> FinalityReceiptSessionView {
    FinalityReceiptSessionView {
        closure_certificate_session_dir: "<missing>".to_string(),
        ...
        session_dir: session_dir.display().to_string(),
        verify_closure_certificate_command_summary:
            finality_receipt_verify_closure_certificate_command_summary(Path::new("<missing>")),
...
fn fallback_notarization_receipt_status(
    _snapshot: &LatestClosureCertificateSnapshot,
    session_dir: &Path,
) -> FinalityReceiptStatusView {
    FinalityReceiptStatusView {
        session_dir: session_dir.display().to_string(),
        closure_certificate_session_dir: "<missing>".to_string(),
...
fn render_report_lines(report: &PackageFinalityReceiptLatestReport) -> String {
    let mut lines = vec![
    ...
    if let Some(value) = &report.nested_closure_certificate_latest_session_dir {
        lines.push(format!(
            "nested_closure_certificate_latest_session_dir={value}"
        ));
    }
    ...
    if let Some(value) = &report.nested_finality_receipt_session_dir {
        lines.push(format!("nested_finality_receipt_session_dir={value}"));
    }
    if let Some(value) = &report.result {
        lines.push(format!("result={value}"));
    }
    if let Some(value) = &report.downstream_finality_receipt_command_summary {
        lines.push(format!(
            "downstream_finality_receipt_command_summary={value}"
        ));
    }
...
mod tests {
    ...
    struct FakeNotarizationReceiptLatestFixture {
        ...
        archive_receipt_latest_plan_json_path: PathBuf,
        archive_receipt_latest_run_json_path: PathBuf,
        archive_receipt_latest_verify_json_path: PathBuf,
    ...
    impl FakeNotarizationReceiptLatestFixture {
        fn plan_config(&self) -> Config {
            Config {
                mode: Mode::PlanLatestFinalityReceipt,
        ...
        fn render_config(&self) -> Config {
            Config {
                mode: Mode::RenderLatestFinalityReceiptScript,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
        ...
        fn run_config(&self) -> Config {
            Config {
                mode: Mode::RunLatestFinalityReceipt,
                root: Some(self.root.clone()),
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
        ...
        fn verify_config(&self) -> Config {
            Config {
                mode: Mode::VerifyLatestFinalityReceipt,
                root: None,
                session_dir: Some(self.wrapper_session_dir.clone()),
                output_path: None,
                json: true,
    ...
    #[test]
    fn plan_mode_resolves_latest_valid_clerestory_chain_to_exact_expected_nested_latest_archive_receipt_lineage(
    ) {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_plan_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let report = plan_latest_closure_certificate_report(&fixture.plan_config()).unwrap();
        let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
        assert_eq!(
            report.verdict,
            TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestPlanReady
        );
    ...
    fn render_mode_emits_exact_downstream_provenance_certificate_contract_not_second_controller_path(
        ...
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_render_ok",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let report =
            render_latest_closure_certificate_script_report(&fixture.render_config()).unwrap();
        assert_eq!(
            report.verdict,
            TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestScriptRendered
        );
        let script = fs::read_to_string(&fixture.output_script_path).unwrap();
        assert!(script.contains(CLOSURE_CERTIFICATE_LATEST_BIN));
        assert!(script.contains(FINALITY_RECEIPT_BIN));
        assert!(script.contains("--closure-certificate-session-dir"));
        assert!(!script.contains("--activation-ticket-session-dir"));
        assert!(script.contains("--run-live-package-finality-receipt"));
        assert!(script.contains("--verify-live-package-finality-receipt"));
    ...
    fn run_mode_refuses_when_latest_chain_missing_invalid_or_below_clerestory() {
        for (label, verdict, reason, expected) in [
            (
                "archive_receipt_latest_missing",
                "tiny_live_package_closure_certificate_latest_refused_by_missing_latest_chain",
                "no persisted immutable chain exists",
                TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestRefusedByMissingLatestChain,
            ),
            (
                "archive_receipt_latest_invalid",
                "tiny_live_package_closure_certificate_latest_refused_by_invalid_latest_chain",
                "latest immutable chain is incomplete",
                TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestRefusedByInvalidLatestChain,
            ),
            (
                "archive_receipt_latest_below",
                "tiny_live_package_closure_certificate_latest_refused_by_invalid_latest_chain",
            ...
            let mut plan: Value = load_json(&fixture.archive_receipt_latest_plan_json_path).unwrap();
            plan["verdict"] = json!(verdict);
            plan["reason"] = json!(reason);
            if label.ends_with("below") {
                plan["latest_top_step_name"] = json!("choir_receipt");
            }
            persist_json(&fixture.archive_receipt_latest_plan_json_path, &plan).unwrap();

            let report = run_latest_closure_certificate_report(&fixture.run_config()).unwrap();
            assert_eq!(report.verdict, expected);
        }
    }

    #[test]
    fn plan_mode_maps_real_latest_archive_receipt_drift_verdict_to_local_drift_refusal() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_plan_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        plan["reason"] = json!("real accepted latest-closure-certificate drift");
        persist_json(&fixture.archive_receipt_latest_plan_json_path, &plan).unwrap();
        ...
        assert_eq!(
            report.result.as_deref(),
            Some("refused_by_drifted_finality_receipt_contract")
        );
    }

    #[test]
    fn run_mode_takes_terminal_drift_refusal_when_nested_latest_archive_receipt_reports_real_drift()
    {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_run_drift",
            "refused_by_drifted_closure_certificate_contract",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert_eq!(report.finality_receipt_verdict.as_deref(), None);
    }

    #[test]
    fn run_mode_refuses_when_nested_release_capsule_lineage_drifts_from_latest_immutable_chain() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_handoff_bundle_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);

        let mut run: Value = load_json(&fixture.archive_receipt_latest_run_json_path).unwrap();
        run["package_dir"] = json!("/tampered/package");
        persist_json(&fixture.archive_receipt_latest_run_json_path, &run).unwrap();
        ...
        assert!(report
            .reason
            .contains("stored latest-closure-certificate run package_dir"));
    }

    #[test]
    fn verify_mode_fails_when_persisted_latest_archive_receipt_artifacts_are_tampered_or_missing_nested_lineage_truth(
    ) {
        {
            let fixture = fake_archive_receipt_latest_fixture(
                "archive_receipt_latest_verify_tampered_copy",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            run_latest_closure_certificate_report(&fixture.run_config()).unwrap();

            let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
            let mut report: Value = load_json(&paths.finality_receipt_report_path).unwrap();
            ...
            let verify =
                verify_latest_closure_certificate_report(&fixture.verify_config()).unwrap();
            assert_eq!(
                verify.verdict,
                TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestVerifyInvalid
            );
        ...
        {
            let fixture = fake_archive_receipt_latest_fixture(
                "archive_receipt_latest_verify_missing_nested_truth",
                "runnable_when_gate_truth_turns_green",
                "ready_for_manual_go_live_signoff",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
            ...
            let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
            fs::remove_file(nested_closure_certificate_latest_report_path(
                &paths.nested_closure_certificate_latest_session_dir,
            ))
            .unwrap();
            ...
            assert!(verify.verification_mismatches.iter().any(|entry| {
                entry
                    .contains("tiny_live_activation_package_closure_certificate_latest.report.json")
            }));
        }
    }

    #[test]
    fn verify_accepts_real_shaped_native_finality_report_paths_on_happy_path() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_real_shaped_finality_paths_green",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let verify = verify_latest_closure_certificate_report(&fixture.verify_config()).unwrap();
        assert_eq!(
            verify.verdict,
            TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestVerifyOk,
            "{:#?}",
    ...
    #[test]
    fn verify_fails_when_copied_native_finality_receipt_session_path_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_finality_session_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.finality_receipt_report_path).unwrap();
        report["finality_receipt_session_path"] = json!("/tampered/finality-receipt-session.json");
        persist_json(&paths.finality_receipt_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt run finality_receipt_session_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_finality_receipt_status_path_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_finality_status_path_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.finality_receipt_report_path).unwrap();
        report["finality_receipt_status_path"] = json!("/tampered/finality-receipt-status.json");
        persist_json(&paths.finality_receipt_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt run finality_receipt_status_path")
        }));
    }

    #[test]
    fn verify_fails_when_copied_nested_latest_archive_receipt_explicit_statement_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_launch_packet_explicit_statement_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.latest_closure_certificate_report_path).unwrap();
        report["explicit_statement"] =
            json!("tampered nested latest release capsule explicit statement");
        persist_json(&paths.latest_closure_certificate_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored latest-closure-certificate copy explicit_statement")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_decision_packet_result_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_decision_packet_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy decision_packet_result")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_attestation_seal_session_dir_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_attestation_seal_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy attestation_seal_session_dir")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_closure_certificate_registry_entry_session_dir_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_registry_entry_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy registry_entry_session_dir")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_closure_certificate_notarization_receipt_session_dir_drifts()
    {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_notarization_receipt_session_dir_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy notarization_receipt_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_summary_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_chain_fingerprint_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy chain_fingerprint_summary")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_sha256_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_chain_fingerprint_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy chain_fingerprint_sha256")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_notarization_receipt_chain_fingerprint_algorithm_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_chain_fingerprint_algorithm_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy chain_fingerprint_algorithm")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_closure_certificate_filing_certificate_sha256_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_filing_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
        let mut report: Value = load_json(&paths.finality_receipt_report_path).unwrap();
        report["filing_certificate_sha256"] = json!("tampered-filing-certificate-sha256");
        persist_json(&paths.finality_receipt_report_path, &report).unwrap();
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy filing_certificate_sha256")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_closure_certificate_archive_receipt_sha256_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_archive_receipt_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy archive_receipt_sha256")
        }));
    }

    #[test]
    fn verify_fails_when_copied_native_closure_certificate_closure_certificate_sha256_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_closure_certificate_sha256_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy closure_certificate_sha256")
        }));
    }
    ...
    fn verify_fails_when_downstream_decision_packet_confirmation_anchor_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_decision_packet_anchor_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify.verification_mismatches.iter().any(|entry| {
            entry.contains("stored finality-receipt copy decision_packet_session_dir")
        }));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_execute_frozen_result_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_current_authorization_result_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| { entry.contains("stored finality-receipt copy execute_frozen_result") }));
    }

    #[test]
    fn verify_fails_when_copied_nested_finality_receipt_verdict_drifts() {
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_release_capsule_verdict_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
        ...
        assert!(verify
            .verification_mismatches
            .iter()
            .any(|entry| entry.contains("stored finality-receipt copy verdict")));
    }
    ...
    fn verify_fails_when_copied_nested_provenance_certificate_current_live_cutover_controller_summary_drifts(
        ...
        let fixture = fake_archive_receipt_latest_fixture(
            "archive_receipt_latest_verify_current_live_cutover_summary_drift",
            "runnable_when_gate_truth_turns_green",
            "ready_for_manual_go_live_signoff",
        );
        let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn all_modes_preserve_stage3_and_pre_activation_refusal_semantics_and_do_not_imply_activation_authorization_by_themselves(
        ...
        {
            let fixture = fake_archive_receipt_latest_fixture(
                "archive_receipt_latest_stage3",
                "refused_now_by_stage3",
                "refused_now_by_stage3",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);

            let plan = plan_latest_closure_certificate_report(&fixture.plan_config()).unwrap();
            assert_eq!(
                plan.verdict,
                TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestPlanReady
            );
            ...
            let run = run_latest_closure_certificate_report(&fixture.run_config()).unwrap();
            assert_eq!(
                run.verdict,
                TinyLivePackageFinalityReceiptLatestVerdict::TinyLivePackageFinalityReceiptLatestRefusedNowByStage3
            );
        ...
        {
            let fixture = fake_archive_receipt_latest_fixture(
                "archive_receipt_latest_pre_activation",
                "refused_now_by_pre_activation_gate",
                "refused_now_by_pre_activation_gate",
            );
            let _path_guard = PathGuard::install(&fixture.bin_dir);
    ...
    fn fake_archive_receipt_latest_fixture(
        label: &str,
        archive_receipt_latest_run_result: &str,
        ...
        let archive_receipt_latest_plan_json_path =
            temp_dir.join("archive_receipt_latest.plan.json");
        let archive_receipt_latest_run_json_path = temp_dir.join("archive_receipt_latest.run.json");
        let archive_receipt_latest_verify_json_path =
            temp_dir.join("archive_receipt_latest.verify.json");
        ...
        let fixture = FakeNotarizationReceiptLatestFixture {
            ...
            install_root,
            backend_command,
            archive_receipt_latest_plan_json_path,
            archive_receipt_latest_run_json_path,
            archive_receipt_latest_verify_json_path,
        ...
        persist_json(
            &fixture.archive_receipt_latest_plan_json_path,
            &default_archive_receipt_latest_plan_report(&fixture),
        )
        .unwrap();
        persist_json(
            &fixture.archive_receipt_latest_run_json_path,
            &default_archive_receipt_latest_run_report(&fixture, archive_receipt_latest_run_result),
        )
        .unwrap();
        persist_json(
            &fixture.archive_receipt_latest_verify_json_path,
            &default_archive_receipt_latest_verify_report(
                &fixture,
                archive_receipt_latest_run_result,
            ),
        )
        .unwrap();
        persist_json(
        ...
        let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
        write_fake_archive_receipt_latest_command(
            &fixture.bin_dir.join(CLOSURE_CERTIFICATE_LATEST_BIN),
            &fixture.archive_receipt_latest_plan_json_path,
            &fixture.archive_receipt_latest_run_json_path,
            &fixture.archive_receipt_latest_verify_json_path,
            &fixture.root,
        ...
        write_fake_release_capsule_command(
            &fixture.bin_dir.join(FINALITY_RECEIPT_BIN),
            &fixture.handoff_bundle_run_json_path,
            &fixture.handoff_bundle_verify_json_path,
            &fixture.handoff_bundle_session_json_path,
            &fixture.handoff_bundle_status_json_path,
    ...
    fn default_archive_receipt_latest_plan_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
    ) -> Value {
        let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
        json!({
            "generated_at": Utc::now(),
            "mode": "plan_latest_archive_receipt",
            "verdict": CLOSURE_CERTIFICATE_LATEST_PLAN_READY,
            "reason": "latest immutable chain resolves cleanly into accepted latest-closure-certificate contract",
            ...
            "session_dir": paths.nested_closure_certificate_latest_session_dir.display().to_string(),
            ...
            "activation_authorized": false,
            "explicit_statement": "latest archive-receipt wrapper plan"
        })
    }

    fn canonical_archive_receipt_latest_result(result: &str) -> &str {
        match result {
            "runnable_when_gate_truth_turns_green" => {
                "ready_for_manual_execution_when_gate_turns_green"
            }
    ...
    fn default_archive_receipt_latest_run_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let result = canonical_archive_receipt_latest_result(result);
        let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
        let (verdict, gate_verdict, gate_reason) = match result {
            "ready_for_manual_execution_when_gate_turns_green" => (
                "tiny_live_package_closure_certificate_latest_ready_for_manual_execution_when_gate_turns_green",
                Value::Null,
                Value::Null,
            ),
            "refused_now_by_stage3" => (
                "tiny_live_package_closure_certificate_latest_refused_now_by_stage3",
                json!("blocked"),
                json!("stage3 blocked"),
            ),
            "refused_now_by_pre_activation_gate" => (
                "tiny_live_package_closure_certificate_latest_refused_now_by_pre_activation_gate",
                json!("blocked"),
        ...
        json!({
            "generated_at": Utc::now(),
            "mode": "run_latest_archive_receipt",
            "verdict": verdict,
            "reason": format!("latest archive-receipt {result}"),
            "latest_top_step_name": "clerestory_certificate",
            "latest_top_session_dir": fixture.latest_top_session_dir.display().to_string(),
            "historical_execute_frozen_session_dir": fixture.historical_execute_frozen_session_dir.display().to_string(),
            "turn_green_session_dir": fixture.turn_green_session_dir.display().to_string(),
            ...
            "clerestory_certificate_algorithm": "sha256",
            "activation_authorized": false,
            "explicit_statement": "latest archive-receipt wrapper run"
        })
    }

    fn default_archive_receipt_latest_verify_report(
        fixture: &FakeNotarizationReceiptLatestFixture,
        result: &str,
    ) -> Value {
        let mut report = default_archive_receipt_latest_run_report(fixture, result);
        report["mode"] = json!("verify_latest_archive_receipt");
        report["verdict"] = json!(CLOSURE_CERTIFICATE_LATEST_VERIFY_OK);
    ...
    fn default_provenance_certificate_session(
        ...
        let paths = finality_receipt_latest_paths(&fixture.wrapper_session_dir);
        let closure_certificate_session_dir =
            expected_downstream_closure_certificate_session_dir(&paths);
        ...
        let snapshot = LatestClosureCertificateSnapshot {
        ...
        json!({
            "session_version": "1",
            "planned_at": Utc::now(),
            "closure_certificate_session_dir": closure_certificate_session_dir.display().to_string(),
            ...
            "release_capsule_session_dir": closure_certificate_session_dir.display().to_string(),
            ...
            "wrapper_timeout_ms": snapshot.wrapper_timeout_ms,
            "service_status_max_staleness_ms": snapshot.service_status_max_staleness_ms,
            "session_dir": finality_receipt_latest_paths(&fixture.wrapper_session_dir).nested_finality_receipt_session_dir.display().to_string(),
            "verify_closure_certificate_command_summary": finality_receipt_verify_closure_certificate_command_summary(&closure_certificate_session_dir),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
            ...
            "explicit_statement": "the verified closure-certificate session is the primary input here; this immutable finality receipt freezes one final end-state identity over the fully closed chain without enabling production execution"
        })
    }
    ...
    fn default_provenance_certificate_status(
        ...
        json!({
            "status_version": "1",
            "updated_at": Utc::now(),
            "session_dir": paths.nested_finality_receipt_session_dir.display().to_string(),
            "closure_certificate_session_dir": expected_downstream_closure_certificate_session_dir(&paths).display().to_string(),
            "result": result,
            "reason": reason,
            ...
            "current_pre_activation_gate_verdict": gate_verdict,
            "current_pre_activation_gate_reason": gate_reason,
            "closure_certificate_step": Value::Null,
            ...
            "finality_receipt_sha256": "fixture-finality-receipt-sha256",
            "finality_receipt_algorithm": "sha256",
            "explicit_statement": "this finality receipt is archival and read-only; it never executes or enables the frozen controller"
        })
    }
    ...
    fn default_provenance_certificate_run_report(
        ...
        let finality_receipt_verdict = match result {
            "ready_for_manual_execution_when_gate_turns_green" => {
                "tiny_live_package_finality_receipt_ready_for_manual_execution_when_gate_turns_green"
            }
            "refused_now_by_stage3" => "tiny_live_package_finality_receipt_refused_now_by_stage3",
            "refused_now_by_pre_activation_gate" => {
                "tiny_live_package_finality_receipt_refused_now_by_pre_activation_gate"
            }
            _ => "tiny_live_package_finality_receipt_refused_now_by_invalid_or_drifted_contract",
        };
        json!({
            "generated_at": Utc::now(),
            "mode": "run_live_package_finality_receipt",
            "verdict": finality_receipt_verdict,
            ...
            "release_capsule_session_dir": expected_downstream_closure_certificate_session_dir(&paths).display().to_string(),
            ...
            "service_status_max_staleness_ms": 4500,
            "session_dir": paths.nested_finality_receipt_session_dir.display().to_string(),
            "finality_receipt_session_path": nested_finality_receipt_session_path(&paths.nested_finality_receipt_session_dir).display().to_string(),
            "finality_receipt_status_path": nested_finality_receipt_status_path(&paths.nested_finality_receipt_session_dir).display().to_string(),
            "archived_closure_certificate_report_path": paths.nested_finality_receipt_session_dir.join("tiny_live_activation_package_finality_receipt.closure_certificate.report.json").display().to_string(),
            "result": result,
            ...
            "closure_certificate_step": Value::Null,
            "verify_closure_certificate_command_summary": finality_receipt_verify_closure_certificate_command_summary(&expected_downstream_closure_certificate_session_dir(&paths)),
            "reviewed_frozen_live_cutover_controller_command_summary": fixture.frozen_controller_command,
    ...
    fn default_provenance_certificate_verify_report(
        ...
        report["mode"] = json!("verify_live_package_finality_receipt");
        report["verdict"] = json!(FINALITY_RECEIPT_VERIFY_OK);
        report["reason"] = json!("verify ok");
    ...
    fn write_fake_archive_receipt_latest_command(
        path: &Path,
        plan_json_path: &Path,
        run_json_path: &Path,
        ...
        let script = format!(
            "#!/bin/sh\nset -eu\ncase \" $* \" in\n  *\" --session-dir {} \"*) : ;;\n  *) echo 'unexpected latest-closure-certificate session-dir' >&2; exit 1;;\nesac\nmode=''\ncase \" $* \" in\n  *\" --plan-latest-closure-certificate \"*) mode='plan';;\n  *\" --run-latest-closure-certificate \"*) mode='run';;\n  *\" --verify-latest-closure-certificate \"*) mode='verify';;\n  *) echo 'unexpected latest-closure-certificate mode' >&2; exit 1;;\nesac\ncase \"$mode\" in\n  plan|run)\n    case \" $* \" in\n      *\" --root {} \"*) : ;;\n      *) echo 'unexpected root' >&2; exit 1;;\n    esac\n    ;;\nesac\ncase \"$mode\" in\n  plan) cat '{}';;\n  run) mkdir -p '{}'; cp '{}' '{}/tiny_live_activation_package_closure_certificate_latest.report.json'; cat '{}';;\n  verify) cat '{}';;\n  *) exit 1;;\nesac\n",
            session_dir.display(),
            root.display(),
