...
pub struct LivePackageKeystoneReceiptContractView {
    pub keystone_receipt_session_dir: PathBuf,
    pub capstone_certificate_session_dir: PathBuf,
    ...
    pub verify_capstone_certificate_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageKeystoneReceiptCornerstoneCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageKeystoneReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageKeystoneReceiptArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub capstone_certificate_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredKeystoneReceiptSession {
    ...
    #[serde(alias = "verify_pinnacle_receipt_command_summary")]
...
#[derive(Debug, Deserialize)]
struct StoredKeystoneReceiptStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredKeystoneReceiptReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct KeystoneReceiptVerifyReportView {
...
pub fn load_live_package_keystone_receipt_contract_for_cornerstone_certificate(
    keystone_receipt_session_dir: &Path,
) -> Result<LivePackageKeystoneReceiptContractView> {
    let paths = keystone_receipt_artifact_paths(keystone_receipt_session_dir);
    let session: StoredKeystoneReceiptSession = load_json(&paths.session_path)?;
    let status: StoredKeystoneReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackageKeystoneReceiptContractView {
        keystone_receipt_session_dir: keystone_receipt_session_dir.to_path_buf(),
        capstone_certificate_session_dir: PathBuf::from(session.capstone_certificate_session_dir),
        ...
        verify_capstone_certificate_command_summary: session
...
pub fn verify_live_package_keystone_receipt_for_cornerstone_certificate(
    keystone_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageKeystoneReceiptCornerstoneCertificateStep> {
    let contract = load_live_package_keystone_receipt_contract_for_cornerstone_certificate(
    ...
    let raw_report = run_json_command(
        ...
        &keystone_receipt_verify_args(
            &contract.capstone_certificate_session_dir,
            &trusted_decision_packet_session_dir,
            keystone_receipt_session_dir,
        ),
    )?;
    let report: KeystoneReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing keystone-receipt verify report")?;
    let reported_capstone_certificate_session_dir =
        PathBuf::from(&report.capstone_certificate_session_dir);
    if reported_capstone_certificate_session_dir != contract.capstone_certificate_session_dir {
        bail!(
            "keystone-receipt verify report capstone_certificate_session_dir {:?} does not match trusted {:?}",
            reported_capstone_certificate_session_dir.display(),
            contract.capstone_certificate_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "keystone-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "keystone-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageKeystoneReceiptCornerstoneCertificateStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageKeystoneReceiptContractView {
            keystone_receipt_session_dir: keystone_receipt_session_dir.to_path_buf(),
            capstone_certificate_session_dir: reported_capstone_certificate_session_dir,
            ...
            verify_capstone_certificate_command_summary: required_string_field(
                report.verify_capstone_certificate_command_summary,
                "verify_capstone_certificate_command_summary",
            )?,
...
pub fn keystone_receipt_verify_args(
...
pub fn keystone_receipt_artifact_paths(
    session_dir: &Path,
) -> PackageKeystoneReceiptArtifactPaths {
    PackageKeystoneReceiptArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_verify(
    ...
    let paths = keystone_receipt_artifact_paths(keystone_receipt_session_dir);
    let archived_report: StoredKeystoneReceiptReportView =
        load_json(&paths.capstone_certificate_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored keystone-receipt contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested capstone-certificate report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("keystone-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_keystone_receipt_files() {
        let dir = temp_dir("keystone_receipt_contract");
        fs::write(
            dir.join("tiny_live_activation_package_keystone_receipt.session.json"),
            serde_json::to_vec_pretty(&stored_keystone_receipt_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_keystone_receipt.status.json"),
            serde_json::to_vec_pretty(&stored_keystone_receipt_status_value(
        ...
        let contract =
            load_live_package_keystone_receipt_contract_for_cornerstone_certificate(&dir).unwrap();
        assert_eq!(
    ...
    #[test]
    fn keystone_receipt_verify_args_are_exact_and_bounded() {
        let args = keystone_receipt_verify_args(
            Path::new("/tmp/capstone-certificate-session"),
            Path::new("/tmp/decision-packet-session"),
            Path::new("/tmp/keystone-receipt-session"),
        );
        assert_eq!(
            args,
            vec![
                "--capstone-certificate-session-dir",
                "/tmp/capstone-certificate-session",
                "--confirm-decision-packet-session-dir",
                "/tmp/decision-packet-session",
                "--session-dir",
                "/tmp/keystone-receipt-session",
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("keystone_receipt_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_keystone_receipt.capstone_certificate.report.json"),
    ...
    fn stored_keystone_receipt_session_value(decision_packet_session_dir: &str) -> Value {
        let mut object = Map::new();
        object.insert(
            "capstone_certificate_session_dir".to_string(),
            json!("/tmp/capstone-certificate-session"),
        );
        object.insert(
        ...
        object.insert(
            ...
            json!("verify capstone certificate"),
        );
        object.insert(
    ...
    fn stored_keystone_receipt_status_value(result: &str, include_nested_results: bool) -> Value {
        let mut object = Map::new();
