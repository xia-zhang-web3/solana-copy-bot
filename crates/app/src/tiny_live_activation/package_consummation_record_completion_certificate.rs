...
pub struct LivePackageConsummationRecordContractView {
    ...
    pub verify_consummation_record_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageConsummationRecordCompletionCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageConsummationRecordContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageConsummationRecordArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub finality_receipt_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredConsummationRecordSession {
    ...
    #[serde(alias = "verify_finality_receipt_command_summary")]
...
#[derive(Debug, Deserialize)]
struct StoredConsummationRecordStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredFinalityReceiptReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct ConsummationRecordVerifyReportView {
...
pub fn load_live_package_consummation_record_contract_for_completion_certificate(
    consummation_record_session_dir: &Path,
) -> Result<LivePackageConsummationRecordContractView> {
    let paths = consummation_record_artifact_paths(consummation_record_session_dir);
    let session: StoredConsummationRecordSession = load_json(&paths.session_path)?;
    let status: StoredConsummationRecordStatus = load_json(&paths.status_path)?;
    Ok(LivePackageConsummationRecordContractView {
        consummation_record_session_dir: consummation_record_session_dir.to_path_buf(),
        finality_receipt_session_dir: PathBuf::from(session.finality_receipt_session_dir),
        ...
        verify_consummation_record_command_summary: session
...
pub fn verify_live_package_consummation_record_for_completion_certificate(
    consummation_record_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageConsummationRecordCompletionCertificateStep> {
    let contract = load_live_package_consummation_record_contract_for_completion_certificate(
    ...
    let raw_report = run_json_command(
        ...
        &consummation_record_verify_args(
            &contract.finality_receipt_session_dir,
            &trusted_decision_packet_session_dir,
            consummation_record_session_dir,
        ),
    )?;
    let report: ConsummationRecordVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing consummation-record verify report")?;
    let reported_finality_receipt_session_dir = PathBuf::from(&report.finality_receipt_session_dir);
    if reported_finality_receipt_session_dir != contract.finality_receipt_session_dir {
        bail!(
            "consummation-record verify report finality_receipt_session_dir {:?} does not match trusted {:?}",
            reported_finality_receipt_session_dir.display(),
            contract.finality_receipt_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "consummation-record verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "consummation-record verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(
        VerifiedLivePackageConsummationRecordCompletionCertificateStep {
            report_json: raw_report,
            verdict: report.verdict.clone(),
            reason: report.reason.clone(),
            generated_at: report.generated_at,
            contract: LivePackageConsummationRecordContractView {
                consummation_record_session_dir: consummation_record_session_dir.to_path_buf(),
                finality_receipt_session_dir: contract.finality_receipt_session_dir,
                ...
                verify_consummation_record_command_summary: required_string_field(
                    report.verify_consummation_record_command_summary,
                    "verify_consummation_record_command_summary",
                )?,
...
pub fn consummation_record_verify_args(
...
pub fn consummation_record_artifact_paths(
    session_dir: &Path,
) -> PackageConsummationRecordArtifactPaths {
    PackageConsummationRecordArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_verify(
    ...
    let paths = consummation_record_artifact_paths(consummation_record_session_dir);
    let archived_report: StoredFinalityReceiptReportView =
        load_json(&paths.finality_receipt_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored consummation-record contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested finality-receipt report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("consummation-record verify report is missing {field}"))
}
...
mod tests {
    ...
    fn load_contract_reads_stored_consummation_record_files() {
        ...
        fs::write(
            dir.join("tiny_live_activation_package_consummation_record.session.json"),
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_consummation_record.status.json"),
        ...
        let contract =
            load_live_package_consummation_record_contract_for_completion_certificate(&dir)
                .unwrap();
        assert_eq!(
    ...
    #[test]
    fn consummation_record_verify_args_are_exact_and_bounded() {
        let args = consummation_record_verify_args(
        ...
        assert_eq!(
            ...
            vec![
                ...
                "--verify-live-package-consummation-record",
                "--json"
            ]
        );
    }
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_consummation_record.finality_receipt.report.json"),
