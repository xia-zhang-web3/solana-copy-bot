...
pub struct LivePackageCulminationReceiptContractView {
    pub culmination_receipt_session_dir: PathBuf,
    pub completion_certificate_session_dir: PathBuf,
    ...
    pub verify_completion_certificate_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageCulminationReceiptSummitCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageCulminationReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageCulminationReceiptArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub completion_certificate_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredCulminationReceiptSession {
    ...
    #[serde(alias = "verify_consummation_record_command_summary")]
...
#[derive(Debug, Deserialize)]
struct StoredCulminationReceiptStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredCompletionCertificateReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct CulminationReceiptVerifyReportView {
...
pub fn load_live_package_culmination_receipt_contract_for_summit_certificate(
    culmination_receipt_session_dir: &Path,
) -> Result<LivePackageCulminationReceiptContractView> {
    let paths = culmination_receipt_artifact_paths(culmination_receipt_session_dir);
    let session: StoredCulminationReceiptSession = load_json(&paths.session_path)?;
    let status: StoredCulminationReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackageCulminationReceiptContractView {
        culmination_receipt_session_dir: culmination_receipt_session_dir.to_path_buf(),
        completion_certificate_session_dir: PathBuf::from(session.completion_certificate_session_dir),
        ...
        verify_completion_certificate_command_summary: session
...
pub fn verify_live_package_culmination_receipt_for_summit_certificate(
    culmination_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageCulminationReceiptSummitCertificateStep> {
    let contract = load_live_package_culmination_receipt_contract_for_summit_certificate(
    ...
    let raw_report = run_json_command(
        ...
        &culmination_receipt_verify_args(
            &contract.completion_certificate_session_dir,
            &trusted_decision_packet_session_dir,
            culmination_receipt_session_dir,
        ),
    )?;
    let report: CulminationReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing culmination-receipt verify report")?;
    let reported_completion_certificate_session_dir =
        PathBuf::from(&report.completion_certificate_session_dir);
    if reported_completion_certificate_session_dir != contract.completion_certificate_session_dir {
        bail!(
            "culmination-receipt verify report completion_certificate_session_dir {:?} does not match trusted {:?}",
            reported_completion_certificate_session_dir.display(),
            contract.completion_certificate_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "culmination-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "culmination-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(
        VerifiedLivePackageCulminationReceiptSummitCertificateStep {
            report_json: raw_report,
            verdict: report.verdict.clone(),
            reason: report.reason.clone(),
            generated_at: report.generated_at,
            contract: LivePackageCulminationReceiptContractView {
                culmination_receipt_session_dir: culmination_receipt_session_dir.to_path_buf(),
                completion_certificate_session_dir: contract.completion_certificate_session_dir,
                ...
                verify_completion_certificate_command_summary: required_string_field(
                    report.verify_completion_certificate_command_summary,
                    "verify_completion_certificate_command_summary",
                )?,
...
pub fn culmination_receipt_verify_args(
...
pub fn culmination_receipt_artifact_paths(
    session_dir: &Path,
) -> PackageCulminationReceiptArtifactPaths {
    PackageCulminationReceiptArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_verify(
    ...
    let paths = culmination_receipt_artifact_paths(culmination_receipt_session_dir);
    let archived_report: StoredCompletionCertificateReportView =
        load_json(&paths.completion_certificate_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored culmination-receipt contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested completion-certificate report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("culmination-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_culmination_receipt_files() {
        let dir = temp_dir("culmination_receipt_contract");
        fs::write(
            dir.join("tiny_live_activation_package_culmination_receipt.session.json"),
            serde_json::to_vec_pretty(&stored_culmination_receipt_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_culmination_receipt.status.json"),
            serde_json::to_vec_pretty(&stored_culmination_receipt_status_value(
        ...
        let contract =
            load_live_package_culmination_receipt_contract_for_summit_certificate(&dir)
                .unwrap();
        assert_eq!(
    ...
    #[test]
    fn culmination_receipt_verify_args_are_exact_and_bounded() {
        let args = culmination_receipt_verify_args(
            Path::new("/tmp/completion-certificate-session"),
            Path::new("/tmp/decision-packet-session"),
            Path::new("/tmp/culmination-receipt-session"),
        );
        assert_eq!(
            args,
            vec![
                "--completion-certificate-session-dir",
                "/tmp/completion-certificate-session",
                "--confirm-decision-packet-session-dir",
                "/tmp/decision-packet-session",
                "--session-dir",
                "/tmp/culmination-receipt-session",
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("culmination_receipt_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_culmination_receipt.completion_certificate.report.json"),
    ...
    fn stored_culmination_receipt_session_value(decision_packet_session_dir: &str) -> Value {
        let mut object = Map::new();
        object.insert(
            "completion_certificate_session_dir".to_string(),
            json!("/tmp/completion-certificate-session"),
        );
        object.insert(
        ...
        object.insert(
            "verify_consummation_record_command_summary".to_string(),
            json!("verify completion certificate"),
        );
        object.insert(
    ...
    fn stored_culmination_receipt_status_value(
        result: &str,
