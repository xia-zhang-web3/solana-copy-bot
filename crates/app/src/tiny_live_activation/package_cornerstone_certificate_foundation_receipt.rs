...
pub struct LivePackageCornerstoneCertificateContractView {
    ...
    pub verify_keystone_receipt_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageCornerstoneCertificateFoundationReceiptStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageCornerstoneCertificateContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageCornerstoneCertificateArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub keystone_receipt_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredCornerstoneCertificateSession {
...
#[derive(Debug, Deserialize)]
struct StoredCornerstoneCertificateStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredCornerstoneCertificateReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct CornerstoneCertificateVerifyReportView {
...
pub fn load_live_package_cornerstone_certificate_contract_for_foundation_receipt(
    cornerstone_certificate_session_dir: &Path,
) -> Result<LivePackageCornerstoneCertificateContractView> {
    let paths = cornerstone_certificate_artifact_paths(cornerstone_certificate_session_dir);
    let session: StoredCornerstoneCertificateSession = load_json(&paths.session_path)?;
    let status: StoredCornerstoneCertificateStatus = load_json(&paths.status_path)?;
    Ok(LivePackageCornerstoneCertificateContractView {
...
pub fn verify_live_package_cornerstone_certificate_for_foundation_receipt(
    cornerstone_certificate_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageCornerstoneCertificateFoundationReceiptStep> {
    let contract = load_live_package_cornerstone_certificate_contract_for_foundation_receipt(
        cornerstone_certificate_session_dir,
    )?;
    let trusted_decision_packet_session_dir =
        load_confirmed_decision_packet_session_dir_for_cornerstone_verify(
            cornerstone_certificate_session_dir,
    ...
    let raw_report = run_json_command(
        ...
        &cornerstone_certificate_verify_args(
            &contract.keystone_receipt_session_dir,
    ...
    if reported_keystone_receipt_session_dir != contract.keystone_receipt_session_dir {
        bail!(
            ...
            reported_keystone_receipt_session_dir.display(),
            contract.keystone_receipt_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "cornerstone-certificate verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "cornerstone-certificate verify report registry_entry_session_dir {:?} does not match trusted {:?}",
...
pub fn cornerstone_certificate_artifact_paths(
    session_dir: &Path,
) -> PackageCornerstoneCertificateArtifactPaths {
    PackageCornerstoneCertificateArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_cornerstone_verify(
    cornerstone_certificate_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
    contract: &LivePackageCornerstoneCertificateContractView,
) -> Result<PathBuf> {
    let paths = cornerstone_certificate_artifact_paths(cornerstone_certificate_session_dir);
    let archived_report: StoredCornerstoneCertificateReportView =
        load_json(&paths.keystone_receipt_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored cornerstone-certificate contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested keystone-receipt report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("cornerstone-certificate verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_cornerstone_certificate_files() {
        let dir = temp_dir("cornerstone_certificate_contract");
        fs::write(
            dir.join("tiny_live_activation_package_cornerstone_certificate.session.json"),
            serde_json::to_vec_pretty(&stored_cornerstone_certificate_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_cornerstone_certificate.status.json"),
            serde_json::to_vec_pretty(&stored_cornerstone_certificate_status_value(
    ...
    #[test]
    fn cornerstone_certificate_verify_args_are_exact_and_bounded() {
        let args = cornerstone_certificate_verify_args(
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("cornerstone_certificate_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_cornerstone_certificate.keystone_receipt.report.json"),
    ...
    fn stored_cornerstone_certificate_session_value(decision_packet_session_dir: &str) -> Value {
