...
pub struct LivePackageClosureCertificateContractView {
    pub closure_certificate_session_dir: PathBuf,
    pub archive_receipt_session_dir: PathBuf,
    ...
    pub verify_archive_receipt_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageClosureCertificateFinalityReceiptStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageClosureCertificateContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageClosureCertificateArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub archive_receipt_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredClosureCertificateSession {
...
#[derive(Debug, Deserialize)]
struct StoredClosureCertificateStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredArchiveReceiptReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct ClosureCertificateVerifyReportView {
...
pub fn load_live_package_closure_certificate_contract_for_finality_receipt(
    closure_certificate_session_dir: &Path,
) -> Result<LivePackageClosureCertificateContractView> {
    let paths = closure_certificate_artifact_paths(closure_certificate_session_dir);
    let session: StoredClosureCertificateSession = load_json(&paths.session_path)?;
    let status: StoredClosureCertificateStatus = load_json(&paths.status_path)?;
    Ok(LivePackageClosureCertificateContractView {
        closure_certificate_session_dir: closure_certificate_session_dir.to_path_buf(),
        archive_receipt_session_dir: PathBuf::from(session.archive_receipt_session_dir),
        ...
        verify_archive_receipt_command_summary: session.verify_archive_receipt_command_summary,
...
pub fn verify_live_package_closure_certificate_for_finality_receipt(
    closure_certificate_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageClosureCertificateFinalityReceiptStep> {
    let contract = load_live_package_closure_certificate_contract_for_finality_receipt(
    ...
    let raw_report = run_json_command(
        ...
        &closure_certificate_verify_args(
            &contract.archive_receipt_session_dir,
            &trusted_decision_packet_session_dir,
            closure_certificate_session_dir,
        ),
    )?;
    let report: ClosureCertificateVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing closure-certificate verify report")?;
    let reported_archive_receipt_session_dir = PathBuf::from(&report.archive_receipt_session_dir);
    if reported_archive_receipt_session_dir != contract.archive_receipt_session_dir {
        bail!(
            "closure-certificate verify report archive_receipt_session_dir {:?} does not match trusted {:?}",
            reported_archive_receipt_session_dir.display(),
            contract.archive_receipt_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "closure-certificate verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "closure-certificate verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageClosureCertificateFinalityReceiptStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageClosureCertificateContractView {
            closure_certificate_session_dir: closure_certificate_session_dir.to_path_buf(),
            archive_receipt_session_dir: contract.archive_receipt_session_dir,
            ...
            verify_archive_receipt_command_summary: required_string_field(
                report.verify_archive_receipt_command_summary,
...
pub fn closure_certificate_verify_args(
...
pub fn closure_certificate_artifact_paths(
    session_dir: &Path,
) -> PackageClosureCertificateArtifactPaths {
    PackageClosureCertificateArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_verify(
    ...
    let paths = closure_certificate_artifact_paths(closure_certificate_session_dir);
    let archived_report: StoredArchiveReceiptReportView =
        load_json(&paths.archive_receipt_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored closure-certificate contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested archive-receipt report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("closure-certificate verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_closure_certificate_files() {
        let dir = temp_dir("closure_certificate_contract");
        fs::write(
            dir.join("tiny_live_activation_package_closure_certificate.session.json"),
            serde_json::to_vec_pretty(&stored_closure_certificate_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_closure_certificate.status.json"),
            serde_json::to_vec_pretty(&stored_closure_certificate_status_value(
        ...
        let contract =
            load_live_package_closure_certificate_contract_for_finality_receipt(&dir).unwrap();
        assert_eq!(
    ...
    #[test]
    fn closure_certificate_verify_args_are_exact_and_bounded() {
        let args = closure_certificate_verify_args(
            Path::new("/tmp/archive-receipt-session"),
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/tmp/archive-receipt-session",
                "--confirm-decision-packet-session-dir",
                "/tmp/decision-packet-session",
                "--session-dir",
                "/tmp/closure-certificate-session",
                "--verify-live-package-closure-certificate",
                "--json"
            ]
        );
    }
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("closure_certificate_confirmed_decision_packet");
    ...
    fn stored_closure_certificate_session_value(decision_packet_session_dir: &str) -> Value {
        let mut object = Map::new();
        object.insert(
            "archive_receipt_session_dir".to_string(),
            json!("/tmp/archive-receipt-session"),
        );
        object.insert(
    ...
    fn stored_closure_certificate_status_value(
        result: &str,
