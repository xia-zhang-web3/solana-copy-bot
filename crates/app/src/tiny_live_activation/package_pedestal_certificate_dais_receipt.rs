...
pub struct LivePackagePedestalCertificateContractView {
    ...
    pub verify_pedestal_certificate_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackagePedestalCertificateDaisReceiptStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackagePedestalCertificateContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackagePedestalCertificateArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub plinth_receipt_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredPedestalCertificateSession {
...
#[derive(Debug, Deserialize)]
struct StoredPedestalCertificateStatus {
...
#[derive(Debug, Deserialize)]
struct StoredPlinthReceiptReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct PedestalCertificateVerifyReportView {
...
pub fn load_live_package_pedestal_certificate_contract_for_dais_receipt(
    pedestal_certificate_session_dir: &Path,
) -> Result<LivePackagePedestalCertificateContractView> {
    let paths = pedestal_certificate_artifact_paths(pedestal_certificate_session_dir);
    let session: StoredPedestalCertificateSession = load_json(&paths.session_path)?;
    let status: StoredPedestalCertificateStatus = load_json(&paths.status_path)?;
    Ok(LivePackagePedestalCertificateContractView {
        pedestal_certificate_session_dir: pedestal_certificate_session_dir.to_path_buf(),
...
pub fn verify_live_package_pedestal_certificate_for_dais_receipt(
    pedestal_certificate_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackagePedestalCertificateDaisReceiptStep> {
    let contract = load_live_package_pedestal_certificate_contract_for_dais_receipt(
    ...
    let report: PedestalCertificateVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing pedestal-certificate verify report")?;
    ...
    if reported_plinth_receipt_session_dir != contract.plinth_receipt_session_dir {
        bail!(
            "pedestal-certificate verify report plinth_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    {
        bail!(
            "pedestal-certificate verify report substructure_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_bedrock_certificate_session_dir != contract.bedrock_certificate_session_dir {
        bail!(
            "pedestal-certificate verify report bedrock_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_basal_receipt_session_dir != contract.basal_receipt_session_dir {
        bail!(
            "pedestal-certificate verify report basal_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_foundation_receipt_session_dir != contract.foundation_receipt_session_dir {
        bail!(
            "pedestal-certificate verify report foundation_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "pedestal-certificate verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "pedestal-certificate verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackagePedestalCertificateDaisReceiptStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackagePedestalCertificateContractView {
            pedestal_certificate_session_dir: pedestal_certificate_session_dir.to_path_buf(),
            ...
            verify_pedestal_certificate_command_summary: required_string_field(
                report.verify_pedestal_certificate_command_summary,
                "verify_pedestal_certificate_command_summary",
            )?,
...
pub fn pedestal_certificate_verify_args(
    pedestal_certificate_session_dir: &Path,
...
fn load_confirmed_decision_packet_session_dir_for_pedestal_verify(
    pedestal_certificate_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
    contract: &LivePackagePedestalCertificateContractView,
) -> Result<PathBuf> {
    let paths = pedestal_certificate_artifact_paths(pedestal_certificate_session_dir);
    let archived_report: StoredPlinthReceiptReportView =
        load_json(&paths.plinth_receipt_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored pedestal-certificate contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested plinth-receipt report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("pedestal-certificate verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_pedestal_certificate_files() {
        let dir = temp_dir("pedestal_certificate_contract");
        fs::write(
            dir.join("tiny_live_activation_package_pedestal_certificate.session.json"),
            serde_json::to_vec_pretty(&stored_pedestal_certificate_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_pedestal_certificate.status.json"),
            serde_json::to_vec_pretty(&stored_pedestal_certificate_status_value(
        ...
        let contract =
            load_live_package_pedestal_certificate_contract_for_dais_receipt(&dir).unwrap();
        assert_eq!(contract.pedestal_certificate_session_dir, dir);
        assert_eq!(
            contract.plinth_receipt_session_dir,
    ...
    #[test]
    fn pedestal_certificate_verify_args_are_exact_and_bounded() {
        let args = pedestal_certificate_verify_args(
            Path::new("/tmp/pedestal-certificate-session"),
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/tmp/decision-packet-session",
                "--session-dir",
                "/tmp/pedestal-certificate-session",
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("pedestal_certificate_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_pedestal_certificate.plinth_receipt.report.json"),
        ...
        let contract =
            load_live_package_pedestal_certificate_contract_for_dais_receipt(&dir).unwrap();
        let error = load_confirmed_decision_packet_session_dir_for_pedestal_verify(
            &dir,
            &trusted_decision_packet_session_dir,
            &contract,
        )
    ...
    fn stored_pedestal_certificate_session_value(decision_packet_session_dir: &str) -> Value {
