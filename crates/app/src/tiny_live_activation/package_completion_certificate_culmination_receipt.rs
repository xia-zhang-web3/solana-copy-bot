...
pub struct LivePackageCompletionCertificateContractView {
    ...
    pub consummation_record_session_dir: PathBuf,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageCompletionCertificateCulminationReceiptStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageCompletionCertificateContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageCompletionCertificateArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub consummation_record_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredCompletionCertificateSession {
...
#[derive(Debug, Deserialize)]
struct StoredCompletionCertificateStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredConsummationRecordReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct CompletionCertificateVerifyReportView {
...
pub fn load_live_package_completion_certificate_contract_for_culmination_receipt(
    completion_certificate_session_dir: &Path,
) -> Result<LivePackageCompletionCertificateContractView> {
    let paths = completion_certificate_artifact_paths(completion_certificate_session_dir);
    let session: StoredCompletionCertificateSession = load_json(&paths.session_path)?;
    let status: StoredCompletionCertificateStatus = load_json(&paths.status_path)?;
    Ok(LivePackageCompletionCertificateContractView {
        completion_certificate_session_dir: completion_certificate_session_dir.to_path_buf(),
        consummation_record_session_dir: PathBuf::from(session.consummation_record_session_dir),
...
pub fn verify_live_package_completion_certificate_for_culmination_receipt(
    completion_certificate_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageCompletionCertificateCulminationReceiptStep> {
    let contract = load_live_package_completion_certificate_contract_for_culmination_receipt(
    ...
    let raw_report = run_json_command(
        ...
        &completion_certificate_verify_args(
            &contract.consummation_record_session_dir,
            &trusted_decision_packet_session_dir,
            completion_certificate_session_dir,
        ),
    )?;
    let report: CompletionCertificateVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing completion-certificate verify report")?;
    let reported_consummation_record_session_dir =
        PathBuf::from(&report.consummation_record_session_dir);
    if reported_consummation_record_session_dir != contract.consummation_record_session_dir {
        bail!(
            "completion-certificate verify report consummation_record_session_dir {:?} does not match trusted {:?}",
            reported_consummation_record_session_dir.display(),
            contract.consummation_record_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "completion-certificate verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "completion-certificate verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(
        VerifiedLivePackageCompletionCertificateCulminationReceiptStep {
...
pub fn completion_certificate_verify_args(
...
pub fn completion_certificate_artifact_paths(
    session_dir: &Path,
) -> PackageCompletionCertificateArtifactPaths {
    PackageCompletionCertificateArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_verify(
    ...
    let paths = completion_certificate_artifact_paths(completion_certificate_session_dir);
    let archived_report: StoredConsummationRecordReportView =
        load_json(&paths.consummation_record_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored completion-certificate contract {:?}",
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("completion-certificate verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_consummation_record_files() {
        let dir = temp_dir("consummation_record_contract");
        fs::write(
            dir.join("tiny_live_activation_package_completion_certificate.session.json"),
            serde_json::to_vec_pretty(&stored_consummation_record_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_completion_certificate.status.json"),
            serde_json::to_vec_pretty(&stored_consummation_record_status_value(
        ...
        let contract =
            load_live_package_completion_certificate_contract_for_culmination_receipt(&dir)
                .unwrap();
        assert_eq!(
    ...
    #[test]
    fn completion_certificate_verify_args_are_exact_and_bounded() {
        let args = completion_certificate_verify_args(
            Path::new("/tmp/consummation-record-session"),
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/tmp/consummation-record-session",
                "--confirm-decision-packet-session-dir",
                "/tmp/decision-packet-session",
                "--session-dir",
                "/tmp/completion-certificate-session",
                "--verify-live-package-completion-certificate",
                "--json"
            ]
        );
    }
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("consummation_record_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_completion_certificate.consummation_record.report.json"),
    ...
    fn stored_consummation_record_session_value(decision_packet_session_dir: &str) -> Value {
        let mut object = Map::new();
        object.insert(
            "consummation_record_session_dir".to_string(),
            json!("/tmp/consummation-record-session"),
        );
        object.insert(
        ...
        object.insert(
            ...
            json!("verify consummation record"),
        );
        object.insert(
    ...
    fn stored_consummation_record_status_value(
        result: &str,
