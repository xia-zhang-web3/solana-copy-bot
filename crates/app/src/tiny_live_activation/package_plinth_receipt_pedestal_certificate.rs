...
pub struct LivePackagePlinthReceiptContractView {
    ...
    pub verify_plinth_receipt_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackagePlinthReceiptPedestalCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackagePlinthReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackagePlinthReceiptArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub substructure_certificate_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredPlinthReceiptSession {
...
#[derive(Debug, Deserialize)]
struct StoredPlinthReceiptStatus {
...
#[derive(Debug, Deserialize)]
struct PlinthReceiptVerifyReportView {
    ...
    current_pre_activation_gate_verdict: Option<String>,
    current_pre_activation_gate_reason: Option<String>,
    #[serde(alias = "verify_substructure_certificate_command_summary")]
...
pub fn load_live_package_plinth_receipt_contract_for_pedestal_certificate(
    plinth_receipt_session_dir: &Path,
) -> Result<LivePackagePlinthReceiptContractView> {
    let paths = plinth_receipt_artifact_paths(plinth_receipt_session_dir);
    let session: StoredPlinthReceiptSession = load_json(&paths.session_path)?;
    let status: StoredPlinthReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackagePlinthReceiptContractView {
        plinth_receipt_session_dir: plinth_receipt_session_dir.to_path_buf(),
        ...
        verify_plinth_receipt_command_summary: session.verify_plinth_receipt_command_summary,
...
pub fn verify_live_package_plinth_receipt_for_pedestal_certificate(
    plinth_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackagePlinthReceiptPedestalCertificateStep> {
    let contract = load_live_package_plinth_receipt_contract_for_pedestal_certificate(
        plinth_receipt_session_dir,
    )?;
    let trusted_decision_packet_session_dir =
        load_confirmed_decision_packet_session_dir_for_plinth_verify(
            plinth_receipt_session_dir,
    ...
    let report: PlinthReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing plinth-receipt verify report")?;
    ...
    {
        bail!(
            "plinth-receipt verify report substructure_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_bedrock_certificate_session_dir != contract.bedrock_certificate_session_dir {
        bail!(
            "plinth-receipt verify report bedrock_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_basal_receipt_session_dir != contract.basal_receipt_session_dir {
        bail!(
            "plinth-receipt verify report basal_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_foundation_receipt_session_dir != contract.foundation_receipt_session_dir {
        bail!(
            "plinth-receipt verify report foundation_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "plinth-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "plinth-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackagePlinthReceiptPedestalCertificateStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackagePlinthReceiptContractView {
            plinth_receipt_session_dir: plinth_receipt_session_dir.to_path_buf(),
            ...
            verify_plinth_receipt_command_summary: required_string_field(
                report.verify_plinth_receipt_command_summary,
...
pub fn plinth_receipt_verify_args(
    plinth_receipt_session_dir: &Path,
...
pub fn plinth_receipt_artifact_paths(session_dir: &Path) -> PackagePlinthReceiptArtifactPaths {
    PackagePlinthReceiptArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_plinth_verify(
    plinth_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
    contract: &LivePackagePlinthReceiptContractView,
) -> Result<PathBuf> {
    ...
    let archived_report: StoredSubstructureCertificateReportView =
        load_json(&paths.substructure_certificate_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored plinth-receipt contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested substructure-certificate report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("plinth-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_plinth_receipt_files() {
        let dir = temp_dir("plinth_receipt_contract");
        fs::write(
            dir.join("tiny_live_activation_package_plinth_receipt.session.json"),
            serde_json::to_vec_pretty(&stored_plinth_receipt_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_plinth_receipt.status.json"),
            serde_json::to_vec_pretty(&stored_plinth_receipt_status_value(
        ...
        let contract =
            load_live_package_plinth_receipt_contract_for_pedestal_certificate(&dir).unwrap();
        assert_eq!(contract.plinth_receipt_session_dir, dir);
        assert_eq!(
            contract.substructure_certificate_session_dir,
    ...
    #[test]
    fn plinth_receipt_verify_args_are_exact_and_bounded() {
        let args = plinth_receipt_verify_args(
            Path::new("/tmp/plinth-receipt-session"),
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("plinth_receipt_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_plinth_receipt.substructure_certificate.report.json"),
        ...
        let contract =
            load_live_package_plinth_receipt_contract_for_pedestal_certificate(&dir).unwrap();
        let error = load_confirmed_decision_packet_session_dir_for_plinth_verify(
            &dir,
            &trusted_decision_packet_session_dir,
            &contract,
        )
    ...
    fn stored_plinth_receipt_session_value(decision_packet_session_dir: &str) -> Value {
        ...
        object.insert(
            ...
            json!("verify substructure certificate"),
        );
        object.insert(
    ...
    fn stored_plinth_receipt_status_value(result: &str, include_nested_results: bool) -> Value {
        let mut object = Map::new();
        object.insert(
            "substructure_certificate_session_dir".to_string(),
            json!("/tmp/substructure-certificate-session"),
