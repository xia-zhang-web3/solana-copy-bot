...
pub struct LivePackagePulpitReceiptContractView {
    ...
    pub verify_lectern_certificate_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackagePulpitReceiptChancelCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackagePulpitReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackagePulpitReceiptArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub lectern_certificate_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredPulpitReceiptSession {
...
#[derive(Debug, Deserialize)]
struct StoredPulpitReceiptStatus {
...
#[derive(Debug, Deserialize)]
struct StoredLecternCertificateReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct PulpitReceiptVerifyReportView {
...
pub fn load_live_package_pulpit_receipt_contract_for_chancel_certificate(
    pulpit_receipt_session_dir: &Path,
) -> Result<LivePackagePulpitReceiptContractView> {
    let paths = pulpit_receipt_artifact_paths(pulpit_receipt_session_dir);
    let session: StoredPulpitReceiptSession = load_json(&paths.session_path)?;
    let status: StoredPulpitReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackagePulpitReceiptContractView {
        pulpit_receipt_session_dir: pulpit_receipt_session_dir.to_path_buf(),
        ...
        verify_lectern_certificate_command_summary: session
...
pub fn verify_live_package_pulpit_receipt_for_chancel_certificate(
    pulpit_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackagePulpitReceiptChancelCertificateStep> {
    let contract = load_live_package_pulpit_receipt_contract_for_chancel_certificate(
        pulpit_receipt_session_dir,
    )?;
    let trusted_decision_packet_session_dir =
        load_confirmed_decision_packet_session_dir_for_pulpit_verify(
            pulpit_receipt_session_dir,
    ...
    let report: PulpitReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing pulpit-receipt verify report")?;
    ...
    if reported_lectern_certificate_session_dir != contract.lectern_certificate_session_dir {
        bail!(
            "pulpit-receipt verify report lectern_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_rostrum_certificate_session_dir != contract.rostrum_certificate_session_dir {
        bail!(
            "pulpit-receipt verify report rostrum_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_pedestal_certificate_session_dir != contract.pedestal_certificate_session_dir {
        bail!(
            "pulpit-receipt verify report pedestal_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_plinth_receipt_session_dir != contract.plinth_receipt_session_dir {
        bail!(
            "pulpit-receipt verify report plinth_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    {
        bail!(
            "pulpit-receipt verify report substructure_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_bedrock_certificate_session_dir != contract.bedrock_certificate_session_dir {
        bail!(
            "pulpit-receipt verify report bedrock_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_basal_receipt_session_dir != contract.basal_receipt_session_dir {
        bail!(
            "pulpit-receipt verify report basal_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_foundation_receipt_session_dir != contract.foundation_receipt_session_dir {
        bail!(
            "pulpit-receipt verify report foundation_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "pulpit-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "pulpit-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackagePulpitReceiptChancelCertificateStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackagePulpitReceiptContractView {
            pulpit_receipt_session_dir: pulpit_receipt_session_dir.to_path_buf(),
            ...
            verify_lectern_certificate_command_summary: required_string_field(
                report.verify_lectern_certificate_command_summary,
                "verify_lectern_certificate_command_summary",
            )?,
...
pub fn pulpit_receipt_verify_args(
    pulpit_receipt_session_dir: &Path,
...
pub fn pulpit_receipt_artifact_paths(session_dir: &Path) -> PackagePulpitReceiptArtifactPaths {
    PackagePulpitReceiptArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_pulpit_verify(
    pulpit_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
    contract: &LivePackagePulpitReceiptContractView,
) -> Result<PathBuf> {
    let paths = pulpit_receipt_artifact_paths(pulpit_receipt_session_dir);
    let archived_report: StoredLecternCertificateReportView =
        load_json(&paths.lectern_certificate_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored pulpit-receipt contract {:?}",
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("pulpit-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    fn load_contract_reads_stored_pulpit_receipt_files() {
        ...
        fs::write(
            dir.join("tiny_live_activation_package_pulpit_receipt.session.json"),
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_pulpit_receipt.status.json"),
        ...
        let contract =
            load_live_package_pulpit_receipt_contract_for_chancel_certificate(&dir).unwrap();
        assert_eq!(contract.pulpit_receipt_session_dir, dir);
        assert_eq!(
            contract.lectern_certificate_session_dir,
    ...
    #[test]
    fn pulpit_receipt_verify_args_are_exact_and_bounded() {
        let args = pulpit_receipt_verify_args(
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_pulpit_receipt.lectern_certificate.report.json"),
        ...
        let contract =
            load_live_package_pulpit_receipt_contract_for_chancel_certificate(&dir).unwrap();
        let error = load_confirmed_decision_packet_session_dir_for_pulpit_verify(
            &dir,
            &trusted_decision_packet_session_dir,
            &contract,
        )
