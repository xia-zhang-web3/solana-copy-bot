...
pub struct LivePackageCapstoneCertificateContractView {
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageCapstoneCertificateKeystoneReceiptStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageCapstoneCertificateContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageCapstoneCertificateArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub pinnacle_receipt_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredCapstoneCertificateSession {
...
#[derive(Debug, Deserialize)]
struct StoredCapstoneCertificateStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredPinnacleReceiptReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct CapstoneCertificateVerifyReportView {
...
pub fn load_live_package_capstone_certificate_contract_for_keystone_receipt(
    capstone_certificate_session_dir: &Path,
) -> Result<LivePackageCapstoneCertificateContractView> {
    let paths = capstone_certificate_artifact_paths(capstone_certificate_session_dir);
    let session: StoredCapstoneCertificateSession = load_json(&paths.session_path)?;
    let status: StoredCapstoneCertificateStatus = load_json(&paths.status_path)?;
    Ok(LivePackageCapstoneCertificateContractView {
        capstone_certificate_session_dir: capstone_certificate_session_dir.to_path_buf(),
        pinnacle_receipt_session_dir: PathBuf::from(session.pinnacle_receipt_session_dir),
...
pub fn verify_live_package_capstone_certificate_for_keystone_receipt(
    capstone_certificate_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageCapstoneCertificateKeystoneReceiptStep> {
    let contract = load_live_package_capstone_certificate_contract_for_keystone_receipt(
    ...
    let raw_report = run_json_command(
        ...
        &capstone_certificate_verify_args(
            &contract.pinnacle_receipt_session_dir,
    ...
    if reported_pinnacle_receipt_session_dir != contract.pinnacle_receipt_session_dir {
        bail!(
            ...
            reported_pinnacle_receipt_session_dir.display(),
            contract.pinnacle_receipt_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "capstone-certificate verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "capstone-certificate verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageCapstoneCertificateKeystoneReceiptStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageCapstoneCertificateContractView {
            capstone_certificate_session_dir: capstone_certificate_session_dir.to_path_buf(),
            pinnacle_receipt_session_dir: reported_pinnacle_receipt_session_dir,
...
pub fn capstone_certificate_verify_args(
...
pub fn capstone_certificate_artifact_paths(
    session_dir: &Path,
) -> PackageCapstoneCertificateArtifactPaths {
    PackageCapstoneCertificateArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_verify(
    ...
    let paths = capstone_certificate_artifact_paths(capstone_certificate_session_dir);
    let archived_report: StoredPinnacleReceiptReportView =
        load_json(&paths.pinnacle_receipt_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored capstone-certificate contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested pinnacle-receipt report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("capstone-certificate verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_capstone_certificate_files() {
        let dir = temp_dir("capstone_certificate_contract");
        fs::write(
            dir.join("tiny_live_activation_package_capstone_certificate.session.json"),
            serde_json::to_vec_pretty(&stored_capstone_certificate_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_capstone_certificate.status.json"),
            serde_json::to_vec_pretty(&stored_capstone_certificate_status_value(
        ...
        let contract =
            load_live_package_capstone_certificate_contract_for_keystone_receipt(&dir).unwrap();
        assert_eq!(
    ...
    #[test]
    fn capstone_certificate_verify_args_are_exact_and_bounded() {
        let args = capstone_certificate_verify_args(
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("capstone_certificate_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_capstone_certificate.pinnacle_receipt.report.json"),
    ...
    fn stored_capstone_certificate_session_value(decision_packet_session_dir: &str) -> Value {
        let mut object = Map::new();
        object.insert(
            "pinnacle_receipt_session_dir".to_string(),
            json!("/tmp/pinnacle-receipt-session"),
        );
        object.insert(
