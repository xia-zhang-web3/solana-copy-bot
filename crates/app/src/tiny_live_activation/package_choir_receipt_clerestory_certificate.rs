...
pub struct LivePackageChoirReceiptContractView {
    pub choir_receipt_session_dir: PathBuf,
    pub transept_certificate_session_dir: PathBuf,
    pub lectern_certificate_session_dir: PathBuf,
    pub sanctuary_certificate_session_dir: PathBuf,
    ...
    pub verify_choir_receipt_command_summary: String,
    ...
    pub apse_receipt_summary: String,
    pub apse_receipt_sha256: String,
    pub apse_receipt_algorithm: String,
    pub sanctuary_certificate_summary: String,
    pub sanctuary_certificate_sha256: String,
    pub sanctuary_certificate_algorithm: String,
    pub nave_receipt_summary: String,
    pub nave_receipt_sha256: String,
    pub nave_receipt_algorithm: String,
    pub transept_certificate_summary: String,
    pub transept_certificate_sha256: String,
    pub transept_certificate_algorithm: String,
    pub choir_receipt_summary: String,
    pub choir_receipt_sha256: String,
    pub choir_receipt_algorithm: String,
    pub explicit_statement: String,
}

#[derive(Debug, Clone)]
pub struct VerifiedLivePackageChoirReceiptClerestoryCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageChoirReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageChoirReceiptArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub transept_certificate_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredChoirReceiptSession {
    ...
    #[serde(alias = "verify_transept_certificate_command_summary")]
...
#[derive(Debug, Deserialize)]
struct StoredChoirReceiptStatus {
    transept_certificate_session_dir: Option<String>,
    lectern_certificate_session_dir: Option<String>,
    sanctuary_certificate_session_dir: Option<String>,
...
#[derive(Debug, Deserialize)]
struct StoredChoirReceiptReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct ChoirReceiptVerifyReportView {
...
pub fn load_live_package_choir_receipt_contract_for_clerestory_certificate(
    choir_receipt_session_dir: &Path,
) -> Result<LivePackageChoirReceiptContractView> {
    let paths = choir_receipt_artifact_paths(choir_receipt_session_dir);
    let session: StoredChoirReceiptSession = load_json(&paths.session_path)?;
    let status: StoredChoirReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackageChoirReceiptContractView {
        choir_receipt_session_dir: choir_receipt_session_dir.to_path_buf(),
        transept_certificate_session_dir: PathBuf::from(
            status
                .transept_certificate_session_dir
                .unwrap_or(session.transept_certificate_session_dir),
        ),
        ...
        sanctuary_certificate_session_dir: PathBuf::from(
            status
                .sanctuary_certificate_session_dir
                .unwrap_or(session.sanctuary_certificate_session_dir),
        ),
        ...
        verify_choir_receipt_command_summary: session.verify_choir_receipt_command_summary,
        ...
        apse_receipt_summary: if status.apse_receipt_summary.is_empty() {
            session.apse_receipt_summary
        } else {
            status.apse_receipt_summary
        },
        apse_receipt_sha256: if status.apse_receipt_sha256.is_empty() {
            session.apse_receipt_sha256
        } else {
            status.apse_receipt_sha256
        },
        apse_receipt_algorithm: if status.apse_receipt_algorithm.is_empty() {
            session.apse_receipt_algorithm
        } else {
            status.apse_receipt_algorithm
        },
        sanctuary_certificate_summary: if status.sanctuary_certificate_summary.is_empty() {
            session.sanctuary_certificate_summary
        } else {
            status.sanctuary_certificate_summary
        },
        sanctuary_certificate_sha256: if status.sanctuary_certificate_sha256.is_empty() {
            session.sanctuary_certificate_sha256
        } else {
            status.sanctuary_certificate_sha256
        },
        sanctuary_certificate_algorithm: if status.sanctuary_certificate_algorithm.is_empty() {
            session.sanctuary_certificate_algorithm
        } else {
            status.sanctuary_certificate_algorithm
        },
        nave_receipt_summary: if status.nave_receipt_summary.is_empty() {
            session.nave_receipt_summary
        } else {
            status.nave_receipt_summary
        },
        nave_receipt_sha256: if status.nave_receipt_sha256.is_empty() {
            session.nave_receipt_sha256
        } else {
            status.nave_receipt_sha256
        },
        nave_receipt_algorithm: if status.nave_receipt_algorithm.is_empty() {
            session.nave_receipt_algorithm
        } else {
            status.nave_receipt_algorithm
        },
        transept_certificate_summary: if status.transept_certificate_summary.is_empty() {
            session.transept_certificate_summary
        } else {
            status.transept_certificate_summary
        },
        transept_certificate_sha256: if status.transept_certificate_sha256.is_empty() {
            session.transept_certificate_sha256
        } else {
            status.transept_certificate_sha256
        },
        transept_certificate_algorithm: if status.transept_certificate_algorithm.is_empty() {
            session.transept_certificate_algorithm
        } else {
            status.transept_certificate_algorithm
        },
        choir_receipt_summary: if status.choir_receipt_summary.is_empty() {
            session.choir_receipt_summary
        } else {
            status.choir_receipt_summary
        },
        choir_receipt_sha256: if status.choir_receipt_sha256.is_empty() {
            session.choir_receipt_sha256
        } else {
            status.choir_receipt_sha256
        },
        choir_receipt_algorithm: if status.choir_receipt_algorithm.is_empty() {
            session.choir_receipt_algorithm
        } else {
            status.choir_receipt_algorithm
        },
...
pub fn verify_live_package_choir_receipt_for_clerestory_certificate(
    choir_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageChoirReceiptClerestoryCertificateStep> {
    let contract = load_live_package_choir_receipt_contract_for_clerestory_certificate(
        choir_receipt_session_dir,
    )?;
    let trusted_decision_packet_session_dir =
        load_confirmed_decision_packet_session_dir_for_choir_verify(
            choir_receipt_session_dir,
    ...
    let raw_report = run_json_command(
        ...
        &choir_receipt_verify_args(
            &contract.transept_certificate_session_dir,
            choir_receipt_session_dir,
            &trusted_decision_packet_session_dir,
        ),
    )?;
    let report: ChoirReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing choir-receipt verify report")?;
    let reported_transept_certificate_session_dir = PathBuf::from(required_string_field(
        report.transept_certificate_session_dir,
        "transept_certificate_session_dir",
    )?);
    if reported_transept_certificate_session_dir != contract.transept_certificate_session_dir {
        bail!(
            "choir-receipt verify report transept_certificate_session_dir {:?} does not match trusted {:?}",
            reported_transept_certificate_session_dir.display(),
            contract.transept_certificate_session_dir.display()
        );
    }
    ...
    let reported_sanctuary_certificate_session_dir = PathBuf::from(required_string_field(
    ...
    if reported_lectern_certificate_session_dir != contract.lectern_certificate_session_dir {
        bail!(
            "choir-receipt verify report lectern_certificate_session_dir {:?} does not match trusted {:?}",
            reported_lectern_certificate_session_dir.display(),
            contract.lectern_certificate_session_dir.display()
        );
    }
    if reported_sanctuary_certificate_session_dir != contract.sanctuary_certificate_session_dir {
        bail!(
            "choir-receipt verify report sanctuary_certificate_session_dir {:?} does not match trusted {:?}",
            reported_sanctuary_certificate_session_dir.display(),
            contract.sanctuary_certificate_session_dir.display()
        );
    }
    if reported_pedestal_certificate_session_dir != contract.pedestal_certificate_session_dir {
        bail!(
            "choir-receipt verify report pedestal_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_plinth_receipt_session_dir != contract.plinth_receipt_session_dir {
        bail!(
            "choir-receipt verify report plinth_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    {
        bail!(
            "choir-receipt verify report substructure_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_bedrock_certificate_session_dir != contract.bedrock_certificate_session_dir {
        bail!(
            "choir-receipt verify report bedrock_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_basal_receipt_session_dir != contract.basal_receipt_session_dir {
        bail!(
            "choir-receipt verify report basal_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_foundation_receipt_session_dir != contract.foundation_receipt_session_dir {
        bail!(
            "choir-receipt verify report foundation_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "choir-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "choir-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageChoirReceiptClerestoryCertificateStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageChoirReceiptContractView {
            choir_receipt_session_dir: choir_receipt_session_dir.to_path_buf(),
            transept_certificate_session_dir: reported_transept_certificate_session_dir,
            lectern_certificate_session_dir: reported_lectern_certificate_session_dir,
            sanctuary_certificate_session_dir: reported_sanctuary_certificate_session_dir,
            ...
            verify_choir_receipt_command_summary: required_string_field(
                report.verify_choir_receipt_command_summary,
            ...
            apse_receipt_summary: required_string_field(
                report.apse_receipt_summary,
                "apse_receipt_summary",
            )?,
            apse_receipt_sha256: required_string_field(
                report.apse_receipt_sha256,
                "apse_receipt_sha256",
            )?,
            apse_receipt_algorithm: required_string_field(
                report.apse_receipt_algorithm,
                "apse_receipt_algorithm",
            )?,
            sanctuary_certificate_summary: required_string_field(
                report.sanctuary_certificate_summary,
                "sanctuary_certificate_summary",
            )?,
            sanctuary_certificate_sha256: required_string_field(
                report.sanctuary_certificate_sha256,
                "sanctuary_certificate_sha256",
            )?,
            sanctuary_certificate_algorithm: required_string_field(
                report.sanctuary_certificate_algorithm,
                "sanctuary_certificate_algorithm",
            )?,
            nave_receipt_summary: required_string_field(
                report.nave_receipt_summary,
                "nave_receipt_summary",
            )?,
            nave_receipt_sha256: required_string_field(
                report.nave_receipt_sha256,
                "nave_receipt_sha256",
            )?,
            nave_receipt_algorithm: required_string_field(
                report.nave_receipt_algorithm,
                "nave_receipt_algorithm",
            )?,
            transept_certificate_summary: required_string_field(
                report.transept_certificate_summary,
                "transept_certificate_summary",
            )?,
            transept_certificate_sha256: required_string_field(
                report.transept_certificate_sha256,
                "transept_certificate_sha256",
            )?,
            transept_certificate_algorithm: required_string_field(
                report.transept_certificate_algorithm,
                "transept_certificate_algorithm",
            )?,
            choir_receipt_summary: required_string_field(
                report.choir_receipt_summary,
                "choir_receipt_summary",
            )?,
            choir_receipt_sha256: required_string_field(
                report.choir_receipt_sha256,
                "choir_receipt_sha256",
            )?,
            choir_receipt_algorithm: required_string_field(
                report.choir_receipt_algorithm,
...
pub fn choir_receipt_verify_args(
...
pub fn choir_receipt_artifact_paths(session_dir: &Path) -> PackageChoirReceiptArtifactPaths {
    PackageChoirReceiptArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_choir_verify(
    choir_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
    contract: &LivePackageChoirReceiptContractView,
) -> Result<PathBuf> {
    let paths = choir_receipt_artifact_paths(choir_receipt_session_dir);
    let archived_report: StoredChoirReceiptReportView =
        load_json(&paths.transept_certificate_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored choir-receipt contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested choir-receipt report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("choir-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_choir_receipt_files() {
        let dir = temp_dir("choir_receipt_contract");
        fs::write(
            dir.join("tiny_live_activation_package_choir_receipt.session.json"),
            serde_json::to_vec_pretty(&stored_nave_receipt_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_choir_receipt.status.json"),
            serde_json::to_vec_pretty(&stored_nave_receipt_status_value(
        ...
        let contract =
            load_live_package_choir_receipt_contract_for_clerestory_certificate(&dir).unwrap();
        assert_eq!(contract.choir_receipt_session_dir, dir);
        assert_eq!(
            contract.transept_certificate_session_dir,
            PathBuf::from("/tmp/transept-certificate-session")
        );
        assert_eq!(
        ...
        assert_eq!(
            contract.sanctuary_certificate_session_dir,
            PathBuf::from("/tmp/sanctuary-certificate-session")
        );
        assert_eq!(
        ...
        assert_eq!(contract.chancel_certificate_sha256, "herald-seal-sha256");
        assert_eq!(contract.apse_receipt_sha256, "herald-certificate-sha256");
        assert_eq!(
            contract.sanctuary_certificate_summary,
            "sanctuary certificate"
        );
        assert_eq!(contract.sanctuary_certificate_sha256, "banner-sha256");
        assert_eq!(contract.sanctuary_certificate_algorithm, "sha256");
        assert_eq!(contract.nave_receipt_sha256, "pennant-sha256");
        assert_eq!(
            contract.transept_certificate_summary,
            "transept certificate"
        );
        assert_eq!(
            contract.transept_certificate_sha256,
            "transept-certificate-sha256"
        );
        assert_eq!(contract.transept_certificate_algorithm, "sha256");
        assert_eq!(contract.choir_receipt_summary, "choir receipt");
        assert_eq!(contract.choir_receipt_sha256, "guidon-sha256");
        assert_eq!(contract.choir_receipt_algorithm, "sha256");
    }

    #[test]
    fn choir_receipt_verify_args_are_exact_and_bounded() {
        let args = choir_receipt_verify_args(
            Path::new("/tmp/transept-certificate-session"),
            Path::new("/tmp/choir-receipt-session"),
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/tmp/transept-certificate-session",
                "--confirm-decision-packet-session-dir",
                "/tmp/decision-packet-session",
                "--session-dir",
                "/tmp/choir-receipt-session",
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("choir_receipt_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_choir_receipt.transept_certificate.report.json"),
        ...
        let contract =
            load_live_package_choir_receipt_contract_for_clerestory_certificate(&dir).unwrap();
        let error = load_confirmed_decision_packet_session_dir_for_choir_verify(
            &dir,
            &trusted_decision_packet_session_dir,
            &contract,
        )
    ...
    fn stored_nave_receipt_session_value(decision_packet_session_dir: &str) -> Value {
        let mut object = Map::new();
        object.insert(
            "transept_certificate_session_dir".to_string(),
            json!("/tmp/transept-certificate-session"),
        );
        object.insert(
        ...
        object.insert(
            ...
            json!("/tmp/sanctuary-certificate-session"),
        );
        object.insert(
        ...
        object.insert("apse_receipt_summary".to_string(), json!("apse receipt"));
        object.insert(
        ...
        object.insert("nave_receipt_summary".to_string(), json!("nave receipt"));
        ...
        object.insert("choir_receipt_summary".to_string(), json!("choir receipt"));
    ...
    fn stored_nave_receipt_status_value(result: &str, include_nested_results: bool) -> Value {
        let mut object = Map::new();
        object.insert(
            "lectern_certificate_session_dir".to_string(),
            json!("/tmp/lectern-certificate-session"),
