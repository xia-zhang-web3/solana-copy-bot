...
pub struct LivePackageBasalReceiptContractView {
    ...
    pub verify_basal_receipt_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageBasalReceiptSubstructureCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageBasalReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageBasalReceiptArtifactPaths {
...
struct StoredBasalReceiptSession {
...
#[derive(Debug, Deserialize)]
struct StoredBasalReceiptStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredBasalReceiptReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct BasalReceiptVerifyReportView {
...
pub fn load_live_package_basal_receipt_contract_for_substructure_certificate(
    basal_receipt_session_dir: &Path,
) -> Result<LivePackageBasalReceiptContractView> {
    let paths = basal_receipt_artifact_paths(basal_receipt_session_dir);
    let session: StoredBasalReceiptSession = load_json(&paths.session_path)?;
    let status: StoredBasalReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackageBasalReceiptContractView {
        basal_receipt_session_dir: basal_receipt_session_dir.to_path_buf(),
        ...
        verify_basal_receipt_command_summary: session.verify_basal_receipt_command_summary,
...
pub fn verify_live_package_basal_receipt_for_substructure_certificate(
    basal_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageBasalReceiptSubstructureCertificateStep> {
    let contract = load_live_package_basal_receipt_contract_for_substructure_certificate(
        basal_receipt_session_dir,
    )?;
    let trusted_decision_packet_session_dir =
        load_confirmed_decision_packet_session_dir_for_basal_verify(
            basal_receipt_session_dir,
    ...
    let raw_report = run_json_command(
        BASAL_RECEIPT_BIN,
        &basal_receipt_verify_args(
            basal_receipt_session_dir,
            &contract.bedrock_certificate_session_dir,
            &trusted_decision_packet_session_dir,
        ),
    )?;
    let report: BasalReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing basal-receipt verify report")?;
    ...
    if reported_bedrock_certificate_session_dir != contract.bedrock_certificate_session_dir {
        bail!(
            "basal-receipt verify report bedrock_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_foundation_receipt_session_dir != contract.foundation_receipt_session_dir {
        bail!(
            "basal-receipt verify report foundation_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "basal-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "basal-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageBasalReceiptSubstructureCertificateStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageBasalReceiptContractView {
            basal_receipt_session_dir: basal_receipt_session_dir.to_path_buf(),
            ...
            verify_basal_receipt_command_summary: required_string_field(
                report.verify_basal_receipt_command_summary,
...
pub fn basal_receipt_verify_args(
    basal_receipt_session_dir: &Path,
...
pub fn basal_receipt_artifact_paths(session_dir: &Path) -> PackageBasalReceiptArtifactPaths {
    PackageBasalReceiptArtifactPaths {
        ...
        bedrock_certificate_report_path: session_dir
            .join("tiny_live_activation_package_basal_receipt.bedrock_certificate.report.json"),
    }
}

fn load_confirmed_decision_packet_session_dir_for_basal_verify(
    basal_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
    contract: &LivePackageBasalReceiptContractView,
) -> Result<PathBuf> {
    let paths = basal_receipt_artifact_paths(basal_receipt_session_dir);
    let archived_report: StoredBasalReceiptReportView =
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored basal-receipt contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested bedrock-certificate report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("basal-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_basal_receipt_files() {
        let dir = temp_dir("basal_receipt_contract");
        fs::write(
            dir.join("tiny_live_activation_package_basal_receipt.session.json"),
            serde_json::to_vec_pretty(&stored_basal_receipt_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_basal_receipt.status.json"),
            serde_json::to_vec_pretty(&stored_basal_receipt_status_value(
        ...
        let contract =
            load_live_package_basal_receipt_contract_for_substructure_certificate(&dir).unwrap();
        assert_eq!(contract.basal_receipt_session_dir, dir);
        assert_eq!(
    ...
    #[test]
    fn basal_receipt_verify_args_are_exact_and_bounded() {
        let args = basal_receipt_verify_args(
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("basal_receipt_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_basal_receipt.bedrock_certificate.report.json"),
        ...
        let contract =
            load_live_package_basal_receipt_contract_for_substructure_certificate(&dir).unwrap();
        let error = load_confirmed_decision_packet_session_dir_for_basal_verify(
            &dir,
            &trusted_decision_packet_session_dir,
            &contract,
        )
    ...
    fn stored_basal_receipt_session_value(decision_packet_session_dir: &str) -> Value {
        ...
        object.insert(
            ...
            json!("verify bedrock certificate"),
        );
        object.insert(
    ...
    fn stored_basal_receipt_status_value(result: &str, include_nested_results: bool) -> Value {
        let mut object = Map::new();
