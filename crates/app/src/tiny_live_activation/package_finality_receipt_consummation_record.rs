...
pub struct LivePackageFinalityReceiptContractView {
    pub finality_receipt_session_dir: PathBuf,
    pub closure_certificate_session_dir: PathBuf,
    ...
    pub verify_finality_receipt_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageFinalityReceiptConsummationRecordStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageFinalityReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageFinalityReceiptArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub closure_certificate_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredFinalityReceiptSession {
    ...
    #[serde(alias = "verify_closure_certificate_command_summary")]
...
#[derive(Debug, Deserialize)]
struct StoredFinalityReceiptStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredClosureCertificateReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct FinalityReceiptVerifyReportView {
...
pub fn load_live_package_finality_receipt_contract_for_consummation_record(
    finality_receipt_session_dir: &Path,
) -> Result<LivePackageFinalityReceiptContractView> {
    let paths = finality_receipt_artifact_paths(finality_receipt_session_dir);
    let session: StoredFinalityReceiptSession = load_json(&paths.session_path)?;
    let status: StoredFinalityReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackageFinalityReceiptContractView {
        finality_receipt_session_dir: finality_receipt_session_dir.to_path_buf(),
        closure_certificate_session_dir: PathBuf::from(session.closure_certificate_session_dir),
        ...
        verify_finality_receipt_command_summary: session.verify_finality_receipt_command_summary,
...
pub fn verify_live_package_finality_receipt_for_consummation_record(
    finality_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageFinalityReceiptConsummationRecordStep> {
    let contract = load_live_package_finality_receipt_contract_for_consummation_record(
    ...
    let raw_report = run_json_command(
        ...
        &finality_receipt_verify_args(
            &contract.closure_certificate_session_dir,
            &trusted_decision_packet_session_dir,
            finality_receipt_session_dir,
        ),
    )?;
    let report: FinalityReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing finality-receipt verify report")?;
    let reported_closure_certificate_session_dir =
        PathBuf::from(&report.closure_certificate_session_dir);
    if reported_closure_certificate_session_dir != contract.closure_certificate_session_dir {
        bail!(
            "finality-receipt verify report closure_certificate_session_dir {:?} does not match trusted {:?}",
            reported_closure_certificate_session_dir.display(),
            contract.closure_certificate_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "finality-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "finality-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageFinalityReceiptConsummationRecordStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageFinalityReceiptContractView {
            finality_receipt_session_dir: finality_receipt_session_dir.to_path_buf(),
            closure_certificate_session_dir: contract.closure_certificate_session_dir,
            ...
            verify_finality_receipt_command_summary: required_string_field(
                report.verify_finality_receipt_command_summary,
...
pub fn finality_receipt_verify_args(
...
pub fn finality_receipt_artifact_paths(session_dir: &Path) -> PackageFinalityReceiptArtifactPaths {
    PackageFinalityReceiptArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_verify(
    ...
    let paths = finality_receipt_artifact_paths(finality_receipt_session_dir);
    let archived_report: StoredClosureCertificateReportView =
        load_json(&paths.closure_certificate_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored finality-receipt contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested closure-certificate report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("finality-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_finality_receipt_files() {
        let dir = temp_dir("finality_receipt_contract");
        fs::write(
            dir.join("tiny_live_activation_package_finality_receipt.session.json"),
            serde_json::to_vec_pretty(&stored_finality_receipt_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_finality_receipt.status.json"),
            serde_json::to_vec_pretty(&stored_finality_receipt_status_value(
        ...
        let contract =
            load_live_package_finality_receipt_contract_for_consummation_record(&dir).unwrap();
        assert_eq!(
    ...
    #[test]
    fn finality_receipt_verify_args_are_exact_and_bounded() {
        let args = finality_receipt_verify_args(
            Path::new("/tmp/closure-certificate-session"),
            Path::new("/tmp/decision-packet-session"),
            Path::new("/tmp/finality-receipt-session"),
        );
        assert_eq!(
            args,
            vec![
                "--closure-certificate-session-dir",
                "/tmp/closure-certificate-session",
                "--confirm-decision-packet-session-dir",
                "/tmp/decision-packet-session",
                "--session-dir",
                "/tmp/finality-receipt-session",
                "--verify-live-package-finality-receipt",
                "--json"
            ]
        );
    }
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("finality_receipt_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_finality_receipt.closure_certificate.report.json"),
    ...
    fn stored_finality_receipt_session_value(decision_packet_session_dir: &str) -> Value {
        let mut object = Map::new();
        object.insert(
            "closure_certificate_session_dir".to_string(),
            json!("/tmp/closure-certificate-session"),
        );
        object.insert(
    ...
    fn stored_finality_receipt_status_value(result: &str, include_nested_results: bool) -> Value {
        let mut object = Map::new();
