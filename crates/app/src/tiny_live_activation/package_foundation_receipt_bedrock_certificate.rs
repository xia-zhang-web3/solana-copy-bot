...
pub struct LivePackageFoundationReceiptContractView {
    pub foundation_receipt_session_dir: PathBuf,
    pub cornerstone_certificate_session_dir: PathBuf,
    pub keystone_receipt_session_dir: PathBuf,
    ...
    pub verify_cornerstone_certificate_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageFoundationReceiptBedrockCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageFoundationReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageFoundationReceiptArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub foundation_receipt_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredFoundationReceiptSession {
    ...
    #[serde(alias = "verify_capstone_certificate_command_summary")]
...
#[derive(Debug, Deserialize)]
struct StoredFoundationReceiptStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredFoundationReceiptReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct FoundationReceiptVerifyReportView {
...
pub fn load_live_package_foundation_receipt_contract_for_bedrock_certificate(
    foundation_receipt_session_dir: &Path,
) -> Result<LivePackageFoundationReceiptContractView> {
    let paths = foundation_receipt_artifact_paths(foundation_receipt_session_dir);
    let session: StoredFoundationReceiptSession = load_json(&paths.session_path)?;
    let status: StoredFoundationReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackageFoundationReceiptContractView {
        foundation_receipt_session_dir: foundation_receipt_session_dir.to_path_buf(),
        cornerstone_certificate_session_dir: PathBuf::from(
            session.cornerstone_certificate_session_dir,
        ),
        keystone_receipt_session_dir: PathBuf::from(session.keystone_receipt_session_dir),
        ...
        verify_cornerstone_certificate_command_summary: session
...
pub fn verify_live_package_foundation_receipt_for_bedrock_certificate(
    foundation_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageFoundationReceiptBedrockCertificateStep> {
    let contract = load_live_package_foundation_receipt_contract_for_bedrock_certificate(
        foundation_receipt_session_dir,
    )?;
    let trusted_decision_packet_session_dir =
        load_confirmed_decision_packet_session_dir_for_foundation_verify(
            foundation_receipt_session_dir,
    ...
    let raw_report = run_json_command(
        ...
        &foundation_receipt_verify_args(
            &contract.cornerstone_certificate_session_dir,
            &trusted_decision_packet_session_dir,
            foundation_receipt_session_dir,
        ),
    )?;
    let report: FoundationReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing foundation-receipt verify report")?;
    let reported_cornerstone_certificate_session_dir =
        PathBuf::from(&report.cornerstone_certificate_session_dir);
    if reported_cornerstone_certificate_session_dir != contract.cornerstone_certificate_session_dir
    {
        bail!(
            "foundation-receipt verify report cornerstone_certificate_session_dir {:?} does not match trusted {:?}",
            reported_cornerstone_certificate_session_dir.display(),
            contract.cornerstone_certificate_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "foundation-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "foundation-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageFoundationReceiptBedrockCertificateStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageFoundationReceiptContractView {
            foundation_receipt_session_dir: foundation_receipt_session_dir.to_path_buf(),
            cornerstone_certificate_session_dir: reported_cornerstone_certificate_session_dir,
            keystone_receipt_session_dir: contract.keystone_receipt_session_dir.clone(),
            ...
            verify_cornerstone_certificate_command_summary: required_string_field(
                report.verify_cornerstone_certificate_command_summary,
                "verify_cornerstone_certificate_command_summary",
            )?,
...
pub fn foundation_receipt_verify_args(
...
pub fn foundation_receipt_artifact_paths(
    session_dir: &Path,
) -> PackageFoundationReceiptArtifactPaths {
    PackageFoundationReceiptArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_foundation_verify(
    foundation_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
    contract: &LivePackageFoundationReceiptContractView,
) -> Result<PathBuf> {
    let paths = foundation_receipt_artifact_paths(foundation_receipt_session_dir);
    let archived_report: StoredFoundationReceiptReportView =
        load_json(&paths.foundation_receipt_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored foundation-receipt contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested cornerstone-certificate report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("foundation-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_foundation_receipt_files() {
        let dir = temp_dir("foundation_receipt_contract");
        fs::write(
            dir.join("tiny_live_activation_package_foundation_receipt.session.json"),
            serde_json::to_vec_pretty(&stored_foundation_receipt_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_foundation_receipt.status.json"),
            serde_json::to_vec_pretty(&stored_foundation_receipt_status_value(
        ...
        let contract =
            load_live_package_foundation_receipt_contract_for_bedrock_certificate(&dir).unwrap();
        assert_eq!(
        ...
        assert_eq!(
            contract.cornerstone_certificate_session_dir,
            PathBuf::from("/tmp/cornerstone-certificate-session")
        );
        assert_eq!(
    ...
    #[test]
    fn foundation_receipt_verify_args_are_exact_and_bounded() {
        let args = foundation_receipt_verify_args(
            Path::new("/tmp/cornerstone-certificate-session"),
            Path::new("/tmp/decision-packet-session"),
            Path::new("/tmp/foundation-receipt-session"),
        );
        assert_eq!(
            args,
            vec![
                "--cornerstone-certificate-session-dir",
                "/tmp/cornerstone-certificate-session",
                "--confirm-decision-packet-session-dir",
                "/tmp/decision-packet-session",
                "--session-dir",
                "/tmp/foundation-receipt-session",
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("foundation_receipt_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_foundation_receipt.cornerstone_certificate.report.json"),
        ...
        let contract =
            load_live_package_foundation_receipt_contract_for_bedrock_certificate(&dir).unwrap();
        let error = load_confirmed_decision_packet_session_dir_for_foundation_verify(
            &dir,
            &trusted_decision_packet_session_dir,
            &contract,
        )
    ...
    fn stored_foundation_receipt_session_value(decision_packet_session_dir: &str) -> Value {
        let mut object = Map::new();
        object.insert(
            "cornerstone_certificate_session_dir".to_string(),
            json!("/tmp/cornerstone-certificate-session"),
        );
        object.insert(
            "keystone_receipt_session_dir".to_string(),
            json!("/tmp/keystone-receipt-session"),
        );
        object.insert(
        ...
        object.insert(
            ...
            json!("verify cornerstone certificate"),
        );
        object.insert(
    ...
    fn stored_foundation_receipt_status_value(result: &str, include_nested_results: bool) -> Value {
        let mut object = Map::new();
