...
pub struct LivePackageDaisReceiptContractView {
    ...
    pub verify_dais_receipt_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageDaisReceiptRostrumCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageDaisReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageDaisReceiptArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub pedestal_certificate_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredDaisReceiptSession {
...
#[derive(Debug, Deserialize)]
struct StoredDaisReceiptStatus {
...
#[derive(Debug, Deserialize)]
struct StoredPedestalCertificateReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct DaisReceiptVerifyReportView {
...
pub fn load_live_package_dais_receipt_contract_for_rostrum_certificate(
    dais_receipt_session_dir: &Path,
) -> Result<LivePackageDaisReceiptContractView> {
    let paths = dais_receipt_artifact_paths(dais_receipt_session_dir);
    let session: StoredDaisReceiptSession = load_json(&paths.session_path)?;
    let status: StoredDaisReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackageDaisReceiptContractView {
        dais_receipt_session_dir: dais_receipt_session_dir.to_path_buf(),
        ...
        verify_dais_receipt_command_summary: session.verify_dais_receipt_command_summary,
...
pub fn verify_live_package_dais_receipt_for_rostrum_certificate(
    dais_receipt_session_dir: &Path,
    ...
    let report: DaisReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing dais-receipt verify report")?;
    ...
    if reported_pedestal_certificate_session_dir != contract.pedestal_certificate_session_dir {
        bail!(
            "dais-receipt verify report pedestal_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_plinth_receipt_session_dir != contract.plinth_receipt_session_dir {
        bail!(
            "dais-receipt verify report plinth_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    {
        bail!(
            "dais-receipt verify report substructure_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_bedrock_certificate_session_dir != contract.bedrock_certificate_session_dir {
        bail!(
            "dais-receipt verify report bedrock_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_basal_receipt_session_dir != contract.basal_receipt_session_dir {
        bail!(
            "dais-receipt verify report basal_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_foundation_receipt_session_dir != contract.foundation_receipt_session_dir {
        bail!(
            "dais-receipt verify report foundation_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "dais-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "dais-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageDaisReceiptRostrumCertificateStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageDaisReceiptContractView {
            dais_receipt_session_dir: dais_receipt_session_dir.to_path_buf(),
            ...
            verify_dais_receipt_command_summary: required_string_field(
                report.verify_dais_receipt_command_summary,
...
pub fn dais_receipt_verify_args(
    dais_receipt_session_dir: &Path,
...
pub fn dais_receipt_artifact_paths(session_dir: &Path) -> PackageDaisReceiptArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_pedestal_verify(
    ...
    let paths = dais_receipt_artifact_paths(dais_receipt_session_dir);
    let archived_report: StoredPedestalCertificateReportView =
        load_json(&paths.pedestal_certificate_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored dais-receipt contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested pedestal-certificate report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("dais-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_dais_receipt_files() {
        let dir = temp_dir("dais_receipt_contract");
        fs::write(
            dir.join("tiny_live_activation_package_dais_receipt.session.json"),
            serde_json::to_vec_pretty(&stored_dais_receipt_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_dais_receipt.status.json"),
            serde_json::to_vec_pretty(&stored_dais_receipt_status_value(
        ...
        let contract =
            load_live_package_dais_receipt_contract_for_rostrum_certificate(&dir).unwrap();
        assert_eq!(contract.dais_receipt_session_dir, dir);
        assert_eq!(
            contract.pedestal_certificate_session_dir,
    ...
    #[test]
    fn dais_receipt_verify_args_are_exact_and_bounded() {
        let args = dais_receipt_verify_args(
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("dais_receipt_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_dais_receipt.pedestal_certificate.report.json"),
    ...
    fn stored_dais_receipt_session_value(decision_packet_session_dir: &str) -> Value {
    ...
    fn stored_dais_receipt_status_value(result: &str, include_nested_results: bool) -> Value {
        let mut object = Map::new();
        object.insert(
            "pedestal_certificate_session_dir".to_string(),
            json!("/tmp/pedestal-certificate-session"),
