...
pub struct LivePackageRegistryEntryContractView {
    ...
    pub verify_notarization_receipt_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageRegistryEntryFilingCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageRegistryEntryContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageRegistryEntryArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub notarization_receipt_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredRegistryEntrySession {
...
#[derive(Debug, Deserialize)]
struct StoredRegistryEntryStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredNotarizationReceiptReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct RegistryEntryVerifyReportView {
...
pub fn load_live_package_registry_entry_contract_for_filing_certificate(
    registry_entry_session_dir: &Path,
) -> Result<LivePackageRegistryEntryContractView> {
    let paths = registry_entry_artifact_paths(registry_entry_session_dir);
    let session: StoredRegistryEntrySession = load_json(&paths.session_path)?;
    let status: StoredRegistryEntryStatus = load_json(&paths.status_path)?;
    Ok(LivePackageRegistryEntryContractView {
        registry_entry_session_dir: registry_entry_session_dir.to_path_buf(),
        ...
        verify_notarization_receipt_command_summary: session
...
pub fn verify_live_package_registry_entry_for_filing_certificate(
    registry_entry_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageRegistryEntryFilingCertificateStep> {
    let contract = load_live_package_registry_entry_contract_for_filing_certificate(
    ...
    let raw_report = run_json_command(
        ...
        &registry_entry_verify_args(
            &contract.notarization_receipt_session_dir,
            &trusted_decision_packet_session_dir,
            registry_entry_session_dir,
        ),
    )?;
    let report: RegistryEntryVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing registry-entry verify report")?;
    let reported_notarization_receipt_session_dir =
        PathBuf::from(&report.notarization_receipt_session_dir);
    if reported_notarization_receipt_session_dir != contract.notarization_receipt_session_dir {
        bail!(
            "registry-entry verify report notarization_receipt_session_dir {:?} does not match trusted {:?}",
            reported_notarization_receipt_session_dir.display(),
            contract.notarization_receipt_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "registry-entry verify report decision_packet_session_dir {:?} does not match trusted {:?}",
            reported_decision_packet_session_dir.display(),
            trusted_decision_packet_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageRegistryEntryFilingCertificateStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageRegistryEntryContractView {
            registry_entry_session_dir: registry_entry_session_dir.to_path_buf(),
            notarization_receipt_session_dir: contract.notarization_receipt_session_dir,
            ...
            verify_notarization_receipt_command_summary: required_string_field(
                report.verify_notarization_receipt_command_summary,
                "verify_notarization_receipt_command_summary",
            )?,
...
pub fn registry_entry_verify_args(
...
pub fn registry_entry_artifact_paths(session_dir: &Path) -> PackageRegistryEntryArtifactPaths {
    PackageRegistryEntryArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_verify(
    ...
    let paths = registry_entry_artifact_paths(registry_entry_session_dir);
    let archived_report: StoredNotarizationReceiptReportView =
        load_json(&paths.notarization_receipt_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored registry-entry contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested notarization-receipt report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("registry-entry verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_registry_entry_files() {
        let dir = temp_dir("registry_entry_contract");
        fs::write(
            dir.join("tiny_live_activation_package_registry_entry.session.json"),
            serde_json::to_vec_pretty(&json!({
                ...
                "verify_notarization_receipt_command_summary": "verify notarization receipt",
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_registry_entry.status.json"),
            serde_json::to_vec_pretty(&json!({
        ...
        let contract =
            load_live_package_registry_entry_contract_for_filing_certificate(&dir).unwrap();
        assert_eq!(
        ...
        assert_eq!(contract.registry_entry_sha256, "c0ffee");
    }

    #[test]
    fn registry_entry_verify_args_are_exact_and_bounded() {
        let args = registry_entry_verify_args(
        ...
        assert_eq!(
            ...
            vec![
                ...
                "--verify-live-package-registry-entry",
                "--json"
            ]
        );
    }
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("registry_entry_confirmed_decision_packet");
