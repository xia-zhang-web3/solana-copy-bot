...
pub struct LivePackageApseReceiptContractView {
    ...
    pub verify_chancel_certificate_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageApseReceiptSanctuaryCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageApseReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageApseReceiptArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub chancel_certificate_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredApseReceiptSession {
...
#[derive(Debug, Deserialize)]
struct StoredApseReceiptStatus {
    lectern_certificate_session_dir: Option<String>,
    chancel_certificate_session_dir: Option<String>,
...
#[derive(Debug, Deserialize)]
struct ApseReceiptVerifyReportView {
...
pub fn load_live_package_apse_receipt_contract_for_sanctuary_certificate(
    apse_receipt_session_dir: &Path,
) -> Result<LivePackageApseReceiptContractView> {
    let paths = apse_receipt_artifact_paths(apse_receipt_session_dir);
    let session: StoredApseReceiptSession = load_json(&paths.session_path)?;
    let status: StoredApseReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackageApseReceiptContractView {
        apse_receipt_session_dir: apse_receipt_session_dir.to_path_buf(),
        ...
        chancel_certificate_session_dir: PathBuf::from(
            status
                .chancel_certificate_session_dir
                .unwrap_or(session.chancel_certificate_session_dir),
        ),
        ...
        verify_chancel_certificate_command_summary: session
...
pub fn verify_live_package_apse_receipt_for_sanctuary_certificate(
    apse_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageApseReceiptSanctuaryCertificateStep> {
    let contract = load_live_package_apse_receipt_contract_for_sanctuary_certificate(
    ...
    let raw_report = run_json_command(
        ...
        &apse_receipt_verify_args(
            apse_receipt_session_dir,
            &contract.chancel_certificate_session_dir,
            &trusted_decision_packet_session_dir,
        ),
    )?;
    let report: ApseReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing apse-receipt verify report")?;
    let reported_lectern_certificate_session_dir = PathBuf::from(required_string_field(
        report.lectern_certificate_session_dir,
        "lectern_certificate_session_dir",
    )?);
    let reported_chancel_certificate_session_dir = PathBuf::from(required_string_field(
    ...
    if reported_lectern_certificate_session_dir != contract.lectern_certificate_session_dir {
        bail!(
            "apse-receipt verify report lectern_certificate_session_dir {:?} does not match trusted {:?}",
            reported_lectern_certificate_session_dir.display(),
            contract.lectern_certificate_session_dir.display()
        );
    }
    if reported_chancel_certificate_session_dir != contract.chancel_certificate_session_dir {
        bail!(
            "apse-receipt verify report chancel_certificate_session_dir {:?} does not match trusted {:?}",
            reported_chancel_certificate_session_dir.display(),
            contract.chancel_certificate_session_dir.display()
        );
    }
    if reported_pedestal_certificate_session_dir != contract.pedestal_certificate_session_dir {
        bail!(
            "apse-receipt verify report pedestal_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_plinth_receipt_session_dir != contract.plinth_receipt_session_dir {
        bail!(
            "apse-receipt verify report plinth_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    {
        bail!(
            "apse-receipt verify report substructure_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_bedrock_certificate_session_dir != contract.bedrock_certificate_session_dir {
        bail!(
            "apse-receipt verify report bedrock_certificate_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_basal_receipt_session_dir != contract.basal_receipt_session_dir {
        bail!(
            "apse-receipt verify report basal_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_foundation_receipt_session_dir != contract.foundation_receipt_session_dir {
        bail!(
            "apse-receipt verify report foundation_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "apse-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "apse-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageApseReceiptSanctuaryCertificateStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageApseReceiptContractView {
            apse_receipt_session_dir: apse_receipt_session_dir.to_path_buf(),
            lectern_certificate_session_dir: reported_lectern_certificate_session_dir,
            chancel_certificate_session_dir: reported_chancel_certificate_session_dir,
            ...
            verify_chancel_certificate_command_summary: required_string_field(
                report.verify_chancel_certificate_command_summary,
                "verify_chancel_certificate_command_summary",
            )?,
...
pub fn apse_receipt_verify_args(
    apse_receipt_session_dir: &Path,
...
pub fn apse_receipt_artifact_paths(session_dir: &Path) -> PackageApseReceiptArtifactPaths {
    PackageApseReceiptArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_apse_verify(
    ...
    let archived_report: StoredPulpitReceiptReportView =
        load_json(&paths.chancel_certificate_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored apse-receipt contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested chancel-certificate report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("apse-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_apse_receipt_files() {
        let dir = temp_dir("apse_receipt_contract");
        fs::write(
            dir.join("tiny_live_activation_package_apse_receipt.session.json"),
            serde_json::to_vec_pretty(&stored_apse_receipt_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_apse_receipt.status.json"),
            serde_json::to_vec_pretty(&stored_apse_receipt_status_value(
        ...
        let contract =
            load_live_package_apse_receipt_contract_for_sanctuary_certificate(&dir).unwrap();
        assert_eq!(contract.apse_receipt_session_dir, dir);
        assert_eq!(
        ...
        assert_eq!(
            contract.chancel_certificate_session_dir,
            PathBuf::from("/tmp/chancel-certificate-session")
        );
        assert_eq!(
        ...
        assert_eq!(
            contract.chancel_certificate_sha256,
            "herald-certificate-sha256"
        );
        assert_eq!(contract.apse_receipt_sha256, "herald-sha256");
    }

    #[test]
    fn apse_receipt_verify_args_are_exact_and_bounded() {
        let args = apse_receipt_verify_args(
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("apse_receipt_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_apse_receipt.chancel_certificate.report.json"),
    ...
    fn stored_apse_receipt_session_value(decision_packet_session_dir: &str) -> Value {
        ...
        object.insert(
            ...
            json!("/tmp/chancel-certificate-session"),
        );
        object.insert(
    ...
    fn stored_apse_receipt_status_value(result: &str, include_nested_results: bool) -> Value {
        let mut object = Map::new();
        object.insert(
            "lectern_certificate_session_dir".to_string(),
            json!("/tmp/lectern-certificate-session"),
