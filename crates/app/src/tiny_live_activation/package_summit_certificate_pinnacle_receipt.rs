...
pub struct LivePackageSummitCertificateContractView {
    ...
    pub verify_summit_certificate_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageSummitCertificatePinnacleReceiptStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageSummitCertificateContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageSummitCertificateArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub culmination_receipt_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredSummitCertificateSession {
...
#[derive(Debug, Deserialize)]
struct StoredSummitCertificateStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredCulminationReceiptReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct SummitCertificateVerifyReportView {
...
pub fn load_live_package_summit_certificate_contract_for_pinnacle_receipt(
    summit_certificate_session_dir: &Path,
) -> Result<LivePackageSummitCertificateContractView> {
    let paths = summit_certificate_artifact_paths(summit_certificate_session_dir);
    let session: StoredSummitCertificateSession = load_json(&paths.session_path)?;
    let status: StoredSummitCertificateStatus = load_json(&paths.status_path)?;
    Ok(LivePackageSummitCertificateContractView {
        summit_certificate_session_dir: summit_certificate_session_dir.to_path_buf(),
        ...
        verify_summit_certificate_command_summary: session
...
pub fn verify_live_package_summit_certificate_for_pinnacle_receipt(
    summit_certificate_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageSummitCertificatePinnacleReceiptStep> {
    let contract = load_live_package_summit_certificate_contract_for_pinnacle_receipt(
    ...
    let raw_report = run_json_command(
        ...
        &summit_certificate_verify_args(
            &contract.culmination_receipt_session_dir,
            &trusted_decision_packet_session_dir,
            summit_certificate_session_dir,
        ),
    )?;
    let report: SummitCertificateVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing summit-certificate verify report")?;
    ...
    if reported_culmination_receipt_session_dir != contract.culmination_receipt_session_dir {
        bail!(
            "summit-certificate verify report culmination_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "summit-certificate verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "summit-certificate verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageSummitCertificatePinnacleReceiptStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageSummitCertificateContractView {
            summit_certificate_session_dir: summit_certificate_session_dir.to_path_buf(),
            ...
            verify_summit_certificate_command_summary: required_string_field(
                report.verify_summit_certificate_command_summary,
                "verify_summit_certificate_command_summary",
            )?,
...
pub fn summit_certificate_verify_args(
...
pub fn summit_certificate_artifact_paths(
    session_dir: &Path,
) -> PackageSummitCertificateArtifactPaths {
    PackageSummitCertificateArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_verify(
    ...
    let paths = summit_certificate_artifact_paths(summit_certificate_session_dir);
    let archived_report: StoredCulminationReceiptReportView =
        load_json(&paths.culmination_receipt_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored summit-certificate contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested culmination-receipt report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("summit-certificate verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_summit_certificate_files() {
        let dir = temp_dir("summit_certificate_contract");
        fs::write(
            dir.join("tiny_live_activation_package_summit_certificate.session.json"),
            serde_json::to_vec_pretty(&stored_summit_certificate_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_summit_certificate.status.json"),
            serde_json::to_vec_pretty(&stored_summit_certificate_status_value(
        ...
        let contract =
            load_live_package_summit_certificate_contract_for_pinnacle_receipt(&dir).unwrap();
        assert_eq!(
    ...
    #[test]
    fn summit_certificate_verify_args_are_exact_and_bounded() {
        let args = summit_certificate_verify_args(
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("summit_certificate_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_summit_certificate.culmination_receipt.report.json"),
    ...
    fn stored_summit_certificate_session_value(decision_packet_session_dir: &str) -> Value {
    ...
    fn stored_summit_certificate_status_value(result: &str, include_nested_results: bool) -> Value {
        let mut object = Map::new();
