...
pub struct LivePackageArchiveReceiptContractView {
    ...
    pub verify_filing_certificate_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageArchiveReceiptClosureCertificateStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageArchiveReceiptContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageArchiveReceiptArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub filing_certificate_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredArchiveReceiptSession {
...
#[derive(Debug, Deserialize)]
struct StoredArchiveReceiptStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredFilingCertificateReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct ArchiveReceiptVerifyReportView {
...
pub fn load_live_package_archive_receipt_contract_for_closure_certificate(
    archive_receipt_session_dir: &Path,
) -> Result<LivePackageArchiveReceiptContractView> {
    let paths = archive_receipt_artifact_paths(archive_receipt_session_dir);
    let session: StoredArchiveReceiptSession = load_json(&paths.session_path)?;
    let status: StoredArchiveReceiptStatus = load_json(&paths.status_path)?;
    Ok(LivePackageArchiveReceiptContractView {
        archive_receipt_session_dir: archive_receipt_session_dir.to_path_buf(),
        filing_certificate_session_dir: PathBuf::from(session.filing_certificate_session_dir),
        ...
        verify_filing_certificate_command_summary: session
...
pub fn verify_live_package_archive_receipt_for_closure_certificate(
    archive_receipt_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageArchiveReceiptClosureCertificateStep> {
    let contract = load_live_package_archive_receipt_contract_for_closure_certificate(
    ...
    let raw_report = run_json_command(
        ...
        &archive_receipt_verify_args(
            &contract.filing_certificate_session_dir,
            &trusted_decision_packet_session_dir,
            archive_receipt_session_dir,
        ),
    )?;
    let report: ArchiveReceiptVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing archive-receipt verify report")?;
    let reported_filing_certificate_session_dir =
        PathBuf::from(&report.filing_certificate_session_dir);
    if reported_filing_certificate_session_dir != contract.filing_certificate_session_dir {
        bail!(
            "archive-receipt verify report filing_certificate_session_dir {:?} does not match trusted {:?}",
            reported_filing_certificate_session_dir.display(),
            contract.filing_certificate_session_dir.display()
        );
    }
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "archive-receipt verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "archive-receipt verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageArchiveReceiptClosureCertificateStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageArchiveReceiptContractView {
            archive_receipt_session_dir: archive_receipt_session_dir.to_path_buf(),
            filing_certificate_session_dir: contract.filing_certificate_session_dir,
            ...
            verify_filing_certificate_command_summary: required_string_field(
                report.verify_filing_certificate_command_summary,
                "verify_filing_certificate_command_summary",
            )?,
...
pub fn archive_receipt_verify_args(
...
pub fn archive_receipt_artifact_paths(session_dir: &Path) -> PackageArchiveReceiptArtifactPaths {
    PackageArchiveReceiptArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_verify(
    ...
    let paths = archive_receipt_artifact_paths(archive_receipt_session_dir);
    let archived_report: StoredFilingCertificateReportView =
        load_json(&paths.filing_certificate_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored archive-receipt contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested filing-certificate report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("archive-receipt verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_archive_receipt_files() {
        let dir = temp_dir("archive_receipt_contract");
        fs::write(
            dir.join("tiny_live_activation_package_archive_receipt.session.json"),
            serde_json::to_vec_pretty(&stored_archive_receipt_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_archive_receipt.status.json"),
            serde_json::to_vec_pretty(&stored_archive_receipt_status_value(
        ...
        let contract =
            load_live_package_archive_receipt_contract_for_closure_certificate(&dir).unwrap();
        assert_eq!(
        ...
        assert_eq!(contract.archive_receipt_sha256, "bead1234");
    }

    #[test]
    fn archive_receipt_verify_args_are_exact_and_bounded() {
        let args = archive_receipt_verify_args(
        ...
        assert_eq!(
            ...
            vec![
                ...
                "--verify-live-package-archive-receipt",
                "--json"
            ]
        );
    }
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("archive_receipt_confirmed_decision_packet");
    ...
    fn stored_archive_receipt_session_value(decision_packet_session_dir: &str) -> Value {
        let mut object = Map::new();
        object.insert(
            "filing_certificate_session_dir".to_string(),
            json!("/tmp/filing-certificate-session"),
        );
        object.insert(
        ...
        object.insert(
            ...
            json!("verify filing certificate"),
        );
        object.insert(
        ...
        object.insert("filing_certificate_sha256".to_string(), json!("c0ffee"));
        object.insert("filing_certificate_algorithm".to_string(), json!("sha256"));
        object.insert(
            "archive_receipt_summary".to_string(),
            json!("archive receipt"),
        );
        object.insert("archive_receipt_sha256".to_string(), json!("bead1234"));
    ...
    fn stored_archive_receipt_status_value(result: &str, include_nested_results: bool) -> Value {
        let mut object = Map::new();
