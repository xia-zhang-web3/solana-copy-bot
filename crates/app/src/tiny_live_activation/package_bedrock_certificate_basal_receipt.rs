...
const BEDROCK_CERTIFICATE_BIN: &str = "copybot_tiny_live_activation_package_bedrock_certificate";

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LivePackageBedrockCertificateContractView {
    ...
    pub verify_foundation_receipt_command_summary: String,
...
#[derive(Debug, Clone)]
pub struct VerifiedLivePackageBedrockCertificateBasalReceiptStep {
    pub report_json: serde_json::Value,
    pub verdict: String,
    pub reason: String,
    pub generated_at: DateTime<Utc>,
    pub contract: LivePackageBedrockCertificateContractView,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageBedrockCertificateArtifactPaths {
    pub session_path: PathBuf,
    pub status_path: PathBuf,
    pub bedrock_certificate_report_path: PathBuf,
}

#[derive(Debug, Deserialize)]
struct StoredBedrockCertificateSession {
    ...
    #[serde(alias = "verify_cornerstone_certificate_command_summary")]
...
#[derive(Debug, Deserialize)]
struct StoredBedrockCertificateStatus {
    result: String,
...
#[derive(Debug, Deserialize)]
struct StoredBedrockCertificateReportView {
    decision_packet_session_dir: String,
}

#[derive(Debug, Deserialize)]
struct BedrockCertificateVerifyReportView {
...
pub fn load_live_package_bedrock_certificate_contract_for_basal_receipt(
    bedrock_certificate_session_dir: &Path,
) -> Result<LivePackageBedrockCertificateContractView> {
    let paths = bedrock_certificate_artifact_paths(bedrock_certificate_session_dir);
    let session: StoredBedrockCertificateSession = load_json(&paths.session_path)?;
    let status: StoredBedrockCertificateStatus = load_json(&paths.status_path)?;
    Ok(LivePackageBedrockCertificateContractView {
        bedrock_certificate_session_dir: bedrock_certificate_session_dir.to_path_buf(),
        ...
        verify_foundation_receipt_command_summary: session
...
pub fn verify_live_package_bedrock_certificate_for_basal_receipt(
    bedrock_certificate_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
) -> Result<VerifiedLivePackageBedrockCertificateBasalReceiptStep> {
    let contract = load_live_package_bedrock_certificate_contract_for_basal_receipt(
        bedrock_certificate_session_dir,
    )?;
    let trusted_decision_packet_session_dir =
        load_confirmed_decision_packet_session_dir_for_bedrock_verify(
            bedrock_certificate_session_dir,
            confirmed_decision_packet_session_dir,
            &contract,
        )?;
    let raw_report = run_json_command(
        BEDROCK_CERTIFICATE_BIN,
    ...
    let report: BedrockCertificateVerifyReportView = serde_json::from_value(raw_report.clone())
        .context("failed parsing bedrock-certificate verify report")?;
    let reported_foundation_receipt_session_dir =
        PathBuf::from(&report.foundation_receipt_session_dir);
    if reported_foundation_receipt_session_dir != contract.foundation_receipt_session_dir {
        bail!(
            "bedrock-certificate verify report foundation_receipt_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_decision_packet_session_dir != trusted_decision_packet_session_dir {
        bail!(
            "bedrock-certificate verify report decision_packet_session_dir {:?} does not match trusted {:?}",
    ...
    if reported_registry_entry_session_dir != contract.registry_entry_session_dir {
        bail!(
            "bedrock-certificate verify report registry_entry_session_dir {:?} does not match trusted {:?}",
            reported_registry_entry_session_dir.display(),
            contract.registry_entry_session_dir.display()
        );
    }
    Ok(VerifiedLivePackageBedrockCertificateBasalReceiptStep {
        report_json: raw_report,
        verdict: report.verdict.clone(),
        reason: report.reason.clone(),
        generated_at: report.generated_at,
        contract: LivePackageBedrockCertificateContractView {
            bedrock_certificate_session_dir: bedrock_certificate_session_dir.to_path_buf(),
            foundation_receipt_session_dir: reported_foundation_receipt_session_dir,
            cornerstone_certificate_session_dir: contract
            ...
            verify_foundation_receipt_command_summary: required_string_field(
                report.verify_foundation_receipt_command_summary,
                "verify_foundation_receipt_command_summary",
            )?,
...
pub fn bedrock_certificate_verify_args(
...
pub fn bedrock_certificate_artifact_paths(
    session_dir: &Path,
) -> PackageBedrockCertificateArtifactPaths {
    PackageBedrockCertificateArtifactPaths {
...
fn load_confirmed_decision_packet_session_dir_for_bedrock_verify(
    bedrock_certificate_session_dir: &Path,
    confirmed_decision_packet_session_dir: &Path,
    contract: &LivePackageBedrockCertificateContractView,
) -> Result<PathBuf> {
    let paths = bedrock_certificate_artifact_paths(bedrock_certificate_session_dir);
    let archived_report: StoredBedrockCertificateReportView =
        load_json(&paths.bedrock_certificate_report_path)?;
    ...
    if confirmed_decision_packet_session_dir != contract.decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match stored bedrock-certificate contract {:?}",
    ...
    if confirmed_decision_packet_session_dir != archived_decision_packet_session_dir {
        bail!(
            "confirmed decision-packet session dir {:?} does not match archived nested foundation-receipt report {:?}",
            confirmed_decision_packet_session_dir.display(),
...
fn required_string_field(value: Option<String>, field: &str) -> Result<String> {
    value.ok_or_else(|| anyhow!("bedrock-certificate verify report is missing {field}"))
}
...
mod tests {
    ...
    #[test]
    fn load_contract_reads_stored_bedrock_certificate_files() {
        let dir = temp_dir("bedrock_certificate_contract");
        fs::write(
            dir.join("tiny_live_activation_package_bedrock_certificate.session.json"),
            serde_json::to_vec_pretty(&stored_bedrock_certificate_session_value(
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_bedrock_certificate.status.json"),
            serde_json::to_vec_pretty(&stored_bedrock_certificate_status_value(
        ...
        let contract =
            load_live_package_bedrock_certificate_contract_for_basal_receipt(&dir).unwrap();
        assert_eq!(
    ...
    #[test]
    fn bedrock_certificate_verify_args_are_exact_and_bounded() {
        let args = bedrock_certificate_verify_args(
            Path::new("/tmp/foundation-receipt-session"),
            Path::new("/tmp/decision-packet-session"),
            Path::new("/tmp/bedrock-certificate-session"),
        );
        assert_eq!(
            args,
            vec![
                ...
                "/tmp/decision-packet-session",
                "--session-dir",
                "/tmp/bedrock-certificate-session",
    ...
    fn confirmed_decision_packet_session_dir_must_match_stored_contract_and_archive() {
        let dir = temp_dir("bedrock_certificate_confirmed_decision_packet");
        ...
        .unwrap();
        fs::write(
            dir.join("tiny_live_activation_package_bedrock_certificate.foundation_receipt.report.json"),
        ...
        let contract =
            load_live_package_bedrock_certificate_contract_for_basal_receipt(&dir).unwrap();
        let error = load_confirmed_decision_packet_session_dir_for_bedrock_verify(
            &dir,
            &trusted_decision_packet_session_dir,
            &contract,
        )
    ...
    fn stored_bedrock_certificate_session_value(decision_packet_session_dir: &str) -> Value {
        ...
        object.insert(
            ...
            json!("verify foundation receipt"),
        );
        object.insert(
    ...
    fn stored_bedrock_certificate_status_value(
        result: &str,
