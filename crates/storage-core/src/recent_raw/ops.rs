...
use rusqlite::{params_from_iter, types::Value as SqlValue, Connection, ErrorCode};
...
impl SqliteDiscoveryStore {
    ...
    pub fn prune_recent_raw_journal_before_batch(
        ...
        self.with_immediate_transaction_retry("recent raw journal retention prune", |conn| {
            crate::observed_retention::validate_schema(conn)?;
            ensure_recent_raw_journal_tables_on_conn(conn)?;
            crate::observed_retention::delete_before_batch(
                conn,
                cutoff,
                batch_limit,
                Some(pruned_at),
