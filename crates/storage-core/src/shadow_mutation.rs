use crate::{
    ...
    shadow_lots::{
        reject_zero_raw_exact_qty, shadow_accounting_bucket_for_qty_exact, to_sql_conversion_error,
        validate_shadow_risk_context,
    },
...
impl SqliteDiscoveryStore {
    ...
    pub fn update_shadow_lot_exact(
        ...
        self.execute_with_retry(|conn| {
            ...
            conn.execute(
                "UPDATE shadow_lots
                 SET qty = ?1, qty_raw = ?2, qty_decimals = ?3, cost_sol = ?4, cost_lamports = ?5
                 WHERE id = ?6",
                params![
                    qty,
        ...
        self.execute_with_retry(|conn| {
            ...
            conn.execute(
                ...
                params![
                    ...
                    token,
                    close_context,
                    shadow_accounting_bucket_for_qty_exact(qty_exact),
                    qty,
                    qty_exact.as_ref().map(|value| value.raw().to_string()),
                    qty_exact.as_ref().map(|value| i64::from(value.decimals())),
                    entry_cost_sol,
