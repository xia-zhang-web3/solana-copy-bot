...
#[test]
fn sqlite_startup_large_wal_checkpoint_skips_missing_wal_before_journal_mode_stage() -> Result<()> {
        ...
        recorded.iter().any(|event| {
            event.stage == SQLITE_STARTUP_LARGE_WAL_CHECKPOINT_TRUNCATE_STAGE
                ...
                && event.detail.as_deref().is_some_and(|detail| {
                    detail.contains("reason=wal_missing") && detail.contains("before_wal_bytes=0")
                })
        }),
...
fn sqlite_startup_large_wal_checkpoint_busy_fails_before_journal_mode_stage() -> Result<()> {
    ...
    assert!(
        format!("{error:#}").contains("sqlite startup large WAL checkpoint truncate remained busy"),
