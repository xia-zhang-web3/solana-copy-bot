...
pub(super) fn seed_wallet_activity_interrupt_fixture(
...
pub(super) fn spawn_interrupt_loop(
...
pub(super) struct CheckpointRecurrenceSummary {
    pub(super) writes_before_reader: usize,
    pub(super) writes_during_reader: usize,
    pub(super) max_backlog_frames: i64,
}

#[derive(Debug, Clone, Copy)]
pub(super) enum CursorCheckpointRecurrenceReader {
...
impl CursorCheckpointRecurrenceReader {
    pub(super) fn db_name(self) -> &'static str {
        match self {
            ...
            Self::LegacySolLegSingleStatement => "observed-sol-leg-single-statement-recurrence.db",
            Self::ChunkedSolLeg => "observed-sol-leg-chunked-recurrence.db",
        }
    }
}

pub(super) fn run_checkpoint_recurrence_scenario(
    ...
    let writer = thread::spawn(move || -> Result<()> {
        ...
        writer_store
            ...
            .context("failed to force aggressive wal_autocheckpoint for recurrence test writer")?;
    ...
    while !reader.is_finished() {
        ...
        max_backlog_frames = max_backlog_frames.max(log_frames.saturating_sub(checkpointed_frames));
        thread::sleep(StdDuration::from_millis(10));
    }
    reader
        .join()
...
pub(super) fn run_cursor_checkpoint_recurrence_scenario(
    ...
    let reader = thread::spawn(move || -> Result<()> {
        ...
        match reader_mode {
            ...
            CursorCheckpointRecurrenceReader::ChunkedSolLeg => {
                reader_store.for_each_observed_sol_leg_swap_in_window_after_cursor_with_budget(
                    reader_window_start,
                    reader_window_end,
                    None,
                    4_096,
