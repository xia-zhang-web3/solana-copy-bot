...
fn observed_wallet_activity_page_does_not_count_future_until_day_from_fast_path() -> Result<()> {
    ...
    let by_wallet: HashMap<String, ObservedWalletActivityRow> = page
        ...
        .collect();
    assert_eq!(
        by_wallet["wallet-until-future"].active_day_count, 1,
...
fn observed_wallet_activity_page_for_exact_wallets_filters_and_resumes_in_order_stage1(
    ...
    let first_page = store.observed_wallet_activity_page_for_exact_wallets_in_window_with_budget(
        &exact_wallet_ids,
        since,
        until,
        None,
    ...
    let second_page = store.observed_wallet_activity_page_for_exact_wallets_in_window_with_budget(
        &exact_wallet_ids,
        since,
        until,
        Some("wallet-exact-a"),
