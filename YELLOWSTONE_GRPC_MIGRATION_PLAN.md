...
5. Cross-plan update (2026-02-19):
   ...
   4. latest runtime hardening synced: bounded retry policy (`max_submit_attempts`), typed submit error taxonomy (`Retryable`/`Terminal`), pre-trade checker contract + RPC pre-trade mode (`paper_rpc_pretrade_confirm`, blockhash + signer balance reserve gate with side-aware threshold: BUY=`notional+reserve`, SELL=`reserve` only, + optional ATA existence policy + optional priority fee cap), RPC confirmer mode, adapter submit mode (`adapter_submit_confirm`: HTTP submitter contract + route allowlist + route-level slippage caps + route-level compute budget policy + fail-closed init + response route/client-order correlation terminal rejects + confirm failure/timeout manual-reconcile flags + optional HMAC request-signing headers), attempt-based ordered route fallback (`default_route` -> allowed fallbacks) applied consistently in pre-trade and submit steps, idempotency insert-outcome disambiguation (`Inserted`/`Duplicate`), parse-reject telemetry by reason, and execution scheduling decoupled from ingestion loop via dedicated blocking execution task.
