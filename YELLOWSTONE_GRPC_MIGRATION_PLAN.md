...
5. Cross-plan update (2026-02-19):
   ...
   4. latest runtime hardening synced: bounded retry policy (`max_submit_attempts`), typed submit error taxonomy (`Retryable`/`Terminal`), pre-trade checker contract + RPC pre-trade mode (`paper_rpc_pretrade_confirm`, blockhash + signer balance reserve gate with side-aware threshold: BUY=`notional+reserve`, SELL=`reserve` only, + optional ATA existence policy + optional priority fee cap in micro-lamports/CU with all-routes validation against submit CU-price policy), RPC confirmer mode, adapter submit mode (`adapter_submit_confirm`: HTTP submitter contract + route allowlist + explicit route order policy `submit_route_order` + route-level slippage/tip/compute-budget policy with tip guardrail `0..=100_000_000 lamports` + fail-closed init + response route/client-order correlation terminal rejects + response contract-version pin + optional strict policy-echo validation (slippage/tip/CU + full fee-breakdown echo `network_fee_lamports`/`base_fee_lamports`/`priority_fee_lamports`) + confirm failure/timeout manual-reconcile flags + optional HMAC request-signing headers over raw request-body bytes + file-based adapter auth secrets resolved relative to config path), confirmed-path reconcile now keeps local exposure consistent even when latest market price is unavailable (fallback pricing + risk-event), confirm-path fee accounting now applies on-chain network fee (`getTransaction.meta.fee`) + applied tip (persisted per-order from submit response/policy) + optional ATA-create rent from adapter response into `fee_sol` and position/PnL updates, fee-breakdown persistence is range-safe (`ata_create_rent_lamports` + optional `network/base/priority` fee hints bounded by `i64::MAX`, checked sqlite cast, non-negative DB triggers), adapter response now fail-closes on inconsistent fee hints (`network_fee_lamports != base_fee_lamports + priority_fee_lamports`), and missing-network-fee telemetry now preserves reason context (`rpc_lookup_error` vs `meta_fee_unavailable`) plus sanitized typed lookup class (`timeout`/`connect`/`invalid_json`/`rpc_error_payload`/`other`, no endpoint/query secrets) for ops triage (with `network_fee_lamports_hint` fallback source tagging when `meta.fee` is unavailable and mismatch telemetry when RPC fee differs from submit hint), attempt-based ordered route fallback applied consistently in pre-trade and submit steps, per-route execution batch telemetry for submit/pretrade/confirm outcomes plus runtime-observed confirm latency sums/samples + confirm network-fee source counters (`rpc_meta` / `submit_hint` / `missing`) + per-route fee lamports sums (`network_fee`/`tip`/`ata_rent`/total + optional `base/priority` hints), with DB snapshot helpers (`tools/execution_fee_calibration_report.sh`, `tools/runtime_snapshot.sh`) for route-level calibration and backward-compatible SQL on legacy `orders` fee columns, and production-profile runtime guard requiring `submit_adapter_require_policy_echo=true` in `adapter_submit_confirm` mode for `prod`/`production` profile families, idempotency insert-outcome disambiguation (`Inserted`/`Duplicate`), parse-reject telemetry by reason, and execution scheduling decoupled from ingestion loop via dedicated blocking execution task;
   ...
   12. fee-decomposition ops evidence helper upgraded: `tools/execution_fee_calibration_report.sh` now emits per-route decomposition completeness/consistency, includes per-order fee accounting consistency check (`fills.fee` vs hinted `network+tip+ata` with lamport-level tolerance), and adapter-mode readiness verdict (`PASS/WARN/NO_DATA/SKIP`) now requires full consistency coverage (`fee_consistency_full_hint_rows == confirmed_orders_total`) + non-missing `ata` coverage to prevent false green on partial hints; `tools/ops_scripts_smoke_test.sh` covers this output path.
   ...
   14. go/no-go evidence helper added: `tools/execution_go_nogo_report.sh` composes calibration and runtime snapshot into a single `GO/NO_GO/HOLD` summary with gate reasons + ingestion context and optional artifact export; verdict logic is fail-closed for `UNKNOWN` gate states (`UNKNOWN` takes precedence over `SKIP/NO_DATA`), and smoke suite validates paper-mode `HOLD`, adapter-mode `NO_GO`, and `UNKNOWN+SKIP => NO_GO` precedence.
   15. adapter preflight helper added for live-path contract discipline: `tools/execution_adapter_preflight.sh` validates adapter mode config/secret invariants before startup and fail-closes on drift (URL policy, signer/contract token, strict policy echo in production-like envs, route/default invariants, secret-file conflicts and HMAC requirements); smoke suite covers pass/fail branches.
   16. Stage C.5 handoff tooling added: `tools/execution_devnet_rehearsal.sh` orchestrates adapter preflight + go/no-go evidence + targeted regression tests into a single devnet rehearsal verdict/artifact bundle (`GO/HOLD/NO_GO`), with operator procedure in `ops/execution_devnet_rehearsal_runbook.md` and smoke coverage in `tools/ops_scripts_smoke_test.sh`.
