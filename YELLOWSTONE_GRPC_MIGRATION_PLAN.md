...
5. Cross-plan update (2026-02-19):
   ...
   6. adapter endpoint transport safety gate added: runtime now fail-closes on invalid adapter URLs (must be explicit `http(s)`), and in production-profile families blocks non-loopback `http://` adapter endpoints (`https://` required unless loopback).
