...
impl Fixture {
    ...
    pub fn guard(&self, reserve: u64) -> SolanaInstruction {
        transfer(self.wallet.pubkey(), self.wallet.pubkey(), reserve)
    }
    ...
    pub fn run(
        ...
        let unsigned = serialize_unsigned_legacy_transaction(
        ...
        if let Some(index) = guard {
            let mut previous_instructions = instructions.to_vec();
            previous_instructions.remove(index);
            ...
            eprintln!("B23_SIGNATURE case={label} old_valid=true new_valid=true reused_old=SignatureFailure");
        }
        ...
        let mut features: Vec<_> = bank
            ...
            .collect();
        features.sort();
        eprintln!("B23 case={label} W={} F={actual_fee} recipient_before={} wallet_after={} recipient_after={} bytes={} message_sha256={} status={:?} units={} rent_min={} features={:?}",
        ...
        for line in &logs {
            eprintln!("B23_LOG case={label} {line}");
        }
        Ok(Outcome {
