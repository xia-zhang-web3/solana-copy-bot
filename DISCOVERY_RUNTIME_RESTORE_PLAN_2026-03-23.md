...
- код restore stack уже раскатан на live server в коммите `f19360f`
...
- live recent-raw snapshot path после throughput hardening больше не выглядит
  как немой зависший run и теперь честно выходит в bounded `deferred`, но пока
  все еще не завершает fresh large-journal snapshot
...
- `copybot-discovery-recent-raw-snapshot.timer` на этом сервере сейчас
  выключен
- причина теперь уже не в giant replay и не в stale artifact:
  snapshot path под реальными live writes стал bounded и честным, но на
  текущем large journal still exits only as transient `deferred`
- значит operational contract на проде сейчас асимметричен:
  - artifact cadence уже автоматизирован
  - recent raw snapshot cadence все еще требует следующего throughput шага,
    если цель именно completed fresh snapshot, а не только bounded defer
...
3. Отдельно довести `live-safe recent raw snapshot` до реального серверного
   состояния.
   - `copybot-discovery-recent-raw-snapshot.timer` должен вернуться в `enabled`
     без остановки `copybot-app`
   - bounded outcome contract уже есть и на реальном сервере подтвержден:
     live run теперь дает честный `deferred`, а не немой hang
   - следующий шаг здесь уже не correctness, а throughput:
     completed fresh snapshot на large journal должен стать practical, а не
     только bounded-deferred
   - operator outcome должен быть однозначным:
     - `written`
     - `self_healed`
     - `retryable_busy/deferred`
     - `hard_failure`
