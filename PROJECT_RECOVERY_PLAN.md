...
proved metadata-only priority-fee tampering remains unclassified. Source and
local checks are accepted for this one-position scope; artifact/package
preflight remains outstanding.

Next authorized action: accept the targeted checks, publish the reviewed diff,
build the matching `copybot-app` release artifact in GitHub Actions, verify and
install it only in the new stopped exit package, apply migrations offline, and
run the package's local preflight. The user alone starts the bounded live SELL.
